...
1. Az RDP-fájl beolvasása művelet meghívja a `https://{gateway-hostname}/api/host/{lab-machine-name}/port/{port-number}` a hitelesítési jogkivonat igényléséhez.
    1. a `{gateway-hostname}` a labor **Beállítások** oldalán megadott átjáró állomásneve a Azure Portalban. 
...
- A HTTPS-forgalom kezeléséhez telepíteni kell az SSL-tanúsítványt az átjáró-gépen. A tanúsítványnak meg kell egyeznie az átjáró-Farm terheléselosztó vagy a gép teljes TARTOMÁNYNEVÉnek teljes tartománynevével (FQDN), ha csak egy gép van. A Wild kártya SSL-tanúsítványai nem működnek.  
...
- Telepítse a távoli asztali átjáró jogkivonat-hitelesítését támogató [csatlakoztatható hitelesítési](https://code.msdn.microsoft.com/windowsdesktop/Remote-Desktop-Gateway-517d6273) modult. Ilyen modul például `RDGatewayFedAuth.msi`, amely [System Center Virtual Machine Manager (VMM) rendszerképeket](/system-center/vmm/install-console?view=sc-vmm-1807)tartalmaz. A System centerrel kapcsolatos további információkért lásd: a [System Center dokumentációja](https://docs.microsoft.com/system-center/) és [díjszabása](https://www.microsoft.com/cloud-platform/system-center-pricing).  
- Az átjárókiszolgáló képes a `https://{gateway-hostname}/api/host/{lab-machine-name}/port/{port-number}`re irányuló kérelmek kezelésére.

    Az átjáró-állomásnév az átjáró-Farm vagy a számítógép teljes tartománynevének teljes TARTOMÁNYNEVE, ha csak egy gép van. A `{lab-machine-name}` a csatlakoztatni kívánt labor gép neve, és a `{port-number}` a port, amelyen a kapcsolat létrejön.  Alapértelmezés szerint ez a port 3389.  Ha azonban a virtuális gép a DevTest Labs [megosztott IP-](devtest-lab-shared-ip.md) szolgáltatását használja, a port eltérő lesz.
- Az Internet Information Server (IIS) [alkalmazás-útválasztási kérelmi](/iis/extensions/planning-for-arr/using-the-application-request-routing-module) modulja átirányíthatja `https://{gateway-hostname}/api/host/{lab-machine-name}/port/{port-number}` kérelmeket az Azure-függvénynek, amely kezeli a kérést, hogy jogkivonatot kapjon a hitelesítéshez.


## <a name="requirements-for-azure-function"></a>Az Azure Function követelményei
Az Azure Function `https://{function-app-uri}/app/host/{lab-machine-name}/port/{port-number}` formátumával kezeli a kérelmeket, és a hitelesítési jogkivonatot az átjáró gépekre telepített azonos aláíró tanúsítvány alapján adja vissza. A `{function-app-uri}` a függvény eléréséhez használt URI. A rendszer automatikusan átadja a függvény kulcsát a kérelem fejlécében. A minta függvényekért lásd: [https://github.com/Azure/azure-devtestlab/blob/master/samples/DevTestLabs/GatewaySample/src/RDGatewayAPI/Functions/CreateToken.cs](https://github.com/Azure/azure-devtestlab/blob/master/samples/DevTestLabs/GatewaySample/src/RDGatewayAPI/Functions/CreateToken.cs). 


## <a name="requirements-for-network"></a>A hálózatra vonatkozó követelmények

- Az átjáró-számítógépeken telepített SSL-tanúsítványhoz társított teljes tartománynév DNS-jének az átjárót futtató géphez vagy az átjáró-farmhoz tartozó terheléselosztó felé kell irányítani az adatforgalmat.
...
A tesztkörnyezet beállításainak frissítése előtt tárolja a függvény sikeres végrehajtásához szükséges kulcsot a tesztkörnyezet kulcstartójában lévő hitelesítési jogkivonat visszaadásához. A függvény kulcsának értékét a **kezelés** lapon érheti el a Azure Portalban. A titkos kulcsnak a kulcstartóban való mentésével kapcsolatos további információkért lásd: [titkos kód hozzáadása Key Vaulthoz](../key-vault/quick-create-portal.md#add-a-secret-to-key-vault). Mentse a titok nevét későbbi használatra.
...
1. A **Távoli asztal** szakaszban adja meg a távoli asztali szolgáltatások átjárójának vagy farmjának teljes tartománynevét (FQDN) vagy IP-címét az **átjáró állomásneve** mezőhöz. Ennek az értéknek meg kell egyeznie az átjáró-gépeken használt SSL-tanúsítvány teljes tartománynevével.
...
2. SSL-tanúsítvány beszerzése. Az SSL-tanúsítvánnyal társított teljes tartománynévnek az Ön által vezérelt tartományhoz kell tartoznia. A tanúsítvány ujjlenyomatának, jelszavának és Base64-kódolásának mentése. A PowerShell használatával történő ujjlenyomat beszerzéséhez használja a következő parancsokat.
    ...
    ```
3. Fájlok letöltése [https://github.com/Azure/azure-devtestlab/tree/master/samples/DevTestLabs/GatewaySample/arm/gateway ról ](https://github.com/Azure/azure-devtestlab/tree/master/samples/DevTestLabs/GatewaySample/arm/gateway).

    A sablonhoz hozzá kell férni néhány más Resource Manager-sablonhoz és kapcsolódó erőforráshoz ugyanahhoz az alap URI-hoz. Másolja az összes fájlt a [https://github.com/Azure/azure-devtestlab/blob/master/samples/DevTestLabs/GatewaySample/arm/gateway](https://github.com/Azure/azure-devtestlab/blob/master/samples/DevTestLabs/GatewaySample/arm/gateway) és a RDGatewayFedAuth. msi fájlból egy Storage-fiókban található blob-tárolóba.  
4. Telepítse a **azuredeploy. JSON** fájlt a [https://github.com/Azure/azure-devtestlab/tree/master/samples/DevTestLabs/GatewaySample/arm/gateway ból ](https://github.com/Azure/azure-devtestlab/tree/master/samples/DevTestLabs/GatewaySample/arm/gateway). A sablon a következő paramétereket veszi figyelembe:
    ...
    - sslCertificate – az átjárót használó számítógép SSL-tanúsítványának Base64-kódolása.
    - sslCertificatePassword – az átjárót használó számítógép SSL-tanúsítványának jelszava.
    - sslCertificateThumbprint – a Tanúsítvány ujjlenyomata az SSL-tanúsítvány helyi tanúsítványtárolójában való azonosításhoz.
    ...
    - Az {Storage-Account-Endpoint} a `az storage account show --name {storage-acct-name} --query primaryEndpoints.blob`futtatásával szerezhető be.  A (z) {Storage-acct-Name} annak a Storage-fióknak a neve, amely a feltöltött fájlokat tárolja.  
    - A (z) {Container-name} a (z) {Storage-acct-Name} tároló neve, amely a feltöltött fájlokat tárolja.  
    - Az {SAS-token} a `az storage container generate-sas --name {container-name} --account-name {storage-acct-name} --https-only –permissions drlw –expiry {utc-expiration-date}`futtatásával szerezhető be. 
...
5. Konfigurálja a DNS-t úgy, hogy az SSL-tanúsítvány teljes tartományneve az előző lépésből származó gatewayIP IP-címére irányítsa.
