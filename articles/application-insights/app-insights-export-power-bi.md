...
[串流分析](http://azure.microsoft.com/services/stream-analytics/) 是一種 Azure 服務，從 Application Insights 匯出的運作方式，做為配接器，連續處理資料。
...
> [AZURE.TIP] 使用範例命令下載一些資料。 將其保留下來做為偵錯查詢的測試範例。
...
提供您 **工作或學校帳戶** 來授權存取 Power BI 資源資料流分析。 接著自創一個輸出的名稱，以及目標 Power BI 資料集和資料表的名稱。
