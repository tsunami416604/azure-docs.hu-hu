...
您可以控制誰具有讀取和更新您在 Visual Studio 中的資料存取 [Application Insights][start], ，使用 [Microsoft Azure 中的角色型存取控制](../role-based-access-control-configure.md)。

> [AZURE.IMPORTANT] 指派存取權的使用者 **資源群組或訂用帳戶** 應用程式資源所屬-不在資源本身。 指派 **Application Insights 元件參與者** 角色。 這可確保 Web 測試和警示以及您的應用程式資源的統一存取控制。 [了解詳細](#access)。
...
使用者必須擁有 [Microsoft 帳戶][account]。 您可以提供存取權給個人，也可以提供給在 Azure Active Directory 中定義的使用者群組。
