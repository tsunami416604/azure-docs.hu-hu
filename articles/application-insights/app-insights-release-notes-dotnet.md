...
* 設定篩選器來顯示已安裝的封裝。 
* 選取 **Microsoft.ApplicationInsights.Web** 選擇 **升級**。 (這也會升級所有相依的封裝。)
...
- ```RequestTelemetry.Name``` 未初始化長。 ```RequestTelemetry.Context.Operation.Name``` 將改用。
