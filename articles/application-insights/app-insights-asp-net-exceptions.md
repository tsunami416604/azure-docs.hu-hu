...
若要監視 ASP.NET 應用程式，您必須 [加入 Application Insights SDK ][greenbrown] 應用程式或 [[redfield] 您的 IIS 伺服器上安裝狀態監視器][redfield], ，或者，如果您的應用程式是 Azure Web 應用程式，加入 [Application Insights 延伸模組 ][azure]。
...
* 沒有相依性失敗? 非常好。 但是若要確認您取得相依性資料，開啟 [效能] 刀鋒視窗，並查看 [相依性持續時間 chart.*
...
* [Trackexception ()](#exceptions) 會傳送堆疊追蹤。 [例外狀況的相關詳細](#exceptions)。
...
如果 [CustomErrors](https://msdn.microsoft.com/library/h0hfz6fc.aspx) 組態是 `關閉`, ，則例外狀況將可供 [HTTP 模組](https://msdn.microsoft.com/library/ms178468.aspx) 收集。 不過，如果是 `RemoteOnly` (預設值) 或 `上`, ，則會清除且無法使用 Application Insights 自動收集的例外狀況。 您可以藉由覆寫修正這個問題 [System.Web.Mvc.HandleErrorAttribute 類別](http://msdn.microsoft.com/library/system.web.mvc.handleerrorattribute.aspx), ，並針對不同的 MVC 版本如下所示的套用覆寫的類別 ([github 來源](https://github.com/AppInsightsSamples/Mvc2UnhandledExceptions/blob/master/MVC2App/Controllers/AiHandleErrorAttribute.cs)):
