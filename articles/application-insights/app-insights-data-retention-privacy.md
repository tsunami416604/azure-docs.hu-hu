...
**每秒**：每個檢測金鑰 (也就是每個應用程式) 每秒最多 500 個資料點。 針對免費 [定價層 ][pricing], ，限制為 100 dp/s。
...
* [API](app-insights-api-custom-events-metrics.md) `追蹤...` 呼叫您已撰寫，例如 `TrackEvent` 或 `trackPageView`。
* [可用性 web 測試](app-insights-monitor-web-app-availability.md) 已設定。

遙測資料包括：

* 每個資料列中 [診斷搜尋](app-insights-diagnostic-search.md)
...
* Use [Sampling](app-insights-sampling.md). 這項技術可減少資料率而不會曲解您的計量，且不會中斷在 [搜尋] 中於相關項目之間瀏覽的能力。 自 ASP.NET SDK 2.0.0-beta3 起，系統預設會啟用自適性取樣。
...
---|---
 [將 Application Insights SDK 加入至.NET web 專案 ][greenbrown]| ServerContext<br/>推斷<br/>效能計數器<br/>要求<br/>**例外狀況**<br/>工作階段<br/>使用者
 [IIS ][redfield]<br/>[加入 AI 擴充至 Azure VM 或 Web 應用程式 ][azure]| 相依性<br/>ServerContext<br/>推斷<br/>效能計數器
 [將 Application Insights SDK 加入至 Java web 應用程式 ][java]| ServerContext<br/>推斷<br/>要求<br/>工作階段<br/>使用者
 [將 JavaScript SDK 加入至 web 網頁 ][client]| ClientContext <br/>推斷<br/>頁面<br/>用戶端效能
 [將 SDK 加入至 Windows 市集應用程式 ][windows]| DeviceContext<br/>使用者<br/>損毀資料
