...
* `<MaxTelemetryItemsPerSecond>5</MaxTelemetryItemsPerSecond>`
    ...
    調整演算法的目標是針對目標速率 **單一伺服器主機上**。 如果 Web 應用程式在許多主機上執行，您會想要減少此值，以保持在您的 Application Insights 入口網站的流量目標速率內。
...
當您 [設定網頁的 Application Insights](app-insights-javascript.md), ，修改您從 Application Insights 入口網站取得程式碼片段。 (在 ASP.NET 應用程式中，程式碼片段通常會出現在 _Layout.cshtml。)在檢測金鑰之前插入類似 `samplingPercentage: 10,` 的一行：
...
> [AZURE.NOTE] 取樣百分比，選擇已接近 100/N 其中 N 是整數百分比。  目前取樣並不支援其他值。
...
* 遵循 [配額](app-insights-pricing.md) 定價層的資料點。 
* 若要從收集的遙測降低網路流量。 
