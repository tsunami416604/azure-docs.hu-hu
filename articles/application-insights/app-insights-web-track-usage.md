...
[了解更多](app-insights-javascript.md)
...
![在 ](./media/app-insights-web-track-usage/02-sessions.png)
...
* `sessionRenewalMs` : 時間 (毫秒)，因為使用者的無活動而讓工作階段過期。 預設值：30 分鐘。
...
![從 ](./media/app-insights-web-track-usage/031-dual.png)
...
不過，對於 Application Insights [web 測試的 ][availability], 、 使用者識別碼會自動設定根據 POP 位置和工作階段識別碼會根據設定測試回合 id。 在預設報告中，預設會篩選掉綜合流量，排除這些使用者和工作階段。 不過，若包含綜合流量，可能會造成整體使用者和工作階段計數少量增加。
...
![從 ](./media/app-insights-web-track-usage/05-games.png)
...
![在 ](./media/app-insights-web-track-usage/04-events.png)

按一下資料表標題來查看事件數的總計。 您可以依各種屬性 (例如事件名稱) 來分段圖表：

![選取的圖表，僅顯示一個度量。 將分組切換成開啟。 選擇屬性。 並非所有屬性提供都。](。 / media/app-insights-web-track-usage/06-eventsSegment.png)
...
![在 ](./media/app-insights-web-track-usage/07-clickEvent.png)
...
![按一下 ](./media/app-insights-web-track-usage/09-relatedTelemetry.png)
...
![在 ](./media/app-insights-web-track-usage/12-searchEvents.png)
