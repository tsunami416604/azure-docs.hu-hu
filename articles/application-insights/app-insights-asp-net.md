...
## <a name="ide"></a> 將 Application Insights 加入至 Visual Studio 中的專案
...
* 建立 Application Insights 資源中的 [Azure 入口網站 ][portal]。 這是您會看到您的資料的位置。 它會擷取可識別資源的「檢測金鑰」**。
* 將 Application Insights Web SDK NuGet 封裝加入您的專案。 若要在 Visual Studio 中看到它，請以滑鼠右鍵按一下專案，然後選擇 [管理 NuGet 封裝]。
* 將在檢測金鑰放 `ApplicationInsights.config`。
...
* *任何使用者或頁面的資料?* - [新增使用者和頁面資料](../article/application-insights/app-insights-asp-net-client.md)
...
- [相依性](../article/application-insights/app-insights-asp-net-dependencies.md#selector1)
