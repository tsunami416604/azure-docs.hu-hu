...
"< TelemetryType > 」 的第一個區段是任何遙測類型名稱的預留位置:
檢視、 要求，並以此類推。
...
* 
    ...
    *預設:* R2: 上 remotedepencyType，如果 dependencyType = 這是必要欄位<br/>        上 clientperformanceType，這是必要欄位
...
* 
    ...
    *預設:* 如果 null 時，使用者代理程式會檢查已知的綜合來源 (webcrawlers 等) 而根據來源可能會設定。
...
* 
    ...
    *預設：*若為 null，則會檢查使用者代理程式是否有已知的綜合代理程式清單。 如果找到相符項目，值會設為 true。<br/>如果 null 時，使用者代理程式設定為 false
...
* 
     遠端相依性的類型。 支援的類型包括 SQL、 AZURE 資源 (儲存體、 佇列等) 和 HTTP。
