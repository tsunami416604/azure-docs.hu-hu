...
中的度量 [Application Insights][start] 測量的值和從您的應用程式傳送的遙測中的事件計數。 它們幫助您偵測效能問題，並觀察使用應用程式方式的趨勢。 標準度量的範圍很廣泛，而您也可以建立自己的自訂度量和事件。
 ...
 * **總和** 加總透過取樣間隔中或圖表的期間收到的所有資料點的值。
 * **平均** 總和除以間隔內接收到的資料點數目。
 * **唯一** 計數用於使用者和帳戶的計數。 在取樣間隔或圖表期間，圖形顯示在該時間看到的不同使用者的計數。
...
[建立您自己的屬性](app-insights-api-custom-events-metrics.md#properties)。 設定 [遙測初始設定式](app-insights-api-custom-events-metrics.md#telemetry-initializers) 讓它出現在所有遙測-包括標準遙測模組傳送的不同 SDK。
...
您看到的內容即為匯出的內容。 如果您想要變更所匯出資料的範圍，請變更時間範圍或篩選器。 對於資料表，如果 **載入更多** 命令會顯示您可以按它再按一下 [匯出]，將匯出的詳細資料。

*匯出僅適用於 Internet Explorer 和 Chrome 目前。 我們正努力新增其他瀏覽器的支援。*
