...
Visual Studio Application Insights 會顯示在 Microsoft Azure 中的應用程式的資料 *資源*。 建立新的資源屬於因此 [來監視新的應用程式設定 Application Insights][start]。 在許多情況下，這是可由 IDE 自動完成的工作，而且也建議盡量使用這種方式。 但在某些情況下，您必須以手動方式建立資源。
...
* **資源群組** 可讓您輕鬆管理屬性例如存取控制。 如果您已經建立其他 Azure 資源，可以選擇將這個新的資源放到同一個群組。
* **訂閱** 是您在 Azure 中的付款帳戶。
* **位置** 是我們保留您資料的地方。 目前無法變更位置。
* **新增至開始面板** 將 Azure 首頁上的快速存取磚以取得您的資源。 建議使用。
