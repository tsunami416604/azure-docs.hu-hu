...
包括桌面應用程式、 背景服務和背景工作角色的所有 Windows 應用程式可以都使用 Application Insights SDK 傳送至 Application Insights 的遙測。 您也可以將 Application Insights SDK 新增至類別庫專案。
...
1.  在 [Azure 入口網站 ][portal], ，建立新的 Application Insights 資源。 針對應用程式類型，選擇 Windows 市集應用程式。
    ...
    ![按一下 ](./media/app-insights-windows-desktop/01-new.png)

    (您選擇的應用程式類型設定的 [概觀] 分頁中可用的屬性內容 [計量瀏覽器 ][metrics]。)

2.  取得檢測金鑰的副本。

    ![按一下 ](./media/app-insights-windows-desktop/02-props.png)

## <a name="sdk"></a>在您的應用程式中安裝 SDK

1. 在 Visual Studio 中，編輯桌面應用程式專案的 NuGet 封裝。

    ![以滑鼠右鍵按一下專案，然後選取 ](./media/app-insights-windows-desktop/03-nuget.png)

2. 安裝應用程式了解 Windows Server 封裝: Microsoft.ApplicationInsights.WindowsServer
...
* `TrackPageView(pageName)` 上切換表單、 頁面或索引標籤
* `Trackevent (eventname)` 用於其他使用者動作
* `TrackMetric (name，值)` 在背景工作以傳送未附加到特定事件之度量的一般報告。
* `Tracktrace (logevent)` 的 [[診斷] 的診斷記錄][diagnostic]
* `TrackException(exception)` 在 catch 子句
* `Flush()` 以確定所有的遙測傳送之前關閉此應用程式。 只有當您只使用核心 API (Microsoft.ApplicationInsights) 時才可以使用此選項。 Web Sdk 會自動實作此行為。 (如果您的應用程式會在內容中執行其中網際網路並非永遠可用，請參閱 [的持續性通道](#persistence-channel).)


#### 內容初始設定式

若要查看使用者和工作階段的計數可以設定值在每個 `TelemetryClient` 執行個體。 或者，您可以使用內容初始設定式來對所有用戶端執行這個加入動作：
...
預設的記憶體中通道將會在應用程式關閉時遺失任何尚未傳送的遙測。 雖然您可以使用 `Flush()` 嘗試傳送緩衝區中剩餘的任何資料，它將還是會遺失資料如果沒有網際網路連線，或如果應用程式關閉之前就完成了傳輸。
...
2. 在適當的初始化位置，將此程式碼納入您的應用程式中：
    ...
    ```
3. 使用 `telemetryClient.Flush()` 之前您應用程式隨即關閉，以確定資料傳送至入口網站或儲存到檔案中。
...
當應用程式關閉時，您會看到中 `%LocalAppData%\Microsoft\ApplicationInsights\`, ，其中包含壓縮的事件。
