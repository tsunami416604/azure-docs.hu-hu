...
- 核心管理於 github 中。 核心 SDK 的未來的版本資訊可以找到 [github](http://github.com/Microsoft/ApplicationInsights-dotnet/releases)

## 1.1 版

### 核心 SDK

- SDK 現在引進了新遙測類型 `DependencyTelemetry` 其中包含從應用程式的相依性呼叫的相關資訊
- 新方法 `TelemetryClient.TrackDependency` 讓使用者能夠從應用程式傳送呼叫相依性的相關資訊

## 1.0.0 版

### Windows 應用程式 SDK

- Windows 應用程式的新初始化。 新 `WindowsAppInitializer` 類別 `InitializeAsync()` 方法允許啟動 SDK 集合的初始化。 這項變更透過先前的 ApplicationInsights.config 技術，允許更精確的控制和重要的應用程式初始化效能改進。
...
- ApplicationInsights.config 只會讀取 `< InstrumentationKey >`, ，WindowsAppInitializer 設定中會忽略所有其他設定。
- SDK 將會自動收集市集市場。
- 許多錯誤修正、穩定性改良和效能增強功能。
...
- 新 API 加入， `TelemetryClient.Flush()`。 這個排清方法會觸發立即封鎖記錄到該用戶端的所有遙測上傳。 這可讓您在程序關閉之前手動觸發上傳。
