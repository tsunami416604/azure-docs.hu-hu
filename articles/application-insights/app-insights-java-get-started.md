...
* 如果您有已經是即時的 web 應用程式時，您可以依照替代的程序 [將 SDK 加入 web 伺服器在執行階段](app-insights-java-live.md)。 這個方法可避免重建程式碼，但您沒有撰寫程式碼來追蹤使用者 activity.* 選項
...
2. 建立新 Application Insights 資源。
    ...
    ![按一下 + 並選擇 ](./media/app-insights-java-get-started/01-create.png)
3. 將應用程式類型設定為 Java Web 應用程式。

    ![填寫名稱，選擇 ](./media/app-insights-java-get-started/02-create.png)
4. 尋找新資源的檢測金鑰。 您很快需要將此資訊貼上到程式碼專案中。

    ![在新資源概觀中，按一下 ](./media/app-insights-java-get-started/03-key.png)
...
* *建置或總和檢查碼驗證錯誤？*嘗試使用特定版本，例如: `< 版本 > 1.0.n < / 版本 >`。 您可以找到最新版本 [SDK 版本資訊](app-insights-release-notes-java.md) 或是在我們 [Maven 成品](http://search.maven.org/#search%7Cga%7C1%7Capplicationinsights)。
...
* * 建置或總和檢查碼驗證錯誤? 嘗試使用特定版本，例如: * `版本: '1.0.n'`。 * 您可以找到最新版本 [SDK 版本資訊](app-insights-release-notes-java.md)。 *
* *更新為新版 SDK*
 * 請重新整理專案的相依項目。
...
* * 什麼是之間的關聯性 `-核心` 和 `-web` zip 中的元件? *

 * `applicationinsights 核心` 提供裸機 API。 您一律會需要它。
...
Application Insights 假設 MVC 應用程式的 HTTP 要求的格式為: `動詞命令控制站/動作`


例如， `取得首頁/產品/f9anuh81`, ，`取得首頁/產品/2dffwrf5` 和 `取得首頁/產品/sdf96vws` 會分組到 `取得首頁/產品`。
...
![向下捲動，並按一下 ](./media/app-insights-java-get-started/21-exceptions.png)

若要收集其他例外狀況的資料，您有兩個選項：

* [[Apiexceptions] 程式碼中插入 trackexception () 呼叫][apiexceptions]。
...
* [將遙測加入至您的 web 網頁 ][usage] 即可監視頁面檢視和使用者度量。
...
![向下捲動，並依序按一下 ](./media/app-insights-java-get-started/31-config-web-test.png)
