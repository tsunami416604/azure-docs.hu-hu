...
診斷搜尋 」 是中的分頁 [Application Insights 的 ][start] 您用來尋找及探索個別遙測項目，例如頁面檢視、 例外狀況，或 web 要求。 而您可以檢視所編寫的記錄追蹤和事件。
...
您可以搜尋任何屬性值中的字詞。 這會特別有用，如果您已撰寫 [自訂事件 ][track] 與屬性值。
...
---|---|---
 ...
 IsLocal| 本機<br/>是<br/>\*local| isl\*<br/>islocal<br/>i\*l\*
 New Delay| w d| 新<br/>延遲<br/>{1>n<1} * AND {1>d<1} *
...
---|---
 ...
 資料庫?| 符合 database01、 databaseAB、...<br/>?不允許出現在搜尋詞彙的開頭。
 database*| 符合 database、 database01、 databaseNNNN<br/>* 不允許出現搜尋詞彙的開頭
 apple AND banana| 尋找同時含有這些詞彙的事件。請使用大寫 "AND"，而不是 "and"。
 apple OR banana<br/>apple banana| 尋找含有任一詞彙的事件。"OR"，而不使用 「 或 」。</br/>簡短形式。
 apple 不 banana<br/>apple-banana| 尋找含有一個詞彙但不是其他事件。<br/>簡短形式。
...
![按一下 ](./media/app-insights-diagnostic-search/08-favorite-save.png)
...
[了解如何傳送記錄和自訂的遙測到 Application Insights 的 ][trace]。
...
* [傳送記錄和自訂的遙測到 Application Insights 的 ][trace]
