...
重新接收。 它會自動加入至您的專案時您 [安裝 SDK ][start]。 它也會加入至 web 應用程式 
由 [[redfield] IIS 伺服器上的狀態監視器][redfield], ，或當您選取應用程式見解 
[延伸模組將 Azure 網站或 VM ][azure]。

沒有同等的檔案可以控制 [web 網頁 ][client]。
...
[會收集系統效能計數器](app-insights-web-monitor-performance.md#system-performance-counters) 例如 CPU、 記憶體和網路負載從 IIS 安裝。 您可以指定要收集哪些計數器，包括您自己所設定的效能計數器。
...
`DeveloperModeWithDebuggerAttachedTelemetryModule` 會強制 Application Insights `TelemetryChannel` 立即傳送資料，一次，當偵錯工具附加至應用程式處理序的一個遙測項目。 這會減少您的應用程式追蹤遙測時，與當遙測出現在 Application Insights 入口網站時之間的時間量。 但是會造成 CPU 和網路頻寬明顯的負擔。
...
`ExceptionTrackingTelemetryModule` 會追蹤 web 應用程式中的未處理例外狀況。 請參閱 [失敗和例外狀況 ][exceptions]。
...
* `Microsoft.ApplicationInsights.PersistenceChannel` 是主控台應用程式的替代選項。 它可以在您的 app 關閉時將任何未清除的資料儲存到永續性儲存體，並在 app 重新啟動時再次傳送資料。
...
* `AccountIdTelemetryInitializer` AccountId 屬性設定。
* `AuthenticatedUserIdTelemetryInitializer` AuthenticatedUserId 屬性設定所設定的 JavaScript SDK。
* `AzureRoleEnvironmentTelemetryInitializer` 更新 `RoleName` 和 `RoleInstance` 屬性 `裝置` 資訊從 Azure 執行階段環境中擷取的所有遙測項目。
* `BuildInfoConfigComponentVersionTelemetryInitializer` 更新 `版本` 屬性 `元件` 從擷取的值的所有遙測項目 `BuildInfo.config` MS 組建所產生的檔案。
* `ClientIpHeaderTelemetryInitializer` 更新 `Ip` 屬性 `位置` 所有遙測項目的內容根據 `X 轉送的` 要求的 HTTP 標頭。
* `DeviceTelemetryInitializer` 更新的下列屬性 `裝置` 所有遙測項目。
 - `類型` 設為"PC"
 - `識別碼` 設為電腦的網域名稱的 web 應用程式執行所在。
 - `OemName` 設定從擷取的值為 `Win32_ComputerSystem.Manufacturer` 欄位使用 WMI。
 - `模型` 設定從擷取的值為 `Win32_ComputerSystem.Model` 欄位使用 WMI。
 - `NetworkType` 設定從擷取的值為 `NetworkInterface`。
 - `語言` 設定的名稱為 `CurrentCulture`。
* `DomainNameRoleInstanceTelemetryInitializer` 更新 `RoleInstance` 屬性 `裝置` 所有內容
遙測項目使用的網域名稱的 web 應用程式執行所在的電腦。
* `OperationNameTelemetryInitializer` 更新 `名稱` 屬性 `RequestTelemetry` 和 `名稱` 屬性 `作業` 所有遙測項目的內容根據 HTTP 方法以及 ASP.NET MVC 控制器和動作來處理要求叫用的名稱。
* `OperationIdTelemetryInitializer` 更新 `Operation.Id` 內容屬性的所有遙測項目追蹤處理自動產生的要求時 `RequestTelemetry.Id`。
* `SessionTelemetryInitializer` 更新 `識別碼` 屬性 `工作階段` 從擷取的值的所有遙測項目 `ai_session` 使用者的瀏覽器中執行的 application Insights JavaScript 檢測程式碼所產生的 cookie。
* `SyntheticTelemetryInitializer` 更新 `使用者`, ，`工作階段` 和 `作業` 處理來自綜合來源的要求，例如可用性測試，或搜尋引擎 bot 時追蹤這些內容屬性的所有遙測項目。 根據預設， [計量瀏覽器](app-insights-metrics-explorer.md) 不會顯示綜合遙測。
* `UserAgentTelemetryInitializer` 更新 `UserAgent` 屬性 `使用者` 所有遙測項目的內容根據 `使用者代理程式` 要求的 HTTP 標頭。
* `UserTelemetryInitializer` 更新 `識別碼` 和 `AcquisitionDate` 屬性 `使用者` 從擷取的值與所有遙測項目 `ai_user` 使用者的瀏覽器中執行的 Application Insights JavaScript 檢測程式碼所產生的 cookie。
...
若要取得新的金鑰， [在 Application Insights 入口網站中 ][new]。
