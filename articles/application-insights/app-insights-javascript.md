...
如果您想要檢查 web 應用程式傳送至 Application Insights 的遙測資料，使用您的瀏覽器偵錯工具 (**F12** 許多瀏覽器上)。 資料會傳送至 dc.services.visualstudio.com。
...
*仍沒有資料？ 按一下 [ **重新整理** 頁面的頂端。 仍然沒有嗎？ 請參閱 [疑難排解](app-insights-troubleshoot-faq.md)。*
...
請注意， *網路連接* 時間是通常低於您所料，因為它是從瀏覽器到伺服器的所有要求的平均值。 許多個別要求的連接時間為 0，因為已經有與伺服器的作用中連線。
...
選取任一事件以查看詳細資料。 在詳細資料頁面中，按一下"..." 若要查看更多詳細資料。
...
* **網頁檢視期間**& #151;它所需的時間載入網頁，並開始執行指令碼。 具體來說，開始載入頁面及執行 trackPageView 之間的間隔。 如果您在指令碼的初始設定之後將 trackPageView 從一般位置移走，它會反映不同的值。
