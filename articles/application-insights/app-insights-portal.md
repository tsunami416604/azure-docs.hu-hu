...
* **任何圖表或並排顯示** 以查看詳細資料。
* **診斷** 以取得預先定義的網頁的其他度量。
* **計量瀏覽器** 來建立您自己選擇的度量資訊頁面。
* **搜尋** 調查的事件，例如要求、 例外狀況或記錄檔的特定執行個體追蹤。
...
> [AZURE.NOTE] 如果您的應用程式會產生大量遙測 (而且您正在使用 ASP.NET SDK 版本 2.0.0-beta3 或更新版本)，調適性取樣模組會自動縮減傳送代表分數的事件傳送至入口網站的磁碟區。 不過，與同一個要求相關的事件會選取或取消選取為群組，讓您可以在相關事件之間瀏覽。 [深入了解取樣](app-insights-sampling.md)。
