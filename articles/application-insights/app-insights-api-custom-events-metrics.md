...
大小限制 `message` 比屬性上的限制高得多。 您可以搜尋訊息內容，但是 (不同於屬性值) 您無法在其中進行篩選。
...
**屬性** 是可用來篩選遙測情況報告中的字串值。 例如，如果您的應用程式提供數個遊戲，則您可以將遊戲的名稱附加至每個事件，以了解哪些遊戲較受歡迎。 

字串長度有 1k 的限制。 (如果您想要傳送大量的資料區塊，使用訊息參數 [TrackTrace](#track-trace)。)
...
**適用 JavaScript web 用戶端**, ，[使用 JavaScript 遙測初始設定式](#js-initializer)。

**若要將屬性加入至所有遙測** 包括標準集合模組中的資料 [建立的遙測初始設定式](app-insights-api-filtering-sampling.md#add-properties)。
...
[詳細資訊](app-insights-api-filtering-sampling.md)
...
1.  您的應用程式具有最多 200 個唯一計量名稱和 200 個唯一屬性名稱。 度量包括透過 TrackMetric，以及其他資料類型，例如事件上的度量單位傳送的資料。  每個檢測金鑰的度量和屬性名稱是全域的，不只限於資料類型。
