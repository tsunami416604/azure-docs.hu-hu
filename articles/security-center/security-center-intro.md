...
> [AZURE.NOTE] 這份文件中的資訊適用於 Azure 資訊安全中心的預覽版本。 這是使用範例部署的服務簡介。
...
> [AZURE.NOTE] 若要編輯安全性原則，您必須是擁有者或參與者的訂用帳戶。

按一下 [ **安全性原則** 並排顯示的訂閱清單，然後選擇 [檢視原則的詳細資料的訂閱。  

![][2]

**資料收集** （如上述） 可讓資料收集安全性原則。 若加以啟用可以：
- 安全監控與建議每日掃描所有支援的虛擬機器
- 安全性事件的分析和威脅的偵測的集合

**顯示建議 ︰** （如上述） 可讓您選擇您想要監視並建議的安全性控制根據訂用帳戶內資源的安全性需求。
...
按一下 [ **建議** 磚的建議清單。 按一下每個建議，檢視其他資訊或採取行動以解決問題。

![][3]

### 資源健康情況

 **資源健全狀況** 磚會顯示資源類型的虛擬機器、 web 應用程式和其他資源環境的整體安全性的情勢。   

在 [選取資源類型 **資源健全狀況** 磚，即可檢視詳細的資訊 （虛擬機器在下面的範例），包括一份已識別出任何潛在安全性漏洞。
