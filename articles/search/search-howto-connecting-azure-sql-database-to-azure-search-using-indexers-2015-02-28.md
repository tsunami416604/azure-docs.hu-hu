...
您可以取得的連接字串 [Azure 傳統入口網站](https://portal.azure.com); 使用 `ADO.NET 連接字串` 選項。
...
有關回應的其他資訊可在 [取得索引子狀態](http://go.microsoft.com/fwlink/p/?LinkId=528198)
...
**間隔** 參數是必需的。 間隔指兩個連續索引子開始執行的時間。 允許的最小間隔為 5 分鐘；最長間隔為一天。 其必須格式化為 XSD"dayTimeDuration"值 (的受限的子集 [ISO 8601 持續時間](http://www.w3.org/TR/xmlschema11-2/#dayTimeDuration) 值)。 這個模式是: `P(nD)(T(nH)(nM))`。 範例: `PT15M` 每隔 15 分鐘， `PT2H` 每隔 2 小時。
...
- 所有項目更新變更資料行的值。
- 每次變更都會增加此資料行的值。
- 使用查詢 `其中` 子句類似 `其中 [上限標準資料行] > [目前的上限標準值]` 可以有效地執行。
...
請注意， **softDeleteMarkerValue** 必須為一個字串 (使用代表實際值的字串)。 例如，如果您有整數資料行且具有值 1 標記為刪除的資料列時，使用 `"1"`; 如果您擁有一個 BIT 資料行，且刪除的資料列標記為布林真值，請使用 `"True"`。
