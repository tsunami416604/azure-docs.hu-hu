...
Az egyes módszerek bevezetését lásd: gyors útmutató [: Azure Cognitive Search index létrehozása a PowerShell](search-create-index-rest-api.md) vagy [ C# a gyors útmutató: Azure Cognitive Search index létrehozása a .net SDK használatával](search-get-started-dotnet.md).
...
A .NET SDK-ban csomagolja az adatait egy `IndexBatch` objektumba. A `IndexBatch` `IndexAction` objektumok gyűjteményét ágyazza be, amelyek mindegyike egy dokumentumot és egy olyan tulajdonságot tartalmaz, amely közli az Azure-Cognitive Search, hogy milyen műveletet kell végrehajtani a dokumentumban. Példaként tekintse meg a [ C# rövid útmutatót.](search-get-started-dotnet.md)
