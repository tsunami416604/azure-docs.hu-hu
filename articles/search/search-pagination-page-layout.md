...
> [AZURE.NOTE] 有效的要求包含一些項目，例如服務 URL 及路徑、 HTTP 動詞命令， `api 版本`, ，依此類推。 為求簡單明瞭，我們縮減此範例，只突顯與分頁相關的語法。 請參閱 [Azure 搜尋服務 REST API](http://msdn.microsoft.com/library/azure/dn798935.aspx) 文件，如需要求語法的詳細資訊。
...
在 Azure 搜尋服務，您可以使用 `$count`, ，`$top`, ，和 `$skip` 參數來傳回這些值。 下列範例示範總點擊數，以傳回的範例要求 `@OData.count`:
...
分頁結果需要 `$top` 和 `$skip`, ，其中 `$top` 指定批次中傳回多少項目和 `$skip` 指定略過多少項目。 在下列範例中，每個頁面顯示下 15 個項目，表示中的遞增跳躍 `$skip` 參數。
...
在 Azure 搜尋服務，您會使用 `$select` 和查閱命令來實作這種體驗。
...
若要擷取的產品描述頁面 **onClick** 事件時，使用 [查閱文件](http://msdn.microsoft.com/library/azure/dn798929.aspx) 傳入要擷取的文件的索引鍵。 索引鍵的資料型別是 `Edm.String`。 在此範例中為 *246810*。
...
在 Azure 搜尋中，排序會根據 `$orderby` 為編製索引的所有欄位的運算式， `"Sortable": true。`
...
您可建立一個方法，它會接受所選取的排序選項做為輸入，然後對於和該選項相關的準則傳回已排序的清單。
 ...
 ![][5]
> [AZURE.NOTE] 雖然預設評分對大多數案例都已足夠，但我們建議改用自訂評分設定檔來建立相關性。 自訂評分設定檔給您提升項目的方式，這會對您的企業更有助益。 請參閱 [新增評分設定檔](http://msdn.microsoft.com/library/azure/dn798928.aspx) 如需詳細資訊。 
...
- [影片和教學課程有關 Azure 搜尋服務](search-video-demo-tutorial-list.md)
