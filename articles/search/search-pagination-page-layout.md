...
Az Azure Search használata a `$count`, `$top`, és `$skip` vissza ezeket az értékeket a paramétereket. A következő példa bemutatja egy mintakérelmet összesítéshez eléri az "online-katalógus", nevű index lekérdezéseire adja vissza a `@odata.count`:
...
Használja az Azure Search szolgáltatásban `$select` és a egy [keresési API-kérelem](https://docs.microsoft.com/rest/api/searchservice/search-documents) a felület megvalósítása.
...
Ha a megoldás kialakításának tartalmazza az adott típusú tartalom (például olyan online kereskedelmi alkalmazás megjelenik az oldal tetején lévő részlegek) dedikált keresési oldalak, beszúrhat egy [szűrési kifejezés](search-filters.md) mellett egy **onClick** esemény előre szűrt állapotban lévő oldal megnyitásához.
