...
在 [Azure App Service](https://tryappservice.azure.com/) 網站，Azure 搜尋服務是 Azure 平台上提供豐富、 全文檢索搜尋體驗，以及搜尋中心功能僅適用於此服務的許多 Web 應用程式範本的一部分。
...
7. 選擇 **編輯與 Visual Studio Team Services** 檢視解決方案，並瀏覽網站。 
9. 在 Visual Studio Team Services 中，展開在頁面頂端的工作階段選項，然後按一下 [ **瀏覽網站**。

    ![][4]

10. 您應該會看到 Azure 搜尋服務網站的開始使用頁面。 按一下 [ **開始** ] 按鈕以開啟網站。

    ![][5]

11. ASP.NET 網站會在瀏覽器中開啟，提供搜尋方塊。 輸入要搜尋的這類的熟悉詞彙 *Yellowstone* 或知名的山 *Mount Rainier*。 從熟悉的地標開始以輕鬆評估結果。
...
Azure 搜尋服務具有可設定 **searchMode** 屬性可讓您控制搜尋運算子行為。 此屬性的有效值為 `Any` (預設值) 或 `All`。 請參閱 [簡單的查詢語法](https://msdn.microsoft.com/library/dn798920.aspx) 如需設定這些選項的詳細指引。

- **searchMode.Any** 規定搜尋詞彙的任何相符項目就足以在搜尋結果中包含一個項目。 如果您的搜尋片語是 `Yellowstone visitor center`，則任何包含這當中任何詞彙的文件都會包含在搜尋結果中。 這種模式偏向 *回收*。
- **searchModel.All**, ，此範例中使用，需要指定之詞彙的所有文件中出現。 這個模式會比更嚴苛 **searchMode.Any**, ，但是如果您偏好 *精確度* 勝過重新叫用，它可能是適用於您的應用程式。 

> [AZURE.NOTE] **searchMode.Any** 時查詢建構包含大部分片語，使用最少量運算子的效果最佳。 根據一般的經驗法則，搜尋消費者應用程式 (例如電子商務網站) 的人員傾向於只使用詞彙，而搜尋內容或資料的人員更有可能在搜尋片語中包含運算子。 如果您認為搜尋可能會包括運算子，尤其是 `NOT (-)` 運算子，開始 **searchModel.All**。 相較之下，其他選擇， **searchMode.Any** 將 `OR`  `NOT` 與其他搜尋詞彙，而不是去除結果可大幅擴充的運算子。 下列範例可協助您了解差異。

在這個工作中，您將變更 **searchMode** 和比較模式為基礎的搜尋結果。

1. 開啟包含範例應用程式的瀏覽器視窗中，選擇 [ **連接到 Visual Studio Team Services**。
...
在重建應用程式視窗中，輸入搜尋詞彙，曾用過，例如 `Yellowstone +center +building -ND`, ，並比較之前和之後的結果變更 **searchMode**。

這是個很大的差異。 您不會取得七個搜尋結果，而是取得超過兩百萬個。 

   ![][11]
 
您正在觀察的行為是因為包含 `NOT` 運算子 (在此情況下，"-ND")，也就是 *或有* 而 *AND 有* 時 **searchMode** 設為 `Any`。
...
無可否認地，這可能是罕見的案例，但它也說明了的效果 **searchMode** 上搜尋片語包含 `NOT` 運算子，因此有助於了解為什麼會發生的行為，並變更它，如果這是不是您想要的。

若要繼續本教學課程中，還原 **searchMode** 回其原始值 (設為 `All` 第 39 行)、 執行程式，並且將重建應用程式用於其餘的工作。
...
既然您進行了一連串的單行程式碼變更，您可能會想要嘗試更深層的修改，需要在多個位置中的程式碼變更。 下列版本 **Search.cshtml** 可貼上的 Search.cshtml 檔案在您目前的工作階段中。 
...
您可能想要儲存一份原始 **Search.cshtml** 檔案來查看這兩種版本的比較。

> [AZURE.NOTE] 若要減少檔案大小已除去註解。
