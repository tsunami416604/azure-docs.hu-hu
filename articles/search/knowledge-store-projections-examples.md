...
Ha ezt a készségkészlet használja a nullával `knowledgeStore` , az első példa az `knowledgeStore` objektum kitöltésére szolgál, amely olyan kivetítésekkel van konfigurálva, amelyek táblázatos adatstruktúrákat hoznak létre, amelyeket más helyzetekben használhatunk. 
...
>
> Figyelje meg az utolsó két `KeyPhrases` csomópontot `Entities`, és. Ezek egy érvényes JSON-objektumba vannak becsomagolva `sourceContext`a következővel:. Erre azért van szükség `keyphrases` , `entities` mert a primitívek gazdagítása szükséges, és az érvényes JSON-ra kell alakítani, mielőtt a projekt elérhetővé válnak.
>
...
Most, hogy minden szükséges adattal rendelkezünk a táblákhoz, frissítse a knowledgeStore objektumot a táblázat-definíciókkal. Ebben a példában három táblázat van megadva, a `tableName` `source` és `generatedKeyName` a tulajdonságok beállításával.
...
A példában `pbiShape` a az összevont alakzat (vagy a dúsítási csomópont). A vetítés definíciójában a `pbiShape` további táblákba kerül, így az alakzat részeit ```keyPhrases``` és. ```Entities``` Power BI ez hasznos lehet, ha több entitást és alkifejezést társít az egyes dokumentumokhoz, és további elemzéseket fog kapni, ha kategorizált adatként látja az entitásokat és a kifejezéseket.

A szeletelés implicit módon létrehoz egy kapcsolatot a szülő és a gyermek tábla között, ```generatedKeyName``` és a fölérendelt táblában létrehoz egy azonos nevű oszlopot az alárendelt táblában. 

### <a name="naming-relationships"></a>Elnevezési kapcsolatok

A ```generatedKeyName``` és ```referenceKeyName``` a tulajdonságok a táblák közötti vagy akár a kivetítési típusok közötti összekötésére szolgálnak. A gyermektábla és a leképezés minden sorának egy tulajdonsága a szülőre mutat vissza. A gyermek oszlopának vagy tulajdonságának neve a szülőből ```referenceKeyName``` származik. Ha a ```referenceKeyName``` nincs megadva, a szolgáltatás alapértelmezés szerint a szülőtől a ```generatedKeyName``` értékre kerül. 
...
A fájlok leképezésének létrehozásához a `files` tömböt használjuk a leképezési objektumban. Ebben a példában a dokumentumból kinyert összes lemezképet egy `samplefile`nevű tárolóba tervezik.
...
A kivetítésekhez szükséges alakzatok beszerzéséhez hozzon létre egy új, formázó nevű objektumot, amely létrehoz egy nevű `crossProjection`alakzatot. 
...
Ez a példa a kivetítések egy másik funkcióját is kiemeli. Ha egyazon kivetítési objektumon belül több típusú kivetítést határoz meg, akkor a különböző típusokon (táblák, objektumok, fájlok) belül és az azokon belüli kapcsolat is megadható, amely lehetővé teszi, hogy a dokumentumhoz tartozó táblázatos sorral kezdődjön, és megkeresse az adott dokumentumban található lemezképek összes OCR-szövegét az objektum leképezésében. 
