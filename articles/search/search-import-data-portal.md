...
Azure 傳統入口網站包含 **匯入資料** 命令在 Azure 搜尋服務儀表板，可引導您在 Azure 搜尋服務中擷取資料。 此命令依賴內建索引子功能來搜耙現有的資料來源，並根據在資料來源中找到的資料列集建立並上傳文件。
...
若要使用索引子或 **匯入資料** 命令時，主要資料來源必須是其中一個支援的資料來源 ︰ Azure SQL Database，Azure DocumentDB 或 Azure VM 上的 SQL Server 關聯式資料庫。
...
3. 在服務儀表板中，您會看到命令列在頂端，其中包括 **匯入資料**。 按一下 [ **匯入資料** 以滑動開啟 [匯入資料] 分頁。

4. 按一下 [ **連接到您的資料** 指定索引子所使用的資料來源定義。 選項包括：
...
5. 按一下 [ **自訂目標索引** 完成的預設索引。
    -  **金鑰** 需要。 您為索引鍵選取的欄位必須是包含唯一值的字串欄位。
    ...
    - 按一下 [ **分析器** 索引標籤上，如果您想要指定欄位層級語言分析器。 請參閱 [中多個語言的文件索引建立](search-language-support.md) 如需詳細資訊。
    - 按一下 [ **建議工具** 如果您想要啟用自動完成或預先輸入查詢建議。

6. 按一下 [ **匯入資料** 執行匯入作業，使用 [立即執行] 選項或設定週期性的排程。
