...
1. 建立資料來源類型的 `azureblob` Azure 儲存體帳戶中參考的容器 (和 (選擇性) 在該容器中的資料夾)
    - 在為儲存體帳戶連接字串中傳遞 `credentials.connectionString` 參數
    - 指定容器名稱。 您可以選擇性地包含資料夾，請使用 `查詢` 參數
...
- 文件的全部文字內容擷取至名為的字串欄位 `內容`。 請注意，我們目前不支援從單一 blob 擷取多個文件：
    - 例如，CSV 檔案會編制索引為單一文件。
...
- 標準 blob 中繼資料屬性會擷取到下列欄位：
    ...
    - **metadata\_storage\_content\_type** (Edm.String) 的內容與您用來上傳 blob 的程式碼所指定的類型。 例如， `應用程式/八位元組資料流`。
...
- **metadata\_storage\_name** -這可能是方便的候選項目，但請注意，1) 的名稱可能不是唯一的因為您可能會有 blob 具有相同的名稱，在不同的資料夾，以及 2) 的名稱可能包含無效的文件索引鍵，例如連字號字元。 您可以藉由處理無效的字元 `base64EncodeKeys` 選項中的索引子內容，如果這麼做，請記得當傳入 API 呼叫，例如查閱編碼文件索引鍵。 (例如，在.NET 中您可以使用 [UrlTokenEncode 方法](https://msdn.microsoft.com/library/system.web.httpserverutility.urltokenencode.aspx) 針對該用途)。

- **metadata\_storage\_path** -使用完整路徑，可確保唯一性，但路徑明確包含 `/` 字元 [無效的文件索引鍵](https://msdn.microsoft.com/library/azure/dn857353.aspx)。 如上所述，您可以選擇編碼使用的索引鍵 `base64EncodeKeys` 選項。
...
此範例中，讓我們挑選 `metadata_storage_name` 做為文件索引鍵欄位。 我們也假設您的索引具有索引鍵欄位，名為 `金鑰` 和欄位 `檔案大小` 來儲存文件大小。 若要連接所需的項目，在建立或更新您的索引子時，指定下列欄位對應：
...
當您設定 blob 的索引子的排程上執行時，它會重新索引只變更的 blob 的 blob 所決定 `LastModified` 時間戳記。
...
例如，如下所示的原則會考量已刪除 blob，有中繼資料屬性 `IsDeleted` 值 `true`:
...
-------------------------------|-------------------------------------------|-------------------
 ...
 XML (application/xml)| `metadata_content_type`</br>`metadata_content_encoding`</br>| 移除 XML 標記，並且擷取文字 </td>
 JSON (application/json)| `metadata_content_type`</br>`metadata_content_encoding`| 擷取文字<br/>附註: 如果您需要擷取多個文件欄位從 JSON blob，請投票 [UserVoice 這項建議](https://feedback.azure.com/forums/263029-azure-search/suggestions/11113539-extract-document-structure-from-json-blobs)
