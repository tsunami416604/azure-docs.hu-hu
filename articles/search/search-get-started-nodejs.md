...
## <a name="1---define-and-create-index"></a>1 – index meghatározása és létrehozása 
...
Azure Search a dokumentumok olyan adatstruktúrák, amelyek mind a lekérdezések indexeléséhez, mind pedig a kimenetekhez tartoznak. A dokumentumok bemenetei lehetnek egy adatbázisban lévő sorok, a blob Storage-ban található Blobok, vagy a példában szereplő JSON-dokumentumok a lemezen. Töltse le a [Hotels. JSON](https://github.com/Azure-Samples/azure-search-javascript-samples/quickstart/blob/master/hotels.json) fájlt, vagy hozzon létre saját **Hotels. JSON** fájlt a következő tartalommal:
...
## <a name="2---a-class-for-azure-search"></a>2 – a Azure Search osztálya 
...
Adja hozzá a következő metódusokat az osztály törzsében:
...
Végül adja hozzá a Azure Search indextel használható metódusokat. Ezek a metódusok mind ugyanazzal a struktúrával rendelkeznek:
...
## <a name="3---create-a-program"></a>3 – program létrehozása
...
* A **Hotels. JSON** fájlból származó adatok.
...
Most adjon hozzá néhány alapszintű lekérdezést: 
...
Az első lekérdezés az összes adatokat (`*`) és a visszaadott rekordok számát adja vissza. A második csak azokat a dokumentumokat fogja visszaadni, amelyek tartalmazzák a "Historic" szót a **hotels_quickstart_index. JSON** fájlban "kereshető" kifejezéssel, és amelynek `Rating` mezője 4-nél nagyobb értéket tartalmaz. További információ a [lekérdezések összeállításáról Azure Searchban](https://docs.microsoft.com/azure/search/search-query-overview). 

A [ **NConf** csomag](https://github.com/indexzero/nconf) lehetővé teszi, hogy különböző formátumokban (például környezeti változókban vagy a parancssorban) megadhatja a konfigurációs adatmennyiséget. A **NConf** alapszintű módon fogjuk használni a **azure_search_config. JSON** fájl olvasásához és a fájl tartalmának a szótárként való visszaküldéséhez. A **NConf** `get(key)` funkciójának használatával gyorsan megtekintheti, hogy a ["Azure Search szolgáltatás adatainak konfigurálása"](#configure) lépés nem lett kihagyva. Végül visszaállítjuk a konfigurációt:
...
A `doQueries()` függvény végrehajt egy `AzureSearchClient` objektumot, és alkalmazza `AzureSearchClient.queryAsync` a `queries` metódust a tömb minden egyes értékére. A `Promise.all()` függvény használatával egyetlen `Promise` műveletet ad vissza, amely csak akkor oldódik fel, ha az összes lekérdezés megoldódott. A lekérdezési `JSON.stringify(body, null, 4)` eredmény olvashatóbbként való formázásának meghívása.
...
Egy sor üzenet jelenik meg, amely leírja, hogy a program milyen műveleteket végez, és a lekérdezés eredményeivel végződik. Ha szeretné megtekinteni a kérések részleteit, a **AzureSearchClient. js** [20-26](https://github.com/Azure-Samples/azure-search-javascript-samples/quickstart/blob/master/AzureSearchClient.js#LL20-LL26) -es sorait törölheti. 
