...
+   [限制和條件約束](search-limits-quotas-capacity.md)
+   [定價詳細資料](http://go.microsoft.com/fwlink/p/?LinkdID=509792)
...
若要協助進行未來規劃，您可能想要檢查儲存體 (使用 [取得索引統計資料](http://msdn.microsoft.com/library/dn798942.aspx)) 查看多少實際上您可以使用。
...
就 Azure Search 而言，角色存取控制可決定下列管理工作：
 ...
 角色| 工作
---|---
 擁有者| 啟動、 停止或刪除服務。<p>產生和檢視管理金鑰和查詢金鑰。<p>檢視服務狀態，包括索引計數、 索引名稱、 文件計數和儲存體大小。<p>(只有 「 擁有者可以管理角色成員資格) 的新增或刪除角色成員資格。<p>訂閱和服務管理員自動具有成員資格的擁有者角色。
 參與者| 除了角色管理以外，具有與「擁有者」相同層級的存取權。例如，參與者可以檢視和重新產生 `api 金鑰`, ，但是他或她不能修改角色成員資格。
