...
Azure 搜尋服務是 Microsoft Azure 上之託管的雲端搜尋服務。 本文件說明 ** 2015年-02-28* Azure 搜尋管理 REST API 版本。 在那之後，較新版本已取代它。 最新消息，請參閱 [Azure 搜尋管理 REST API 2015-08-19](https://msdn.microsoft.com/library/dn832684.aspx) MSDN 上。
...
- 建立或刪除 Azure 搜尋服務。
- 建立、 重新產生，或擷取 `api-key` 自動化例行系統管理金鑰，用來驗證搜尋資料作業的變更。
- 調整 Azure 搜尋服務級別以回應查詢量或儲存空間需求的變更。

若要完全以程式設計方式管理您的服務，您將需要兩個 Api: 管理 REST API 的 Azure 搜尋服務，再加上一般 [Azure 資源管理員 REST API](https://msdn.microsoft.com/library/azure/dn790568.aspx)。 資源管理員 API 用於非服務特定的一般用途作業，例如查詢訂用帳戶資料、列出地理位置等。 若要在您的訂用帳戶建立和管理 Azure 搜尋服務，請確定您的 HTTP 要求包含資源管理員端點、訂用帳戶識別碼、提供者 (在此情況下為 Azure 搜尋服務) 及搜尋服務特定的作業。

[開始使用 Azure 搜尋管理 REST API](http://go.microsoft.com/fwlink/p/?linkID=516968) 會逐步解說示範應用程式組態和服務管理作業的範例程式碼。 此範例應用程式發出要求給 Azure 資源管理員 API 以及 Azure 搜尋服務管理 API，讓您了解如何將利用此兩種 API 的應用程式組合為緊密結合的應用程式。

### 端點

服務管理作業的端點是 Azure 資源管理員的 URL， `https://management.azure.com`。

請注意所有管理 API 呼叫都必須包含訂用帳戶 ID 和 API 版本。

### 版本

目前的 Azure 搜尋管理 REST API 版本是 `api 版本 = 2015年-02-28`。 先前的版本， `api 版本 = 2014年-07-31-預覽` 已被取代。 雖然它仍會在接下來幾個月持續運作，但仍建議您儘速轉換至新的版本。
...
- 對 Azure 搜尋服務端點，例如建立索引或搜尋文件的資料要求使用 `api 金鑰` 要求標頭中。 請參閱 [Azure 搜尋服務 REST API](https://msdn.microsoft.com/library/azure/dn798935.aspx) 如需驗證資料要求的相關資訊。

範例應用程式記錄在 [開始使用 Azure 搜尋管理 REST API](http://go.microsoft.com/fwlink/p/?linkID=516968) 示範各種作業類型的驗證技術。 使用 Active Directory 來設定用戶端應用程式的指示包含於「開始使用」中。
...
- <a name="CreateService">建立搜尋服務</a>
    ...
    `將 [subscriptionId]/resourceGroups/[resourceGroupName]/providers/Microsoft.Search/searchServices/[serviceName]?api-version=2015-02-28 https://management.azure.com/subscriptions/`

- <a name="GetService">取得搜尋服務</a>

    `取得 [subscriptionId]/resourceGroups/[resourceGroupName]/providers/Microsoft.Search/searchServices/[serviceName]?api-version=2015-02-28 https://management.azure.com/subscriptions/`

- <a name="ListService">列出搜尋服務</a>

    `取得 [subscriptionId]/resourceGroups/[resourceGroupName]/providers/Microsoft.Search/searchServices?api-version=2015-02-28 https://management.azure.com/subscriptions/`

- <a name="DeleteService">刪除搜尋服務</a>

    `刪除 [subscriptionId]/resourceGroups/[resourceGroupName]/providers/Microsoft.Search/searchServices/[serviceName]?api-version=2015-02-28 https://management.azure.com/subscriptions/`

- <a name="UpdateService">更新搜尋服務</a>

    `修補程式 https://management.azure.com/subscriptions/ [subscriptionId]/resourceGroups/[resourceGroupName]/providers/Microsoft.Search/searchServices/[serviceName]?api-version=2015-02-28`
...
- <a name="ListQueryKey">列出查詢金鑰</a>
    ...
    `取得 [subscriptionId]/resourceGroups/[resourceGroupName]/providers/Microsoft.Search/searchServices/[serviceName]/listQueryKeys?api-version=2015-02-28 https://management.azure.com/subscriptions/`

- <a name="DeleteQueryKey">刪除查詢金鑰</a>

    `刪除 [subscriptionId]/resourceGroups/[resourceGroupName]/providers/Microsoft.Search/searchServices/[serviceName]/deleteQueryKey/[key]?api-version=2015-02-28 https://management.azure.com/subscriptions/`
...
`subscriptionId`: 必要。  `SubscriptionID` Azure 使用者。 您可以從 Azure 資源管理員 API 或入口網站取得這個值。

`resourceGroupName`: 必要。 使用者訂用帳戶內的資源群組名稱。 您可以從 Azure 資源管理員 API 或入口網站取得這個值。

`serviceName`: 必要。指定資源群組內的搜尋服務名稱。服務名稱只能包含小寫字母、數字或連字號，但不能將連字號用在前兩個或最後一個字元，也不能包含連續的連字號，而且長度必須介於 2 到 15 個字元。由於所有名稱都最後 <name>。.search.windows.net，所以服務名稱必須是全域唯一。訂用帳戶和資源群組之內或之間的任兩個服務名稱都不能相同。服務名稱建立後就不能再變更。

`api-version`: 必要。 指定用於此要求的通訊協定版本。 目前的版本是 `2015年-02-28`。
...
`x-用戶端要求-ms-request-id`: 選擇性。 用戶端產生之可識別此要求的 GUID 值。 如果指定，這會包含於回應資訊做為對應要求的方式。
...
`位置`: 必要。 一個受支援且已註冊的 Azure 地理區域 (例如：美國西部、美國東部、東南亞等)。 請注意，一旦建立資源位置後就不能再變更。

`標記`: 選擇性。 描述此資源的金鑰值組清單。 這些標記可用來在資源群組間檢視資源，以及對資源分組。 對於一項資源可提供最多十個標記。 每個標記的金鑰都不得多於 128 個字元，值則不得多於 256 個字元。

`sku`: 必要。 有效值為 `免費` 和 `標準`。 `standard2` 也有效，但僅適用於您 Azure 訂用帳戶啟用由 Microsoft 支援人員時。 `免費` 會在共用叢集。 `標準` 會在專用的叢集中。 您只能在免費定價層建立一項搜尋服務。 其他服務必須在標準定價層建立。 根據預設，一項服務會以一個分割區和一個複本來建立。 其他分割區和複本會依據搜尋單位計價。 請參閱 [限制和條件約束](search-limits-quotas-capacity.md) 如需詳細資訊。 您無法變更 `sku` 一旦建立服務。

`replicaCount`: 選擇性。 預設值為 1。 有效值包含 1 到 6。 時才有效 `sku` 是 `標準`。

`partitionCount`: 選擇性。 預設值為 1。 有效值包含 1、2、3、4、6 或 12。 時才有效 `sku` 是 `標準`。
...
`識別碼`: 此 id 為這項搜尋服務的 URL (不包含主機名稱和配置)。

`名稱`: 搜尋服務的名稱。

`位置`: 其中一個支援並註冊的 Azure 地理區域 (例如，美國西部、 美國東部、 東南亞等等)。

`標記`: 描述用於檢視和群組資源的資源群組中的資源的索引鍵-值組清單。
...
`狀態`: 呼叫作業時的搜尋服務的狀態。 有效值包含：

- `執行`: 建立搜尋服務。
- `佈建`: 正在佈建搜尋服務。
- `刪除`: 正在刪除搜尋服務。
- `降級`: 搜尋服務已降級。 當叢集發生不一定會使服務無法正確運作的錯誤時，則產生這些值。
- `停用`: 搜尋服務停用。 在此狀態下，此服務將拒絕所有 API 要求。
- `錯誤`: 搜尋服務處於錯誤狀態。

**注意**: 如果您的服務是在 `降級`, ，`停用`, ，或 `錯誤` 狀態，這代表 Azure 搜尋服務團隊正主動調查潛在問題。 根據已佈建的搜尋單位數目，在這些狀態下的專用服務仍需收費。

`statusDetails`: 狀態的詳細資料。

`provisioningState`: 表示目前佈建服務的狀態。 有效值包含：

- `成功`: 佈建已順利完成。
- `佈建`: 正在佈建服務。
- `失敗`: 本服務無法佈建。

佈建是一種過渡狀態，當服務容量正在建立時發生。 設定容量之後, `provisioningState` 變更為 「 成功 」 或 「 失敗 」。 用戶端應用程式可以輪詢佈建狀態 (建議的輪詢間隔為 30 秒，最多 1 分鐘)，可藉由使用**取得搜尋服務**作業查看作業何時完成。 如果您正在使用免費服務，這個值通常會做為「成功」而直接在呼叫中返回，以建立服務。 這是因為免費服務使用早已設定的容量。
...
`位置`: 資源的位置。 這會是一個受支援且已註冊的 Azure 地理區域 (例如：美國西部、美國東部、東南亞等)。

`標記`: 標記為描述此資源的機碼值組的清單。 這些標記可用來在資源群組間檢視資源，以及對資源分組。
...
請注意， `nextLink` 欄位一律為 null，因為目前的版本不支援分頁。 這會以空值返回，以保留未來的相容性。
...
請注意，您不能變更名稱、位置以及 SKU。 變更這些屬性的任何一種都需要建立新的服務。
