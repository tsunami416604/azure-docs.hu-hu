...
此命令會使用 **Name** 參數來指定資源群組的名稱，並使用 **Location** 參數來指定其位置。 根據我們在上一節中所發現，我們將使用 「 美國西部 」 
位置。
...
您指定資源群組和範本的位置。 如果您的範本並非本機，可以使用 **-TemplateUri** 參數，並針對範本指定 URI。 您可以設定 
**-模式** 參數為 **累加** 或 **完成**。 資源管理員預設會在部署期間執行累加式更新；因此，當您想要使用 **Incremental** 時不需要設定 **-Mode**。 
