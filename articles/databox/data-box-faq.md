...
description: Gyakran ismételt kérdéseket és válaszokat tartalmaz az Azure Data Box felhőalapú megoldásához, amely lehetővé teszi, hogy nagy mennyiségű adatot továbbítson az Azure-ba.
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
A. Az Azure Data Box lehetővé teszi a terabájtnyi adatok gyors, olcsó és biztonságos átvitelét az Azure-ba. A Data Box eszköz az Azure Portalon keresztül rendelhető meg. A Microsoft egy 80 TB-os kapacitású tárolóeszközt szállít önnek egy regionális szolgáltatón keresztül. 
...
A. Ha 40 és 500 TB közötti mennyiségű adata van, amelyet szeretne átvinni az Azure-ba, akkor hasznos lehet a Data Box használata. A 40 TB-os adatméretek < a Data Box Disk-et, az adatméretet pedig > 500 TB-os adatméreteket használja, regisztráljon a [Data Box Heavy](data-box-heavy-overview.md)mezőre.
...
A. A Data Box összes kapacitása 100 TB, használható kapacitása pedig 80 TB. A Data Box segítségével akár 80 TB adatot is átvihet. Több adat átviteléhez több eszköz megrendelése szükséges.
...
A.  Ha tudni szeretné, hogy mely országok/régiók közül érhető el az Adatmező, látogasson el a [régió elérhetőségére.](data-box-overview.md#region-availability)  

### <a name="q-which-regions-can-i-store-data-in-with-data-box"></a>K. Mely régiókban tárolhatok adatokat a Data Box segítségével?
A. A Data Box az Egyesült Államok, Nyugat-Európa, Észak-Európa, Franciaország, az Egyesült Királyság, Japán, Ausztrália és Kanada összes régiójában támogatott. További információt a [Regionális elérhetőség](data-box-overview.md#region-availability) témakörben talál.
...
### <a name="q-i-have-lost-my-data-box-is-there-a-lost-device-charge"></a>K. Elvesztettem a Data Boxomat. Elveszett az eszköz töltöttségi díja?
A. Igen. Elveszett vagy sérült készüléktöltés. Ezt a díjat az [Árképzés i. oldal](https://azure.microsoft.com/pricing/details/storage/databox/) és a [Termékszolgáltatási feltételek](https://www.microsoft.com/licensing/product-licensing/products)is fedezik.
...
### <a name="q-can-i-connect-to-both-the-10-gbe-interfaces-on-the-data-box-to-transfer-data"></a>K. Csatlakozhatok az adatdoboz 10 GbE-s felületéhez az adatátvitelhez?
A. Igen. Mind a 10 GbE interfészek csatlakoztatható a Data Box adatok másolása egy időben. Az adatok másolásával kapcsolatos további információkért látogasson el [az Oktatóanyag: Adatok másolása az Azure Data Boxba című lapban.](data-box-deploy-copy-data.md)
...
A. Ha a rendszerhibajelző LED világít, az azt jelzi, hogy a rendszer állapota nem kifogástalan. A következő lépésekért forduljon a [Microsoft támogatási szolgálatához.](data-box-disk-contact-microsoft-support.md)
...
A. Ha nem fér hozzá a Data Box zárolásfeloldási jelszavához az Azure Portalon, ellenőrizze az előfizetése és a tárfiókja jogosultságait. Győződjön meg arról, hogy közreműködői vagy tulajdonosi jogosultságokkal rendelkezik az erőforráscsoport szintjén. Ha nem, akkor legalább Data Box Operator szerepkör-engedéllyel kell rendelkeznie a hozzáférési hitelesítő adatok megtekintéséhez.

### <a name="q-is-port-channel-configuration-supported-on-data-box-how-about-mpio"></a>K. Támogatja a portcsatorna-konfigurációt a Data Box? Mit szólsz az MPIO-hoz?
A. Nem támogatjuk a portcsatorna-konfigurációt, a többutas IO (MPIO) konfigurációt vagy a vLAN-konfigurációt a Data Boxon.
...
A.  A Data Box hasznos tárolókapacitása 80 TB. Egyetlen Data Box-eszközön 40 TB és 80 TB közti mennyiségű adatot tárolhat. Az akár 500 TB-os adatméretek esetén több Data Box eszközt is megrendelhet. Az 500 TB-ot meghaladó adatmennyiségek esetén regisztráljon a Data Box Heavy szolgáltatásba.  
...
### <a name="q-can-i-pick-up-my-data-box-order-myself-can-i-return-the-data-box-via-a-carrier-that-i-choose"></a>K. Magam is felvehetem a Data Box rendelésemet? Visszaküldhetem a Data Boxot az általam választott szolgáltatón keresztül?
A. Igen. A Microsoft csak az Egyesült Államok kormányának régiójában kínál saját irányítású szállítást is. A Data Box rendelés elhelyezéseke során kiválaszthatja a saját maga által kezelt szállítási lehetőséget. A Data Box-eszköz felvételéhez tegye a következő lépéseket:
    
1. A megrendelés lerendelése után a rendszer feldolgozza a rendelést, és előkészíti az Adatdobozt. E-mailben értesítjük, hogy a megrendelés készen áll a felvételre. 
...
Az adatmásolás befejezése után hajtsa végre az alábbi lépéseket az eszköz visszaküldéséhez:

1. Miután az adatmásolás hiba nélkül befejeződött, **futtassa a Felkészülés a szállításhoz**parancsot. Az előkészítés befejezése után kap egy kódot az eszköz helyi webes felhasználói felületén. Másolja és mentse a kódot.
2. Kapcsolja ki a készüléket, és távolítsa el a csatlakozó kábeleket.
3. Tekerje fel, és biztonságosan helyezze el az eszközhöz mellékelt tápkábelt az eszköz hátuljában.
4. Küldje el e-mailben az [Azure Data Box Operations csapatát,](mailto:adbops@microsoft.com) és adja meg nekik a korábban mentett kódot. Ők ad információt, hogy hol és mikor kell leadni a készüléket.

### <a name="q-will-my-data-box-devices-cross-country-borders-during-shipping"></a>K. A Data Box-eszközeim a szállítás során országhatárokon is átfognak lépni?
A. Minden Data Box eszközt ugyanabból az országból szállítunk, mint a célállomást, és nem lépik át a nemzetközi határokat. Az egyetlen kivétel az Európai Unióban (EU) érkező megrendelések, ahol az eszközök bármely uniós országba és onnan kiszállíthatók. Ez a Data Box és a Data Box Heavy eszközökre egyaránt vonatkozik.

### <a name="q-i-ordered-a-data-box-in-us-east-but-i-received-a-device-that-was-shipped-from-a-location-in-us-west-where-should-i-return-the-device-to"></a>K. Rendeltem egy Data Box az USA-ban Keleten, de kaptam egy eszközt, hogy szállították egy helyen az USA Nyugati. Hova küldjem vissza az eszközt?
A. Igyekszünk, hogy a Data Box eszköz a lehető leggyorsabban az Ön számára. A tárfiók helyéhez legközelebb eső adatközpontból származó szállítmányt prioritásként kezeljük, de bármely azure-os adatközpontból, amely rendelkezik rendelkezésre álló készlettel, szállítunk egy eszközt. A Data Box-ot vissza kell küldeni ugyanarra a helyre, ahonnan a szállítási címkén látható módon szállították.
...
- Nyissa meg az Azure Portalon a megrendelését. Nyissa meg **az Áttekintés** és a Szállítási **címke letöltése című témakört.** További információ a [fuvarlevélcímke letöltéséről](data-box-portal-admin.md#download-shipping-label).
...
### <a name="q-i-received-my-device-and-it-is-not-booting-up-how-do-i-ship-the-device-back"></a>K. Megkaptam a készüléket, és nem indul el? Hogyan szállíthatom vissza az eszközt?
A. Ha az eszköz nem indul el, nyissa meg a rendelést az Azure Portalon. Töltse le a szállítási címkét, és elhelyezi, hogy az eszközön. További információ a [fuvarlevélcímke letöltéséről](data-box-portal-admin.md#download-shipping-label).
...
A.  Amint az **Adatmásolat** rendelési állapota **készként**jelenik meg, azonnal hozzáférhet az adatokhoz.
...
A.  Amikor másolja az adatokat a Data Box, attól függően, hogy az adatok blokk blob vagy lap blob vagy Azure-fájlok, az adatok feltöltése az alábbi elérési utak az Azure Storage-fiókban.
...
A.  Ha a tárolónevek nagybetűsek, akkor ezek a nevek automatikusan kisbetűssé alakulnak. Ha a nevek valami más szempontból nem megfelelőek (például speciális karaktereket tartalmaznak, más nyelvűek stb.) akkor a feltöltés meghiúsul. További információk a megosztások, tárolók és fájlok elnevezésének ajánlott eljárásairól:
...
A.  Az adatmásolást követően a **szállításra való előkészítés** művelet futtatásakor a rendszer ellenőrzi az adatokat. Az ellenőrzési folyamat során létrehoz egy listát az adatfájlokról és azok ellenőrzőösszegeiről. Letöltheti a fájlok listáját, és ellenőrizheti a listát a forrásadatokban lévő fájlok között. További információ [a szállításra való előkészítésről](data-box-deploy-picked-up.md#prepare-to-ship).
...
- Szállítási logisztika a UPS-től.
...
### <a name="are-the-carrier-tracking-logs-from-ups-available"></a>Elérhetők-e a (UPS-től) a szállítmányozói nyomon követési naplók? 
...
A.  Nem. Jelenleg az Azure datacenter nem fogadja el a Data Box kézbesítését az ügyfelektől vagy a UPS-től eltérő szolgáltatóktól.
