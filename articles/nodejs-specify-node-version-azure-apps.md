...
其中 *version* 是要使用的特定版本號碼。 您可以指定較複雜的條件，如版本，例如:
...
2. 建立新網站並記下網站的名稱。 例如，下列方式使用 [Azure 命令列工具] 來建立新的 Azure 網站，名為 **mywebsite**, ，然後啟用網站的 Git 儲存機制。
...
現在您已了解如何指定應用程式所使用的 Node.js 版本，了解如何 [模組]、 [建置和部署 Node.js 網站] 和 [如何使用適用於 Mac 和 Linux 的 Azure 命令列工具]。
