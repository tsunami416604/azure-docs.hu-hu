...
或者，您可以在其中建立磁碟分割的佇列或主題，在 Visual Studio 中或 [Azure 傳統入口網站][]。 當您在入口網站中建立新的佇列或主題時，請檢查 **啟用分割** 選項 **設定** 佇列或主題視窗索引標籤。 在 Visual Studio 中，按一下 [ **啟用分割** 中的核取方塊 **新佇列** 或 **新主題** 對話方塊。
...
若要傳送到工作階段感知的主題或佇列的交易式訊息，訊息必須 [BrokeredMessage.SessionId][] 屬性集。 如果 [BrokeredMessage.PartitionKey][] 也指定屬性，它必須等於 [SessionId][] 屬性。 如果兩者不同，則服務匯流排會傳回 **InvalidOperationException** 例外狀況。
