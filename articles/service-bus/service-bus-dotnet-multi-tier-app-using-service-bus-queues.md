...
5.  確定命名空間名稱可用之後，請選擇
    您的命名空間應裝載在哪個國家或地區 (請
    確實使用您部署運算資源所在的相同
    國家/地區)。 另外，請確定您在命名空間的 [**類型**] 欄位中選取 [**傳訊**]，並在 [**傳訊層**] 欄位中選取 [**標準**]。
...
具現化用戶端 (例如，服務匯流排 [QueueClient []][])，您可以表示成連接字串的組態資訊。 在用戶端，有個 `CreateFromConnectionString()` 方法可以使用該連接字串來具現化該用戶端類型。 例如，假設有下列組態區段
...
您現在將建立背景工作角色來處理所提交的
順序。 本範例使用 **Worker Role with Service Bus Queue** Visual Studio 專案範本。 首先，使用 Visual Studio 中的 [伺服器總管] 取得必要認證。
...
11. 在 **WorkerRole.cs**, 的值取代 **QueueName** 變數中 **WorkerRole.cs** 從 `"ProcessingQueue"` 至 `"OrdersQueue"` 如下列程式碼所示。
...
13. 在 `run ()` 函式 `OnMessage` 呼叫中，新增下列程式碼 `嘗試` 子句。
...
* [.NET 多層式應用程式使用儲存體資料表、 佇列和 Blob ][mutitierstorage]
