...
1. A [klasszikus Azure portálra][] fő ablakában kattintson az előző lépésben létrehozott névtér nevére.

1. Kattintson a **Configure** (Konfigurálás) lapra.

1. A **megosztotthívóbetű-készítő** területen jegyezze fel a **RootManageSharedAccessKey** házirendhez társított **elsődleges kulcs** értékét, vagy másolja a vágólapra. Ezt az értéket az oktatóanyag későbbi részében fogja használni.
...
- A szolgáltatásnévtér és a [közös hozzáférésű jogosultságkód](service-bus-sas-overview.md) (SAS-) kulcs segítségével fér hozzá a Service Bus-névtér erőforrásaihoz.
...
Miután az első lépésben beszerzeni a szolgáltatásnévteret és a hitelesítő adatokat, egy alapszintű Visual Studio-konzolalkalmazást kell létrehoznia.
...
1. A Program.cs fájlban győződjön meg róla, hogy a(z) `using` utasítás a következőképpen jelenik meg:
...
1. Ha szükséges, nevezze át a program névterét a Visual Studio alapértelmezett értékéről a következőre: `Microsoft.ServiceBus.Samples`.

1. A(z) `Program` osztályon belül adja hozzá a következő globális változókat:
...
1. A(z) `Main()` területre illessze be a következő kódot:
...
1. A közvetítőalapú üzenet standard tulajdonságai egy `BrokerProperties` HTTP-fejlécben vannak elhelyezve. A közvetítő tulajdonságait JSON formátumban kell szerializálni. Ahhoz, hogy a program 30 másodperces **TimeToLive** értéket határozzon meg, és hozzáadja az „M1” címkét az üzenethez, vegye fel a következő kódot közvetlenül az előző példában szereplő `webClient.UploadData()` hívás elé:
...
1. Az egyéni üzenettulajdonságok kulcs-érték párok készleteként definiálhatók. Minden egyéni tulajdonságot külön TPPT-fejléc tárol. A „Prioritás” és „Ügyfél” egyéni tulajdonságok felvételéhez, vegye fel a következő kódot közvetlenül az előző példában szereplő `webClient.UploadData()` hívás elé:
...
Most már lefordíthatja és futtathatja az alkalmazást. A Visual Studio **Fordítás** kattintson a **Megoldás fordítása** elemre, vagy nyomja le az F6 billentyűt.
...
- [Azure Service Bus fundamentals (Az Azure Service Bus alapjai)](service-bus-fundamentals-hybrid-solutions.md)
- [Service Bus relay REST tutorial (A Service Bus Relay REST-oktatóanyaga)](service-bus-relay-rest-tutorial.md)
