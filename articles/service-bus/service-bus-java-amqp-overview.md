...
*    **有效率**: AMQP 1.0 是連線導向的通訊協定，可使用二進位編碼通訊協定指示和商務訊息傳輸。 其中包含精密的流程控制配置，可以將網路與連接的元件使用率最大化。 也就是說，此通訊協定的設計目的是在效率、彈性及交互操作性之間取得平衡。
*    **可靠**: AMQP 1.0 通訊協定可讓與某個範圍的可靠性保證，從射後不理變成可靠且完全交換的訊息-一次認可傳遞。
*    **彈性**: AMQP 1.0 是一種彈性的通訊協定，可用來支援不同的拓撲。 您可以使用相同的通訊協定，來進行用戶端對用戶端、用戶端對代理人的通訊，以及代理人對代理人的通訊。
*    **代理人模型無關**: AMQP 1.0 規格不會在代理人使用的訊息模型上強制執行任何需求。 這表示您能夠將 AMQP 1.0 支援新增至現有的訊息代理人。
...
*    **技術廠商**: Axway Software，華為技術，IIT 軟體，INETCO Systems、 Kaazing、 Microsoft、 Mitre Corporation、 Primeton Technologies，進度軟體，Red Hat，SITA、 Software AG、 Solace Systems、 VMware、 WSO2、 Zenika。
...
如需如何取得及使用這些程式庫與服務匯流排的詳細資訊，請參閱 [服務匯流排 AMQP 開發人員指南][]。 請參閱 [後續步驟](service-bus-java-amqp-overview.md#next-steps) 區段，如需進一步資訊的連結。
