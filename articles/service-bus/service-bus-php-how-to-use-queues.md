...
本指南將說明如何使用服務匯流排佇列。 在 PHP 中並使用這些範例 [Azure SDK for PHP](../php-download-sdk.md)。 本文說明的案例包括**建立佇列**、**傳送並接收訊息**，以及**刪除佇列**。
...
在下面 `require_once` 陳述式一律會顯示，但只讓範例執行所需的類別所參考。
...
若要傳送訊息至服務匯流排佇列，應用程式會呼叫 **ServiceBusRestProxy->sendQueueMessage** 方法。 下列程式碼示範如何傳送訊息至 `myqueue` 先前中建立佇列
