...
## <a name="problem-i-cannot-access-my-website-but-my-deployment-is-started-and-all-role-instances-are-ready"></a>Probléma: Nem érhetők el a webhelyem, azonban az üzembe helyezés elindul, és készen áll az összes szerepkörpéldány
...
## <a name="problem-my-role-instances-recycled-without-me-doing-anything"></a>Probléma: Saját szerepkör példányai újrahasznosított me csinál semmit nélkül
...
További információkért tekintse meg a következő blogbejegyzésben: [szerepkör példány újraindítása miatt az operációs rendszer frissítése](https://blogs.msdn.com/b/kwill/archive/2012/09/19/role-instance-restarts-due-to-os-upgrades.aspx)

## <a name="problem-i-cannot-do-a-vip-swap-and-receive-an-error"></a>Probléma: Nem lehet egy virtuális IP-címcsere tegye és egy hibaüzenetet
...
## <a name="problem-a-role-instance-is-looping-between-started-initializing-busy-and-stopped"></a>Probléma: Egy szerepkörpéldány a hurkolás közötti elindítva, inicializálás, foglalt és leállítva
...
A probléma elhárítására vonatkozó további információkért tekintse meg a következő blogbejegyzésben: [Azure PaaS számítási diagnosztikai adatok](https://blogs.msdn.com/b/kwill/archive/2013/08/09/windows-azure-paas-compute-diagnostics-data.aspx) és [szerepkör-újrahasznosítást kiváltó gyakori problémák](cloud-services-troubleshoot-common-issues-which-cause-roles-recycle.md).
...
2. Az a **tulajdonságok** panel az Azure Portal, fontolja meg a probléma megoldása érdekében a következő feltételeknek:
   ...
   * Ha a szerepkör-példány **foglalt**, ellenőrizze az alkalmazás kódjában annak ellenőrzéséhez, hogy a [StatusCheck](/previous-versions/azure/reference/ee758135(v=azure.100)) esemény történik. Előfordulhat, hogy szeretne hozzáadni vagy javítani egy kódrészletet, amely kezeli ezt az eseményt.
   * Haladjon végig a diagnosztikai adatok és az ebben a blogbejegyzésben hibakeresésekhez [Azure PaaS számítási diagnosztikai adatok](https://blogs.msdn.com/b/kwill/archive/2013/08/09/windows-azure-paas-compute-diagnostics-data.aspx).
