...
[RoleEntryPoint] 類別，其中包含 [OnStart]，方法會擲回任何例外狀況 [OnStop]，[執行]，會發生未處理的例外狀況。 如果其中有一個方法發生未處理的例外狀況，角色就會進行回收。 如果角色重複回收，它可能會在每次嘗試啟動時擲回未處理的例外狀況。


## 角色因 Run 方法而回收

[執行] 方法，被要無限期地執行。 如果您的程式碼會覆寫 [Run] 方法，它應該要無限期地進入睡眠。 [執行] 方法傳回時，如果在角色回收。
...
如果應用程式使用 Azure 診斷，則必須指定服務組態檔 `DiagnosticsConnectionString` 組態設定。 此設定應指定您在 Azure 中的儲存體帳戶的 HTTPS 連線。

為了確保您 `DiagnosticsConnectionString` 應用程式封裝部署到 Azure 之前，設定是否正確，請確認下列:

- `DiagnosticsConnectionString` 點設為 Azure 中的有效儲存體帳戶。  
  根據預設，此設定會指向模擬儲存體帳戶，因此您必須在部署應用程式封裝之前明確變更這項設定。 若未變更此設定，當角色執行個體嘗試啟動診斷監視器時，將會擲回例外狀況。 這可能會導致角色執行個體無限期地回收。

- 在下列指定的連接字串 [格式](storage-configure-connection-string.md) (通訊協定必須指定為 HTTPS)。 以您的儲存體帳戶名稱取代 *MyAccountName*，並以您的存取金鑰取代 *MyAccountKey*：
