...
2. 在 **Team Explorer**, ，選擇 [ **複製這個儲存機制** 連結。
...
1. 在 **Team Explorer**, 中 **解決方案** 區段中，選擇 **新增** 連結本機儲存機制中建立新的專案。
...
如果您想要建立 web 應用程式，選擇 [ **ASP.NET Web 應用程式** 專案範本，然後選擇 **MVC**。 請參閱 [Azure App Service 中建立 ASP.NET web 應用程式](../web-sites-dotnet-get-started.md) 如需詳細資訊。

3. 開啟方案的捷徑功能表，然後選擇 **認可**。

    ![][7]

4. 如果是第一次在 Visual Studio Team Services 中使用 Git，您需要提供一些資訊在 Git 中識別您的身分。 在 **暫止的變更** 區域 **Team Explorer**, 、 輸入您的使用者名稱和電子郵件地址。 輸入認可的註解，然後選擇 **認可** ] 按鈕。

    ![][8]

5. 請注意簽入時用來包含或排除特定變更的選項。 如果已排除所需的變更，請選擇 **全部包含**。

6. 您現在已在儲存機制的本機複本中認可變更。 接下來，選擇同步這些變更與伺服器 **同步** 連結。

## 步驟 3：將專案連接至 Azure

1. 現在，您在 Visual Studio Team Services 中有一個 Git 儲存機制，裡面還有一些原始程式碼，您可以準備將 Git 儲存機制連接至 Azure。  在 [Azure 傳統入口網站](http://manage.windowsazure.com), ，選取 [雲端服務或 web 應用程式，或建立一個新的選擇 + 底部離開，且選擇圖示 **定域機組服務** 或 **Web 應用程式** 然後 **快速建立**。

    ![][9]

3. 對於雲端服務，選擇 [ **設定發行與 Visual Studio Team Services** 連結。 針對 web 應用程式中，選擇 **設定從原始檔控制進行部署** 連結。

    ![][10]

2. 在精靈中，在文字方塊中輸入 Visual Studio Team 服務帳戶的名稱，然後選擇 [ **立即授權** 連結。 可能會要求您登入。

    ![][11]

3. 在 **連線要求** 快顯對話方塊中，選擇 [ **接受** ，授權 Azure 在 Visual Studio Team Services 中設定您的 team 專案。
...
3. 在 **方案總管] 中**, ，開啟方案節點、 專案節點或您變更，並選擇 [檔案的捷徑功能表 **認可**。
...
6. 選擇 **推播** 連結，將認可推送至 Visual Studio Team Services 中的儲存機制。 (您也可以使用 **同步處理** ] 按鈕，將認可複製到儲存機制。 其差異在於， **同步** 也會從儲存機制中提取最新的變更。)

    ![][39]

7. 選擇 **家用** ] 按鈕回到 **Team Explorer** 首頁。
...
11. 在 **觸發程序** 索引標籤上，您會看到組建定義來建立的每個簽入，設定預設。 (若是雲端服務，Visual Studio Team Services 會自動建置主要分支並部署至預備環境。 您仍然必須執行手動步驟來部署至即時網站。 對於沒有預備環境的 Web 應用程式，它會將主要分支直接部署到即時網站。

    ![][26]

1. 在 **程序** 索引標籤上，您可以看到部署環境已設為雲端服務或 web 應用程式的名稱。
...
1.  瀏覽至網站的 URL。 若是 web 應用程式，請選擇 **瀏覽** 入口網站中的按鈕。 雲端服務中，選擇 [在 URL **快速概覽** 區段 **儀表板** 頁面，以顯示預備環境。

    依預設，來自雲端服務連續整合的部署會發行至預備環境。 您可以變更此設定 **替代雲端服務環境** 屬性 **生產**。 以下是網站 URL 在雲端服務儀表板頁面上的位置。
...
此為選用步驟。 在 Azure 傳統入口網站中，選擇 [先前的部署，然後選擇 [ **重新部署** 倒轉舊版簽入您的網站。 請注意，這會在 TFS 中觸發新的組建，並在部署歷程記錄中建立新的項目。

![][34]

## 步驟 6：變更生產部署

準備好時，您可以選擇將升級預備環境到生產環境 **交換** Azure 傳統入口網站中。 新部署的預備環境會升級至「生產」，而先前的生產環境 (若有的話) 會變成預備環境。 「作用中」部署可能與生產和預備環境不同，但最近組建的部署歷程記錄都一樣，與環境無關。
...
1. 在 **Team Explorer**, ，選擇 [ **首頁** 按鈕，然後選擇 **分支** ] 按鈕。
...
3. 輸入分支，例如"working"的名稱，然後選擇 **建立分支**。 這樣會建立新的本機分支。

    ![][42]

4. 發佈分支。 選擇分支名稱在 **取消發行分支**, ，然後選擇 [ **發行**。

    ![][44]

6. 依預設，只有主要分支的變更才會觸發連續組建。 若要設定工作分支的連續組建，請選擇 [ **組建** 頁面 **Team Explorer**, ，然後選擇 [ **編輯組建定義**。

7. 開啟 **來源設定** ] 索引標籤。 在 **監視連續整合和組建的分支**, ，選擇 [ **按一下此處以加入新的資料列**。
...
9. 變更程式碼中，開啟已變更之檔案的捷徑功能表，再選擇 **認可**。
