...
- 您必須先將自己建立的虛擬機器新增到可用性設定組，才能調整使用這些虛擬機器的應用程式。 您所新增的虛擬機器可以一開始處於開啟或關閉狀態，但是它們將在向上調整動作中開啟，並在向下調整動作中關閉。 如需虛擬機器和可用性設定組的詳細資訊，請參閱 [管理虛擬機器的可用性](../virtual-machines-manage-availability.md)。

- 調整受到核心使用量影響。 較大型的角色執行個體或虛擬機器會使用較多的核心。 您只能在訂閱的核心限制內調整應用程式。 例如，如果您的訂閱設有 20 個核心的限制，而您使用兩個中型大小的虛擬機器來執行應用程式 (總計 4 個核心)，則您只能在訂閱中最多增加 16 個核心的其他雲端服務部署。 可用性設定組中所有用於調整應用程式的虛擬機器都必須具有相同大小。 如需核心使用量和電腦大小的詳細資訊，請參閱 [虛擬機器和雲端服務大小](http://msdn.microsoft.com/library/dn197896.aspx)。
...
- [平均 CPU 使用量](#averagecpu) -如果平均 CPU 使用量百分比高於或低於指定的閾值，會建立或刪除，角色執行個體或虛擬機器就會開啟或關閉在可用性設定組中。
- [佇列訊息](#queuemessages) -如果佇列中的訊息數目高於或低於指定的閾值，會建立或刪除，角色執行個體或虛擬機器就會開啟或關閉在可用性設定組中。
