...
憑證可用於在 Azure 中雲端服務 ([服務憑證](#what-are-service-certificates)) 和管理 API 與驗證 ([管理憑證](#what-are-management-certificates))。 本主題提供的一般概觀，這兩種憑證類型，如何以 [建立](#create) ，以及如何 [部署](#deploy) 到 Azure。
...
有兩種方法來建立在 Windows 中，憑證與 `makecert.exe` 公用程式或 IIS。
