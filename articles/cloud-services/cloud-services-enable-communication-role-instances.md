...
雲端服務角色透過內部和外部連線通訊。 外部連線稱為「輸入端點」****，而內部連線稱為「內部端點」****。 本主題說明如何修改 [服務定義](cloud-services-model-and-package.md#csdef) 建立端點。
