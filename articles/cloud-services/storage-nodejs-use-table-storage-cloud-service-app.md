...
使用 Microsoft 的 [Node.js Web 應用程式使用 Express] 教學課程
...
透過 REST Api。 如需詳細資訊，請參閱 [儲存和存取 Azure 中的資料]。

本教學課程假設您已完成 [Node.js Web
應用程式] 和 [Express][node.js web application using express] 教學課程。
...
接著再由 Azure SDK 讀取。
> [AZURE.NOTE] 只有當將應用程式部署到 Azure 時，
...
**views** 目錄中的 **layout.jade** 檔是用來作為其他 **.jade** 檔案的全域範本。 在此步驟中您將修改它以使用 [Twitter Bootstrap](https://github.com/twbs/bootstrap), ，工具組能夠方便設計美觀的網站的。
...
![顯示 「 我的工作清單 」 頁面的瀏覽器視窗。 URL 會指出頁面現在裝載在 Azure 上。](。 / media/storage-nodejs-use-table-storage-cloud-service-app/getting-started-1.png)
