...
---|---
 0| 成功。
 -1| 一般錯誤。
 -2| 無法載入 rcf 檔。<p>這是內部錯誤，如果客體代理程式外掛程式啟動器手動叫用時，不正確地在 VM 上只會發生。
 -3| 無法載入診斷組態檔。<p><p>方案: 這是組態檔未通過結構描述驗證結果。解決方法是提供以結構描述編譯的組態檔。
 -4| 診斷監視代理程式的另一個執行個體已在使用本機資源目錄。<p><p>方案: 指定不同的值為 **LocalResourceDirectory**。
 -6| 客體代理程式外掛程式啟動器嘗試使用無效的命令列來啟動診斷。<p><p>這是內部錯誤，如果客體代理程式外掛程式啟動器手動叫用時，不正確地在 VM 上只會發生。
 -10| 診斷外掛程式結束並發生未處理的例外狀況。
 -11| 客體代理程式無法建立負責啟動及監視監視代理程式處理序。<p><p>解決方案: 確認有足夠的系統資源，可啟動新處理序。<p>
 -101| 呼叫診斷外掛程式時的參數不正確。<p><p>這是內部錯誤，如果客體代理程式外掛程式啟動器手動叫用時，不正確地在 VM 上只會發生。
 -102| 外掛程式處理序無法對本身進行初始化。<p><p>解決方案: 確認有足夠的系統資源，可啟動新處理序。
 -103| 外掛程式處理序無法對本身進行初始化。尤其，它是無法建立記錄器物件。<p><p>解決方案: 確認有足夠的系統資源，可啟動新處理序。
 -104| 無法載入客體代理程式所提供的 rcf 檔。<p><p>這是內部錯誤，如果客體代理程式外掛程式啟動器手動叫用時，不正確地在 VM 上只會發生。
 -105| 診斷外掛程式無法開啟診斷組態檔。<p><p>這是內部錯誤，如果以手動方式叫用診斷外掛程式，不正確地在 VM 上只會發生。
 -106| 無法讀取診斷組態檔。<p><p>方案: 這是組態檔未通過結構描述驗證結果。因此，解決方法是提供以結構描述編譯的組態檔。您可以在 VM 上的 *%SystemDrive%\WindowsAzure\Config* 資料夾中，找到傳送給診斷擴充功能的 XML。開啟適當的 XML 檔並搜尋 **Microsoft.Azure.Diagnostics**，然後搜尋 [xmlCfg]**** 欄位。資料是以 base64 編碼，因此您將需要 [解碼](http://www.bing.com/search?q=base64+decoder) 能查看診斷載入的 XML。<p>
 -107| 若要監視的代理程式傳遞的資源目錄無效。<p><p>這是內部錯誤，如果監視代理程式以手動方式叫用時，不正確地在 VM 上只會發生。</p>
 -108| 無法將診斷組態檔轉換成監視代理程式組態檔。<p><p>這是內部錯誤，如果使用無效的組態檔以手動方式叫用診斷外掛程式只會發生。
 -110| 一般診斷組態錯誤。<p><p>這是內部錯誤，如果使用無效的組態檔以手動方式叫用診斷外掛程式只會發生。
