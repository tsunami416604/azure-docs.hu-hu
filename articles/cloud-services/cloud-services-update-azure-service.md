...
-   [允許在更新期間的服務變更](#AllowedChanges)
-   [更新回復](#RollbackofanUpdate)
-   [初始化多個變更的作業上持續進行的部署](#multiplemutatingoperations)
-   [散發到升級網域的角色](#distributiondfroles)
-   [升級如何繼續](#howanupgradeproceeds)
...
> [AZURE.NOTE] 將服務從單一執行個體升級為多個執行個體時，由於 Azure 升級服務的方式，您的服務將會在執行升級時關閉。 保證服務可用性的服務等級協定僅適用於已部署多個執行個體的服務。 下列清單說明每個 Azure 服務升級案例如何影響每個磁碟機上的資料：
>
>VM 重新啟動：
>
-   C: 已保留
...
Azure 讓您在 Azure 網狀架構控制器接受初始更新要求後，於服務上起始其他作業，以提供在更新期間管理服務的彈性。 只有當部署上的更新 (組態變更) 或升級處於**進行中**狀態時，才能執行復原。 只要服務有至少一個執行個體尚未更新為新版本，更新或升級就會被視為進行中。 若要測試是否允許回復，請檢查 RollbackAllowed 旗標，所傳回的值 [取得部署](https://msdn.microsoft.com/library/azure/ee460804.aspx) 和 [取得雲端服務屬性](https://msdn.microsoft.com/library/azure/ee460806.aspx) 作業，會設定為 true。
...
-   任何角色執行個體已被更新或升級到新版的服務封裝 (\*.cspkg) 檔案或服務組態 (\*.cscfg) 檔案 (或兩個檔案) 會還原這些檔案的升級前的版本。
