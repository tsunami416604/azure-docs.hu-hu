...
當您建立雲端服務時，Azure 會指派它的子網域 **cloudapp.net**。 比方說，如果您的雲端服務名稱為 「 contoso 」，您的使用者將能夠存取您的應用程式的 url，例如 http://contoso.cloudapp.net。 Azure 也會指派虛擬 IP 位址。

不過，您也可以公開您的應用程式在您自己的網域名稱，例如 **contoso.com**。 本文說明如何保留或設定雲端服務 Web 角色的自訂網域名稱。
...
1. 使用其中一種方法來尋找 **。 cloudapp.net** 指派給您的雲端服務的網域名稱。
    ...
    * 登入 [Azure portal], ，選取您的雲端服務，查看 **Essentials** 區段，然後尋找 **網站 URL** 項目。
...
1. 使用下列其中一種方法取得雲端服務的 IP 位址。
    ...
    * 登入 [Azure portal], ，選取您的雲端服務，查看 **Essentials** 區段，然後尋找 **的公用 IP 位址** 項目。
