...
若要檢視快取度量 [瀏覽](cache-configure.md) 到快取執行個體中 [Azure 入口網站](https://portal.azure.com)。 您可以在 [Redis 快取]**** 分頁上存取 Azure Redis 快取執行個體的度量。
...
若要檢視預存的度量，請檢查 [啟動儲存體帳戶名稱中的資料表 `WADMetrics`。 如需存取 Azure 入口網站外部預存的度量的詳細資訊，請參閱 [存取 Redis 快取監視資料](https://github.com/rustd/RedisSamples/tree/master/CustomMonitoring) 範例。
...
每個度量都包含兩個版本。 一個度量會測量整個快取，以及使用的快取效能 [叢集](cache-how-to-premium-clustering.md), ，度量資訊會包含第二個版本 `(分區 0-9)` 名稱測量效能的快取中的單一分區。 例如，如果快取有 4 個分區， `快取點擊` 是針對整個快取中的總數量和 `快取命中 (分區 3)` 是只針對該分區的快取叫用。
>[AZURE.NOTE] 即使快取閒置而沒有連線的作用中用戶端應用程式，您可能還是會看到某個快取活動 (例如連線的用戶端、記憶體使用量，以及正在執行的作業)。 在 Azure Redis 快取執行個體的作業期間，此活動正常。

| 度量| 說明|
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 快取點擊| 所指定報告間隔期間的成功金鑰查閱數目。這個值會對應至 Redis INFO `keyspace_hits 命令`。|
| 快取遺漏| 所指定報告間隔期間的失敗金鑰查閱數目。這個值會對應至 Redis INFO `keyspace_misses` 命令。快取遺漏不一定表示快取發生問題。例如，使用另行快取程式設計模式時，應用程式會先在快取中尋找項目。如果項目不存在 (快取遺漏)，項目會從資料庫中擷取，並在下次新增至快取中。快取遺漏是另行快取程式設計模式的正常行為。如果快取遺漏數目高於預期，請檢查可填入且讀取自快取的應用程式邏輯。如果快取，因為記憶體壓力而正在收回項目，則可能會有一些快取遺漏，但會是更好的度量資訊，來監視記憶體壓力 `Used Memory` 或 `收回金鑰`。|
...
| 收回的金鑰| 由於指定報告間隔期間從快取收回的項目數目 `maxmemory` 限制。這個值會對應至 Redis INFO `evicted_keys` 命令。|
| 到期的金鑰| 所指定報告間隔期間的快取到期項目數。這個值會對應至 Redis INFO `expired_keys` 命令。|
| 取得| 所指定報告間隔期間的快取 get 作業數目。這個值是 Redis INFO all 命令為下列的總和值: `cmdstat_get`, ，`cmdstat_hget`, ，`cmdstat_hgetall`, ，`cmdstat_hmget`, ，`cmdstat_mget`, ，`cmdstat_getbit`, ，和 `cmdstat_getrange`, ，而且報告間隔期間相當於快取點擊和遺漏的總和。|
...
| 設定| 所指定報告間隔期間的快取 set 作業數目。這個值是 Redis INFO all 命令為下列的總和值: `cmdstat_set`, ，`cmdstat_hset`, ，`cmdstat_hmset`, ，`cmdstat_hsetnx`, ，`cmdstat_lset`, ，`cmdstat_mset`, ，`cmdstat_msetnx`, ，`cmdstat_setbit`, ，`cmdstat_setex`, ，`cmdstat_setrange`, ，和 `cmdstat_setnx`。|
| 總作業數| 所指定報告間隔期間，快取伺服器所處理命令的總數。這個值會對應至 Redis INFO `total_commands_processed` 命令。請注意，Azure Redis 快取純粹用於發佈/訂閱時將會有任何的度量 `快取點擊`, ，`快取區遺漏`, ，`取得`, ，或 `集`, ，但是不會有 `作業總數` 度量可反映發佈/訂閱作業的快取使用量。|
...
| CPU| 所指定報告間隔期間的 Azure Redis 快取伺服器 CPU 使用率 (百分比)。這個值會對應至作業系統 `\Processor (_Total) \ %處理器時間` 效能計數器。|
...
如果您按一下 [儲存]****，您的變更將會保留到您退出 [度量]**** 分頁為止。 當您後續返回頁面時，您將會看見原有的度量和時間範圍。 如需有關自訂圖表的詳細資訊，請參閱 [監視服務計量](../azure-portal/insights-how-to-customize-monitoring.md)。
...
進階版快取具有 [叢集](cache-how-to-premium-clustering.md) 啟用可以有多達 10 個分區。 每個分區都有它自己的度量，而且這些度量會彙總提供整個度量供快取。 每個度量都包含兩個版本。 一個度量會測量整個快取和度量資訊會包含第二個版本的效能 `(分區 0-9)` 名稱測量效能的快取中的單一分區。 例如，如果快取有 3 個分區， `快取點擊` 是針對整個快取中的總數量和 `快取命中 (分區 2)` 是只針對該分區的快取叫用。
