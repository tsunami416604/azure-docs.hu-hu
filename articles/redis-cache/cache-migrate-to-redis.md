...
移轉作業的第一步是挑選符合先前受管理的快取服務快取大小的容量，然後再依據應用程式的需求擴大或縮小規模。 如需選擇正確的 Azure Redis 快取提供項目詳細指引，請參閱 [哪些 Redis 快取提供項目和大小應該使用?](cache-faq.md#what-redis-cache-offering-and-size-should-i-use).
...
若要解除安裝受管理的快取服務 NuGet 封裝，請在 [方案總管]**** 中的用戶端專案上按一下滑鼠右鍵，然後選擇 [管理 NuGet 封裝]****。 選取 **已安裝的套件** 節點，然後輸入 W**indowsAzure.Caching** 搜尋已安裝的封裝中。 選取 [Microsoft Azure 快取 (Microsoft Azure Cache)]******** (或 [Microsoft Azure 快取 (Microsoft Azure Caching)]********，視 NuGet 封裝的版本而定)，按一下 [解除安裝]****，然後按一下 [關閉]****。
...
請確認 `dataCacheClients` 項目已移除從 `configSections` 項目。 請勿移除整個 `configSections` 項目，只需移除 `dataCacheClients` 項目，如果有的話。
...
請確認 `dataCacheClients` 就會移除區段。  `DataCacheClients` 區段必須與下列範例類似。
...
在受管理快取服務中，連線至快取已由 `DataCacheFactory` 和 `DataCache` 類別。 Azure Redis 快取中，這些連線由 `ConnectionMultiplexer` 類別。
...
若要連接到 Azure Redis 快取執行個體，呼叫靜態 `ConnectionMultiplexer.Connect` 方法並傳入端點和金鑰。 其中一種方式共用 `ConnectionMultiplexer` 應用程式中的執行個體是靜態屬性傳回已連接的執行個體，類似下列的範例。 這會提供執行緒安全的方式，來初始化只有一個連線 `ConnectionMultiplexer` 執行個體。 在此範例中 `abortConnect` 設為 false，表示將繼續呼叫，即使未建立快取的連線。 主要功能之一 `ConnectionMultiplexer` 的原因是它會自動還原連線至快取一次網路問題或其他原因，在解決。
...
StackExchange.Redis 用戶端使用 `RedisKey` 和 `RedisValue` 存取並儲存在快取中的項目類型。 這些型別會對應到最基本的語言型別 (包括字串)，但通常不會直接使用。 Redis 字串是最基本的 Redis 值，而且只能包含許多類型的資料，包括序列化的二進位資料流和當您不可能直接使用型別，會使用包含方法 `字串` 名稱中。 最基本的資料類型的儲存和擷取項目從快取使用 `StringSet` 和 `StringGet` 方法，除非您儲存了集合或其他 Redis 資料類型在快取。

`StringSet` 和 `StringGet` 非常類似於受管理的快取服務 `放` 和 `取得` 方法，最主要的不同點在於您設定並取得 a.NET 物件至快取之前您必須先序列化。

當呼叫 `StringGet`, ，如果物件存在，則會傳回，如果不存在，則傳回 null。 在此情況下，您可以從需要的資料來源中擷取值，並將它儲存在快取中供後續使用。 這稱為另行快取模式。
