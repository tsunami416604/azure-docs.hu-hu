...
| Elévülési szabályzatának |Az alapértelmezett lejárati házirendet abszolút, és az alapértelmezett lejárati időszak 10 perc. A késleltetett és Never házirendek is elérhetők. |Alapértelmezés szerint a gyorsítótárban elemek nem jár le, de egy lejárati gyorsítótár set túlterhelés használatával / írási szinten konfigurálható. |
...
A gyorsítótár létrehozása és konfigurálása után a következő lépéssel távolítsa el a Managed Cache Service-konfigurációt, és adja hozzá az Azure Redis Cache konfigurációs és, hogy a gyorsítótár-ügyfelek hozzáférhetnek a gyorsítótár hivatkozik.
...
Ha ehhez a névtérhez nem oldja meg, ne feledje, hogy hozzáadta a StackExchange.Redis NuGet-csomagot a [gyors üzembe helyezés: használata Azure Redis Cache a .NET-alkalmazás](cache-dotnet-how-to-use-azure-redis-cache.md).
...
Csatlakozás az Azure Redis Cache példányt, hívja meg a statikus `ConnectionMultiplexer.Connect` metódus és a végpont és a kulcsot adjon át. Az alkalmazásban egy `ConnectionMultiplexer` példány megosztására egy lehetséges módszer, ha létrehoz egy statikus tulajdonságot, amely egy csatlakoztatott példányt ad vissza, a következő példához hasonlóan. Ezt a módszert biztosít egy szálbiztos módon csatlakoztatott egyetlen inicializálása `ConnectionMultiplexer` példány. Ebben a példában `abortConnect` értéke HAMIS, ami azt jelenti, hogy a hívás sikeres lesz, még akkor is, ha nem jön létre a kapcsolat a gyorsítótárba. A `ConnectionMultiplexer` egyik fontos szolgáltatása, hogy automatikusan visszaállítja a kapcsolatot a gyorsítótárral, amint a hálózati problémák vagy egyéb hibák elhárulnak.
...
`StringSet` és `StringGet` hasonlóak a Managed Cache Service `Put` és `Get` módszerek, az egyik fő különbség, hogy állítsa be, és a gyorsítótárba egy .NET-objektum beolvasása előtt meg kell (MUST) azt először. 

Meghívásakor `StringGet`, ha az objektum létezik, akkor adja vissza, és ha nem, null ad vissza. Ebben az esetben beolvassa a kívánt adatforrás és a későbbi használatra a gyorsítótárban tárolja. Ebben a mintában a gyorsítótár-tartalékoljon mintát néven ismert.
