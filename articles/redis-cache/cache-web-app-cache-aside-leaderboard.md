---
title: Oktatóanyag webalkalmazás létrehozásához a gyorsítótár-feltöltési mintát használó Redis Cache használatával | Microsoft Docs
description: Megtudhatja, hogyan hozhat létre webalkalmazást a gyorsítótár-feltöltési mintát használó Redis Cache használatával
...
Ebben az oktatóanyagban az Azure Redis Cache segítségével frissíteni fogja az [ASP.NET gyors útmutató az Azure Redis Cache-hez](cache-web-app-howto.md) című rövid útmutatóban létrehozott, *ContosoTeamStats* nevű, ASP.NET-alapú webalkalmazást, hogy tartalmazza a [gyorsítótár-feltöltési mintát](https://docs.microsoft.com/azure/architecture/patterns/cache-aside) használó ranglistát. Ez a mintaalkalmazás a csoportstatisztikák adatbázisból származó listáját jeleníti meg, illetve különböző módokat mutat be, amelyekkel az Azure Redis Cache használható a gyorsítótár adatainak tárolására és beolvasására a teljesítmény javítása érdekében. Az oktatóanyag befejezését követően egy olyan futó webalkalmazással fog rendelkezni, amely adatokat olvas be és ír egy adatbázisba, az Azure Redis Cache használatával lett optimalizálva, és az Azure-ban van üzemeltetve.

Eben az oktatóanyagban az alábbiakkal fog megismerkedni:

> [!div class="checklist"]
> * Az adatátviteli sebesség növelése és az adatbázis-terhelés csökkentése az Azure Redis Cache használatával történő adattárolás és -beolvasás révén.
...
* Ez az oktatóanyag az [ASP.NET gyors útmutató az Azure Redis Cache-hez](cache-web-app-howto.md) című rövid útmutató folytatása. Ha még nem tette meg, kövesse a rövid útmutató utasításait.
...
1. A Visual Studióban nyissa meg a *ContosoTeamStats* megoldást, amelyet az [ASP.NET rövid útmutató az Azure Redis Cache-hez](cache-web-app-howto.md) című rövid útmutatóban hozott létre.
...
1. A `body` szakaszban adja hozzá a *Contoso Team Stats* következő új `Html.ActionLink` utasítását közvetlenül az *Azure Redis Cache Test* hivatkozása alatt.
...
## <a name="configure-the-app-for-redis-cache"></a>Az alkalmazás konfigurálása a Redis Cache-hez
...
Ebben a tesztben az alkalmazás és az adatbázis is helyben fut. A Redis Cache ezzel szemben az Azure-ban, távolról fut. Ezért valószínű, hogy a gyorsítótár teljesítménye az adatbázisénál kisebb lesz. A legjobb teljesítmény érdekében az ügyfélalkalmazásnak és az Azure Redis Cache-példánynak azonos helyen kell lennie. A következő szakaszban a gyorsítótár használatával járó nagyobb teljesítmény kipróbálása érdekében minden erőforrást az Azure-ban fog üzembe helyezni.
...
1. Ha az új adatbázis létrejött, kattintson az **Adatbázis kapcsolati karakterláncainak megjelenítése** elemre, és másolja az **ADO.NET** kapcsolati karakterláncot.
    ...
    ![Kapcsolati karakterláncok megjelenítése](./media/cache-web-app-cache-aside-leaderboard/cache-show-connection-strings.png)
...
> [Az Azure Redis Cache skálázása](./cache-how-to-scale.md)