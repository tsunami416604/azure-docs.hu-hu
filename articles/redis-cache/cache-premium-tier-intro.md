...
## 效能優於標準或基本層。
**較高的效能標準或基本層。**高階層中的快取是部署在擁有較快處理器的硬體上，因此效能優於基本或標準層。 高階層快取的輸送量較高，延遲較低。 

**在高階相較於標準層級較高者為相同大小的快取的輸送量。**例如 為 53 GB 快取中，第 4 頁的輸送量 (Premium) 是 250 K 每秒要求數相較於為 C6 150 K (標準)。
...
**Redis 叢集可讓您最大小數位數和輸送量。**當您增加叢集中的分區 (節點) 數目時，輸送量會呈線性增加。 例如 如果您建立 10 個分區的 P4 叢集則可以使用輸送量 250k 個 * 10 = 2.5 百萬個每秒要求數。 請參閱 [Azure Redis 快取常見問題集](cache-faq.md#what-redis-cache-offering-and-size-should-i-use) 如需詳細資訊大小、 輸送量和進階版快取使用的頻寬。
