...
可從設定持續性 **新增 Redis 快取** 刀鋒視窗中建立快取期間和 **設定** 刀鋒視窗中現有的進階版快取。
...
下節的步驟說明如何在您的新高階快取上設定 Redis 永續性。 Redis 持續性設定之後，按一下 [ **建立** Redis 持續性以建立新的高階快取。

## 設定 Redis 永續性

Redis 持續性在設定 **Redis 資料持續性** 刀鋒視窗。 若為新的快取，則在快取建立程序期間存取此刀鋒視窗，如上節所述。 現有的快取， **Redis 資料持續性** 刀鋒視窗存取從 **設定** 快取的刀鋒視窗。
...
按一下 [ **儲存體帳戶** 選取儲存體帳戶使用，然後選擇 [ **主索引鍵** 或 **次要金鑰** 如何從使用 **儲存體金鑰** 下拉式清單。 您必須選擇儲存體帳戶中快取中，相同的區域和 **高階儲存體** 建議的帳戶，因為高階儲存體具有更高的輸送量。 

>[AZURE.IMPORTANT] 如果您持續性的帳戶的儲存體金鑰重新產生時，您必須重新選擇所需的索引鍵，從 **儲存體金鑰** 下拉式清單。
...
是，您可以變更的備份頻率上 **Redis 資料持續性** 刀鋒視窗。 如需指示，請參閱 [設定 Redis 持續性](#configure-redis-persistence)。
