...
1. Cserélje le a fájl tartalmát a következő JSON-ra, és `{your name}` cserélje le `@id` a mezőt és a `EnvironmentalSensor` felületet a **EnvironmentalSensor. Interface. JSON** fájlban használt értékkel. A csatoló AZONOSÍTÓjának egyedinek kell lennie, hogy mentse a felületet az adattárba:
