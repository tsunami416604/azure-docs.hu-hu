...
一旦您已下載的範例程式碼，請開啟 Visual Studio `.sln` 檔案，以便開始使用。 您會注意到方案中有兩個專案: `TaskWebApp` 專案和 `TaskService` 專案。  `TaskWebApp` 是 WPF 
web 應用程式前端使用者進行互動。  `TaskService` 是應用程式的後端 web API 儲存每個使用者的待辦事項清單。

## 5.設定工作服務

當 `TaskService` receieves 要求從 `TaskWebApp`, ，它會檢查有效的存取權杖來驗證的要求。 若要驗證的存取語彙基元 
...
本文將包含詳細資料的保護 `TaskService`。 如果您想要了解 web API 如何安全地驗證使用 Azure AD B2C 要求，請參閱我們
[Web API 的快速入門文章](active-directory-b2c-devquickstarts-api-dotnet.md)。

## 6.設定工作 Web 應用程式

為了讓 `TaskWebApp` 與 Azure AD B2C 通訊，有幾個通用參數，您必須提供。 在 `TaskWebApp` 專案中，開啟
`web.config` 檔案位於專案的根目錄，並中的值取代 `< g s >` 一節。這些值將用於整個 Web 應用程式。
...
另外還有兩個 `[PolicyAuthorize]` 裝飾項目，您要提供您登入原則的名稱。  `[PolicyAuthorize]` 屬性用來叫用特定的
...
每當您的 .NET Web 應用程式需要從 Azure AD 取得存取權杖時，您就可以使用 **Active Directory 驗證程式庫 (ADAL)**。  
您不需要此程序，使用 ADAL 但 ADAL 可讓您更輕鬆為您的許多詳細資料，例如傳送 OAuth
2.0 的驗證訊息，快取和重新整理權杖。

首先，安裝到 ADAL `TaskWebApp` 一次使用封裝管理員主控台的專案:
...
在 `App_Start\Startup.Auth.cs`, ，實作 `OnAuthorizationCodeReceived` 使用 ADAL 的通知處理常式:
...
既然我們已經獲得存取權杖 `TaskService` 後端並將它儲存在 ADAL 的權杖快取中，我們必須實際使用它。
`TasksController` 負責與通訊 `TaskService` API 和傳送 HTTP 要求的 api 來讀取、 建立，
...
您現在已語彙基元，可將它連接至 HTTP GET 要求中 `授權` 標頭來安全地呼叫 `TaskService`:
...
您可以依照相同的確切模式中傳送 POST 和 DELETE 要求 `TaskService`。 只要呼叫 `AuthenticationContext.AcquireTokenSilentAsync(...)`,，
並將產生的權杖附加至要求中 `授權` 標頭。 我們已經實作 `建立` 為您的動作。 請嘗試完成 `刪除` 中的動作
`TasksController.cs` 自己。
...
最後，建置並執行這兩者 `TaskClient` 和 `TaskService`。 註冊或登入應用程式，並為登入的使用者建立工作。 登出後，再以不同使用者重新登入，並為該使用者建立工作。
...
`git 複製-分支完成 https://github.com/AzureADQuickStarts/B2C-WebApp-WebAPI-OpenIDConnect-DotNet.git`
