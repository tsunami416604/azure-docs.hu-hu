---
title: REST API jogcímek cseréjének integrálása a Azure Active Directory B2C felhasználói úton
...
1. Az **új projekt** ablakban válassza  > a **Visual C#**  **web** > **ASP.net Web Application (.NET-keretrendszer)** elemet.
...
1. Nevezze el az `InputClaimsModel`osztályt, majd adja hozzá a következő tulajdonságokat `InputClaimsModel` a osztályhoz:
...
1. Hozzon létre egy új `OutputClaimsModel`modellt, majd adja hozzá a következő tulajdonságokat a `OutputClaimsModel` osztályhoz:
...
1. Hozzon létre még egy `B2CResponseContent`modellt, amellyel eldöntheti a bemeneti érvényesítési hibaüzeneteket. Adja hozzá a következő tulajdonságokat a `B2CResponseContent` osztályhoz, adja meg a hiányzó hivatkozásokat, majd mentse a fájlt:
