...
A Azure Active Directory B2C (Azure AD B2C) számos modern alkalmazás-architektúrán támogatja a hitelesítést. Ezek mindegyike az iparági szabványnak számító [OAuth 2.0](protocols-overview.md) vagy [OpenID Connect](protocols-overview.md) protokollon alapul. Ez a cikk ismerteti a felépíthető alkalmazások típusait, a kívánt nyelvtől és platformtól függetlenül. Emellett az alkalmazások létrehozásának megkezdése előtt is segít megérteni a magas szintű forgatókönyveket.
...
Ebben a folyamatban az alkalmazás [szabályzatokat](user-flow-overview.md) hajt végre, és egy `authorization_code` kap az Azure ad-től, miután a felhasználó befejezte a házirendet. A `authorization_code` az alkalmazásnak az aktuálisan bejelentkezett felhasználó nevében történő meghívására vonatkozó engedélyét jelöli. Az alkalmazás ezután átadhatja a `authorization_code` a háttérben egy `access_token` és egy `refresh_token`.  Az alkalmazás a `access_token` használatával hitelesítheti a háttérbeli webes API-t a HTTP-kérelmekben. Az `refresh_token` alkalmas ezenfelül új `access_token` kérésére is, ha a régi lejárna.
