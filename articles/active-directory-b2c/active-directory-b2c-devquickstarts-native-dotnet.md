...
一旦您已下載的範例程式碼，請開啟 Visual Studio `.sln` 檔案，以便開始使用。 您會注意到方案中有兩個專案: `TaskClient` 專案和 `TaskService` 專案。  `TaskClient` 是 WPF 
與使用者互動的桌面應用程式。  `TaskService` 是應用程式的後端 web API 儲存每個使用者的待辦事項清單。 這兩個 `TaskClient` 和 `TaskService` 會由單一 **應用程式識別碼**
在此情況下，因為它們都包含一個邏輯應用程式。

## 5.設定工作服務

當 `TaskService` receieves 要求從 `TaskClient`, ，它會檢查有效的存取權杖來驗證的要求。 若要驗證的存取語彙基元 
...
[Web API 的快速入門文章](active-directory-b2c-devquickstarts-api-dotnet.md)。

## 6.執行原則

既然 `TaskService` 是我們可以實作準備驗證要求， `TaskClient`。 您的應用程式會傳送 HTTP 驗證要求，與 Azure AD B2C 通訊
...
開啟檔案 `Globals.cs` 並取代每個屬性值。 這個類別用在整個 `TaskClient` 的常用參考值。
...
ADAL 的主要類別是 `AuthenticationContext` -它代表您的應用程式連線，B2C 目錄。 當應用程式啟動時，
...
在所有這些情況下，ADAL 會回復中的語彙基元其 `AuthenticationResult` 或擲回例外狀況。 每當您從 ADAL，取得權杖 
您可以使用 `AuthenticationResult.UserInfo` 更新應用程式，例如 UI 中的使用者資料的物件。 ADAL 也會快取的權杖，供使用 
在應用程式的其他部分。

## 7.呼叫 API

我們已經使用 ADAL 來執行原則並取得權杖。 不過，在許多情況下，您只想要檢查現有快取的權杖，而不執行任何原則。
這種情況是當應用程式會嘗試擷取使用者的待辦事項清單從 `TaskService`。 您可以使用相同 `authContext.AcquireTokenAsync(...)` 方法來執行這項操作，一旦
...
當呼叫 `AcquireTokenAsync(...)` 成功和快取中找到權杖，您可以加入要的語彙基元 `授權` HTTP 要求標頭，讓 `TaskService` 可以驗證
要求讀取使用者的待辦事項清單:
...
最後，建置並執行這兩者 `TaskClient` 和 `TaskService`。 使用電子郵件地址或使用者名稱來註冊應用程式。 登出，再以相同的使用者身分重新登入。 編輯該使用者的設定檔。 登出，再註冊
使用不同的使用者。
...
`git 複製-分支完成 https://github.com/AzureADQuickStarts/B2C-NativeClient-DotNet.git`
