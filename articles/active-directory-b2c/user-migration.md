...
![A UsersData. JSON fájl egy része, amely két felhasználó JSON-blokkját mutatja](./media/user-migration/pre-migration-data-file.png)
...
![A UserMigration. exe parancs kimenetét megjelenítő parancssori ablak](./media/user-migration/pre-migration-demo.png)
...
- A felhasználó bejelentkezési e-mail-címével való lekéréséhez használja a minta alkalmazást:
   ...
   1. A felhasználó adatainak megtekintéséhez nyissa meg a profile. JSON fájlt egy JSON-szerkesztőben.
      ...
      ![A felhasználói profil. JSON fájl megnyitása a Visual Studio Code Editorban](./media/user-migration/pre-migration-get-by-email2.png)
...
A jelszó-visszaállítási házirendre mutató hivatkozás beszerzéséhez kövesse az alábbi lépéseket. Ez az eljárás azt feltételezi, hogy korábban létrehozott egy új jelszó kérése [Egyéni szabályzatot](custom-policy-get-started.md).
...
1. Másolja a **Futtatás most végpont** szövegmezőben látható URL-címet, majd küldje el a felhasználóknak.
    ...
    ![Jelszó-visszaállítási házirend lap a Futtatás most végponttal kiemelve](./media/user-migration/pre-migration-policy-uri.png)
...
Miután meghatározta a REST API technikai profilját, konfigurálja a meglévő `SelfAsserted-LocalAccountSignin-Email` műszaki profilt, hogy a REST API technikai profilt is meghívja, ha felülbírálja a *TrustFrameworkExtensions. XML* fájlon belül:
...
Ezután módosítsa a `LocalAccountSignIn` műszaki profil `Id`át `LocalAccountUserMigration`re.
...
1. Nyissa meg *B2C_1A_signup_signin*, a függő entitás (RP) egyéni házirendjét, majd válassza a **Futtatás most**lehetőséget.
1. Adja meg az áttelepített felhasználók egyikének hitelesítő adatait, majd válassza a **Bejelentkezés**lehetőséget. A REST API a következő hibaüzenetet kell eldobnia:
    ...
    ![Bejelentkezési oldalon a jelszó módosítása hibaüzenet jelenik meg](./media/user-migration/pre-migration-error-message.png)
...
1. Kattintson a **Mentés** gombra
    ...
    ![Diagnosztikai naplók konfigurációs lapja Azure Portal](./media/user-migration/pre-migration-diagnostic-logs.png)
