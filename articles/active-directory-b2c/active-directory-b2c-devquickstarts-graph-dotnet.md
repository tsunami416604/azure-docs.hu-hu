<properties
    pageTitle="Azure AD B2C 預覽: 使用 Graph API |Microsoft Azure"
...
參數與 `ObjectId` 上述。 若要查看所有目錄角色，請嘗試在執行清單 `Get-msolrole`。
...
開啟 `B2CGraphClient\B2CGraphClient.sln` Visual Studio 中的 Visual Studio 方案。 在 `B2CGraphClient` 專案中，開啟檔案 `App.config`。 將
三個應用程式設定為您自己的值，就像這樣:
...
若要使用 B2CGraphClient，開啟 cmd Windows 命令提示字元，切換至 `偵錯` 目錄。 然後執行 `B2C 協助` 命令。
...
B2CGraphClient 執行時，它會建立的執行個體 `B2CGraphClient` 類別。 這個類別的建構函式會設定 ADAL 的驗證架構：
...
讓我們使用 `B2C 取得使用者` 命令做為範例。 當 `取得使用者` 而不需要任何額外的輸入，CLI 呼叫叫用 `B2CGraphClient.GetAllUsers(...)` 方法。
這個方法會呼叫 `B2CGraphClient.SendGraphGetRequest(...)`, ，這將對 Graph API 的 HTTP GET 要求送出。 在傳送之前的 GET 要求，它第一次取得存取權
使用 ADAL 的權杖:
...
如您所見，您可以取得存取權杖 Graph API 的呼叫 ADAL 的 `AuthenticationContext.AcquireToken(...)` 方法。 ADAL 會傳回代表 access_token
應用程式的身分識別。

### 讀取使用者

當您想要從 Graph API 取得的使用者清單，或取得特定的使用者時，您可以傳送 HTTP GET 要求來 `/使用者` 端點。 租用戶中的所有使用者的要求
應該像這樣:
...
- 透過 ADAL 取得存取權杖加入 `授權` 標頭使用 `持有人` 配置。
- B2C 租用戶，您必須使用查詢參數 `api 版本 = beta`。
...
這兩個這些詳細資料會在處理 `B2CGraphClient.SendGraphGetRequest(...)` 方法:
...
當 B2C 租用戶中建立使用者帳戶，您可以傳送 HTTP POST 要求至 `/使用者` 端點:
...
`Create User` 命令會採用 `.json` 檔案做為輸入參數，其中包含使用者物件的 JSON 表示法。 有兩個範例 `.json` 檔案
...
您可以看到此 POST 要求的方式在 contructed `B2CGraphClient.SendGraphPostRequest(...)`, ，它:
...
檢查 `B2CGraphClient.SendGraphPatchRequest(...)` 如需有關如何傳送此要求的方法。
...
檢查 `B2CGraphClient.SendGraphDeleteRequest(...)` 如需有關如何傳送此要求的方法。
...
您可以使用完整名稱，例如 `extension_55dc0861f9a44eb999e0a8a872204adb_Jersey_Number` 為您的使用者物件的屬性。 只會更新您 `.json` 與新的檔案
屬性的值屬性，然後執行:
...
- 當呼叫 Graph API，使用 [`api 版本 = beta`] (http://blogs.msdn.com/b/aadgraphteam/archive/2015/04/10/graph-api-versioning-and-the-new-beta-version.aspx)。
- 當建立和更新取用者使用者，有幾個必要的屬性，如上所述。

> [AZURE.IMPORTANT]
您將需要帳戶複寫的特性目錄服務基礎的 Azure AD B2C (讀取 [這](http://blogs.technet.com/b/ad/archive/2014/09/02/azure-ad-under-the-hood-of-our-geo-redundant-highly-available-geo-distributed-cloud-directory.aspx) 文章以進一步) B2C 應用程式中使用 Azure AD Graph API 時。 取用者使用**註冊**原則註冊您的 B2C 應用程式後，如果您反過來立即嘗試讀取在您的應用程式中使用 Azure AD 圖形 API 的使用者物件，它可能會無法使用。 您必須等候數秒鐘讓複寫程序完成。 在正式發行的版本中，我們將發行關於 Azure AD 圖形 API 和目錄服務提供的「讀寫一致性保證 」的更具體指引。
