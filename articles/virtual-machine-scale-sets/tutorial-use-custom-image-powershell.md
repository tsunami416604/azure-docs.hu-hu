---
title: Oktatóanyag – Egyéni virtuálisgép-lemezkép használata méretezési készletben az Azure PowerShell használatával
...
Először hozzon létre egy erőforráscsoportot a [New-AzResourceGroup](/powershell/module/az.resources/new-azresourcegroup)segítségével, majd hozzon létre egy virtuális gép [új-AzVM.](/powershell/module/az.compute/new-azvm) Ez a virtuális gép szolgál majd az egyéni virtuálisgép-rendszerkép forrásaként. A következő példában létrehozunk egy *myCustomVM* nevű virtuális gépet a *myResourceGroup* nevű erőforráscsoportban. Amikor a rendszer kéri, adja meg a virtuális gép bejelentkezési hitelesítő adataiként használni kívánt felhasználónevet és jelszót:
...
A virtuális géphez való csatlakozáshoz sorolja fel a nyilvános [IP-címet a Get-AzPublicIpAddress címel](/powershell/module/az.network/get-azpublicipaddress) az alábbiak szerint:
...
Rendszerkép létrehozásához fel kell szabadítani a virtuális gépet. A virtuális gép [felszabadítása a Stop-AzVm-mel.](/powershell/module/az.compute/stop-azvm) Ezután állítsa be a virtuális gép állapotát a [Set-AzVm](/powershell/module/az.compute/set-azvm) általánosítva, hogy az Azure platform tudja, hogy a virtuális gép készen áll egy egyéni lemezkép használatára. Rendszerképet csak általánosított virtuális gépből hozhat létre:
...
Most hozzon létre egy képet a virtuális gép a [New-AzImageConfig](/powershell/module/az.compute/new-azimageconfig) és [a New-AzImage](/powershell/module/az.compute/new-azimage). Az alábbi példa létrehoz egy *myImage* nevű rendszerképet a virtuális gépéből:
...
A méretezési csoport létrehozása előtt konfigurálnunk kell a hálózati biztonsági csoport társítására vonatkozó szabályokat, hogy lehetővé tegyék a HTTP-hez, rdp-hez és remotinghoz való hozzáférést. 
...
A méretezési csoport működés közbeni megtekintéséhez a [Get-AzPublicIpAddress](/powershell/module/az.network/Get-AzPublicIpAddress) segítségével a terheléselosztó nyilvános IP-címét az alábbiak szerint kapja meg:
