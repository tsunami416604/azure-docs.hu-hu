...
A Forrásvezérlés integrációlehetővé teszi, hogy az Automation-fiókban lévő runbookokat egy GitHub-forrásvezérlő tárházhoz társítsa. A forrásvezérlés lehetővé teszi, hogy könnyedén együttműködjön a csapatával, nyomon követhesse a változásokat, és visszaváltson a runbookok korábbi verzióira. A forrásvezérlés például lehetővé teszi a forrásvezérlés különböző ágainak szinkronizálását a fejlesztési, tesztelési vagy éles automatizálási fiókokhoz, így egyszerűen előléptetheti a fejlesztői környezetben tesztelt kódot az éles automatizálási fiókba.
...
Két egyszerű lépés szükséges az Automation-fiók forrásvezérlésének konfigurálásához, és csak egy, ha már rendelkezik Egy GitHub-fiókkal. 
...
### <a name="set-up-source-control"></a>Forrásvezérlő beállítása
...
3. Ha például van egy **PowerShellScripts** nevű tárháza, amely egy **Gyökérmappa**nevű mappát tartalmaz, amely egy **Almappa**nevű mappát tartalmaz. Az egyes mappaszinteket a következő karakterláncokkal szinkronizálhatja:
   ...
   1. A runbookok **tárházból**való szinkronizálásához **/** a runbook mappa elérési útja a.
   2. A runbookok **RootFolder mappából**történő szinkronizálásához a runbook mappa elérési útja **/RootFolder**.
...
6. A forrásellenőrzés beállítása után két [változóeszköz](automation-variables.md) jön létre az Automation-fiókban. Emellett egy engedélyezett alkalmazás hozzáadódik a GitHub-fiókhoz.
   ...
   * **Automation Source Control** hozzá adódik egy engedélyezett alkalmazás a GitHub-fiókhoz. Az alkalmazás megtekintéséhez a GitHub kezdőlapjáról keresse meg a > **profilbeállítások** > **alkalmazások at.** **profile** Ez az alkalmazás lehetővé teszi, hogy az Azure Automation szinkronizálja a GitHub-tárházat egy Automation-fiókkal.  

     ![Alkalmazásbeállítások a GitHubon](media/source-control-integration-legacy/automation-GitApplication.png)

## <a name="using-source-control-in-automation"></a>Forrásvezérlés használata az Automatizálásban
...
2. A runbook szerkesztése után mentse, és kattintson a **Beadás** gombra a szerkesztési lapon.  
...
5. A módosított runbook nevét a rendszer a beadott runbook bemeneti paramétereként küldi el. A [feladat részleteit a](automation-runbook-execution.md#viewing-job-status-from-the-azure-portal) Runbook a Tártár szinkronizálása lapon való kibontásával tekintheti meg.  
...
2. A Sync elindítja a **Sync-MicrosoftAzureAutomationAccountFromGitHubV1** runbookot, amely csatlakozik a GitHubhoz, és lekéri a módosításokat a tárházból az Azure Automation becsatornába. Ekkor megjelenik egy új feladat a tárház szinkronizálása lapon ehhez a művelethez. A szinkronizálási feladat részleteinek megtekintéséhez kattintson ide a feladat részletei lap megnyitásához.  
