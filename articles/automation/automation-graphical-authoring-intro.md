...
您可以加入多個條件 [邏輯運算子](https://technet.microsoft.com/library/hh847789.aspx) 例如 **-和** 或 **-或**。 例如，下列條件會檢查上述範例中相同虛擬機器的狀態是否為「已停止」**或「正在停止」**。

    ($ActivityOutput["Get-AzureVM"].PowerState –eq "Stopped") -or ($ActivityOutput["Get-AzureVM"].PowerState –eq "Stopping") 

### 雜湊表

[Hashtable](http://technet.microsoft.com/library/hh847780.aspx) 是名稱/值組，傳回一組值很有用。 某些活動的屬性可能是雜湊表而不是簡單值。 雜湊表也可能稱為字典。
