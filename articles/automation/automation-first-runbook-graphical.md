...
4. Adja meg a runbook a "MyFirstRunbook-grafikus" nevet.
...
1. A könyvtár vezérlőelemben kattintson a Keresés mezőre, és írja be a "Write-output" kifejezést. A keresési eredmények az alábbi képen láthatók: <br> ![Microsoft.PowerShell.Utility](media/automation-first-runbook-graphical/search-powershell-cmdlet-writeoutput.png)
...
1. A **label (címke** ) mező alapértelmezett értéke a parancsmag neve, de megváltoztathatja valami barátságosabbra. Módosítsa a kimenetre "írás"Helló világ!"alkalmazás.
...
1. Jelölje ki az *InputObject* paramétert. Ezt a paramétert használja a kimeneti adatfolyamnak küldendő szöveg megadására.
1. Az **adatforrás** legördülő menüje olyan forrásokat biztosít, amelyek segítségével feltöltheti a paraméter értékét. Ebben a menüben válassza a **PowerShell-kifejezés**lehetőséget. 
   ...
   Az ilyen forrásokból származó kimeneteket egy másik tevékenységként, egy Automation-eszközként vagy egy PowerShell-kifejezésként is használhatja. Ebben az esetben a kimenet csak ""Helló világ!"alkalmazás". Megadhat egy sztringet egy PowerShell-kifejezéssel is.<br>

1. A kifejezés mezőbe írja be a ""Helló világ!"alkalmazás" **kifejezést** , majd kattintson kétszer az **OK gombra** a vászonra való visszatéréshez.
...
1. Ha a runbook-feladatok befejeződik, a teszt pagn megjeleníti a kimenetét. Ebben az esetben a ""Helló világ!"alkalmazás" jelenik meg.<br> ![Hello World](media/automation-first-runbook-graphical/runbook-test-results.png)
...
1. Megnyílik a létrehozott runbook-feladatokhoz tartozó feladatok ablaktábla. Ellenőrizze, hogy a **feladatok állapota kész állapotú** -e.
1. Kattintson a **kimenet** elemre a kimenet lap megnyitásához, ahol a ""Helló világ!"alkalmazás" látható.
1. A kimeneti oldal bezárásához.
1. A forgatókönyv-feladathoz tartozó Streamek panel megnyitásához kattintson **Az összes napló** lehetőségre. A kimeneti adatfolyamban csak a ""Helló világ!"alkalmazás" látható. 
...
1. Válassza **a + változó hozzáadása**elemet.
1. Az új változó lapon végezze el a következő beállításokat a megadott mezőkben.

    * **Név** – adja meg a "AzureSubscriptionId" értéket.
...
1. Többé nem szükséges a "Write"Helló világ!"alkalmazás a kimenethez" bejegyzés. Csak kattintson a három pontra, majd válassza a **Törlés**lehetőséget.
1. A könyvtár vezérlőben bontsa ki az **eszközök**, majd a **kapcsolatok**elemet. Adja hozzá a "Azurerunasconnection elemet" kifejezést a vászonhoz a **Hozzáadás a vászonhoz**lehetőség kiválasztásával.
1. A könyvtár vezérlőben írja be a "kapcsolat-AzAccount" kifejezést a keresőmezőbe.
...
1. A vásznon válassza a **kapcsolat-AzAccount**lehetőséget. A konfigurálási vezérlő panelen írja be a "Bejelentkezés az Azure-ba" kifejezést a **label (címke** ) mezőbe.
1. Kattintson a **Paraméterek**elemre, és megjelenik a tevékenység paraméterének konfigurációja lap.
1. A **AzAccount** parancsmagnak több paramétere is van, és a paraméterek megadása előtt ki kell választania egyet. Kattintson a **paraméter beállítása** elemre, majd válassza ki a serviceprincipalcertificate paraméterkészletet paramétert.
...
1. A könyvtár vezérlőben írja be a "set-AzContext" kifejezést a keresőmezőbe.
1. Adja hozzá a **set-AzContext** a vászonhoz.
1. Válassza a **set-AzContext** elemet a vásznon. A konfigurálási vezérlő panelen írja be az "előfizetés-azonosító megadása" kifejezést a **label (címke** ) mezőbe.
1. Kattintson a **Paraméterek** elemre, és megjelenik a tevékenység paraméterének konfigurációja lap.
1. A **set-AzContext** parancsmagnak több paramétere is van, és a paraméterek megadása előtt ki kell választania egyet. Kattintson a **paraméter beállítása** elemre, majd válassza ki a SubscriptionId paramétert.
...
Most hozzá kell adnia egy **Start-AzVM** tevékenységet a virtuális gép elindításához. Bármelyik virtuális gépet kiválaszthatja az Azure-előfizetésében, és most már rögzítjük ezt a nevet a [Start-AzVM](https://docs.microsoft.com/powershell/module/az.compute/start-azvm?view=azps-3.5.0) parancsmagnak.
...
1. Válassza ki a **ResourceGroupNameParameterSetName** paraméterkészletet. A **ResourceGroupName** és a **Name** mezőhöz tartozó felkiáltójelek a mellettük lévő felkiáltójelek jelzik, hogy kötelező paraméterek. Vegye figyelembe, hogy mindkét mező sztring értékeket vár.
...
1. Kattintson az **Indítás** gombra a teszt elindításához. A művelet befejezését követően győződjön meg arról, hogy a virtuális gép elindult. 
...
Most úgy módosítja a runbook, hogy csak akkor próbálja meg elindítani a virtuális gépet, ha még nem indult el. Ezt úgy teheti meg, hogy hozzáad egy [Get-AzVM](https://docs.microsoft.com/powershell/module/Az.Compute/Get-AzVM?view=azps-3.5.0) parancsmagot, amely lekéri a virtuális gép példány-szintjének állapotát. Ezután hozzáadhat egy Get status nevű PowerShell-munkafolyamati modult egy PowerShell-kódrészlettel annak megállapításához, hogy a virtuális gép állapota fut vagy leállt. Az állapot beolvasása modulból származó feltételes hivatkozás csak a **Start-AzVM** parancsot futtatja, ha az aktuálisan futó állapot le van állítva. Az eljárás végén a runbook a **Write-output** parancsmaggal küld egy üzenetet, amely tájékoztatja, hogy a virtuális gép sikeresen elindult-e.
...
1. Válassza a **Get-AzVM** , majd a **paraméter beállítást** a **Get-AzVM**készletének megtekintéséhez. 
1. Válassza ki a GetVirtualMachineInResourceGroupNameParamSet paramétert. A **ResourceGroupName** és a **Name** mezőhöz felkiáltójel van, ami azt jelzi, hogy kötelező paramétereket határoznak meg. Vegye figyelembe, hogy mindkét mező sztring értékeket vár.
...
1. Illessze be a következő kódrészletet a kódszerkesztőba.
...
1. A **inputobject elemnél**módosítsa az **adatforrást** a **PowerShell-kifejezésre**, és írja be a "$VMName sikeresen elindítva" kifejezést.
...
1. A **inputobject elemnél**módosítsa az **adatforrást** a **PowerShell-kifejezésre**, és írja be a "$VMName nem indítható el" kifejezést.
