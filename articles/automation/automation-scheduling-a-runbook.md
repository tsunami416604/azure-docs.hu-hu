...
1. 在視窗的底端，按一下 [ **加入設定**。
1. 按一下 [ **加入排程**。
1. 型別 a **名稱** 並選擇性地 **描述** 新排程。
1. 選取排程將執行 **一次**, ，**每小時**, ，或 **每日**。
1. 指定 **開始時間** 和其他選項，視您選取的排程類型而定。

### 使用 Windows PowerShell 建立新排程

您可以使用 [New-azureautomationschedule](http://msdn.microsoft.com/library/azure/dn690271.aspx) 指令程式可在 Azure 自動化中建立新的排程。 您必須指定排程的開始時間，以及是要執行一次、每小時或每日。
...
1. 按一下 [ **排程** ] 索引標籤。
2. 如果 runbook 目前未連結至排程，則會提供您選擇 **連結至新的排程** 或 **連結至現有排程**。  如果 runbook 目前連結至排程中，按一下 [ **連結** 來存取這些選項視窗的底部。
1. 如果 Runbook 有參數，系統會提示您輸入它們的值。  

### 使用 Windows PowerShell 將排程連結至 Runbook

您可以使用 [Register-azureautomationscheduledrunbook](http://msdn.microsoft.com/library/azure/dn690265.aspx) 將排程連結至 runbook。 您可以使用 Parameters 參數來指定 Runbook 參數的值。 請參閱 [Azure 自動化中啟動 Runbook](automation-starting-a-runbook.md) 如需有關指定參數值。
...
您可以使用 [Set-azureautomationschedule](http://msdn.microsoft.com/library/azure/dn690270.aspx) cmdlet 來變更現有排程的屬性。 若要停用排程，請指定 **false** 的 **IsEnabled** 參數。
