...
3. 按一下您想要編輯，然後按一下 [runbook 名稱 **編輯** ] 按鈕。
6. 執行必要的編輯。
7. 按一下 [ **儲存** 完成編輯時。
8. 按一下 [ **發行** 如果您想要發佈 runbook 的最新草稿版本。

### 將 Cmdlet 插入 Runbook

2. 在文字式編輯器的 [畫布] 中，將游標移至您要放置 Cmdlet 的位置。
3. 展開 **Cmdlet** 程式庫控制項中的節點。 
3. 展開包含您想要使用的 Cmdlet 的模組。
4. 以滑鼠右鍵按一下插入與選取的 cmdlet **加入至畫布**。  如果 Cmdlet 有一個以上的參數集合，會新增預設集合。  您也以展開 Cmdlet 以選取不同的參數集合。
...
3. 展開 **Runbook** 程式庫控制項中的節點。 
3. 以滑鼠右鍵按一下要插入與選取的 runbook **加入至畫布**。
...
3. 展開 **資產** 程式庫控制項中的節點。 
4. 展開您想要的資產類型的節點。
3. 以滑鼠右鍵按一下插入並選取資產 **加入至畫布**。  如 [變數資產](variable-assets.md), ，選取 [ **加入至畫布的 「 取得變數 」** 或 **加入至畫布的 「 設定變數 」** 取決於您要取得或設定變數。
...
2. 選取 **Runbook** ] 索引標籤。
3. 按一下您想要編輯，然後選取 [runbook 名稱 **作者** ] 索引標籤。
5. 按一下 [ **編輯** 在畫面底部的按鈕。
6. 執行必要的編輯。
7. 按一下 [ **儲存** 完成編輯時。
8. 按一下 [ **發行** 如果您想要發佈 runbook 的最新草稿版本。
...
1. 在畫面底部，按一下 [ **插入** 然後 **活動**。
1. 在 **整合模組** 資料行中，選取包含活動的模組。
1. 在 **活動** ] 窗格中，選取一個活動。
1. 在 **描述** 資料行中，記下活動的描述。 您可以選擇性按一下 [檢視] 詳細的說明以在瀏覽器中啟動活動的說明。
