---
title: PowerShell-runbook létrehozása az Azure Automationben
description: Oktatóanyag, amely bemutatja, hogyan hozhat létre, tesztelt és tehet közzé egy egyszerű PowerShell-runbookot.
...
---
# <a name="tutorial-create-a-powershell-runbook"></a>Oktatóanyag: Hozzon létre egy PowerShell-runbookot

Ez az oktatóanyag bemutatja, hogyan hozhat létre az Azure Automationben egy [PowerShell-forgatókönyvet](../automation-runbook-types.md#powershell-runbooks). A PowerShell-runbookok a Windows PowerShell-alapúak. Közvetlenül szerkesztheti a runbook kódját az Azure Portalon található szövegszerkesztő használatával.
...
5. Meg van nyitva egy feladat ablaktábla a létrehozott runbook-feladathoz. Bár bezárhatja ezt az ablaktáblát, hagyja nyitva most, hogy megnézhesse a feladat előrehaladását. A feladat állapota megjelenik a **Feladat összegzése**, és a lehetséges állapotok a runbook teszteléséhez leírt módon jelennek meg.

   ![Feladat összegzése](../media/automation-tutorial-runbook-textual-powershell/job-pane-status-blade-jobsummary.png)

6. Miután a runbook állapota **Output** látható befejeződött, kattintson a `Hello World` Kimenet gombra a Kimenet lap megnyitásához, ahol megjelenik a megjelenítés.
...
9. Zárja be a Streams és a Feladat ablaktáblát a MyFirstRunbook-PowerShell lapra való visszatéréshez.

10. A **Részletek csoportban**kattintson a **Feladatok** elemre a runbook Feladatok lapjának megnyitásához. Ez a lap a runbook által létrehozott összes feladatot megjeleníti. Csak egy feladat jelenik meg a listában, mivel csak egyszer futtatta a feladatot.
...
2. Már nincs szükséged `Write-Output` a vonalra. Csak rajta, töröld ki.

3. Írja be vagy másolja be a következő kódot, amely kezeli a hitelesítést az Automation Run As fiókkal.
...
4. Kattintson **a Teszt ablaktábla** elemre, hogy tesztelhesse a runbookot.

5. Kattintson az **Indítás** gombra a teszt elindításához. Miután befejeződött, a következőhöz hasonló kimenetet kell látnia, amely a fiókjából származó alapvető információkat jeleníti meg. Ez a kimenet megerősíti, hogy a Futtatás partner érvényes.
...
3. A teszt megkezdéséhez kattintson a **Start** gombra. Miután befejeződött, győződjön meg arról, hogy a virtuális gép elindult.
...
7. Írja be az elindítani kívánt virtuális gép **VMNAME** és **RESOURCEGROUPNAME** értékeit, majd kattintson az **OK**gombra.
