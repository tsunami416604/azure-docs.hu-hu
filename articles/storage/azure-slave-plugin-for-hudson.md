...
2. 按一下 [ **設定系統**。
3. 若要尋找頁面上向下的捲動 **定域機組** 一節。
4. 內 **定域機組** 區段中，尋找 **新增 Azure 虛擬機器範本** 按一下 **新增** ] 按鈕。



    ![add vm template](./media/azure-slave-plugin-for-hudson/hudson-setup-addnewvmtemplate.png)

5. 指定雲端服務名稱在 **名稱** 欄位。 如果您指定的名稱參照現有雲端服務，便會在該服務中佈建 VM。 否則，Azure 會建立一個新的。 
6. 在 **描述** 欄位中，輸入您要建立之範本的說明文字。 此資訊僅供記錄之用，並不會用於佈建 VM。
...
12. 在 **使用量**, ，選取適當的條件，將會使用此從屬節點。 現在，選取 **利用此節點儘可能**。
...
13. 在 **映像系列或識別碼** 您必須指定您的 VM 上安裝系統映像。 您可以從映像系列清單中進行選取，或指定自訂映像。

    如果您想從映像系列清單中進行選取，請輸入映像系列名稱的第一個字元 (需區分大小寫)。 例如，輸入 **U** 會顯示 Ubuntu Server 系列的清單。 在從清單中進行選取後，Jenkins 將會在佈建 VM 時使用該系列的該系統映像的最新版本。

    ![作業系統系列清單](./media/azure-slave-plugin-for-hudson/hudson-oslist.png)
    
    如果您想要改用自訂映像，請輸入該自訂映像的名稱。 清單中不會顯示自訂映像名稱，因此您必須確實輸入正確的名稱。    

    本教學課程中，輸入 **U** 顯示 Ubuntu 映像的清單，並選取 **Ubuntu Server 14.04 LTS**。
 
14. 如 **啟動方法**, ，請選取 **SSH**。
15. 將下列指令碼複製並貼入 **Init 指令碼** 欄位。
     ...
     **Init 指令碼** VM 建立後就會執行。 在此範例中，指令碼會安裝 Java、git 和 ant。
    
16. 在 **Username** 和 **密碼** 欄位中，將會在 VM 上建立的系統管理員帳戶輸入偏好使用的值。
...
2. 輸入要建立之工作的名稱。
3. 針對工作類型] 選取 **建置自由樣式的軟體作業**。
4. 按一下 [ **確定**。
5. 在工作組態頁面中，選取 **限制可以執行此專案的位置**。
6. 選取 **節點和標籤功能表** ，然後選取 **linux** （在上一節中建立虛擬機器範本時，我們指定此標籤）。 
...
9. 按一下 [ **儲存**。
10. 在 Hudson 儀表板中，尋找您剛才建立的工作，然後按一下 **排程組建** 圖示。 
