...
2. 安裝好 Azure CLI 之後，您就能從命令列介面 (Bash、終端機、命令提示字元) 中使用 azure 命令存取 Azure CLI 命令。 型別 `azure` 命令，您應該會看到下列輸出。

    ![Azure 命令輸出][image1]

3. 在命令列介面中，輸入 `azure 儲存體` 可列出所有 azure 儲存體命令，功能有初步印象 Azure CLI 提供。 您可以輸入命令名稱搭配 **-h** 參數 (例如， `azure 儲存體共用建立-h`) 以查看命令語法的詳細資料。
...
6. 現在，您需要根據您的組態設定更新指令碼變數。
    ...
    - * *<storage_account_name>* * 指令碼中使用的指定名稱或輸入新名稱儲存體帳戶。**重要事項：**儲存體帳戶的名稱在 Azure 中必須是唯一的名稱。而且必須是小寫字母！
...
Azure 儲存體中的每個 Blob 必須位於一個容器中。 您可以建立私用容器使用 `azure 儲存體容器建立` 命令:
...
若要上傳至容器的 blob，您可以使用 `azure 儲存體 blob 上傳`。 根據預設，此命令會將本機檔案上傳至區塊 Blob。 若要指定 blob 的類型，您可以使用 `--blobtype` 參數。
...
此範例會執行非同步複製。 您可以監視每個複製作業的狀態執行 `azure 儲存體 blob 複製顯示` 作業。
