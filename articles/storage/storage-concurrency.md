...
2.  當您更新 blob 時，包括您在步驟 1 中接收到的 ETag 值 **If-match** 要求傳送至服務的條件式標頭。
...
儲存體服務也包含額外的條件式標頭的支援例如 **修改 If-modified-since**, ，**未經修改的 If-modified-since** 和 **If-none-Match** 及其組合。 如需詳細資訊，請參閱 [為 Blob 服務作業指定條件式標頭](http://msdn.microsoft.com/library/azure/dd179371.aspx) MSDN 上。  

下表摘要說明的容器作業接受條件式標頭，例如 **If-match** 要求中傳回 ETag 值在回應中的。  
...
下表摘要說明的 blob 作業接受條件式標頭，例如 **If-match** 要求中傳回 ETag 值在回應中的。  
...
### Blob 的封閉式並行存取
若要鎖定 blob 以進行獨佔使用，您可以取得 [租用](http://msdn.microsoft.com/library/azure/ee691972.aspx) 在其上。 您取得租用時，可指定需要租用的時間長度：這可以是 15 至 60 秒，也可以是無限期 (等於獨佔鎖定)。 您可以更新有限租用而加以延伸，而且您可以在用完任何租用後加以釋放。 Blob 服務會在有限租用到期時自動加以釋放。  
...
下列 C# 程式碼片段說明對某個 Blob 取得獨佔租用 30 秒、更新該 Blob 的內容，然後釋放租用的範例。 如果當您嘗試取得新租用時已有對 Blob 的有效租用存在，Blob 服務將會傳回「HTTP (409) 衝突」狀態結果。 使用下列程式碼片段 **AccessCondition** 物件封裝租用資訊更新的 blob 儲存體服務中的要求時。 您可以下載完整的範例 [這裡](http://code.msdn.microsoft.com/windowsazure/Managing-Concurrency-using-56018114)。  
...
如果您直接嘗試對已租用的 Blob 執行寫入作業，而未傳送租用識別碼，要求將會失敗，並出現 412 錯誤。 請注意，如果租用到期之前，先呼叫 **UploadText** 方法，但您仍傳送租用識別碼，要求也失敗，並 **412** 錯誤。 如需管理租用到期時間和租用識別碼的詳細資訊，請參閱 [租用 Blob](http://msdn.microsoft.com/library/azure/ee691972.aspx) REST 文件。  
...
- [為 Blob 服務作業指定條件式標頭](http://msdn.microsoft.com/library/azure/dd179371.aspx)
...
若要明確停用並行存取檢查，您應該設定 **ETag** 屬性 **員工** 物件傳遞給 「 * 」 執行取代作業之前。  
...
請注意， **插入或取代實體** 和 **插入或合併實體** 作業並 *不* 執行任何並行存取檢查，因為這些作業將 ETag 值傳送至資料表服務。  
