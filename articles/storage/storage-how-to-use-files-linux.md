...
- 監視每個檔案共用的實際使用狀況。
- 調整檔案室共用大小配額。
- 複製 `net 使用` 命令，以使用 Windows 用戶端從掛接檔案共用。
...
身為開發人員，您可以使用建置應用程式使用檔案儲存體 [Azure Storage Client Library for Java](https://github.com/azure/azure-storage-java)。 程式碼範例，請參閱 [如何使用 java 的檔案儲存體](storage-java-how-to-use-file-storage.md)。
