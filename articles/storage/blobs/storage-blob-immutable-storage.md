...
- **Audit naplózás támogatást**: A tárolók magában foglalja a csoportházirend-auditnaplók bejegyzéseit. Bemutatja azt legfeljebb hét időalapú adatmegőrzési zárolt időalapú adatmegőrzési szabályzatok parancsokat és tartalmazza a felhasználói azonosító, a parancs típusa, az időbélyegeket és a megőrzési időtartam. Jogi célú visszatartással a napló tartalmazza a felhasználói azonosító, a parancs típusa, az időbélyegzőket és jogi célú visszatartási címkék. Ez a napló őrizzük meg a szabályzatot, az mp-ben 17a-4(f) szabályozási irányelvek megfelelően élettartamát. A [Azure-tevékenységnapló](../../azure-monitor/platform/activity-logs-overview.md) a vezérlési sík tevékenységek; engedélyezésekor átfogóbb naplóját jeleníti meg [Azure diagnosztikai naplók](../../azure-monitor/platform/diagnostic-logs-overview.md) megőrzi, és megjeleníti az adatok síkjával végzett műveletek. Feladata a felhasználó, szabályozási és más célokra is szükség lehet, tartósan tárolja ezeket a naplókat.
...
Tároló és a fiók törlése még nem engedélyezettek van-e azokat a blobokat egy nem módosítható házirend által védett. A tároló törlése művelet sikertelen lesz, ha legalább egy blob már létezik a zárolt időalapú adatmegőrzési szabály vagy a jogi céllal zároltak közé. A tárfiók törlése sikertelen lesz, ha legalább egy jogi célú visszatartással ellátott WORM-tárolót vagy aktív adatmegőrzési időtartammal rendelkező blobot tartalmaz. 

### <a name="time-based-retention"></a>Időalapú adatmegőrzés

> [!IMPORTANT]
> Időalapú adatmegőrzési kell *zárolva* egy megfelelő legyen a BLOB nem módosítható (írási és törlési védett) állapot mp 17a-4(f) és egyéb szabályozásoknak való megfelelőséget. Azt javasoljuk, hogy akkor zárolja a szabályzatot egy ésszerű időn belül általában legfeljebb 24 óra. Egy alkalmazott időalapú adatmegőrzési kezdeti állapota *oldják a zárolást*, lehetővé téve, hogy a szolgáltatás tesztelése, és hajtsa végre a módosításokat a szabályzat előtt zárolni tudja. Bár a *oldják a zárolást* állapot módosíthatatlansági védelmet biztosít, nem javasoljuk a *oldják a zárolást* állapot rövid távú funkció kísérletek kívül más célra. 
...
Egy nem zárolt időalapú adatmegőrzési ajánlott csak tesztelésre funkció, és egy házirend zárolni kell ahhoz, hogy meg kell felelnie az mp-ben 17a-4(f) és egyéb szabályozásoknak való megfelelőséget. Időalapú adatmegőrzési zárolva van, ha a házirend nem távolítható el, és 5 növekszik a tényleges megőrzési időszak maximális engedélyezett. Zárolhatja az időalapú adatmegőrzési házirendek, és hogyan további információkért lásd: a [bevezetés](#getting-started) szakaszban.
...
### <a name="time-based-retention"></a>Időalapú adatmegőrzés
- Egy storage-fiókok a szabályzatokkal zárolt időalapú nem módosítható a tárolók maximális száma 1000.
- A minimális megőrzési időtartam 1 nap. A maximális hossz 146,000 nap (400 év).
- Egy tároló kiterjesztése a megőrzési időtartam zárolt időalapú nem módosítható szabályzatok módosításainak maximális száma érték az 5.
- Egy tároló legfeljebb 7 időalapú adatmegőrzés csoportházirend-auditnaplók a szabályzat időtartamának jelennek meg.

### <a name="legal-hold"></a>Jogi céllal zároltak közé
...
- A jogi céllal zároltak közé címke minimális hossza 3 alfanumerikus karaktereket. A maximális hossz 23 alfanumerikus karaktereket.
- Egy tároló legfeljebb 10 jogi tartsa szabályzati naplózás naplók megmaradnak a szabályzat időtartamára.
...
6. Válassza ki **zárolási szabályzat** , majd erősítse meg a zárolást. A szabályzat zárolva van, és nem lehet törölni, csak a megőrzési időtartam bővítmény engedélyezve lesz. Törli a blobot, és felülbírálások használata nem engedélyezett. 

    ![Erősítse meg a menü "zárolási házirendet"](media/storage-blob-immutable-storage/portal-image-5-lock-policy.png)

7. Válassza ki ahhoz, hogy a jogi célú visszatartással, **+ szabályzat hozzáadása**. Válassza ki **megadva jogi célú visszatartás** a legördülő menüből.
...
8. Hozzon létre egy vagy több címkét egy jogi céllal zároltak közé.

    !["A címke neve" mező mellett a házirend típusát](media/storage-blob-immutable-storage/portal-image-set-legal-hold-tags.png)

9. Ha törölni szeretne egy jogi céllal zároltak közé, egyszerűen alkalmazott megadva jogi célú visszatartás azonosító címke eltávolítása.
...
Az Az.Storage modul nem módosítható storage támogatja.  A funkció engedélyezéséhez kövesse az alábbi lépéseket:
...
**Kell hozzon létre egy új tárfiókot, a szolgáltatás használatához?**

Nem, nem módosítható storage használható bármely meglévő vagy újonnan létrehozott általános célú v2 és Blob storage-fiókok. Ez a funkció a blokkblobok, a GPv2 és Blob Storage-fiókok használati szól. Általános célú v1-tárfiókok nem támogatottak, de általános célú v2 verzióra egyszerűen frissíthetők. Egy meglévő általános célú v1 storage-fiók frissítésével kapcsolatos információkért lásd: [tárfiók frissítése](../common/storage-account-upgrade.md).

**Jogi céllal zároltak közé és időalapú adatmegőrzési szabály is használhatom?**

Igen, a tároló lehet megadva jogi célú visszatartás és a egy időalapú adatmegőrzési szabály egyszerre. A tárolóban lévő összes BLOB állapotban marad a nem módosítható mindaddig, amíg minden jogi célú visszatartással nincsenek bejelölve, akkor is, ha azok hatékony megőrzési ideje lejárt. Ezzel szemben egy blob marad nem módosítható állapotban addig, amíg a tényleges megőrzési időtartam lejár, annak ellenére, hogy minden jogi célú visszatartással törölve lett.
...
**Eltávolítható egy *zárolva* időalapú adatmegőrzési szabály vagy a jogi céllal zároltak közé?**

Csak a feloldott időalapú adatmegőrzési házirendek távolíthatja el a tárolóból. Időalapú adatmegőrzési szabály zárolása, után nem lehet eltávolítani; csak tényleges megőrzési időszak bővítmények használata engedélyezett. Jogi céllal zároltak közé címkék lehet törölni. Minden jogi címkék törlésekor a rendszer eltávolítja a jogi céllal zároltak közé.
...
Egy nem zárolt módosíthatatlansági szabályzat eltávolítása az (add - Force elvetése a rendszer kéri):
```powershell
# with an immutability policy object
$policy = Get-AzRmStorageContainerImmutabilityPolicy -ResourceGroupName `
    $ResourceGroup -StorageAccountName $StorageAccount -ContainerName $container
