...
|-|------------------------|-------------------------------|-----------------------------------------------|
| Az adatok elrendezése| Támogatja az adatokon a fájlok és mappák | Támogatja a tárolt objektumok/blobok, valamint a mappák és fájlok - adatok [hivatkozás](https://docs.microsoft.com/azure/storage/data-lake-storage/namespace) | Támogatja a mappákban és fájlban tárolt, *jelenleg elérhetővé* tett fájlokat <br><br> Az objektumként/blobként tárolt, *még nem elérhető* adatkészleteket támogatja. |
...
| Adatműveletek – naplók  | Igen | Támogatási jegy integrációs Azure Monitoring segítségével adott időtartamának naplók egyszeri kérelmek | Egyszeri kérések egy adott időtartamra vonatkozó naplókhoz támogatási jegy használatával – *jelenleg elérhető*<br><br> Azure monitoring-integráció – *még nem érhető el* |
...
Ez a táblázat ismerteti, amely API-t állítja be, amely az egyéni alkalmazások érhetők el. Ha egy kicsit világosabban szeretné elkészíteni a dolgokat, ezeket az API-készleteket két típusra osztjuk: felügyeleti API-k és fájlrendszer API-k.
...
|------------------------------------|-----------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Lift-and-shift**                 | [Azure Data Factory](https://docs.microsoft.com/azure/data-factory/load-azure-data-lake-storage-gen2-from-gen1) | Felügyelt felhőbeli szolgáltatás                                                                                                                | A jelenleg tárolt adat-és ACL-ek is átmásolhatók.                                                                                                                                                                                                                                                                      |
...
Vegye figyelembe, hogy vannak olyan külső felek, akik a fenti adatok/meta-adatmásolási eszközök bevonása nélkül képesek kezelni a Data Lake Storage Gen1 Data Lake Storage Gen2 a frissítésre (például: [Cloudera](https://blog.cloudera.com/blog/2017/08/use-amazon-s3-with-cloudera-bdr/)). Adatok áttelepítése, valamint a számítási feladatok migrálását végző egy "egyablakos" élményt nyújtanak. Előfordulhat, hogy rendelkezik olyan eszközökkel, amelyek ökoszisztémát kívül a sávon kívüli frissítés végrehajtásához.
...
A Data Lake Storage Gen2 fájlok eléréséhez szükséges URI-sémát részletesen ismertetjük, de széles körben szólva. [](../../hdinsight/hdinsight-hadoop-use-data-lake-storage-gen2.md) `abfss://<FILE_SYSTEM_NAME>@<ACCOUNT_NAME>.dfs.core.widows.net/<PATH>`
...
A hívott ki a a *mérje fel az upgrade Readiness-szel* szakaszban SDK-k nem érhető el. Ha azt szeretné, hogy a port az alkalmazásokon keresztül Data Lake Storage Gen2, javasoljuk, hogy várjon, amíg a támogatott SDK-k elérhetővé válnak.
...
Ezután a fent megnevezett elemek (például: Az URI-t és a hitelesítő adatokat) módosítani kell. A meglévő példányt, amely együttműködik a Data Lake Storage Gen1, módosításával, vagy létrehozhat egy új példányt, amely a Data Lake Storage Gen2 működne.
