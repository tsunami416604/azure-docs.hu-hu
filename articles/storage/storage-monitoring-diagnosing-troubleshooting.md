...
+ [移難排解指引]
    ...
    + [度量顯示低 AverageE2ELatency 與低 AverageServerLatency，但用戶端端正經歷高延遲]
    ...
    + [度量顯示低 PercentSuccess 或分析記錄檔項目的交易狀態為 ClientOtherErrors 的作業]
    + [容量度量顯示非預期增加儲存體容量使用]
    + [您遇到未預期的重新開機，有大量的附加 Vhd 的虛擬機器]
    + [您的問題起因於使用儲存體模擬器進行開發或測試]
    + [您遇到問題，安裝 Azure SDK for.NET]
    + [您必須對儲存體服務出現其他問題]
+ [附錄]
    ...
    + [附錄 5: 使用 Application Insights for Visual Studio Team 服務監視]
...
「 [監視儲存體服務] 」 一節描述如何監視的健全狀況和效能的 Azure 儲存體服務使用 Azure 儲存體分析度量 (儲存體度量)。

"[診斷儲存體問題 > 一節] 說明如何使用 Azure 儲存體分析記錄 (儲存體記錄) 來診斷問題。 並說明如何透過 Storage Client Library for .NET 或是 Azure SDK for Java 之類的任何一項用戶端程式庫來啟用用戶端記錄。

"[端對端追蹤] 」 的章節將說明如何相互關聯各種記錄檔與度量資料中所包含的資訊。

「 [疑難排解指引] 」 一節提供一些您可能會遇到的常見儲存體相關問題的疑難排解指引。

「 [附錄] 」 包含使用 Wireshark 與 Netmon 等其他工具來分析網路封包資料，Fiddler 來分析 HTTP/HTTPS 訊息，以及 Microsoft Message Analyzer 相互關聯記錄資料的相關資訊。


## <a name="monitoring-your-storage-service"></a>監視儲存體服務

如果您熟悉 Windows 效能監視，可以將儲存體度量當成相當於 Windows 效能監視器計數的 Azure 儲存體。 儲存體度量中您會發現一組完整的度量 (Windows 效能監視器詞彙裡的計數器)，例如服務可用性、 服務，要求的總數或服務要求成功的百分比 (如需可用度量的完整清單，請參閱 <a href="http://msdn.microsoft.com/library/azure/hh343264.aspx" target="_blank">儲存體分析度量資料表結構描述</a> MSDN 上)。 您可以指定讓儲存體服務每小時或每分鐘收集並彙總度量一次。 如需如何啟用度量並監視儲存體帳戶的詳細資訊，請參閱 <a href="http://go.microsoft.com/fwlink/?LinkId=510865" target="_blank">啟用儲存體度量</a> MSDN 上。
...
在 Azure 入口網站中，您可以檢視可用性、總要求數，與儲存體帳戶平均延遲數等度量。 同時設定了一個通知規則，會在可用性降至特定水準以下時，通知管理員。 從檢視這項資料，其中一個可能的區域，以便調查是資料表服務成功百分比低於 100%(如需詳細資訊，請參閱 「 [度量顯示低 PercentSuccess 或分析記錄檔項目的交易狀態為 ClientOtherErrors 的作業]")。
...
- 調查潛在的問題，使用診斷工具，如稍後所述"[診斷儲存體問題 > 一節]。
...
注意: 這項資訊是先前可用，隨著歷程資料，在 Azure 服務儀表板 <a href="http://status.azure.com" target="_blank">http://status.azure.com</a>。

雖然 [Azure 入口網站](portal.azure.com) 收集健全狀況資訊從 (內到外監視)、 Azure 資料中心內您可以考慮採用從外的方式來產生定期存取來自多個位置的 Azure 裝載的 web 應用程式的綜合交易。 所提供的服務 <a href="http://www.keynote.com/solutions/monitoring/web-monitoring" target="_blank">演說</a>, ，<a href="https://www.gomeznetworks.com/?g=1" target="_blank">Gomez</a>, ，和 Application Insights for Visual Studio Team Services 是這個外部中方法的範例。 如需有關 Application Insights for Visual Studio Team Services，請參閱 「 附錄 」 [附錄 5: 使用 Application Insights for Visual Studio Team Services 監視]。 」

### <a name="monitoring-capacity"></a>監視容量

儲存體度量只會儲存 Blob 服務的容量度量，這是因為 Blob 通常佔已儲存的資料最大宗 (寫入期間無法使用儲存體度量來監視資料表與佇列的容量)。 如果您為 Blob 服務啟用監視功能的話，可以在 **$MetricsCapacityBlob** 資料表中找到這項資料。 儲存體度量每天會記錄這項資料一次，而您可以使用 **RowKey** 的值來判斷資料列是否包含與使用者資料 (值 **data**) 或分析資料 (值 **analytics**) 相關聯的實體。 每一個儲存的實體都含有使用的儲存體容量相關資訊 (以位元組數測量的 **Capacity**)，以及儲存體帳戶中使用的目前容器編號 (**ContainerCount**) 及 Blob (**ObjectCount**)。 如需有關儲存的容量度量 **$MetricsCapacityBlob** 資料表，請參閱 <a href="http://msdn.microsoft.com/library/azure/hh343264.aspx" target="_blank">儲存體分析度量資料表結構描述</a> MSDN 上。
> [AZURE.NOTE] 建議您監視這些數值，以便在儲存體帳戶接近容量限制時提早收到警告。 在 Azure 入口網站中，您可以新增警示規則，以便在彙總儲存體使用量超出或低於指定的閥值時通知您。

如需估計 blob 等各種儲存體物件大小請參閱部落格文章 <a href="http://blogs.msdn.com/b/windowsazurestorage/archive/2010/07/09/understanding-windows-azure-storage-billing-bandwidth-transactions-and-capacity.aspx" target="_blank">了解 Azure 儲存體計費 – 頻寬、 交易和容量</a>。
...
任何小於 100% 的值，皆表示某些儲存體要求已經失敗。 您可以檢視度量資料裡的其他資料欄，查看裡面帶有各種錯誤類型 (例如 **ServerTimeoutError**) 的要求數量，以了解這些要求失敗的原因。 當暫時性伺服器逾時狀態出現，以致於服務移動資料分割以便提供更佳的負載平衡要求時，您應該會看到 [可用性]**** 百分比暫時低於 100%；用戶端應用程式裡的重試邏輯應該會處理此類間歇性狀況。 頁面 <a href="http://msdn.microsoft.com/library/azure/hh343260.aspx" target="_blank"></a> 列出交易類型的儲存體度量納入其 **可用性** 計算。

在 [Azure 入口網站](portal.azure.com), ，您可以新增警示規則，以便通知您，如果 **可用性** 服務低於您指定的閥值。

本指南 「 [疑難排解指引] 」 一節描述一些常見儲存體服務相關問題可用性。

### <a name="monitoring-performance"></a>監視效能

若要監視儲存體服務效能，您可以使用下列來自每小時與每分鐘度量表的度量。

- **AverageE2ELatency** 與 **AverageServerLatency** 資料行中的值顯示儲存體服務或是 API 操作類型用來處理要求所需的平均時間。 **AverageE2ELatency** 是一項端對端延遲量值，內含讀取要求及傳送回應所花的時間，以及處理要求所花的時間 (因此，內含一旦要求抵達儲存體服務時的網路延遲)；**AverageServerLatency** 指示一項處理時間量值，因此會將任何與用戶端通訊相關的網路延遲排除在外。 請參閱一節 「 [度量顯示高 AverageE2ELatency 與低 AverageServerLatency] 」 原因的討論本指南稍後可能會有顯著的差異，這兩個值之間。
...
本指南 「 [疑難排解指引] 」 一節說明與效能有關的一些常見儲存體服務問題。
...
- 導致應用程式當機或是停止運作的重大失敗。
- "[監視儲存體服務]。 「 上一節中所述，您要監視的度量中基準值的重大變更
...
以下章節概述當您對這四個類別分別進行問題診斷與疑難排解時，應該遵循的步驟。 本指南稍後的 「 [疑難排解指引] 」 一節提供進一步的詳細資訊，您可能會遇到一些常見的問題。

### <a name="service-health-issues"></a>服務健康情況問題

服務健康情況問題通常是您無法掌控的部分。  [Azure 入口網站](portal.azure.com) 與 Azure 服務，包括儲存體服務提供進行中的任何問題的相關資訊。 若您在建立儲存體帳戶時選擇使用「讀取存取異地備援備援儲存體」，則當主要位置無法提供您的資料時，您的應用程式會暫時切換到次要位置的唯讀副本。 若要這麼做，您的應用程式必須要能切換使用主要與次要儲存位置，並能在降低功能模式下使用唯讀資料。 Azure 儲存體用戶端程式庫可讓您定義重試原則，當無法從主要儲存體讀取資料時，才能嘗試從次要儲存體讀取資料。 您的應用程式還需要了解次要位置的資料最終會與主要位置的資料維持一致。 如需詳細資訊，請參閱部落格文章 <a href="http://blogs.msdn.com/b/windowsazurestorage/archive/2013/12/04/introducing-read-access-geo-replicated-storage-ra-grs-for-windows-azure-storage.aspx" target="_blank">Azure 儲存體備援選項與讀取存取地理區域備援儲存體</a>。
...
"[疑難排解指引 」 一節] 稍後在本指南提供一些常見效能相關的詳細資訊，發出您可能會發生。
...
本指南 「 [疑難排解指引] 」 一節說明使用儲存體模擬器會碰到的一些常見問題。

### <a name="storage-logging-tools"></a>儲存體記錄工具

儲存體記錄功能可針對您的 Azure 儲存體帳戶，提供伺服器端的儲存體要求記錄服務。 如需如何啟用伺服器端記錄及存取記錄資料的詳細資訊，請參閱 <a href="http://go.microsoft.com/fwlink/?LinkId=510867" target="_blank">使用伺服器端記錄</a> MSDN 上。

Storage Client Library for .NET 能讓您針對應用程式所執行的儲存體操作，收集與其相關的用戶端記錄資料。 如需如何啟用用戶端記錄及存取記錄資料的詳細資訊，請參閱 <a href="http://go.microsoft.com/fwlink/?LinkId=510868" target="_blank">用戶端使用儲存體用戶端程式庫記錄</a> MSDN 上。
...
- Fiddler (<a href="http://www.telerik.com/fiddler" target="_blank">http://www.telerik.com/fiddler</a>) 是免費的 web 偵錯 proxy，可讓您檢查標頭和 HTTP 和 HTTPS 要求和回應訊息的內容資料。 如需詳細資訊，請參閱 「 [附錄 1: 使用 Fiddler 擷取 HTTP 與 HTTPS 流量] 」。
- Microsoft Network Monitor (Netmon) (<a href="http://www.microsoft.com/download/details.aspx?id=4865" target="_blank">http://www.microsoft.com/download/details.aspx?id=4865</a>) 與 Wireshark (<a href="http://www.wireshark.org/" target="_blank">http://www.wireshark.org/</a>) 都是免費的網路通訊協定分析器，可讓您檢視各種不同的網路通訊協定的詳細的封包資訊。 如需有關 Wireshark 的詳細資訊，請參閱 「 [附錄 2: 使用 Wireshark 擷取網路流量] 」。
...
- 如果您想要執行基本的連線測試以確認用戶端機器能夠透過網路與 Azure 儲存體服務連線的話，您無法使用用戶端上的標準 **ping** 工具來執行。 不過，您可以使用 **tcping** 工具來檢查連線能力。 **Tcping** 是下載 <a href="http://www.elifulkerson.com/projects/tcping.php" target="_blank">http://www.elifulkerson.com/projects/tcping.php</a>。
...
### <a name="correlating-log-data"></a>記錄資料建立相互關聯
...
> [AZURE.NOTE] 您也可以在儲存體記錄資料中，檢視個別儲存體作業的 **E2ELatency** 與 **ServerLatency**。
...
您應該檢查用戶端記錄檔，以查看多少個要求送出您的用戶端應用程式，以及檢查一般的.NET 相關的效能瓶頸，您的用戶端，例如 CPU、.NET 記憶體回收、 網路使用率或記憶體中 (做為疑難排解.NET 用戶端應用程式的起始點，請參閱 <a href="http://msdn.microsoft.com/library/7fe0dd2y(v=vs.110).aspx" target="_blank">偵錯、 追蹤和分析</a> MSDN 上)。
...
如需有關使用 Microsoft Message Analyzer 對網路問題進行疑難排解的詳細資訊，請參閱 「 [附錄 3: 使用 Microsoft Message Analyzer 擷取網路流量]。 」

### <a name="metrics-show-low-AverageE2ELatency-and-low-AverageServerLatency"></a>度量顯示低 AverageE2ELatency 與低 AverageServerLatency，但用戶端端正經歷高延遲
...
- 對程式碼偵錯，並檢查與要求相關聯之 **OperationContext** 物件的屬性。 如果有重試此作業，**RequestResults** 屬性會包含多個唯一的伺服器要求 ID。 您也可以檢查每個要求的開始和結束時間。 如需詳細資訊，請參閱 [伺服器要求 ID] 區段中的程式碼範例。
...
當 Blob 下載要求出現高 **AverageServerLatency** 的時候，請使用儲存體記錄功能來查看同一個 Blob (或是 Blob 集合) 是否重複出現要求。 在 Blob 上傳要求中，您應該調查用戶端所使用的區塊大小 (例如，小於 64K 的區塊大小，可能導致額外的負荷，除非讀取的區塊流量也小於 64K)，以及是否有多個用戶端同時間上傳區塊給同一個 Blob。 您也應該檢查每分鐘度量是否超出要求數目突然暴增的每秒延展性目標: 另請參閱 「 [度量顯示 percenttimeouterror 增加]。 」
...
### <a name="you-are-experiencing-unexpected-delays-in-message-delivery"></a>您遇到未預期的佇列上的訊息傳遞延遲
...
此錯誤最常見的原因，就是用戶端在儲存體服務逾時之前就中斷連線。 您應該調查用戶端裡的程式碼，了解用戶端何時及為何與儲存體服務中斷連線。 您還可以使用 Wireshark、Microsoft Message Analyzer 或是 Tcping 調查來自用戶端的網路連線問題。 [附錄] 將描述這些工具。
...
- SAS 金鑰中的版本參數 (例如 **sv=2015-04-05**) 是否與您所使用的儲存體用戶端程式庫版本相符。 我們建議您一律使用最新版的 [儲存體用戶端程式庫](https://www.nuget.org/packages/WindowsAzure.Storage/)。 如需有關 SAS 權杖版本以及用戶端程式庫版本相依性，請參閱 [Microsoft Azure 儲存體的最新消息](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/05/14/what-s-new-for-microsoft-azure-storage-at-teched-2014.aspx)。
- 如果您重新產生儲存體存取金鑰，會讓任何現有的 SAS 權杖失效。 如果您產生的 SAS 權杖，內含很長的到期時間以便用戶端應用程式快取處理，則可能會出現問題。

如果您是使用儲存體用戶端程式庫來產生 SAS 權杖，則您可以輕易地建立有效的權杖。 不過，如果您使用儲存體 REST API 並手動建構 SAS 權杖您應該仔細閱讀主題 <a href="http://msdn.microsoft.com/library/azure/ee395415.aspx" target="_blank">使用共用存取簽章委派存取</a> MSDN 上。
...
在此範例中，記錄顯示用戶端會要求從 **CreateIfNotExists** 方法 (...要求 id e2d06d78) 來自要求 **UploadFromStream** 方法 (de8b1c3c-...)，因為用戶端應用程式會叫用這些方法以非同步的方式，是否發生此情形。 您應該修改用戶端裡的非同步程式碼，確保該程式碼在嘗試將任何資料上傳至容器的 Blob 之前，先建立該容器。 理想的情況是，您應該事先建立所有容器。
...
由於網頁瀏覽器中執行，就會發生這些錯誤 <a href="http://www.w3.org/Security/wiki/Same_Origin_Policy" target="_blank">相同原始原則</a> 卻防止網頁呼叫的 API 不同網域中網域頁面的安全性限制來自。

若要解決這個 JavaScript 問題，您可以針對用戶端存取的儲存體服務，設定跨原始來源資源分享 (CORS)。 如需詳細資訊，請參閱 <a href="http://msdn.microsoft.com/library/azure/dn535601.aspx" target="_blank">Azure 儲存體服務的跨原始資源共用 (CORS) 支援</a> MSDN 上。
...
用戶端的例外狀況詳細資料包括要求 id (7e84f12d...) 資料表服務要求所指派: 您可以使用這項資訊至伺服器端儲存體記錄中尋找要求詳細資料中搜尋 **要求 id 標頭** 記錄檔中的資料行。 您也可以使用度量，判定此類失敗情況何時發生，然後依據度量記錄此錯誤的時間搜尋記錄檔。 此記錄項目顯示刪除作業失敗，並顯示「HTTP (404) 用戶端其他錯誤」的狀態訊息。 同一個記錄項目同時包含了用戶端在 **client-request-id** 資料欄中所產生的要求 ID (813ea74f…)。

伺服器端記錄檔也包含具有相同的另一個項目 **用戶端要求 id** 值 (813ea74f …) 針對相同的實體，並從相同的用戶端順利完成的刪除作業。 此順利完成的刪除作業會在刪除要求失敗之前很快地發生。
...
用戶端應用程式中的程式碼會刪除，然後立即重新建立使用相同名稱的 blob 容器: **CreateIfNotExists** 方法 (用戶端要求 ID bc881924-…) 最終會失敗，出現 HTTP 409 (衝突) 錯誤。 當用戶端刪除 Blob 容器、資料表或佇列時，需要等候簡短時間，才能使該名稱再度生效。

每當用戶端應用程式建立新的容器時，應該使用唯一的容器名稱 (如果經常出現「刪除」重新建立作業模式的話)。

### <a name="metrics-show-low-percent-success"></a>度量顯示低 PercentSuccess 或分析記錄檔項目的交易狀態為 ClientOtherErrors 的作業
...
您可以找到常見 REST API 錯誤碼清單頁面上的儲存體服務傳回 <a href="http://msdn.microsoft.com/library/azure/dd179357.aspx" target="_blank">常見的 REST API 錯誤碼</a>。

### <a name="capacity-metrics-show-an-unexpected-increase"></a>容量度量顯示非預期增加儲存體容量使用
...
### <a name="you-are-experiencing-unexpected-reboots"></a>您遇到未預期的重新開機的 Azure 虛擬機器，有大量的附加 Vhd

當 Azure 虛擬機器 (VM) 裡有大量的附加 VHD 位於同一個儲存體帳戶內時，您可能會超出個別儲存體帳戶的延展性目標，導致 VM 作業失敗。 您應該檢查儲存體帳戶的每分鐘度量 (**TotalRequests**/**TotalIngress**/**TotalEgress**)，查看是否有超出儲存體帳戶延展性目標的流量暴增情況。 請參閱 [[度量顯示 percentthrottlingerror 增加的]] 以協助判斷節流發生儲存體帳戶中。

一般來說，VHD 每次來自虛擬機器的個別流量輸入或輸出，都會在底層的分頁 Blob 上轉換為 **Get Page** 或 **Put Page** 作業。 因此，您可以在環境中使用預估的 IOPS，依據應用程式的特定行為調節單一儲存體帳戶中應該具備的 VHD 數量。 我們不建議單一儲存體帳戶內具備超過 40 的磁碟。 請參閱 <a href="http://msdn.microsoft.com/library/azure/dn249410.aspx" target="_blank">Azure 儲存體延展性和效能目標</a> 的儲存體帳戶目前延展性目標的詳細資訊，特別是儲存體帳戶類型的總要求率與總頻寬使用。
...
- [功能"X"未能發揮作用中的儲存體模擬器]
- ["其中一個 HTTP 標頭的值不正確的格式 」 錯誤時使用的儲存體模擬器]
- [執行儲存體模擬器需要系統管理權限]

#### <a name="feature-X-is-not-working"></a>功能"X"未能發揮作用中的儲存體模擬器

儲存體模擬器並未支援所有的 Azure 儲存體服務，例如檔案服務。 如需詳細資訊，請參閱 <a href="http://msdn.microsoft.com/library/azure/gg433135.aspx" target="_blank">差異之間儲存體模擬器和 Azure 儲存體服務</a> MSDN 上。

如需了解儲存體模擬器不支援哪些功能，請使用雲端裡的 Azure 儲存體服務。

#### <a name="error-HTTP-header-not-correct-format"></a>「 其中一個 HTTP 標頭的值不正確的格式 」 錯誤時使用的儲存體模擬器
...
如需詳細資訊，請參閱 <a href="http://msdn.microsoft.com/library/azure/gg433132.aspx" target="_blank">使用命令列工具初始化儲存體模擬器</a> msdn (您也可以初始化儲存體模擬器，在 Visual Studio 中也會需要系統管理權限)。

### <a name="you-are-encountering-problems-installing-the-Windows-Azure-SDK"></a>您所遇到安裝 Azure SDK for.NET 時發生問題
...
### <a name="you-have-a-different-issue-with-a-storage-service"></a>您有儲存體服務出現其他問題
...
如需有關如何使用 Fiddler 的詳細資訊，請參閱 「 [附錄 1: 使用 Fiddler 擷取 HTTP 與 HTTPS 流量]。 」
...
- 儲存擷取的流量資料。 在主功能表上，前往 [檔案]****，按一下 [儲存]****，然後按一下 [所有工作階段]****：這麼做可讓您將流量儲存在「工作階段封存」檔案中。 您稍後可以重新載入工作階段封存以供分析，或是因應要求傳送給 Microsoft 支援服務中心。
...
您也可以選擇應用程式層所顯示的 TCP 資料，方法是以滑鼠右鍵按一下 TCP 資料，然後選取 [Follow TCP Stream]****。 當您不使用擷取篩選器而擷取到傾印時，這個方法會特別有用。 請參閱 <a href="http://www.wireshark.org/docs/wsug_html_chunked/ChAdvFollowTCPSection.html" target="_blank">這裡</a> 如需詳細資訊。
...
如需有關 Microsoft Message Analyzer **Web Proxy** 追蹤，請參閱 <a href="http://technet.microsoft.com/library/jj674814.aspx" target="_blank">Pef-webproxy 提供者</a> TechNet 上。
...
如需有關 Microsoft Message Analyzer 本機連結層追蹤的詳細資訊，請參閱 <a href="http://technet.microsoft.com/library/jj659264.aspx" target="_blank">PEF-NDIS-PACKETCAPTURE 功能提供者</a> TechNet 上。

### <a name="appendix-4"></a>附錄 4: 使用 Excel 檢視度量與記錄資料

許多工具都可讓您從 Azure 資料表儲存體中下載使用分隔格式的儲存體度量資料，方便您將資料載入 Excel 以供檢視及分析。 來自 Azure Blob 儲存體的儲存體記錄資料已經使用分隔格式，方便您直接載入 Excel。 不過，您將需要新增適當的資料行標題中的資訊基礎 <a href="http://msdn.microsoft.com/library/azure/hh343259.aspx" target="_blank">儲存體分析記錄格式</a> 和 <a href="http://msdn.microsoft.com/library/azure/hh343264.aspx" target="_blank">儲存體分析度量資料表結構描述</a>。
...
本文撰寫期間，Application Insights 已經進入預覽階段。 您可以找到詳細資訊，在 <a href="http://msdn.microsoft.com/library/azure/dn481095.aspx" target="_blank">MSDN 上的 Visual Studio Team 服務的 Application Insights</a>。
