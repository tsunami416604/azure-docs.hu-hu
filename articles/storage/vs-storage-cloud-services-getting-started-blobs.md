...
本文描述如何在 Visual Studio 雲端服務專案中使用 [**加入已連接服務**] 對話方塊建立或參考 Azure 儲存體帳戶後，開始搭配使用 Azure Blob 儲存體。 我們會說明如何存取及建立 Blob 容器，以及如何執行上傳、列出和下載 Blob 等一般工作。 在 C# 中並使用這些範例 [Azure Storage Client Library for.NET](https://msdn.microsoft.com/library/azure/dn261237.aspx)。
...
> [AZURE.NOTE] 有些對外向 ASP.NET 中 Azure 儲存體執行呼叫的 API 是非同步的。 請參閱 [使用 Async 和 Await 進行非同步程式設計](http://msdn.microsoft.com/library/hh191443.aspx) 如需詳細資訊。 以下範例中的程式碼假設使用非同步程式設計方法。
...
若要列出容器中的 Blob，請先取得容器參照。 然後您即可使用容器的 **ListBlobs** 方法來擷取 Blob 和 (或) 其中的目錄。 若要存取一組豐富的屬性和方法傳回 **IListBlobItem**, ，您必須進行轉換， **CloudBlockBlob**, ，**CloudPageBlob**, ，或 **CloudBlobDirectory** 物件。 如果不清楚類型，可使用類型檢查來決定要將其轉換至何種類型。 下列程式碼示範如何擷取和輸出 **photos** 容器中每個項目的 URI：
...
**true**. 如此會導致不論目錄為何，都將每個 Blob 當成 **CloudBlockBlob** 傳回。 以下是對 **ListBlobs** 的呼叫︰
