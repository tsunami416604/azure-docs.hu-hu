...
您可以選擇當小時度量超出特定閥值時，電子郵件的每小時度量，您想要顯示在 Azure 傳統入口網站，並設定通知系統管理員的規則 (如需詳細資訊，請參閱頁面 <a href="http://msdn.microsoft.com/library/azure/dn306638.aspx" target="_blank">How to: 接收警示通知及管理在 Azure 中的警示規則</a>)。 儲存體服務會盡其所能收集各項度量，但是不一定會記錄每一次的儲存體操作。

下圖 2 顯示「Azure 傳統入口網站」中的「監視」頁面，您可以在此頁面檢視度量，例如可用性、要求總計，以及儲存體帳戶的平均延遲數。 同時設定了一個通知規則，會在可用性降至特定水準以下時，通知管理員。 從檢視這項資料，其中一個可能的區域，以便調查是資料表服務成功百分比低於 100%(如需詳細資訊，請參閱 「 [度量顯示低 PercentSuccess 或分析記錄檔項目的交易狀態為 ClientOtherErrors 的作業]")。
...
- **AverageE2ELatency** 與 **AverageServerLatency** 中的值顯示儲存體服務或是 API 操作類型用來處理要求所需的平均時間。 **AverageE2ELatency** 是一項端對端延遲量值，內含讀取要求及傳送回應所花的時間，以及處理要求所花的時間 (因此，內含一旦要求抵達儲存體服務時的網路延遲)；**AverageServerLatency** 指示一項處理時間量值，因此會將任何與用戶端通訊相關的網路延遲排除在外。 請參閱一節 「 [度量顯示高 AverageE2ELatency 與低 AverageServerLatency] 」 原因的討論本指南稍後可能會有顯著的差異，這兩個值之間。
...
用戶端延遲傳送要求的可能原因，包括可用的連線或執行緒數量有限。 您應該同時檢查用戶端是否執行多次的重試，並調查是否真是如此。 您可以以程式設計方式查看與該要求相關聯的 **OperationContext** 物件，並擷取 **ServerRequestId** 值來執行這項調查。 如需詳細資訊，請參閱 「 [伺服器要求 ID]。 」 一節中的程式碼範例
...
- SAS 金鑰中的版本參數 (例如 **sv=2012-02-12**) 是否與您所使用的儲存體用戶端程式庫版本相符。 您應該一律使用最新版的儲存體用戶端程式庫。 如需有關 SAS 權杖版本以及用戶端程式庫版本相依性，請參閱 <a href="http://blogs.msdn.com/b/windowsazurestorage/archive/2014/05/14/what-s-new-for-microsoft-azure-storage-at-teched-2014.aspx" target="_blank">Microsoft Azure 儲存體的最新消息</a>。
