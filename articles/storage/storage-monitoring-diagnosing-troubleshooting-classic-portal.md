...
下圖 2 顯示「Azure 傳統入口網站」中的「監視」頁面，您可以在此頁面檢視度量，例如可用性、要求總計，以及儲存體帳戶的平均延遲數。 同時設定了一個通知規則，會在可用性降至特定水準以下時，通知管理員。 從檢視這項資料，其中一個可能的區域，以便調查是資料表服務成功百分比低於 100%(如需詳細資訊，請參閱 「[Metrics show low PercentSuccess or analytics log entries have operations with transaction status of ClientOtherErrors]」)。
...
您可以使用 [Azure 傳統入口網站](manage.windowsazure.com) 來檢視全球各個 Azure 區域中的儲存體服務 （和其他 Azure 服務） 的健全狀況。 藉此可以立即瞭解，不受您控制的問題所影響的區域，是否也涵蓋了您為應用程式使用儲存體服務區域。
...
> [AZURE.NOTE] 您應該監視這些數值，以便在接近儲存體帳戶的容量限制時早期預警。 在 Azure 傳統入口網站，在 **監視** 頁面，您可以針對儲存體帳戶，新增警示規則，以便彙總儲存體使用量超出或低於您指定的閥值時通知您。
...
在 Azure 傳統入口網站，在 **監視** 頁面，您可以針對儲存體帳戶，新增警示規則，以便通知您，如果 **可用性** 服務低於您指定的閥值。
...
- 中的值 **AverageE2ELatency** 和 **AverageServerLatency** 顯示的平均時間的儲存體服務或是 API 操作類型來處理要求。 **AverageE2ELatency** 是端對端延遲，內含讀取要求並傳送回應的時間來處理要求所花費的時間量值 （因此包括網路延遲一旦要求抵達儲存體服務）; **AverageServerLatency** 的處理時間量值，因此會排除任何與用戶端通訊相關的網路延遲。 請參閱一節 「[Metrics show high AverageE2ELatency and low AverageServerLatency]」 原因的討論本指南稍後可能會有顯著的差異，這兩個值之間。
...
在 Azure 傳統入口網站，在 **監視** 頁面，您可以針對儲存體帳戶，新增警示規則，以便將任何此服務的效能度量低於或超過您指定的閥值時通知您。
...
Azure 傳統入口網站監視工具的下方圖表示範其中 **AverageE2ELatency** 遠遠高 **AverageServerLatency**。
...
用戶端延遲傳送要求的可能原因，包括可用的連線或執行緒數量有限。 您應該同時檢查用戶端是否執行多次的重試，並調查是否真是如此。 您可以透過程式設計方式查看 **OperationContext** 與要求相關聯的物件，並擷取 **ServerRequestId** 值。 如需詳細資訊，請參閱下一節中的程式碼範例 」[Server request ID]。 」
...
> [AZURE.NOTE] 您可以找到完整的檢查清單包括要注意的其他問題: 「[設計可擴充且高效能儲存體為基礎的應用程式檢查清單](storage-performance-checklist.md)。 」
...
- SAS 金鑰中的版本參數 (例如 **sv = 2012年-02-12**) 符合您所使用的儲存體用戶端程式庫版本。 您應該一律使用最新版的儲存體用戶端程式庫。 如需有關 SAS 權杖版本設定及用戶端程式庫版本相依性的詳細資訊，請參閱 <a href="http://blogs.msdn.com/b/windowsazurestorage/archive/2014/05/14/what-s-new-for-microsoft-azure-storage-at-teched-2014.aspx" target="_blank">Microsoft Azure 儲存體的新功能</a>。
- 如果您重新產生儲存體存取金鑰 (按一下 **管理存取金鑰** Azure 傳統入口網站中的儲存體帳戶中的任何頁面上) 會讓任何現有的 SAS 權杖失效。 如果您產生的 SAS 權杖，內含很長的到期時間以便用戶端應用程式快取處理，則可能會出現問題。
...
- **ConditionNotMet** (未修改 304)，例如來自條件式作業，例如當用戶端傳送 **ETag** 值與 HTTP **If-none-Match** 標頭以要求映像，只有當上次作業後已經更新它。
...
當 Azure 虛擬機器 (VM) 裡有大量的附加 VHD 位於同一個儲存體帳戶內時，您可能會超出個別儲存體帳戶的延展性目標，導致 VM 作業失敗。 您應該檢查儲存體帳戶的每分鐘度量 (**TotalRequests**/**TotalIngress**/**TotalEgress**) 超出儲存體帳戶的延展性目標的流量暴增情況。 請參閱一節 「[Metrics show an increase in PercentThrottlingError]」 儲存體帳戶是否發生在判斷節流協助的。

一般情況下，每個個別的輸入或輸出作業，從虛擬機器的 VHD 上會轉譯為 **Get Page** 或 **Put Page** 底層的分頁 blob 上的作業。 因此，您可以在環境中使用預估的 IOPS，依據應用程式的特定行為調節單一儲存體帳戶中應該具備的 VHD 數量。 我們不建議單一儲存體帳戶內具備超過 40 的磁碟。 如需儲存體帳戶目前延展性目標的詳細資訊，尤其是您所使用之儲存體帳戶類型的要求率總計和頻寬總計，請參閱 <a href="http://msdn.microsoft.com/library/azure/dn249410.aspx" target="_blank">Azure 儲存體延展性和效能目標</a>。
...
您要測試應用程式使用儲存體用戶端程式庫，本機儲存體模擬器和方法呼叫，例如 **CreateIfNotExists** 失敗錯誤訊息 「 其中一個 HTTP 標頭值不正確的格式 」。 這表示您所使用的儲存體模擬器版本，並不支援您所使用的儲存體用戶端程式庫版本。 儲存體用戶端程式庫加入標頭 **x ms 版本** 到它所提出的所有要求。 如果儲存體模擬器無法辨識中的值 **x ms 版本** 標頭，就會拒絕要求。

您可以使用儲存體程式庫用戶端記錄檔以查看值 **x ms 版本標頭** 它所傳送。 您也可以看到的值 **x ms 版本標頭** 如果您使用 Fiddler 追蹤用戶端應用程式的要求。
 ...
 **刪除** 指令會移除先前安裝的儲存體模擬器中的任何舊的資料庫檔案。
...
如需有關使用 Wireshark 的詳細資訊，請參閱 「[Appendix 2: Using Wireshark to capture network traffic]。 」

如需有關使用 Microsoft Message Analyzer 的詳細資訊，請參閱 「[Appendix 3: Using Microsoft Message Analyzer to capture network traffic]。 」
...
> [AZURE.NOTE] Fiddler 可以解碼 HTTPS 流量。您應該閱讀 Fiddler 文件，仔細了解其運作原理，並了解安全性含意。
...
- 停止與開始擷取流量。 在主功能表中，移至 **檔案** 然後按一下 [ **擷取流量** 來切換開啟及關閉擷取。
- 儲存擷取的流量資料。 在主功能表中，移至 **檔案**, ，按一下 [ **儲存**, ，然後按一下 [ **所有工作階段**︰ 這可讓您將流量儲存在工作階段封存檔。 您稍後可以重新載入工作階段封存以供分析，或是因應要求傳送給 Microsoft 支援服務中心。

若要限制 Fiddler 所擷取的流量數量，您可以使用您在設定的篩選器 **篩選** ] 索引標籤。 下圖顯示 [擷取流量傳送到篩選 **contosoemaildist.table.core.windows.net** 儲存體端點 ︰
...
2.  在 **啟動** 區段中，選取本機網路介面或是連線到網際網路的介面。
3.  按一下 [ **擷取選項**。
4.  加入篩選以 **擷取篩選器** 文字方塊。 例如， **裝載 contosoemaildist.table.core.windows.net** 將 Wireshark 設定為擷取只或從資料表服務端點中傳送的封包 **contosoemaildist** 儲存體帳戶。 如需「擷取篩選器」的完整清單，請參閱 <a href="http://wiki.wireshark.org/CaptureFilters" target="_blank">http://wiki.wireshark.org/CaptureFilters</a>。

    ![][6]

5.  按一下 [ **啟動**。 現在當您於本機電腦上使用用戶端應用程式時，Wireshark 會開始擷取流入/流出資料表服務端點的所有封包。
6.  當您完成時，在主功能表中按一下 **擷取** 然後 **停止**。
7.  若要將擷取的資料儲存在 Wireshark 擷取檔案，在主功能表上按一下 **檔案** 然後 **儲存**。

WireShark 會反白顯示存在於任何錯誤 **packetlist** 視窗。 您也可以使用 **Expert Info** 視窗 (按一下 **分析**, ，然後 **Expert Info**) 若要檢視的錯誤和警告摘要。

![][7]

您也可以選擇檢視 TCP 資料，應用程式層所顯示的 TCP 資料上按一下滑鼠右鍵，然後選取 **Follow TCP Stream**。 當您不使用擷取篩選器而擷取到傾印時，這個方法會特別有用。 如需詳細資訊，請參閱<a href="http://www.wireshark.org/docs/wsug_html_chunked/ChAdvFollowTCPSection.html" target="_blank">這裡</a>。
...
若要設定 web 追蹤工作階段使用 Microsoft Message Analyzer 中，執行 Microsoft Message Analyzer 應用程式的 HTTP 和 HTTPS 流量，然後在 **檔案** ] 功能表上，按一下 [ **Capture/Trace**。 在可用的追蹤案例清單中選取 **Web Proxy**。 接著在 **Trace Scenario Configuration** ] 面板的 **HostnameFilter** 文字方塊中，加入您的儲存體端點名稱 （您可以查詢這些名稱在 Azure 傳統入口網站）。 例如，如果您的 Azure 儲存體帳戶名稱是 **contosodata**, ，您應該將下列內容加入 **HostnameFilter** 文字方塊 ︰

    contosodata.blob.core.windows.net contosodata.table.core.windows.net contosodata.queue.core.windows.net

> [AZURE.NOTE] 空格字元分隔了主機名稱。

當您準備好收集追蹤資料，請按一下 [ **Start With** ] 按鈕。
...
內建 **Web Proxy** 追蹤在 Microsoft Message Analyzer 依據 fiddler 來設計，可以擷取用戶端 HTTPS 流量，並顯示未加密的 HTTPS 訊息。  **Web Proxy** 追蹤功能會藉由設定本機 proxy，針對所有 HTTP 和 HTTPS 流量，讓它存取未加密的訊息。

#### 使用 Microsoft Message Analyzer 診斷網路問題

除了使用 Microsoft Message Analyzer **Web Proxy** 追蹤功能來擷取用戶端應用程式與儲存體服務之間的 HTTP/HTTPs 流量詳細資料，您也可以使用內建 **本機連結層** 追蹤功能來擷取網路封包資訊。 這麼做可讓您擷取到類似於使用 Wireshark 所擷取的資料，並診斷捨棄的封包之類的網路問題。

下列螢幕擷取畫面顯示範例 **本機連結層** 追蹤某些 **參考** 訊息 **DiagnosisTypes** 資料行。 中的圖示上按一下 **DiagnosisTypes** 欄會顯示訊息的詳細資料。 在以下範例中，由於伺服器並未收到來自用戶端的認可，因此重新傳送訊息 #305：

![][9]

當您在 Microsoft Message Analyzer 中建立追蹤工作階段時，可以指定篩選器來減少追蹤所產生的雜訊。 在 **Capture / Trace** 可讓您定義追蹤頁面按一下 **設定** 連結 **Microsoft Windows-功能**。 下列螢幕擷取畫面顯示針對三個儲存體服務的 IP 位址進行 TCP 流量篩選的組態：
...
- 在 **資料** ] 功能表上，按一下 [ **從文字**。
- 瀏覽至您想要檢視並按一下 [記錄檔 **匯入**。
- 在步驟 1 的 **Text Import Wizard**, ，請選取 **分隔**。

在步驟 1 的 **Text Import Wizard**, ，請選取 **分號** 為唯一的分隔字元並選擇雙引號作為 **文字限定詞**。 然後按一下 [ **完成** 並選擇將資料放在您的活頁簿中的位置。
