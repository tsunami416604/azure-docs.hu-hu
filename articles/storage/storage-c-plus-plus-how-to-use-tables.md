...
本指南將示範如何使用 Azure 資料表儲存體服務執行一般案例。 在 c + + 中並使用這些範例 [Azure Storage Client Library for c + +](https://github.com/Azure/azure-storage-cpp/blob/v1.0.0/README.md)。 所涵蓋的案例包括**建立和刪除資料表**，以及**使用資料表實體**。
...
-   **Windows：**在 Visual Studio 中，按一下 [工具] > [NuGet 套件管理員] > [套件管理員主控台]****。 輸入下列命令到 [NuGet Package Manager console](http://docs.nuget.org/docs/start-here/using-the-package-manager-console) 按下 Enter。
...
-   [儲存體用戶端程式庫的 c + + 參考](http://azure.github.io/azure-storage-cpp)
