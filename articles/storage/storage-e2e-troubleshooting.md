...
- **Azure 入口網站**。 您可以在儲存體帳戶設定的度量和記錄 [Azure 入口網站](portal.azure.com)。 您也可以檢視圖表和圖形 (其中顯示應用程式在一段時間內的執行狀況)，並設定警示，通知應用程式是否對指定的度量以超乎預期的方式執行。

    請參閱 [如何監視儲存體帳戶](storage-monitor-storage-account.md) 如需設定 Azure 網站中的監視資訊。
...
首先，我們必須設定 Azure 儲存體記錄和度量，如此我們才有用戶端應用程式提供的資料可分析。 您可以透過設定記錄和度量以各種方式- [Azure 入口網站](portal.azure.com), ，使用 PowerShell，或以程式設計的方式。 請參閱 [啟用儲存體度量和檢視度量資料](http://msdn.microsoft.com/library/azure/dn782843.aspx) 和 [啟用儲存體記錄和存取記錄檔資料](http://msdn.microsoft.com/library/azure/dn782840.aspx) msdn 如需設定記錄和度量的詳細資訊。

**透過 Azure 入口網站**

若要設定記錄和度量的儲存體帳戶使用 [Azure 入口網站](portal.azure.com), ，遵循 [如何監視儲存體帳戶](storage-monitor-storage-account.md)。
...
一旦您的應用程式已執行了一段時間，您可以檢閱的度量圖表，會出現在 [Azure 入口網站](portal.azure.com) 觀察如何會執行您的服務。 首先，瀏覽至您在 Azure 入口網站中的儲存體帳戶，然後加入**成功百分比**度量的圖表。
...
下圖顯示 [檢視版面配置]**** 功能表，此功能表可從工具列功能區選取 [檢視版本配置]**** 取得。 Azure 儲存體的檢視版本配置位於功能表的 [Azure 儲存體]**** 節點之下。 您可以搜尋 `Azure 儲存體` 在搜尋方塊中篩選 Azure 儲存體檢只。 您也可以選取檢視版面配置旁邊的星號，使其成為我的最愛並顯示在功能表的頂端。
...
用戶端記錄檔資料列不會顯示任何資料 **時間戳記**, ，**TimeElapsed**, ，**來源**, ，和 **目的地** 資料行，因為這些資料行不存在於用戶端記錄檔，但存在於網路追蹤。 同樣地，**Timestamp** 資料行會顯示伺服器記錄檔中的時間戳記資料，但不會顯示 **TimeElapsed**、**Source** 和 **Destination** 資料行 (不屬於伺服器記錄檔) 的資料。 
...
1. 如果 [檢視篩選條件] 工具視窗尚未顯示，請予以顯示 。 在工具列功能區中，選取 [工具視窗]****，然後選取 [檢視篩選條件]****。
2. 在 [檢視篩選器] 視窗中，選取 **程式庫**, ，然後搜尋 `Azure 儲存體` 以尋找 Azure 儲存體篩條件。 選取**所有記錄檔中的 404 (找不到) 訊息**的篩選條件。
...
| Nagle 警告| ((AzureStorageLog.EndToEndLatencyMS-AzureStorageLog.ServerLatencyMS) > (AzureStorageLog.ServerLatencyMS * 1.5)) 和 (AzureStorageLog.RequestPacketSize <1460) and (AzureStorageLog.EndToEndLatencyMS -   AzureStorageLog.ServerLatencyMS >= 200)| 伺服器|
| 在伺服器和網路中的時間範圍內的記錄| #Timestamp > = 2014年-10-20T16:36:38 和 #Timestamp < = 2014年-10-20T16:36:39| 伺服器、網路|
...
- [監視、 診斷與疑難排解儲存體](storage-monitoring-diagnosing-troubleshooting.md)
