...
* **Georeplikáció:** Az Azure Storage tárolókról georeplikálás készíthető. Bár ez földrajzi helyreállítást és adatredundanciát biztosít, a georeplikált helyre végzett feladatátvétel súlyos hatással van a teljesítményre, és további költségekkel járhat. Így a javasoljuk, hogy a georeplikáció georeplikációt válassza, és csak érték esetén az adatok megérik a további költségeket.
* **Életciklus-felügyeletének:** bármely fájlrendszerben lévő összes adatot a saját életciklusuk nagyon értékes és gyakran használt kisebb érték révén, kevesebb archiválása vagy törlése keresztül érhetők el, amely nem kerül. Az Azure Storage biztosít adatokat rétegezéséhez és életciklus adatok megfelelően réteg esetében a életciklusfázis-kezelési házirenddel.
...
> [!NOTE]
> A legtöbb HDFS parancs (például `ls`, `copyFromLocal` és `mkdir`) továbbra is a várt módon működik. Csak azok a parancsok, amelyek például a natív HDFS implementációra (más nevezzük DFS), az adott `fschk` és `dfsadmin`, az Azure storage másképp viselkednek megjelenítése.
...
> [!NOTE]
> A nyilvános előzetes a Data Lake tárolási Gen2 csak `--type LRS` esetén támogatott. További redundancia lehetőségek egész előzetes programban elérhető lesz.
...
A &lt;fájlrendszer&gt; elérési útját a fájlrendszerben Azure Data Lake tárolási azonosítja.
A &lt;AccountName&gt; azonosítja az Azure Storage-fiók nevét. Szükség van a teljes tartománynévre (FQDN-re).

Ha értékei &lt;fájlrendszer&gt; sem &lt;AccountName&gt; van megadva, az alapértelmezett fájlrendszert használja. Az alapértelmezett fájlrendszeren tárolt fájlok esetén relatív elérési utat vagy abszolút elérési utat használhat. Például a *hadoop-mapreduce-examples.jar* a HDInsight-fürtök fájlt a következő elérési utak egyikének használatával lehet hivatkozni:
...
Az &lt;elérési út&gt; a fájl vagy könyvtár HDFS elérési útja neve.
