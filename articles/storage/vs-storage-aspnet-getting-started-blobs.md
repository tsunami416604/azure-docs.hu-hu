...
> [AZURE.NOTE] 對外向 ASP.NET 5 中的 Azure 儲存體執行呼叫的部分 API 是非同步的。 請參閱 [使用 Async 和 Await 進行非同步程式設計](http://msdn.microsoft.com/library/hh191443.aspx) 如需詳細資訊。
...
**true**. 無論目錄為何，都會以 **CloudBlockBlob** 傳回每個 Blob。 下列範例顯示對 **ListBlobs** 的呼叫。
