...
根據預設，儲存體服務未啟用 [儲存體度量]。 您可以啟用監視使用 [Azure 傳統入口網站](manage.windowsazure.com), ，Windows PowerShell 或以程式設計方式透過儲存體 API。
...
## 如何使用 Azure 傳統入口網站啟用儲存體度量

在 [Azure 傳統入口網站](manage.windowsazure.com), ，您可以使用 [設定] 頁面來控制儲存體度量的儲存體帳戶。 如需監視，您可以針對每個 Blob、資料表和佇列設定層級和保留期限 (以天為單位)。 在各個案例中，層級會是下列其中一項：
...
您應該考慮在 Azure 傳統入口網站的 [監視] 頁面上設定警示，讓儲存體度量可自動通知您儲存體服務行為的任何重要變更。如果您使用儲存體總管工具來下載這個度量資料 (以分隔的格式)，就可以使用 Microsoft Excel 來分析資料。 請參閱部落格文章 [Microsoft Azure 儲存體總管](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx) 可用的存放裝置總管工具的清單。
...
[啟用儲存體分析記錄和存取記錄資料](https://msdn.microsoft.com/library/dn782840.aspx)
