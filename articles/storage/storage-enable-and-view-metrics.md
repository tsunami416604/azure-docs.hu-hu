...
根據預設，儲存體服務未啟用 [儲存體度量]。 您可以啟用透過監視 [Azure 入口網站](portal.azure.com) 或 Windows PowerShell 或以程式設計方式透過儲存體用戶端程式庫。
...
請注意， [Azure 入口網站](portal.azure.com) 目前無法讓您設定每分鐘度量，在您的儲存體帳戶，您必須啟用每分鐘度量使用 PowerShell 或以程式設計的方式。
...
您應該考慮中的警示設定 [Azure 入口網站](portal.azure.com) 監視頁面上，讓儲存體度量可自動通知您的儲存體服務行為中的任何重要變更。如果您使用儲存體總管工具來下載這個度量資料，以分隔格式，您可以使用 Microsoft Excel 來分析資料。 請參閱部落格文章 [Microsoft Azure 儲存體總管](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx) 可用的存放裝置總管工具的清單。
