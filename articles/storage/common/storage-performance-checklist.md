...
Maga az Azure Storage régiónként és előfizetésenként legfeljebb 250 tárfiókot tartalmaz. Ha eléri ezt a korlátot, nem hozhat létre további tárfiókokat ebben az előfizetésben ebben a régióban.
...
Az Azure Storage egy tartományalapú particionálási sémát, és méretezés és a terhelés elosztása a rendszer használ. A partíciókulcs (fiók + tároló + blob) adatok tartományok és a tartományok terhelés-között oszlanak el a rendszer partíció szolgál. Ez azt jelenti, például lexikai rendezése elnevezési konvenciók (például *mypayroll*, *myperformance*, *myemployees*használatához és így tovább) vagy az időbélyegek használata ( *log20160101*, *log20160102*, *log20160102*használatához és így tovább) fog jellegűvé folyamatban potenciálisan közös elhelyezésű ugyanazon a kiszolgálón partíció, amíg a partíciók egy terheléselosztási művelet bontja ki azokat, kisebb tartományt. Például egy tárolóban lévő összes BLOB kiszolgálható egyetlen kiszolgáló, mindaddig, amíg a terhelés, az ilyen blobok igényel, további újra terheléselosztás, a partíció-címtartományok. Ehhez hasonlóan kevésbé terhelt fiókokat a nevük lexikai sorrendbe rendezve csoportja kiszolgálása az internetszolgáltatójuk egyetlen kiszolgáló csak az egyik terhelését, vagy ezek a fiókok mindegyikét hitelesíteni feloszthatók több partíció-kiszolgáló. Minden terheléselosztási művelet közben a művelet hatással lehet a storage hívások késése. Mindaddig, amíg a terheléselosztási művelet megrúgni a, és újra elosztja a partíciókulcs-tartományok korlátozza egy egypartíciós kiszolgáló méretezhetőségét partíciók adatforgalom hirtelen kezeli a rendszer riasztásészlelési képességére.

Az ilyen műveletek gyakoriságának csökkentése érdekében néhány ajánlott eljárást is követheti.  

* Ha lehetséges használjon (nagyobb, mint a standard szintű fiók 4 MiB és nagyobb, mint 256 KiB premium-fiókok) nagyobb Put Blob- vagy Blokkelraktározási méretek nagy átviteli sebességű Block Blob (HTBB) aktiválásához. HTBB biztosít magas teljesítményt, amely nem befolyásolja a partíció-elnevezési betöltési.
* Vizsgálja meg az elnevezési szabályok tartozó fiókokat, tárolók, blobok, táblák és üzenetsorok, szorosan. Fontolja meg a fiók, tároló vagy a blob neve előtag használatával a kivonatolási függvény, amely a legjobban az igényeinek megfelelő 3 számjegyből kivonattal.  
* Ha, ahogy az időbélyegek vagy numerikus azonosítók használata esetén az adatok, akkor győződjön meg arról, nem használ egy csak hozzáfűzéssel bővíthető (vagy csak illesztenie) forgalmi minták. Ezek a minták nem alkalmasak a tartomány-alapú particionálási rendszert, és sikerült érdeklődőt fogja egy partíció, és hatékonyan korlátozza a rendszer az összes forgalom a terheléselosztás. Például ha egy blob objektumot használó időbélyegzővel ellátott például napi üzemeltetési *ÉÉÉÉHHNN*, akkor az a napi művelet összes forgalom irányul, egyetlen objektum, amely egy partíció-kiszolgáló által kiszolgált. Keresse meg, hogy a blob korlátokat partíciónként korlátok az igényeknek, és érdemes feldarabolni Ez a művelet több blobokba, ha szükséges. Ehhez hasonlóan ha idősorozat-adatok tárolása a táblák, mind a forgalom irányítja az utolsó kulcs névtér a része. Időbélyegeket vagy numerikus azonosítót kell használnia, ha 3 számjegyből kivonattal, illetve időbélyegeket előtag azonosítója például előtag-e az időt a másodperc részét *ssyyyymmdd*. Ha listázás és lekérdezés műveleteket rendszeresen hajtja végre, válassza ki a kivonatoló függvényt, amely korlátozza a lekérdezések száma. Egyéb esetben elegendő lehet egy véletlenszerű előtagot.  
* További információk az Azure Storage-ban használt particionálási sémát, [Azure Storage: Erős konzisztencia magas rendelkezésre állású Felhőbeli Tárolószolgáltatásba](https://sigops.org/sosp/sosp11/current/2011-Cascais/printable/11-calder.pdf).
...
#### <a name="subheading10"></a>Növeli a szálak minimális számát, ha szinkron kóddal az aszinkron feladatok

Ez a kód növeli a szálkészletben lévő szálak minimális száma:  
...
#### <a name="subheading11"></a>.NET 4.5-ös és újabb szemétgyűjtés előnyeit
...
#### <a name="subheading14"></a>Szabályozás és a kiszolgáló foglaltsága miatti hibák
...
#### <a name="subheading15"></a>Nem újrapróbálható hiba
...
### <a name="rapid-uploading"></a>Gyors feltöltése

A blobfeltöltéshez gyorsan, az első kérdést megválaszolni van: Ön feltöltése a blob egy vagy több?  Használja az alábbi útmutatás meg, hogy a megfelelő módszert a forgatókönyvtől függően.  
...
#### <a name="entities-per-second-storage-account"></a>Entitások (tárfiók) másodpercenként
...
#### <a name="subheading26"></a>Nagle letiltása
...
##### <a name="limiting-the-amount-of-data-returned"></a>Visszaadott adatok mennyisége korlátozza.
...
### <a name="subheading40"></a>Nagle letiltása
...
Használhatja a **üzenet frissítése** művelet növelheti az láthatatlansági időkorlátot, vagy a állapotadatokat, egy üzenet frissítéséhez. Amíg ez a hatékony, ne feledje, hogy minden egyes **üzenet** művelettel counts a skálázhatósági célok felé. Azonban ez lehet egy sokkal hatékonyabb megközelítést, mint ha egy munkafolyamatot, amely egy feladat a következő egy olyan sort adja át a feladat egyes lépéseinek befejezettként. Használatával a **üzenet** művelet lehetővé teszi, hogy az alkalmazása az üzenetre a feladat állapot mentése és folytathatják a munkát, az üzenet a feladat a következő lépéshez requeuing minden alkalommal, amikor egy lépés befejezi helyett.  
