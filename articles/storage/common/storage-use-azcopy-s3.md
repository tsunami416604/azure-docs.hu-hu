...
> [!NOTE]
> A jelen cikkben szereplő példák az AWS S3-gyűjtők Path stílusú URL-jeit használják (például: `http://s3.amazonaws.com/<bucket-name>`). 
>
...
A AzCopy két leggyakoribb problémát okoz, amelyek felmerülhetnek; az egymást követő kötőjeleket tartalmazó pontokat és gyűjtőket tartalmazó gyűjtők. Az AWS S3-gyűjtők neve tartalmazhat pontokat és egymást követő kötőjeleket, de az Azure-beli tárolók nem. A AzCopy lecseréli a kötőjelekkel és egymást követő kötőjelekkel rendelkező időszakokat egy számmal, amely az egymást követő kötőjelek számát jelöli (például: egy `my----bucket` nevű gyűjtő `my-4-bucket` lesz. 

Továbbá, mivel a AzCopy fájlokba másolja a fájlokat, ellenőrzi a névadási ütközéseket, és megkísérli a megoldását. Ha például vannak olyan gyűjtők, amelyek neve `bucket-name` és a `bucket.name`, a AzCopy a `bucket.name` nevű gyűjtőt először a `bucket-name`, majd a `bucket-name-2` értékre oldja fel.
...
Egy AzCopy `copy` parancs részeként megadhatja az `s2s-invalid-metadata-handle` jelzőt, amely megadja, hogy miként szeretné kezelni azokat a fájlokat, amelyekben a fájl metaadatai nem kompatibilis kulcsok nevét tartalmazzák. A következő táblázat ismerteti az egyes jelző értékeket.
