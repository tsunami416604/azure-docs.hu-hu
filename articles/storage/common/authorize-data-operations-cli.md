...
A blob-és üzenetsor-információk olvasására és írására szolgáló Azure CLI-parancsok tartalmazzák a választható `--auth-mode` paramétert. Adja meg ezt a paramétert, hogy jelezze, hogyan legyen engedélyezve az adatművelet:

- Állítsa be a `--auth-mode` paramétert úgy, hogy `login` az Azure AD rendszerbiztonsági tag (ajánlott) használatával történő bejelentkezéshez.
...
A `--auth-mode` paraméter használatához győződjön meg arról, hogy telepítette az Azure CLI 2.0.46 vagy újabb verzióját. `az --version` futtatásával keresse meg a telepített verziót.

> [!IMPORTANT]
> Ha kihagyja a `--auth-mode` paramétert, vagy beállítja `key`ra, akkor az Azure CLI a fiók hozzáférési kulcsát próbálja használni az engedélyezéshez. Ebben az esetben a Microsoft azt javasolja, hogy a hozzáférési kulcsot a parancson vagy a **AZURE_STORAGE_KEY** környezeti változóban adja meg. A környezeti változókról további információt a [környezeti változók beállítása az engedélyezési paraméterekhez](#set-environment-variables-for-authorization-parameters)című szakaszban talál.
>
...
1. A tároló Azure AD-beli hitelesítő adataival történő létrehozásához hívja meg az az [Storage Container Create](/cli/azure/storage/container#az-storage-container-create) parancsot a `--auth-mode` paraméterrel, amely a `login` értékre van állítva. Ne felejtse el lecserélni a helyőrző értékeket a saját értékeire a szögletes zárójelekben:
...
Az alábbi példa bemutatja, hogyan hozhat létre egy tárolót a fiók hozzáférési kulcsa alapján. Adja meg a fiók kulcsát, és adja meg a `--auth-mode` paramétert a `key` értékkel:
...
|    AZURE_STORAGE_AUTH_MODE            |    Az engedélyezési mód, amellyel a parancsot futtatni kívánja. A megengedett értékek `login` (ajánlott) vagy `key`. Ha `login`ad meg, az Azure CLI az Azure AD-beli hitelesítő adataival engedélyezi az adatok műveletét. Ha az örökölt `key` módot adta meg, az Azure CLI megkísérli lekérdezni a fiók elérési kulcsát, és engedélyezi a parancsot a kulccsal.    |
