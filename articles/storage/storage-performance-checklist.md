<properties
    pageTitle="Azure 儲存體效能與延展性檢查清單 | Microsoft Azure"
...
| 完成| 領域| 類別| 問題
|----|------|-----------|-----------
| | 所有服務| 延展性目標| [您應用程式是否旨在避免達到延展性目標?](#subheading1)
| | 所有服務| 網路| [用戶端裝置是否有足夠高的頻寬和低延遲，以達到所需的效能?](#subheading2)
| | 所有服務| 網路| [用戶端裝置是否有足夠高的品質連結?](#subheading3)
| | 所有服務| 網路| [位於用戶端應用程式 」 附近 「 儲存體帳戶?](#subheading4)
| | 所有服務| 內容發佈| [您使用 CDN 進行內容發佈?](#subheading5)
| | 所有服務| 直接用戶端存取| [您是否使用 SAS 和 CORS 來允許直接存取存放裝置，而不是 proxy?](#subheading6)
| | 所有服務| 快取| [很少是您的應用程式重複使用的快取資料和變更嗎?](#subheading7)
| | 所有服務| 快取| [您的應用程式批次更新 (用戶端快取，然後以大型集合上傳)?](#subheading8)
| | 所有服務| .NET 組態| [您是否已設定您的用戶端使用足夠數量的並行連線?](#subheading9)
| | 所有服務| .NET 組態| [您已設定.NET 使用足夠數量的執行緒?](#subheading10)
| | 所有服務| .NET 組態| [您是否使用.NET 4.5 或更新版本中，已改善記憶體回收?](#subheading11)
| | 所有服務| 平行處理原則| [您是否已確保，讓您在用戶端功能或延展性目標不多載平行處理原則已適當地受到限制?](#subheading12)
| | 所有服務| 工具| [您是否使用最新版的 Microsoft 所提供用戶端程式庫和工具?](#subheading13)
| | 所有服務| 重試| [是您使用指數輪詢重試原則針對節流錯誤和逾時?](#subheading14)
| | 所有服務| 重試| [非可重試的錯誤是您的應用程式是否避免重試?](#subheading15)
| | Blob| 延展性目標| [您的應用程式是否不超過單一 blob 的頻寬或操作延展性目標?](#subheading16)
| | Blob| 複製 Blob| [有效率的方式，都是您複製 blob?](#subheading17)
| | Blob| 複製 Blob| [您是否使用 AzCopy 進行 blob 的大量複製?](#subheading18)
| | Blob| 複製 Blob| [您會使用 Azure 匯入/匯出傳送非常大量的資料嗎?](#subheading19)
| | Blob| 使用中繼資料| [您是否要將 blob 相關的常用中繼資料儲存在其中繼資料?](#subheading20)
| | Blob| 快速上傳| [嘗試快速上傳一個 blob，當您上傳區塊以平行方式?](#subheading21)
| | Blob| 快速上傳| [當嘗試快速上傳多個 blob，您要上傳 blob 以平行方式?](#subheading22)
| | Blob| 正確的 Blob 類型| [您是否使用分頁 blob 或區塊 blob 適當的時候?](#subheading23)
| | 資料表| 延展性目標| [您是否逐漸達到延展性目標的每秒的實體?](#subheading24)
| | 資料表| 組態| [您是否使用 JSON 來處理資料表要求?](#subheading25)
| | 資料表| 組態| [您已關閉 Nagle 以提高小型要求的效能?](#subheading26)
| | 資料表| 資料表和資料分割| [您已正確分割您的資料?](#subheading27)
| | 資料表| 常用資料分割| [您是否避免只附加和僅前面加上模式?](#subheading28)
| | 資料表| 常用資料分割| [您的插入/更新是否散佈到許多資料分割?](#subheading29)
| | 資料表| 查詢範圍| [您是否設計結構描述以允許在大部分情況下，使用點查詢，並盡量少用資料表查詢?](#subheading30)
| | 資料表| 查詢密度| [您是否查詢通常只掃描並傳回應用程式將使用的資料列?](#subheading31)
| | 資料表| 限制傳回資料| [您是否使用篩選來避免傳回不需要的實體?](#subheading32)
| | 資料表| 限制傳回資料| [您是否使用投射來避免傳回不需要的屬性?](#subheading33)
| | 資料表| 反正規化| [以便嘗試取得資料時，避免無效率的查詢或多個讀取的要求有反正規化資料?](#subheading34)
| | 資料表| 插入/更新/刪除| [您是否批次要求必須是交易式或可同時完成以減少反覆存取的相同時間是否?](#subheading35)
| | 資料表| 插入/更新/刪除| [您是否避免擷取實體，以判斷是否要呼叫插入或更新?](#subheading36)
| | 資料表| 插入/更新/刪除| [您是否考慮系列會經常擷取的資料一起儲存在儲存單一實體而不是多個實體的屬性?](#subheading37)
| | 資料表| 插入/更新/刪除| [針對總是會一起擷取並可以撰寫批次 (例如時間序列資料) 的實體，您是否考慮使用 blob，而不資料表?](#subheading38)
| | 佇列| 延展性目標| [您是否逐漸達到延展性目標的每秒的訊息?](#subheading39)
| | 佇列| 組態| [您已關閉 Nagle 以提高小型要求的效能?](#subheading40)
| | 佇列| 訊息大小| [您的訊息是否精簡以提高佇列效能?](#subheading41)
| | 佇列| 大量擷取| [您擷取在單一"Get"作業中的多個訊息?](#subheading41)
| | 佇列| 輪詢頻率| [您是否夠常輪詢以降低可察覺的應用程式延遲?](#subheading42)
| | 佇列| 更新訊息| [您是否使用 UpdateMessage 來儲存處理訊息時的進度，避免發生錯誤時，重新處理整個訊息?](#subheading43)
| | 佇列| 架構| [您會使用佇列來保留長時間執行的工作負載排除在重要路徑，讓整個應用程式更有彈性外，然後再個別嗎?](#subheading44)
...
-   [每秒資料表實體](#subheading24)
-   [每秒佇列訊息](#subheading39)

#### <a name="sub1bandwidth"></a>所有服務的頻寬延展性目標

本文撰寫期間，美國地理區域備援儲存體 (GRS) 帳戶的輸入 (傳送至儲存體帳戶的資料) 頻寬目標是每秒 10 GB (Gbps)，輸出 (從儲存體帳戶傳送出去的資料) 頻寬目標是 20 Gbps。 若是本機備援儲存體 (LRS) 帳戶，則會有較高的限制 – 輸入是 20 Gbps，輸出是 30 Gbps。 國際頻寬限制可能較低，請參閱我們 [延展性目標頁面](http://msdn.microsoft.com/library/azure/dn249410.aspx)。 如需有關儲存體備援選項的詳細資訊，請參閱中的連結 [有用資源](#sub1useful) 下方。
...
-   如果您的應用程式達到延展性目標，請確定使用指數輪詢進行重試 (請參閱 [重試](#subheading14))。 最好的方式是確保您一定不會達到延展性目標 (方法是使用上述其中一個方法)，但這可確保您的應用程式不會持續快速重試，而降低節流的效能。

#### 有用資源

下列連結提供有關延展性目標的其他詳細資料：
-   請參閱 [Azure 儲存體延展性和效能目標](storage-scalability-targets.md) 有關延展性目標。
-   請參閱 [Azure 儲存體複寫](storage-redundancy.md) 和部落格文章 [Azure 儲存體備援選項與讀取存取地理區域備援儲存體](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/12/11/introducing-read-access-geo-replicated-storage-ra-grs-for-windows-azure-storage.aspx) 的儲存體備援選項的相關資訊。
-   目前的 Azure 服務的定價的詳細資訊，請參閱 [Azure 定價](http://azure.microsoft.com/pricing/overview/)。
...
頻寬的問題經常是用戶端的功能。 例如，當單一儲存體帳戶可處理 10 Gbps 或以上的輸入 (請參閱 [頻寬延展性目標](#sub1bandwidth)), ，才能夠達到 100 Mbps 左右的 「 小型 」 Azure 背景工作角色執行個體的網路速度。 較大型的 Azure 執行個體擁有較大容量的 NIC，因此，如果您需要單一機器的較高網路限制，您應考慮使用較大型的執行個體或更多的 VM。 如果您從內部部署應用程式存取儲存體服務，則適用相同的規則：了解用戶端裝置的網路功能和與 Azure 儲存體位置的網路連線能力，以及視需要進行改善或設計您的應用程式以便使用其功能。
...
如需虛擬機器大小與所配置的頻寬的詳細資訊，請參閱 [虛擬機器的大小](../virtual-machines/virtual-machines-size-specs.md)。
...
如需 Azure CDN 的詳細資訊，請參閱 [CDN](http://azure.microsoft.com/services/cdn/)。
...
如需有關 SAS 的詳細資訊，請參閱 [共用存取簽章，第 1 部分: 了解 SAS 模型](../storage-dotnet-shared-access-signature-part-1/)。
...
如需如何取得 blob 的屬性，以探索使用.NET 的上次修改的日期的範例，請參閱 [設定和擷取屬性及中繼資料](storage-properties-metadata.md)。 如需條件式下載的詳細資訊，請參閱 [有條件地重新整理 Blob 的本機複本](http://msdn.microsoft.com/library/azure/dd179371.aspx)。

#### <a name="subheading8"></a>以批次上傳資料
...
如需詳細資訊，請參閱部落格文章 [Web 服務: 並行連線](http://blogs.msdn.com/b/darrenj/archive/2005/03/07/386655.aspx)。

#### <a name="subheading10"></a>如果使用同步程式碼搭配 Async Task，增加執行緒集區的執行緒數量下限
...
#### <a name="subheading11"></a>利用.NET 4.5 記憶體回收

在用戶端應用程式中使用 .NET 4.5 或更新版本，以便在伺服器記憶體回收中充分運用效能改善。

如需詳細資訊，請參閱文章 [的效能改善概觀.NET 4.5 中](http://msdn.microsoft.com/magazine/hh882452.aspx)。
...
#### <a name="subheading14"></a>ServerBusy 節流
...
如需儲存體錯誤碼的詳細資訊，請參閱 [狀態和錯誤碼](http://msdn.microsoft.com/library/azure/dd179382.aspx) 在 Microsoft Azure 網站上。

## Blob

除了已經實證做法 [所有服務](#allservices) 先前所述，下列已經實證的做法尤其適用於 blob 服務。

### Blob 特定的延展性目標

#### <a name="subheading16"></a>頻寬和每個 Blob 的作業
...
Azure 儲存體團隊推出一個命令列工具 “AzCopy”，旨在協助將多個 Blob 大量傳輸至儲存體帳戶、從儲存體帳戶複製，及跨儲存體帳戶進行複製。 此工具已針對此案例進行最佳化，且可達到高傳輸率。 建議在大量上傳、下載和複製案例中使用此工具。 您可以深入了解並下載 [這裡](storage-use-azcopy.md)。

#### <a name="subheading19"></a>Azure 匯入/匯出服務

Azure 儲存體針對超大量的資料 (大於 1TB) 提供匯入/匯出服務，這允許透過寄送硬碟的方式，從 Blob 儲存體進行上傳和下載。 您可以將資料放在硬碟上，並將它寄送到 Microsoft 進行上傳，或寄送空白硬碟到 Microsoft 以下載資料。 閱讀更多有關它 [這裡](storage-import-export-service.md)。 這會比透過網路來上傳/下載此大量資料還要有效率。
...
如需如何使用.NET 的 blob 的中繼資料的範例，請參閱 [設定和擷取屬性及中繼資料](storage-properties-metadata.md)。
...
若要快速上傳多個 Blob，請平行上傳 Blob。 因為這個方法會將上傳散佈到儲存體服務的多個資料分割，因此會比搭配平行區塊上傳一次上傳單一 Blob 的方法要快。 單一 Blob 僅支援 60 MB/秒 (大約是 480 Mbps) 的輸送量。 本文撰寫期間，位於美國的 LRS 帳戶支援高達 20 Gbps 輸入，這比個別 Blob 所支援的輸送量要大的多。 [AzCopy](#subheading18) 根據預設，以平行方式執行上傳，建議在此案例中。

### <a name="subheading23"></a>選擇正確的 blob 類型
...
除了已經實證做法 [所有服務](#allservices) 先前所述，下列已經實證的做法尤其適用於資料表服務。
...
如需詳細資訊，請參閱下列文章 [Microsoft Azure 資料表: JSON 簡介](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/12/05/windows-azure-tables-introducing-json.aspx) 和 [表格服務作業的裝載格式](http://msdn.microsoft.com/library/azure/dn535600.aspx)。
...
如需詳細資訊，請參閱我們的部落格文章 [Nagle 的演算法並不適合用於小型要求](http://blogs.msdn.com/b/windowsazurestorage/archive/2010/06/25/nagle-s-algorithm-is-not-friendly-towards-small-requests.aspx), ，其中說明 Nagle 的演算法與資料表和佇列要求互動不佳的原因，以及說明如何停用此用戶端應用程式中。
...
##### <a name="subheading28"></a>只附加和前面加上唯一的模式
...
與為了尋找傳回的集合所掃描的實體數相較之下，查詢效率的另一個主要因素是傳回的實體數。 如果您的應用程式使用屬性值只有 1% 資料共用的篩選來執行資料表查詢，則查詢會為所傳回的每一個實體掃描 100 個實體。 先前討論的資料表延展性目標與掃描的實體數目相關，而與傳回的實體數目無關：低查詢密度會輕易導致資料表服務將您的應用程式節流，因為它必須掃描這麼多的實體，才可擷取您要尋找的實體。 請參閱以下章節上 [反正規化](#subheading34) 如需有關如何避免這種情況。

##### 限制傳回的資料量

###### <a name="subheading32"></a>篩選

在您知道查詢將傳回用戶端應用程式不需要之實體的情況下，請考慮使用篩選來減少傳回集合的大小。 雖然未傳回至用戶端的實體仍然算在延展性目標內，但您的應用程式效能將會因為網路裝載大小減輕以及用戶端應用程式必須處理的實體數減少而提升。 請參閱上面的注意事項 [查詢密度](#subheading31), ，不過延展性目標與掃描的實體數目，篩選出許多實體的查詢可能仍會導致節流，即使只傳回幾個實體。
...
或者，您的應用程式可以儲存每小時 CPU 使用量做為單一實體的個別屬性：若要每小時更新，您的應用程式可以使用單一 **InsertOrMerge Upsert** 呼叫來更新最近一小時的值。 若要繪製此資料，應用程式只需擷取單一實體，而不是 24，提供極有效率的查詢 (請參閱上述討論有關 [查詢範圍](#subheading30))。

##### <a name="subheading38"></a>在 blob 中儲存結構化的資料
...
檢視目前的延展性目標，在 [Azure 儲存體延展性和效能目標](storage-scalability-targets.md)。
...
最新成本資訊，請參閱 [儲存體定價詳細資料](http://azure.microsoft.com/pricing/details/storage/)。
...
如需詳細資訊，請參閱文章 [How to: 變更佇列訊息的內容](../storage-dotnet-how-to-use-queues/#change-contents)。
