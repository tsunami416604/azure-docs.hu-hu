...
-   [每秒資料表實體數](#subheading24)
-   [每秒佇列訊息數](#subheading39)  
...
####<a name="subheading11"></a>允分運用 .NET 4.5 記憶體回收
...
####<a name="subheading15"></a>不能再嘗試的錯誤
...
###Blob 特定的延展性目標
####<a name="subheading16"></a>每個 Blob 的頻寬和操作
...
Azure 儲存體支援兩種 blob 類型: *頁面* blob 和 *區塊* blob。 在指定使用的案例中，您的 Blob 類型選擇將會影響解決方案的效能和延展性。 區塊 Blob 適用於您想要有效地上傳大量資料時：例如，用戶端應用程式可能需要將相片或視訊上傳至 Blob 儲存體。 分頁 Blob 則適用於應用程式需要執行隨機寫入資料時：例如，將 Azure VHD 儲存為分頁 Blob。  
...
使用資料表 **Upsert** 作業可能的情況。 有兩種類型的 **Upsert**, ，這兩種可能會更有效率，比傳統 **插入** 和 **更新** 作業:  

-   **InsertOrMerge**: 當您想要上傳的實體屬性的子集，請使用這個，但不確定此實體是否已存在。 如果實體存在，此呼叫會更新中包含的屬性 **Upsert** 作業，並將保留所有現有的屬性時，如果實體不存在，則會插入新實體。 這類似於在查詢中使用投射，在此情況下，您只需要上傳正在變更的屬性。
-   **InsertOrReplace**: 當您想要上傳全新的實體，但您不確定它是否已存在時使用它。 您應該只在知道此全新上傳的實體完全正確時才使用此選項，因為它會完全覆寫舊的實體。 例如，您想要更新儲存使用者目前位置的實體，而不管應用程式先前是否已儲存使用者的位置資料；新的位置實體已完成，而且您無需任何先前實體的任何資訊。
...
或者，您的應用程式可以儲存的 CPU 使用量每小時做為單一實體的個別屬性: 若要每小時更新，您的應用程式可以使用單一 **InsertOrMerge Upsert** 呼叫來更新最近一小時的值。 若要繪製此資料，應用程式只需擷取單一實體，而不是 24，提供極有效率的查詢 (請參閱上述討論有關 [查詢範圍](#subheading30))。
...
###< 名稱 = subheading44"></a>UpdateMessage
您可以使用 **UpdateMessage** 來增加隱藏逾時或更新訊息的狀態資訊。 雖然這項功能強大，請記住，每個 **UpdateMessage** 在延展性目標會計算作業。 不過，相較於在工作的每個階段完成時，將工作從一個佇列傳遞到下一個佇列的工作流程，這會更有效率。 使用 **UpdateMessage** 作業可讓您將工作狀態儲存到訊息，然後繼續工作，而不是重新佇列訊息以進行下一個步驟的作業，每次步驟完成的應用程式。  

如需詳細資訊，請參閱文章 [How to: 變更佇列訊息的內容](../storage-dotnet-how-to-use-queues/#change-contents)。  

###< 名稱 = subheading45"></a>應用程式架構
