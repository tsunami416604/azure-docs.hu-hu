...
1. 按一下 [**開始**] 按鈕或按下 [**Windows**] 鍵。 開始輸入 `Azure 儲存體模擬器` 看起來似乎顯示儲存體模擬器命令列工具時，請選取它。
2. 在命令提示字元] 視窗中，輸入下列命令，其中 `< SQLServerInstance >` 是 SQL Server 執行個體的名稱。若要使用 LocalDb，請指定 `(localdb) \v11.0` 與 SQL Server 執行個體。
...
> [AZURE.NOTE] 如需以程式設計方式使用儲存體模擬器來存取次要位置，請使用 Storage Client Library for.NET 3.2 版或更新版本。 請參閱 [儲存體用戶端程式庫參考](https://msdn.microsoft.com/library/azure/dn261237.aspx) 如需詳細資訊。
...
若要檢視選項清單，請輸入 `/help` 在命令提示字元。

| 選項| 說明| 命令| 引數|
|--------|----------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------|
| **啟動**| 啟動儲存體模擬器。| `AzureStorageEmulator 開始 [-同處理序]`| *-inprocess*：在目前的處理序中啟動模擬器，而不是建立新的處理序。|
| **停止**| 停止儲存體模擬器。| `AzureStorageEmulator 停止`| |
| **狀態**| 列印儲存體模擬器的狀態。| `AzureStorageEmulator 狀態`| |
| **Clear**| 清除命令列上指定的所有服務中的資料。| `AzureStorageEmulator 清除 [blob] [資料表] [all] 的 [佇列]                                                    `| *blob*: 清除 blob 資料。<br/>*佇列*: 清除佇列資料。<br/>*資料表*: 清除資料表資料。<br/>*所有*: 清除所有服務中的所有資料。|
| **Init**| 執行一次初始化以設定模擬器。| `AzureStorageEmulator.exe init [-server serverName] [-sqlinstance instanceName] [-forcecreate] [-同處理序]`| *-server serverName\instanceName*: 指定裝載 SQL 執行個體的伺服器。<br/>*-sqlinstance instanceName*: 指定使用預設伺服器執行個體中的 SQL 執行個體的名稱。<br/>*-forcecreate*: 強制建立 SQL 資料庫，即使它已經存在。<br/>*-inprocess*: 目前的處理序，而不是繁衍新的處理序中執行初始化。您必須以更高權限啟動目前的處理序，才能執行初始化。|
...
- 從 3.1 版開始，儲存體模擬器帳戶就支援讀取存取地理備援複寫 (RA-GRS)。 在模擬器中，所有帳戶均已啟用 RA-GRS，而且主要和次要複本之間絕對不會有延遲。 次要帳戶支援 Get Blob Service Stats、 Get Queue Service Stats 和 Get Table Service Stats 作業，且一律會傳回的值 `LastSyncTime` 回應項目，以根據基礎 SQL 資料庫目前的時間。
...
- 在儲存體模擬器中，屬性的資料，請輸入 `edm.guid 更新` 或 `Edm.Binary` 僅支援 `等 (eq)` 和 `不等於 (ne)` 比較運算子，在查詢中的篩選字串。
