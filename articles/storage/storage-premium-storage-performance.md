...
|---|---|---|---|
| **IOPS 或每秒交易數**| 每秒發出給儲存體磁碟的 I/O 要求數。| Disk Reads/sec <br> 磁碟寫入數/秒| tps <br> r/s <br> w/s|
| **磁碟讀取和寫入**| 磁碟上執行的讀取和寫入作業 %。| %磁碟讀取時間 <br> %的磁碟寫入時間| r/s <br> w/s|
| **輸送量**| 每秒讀取或寫入磁碟的資料量。| 磁碟讀取位元組/秒 <br> 磁碟寫入位元組/秒| kB_read/s <br> kB_wrtn/秒|
| **延遲**| 完成磁碟 IO 要求的總時間。| 平均 Disk sec/Read <br> 平均 disk sec/Write| 等候 <br> svctm|
| **IO 大小**| 發出給儲存體磁碟的 I/O 要求大小。| 平均 Disk Bytes/Read <br> 的平均位元組/寫入磁碟| avgrq-sz|
| **佇列深度**| 等候讀取或寫入儲存體磁碟的未完成 I/O 要求數。| 目前磁碟佇列長度| avgqu-sz|
| * * 最大值。記憶體 * *| 順暢執行應用程式所需的記憶體數量| % Committed Bytes in Use| Use vmstat|
| * * 最大值。CPU * *| 順暢執行應用程式所需的 CPU 數量| % Processor time| %util|
...
|---|---|---|---|---|---|---|---|
| Standard_DS14| 16| 112 GB| 作業系統 = 1023 GB <br> 本機 SSD = 224 GB| 32| 576 GB| 50000 IOPS <br> 秒 512 MB| 4,000 IOPS 和每秒 33 MB|
...
在 Linux 上，請使用 MDADM 公用程式將磁碟串接在一起。 請參閱在 Linux 上的等量磁碟上的詳細的步驟， [Linux 上設定軟體 RAID](virtual-machines-linux-configure-raid.md)。
...
- [Azure 高階儲存體提供 Azure VM 中 SQL Server 最高的效能](http://blogs.technet.com/b/dataplatforminsider/archive/2015/04/23/azure-premium-storage-provides-highest-performance-for-sql-server-in-azure-vm.aspx)
