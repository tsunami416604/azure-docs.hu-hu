...
1.  **訂用帳戶和儲存體帳戶：**您必須有現有的 Azure 訂用帳戶以及一或多個儲存體帳戶，才能使用匯入/匯出服務。 每項工作都只能從僅只一個儲存體帳戶收送資料。 換句話說，一項工作不能跨越多個儲存體帳戶。 如需建立新的儲存體帳戶資訊，請參閱 [如何建立儲存體帳戶](storage-create-storage-account.md)。
...
1.  一旦您已備妥磁碟機，瀏覽至管理入口網站中，在儲存體帳戶，並檢視 [儀表板。 在 [Quick Glance]**** 之下，按一下 [Create an Import Job]****。
