...
本文將說明如何開始建立或使用 Visual Studio 中參考了 ASP.NET 5 專案中的 Azure 儲存體帳戶之後，Visual Studio 中使用 Azure 資料表儲存體 **新增連接的服務** ] 對話方塊。
...
**請注意** -一些對外呼叫 Azure 儲存體來執行 ASP.NET 5 中的 Api 是非同步。 請參閱 [使用 Async 和 Await 進行非同步程式設計](http://msdn.microsoft.com/library/hh191443.aspx) 如需詳細資訊。 以下程式碼假設使用非同步程式設計方法。
...
若要建立 Azure 資料表，請新增呼叫至 **CreateIfNotExistsAsync()**。
