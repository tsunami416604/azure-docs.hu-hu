...
3. 按一下 [ **可用** ] 索引標籤。
4. 按一下 [ **其他**。
5. 在 **Artifact Uploaders** 區段中，選取 **Microsoft Azure 儲存體外掛程式**。
6. 按一下 [ **安裝**。
...
3. 在 **Microsoft Azure 儲存體帳戶組態** 區段 ︰
    ...
    c. 使用的預設值為 **Blob 服務端點 URL** 如果您使用公用 Azure 雲端。 如果您使用不同的 Azure 雲端，使用的端點中所指定 [Azure 入口網站](portal.azure.com) 儲存體帳戶。

    d. 按一下 [ **驗證儲存體認證** 以驗證您的儲存體帳戶。

    e. [選用]如果您有其他儲存體帳戶要提供給 Hudson CI 使用，請按一下 **新增更多的儲存體帳戶**。
...
5. 在 **建置後動作** 區段的工作組態中，按一下 [ **將成品上傳至 Microsoft Azure Blob 儲存體**。
...
12. 按一下 [ **儲存** 以儲存設定。
13. 在 Hudson 儀表板中，按一下 [ **現在建置** 執行 **MyJob**。 檢查主控台輸出中的狀態。 當建置後動作開始上傳組建成品時，主控台輸出將會包含 Azure 儲存體的狀態訊息。
...
5. [選用]如 **下載路徑**, ，指定您要從 Azure Blob 儲存體下載檔案在 Hudson 機器上的路徑。 也可以使用環境變數 (如果您未提供值給 **下載路徑**, ，從 Azure Blob 儲存體檔案將下載到工作的工作區。)
