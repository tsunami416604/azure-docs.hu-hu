...
屬性必須是支援的型別，其會同時公開的公用屬性 `取得` 和 `設定`。
...
由 **TableOperation** 物件。 下列程式碼範例會依序建立 **CloudTable** 物件及 **CustomerEntity** 物件。 為了準備作業，已建立 **TableOperation** 物件以將客戶實體插入資料表。 最後，其呼叫了 **CloudTable.Execute** 來執行操作。
...
下列程式碼範例會建立兩個實體物件，
若要 **TableBatchOperation** 使用 **插入** 方法。 然後會呼叫 **CloudTable.Execute** 以執行作業。
...
- 了解如何簡化程式碼撰寫使用 Azure 儲存體使用 [Azure WebJobs SDK](../app-service-web/websites-dotnet-webjobs-sdk-get-started.md)
