...
用戶端程式庫目前僅支援整個 Blob 的加密。 當使用者使用，特別是，支援加密 **上傳 *** 方法或 **openOutputStream** 方法。 針對下載，則皆支援完整與範圍下載。
...
有三個金鑰保存庫封裝：
- azure 金鑰保存庫核心包含 IKey 和 IKeyResolver。 這是不具有相依性的小型封裝。 Java 的儲存體用戶端程式庫會將其定義為相依。
...
下載 [Azure Storage Client Library for Java Maven 套件] (<fix URL>)  
下載 [Azure Storage Client Library for Java 來源程式碼，從 GitHub](https://github.com/Azure/azure-storage-java)   
