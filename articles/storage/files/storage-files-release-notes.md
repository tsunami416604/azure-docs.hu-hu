...
| Március a 2019 -. kumulatív frissítés [KB4481060](https://support.microsoft.com/help/4481060)| 5.1.0.0 | 2019. március 7. | Támogatott |
...
A következő kiadási megjegyzések 7.0.0.0 az Azure File Sync ügynök verziója (kiadott 2019. június 19.) vonatkoznak.

### <a name="improvements-and-issues-that-are-fixed"></a>Megoldott problémák és fejlesztései

- Nagyobb méretűek megosztás támogatása
    - Az előzetes nagyobb Azure-fájlmegosztások növeljük a fájlszinkronizálás, valamint a vonatkozó támogatási korlátozásokkal. Az első lépésben az Azure File Sync mostantól támogatja az fel egyetlen, 25TB, illetve 50million fájlok névtér szinkronizálása. A alkalmazni a nagy méretű fájlok megosztási előzetes verzió, töltse ki ezt a kérdőívet https://aka.ms/azurefilesatscalesurvey. 
- Továbbfejlesztett Azure Backup fájlszintű visszaállítás
    - Egyedi fájlok visszaállítása az Azure Backup használatával most észlelt, és gyorsabban szinkronizált, a kiszolgálói végpontot.
- Továbbfejlesztett felhőrétegzés visszaírási parancsmag megbízhatóság 
    - A felhőrétegzés visszaírási parancsmag (Invoke-StorageSyncFileRecall) mostantól támogatja a fájlonkénti újrapróbálkozások száma és újrapróbálkozási késleltetés, robocopy hasonló.
- A TLS 1.2 csak támogatása (a TLS 1.0 és 1.1 le van tiltva)
    - Az Azure File Sync mostantól támogatja a TLS 1.2 használatával csak a kiszolgálókat, amelyeken a TLS 1.0 és 1.1 le van tiltva. Ennek a fejlesztésnek előtt kiszolgáló regisztrálása sikertelen lesz, ha a TLS 1.0 és 1.1 le lett tiltva a kiszolgálón.
- Egyéb teljesítmény és megbízhatóság fejlesztései a szinkronizálás és a felhőbeli rétegezés
    - Nincsenek ebben a kiadásban megbízhatóságát és teljesítményét számos fejlesztést. Némelyike jobbá felhőbeli rétegezés hatékonyabb és az Azure File Sync, a teljes munkahelyi azokban a helyzetekben a sávszélesség-szabályozási ütemezés beállítása, ha az célozzák meg.

### <a name="evaluation-tool"></a>Kiértékelési eszközével
Az Azure File Sync üzembe helyezése előtt, ki kell értékelni a rendszer az Azure File Sync értékelési eszközzel kompatibilis-e. Ezt az eszközt az Azure PowerShell-parancsmagot, amely ellenőrzi a potenciális problémákat, a fájlrendszert és adatkészlethez, például a nem támogatott karaktereket vagy nem támogatott operációsrendszer-verzió. Telepítés és használat utasításokat: [Megoldásértékelési eszköz](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#evaluation-cmdlet) az útmutató a tervezési szakaszban. 
...
Ebben a kiadásban megoldott problémák listája:  
- Miután létrehozta a kiszolgálói végpont, magas CPU-használat jelentkezhet, ha a háttérben visszaírási fájlok letöltése a kiszolgálóra
- Szinkronizálás és a felhőbeli rétegezés műveletek sikertelenek lehetnek ECS_E_SERVER_CREDENTIAL_NEEDED hiba miatt a jogkivonat lejárati
- Egy fájl meghiúsulhat, ha a fájl letöltése az URL-cím fenntartott karaktereket tartalmaz. 

## <a name="agent-version-6100"></a>Ügynök verziója 6.1.0.0
Az alábbi kibocsátási megjegyzések olyan verziójára 6.1.0.0 az Azure File Sync ügynök 2019. május 6. Ezek a megjegyzések kibocsátási megjegyzéseinek kiegészítéséül verzió 6.0.0.0 kívül vannak.

Ebben a kiadásban megoldott problémák listája:  
- Windows Admin Center nem jelennek meg az ügynök verziója és a kiszolgálói végpont-konfiguráció kiszolgálókon, amelyek rendelkeznek az Azure File Sync ügynök verziója 6.0-s verzióját.

## <a name="agent-version-6000"></a>Ügynök verziója 6.0.0.0
Az alábbi kibocsátási megjegyzések olyan verziójára 6.0.0.0 az Azure File Sync ügynök (2019 április 22).

### <a name="improvements-and-issues-that-are-fixed"></a>Megoldott problémák és fejlesztései

- Az ügynök automatikus frissítésének támogatása
  - Tudjuk, visszajelzések és hozzáadva egy automatikus frissítés funkció az Azure File Sync-kiszolgáló ügynök. További információkért lásd: [Azure File Sync ügynökének frissítési szabályzata](https://docs.microsoft.com/azure/storage/files/storage-files-release-notes#azure-file-sync-agent-update-policy).
- Az Azure-támogatási megosztás hozzáférés-vezérlési listák
  - Az Azure File Sync mindig rendelkezik támogatott kiszolgálói végpontok közötti szinkronizálása a hozzáférés-vezérlési listákat, de a hozzáférés-vezérlési listák nem lettek szinkronizálva a felhőbeli végponthoz (Azure-fájlmegosztás). Ebben a kiadásban az szinkronizálja ACL-kiszolgáló és a felhőbeli végpontok közötti támogatást.
- Párhuzamos feltöltése, és töltse le a kiszolgálói végpont szinkronizálási munkamenetek 
  - Kiszolgálóvégpontok mostantól támogatja a fel- és fájlok letöltésével egy időben. Nincs több Várakozás a letöltés befejezéséhez Igen fájlokat is feltölthetők a az Azure-fájlmegosztást. 
- Új, Felhőbeli Rétegezés parancsmagok kötet és a felhőrétegzés állapota
  - Két új, a kiszolgálói helyi PowerShell-parancsmagok mostantól használható felhőbeli rétegezés és a fájl visszaírási információkhoz juthat. Elérhetővé tesznek naplózási adatait két eseménycsatornák a kiszolgálón:
    - Get-StorageSyncFileTieringResult felsorolja minden fájl és azok még nem rétegzett elérési utak és okát a jelentések miért.
    - Get-StorageSyncFileRecallResult jelentések az összes fájl visszaírási esemény. Minden idézni fájl- és az elérési út a listáját, valamint a sikeres vagy a visszaírási által jelzett hibát.
  - Alapértelmezés szerint mind eseménycsatornák tud tárolni legfeljebb 1MB, – növelheti a fájlokat a esemény csatorna méretének növelésével jelentett mennyisége.
- FIPS-módban támogatása
  - Az Azure File Sync engedélyezése FIPS-módban mostantól támogatja a kiszolgálókon, amelyek az Azure File Sync-ügynök telepítve.
    - Engedélyezi a kiszolgálón a FIPS-módban, mielőtt az Azure File Sync ügynök telepítése és [PackageManagement modul](https://www.powershellgallery.com/packages/PackageManagement/1.1.7.2) a kiszolgálón. Ha FIPS már engedélyezve van a kiszolgálón [manuális letöltéséhez](https://docs.microsoft.com/powershell/gallery/how-to/working-with-packages/manual-download) a [PackageManagement modul](https://www.powershellgallery.com/packages/PackageManagement/1.1.7.2) a kiszolgálóhoz.
- Vegyes megbízhatóság fejlesztései a felhőbeli rétegezés és szinkronizálás
...
Az alábbi kibocsátási megjegyzések olyan verziójára 5.2.0.0 az Azure File Sync ügynök 2019. április 4. Ezek a megjegyzések kibocsátási megjegyzéseinek kiegészítéséül verzió 5.0.2.0 kívül vannak.

Ebben a kiadásban megoldott problémák listája:  
- Offline adatok átvitelét és az adatforgalom megbízhatóság fejlesztései szolgáltatások folytatása
- Szinkronizálási telemetriai fejlesztései
