...
Az alábbi példák bemutatják, hogyan kezelheti az Azure-fájlmegosztást a fájl REST protokollal a Azure PowerShell modul használatával. A `-Context` paraméter a Storage-fiók kulcsának beolvasására szolgál a jelzett műveletek a fájlmegosztás alapján történő végrehajtásához. A Storage-fiók kulcsainak lekéréséhez a RBAC szerepkörrel kell rendelkeznie `Owner` a Storage-fiókban.
...
Habár a `Start-AzStorageFileCopy` parancsmag kényelmes az Azure-fájlmegosztás, az áttelepítések és a nagyobb adatáthelyezések között az ad hoc fájlokhoz, javasoljuk, hogy a Windows és a `rsync` a macOS és Linux rendszereken is `robocopy`. `robocopy` és `rsync` az SMB használatával hajtja végre az adatáthelyezést a legtöbbet a legtöbbes API helyett.
...
Megosztási pillanatképet a `Snapshot` metódus használatával hozhat létre a fájlmegosztás PowerShell-objektumán, amely a [Get-AzStorageShare](/powershell/module/az.storage/Get-AzStorageShare) parancsmaggal olvasható be. 
...
A pillanatkép referenciáját (`$snapshot`) a `-Share` parancsmag `Get-AzStorageFile` paraméterébe illesztve böngészhet a megosztási pillanatkép tartalmában.
...
A [Remove-AzStorageShare](/powershell/module/az.storage/Remove-AzStorageShare) parancsmag használatával törölheti a megosztási pillanatképet a `-Share` paraméterre mutató `$snapshot` hivatkozást tartalmazó változóval.
