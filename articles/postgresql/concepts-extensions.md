...
Az alábbi táblázatok a szabványos PostgreSQL-bővítmények PostgreSQL Azure-adatbázis által jelenleg támogatott. Ez az információ is rendelkezésre áll futtatásával `SELECT * FROM pg_available_extensions;`.
...
| [Adatkocka](https://www.postgresql.org/docs/9.6/static/cube.html) | Többdimenziós kockák adattípust biztosít. |
| [hstore](https://www.postgresql.org/docs/9.6/static/hstore.html) | Itt adattípus-készletek kulcs/érték párok tárolására. |
| [nem](https://www.postgresql.org/docs/9.6/static/isn.html) | Nemzetközi termék szabványok számozására adattípusok biztosít. |
...
| [a védelmi\_trgm](https://www.postgresql.org/docs/9.6/static/pgtrgm.html) | Biztosítja a funkciók és operátorok trigram megfelelő alfanumerikus szöveg hasonlóság meghatározásához. |
...
| [a védelmi\_stat\_utasítások](https://www.postgresql.org/docs/9.6/static/pgstatstatements.html) | Lehetővé teszi a kiszolgáló által végrehajtott összes SQL-utasítások végrehajtása statisztikáinak nyomon követése. (Lásd alább ezt a bővítményt a Megjegyzés). |
...
| [hypopg](https://hypopg.readthedocs.io/en/latest/) | Lehetővé teszi, hogy a CPU, illetve a lemez nem költség elméleti indexekhez létrehozásának. |
...
A [pg\_stat\_utasítások bővítmény](https://www.postgresql.org/docs/9.6/static/pgstatstatements.html) előre betöltött minden Azure adatbázishoz PostgreSQL-kiszolgáló az SQL-utasítások végrehajtása statisztikáinak nyomon követése módszert biztosít.
A beállítás `pg_stat_statements.track`, amely meghatározza, milyen utasítás által a bővítményt, az alapértelmezett érték a számlálási `top`, ami azt jelenti, közvetlenül az ügyfelek által kiállított összes utasításokat követi. A két más nyomkövetési szintek a következők `none` és `all`. Ez a beállítás akkor a kiszolgáló paramétert keresztül beállítható a [Azure-portálon](https://docs.microsoft.com/en-us/azure/postgresql/howto-configure-server-parameters-using-portal) vagy a [Azure CLI](https://docs.microsoft.com/en-us/azure/postgresql/howto-configure-server-parameters-using-cli).

A lekérdezés végrehajtása pg_stat_statements biztosít, valamint a kiszolgáló teljesítményre gyakorolt hatása közötti kompromisszumot van, minden egyes SQL-utasítás naplózza. Ha a pg_stat_statements bővítmény aktívan nem használ, azt javasoljuk, hogy állítsa `pg_stat_statements.track` való `none`. Vegye figyelembe, hogy néhány külső-szolgáltatások figyelésének pg_stat_statements lekérdezési teljesítmény insights biztosítanak, ezért győződjön meg arról, hogy az eset, vagy nem hivatkozhatnak.
