...
az login
```
A CLI-bővítmény telepítése az Azure Database for PostgreSQL virtuális hálózati Szolgáltatásvégpontok használatával a `az extension add --name rdbms-vnet` parancsot. 
```azurecli-interactive
...
az extension list
```
A parancs kimenete listázza az összes telepített bővítmények. Az Azure Database for PostgreSQL CLI-bővítmény a következő:
