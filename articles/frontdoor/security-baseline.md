...
**Útmutató** : nem alkalmazható; Az Azure-beli bejárati ajtó nem a magánhálózaton való üzembe helyezésre, illetve a magánhálózathoz való biztonságos használatra készült, ezért ez a vezérlő a hálózati kapcsolat leírására szolgál, és nem alkalmazható.

**Azure Security Center figyelés** : nem alkalmazható

**Felelősség** : nincs beállítva. Adjon meg egy értéket a munkaelemben.
...
**Útmutató** : nem alkalmazható, az Azure bejárati ajtaját nem úgy tervezték, hogy egy virtuális hálózat a magánhálózati hozzáféréshez legyen telepítve vagy biztonságossá tétele.
...
**Útmutató** : titkosítás használata a külső és a nyilvános hálózatok forgalmának védelme érdekében, mivel az adatvédelem szempontjából kritikus fontosságú. Továbbá:

- Hozzáférés-vezérlés használata

- a "sávon kívüli" támadások (például a forgalom rögzítése) elleni adatátvitel titkosítással biztosítható, hogy a támadók ne tudják egyszerűen beolvasni vagy módosítani az adatokat.
- Győződjön meg arról, hogy az Azure-erőforrásokhoz a HTTP-forgalomhoz csatlakozó ügyfelek a TLS v 1.2-es vagy újabb verzióját tárgyalják.
- A távoli felügyelethez használja az SSH-t (Linuxon) vagy RDP/TLS-t (Windows rendszeren) a titkosítatlan protokollok helyett

- Elavult SSL/TLS/SSH verziók, protokollok és gyenge titkosítás letiltása

Az Azure alapértelmezés szerint az Azure-adatközpontok közötti adatforgalom továbbítására szolgáló titkosítást biztosít. 
...
**Útmutató** : nem alkalmazható; Az Azure bejárati ajtaja nem használható az eszközök biztonságának biztosításához az életciklus-kezelési folyamatokban. Az ügyfél feladata, hogy fenntartsa a nagy hatásnak számító eszközök attribútumait és hálózati konfigurációit. 
