...
-   [How to: Retrieve an Entity][How to: Retrieve an Entity]
...
-   [How to: Work with Shared Access Signatures][How to: Work with Shared Access Signatures]
...
Both **PartitionKey** and **RowKey** must be string values. For more information, see [Understanding the Table Service data model][Understanding the Table Service data model].
...
If the operation is successful, `result` will contain the [ETag][ETag] of the inserted record and `response` will contain information about the operation.
...
> [WACOM.NOTE] You should consider using ETags when deleting items, to ensure that the item hasn't been modified by another process. See [How To: Update an Entity][How To: Update an Entity] for information in using ETags.
