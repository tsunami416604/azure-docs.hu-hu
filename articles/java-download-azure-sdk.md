...
**附註:** Azure Libraries for Java 分散在 [Apache 授權，2.0 ][license]。
...
如需設定在 Eclipse 中的組建路徑的詳細資訊，請參閱 [[Java Build Path]][] Eclipse 網站的發行項。
...
在 Eclipse 中使用 Java 的 Azure 程式庫中的指示建立 Maven 專案 [開始使用 Azure 管理程式庫 ][maven-getting-started] 文件。
...
**附註:** 在每個 `< 版本 >` 項目，在上述範例中，在此範例中的版本號碼取代有效的版本號碼，您可以從取得 [Azure Libraries Repository on Maven []][]。
...
1. 列出 Windows operting 系統 [Azure Toolkit for Eclipse 的 [] 中的新][] 文件。
1. 列在 Macintosh 或 Linux operting 系統 [Azure Toolkit for Eclipse 的 [] 中的新][] 文件。
...
[Azure Toolkit for Eclipse 的]][]

[安裝 Azure Toolkit for Eclipse 的]][]
