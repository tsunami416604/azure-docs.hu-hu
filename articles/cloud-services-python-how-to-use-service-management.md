...
-   [How to: List available operating systems][How to: List available operating systems]
-   [How to: Create an operating system image][How to: Create an operating system image]
-   [How to: Delete an operating system image][How to: Delete an operating system image]
...
The command will create the `.cer` file, and install it in the **Personal** certificate store. For more details, see [Create and Upload a Management Certificate for Azure][Create and Upload a Management Certificate for Azure].
...
You can use [OpenSSL][OpenSSL] to create your management certificate. You actually need to create two certificates, one for the server (a `.cer` file) and one for the client (a `.pem` file). To create the `.pem` file, execute this:
...
For more information about Azure certificates, see [Managing Certificates in Azure][Managing Certificates in Azure]. For a complete description of OpenSSL parameters, see the documentation at [][]<http://www.openssl.org/docs/apps/openssl.html></a>.
...
The **create\_deployment** method uploads a new [service package][service package] and creates a new deployment in the staging or production environment. The parameters for this method are as follows:
...
A [storage service][storage service] gives you access to Azure [Blobs][Blobs], [Tables][Tables], and [Queues][Queues]. To create a storage service, you need a name for the service (between 3 and 24 lowercase characters and unique within Azure), a description, a label (up to 100 characters, automatically encoded to base64), and either a location or an affinity group. The following example shows how to create a storage service by specifying a location. If you want to use an affinity group, you have to create an affinity group first (see [How to: Create an affinity group][How to: Create an affinity group]) and set it with the **affinity\_group** parameter.
...
To create a virtual machine, you first need to create a [cloud service][How to: Create a cloud service]. Then create the virtual machine deployment using the **create\_virtual\_machine\_deployment** method:
...
-   See the MSDN Reference: [Cloud Services][service package]
-   See the MSDN Reference: [Virtual Machines][Virtual Machines]
  ...
  [create an Azure account]: http://www.windowsazure.com/en-us/pricing/free-trial/
  ...
  [Blobs]: https://www.windowsazure.com/en-us/develop/python/how-to-guides/blob-service/
  [Tables]: https://www.windowsazure.com/en-us/develop/python/how-to-guides/table-service/
  [Queues]: https://www.windowsazure.com/en-us/develop/python/how-to-guides/queue-service/
