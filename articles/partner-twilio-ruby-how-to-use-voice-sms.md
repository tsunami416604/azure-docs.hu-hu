...
Twilio 動詞是指示 Twilio 如何的 XML 標記 **執行**。 例如， **& l t;Y & gt;** 動詞指示 Twilio 在通話中用語音傳遞訊息上呼叫。 
...
使用 Twilio 服務且執行於 Azure 的 Ruby 應用程式，與其他使用 Twilio 服務的 Ruby 應用程式並無不同。 雖然 Twilio 服務是以 rest 為基礎，並且可以透過數種方式從 Ruby 撥打，本文將著重在如何使用 Twilio 服務與 [用於 Ruby 的 Twilio 協助程式庫][twilio_ruby]。
