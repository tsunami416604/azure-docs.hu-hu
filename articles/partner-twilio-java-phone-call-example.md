...
* 除了使用 Web 表單以外，您也可以使用 Azure 儲存體 Blob 或 SQL Database 來儲存電話號碼和通話文字。 使用 java 的 Azure 儲存體 blob 的相關資訊，請參閱 [如何使用 Blob 儲存體服務，從 ][howto_blob_storage_java]。 使用 java 的 SQL Database 的相關資訊，請參閱 [Java ][howto_sql_azure_java]。
* 您可以使用 **RoleEnvironment.getConfigurationSettings**，從部署的組態設定中擷取 Twilio 帳戶 ID 和驗證權杖，而不要在 makecall.jsp 中進行值的硬式編碼。 如需有關資訊 **RoleEnvironment** 類別，請參閱 [使用 JSP ][azure_runtime_jsp] 和 Azure Service Runtime 封裝文件 [http://dl.windowsazure.com/javadoc ][azure_javadoc]。
* Makecall.jsp 程式碼會將 Twilio 提供的 URL，指派 [http://twimlets.com/message ][twimlet_message_url], 至 **Url** 變數。 此 URL 會提供 Twilio 標記語言 (TwiML) 回應，告知 Twilio 應如何執行通話。 例如，傳回的 TwiML 可能會包含 **&lt;Say&gt;** 動詞，而產生要傳達給受話方的文字。 除了使用 Twilio 提供的 URL，您也可以建置自己的服務來回應 Twilio 的要求。如需詳細資訊，請參閱 [如何透過 Twilio 使用語音和簡訊功能以 Java ][howto_twilio_voice_sms_java]。 TwiML 的詳細資訊，請參閱 [http://www.twilio.com/docs/api/twiml ][twiml], ，和其他相關資訊 **< y >** 和其他 Twilio 動詞命令，請參閱 [http://www.twilio.com/docs/api/twiml/say ][twilio_say]。
...
* [如何透過 Twilio 使用語音和簡訊功能以 Java ][howto_twilio_voice_sms_java]
