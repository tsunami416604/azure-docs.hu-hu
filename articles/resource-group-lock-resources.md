...
無法刪除儲存體帳戶。 在此案例中，您會在儲存體帳戶上使用鎖定層級為 **CanNotDelete** 的資源鎖定。
...
若要建立或刪除管理鎖定，您必須存取 **Microsoft.Authorization/\*** 或 **Microsoft.Authorization/locks/\*** 動作。 內建角色中，只有**擁有者**和**使用者存取系統管理員**被授與這些動作。 如需指派存取控制的詳細資訊，請參閱 [管理資源的存取權](resource-group-rbac.md)。
...
搭配 concat () 函式。 結果會是資源名稱的結尾附加了「Microsoft.Authorization」，然後是鎖定的名稱 (在此案例中為 **myLock**)。
...
- 如需使用資源鎖定的詳細資訊，請參閱 [您 Azure 資源下鎖定](http://blogs.msdn.com/b/cloud_solution_architect/archive/2015/06/18/lock-down-your-azure-resources.aspx)
