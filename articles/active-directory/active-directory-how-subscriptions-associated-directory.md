...
雖然 Azure 原先只允許存取 Microsoft 帳戶使用者，它現在可讓使用者從存取 *兩者* 系統。 此乃透過讓所有 Azure 屬性信任 Azure AD 進行驗證、讓 Azure AD 驗證組織使用者，以及透過建立同盟關係，讓 Azure AD 信任 Microsoft 帳戶消費者識別系統進行消費者驗證等方式達成。 如此一來，Azure AD 便能驗證「來賓」Microsoft 帳戶，以及「原生」Azure AD 帳戶。
...
> [AZURE.NOTE]
> 只有當使用者使用 Microsoft 帳戶登入時，才可以完成下列步驟。 如果使用者登入工作或學校帳戶選項 **使用現有的目錄** 無法使用，因為工作或學校帳戶可以驗證只透過其主目錄 (也就是工作或學校帳戶的儲存位置的目錄且由公司或學校所擁有)。

1. 使用您的 Microsoft 帳戶登入 Azure 傳統入口網站。
2. 按一下 [ **新增** > **應用程式服務** > **Active Directory** > **目錄** > **自訂建立**。
3. 按一下 [ **使用現有的目錄** 並檢查 **我已經準備好立即登出** 按一下核取記號以完成動作。
4. 使用擁有工作或學校目錄全域系統管理員權限的帳戶登入 Azure 傳統入口網站。
5. 當系統提示您 **搭配 Azure 使用 Contoso 目錄？**, ，然後按一下 **繼續**。
6. 按一下 [ **立即登出**。
