...
|   ----------------------- |   ----------------------- |
|  允許的字元   |   <ul><li>A – Z</li><li>-z </li><li>0 – 9</li> <li>@ # $ % ^ & * - _ ! + = [ ] { } &#124; \ : ‘ , . ? / ` ~ “ ( ) ;</li></ul> |
|  不允許的字元   |       <ul><li>Unicode 字元</li><li>空格</li><li>空格</li><li> **只有強式密碼**︰ 不能包含點字元 '。 '前面' @' 符號</li></ul> |
|   密碼限制 | <ul><li>8 個字元的最小和最多 16 個字元</li><li>**只有強式密碼**︰ 需要 3，4 下列動作 ︰<ul><li>小寫字元</li><li>大寫字元</li><li>數字 (0-9)</li><li>（請參閱上面的密碼限制） 的符號</li></ul></li></ul> |
| 密碼到期時間      | <ul><li>預設值 ︰ **90** 天 </li><li>值為可使用 Azure Active Directory 的 Windows powershell 模組 Set-msolpasswordpolicy 指令程式設定。</li></ul> |
| 密碼到期通知 |  <ul><li>預設值 ︰ **14** 天 （密碼到期前）</li><li>值為可使用 Set-msolpasswordpolicy 指令程式設定。</li></ul> |
| 密碼到期 |  <ul><li>預設值 ︰ **false** 天 （表示已啟用該密碼到期） </li><li>可以使用 Set-msoluser 指令程式的個別使用者帳戶的設定值。 </li></ul> |
|  密碼歷程記錄  | 無法再次使用上次密碼。 |
|  密碼歷程記錄期間 | 不限次數 |
