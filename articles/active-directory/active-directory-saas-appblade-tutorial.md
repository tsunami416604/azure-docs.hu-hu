...
本教學課程的目標在於告訴您如何使用 Azure Active Directory (Azure AD) 整合 AppBlade。<br>整合 AppBlade 與 Azure AD 為您提供下列優點:
...
建立此連結關聯性的方法，就是將 Azure AD 中 [使用者名稱]**** 的值，指派為 AppBlade 中 [Username]**** 的值。
...
4. * *[建立 AppBlade 測試使用者](#creating-a-appblade-test-user)* *-若要連結到她在 Azure AD 代表 AppBlade 中有對應的 Britta Simon。
...
本節目標是在 AppBlade 中建立名為 Britta Simon 的使用者。 AppBlade 支援預設啟用的 Just-In-Time 佈建。 * * 請請確定您的網域名稱與 AppBlade 設定的使用者佈建。 將之後，只有在 just-in-time 佈建使用者 work.* *
