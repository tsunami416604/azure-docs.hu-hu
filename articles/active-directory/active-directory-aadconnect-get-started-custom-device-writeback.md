...
3.  使用企業系統管理員認證執行下列命令，然後結束 PowerShell。
    ...
    `匯入模組 'C:\Program Files\Microsoft Azure Active Directory Connect\AdPrep\AdSyncPrep.psm1'`

    `初始化 ADSyncDeviceWriteback {選擇性: – DomainName [名稱] 選項:-AdConnectorAccount [帳戶]}`
...
2.  在 [選用功能] 頁面中，裝置回寫不再呈現灰色。 請注意，如果 Azure AD Connect 準備步驟未完成，[選用功能] 頁面中的裝置回寫會變成灰色。 核取裝置回寫的方塊，然後按一下 [下一步****]。 如果仍然已停用核取方塊，請參閱 [疑難排解區段](#the-writeback-checkbox-is-still-disabled)。
...
- 確定您在初始化指令碼中提供的帳戶是 Active Directory 連接器實際使用的正確使用者。 若要確認，請依照下列步驟執行：
    ...
    - 選取 [動作]**** 下方的 [屬性]****。
    - 前往 [連線至 Active Directory 樹系]****。 確認網域和使用者名稱指定在此畫面相符項目上提供的指令碼的帳戶。
...
- [設定內部部署條件式存取使用 Azure Active Directory 裝置註冊](https://msdn.microsoft.com/library/azure/dn788908.aspx)
