---
title: Önkiszolgáló jelszó-visszaállítás hibaelhárítása – Azure Active Directory
...
ms.locfileid: "72024639"
---
# <a name="troubleshoot-self-service-password-reset"></a>Önkiszolgáló jelszó-visszaállítás hibáinak megoldása
...
| SsprNotEnabledInUserPolicy = 11 | Sajnos nem lehet alaphelyzetbe állítani a jelszót, mert a rendszergazda nem konfigurálta a jelszó-visszaállítást a szervezet számára. Ez a helyzet nem oldható fel további művelettel. Forduljon a rendszergazdához, és kérje meg őket, hogy konfigurálja a jelszó-visszaállítást. A jelszó-visszaállítási konfigurációval kapcsolatos további tudnivalókért tekintse meg a [Quick Start: Azure AD önkiszolgáló jelszó-visszaállítás @ no__t-0. | SSPR_0011: A szervezet nem definiált új jelszó-visszaállítási házirendet. Forduljon a rendszergazdához, és kérje meg őket, hogy definiálják a jelszó-visszaállítási szabályzatot. |
...
| 31008 | ChangePasswordFail | Ez az esemény azt jelzi, hogy a felhasználó jelszót adott meg, és a jelszó sikeresen megérkezett a helyszíni környezetbe, de amikor megpróbálta beállítani a jelszót a helyi Active Directory környezetben, hiba történt. Ez a hiba több okból is előfordulhat: <br><ul><li>A felhasználó jelszava nem felel meg a tartomány életkor-, előzmény-, összetettségi vagy szűrési követelményeinek. A probléma megoldásához hozzon létre egy új jelszót.</li><li>Az ADMA-szolgáltatásfiók nem rendelkezik a megfelelő engedélyekkel az új jelszó megadásához a szóban forgó felhasználói fiókban.</li><li>A felhasználó fiókja egy védett csoportban található, például tartomány vagy vállalati rendszergazda, amely nem engedélyezi a jelszó-beállítás műveleteit.</li></ul> |
...
| 32002| ServiceBusError| Ez az esemény azt jelzi, hogy hiba történt a bérlő Service Bus példányához való csatlakozáskor. Ez akkor fordulhat elő, ha blokkolja a kimenő kapcsolatokat a helyszíni környezetben. Ellenőrizze a tűzfalon, hogy engedélyezi-e a TCP 443-es és a https://ssprsbprodncu-sb.accesscontrol.windows.net/ közötti kapcsolatokat, majd próbálkozzon újra. Ha továbbra is problémákat tapasztal, próbálja meg letiltani, majd újból engedélyezni a jelszó-visszaírási.|
...
| 32011| OnBoardingServiceError| Ez az esemény azt jelzi, hogy a helyszíni szolgáltatás nem tudott megfelelően kommunikálni a jelszó-visszaállítási webszolgáltatással a bevezetési folyamat elindításához. Ez egy tűzfalszabály miatt fordulhat elő, vagy ha probléma van a bérlőhöz tartozó hitelesítési jogkivonat beszerzésével. A probléma megoldásához ellenőrizze, hogy nem blokkolja-e a kimenő kapcsolatokat a TCP 443-es és a TCP 9350-9354-es vagy a https://ssprsbprodncu-sb.accesscontrol.windows.net/ protokollon keresztül. Győződjön meg arról is, hogy a bevezetéshez használt Azure AD-rendszergazdai fiók nem összevont.|
| 32013| OffBoardingError| Ez az esemény azt jelzi, hogy a helyszíni szolgáltatás nem tudott megfelelően kommunikálni a jelszó-visszaállítási webszolgáltatással a offboarding folyamat elindításához. Ez egy tűzfalszabály miatt fordulhat elő, vagy ha probléma merül fel a bérlőhöz tartozó engedélyezési jogkivonat beszerzése során. A probléma megoldásához győződjön meg arról, hogy nem blokkolja a 443-as vagy a https://ssprsbprodncu-sb.accesscontrol.windows.net/ kimenő kapcsolatokat, és hogy a regisztrációjának megszüntetésére szolgáló használt Azure Active Directory rendszergazdai fiók nem összevont.|
...
| 32015| ReportServiceHealthError| A jelszó-visszaírási szolgáltatás állapotának figyeléséhez öt percenként küldünk szívverési adatait a jelszó-visszaállítási webszolgáltatásnak. Ez az esemény azt jelzi, hogy hiba történt az állapotadatok Felhőbeli webszolgáltatásba való visszaküldésekor. Ezek az állapotadatok nem tartalmazzák az objektum azonosításra alkalmas adatait (OII) vagy személyes azonosításra alkalmas adatokat, és csak szívverési és alapszintű szolgáltatási statisztikákat biztosítanak, hogy a szolgáltatás állapotára vonatkozó információkat a felhőben is elérhetővé lehessen tenni.|
...
| 33004| ADPermissionsError| Ez az esemény azt jelzi, hogy a Active Directory felügyeleti ügynök (ADMA) szolgáltatásfiók nem rendelkezik a megfelelő engedélyekkel a szóban forgó fiókhoz új jelszó megadásához. Győződjön meg arról, hogy a felhasználó erdőjében található ADMA-fiók alaphelyzetbe áll, és módosítsa a jelszó engedélyeit az erdőben lévő összes objektumra vonatkozóan. Az engedélyek beállításával kapcsolatos további tudnivalókért tekintse meg a 4. lépést: Állítsa be a megfelelő Active Directory engedélyeket.|
...
> [!NOTE]
> A SSPR akkor is sikertelen lehet, ha a fiók, ha a "jelszó soha nem jár le" vagy "a felhasználó nem módosíthatja a jelszót" beállítást a AD DS helyszíni fiókjában konfigurálja. 
...
1. Jelentkezzen be a Azure AD Connect kiszolgálóra, és indítsa el a **synchronization Service Manager** a **Start** > **szinkronizációs szolgáltatás**kiválasztásával.
1. Az **Összekötők** lapon válassza ki a helyszíni **Active Directory tartományi szolgáltatások** -összekötőt, majd válassza a **Tulajdonságok**lehetőséget.  
   ![Synchronization Service Manager a tulajdonságok szerkesztésének módját mutatja @ no__t-1  
...
* **Lap**: Mi az oldal, amikor észrevette a hibát? Adja meg az URL-címet, ha tudja, és képernyőképet is tartalmaz az oldalról.
...
* **Dátum, idő és időzóna**: Adja meg a pontos dátumot és időt a hiba előfordulási *időzónájának megfelelően* .
* **Felhasználói azonosító**: Ki volt a felhasználó, aki látta a hibát? Ilyen például a *User\@contoso.com*.
...
* **Licencelés**: A felhasználónak van hozzárendelve Azure AD-licence?
* **Alkalmazás eseménynaplója**: Ha jelszó-visszaírási használ, és a hiba a helyszíni infrastruktúrában található, az alkalmazás eseménynaplójának tömörített másolatát adja meg a Azure AD Connect-kiszolgálóról.
