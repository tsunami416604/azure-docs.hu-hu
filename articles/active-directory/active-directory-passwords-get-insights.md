...
- [* * 檢視密碼重設您的組織 * * 的活動](#view-password-reset-activity)
...
下表描述每個資料行允許的不同值：
 ...
 欄| 允許的值及其意義
---|---
 已註冊資料| **備用電子郵件地** – 使用者使用替代電子郵件或驗證來驗證<p><p>**辦公室電話**– 使用者用來驗證的辦公室電話<p>**行動電話** -使用者使用行動電話或驗證的驗證電話<p>**安全性問題** – 使用者用來驗證安全性問題<p>**上述項目的任何組合 (例如替代電子郵件 + 行動電話)** – 當指定 2 個閘道原則，並顯示使用者使用哪兩個方法來驗證其密碼重設要求。
...
下表描述每個資料行允許的不同值：
 ...
 欄| 允許的值及其意義
---|---
 使用方法| **備用電子郵件地** – 使用者使用替代電子郵件或驗證來驗證<p>**辦公室電話** – 使用者用來驗證的辦公室電話<p>**行動電話** – 使用者使用行動電話或驗證的驗證電話<p>**安全性問題** – 使用者用來驗證安全性問題<p>**上述項目的任何組合 (例如替代電子郵件 + 行動電話)** – 當指定 2 個閘道原則，並顯示使用者使用哪兩個方法來驗證其密碼重設要求。
 結果| **已放棄的** – 使用者開始重設密碼，但是又中途停止而不會完成<p>**已封鎖** – 使用者的帳戶，所以無法使用密碼重設，因為嘗試使用密碼重設頁面或單一密碼重設閘道太多次在 24 小時內<p>**Cancelled** – 使用者開始重設密碼，但是隨後按一下 [取消] 按鈕，取消部分到工作階段 <p>**連絡系統管理員** – 使用者在他無法解決的工作階段期間發生了問題讓使用者按一下 「 連絡您的系統管理員 」 連結，而不是完成密碼重設流程<p>**失敗** – 使用者無法重設密碼，可能是因為使用者未設定使用功能 (例如沒有授權、 遺失驗證資訊、 在內部管理密碼但是回寫已關閉)。<p>**成功** – 密碼重設成功。
 詳細資料| 請參閱下表
