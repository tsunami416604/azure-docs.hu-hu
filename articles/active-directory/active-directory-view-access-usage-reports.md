...
> [AZURE.NOTE]
>
- 當您啟用時，才有一些進階的異常和資源使用情況報告 [Azure Active Directory Premium](active-directory-get-started-premium.md)。 進階報告可協助您改善存取安全性、回應潛在威脅，以及存取裝置存取與應用程式使用情況的分析資料。
...
此報告包含登入 _所有_ 應用程式，您的目錄有存取權限，包括預先整合的 Microsoft 應用程式。
...
| 事件類型                                        | 已包含？     |
| ----------------------                                | ---------     |
| 登入 [存取面板](http://myapps.microsoft.com/)              | 是           |
| 登入 [Azure 管理入口網站](https://manage.windowsazure.com/)       | 是           |
| 登入 [Microsoft Azure 入口網站](http://portal.azure.com/)            | 是           |
| 登入 [Office 365 入口網站](http://portal.office.com/)            | 是           |
...
- 連絡使用者來確認活動
- 重設使用者的密碼
- [啟用多因素驗證](multi-factor-authentication-get-started.md) 提供額外的安全性

## 檢視或下載報告

1. 在 Azure 管理入口網站中，按一下 [ **Active Directory**, ，按一下您的組織目錄的名稱，然後按一下 **報表**。
2. 在 [報告] 頁面上，按一下您要檢視和/或下載的報告。

    > [AZURE.NOTE] 如果這是您已使用 Azure Active Directory 的報告功能的第一次，您會看到訊息，以選擇加入。 如果您同意，請按一下核取記號圖示繼續進行。
...
5. 如果適用的話，按一下 [ **下載** 來報告下載為逗號分隔值 (CSV) 格式的壓縮檔供離線檢視或封存。
    - 最多 75,000 個事件會包含在下載的檔案中。
    - 如需更多資料，請參閱 [Azure AD 報告 API](active-directory-reporting-api-getting-started.md)。

## 忽略事件

如果正在檢視任何異常報告，您可能會注意到您可以忽略顯示在相關報告中的各種事件。 忽略事件，只要在報表中的事件反白，然後按一下 **忽略**。  **略過** 按鈕將會永久移除報告中反白顯示的事件，僅能由獲得授權的全域管理員。
