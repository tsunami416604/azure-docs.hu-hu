...
4.  在 **RunMyProcess 在設定單一登入** 頁面上，按一下 **下載憑證**, ，然後儲存您的電腦上的憑證檔案。
...
8.  在 **驗證方法** 區段中，執行下列步驟 ︰
    ...
    1.  做為 **方法**, ，請選取 **SSO 與 Samlv2**。
    2.  在 Azure 網站中，在 **RunMyProcess 在設定單一登入** 對話方塊頁面中，複製 **SAML SSO URL** 值，並接著將它貼入 **SSO 重新導向** 文字方塊。
    3.  在 Azure 網站中，在 **RunMyProcess 在設定單一登入** 對話方塊頁面中，複製 **單一登出服務 URL** 值，並接著將它貼入 **登出重新導向** 文字方塊。
    4.  在 **名稱識別碼格式** 文字方塊中，輸入 **urn: oasis ︰ 名稱 ︰ tc: SAML:1.1:nameid-格式 ︰ emailAddress**。
...
1.  登入您 **RunMyProcess** 系統管理員身分的公司網站。
...
3.  在 **使用者設定** 區段中，執行下列步驟 ︰
    ...
    1.  型別 **名稱** 和 **電子郵件** 您想要佈建到相關文字方塊之有效 AAD 帳戶。
    2.  選取 **IDE 語言**, 、 **語言** 和 **設定檔**。
    3.  選取 **帳戶建立電子郵件傳送給我**。
    4.  按一下 [ **儲存**。

>[AZURE.NOTE]您可以使用任何其他 RunMyProcess 使用者帳戶建立工具或 Api 提供 RunMyProcess 所要佈建 Azure Active Directory 使用者帳戶。
