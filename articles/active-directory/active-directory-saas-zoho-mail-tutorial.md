...
8.  在 **SAML 驗證詳細資料** 區段中，執行下列步驟 ︰
    ...
    1.  在 Azure 網站中，在 **Zoho Mail 在設定單一登入** 對話方塊頁面中，複製 **遠端登入 URL** 值，並接著將它貼入 **登入 URL** 文字方塊。
    2.  在 Azure 網站中，在 **Zoho Mail 在設定單一登入** 對話方塊頁面中，複製 **遠端登出 URL** 值，並接著將它貼入 **登出 URL** 文字方塊。
    3.  在 Azure 網站中，在 **Zoho Mail 在設定單一登入** 對話方塊頁面中，複製 **變更密碼 URL** 值，並接著將它貼入 **變更密碼 URL** 文字方塊。
    4.  建立 **base 64 編碼** 檔案下載的憑證。  

        >[AZURE.TIP] 如需詳細資訊，請參閱 [如何轉換的文字檔案中的二進位檔案的憑證](http://youtu.be/PlgrzUZ-Y1o)

    5.  在記事本中開啟 base-64 編碼的憑證，將它的內容複製到剪貼簿，然後將它貼到 **PublicKey** 文字方塊。
    6.  做為 **演算法**, ，請選取 **RSA**。
    7.  按一下 [ **確定**。
...
4.  在 **將使用者新增** ] 對話方塊中，執行下列步驟 ︰
    ...
    1.  型別 **名字**, ，**姓氏**, ，**電子郵件識別碼**, ，**密碼** 有效的 Azure Active Directory 帳戶，您想要佈建相關的文字方塊。
    2.  按一下 [ **確定**。  

        >[AZURE.NOTE] Azure Active Directory 帳戶持有者將會收到一封電子郵件的連結，確認其帳戶之前就會生效。

>[AZURE.NOTE] 您可以使用任何其他 Zoho Mail 使用者帳戶建立工具或 Api 來佈建 AAD 使用者帳戶 Zoho Mail 所提供。
