...
1. Windows 10 中新增的**加入 Azure AD** 是一種自助式工作佈建體驗，適用於主要存取雲端中的資源 (例如 Office 365) 的工作者。
1. Windows 10 中的**加入網域**在連接到 Azure AD 時的使用性已有改善，適用於大量投資於內部部署應用程式和資源的組織。
1. **簡化的新式 BYOD 體驗**可讓使用者將工作帳戶新增至 Windows，而在個人裝置上輕鬆存取工作資源。
...
在 Windows 10 中不行。 在 Windows 8.1 裝置已加入網域的使用者可以 「 連接 」 其 MSA (例如 Hotmail、 即時、 Outlook、 XBox 等) 若要啟用 SSO 像 Live 服務特定體驗其網域帳戶，使用 Windows 存放區和裝置之間漫遊使用者設定。 在 Windows 10 中，MSA Connect 功能已被淘汰。 使用者可以新增一或多個 MSA 作為其他帳戶，以啟用對消費者服務 (如 Windows 市集) 的 SSO。 這作業可在 [設定]**** > [帳戶]**** > [您的帳戶]**** 中完成。
