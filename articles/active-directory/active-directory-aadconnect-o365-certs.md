...
其中 `(your_FS_name) `會取代您的組織使用，例如 fs.contoso.com 的同盟服務主機名稱。 如果您能夠成功確認上述兩個設定，您就不必執行任何動作。
...
`PS C:\ > C:>get-adfscertificate – CertificateType 權杖簽署。`
...
- 若要產生新的憑證，請執行下列命令在 PowerShell 命令提示字元: `PS C:\ > 更新 C:>get-adfscertificate – CertificateType 權杖簽署`。
...
>[AZURE.NOTE] 如果您需要支援多個頂層網域，例如 contoso.com 和 fabrikam.com，則您使用任何 Cmdlet 時必須搭配使用 SupportMultipleDomain 參數。 如需詳細資訊，請參閱〈支援多個頂層網域〉。
