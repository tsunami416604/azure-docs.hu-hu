...
6.  在 **搜尋方塊**, ，型別 **Panorama9**。

    ![應用程式庫](./media/active-directory-saas-panorama9-tutorial/IC790017.png "Application Gallery")

7.  在 [結果] 窗格中，選取 **Panorama9**, ，然後按一下 [ **完成** 加入應用程式。
...
4.  在 **Panorama9 在設定單一登入** ] 頁面上，下載您的憑證，按一下 [ **下載憑證**, ，然後將它儲存在本機電腦上。
...
8.  在 **設定** 區段中，執行下列步驟:
    ...
    1.  在 Azure 網站中，在 **Panorama9 在設定單一登入** 對話方塊頁面中，複製 **單一登入服務 URL** 值，並接著將它貼入 **身分識別提供者 URL** 文字方塊。
    2.  複製 **指紋** 值匯出的憑證，並接著將它貼入 **憑證指紋** 文字方塊。  
...
1.  登入您 **Panorama9** 系統管理員身分的公司網站。

2.  在頂端功能表中，按一下 [ **管理**, ，然後按一下 [ **使用者**。
...
>[AZURE.NOTE]您可以使用任何其他 Panorama9 使用者帳戶建立工具或 Api 來佈建 AAD 使用者帳戶提供 Panorama9。
