...
不支援具有多個 Azure AD Connect 同步處理伺服器連接到相同的 Azure AD 目錄，即使它們已同步處理互斥的一組物件 (除了 [開發用伺服器](#staging-server))。 可以嘗試這麼做，因為樹系中的一個網域無法從常見網路位置連線，或嘗試跨數個伺服器散佈同步處理負載。
...
在 Azure AD Connect 同步處理所提供的預設組態，會進行下列假設:
...
如果您的環境不符合這些假設，則會發生下列:
-   如果您有多個使用中的帳戶或多個信箱，同步處理引擎會挑選其中一個，並忽略其他。
-   如果您已連結的信箱，但沒有其他帳戶，這些帳戶將不會匯出至 Azure AD，而且使用者不會為任何群組的成員。 在 DirSync 中，已連結的信箱會顯示為一般信箱，因此這是刻意的不同行為，可更有效支援多個樹系案例。
...
不支援有一個以上的 Azure AD Connect 同步處理伺服器連線到單一 Azure AD 目錄 (除了 [開發用伺服器](#staging-server))。
...
每個樹系具有自己的 Exchange 組織，且在樹系間沒有 GALSync。 這個情況可能是合併/收購之後，或是在一個每個業務單位各自獨立作業的組織中。 在 Azure AD 中，這些樹系將會在相同的組織中並與統一 GAL 一起出現。
...
| 主題| |
| --------- | --------- |
| 使用管理單位的委派| [Azure AD 中的管理單位管理 ](active-directory-administrative-units-management.md)
...
若要了解如何安裝 Azure AD Connect，在這些情況下的，請參閱 [的 Azure AD Connect 的自訂安裝](active-directory-aadconnect-get-started-custom.md)。
