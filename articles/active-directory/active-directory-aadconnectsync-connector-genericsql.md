...
若要建立一般 SQL 連接器，在 **同步處理服務** 選取 **管理代理程式** 和 **建立**。 選取 **一般 SQL (Microsoft)** 連接器。

![CreateConnector](./media/active-directory-aadconnectsync-connector-genericsql/createconnector.png)

### 連線能力

連接器會使用 ODBC DSN 檔案進行連線。 建立資料來源名稱的檔案使用 **ODBC 資料來源** 底下的 [開始] 功能表中找到 **系統管理工具**。 在系統管理工具，建立 **檔案 DSN** 讓它可以提供給連接器。
...
每個物件類型會顯示為磁碟分割，並設定更進一步 **設定分割和階層**。
...
- 不會列出多重值和布林值屬性。
- DN 和錨點，無法使用相同的屬性，除非 **DN 是錨點** 已選取 [連線] 頁面上。
- 如果 **DN 是錨點** 已選取 [連線] 頁面中，在此頁面需要 DN 屬性。 這個屬性也會做錨點屬性。
![schema3b](./media/active-directory-aadconnectsync-connector-genericsql/schema3b.png)

### 結構描述 4 (定義屬性類型、參考和方向)

此頁面可讓您設定每個屬性的屬性類型，如整數、參考、字串、二進位或布林值和方向。 從頁面的所有屬性 **結構描述 2** 都會列出包括多重值的屬性。
...
- **巢狀資料表** 可以視為單一資料行的資料庫資料表。 Oracle 不會以任何特定順序儲存巢狀資料表的資料列。 不過，當您將巢狀資料表擷取至 PL/SQL 變數時，資料列便會有從 1 開始的連續下標。 這可讓您取得個別資料列的類似陣列存取。
- **VARRYS** 連接器中不支援。
...
如果您使用 **DN 是錨點** 則必須使用相同的物件類型與您要從參考。 您無法參考其他物件類型。
...
若要匯入物件的多重值的屬性，您必須提供以逗號分隔的資料表/檢視表名稱中 **名稱的多重資料表/檢視表** 和個別的聯結條件中 **聯結條件** 與父資料表。
...
- 如果使用者選擇追蹤差異變更觸發程序或快照集的方法，使用者可以提供記錄資料表或快照集資料庫中的 **歷程記錄資料表或快照集資料庫名稱** 方塊。
- 使用者也必須提供歷程記錄資枓表與父資料表之間的聯結條件。
範例：`Employee.ID=History.EmployeeID`
- 從歷程記錄資料表追蹤父資料表上的交易，使用者必須提供包含新增/更新/刪除等作業資訊的資料行名稱。
- 如果使用者選擇要追蹤差異變更浮水印，使用者必須提供資料行名稱，其中包含作業資訊 **上限標記資料行名稱**。 因此，連接器可以在執行差異匯入時考慮此資料行。
-  **變更型別屬性** 資料行是必要的變更類型。 此資料行會將主要資料表或多重值資料表中發生的變更，對應至差異檢視中的變更類型。 此資料行可以包含適用於屬性層級變更的 Modify_Attribute 變更類型，或適用於物件層級變更的 [新增]、[修改] 或 [刪除] 變更類型。 如果除了預設值 [新增]、[修改] 或 [刪除] 以外，使用者可以使用此選項來定義這些值。
...
- **加入預存程序名稱**, ，如果任何物件都有個別的資料表中插入作業的連接器，執行此預存程序。
- **更新預存程序名稱**, ，如果任何物件都有個別的資料表中更新的連接器，執行此預存程序。
- **刪除預存程序名稱**, ，如果任何物件都有個別的資料表中要刪除連接器，執行此預存程序。
