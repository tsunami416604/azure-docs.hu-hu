...
本教學課程的目標在於告訴您如何 Moxtra 整合 Azure Active Directory (Azure AD)。<br>Moxtra 整合與 Azure AD 讓您獲得下列優勢 ︰ 
...
![應用程式][4]<br>
6. 在 [搜尋] 方塊中，輸入 **Moxtra**。<br><br>
![建立 Azure AD 測試使用者](./media/active-directory-saas-moxtra-tutorial/tutorial_moxtra_01.png)<br>
7. 在 [結果] 窗格中，選取 **Moxtra**, ，然後按一下 [ **完成** 加入應用程式。
<br><br>
![建立 Azure AD 測試使用者](./media/active-directory-saas-moxtra-tutorial/tutorial_moxtra_02.png)<br>

##  設定並測試 Azure AD 單一登入
本節目標是說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試對 Moxtra 的 Azure AD 單一登入。

若要讓單一登入運作，Azure AD 必須知道 Moxtra 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者與 Moxtra 中的相關的使用者之間的連結關聯性。<br>
藉由指定的值建立此連結關聯性 **使用者名稱** 在 Azure AD 中的值為 **Username** Moxtra 中。
...
4. **[建立測試使用者 Moxtra](#creating-a-moxtra-test-user)** -若要連結到她在 Azure AD 代表 Moxtra 中有對應的 Britta Simon。
...
1. 在 Azure AD 入口網站上 **Moxtra** 應用程式整合頁面上，按一下 [ **設定單一登入** 開啟 **設定單一登入**  ] 對話方塊。
<br><br> ![設定單一登入][6] <br>

2. 在 **您希望使用者如何登入 Moxtra** 頁面上，選取 **Azure AD 單一登入**, ，然後按一下 [ **下一步**。
...
1. 在左邊工具列中，按一下 [ **系統管理員主控台 > SAML 單一登入**, ，然後 **新增**。
...
<br><br>![設定單一登入](./media/active-directory-saas-moxtra-tutorial/tutorial_moxtra_08.png) <br>
    ...
    b. 在 Azure 網站中，在 **Moxtra 在設定單一登入** 對話方塊頁面中，複製 **實體識別碼** 值，並接著將它貼入 **IdP 實體識別碼** 文字方塊。 

    c. 在 Azure 網站中，在 **Moxtra 在設定單一登入** 對話方塊頁面中，複製 **遠端登入 URL** 值，並接著將它貼入 **登入 URL** 文字方塊。 

    d. 在 **AuthnContextClassRef** 文字方塊中，tyoe **urn: oasis ︰ 名稱 ︰ tc: SAML:2.0:ac:classes:Password**。

    e. 在 Azure 網站中，在 **Moxtra 在設定單一登入** 對話方塊頁面中，複製 **名稱識別碼格式** 值，並接著將它貼入 **NameID 格式** 文字方塊。 

    f. 在記事本中開啟下載的憑證，複製內容，然後將它貼入 **憑證** 文字方塊。    

    g. 在 SAML 電子郵件網域文字方塊中，輸入您的 SAML 電子郵件網域。    
    > [AZURE.NOTE] 若要查看的步驟來驗證網域，請按一下"**我**」 下方。
...
1. 若要新增自訂屬性對應到您的 saml 權杖屬性組態，在最上層功能表中按一下 **屬性** 開啟 **SAML 權杖屬性** ] 對話方塊。 
...
1. 針對下表中的每個資料列，執行下列步驟：
  	...
  	| firstname      | givenname       |
  	| lastname       | surname         |
  	| idpid          | *\ < **實體識別碼** 值 **Moxtra 在設定單一登入** Azure 入口網站中的對話方塊 \ >* |
...
本節目標是讓 Britta Simon 到她的存取權授與 Moxtra 使用 Azure 單一登入。
<br><br>![指派使用者][200] <br>
