...
> [AZURE.NOTE] Azure AD 系統管理入口網站是 Azure 傳統入口網站的一部分。 雖然使用 Azure AD 不需要購買任何 Azure，存取此入口網站需要有效的 Azure 訂閱或 [Azure 試用版訂閱](http://azure.microsoft.com/pricing/free-trial/)。
...
> [AZURE.IMPORTANT] Azure AD Premium 和 Basic，以及 Enterprise Mobility Suite 訂閱會受限於其佈建目錄/租用戶。 訂用帳戶無法分割至不同目錄，或用來賦予權利給其他目錄中的使用者。 在目錄之間可以移動訂用帳戶，但是需要提交支援票證，如果是直接購買的情況，則需要取消並重新購買。

> [AZURE.IMPORTANT] 購買 Azure AD 時或 Enterprise Mobility Suite，透過大量授權訂閱啟動將會自動發生的協議時包含其他 Microsoft 線上服務，例如 Office 365。

付費 Azure AD 功能可以擴展目錄的範圍。 範例包括：
- 群組為基礎的應用程式指派，您所管理之特定應用程式下啟用。
- 進階和自助式群組管理功能都可在目錄組態或特定群組內。
- 進階安全性報告位於 [報告] 索引標籤
- 雲端應用程式探索會顯示在 Azure 入口網站的 [身分識別。
...
若要將授權指派給群組或個別使用者，選取您想要指派，按一下 [授權方案 **指派** 命令列上。

![啟用的試用版授權方案](./media/active-directory-licensing-what-is/assign_licenses.png)

一旦在所選的計劃設定] 對話方塊中，您可以選取使用者，並將他們加入 **指派** 右邊的資料行。 您可以逐頁查看使用者清單，或是利用使用者方格右上角的放大鏡來搜尋特定的個人。 若要指派群組，請選取 [群組] 從 **顯示** 功能表，然後按一下核取按鈕以重新整理顯示的指派權限。

![指派授權給群組](./media/active-directory-licensing-what-is/assign_licenses_to_groups.png)

您現在可以搜尋或逐頁查看群組，並將其加入 **指派** 資料行相同的方式。 您可以使用這些方式，在單一作業中指派使用者和群組的組合。 若要完成指派程序，請按一下頁面右下角的核取按鈕。
...
指派的授權，包括可用、 指派，且下一步訂用帳戶的生命週期事件的摘要檢視會顯示在 **授權** ] 索引標籤。
...
移除授權就和指派授權一樣簡單。 如果直接指派給使用者，或指派的群組，您可以移除其授權選取授權類型，選取 **移除**, 、 使用者或群組加入移除清單，然後確認動作。 或者，開啟授權類型、 選取特定使用者或群組，並點選 **移除** 命令列上。 若要結束使用者從群組繼承的授權，只要從群組移除使用者即可。
