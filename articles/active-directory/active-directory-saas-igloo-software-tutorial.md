...
1.  在 Azure 傳統入口網站中，在左的導覽窗格中，按一下 [ **Active Directory**。
...
6.  在 **搜尋方塊**, ，型別 **Igloo Software**。

    ![應用程式庫](./media/active-directory-saas-igloo-software-tutorial/IC783962.png "Application Gallery")

7.  在 [結果] 窗格中，選取 **Igloo Software**, ，然後按一下 [ **完成** 加入應用程式。
...
9.  在 **一般組態** 區段中，執行下列步驟:
    ...
    1.  在 **連接名稱** 文字方塊中，輸入您的組態的自訂名稱。
    2.  在 Azure 傳統入口網站上 **Igloo software 設定單一登入** ] 對話方塊頁面中，複製 **遠端登入 URL** 值，並接著將它貼入 **IdP 登入 URL** 文字方塊。
    3.  在 Azure 傳統入口網站上 **Igloo software 設定單一登入** ] 對話方塊頁面中，複製 **遠端登出 URL** 值，並接著將它貼入 **IdP 登出 URL** 文字方塊。
    4.  做為 **登出回應和要求的 HTTP 型別**, ，請選取 **POST**。
    5.  從下載的憑證建立文字檔。
...
10. 在 **回應和驗證設定**, ，執行下列步驟:
    ...
    1.  做為 **身分識別提供者**, ，請選取 **Microsoft ADFS**。
    2.  做為 **識別碼類型**, ，請選取 **電子郵件地址**。
    3.  在 **電子郵件屬性** 文字方塊中，輸入 **emailaddress**。
    4.  在 **First Name 屬性** 文字方塊中，輸入 **givenname**。
    5.  在 **最後一個名稱屬性** 文字方塊中，輸入 **姓氏**。

11. 執行以下步驟來完成設定：

    ![登入時建立使用者](./media/active-directory-saas-igloo-software-tutorial/IC783972.png "User creation on Sign in")

    1.  做為 **在登入的使用者建立**, ，請選取 **網站中建立新的使用者，登入時**。
    2.  做為 **登入設定**, ，請選取 **使用 SAML] 按鈕，在 [登入] 畫面上的**。
    3.  按一下 [ **儲存**。
