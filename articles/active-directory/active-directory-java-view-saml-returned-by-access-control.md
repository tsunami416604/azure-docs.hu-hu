...
本指南說明如何檢視 Azure 存取控制服務 (ACS) 傳回給應用程式的基本安全性聲明標記語言 (SAML)。 本指南是根據 [方式來使用 Azure 存取控制服務驗證 Web 使用者 Eclipse []][] 主題，提供程式碼來顯示 SAML 資訊。 完成後的應用程式如下所示：
...
若要完成本指南中的工作，完成的範例， [方式來使用 Azure 存取控制服務驗證 Web 使用者 Eclipse []][] 作為本教學課程的起點。
...
1. 在電腦模擬器中執行您的應用程式或部署至 Azure，記載的步驟 [方式來使用 Azure 存取控制服務驗證 Web 使用者 Eclipse []][]。
