<properties
   ...
   description="了解如何從 DirSync 升級至 Azure AD Connect。此文章說明您目前的 Windows Azure AD 同步作業工具 (DirSync) 升級至 Azure AD Connect 的步驟。"
...
# Windows Azure Active Directory 同步處理 (DirSync) 升級至 Azure AD Connect
...
會顯示有關 DirSync 所使用之現有 SQL Server 資料庫伺服器資訊。 如果需要，請進行適當的調整。 按一下 [下一步]**** 繼續安裝。
...
若要繼續進行就地升級時，按一下此訊息旁的核取方塊: **繼續在此電腦上升級 DirSync。**
若要執行 [平行部署](#parallel-deployment) 改為將匯出 DirSync 組態設定，並移動到新的伺服器。
5. 提供您目前用來連線到 Azure AD 之帳戶的密碼。 這必須是目前 DirSync 所使用的帳戶。
...
7. 按 [下一步]****。
8. 在 [準備設定]**** 頁面上，保持核取 [設定一完成，即開始同步處理程序]****。 伺服器將處於 [預備模式](active-directory-aadconnectsync-operations.md#staging-mode) 如此變更將不會匯出至 Azure AD 在此階段。
9. 按一下 [Install]****。
