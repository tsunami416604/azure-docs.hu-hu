<properties 
    ...
    description="了解如何使用 Azure Active Directory 中的 Kudos，來啟用單一登入、 自動化佈建和更多功能!" 
...
8.  在 [SSO]**** 區段中，執行下列步驟：
    ...
    1.  在 Azure 網站中，在 **Kudos 在設定單一登入** 對話方塊頁面中，複製 **單一登入服務 URL** 值，並接著將它貼入 ** 登入 URL ** 文字方塊。
    2.  從您下載的憑證建立「Base-64 編碼」****檔案。
        >[AZURE.TIP]
        如需詳細資訊，請參閱 [如何將二進位檔案憑證轉換成文字檔](http://youtu.be/PlgrzUZ-Y1o)

    3.  在記事本中開啟您的 base-64 編碼的憑證，將它的內容複製到您的剪貼簿，然後貼至 [X.509 憑證]**** 文字方塊中。
    4.  在 Azure 網站中，在 **Kudos 在設定單一登入** 對話方塊頁面中，複製 **單一登出服務 URL** 值，並接著將它貼入 ** 登出以 URL ** 文字方塊。
...
5.  在 [加入使用者]**** 區段中，執行下列步驟：
    ...
    ![新增使用者](./media/active-directory-saas-kudos-tutorial/IC787810.png "Add a User")
...
2.  在 **Kudos * * 應用程式整合頁面上，按一下 [* * 將使用者指派**。
