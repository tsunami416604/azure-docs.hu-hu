...
| Felügyeleti felügyelet|A szervezetek tartományokat, szervezeti egységeket és csoportokat fognak használni az AD-ben a rendszergazdai jogosultságok delegálásához a címtár és az általa felügyelt erőforrások kezeléséhez.| Az Azure AD [beépített szerepköröket](./active-directory-users-assign-role-azure-portal.md) biztosít az Azure ad szerepköralapú hozzáférés-vezérlési (Azure ad RBAC) rendszerével, amely korlátozott támogatást nyújt az [Egyéni szerepkörök létrehozásához](../users-groups-roles/roles-custom-overview.md) , hogy az identitásrendszer, az alkalmazások és az általa felügyelt erőforrások jogosultsági szintű hozzáférését delegálja.</br>A Szerepkörök kezelése [Privileged Identity Management (PIM)](../privileged-identity-management/pim-configure.md) segítségével biztosítható, hogy igény szerint, időben korlátozott vagy munkafolyamat-alapú hozzáférést biztosítson a Kiemelt szerepkörökhöz. |
