...
7. 在您的主要活動建立 AuthenticationContext 的執行個體。 這個呼叫的詳細資料超出範圍的此讀我檔案，但您可以取得一個不錯的起點，藉由查看 [Android 原生用戶端範例](https://github.com/AzureADSamples/NativeClient-Android)。 以下是範例：
...
11. **Broker**:
 ...
 開發人員必須註冊特殊 redirectUri 供 Broker 使用。 RedirectUri 的 msauth://packagename/Base64UrlencodedSignature 的格式。 您可以使用指令碼 "brokerRedirectPrint.ps1" 或使用 API call mContext.getBrokerRedirectUri，以取得應用程式的 redirecturi。 簽章與您的簽署憑證有關。
