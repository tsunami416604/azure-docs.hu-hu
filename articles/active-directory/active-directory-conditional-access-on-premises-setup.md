...
* Windows Server 2012 R2 Web Application Proxy Microsoft Azure Active Directory Connect (Azure AD Connect)。 [下載 Azure AD Connect](http://www.microsoft.com/en-us/download/details.aspx?id=47594)。
* 已驗證的網域。
...
5.  在 [部署和管理]**** 區段之下，遵循步驟 1 到 3 來整合 Azure Active Directory 與您的本機目錄。
  ...
  3. 驗證及管理目錄同步作業。 在此步驟中的單一登入的指示。
  >[AZURE.NOTE] 上述連結的文件中所述，使用 AD FS 設定同盟。 
  >[AZURE。附註] 您不需要設定任何預覽功能。
...
其中 `您的網域名稱` 是您設定與 Azure Active Directory 的網域名稱。 例如，如果您的網域名稱為 contoso.com，URL 將是：
...
[Android 主題的 Azure Authenticator](active-directory-conditional-access-azure-authenticator-app.md) 如何在您的 Android 裝置上安裝 Azure authenticator 應用程式及新增工作帳戶中的指示。 在 Android 裝置上成功建立工作帳戶時，該裝置是加入至組織的工作場所。
...
**如果您使用 Windows 8.1 裝置**, ，您可以加入您的裝置，請前往 **電腦設定 * * > * * 網路** > **工作場所**。
