...
外部 URL 通訊協定 | <p>根據預設，應用程式會使用 HTTPS 通訊協定發佈。 此服務會自動重新導向輸入含 http 之 URL 的使用者。</p> <p>若要為內部應用程式啟用 HTTP，您必須設定為 [通過的預先驗證方法，然後您就可以將外部 URL 通訊協定從 HTTPS 變更為 HTTP。 請注意，使用 HTTP 發佈應用程式可能會產生您的應用程式和使用者的安全性問題。</p> <p>您可以插入自訂網域，而不是使用預設後置詞 msappproxy.net。 如需詳細資訊，請參閱 [使用自訂網域](active-directory-application-proxy-custom-domains.md)。</p>
