...
2. Kattintson a felhasználónevére a jobb felső sarokban. Ellenőrizze, hogy egy alkalmazásproxy használó könyvtárba van bejelentkezve. Ha módosítsa a könyvtárakat van szüksége, válassza ki a **címtár váltása** és a egy könyvtárat, amely használja az Application Proxy kiválasztása.
3. A bal oldali navigációs panelen válassza a **Azure Active Directory**. 
4. A **kezelés**válassza **alkalmazásproxy**.
5. Válassza ki **összekötőszolgáltatás letöltése**.
    
    ![Összekötőszolgáltatás letöltése](./media/application-proxy-add-on-premises-application/application-proxy-download-connector-service.png)

6. Olvassa el a szolgáltatási feltételeket.  Amikor elkészült, válassza ki a **feltételek elfogadása és letöltés**.
7. Az ablak alján válassza **futtatása** az összekötő telepítéséhez. Megnyílik a telepítési varázsló. 
8. Kövesse a varázsló utasításait, telepítse a szolgáltatást. Amikor a rendszer kéri, hogy az összekötő regisztrálására az Azure AD-bérlő a Proxy, adja meg az alkalmazás rendszergazdai hitelesítő adatait.
...
2. A bal oldali navigációs panelen válassza ki a **Azure Active Directory**, majd válassza ki **alkalmazásproxy** alatt a **kezelés** szakaszban. Az összes összekötőcsoportok és összekötők ezen az oldalon jelennek meg. 
3. Az összekötő ellenőrzéséhez a részletek megtekintéséhez. Az összekötők alapértelmezés szerint ki kell terjeszteni. Ha meg szeretné tekinteni az összekötő nincs kibontva, bontsa ki az összekötő részleteinek megtekintéséhez. Egy aktív zöld címke azt jelzi, hogy az összekötő képes-e csatlakozni a szolgáltatáshoz. Azonban annak ellenére, hogy a címke zöld, hálózati probléma továbbra is letilthatják az összekötő-üzenetek fogadása. 
...
3. Ha a szolgáltatás állapota nem **futtató**, kattintson a jobb gombbal az egyes szolgáltatásokhoz válasszon, majd válassza **Start**. 
...
2. A bal oldali navigációs panelen válassza ki a **Azure Active Directory**.
3. Válassza ki **vállalati alkalmazások**, majd válassza ki **új alkalmazás**.
4. Válassza ki **helyszíni alkalmazás**.  
5. Az a **saját helyszíni alkalmazás hozzáadása** területén adja meg a következő információkat az alkalmazásról:
...
2. Válassza ki **bevezetés**, majd válassza ki **felhasználó hozzárendelése teszteléshez**.
3. A **felhasználók és csoportok**válassza **felhasználó hozzáadása**.
4. A **-hozzárendelés hozzáadása**válassza **felhasználók és csoportok**. A **felhasználók és csoportok** szakasz jelenik meg. 
5. Válassza ki a hozzáadni kívánt fiókot. 
6. Válasszon **kiválasztása**, majd válassza ki **hozzárendelése**.
...
2. Jelentkezzen be az előző szakaszban létrehozott felhasználóként.
