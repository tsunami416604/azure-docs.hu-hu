...
Az Azure Active Directory (Azure AD) tartalmaz egy [létesítési szolgáltatás felhasználói fiók](user-provisioning.md) , amelynek segítségével automatizálhatja a kiépítési megszüntetést felhasználói fiókok a SaaS-alkalmazások és más rendszerek, teljes körű identitás-életciklus céljából felügyeleti. Az Azure AD támogatja az összes, az alkalmazások és rendszerek "Kiemelt" szakaszában összekötők előre integrált felhasználókiépítés a [Azure AD alkalmazáskatalógusában](https://azuremarketplace.microsoft.com/marketplace/apps/category/azure-active-directory-apps?page=1&subcategories=featured).
...
* **Az Azure portal** – Ez a cikk elsősorban ismerteti a jelentés adatainak beolvasása a [az Azure portal](https://portal.azure.com), amely biztosít egy összesítő jelentés kiépítés és is részletes kiépítés auditnaplók egy adott az alkalmazás.
...
* **Cél rendszer** -, akik az Azure AD létesítési szolgáltatás szinkronizálja a tárházban. Ez általában az SaaS-alkalmazás (példák: A Salesforce, ServiceNow, a G Suite, Dropbox Business), de néhány esetben lehet egy helyszíni rendszer, például az Active Directory (Példa: Munkanapi bejövő szinkronizálási az Active Directory).

## <a name="getting-provisioning-reports-from-the-azure-portal"></a>Bevezetés a kiépítési jelentéseket az Azure Portalról

Jelentési adatok egy adott alkalmazás első üzembe helyezés esetében először indítása a [az Azure portal](https://portal.azure.com) , és keresse meg a vállalati alkalmazás, amelynek kiépítés van konfigurálva. Például ha a felhasználók a LinkedIn jogosultságszint-emelés szeretne kiépíteni, a navigációs az alkalmazás részletes útvonala:
...
1. Esemény importálása: Célrendszer kérdezi le a lekért felhasználói meglétének ellenőrzése.
1. Szinkronizálási szabály esemény: Felhasználói adatokat a forrás- és rendszerek értékeli ki a konfigurált attribútum-leképezési szabályok és meghatározni, hogy milyen műveletet, ha vannak szabad elvégezni Hatókörszűrő szemben.
1. Esemény exportálása: Ha a szinkronizálási szabály esemény során, hogy a művelet legyen-e végre (Hozzáadás, Update, Delete), majd a művelet eredményeit rögzíti az exportálási esemény.

   ![Példa: A tevékenységek és állapotát megjelenítő lapon naplózása](./media/check-status-user-account-provisioning/audit_logs.PNG)
...
1. Az a **kategória** menüjében válassza **fiók üzembe helyezésének**.
1. Az a **dátumtartomány** menüben válassza ki a keresendő dátumtartományt.
1. Az a **keresési** sáv, adja meg a keresni kívánt felhasználó felhasználói azonosítója. Azonosító értékének formátuma meg kell egyeznie a függetlenül van kiválasztva, mint az elsődleges egyező azonosító attribútumleképezés konfigurációjában (például userPrincipalName vagy az alkalmazott azonosítója szám). A szükséges azonosító értéke meg fognak jelenni a cél(ok) oszlop.
1. Nyomja le az Enter billentyűt a keresés. A legutóbbi kiépítési események először vissza kell.
1. Ha eseményeket ad vissza, vegye figyelembe a tevékenység típusát, és hogy azok sikeres vagy sikertelen volt. Ha nem jár eredménnyel, majd azt azt jelenti, hogy a felhasználó nem létezik vagy nem még észlelt a kiépítési folyamat által, ha a teljes szinkronizálás még nem fejeződött be.
1. Kattintson az egyes események megtekintéséhez kiterjesztett adatait, például az összes felhasználói tulajdonságok, amelyek lekérni, értékeli ki, vagy az esemény részeként írt.
