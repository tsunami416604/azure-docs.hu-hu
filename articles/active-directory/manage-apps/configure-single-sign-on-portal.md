...
1. Az ismertetett információk, kérje meg az alkalmazás szállítójához [alapszintű SAML-beállítások konfigurálása](#configure-basic-saml-options).
1. Ebben az oktatóanyagban a lépéseket teszteléséhez használja a egy nem éles környezetben. Ha nem rendelkezik nem éles Azure AD-környezettel, szerezze be az [egy hónapos próbaverziót](https://azure.microsoft.com/pricing/free-trial/).
1. Jelentkezzen be a [az Azure portal](https://portal.azure.com) egy felhőalapú alkalmazás, vagy egy alkalmazás az Azure AD-bérlő rendszergazdája.
...
1. A **kezelés** a a **Azure Active Directory** navigációs panelt, amely akkor jelenik meg, válassza ki **vállalati alkalmazások**. Megjelenik egy véletlenszerűen vett minta az alkalmazások az Azure AD-bérlőben.
1. Az a **alkalmazástípus** menüjében válassza **minden alkalmazás**, majd válassza ki **alkalmaz**.
1. Adja meg az alkalmazás nevét, amelyhez egyszeri bejelentkezést szeretne konfigurálni. Beírhatja például **GitHub-tesztelési** konfigurálnia az alkalmazást, akkor a [alkalmazás hozzáadása](add-application-portal.md) rövid.  

   ![Az alkalmazás keresősávba bemutató képernyőkép](media/configure-single-sign-on-portal/azure-portal-application-search.png)

1. Válassza ki az alkalmazást, amelynek szeretné az egyszeri bejelentkezés konfigurálása.
1. Alatt a **kezelés** szakaszban jelölje be **egyszeri bejelentkezési**.
1. Válassza ki **SAML** egyszeri bejelentkezés konfigurálásához. A **állítsa be egyszeri bejelentkezést az SAML - előzetes verzió** lap jelenik meg.
...
1. Az alapvető SAML-konfigurációs beállítások szerkesztéséhez válassza ki a **szerkesztése** (a Ceruza) ikonra a jobb felső sarkában a **alapszintű SAML-konfigurációja** szakaszban.

     ![Alapszintű SAML konfigurációs beállításainak módosítása](media/configure-single-sign-on-portal/basic-saml-configuration-edit-icon.png)

1. Az oldalon a megfelelő mezőkben adja meg az információkat, az 1. lépésben az alkalmazás gyártója által biztosított.
1. A lap tetején válassza **mentése**.
...
1. Módosíthatja a **neve azonosító értékét**, jelölje be a **szerkesztése** (a Ceruza) ikonra a **neve azonosító értékét** mező. A megfelelő módosításokat az azonosító formátuma és a forrás, igény szerint. Ha elkészült, mentse a módosításokat. Jogcímek testreszabása kapcsolatos további információkért lásd: a [vállalati alkalmazásokhoz SAML-jogkivonatban kiadott jogcímek testreszabása](../develop/active-directory-saml-claims-customization.md) útmutató cikk.
1. Jogcím hozzáadásához válassza **hozzáadása új jogcímet** az oldal tetején. Adja meg a **neve** , és válassza ki a megfelelő forrás. Ha a **attribútum** forrás, kell választania a **forrásattribútum** is használni szeretné. Ha a **fordítási** forrás, kell választania a **átalakítási** és **paraméter 1** használni kívánt.
1. Kattintson a **Mentés** gombra. Az új jogcímet a tábla jelenik meg.
...
1. Az a **SAML-aláíró tanúsítvány** szakaszban jelölje be **új tanúsítvány**.
1. Az új tanúsítvány sorban jelenik meg, állítsa be a **lejárati dátum**. Rendelkezésre álló konfigurációs lehetőségekkel kapcsolatos további információkért lásd: a [speciális tanúsítvány-aláírási beállítások](certificate-signing-options.md) cikk.
1. Válassza ki **mentése** felső részén a **SAML-aláíró tanúsítvány** szakaszban.
...
1. A bal oldali navigációs panelen az alkalmazáshoz, jelölje be a **felhasználók és csoportok**.
1. Válassza ki **felhasználó hozzáadása**.
1. Az a **hozzárendelés hozzáadása** szakaszban jelölje be **felhasználók és csoportok**.
1. Egy adott felhasználó megkereséséhez írja be a felhasználói nevet a **tag kiválasztása vagy egy külső felhasználó meghívása** mezőbe. Ezután válassza ki a felhasználó profilfényképének vagy embléma, és válassza a **kiválasztása**.
1. Az a **hozzárendelés hozzáadása** szakaszban jelölje be **hozzárendelése**. Amikor végzett, a kijelölt felhasználók megjelennek a **felhasználók és csoportok** listája.
...
1. Görgessen a **egyszeri bejelentkezésre az ellenőrzése \<applicationName >** szakaszban. A jelen oktatóanyag esetében ez a szakasz hívja **állítsa be a GitHub-tesztelési**.
1. Válassza ki **teszt**. Megjelennek a tesztelési beállítások.
1. Válassza ki **jelentkezzen be a jelenlegi felhasználói fiók**. Ez a vizsgálat Ha egyszeri bejelentkezést működik, a rendszergazda először megtekintheti, hogy
...
1. Másolja ki és illessze be a részleteket a **hiba részleteit ismertető** mezőbe.
    ...
    ![A "Mi does hiba megjelenését, például" mező a megoldási útmutató használata](media/configure-single-sign-on-portal/error-guidance.png)

1. Válassza ki **megoldási útmutató**. Az alapvető ok és megoldás útmutatást jelennek meg.  Ebben a példában a felhasználó az alkalmazás nem lett hozzárendelve.
1. Olvassa el a megoldási útmutató, és ezután, ha lehetséges, hárítsa el a problémát.
