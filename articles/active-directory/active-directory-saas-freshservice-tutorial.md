...
4.  在 **FreshService 在設定單一登入** ] 頁面上，下載您的憑證，按一下 [ **下載憑證**, ，然後將儲存在本機電腦上的憑證檔案。
...
8.  在 **安全性** 區段中，執行下列步驟 ︰
    ...
    1.  交換器 **單一登 OnON**。
    2.  選取 **SAML SSO**。
    3.  在 Azure 網站中，在 **FreshService 在設定單一登入** 對話方塊頁面中，複製 **遠端登入 URL** 值，並接著將它貼入 **SAML 登入 URL** 文字方塊。
    4.  在 Azure 網站中，在 **FreshService 在設定單一登入** 對話方塊頁面中，複製 **遠端登出 URL** 值，並接著將它貼入 **登出 URL** 文字方塊。
    5.  複製 **指紋** 值匯出的憑證，並接著將它貼入 **安全性憑證指紋** 文字方塊。
...
1.  登入您 **FreshService** 系統管理員身分的公司網站。
...
5.  在 **新要求者** 區段中，執行下列步驟 ︰
    ...
    1.  輸入 **名字** 和 **電子郵件** 有效的 Azure Active Directory 帳戶，您想要佈建到相關文字方塊的屬性。
    2.  按一下 [ **儲存**。

    >[AZURE.NOTE] Azure Active Directory 帳戶持有者將會收到電子郵件，其中包含的連結，確認其帳戶之前就會生效

>[AZURE.NOTE] 您可以使用任何其他 FreshService 使用者帳戶建立工具或 Api 來佈建 AAD 使用者帳戶 FreshService 所提供。
