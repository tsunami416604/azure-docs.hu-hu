...
本教學課程的目標在於告訴您如何使用 Azure Active Directory (Azure AD) 整合 @Task。<br>與 Azure AD 整合 @Task 可以提供下列優點: 
...
若要讓單一登入運作，Azure AD 必須知道 @Task 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者和 @Task 中的相關使用者之間建立連結關聯性。<br>
藉由指定的值建立此連結關聯性 **使用者名稱** 在 Azure AD 中的值為 **Username** @Task 中。
...
<br><br>![設定應用程式設定][8] <br>
     ...
     a. 在 **登入 URL** 文字方塊中，輸入您的使用者用於登入 @Task 應用程式使用的 URL (例如:*https://<Tenant name>.attask ondemand.com*)。
...
<br><br>![設定單一登入][23]<br>
    ...
    a. 做為 **類型**, ，請選取 **SAML 2.0**。

    b. 選取 **服務提供者 ID**。

    c. 在 Azure 入口網站中，複製 **遠端登入 URL**, ，然後將它貼入 **登入入口網站 URL** 文字方塊。

    d. 在 Azure 入口網站中，複製 **單一登出服務 URL**, ，然後將它貼入 **登出 URL** 文字方塊。

    e. 在 Azure 入口網站中，複製 **變更密碼 URL**, ，然後將它貼入 **變更密碼 URL** 文字方塊。
...
<br><br>![建立 @Task 測試使用者][21] <br>
    ...
    c. 在 **電子郵件地址** 文字方塊中，在 Azure Active Directory 中輸入 Britta Simon 的電子郵件地址。
