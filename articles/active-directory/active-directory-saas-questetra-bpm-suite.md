...
4. **[建立測試使用者 Questetra BPM 套件](#creating-a-questetra-bpm-suite-test-user)** -使對應的 Britta Simon 連結到她在 Azure AD 代表 Questetra BPM 組件中。
...
3. 在不同的網頁瀏覽器視窗中，登入您 **Questetra BPM 套件** 系統管理員身分的公司網站。
...
6. 在 Azure 網站中，在 **設定應用程式設定** 對話方塊頁面上，執行下列步驟 ︰ <br><br>![設定應用程式設定][13]
    ...
    b. 在您 **Questetra BPM 套件** 公司網站，在預存程序資訊] 區段中，複製 **實體識別碼**, ，然後將它貼入 **簽發者 URL** 文字方塊。

    c. 在您 **Questetra BPM 套件** 公司網站，在預存程序資訊] 區段中，複製 **ACS URL**, ，然後將它貼入 **回覆 URL** 文字方塊。
...
8. 在您 **Questetra BPM 套件** 公司網站中，執行下列步驟 ︰ <br><br>![設定單一登入][15]
    ...
    b. 在 Azure 入口網站中，複製 **簽發者 URL** 值，並接著將它貼入 **實體識別碼** 文字方塊。

    c. 在 Azure 入口網站中，複製 **單一登入服務 URL** 值，並接著將它貼入 **登入頁面 URL** 文字方塊。
...
9. 在 Azure AD 入口網站中，選取單一登入設定確認，，然後按一下 [ **下一步**。 <br><br>![什麼是 Azure AD Connect][17]
...
3.  在 [新使用者] 對話方塊中，執行下列步驟 ︰ <br><br>![建立測試使用者][300] 
    ...
    b. 在 **電子郵件** 文字方塊中，在 Azure AD 中輸入 Britta 的使用者名稱。
