...
1. Importálnia kell a metaadat-tanúsítványt az F5-ben (fejléc alapján), amelyet később a telepítési folyamat során fog használni. Nyissa meg a **System > Certificate management > forgalom-tanúsítványkezelő > > SSL-tanúsítványok listáját**. Kattintson a jobb oldali sarokban található **Importálás** elemre.

    ![F5 (fejléc alapú) konfiguráció](./media/headerf5-tutorial/configure12.png)
 
1. Emellett szüksége lesz egy **SSL** -tanúsítványra is az állomásnév`headerapp.superdemo.live`() számára, ebben a példában a helyettesítő tanúsítványt használtuk.

    ![F5 (fejléc alapú) konfiguráció](./media/headerf5-tutorial/configure13.png)

1. Ugrás – **F5 (fejléc alapú) Big-IP kattintson a hozzáférés > irányított konfiguráció > összevonási > SAML-szolgáltató elemre**.
...
1. Hozzon létre egy új virtuális kiszolgálót, és válassza a **cél címet**, az átirányítási **portot** pedig nem kötelező megadni. Válassza ki a korábban feltöltött, illetve a **hozzá tartozó titkos kulcsot**tartalmazó **Wild Card-tanúsítványt** (vagy az alkalmazáshoz feltöltött tanúsítványt).
...
1. Az egyszeri bejelentkezés területen válassza a **http-fejléc-alapú**lehetőséget. Az alkalmazás alapján további fejléceket is hozzáadhat. Tekintse meg az SAML-munkamenetek változóinak listáját a függelékben.

    ![F5 (fejléc alapú) konfiguráció](./media/headerf5-tutorial/configure07.png) 

1. Lépjen kapcsolatba az [F5 (fejléc alapú) ügyfél-támogatási csapattal](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) a **végpont-ellenőrzési tulajdonságok** dokumentációjának részleteiért.

    ![F5 (fejléc alapú) konfiguráció](./media/headerf5-tutorial/configure08.png)

1. Lépjen kapcsolatba az [F5 (fejléc alapú) ügyfél-támogatási csapattal](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) a **munkamenet-felügyeleti tulajdonságok** dokumentációjának részleteivel.
