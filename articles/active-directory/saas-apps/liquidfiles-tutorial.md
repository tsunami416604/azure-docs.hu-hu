...
1. Az **egyszeri bejelentkezés konfigurálása** lapon hajtsa végre a következő lépéseket
    ...
    e. A név azonosítójának formázása szövegmezőbe írja be az `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress`értéket.

    f. A Authn-környezet szövegmezőbe írja be az `urn:oasis:names:tc:SAML:2.0:ac:classes:PasswordProtectedTransport`értéket.
