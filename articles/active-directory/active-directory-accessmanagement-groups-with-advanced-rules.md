<properties
    ...
    描述 = 「 如何-的來建立進階的規則群組，包括支援的運算式規則運算子和參數。 」
    服務 = [active directory]
    documentationCenter=""
    作者 ="curtand"
    管理員 ="stevenpo"
...
**若要建立進階的規則**
在 Azure 網站中，在群組 **設定** 索引標籤上，選取 **進階的規則** 選項，然後在進階規則所提供的文字方塊中輸入。 您可以使用下列資訊建立進階規則。
...
包含引號 " 的字串應該使用 ' 字元逸出，例如 user.department -eq "Sa`"les"。
...
* [疑難排解群組動態成員資格](active-directory-accessmanagement-troubleshooting.md)
