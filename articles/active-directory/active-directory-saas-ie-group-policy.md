...
4. 完成 [新增共用精靈]**** 並設定權限以確保可以從您的使用者電腦存取該精靈。 [進一步了解共用。](https://technet.microsoft.com/library/cc753175.aspx)
...
1. 決定您想要根據 [電腦設定]**** 或 [使用者設定]**** 部署延伸模組。 當使用 [電腦設定](https://technet.microsoft.com/library/cc736413%28v=ws.10%29.aspx), ，不論哪一個使用者登入電腦上，將會安裝延伸模組。 相反地，與 [使用者設定](https://technet.microsoft.com/library/cc781953%28v=ws.10%29.aspx), ，使用者必須安裝，無論哪一台電腦在其登入的延伸模組。
...
5. 在 [部署軟體]**** 提示中，針對您的部署方法選取 [已指派]****。 然後按一下 [確定]****。
    ...
    ![選取 ](./media/active-directory-saas-ie-group-policy/deployment-method.png)
...
1. 在 **群組原則管理編輯器** ] 視窗中，移至下列路徑，視哪種類型的組態中選擇其中一項 [步驟 3: 指定安裝套件](#step-3-assign-the-installation-package):
    - `電腦設定/原則/系統管理範本 /windows 元件/Internet Explorer/安全性功能/附加的管理`
    - `使用者設定/原則/系統管理範本 /windows 元件/Internet Explorer/安全性功能/附加的管理`
...
3. 在 [附加元件清單]**** 視窗中，選取 [已啟用]****。 然後，在 [選項]**** 區段中，按一下 [顯示...]****。
    ...
    ![按一下 ](./media/active-directory-saas-ie-group-policy/edit-add-on-list-window.png)
...
1. 在 [群組原則管理編輯器]**** 視窗中，移至下文列出的路徑。 請注意，這項組態設定僅提供於 [使用者設定]****。
    - `使用者設定/原則/系統管理範本 /windows 元件/Internet Explorer /`

2. 尋找名為 [開啟表單上使用者名稱和密碼的自動完成功能]**** 的設定。
    > [AZURE.NOTE] 舊版的 Active Directory 可能會列出這項設定，但名為 [不允許以自動完成儲存密碼]****。 該設定的組態不同於本教學課程中所描述的設定。

    ![記得要在 ](./media/active-directory-saas-ie-group-policy/disable-auto-complete.png)

3. 以滑鼠右鍵按一下上述設定，然後選取 [編輯]****。

4. 在標題為 [開啟表單上使用者名稱和密碼的自動完成功能]**** 的視窗中選取 [停用]****。

    ![選取 ](./media/active-directory-saas-ie-group-policy/disable-passwords.png)
...
1. 如果您部署使用 **電腦設定**, ，登入用戶端電腦所屬的組織單位，您在選取登入 [步驟 2: 建立群組原則物件](#step-2-create-the-group-policy-object)。 如果您使用 [使用者組態]**** 進行部署，請務必以屬於該 OU 的使用者身分登入。
