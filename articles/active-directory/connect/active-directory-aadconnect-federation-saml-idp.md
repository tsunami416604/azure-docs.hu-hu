...
description: Ez a dokumentum ismerteti az egyszeri bejelentkezéshez a SAML 2.0-s megfelelő Idp használ.
...
A Microsoft támogatja a bejelentkezést egy Microsoft felhőszolgáltatásra, például az Office 365, az a megfelelően konfigurált SAML 2.0-s profil-alapú IdP integrációja. SAML 2.0 identitás-szolgáltatóktól harmadik féltől származó termékekre, és ezért a Microsoft nem támogatja a központi telepítés konfigurációs, ajánlott eljárások ezekkel kapcsolatos hibaelhárítás. Egyszer megfelelően konfigurálva, az identitásszolgáltató tesztelhető a megfelelő konfigurációt a Microsoft-kapcsolat Analyzer eszközt, amely az alábbi további részletes leírását lásd a SAML 2.0 integrációja. További információ a SAML 2.0 SP-Lite profil-alapú identitás-szolgáltatójánál kérje meg a szervezet, amely megadja azt.
...
Más ügyfelek a bejelentkezés forgatókönyvet, és a SAML 2.0 identitásszolgáltató nem érhetők el. Például a Lync 2010 asztali ügyfél nem nem tud bejelentkezni a szolgáltatásba a SAML 2.0 identitásszolgáltatóval egyszeri bejelentkezés beállítása.

## <a name="azure-ad-saml-20-protocol-requirements"></a>Azure AD SAML 2.0 protokoll követelmények
Ez a dokumentum részletes követelményeket a protokoll és, hogy a SAML 2.0 identitásszolgáltató kell valósítania az Azure AD-bejelentkezés engedélyezése (például az Office 365) egy vagy több Microsoft felhőszolgáltatások összevonni kívánt formázási üzenetet tartalmaz. A SAML 2.0 függő entitás (SP-STS), egy Microsoft felhőszolgáltatásra, ebben a forgatókönyvben használt az Azure AD.
...
Kötések a szükséges átviteli kapcsolatos üzeneteket paraméterek. Az alábbi követelmények vonatkoznak a kötések
...
|Attribútum|Leírás|
| ----- | ----- |
|NameID|A helyességi feltétel értéke ugyanaz, mint az Azure AD-felhasználó ImmutableID kell lennie. Lehet, hogy a legfeljebb 64 alfanumerikus karakterből állhat. A biztonságos nem HTML-formátumú karaktereket kell kódolni, például a "+" karaktert jelenik meg, mint ".2B".|
|IDPEmail|Az egyszerű felhasználónév (UPN) az SAML-válasz, IDPEmail nevű elemként szerepel a felhasználó UserPrincipalName (UPN) az Azure AD vagy Office 365-ben. Az egyszerű felhasználónév az e-mail cím formátumú. Egyszerű felhasználónév értéke Windows Office 365 (az Azure Active Directory).|
|Kiállító|Az identitásszolgáltató URI lehet szükséges. Ne használja ugyanazt a minta üzenetekből a kibocsátó. Ha több felső szintű tartomány szerepel az Azure AD-bérlő a kibocsátó meg kell egyeznie a megadott URI-beállítás konfigurálva tartományonként.|
...
Egy minta kérelemüzenet, az Azure AD egy minta SAML 2.0 identitásszolgáltató küldött a következő: A minta SAML 2.0 identitásszolgáltató az Active Directory összevonási szolgáltatások (AD FS) SAML-P protokoll használatára konfigurálva. Együttműködés ellenőrzésére is befejeződött egyéb SAML 2.0 identitás-szolgáltatóktól.
...
Az alábbiakban egy példa SAML 2.0-s megfelelő identitásszolgáltató az Azure AD által küldött minta válaszüzenetet vagy Office 365.
...
Ajánlott mindig importálni a legújabb Azure AD-metaadatok a SAML 2.0 identitásszolgáltató konfigurálásakor.

>[!NOTE]
>Az Azure AD kiolvasni az identitásszolgáltató metaadatok.

## <a name="add-azure-ad-as-a-relying-party"></a>Az Azure AD hozzáadni egy függő entitás
Engedélyeznie kell a SAML 2.0 identitásszolgáltató és az Azure AD közötti kommunikáció. Ez a konfiguráció a megadott identitásszolgáltató függ, és azt dokumentációjában tájékozódhat. Akkor általában értékre kell állítania a függő entitás azonosítója megegyezik a entityid beállítást az Azure AD metaadatok alapján.
...
Az alábbi eljárás bemutatja, hogyan használja a SAML 2.0 SP-Lite összevont tartományhoz egy meglévő szabványos tartomány átalakításának lépésein. 

>[!NOTE]
>A tartományhoz, amely hatással van a felhasználók fel 2 óra után ezzel a lépéssel kimaradás tapasztalhat.
...
Összevonási konfigurálása után átválthatja vissza a "nem összevont" (vagy "kezelt"), azonban ez a módosítás befejezéséhez akár két órát vesz igénybe, és új véletlenszerű jelszót a felhő alapú bejelentkezéshez hozzárendelése minden felhasználóhoz van szükség. Váltás vissza a "kezelt" lehet kell bizonyos esetekben hibát jelez a beállítások visszaállítására. További információ a tartomány átalakítás:: [ http://msdn.microsoft.com/library/windowsazure/dn194122.aspx ](http://msdn.microsoft.com/library/windowsazure/dn194122.aspx).
...
Mielőtt hitelesítheti a felhasználókat, hogy az Office 365, a felhasználó rendszerbiztonsági tagok, amelyek megfelelnek a helyességi feltétel a SAML 2.0 jogcímek el kell juttatnia az Azure AD. Ha a felhasználó rendszerbiztonsági tagok nem ismert az Azure AD előzetesen, majd azok nem használható az összevont bejelentkezés. Az Azure AD Connect vagy a Windows PowerShell felhasználói rendszerbiztonsági tagok kiépítéséhez használható.
...
Győződjön meg arról, hogy az egyszeri bejelentkezés beállítása helyes, hajtsa végre az alábbi eljárást, győződjön meg arról, hogy tud bejelentkezni a vállalati hitelesítő adataival a felhőalapú szolgáltatáshoz.
...
5.  A kapcsolat Analyzer csak akkor nyílik meg, hogy jelentkezzen be, adja meg a hitelesítő adatait egyszerű teszteli a SAML 2.0 IDP: ![SAML](media/active-directory-aadconnect-federation-saml-idp/saml1.png)
6.  A összevonási teszt bejelentkezési időszakban meg kell adnia fióknevet és jelszót, amely konfigurálva van a SAML 2.0 identitásszolgáltatóval átalakításakor az Azure AD-bérlő. Az eszköz megkísérli jelentkezzen be ezen hitelesítő adatok használatával, és a bejelentkezési kísérlet során végrehajtott ellenőrzések részletes eredményét nyújtanak output típusúként.
...
2.  Kattintson a jelszó mezőbe. Ha egyszeri bejelentkezésre van beállítva, a jelszó mező árnyékolt lesz, és a következő üzenet jelenik meg: "szükségesek most jelentkezik be &lt;a vállalat&gt;."
3.  Kattintson a bejelentkezés, &lt;a vállalat&gt; hivatkozásra. Ha tudja bejelentkezési, majd egyszeri bejelentkezés beállítása.
