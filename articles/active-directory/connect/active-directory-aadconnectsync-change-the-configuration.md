...
![Szinkronizálási Szabályszerkesztő a Start menüben](./media/active-directory-aadconnectsync-change-the-configuration/startmenu2.png)

Amikor megnyitja a szerkesztő, tekintse meg az alapértelmezett beépített szabályokat.

![Szinkronizálási Szabályszerkesztő](./media/active-directory-aadconnectsync-change-the-configuration/sre2.png)
...
A [scheduler](active-directory-aadconnectsync-feature-scheduler.md) alapértelmezés szerint 30 percenként fut. Ellenőrizze, hogy nem indul, módosítása és hibáinak elhárítása az új szabályokat. Az ütemező ideiglenes letiltásához indítsa el a Powershellt, és futtassa `Set-ADSyncScheduler -SyncCycleEnabled $false`.
...
3. Az a **Scoping szűrő** lap, adja meg **givenName ISNOTNULL**.  
   ![Bejövő szabály a hatókör-beállítási szűrője](./media/active-directory-aadconnectsync-change-the-configuration/scopingfilter.png)  
...
A forgatókönyv esetében ez a szakasz a háttér-információkért lásd: [szabályozhatja az attribútum folyamatot](active-directory-aadconnectsync-understanding-declarative-provisioning.md#control-the-attribute-flow-process).

Az attribútum nem flow két módon lehet. Az első az a telepítési varázsló használatával [távolítsa el a kiválasztott attribútumok](active-directory-aadconnect-get-started-custom.md#azure-ad-app-and-attribute-filtering). Ez a beállítás akkor működik, ha még nem szinkronizálta az attribútum előtt. Azonban ha ezt az attribútumot szinkronizálja, és később ezzel a funkcióval eltávolításához elindította, a szinkronizálási motor leáll, az attribútumot és a már létező értékei kezelése van hátra az Azure ad-ben.
...
![A bővítményattribútumok hibás](./media/active-directory-aadconnectsync-change-the-configuration/badextensionattribute.png)

1. Hozzon létre egy új bejövő szinkronizálási szabály, és töltse ki a leírást.
  ![Leírások](./media/active-directory-aadconnectsync-change-the-configuration/syncruledescription.png)
2. Hozzon létre az attribútumfolyamok **kifejezés** a **FlowType** és **AuthoritativeNull** a **forrás**. A konstans **AuthoritativeNull** azt jelzi, hogy az érték lehet üres a metaverzumban, még akkor is, ha alacsonyabb-elsőbbséget szinkronizálási szabály megpróbálja töltse fel az értéket.
  ![A bővítményattribútumok átalakításában](./media/active-directory-aadconnectsync-change-the-configuration/syncruletransformations.png)
...
4. Ellenőrizze, hogy a kívánt módosításokat az Összekötőtérben kereséssel exportálásra váró adatokat.
  ![A kétlépcsős delete](./media/active-directory-aadconnectsync-change-the-configuration/deletetobeexported.png)
...
![PowerShell módosítás után](./media/active-directory-aadconnectsync-change-the-configuration/powershell2.png)  
...
![Bejövő szinkronizálási szabály létrehozása](./media/active-directory-aadconnectsync-change-the-configuration/usertype3.png)
