...
```
- 如果需要互動式驗證，ADAL 會使用 Windows Phone 的 Web 驗證代理人 (WAB) 和 [接續模型](http://www.cloudidentity.com/blog/2014/06/16/adal-for-windows-phone-8-1-deep-dive/) 來顯示 Azure AD 登入頁面。 當使用者登入時，您的應用程式必須將與 WAB 互動的結果傳遞給 ADAL。 這很簡單，只要實作 `ContinueWebAuthentication` 介面:
...
- 現在就可以使用 `AuthenticationResult` ADAL 傳回應用程式。 在 `QueryGraph(...)` 回呼，將您取得的 access_token 附加至 GET 要求的授權標頭中:
...
```
- 您也可以使用 `AuthenticationResult` 物件，在您的應用程式中顯示使用者的相關資訊。 在 `QueryGraph(...)` 方法中，使用此結果在頁面上顯示使用者的識別碼:

```C#
// Update the Page UI to represent the signed in user
ActiveUser.Text = result.UserInfo.DisplayableId;
```
- 最後，您也可以使用 ADAL 來將使用者登出應用程式。 當使用者按一下 [登出] 按鈕時，我們想要確保下次呼叫 `AcquireTokenSilentAsync(...)` 將會失敗。 有了 ADAL，這會和清除權杖快取一樣簡單：
