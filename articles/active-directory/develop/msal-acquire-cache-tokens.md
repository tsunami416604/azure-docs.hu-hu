...
A Microsoft Graph API esetében csak a hatókör értékének `user.read` a formátumra `https://graph.microsoft.com/User.Read` való leképezése és a szinonimaként használható.

> [!NOTE]
> Bizonyos webes API-k (például ahttps://management.core.windows.net/) Azure Resource Manager API) ("/") a hozzáférési jogkivonat célközönségi jogcímében (AUD). Ebben az esetben fontos átadni a hatókört https://management.core.windows.net//user_impersonation (jegyezze fel a dupla perjelet), hogy a token érvényes legyen az API-ban.
