...
A portál vagy programozott módon az Azure-on keresztül az alkalmazás attribútumokat is beállíthat [REST API-val](https://docs.microsoft.com/previous-versions/azure/ad/graph/api/entity-and-complex-type-reference#application-entity) vagy [PowerShell](https://docs.microsoft.com/powershell/module/azuread/?view=azureadps-2.0#applications). Vannak azonban bizonyos forgatókönyvek, ahol kell az alkalmazásjegyzék konfigurálása az alkalmazás-attribútum szerkesztése. Ezek a forgatókönyvek a következőket biztosítják:
...
1. Válassza ki a **Azure Active Directory** szolgáltatásra, és válassza ki **alkalmazásregisztrációk**.
...
| `publicClient` | Boolean | Megadja, hogy az alkalmazás nyilvános ügyfél (például egy mobileszközön futó telepített alkalmazás). <br><br> _Megjegyzés: Ez a lehetőség csak az alkalmazás regisztrációk (örökölt) felhasználói felülete. Lecserélve `allowPublicClient` a a [alkalmazásregisztrációk](https://go.microsoft.com/fwlink/?linkid=2083908) tapasztalható._ | |
| `publisherDomain` | String | Az ellenőrzött közzétevői tartományt az alkalmazás. Csak olvasható. | https://www.contoso.com |
| `replyUrls` | Karakterlánc-tömbben | Ez több érték a tulajdonság, amely elfogadja az Azure ad-ben regisztrált redirect_uri értékek listáját tartalmazza a célként jogkivonatok visszaadásakor. <br><br> _Megjegyzés: Ez a lehetőség csak az alkalmazás regisztrációk (örökölt) felhasználói felülete. Lecserélve `replyUrlsWithType` a a [alkalmazásregisztrációk](https://go.microsoft.com/fwlink/?linkid=2083908) tapasztalható._ | |
...
1. Ha a munkafolyamatot, hogy a jegyzékek mentse a forrás-tárházában későbbi használatra van szüksége, javasoljuk, hogy a tárházban a egy jelenik meg a mentett jegyzékek rebasing a **alkalmazásregisztrációk** tapasztalható.
