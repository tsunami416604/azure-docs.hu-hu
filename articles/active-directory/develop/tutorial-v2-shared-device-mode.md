...
A MSAL konfigurációs fájljában állítsa be `"shared_device_mode_supported"` `true`.

Előfordulhat, hogy nem tervezi több fiókos üzemmód használatát. Ez akkor lehet lehetséges, ha nem megosztott eszközt használ, és a felhasználó egyszerre több fiókkal is bejelentkezhet az alkalmazásba. Ha igen, állítsa `"account_mode"` `"SINGLE"`re. Ez garantálja, hogy az alkalmazás mindig `ISingleAccountPublicClientApplication`kap, és jelentősen leegyszerűsíti a MSAL-integrációt. Az `"account_mode"` alapértelmezett értéke `"MULTIPLE"`, ezért fontos, hogy a konfigurációs fájlban módosítsa ezt az értéket, ha `"single account"` módot használ.

Íme egy példa a auth_config. JSON fájlra, **amely a minta alkalmazás**>**fő**>**res**>**nyers** könyvtárában található:
...
A `isSharedDevice()` használatával megállapíthatja, hogy egy alkalmazás egy megosztott eszköz módban lévő eszközön fut-e. Az alkalmazás ennek a jelzőnek a használatával állapíthatja meg, hogy ennek megfelelően kell-e módosítania az UX-t.

Az alábbi kódrészletből megtudhatja, hogyan használhatja a `isSharedDevice()`.  A minta alkalmazás `SingleAccountModeFragment` osztálya:
...
Ha a MSAL konfigurációs fájlban a `"account_mode":"SINGLE"` értéket állítja be, akkor a visszaadott alkalmazás-objektumot `ISingleAccountPublicCLientApplication`ként biztonságosan elvégezheti.
...
A `loadAccount` metódus lekéri a bejelentkezett felhasználó fiókját. A `onAccountChanged` módszer határozza meg, hogy a bejelentkezett felhasználó módosult-e, és ha igen, törölje a következőt:
...
![az alkalmazás regisztrációja képernyő](media/tutorial-v2-shared-device-mode/register-app.png) válassza a **módosítás elvégzése nekem** lehetőséget, majd adja meg, hogy a rövid útmutató milyen értékeket kér a Azure Portalban. Ha elkészült, a rendszer létrehozza az összes szükséges konfigurációs fájlt.
...
Tesztelési célból állítsa be a következőt a bérlőben: legalább két alkalmazott, egy felhőalapú eszköz rendszergazdája és egy globális rendszergazda. A Azure Portalban állítsa be a felhőalapú eszköz rendszergazdáját a szervezeti szerepkörök módosításával. A Azure Portal a szervezeti szerepköröket **Azure Active Directory** > **szerepkörök és rendszergazdák** lehetőség kiválasztásával > **Cloud Device Administrator**. Adja hozzá azokat a felhasználókat, akik számára az eszköz megosztott módba helyezhető.
...
A minta alkalmazás egy egyszerű alkalmazás, amely meghívja a szervezet Graph APIét. n első futtatáskor a rendszer megkéri, hogy az alkalmazás új legyen az alkalmazott fiókjában.
...
További információ a megosztott üzemmódról az [Android-eszközök megosztott eszközének módjáról](shared-device-mode.md)