...
- Ha alkalmazni szeretné a szabályzatot, meg kell hívnia egy `authority` paramétert tartalmazó `AcquireTokenInteractive` felülbírálását.
...
a `IPublicClientApplication` metódust tartalmaz:
```csharp
...
- `tid`, amely az Azure AD-bérlő azonosítója, és 
- `preferred_username` 
