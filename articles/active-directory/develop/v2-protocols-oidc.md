...
| --- | --- | --- |
| `tenant` | Kötelező | A kérelem elérési `{tenant}` útjának értékét követve szabályozhatja, hogy ki jelentkezhet be az alkalmazásba. Az engedélyezett értékek `common` `organizations` :,,ésbérlőiazonosítók`consumers`. További információ: [protokoll alapjai](active-directory-v2-protocols.md#endpoints). |
...
| `response_type` | Kötelező | Tartalmaznia `id_token` kell az OpenID Connect bejelentkezést. Más `response_type` értékeket is tartalmazhatnak, `code`például:. |
...
| `nonce` | Kötelező | Az alkalmazás által generált kérelemben szereplő érték, amely az eredményül kapott id_token-értékként fog szerepelni a jogcímként. Az alkalmazás ellenőrizheti ezt az értéket a jogkivonat-Visszajátszási támadások enyhítése érdekében. Az érték általában egy véletlenszerű, egyedi karakterlánc, amely a kérelem forrásának azonosítására szolgál. |
