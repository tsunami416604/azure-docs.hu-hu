...
A jogcímek csak akkor jelennek meg, ha egy érték van kitöltve. Az alkalmazás nem veheti igénybe a függőségeket. Ilyenek például a következők: `pwd_exp` (nem minden bérlő számára szükséges a jelszó lejárata), és `family_name` ([ügyfél hitelesítő adatai] (v2-oauth2-Client-creds-Grant-flow.MD) folyamatai a névvel nem rendelkező alkalmazások nevében találhatók). A hozzáférési jogkivonat érvényesítéséhez használt jogcímek mindig jelen lesznek.
