...
完成本教學課程之後, 您已指派給 xMatters OnDemand 的 Azure AD 使用者將能夠登入位於您 xMatters OnDemand 公司網站 （服務提供者起始登入），應用程式的單一登入或使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
1.  在 Azure AD 入口網站上 **XMatters OnDemand** 應用程式整合頁面上，按一下 [ **設定單一登入** 開啟 * * 設定單一登入 * * 對話方塊。
...
3.  在 **設定應用程式 URL** 頁面上，於 **XMatters OnDemand 登入 URL** 文字方塊中，輸入您的 URL 使用下列模式 」*https://\<tenant-name\>。XMattersOnDemandapp.com*」，然後按一下 [ **下一步**。

    ![設定應用程式 URL](./media/active-directory-saas-xmatters-ondemand-tutorial/IC776793.png "Configure app URL")

4.  在 **設定單一登入在 XMatters OnDemand** ] 頁面上，下載您的憑證，按一下 [ **下載憑證**, ，然後將儲存在本機憑證檔當做 **c:\\XMatters OnDemand.cer**。

    >[AZURE.IMPORTANT] 您需要將憑證轉寄給 xMatters 支援小組。 xMatters 支援小組需要先上傳憑證，您才能完成單一登入組態。
...
6.  在頂端工具列中，按一下 [ **管理員**, ，然後按一下 [ **公司詳細資料** 左側的導覽列中。
...
7.  在 **SAML 組態** 頁面上，執行下列步驟 ︰
    ...
    1.  選取 **啟用 SAML**。
    2.  在 Azure 網站中上, **設定單一登入在 XMatters OnDemand** 對話方塊頁面中，複製 **身分識別提供者 ID** 值，並接著將它貼入 **身分識別提供者 ID** 文字方塊。
    3.  在 Azure 網站中，在 **設定單一登入在 XMatters OnDemand** 對話方塊頁面中，複製 **單一登入服務 URL** 值，並接著將它貼入 **單一登入 URL** 文字方塊。
    4.  在 Azure 網站中，在 **設定單一登入在 XMatters OnDemand** 對話方塊頁面中，複製 **單一登出服務 URL** 值，並接著將它貼到 **單一登出 URL** 文字方塊。
    5.  在公司的詳細資料] 頁面的頂端，按一下 **儲存變更**。
...
5.  在 **將使用者新增** 區段中，執行下列步驟 ︰
    ...
    1.  輸入 **UserID**, ，**名字**, ，**姓氏**, ，**網站** 您想要佈建之有效 AAD 帳戶。
    2.  按一下 [ **儲存**。

>[AZURE.NOTE] 您可以使用任何其他 XMatters OnDemand 使用者帳戶建立工具或 Api 來佈建 AAD 使用者帳戶 XMatters OnDemand 所提供。
