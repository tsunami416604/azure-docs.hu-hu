...
Azure AD Connect \(augusztus 2016-es vagy\)újabb verziójának legújabb verziójával a szinkronizálási hibákról szóló jelentés a [Azure Portal](https://aka.ms/aadconnecthealth) Azure ad Connect Health szinkronizálási szolgáltatás részeként érhető el.
...
Az Azure AD-ba való exportálás során felmerülő hibák azt jelzik, hogy a művelet \(Hozzáadás, frissítés, törlés stb.\) Azure AD Connect \(szinkronizálási motor\) sikertelen volt.
...
* Ha Azure AD Connect \(Sync Engine\) arra utasítja a Azure Active Directoryt, hogy objektumokat adjon hozzá vagy frissít, az Azure ad a **sourceAnchor** attribútum használatával a bejövő objektumra illeszkedik az Azure ad-ban lévő objektumok **immutableId** attribútumához. Ezt a megfeleltetést **nehéz egyezésnek**nevezzük.
...
A InvalidSoftMatch hibájának leggyakoribb oka két olyan objektum, amelyeknek különböző SourceAnchor \(immutableId\) azonos értékkel rendelkeznek a ProxyAddresses és/vagy a UserPrincipalName attribútumok esetében, amelyeket az Azure AD-beli Soft-Match folyamat során használ a rendszer. Az érvénytelen lágy egyezés kijavításához

1. Azonosítsa a hibát okozó duplikált proxyAddresses, userPrincipalName vagy más attribútumérték értékét. Azt is azonosítsa, hogy mely két \(vagy több\) objektum vesz részt az ütközésben. A Azure AD Connect Health által a [szinkronizáláshoz](https://aka.ms/aadchsyncerrors) létrehozott jelentés segít azonosítani a két objektumot.
...
1. A rendszergazda létrehoz egy új, e-mailek használatára képes biztonsági csoportot az Office 365-ben az adórendszer számára, és e-mail-címet biztosít tax@contoso.comként. Ez a csoport a ProxyAddresses attribútum értékét rendeli hozzá az **SMTP: tax\@contoso.com**
...
1. Azonosítsa a hibát okozó duplikált proxyAddresses (vagy más attribútum) értékét. Azt is azonosítsa, hogy mely két \(vagy több\) objektum vesz részt az ütközésben. A Azure AD Connect Health által a [szinkronizáláshoz](https://aka.ms/aadchsyncerrors) létrehozott jelentés segít azonosítani a két objektumot.
2. Azonosítsa, hogy az objektumnak továbbra is legyen a duplikált értéke, és hogy melyik objektum ne legyen.
...
A AttributeValueMustBeUnique hibájának leggyakoribb oka két olyan objektum, amely különböző SourceAnchor \(immutableId,\) a ProxyAddresses és/vagy a UserPrincipalName attribútumok esetében azonos értékkel rendelkezik. AttributeValueMustBeUnique-hiba javítása érdekében
...
#### <a name="related-articles"></a>Kapcsolódó cikkek
-[duplikált vagy érvénytelen attribútum megakadályozza a címtár-szinkronizálást az Office 365-ben](https://support.microsoft.com/kb/2647098)
...
2. A szinkronizálás megkezdéséhez engedélyezze a következő szinkronizálási ciklust. Az idő szinkronizálása sikeres lesz, és a rendszer frissíti a UserPrincipalName a várt módon bob@fabrikam.com.
