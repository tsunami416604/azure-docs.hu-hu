---
title: Az Azure Data Lake Store elérése Windows VM-beli felügyeltszolgáltatás-identitással (MSI)
description: Ez az oktatóanyag bemutatja, hogyan használhat Windows VM-beli felügyeltszolgáltatás-identitást (MSI) az Azure Data Lake Store eléréséhez.
...
Az oktatóanyag bemutatja, hogyan használhat felügyeltszolgáltatás-identitást (MSI) egy Windows rendszerű virtuális gépen (VM) egy Azure Data Lake Store eléréséhez. A felügyeltszolgáltatás-identitások kezelését automatikusan az Azure végzi, és lehetővé teszi a hitelesítést az Azure AD-hitelesítést támogató szolgáltatásokban anélkül, hogy be kellene szúrnia a hitelesítő adatokat a kódba. Az alábbiak végrehajtásának módját ismerheti meg:
...
A Data Lake Store-ban hozzon létre egy új mappát, és adjon engedélyt a VM MSI-nek a mappában lévő fájlok olvasásához, írásához és futtatásához:
...
A VM MSI mostantól az összes műveletet el tudja végezni a létrehozott mappában lévő fájlokon.  További információkat a Data Lake Store hozzáférésének kezeléséről [a hozzáférés-vezérlés Data Lake Store-ban való használatával kapcsolatos](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-access-control) cikkben talál.

## <a name="get-an-access-token-using-the-vm-msi-and-use-it-to-call-the-azure-data-lake-store-filesystem"></a>Hozzáférési jogkivonat lekérése a VM MSI-vel, majd az Azure Data Lake Store-fájlrendszer hívása a jogkivonat segítségével
...
Ebben az oktatóanyagban a Data Lake Store-fájlrendszer REST API-jában fog hitelesíteni a PowerShell használatával REST-kérések elindításához. Ha a VM MSI-t hitelesítéshez szeretné használni, a virtuális gépről kell indítania a kéréseket.
...
4. A Powershell `Invoke-WebRequest` parancsával küldjön kérést a helyi MSI-végpontra, hogy lekérjen egy hozzáférési jogkivonatot az Azure Data Lake Store-hoz.  A Data Lake Store erőforrás-azonosítója „https://datalake.azure.net/”.  A Data Lake az erőforrás-azonosítót pontosan egyezteti, így a záró perjel nem hagyható el.
...
Gratulálunk!  Hitelesítést végzett a Data Lake Store-fájlrendszeren egy VM MSI-vel.
