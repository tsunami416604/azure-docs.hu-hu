...
完成本教學課程之後, 您指派給 Zendesk 的 Azure AD 使用者都能夠單一的登入該應用程式在您的 Zendesk 公司網站 (服務提供者初始化登入)，或使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
1.  在 Azure 的 AD 入口網站，在 **Zendesk** 應用程式整合] 頁面上，按一下 [ **設定單一登入** 開啟 * * 設定單一登入 * * 對話方塊。

    ![單一登入](./media/active-directory-saas-zendesk-tutorial/IC773086.png "Single sign-on")

2.  在 **您要如何才能登入 Zendesk 的使用者** ] 頁面上，選取 **Microsoft Azure AD 單一登入**, ，然後按一下 [ **下一步**。

    ![設定單一登入](./media/active-directory-saas-zendesk-tutorial/IC773087.png "Configure single sign-on")

3.  上 **設定應用程式 URL** 頁，在 **中 URL 的 Zendesk 號** ] 文字方塊中，輸入您使用下列模式的 URL"*https://\<tenant-name\>.zendesk.com*"，，然後按一下 [ **下一步**。

    ![設定應用程式 URL](./media/active-directory-saas-zendesk-tutorial/IC773088.png "Configure app URL")

4.  在 **設定單一登入，在 Zendesk** 頁面，以下載您的憑證，按一下 [ **下載憑證**, ，然後將儲存在本機憑證檔當做 **c:\\zendesk.cer**。
...
7.  在左邊的瀏覽] 窗格中，按一下 [ **設定**, ，然後按一下 [ **安全性**。

    ![安全性](./media/active-directory-saas-zendesk-tutorial/IC773089.png "Security")

8.  在 **安全性** 頁面上，選取 **管理與代理程式** ] 索引標籤。

9.  選取 [ **單一登入 (SSO) 和 SAML**, ，然後選取 **SAML**。

10. 在 Azure 的 AD 入口網站，在 **設定單一登入，在 Zendesk** 頁面上，複製 **SAML SSO URL** 值，而且再將其貼入 **SAML SSO URL** 文字方塊。

11. 在 Azure 的 AD 入口網站，在 **設定單一登入，在 Zendesk** 頁面上，複製 **遠端登出 URL** 值，而且再將其貼入 **遠端登出 URL** 文字方塊。

    ![單一登入](./media/active-directory-saas-zendesk-tutorial/IC773090.png "Single sign-on")

12. 複製 **指紋** 值從匯出的憑證，然後將它貼到 **憑證指紋** 文字方塊。
...
若要啟用 Azure AD 使用者登入 **Zendesk**, ，必須提供到 **Zendesk**。  
中的例子中 **Zendesk**, ，佈建是手動的工作。
...
4.  輸入您想要佈建，，，然後按一下的現存 Azure AD 帳戶的電子郵件地址 **儲存**。

    ![新增使用者](./media/active-directory-saas-zendesk-tutorial/IC773633.png "New user")

>[AZURE.NOTE] 您可以使用任何其他 Zendesk 使用者帳戶建立工具或 Api 提供由 Zendesk 提供 AAD 的使用者帳戶。
...
2.  在 * * Zendesk * * 應用程式整合] 頁面上，按一下 [ **將使用者指派**。
