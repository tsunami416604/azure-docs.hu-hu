...
在您的同盟伺服器上，開啟 Windows PowerShell 命令視窗並輸入：
  ...
  `設定 e TargetName < RPObjectName > AllowedAuthenticationClassReferences wiaormultiauthn`

其中 <RPObjectName> 是您 Azure Active Directory 信賴憑證者信任物件的信賴憑證者合作對象物件名稱。此物件通常命名為「Microsoft Office 365 身分識別平台」。
...
* [設定自動裝置註冊適用於 Windows 8.1 網域加入的裝置](active-directory-conditional-access-automatic-device-registration-windows8_1.md)

* [設定自動裝置註冊適用於 Windows 7 加入網域的裝置](active-directory-conditional-access-automatic-device-registration-windows7.md)
