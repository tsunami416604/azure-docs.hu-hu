<properties
    ...
    描述 = 「 可協助定義混合式身分識別管理工作，根據每個生命週期階段可用的選項 」。
    documentationCenter=""
    服務 = [active directory]
    作者 ="femila"
    管理員 ="stevenpo"
...
|-------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| 帳戶管理和佈建| 您可以利用 Active Directory ® 網域服務 (AD DS) 伺服器角色，建立可擴充、安全且容易管理的基礎結構來管理使用者與資源，並支援 Microsoft® Exchange Server 等具有目錄功能的應用程式。<br><br> [可以佈建身分識別管理員透過 AD DS 中](https://technet.microsoft.com/library/ff686261.aspx) <br>[ 可以佈建在 AD DS 中的使用者](https://technet.microsoft.com/library/ff686263.aspx) <br><br> 系統管理員可以使用存取控制來管理共用資源的 「 使用者 」 存取權，基於安全性考量。在 Active Directory 中，存取控制的管理方式是在物件層級設定物件的不同存取層級 (或權限)，例如完全控制、寫入、讀取或沒有存取權。Active Directory 中的存取控制定義不同的使用者如何使用 Active Directory 物件。根據預設，Active Directory 中的物件權限會設定為最安全的設定。| 您必須為每一個會存取 Microsoft 雲端服務的使用者建立帳戶。您也可以變更使用者帳戶，或在已不需要時將它們刪除。根據預設，使用者沒有系統管理員權限，但是您可以選擇性地指派給他們。如需詳細資訊，請參閱 [Azure AD 中管理使用者](active-directory-create-users.md)。<br><br> Azure Active directory，其中一項主要功能是管理資源的存取權的能力。這些資源可以是目錄的一部分，例如透過目錄中的角色或目錄外部的資源 (例如 SaaS 應用程式、Azure 服務以及 SharePoint 網站或內部部署資源) 管理物件的權限。<br><br> 在中央的 Azure Active Directory 的存取管理解決方案是安全性群組。資源擁有者 (或目錄的系統管理員) 可以指派群組，對所擁有的資源提供特定的存取權限。群組的成員將會提供存取權，以及資源擁有者可以委派權限來管理其他人 – 例如部門經理或技術服務管理員的群組成員清單<br> <br> Azure AD 主題中的管理群組提供有關管理群組的存取權的詳細資訊。| 透過同步處理與同盟將 Active Directory 身分識別延伸至雲端|
