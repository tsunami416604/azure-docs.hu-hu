...
- 遵照提示進行，並建立新的 **Web 應用程式和/或 WebAPI**。
    ...
    -   [**登入 URL**] 是指應用程式的基底 URL。 基本架構的預設值是 ' http://localhost:3000//驗證/openid/傳回 '。
...
- 此外，您將需要我們 `passport azure ad` 以及:
...
- 最後，讓我們加入的路由，實際的登入要求會遞交 `passport azure ad` 引擎:
...
`git 複製-分支完成 https://github.com/AzureADQuickStarts/WebApp-OpenIDConnect-NodeJS.git`
