...
完成本教學課程之後, 您已指派給 Samanage 的 Azure AD 使用者將能夠登入位於您 Samanage 公司網站 （服務提供者起始登入），應用程式的單一登入或使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
6.  在 **搜尋方塊**, ，型別 **Samanage**。

    ![應用程式庫](./media/active-directory-saas-samanage-tutorial/IC771707.png "Application gallery")

7.  在 [結果] 窗格中，選取 **Samanage**, ，然後按一下 [ **完成** 加入應用程式。
...
8.  在 **使用 SAML 登入** 對話方塊頁面上，執行下列步驟，以及 [ **儲存變更**:

    1.  按一下 [ **啟用單一登入的 SAML**。
        ![使用 SAML 登入](./media/active-directory-saas-samanage-tutorial/IC771719.png "Login using SAML")
    2.  在 Azure 網站中，在 **Samanage 在設定單一登入** 對話方塊頁面中，複製 **身分識別提供者 ID** 值，並接著將它貼到 * * 身分識別提供者 URL * * 文字方塊。
        ![設定單一登入](./media/active-directory-saas-samanage-tutorial/IC771720.png "Configure Single Sign-On")
    3.  在 Azure 網站中，在 **Samanage 在設定單一登入** 對話方塊頁面中，複製 **遠端登入 URL** 值，並接著將它貼入 **登入 URL** 文字方塊。
    4.  在 Azure 網站中，在 **Samanage 在設定單一登入** 對話方塊頁面中，複製 **遠端登出 URL** 值，並接著將它貼入 **登出 URL** 文字方塊。
...
4.  按一下 [ **新使用者**。
    ...
    ![新使用者](./media/active-directory-saas-samanage-tutorial/IC771726.png "New User")

5.  型別 **電子郵件地址** 和 **名稱** 您想要佈建，並按一下 [Azure AD 帳戶 **建立使用者**。
...
>[AZURE.NOTE]您可以使用任何其他 Samanage 使用者帳戶建立工具或 Api 來佈建 AAD 使用者帳戶提供 Samanage。
