...
4.  在 **BlueJeans 在設定單一登入** ] 頁面上，下載您的憑證，按一下 [ **下載憑證**, ，然後儲存您的電腦上的憑證檔案。

    ![設定單一登入](./media/active-directory-saas-bluejeans-tutorial/IC785866.png "Configure Single Sign-On")

5.  在不同的網頁瀏覽器視窗中，登入您 **BlueJeans** 系統管理員身分的公司網站。
...
8.  繼續執行下列步驟：
    ...
    1.  按一下 [ **選擇檔案**, ，然後再上傳已下載的憑證。
    2.  在 Azure 網站中，在 **BlueJeans 在設定單一登入** 對話方塊頁面中，複製 **遠端登入 URL** 值，並接著將它貼入 **登入 URL** 文字方塊。
    3.  在 Azure 網站中，在 **BlueJeans 在設定單一登入** 對話方塊頁面中，複製 **變更密碼 URL** 值，並接著將它貼入 **密碼變更 URL** 文字方塊。
    4.  在 Azure 網站中，在 **BlueJeans 在設定單一登入** 對話方塊頁面中，複製 **遠端登出 URL** 值，並接著將它貼入 **登出 URL** 文字方塊。
...
1.  登入您 **BlueJeans** 系統管理員身分的公司網站。

2.  移至 **ADMIN \ > 管理使用者 \ > 將使用者加入**。

    ![Admin](./media/active-directory-saas-bluejeans-tutorial/IC785877.png "Admin")

    >[AZURE.IMPORTANT]  **新增使用者** ] 索引標籤，才可以使用 if、 在 **安全性] 索引標籤**, ，**啟用自動佈建** 未選取。

3.  在 **新增使用者** 區段中，執行下列步驟 ︰

    ![新增使用者](./media/active-directory-saas-bluejeans-tutorial/IC785886.png "Add User")

    1.  型別 a **BlueJeans 使用者名稱**, 、 **電子郵件地址**, 、 **BlueJeans 會議 ID**, 、 **仲裁者密碼**, 、 **全名**, 、 **公司** 您想要佈建到相關文字方塊之有效 AAD 帳戶。
    2.  按一下 [ **將使用者加入**。

>[AZURE.NOTE] 您可以使用任何其他 BlueJeans 使用者帳戶建立工具或 Api 來佈建 AAD 使用者帳戶 BlueJeans 所提供。
