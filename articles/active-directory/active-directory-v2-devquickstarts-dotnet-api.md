<properties
    pageTitle ="應用程式模型 v2.0 > Net Web api |Microsoft Azure 「
    描述 ="如何建置.NET MVC Web Api 可接受的權杖從兩個人的 Microsoft 帳戶和工作或學校帳戶 」。
    服務 = [active directory]
...
`git 複製-分支基本架構 https://github.com/AzureADQuickStarts/AppModelv2-WebAPI-DotNet.git`
...
- 為您的應用程式新增**行動**平台。
- 從入口網站複製完整的**重新導向 URI**。 您必須使用的預設值 `urn: ietf:wg:oauth:2.0:oob`。
...
-   將 OWIN 啟動類別加入至呼叫的 TodoListService 專案 `Startup.cs`。 以滑鼠右鍵按一下專案 --> [**新增**] -->[ **新增項目**] --> 搜尋 "OWIN"。 OWIN 中介軟體將會叫用 `Configuration(...)` 應用程式啟動時的方法。
-   將類別宣告變更為 `公用部分類別啟動` -我們已實作了此類別的部分為您在另一個檔案。 在 `Configuration(...)` 方法，請呼叫 confgureauth (...) 若要設定 web 應用程式的驗證。
...
-   開啟檔案 `App_Start\Startup.Auth.cs` 和實作 `ConfigureAuth(...)` 方法，它會設定為接受來自 v2.0 端點權杖的 Web API。
...
-   現在您可以使用 `Authorize` 屬性來保護您的控制器和動作的 OAuth 2.0 承載驗證。 裝飾 `Controllers\TodoListController.cs` 使用授權標記的類別。 這樣會強制使用者在存取該頁面之前必須先登入。
...
- 當授權的呼叫者成功叫用的其中一個 `TodoListController` Api，此動作可能需要存取呼叫者的相關資訊。 OWIN 可讓您透過持有人權杖內部的宣告存取 `Claimsprincipal` 物件。
...
-   最後，請開啟 `web.config` TodoListService 專案根目錄中的檔案，並輸入您的設定值，在 `< g s >` 一節。
  - 您 `ida: 對象` 是 **應用程式識別碼** 您輸入在入口網站的應用程式。

## 3.設定用戶端應用程式及執行服務

在看到 Todo 清單服務運作之前，您需要設定 Todo 清單用戶端，以便它可以取得 v2.0 端點的權杖，並對服務進行呼叫。

- 開啟 TodoListClient 專案中 `App.config` 並輸入您的設定值，在 `< g s >` 一節。
  - 您 `ida: ClientId` 您從入口網站複製的應用程式識別碼。
...
`git 複製-分支完成 https://github.com/AzureADQuickStarts/AppModelv2-WebAPI-DotNet.git`

## 後續步驟

您現在可以繼續探索其他主題。 您可以嘗試：

[從 2.0 版應用程式模型與 Web 應用程式呼叫 Web API >>](active-directory-devquickstarts-webapp-webapi-dotnet.md)
