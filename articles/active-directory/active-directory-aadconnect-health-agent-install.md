<properties
    ...
    description="這是說明如何針對 AD FS 和同步處理代理程式安裝 Azure AD Connect Health 頁面。"
...
本文將逐步引導您完成安裝和設定 Azure AD Connect Health 代理程式的 AD FS 與同步處理。
...
4. 使用提高的權限開啟命令提示字元並執行下列命令以啟用稽核: <code>auditpol.exe /set /subcategory:"Application Generated"/failure /success:enable。</code>
...
## 安裝 Azure AD Connect Health 代理程式的同步處理
...
- 監視服務的健全狀況 AadSync 的 azure AD Connect

![驗證 Azure AD Connect 同步處理健全狀況](./media/active-directory-aadconnect-health-sync/services.png)
...
>[AZURE.NOTE] 您必須重新啟動所有 Azure AD Connect Health 代理程式服務，Proxy 設定才會更新。執行下列命令:<br>
    重新啟動服務 AdHealth *
