...
例如，假設的使用者的電子郵件是 Dan@BellowsCollege.com 接收透過電子郵件的機密檔案。 檔案已受 Azure 版權管理 (Azure RMS) 保護。 但是 Dan 的組織 (Bellows College) 尚未註冊 Azure RMS，也未部署 Active Directory RMS。 在此情況下，Dan 可以註冊個人版 RMS 的免費訂閱，以便讀取受保護的檔案。
...
+ 內部接管 （例如，可探索的自助服務、 未受管理的租用戶，和想要轉換成受管理租用戶系統管理員）
...
例如，假設您是 IT 管理員，而且已有 Contoso.com (貴組織註冊的網域名稱) 的受管理租用戶。 您會發現，使用者從您的組織有執行自我服務供應項目的使用電子郵件網域名稱 user@contoso.co.uk，也就是您的組織擁有的另一個網域名稱。 這些使用者目前在 contoso.co.uk 的未受管理租用戶中有帳戶。
...
2.  Office 365
    ...
    您可以使用的選項上 [管理網域](https://support.office.com/article/Navigate-to-the-Office-365-Manage-domains-page-026af1f2-0e6d-4f2d-9b33-fd147420fac2/) Office 365，才能使用您的網域和 DNS 記錄中的頁面。 請參閱 [驗證 Office 365 中的網域](https://support.office.com/article/Verify-your-domain-in-Office-365-6383f56d-3d09-4dcb-9b41-b5f5a5efd611/)。
...
1. 連接到 Azure AD 中使用的認證，用來回應自助式供應項目 ︰
        匯入模組 MSOnline
        $msolcred = 取得認證
        連接 msolservice-認證 $msolcred
