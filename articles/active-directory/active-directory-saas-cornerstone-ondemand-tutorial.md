...
4.  在 **Cornerstone OnDemand 在設定單一登入** ] 頁面上，下載您的憑證，按一下 [ **下載憑證**, ，然後將儲存在本機電腦上的憑證檔案。
...
5.  將下列項目傳送至 Cornerstone OnDemand 支援小組：
    ...
    >[AZURE.NOTE] 單一登入必須由 Cornerstone OnDemand 支援團隊設定。
    設定完成後，您會收到來自支援小組的通知。
...
>[AZURE.NOTE] 您可以使用任何其他 Cornerstone OnDemand 使用者帳戶建立工具或 Api 來佈建 AAD 使用者帳戶 Cornerstone OnDemand 所提供。
