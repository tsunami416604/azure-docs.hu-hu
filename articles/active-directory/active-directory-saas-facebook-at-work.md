<properties
    pageTitle="教學課程: Azure Active Directory 整合，以在工作的 Facebook |Microsoft Azure"
    description="了解如何設定單一登入 Azure Active Directory 與 Facebook 之間工作。"
...
# 教學課程: Azure Active Directory 整合，以在工作的 Facebook

本教學課程的目標在於告訴您如何使用 Azure Active Directory (Azure AD) 整合 Facebook 工作。<br>工作與 Azure AD 整合的 Facebook 會將您提供下列優點:

- 您可以控制能存取 Facebook 上工作的 Azure AD 中
- 您可以自動佈建帳戶已獲授與存取 Facebook at Work 的使用者
- 您可以讓您自動登入 facebook (單一登入) 的工作時使用其 Azure AD 帳戶的使用者
- 您可以在單一中央位置管理您的帳戶
...
若要設定 Facebook 整合至 Azure AD 的工作，您需要加入 Facebook 工作從資源庫加入您的受管理 SaaS 應用程式清單。

**若要從組件庫中新增工作 Facebook，請執行下列步驟:**
...
本節將概述如何讓使用者透過 Azure Active Directory，基於 SAML 通訊協定的同盟，使用中的帳戶驗證至上班的 Facebook。

**設定 Azure AD 單一登入工作的 facebook，請執行下列步驟:**

1.  在 Azure 管理入口網站中加入 Facebook at Work 後，按一下 [設定單一登入]****。

2.  在 [設定應用程式 URL]**** 畫面上，輸入使用者將登入您的 Facebook at Work 應用程式的 URL。 這是您在工作的租用戶 URL 的 Facebook 
(範例: https://example.facebook.com/)。 完成後，按一下 [ **下一步**。

3.  在不同的網頁瀏覽器視窗中，系統管理員身分登入您的 Facebook 工作公司站台。
