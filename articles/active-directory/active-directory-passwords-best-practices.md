...
* [**如何設定密碼管理的使用者**](#how-to-get-users-configured-for-password-reset)
  * [如何針對密碼重設設定帳戶](#what-makes-an-account-configured)
  * [自行填入驗證資料的方式](#ways-to-populate-authentication-data)
* [**若要啟用密碼重設為您的組織最好的方式**](#what-is-the-best-way-to-roll-out-password-reset-for-users)
...
### 使用強制註冊
如果您想讓使用者註冊密碼重設本身，您也可以強制這些登入的存取面板時註冊 [http://myapps.microsoft.com](http://myapps.microsoft.com)。  您可以從您的目錄啟用此選項 **設定** ] 索引標籤啟用 **要求使用者在登入存取面板時註冊** 選項。  

您可以選擇性地定義來重新註冊可設定的一段時間後修改的要求是否 **的使用者必須確認其連絡資料之前的天數** 必須為非零值的選項。 請參閱 [自訂使用者密碼管理行為](active-directory-passwords-customize.md#password-management-behavior) 如需詳細資訊。
...
按一下 **立即驗證** 予以更正，使 **密碼重設註冊入口網站** 在 [http://aka.ms/ssprsetup](http://aka.ms/ssprsetup) 及要求註冊。  透過這種方式註冊可以依序按一下關閉 **取消** 按鈕或關閉此視窗中，但是使用者會收到提醒每一次，如果不註冊登入。
...
2.  按一下 [ **使用者** ] 索引標籤。
3.  從清單中選取您需要的使用者。
4.  在第一個索引標籤中，您會發現 **備用電子郵件**, ，這可用來當做屬性來啟用密碼重設。 

    ![][005]

5.  按一下 [ **工作資訊** ] 索引標籤。
6.  在此頁面上，您會發現 **辦公室電話**, ，**行動電話**, ，**驗證電話**, ，和 **驗證電子郵件**。  您也可以設定這些屬性，以允許使用者重設密碼。 
