---
title: Az Azure AD feltételes hozzáférésfejlesztői útmutatója
description: Fejlesztői útmutató és forgatókönyvek az Azure AD feltételes hozzáféréshez
...
Az [egy- és több-bérlős](../develop/howto-convert-app-to-be-multi-tenant.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) alkalmazások és a [közös hitelesítési minták](v1-authentication-scenarios.md) ismerete feltételezhető.
...
* Egyoldalas alkalmazások az ADAL.js használatával
...
Az Azure AD feltételes hozzáférés az [Azure AD Premium](https://docs.microsoft.com/azure/active-directory/active-directory-whatis)szolgáltatása. A licencelési követelményekről a [licencnélküli használati jelentésben olvashat bővebben.](../active-directory-conditional-access-unlicensed-usage-report.md) A fejlesztők csatlakozhatnak a [Microsoft Developer Networkhálózathoz](https://msdn.microsoft.com/dn308572.aspx), amely ingyenes előfizetést tartalmaz az Enterprise Mobility Suite csomagra, amely tartalmazza az Azure AD Premium szolgáltatást.
...
![A folyamatábrát végző alkalmazás](./media/conditional-access-dev-guide/app-performing-on-behalf-of-scenario.png)
...
Ebben a forgatókönyvben a [.NET kódminta a](https://github.com/Azure-Samples/active-directory-dotnet-webapi-onbehalfof-ca)következőben is megjelenik. Bemutatja, hogyan adja át a jogcímkérést a web API 1-ről a natív alkalmazásra, és hogyan hozhat létre egy új kérelmet az ügyfélalkalmazáson belül.
...
![Több szolgáltatásból álló folyamatdiagram alkalmazáshoz való hozzáférés](./media/conditional-access-dev-guide/app-accessing-multiple-services-scenario.png)
...
![Új jogkivonatot kérő alkalmazás több szolgáltatás elérése](./media/conditional-access-dev-guide/app-accessing-multiple-services-new-token.png)

Ha az alkalmazás az ADAL-kódtárat használja, a jogkivonat beszerzésének sikertelenségét a rendszer mindig interaktívmódon próbálja meg újra. Az interaktív kérelem esetén a végfelhasználónak lehetősége van a feltételes hozzáférésnek való megfelelésre. Ez igaz, kivéve, `AcquireTokenSilentAsync` `PromptBehavior.Never` ha a kérelem egy vagy ```AcquireToken``` ebben az esetben az alkalmazásnak kell végrehajtania egy interaktív kérést, hogy a végfelhasználó a lehetőséget, hogy megfeleljenek a szabályzatnak.

## <a name="scenario-single-page-app-spa-using-adaljs"></a>Eset: Egyoldalas alkalmazás (SPA) az ADAL.js használatával

Ebben a forgatókönyvben végigvezetjük az esetet, amikor egy egyoldalas alkalmazás (SPA) használatával ADAL.js egy feltételes hozzáféréssel védett webes API-t. Ez egy egyszerű architektúra, de van néhány árnyalatok, amelyeket figyelembe kell venni, amikor a fejlődő körül feltételes hozzáférés.

Az ADAL.js-ben van néhány olyan függvény, `acquireToken(...)` `acquireTokenPopup(…)`amely `acquireTokenRedirect(…)`jogkivonatokat szerez be: `login()`, , , és .

* `login()`Egy azonosító jogkivonatot szerez be egy interaktív bejelentkezési kérelemen keresztül, de nem kap hozzáférési jogkivonatokat semmilyen szolgáltatáshoz (beleértve a feltételes hozzáféréssel védett webes API-t is).
* `acquireToken(…)`ezután használható a hozzáférési jogkivonat csendes megszerzésére, ami azt jelenti, hogy semmilyen körülmények között nem jeleníti meg a felhasználói felületet.
...
Ha egy alkalmazásnak hozzáférési jogkivonatra van szüksége `acquireToken(…)`egy webes API hívásához, megkísérli a. Ha a jogkivonat-munkamenet lejárt, vagy meg kell felelnünk egy feltételes hozzáférési szabályzatnak, akkor a *acquireToken* függvény sikertelen lesz, és az alkalmazás a vagyat `acquireTokenPopup()` `acquireTokenRedirect()`használja.

![Egyoldalas alkalmazás ADAL folyamatábrával](./media/conditional-access-dev-guide/spa-using-adal-scenario.png)

Mutassunk be egy példát a feltételes hozzáférés forgatókönyvével. A végfelhasználó most landolt az oldalon, és nincs munkamenet. Hívást hajtunk végre, `login()` többtényezős hitelesítés nélkül kapunk egy azonosító jogkivonatot. Ezután a felhasználó megnyom egy gombot, amely megköveteli, hogy az alkalmazás adatokat kérjen egy webes API-ból. Az alkalmazás megpróbál `acquireToken()` hívást végrehajtani, de sikertelen, mivel a felhasználó még nem hajtott végre többtényezős hitelesítést, és meg kell felelnie a feltételes hozzáférési szabályzatnak.
...
Ennek a forgatókönyvnek a kipróbálásához tekintse meg a [JS SPA A kódminta nevében](https://github.com/Azure-Samples/active-directory-dotnet-webapi-onbehalfof-ca). Ez a kódminta a feltételes hozzáférés-házirendet és a JS SPA-nál korábban regisztrált webes API-t használja a forgatókönyv bemutatásához. Bemutatja, hogyan kell megfelelően kezelni a jogcímkihívást, és kap egy hozzáférési jogkivonatot, amely használható a webes API-hoz. Alternatív megoldásként, pénztár az általános [Angular.js kód minta](https://github.com/Azure-Samples/active-directory-angularjs-singlepageapp) útmutatást egy angular SPA

## <a name="see-also"></a>Lásd még

* A képességekről a Feltételes hozzáférés az Azure Active Directoryban című témakörben olvashat [bővebben.](../active-directory-conditional-access-azure-portal.md)
* További Azure AD-kódminták, lásd: [GitHub-tárlatnyi kódminták.](https://github.com/azure-samples?utf8=%E2%9C%93&q=active-directory)
* Az ADAL SDK-król és a referenciadokumentáció eléréséről további információt a [könyvtárútmutatóban talál.](active-directory-authentication-libraries.md)
* Ha többet szeretne megtudni a több-bérlős forgatókönyvekről, olvassa [el a Felhasználók bejelentkezése a több-bérlős minta használatával című témakört.](../develop/howto-convert-app-to-be-multi-tenant.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json)
