...
2. 在 [啟用帳戶] 畫面中，按一下 [權限，然後選取 [內容] 功能表 **工作帳戶**。
3. 在 [新增帳戶] 畫面上，選取 [工作帳戶]。
4. 啟用裝置系統管理員在畫面上，按一下 [ **啟動**。
5. 在隱私權原則畫面上，選取此核取方塊，然後按一下 **確認**。
6. 在場所] 畫面中，輸入您的組織與按一下所提供的使用者識別碼 **加入**。
7. 若要登入 Azure Authenticator 應用程式，請輸入您組織 * * * 帳戶和密碼並按 [ **登入**。
8. 下一個畫面會顯示 Multi-Factor Authentication (MFA) 的相關資訊，這是為了增加安全性的選擇性畫面。 如果您的公司或學校要求次要因素驗證才能建立工作帳戶，您將會看到這個畫面。 它會提供進一步驗證帳戶的指示。
9. 場所] 畫面會顯示訊息，「**加入您的工作場所**」。 Azure Authenticator 應用程式會嘗試將您的裝置加入至您的工作場所。
...
2. 在 [啟用帳戶] 畫面中，按一下 [權限，然後選取 [內容] 功能表 **工作帳戶**。
3. 在 [帳戶] 畫面中，按一下 [ **新增帳戶**。
4. 如果您看到帳戶] 畫面上，按一下 [ **將帳戶新增**。 如果先前已建立工作帳戶，您會看到 [同步處理] 畫面顯示現有的工作帳戶。 您只需點選主畫面的上一頁箭頭，即可保留此工作帳戶。 或者，您可以選取要移除並重新建立新的工作帳戶的帳戶
在場所] 畫面中，輸入您的組織所提供的使用者識別碼，按一下 [加入。
5. 若要登入 Azure Authenticator 應用程式，輸入您的組織帳戶和密碼，按一下 **登入**。
7. 下一個畫面會顯示 Multi-Factor Authentication (MFA) 的相關資訊，這是為了增加安全性的選擇性畫面。 如果您的公司或學校要求次要因素驗證才能建立工作帳戶，您將會看到這個畫面。 它會提供進一步驗證帳戶的指示。
8. 按一下 [ **確定** 在下一個畫面。 請勿變更憑證的名稱。
「 加入工作場所 」 訊息。 Azure Authenticator 應用程式會嘗試將您的裝置加入至您的工作場所。
...
1. 從 **設定** ] 功能表中，navigat * * 至帳戶，然後按一下 e **新增帳戶**。
...
2. 從快顯功能表上，按一下 [ **記錄**。
3. 如果您有未結案的 Microsoft 支援事件，請記下事件號碼 (後面的步驟需要使用它)。 如果您還沒有建立支援事件，並想要的協助，請依照下列指引 [Microsoft 支援服務](https://support.microsoft.com/en-us/contactus) ，開啟新的事件。
4. 在 [記錄] 畫面中，按一下 [ **立即傳送**。
5. 選取您想要使用的電子郵件提供者。
...
9. 編輯 **至** 列和 **主體** 您收到來自 Microsoft 支援服務的資訊列。
...
1. 從帳戶管理員中，選取 **工作帳戶**。
2. 在 [工作帳戶] 畫面中 **一般設定**, ，請選取 **帳戶設定 – 離開工作場所網路**。
3. 選取 **保留** 上 **地點** 畫面。
4. 按一下 [ **確定** 」 是您確定要離開工作場所 」 的訊息顯示時。
5. 這可確保您已從工作場所中刪除您的工作帳戶。

>[AZURE。請注意 >
>建議您務必使用 [移除帳戶] 選項來刪除工作帳戶，因為這個選項可能不適用於某些舊版的 Android。

##解除安裝應用程式

在 Samsung Android 裝置上，裝置系統管理員權限必須先移除，如下所示才能解除安裝 
1. 從 **設定**, 下 **系統**, ，請選取 **安全性**。
2. D 內**evice 管理**, ，按一下 [ **裝置系統管理員**。 請確認核取方塊旁 **Azure Authenticator** 已清除。

##疑難排解

如果您看到  **金鑰存放區錯誤**, ，這可能是因為您不需要鎖定組畫面上，使用 PIN。 若要解決這個問題，請將 Azure Authenticator 應用程式解除安裝、設定鎖定螢幕的 PIN，以及重新安裝應用程式。
