<properties
    ...
    描述 = 「 Microsoft Azure Authenticator 應用程式可以用來登入來存取工作資源。 Azure Authenticator 應用程式會通知您的暫止的雙因素驗證要求您的行動裝置上顯示警告。 」
    服務 = [active directory]
    documentationCenter=""
    作者 ="femila"
    管理員 ="stevenpo"
...
2. 在 [啟用帳戶] 畫面上，按一下右邊的內容功能表，然後選取 [工作帳戶]****。
3. 在 [帳戶] 畫面上，按一下 [新增帳戶]****。
4. 如果您看到 [帳戶] 畫面，請按一下 [新增帳戶]****。 如果先前已建立工作帳戶，您會看到 [同步處理] 畫面顯示現有的工作帳戶。 您只需點選主畫面的上一頁箭頭，即可保留此工作帳戶。 或者，您可以選取要移除並重新建立新的工作帳戶的帳戶
...
1. 從 **設定** ] 功能表中，navigat**e 至帳戶，然後按一下 * * 新增帳戶**。
...
2. D 內**evice 管理**, ，按一下 [ **裝置系統管理員**。 確定已清除 [Azure Authenticator]**** 旁邊的核取方塊。
