...
- [判定商務需求](active-directory-hybrid-identity-design-considerations-business-needs.md)
- [判斷目錄同步作業需求](active-directory-hybrid-identity-design-considerations-directory-sync-requirements.md)
- [判斷多因素驗證需求](active-directory-hybrid-identity-design-considerations-multifactor-auth-requirements.md)
...
| 策略| 優點| 缺點|
|------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **雲端身分識別**| 在小型組織中易於管理。<br> 安裝在內部部署不需要額外的硬體<br>輕易地停用在使用者離開公司時| 使用者必須登入存取雲端中的工作負載時 <br> 密碼可能會或可能不是相同的雲端和內部部署身分識別|
| **已同步處理**| 內部部署密碼將會驗證這兩個內部部署和雲端目錄 <br>更容易管理的小型、 中型或大型組織 <br>使用者可以擁有單一登入 (SSO)，某些資源 <br> Microsoft 慣用的方法進行同步處理 <br> 更容易管理| 基於公司的特定原則，有些客戶可能不願意將目錄與雲端同步|
| **同盟**| 使用者可以擁有單一登入 (SSO) <br>如果使用者已終止或分葉的帳戶，可立即停用和存取撤銷，<br> 支援進階的案例，不能使用完成同步處理| 安裝和設定的詳細步驟 <br> 較高的維護 <br> STS 基礎結構可能需要額外的硬體 <br> 可能需要額外的硬體來安裝同盟伺服器。如果使用 AD FS，則需要額外的軟體 <br> 廣泛的安裝程式所需 SSO <br> 重大失敗點的同盟伺服器當機時，使用者會無法驗證|
