...
-   遵照提示進行，並建立新的 **Web 應用程式和/或 WebAPI**。
    ...
    -   「**重新導向 Uri**」是配置和字串的組合，Azure AD 可用它來傳回應用程式要求的任何權杖。 輸入 `https://localhost:44321 /` 此值。
-   完成註冊後，請瀏覽至 [**設定**] 索引標籤，並找到 [**應用程式識別碼 URI**] 欄位。 此值中，輸入租用戶特定識別碼，例如 `https://contoso.onmicrosoft.com/TodoListService`
...
-   開啟檔案 `App_Start\Startup.Auth.cs` 和實作 `ConfigureAuth(...)` 方法。 在您所提供的參數 `WindowsAzureActiveDirectoryBearerAuthenticationOptions` 將做為您的應用程式與 Azure AD 進行通訊的座標。
...
-   現在您可以使用 `Authorize` 屬性來保護您的控制器和動作搭配 JWT 承載驗證。 裝飾 `Controllers\TodoListController.cs` 使用授權標記的類別。 這樣會強制使用者在存取該頁面之前必須先登入。
...
-   最後，請開啟 `web.config` TodoListService 專案根目錄中的檔案，並輸入您的設定值，在 `< g s >` 一節。
  - `Ida: 租用戶` 是 Azure AD 租用戶，例如"contoso.onmicrosoft.com"的名稱。
  - 您 `ida: 對象` 是您在 Azure 入口網站中輸入的應用程式的應用程式識別碼 URI。
...
- 在 Visual Studio 中開啟 `App.config` TodoListClient 專案中，輸入您的設定值，在 `< g s >` 一節。
  - `Ida: 租用戶` 是 Azure AD 租用戶，例如"contoso.onmicrosoft.com"的名稱。
  - 您 `ida: ClientId` 您從 Azure 入口網站複製的應用程式識別碼。
  - 您 `todo:TodoListResourceId` 是您在 Azure 入口網站中輸入的待辦事項清單服務應用程式的應用程式識別碼 URI。
...
[建置.NET 原生用戶端與 Azure AD >>](../active-directory-devquickstarts-native-dotnet.md)
