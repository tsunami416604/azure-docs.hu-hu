...
`git 複製-分支基本架構 https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet.git`
...
`git 複製-分支完成 https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet.git`
...
-   若要開始，請開啟 `web.config` 檔案的根目錄中 `TodoList WebApp` 專案，然後輸入您的應用程式中的組態值 `< g s >` 一節。
    -   `Ida: ClientId` 是 **應用程式識別碼** 指派給您的應用程式中註冊入口網站。
    - `Ida: ClientSecret` 是 **應用程式密鑰** 您在註冊入口網站中建立。
    -   `Ida: RedirectUri` 是 **重新導向 Uri** 您輸入在入口網站。
- 開啟 `web.config` 檔案的根目錄中 `TodoList 服務` 專案，並取代 `ida: 對象` 具有相同 **應用程式識別碼** 與以上所述。

-   現在加入 OWIN 中介軟體 NuGet 封裝，以 `TodoList WebApp` 使用封裝管理員主控台的專案。
...
-   開啟檔案 `App_Start\Startup.Auth.cs` ，並新增 `使用` 陳述式，如上述的程式庫。
- 在相同檔案中，實作 `ConfigureAuth(...)` 方法。 在您所提供的參數 `OpenIDConnectAuthenticationOptions` 將做為您的應用程式與 Azure AD 進行通訊的座標。
...
`PM > 安裝套件 Microsoft.Experimental.IdentityModel.Clients.ActiveDirectory-ProjectName TodoList WebApp IncludePrerelease`
- 並加入另一個 `使用` 陳述式來 `App_Start\Startup.Auth.cs` ADAL 的檔案。
...
- 在 Web Apps 中，ADAL 具有可延伸的權杖快取，可以用來儲存權杖。 這個範例會實作 `NaiveSessionCache` 使用快取權杖的 http 工作階段存放區。
...
現在可以實際使用您在步驟 3 中取得的 access_token。 開啟 web 應用程式的 `Controllers\TodoListController.cs` 檔案，使得所有 CRUD 要求待辦事項清單 API。

- 這裡可再次使用 ADAL，從 ADAL 快取擷取 access_tokens。 首先，加入 `使用` 用於 ADAL 的陳述式，這個檔案。

    `使用 Microsoft.Experimental.IdentityModel.Clients.ActiveDirectory;`

- 在 `索引` 動作，使用 ADAL 的 `AcquireTokenSilentAsync` 方法來取得可用來從待辦事項清單服務讀取資料 access_token:
...
- 此範例將其加入產生的權杖做為 HTTP GET 要求 `授權` 待辦事項清單服務用來驗證的要求標頭。
- 如果待辦事項清單服務傳回 `401 未經授權` 回應，在 ADAL access_tokens 已經變成無效基於某些原因。 在此情況下，您應該卸除所有來自 ADAL 快取的 access_tokens，並向使用者顯示訊息告知可能需要再次登入，而這會重新啟動權杖的取得流程。
...
- 同樣地，如果 ADAL 因為任何原因而無法傳回 access_token，您也應指示使用者再次登入。 這很簡單，只要攔截任何 `AdalException`:
...
- 完全相同 `AcquireTokenSilentAsync` 呼叫是在 implementd `建立` 和 `刪除` 動作。 在 Web 應用程式中，只要應用程式有需要，就可以使用這個 ADAL 方法取得 access_tokens。 ADAL 會為您取得、快取和重新整理權杖。
