...
- 它也可讓 **撰寫的應用程式使用 [Graph API][GRAPH-API]** 來更新您的應用程式，而這需要最多精力。 如果您要撰寫管理軟體或需要自動定期更新應用程式屬性，這可能是個不錯的選擇。
...
如此一來，若要了解的格式和內容應用程式資訊清單，您必須參考 Graph API [應用程式的實體][APPLICATION-ENTITY] 文件。 可更新的範例不過執行應用程式上傳資訊清單
包括：

- 宣告您的 web API 所公開的權限範圍 (oauth2Permissions)。 請參閱中的 [公開 Web Api 至其他應用程式 」 主題 [與 Azure Active Directory 整合的應用程式][INTEGRATING-APPLICATIONS-AAD] 如需實作使用者模擬使用 oauth2Permissions 委派權限範圍。 如先前所述，所有的應用程式的實體屬性都記錄在 Graph API [實體和複雜型別參考][APPLICATION-ENTITY] 參考文件，包括 oauth2Permissions 成員，這是一組型別 [OAuth2Permission][APPLICATION-ENTITY-OAUTH2-PERMISSION]。
- 宣告您的應用程式所公開的應用程式角色 (appRoles)。 應用程式的實體 appRole 成員是型別集合 [AppRole][APPLICATION-ENTITY-APP-ROLE]。 請參閱 [角色為基礎的雲端應用程式使用 Azure AD 存取控制][RBAC-CLOUD-APPS-AZUREAD] 實作範例的文章。
- 宣告已知的用戶端應用程式。
- 要求 Azure AD 對登入使用者發出群組成員資格宣告。  注意：可設定為額外發出有關使用者目錄角色成員資格的宣告。 請參閱 [雲端應用程式使用 AD 群組中的授權][AAD-GROUPS-FOR-AUTHORIZATION] 實作範例的文章。
- 可讓您的應用程式支援 OAuth 2.0 隱含授與流程 (適用於內嵌的 JavaScript 網頁或單一頁面應用程式 (SPA))
- 啟用 X509 憑證做為密碼金鑰。 請參閱 [建置服務與背景程式的應用程式，在 Office 365][O365-SERVICE-DAEMON-APPS] 和 [驗證 Azure 資源管理員 api 的開發人員指南 ][DEV-GUIDE-TO-AUTH-WITH-ARM] 文章中的實作範例。
...
5. 在此範例中，我們在本機儲存檔案，讓我們可以在編輯器中開啟、對 JSON 進行變更然後再次儲存。 以下是 JSON 結構在 Visual Studio JSON 編輯器內的外觀。 請注意它所遵循的結構描述 [應用程式的實體][APPLICATION-ENTITY] 如我們先前所述 ︰
