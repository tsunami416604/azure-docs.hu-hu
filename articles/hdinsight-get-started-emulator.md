...
-   [Install the HDInsight Emulator][Install the HDInsight Emulator]
-   [Run the word count sample][Run the word count sample]
-   [Run the getting started samples][Run the getting started samples]
-   [Connect to Azure Blob storage][Connect to Azure Blob storage]
-   [Run HDInsight PowerShell][Run HDInsight PowerShell]
...
>
> -   Microsoft HDInsight Emulator for Windows Azure or HDInsight Developer Preview, whichever is installed.
...
1.  Open Internet Explorer, and then browse to the [Microsoft HDInsight Emulator for Azure installation page][Microsoft HDInsight Emulator for Azure installation page].
2.  Click **Install Now**.
...
7.  Verify the Web Platform Installer shows **The following products were successfully installed** status, and then click **Finish**.
...
The services related to HDInsight Emulator are not started by default. To start the services, from the Hadoop command line, run **start\_local\_hdp\_services.cmd** under <system drive\>\\hdp. To automatically start the services after the computer restarts, run **set-onebox-autostart.cmd**.
...
The samples are organized around processing IIS W3C log data scenarios. A data generation tool is provided to create and import the data sets in various sizes to HDFS or WASB (Azure Blob storage). See [Use Azure Blob storage for HDInsight][Use Azure Blob storage for HDInsight] for more information). MapReduce, Pig, or Hive jobs can then be run on the pages of data generated by the PowerShell script. Note that the Pig and Hive scripts are a layer of abstraction over MapReduce, and eventually compile to MapReduce programs. Users may run a series of jobs to observe the effects of using these different technologies and how the data size affects the execution of the processing tasks.

### In this section

-   [The IIS w3c log data scenarios][The IIS w3c log data scenarios]
-   [Load sample w3c log data][Load sample w3c log data]
-   [Run Java MapReduce jobs][Run Java MapReduce jobs]
-   [Run Hive jobs][Run Hive jobs]
-   [Run Pig jobs][Run Pig jobs]
-   [Rebuild the samples][Rebuild the samples]
...
3.  Run the following command to generate and import data to HDFS:
    ...
    If you want to load data into WASB instead, see [Connect to Azure Blob storage][Connect to Azure Blob storage].
...
4.  Run the following command:
    ...
    <td>
    The jar file is located in the C:\\hdp\\GettingStarted\\Java folder.
    ...
    <td>
    The type can be substituted by one of the following:
    ...
    <td>
    The input file can be substituted by the following:
...
6.  Run the following command to run the **w3ctotalhitsbypage.hql** HiveQL script file.
    ...
    <td>
    You can substitute the Hive script file with one of the following:
...
3.  Run the following command to submit a Pig job:
    ...
    <td>
    You can substitute the Pig Latin script file with one of the following:
    ...
    <td>
    You can substitute the parameter with a larger file:
...
> [WACOM.NOTE] For more information on how HDInsight uses Azure Blob storage, see [Use Azure blob Storage with HDInsight][Use Azure Blob storage for HDInsight].

Before you start with the instructions below, you must have created a storage account. For instructions, see [How To Create a Storage Account][How To Create a Storage Account].
...
For more information for submitting Hadoop jobs, see [Submit Hadoop jobs programmatically][Submit Hadoop jobs programmatically]. For more information about the HDInsight PowerShell cmdlets, see [HDInsight cmdlet reference][HDInsight cmdlet reference].
...
-   [Get started using Azure HDInsight][Get started using Azure HDInsight]
...
-   [MSDN forum for discussing HDInsight][MSDN forum for discussing HDInsight]
  ...
  [Microsoft HDInsight Emulator for Azure installation page]: http://www.microsoft.com/web/gallery/install.aspx?appid=HDINSIGHT
