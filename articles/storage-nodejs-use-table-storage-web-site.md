...
This tutorial shows you how to use Table service provided by Azure Data Management to store and access data from a [node][node] application hosted on Azure. This tutorial assumes that you have some prior experience using node and [Git][Git].
...
-   [node][node] version 0.10.24 or higher
...
1.  Open your web browser and go to the [Azure Portal][Azure Portal]. If prompted, login with your Azure subscription information.
...
In this section you will create a new Node application and use npm to add module packages. For the task-list application you will use the [Express][Express] and [Azure][Azure] modules. The Express module provides a Model View Controller framework for node, while the Azure modules provides connectivity to the Table service.
...
2.  Next, enter the following command to install the [azure][Azure], [node-uuid], [nconf] and [async] modules locally as well as to save an entry for them to the **package.json** file:
...
3.  Open a web browser and navigate to <http://127.0.0.1:3000>. This should display a web page similar to the following:
    ...
    ![A webpage displaying an empty tasklist][A web page displaying an empty tasklist]
...
> [WACOM.NOTE] These steps can also be performed by using the Azure portal. For steps on using the Azure portal to deploy a Node.js application, see [Create and deploy a Node.js application to an Azure Web Site][Create and deploy a Node.js application to an Azure Web Site].
...
If you do not already have an Azure subscription, you can sign up [for free][Azure Portal]. After signing up, follow these steps to continue this tutorial.
...
> [WACOM.NOTE] For more information, see [Install and configure the Azure Cross-Platform Command-Line Interface][Install and configure the Azure Cross-Platform Command-Line Interface];
...
3.  In the **app settings** section, enter **STORAGE\_NAME** in the **KEY** field, and the name of your storage account in the **VALUE** field. Click the checkmark to move to the next field. Repeat this process for the following keys and values:

    -   **STORAGE\_KEY** - the access key for your storage account
...
Once the changes have been deployed to Azure, your web application should continue to work as it is now reading the connection string from the **app settings** entry. To verify this, change the value for the **STORAGE\_KEY** entry in **app settings** to an invalid value. Once you have saved this value, the website should fail due to the invalid storage access key setting.

## Next steps

While the steps in this article describe using the Table Service to store information, you can also use MongoDB. See [Node.js Web Application with MongoDB][Node.js Web Application with MongoDB] for more information.

## Additional resources

[Azure command-line tool for Mac and Linux] [Create and deploy a Node.js application to Azure Web Sites]: /en-us/documentation/articles/web-sites-nodejs-develop-deploy-mac/
[Publishing to Azure Web Sites with Git][Publishing to Azure Web Sites with Git]: /en-us/documentation/articles/web-sites-publish-source-control/
[Azure Developer Center]: /en-us/develop/nodejs/
  ...
  [Install and configure the Azure Cross-Platform Command-Line Interface]: /en-us/documentation/articles/xplat-cli/
