...
下列各節說明使用 ASP.NET Web API 範本建置 RESTful Web API 的最佳作法。 如需使用 Web API 範本的詳細資訊，請瀏覽 [了解 ASP.NET Web API](http://www.asp.net/web-api) Microsoft 網站上的頁面。

## 實作要求路由的考量

使用 ASP.NET Web API 實作的服務，在每個要求都會路由傳送中的方法 _控制器_ 類別。 Web API 架構提供兩個主要選項實作路由。 _以慣例為基礎_ 路由和 _屬性為基礎_ 路由。 當您決定在 Web API 中路由傳送要求的最佳方式時，請考慮下列幾點：
...
- **了解限制和需求的慣例為基礎的路由**。
    ...
    這個路由會將符合 URI 的要求導向 _Customers_ 方法中的 _客戶_ 控制站。 此方法必須採用單一參數，名為 _custI_:
...
- **偏好以屬性為基礎的路由**。
    ...
    以屬性為基礎的路由會提供將路由連接至控制器中方法的替代方式。 不需依賴以慣例為基礎的路由的模式比對功能，您可以使用應相關聯之路由的詳細資料來明確地標註控制器中的方法。 此方法有助於移除可能模稜兩可的情形。 此外，因為明確路由定義於設計階段，所以此方法比執行階段以慣例為基礎的路由更有效率。 下列程式碼示範如何套用 _路由_ 屬性至 Customers 控制器中的方法。 這些方法也會使用 HttpGet 屬性來表示它們應該回應 _HTTP GET_ 要求。 此屬性可讓您使用任何方便的命名方式 (而非以慣例為基礎的路由所預期的方式) 替方法命名。 您也可以加上註解方法具有 _HttpPost_, ，_HttpPut_, ，和 _HttpDelete_ 屬性以定義可回應其他 HTTP 要求類型的方法。
    ...
    屬性為基礎的路由也有實用的副作用成為未來; 維護程式碼需要開發人員的說明文件立即清除是哪一種方法屬於哪個路由，而 _HttpGet_ 屬性可釐清方法所回應的 HTTP 要求的型別。

    以屬性為基礎的路由可讓您定義條件約束，以限制參數的比對方式。 條件約束可以指定參數的型別，而在某些情況下，也可以指出可接受的參數值範圍。 在下列範例中，id 參數 _FindCustomerByID_ 方法必須為非負整數。 如果應用程式提交具有負數客戶編號的 HTTP GET 要求，則 Web API 架構將以 HTTP 405 (不允許的方法) 訊息回應：
    ...
    如需以屬性為基礎的路由的詳細資訊，請參閱頁面 [Web API 2 中的屬性路由](http://www.asp.net/web-api/overview/web-api-routing-and-actions/attribute-routing-in-web-api-2) Microsoft 網站上。
...
- **區別不應該路由傳送的方法**。
    ...
    如果您使用以慣例為基礎的路由，表示由以未對應到 HTTP 動作的方法 _NonAction_ 屬性。 這通常適用於為了供控制器內的其他方法使用而定義的協助程式方法，而這個屬性會防止錯誤的 HTTP 要求比對並叫用這些方法。

- **請考慮將 API 置於子網域中的優缺點**。

    根據預設，ASP.NET web API 會組織成 _/api_ 目錄中的網域，例如 _http://www.adventure-works.com/api/orders_。 此目錄位於與相同主機所公開的任何其他服務相同的網域中。 可能會很有幫助 web API 分割出來成為自己的子網域執行在不同主機上，使用 Uri，例如 _http://api.adventure-works.com/orders_。 這種區隔可讓您分割和更有效地調整 web API，而不會影響任何其他 web 應用程式或服務中執行 _www.adventure-works.com_ 網域。

    不過，將 Web API 放在不同的子網域中也可能造成安全性問題。 任何 web 應用程式或服務裝載於 _www.adventure-works.com_ ，叫用的 web API 在其他位置執行，可能會違反許多網頁瀏覽器的同源原則。 在此情況下，必須啟用主機間的跨來源資源共用 (CORS)。 如需詳細資訊，請參閱＜API 安全性指引＞文件。
...
> [AZURE.NOTE] 發行項 [等冪性模式](http://blog.jonathanoliver.com/idempotency-patterns/) Jonathan Oliver 部落格上提供概觀，等冪性，以及它如何與資料管理作業。

- **建立新資源的 POST 動作應該這麼沒有不相關的副作用**。
...
- **避免實作繁複的 POST、 PUT 和 DELETE 作業**。
    ...
    請注意，ASP.NET Web API 2 內含的 OData 支援可提供批次要求的功能。 用戶端應用程式可以封裝數個 Web API 要求並將它們傳送至單一 HTTP 要求中的伺服器，以及接收包含各要求之回覆的單一 HTTP 回應。 如需詳細資訊，請參閱頁面 [將批次支援簡介 Web API 和 Web API OData 中](http://blogs.msdn.com/b/webdev/archive/2013/11/01/introducing-batch-support-in-web-api-and-web-api-odata.aspx) Microsoft 網站上。

- **傳送回應給用戶端應用程式時，請遵守 HTTP 通訊協定**。
...
- **提供連結，以支援資源的 hateoas 式導覽和探索**。
    ...
    - URI 的 HTTP DELETE 要求 _http://adventure-works.com/customers/2_ 用以刪除客戶。 此要求並不預期有任何其他資訊或在回應訊息內文中傳回資料。

    - URI 的 HTTP GET 要求 _http://adventure-works.com/customers/2/orders_ 用以尋找客戶的所有訂單。 此資料可以 XML 或 JSON 格式傳回。

    - URI 的 HTTP PUT 要求 _http://adventure-works.com/customers/2/orders_ 用以建立此客戶的新訂單。 此資料必須在要求訊息中以 x-www-form-urlencoded 格式提供。
...
- **擷取例外狀況，並傳回有意義的回應至用戶端**。

    實作 HTTP 作業的程式碼應提供完善的例外狀況處理，而不是讓未攔截到的例外狀況傳播到 Web API 架構。 如果例外狀況使作業無法成功完成，可以在回應訊息中傳回例外狀況，但應包含造成例外狀況之錯誤的有意義說明。 例外狀況也應包含適當的 HTTP 狀態碼，而不只是對每種狀況傳回狀態碼 500。 例如，如果使用者要求造成違反條件約束的資料庫更新 (例如嘗試刪除有未處理訂單的客戶)，您應傳回狀態碼 409 (衝突) 以及可指出衝突原因的訊息內本。 如果某些其他條件呈現要求無法達成，您可以傳回狀態碼 400 (不正確的要求)。 您可以在找到 HTTP 狀態碼的完整清單 [狀態碼定義](http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html) W3C 網站上的頁面。
    ...
    > [AZURE.TIP] 請勿包含可能有利於攻擊者嘗試滲透您的 web API 的資訊。如需詳細資訊，請瀏覽 [ASP.NET Web API 中的例外狀況處理](http://www.asp.net/web-api/overview/error-handling/exception-handling) Microsoft 網站上的頁面。

    > [AZURE.NOTE] 到達 web API 之前，許多網頁伺服器設陷錯誤條件本身。 例如，如果您設定網站的驗證，而使用者無法提供正確的驗證資訊，Web 伺服器應該以狀態碼 401 (未經授權) 回應。 用戶端經過驗證後，您的程式碼可以執行自己的檢查，確認用戶端應能夠存取所要求的資源。 如果此授權失敗，您應該傳回狀態碼 403 (禁止)。

- **以一致的方式和記錄錯誤的資訊處理例外狀況**。

    若要以一致的方式處理例外狀況，請考慮在整個 Web API 實作全域錯誤處理策略。 您可以建立一個例外狀況篩選，以便在控制器擲回任何未處理的例外狀況 (非 `HttpResponseException` 例外狀況) 時執行，部分達成此目標。 這種方法都會描述 [ASP.NET Web API 中的例外狀況處理](http://www.asp.net/web-api/overview/error-handling/exception-handling) Microsoft 網站上的頁面。
    ...
    若要處理這類情況，您可能需要實作更多自訂的方法。 您也應該併入錯誤記錄，以擷取每種例外狀況的完整詳細資料；此錯誤記錄檔可包含詳細的資訊 (只要用戶端無法透過 Web 存取即可)。 發行項 [Web API 全域錯誤處理](http://www.asp.net/web-api/overview/error-handling/web-api-global-error-handling) microsoft 網站會顯示其中一種執行這項工作。
...
- **支援用戶端快取**。
    ...
    在此範例中，快取控制標頭指定傳回的資料應在 600 秒後過期只適用於單一用戶端且不得儲存在其他用戶端所使用的共用快取中 (它是 _私人_)。 快取控制標頭可以指定 _公用_ 而 _私人_ 在此情況下資料可以儲存在共用快取，或者它也可以指定 _沒有存放區_ 資料必須在此情況下 **不** 由用戶端快取。 下列程式碼範例示範如何在回應訊息中建構 Cache-Control 標頭：
    ...
    > [AZURE.NOTE] HTTP 通訊協定也會定義 _不快取_ 快取控制標頭。 令人不解的是，此指示詞並非表示「不快取 」，而是「在傳回快取的資訊前使用伺服器進行重新驗證」；此資料仍會快取，但每次用來確保其仍為當前資料時會進行檢查。

    快取管理是用戶端應用程式或中繼伺服器的責任，但若已正確實作，則可藉由移除提取最近擷取之資料的需求來節省頻寬及改善效能。

     _存留期上限_ 快取控制標頭中的值是只是指南，但不保證對應的資料不會變更指定的時間。 Web API 應該根據預期的資料變動性，將 max-age 設定為適當的值。 在此期間過期時，用戶端應捨棄快取中的物件。

    > [AZURE.NOTE] 現今大部分的網頁瀏覽器支援用戶端快取藉由將適當的快取控制標頭加入至要求，並檢查結果的標頭，如所述。 不過，有些較舊的瀏覽器不會快取從包含查詢字串的 URL 傳回的值。 對於根據此處討論的通訊協定，實作自己的快取管理策略的自訂用戶端應用程式而言，這通常不是問題。
    >
...
- **提供 Etag 來最佳化查詢處理**。
    ...
    > [AZURE.TIP] 基於安全性理由，不允許機密資料或透過驗證 (HTTPS) 連線至快取傳回的資料。
    ...
    > [AZURE.NOTE] 如果在回應標頭包含快取控制標頭 no-store，則應該一律從不論 HTTP 狀態碼快取移除的物件。
    ...
    > [AZURE.TIP] 在此範例中，資料的 ETag 會產生雜湊從基礎資料來源擷取的資料。 如果以其他方式計算 ETag，則可進一步最佳化程序，而如果資料已經變更，只需從資料來源提取。  如果是大型資料或存取資料來源可能導致明顯延遲 (例如，如果資料來源是遠端資料庫)，這個方法特別有用。
...
- **使用 Etag 支援開放式並行存取**。
    ...
    > [AZURE.TIP] 使用 If-match 標頭完全是選擇性的如果省略的 web API 將永遠嘗試更新指定的順序，可能會盲目地覆寫另一位使用者所做的更新。 若要避免遺失更新所衍生的問題，一定要提供 If-match 標頭。
...
- **最佳化涉及大型物件的要求和回應**。
    ...
    > [AZURE.TIP] 您可以上傳至 web 服務的資料量不受到資料流，並以單一要求可能會導致耗用相當多資源的巨大物件。 在串流處理過程中，如果 Web API 判定要求中的資料量超過一些可接受的界限，則可以中止作業並傳回具有狀態碼 413 (要求實體太大) 的回應訊息。
    ...
    > [AZURE.TIP] 您可以結合編碼的壓縮與串流;壓縮資料串流前先，並指定 gzip 內容編碼方式和區塊的傳輸編碼訊息標頭中。 另請注意，可將某些 Web 伺服器 (例如 Internet Information Server) 設定為自動壓縮 HTTP 回應 (不論 Web API 是否壓縮資料)。
...
- **避免在用戶端應用程式中傳送不必要的 Continue 狀態訊息**。
    ...
    您也可以設定 `ServicePointManager` 類別的靜態 `Expect100Continue` 屬性，以針對所有後續建立的 `ServicePoint` 物件指定此屬性的預設值。 如需詳細資訊，請參閱 [ServicePoint 類別](https://msdn.microsoft.com/library/system.net.servicepoint.aspx) Microsoft 網站上的頁面。
...
- **支援對可能傳回大量物件的要求進行分頁**。
    ...
    用戶端應用程式可以發出要求來擷取從位移 50 開始使用 URI 的 30 張訂單 _http://www.adventure-works.com/api/orders?limit=30&offset=50_。

    > [AZURE.TIP] 避免讓用戶端應用程式指定會導致長時間已超過 2000年個字元的 URI 的查詢字串。 許多 Web 用戶端和伺服器都無法處理此種長度的 URI。
...
- **提供非同步支援長時間執行的要求**。
    ...
    > [AZURE.NOTE] 如需有關如何使用 WebJobs 與 Azure 網站的詳細資訊，請瀏覽的頁面 [使用 Webjob 在 Microsoft Azure 網站中執行背景工作](web-sites-create-web-jobs.md) Microsoft 網站上。

    Web API 也應提供一種機制，將處理的結果傳回給用戶端應用程式。 提供輪詢機制，以供用戶端應用程式定期查詢處理是否已完成並取得結果，或讓 Web API 在作業完成時傳送通知，即可達到此目的。

    您可以藉由實作簡單的輪詢機制 _輪詢_ 當作虛擬資源，使用下列方法的 URI:

    1. 用戶端應用程式會將初始要求傳送至 Web API。

    2. Web API 會將要求相關資訊儲存在資料表儲存體或 Microsoft Azure 快取中保存的資料表中，並為此項目產生唯一索引鍵 (可能是 GUID 格式)。

    3. Web API 會將此處理當作個別的工作起始。 Web API 與資料表中記錄工作的狀態 _執行_。

    4. Web API 會傳回具有 HTTP 狀態碼 202 (已接受) 的回應訊息，以及訊息內文中資料表項目的 GUID。

    5. 當工作完成時，web API 會將結果儲存在資料表中，並設定工作的狀態 _完成_。 請注意，是否工作失敗，web API 可能也儲存失敗的相關資訊，將狀態設為 _失敗_。
    ...
    - 使用 Azure 通知中樞，將非同步回應推送至用戶端應用程式。 頁面 [Azure 通知中樞通知使用者](notification-hubs-aspnet-backend-windows-dotnet-notify-users.md) Microsoft 網站提供進一步的詳細資料。

    - 使用 Comet 模型來保持用戶端與裝載 Web API 的伺服器之間的持續性網路連線，並使用此連線將訊息從伺服器推送回到用戶端。 MSDN 雜誌文章 [建置簡單的 Comet 應用程式，Microsoft.NET Framework 中](https://msdn.microsoft.com/magazine/jj891053.aspx) 舉例說明解決方案。

    - 使用 SignalR 並透過持續性網路連線，即時將資料從 Web 伺服器推送到用戶端。 SignalR 可以 NuGet 封裝形式使用於 ASP.NET Web 應用程式。 您可以在找到更多資訊 [ASP.NET SignalR](http://signalr.net/) 網站。

    > [AZURE.NOTE] Comet 與 SignalR 均利用 web 伺服器和用戶端應用程式之間的持續性網路連線。 這可能會影響延展性，因為大量用戶端可能需要同樣大量的並行連線。
...
- **請小心管理持續性 HTTP 連線**。
    ...
    > [AZURE.NOTE] 持續性 HTTP 連線是純粹是選擇性的功能，可減少與重複建立通訊通道相關聯的網路負擔。 Web API 和用戶端應用程式都不應該依賴持續性 HTTP 連線才可使用。 請勿使用持續性 HTTP 連線來實作 Comet 式通知系統；而是應該利用 TCP 層的通訊端 (或可用的 Websocket)。 最後請注意，如果用戶端應用程式透過 Proxy 與伺服器進行通訊，則 Keep-Alive 標頭的使用受限；只有與用戶端和 Proxy 的連線具持續性。
...
讓這些問題能與 Web API 實作的相關技術問題脫鉤，很有助益。 基於這個原因，請考慮建立 [外觀](http://en.wikipedia.org/wiki/Facade_pattern), 、 形式執行不同的處理序，並將要求路由到 web API。 façade 可以提供管理作業並將已驗證的要求轉送至 Web API。 使用 façade 也可帶來許多功能性優點，包括：
...
## 測試 Web API 的考量
Web API 應與軟體的任何其他部分一樣徹底進行測試。 您應考慮建立單位測試來驗證每個作業的功能，就像處理任何其他類型的應用程式一樣。 如需詳細資訊，請參閱頁面 [使用單元測試驗證程式碼](https://msdn.microsoft.com/library/dd264975.aspx) Microsoft 網站上。

> [AZURE.NOTE] 範例 web API 可以使用本指南包含示範如何執行單元測試選取的作業上的測試專案。
...
- 確認所有路由都得到適當保護，而且都遵守適當的驗證和授權檢查。
    ...
    > [AZURE.NOTE] 某些層面的安全性，例如使用者驗證最可能的主機環境，而不是 web API，但仍必須將安全性測試納入部署程序的一部分。
...
- 驗證回應訊息中的所有連結和 URI。 例如，HTTP POST 訊息應該會傳回新建資源的 URI。 所有 HATEOAS 連結都應該有效。
    ...
    > [AZURE.IMPORTANT] 如果您發佈 web API 透過 API 管理服務，然後這些 Uri 應該會反映管理服務的 URL，而非裝載 web API 的 web 伺服器。
...
您也應該建立和執行效能測試，檢查 Web API 在受限的情況下是否運作順利。 您可以使用 Visual Studio Ultimate，建置 Web 效能和負載測試專案。 如需詳細資訊，請參閱頁面 [發行前先對應用程式執行效能測試](https://msdn.microsoft.com/library/dn250793.aspx) Microsoft 網站上。

## 使用 Azure API 管理服務來發佈和管理 Web API

Azure 提供 [API 管理服務](http://azure.microsoft.com/documentation/services/api-management/) 可用來發佈和管理 web API。 您可以使用這項工具來產生一個服務，以做為一或多個 Web API 的 façade。 此服務本身是可使用 Azure 管理入口網站建立及設定的可延伸 Web 服務。 您可以使用這項服務來發佈和管理 Web API，如下所示：
...
2. 將 API 管理服務連接到 Web API。 傳送至管理 API 之 URL 的要求會對應至 Web API 中的 URI。 相同的 API 管理服務可將要求路由傳送到多個 Web API。 這可讓您將多個 Web API 彙總成單一管理服務。 同樣地，如果您需要限制或分割不同應用程式可用的功能，可以從一個以上的 API 管理服務參考相同的 Web API。
    ...
    > [AZURE.NOTE] HATEOAS 連結中產生的 HTTP GET 要求的回應一部分的 Uri 應參考 API 管理服務，而非裝載 web API web 伺服器的 URL。
...
5. 建立產品。 產品是發佈的單位；您會將先前連接的 Web API 加入至產品的管理服務。 發佈產品時，Web API 即可供開發人員使用。
    ...
    > [AZURE.NOTE] 在之前發行的產品，您也可以定義使用者群組可以存取產品，並將使用者新增到這些群組。 這可讓您控制可使用 Web API 的開發人員和應用程式。 如果 Web API 需獲得核准，開發人員必須先將要求傳送給產品系統管理員，才能夠加以存取。 系統管理員可以授與或拒絕程式開發人員的存取。 如果情況變更，也可能會封鎖現有的開發人員。
...
您可以找到說明如何執行這些工作的完整詳細資料 [API 管理](http://azure.microsoft.com/services/api-management/) Microsoft 網站上的頁面。 Azure API 管理服務也提供自己的 REST 介面，讓您建置自訂介面，以簡化設定 Web API 的程序。 如需詳細資訊，請瀏覽 [Azure API 管理 REST API 參考](https://msdn.microsoft.com/library/azure/dn776326.aspx) Microsoft 網站上的頁面。

> [AZURE.TIP] Azure 提供 Azure 流量管理員可讓您實作容錯移轉和負載平衡和跨網站裝載於不同地理位置中的多個執行個體減少延遲。 您可以使用 Azure 流量管理員搭配 API 管理服務；API 管理服務可以透過 Azure 流量管理員，將要求路由傳送至網站的執行個體。  如需詳細資訊，請瀏覽 [關於流量管理員負載平衡方法](../traffic-manager/traffic-manager-load-balancing-methods.md) Microsoft 網站上的頁面。
...
- 產品文件，其中列出所公開的作業、必要的參數，以及可以傳回的各種回應。 請注意，這項資訊由一節中的步驟 3 所提供的詳細資料產生 [使用 Microsoft Azure API 管理服務發佈 web API](#publishing-a-web-API)。
...
您可以從 Azure 管理入口網站即時檢視此資料。 您也可以建立用以監控 Web API 健全狀況的 Web 測試。 Web 測試會傳送定期要求至 Web API 中指定的 URI，並擷取回應。 您可以指定成功回應 (例如 HTTP 狀態碼 200) 的定義，而如果要求未傳回此回應，您可以安排要傳送給系統管理員的警示。 必要時，系統管理員可以重新啟動裝載 Web API 的伺服器 (如果失敗的話)。
 ...
 [Application Insights-開始監控應用程式的健全狀況和使用量](app-insights-start-monitoring-app-health-usage/) Microsoft 網站上的頁面會提供詳細資訊。
...
- **使用**。 此索引標籤提供已進行的 API 呼叫數目以及用來處理這些呼叫的頻寬相關資訊。 您可以依照產品、API 和作業篩選使用量詳細資料。
...
- **一眼**。 此索引標籤會顯示效能資料的摘要，包括負責進行大部分 API 呼叫的開發人員，以及收到這些呼叫的產品、Web API 和作業。

您可以使用這項資訊來判斷是否特定 Web API 或作業造成瓶頸，以及視需要調整主機環境並加入更多伺服器。 您也可以確定一或多個應用程式是否使用不相稱的資源量，並套用適當的原則來設定配額及限制呼叫率。

> [AZURE.NOTE] 您可以變更已發行的產品，詳細資料，所做的變更會立即套用。 例如，您可以在 Web API 中新增或移除作業，而不需要重新發佈含有 Web API 的產品。

## 相關的模式
-  [外觀](http://en.wikipedia.org/wiki/Facade_pattern) 模式描述如何提供 web API 的介面。

## 詳細資訊
- 頁面 [了解 ASP.NET Web API](http://www.asp.net/web-api) Microsoft 網站提供如何使用 Web API 建置 RESTful web 服務的詳細的介紹。
- 頁面 [ASP.NET Web API 中路由](http://www.asp.net/web-api/overview/web-api-routing-and-actions/routing-in-aspnet-web-api) microsoft 網站將告訴您如何以慣例為基礎的 ASP.NET Web API 架構中的路由運作。
- 如需以屬性為基礎的路由的詳細資訊，請參閱頁面 [Web API 2 中的屬性路由](http://www.asp.net/web-api/overview/web-api-routing-and-actions/attribute-routing-in-web-api-2) Microsoft 網站上。
-  [基本教學課程](http://www.odata.org/getting-started/basic-tutorial/) OData 網站上的頁面提供 OData 通訊協定的功能簡介。
-  [ASP.NET Web API OData](http://www.asp.net/web-api/overview/odata-support-in-aspnet-web-api) Microsoft 網站上的頁面包含相關範例以及更多有關使用 ASP.NET 實作 OData web API。
- 頁面 [將批次支援簡介 Web API 和 Web API OData 中](http://blogs.msdn.com/b/webdev/archive/2013/11/01/introducing-batch-support-in-web-api-and-web-api-odata.aspx) microsoft 網站說明如何使用 OData web API 中實作批次作業。
- 發行項 [等冪性模式](http://blog.jonathanoliver.com/idempotency-patterns/) Jonathan Oliver 部落格上提供概觀，等冪性，以及它如何與資料管理作業。
-  [狀態碼定義](http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html) W3C 網站上的頁面包含 HTTP 狀態碼及其描述的完整清單。
- 如需處理與 ASP.NET Web API 的 HTTP 例外狀況的詳細資訊，請瀏覽 [ASP.NET Web API 中的例外狀況處理](http://www.asp.net/web-api/overview/error-handling/exception-handling) Microsoft 網站上的頁面。
- 發行項 [Web API 全域錯誤處理](http://www.asp.net/web-api/overview/error-handling/web-api-global-error-handling) microsoft 網站說明如何實作全域錯誤處理和記錄 web API 的策略。
- 頁面 [使用 Webjob 在 Microsoft Azure 網站中執行背景工作](web-sites-create-web-jobs.md) Microsoft 網站提供有關使用 Webjob 執行背景作業在 Azure 網站上的資訊和範例。
- 頁面 [Azure 通知中樞通知使用者](notification-hubs-aspnet-backend-windows-dotnet-notify-users/) microsoft 網站會顯示如何使用 Azure 通知中樞將非同步回應推送至用戶端應用程式。
-  [API 管理](http://azure.microsoft.com/services/api-management/) Microsoft 網站上的頁面說明如何發佈產品，以提供受控制且安全的 web API 存取。
-  [Azure API 管理 REST API 參考](https://msdn.microsoft.com/library/azure/dn776326.aspx) Microsoft 網站上的頁面說明如何使用 API 管理 REST API 建置自訂管理應用程式。
-  [關於流量管理員負載平衡方法](../traffic-manager/traffic-manager-load-balancing-methods.md) Microsoft 網站上的頁面摘要說明 Azure 流量管理員可以用負載平衡要求多個執行個體裝載 web API 的網站。
