...
3. Itt meg kell adnia a `wsid` (log Analytics munkaterület azonosítóját) és `wskey` (a munkaterület azonosítójának elsődleges kulcsát). Ha mindkettőre `wsid` szüksége `wskey` van, és létre kell hoznia <https://mms.microsoft.com>egy fiókot a következő helyen:.
   A fiók létrehozásához kövesse a lépéseket. Ha elkészült a fiók létrehozásával `wsid` , be kell szereznie és `wskey` a **Beállítások**, majd a **csatlakoztatott források**, majd a Linux- **kiszolgálók**elemre kell kattintania az alább látható módon.
...
Jelentkezzen be Azure Portal a <https://portal.microsoft.com/>következő címen:. Lépjen a **piactérre**, válassza a **figyelés + felügyelet** elemet, majd kattintson **az összes**megjelenítése elemre. Ezután írja `containers` be a keresést. A keresési eredmények között megjelenik a "tárolók". Válassza a **tárolók** lehetőséget, majd kattintson a **Létrehozás**gombra.
...
Ha a csomópontok tényleges száma vagy a virtuálisgép-méretezési csoport méretezése több virtuális gép hozzáadásával történik, akkor ezt a `msoms` szolgáltatás skálázásával teheti meg. log Analytics
