...
A futtatásával ellenőrizheti, hogy telepítve van-e az `az` eszköz:

```azurecli
az --version
```

Ha nincs `az` telepítve az eszköz, [itt](https://github.com/azure/azure-cli#installation)talál útmutatást.

A futtatásával ellenőrizheti, hogy telepítve van-e az `kubectl` eszköz:
...
A tárolók figyeléséhez a Sysdig egy Kubernetes-t használó minden gépen futtat egy folyamatot `DaemonSet` .
...
A Sysdig-daemonset elemet telepítéséhez először le kell töltenie [a sablont](https://github.com/draios/sysdig-cloud-scripts/tree/master/agent_deploy/kubernetes) a Sysdig-ből. Mentse a fájlt másként `sysdig-daemonset.yaml` .
