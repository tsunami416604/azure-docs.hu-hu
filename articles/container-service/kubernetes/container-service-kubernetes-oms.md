...
Azt is megteheti, hogy a [Azure Cloud Shell](https://docs.microsoft.com/azure/cloud-shell/overview)is `az` használhatja, amely `kubectl` rendelkezik az Azure CLI-vel és a már telepített eszközökkel.
...
Ahhoz, hogy a Log Analytics-ügynök kommunikáljon a szolgáltatással, meg kell adni egy munkaterület-azonosítót és egy munkaterület-kulcsot. A munkaterület-azonosító és-kulcs beszerzéséhez létre kell hoznia <https://mms.microsoft.com>egy fiókot a következő helyen:.
...
Itt látható a [DAEMONSET elemet YAML-fájlja](https://github.com/Microsoft/OMS-docker/tree/master/Kubernetes). Mentse egy nevű `oms-daemonset.yaml` fájlba, és cserélje le a fájlhoz tartozó `WSID` `KEY` , a munkaterület azonosítóját és kulcsát tartalmazó értéket a helyére.

Miután hozzáadta a munkaterület AZONOSÍTÓját és kulcsát a Daemonset elemet-konfigurációhoz, telepítheti a Log Analytics ügynököt a fürtön `kubectl` a parancssori eszközzel:
...
- Futtassa a szkriptet. A parancsfájl kérni fogja a Log Analytics munkaterület AZONOSÍTÓját és elsődleges kulcsát. Szúrja be, és a szkript létrehoz egy titkos YAML-fájlt, amellyel futtathatja.
  ...
  - A Secrets Pod létrehozásához futtassa a következőt:
