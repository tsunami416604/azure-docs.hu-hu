...
Azt is megteheti, hogy a [Azure Cloud Shell](https://docs.microsoft.com/azure/cloud-shell/overview)is használhatja, amely rendelkezik az `az` Azure CLI-vel és a `kubectl` már telepített eszközökkel.
...
Itt látható a [DAEMONSET elemet YAML-fájlja](https://github.com/Microsoft/OMS-docker/tree/master/Kubernetes). Mentse egy nevű fájlba, `oms-daemonset.yaml` és cserélje le a fájlhoz tartozó, a `WSID` `KEY` munkaterület azonosítóját és kulcsát tartalmazó értéket a helyére.

Miután hozzáadta a munkaterület AZONOSÍTÓját és kulcsát a Daemonset elemet-konfigurációhoz, telepítheti a Log Analytics ügynököt a fürtön a `kubectl` parancssori eszközzel:
