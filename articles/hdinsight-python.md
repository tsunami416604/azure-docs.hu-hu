...
<tags ms.service="hdinsight" ms.workload="big-data" ms.tgt_pltfrm="na" ms.devlang="python" ms.topic="article" ms.date="09/17/2014" ms.author="larryfr"></tags>
...
-   [Python on HDInsight][Python on HDInsight]
-   [Hive and Python][Hive and Python]
...
See [Running the examples][Running the examples] for how to run this example on your HDInsight cluster.
...
4.  Next, it iterates over the records in **LOG** and uses **GENERATE** to invoke the **create\_structure** method contained in the **jython.py** script loaded as **myfuncs**. **LINE** is used to pass the current record to the function.
...
2.  Next, the **def create\_structure(input)** defines the function that Pig will pass line items to.
...
These steps use Windows Azure PowerShell. If this is not already installed and configured on your development machine, see [How to install and configure Azure PowerShell][How to install and configure Azure PowerShell] before using the following steps.

1.  Using the Python examples [streaming.py][streaming.py] and [jython.py][jython.py], create local copies of the files on your development machine.
...
2.  Use the following PowerShell script to upload the **streaming.py** and **jython.py** files to the server. Substitute the name of your Azure HDInsight cluster, and the path to the **streaming.py** and **jython.py** files on the first three lines of the script.
    ...
    > [WACOM.NOTE] Other methods of uploading the scripts can be found in the [Upload data for Hadoop jobs in HDInsight][Upload data for Hadoop jobs in HDInsight] document.
...
<td>
Look at these files in the blob container
...
If you need to load Python modules that aren't provided by default, see [How to deploy a module to Azure HDInsight][How to deploy a module to Azure HDInsight] for an example of how to do this.

If you would like to run jobs on HDInsight remotely without using PowerShell, see [How to use Azure HDInsight from Linux][How to use Azure HDInsight from Linux] for an example of using Python to run jobs through the WebHCat REST API.
  ...
  [How to deploy a module to Azure HDInsight]: http://blogs.msdn.com/b/benjguin/archive/2014/03/03/how-to-deploy-a-python-module-to-windows-azure-hdinsight.aspx
  [How to use Azure HDInsight from Linux]: http://blogs.msdn.com/b/benjguin/archive/2014/02/18/how-to-use-hdinsight-from-linux.aspx
