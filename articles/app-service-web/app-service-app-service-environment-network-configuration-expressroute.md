...
## 概觀 ##
客戶可以連接的 [Azure ExpressRoute] [ExpressRoute] 電路到其虛擬網路基礎結構，因此將其內部部署網路延伸至 Azure。  App Service 環境可以建立子網路的 [虛擬網路] 中 [virtualnetwork] 基礎結構。  在 App Service 環境上執行的應用程式接著可以建立與後端資源的安全連線，而後端資源只能透過 ExpressRoute 連線來存取。  
...
-  [本文] [requiredports] 中所述，必須允許輸入的網路存取 App Service 環境所需的連接埠。

確定已針對虛擬網路設定及維護有效的 DNS 基礎結構，即可符合 DNS 需求。  如果 DNS 設定在建立 App Service 環境之後因為任何原因而變更，開發人員可以強制 App Service 環境挑選新的 DNS 組態。  觸發輪流的環境重新開機，使用 「 重新啟動 」 圖示位於頂端的 [Azure 網站] [NewPortal] 中的 App Service 環境管理分頁會挑選新的 DNS 組態的環境。

可以藉由設定 [網路安全性群組] 符合輸入的網路存取的需求 [NetworkSecurityGroups] 以允許必要的存取 [本文] [requiredports] 中所述的 App Service 環境的子網路上。
...
使用者定義路由的背景資訊可用於此 [概觀] [UDROverview]。  

使用此 [作法指南] [UDRHowTo] 中建立及設定使用者定義的路由的詳細資訊。

## App Service 環境的範例 UDR 組態 ##

**必要條件**

1. 從 [Azure 下載頁面] 安裝最新的 Azure Powershell [AzureDownloads] (年 6 月，2015年或更新版本)。  在 [命令列工具] 的 [Windows Powershell] 下，有一個 [安裝] 連結可安裝最新的 Powershell Cmdlet。
...
或者，您可以下載完整和更新的由 Azure 使用中的 CIDR 範圍清單。  包含所有的 Azure IP 位址範圍的 Xml 檔案可從 [Microsoft 下載中心] [DownloadCenterAddressRanges]。  
