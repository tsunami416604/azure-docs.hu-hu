...
**附註:**  無法在"v2"的虛擬網路中建立 App Service 環境。 App Service 環境目前僅支援處理傳統「v1」虛擬網路。
...
**非常重要:**  ExpressRoute 組態不支援 App Service 環境的 **不正確地跨-通告路由公用互連路徑從私用對等互連路徑**。 已設定公用對等互連的 ExpressRoute 組態，會收到來自 Microsoft 的一大組 Microsoft Azure IP 位址範圍的路由通告。 如果這些位址範圍在私人對等互連路徑上不正確地交叉通告，最後的結果會是來自 App Service 環境子網路的所有輸出網路封包都會不正確地使用強制通道傳送至客戶的內部部署網路基礎結構。 此網路流程將會破壞 App Service 環境。 此問題的解決方案是停止從公用對等互連路徑至私人對等互連路徑的交叉通告路由。
...
1. 安裝最新的 Azure Powershell 從 [Azure 下載頁面上的 ][azuredownloads] (年 6 月，2015年或更新版本)。 在 [命令列工具] 的 [Windows Powershell] 下，有一個 [安裝] 連結可安裝最新的 Powershell Cmdlet。
