...
技術程序限制通常會妨礙成功的實作敏捷式方法。 Azure App Service 與功能，例如 [連續發行](web-sites-publish-source-control.md), ，[預備環境](web-sites-staged-publishing.md) (位置)，和 [監視](web-sites-monitor.md), 、 明智地結合的協調流程與管理部署在 [Azure 資源管理員](resource-group-overview.md), ，對於採納敏捷式軟體開發的開發人員可以參與的絕佳解決方案。
...
| - 在生產環境的複製品中執行測試| Azure 資源管理員範本可用來建立 Azure 生產環境 (包括應用程式設定、 連接字串範本、 縮放等) 的複製程式碼測試快速且可預測。|
| - 輕鬆地檢視最新組建結果| 從儲存機制持續部署至 Azure，表示您可以在認可變更之後立即於即時應用程式中測試新程式碼。|
| -認可至主要分支每天<br>-自動化部署| 生產應用程式與儲存機制主要分支的連續整合會自動將主要分支的每次認可/合併部署至生產環境。|
...
生產和預備環境的範本所定義 [* < repository_root > */ARMTemplates/ProdandStage.json](https://github.com/azure-appservice-samples/ToDoApp/blob/master/ARMTemplates/ProdAndStage.json)。

開發和測試環境的範本所定義 [* < repository_root > */ARMTemplates/Dev.json](https://github.com/azure-appservice-samples/ToDoApp/blob/master/ARMTemplates/Dev.json)。
...
4.  您的本機副本之後，請瀏覽至 *< repository_root >*\ARMTemplates，並執行 deploy.ps1 指令碼，如下所示:
...
4.  系統提示時，請輸入所需的使用者名稱和密碼來存取資料庫。
    ...
    >[AZURE.TIP] 看看 *< repository_root >*\ARMTemplates\Deploy.ps1，以查看它如何產生資源的唯一識別碼。 您可以使用相同的方法來建立相同部署的複製品，而不需擔心衝突的資源名稱。
...
7.  當指令碼完成時，請回到瀏覽至前端的位址 (http://ToDoApp*< unique_string >*master.azurewebsites.net/) 若要查看在生產環境中執行的應用程式。
...
2.  簡單地變更應用程式的 UI 層藉由變更程式碼來使用 [Bootstrap](http://getbootstrap.com/components/) 列出。 開啟 *< repository_root >*\src\MultiChannelToDo.Web\index.cshtml，並反白顯示的下列變更:

    ![](./media/app-service-agile-software-development/commit-1-changes.png)
    >[AZURE.NOTE] 如果您無法讀取上述的影像： 
    >
    >-在第 18 行，變更 `檢查清單` 至 `清單群組`。
    >-在第 19 行，變更 `類別 = 「 檢查清單的項目 」` 至 `類別 = 「 清單群組的項目 」`。
...
3.  儲存變更。 回到 Git Shell，並執行下列命令：
    ...
    這些 git 命令與另一個原始檔控制系統中的「簽入程式碼」類似 (例如 TFS)。 當您執行 `git push`, ，新的認可觸發程序的自動程式碼推送至 Azure，然後重建應用程式，以反映開發環境中的變更。
...
請記住，根據在 ProdandStage.json 中設定預備和生產環境的方式，新的程式碼會推送至 [預備]**** 位置並在該處執行。 因此，如果您導覽至預備位置的 URL，則會看到新的程式碼正在該處執行。 若要這樣做，請執行 `Show-azurewebsite` Git Shell 中的指令程式。
...
-   [敏捷式開發作法: 現代化的開發週期的秘訣與竅門](http://channel9.msdn.com/Events/Ignite/2015/BRK3707)
-   [使用資源管理員範本的 Azure Web 應用程式的進階的部署策略](http://channel9.msdn.com/Events/Build/2015/2-620)
...
-   [建立或編輯在 Azure AD 中的使用者](https://msdn.microsoft.com/library/azure/hh967632.aspx#BKMK_1)
