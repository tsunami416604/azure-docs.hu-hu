...
* 如何使用 Visual Studio **伺服器總管** 開啟遠端檔案，並啟動遠端偵錯工作階段。 
* 如何使用 [Azure 入口網站](/overview/preview-portal/) 來監視和管理 web 應用程式。

> [AZURE.NOTE] 本教學課程是關於 ASP.NET 中使用 Azure 應用程式服務。它不會教導如何開發 ASP.NET web 應用程式。 ASP.NET MVC 5 的簡介，請參閱 [開始使用 ASP.NET MVC 5](http://www.asp.net/mvc/overview/getting-started/introduction/getting-started) 上 [ASP.NET](http://asp.net/) 站台。 進入關於如何使用 Azure App Service 有深入的探討其他文件的連結，請參閱 [後續步驟](#next-steps) 一節。
> 
...
2. 從 **檔案** ] 功能表上，按一下 [ **新增 > 專案**。

3. 在 **新的專案** 對話方塊中，按一下 [ **C# > Web > ASP.NET Web 應用程式**。 您也可以選擇 **Visual Basic**。

3. 請確定 **.NET Framework 4.5.2** 選取作為目標架構。

4.  [Azure 的 Application Insights](app-insights-overview.md) 監視 web 應用程式的可用性、 效能和使用方式。 清除 **將 Application Insights 加入專案** 核取方塊，如果您不想要試試看吧。
...
5. 在 **新增 ASP.NET 專案** 對話方塊方塊中，保留下的設定 **Microsoft Azure** 保持不變，，然後按一下 [ **確定**。
...
5. 如果您尚未登入 Azure，Visual Studio 會提示您登入。 使用您用來管理 Azure 訂用帳戶之帳戶的識別碼和密碼來登入。
    ...
    當您登入時， **建立應用程式服務** 對話方塊會詢問您想要建立哪些的資源。

    ![已登入 Azure](./media/web-sites-dotnet-get-started/configuresitesettings.png)

3. 在 **建立應用程式服務** ] 對話方塊中，輸入 **Web 應用程式名稱** ，都是唯一的 *azurewebsites.net* 網域。 例如，您可以將它命名為 MyExample 並於右邊加上數字，使其成為唯一的名稱，例如 MyExample810。 如果預設的 Web 名稱針對您所建立，則它將是唯一的，且您可以使用該名稱。

    如果有其他人使用了您輸入的名稱，您就會在右邊看到紅色驚嘆號，而不是綠色勾號，這代表您需要輸入不同的名稱。

    Azure 將使用這個名稱做為應用程式 URL 的前置詞。 完整的 URL 將包含此名稱加上 *。 azurewebsites.net* (旁邊所示 **Web 應用程式名稱** 文字方塊)。 例如，若名稱為 `MyExample810`，URL 會是 `MyExample810.azurewebsites.net`。 URL 必須是唯一的。

4. 旁邊 **應用程式服務方案** 下拉式清單中，按一下 [ **新增...** ] 按鈕。

     [後續步驟](#next-steps) 一節含有 App Service 方案的相關資訊的連結。

5. 輸入 **MyExamplePlan**, ，或另一個名稱，如果您偏好的計劃名稱。

6. 在 **資源群組** 下拉式清單中，輸入新的資源群組的名稱。

     [後續步驟](#next-steps) 一節含有資源群組的相關資訊的連結。

5. 輸入 **MyExampleGroup**, ，或如果您偏好的資源群組名稱的另一個名稱。
...
1. 在 **方案總管] 中**, ，以滑鼠右鍵按一下專案，然後選擇 [ **發行**。
    ...
    在幾秒鐘， **發行 Web** 精靈] 隨即出現。 精靈會開啟至 *發行設定檔* ，將 web 專案部署到新的 web 應用程式已設定。 如果您想要部署到不同的 web 應用程式，您可以按一下 **設定檔** 索引標籤來建立不同的設定檔。 在本教學課程中，您將會接受部署到您稍早建立的 Web 應用程式的設定。

8. 在 **連接** ] 索引標籤的 **發行 Web** 精靈] 中，按一下 [ **下一步**。

    ![連線驗證成功](./media/web-sites-dotnet-get-started/GS13ValidateConnection.png)

10. 在 **設定** 索引標籤上，按一下 [ **下一步**。

    您可以接受預設值 **組態** 和 **檔案發行選項**。

    您可以使用 **組態** 下拉式清單來部署用於遠端偵錯的偵錯組建。  [後續步驟](#next-steps) 一節會說明如何從遠端在偵錯模式執行 Visual Studio 的教學課程的連結。

    ![[設定] 索引標籤](./media/web-sites-dotnet-get-started/GS13SettingsTab.png)

11. 在 **預覽** 索引標籤上，按一下 [ **發行**。

    如果您想要查看哪些檔案會複製到 Azure，您可以按一下 **開始預覽** 後，再按一下 **發行**。

    ![](./media/web-sites-dotnet-get-started/GS13previewoutput.png)

    當您按一下 **發行** Visual Studio 隨即開始進行將檔案複製至 Azure 伺服器的程序。

     **輸出** 和 **Azure App Service 活動** windows 顯示所執行的部署動作，並報告部署成功完成。
...
當您在測試和偵錯 web 應用程式，您可以快速暫時的變更，遠端網站上開啟並編輯中的檔案 **伺服器總管**。

1.  在 **伺服器總管**, ，瀏覽至 **Azure > App Service > MyExampleGroup**, ，然後展開 web 應用程式的節點。
...
在 **伺服器總管** ，您也可以以滑鼠右鍵按一下應用程式服務節點，並取得存取 web 應用程式設定，在 Visual Studio 視窗，請啟動遠端偵錯工作階段中，檢視應用程式即時記錄檔時，應用程式寫入它們。
...
2. 按一下 [ **應用程式服務**, ，然後按一下 [web 應用程式的名稱。

     **Web 應用程式** 刀鋒視窗會顯示設定和 web 應用程式的使用量統計資料的概觀。
...
3. 按一下 [ **設定** 查看更多選項來設定您的 web 應用程式。

    ![按一下 [設定]](./media/web-sites-dotnet-get-started/portaldashboard2.png)

    您會看到一份設定類型清單。

    ![](./media/web-sites-dotnet-get-started/portalconfigure1.png)

4. 按一下 [ **應用程式設定** 以查看您可以在入口網站中設定的設定種類的範例。
