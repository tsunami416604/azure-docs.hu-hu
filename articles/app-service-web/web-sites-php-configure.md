...
本指南將示範如何設定 Web 應用程式中的內建 PHP 執行階段 [Azure App Service](http://go.microsoft.com/fwlink/?LinkId=529714), 、 提供自訂 PHP 執行階段，以及啟用擴充功能。 若要使用應用程式服務，申請 [free trial]。 若要充分利用本指南，您應該先在 App Service 中建立 PHP Web 應用程式。

[AZURE.INCLUDE [app-service-web-to-api-and-mobile](../../includes/app-service-web-to-api-and-mobile.md)]

## 作法：變更內建 PHP 版本
當您建立 App Service Web 應用程式時，預設會安裝 PHP 5.4，而且可立即使用。 若要查看可用的修訂版、 其預設組態及啟用的擴充，最好是部署指令碼中呼叫 [phpinfo()] 函式。
...
3. 按一下 [ **儲存** 頂端的按鈕 **Web 應用程式設定** 刀鋒視窗。
...
若要使用 Azure 命令列介面，您必須擁有 **Node.js** 安裝在電腦上。
...
對於任一個內建的 PHP 執行階段，您可以遵循下列步驟，來變更任何組態選項。 (如需 php.ini 指示詞的詳細資訊，請參閱 [List of php.ini directives]。)
...
除了使用 `.user.ini` 檔案，您可以使用 [ini_set()] 設定不是系統層級指示詞的組態選項的指令碼中的函式。

### 變更 PHP\_INI\_SYSTEM 組態設定

1. 使用機碼 `PHP_INI_SCAN_DIR` 和值 `d:\home\site\ini` 將應用程式設定新增至 Web 應用程式
2. 建立 `settings.ini` 檔案使用 Kudo 主控台 (http:// & l t，則為站台名稱 & gt;。scm.azurewebsite.net) 在 `d:\home\site\ini` 目錄。
...
上一節所述，若要查看預設 PHP 版本、 其預設組態及啟用的擴充，最好是部署指令碼中呼叫 [phpinfo()]。 若要啟用擴充，請依照下列步驟執行：
...
3. 部署 Web 應用程式。
4. 瀏覽至您的 web 應用程式在 Azure 入口網站，然後按一下 **設定** ] 按鈕。

    ![Web 應用程式設定][settings-button]

5. 從 **設定** 分頁中，選取 **應用程式設定** ，並捲動至 **應用程式設定** 一節。
6. 在 **應用程式設定** 區段中，建立 **PHP_EXTENSIONS** 索引鍵。 此索引鍵的值是相對於網站根目錄的路徑: **bin\your 副檔名的檔案**。

    ![啟用應用程式設定中的擴充][php-extensions]

7. 按一下 [ **儲存** 頂端的按鈕 **Web 應用程式設定** 刀鋒視窗。
...
2. 為您的執行階段修改 `php.ini` 檔案。 請注意，Web Apps 將忽略僅系統層級指示詞的任何組態設定 (僅系統層級指示詞的詳細資訊，請參閱 [List of php.ini directives])。
...
7. 從 **設定** 分頁中，選取 **應用程式設定** ，並捲動至 **處理常式對應** 一節。 將 `*.php` 新增至 [副檔名] 欄位，然後將路徑加入 `php-cgi.exe` 可執行檔。 如果您將 PHP 執行階段放入應用程式根目錄內的 `bin` 目錄中，該路徑將是 `D:\home\site\wwwroot\bin\php\php-cgi.exe`。

    ![指定處理常式對應中的處理常式][handler-mappings]

8. 按一下 [ **儲存** 頂端的按鈕 **Web 應用程式設定** 刀鋒視窗。
