...
由於客戶通常會使用自訂 (或虛名) 網域存取應用程式，因此開發人員必須要有將應用程式要求分散到所有 App Service 環境執行個體的方法。  若要這麼做的好方法是解決使用 [Azure 流量管理員設定檔] 的自訂網域 [AzureTrafficManagerProfile]。  流量管理員設定檔可設定為指向所有個別的 App Service 環境。  流量管理員會根據流量管理員設定檔中的負載平衡設定，自動將客戶分散到所有的 App Service 環境間。  無論所有的 App Service 環境是部署在單一 Azure 區域，還是跨多個 Azure 區域部署於世界各地，此方法都可運作。
...
- **流量管理員網域:**  的網域名稱，就需要建立的 [Azure 流量管理員設定檔] 時選擇 [AzureTrafficManagerProfile]。  此名稱將會結合 *trafficmanager.net* 尾碼來登錄管理流量管理員網域項目。  範例應用程式中，選擇的名稱是 *可擴充 ase 示範*。  如此一來管理流量管理員的完整網域名稱是 *可擴充 ase demo.trafficmanager.net*。
...
最簡單的方法，以註冊多個 Azure 應用程式服務端點，在所有執行 **相同** Azure 區域，是預覽 Powershell [Azure 資源管理員 (ARM) 流量管理員支援] 與 [ARMTrafficManager]。  
...
註冊自訂網域與 Azure App Service 應用程式的概述，請參閱下列文件 [註冊自訂網域] [RegisterCustomDomain]。
...
## 其他連結和資訊 ##
預覽 Powershell [Azure 資源管理員 (ARM) 流量管理員支援] 上的文件 [ARMTrafficManager]。  
