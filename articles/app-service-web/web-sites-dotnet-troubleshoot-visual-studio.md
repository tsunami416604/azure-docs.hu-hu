...
1. 如果您尚未在 Visual Studio 中登入 Azure，按一下 [伺服器總管]**** 中的 [連線到 Azure]**** 按鈕。
    ...
    替代方式為安裝可讓您存取帳戶的管理憑證。 如果您選擇安裝憑證，請以滑鼠右鍵按一下 [伺服器總管]**** 中的 [Azure]**** 節點，然後按一下內容功能表中的 [管理訂閱]****。 在 [管理 Azure 訂閱]**** 對話方塊中，按一下 [憑證]**** 索引標籤，再按一下 [匯入]****。 請依照指示下載，然後匯入您 Azure 帳戶的訂用帳戶檔案 (亦稱為 *.publishsettings* 檔案)。
    > [AZURE.NOTE]
...
## <a name="remoteview"></a>在 [伺服器總管存取 web 應用程式檔案

您通常會部署 web 專案 `customErrors` 旗標設為 Web.config 檔案中的 `上` 或 `RemoteOnly`, ，這表示您不會收到有用的錯誤訊息時，發生錯誤。 對許多錯誤而言，您只會看到如下列之一的頁面。
...
3. 將下列字行新增至 `system.web` 元素：
    ...
    `< customErrors 模式 ="off">< / customErrors >`
...
## <a name="apptracelogs"></a>建立和檢視應用程式追蹤記錄
...
2. 開啟應用程式的 Web.config 檔案 (位於專案資料夾中的一個)，並新增 `< i >` 前面檔案結尾處的項目 `< / 組態 >` 項目:
    ...
    `WebPageTraceListener` 可讓您檢視追蹤輸出，瀏覽至 `/trace.axd`。

3. 新增 <a href="http://msdn.microsoft.com/library/vstudio/6915t83k(v=vs.100).aspx">追蹤項目</a> 下 `< system.web >` 在 Web.config 檔案中，如以下範例:

        <trace enabled="true" writeToDiagnosticsTrace="true" mostRecent="true" pageOutput="false" />

3. 按 CTRL+F5 執行應用程式。

4. 在瀏覽器視窗的網址列中，將 *trace.axd* 新增至 URL，然後按 Enter (URL 類似於 http://localhost:53370/trace.axd)。
...
5. 在 [應用程式追蹤]**** 頁面上，按一下第一行 (不是 BrowserLink 行) 上的 [檢視詳細資料]****。
    ...
    根據預設，`trace.axd` 只能在本機使用。 如果您想讓其可從遠端 web 應用程式，您可以將 `localOnly ="false"` 至 `追蹤` 中的項目 *Web.config* 檔案，如下列範例所示:

        <trace enabled="true" writeToDiagnosticsTrace="true" localOnly="false" mostRecent="true" pageOutput="false" />

    不過，啟用 `trace.axd` 在生產 web 應用程式通常不建議基於安全性理由，以及下列各節中，您會看到更簡易的方式讀取追蹤記錄中的 Azure web 應用程式。
...
2. 將 *.zip* 檔案解壓縮後，您會看到下列資料夾結構：
    ...
    * Web 伺服器記錄位於 *LogFiles\http\RawLogs* 資料夾的 *.log* 檔案中。您可以使用一種工具，例如 [Log Parser](http://www.microsoft.com/download/details.aspx?displaylang=en&id=24659) 檢視和管理這些檔案。
...
4. 按一下 [管理資料表儲存體]****。
    ...
    ![按一下 ](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-stgsettingsmgmtportal.png)
...
8. 在 Visual Studio 的 [Azure Web 應用程式]**** 視窗的 [記錄]**** 索引標籤中，按一下 [診斷摘要]**** 下方的 [重新整理]****。
    ...
    ![按一下 ](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-refreshstorage.png)
...
* [Azure web 應用程式線上工具，您應該了解](/blog/2014/03/28/windows-azure-websites-online-tools-you-should-know-about-2/)。 取自 Amit Apple 的部落格文章。
...
* [遠端偵錯 Azure App Service Web Apps 第 2 部分-內部遠端偵錯簡介](/blog/2014/05/07/introduction-to-remote-debugging-azure-web-sites-part-2-inside-remote-debugging/)
* [介紹遠端偵錯 Azure App Service Web Apps 第 3 部分-多重執行個體環境和 GIT](/blog/2014/05/08/introduction-to-remote-debugging-on-azure-web-sites-part-3-multi-instance-environment-and-git/)
...
* [使用 Web 應用程式記錄與診斷功能-與 David ebbo 一起](/documentation/videos/azure-web-site-logging-and-diagnostics/) 和 [串流記錄從 Web 應用程式-與 David ebbo 一起](/documentation/videos/log-streaming-with-azure-web-sites/)<br>
