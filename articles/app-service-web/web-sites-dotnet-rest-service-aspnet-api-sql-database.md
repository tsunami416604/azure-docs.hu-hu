...
本教學課程示範如何將 ASP.NET web 應用程式的部署 [Azure App Service](http://go.microsoft.com/fwlink/?LinkId=529714) 使用 Visual Studio 2013 或 Visual Studio 2013 Community Edition 中的發行 Web] 精靈。 
...
3. 在 **新的專案** 對話方塊方塊中，展開 **Visual C#** ，然後選取 **Web**  ，然後選取 **ASP.NET Web 應用程式**。 將應用程式 **ContactManager** 按一下 **確定**。

    ![[新增專案] 對話方塊](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr4.png)

1. 在 **新增 ASP.NET 專案** 對話方塊中，選取 **MVC** 範本，勾選 **Web API** 然後按一下 [ **變更驗證**。
...
1. 在 **變更驗證** 對話方塊中，按一下 [ **不需要驗證**, ，然後按一下 [ **確定**。
    ...
    您要建立的範例應用程式將不會有需要使用者登入的功能。 如需如何實作驗證與授權功能的資訊，請參閱 [下一步](#nextsteps) 本教學課程的最後一節。 

1. 在 **新增 ASP.NET 專案** 對話方塊方塊中，請確定 **定域機組中的主機** 已勾選，按 **確定**。

    ![[New ASP.NET Project] 對話方塊](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rt3.png)

如果您先前未登入 Azure，系統將提示您登入。

1. 「 組態精靈 」 將會建議唯一名稱，根據 *ContactManager* (請參閱下圖)。 選取您附近的區域。 您可以使用 [azurespeed.com](http://www.azurespeed.com/ "AzureSpeed.com") 尋找最低延遲的資料中心。 
2. 如果您之前尚未建立資料庫伺服器，請選取 **建立新的伺服器**, ，輸入資料庫使用者名稱和密碼。
...
1. 在 **方案總管] 中**, ，依序展開 *Views\Shared* 資料夾，然後開啟 *_Layout.cshtml* 檔案。

    ![方案總管] 中的 _Layout.cshtml][] newapp004

1. 內容取代 *Views\Shared_Layout.cshtml* 以下列程式碼檔案:
...
以上的標記變更為"Contact Manager"，"My ASP.NET App"的應用程式名稱，並移除連結，以 **首頁**, ，**有關** 和 **連絡人**。

### 在本機執行應用程式

1. 按 CTRL+F5 執行應用程式。
應用程式首頁隨即出現在預設瀏覽器中。
    ![待辦事項清單首頁](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr5.png)
...
Visual Studio 隨即開始進行將檔案複製至 Azure 伺服器的程序。  **輸出** 視窗會顯示所執行的部署動作，並報告部署成功完成。

14. 預設瀏覽器會自動開啟已部署之網站的 URL。

    您建立的應用程式現在正在雲端中執行。
    
    ![若要在 Azure 中執行待辦事項清單首頁][] rxz2
 ...
 **連絡** 類別會定義每個連絡人，以及主索引鍵 ContactID 所需的資料庫來儲存資料。 您可以取得資料模型的詳細資訊 [下一步](#nextsteps) 本教學課程的最後一節。
...
1. 在 **方案總管] 中**, ，展開 Controllers 資料夾。

3. 建置專案 **(Ctrl + Shift + B)**。 (使用樣板機制前必須先建置專案。) 

4. 以滑鼠右鍵按一下 Controllers 資料夾，然後按一下 [ **新增**, ，然後按一下 [ **控制器**。

    ![在 Controllers 資料夾內容功能表中新增控制器][] addcode001

1. 在 **Add Scaffold** 對話方塊中，選取 **MVC 控制器具有檢視、 使用 Entity Framework** 按一下 **新增**。

 ![新增控制器](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rrAC.png)

6. 將控制器名稱設定為 **HomeController**。 選取 **連絡人** 作為模型類別。 按一下 [ **新的資料內容** 按鈕，接受預設值"ContactManager.Models.ContactManagerContext" **新資料內容類型**。 按一下 [ **新增**。
    ...
    Visual Studio 會建立控制器方法與檢視的 CRUD 資料庫操作，如 **連絡人** 物件。

## 啟用移轉、建立資料庫、新增範例資料和資料初始設定式 ##

下一個工作是啟用 [Code First 移轉](http://curah.microsoft.com/55220) 您建立資料模型為基礎的功能，以便建立資料庫。

1. 在 **工具** 功能表上，選取 **程式庫套件管理員** 然後 **Package Manager Console**。

    ![工具] 功能表中的封裝管理員主控台][] addcode008

2. 在 **Package Manager Console** ] 視窗中，輸入下列命令:

        enable-migrations 
  
     **啟用移轉** 命令會建立 *移轉* 資料夾，並放入該資料夾中 *Configuration.cs* 來設定 [移轉，您可以編輯的檔案。 

2. 在 **Package Manager Console** ] 視窗中，輸入下列命令:

        add-migration Initial

     **Initial** 命令會產生名為的類別 **& lt; date_stamp & gt; 初始** 可建立資料庫。 第一個參數 ( *初始* ) 是任意的用於建立檔案的名稱。 您可以看到新的類別檔案中 **方案總管] 中**。
...
5. 取代 *種子* 方法取代下列程式碼:
    ...
    以上的這個程式碼會以連絡人資訊初始化資料庫。 如需植入資料庫的詳細資訊，請參閱 [偵錯 Entity Framework (EF) Db](http://blogs.msdn.com/b/rickandy/archive/2013/02/12/seeding-and-debugging-entity-framework-ef-dbs.aspx)。


1. 在 **Package Manager Console** 輸入命令:
...
1. 開啟 *Views\Home\Index.cshtml* 檔案。 在下一個步驟中，我們將以使用的程式碼取代產生的標記 [jQuery](http://jquery.com/) 和 [Knockout.js](http://knockoutjs.com/)。 這個新的程式碼會使用 Web API 和 JSON 來擷取連絡人清單，然後再使用 knockout.js 使連絡人資料與 UI 繫結。 如需詳細資訊，請參閱 [下一步](#nextsteps) 本教學課程的最後一節。 
...
3. 以滑鼠右鍵按一下 [內容] 資料夾，然後按一下 [ **新增**, ，然後按一下 [ **新項目**。

    ![新增內容的資料夾內容功能表中的樣式表][] addcode005

4. 在 **加入新項目** ] 對話方塊中，輸入 **樣式** 右上角的搜尋方塊，然後選取 **樣式表**。
    ![加入新項目對話方塊][] rxStyle

5. 將檔案命名 *Contacts.css* 按一下 **新增**。 以下列程式碼取代檔案的內容。
...
7. 加入下列程式碼以註冊 [Knockout](http://knockoutjs.com/index.html "KO") 外掛程式。

        bundles.Add(new ScriptBundle("~/bundles/knockout").Include(
                    "~/Scripts/knockout-{version}.js"));
    此範例使用 knockout 來簡化處理螢幕範本的動態 JavaScript 程式碼。

8. 修改內容/css 項目以註冊 *contacts.css* 樣式表。 變更以下文字行：
...
1. 在 **方案總管] 中**, ，控制站上按一下滑鼠右鍵，然後按一下 **新增** 然後 **控制器...** 

1. 在 **Add Scaffold** ] 對話方塊中，輸入 **Web API 2 控制器與動作，使用 Entity Framework** 然後按一下 [ **新增**。

    ![新增 API 控制器](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rt1.png)

4. 在 **加入控制器** ] 對話方塊中，輸入"ContactsController"作為控制器名稱。 選取"Contact (ContactManager.Models)" **模型類別**。  保留預設值為 **資料內容類別**。 
...
2. 輸入連絡人，然後按一下 [ **新增**。 應用程式會返回首頁並顯示您輸入的連絡人。
    ...
    ![待辦事項清單項目索引頁][] addwebapi004

3. 在瀏覽器中，附加 **/api/contacts** 的 url。

    產生的 URL 將類似 http://localhost:1234/api/contacts。 您新增之符合 REST 限制的 Web API 會傳回儲存的連絡人。 Firefox 和 Chrome 會顯示 XML 格式的資料。

    ![待辦事項清單項目索引頁][] rxFFchrome
...
1. 在 **方案總管] 中**, ，以滑鼠右鍵 **ContactManager** 專案，然後按一下 **新增** 然後按一下 [ **類別**。

2. 將檔案命名 *ValidateHttpAntiForgeryTokenAttribute.cs* 並新增下列程式碼:
...
1. 新增下列 *使用* 陳述式至連絡人控制器，使您得以存取 **[ValidateHttpAntiForgeryToken]** 屬性。

        using ContactManager.Filters;

1. 新增 **[ValidateHttpAntiForgeryToken]** 屬性的 Post 方法 **ContactsController** 來保護它免於遭受 XSRF 威脅的攻擊。 您需要將其新增至 PutContact"、"PostContact"及 **DeleteContact** 動作方法。

        [ValidateHttpAntiForgeryToken]
            public IHttpActionResult PutContact(int id, Contact contact)
            {

1. 更新 *指令碼* 區段 *Views\Home\Index.cshtml* 包含取得 XSRF 權杖的程式碼檔案。
...
1. 在 **Contactsmanagercontext**, ，按一下 [ **v** 圖示以變更 *遠端連接字串* 為連絡人資料庫的連接字串。 按一下 [ **ContactDB**。

    ![設定](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rt5.png)

7. 核取方塊， **Execute Code First Migrations (在應用程式啟動時執行)**。

1. 按一下 [ **下一步** 然後按一下 [ **預覽**。 Visual Studio 會顯示即將新增或更新的檔案清單。
...
部署完成後，瀏覽器會開啟應用程式的首頁。
    ...
    Visual Studio 發行程序會自動設定的連接字串中已部署 *Web.config* 指向 SQL 資料庫的檔案。 它也設定了 Code First 移轉，使其在應用程式於部署完成後首次存取資料庫時，將資料庫自動升級為最新版本。

    由於此組態中，第一個程式碼建立資料庫中執行程式碼 **初始** 您稍早建立的類別。 它會在應用程式於部署完成後首次嘗試存取資料庫時執行這項作業。
...
真實的應用程式會要求驗證和授權，而您需要使用成員資格資料庫來達成目的。 本教學課程 [部署含有 OAuth、 成員資格及 SQL Database 的安全 ASP.NET MVC 應用程式](web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database.md) 本教學課程為基礎，並示範如何部署 web 應用程式與成員資格資料庫。

另一個儲存 Azure 應用程式資料的方法是使用 Azure 儲存體，它能以 Blob 和資料表的形式提供非關聯式的資料儲存。 以下連結提供 Web API、ASP.NET MVC 及 Window Azure 的詳細資訊。
 

* [開始使用 MVC 的 Entity framework][] EFCodeFirstMVCTutorial
...
* [在 Visual Studio 中疑難排解 Azure 網站](web-sites-dotnet-troubleshoot-visual-studio.md)

本教學課程和範例應用程式所編寫的 [Rick Anderson](http://blogs.msdn.com/b/rickandy/) (Twitter [@RickAndMSFT](https://twitter.com/RickAndMSFT)) 協助 Tom Dykstra 和 Barry Dorrans (Twitter [@blowdart](https://twitter.com/blowdart))。 
