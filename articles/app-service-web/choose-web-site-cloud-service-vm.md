...
Azure 提供多種方式來裝載網站: [Azure 應用程式服務]][], ，[雲端服務 []][], ，和 [[虛擬機器]][]。 本文協助您了解這些選項，為您的 Web 應用程式做出正確的選擇。
...
- [我需要具有背景處理和資料庫後端以執行與內部部署資源整合的商業應用程式的 web 前端。](#onprem)
- [我需要可靠的方式來裝載可靈活公司網站，且可供全球存取。](#corp)
- [我有 Windows Server 2003 上執行的 IIS6 應用程式。](#iis6)
- [我是小型企業業主，而且我需要經濟的方式來裝載我的網站，但未來成長的。](#smallbusiness)
- [我是網頁或平面設計師，並想要設計及建立我的客戶網站。](#designer)
- [我想將含 web 前端多層式應用程式移轉至雲端。](#multitier)
- [我的應用程式需要在高度自訂的 Windows 或 Linux 環境，我想要將它移至雲端。](#custom)
- [我的網站使用開放原始碼軟體，我想要裝載在 Azure 中。](#oss)
- [我有特定業務應用程式需要連線到公司網路。](#lob)
- [我想要裝載 REST API 或 web 服務供行動用戶端。](#mobile)


### <a id="onprem"></a> 我需要具有背景處理和資料庫後端以執行與內部部署資源整合的商業應用程式的 web 前端。
...
### <a id="corp"></a> 我需要可靠的方式來裝載可靈活公司網站，且可供全球存取。
...
### <a id="iis6"></a> 我有 Windows Server 2003 上執行的 IIS6 應用程式。

Azure App Service 可讓您輕鬆地省去移轉舊版 IIS6 應用程式時相關的基礎結構成本。 Microsoft 已建立 [簡單易用的移轉工具和詳細的移轉指引](https://www.movemetowebsites.net/) ，可讓您檢查相容性和識別任何需要進行的變更。 與 Visual Studio、TFS 和一般 CMS 工具整合，讓您輕鬆地將 IIS6 應用程式直接部署到雲端。 部署之後，Azure 入口網站提供健全的管理工具，可讓您依需要而縮小規模來管理成本，或擴大規模來符合需求。 移轉工具可讓您：

* 快速又輕鬆地將舊式的 Windows Server 2003 Web 應用程式移轉至雲端。
* 選擇將連接的 SQL 資料庫留在內部部署，以建立混合式應用程式。
* 自動隨著舊式應用程式一起移動 SQL 資料庫。

### <a id="smallbusiness"></a>我是小型企業業主，而且我需要經濟的方式來裝載我的網站，但未來成長的。
...
### <a id="designer"></a> 我是網頁或平面設計師，而我想要設計及建立網站，為我的客戶
...
- 使用命令列工具執行 [自動化工作的 ][scripting]。
- Work with popular languages such as [.Net][dotnet], [PHP[]][], [Node.js][nodejs], and [Python[]][].
- 有三種不同的調整層級可選，以向上調整至非常高的容量。
- 整合與其他 Azure 服務，例如 [SQL 資料庫 ][sqldatabase], ，[服務匯流排 ][servicebus] 和 [儲存體 []][], ，或合作夥伴供應項目從 [Azure 存放區 ][azurestore], ，例如 MySQL 和 MongoDB。
- 與 Visual Studio、Git、WebMatrix、WebDeploy、TFS 和 FTP 等工具整合。

### <a id="multitier"></a>我想將含 web 前端多層式應用程式移轉至雲端
...
### <a id="custom"></a>我的應用程式需要在高度自訂的 Windows 或 Linux 環境，我想要將它移至雲端。
...
### <a id="oss"></a>我的網站使用開放原始碼軟體，我想要裝載在 Azure 中
...
### <a id="lob"></a>我有特定業務應用程式需要連線到公司網路
...
### <a id="mobile"></a>我想要裝載 REST API 或 web 服務供行動用戶端
...
下表比較 App Service、「雲端服務」與「虛擬機器」的功能，以協助您做出最佳選擇。 目前的 SLA，每個選項的詳細資訊，請參閱 [Azure 服務等級協定](/support/legal/sla/)。
...
---|---|---|---|---
 ...
 裝載多層式架構的中間層| X| X| X| 應用程式服務 web 應用程式可以輕易裝載 REST API 中間層，而 [WebJobs](http://go.microsoft.com/fwlink/?linkid=390226) 功能可以裝載背景處理工作。您可以在專用網站中執行 WebJobs，以實現此層的獨立擴充性。預覽 [API 應用程式](../app-service-api/app-service-api-apps-why-best-platform.md) 功能提供更多裝載 REST 服務的功能。
 ...
 支援 SSL| X| X| X| 在 App Service Web 應用程式中，只有基本和標準模式才支援自訂網域名稱的 SSL。使用 SSL 的 web 應用程式的相關資訊，請參閱 [設定 Azure 網站的 SSL 憑證](../app-service-web/web-sites-configure-ssl-certificate.md)。
 整合 Visual Studio| X| X| X|
 遠端偵錯| X| X| X|
 利用 TFS 來部署程式碼| X| X| X|
 網路隔離與 [Azure 虛擬網路](/services/virtual-network/)| X| X| X| 另請參閱 [Azure 網站虛擬網路整合](/blog/2014/09/15/azure-websites-virtual-network-integration/)
...
> [AZURE.NOTE]
> 如果您想要註冊帳戶前開始使用 Azure App Service，請移至 <a href="https://trywebsites.azurewebsites.net/">https://trywebsites.azurewebsites.net</a>, ，您可以立即建立短期入門應用程式在 Azure App Service 中免費。 不需要信用卡，沒有承諾。
