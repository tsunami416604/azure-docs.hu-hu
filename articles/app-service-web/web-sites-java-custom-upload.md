...
- 要求目錄具備寫入權限的應用程式必須位於 Azure 的 web 應用程式的內容目錄，也就是 **D:\home**。 環境變數 `首頁` 係指 D:\home。
...
> [AZURE.NOTE] `%Http_platform_port%` 是個特殊預留位置，必須將其指定為屬於 **引數** 或做為一部分 **httpPlatform** **environmentVariables** 清單。 這將會被透過 **HttpPlatformHandler** 內部產生的連接埠取代，以便 **processPath** 所指定的程序可以接聽此連接埠。
...
App Service Web Apps 不支援 Direct3d 呼叫。 若要停用，新增下列 Java 選項，您的應用程式應該進行下列呼叫: `-Dsun.java2d.d3d=false`
...
- 關機連接埠變為 -1。
- 將 HTTP 連接器變更為 
        `< 連接器 port.http}"通訊協定 ="HTTP/1.1"connectionTimeout ="600000"address ="127.0.0.1"URIEncoding ="utf-8"/ >`
- 註解化 AJP 連接器。
