...
若要設定 Memcache 填充碼，您必須建立三個應用程式設定。 這可以使用各種不同的方法包括 [Azure 入口網站](http://go.microsoft.com/fwlink/?LinkId=529715), 、 [傳統入口網站][3], 、 [Azure PowerShell Cmdlet][5] 或 [Azure 命令列介面][5]。 基於這篇文章的目的，我要使用 [Azure 入口網站][4] 若要設定的應用程式。 下列的值可以擷取自 **設定** Redis 快取執行個體的分頁。
...
您必須建立第一個應用程式設定是 **REDIS\_HOST** 應用程式設定。 此設定會設定目的地，供填充碼轉送快取資訊至此目的地。 可以從擷取 REDIS_HOST 應用程式設定所需要的值 **屬性** Redis 快取執行個體的分頁。
...
您必須建立第二個應用程式設定是 **REDIS\_KEY** 應用程式設定。 此設定提供安全存取 Redis Cache 執行個體所需的驗證權杖。 您可以擷取從 REDIS_KEY 應用程式設定所需的值 **存取金鑰** 刀鋒視窗中的 Redis 快取執行個體。
...
瀏覽至 [PECL][6]。 在快取類別底下按一下 [ [memcache][7]。 在下載欄下按一下 DLL 連結。
...
> [AZURE.NOTE] 如果 web 應用程式需要載入多個 PHP 延伸模組，PHP_EXTENSIONS 的值應該是 DLL 檔案相對路徑以逗號分隔清單。
...
>[AZURE.NOTE] 遵循此部落格 [如何啟用 Web 應用程式中的網站擴充功能][8] 安裝 Visual Studio Team Services。
...
下一個步驟是啟用 object-cache 外掛程式。 這是藉由拖放 **object-cache.php** 從 **wp-content/memcached** 資料夾 **wp 內容** 資料夾即可啟用 Memcache 物件快取功能。
...
既然 **object-cache.php** 檔案位於 **wp 內容** 現在已啟用 Memcached 物件快取的資料夾。
...
>[AZURE.NOTE] 在撰寫本文時，Redis CLI 不支援 SSL 連線，因此下列步驟所需。

在 Azure 入口網站中，瀏覽至您為此 Web 應用程式建立的 Redis Cache 執行個體。 快取的刀鋒視窗開啟後，按一下 [ **設定** 圖示。
...
>[AZURE.NOTE] 這個步驟假設 redis 安裝在本機開發機器上。 [在本機上使用下列指示安裝 Redis][9]。
...
取代 **< 主機名稱的-redis-快取 >** 實際的 xxxxx.redis.cache.windows.net 主機名稱和 **< 主索引鍵-如-redis-快取 >** 快取的存取金鑰，然後按 **Enter**。 CLI 連線至 Redis Cache 執行個體後，發出任何 redis 命令。 在下列螢幕擷取畫面中，我選擇列示索引鍵。
