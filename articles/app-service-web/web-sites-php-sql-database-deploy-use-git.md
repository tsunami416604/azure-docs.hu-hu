...
本教學課程會示範如何建立 PHP web 應用程式中的 [Azure App Service](http://go.microsoft.com/fwlink/?LinkId=529714) 以連接到 Azure SQL Database 和如何使用 Git 來部署它。 本教學課程假設您有 [PHP ][install-php], ，[SQL Server Express 的 ][install-sqlexpress], 、 [Microsoft Drivers for SQL Server for PHP](http://www.microsoft.com/download/en/details.aspx?id=20098), ，和 [Git ][install-git] 安裝在電腦上。 完成本指南的步驟後，您將擁有在 Azure 上運作的 PHP-SQL Web 應用程式。
> [AZURE.NOTE]
...
3. 從 **PHP** 區段的 [結果] 對話方塊中，請記下的值 `伺服器`, ，`SQL Database`, ，和 `使用者名稱`。 稍後將 PHP Web 應用程式發行至 Azure App Service 時，您將使用這些值。
...
若要在本機執行應用程式，請遵循下列步驟。 請注意，這些步驟假設您具有 PHP 和 SQL Server Express 在本機電腦上設定，且您已啟用 [[pdo sqlsrv] 的 SQL Server 的 PDO 延伸功能][pdo-sqlsrv]。
...
2. 在應用程式根目錄，會建立兩個檔案，一個稱為 `createtable.php` ，另一個名 `index.php`。
...
3. 在文字編輯器或 IDE 中開啟 `createtable.php` 檔案，加入下列程式碼。 此程式碼將會在 `registration` 資料庫中建立 `registration_tbl`。
    ...
    請注意，您必須更新的值 <code>$user</code> 和 <code>$pwd</code> 與您的本機 SQL Server 使用者名稱和密碼。
...
當您在本機完成應用程式測試之後，就可以使用 Git 將其發行至 App Service Web Apps。 不過，您必須先更新應用程式中的資料庫連接資訊。 使用您稍早取得的資料庫連接資訊 (在＜**取得 SQL Database 連線資訊**＞一節中），將檔案 **createdatabase.php** 和 `index.php` `兩者`的資訊都更新為適當的值：
...
> [AZURE.NOTE]
> 在 <code>$host</code>, ，Server 值的前面必須加 <code>tcp:</code>。
...
1. 開啟 GitBash (或終端機，如果 Git 位於您 `路徑`)，將目錄變更為您的應用程式的根目錄 ( **註冊** 目錄)，然後執行下列命令:
