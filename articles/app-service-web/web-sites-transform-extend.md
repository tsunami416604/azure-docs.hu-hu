...
使用 [XML 文件轉換](http://msdn.microsoft.com/library/dd465326.aspx) 您可以將轉換 (XDT) 宣告 [ApplicationHost.config](http://www.iis.net/learn/get-started/planning-your-iis-architecture/introduction-to-applicationhostconfig) Azure App Service 中 web 應用程式中的檔案。 您也可以使用 XDT 宣告來新增私用擴充功能，以啟用自訂的 Web 應用程式系統管理動作。 本文包含一個 PHP Manager Web 應用程式擴充功能範例，該範例會透過 Web 介面來啟用 PHP 設定的管理功能。

[AZURE.INCLUDE [app-service-web-to-api-and-mobile](../../includes/app-service-web-to-api-and-mobile.md)]

## <a id="transform"></a>透過 ApplicationHost.config 的進階的設定
...
若要利用此轉換功能，您使用 XDT 內容建立 ApplicationHost.xdt 檔案並置於網站根目錄 (d:\home\site) 之下，在 [Kudu 主控台](https://github.com/projectkudu/kudu/wiki/Kudu-console)。 您可能需要重新啟動 Web 應用程式，變更才會生效。
...
3. Web 應用程式擴充功能 **部署**: 將內容放在下方的 SiteExtensions 資料夾 `根`

Web 應用程式的內部連結應指向在 ApplicationHost.xdt 檔案中指定之應用程式路徑的相對路徑。 對 ApplicationHost.xdt 檔案所做的任何變更都需要回收 Web 應用程式。

**注意**: 這些主要元素的其他資訊將會位於 [https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions](https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions)。
...
Web 應用程式擴充功能中值得注意的一點與內部連結的處理有關。 如果 HTML 檔案中有任何連結可提供 Web 應用程式上內部連結的絕對路徑，您就必須確定那些連結的前面都已加上您的擴充功能名稱作為根目錄。 這需要因為您的擴充功能的根目錄現在是"/`[您的擴充功能-name]`/"而只是"/"，所以所有內部連結必須隨之更新。 例如，假設程式碼包含下列項目的連結：

`"< href ="/ Home/設定"> PHP 設定 </a >"`

當連結是 Web 應用程式擴充功能的一部分時，連結的格式必須如下：

`"< href ="[您站台名稱] / Home/設定"> 設定 </a >"`

您可以透過下列方式解決這項需求只使用相對路徑 web 應用程式中或在 ASP.NET 應用程式的情況下使用 `@Html.ActionLink` 方法為您建立適當的連結。

#### <a id="XDT"></a> ApplicationHost.xdt 檔案

Web 應用程式擴充功能的程式碼會在 %home%\siteextensions\[your-extension-name]。 我們將此稱為擴充功能根目錄。
...
若要安裝 web 應用程式擴充功能，您可以使用 FTP 將您的 web 應用程式的所有檔案都複製 `\SiteExtensions\[your-extension-name]` 您要安裝擴充功能的 web 應用程式的資料夾。 請務必同時將 ApplicationHost.xdt 檔案複製到此位置。 重新啟動 Web 應用程式以啟用擴充功能。
...
新增應用程式設定索引鍵開發和調查期間停用 (所有私人非預先安裝) 擴充功能的 web 應用程式可以 `WEBSITE_PRIVATE_EXTENSIONS` 且值為 `0`。
