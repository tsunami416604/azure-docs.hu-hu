...
- .NET 應用程式，這些設定會插入至.NET 組態的 `AppSettings` 在執行階段，覆寫現有的設定。
...
.NET 應用程式，這些連接字串會插入.NET 組態 `connectionStrings` 設定在執行階段，其中索引鍵等於所連結的資料庫名稱的現有項目。
...
例如，如果 MySql 連接字串命名為 `connectionstring1`, ，則可透過環境變數存取 `MYSQLCONNSTR_connectionString1`。
...
- **指令碼處理器路徑**。 指令碼處理器的絕對路徑。 符合該副檔名的檔案要求，將由指令碼處理器來處理。 使用路徑 `D:\home\site\wwwroot` 來參考您的應用程式根目錄。
- **其他引數**。 指令碼處理器選用的命令列引數
...
- 設定連續部署。 請參閱 [使用 Git 來部署 Web 應用程式在 Azure App Service 中](web-sites-publish-source-control.md)
...
- [在 Azure App Service 中調整 web 應用程式](web-sites-scale.md)
- [監視 Azure App Service 中的 Web 應用程式的基本概念](web-sites-monitor.md)
