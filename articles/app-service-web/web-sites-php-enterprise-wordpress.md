...
* **MySQL 資料庫** -可透過 [Azure Marketplace 中的 ClearDB][cdbnstore], ，或者您也可以管理自己的 MySQL 安裝 Azure 虛擬機器上使用 [Windows][mysqlwindows] 或 [Linux][mysqllinux]。

    > [AZURE.NOTE] ClearDB 提供數個 MySQL 組態，每個設定的不同效能特性。 請參閱 [Azure 市集][cdbnstore] 取得透過 Azure 市集所提供的產品相關資訊或 [ClearDB 定價](http://www.cleardb.com/pricing.view) 為直接來自 ClearDB 的供應項目。

* **PHP 5.2.4 或更高** -Azure App Service 目前提供 [PHP 版本 5.4、 5.5 和 5.6][phpwebsite]。

    > [AZURE.NOTE] 我們建議您務必在最新版的 PHP，才能確保您擁有最新的安全性修正程式上執行。
...
作法... | 目的...
------------------------|-----------
**負載平衡網站** 或 **地理-發佈網站** | [使用 Azure 流量管理員路由傳送流量][trafficmanager]
**備份與還原** | [備份 Azure App Service 中的 web 應用程式][backup] 和 [還原 Azure App Service 中的 web 應用程式][restore]
...
**快取資源** | [Redis 快取][rediscache], ，[Memcache Cloud](https://azure.microsoft.com/marketplace/partners/garantiadata/memcached/), ，[MemCachier](https://azure.microsoft.com/marketplace/partners/memcachier/memcachier/), ，或其他快取提供項目中的其中一個 [Azure 存放區](/gallery/store/)
**調整應用程式** | [在 Azure App Service 中調整 web 應用程式][websitescale] 和 [ClearDB 高可用性路由][cleardbscale]。 如果您選擇主控與管理您自己的 MySQL 安裝，您應該考慮 [MySQL 叢集 CGE][cge] 向外延展
...
* **[WordPress 匯出][export]** -這將匯出您的部落格，然後在 Azure App Service 使用的新 WordPress 網站匯入內容 [WordPress importer 外掛程式][import]。
    ...
    > [AZURE.NOTE] 此程序可讓您移轉內容，它無法移轉任何外掛程式、 主題或其他自訂。 您必須手動重新安裝這些項目。
...
2. 請依照 [Azure App Service 中建立 WordPress web 應用程式][createwordpress] 來建立新的 WordPress web 應用程式。 在建立 web 應用程式，選取 **使用現有的 MySQL 資料庫** ，然後選取在步驟 1 中建立的資料庫。
...
* **匯出和匯入** -適用於沒有許多自訂的網站或只想要移動內容。

* **備份與還原** -適用於您要移動所有項目具有許多自訂網站。
...
3. 登入 WordPress 網站上 Web 應用程式，然後按一下 [ **外掛程式** ]-> [ **新增**。 搜尋與安裝時， **WordPress Importer** 外掛程式。

4. 已安裝匯入工具外掛程式後，請按一下 **工具** ]-> [ **匯入**, ，然後選取 **WordPress** 以使用 WordPress 匯入工具外掛程式。

5. 在 **Import WordPress** 頁面上，按一下 **選擇檔案**。 瀏覽至從現有的 WordPress 網站匯出的 WXR 檔案，然後選擇 [ **上傳的檔案和匯入**。
...
如果使用... | 執行此動作...
------------------ | ----------
**Permalinks** | 從新網站的 WordPress 儀表板，按一下 [ **設定** ]-> [ **Permalinks** ，然後更新固定連結結構
**映像/媒體連結** | 若要更新連結至新位置，請使用 [Velvet Blues Update URLs 外掛程式][velvet], 、 搜尋和取代的工具，或手動在資料庫中
**主題** | 移至 **外觀** ]-> [ **佈景主題** 並視需要更新網站佈景主題
**功能表** | 如果您的主題支援功能表，前往首頁的連結可能仍然內嵌舊的子目錄。 移至 **外觀** ]-> [ **功能表** 並且更新清單
...
4. 在 Azure 入口網站中建立新的 Web 應用程式，並發佈 WordPress 備份。
    ...
    2. 在 WordPress 備份中，找出 **wp-config.php** 檔案，並在編輯器中開啟它。 使用新 MySQL 資料庫的資訊來取代下列項目。
        ...
        * **DB_PASSWORD** -使用者密碼
        ...
        在之後變更這些項目，儲存並關閉 **wp-config.php** 檔案。
...
**Enable automated backups** | [在 Azure App Service 中備份 Web 應用程式][backup]
...
* [在 Azure App Service 中將 WordPress 轉換成多網站](web-sites-php-convert-wordpress-multisite.md)

* [適用於 Azure 的 ClearDB 升級精靈](http://www.cleardb.com/store/azure/upgrade)

* [將 WordPress 裝載到 Azure 應用程式服務的 Web 應用程式的](http://blogs.msdn.com/b/webapps/archive/2013/02/13/hosting-wordpress-in-a-subfolder-of-your-windows-azure-web-site.aspx)
...
* [如何在 Azure 應用程式服務上移轉與執行 WordPress 部落格](http://www.kefalidis.me/2012/06/how-to-migrate-and-run-your-wordpress-blog-on-windows-azure-websites/)

* [如何在 Azure 應用程式服務上免費執行 WordPress](http://architects.dzone.com/articles/how-run-wordpress-azure)
...
* [如何在 Azure 帳戶中設定 WordPress Web 應用程式](http://www.itexperience.net/2014/01/20/how-to-set-up-a-wordpress-website-in-your-windows-azure-account/)
