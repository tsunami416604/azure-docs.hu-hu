...
本逐步解說會示範如何建立 Azure SDK for Java 應用程式中的 Web 應用程式會建立 [Azure 應用程式服務]][], ，然後應用程式部署到它。 其中包括兩個部分：
...
在本文中的 AzureWebDemo 應用程式程式碼以 Azure Java SDK 0.7.0，您可以使用安裝 [[Web Platform Installer]][] (WebPI)。 此外，務必使用最新版的 [Azure Toolkit for Eclipse]][]。 安裝 SDK 之後，在 **Maven 儲存機制**中執行**更新索引**以在 Eclipse 專案中更新相依性，然後在 [**相依性**] 視窗中重新新增各封裝的最新版本。 按一下 **[說明] > [安裝詳細資料]**，可以驗證 Eclipse 中安裝的軟體版本；您至少要有下列版本：
...
如果您還沒有 Active Directory (AD) 在您的 Azure 訂用帳戶，登入 [Azure 傳統入口網站 []][] 與您的 Microsoft 帳戶。 如果您有多個訂用帳戶，請按一下 [**訂用帳戶**] 並針對您要用於此專案的訂用帳戶選取預設目錄。 接著按一下 [**套用**] 切換至該訂用帳戶檢視。
...
4. 在 [**網域**] 中，輸入網域名稱。這是與您目錄中，預設會包含基本網域名稱其形式 `_ < n a m >。 onmicrosoft.com`。您可以根據此目錄名稱或您擁有的其他網域名稱予以命名。之後，您可以新增貴組織已使用的其他網域名稱。
...
當您完成這個程序時，CER 憑證會位於 Azure 訂用帳戶，而 JKS 憑證會位於本機磁碟機。 如需有關管理憑證的詳細資訊，請參閱 [建立並上傳管理憑證 ][]。
...
> **附註:**  執行此命令的電腦必須已安裝 JDK。 此外，keytool 的路徑取決於您安裝 JDK 的位置。 如需詳細資訊，請參閱 [金鑰和憑證管理工具 (keytool) []][] Java 線上文件中。
...
- `< java-安裝目錄的 >` 是您安裝 Java 的目錄的路徑。
- `< 金鑰存放區識別碼 >` 是 keystore 項目識別碼 (例如， `AzureRemoteAccess`)。
- `< 憑證的存放區-dir >` 是您要儲存憑證的目錄的路徑 (例如 `c: / 憑證`)。
- `< 憑證檔案的名稱 >` 是憑證檔案的名稱 (例如 `AzureWebDemoCert`)。
- `< 密碼 >` 的密碼您選擇要保護的憑證; 它必須至少 6 個字元。您可以不輸入密碼，但不建議這麼做。
- `< dname >` 是要與別名相關聯的 X.500 辨別名稱，做為自我簽署憑證的簽發者和主旨欄位。

如需詳細資訊，請參閱 [建立並上傳管理憑證 ][]。
...
在 Windows 命令提示字元 (以系統管理員身分執行)，切換至目錄，包含憑證，並執行下列命令，其中 `< java-安裝目錄的 >` 就是您安裝 Java 在電腦的目錄:
...
6. 在 [**相依性**] 中，按一下 [**新增**]。 在 **輸入群組識別碼...** 輸入 `azure 管理`。 選取用於基礎管理和 App Service Web Apps 管理的封裝：
...
因為 AzureWebDemo 應用程式的目的是要建立應用程式服務 Web 應用程式，主要的類別命名為此應用程式 `WebAppCreator`。 此類別提供主要進入點程式碼，以便呼叫 Azure 服務管理 API 來建立 Web 應用程式。
...
- `< n-i d >` 是您要建立資源的 Azure 訂用帳戶識別碼。
- `< 憑證存放區的路徑 >` 是本機憑證存放區目錄中 JKS 檔案的檔案名稱與路徑。例如， `C:/Certificates/CertificateName.jks` 適用於 Linux 和 `C:\Certificates\CertificateName.jks` 適用於 Windows。
- `< 憑證密碼 >` 是您在建立 JKS 憑證時所指定的密碼。
- `webAppName` 可以是您所選擇的任何名稱，此程序會使用名稱 `WebDemoWebApp`。 完整網域名稱是 `webAppName` 與 `domainName` 附加，因此在此案例的完整網域是 `webdemowebapp.azurewebsites.net`。
- `domainName` 應該指定如上所示。
- `webSpaceName` 應該有一個定義中的值 [WebSpaceNames []][] 類別。
- `appServicePlanName` 應該指定如上所示。

> **附註:** 每次執行此應用程式時，您需要變更值 
...
接著，定義用以建立 Web 應用程式的方法。 這個方法中， `createWebApp`, ，指定 web 應用程式和網路空間的參數。 它也會建立並設定所定義的 App Service Web Apps 管理用戶端 [WebSiteManagementClient []][] 物件。 此管理用戶端是建立 Web Apps 的關鍵。 它可提供符合 REST 限制的 Web 服務，以便應用程式藉由呼叫服務管理 API 來管理 Web 應用程式 (執行作業，例如建立、更新和刪除)。
...
執行 AzureWebDemo 並建立新的 Web 應用程式之後，登入傳統入口網站，按一下 **[Web Apps]**，然後選取 [**Web Apps**] 清單中的 [**WebDemoWebApp**]。 在 web 應用程式的儀表板] 頁面上，按一下 [ **瀏覽** (或按一下 URL， `webdemowebapp.azurewebsites.net`) 來巡覽找到它。 您將會看見空白預留位置頁面，因為尚未將任何內容發佈至此 Web 應用程式。
...
1. 按一下 **[檔案] > [新增] > [動態 Web 專案]**。 其命名為 `JSPHello`。 您不需要變更此對話方塊中的任何其他設定。 按一下 [完成]****。

    ![][3]

2. 在 [專案總管] 中，展開 **JSPHello** 專案，以滑鼠右鍵按一下 **WebContent**，然後按一下 **[新增] > [JSP 檔案]**。 在 [New JSP File] 對話方塊中，新檔案命名為 `index.jsp`。 按 [下一步]****。

3. 在 [Select JSP Template]**** 對話方塊中，選取 [New JSP File (html)]****，然後按一下 [完成]****。

4. 在 index.jsp，加入下列程式碼中的 `< head >` 和 `< 主體 >` 標記區段:
...
4. 在 [**新增伺服器執行階段環境**] 對話方塊中，選取伺服器 (例如 **Apache Tomcat v7.0**) 並按 [**下一步**]。 在 **Tomcat 伺服器** 對話方塊上，將 **名稱** 至 `Apache Tomcat v7.0`, ，並設定 **Tomcat 安裝目錄** 到您安裝 Tomcat 伺服器，您想要使用的版本的目錄。
...
7. 當應用程式執行時，您應該會看到 **JSPHello** 頁面就會出現在 Eclipse 的 localhost 視窗 (`http://localhost:8080//JSPHello/`)，顯示下列訊息:
    ...
    `Hello World 的時間是星期二 3 月 24 日 23:21:10 GMT 2015`
...
3. 在 [**WAR 匯出**] 對話方塊中，選取目前專案中的 src 目錄，然後在結尾包含 WAR 檔案名稱。 例如：
    ...
    `< 專案路徑 > /JSPHello/src/JSPHello.war`
...
1. 在 [**基本功能**] 之下，尋找並複製 [**FTP 主機名稱**]。 這是相似的 URI `ftp://waws-prod-bay-NNN.ftp.azurewebsites.windows.net`。

2. 在 [**基本功能**] 之下，尋找並複製 [**FTP/部署使用者名稱**]。 這會在表單 *webappname\deployment-username*; 例如 `WebDemoWebApp\deployer77`。

若要從發行設定檔取得 FTP 連線資訊：

1. 在 Web 應用程式的刀鋒視窗中，按一下 [**取得發行設定檔**]。 這會將 .publishsettings 檔案下載到本機磁碟機。

2. 使用 XML 編輯器或文字編輯器中開啟.publishsettings 檔案並尋找 `< f i l >` 項目包含 `publishMethod ="FTP"`。該元素如下所示：
...
3. 請注意，web 應用程式的 `f i l` 設定會對應至 FileZilla Site Manager 設定，如下所示:

- `publishUrl` 相同 **FTP 主機名稱**, ，您的設定值 **主機**。
- `publishMethod ="FTP"` 表示您將 **通訊協定** 至 **FTP-檔案傳輸通訊協定**, ，和 **加密** 至 **使用一般 FTP**。
- `userName` 和 `userPWD` 是實際的使用者名稱和密碼值重設部署認證時指定的索引鍵。 `userName` 相同 **部署 / FTP 使用者**。 這些對應至 FileZilla 中的 [**使用者**] 和 [**密碼**]。
...
3. 在主控台命令列中，瀏覽至 `/site/wwwroot` (或按一下 `網站`, ，然後 `wwwroot` 在頁面頂端的目錄檢視中):
...
5. 拖曳 JSPHello.war 從 `< 專案路徑 >/JSPHello/src/` 拖放到之下的 Kudu 目錄檢視 `/site/wwwroot/webapps`。請勿將它拖曳至 [拖曳至此以便上傳和壓縮] 區域，因為 Tomcat 會將它解壓縮。
...
FileZilla 是另一項可用來發佈應用程式的工具 ，這是具有便利圖形 UI 的熱門協力廠商 FTP 用戶端。 您可以下載並安裝從 FileZilla [http://filezilla-project.org/](http://filezilla-project.org/) 如果您還沒有它。 如需有關如何使用用戶端的詳細資訊，請參閱 [FileZilla 文件](https://wiki.filezilla-project.org/Documentation) ，在此部落格項目 [FTP 用戶端-第 4 部分: FileZilla](http://blogs.msdn.com/b/robert_mcmurray/archive/2008/12/17/ftp-clients-part-4-filezilla.aspx)。

1. 在 FileZilla 中，按一下 **[檔案] > [網站管理員]**。
2. 在 [**網站管理員**] 對話方塊中，按一下 [**新增網站**]。 [**選取項目**] 中將會出現新的空白 FTP 網站，提示您提供名稱。 此程序，將它命名 `AzureWebDemo FTP`。
...
3. 按一下 [連接]****。 如果成功，FileZilla 的主控台將顯示 `狀態: 連接` 訊息並發出 `清單` 命令，列出目錄內容。

4. 在 [**本機**] 網站面板中，選取 JSPHello.war 檔案所在的來源目錄；路徑將類似下列路徑：

    `< 專案路徑 >/JSPHello/src /`

5. 在 [**遠端**] 網站面板中，選取目的地資料夾。 您將部署的 WAR 檔案將 `webapps` web 應用程式的根目錄下的目錄。 瀏覽至 `/site/wwwroot`, ，以滑鼠右鍵按一下 `wwwroot`, ，然後選取 **建立目錄**。 將目錄命名為 `webapps` 並輸入該目錄。

6. 將 JSPHello.war 傳輸至 `/site/wwwroot/webapps`。 選取 [**本機**] 檔案清單中的 JSPHello.war，在其上按一下滑鼠右鍵並選取 [**上傳**]。 您應該會看到它出現在 `/site/wwwroot/webapps`。
...
1. 您已經上傳 WAR 檔案，並確認 Tomcat server 已建立解壓縮後 `JSPHello` 目錄中，瀏覽至 `http://webdemowebapp.azurewebsites.net/JSPHello` 執行應用程式。

   > **附註:** 如果您按一下 **瀏覽** 從傳統入口網站中，您可能會
