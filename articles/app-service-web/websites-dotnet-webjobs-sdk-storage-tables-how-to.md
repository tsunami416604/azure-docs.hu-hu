...
本指南提供 C# 程式碼範例示範如何讀取和寫入 Azure 儲存體資料表使用 [WebJobs SDK](websites-dotnet-webjobs-sdk.md) 版本 1.x。

本指南假設您知道 [如何在 Visual Studio 中建立 WebJob 專案與連接字串指向儲存體帳戶](websites-dotnet-webjobs-sdk-get-started.md)。

一些程式碼片段顯示 `資料表` 屬性的函式中使用 [以手動方式呼叫](websites-dotnet-webjobs-sdk-storage-queues-how-to.md#manual), ，也就是不是使用其中一個觸發程序屬性。

## <a id="ingress"></a> 如何將實體加入至資料表

若要將實體加入至資料表，使用 `資料表` 屬性搭配 `設為 < T >` 或 `IAsyncCollector < T >` 參數其中 `T` 指定您想要加入之實體的結構描述。屬性建構函式採用字串參數，來指定資料表的名稱。
...
## <a id="storageapi"></a> 如何使用.NET 儲存體 API 來使用資料表
...
## <a id="queues"></a>佇列操作說明文章所涵蓋的相關的主題

如需如何處理佇列訊息，所觸發的資料表處理的詳細資訊，或資料表處理特有的 WebJobs SDK 案例，請參閱 [如何透過 WebJobs SDK 使用 Azure 佇列儲存體](websites-dotnet-webjobs-sdk-storage-queues-how-to.md)。
