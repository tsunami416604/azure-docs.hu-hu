...
> [AZURE.NOTE] 每個備份都是您的應用程式不是增量更新的完整離線複本。
...
5. 在 **設定備份設定** 刀鋒視窗中，仍處於開啟狀態，按一下 **資料庫設定**, ，然後選取您想要包含在備份 (SQL database 或 MySQL) 中，然後按一下 [的資料庫 **確定**。  
    ...
    > [AZURE.NOTE]  若要顯示在清單中的資料庫，其連接字串必須存在於 **連接字串** 區段 **Web 應用程式設定** 入口網站中的分頁。
...
若要從備份中排除檔案和資料夾，請在 Web 應用程式的 [wwwroot] 資料夾中建立 `_backup.filter` 檔案，並在其中指定您想要排除的檔案和資料夾的清單。 若要存取這個簡單的方法是透過 [Kudu 主控台](https://github.com/projectkudu/kudu/wiki/Kudu-console)。 
...
3. 上傳此檔案來 `D:\home\site\wwwroot\` 目錄的站台使用 [ftp](web-sites-deploy.md#ftp) 或任何其他方法。 如果您希望的話，也可以直接在 `http://{yourapp}.scm.azurewebsites.net/DebugConsole` 中建立該檔案，然後在該處插入內容。
...
在 web 應用程式進行一或多個備份後，備份將會顯示在 **容器** 的儲存體帳戶，以及您的 web 應用程式] 刀鋒視窗。 在儲存體帳戶中，每個備份都會有一個包含備份資料的 .zip 檔案，以及一個包含 .zip 檔案內容資訊清單的 .xml 檔案。 如果您要存取備份而不實際執行 Web 應用程式還原，則可以將這些檔案解壓縮並加以瀏覽。
...
> [AZURE.WARNING] 中的檔案進行任何您 **websitebackups** 容器可能導致備份失效，因此還原。
