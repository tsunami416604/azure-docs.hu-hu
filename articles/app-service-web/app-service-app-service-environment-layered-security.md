...
- **判斷上游呼叫端的傳出的 IP 位址:**  的 IP 位址或上游呼叫者的地址? 這些位址在 NSG 中必須明確地允許存取。 由於 App Service 環境之間的呼叫會被視為「網際網路」呼叫，因此這表示指派給這三個上游 App Service 環境的輸出 IP 位址，在 "apiase" 子網路的 NSG 中必須是允許存取的。 如需有關判斷輸出的 IP 位址，如在 App Service 環境中執行的應用程式，請參閱 [網路架構 ][networkarchitecture] 概觀文章。
