...
本教學課程會說明製作及設定基本 Web 伺服器閘道介面 (WSGI) 相容 Python 應用程式上的選項 [Azure App Service Web Apps](http://go.microsoft.com/fwlink/?LinkId=529714)。
...
如果您沒有現有的 web 應用程式，您可以建立一個從 [Azure 入口網站](https://portal.azure.com)。  按一下 [新增] 按鈕在左下角，然後按一下 [ **Web + 行動** > **Web 應用程式**。

## Git 發行

設定 Git 發行功能新建立的 web 應用程式的指示，在 [Azure App Service 中使用 GIT 連續部署](web-sites-publish-source-control.md)。 本教學課程將使用 Git 來建立、管理並將您的 Python Web 應用程式發佈至 Azure App Service。
...
WSGI 是一項 Python 標準所描述 [PEP 3333](http://www.python.org/dev/peps/pep-3333/) 定義了 web 伺服器與 Python 之間的介面。 此標準為您提供標準化介面，方便您使用 Python 撰寫各種 Web 應用程式與架構。 今日熱門的 Python Web 架構都採用 WSGI。 Azure App Service Web Apps 針對此類任何架構提供支援，而進階使用者甚至可以撰寫自己的架構，但前提是自訂處理常式必須遵守 WSGI 規範指示。
