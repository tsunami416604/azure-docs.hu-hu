...
>[AZURE.NOTE] Azure App Service Web 應用程式中，您可以設定 Azure Active Directory 租用戶驗證按鈕的按幾下。 如需詳細資訊，請參閱 [使用 Active Directory 進行驗證，在 Azure App Service 中](web-sites-authentication-authorization.md)。
...
2.  請依照下列指示 [方式來執行範例為單一租用戶應用程式](https://github.com/Azure-Samples/active-directory-dotnet-webapp-roleclaims#how-to-run-the-sample-as-a-single-tenant-app) 來設定 Azure Active Directory 應用程式和專案。
...
3.  在 [Azure 傳統入口網站](https://manage.windowsazure.com) 檢視您剛建立的 Azure Active Directory 應用程式中，按一下 **使用者** ] 索引標籤。 然後，將想要的使用者指派給想要的角色。
    ...
    >[AZURE.NOTE] 如果您想要將角色指派給群組，以及使用者群組，您必須升級您的 Azure Active Directory 租用戶 [Azure Active Directory Premium](/pricing/details/active-directory/)。 在您的應用程式的傳統入口網站 UI 中，如果您看到 **使用者** ] 索引標籤上，而不是 * * 使用者及群組] 索引標籤上，您可以移至您的 Azure Active Directory 租用戶嘗試 Azure Active Directory Premium **授權** ] 索引標籤。 

3.  完成應用程式設定後，在 Visual Studio 中輸入 `F5` 以執行 ASP.NET 應用程式。

4.  應用程式載入後，按一下 [ **登入** 並使用 Azure 傳統入口網站中具有 「 管理員 」 角色的使用者登入。 
...
3. 如果您還沒有登入 Azure，按一下 [ **將帳戶加入** ，並使用您的 Azure 訂閱的 Microsoft 帳戶登入。

4. 登入之後，按一下 [ **新增** 在 Azure 中建立新的 web 應用程式。

5. 在 **主控**, ，填寫所有必要欄位。 

    ![](./media/web-sites-dotnet-lob-application-azure-ad/4-create-website.png)

5. 您必須具備資料庫連線，此應用程式才能儲存角色對應、快取的權杖及任何應用程式資料。 在 **建立應用程式服務** ] 對話方塊中，按一下 [ **服務**。  旁邊 **SQL Database** 按一下加號以加入新的資料庫。

    ![](./media/web-sites-dotnet-lob-application-azure-ad/4-create-database.png)

5. 在 **設定 SQL Database** ] 對話方塊中，選取或建立伺服器、 設定的名稱，然後按一下 **確定**。

     ![](./media/web-sites-dotnet-lob-application-azure-ad/4-config-database.png)

6. 按一下 [ **建立**。 一旦建立 web 應用程式， **發行 Web** 對話方塊會開啟。

7. 在 **目的地 URL**, ，變更 **http** 至 **https**。 將完整的 URL 複製到文字編輯器。 您稍後將使用它。 然後按一下 [ **下一步**。
...
4. 指定應用程式名稱，然後按一下 [ **下一步**。

5. 在應用程式屬性，設定 **登入 URL** 儲存稍 web 應用程式 url (例如 `https://<site-name>.azurewebsites.net/`)，而 **應用程式識別碼 URI** 到 `https://<aad-tenanet-name>/<app-name>`。 然後按一下 [ **完成**。
...
7. 在 **金鑰**, ，選取 [建立新的金鑰 **1 年** 下拉式清單中。

8. 在 **其他應用程式的權限**, ，如 **Azure Active Directory** 項目選取 **登入並讀取的使用者設定檔** 和 **讀取目錄資料** 中 **委派的權限** 下拉式清單。

    > [AZURE.NOTE] 您在這裡需要的確切權限取決於您的應用程式所需的功能。 有些權限需要 **全域管理員** 角色集，但本教學課程所需的權限只需要 **使用者** 角色。
...
2. 按一下 [ **發行** 發行至 Azure App Service Web 應用程式。

當您完成時，您會在 Azure 傳統入口網站中設定兩個 Azure Active Directory 應用程式：一個適用於 Visual Studio 中的偵錯環境，另一個適用於 Azure 中的已發佈 Web 應用程式。 偵錯期間，Web.config 中的應用程式設定可讓您 **偵錯** 與 Azure Active Directory，以及發行它的組態工作 (根據預設， **版本** 發行組態)，會上傳轉換的 Web.config，其中包含 Web.Release.config 的應用程式設定變更。
...
8.  將新的建構項目 `WorkItemsController` 新增至 Controllers 資料夾。 若要這樣做，請以滑鼠右鍵按一下 **控制器**, ，指向 [ **新增**, ，然後選取 **新建構的項目**。 

9.  選取 **MVC 5 控制器具有檢視、 使用 Entity Framework** 按一下 **新增**。

10. 選取您剛建立的模型，然後按一下 [ **新增**。
...
12.  在 App_Start\Startup.Auth.cs 中，於 `ConfigureAuth` 方法中加入下行程式碼。 以滑鼠右鍵按一下每個命名的解決方案錯誤來修正這個問題。
...
16. 填寫表單的其餘部分，按一下 [ **建立**。 ~/WorkItems/Index 頁面現在會顯示新建立的工作項目。 您還會看到下面的螢幕擷取畫面，顯示我移除了 Views\WorkItems\Index.cshtml 中的 `AssignedToID` 資料行。 

    ![](./media/web-sites-dotnet-lob-application-azure-ad/10-workitem-index.png)

11. 現在，進行類似的變更 **編輯** 檢視。 在 Views\WorkItems\Edit.cshtml 中，對 `Html.BeginForm` Helper 方法 (與上個步驟中 Views\WorkItems\Create.cshtml 的方法相同) 進行變更 (將上述反白顯示的程式碼中的 "Create" 取代成 "Edit")。
...
- [在 Azure 應用程式服務中建立使用 AD FS 驗證的 .NET MVC Web 應用程式](web-sites-dotnet-lob-application-adfs.md)
- [Microsoft Azure Active Directory 範例與文件](https://github.com/AzureADSamples)
- [Vittorio Bertocci 的部落格 (英文)](http://blogs.msdn.com/b/vbertocci/)
- [將 VS2013 Web 專案從 WIF 移轉到 Katana (英文)](http://www.cloudidentity.com/blog/2014/09/15/MIGRATE-A-VS2013-WEB-PROJECT-FROM-WIF-TO-KATANA/)
- [Azure 的新混合式連線不是您父執輩的 #hybridCloud (英文)](/documentation/videos/new-hybrid-connections-not-your-fathers-hybridcloud/)
- [Active Directory 與 Azure Active Directory 之間的相似處y (英文)](http://technet.microsoft.com/library/dn518177.aspx)
- [搭配單一登入的目錄同步作業實例 (英文)](http://technet.microsoft.com/library/dn441213.aspx)
- [zure Active Directory 支援的權杖和宣告類型 (英文)](http://msdn.microsoft.com/library/azure/dn195587.aspx)
