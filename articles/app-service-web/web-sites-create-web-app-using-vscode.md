...
4. 使用方向鍵來選取 **Web 應用程式基本** 類型從 ASP.NET 5 產生器] 功能表上，然後按 **< Enter >**。

    ![Yeoman-ASP.NET 5 產生器](./media/web-sites-create-web-app-using-vscode/01-yo-aspnet.png)

5. 將新的 ASP.NET web 應用程式的名稱設定為 **SampleWebApp**。 因為整個教學課程會使用此名稱，所以如果您選取不同的名稱，您需要取代每次出現的 **SampleWebApp**。 當您按 **< Enter >**, ，Yeoman 將建立名為的新資料夾 **SampleWebApp** 以及新應用程式必要的檔案。
