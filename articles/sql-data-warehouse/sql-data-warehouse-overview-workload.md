...
>[AZURE.NOTE] 記憶體中資料行存放區索引中，使用資料行存放區，提供最多 5 倍壓縮速度提升與 10 倍查詢效能傳統二進位樹狀目錄的報告和分析的查詢。 我們將資料行存放區索引視為在資料倉儲中儲存和掃描大量資料的標準。
...
> [AZURE.NOTE] SQL Server ctp2，您現在可以執行分析即時在 OLTP 的資料表。 雖然這不會取代資料倉儲儲存和分析資料的需求，但的確能夠用來進行即時執行分析。
