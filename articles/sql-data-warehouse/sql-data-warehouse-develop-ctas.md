...
CTAS 最常見的用途之一，就是建立資料表複本，讓您可以變更 DDL。 例如，您原本將資料表建立為 ROUND_ROBIN，而現在想變更為在資料行上散發的資料表，CTAS 就是您將變更散發資料行的方式。 CTAS 也可用於變更資料分割、索引，或資料行類型。

假設您因為在 CREATE TABLE 中沒有指定散發資料行，而使用 ROUND_ROBIN 散發的預設散發類型建立資料表。
