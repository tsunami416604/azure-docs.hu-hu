...
> [AZURE.NOTE] 使用的資料行 `加入`, ，`GROUP BY`, ，`DISTINCT` 和 `HAVING` 子句都會產生的雜湊資料行的候選項目。 其他手中的資料行 `其中` 子句 **不** 進行良好的雜湊資料行的候選項目。 請參閱下列與平衡的執行相關的一節。

資料移動可能也會發生的查詢語法 (`COUNT DISTINCT` 和 `透過` 子句都是絕佳範例) 時不包含雜湊散發索引鍵的資料行搭配使用。
...
如您所見，資料扭曲可能會影響平衡的執行。 不過，也因此可以選擇雜湊散發索引鍵。 如果已選擇資料行中顯示的 `其中` 子句的查詢是很有可能不會平衡查詢。
> [AZURE.NOTE] `其中` 子句通常有助於識別最適用於資料分割的資料行。

會出現在資料行的理想範例 `其中` 子句應該是日期欄位。 日期欄位是絕佳資料分割資料行的傳統範例，但通常是不佳的雜湊散發資料行。 一般而言，資料倉儲查詢會維持一段指定的時間，例如日、週或月。 依據日期的雜湊散發可能已實際限制我們的延展性並損及我們的效能。 例如，如果指定的日期範圍是一週 (也就是 7 天)，則雜湊的最大數目就是 7 - 每天一個。 這表示只有 7 個散發會包含資料。 其餘的散發沒有任何資料。 這會導致不平衡的查詢執行，因為只有 7 個散發在處理資料。
...
1. 用於 `加入`, ，`GROUP BY`, ，`DISTINCT`, ，或 `HAVING` 在查詢中的子句。
2. 不會用於 `其中` 子句
3. 有許多不同的值，至少 1000 個。
...
> [AZURE.NOTE] 在 SQL 資料倉儲中，資料類型一致性相當重要！ 請確定現有的結構描述一致使用相同類型的資料行。 這對散發索引鍵尤其重要。 如果未同步處理散發索引鍵資料類型，且資料表已聯結，就會發生不需要的資料移動。 如果資料表很大，而且會減少的輸送量和效能，這可能是高成本。
