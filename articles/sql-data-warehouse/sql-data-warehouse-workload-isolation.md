---
title: Munkaterhelés elkülönítése
...
A munkaterhelés-tárolás konfigurálása implicit módon meghatározza a maximális párhuzamossági szintet.  Ha a CAP_PERCENTAGE_RESOURCE 60%-ra van beállítva, és egy REQUEST_MIN_RESOURCE_GRANT_PERCENT 1%-ra van beállítva, akkor legfeljebb 60-párhuzamossági szint adható meg a munkaterhelés csoport számára.  Vegye figyelembe az alábbi módszert a maximális párhuzamosság meghatározásához:

[Maximális Egyidejűség] = [`CAP_PERCENTAGE_RESOURCE`]/[`REQUEST_MIN_RESOURCE_GRANT_PERCENT`]

> [!NOTE] 
> A munkaterhelési csoport tényleges CAP_PERCENTAGE_RESOURCE nem éri el a 100%-ot, ha a MIN_PERCENTAGE_RESOURCE szintnél nagyobb munkaterhelés-csoportok jönnek létre.  Lásd: [sys. DM _workload_management_workload_groups_stats](https://review.docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-workload-management-workload-group-stats-transact-sql?view=azure-sqldw-latest) a tényleges futtatókörnyezeti értékekhez.

## <a name="resources-per-request-definition"></a>Erőforrások/kérelmek definíciója

A munkaterhelési csoportok olyan mechanizmust biztosítanak, amely meghatározza, hogy a REQUEST_MIN_RESOURCE_GRANT_PERCENT és a REQUEST_MAX_RESOURCE_GRANT_PERCENT paraméterekkel kérelmekben lefoglalt erőforrások minimális és maximális mennyisége a [munkaterhelés-csoport létrehozása](https://review.docs.microsoft.com/sql/t-sql/statements/create-workload-group-transact-sql?view=azure-sqldw-latest) szintaxisban történjen.  Ebben az esetben a processzor és a memória erőforrásai.  Ezeknek az értékeknek a beállítása azt határozza meg, hogy mennyi erőforrást és milyen szintű párhuzamosságot érhet el a rendszeren.

> [!NOTE] 
> A REQUEST_MAX_RESOURCE_GRANT_PERCENT egy opcionális paraméter, amely alapértelmezett értéke a REQUEST_MIN_RESOURCE_GRANT_PERCENT számára megadott érték.

Az erőforrás-osztály kiválasztásához hasonlóan a REQUEST_MIN_RESOURCE_GRANT_PERCENT konfigurálása beállítja a kérések által használt erőforrások értékét.  A set érték által jelzett erőforrások mennyisége garantált a kérelemnek a végrehajtás megkezdése előtt történő kiosztásához.  Az erőforrás-osztályokból a munkaterhelés-csoportokba áttelepítést végző ügyfelek esetében vegye figyelembe, hogyan kell kiindulási pontként bemutatni az erőforrások osztályairól a munkaterhelés-csoportokba való hozzárendelésének [módját](sql-data-warehouse-how-to-convert-resource-classes-workload-groups.md) .

A REQUEST_MAX_RESOURCE_GRANT_PERCENT a REQUEST_MIN_RESOURCE_GRANT_PERCENT-nál nagyobb értékre való konfigurálása lehetővé teszi, hogy a rendszer több erőforrást foglaljon le egy kérésre.  A kérelem ütemezése során a rendszer meghatározza a kérelem tényleges erőforrás-elosztását, amely a REQUEST_MIN_RESOURCE_GRANT_PERCENT és a REQUEST_MAX_RESOURCE_GRANT_PERCENT között van, a megosztott készletben lévő erőforrások rendelkezésre állása és a jelenlegi terhelés alapján. rendszer.  Az erőforrásoknak léteznie kell az erőforrások [megosztott készletében](#shared-pool-resources) , ha a lekérdezés ütemezve van.  

> [!NOTE] 
> A REQUEST_MIN_RESOURCE_GRANT_PERCENT és a REQUEST_MAX_RESOURCE_GRANT_PERCENT érvényes értékekkel rendelkeznek, amelyek az érvényes MIN_PERCENTAGE_RESOURCE és CAP_PERCENTAGE_RESOURCE értéktől függenek.  Lásd: [sys. DM _workload_management_workload_groups_stats](https://review.docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-workload-management-workload-group-stats-transact-sql?view=azure-sqldw-latest) a tényleges futtatókörnyezeti értékekhez.
...
A megosztott készlet erőforrásai az erőforrások elkülönítésére nincsenek konfigurálva.  A MIN_PERCENTAGE_RESOURCE beállított munkaterhelési csoportok a megosztott készletben lévő erőforrásokat a kérelmek végrehajtásához használják.  A MIN_PERCENTAGE_RESOURCE-nál nagyobb CAP_PERCENTAGE_RESOURCE rendelkező munkaterhelési csoportok megosztott erőforrásokat is használnak.  A megosztott készletben elérhető erőforrások mennyisége a következőképpen számítható ki.
...
- [MUNKATERHELÉS-CSOPORT LÉTREHOZÁSA](https://review.docs.microsoft.com/sql/t-sql/statements/create-workload-group-transact-sql?view=azure-sqldw-latest)
