...
資料庫還原的設計是為了將您的資料庫還原到較早的時間點。 Azure SQL 資料倉儲服務至少每 8 小時會以自動儲存體快照集來保護所有資料庫，並保留 7 天，為您提供一組獨立的還原點。 這些備份儲存在 RA-GRS Azure 儲存體，因此，依預設為異地備援。 自動備份和還原功能不需額外費用，並提供零成本和免管理的方式，避免資料庫遭到意外損毀或刪除。 若要深入了解資料庫還原，請參閱 [從使用者錯誤 [] 修復][]。
...
若要深入了解其他 SQL Database 版本的業務持續性功能，請閱讀 [SQL Database 商務持續性概觀 []][]。
