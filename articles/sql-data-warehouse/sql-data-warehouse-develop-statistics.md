...
> [AZURE.NOTE] 如果您使用 SQL Server，您可能會依賴 SQL Server 建立，並讓您視需要更新單一資料行統計資料。 SQL 資料倉儲在這方面有所不同。 由於資料已分散，所以 SQL 資料倉儲不會自動彙總所有分散式資料的統計資料。 它只會在您建立和更新統計資料時產生彙總統計資料。
...
這個問題不是可依存留期回答的問題。 最新的統計資料物件有可能非常舊。 當資料列數目或特定資料行的值分佈的實質變更 *然後* 需要更新統計資料。
...
> [AZURE.NOTE] 查詢最佳化工具考慮使用篩選的統計資料，選擇分散式的查詢計劃時，查詢必須配合統計資料物件的定義。 使用上述範例，查詢的 where 子句需要指定介於 2000101 和 20001231 之間的 col1 值。
...
> [AZURE.NOTE] 長條圖，用來估計查詢結果中的資料列數目，只適用於統計資料物件定義中所列的第一個資料行。
