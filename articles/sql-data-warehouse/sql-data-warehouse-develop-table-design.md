...
# SQL 資料倉儲中的資料表設計 #
SQL 資料倉儲是大量平行處理 (MPP) 分散式資料庫系統。 將資料儲存在許多不同的位置，稱為 **分佈**。 每個 **發佈** 都像是一個貯體; 可儲存在資料倉儲中的唯一資料子集。 將資料和處理能力散佈於多個節點，SQL 資料倉儲就能夠提供極大的延展性 — 遠超過任何單一系統。
...
此查詢包含任何不支援的使用者定義資料類型。

以下是一些您可以用來取代不支援的資料類型的替代方案。
...
第一個選項即為 **循環配置資源** 散發-有時也稱為隨機雜湊。 您可以將此視為預設或保全選項。

第二個選項稱為 **雜湊** 發佈。 您可以將它視為資料散發的最佳化形式。 當資料表叢集共用常見的聯結及/或彙總準則時，慣用這個選項。
...
> [AZURE.NOTE] 當資料進行循環配置資源散發資料配置在發佈給 *緩衝區* 層級。
...
雜湊散發使用內部函式，藉由雜湊單一資料行來將資料集分散於各個散發。 雜湊資料後，沒有明確的順序可將資料配置到散發。 不過，雜湊本身是具決定性的程序。 這會讓雜湊的結果變得可預測。 例如，雜湊包含值 10 的整數資料行時，一定會產生相同的雜湊值。 這表示 ***任何*** 雜湊的整數資料行包含值 10 最終都會配置到相同的散發。 這適用於所有的資料表。
...
> [AZURE.NOTE] 當資料進行雜湊散發資料被配置分配資料列層級。
...
當使用者查詢資料表時，SQL 資料倉儲會使用分散式查詢最佳化工具來建立適當的查詢計劃。 一旦建立，查詢計劃會提供資料庫用來存取資料和履行使用者要求的策略和方法 SQL 資料倉儲的查詢最佳化工具是以成本為基礎。 換句話說，它會根據相對成本來比較各種選項 (計劃) 並選擇最有效率的可用計劃。 因此，SQL 資料倉儲需要大量資訊，才能做出以成本為基礎的明智決策。 它會保留有關資料表 (資料表大小) 的統計資訊，而在資料庫物件稱為 `STATISTICS`。

統計資料會真對索引或資料表的單一或多重資料行進行保留。 這些資料可將值的基數和選擇性相關重要資訊，提供給以成本為基礎的最佳化工具。 當最佳化工具需要評估查詢中的 JOIN、GROUP BY、HAVING 和 WHERE 子句時，這是特別有用。 因此是非常重要的資訊包含在這些統計資料物件 *精確* 反映出資料表的目前狀態。 請務必了解成本的精確度很很重要。 如果統計資料精確地反映出資料表的狀態，即可比較計劃以找出最低的成本。 如果統計資料不精確，則 SQL 資料倉儲可能會選擇錯誤的計劃。
...
>[AZURE.NOTE] 它是很常見的 SQL Server 資料倉儲，完全依賴 `AUTOSTATS` 來保持最新的資料行統計資料。 這不是最佳做法 SQL Server 資料倉儲。 `AUTOSTATS` 這對於包含數百萬或數十億的資料列的大型事實資料表可能不太足夠變更的 20%率會觸發。 因此，最好隨時掌握統計資料更新，以確保統計資料能精確地反映出資料表的基數。
