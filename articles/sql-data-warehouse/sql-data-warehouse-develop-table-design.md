...
當使用者查詢資料表時，SQL 資料倉儲會使用分散式查詢最佳化工具來建立適當的查詢計劃。 一旦建立，查詢計劃會提供資料庫用來存取資料和履行使用者要求的策略和方法 SQL 資料倉儲的查詢最佳化工具是以成本為基礎。 換句話說，它會根據相對成本來比較各種選項 (計劃) 並選擇最有效率的可用計劃。 因此，SQL 資料倉儲需要大量資訊，才能做出以成本為基礎的明智決策。 它會保留有關資料表 (資料表大小) 的統計資訊在資料庫物件稱為 `統計資料`。
...
1. 在使用中的資料行上建立單一資料行統計資料 `其中`, ，`加入`, ，`GROUP BY`, ，`ORDER BY` 和 `DISTINCT` 子句
2. 產生複合子句的多重資料行統計資料
3. 定期更新統計資料。 請記住，此作業不會自動完成！

>[AZURE.NOTE] 它是很常見的 SQL Server 資料倉儲，完全依賴 `AUTOSTATS` 來保持最新的資料行統計資料。 這不是最佳做法 SQL Server 資料倉儲。 `AUTOSTATS` 的變更可能不太足夠的大型事實資料表包含數百萬或數十億的資料列的 20%率會觸發。 因此，最好隨時掌握統計資料更新，以確保統計資料能精確地反映出資料表的基數。
