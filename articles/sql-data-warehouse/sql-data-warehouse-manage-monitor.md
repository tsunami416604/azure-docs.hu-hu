...
您可以使用 *sys.dm_pdw_nodes_exec_connections* 檢視，擷取與 Azure SQL 資料倉儲資料庫建立連接的資訊。 此外，*sys.dm_exec_sessions* 檢視有助於擷取所有作用中使用者的連接資訊。
...
檢查長時間執行查詢步驟的 *operation_type* 資料行：
...
- 如果是長時間執行的散發，請檢查 *rows_processed* 資料行，查看從該散發移動的資料列數是否遠多過其他散發。 這麼做可看到有資料扭曲情況的查詢。
