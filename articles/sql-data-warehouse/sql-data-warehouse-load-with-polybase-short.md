...
本教學課程將示範如何將資料載入至您的 Azure SQL 資料倉儲使用 PolyBase。 若要深入了解 PolyBase，請參閱 [SQL 資料倉儲的教學課程 [] 中的 PolyBase][]。
...
- [建立資料庫範圍認證]: 若要指定儲存體帳戶的驗證資訊。
- [建立外部資料來源]: 若要指定 Azure blob 儲存體的位置。
- [建立外部檔案格式]: 指定的資料配置。
- [建立外部資料表]: 參考 Azure 儲存體資料。
