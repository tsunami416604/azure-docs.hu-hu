...
> [AZURE.NOTE] <a name="note"></a>邏輯的 SQL server:
  ...
  > + 不是內部部署伺服器的實體硬體。 它是服務軟體的一部分。 這就是為什麼我們稱之為 「 *邏輯伺服器*。
  > + 可以裝載多個資料庫，而不會影響其效能。
  > + 使用大小寫 *s* 在其名稱。 SQL **s**伺服器是 Azure 邏輯伺服器，而 SQL **S**伺服器是 Microsoft 的內部部署資料庫的產品。

1. 按一下 [ **伺服器** > **建立新的伺服器**。 伺服器不會收取費用。 如果您已經有想要使用的 V12 邏輯 SQL 伺服器，請選擇現有的伺服器，然後移至下一個步驟。 
...
3. 填寫 **新伺服器** 資訊。 
    ...
    >[AZURE.NOTE] 請確定伺服器名稱、 伺服器管理員名稱和密碼儲存某處。  您需要此資訊才能登入伺服器。

1. 按一下 [ **確定** 儲存邏輯的 SQL server 組態設定，並回到 SQL 資料倉儲] 分頁。
...
2. 在 **SQL 資料倉儲** 刀鋒視窗中，填滿剩下的欄位中。 

    ![建立資料庫](./media/sql-data-warehouse-get-started-provision/create-database.png)
    
    - **效能**︰ 我們建議以 400 Dwu 開始。 您可以將滑桿向左邊或向右移動來調整您的資料庫的效能層級，現在與之後都會建立資料庫。 

        > [AZURE.NOTE] SQL 資料倉儲可測量資料倉儲單位 (Dwu) 中的效能。 當您增加 DWU 時，SQL 資料倉儲會為您的資料庫作業增加可用的運算資源。 當您執行您的工作負載時，將能夠看到 DWU 與您的工作負載效能的關係。 
        > 
        > 在資料庫建立後，您可以快速、輕鬆地變更效能等級。  例如，如果您目前未使用資料庫，向左移動滑桿可降低成本。 或是在需要更多資源時提高效能。 若不要產生成本，您可以暫停資料庫。 這是 SQL 資料倉儲的可擴充功能。

    - **選取來源**。 按一下 [ **選取來源** > **範例**。 因為只有一個範例資料庫可在此階段，當您選取範例 Azure 自動填入 **選取範例** AdventureWorksDW 選項。 
...
1. 按一下 [ **瀏覽** > **SQL 伺服器** > 選擇您的伺服器 > **設定** > **防火牆**。
...
1. 建立某個 IP 位址範圍的防火牆規則。 您可以現在或稍後執行這項操作。
    ...
    >[AZURE.IMPORTANT] 您可能會不時，變更 IP 位址且您可能無法存取您的伺服器，您必須建立新的防火牆規則。 若要確保一致的存取，建議您新增 IP 位址範圍。 如需詳細資訊，請參閱 [如何設定防火牆設定](../sql-database/sql-database-configure-firewall-settings.md)。

    若要建立規則時，輸入名稱和 IP 位址範圍，按一下 **儲存**。 
...
既然您已建立範例資料庫的 SQL 資料倉儲，您就準備好 [連接](./sql-data-warehouse-get-started-connect.md) 到您的資料庫。
