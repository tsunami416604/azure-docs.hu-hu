...
- 儲存在 Azure 專案檔 (.ccproj) 中的發行設定檔會在移至不同的檔案，副檔名為.azurePubXml 與 **發行** 子目錄。

- 發佈設定檔中的某些屬性會更新為支援新的和變更的功能。 **{1>allowupgrade** 取代 **DeploymentReplacementMethod** 因為您可以在同時或以累加方式更新部署的雲端服務。

- 屬性 **UseIISExpressByDefault** 加入，並使用於偵錯的 web 伺服器不會自動將從網際網路資訊服務 (IIS) 變更為 IIS Express 設定為 false。 IIS Express 是由較新版本的工具所建立之專案的預設 Web 伺服器。
...
>[AZURE.IMPORTANT] F # 專案中，您必須手動更新 Azure 組件的參考，使其參考這些組件的較新版本。
...
1. 在 [方案總管] 中，開啟專案節點的捷徑功能表，選擇 [ **屬性**, ，然後選擇 [ **應用程式** ] 索引標籤會出現在對話方塊中的。

     **應用程式** 索引標籤會顯示與專案相關聯的工具版本。 如果出現目前版本的 Azure Tools，表示已經升級專案。 如果您已經安裝比索引標籤會顯示較新版本的工具 **升級** ] 按鈕會出現。
