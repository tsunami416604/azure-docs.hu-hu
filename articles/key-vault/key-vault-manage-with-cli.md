...
--help 或 -h 參數可用於檢視特定命令的說明。 或者，azure 說明 [命令 ][options] 格式也可用來傳回相同的資訊。 例如，以下命令全部都會傳回相同的資訊：
...
第一個參數是資源群組名稱，而第二個參數是位置。 位置，請使用命令 `azure 位置清單` 以了解如何指定要在此範例中的一個替代位置。 如果您需要更多資訊，請輸入: `azure 說明位置`



## 建立金鑰保存庫

使用 `azure 金鑰保存庫建立` 命令來建立金鑰保存庫。 這個指令碼包含三個必要參數：資源群組名稱、金鑰保存庫名稱和地理位置。
...
如果您想要 Azure 金鑰保存庫，為您建立一個軟體防護金鑰，使用 `azure 金鑰建立` 命令，然後輸入下列命令:
...
- 若要檢視您的金鑰，請輸入: `azure 金鑰保存庫金鑰清單--保存庫名稱 'ContosoKeyVault'`
- 若要檢視您的密碼，請輸入: `azure 金鑰保存庫密碼清單--保存庫名稱 'ContosoKeyVault'`
...
若要授權應用程式存取金鑰或密碼保存庫中的，使用 `azure 金鑰保存庫設定原則` 命令。
