...
A helyreállítható törlés i. szolgáltatás kezdetben a [REST,](/rest/api/keyvault/) [CLI](key-vault-soft-delete-cli.md), [PowerShell](key-vault-soft-delete-powershell.md) és [.NET/C#](/dotnet/api/microsoft.azure.keyvault?view=azure-dotnet) felületeken keresztül érhető el.
...
Új kulcstartó létrehozásakor a helyreállítható törlés alapértelmezés szerint be van kapcsolva. Az [Azure CLI-n](key-vault-soft-delete-cli.md) vagy az [Azure Powershellen](key-vault-soft-delete-powershell.md)keresztül is létrehozhat egy kulcstartót a helyreállítható törlés nélkül. Ha a helyreállítható törlés engedélyezve van egy kulcstartóban, az nem tiltható le.
...
A védelem kiürítése a Key Vault nem kötelező viselkedése, és **alapértelmezés szerint nincs engedélyezve.** Ez lehet esztergált -ra keresztül [CLI](key-vault-soft-delete-cli.md#enabling-purge-protection) vagy [Powershell](key-vault-soft-delete-powershell.md#enabling-purge-protection).
