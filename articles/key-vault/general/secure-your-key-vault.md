...
A felügyeleti síkban az RBAC(szerepköralapú hozzáférés-vezérlés) használatával engedélyezheti a hívó által végrehajtható műveleteket. Az RBAC-modellben minden Azure-előfizetés rendelkezik egy Azure AD-példánysal. Hozzáférést biztosít a felhasználóknak, csoportoknak és alkalmazásoknak ebből a címtárból. Az Access az Azure Resource Manager telepítési modelljét használó Azure Resource Manager-telepítési modellt használó erőforrások kezeléséhez érhető el. A hozzáférés engedélyezéséhez használja az [Azure Portalt,](https://portal.azure.com/)az [Azure CLI-t](/cli/azure/install-azure-cli?view=azure-cli-latest), az [Azure PowerShellt](/powershell/azureps-cmdlets-docs)vagy az [Azure Resource Manager REST API-kat.](https://msdn.microsoft.com/library/azure/dn906885.aspx)
...
A biztonsági csoport létrehoz egy key vaultot, és beállítja a naplózási és hozzáférési engedélyeket.
...
Javasoljuk, hogy a [Key Vault tűzfalainak és virtuális hálózatainak konfigurálásával](network-security.md)további biztonságos hozzáférést biztosítson a kulcstartóhoz.
...
Konfigurálja [a Key Vault tűzfalait és a virtuális hálózatokat.](network-security.md)

A rendszergazda immár a kezdéshez: [Mi az Azure Key Vault?](overview.md)

A Key Vault használati naplózásáról az [Azure Key Vault naplózása című](logging.md)témakörben talál további információt.
