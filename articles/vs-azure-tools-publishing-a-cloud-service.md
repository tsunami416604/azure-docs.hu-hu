...
1. Ahhoz, hogy a **engedélyezéséhez a Web Deploy** az összes webes szerepkörök jelölőnégyzetet, először konfigurálnia kell távoli asztali kapcsolatokhoz. Válassza ki **távoli asztal engedélyezése** összes szerepkörhöz, majd megadja a távolról a kapcsolódáshoz használt hitelesítő adatait a **a távoli asztal konfigurálásának** meg. Lásd: [távoli asztal használata Azure szerepkörök](vs-azure-tools-remote-desktop-roles.md).
...
1. Válassza ki **tovább** megjelenítése a **összegzés** képernyőn, és válassza a **közzététel** a felhőalapú szolgáltatás telepítéséhez.
   ...
   > [!NOTE]
   > Ha a konfigurált webes szerepkör több példánya van, egy figyelmeztető üzenet jelenik meg, arról, hogy minden webes szerepkör csak a csomagban, amely az alkalmazás közzétételére létrejön egy példányra korlátozott. Válassza ki **OK** folytatja. A követelményeket ismertető részben leírtaknak egynél több webes szerepkör, de csak egy példányát minden egyes szerepkör lehet.
...
1. Megbízható az SSL-tanúsítvány hozzáadása az IIS használata a távoli kapcsolatokhoz, kövesse az alábbi lépéseket:
  ...
  a. A webes szerepkör-példányt jelöli ki, hogy a webes szerepkör fut a virtuális gép csatlakozni **Cloud Explorer** vagy **Server Explorer**, majd válassza a **csatlakozzon a távoli asztali kapcsolattal**  parancsot. A virtuális géphez történő csatlakozáshoz kapcsolatos részletes lépéseiért lásd: [a távoli asztal Azure szerepkörök](vs-azure-tools-remote-desktop-roles.md). A böngésző felszólítja, hogy töltse le egy `.rdp` fájlt.
