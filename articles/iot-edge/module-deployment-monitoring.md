...
Ez a cikk az eszközök flottáinak konfigurációját és figyelési szakaszait ismerteti, amelyeket együttesen IoT Edge automatikus központi telepítésnek nevezünk. Az általános telepítési lépések a következők: 
...
2. A IoT Hub szolgáltatás az összes megadott eszközzel kommunikál, hogy a kívánt modulokkal konfigurálja őket. 
...
Például egy üzembe helyezési feltételt tartalmazó címkével rendelkezik. környezet = "Prod". Az üzembe helyezés elindítását követően 10 éles eszköz található. A modulok sikeresen telepítve vannak a 10 eszközön. A IoT Edge ügynök állapota 10 összes eszköz, 10 sikeres válasz, 0 sikertelen válasz és 0 függőben lévő válaszként jelenik meg. Most öt további eszközt ad hozzá címkékkel. environment = "Prod". A szolgáltatás észleli a változást, és a IoT Edge ügynök állapota 15 teljes eszköz, 10 sikeres válasz, 0 sikertelen válasz, valamint 5 függőben lévő válasz lesz, amikor az öt új eszközre próbál üzembe helyezni.

A megcélzott eszközök kiválasztásához használjon bármilyen logikai feltételt az eszközök ikrek-címkék vagy a deviceId használatával. Ha címkével szeretné feltételt használni, hozzá kell adnia a "címkék":{} szakaszt az eszköz twin (a tulajdonságok között azonos szinten). [További információ a címkékről az eszköz Twin-ben](../iot-hub/iot-hub-devguide-device-twins.md)

Cél feltételi példák:
...
Az alábbiakban néhány korlátozást talál a cél feltételének összeállításakor:

* A Device Twin eszközben csak címkék vagy deviceId használatával hozhat létre célként feltételt.
...
* A **sikeres jelentéskészítés** megjeleníti azokat a IoT Edge eszközöket, amelyek visszajelentették a szolgáltatásnak a modulok központi telepítését. 
* A **jelentéskészítési hiba** azokat a IoT Edge eszközöket jeleníti meg, amelyek egy vagy több modul nem lettek telepítve a szolgáltatásba. A hiba további kivizsgálásához kapcsolódjon távolról az eszközökhöz, és tekintse meg a naplófájlokat.

Emellett saját egyéni metrikákat is meghatározhat a telepítés figyeléséhez és kezeléséhez. 

A metrikák összegző számlálást biztosítanak azon különböző állapotok számára, amelyeket az eszközök jelenthetnek a telepítési konfiguráció alkalmazásának eredményeképpen. A metrikák lekérhetik a [edgeHub modul két jelentett tulajdonságát](module-edgeagent-edgehub.md#edgehub-reported-properties), például az utolsó kívánt állapotot vagy a legutóbbi csatlakozási időt. Példa: 
...
A rendszer-futtatókörnyezeti modulok, a edgeAgent és a edgeHub nem egy rétegzett telepítés részeként vannak konfigurálva. A többrétegű üzemelő példány által megszabott IoT Edge-eszközökön a szabványos automatikus központi telepítésre van szükség ahhoz, hogy megadják az alapot, amelyre a rétegzett központi telepítések adhatók. 
...
![A többrétegű automatikus üzembe helyezés egyszerűsíti az olyan forgatókönyveket, amelyekben ugyanazok a modulok különböző módokon kombinálhatók](./media/module-deployment-monitoring/layered-deployment.png)
...
Normál telepítés esetén például a szimulált hőmérséklet-érzékelő modult a következő kívánt tulajdonságokkal veheti fel, amelyek azt jelzik, hogy az adatküldés 5 másodperces intervallumban történik:
...
Egy rétegzett központi telepítésben, amely ugyanazokat az eszközöket vagy azonos eszközök egy részhalmazát célozza meg, érdemes lehet hozzáadni egy további tulajdonságot, amely közli a szimulált érzékelővel, hogy 1000 üzenetet küldjön, majd leállítja. Nem kívánja felülírni a meglévő tulajdonságokat, ezért hozzon létre egy új szakaszt a kívánt, `layeredProperties` nevű, az új tulajdonságot tartalmazó tulajdonságok között:
...
Az olyan eszközök, amelyeken mindkét üzemelő példány van alkalmazva, a szimulált hőmérséklet-érzékelőnél a következő modulban láthatók: 
...
Egy központi telepítés törlése nem távolítja el a modulokat a célként megadott eszközökről. Egy másik központi telepítésnek kell lennie, amely új konfigurációt határoz meg az eszközökhöz, még akkor is, ha ez egy üres telepítés. 
