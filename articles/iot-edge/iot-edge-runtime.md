...
Az IoT Edge-futtatókörnyezet programok, kapcsolja be az eszközt az IoT Edge-eszköz gyűjteménye. Az IoT Edge-modul összetevők együttesen kattintva kérhet kódot futtatása a peremhálózaton IoT Edge-eszközök engedélyezése, és az eredmények kommunikációhoz. 
...
Az IoT Edge-futtatókörnyezet feladatai két kategóriába sorolhatók: kommunikációs és modul-kezelés. E két szerepkör két összetevőből, amelyek részei az IoT Edge-futtatókörnyezet végzi. A *IoT Edge hubot* felelős a kommunikációs, amíg a *IoT Edge-ügynök* helyez üzembe, és a modulok figyeli. 
...
>[!NOTE]
> IoT Edge hub az MQTT vagy AMQP használatával csatlakozó ügyfelek támogatja. Nem támogatja a HTTP Protokollt használó ügyfeleket. 
...
Egy üzenet jelenik meg, hogy regisztráljon egy visszahívást, amelyet a megadott bemeneti érkező üzeneteket dolgoz fel. A következő pseudocode regisztrálja az összes, a fogadott üzenetek feldolgozásához használandó függvény messageProcessor **input1**:
...
* **restartPolicy** – az IoT Edge-ügynök újraindítása modul. A lehetséges értékek:
   * `never` – Az IoT Edge-ügynök nem indul újra a modult.
   * `on-failure` – Ha összeomlik, a modul, az az IoT Edge-ügynök újraindítása. Ha a modul szabályszerűen álljon le, az IoT Edge-ügynök nem indítja újra azt.
   * `on-unhealthy` -Ha a modul összeomlik, vagy sérültnek, az IoT Edge-ügynök újraindítja.
   * `always` -Ha a modul összeomlik, nem megfelelő állapotúnak számít, vagy bármilyen módon leáll, az az IoT Edge-ügynök újraindítása. 
...
További információkért lásd: [megtudhatja, hogyan helyezhet üzembe modulokat, és ezekkel létesíthetnek útvonalat az IoT Edge](module-composition.md).   
