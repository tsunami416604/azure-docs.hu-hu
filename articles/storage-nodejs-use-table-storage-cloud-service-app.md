...
<tags ms.service="storage" ms.workload="storage" ms.tgt_pltfrm="na" ms.devlang="nodejs" ms.topic="article" ms.date="09/17/2014" ms.author="larryfr"></tags>

# Node.js Web Application using Storage

In this tutorial, you will extend the application created in the
[Node.js Web Application using Express][Node.js Web Application using Express] tutorial by using the Windows
...
3.  In the app.js file, scroll down to where you see the following line:
    ...
    Replace the above lines with the code shown below. This will initialize an instance of **Task** with a connection to your storage account. This is passed to the **TaskList**, which will use it to communicate with the Table service:
...
The **layout.jade** file in the **views** directory is used as a global template for other **.jade** files. In this step you will modify it to use [Twitter Bootstrap][Twitter Bootstrap], which is a toolkit that makes it easy to design a nice looking website.

1.  Download and extract the files for [Twitter Bootstrap][1]. Copy the **bootstrap.min.css** file from the **bootstrap\\dist\\css** folder to the **public\\stylesheets** directory of your tasklist application.
...
![A browser window displaying the My Task List page. The URL indicates the page is now being hosted on Azure.][The completed web page in internet explorer]
...
2.  To delete the service, call the following cmdlet:
  ...
  [Node.js Web
  Application]: http://www.windowsazure.com/en-us/develop/nodejs/tutorials/getting-started/
