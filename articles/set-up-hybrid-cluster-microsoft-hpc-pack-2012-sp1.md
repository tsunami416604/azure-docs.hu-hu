...
-   Installation files for HPC Pack 2012 R2, which is available free of charge. [Download][Download] the full installation package and copy the files to the head node computer or to a network location. Choose installation files in the same language as your installation of Windows Server.
...
Use the [Management Portal][Management Portal] to perform the following steps with your Azure subscription. These are needed so you can later deploy Azure nodes from the on-premises head node.
...
-   Create an Azure storage account
    ...
    > [WACOM.NOTE]Also make a note of your Azure subscription ID, which you will need later. Find this in your Azure [account information][account information].
...
1.  From the head node computer, sign in to the [Management Portal][Management Portal].
...
6.  In the **Installation Credentials** dialog box, type the credentials of the domain account that you used to install HPC Pack. Then click **OK**.
    ...
    ![Installation Credentials][Installation Credentials]
...
10. In the Create Node Template Wizard, do the following:
    ...
    c. On the **Provide Subscription Information** page, enter your Azure subscription ID (available in your Azure [account information][account information]). Then, in **Management certificate**, click **Browse** and select **Default HPC Azure Management.** Then click **Next**.
...
3.  On the **Specify New Nodes** page, select the Azure node template you created previously (called by default **Default AzureNode Template**). Then specify **2** nodes of size **Small**, and then click **Next**.
    ...
    ![Specify Nodes][Specify Nodes]

    For details about the available virtual machine sizes, see [Virtual Machine and Cloud Service Sizes for Azure][Virtual Machine and Cloud Service Sizes for Azure].
...
4.  To confirm that the role instances are running, in the [Management Portal][Management Portal], click **Cloud Services**, click the name of your cloud service, and then click **Instances**.
...
3.  The nodes transition to the **Stopping** state. After a few minutes, HPC Cluster Manager shows that the nodes are **Not-Deployed**.
    ...
    ![Not Deployed Nodes][Not Deployed Nodes]

4.  To confirm that the role instances are no longer running in Azure, in the [Management Portal][Management Portal], click **Cloud Services**, click the name of your cloud service, and then click **Instances**. No instances will be deployed in the production environment.
...
-   [HPC Pack 2012 R2 and HPC Pack 2012][HPC Pack 2012 R2 and HPC Pack 2012]
-   [Burst to Azure with Microsoft HPC Pack][detailed guidance]
-   [Deploying Applications to Azure Nodes][Deploying Applications to Azure Nodes]
-   [Microsoft HPC Pack in Azure VMs][Microsoft HPC Pack in Azure VMs]
