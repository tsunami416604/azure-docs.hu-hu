...
ms.locfileid: "73828526"
---
# <a name="tutorial-extract-structured-data-with-machine-learned-entities-in-language-understanding-luis"></a>Oktatóanyag: strukturált adatok kinyerése géppel megtanult entitásokkal Language Understanding (LUIS)
...
A gyakorlatban egy géppel megtanult entitást hoz létre, amely egy pizza-alkalmazás rendelését jelöli. A sorrendnek tartalmaznia kell a rendelés fullfil szükséges összes részt. Ennek elkezdéséhez az entitás tartalmazza az összes rendeléssel kapcsolatos szöveget, és kifejezetten lekéri a méretet és a mennyiséget. 

A `deliver one large cheese pizza` kimondása esetén a teljes teljességet megrendelésnek kell kibontania, majd ki kell bontania a `1` és a `large`is. 

További bontást is végezhet, például fehéráru vagy kéreg. Az oktatóanyag után érdemes felvennie ezeket az alösszetevőket a meglévő `Order` entitásba.
...
1. A **kezelés** szakasz **verziók** lapján válassza ki a verziót, majd válassza a **klónozás** elemet a verzió klónozásához, és nevezze el `mach-learn`. Ezután válassza a **kész** lehetőséget a klónozási folyamat befejezéséhez. Mivel a verzió neve az URL-útvonal részét képezi, a név nem tartalmazhat olyan karaktert, amely URL-címben nem érvényes.

    > [!TIP] 
    > Az alkalmazás módosítása előtt a klónozás ajánlott eljárás. Ha elkészült egy verzióval, exportálja egy verziót. JSON vagy. lu fájlként, és győződjön meg róla, hogy a verziókövetés.

1. Válassza a **Létrehozás** lehetőséget, hogy megtekintse a **Luis-alkalmazás** fő építőelemeit, a szándékokat.
...
1. A példa hosszúságú kimondott szöveg listán válassza ki a következő megjelölést. 
    ...
    > [!NOTE]
    > Az entitások nem mindig a teljes Kimondás. Ebben az esetben a `pickup` azt jelzi, hogy a rendszer hogyan kapja meg a sorrendet, így a sorrendnek a címkével ellátott entitás részét kell képeznie. 

1. Az **entitás típusának kiválasztása** mezőben válassza a **struktúra hozzáadása** lehetőséget, majd válassza a **tovább**lehetőséget. Az alösszetevők, például a méret és a mennyiség lehetővé tételéhez struktúra szükséges.
...
1. Az **új kifejezés létrehozása lista leíró** mezőjébe írja be a nevet `SizeDescriptor` majd adja meg a (z) `small`, `medium`és `large`értékeit. A **javaslatok** mező kitöltése után válassza a `extra large`lehetőséget, és `xl`. Az új kifejezések listájának létrehozásához kattintson a **kész** gombra. 
    ...
    Ez a kifejezési lista leírója segít a `Size` alösszetevő számára a mérettel kapcsolatos szavak megkeresésében, például a szó megadásával. A listának nem kell tartalmaznia minden méret szót, de tartalmaznia kell azokat a szavakat, amelyek várhatóan a méretet jelzik. 
...
1. Címkézze fel a `Order` entitást a fennmaradó hosszúságú kimondott szöveg és a méret entitással együtt. A szöveg szögletes zárójelei azt jelzik, hogy a címkével ellátott `Order` entitás és a `Size` entitás található.
    ...
    > [!CAUTION]
    > Hogyan kezelheti az olyan vélelmezett adatszolgáltatásokat, mint például a levél `a` egyetlen pizzát? Vagy `pickup` hiánya és `delivery`, hogy jelezze, hol várható a pizza? Vagy a méret hiánya az alapértelmezett méret vagy a kis vagy nagy érték jelzésére? Tekintse át az ügyfél-alkalmazás üzleti szabályainak részeként kezelt burkolt adatkezelési eljárást. 
...
1. A képzés után adjon hozzá egy új példát, amelyből megismerheti, hogy milyen jól ismeri a gép által megtanult entitást. 
    ...
    A pontozott hivatkozás a jóslatot jelzi. 

1. Az előrejelzés egy címkével rendelkező entitásba való módosításához válassza ki a sort, majd válassza az **entitások előrejelzésének megerősítése**lehetőséget.
    ...
    Ezen a ponton a gép által megtanult entitás működik, mert az új példán belül megtalálja az entitást. Ha például hosszúságú kimondott szöveg ad hozzá, ha az entitás nem megfelelően van előre jelezve, címkézze fel az entitást és az alösszetevőket. Ha az entitás előre jelezve van, győződjön meg róla, hogy az előrejelzések megerősítése. 

## <a name="add-prebuilt-number-to-app-to-help-extract-data"></a>Előre elkészített szám hozzáadása az alkalmazáshoz az adatok kinyeréséhez

A rendelési adatoknak tartalmaznia kell azt is, hogy a megrendelés hány eleme, például hány pizzát tartalmaz. Az adatok kinyeréséhez új, géppel megismert alösszetevőt kell hozzáadni a `Order`hoz, és az összetevőnek előre elkészített számra vonatkozó korláttal kell rendelkeznie. Az entitás előre elkészített számra való korlátozásával az entitás megkeresi és Kinyeri a számokat, hogy a szöveg számjegy, `2`vagy szöveg, `two`.

Először adja hozzá az előre elkészített számot az alkalmazáshoz. 
...
## <a name="label-example-utterance-with-subcomponent-for-quantity-to-teach-luis-about-the-entity"></a>Példa az alösszetevővel való Kimondás kifejezésre a LUIS az entitással kapcsolatos tanítani a mennyiségre
