...
1. A Azure Portal hozzon létre két **Language Understanding** kulcsot, amelyek közül az `West US` egyik a és egy `East US`. Használja az előző szakaszban létrehozott meglévő erőforráscsoportot `luis-traffic-manager`. 
...
1. Profil létrehozása a **[New-AzTrafficManagerProfile](https://docs.microsoft.com/powershell/module/az.trafficmanager/new-aztrafficmanagerprofile)** parancsmaggal
    ...
    A profil létrehozásához használja a következő parancsmagot. Ügyeljen rá, hogy módosítsa `appIdLuis` a `subscriptionKeyLuis`és a. A subscriptionKey az USA keleti régiója, LUIS kulcs. Ha az elérési út nem megfelelő, beleértve a LUIS-alkalmazás AZONOSÍTÓját és a végpont kulcsát, akkor a Traffic Manager `degraded` lekérdezés állapota, mert a forgalom kezelése nem tudja sikeresen kérni a Luis-végpontot. Győződjön meg arról, hogy `q` az `traffic-manager-east` érték az, hogy ez az érték látható a Luis-végpont naplófájljaiban.
...
1. Profil létrehozása a **[New-AzTrafficManagerProfile](https://docs.microsoft.com/powershell/module/az.TrafficManager/New-azTrafficManagerProfile)** parancsmaggal
    ...
    A profil létrehozásához használja a következő parancsmagot. Ügyeljen rá, hogy módosítsa `appIdLuis` a `subscriptionKeyLuis`és a. A subscriptionKey az USA keleti régiója, LUIS kulcs. Ha az elérési út nem megfelelő, beleértve a LUIS-alkalmazás AZONOSÍTÓját és a végpont kulcsát, akkor a `degraded` Traffic Manager lekérdezés állapota, mert a forgalom kezelése nem tudja sikeresen kérni a Luis-végpontot. Győződjön meg arról, hogy `q` az `traffic-manager-west` érték az, hogy ez az érték látható a Luis-végpont naplófájljaiban.
...
2. Az USA keleti gyermek-profiljának hozzáadása a **[AzTrafficManagerEndpointConfig](https://docs.microsoft.com/powershell/module/az.TrafficManager/Add-azTrafficManagerEndpointConfig)** és a **NestedEndpoints** típussal
    ...
    A sikeres válasz a következőhöz hasonlóan néz ki, és `child-endpoint-useast` az új végpontot tartalmazza:    
...
Annak ellenőrzéséhez, hogy a Traffic Manager-profilok működnek- `Online` e, a profiloknak ennek az állapotnak az állapotát a végpont lekérdezési útvonalán kell alapulnia. 

### <a name="view-new-profiles-in-the-azure-portal"></a>Új profilok megtekintése a Azure Portal
Az `luis-traffic-manager` erőforráscsoport erőforrásainak megtekintésével ellenőrizheti, hogy mindhárom profil létrejött-e.
