...
description: Néhány válasz a Bing Search API-k tartalmaz a Bing által szolgáltatott miniatűr rendszerképekre mutató URL-címeket, amelyek átméretezhetők és elhelyezhetők, és lekérdezési paramétereket is tartalmazhatnak.
...
A miniatűr átméretezéséhez a Bing javasolja, hogy csak egy `w` (szélesség) vagy `h` (magasság) lekérdezési paramétert határozzon meg a miniatűr URL-címében. Csak a magasság vagy a szélesség meghatározása lehetővé teszi, hogy a Bing fenntartsa a rendszerkép eredeti aspektusát. Itt adhatja meg a szélességet és a magasságot képpontban megadva. 

Ha például az eredeti miniatűr a 480x620:

`https://<host>/th?id=JN.5l3yzwy%2f%2fHj59U6XhssIQ&pid=Api&w=480&h=620`

Ha szeretné csökkenteni a méretét, állítsa a `w` paramétert egy új értékre (például `336`), és távolítsa el a `h` paramétert:
...
Ha más miniatűr képméretet szeretne kérni, távolítsa el az összes lekérdezési paramétert a miniatűr URL-címéből, kivéve a `id` és `pid` paramétereket. Ezután adja hozzá a `&w` (szélesség) vagy a `&h` (magasság) lekérdezési paramétert a kívánt képmérethez a képpontban, de nem mindkettőt. A Bing a kép eredeti oldalarányát fogja fenntartani. 
...
Ha a rendszerkép eredeti méreténél nagyobb méretű képet kér, a Bing szükség szerint fehér margót hoz létre a rendszerkép körül. Ha például a rendszerkép eredeti mérete 474x316, és a `&w` 500 értékre állítja, a Bing egy 500x333-képet ad vissza. Ennek a képnek 8,5 képpont fehér kitöltése lesz a felső és az alsó szélen, a bal és a jobb szélen pedig 13 képpont kitöltéssel.

Ha meg szeretné akadályozni, hogy a Bing ne adjon hozzá fehér kitöltést, ha a kért méret nagyobb, mint a rendszerkép eredeti mérete, állítsa 0-ra a `&p` lekérdezési paramétert. Ha például a fenti URL-címben a `&p=0` paramétert adja meg, a Bing egy 474x316-rendszerképet ad vissza egy 500x333-rendszerkép helyett:

`https://<host>/th?id=AMMS_92772df988...&w=500&p=0&pid=16.1`

Ha a `&w` és `&h` lekérdezési paramétereket is megadja, a Bing megtartja a kép méretarányát, és szükség szerint fehér kitöltés hozzáadását teszi szükségessé. Ha például a rendszerkép eredeti mérete 474x316, és a szélességi és a magassági paramétereket a 200x200 (`&w=200&h=200`) értékre állítja, a Bing egy olyan képet ad vissza, amely a fent és lenti 33 képpontos fehér kitöltést tartalmaz. Ha a `&p` lekérdezési paramétert is tartalmazza, a Bing egy 200x134-képet ad vissza.
...
Ha intelligens oldalarány-levágást kér (a `c` paramétert `7`re állítja), a Bing a kép oldalarányának közepétől kifelé haladva levágja a képet. Az érdeklődési régió a Bing által a legtöbb importálási rész részét képező rendszerkép területe. A következő példa egy érdekes régiót mutat be.  
...
Ha a vak arány levágását kéri (a `c` paramétert `4`), a Bing a következő szabályok használatával vágja le a rendszerképet.  
  
- Ha `(Original Image Width / Original Image Height) < (Requested Image Width / Requested Image Height)`, a rendszer a képet a bal felső sarokban, majd az alján levágja.  
- Ha `(Original Image Width / Original Image Height) > (Requested Image Width / Requested Image Height)`, a rendszer a képet a középpontból méri, és a bal és a jobb oldalon bevágja.  
