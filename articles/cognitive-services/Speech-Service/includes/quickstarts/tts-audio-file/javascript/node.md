...
```
## <a name="load-the-file-into-an-pullaudiooutputstream"></a>A fájl betöltése egy PullAudioOutputStream

A NodeJS a Speech SDK nem támogatja natív módon a fájlok közvetlen elérését, ezért megnyílik a fájl, és a használatával írja azt `PullAudioOutputStream` .
...
Az objektumok inicializálásához `SpeechSynthesizer` létre kell hoznia egy olyan konfigurációt, amely az előfizetési kulcsot és az előfizetési régiót használja. Szúrja be a következő kódot.
...
Most létre kell hoznia egy `AudioConfig` objektumot, amely az Ön számára mutat `PullAudioOutputStream` . Szúrja be a kódot közvetlenül a beszédfelismerési konfiguráció alá.
...
Most hozzuk létre az `SpeechSynthesizer` objektumot a `SpeechConfig` `AudioConfig` korábban létrehozott és objektumok használatával.
...
Az `SpeechSynthesizer` objektumból meg fogja hívni a `speakTextAsync()` metódust. Ez a módszer lehetővé teszi, hogy a beszédfelismerési szolgáltatás tudja, hogy szöveget küld a szintézishez.

A visszaadott eredményt vagy hibákat is írunk a konzolra, és végül lezárjuk a szintetizátort.
```JavaScript
