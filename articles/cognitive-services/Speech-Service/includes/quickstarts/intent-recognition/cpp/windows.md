...
Vegyünk fel egy olyan kódot, amely csontvázként működik a projekthez. Jegyezze fel, hogy létrehozta a nevű `recognizeIntent()`aszinkron metódust.
...
Szúrja be ezt a `recognizeIntent()` kódot a metódusba. Győződjön meg róla, hogy frissíti ezeket az értékeket:
...
Most hozzon létre egy `IntentRecognizer`. Szúrja be ezt a `recognizeIntent()` kódot a metódusba közvetlenül a beszédfelismerési konfiguráció alatt.
...
Szúrja be a `auto result = recognizer->RecognizeOnceAsync().get();`következő kódot:
