...
Ezután hozza létre `SpeechSynthesizer` az `speechConfig` objektumot és az `audioConfig` objektumot paraméterként. Ezután a beszédfelismerés végrehajtása és a fájlba való írás olyan egyszerű, mintha egy szöveges `SpeakText()` karakterláncot futtasson.
...
Ezután módosítania kell a beszédfelismerési kérést az XML-fájlra való hivatkozáshoz. A kérelem többnyire azonos, de a `SpeakText()` függvény használata helyett használja `SpeakSsml()`a parancsot. Ez a függvény egy XML-karakterláncot vár, ezért először létre kell hoznia egy függvényt egy XML-fájl betöltéséhez, és karakterláncként kell visszaadnia.
