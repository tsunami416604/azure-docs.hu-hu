...
Hozzon létre egy új Python-projektet a kedvenc IDE vagy szerkesztő használatával, vagy hozzon létre egy új mappát az asztalon. Másolja ezt a kódrészletet a projektbe vagy mappájába egy nevű fájlba `dictionary-lookup.py` .
...
A fordító globális végpontja a következőképpen van beállítva: `endpoint` . A `path` tulajdonság a `dictionary/lookup` útvonalat állítja be, és meghatározza, hogy a 3-as API-verziót szeretnénk használni.
