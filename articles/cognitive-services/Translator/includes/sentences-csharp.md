...
A `Program` osztályban hozzon létre egy nevű `BreakSentenceRequest()`új függvényt. Ez a függvény négy argumentumot `subscriptionKey`vesz `endpoint`igénybe: `inputText` `route`,, és.
...
Megfigyelheti, hogy `Main`a-ben `subscriptionKey`deklarálja `endpoint` `route`,, és a kiértékelni `breakSentenceText`kívánt szöveget.
