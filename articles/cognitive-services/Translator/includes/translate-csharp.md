...
Hozzon `Program` létre egy nevű `TranslateTextRequest()`aszinkron függvényt a osztályon belül. Ez a függvény négy argumentumot `subscriptionKey`vesz `host`igénybe: `inputText` `route`,, és.
...
Megfigyelheti, hogy `Main`a-ben Ön `subscriptionKey`deklarálja `host`, és `route`. Emellett a felhasználónak kell megadnia a bemenetet `Console.Readline()` , és hozzá kell `textToTranslate`rendelnie az értéket.
