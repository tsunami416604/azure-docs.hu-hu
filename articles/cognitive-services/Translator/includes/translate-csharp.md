...
A `Program` osztályban hozzon létre egy nevű `TranslateTextRequest()`aszinkron függvényt. Ez a függvény négy argumentumot `subscriptionKey`vesz `host`igénybe: `inputText` `route`,, és.
...
Az utolsó lépés a `TranslateTextRequest()` `Main` függvény hívása. Ebben a példában`de`a német (), az olasz (`it`), a japán (`ja`) és a thai (`th`) fordítást használjuk. Keresse `static void Main(string[] args)` meg és cserélje le a következő kódra:
