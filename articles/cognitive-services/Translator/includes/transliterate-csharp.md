...
Hozzon `Program` létre egy nevű `TransliterateTextRequest()`aszinkron függvényt a osztályon belül. Ez a függvény négy argumentumot `subscriptionKey`vesz `host`igénybe: `inputText` `route`,, és.
...
Megfigyelheti, hogy `Main`a-ben `subscriptionKey`deklarálja `host` `route`,, és a átbetűzés `textToTransliterate`szkriptet.
