...
Az első parancs két dolgot mutat be. Létrehoz egy új .NET-konzol alkalmazást, és létrehoz egy nevű `transliterate-sample`könyvtárat. A második parancs a projekt könyvtárára változik.
...
Nyissa meg a projektet, `transliterate-sample.csproj`majd nyissa meg a t. Győződjön meg arról `LangVersion` , hogy a értéke 7,1 vagy újabb. Ha nincs a nyelvi verzióhoz tartozó tulajdonságérték, adja hozzá a következő sorokat:
...
Hozzon `Program` létre egy nevű `TransliterateTextRequest()`aszinkron függvényt a osztályon belül. Ez a függvény négy argumentumot `subscriptionKey`vesz `endpoint`igénybe `route`: `inputText`,, és.
...
Megfigyelheti, hogy `Main`a-ben deklarálja `subscriptionKey` `endpoint` `route`,, és a átbetűzés `textToTransliterate`szkriptet.
