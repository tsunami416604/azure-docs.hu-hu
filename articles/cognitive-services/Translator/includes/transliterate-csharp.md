...
Az első parancs két dolgot tesz. Létrehoz egy új .NET konzolalkalmazást, és `transliterate-sample`létrehoz egy könyvtárat. A második parancs a projekt könyvtárára változik.
...
Ehhez a rövid útmutatóhoz C 7.1-es vagy újabb verzió szükséges. A projekt C# verziójának módosítására többféleképpen is módosíthatja. Ebben az útmutatóban bemutatjuk, hogyan `transliterate-sample.csproj` állíthatja be a fájlt. Az összes elérhető beállításról, például a Visual Studio nyelvének módosításáról a [C# nyelvi verzió kiválasztása című](https://docs.microsoft.com/dotnet/csharp/language-reference/configure-language-version)témakörben található.

Nyissa meg a `transliterate-sample.csproj`projektet, majd nyissa meg a programot. Győződjön `LangVersion` meg róla, hogy 7.1-es vagy újabb beállítással van beállítva. Ha a nyelvi verzióhoz nincs tulajdonságcsoport, adja hozzá a következő sorokat:
...
Az `Program` osztályon belül hozzon létre egy `TransliterateTextRequest()`aszinkron függvényt, amelynek neve . Ez a függvény négy `subscriptionKey` `endpoint`argumentumot foglal el: , , `route`, és `inputText`.
...
Az utolsó lépés `TransliterateTextRequest()` a `Main` függvény hívása. Ebben a mintában japánról latin ra írunk át. Keresse `static void Main(string[] args)` meg és cserélje ki ezzel a kóddal:
...
Észre fogod venni, hogy a `Main` `subscriptionKey`, `endpoint` `route`te deklarál , `textToTransliterate`, és a forgatókönyvet, hogy átírás.
