...
Hozzon létre egy új Go projektet kedvenc IDE vagy szerkesztő használatával. Ezután másolja a következő kódrészletet egy `transliterate-text.go` nevű fájlba a projektjében.
...
## <a name="create-a-function-to-transliterate-text"></a>Szöveg átírására létrehozott függvény létrehozása

Hozzunk létre egy függvényt a szöveg átírásához. Ez a függvény egyetlen argumentumot, a Translator Text előfizetési kulcsot vesz igénybe.
...
Ezután készítsük el az URL-címet. Az URL-cím `Parse()` a `Query()` és a módszerek használatával épül fel. Észre fogja venni, hogy a `Add()` paraméterek hozzá a módszerrel. Ebben a példában egy japán nyelvű szövegből hozunk létre latin betűs átiratot.

Másolja ezt a `transliterate` kódot a függvénybe.
...
Ezután hozzon létre egy névtelen struktúrát a kérelemtörzsszámára, és kódolja JSON-ként a segítségével. `json.Marshal()` Adja hozzá ezt `transliterate` a kódot a függvényhez.
...
Adja hozzá ezt `transliterate` a kódot a függvényhez a JSON-válasz dekódolásához, majd formázza és nyomtassa ki az eredményt.
