...
Hozzon létre egy új projektet a kedvenc IDE vagy szerkesztőjével, vagy egy új mappát, amely egy nevű fájlt használ az `transliterate-text.py` asztalon. Ezután másolja a kódrészletet a projektbe vagy fájlba:
...
A fordító globális végpontja a következőképpen van beállítva: `endpoint` . A `path` tulajdonság a `transliterate` útvonalat állítja be, és meghatározza, hogy a 3-as API-verziót szeretnénk használni.
