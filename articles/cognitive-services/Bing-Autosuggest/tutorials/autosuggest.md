---
title: Bing automatikus kiegészítési API-jával (Azure kognitív szolgáltatások) automatikus kiegészítési eredmények első |} Microsoft Docs
description: Bing automatikus kiegészítési használatát mutatja.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-autosuggest
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: f66af61ed06f4c6a7fe36cd44aa7781b71bb18ea
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 06/23/2018
ms.locfileid: "35349007"
---
# <a name="build-a-web-page-autosuggest-client"></a>A weblap automatikus javaslatokba ügyfél létrehozása

Az oktatóanyag azt fogja build egy weblap, amelyen lehetővé teszi a felhasználóknak a Bing automatikus kiegészítési API lekérdezése.

Ez az oktatóanyag a következőket mutatja be:

> [!div class="checklist"]
> - Egy egyszerű lekérdezést ellenőrizze a Bing automatikus kiegészítési API-val
> - Megjeleníti a lekérdezés eredményei

## <a name="prerequisites"></a>Előfeltételek

Kövesse az oktatóanyag együtt, a Bing automatikus kiegészítési API szükség van egy előfizetés kulcs. Ha még nincs fiókja, [regisztráljon egy ingyenes próbaverzióra](https://azure.microsoft.com/try/cognitive-services/?api=autosuggest-api).

## <a name="create-a-new-web-page"></a>Egy új weblap

Nyisson meg egy szövegszerkesztőt. Hozzon létre egy új fájlt, például autosuggest.html.

## <a name="html-header"></a>HTML-fejléc

Adja meg a HTML-fejléc információkat, és a parancsfájl a szakasz a következőképpen kezdődik.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Bing Autosuggest</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>getSubscriptionKey függvény

A getSubscriptionKey függvény a Bing automatikus kiegészítési API-kulcsot. Ez lekéri a helyi tároló (Ez azt jelenti, hogy a cookie-k) vagy bekéri a felhasználótól, ha szükséges.

A getSubscriptionKey függvény kezdődik, és a következőképpen deklarálja a cookie nevét.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-autosuggest-api-key";   // name used to store API key in key/value storage
```

A findCookie segítő függvény a megadott cookie-k; értékét adja vissza. Ha a cookie nem található, egy üres karakterláncot ad vissza.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

A getSubscriptionKeyCookie segítő függvény bekéri a felhasználótól a Bing automatikus kiegészítési API-kulcs értékét, és a kulcs értékét adja vissza.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

A getSubscriptionKeyLocalStorage segítő függvény először próbálja beolvasni a Bing automatikus kiegészítési API-kulcsot a megfelelő cookie megkeresésével. Ha a cookie nem található, a rendszer kérni fogja a felhasználótól a kulcs értékét. Ezután a kulcs értékét adja vissza.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

A getSubscriptionKey segítő függvény egy paramétert, **érvénytelenítéséhez**. Ha **érvénytelenítéséhez** van **igaz**, getSubscriptionKey törli a cookie-t, amely tartalmazza a Bing automatikus kiegészítési API-kulcsot. Ha **érvénytelenítéséhez** van **hamis**, getSubscriptionKey a Bing automatikus kiegészítési API-kulcs értékét adja vissza.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Térjen vissza a getSubscriptionKey segítő függvény a külső getSubscriptionKey függvény eredményeként. Zárja be a külső getSubscriptionKey függvény definíciója.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Segédfüggvények találhatók

A előtti segítő függvény a megadott szöveg-ként formázott rendelkező a [előtti](https://www.w3schools.com/tags/tag_pre.asp) HTML-címkében.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

A renderSearchResults függvény jeleníti meg a Bing automatikus kiegészítési API-t használó JSON megadott eredményeinek közérthető nyomtatás.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

A renderErrorMessage függvény jeleníti meg, a megadott hibaüzenetről és a hibakód.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingautosuggest-function"></a>bingAutosuggest függvény

A bingAutosuggest függvény hívása esetén minden alkalommal, amikor a felhasználó megadja a szöveg a HTML-űrlapot mezőben.
Két paraméter szükséges: a HTML tartalmát alkotnak mező, és a Bing automatikus kiegészítési API-kulcsot.

```html
function bingAutosuggest(query, key) {
```

Adja meg a Bing automatikus kiegészítési API-végpont, és egy XMLHttpRequest objektum, amely segítségével kérelmeket küldeni a végpont deklarálható.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/Suggestions";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?q=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Állítsa be a **Ocp-Apim-előfizetés-kulcs** fejlécének a Bing automatikus kiegészítési API-kulcs értékét.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

A végpont válaszát kezelni. Ha az állapot 200 (OK), az eredmények; megjelenítéséhez. Ellenkező esetben jeleníti meg a hiba adatait.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

A XMLHttpRequest objektum lehetséges hibaesemények kezelik.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Küldje el a kérést. Zárja be a bingAutosuggest függvény a **parancsfájl** címkét, és a **head** címke.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>HTML törzs

Betölti a weblap, ellenőrizze, a Bing automatikus kiegészítési API-kulcs vezetünk be azt a felhasználó értesítése, ha szükséges.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

A Bing emblémát.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Hozzon létre egy HTML-űrlaphoz egy szövegmezőben. Kezeli a **oninput** esemény és hívás a bingAutosuggest függvény, a szövegmezőbe, és a Bing automatikus kiegészítési API-kulcs tartalmát átadásakor.

```html
<form name="bing" oninput="return bingAutosuggest(this.query.value, getSubscriptionKey())">
    <h2>Autosuggest</h2>
    <input type="text" name="query" size="80" placeholder="Autosuggest" autocomplete=off>
</form>
```

Adja hozzá a HTML **div** tag, az eredmények megtekintése céljából használjuk. A korábban meghatározott JavaScript hivatkozik, ez **div** címke.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Mentse a fájlt.

## <a name="display-results"></a>Eredmények megjelenítéséhez

A weblap megnyitása a böngészőben. A parancssorba írja be a Bing automatikus kiegészítési API-előfizetés kulcs. Adja meg a lekérdezés (a például "hajóznak") a **automatikus javaslatokba** szövegmezőben. Írja be, mert a weblap automatikusan frissíti az automatikus javaslatokba eredmények megtekintése céljából.

```json
{
  "_type": "Suggestions",
  "queryContext": {
    "originalQuery": "sail"
  },
  "suggestionGroups": [
    {
      "name": "Web",
      "searchSuggestions": [
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=vheQSvKZylM3dlX_B9bQ8-hQEsEJo8zDD2y7H1nsBjE&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%2bcook%26FORM%3dUSBAPI&p=DevEx,5003.1",
          "displayText": "sailor brinkley cook",
          "query": "sailor brinkley cook",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=EStLqAfxGCa44Ur3jEMXBv-Qp-lXUSFJbkBfnUdKKDg&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%26FORM%3dUSBAPI&p=DevEx,5004.1",
          "displayText": "sailor brinkley",
          "query": "sailor brinkley",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=gvtP9TS9NwhajSapY2Se6y1eCbP2fq_GiP2n-cxi6OY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailrite%26FORM%3dUSBAPI&p=DevEx,5005.1",
          "displayText": "sailrite",
          "query": "sailrite",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=c0QOA_j6swCZJy9FxqOwke2KslJE7ZRmMooGClAuCpY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboats%2bfor%2bsale%26FORM%3dUSBAPI&p=DevEx,5006.1",
          "displayText": "sailboats for sale",
          "query": "sailboats for sale",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=mnMdREUH20SepmHQH1zlh9Hy_w7jpOlZFm3KG2R_BoA&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailing%2banarchy%26FORM%3dUSBAPI&p=DevEx,5007.1",
          "displayText": "sailing anarchy",
          "query": "sailing anarchy",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=0udadFl0gCTKCp0QmzQTXS3_y08iO8FpwsoKPHPS6kw&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboatdata%26FORM%3dUSBAPI&p=DevEx,5008.1",
          "displayText": "sailboatdata",
          "query": "sailboatdata",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=BTS0G6AakxntIl9rmbDXtk1n6rQpsZZ99aQ7ClE7dTY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsail%2bsand%2bpoint%26FORM%3dUSBAPI&p=DevEx,5009.1",
          "displayText": "sail sand point",
          "query": "sail sand point",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=quBMwmKlGwqC5wAU0K7n416plhWcR8zQCi7r-Fw9Y0w&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailflow%26FORM%3dUSBAPI&p=DevEx,5010.1",
          "displayText": "sailflow",
          "query": "sailflow",
          "searchKind": "WebSearch"
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>További lépések

> [!div class="nextstepaction"]
> [Használja ki és jelenítheti meg a követelményeknek](../UseAndDisplayRequirements.md)