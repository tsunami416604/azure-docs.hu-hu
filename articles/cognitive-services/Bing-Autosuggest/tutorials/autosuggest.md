---
title: 'Oktatóanyag: Automatikus javaslatok eredményei a Bing Autosuggest API használatával'
titleSuffix: Azure Cognitive Services
description: Ebben az oktatóanyagban létrehoz egy weblapot, amely lehetővé teszi a felhasználók számára a Bing Autosuggest API lekérdezését, és megjeleníti a lekérdezés eredményeit.
...
---
# <a name="tutorial-get-search-suggestions-on-a-web-page"></a>Oktatóanyag: Keresési javaslatok beszerezni egy weboldalon
...
> [!div class="checklist"]
> - Egyszerű lekérdezés intézése a Bing Autosuggest API-hoz
> - Lekérdezési eredmények megjelenítése

## <a name="prerequisites"></a>Előfeltételek

Ahhoz, hogy követni tudja az oktatóanyagot, szüksége lesz egy előfizetői azonosítóra a Bing Autosuggest API-hoz. Ha még nem rendelkezik, [hozzon létre egy Bing autosuggest erőforrást](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesBingAutosuggest-v7) az Azure Portalon. Próbakulcsot is [trial key](https://azure.microsoft.com/try/cognitive-services/)használhat.

## <a name="create-a-new-web-page"></a>Új weboldal létrehozása

Nyisson meg egy szövegszerkesztőt. Hozzon létre egy új fájlt, például autosuggest.html néven.
...
A getSubscriptionKey függvény visszaadja a Bing Autosuggest API-kulcsot. A függvény a kulcsot a helyi tárolóból (vagyis egy cookie-ból) kéri le, vagy szükség esetén a felhasználótól kéri el.
...
A getSubscriptionKey segítő függvény elkéri a felhasználótól a Bing Autosuggest API kulcsának értékét, és kiadja azt.
...
A getSubscriptionKeyLocalStorage segítő függvény először megpróbálja lekérni a Bing Autosuggest API kulcsát a megfelelő cookie megkeresésével. Ha a cookie nem található, elkéri a felhasználótól a kulcs értékét. Ezután visszaadja a kulcs értékét.
...
A renderSearchResults függvény a Bing Autosuggest API adott eredményeit jeleníti meg a JSON rendezett formátumában.
...
A bingAutosuggest függvény van meghívva minden alkalommal, amikor a felhasználó szöveget ad meg a HTML-űrlap mezőjében.
Ehhez két paraméterre van szüksége: a HTML-űrlap mezőjének tartalmára és a Bing Autosuggest API kulcsára.

```html
function bingAutosuggest(query, key) {
```

Adja meg a Bing Autosuggest API-végpontot, és deklarálja az XMLHttpRequest objektumot, amelyet a kérelmek küldésére fogunk használni. Használhatja az alábbi globális végpontot, vagy az [egyéni altartomány-végpontot,](../../../cognitive-services/cognitive-services-custom-subdomains.md) amely az azure-portálon jelenik meg az erőforráshoz.
...
Állítsa be az **Ocp-Apim-Subscription-Key** fejlécet a Bing Autosuggest API-kulcs értékére.
...
Küldje el a kérést. Zárja be a bingAutosuggest függvényt, a **script** és a **head** címkét.
...
A weboldal betöltésekor győződjön meg róla, hogy rendelkezik a Bing Autosuggest API kulcsával, és szükség esetén kérje el a felhasználótól.
...
Hozzon létre egy szövegmezővel rendelkező HTML-űrlapot. Kezelje `oninput` az eseményt, `bingAutosuggest()` és hívja meg a függvényt, adja át a szövegmező és a Bing Autosuggest API-kulcs tartalmát.
