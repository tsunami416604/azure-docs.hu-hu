...
|Fejléc|Leírás|  
|------------|-----------------|  
|fogadja el|Választható kérelemfejléc.<br /><br /> Az alapértelmezett adathordozó-típus az Application/JSON. Annak megadásához, hogy a válasz [JSON-ld-](https://json-ld.org/)t használ, állítsa az elfogadás fejlécet Application/ld + JSON értékre.|  
...
|----------|-----------|----------|--------------|
|<a name="count" />száma|A visszaadni kívánt eredmények száma, a `offset` paraméter által megadott indextől kezdődően.|Sztring|Nem|   
...
|<a name="offset"/>eltolás|Az `count` paraméter által megadott eredmények elindítására szolgáló index.|Egész szám|Nem|  
...
|<a name="error-subcode" />subCode|A hibát azonosító hibakód. Ha például `code` InvalidRequest, `subCode` lehet ParameterInvalid vagy ParameterInvalidValue. |Sztring|  
