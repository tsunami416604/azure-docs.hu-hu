---
title: Keresési lekérdezések küldése a Bing Visual Search API-ba
titleSuffix: Azure Cognitive Services
description: Ez a cikk a Bing Visual Search API-nak küldött kérelmek, valamint a válaszobjektum paramétereit és attribútumait ismerteti.
...
ms.locfileid: "75913066"
---
# <a name="sending-search-queries-to-the-bing-visual-search-api"></a>Keresési lekérdezések küldése a Bing Visual Search API-ba

Ez a cikk a Bing Visual Search API-nak küldött kérelmek, valamint a válaszobjektum paramétereit és attribútumait ismerteti. 

A képekkel kapcsolatos elemzéseket háromféleképpen szerezheti meg:

- Egy insights-jogkivonat, amely egy korábbi hívás egy korábbi hívás egy korábbi hívás a [Bing Image Search API-végpontok](https://docs.microsoft.com/rest/api/cognitiveservices/bing-images-api-v7-reference) használatával.
- Egy kép URL-címének elküldése.
- Kép feltöltése (bináris formátumban).

## <a name="bing-visual-search-requests"></a>Bing vizuális keresési kérései

Ha képtokent vagy URL-címet küld a Visual Search nek, a következő kódrészlet a POST törzsébe beillesztendő JSON-objektumot jeleníti meg:
...
Az `imageInfo` objektumnak tartalmaznia kell az `url` vagy az `imageInsightsToken` mezőt, de nem mindkettőt. Állítsa `url` be a mezőt egy interneten elérhető kép URL-címére. A legnagyobb támogatott képméret 1 MB.
...
A `cropArea` mező kitöltése nem kötelező. A vágási terület egy érintett terület bal felső és jobb alsó sarkát határozza meg. Adja meg az értékeket egy 0,0 és a 1,0 közötti tartományban. Az értékek a teljes szélesség vagy magasság százalékos értékei. Például a fenti példa a kép jobb oldalát jelöli meg releváns tartományként. Akkor használja, ha a releváns tartományra szeretné korlátozni a megállapítások lekérését.

A `filters` objektum egy webhelyszűrőt tartalmaz (lásd a `site` mezőt), amellyel egy adott tartományra korlátozhatja a hasonló képek és hasonló termékek eredményeit. Ha például a kép egy Surface Book, `site` `www.microsoft.com`beállíthatjuk a beállítását.
...
Az attribútumot a `enableEntityData` fejlécben `true` beállíthatja a feltöltött kép fő entitásáról szóló részletes információkért, beleértve a webes hivatkozásokat és a hozzárendelési információkat. Ez a `false` mező alapértelmezés szerint.
...
Ha helyi képet tölt fel, a következő kódrészlet a POST törzsébe bekell illesztenie az űrlapadatokat. Az űrlapadatoknak `Content-Disposition` tartalmazniuk kell a fejlécet. A `name` paraméter értéke „image” legyen, a `filename` paraméter értéke viszont bármilyen sztring lehet. A `Content-Type` fejléc bármely általánosan használt képmimé típusra beállítható. Az űrlap tartalma a kép bináris adatai. A legnagyobb feltölthető képméret 1 MB. A legnagyobb szélesség vagy magasság legfeljebb 1500 képpont lehet.
...
A következő kódrészlet bemutatja, hogyan adható meg a feltöltött kép érdeklődési területe:
...
A következő kódrészlet egy teljes képelemzési kérelmet jelenít meg, amely egy képtokent és az érdeklődési régiót ad át. Az insights token egy korábbi hívásból származik a /images/search címre:
...
## <a name="bing-visual-search-responses"></a>Bing vizuális keresési válaszai


[!INCLUDE [cognitive-services-bing-url-note](../../../../includes/cognitive-services-bing-url-note.md)]

Ha vannak elérhető megállapítások a képhez, a válasz egy vagy több `tags` mezőt tartalmaz, bennük a megállapításokkal. A `image` mező a bemeneti lemezkép elemzési jogkivonatát tartalmazza:
...
Az alapértelmezett elemzések listáját az [Alapértelmezett elemzési címke című témakörben olvashatja.](../default-insights-tag.md)

A többi címke más megállapításokat tartalmaz, amelyeket a felhasználó érdekesnek találhat. Például ha a kép szöveget tartalmaz, az egyik címke egy TextResults megállapítást tartalmazhat, benne a felismert szöveggel. Vagy ha a Bing felismer egy entitást (azaz egy kulturálisan jól ismert/népszerű személyt, helyet vagy dolgot) a képen, az egyik címke azonosíthatja az entitást. A Visual Search emellett egy, a bemeneti képből kinyert kifejezéskészletet (címkék) is visszaad. Ezek a címkék lehetővé teszik a felhasználók számára, hogy felfedezzék a képen található fogalmakat. Például ha a bemeneti kép egy híres sportolót ábrázol, az egyik címke lehet a Sport, amely sporttal kapcsolatos képekre mutató hivatkozásokat tartalmaz.
...
Ha a kép olyan szöveget tartalmaz, amelyet a szolgáltatás felismer, az egyik címke egy TextResults megállapítást (műveletet) fog tartalmazni. A betekintés a `displayName` felismert szöveget tartalmazza:
...
Ha a kép egy felismert entitást tartalmaz, például egy kulturálisan jól ismert/népszerű személyt, helyet vagy dolgot, akkor a címkék egyike tartalmazhat entitásbetekintést. A `mainEntity` `data` és a mezők `enableEntityData` csak akkor `Content-Type` érhetők el, `true`ha a fejlécben lévő attribútum beállítása .
...
- [Oktatóanyag: Vizuális keresés egyoldalas webalkalmazás létrehozása](../tutorial-bing-visual-search-single-page-app.md)
