...
A `filters` objektum egy webhelyszűrőt tartalmaz (lásd a `site` mezőt), amellyel egy adott tartományra korlátozhatja a hasonló képek és hasonló termékek eredményeit. Ha például a rendszerkép egy felszíni könyvből áll, akkor a `site` `www.microsoft.com`következőre állíthatja:.
...
| <a name="setlang" />setLang  | A felhasználói felület sztringjeihez használni kívánt nyelv. A nyelvet az ISO 639-1 kétbetűs nyelvi kóddal kell megadnia. Az angol nyelv nyelvkódja például az EN. Az alapértelmezett érték az EN (angol).<br /><br /> Bár nem kötelező, javasoljuk, hogy mindig adja meg a nyelvet. A `setLang` paramétert általában az `mkt` által meghatározott nyelvre kell állítani, hacsak a felhasználó nem szeretné más nyelven megjeleníteni a felhasználói felület sztringjeit.<br /><br /> Ez a paraméter és az [](#acceptlanguage)Accept-Language&mdash; fejléc kölcsönösen kizárják egymást, ne adja meg mindkettőt.<br /><br /> A felhasználóifelület-sztring egy olyan sztring, amelyet feliratként használnak a felhasználói felületen. A JSON-válaszobjektumok tartalmaznak néhány felhasználóifelület-sztringet. A válaszobjektumokban található, a Bing.com tulajdonságaira mutató hivatkozások is a megadott nyelvet alkalmazzák. | Sztring | Nem   |

## <a name="headers"></a>Fejlécek

Az alábbiakban azok a fejlécek láthatók, amelyeket a kérelmének meg kell adnia. A `Content-Type` és `Ocp-Apim-Subscription-Key` a fejlécek az egyetlen szükséges fejlécek, de a következőket `User-Agent`is `X-MSEdge-ClientID`tartalmaznia `X-MSEdge-ClientIP`kell: `X-Search-Location`,, és.
...
Megadhatja a fejlécben `enableEntityData` `true` található attribútumot a feltöltött rendszerkép fő entitásának részletes adataihoz, beleértve a webes és a hozzárendelési információkra mutató hivatkozásokat is. Alapértelmezés szerint ez `false` a mező.
...
Ha a kép olyan szöveget tartalmaz, amelyet a szolgáltatás felismer, az egyik címke egy TextResults megállapítást (műveletet) fog tartalmazni. Az Insight a `displayName` felismert szöveget tartalmazza:
...
Ha a rendszerkép olyan felismert entitást tartalmaz, mint a kulturálisan ismert/népszerű személy, hely vagy dolog, akkor az egyik címke tartalmazhatja az entitások betekintését. A `mainEntity` és `data` a mezők csak akkor érhetők el `enableEntityData` , ha a `Content-Type` fejlécben lévő attribútum `true`a következőre van beállítva:.
