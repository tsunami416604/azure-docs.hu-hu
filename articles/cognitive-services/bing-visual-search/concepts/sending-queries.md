...
Az `imageInfo` objektumnak tartalmaznia kell az `url` vagy az `imageInsightsToken` mezőt, de nem mindkettőt. Állítsa a `url` mezőt egy internetről elérhető rendszerkép URL-címére. A legnagyobb támogatott képméret 1 MB.
...
A `filters` objektum egy webhelyszűrőt tartalmaz (lásd a `site` mezőt), amellyel egy adott tartományra korlátozhatja a hasonló képek és hasonló termékek eredményeit. Ha például a rendszerkép egy felületi könyv, beállíthatja, hogy a `site` `www.microsoft.com`.
...
| Név | Value (Díj) | Type (Típus) | Szükséges |
| --- | --- | --- | --- |
...
Minden kérelemnek tartalmaznia kell a `Content-Type` fejlécet. A fejlécet a következőre kell beállítani: `multipart/form-data; boundary=\<boundary string\>`, ahol a \<határ karakterlánca\> egy egyedi, átlátszatlan karakterlánc, amely az űrlapadatok határát azonosítja. Például: `boundary=boundary_1234-abcd`.

Ha képtokent vagy URL-címet küld Visual Search, a következő kódrészlet a bejegyzés törzsében szerepeltetni kívánt űrlapadatokat jeleníti meg. Az űrlapon szerepelnie kell a `Content-Disposition` fejlécnek, és a `name` paramétert "knowledgeRequest" értékre kell állítania. A `imageInfo` objektum részletes ismertetését lásd a kérelemben.
...
A fejlécben megadhatja a `enableEntityData` attribútumot is `true` a feltöltött rendszerkép fő entitásának részletes információit, beleértve a webes és a hozzárendelési információkra mutató hivatkozásokat is. Alapértelmezés szerint ez a mező `false`.
...
Ha egy helyi rendszerképet tölt fel, az alábbi kódrészlet a bejegyzés törzsében szerepeltetni kívánt űrlapadatokat jeleníti meg. Az űrlap adattípusának tartalmaznia kell a `Content-Disposition` fejlécet. A `name` paraméter értéke „image” legyen, a `filename` paraméter értéke viszont bármilyen sztring lehet. Előfordulhat, hogy a `Content-Type` fejléc bármely gyakran használt képmime-típusra van beállítva. Az űrlap tartalma a rendszerkép bináris adatok. A legnagyobb feltölthető képméret 1 MB. A legnagyobb szélesség vagy magasság legfeljebb 1500 képpont lehet.
