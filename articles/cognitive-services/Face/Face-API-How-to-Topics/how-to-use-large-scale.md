...
Az Face API ügyféloldali kódtár használatakor az előfizetési kulcsot és az előfizetési végpontot a FaceClient osztály konstruktorán keresztül adja át a rendszer. Például:
...
Ahogy az `TrainLargeFaceList()`ban is látható, a végtelen betanítási állapotának késleltetését jelző idő ezredmásodpercben. A több arcot tartalmazó LargeFaceList esetén nagyobb időköz választása csökkenti a meghívások számát és költségét. Szabja testre az időintervallumot a LargeFaceList várt kapacitásának megfelelően.

Ugyanez a stratégia a LargePersonGroup is vonatkozik. Ha például a LargePersonGroup 1 000 000 személyre van betanítva, `timeIntervalInMilliseconds` lehet 60 000, amely egy 1 perces intervallum.
...
Tegyük fel, hogy a `TrainLargeFaceList`hoz hasonló `TrainLargePersonGroup` függvény. A LargePersonGroup önálló képzésének tipikus implementációja, ha a `System.Timers` [`Timer`](https://msdn.microsoft.com/library/system.timers.timer(v=vs.110).aspx) osztályát hívja meg:
