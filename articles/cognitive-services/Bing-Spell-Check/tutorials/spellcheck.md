---
title: Bing helyesírás ellenőrizze API-val (a Microsoft Azure kognitív szolgáltatások) helyesírás ellenőrizze eredmények első |} Microsoft Docs
description: Bemutatja, hogyan használható a Bing helyesírás-ellenőrzés.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-spell-check
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: 4e4cdbb8a3d6ab01888d8f273083155c33eb06c1
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 06/23/2018
ms.locfileid: "35347527"
---
# <a name="build-a-web-page-spell-check-client"></a>A weblap helyesírás-ellenőrzés ügyfél létrehozása

Ebben az oktatóanyagban azt fogja létrehozni egy weblap, amelyen lehetővé teszi a felhasználóknak a Bing helyesírás-ellenőrzése API lekérdezése.

Ez az oktatóanyag a következőket mutatja be:

> [!div class="checklist"]
> - Egy egyszerű lekérdezést teremtsen a Bing helyesírás-ellenőrzése API
> - Megjeleníti a lekérdezés eredményei

## <a name="prerequisites"></a>Előfeltételek

Kövesse az oktatóanyag együtt, a Bing helyesírás-ellenőrzése API szükség van egy előfizetés kulcs. Ha még nincs fiókja, [regisztráljon egy ingyenes próbaverzióra](https://azure.microsoft.com/try/cognitive-services/?api=spellcheck-api).

## <a name="create-a-new-web-page"></a>Egy új weblap

Nyisson meg egy szövegszerkesztőt. Hozzon létre egy új fájlt, például spellcheck.html.

## <a name="add-html-header"></a>HTML-fejléc hozzáadása

Adja meg a HTML-fejléc információkat, és a parancsfájl a szakasz a következőképpen kezdődik.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8"> 
    <title>Bing Spell Check</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>getSubscriptionKey függvény

Bing helyesírás ellenőrizze API-kulcsot a getSubscriptionKey függvény. Ez lekéri a helyi tároló (Ez azt jelenti, hogy a cookie-k) vagy bekéri a felhasználótól, ha szükséges.

A getSubscriptionKey függvény kezdődik, és a következőképpen deklarálja a cookie nevét.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-spell-check-api-key";   // name used to store API key in key/value storage
```

A findCookie segítő függvény a megadott cookie-k; értékét adja vissza. Ha a cookie nem található, egy üres karakterláncot ad vissza.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

A getSubscriptionKeyCookie segítő függvény bekéri a felhasználótól a Bing helyesírás ellenőrizze API-kulcs értékét, és a kulcs értékét adja vissza.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

A getSubscriptionKeyLocalStorage segítő függvény először próbálja beolvasni a Bing helyesírás ellenőrizze API-kulcsot a megfelelő cookie megkeresésével. Ha a cookie nem található, a rendszer kérni fogja a felhasználótól a kulcs értékét. Ezután a kulcs értékét adja vissza.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

A getSubscriptionKey segítő függvény egy paramétert, **érvénytelenítéséhez**. Ha **érvénytelenítéséhez** van **igaz**, getSubscriptionKey törli a cookie-t, amely tartalmazza a Bing helyesírás ellenőrizze API-kulcsot. Ha **érvénytelenítéséhez** van **hamis**, getSubscriptionKey a Bing helyesírás ellenőrizze API-kulcs értékét adja vissza.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Térjen vissza a getSubscriptionKey segítő függvény a külső getSubscriptionKey függvény eredményeként. Zárja be a külső getSubscriptionKey függvény definíciója.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Segédfüggvények találhatók

A előtti segítő függvény a megadott szöveg-ként formázott rendelkező a [előtti](https://www.w3schools.com/tags/tag_pre.asp) HTML-címkében.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

A renderSearchResults függvény jeleníti meg a Bing helyesírás ellenőrizze API-t használó JSON megadott eredményeinek közérthető nyomtatás.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

A renderErrorMessage függvény jeleníti meg, a megadott hibaüzenetről és a hibakód.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingspellcheck-function"></a>bingSpellCheck függvény

A bingSpellCheck függvény hívása esetén minden alkalommal, amikor a felhasználó megadja a szöveg a HTML-űrlapot mezőben.
Két paraméter szükséges: a HTML tartalmát alkotnak mező, és a Bing helyesírás ellenőrizze API-kulcsot.

```html
function bingSpellCheck(query, key) {
```

Adja meg a Bing helyesírás ellenőrizze API-végpont, és egy XMLHttpRequest objektum, amely segítségével kérelmeket küldeni a végpont deklarálható.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/spellcheck/";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?mode=proof&mkt=en-US&text=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Állítsa be a **Ocp-Apim-előfizetés-kulcs** fejlécének a Bing helyesírás ellenőrizze API-kulcs értékét.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

A végpont válaszát kezelni. Ha az állapot 200 (OK), az eredmények; megjelenítéséhez. Ellenkező esetben jeleníti meg a hiba adatait.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

A XMLHttpRequest objektum lehetséges hibaesemények kezelik.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Küldje el a kérést. Zárja be a bingSpellCheck függvény a **parancsfájl** címkét, és a **head** címke.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>HTML törzs

Betölti a weblap, ellenőrizze, a Bing helyesírás ellenőrizze API-kulcs vezetünk be azt a felhasználó értesítése, ha szükséges.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

A Bing emblémát.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Hozzon létre egy HTML-űrlaphoz egy szövegmezőben. Kezeli a **onsubmit** esemény és hívás a bingSpellCheck függvény, a szövegmezőbe, és a Bing helyesírás ellenőrizze API-kulcs tartalmát átadásakor.

```html
<form name="bing" onsubmit="return bingSpellCheck(this.query.value, getSubscriptionKey())">
    <h2>Spell Check</h2>
    <input type="text" name="query" size="80" placeholder="Spell Check" autocomplete=off>
</form>
```

Adja hozzá a HTML **div** tag, az eredmények megtekintése céljából használjuk. A korábban meghatározott JavaScript hivatkozik, ez **div** címke.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Mentse a fájlt.

## <a name="display-results"></a>Eredmények megjelenítéséhez

A weblap megnyitása a böngészőben. A parancssorba írja be a Bing helyesírás ellenőrizze API előfizetés kulcs. Adjon meg egy lekérdezést (például "Hollo, wlrd!") a **helyesírás-ellenőrzés** szövegmezőbe, és nyomja le az **Enter**. A weblap majd a lekérdezés eredményeit jeleníti meg.

```json
{
  "_type": "SpellCheck",
  "flaggedTokens": [
    {
      "offset": 0,
      "token": "Hollo",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "Hello",
          "score": 0.856629936217145
        },
        {
          "suggestion": "Hollow",
          "score": 0.816717853225633
        }
      ]
    },
    {
      "offset": 7,
      "token": "wlrd",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "world",
          "score": 0.856629936217145
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>További lépések

> [!div class="nextstepaction"]
> [Használja ki és jelenítheti meg a követelményeknek](../UseAndDisplayRequirements.md)
