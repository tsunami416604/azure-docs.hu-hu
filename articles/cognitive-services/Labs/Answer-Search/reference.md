...
ms.locfileid: "68705335"
---
# <a name="project-answer-search-v7-reference"></a>A Project answer Search v7-referenciája

A Bing answer SearchAPI egy lekérdezési paramétert ad `searchResponse` vissza `answerType`, amely `entities`a következőket adja vissza: `facts` vagy. 

A válasz keresési API-t használó alkalmazások a lekérdezési paraméterben lévő előzetes verziójú URL-címmel küldenek kéréseket a végpontnak.  A kérelemnek tartalmaznia kell `q=searchTerm` a paramétert és a *OCP-APIM-Subscription-Key* fejlécet.   
...
|----------|-----------|----------|--------------|  
|<a name="mkt" />mkt|A piac, ahonnan az eredmények származnak. <br /><br />A lehetséges piaci értékek listáját a piaci kódok részben tekintheti meg.<br /><br /> **MEGJEGYZÉS:** Az URL-cím előnézeti API jelenleg csak az en-us piacot és nyelvet támogatja.<br /><br />|Karakterlánc|Igen|  
...
|Elem|Leírás|Type|  
|-------------|-----------------|----------|  
|<a name="error-code" />kód|A hiba kategóriáját azonosító hibakód. A lehetséges kódok listáját lásd: hibakódok [](#error-codes).|Karakterlánc|  
...
|Name (Név)|Érték|Type|  
|----------|-----------|----------|  
|_type|Egy Type mutató, amely a LicenseAttribution értékre van állítva.|Karakterlánc|  
|licenc|A licenc, amely alatt a tartalom használható.|[Engedély](#license)|  
|licenseNotice|A célként megadott mező mellett megjelenítendő licenc. Például: "szöveg a CC-BY-SA licenc alapján".<br /><br /> A `license` mezőben a licenc nevét és URL-címét használva hozzon létre egy hiperhivatkozást a webhelyre, amely leírja a licenc részleteit. Ezután cserélje le a licenc nevét a `licenseNotice` karakterláncban (például cc-by-SA) az imént létrehozott hiperhivatkozásra.|Sztring|  
...
|Name (Név)|Value|Type|  
|----------|-----------|----------|  
|name|A közzétevő neve.|Karakterlánc|  
...
|Name (Név)|Value|Type|  
|----------|-----------|----------|
|name|Az oldal címe, nem feltétlenül a HTML-cím|Sztring|
...
|primaryImageOfPage/contentUrl|Az előzetes verzióban szerepeltetni kívánt reprezentatív kép URL-címe|Karakterlánc| 
...
|Name (Név)|Érték|Type|  
|----------|-----------|----------|  
|_type|Írja be a következőt: SearchResponse.|Karakterlánc|  
|WebPage|Az előnézetet definiáló JSON-objektum|Karakterlánc|  
...
|Name (Név)|Value|Type|  
|----------|-----------|----------|  
|_type|Egy Type mutató, amely a TextAttribution értékre van állítva.|Karakterlánc|  
|text|A jóváírás szövege.<br /><br /> A szöveges hozzárendelés az entitás egészére vonatkozik, és az entitások megjelenítését követően azonnal megjelenik. Ha több szöveges vagy hivatkozáshoz tartozó olyan szabály van, amely nem határoz meg célt, fűzze össze őket, és jelenítse meg őket "adatok forrása:" címkével.|Karakterlánc| 
