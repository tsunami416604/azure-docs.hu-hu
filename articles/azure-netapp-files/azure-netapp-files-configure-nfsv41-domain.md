...
A gyökér-leképezés alapértelmezett értéke `nobody` a felhasználó, mert a nfsv4 névleképezője tartománya `localdomain`a következőre van beállítva:. Ha a Azure NetApp Files NFSv 4.1-es kötetet root-ként csatlakoztatja, a fájlok engedélyei a következőképpen jelennek meg:  
...
Ahogy a fenti példában is látható, a felhasználónak `file1` kell lennie `root`, de alapértelmezés szerint `nobody` le van képezve.  Ez a cikk bemutatja, hogyan állíthatja be `file1` a felhasználót `root`a következőre:.  

## <a name="steps"></a>Lépések 

1. Szerkessze `/etc/idmapd.conf` a fájlt az NFS-ügyfélen.   
    Szüntesse meg `#Domain` `defaultv4iddomain.com`a sor megjegyzését (azaz távolítsa `#` el a sort), és módosítsa az értéket `localdomain` a következőre:. 
...
4. Indítsa újra `rpcbind` a szolgáltatást a gazdagépen`service rpcbind restart`(), vagy egyszerűen indítsa újra a gazdagépet.
...
A példa azt mutatja, `nobody` hogy `root`a felhasználó/csoport mostantól a verzióról a verzióra módosult.
...
A következő példában három meglévő `Host1` tesztelési felhasználói fiók van (`testuser01`, `testuser02`, `testuser03`): 

![Eredményül kapott konfiguráció a NFSv 4.1 számára](../media/azure-netapp-files/azure-netapp-files-nfsv41-host1-users.png)

Vegye `Host2`figyelembe, hogy a teszt felhasználói fiókok nem lettek létrehozva, de a kötet mindkét gazdagépre csatlakoztatva van:
