...
Mielőtt elkezdené, a szerkesztőnek el kell végeznie legalább az első Build-oktatóanyagot az Azure IoT Central-alkalmazás létrehozásához: adjon [meg egy új eszközt](tutorial-define-device-type.md) (kötelező).

Telepítse a [Node. js](https://nodejs.org/) -verziót 8.0.0 vagy újabb verzióra a fejlesztői gépen. Az `node --version` a parancssorban futtatva ellenőrizhető a verzió. A Node.js az operációs rendszerek széles körében elérhető.
...
2. Figyelje meg, hogy a **csatlakoztatott légkondicionáló** eszköz sablonja a **Device Explorer**kiválasztott. A sablont használó, valós légkondicionáló eszköz csatlakoztatásának megkezdéséhez válassza a **+** , majd a **valós**lehetőséget:
...
1. Adja hozzá a következő változó deklarációkat a fájlhoz. Cserélje le a helyőrzőket `{your Scope ID}`, `{your Device ID}`és `{your Primary Key}`re a korábban feljegyzett eszköz-csatlakoztatási adatokkal:
