...
1. Szerkessze a letöltött `app.py` fájlt, és cserélje le a `DEVICE_ID`, `SCOPE_ID`és `PRIMARY/SECONDARY device KEY` helyőrzőket a korábban jegyzett kapcsolatok értékekre. Mentse a módosításokat.
...
1. Az Azure IoT Central alkalmazásban láthatja, hogy a málna PI-on futó kód hogyan működik együtt az alkalmazással:
    ...
    * A **Beállítások** lapon módosíthatja a málna PI beállításait, például a feszültséget és a ventilátor sebességét. Ha a málna PI felismeri a változást, a beállítás **szinkronizált**értékként jelenik meg.
