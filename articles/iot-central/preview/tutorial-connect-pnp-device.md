...
1. Adja hozzá az alábbi sort a `CMakeLists.txt` fájl alján, hogy a rendszer a következő fordításkor tartalmazza az eszköz kód stub mappáját:

    ```txt
    add_subdirectory(central_app/devkit_device)
    ```

1. Hozzon létre egy `cmake` mappát a `azure-iot-sdk-c` mappában, és a parancssorban navigáljon a mappához:
