...
- 您可以變更診斷組態設定，透過 **診斷組態** Visual Studio 中的對話方塊。 此設定會儲存在稱為 diagnostics.wadcfgx (diagnostics.wadcfg 在 Azure SDK 2.4 或更早版本中) 的檔案。 或者，您可以直接修改組態檔。 如果您手動更新檔案，組態變更會在您下一次將雲端服務部署至 Azure 或在模擬器中執行服務時生效。

- 使用 **雲端總管** 或 **伺服器總管** 在 Visual Studio 來變更執行中雲端服務或虛擬機器的診斷設定。
...
- .Cscfg 檔案中的連接字串也會使用 Azure 入口網站來存取診斷資料，因此它可以出現在 **監視** ] 索引標籤。 若要在入口網站中顯示詳細監視資料，必須要有連接字串。
...
- **組態 Api 已過時** – Azure SDK 2.4 或較早版本，會提供以程式設計方式設定診斷，但在 Azure SDK 2.5 和更新版本已被取代。 如果診斷組態目前以程式碼定義，您將需要在移轉專案中從頭開始進行這些設定才能讓診斷保持運作。 Azure SDK 2.4 的診斷組態檔是 diagnostics.wadcfg，而 diagnostics.wadcfgx 是 Azure SDK 2.5 及更新版本的診斷組態檔。

- **雲端服務應用程式的診斷只能在角色層級設定，而不是在執行個體層級。**

- **每次部署您的應用程式時，更新診斷設定** – 如果您從 [伺服器總管] 中變更您的診斷組態，然後重新部署您的應用程式，這可能會導致同位檢查的問題。

- **在 Azure SDK 2.5 和更新版本中，診斷組態檔中，不在程式碼中設定損毀傾印** – 如果您有程式碼中設定損毀傾印，您必須手動將組態從程式碼至組態檔中，因為損毀傾印不會在移轉期間移轉至 Azure SDK 2.6。
...
1. 在您感興趣之角色的捷徑功能表，選擇 [ **屬性**, ，然後選擇 [ **組態** ] 索引標籤中的角色 **屬性** 視窗。
...
1. 在 **Create Storage Connection String** 對話方塊方塊中，指定是否要使用 Azure 儲存體模擬器，Azure 訂閱，連接或手動輸入的認證。
...
1.  **磁碟配額，以 mb 為單位** 方塊指定要配置儲存體帳戶中的診斷資料的空間。 您可以任意變更預設值。

1. 在您想要收集診斷資料的每個索引標籤上，選取其 **啟用傳輸 <log type>** 核取方塊。例如，如果您想要收集應用程式記錄檔中，選取 **啟用傳輸應用程式記錄檔** ] 核取方塊 **應用程式記錄檔** ] 索引標籤。 此外，請指定每個診斷資料類型所需的其他資訊。 請參閱章節 **設定診斷資料來源** 稍後本主題中的每個索引標籤上的設定資訊。

1. 您已啟用您想要的所有診斷資料收集之後，請選擇 **確定** ] 按鈕。
...
1. 在 **伺服器總管**, 選擇 [Azure] 節點，然後連接到 Azure 訂閱時，如果您未連線。

1. 展開 **虛擬機器** 節點。 您可以建立新的虛擬機器，或選取一個已經存在的虛擬機器。

1. 在您感興趣的虛擬機器的捷徑功能表，選擇 [ **設定**。 隨即顯示 [虛擬機器組態] 對話方塊。
...
1. 如果尚未安裝，請新增 Microsoft Monitoring Agent 診斷延伸模組。 這個延伸模組可讓您收集 Azure 虛擬機器的診斷資料。 在已安裝的延伸模組清單中，選擇 [選取可用的延伸模組] 下拉式功能表，然後選擇 Microsoft Monitoring Agent 診斷。
    ...
    >[AZURE.NOTE] 其他診斷延伸模組可供您的虛擬機器。 如需詳細資訊，請參閱 Azure VM 延伸模組和功能。

1. 選擇 **新增** 按鈕來新增延伸模組及檢視其 **診斷組態** 對話方塊。

1. 選擇 **設定** ] 按鈕以指定儲存體帳戶，然後選擇 [ **確定** ] 按鈕。

    每個索引標籤 (除了 **一般** 和 **記錄檔目錄**) 表示您可以收集的診斷資料來源。
...
1. 在您想要收集診斷資料的每個索引標籤上，選取其 **啟用傳輸 <log type>** 核取方塊。
    ...
    例如，如果您想要收集應用程式記錄檔中，選取 **啟用傳輸應用程式記錄檔** ] 核取方塊 **應用程式記錄檔** ] 索引標籤。 此外，請指定每個診斷資料類型所需的其他資訊。 請參閱章節 **設定診斷資料來源** 稍後本主題中的每個索引標籤上的設定資訊。

1. 您已啟用您想要的所有診斷資料收集之後，請選擇 **確定** ] 按鈕。

1. 儲存更新的專案。

    您會看到訊息 **Microsoft Azure 活動記錄** 已經更新虛擬機器的視窗。

## 設定診斷資料來源

啟用診斷資料收集之後，您可以準確選擇您要收集的資料來源和所收集的資訊。 以下是在索引標籤清單 **診斷組態** ] 對話方塊中，以及每個組態選項的意義。
...
若要追蹤未列出的效能計數器，輸入它使用建議的語法，然後選擇 **新增** ] 按鈕。 虛擬機器上的作業系統會決定您可以追蹤的效能計數器。 如需有關語法的詳細資訊，請參閱 [指定計數器路徑](https://msdn.microsoft.com/library/windows/desktop/aa373193.aspx)。

### 基礎結構記錄檔

RemoteAccess 模組和 RemoteForwarder 模組，如果您想要擷取基礎結構記錄檔，其中包含 Azure 診斷基礎結構的相關資訊，請選取 **啟用傳輸基礎結構記錄檔** 核取方塊。 您可以在記錄檔傳輸至儲存體帳戶時，藉由變更傳輸期間 (分鐘) 值來增加或減少分鐘數。
...
此外，您可以從任何資料夾擷取記錄檔。 只要指定的路徑， **絕對目錄中的記錄檔** 區段，然後選擇 [ **新增目錄** ] 按鈕。 記錄檔將會擷取至指定的容器。
...
1. 您可以在 Visual Studio 產生的報告或儲存體帳戶的資料表中檢視診斷資料。 若要檢視資料在報表中，開啟 **雲端總管** 或 **伺服器總管**, ，開啟您感興趣之角色節點的捷徑功能表，然後選擇 **檢視診斷資料**。
    ...
    選擇 **重新整理** 連結到立即更新資料，或選擇在間隔 **自動重新整理** 下拉式清單方塊會自動更新的資料。 若要匯出資料時發生錯誤，請選擇 [ **匯出至 CSV** ] 按鈕以建立以逗號分隔值檔案，您可以在試算表中開啟。

    在 **雲端總管** 或 **伺服器總管**, ，開啟與部署相關聯的儲存體帳戶。
...
1. 在 [伺服器總管] 中，展開 **雲端服務** 節點，然後展開節點，以找出的角色或您想要調查的執行個體或兩者。

    ![設定診斷](./media/vs-azure-tools-diagnostics-for-cloud-services-and-virtual-machines/IC748913.png)

1. 在執行個體節點或角色節點的捷徑功能表，選擇 [ **更新診斷設定**, ，然後選擇您想要收集的診斷設定。

    組態設定的詳細資訊，請參閱 **設定診斷資料來源** 本主題中。 如需有關如何檢視診斷資料的資訊，請參閱 **檢視診斷資料** 本主題中。

    如果您變更中的資料收集 **伺服器總管**, ，這些變更都有效，除非您完全重新部署您的雲端服務。 如果您使用預設發佈設定，變更不會遭到覆寫，因為預設發佈設定會更新現有的部署，而不會執行完整的重新部署。 若要確定在部署階段清除的設定，請移至 **進階設定** 在 [發行精靈] 索引標籤並清除 **部署更新** 核取方塊。 當您在清除該核取方塊時重新部署，設定將還原為.wadcfgx (或.wadcfg) 檔案中透過角色的屬性編輯器進行的設定。 如果您更新您的部署，Azure 會保留舊的設定。
...
時間戳記位於裝載雲端服務之資料中心的當地時區。 使用下列記錄資料表中的三個時間戳記資料行。
  ...
  - **Precisetimestamp 無條件** 是 ETW 事件的時間戳記。 也就是從用戶端記錄事件的時間。

  - **時間戳記** 是 precisetimestamp 無條件捨去到上傳頻率界限。 因此如果您的上傳頻率為 5 分鐘，而事件時間為 00:17:12，則 TIMESTAMP 是 00:15:00。

  - **時間戳記** 是位於 Azure 資料表中建立實體的時間戳記。

**收集診斷資訊時如何管理成本？**

預設設定 (**記錄層級** 設 **錯誤** 和 **傳輸期間** 設 **1 分鐘**) 設計用來降低成本。 如果您收集更多診斷資料或縮短傳輸週期，會增加運算成本。 請勿收集超過需求量的資料，也別忘了在您不需要時停用資料收集。 您可以隨時再次啟用，即使在執行階段亦然，如上一節中所示。
