...
1. Küldjön le egy `hello-world:latest` rendszerképet a beállításjegyzékbe, vagy helyettesítse be egy másik, választott tesztelési képet.
...
Ha le szeretne tiltani egy adatmegőrzési szabályt egy beállításjegyzékben, futtassa az az [ACR config adatmegőrzési frissítés][az-acr-config-retention-update] parancsot, és állítsa be `--status disabled`:
