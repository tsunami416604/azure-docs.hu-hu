---
title: A DPM-kiszolgáló előkészítése a számítási feladatok biztonsági mentésére az Azure-ba
description: Bevezetés a DPM-beli Azure Recovery Services-tárolóba való biztonsági mentéséhez.
author: kasinh
manager: vvithal
ms.service: backup
ms.topic: conceptual
...
Támogatott fájltípusok | Ezek a fájltípusok biztonsági mentést készíthetnek Azure Backup használatával: Titkosított (csak teljes biztonsági mentés); Tömörített (növekményes biztonsági másolatok támogatottak); Ritka (növekményes biztonsági másolatok támogatottak); Tömörítve és ritka (Ritkaként kezelve).
...
Tár tárolója | A Azure Backup-tárolóba felhasználható adatmennyiségnek nincs korlátja, de az adatforrások (például virtuális gépek vagy adatbázisok) mérete nem haladhatja meg a 54400 GB-ot.
...
9. A **Backup**-tárolóban keresse meg és válassza ki a letöltött tároló hitelesítő adatait tartalmazó fájlt.

    ![Tároló hitelesítő adatai](../../includes/media/backup-install-agent/DPM_SetupOnlineBackup_Credentials.jpg)

10. A **szabályozás beállítása beállításnál**engedélyezheti a sávszélesség-szabályozást a biztonsági mentésekhez. Megadhatja a munkaórák és napok megadásának korlátait.

    ![Szabályozás beállítása](../../includes/media/backup-install-agent/DPM_SetupOnlineBackup_Throttling.png)

11. A **helyreállítási mappa beállítása**területen válasszon egy helyet, amelyet az Adathelyreállítás során használhat.
...
12. A **titkosítási beállításban** állítson be vagy adjon meg egy jelszót.
...
13. Kattintson a **regisztráció** gombra a DPM-kiszolgáló a tárolóhoz való regisztrálásához.

Miután a kiszolgáló regisztrálása sikeresen megtörtént a tárolóban, és most már készen áll a Microsoft Azurere való biztonsági mentésre.
