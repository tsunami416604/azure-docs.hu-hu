...
4. Operációsrendszer-lemez és adatlemezek csatolása. Ebben a lépésben a példák a különböző felügyelt és a Virtuálisgép-konfigurációk titkosítva biztosít. A Virtuálisgép-konfiguráció megfelelő a példát követve.
   ...
   * **Felügyelt és nem titkosított virtuális gépek** – felügyelt lemez gyors létrehozásához a blob storage-ból, és ezután csatlakoztassa a lemezeket kell felügyelt nem titkosított virtuális gépek esetén. Részletes információkért lásd: a cikk [adatlemez csatolása a PowerShell használatával Windows virtuális gép](../virtual-machines/windows/attach-disk-ps.md). Az alábbi mintakód bemutatja, hogyan az adatlemezek felügyelt nem titkosított virtuális gépek számára.
   ...
   * **Felügyelt és a titkosított virtuális gépek (csak a blokktitkosítási kulcsot)** – felügyelt lemez gyors létrehozásához a blob storage-ból, és ezután csatlakoztassa a lemezeket kell felügyelt titkosított virtuális gépek (rendelkeznek BEk-KEL csak használatával titkosított) esetén. Részletes információkért lásd: a cikk [adatlemez csatolása a PowerShell használatával Windows virtuális gép](../virtual-machines/windows/attach-disk-ps.md). Az alábbi mintakód bemutatja, hogyan az adatlemezek felügyelt, titkosított virtuális gépek számára.
