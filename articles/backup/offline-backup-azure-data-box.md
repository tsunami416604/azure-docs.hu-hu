...
\* * Ha várhatóan több mint 80 TB kezdeti biztonsági mentési adatra van szüksége egyetlen MARS-kiszolgálóról, forduljon a [AskAzureBackupTeam@microsoft.comhoz ](mailto:AskAzureBackupTeam@microsoft.com).
...
1. Töltse le a PSExec-t a <https://download.sysinternals.com/files/PSTools.zip>ról a kiszolgálóra, amelyen telepítve van a MARS-ügynök.
1. Nyisson meg egy rendszergazda jogú parancssort, és futtassa a következő parancsot a *PSExec. exe fájlt* tartalmazó könyvtárral, amely az aktuális könyvtár.
...
1. Válassza **az eszköz** észlelése lehetőséget a **Data Box eszköz észlelése** lapon. Ezzel a művelettel a MARS-ügynök bekeresi a helyileg csatlakoztatott Azure Data Box lemezeket, és felismeri azokat.
    ...
    >
    >Ha például a lemez elérési útja `\\mydomain\myserver\disk1\`, és a *Disk1* a *PageBlob*nevű könyvtárat tartalmazza, akkor a Mars-ügynök varázsló lapján megadott elérési út `\\mydomain\myserver\disk1\`.
    >
    >Ha [Azure Data Box 100 – TB eszközt állít be](#set-up-azure-data-box-devices), adja meg a `\\<DeviceIPAddress>\<StorageAccountName>_PageBlob`t az eszköz hálózati elérési útjaként.
...
1. A *CBUICurr* fájlban görgessen az utolsó sorra, és ellenőrizze, hogy a probléma megegyezik-e a következő hibaüzenettel: `Unable to create an Azure AD application credential in customer's account. Exception: Update to existing credential with KeyId <some guid> is not allowed`.
...
Ha egy másik kiszolgáló nem rendelkezik kapcsolat nélküli előkészítéssel, és a `AzureOfflineBackup_<Azure User Id>` alkalmazástól nem függ más kiszolgáló, törölje az alkalmazást. Válassza ki **Azure Portal** > **Azure Active Directory** > **Alkalmazásregisztrációk**.

>[!NOTE]
> Ellenőrizze, hogy a `AzureOfflineBackup_<Azure User Id>` alkalmazás nem rendelkezik-e más kapcsolat nélküli előkészítéssel, és hogy nem függ-e az alkalmazástól más kiszolgálók. Lépjen a **beállítások** > **kulcsok** elemre a **nyilvános kulcsok** szakaszban. Nem adhat hozzá további nyilvános kulcsokat. Tekintse meg a következő képernyőképet a hivatkozáshoz.
>
...
1. Nyissa meg a **számítógép-tanúsítvány alkalmazása** > **személyes** lapot, és keresse meg a `CB_AzureADCertforOfflineSeeding_<ResourceId>`nevű tanúsítványt.
...
5. Ugrás a beállításjegyzék- *számítógép \ HKEY_LOCAL_MACHINE \Software\microsoft\windows Azure Backup\Config\CloudBackupProvider.* Kattintson a jobb gombbal a **CloudBackupProvider**elemre, és adjon hozzá egy új karakterlánc-értéket `AzureADAppCertThumbprint_<Azure User Id>`néven.
...
Ha bármilyen kérdése vagy pontosítása merülne fel, forduljon a [AskAzureBackupTeam@microsoft.comhoz ](mailto:AskAzureBackupTeam@microsoft.com).
