...
### SQL Server
DPM 會以本機系統身分執行，並備份 SQL Server 資料庫，它需要在該帳戶上具備 SQL Server 的 sysadmin 權限。 設定為 NT AUTHORITY\SYSTEM *sysadmin* 您想要備份的 SQL Server 上。
...
>[AZURE.NOTE] 您必須在 SharePoint 伺服器陣列系統管理員認證變更時重新執行 ConfigureSharePoint.exe。
...
2. 在 **選取保護群組類型** 畫面 **建立新保護群組** 精靈，請選取 **伺服器** 按一下 **下一步**。

    ![選擇保護群組類型](./media/backup-azure-backup-sharepoint/select-protection-group-type.png)

3. 在 **選擇群組成員** 畫面上，選取您想要保護，並按一下 [SharePoint 伺服器的核取方塊 **下一步**

    ![選擇群組成員](./media/backup-azure-backup-sharepoint/select-group-members2.png)

    >[AZURE.NOTE] 安裝 DPM 代理程式的情況下，您可以看到在精靈中的伺服器。 DPM 也會顯示其結構。 由於已執行 ConfigureSharePoint.exe，DPM 會與 SharePoint VSS 寫入器及其對應的 SQL 資料庫通訊，並辨識 SharePoint 伺服器陣列結構 (相關聯的內容資料庫和任何對應的項目)。

4. 在 **選擇資料保護方式** 畫面上，輸入名稱 *保護群組* 然後選取您偏好 *保護方法*。 按一下 [ **下一步**。

    ![選擇資料保護方式](./media/backup-azure-backup-sharepoint/select-data-protection-method1.png)

    >[AZURE.NOTE] 選擇磁碟保護有助於符合簡短復原時間的復原時間目標。 相較於磁帶，Azure 是符合成本效益的長期保護目標。 如需詳細資訊請參閱此 [文章](https://azure.microsoft.com/documentation/articles/backup-azure-backup-cloud-as-tape/)

5. 在 **指定短期目標** 畫面上，選取您偏好 *保留範圍* 並找出想要進行備份。

    ![指定短期目標](./media/backup-azure-backup-sharepoint/specify-short-term-goals2.png)

    >[AZURE.NOTE] 由於復原大部分通常所需的資料少於五天的舊的資料，我們選取磁碟上 5 天的保留範圍，並確保在此範例中的非生產時段進行備份。

6. 檢閱保護群組配置的存放集區磁碟空間，然後按一下 **下一步**。

7. 針對每個保護群組，DPM 會配置磁碟空間來儲存和管理複本。 此時，DPM 必須建立所選資料的複本。 選取您如何及何時建立複本，然後按一下 **下一步**。

    ![選擇複本的建立方式](./media/backup-azure-backup-sharepoint/choose-replica-creation-method.png)

    >[AZURE.NOTE] 若要確保不影響網路流量，請選取生產時段之外的時間。

8. DPM 可為複本執行一致性檢查，以確保資料完整性。 有兩個可用的選項。 您可以定義執行一致性檢查的排程，或在複本變得不一致時，即讓 DPM 自動執行一致性檢查。 選取您慣用的選項，然後按一下 **下一步**。

    ![一致性檢查](./media/backup-azure-backup-sharepoint/consistency-check.png)

9. 在 **指定線上保護資料** 畫面上，選取您想要保護，並按一下 [SharePoint 伺服器陣列 **下一步**。

    ![DPM SharePoint Protection1](./media/backup-azure-backup-sharepoint/select-online-protection1.png)

10. 在 **指定線上備份排程** 畫面中選取您慣用的排程，然後按一下 **下一步**。

    ![Online_backup_schedule](./media/backup-azure-backup-sharepoint/specify-online-backup-schedule.png)

    >[AZURE.NOTE] DPM 允許 2 個每日備份至 Azure 的不同時間。

11. 根據備份排程選取，請在 **指定線上保留原則** 畫面上，選取每日、 每週、 每月和每年備份點的保留原則。

    ![Online_retention_policy](./media/backup-azure-backup-sharepoint/specify-online-retention.png)

    >[AZURE.NOTE] DPM 會使用不同的備份點的祖父位父親的兒子保留配置，可以選擇不同的保留原則。

12. 類似於磁碟，需要在 Azure 中建立初始參考點複本。 選取您慣用的選項來建立初始備份至 Azure，然後按一下 **下一步**。

    ![Online_replica](./media/backup-azure-backup-sharepoint/online-replication.png)

13. 檢閱您選取的設定在 **摘要** 頁面上，按一下 [ **建立群組**。 建立保護群組之後，您會看到成功訊息。
...
3. 您可以搜尋的 SharePoint *復原 SharePoint 項目* 使用萬用字元型搜尋復原點範圍內。

    ![DPM SharePoint Protection6](./media/backup-azure-backup-sharepoint/dpm-sharepoint-protection7.png)

4. 從搜尋結果中選取適當的復原點，以滑鼠右鍵按一下，然後選取 **復原**

5. 您也可以瀏覽不同的復原點，並選取要復原的資料庫或項目。 選取 **日期 > 復原時間** 然後選取正確 **資料庫 > SharePoint 伺服器陣列 > 復原點 > 項目**。

    ![DPM SharePoint Protection7](./media/backup-azure-backup-sharepoint/dpm-sharepoint-protection8.png)

6. 以滑鼠右鍵按一下項目，然後選取 **復原** 開啟 **復原精靈]**。 按一下 [ **下一步**。

    ![檢閱復原選項](./media/backup-azure-backup-sharepoint/review-recovery-selection.png)

7. 選取您要執行，並且按一下的復原類型 **下一步**。

    ![復原類型](./media/backup-azure-backup-sharepoint/select-recovery-type.png)

    >[AZURE.NOTE] 上述範例顯示項目復原到原始的 SharePoint 網站。

8. 選取 **復原程序** 您想要使用。
    - 選取 **復原並且使用復原伺服器陣列** 如果 SharePoint 伺服器陣列並未變更，且正在還原的復原點相同。
    - 選取 **使用復原伺服器陣列復原** 之後建立復原點，在 SharePoint 伺服器陣列已變更。
...
10. 選取 **指定復原選項** 和安全性設定套用至 SharePoint 伺服器陣列，或套用復原點的安全性設定。 按一下 [ **下一步**。
...
12. 現在選取 **監視** 索引標籤中 **DPM 系統管理員主控台** 檢視 *狀態* 的復原。
...
4. 按一下顯示在 DPM 中的 SharePoint 物件 **復原** 索引標籤，以取得內容資料庫結構。 以滑鼠右鍵按一下 > **復原**。
