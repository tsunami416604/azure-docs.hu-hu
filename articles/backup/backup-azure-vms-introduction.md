...
>[AZURE.NOTE] 針對 Linux 虛擬機器，檔案一致的備份是可行的因為 Linux 沒有相當的平台 」 至 vss。
...
### 磁碟數量
備份程序會竭盡所能耗用資源，目標是盡速完成備份。 不過，所有的 I/O 作業都受到 *單一 Blob 的目標輸送量*, ，其具有每秒 60 MB 的限制。 為了加速備份程序，請嘗試執行 VM 的每個磁碟備份 *平行*。 所以如果 VM 有 4 個磁碟，Azure 備份便會嘗試平行備份全部 4 個磁碟。 基於此原因，是一項最重要因素，決定備份資料傳輸退出客戶儲存體帳戶 **的磁碟編號** 備份儲存體帳戶。

### 備份排程
另一個因素會影響效能是 **備份排程**。 如果您設定備份相同的所有 Vm 的時間，然後要備份的磁碟數目 *平行* 都會增加-Azure 備份將會嘗試盡可能多個磁碟備份。 因此，為了減少從儲存體帳戶輸出的備份流量，一種作法是確保在不同的時段備份 VM，時間不要重疊。
...
| ---------------- | ---------- |
| 初始備份 | 160 Mbps |
| 增量備份 (DR) | 640 Mbps <br><br> 如果有很多需要備份的磁碟上分散的變換資料，可以大幅置此輸送量。 |
...
## 受保護的執行個體如何計算？
透過 Azure Backup 備份的 azure 虛擬機器受限於 [Azure 備份定價](http://azure.microsoft.com/pricing/details/backup/)。 受保護的執行個體計算根據 *實際* 大小的虛擬機器，也就是在虛擬機器--排除 「 資源磁碟 」 的所有資料的總和。

您是 *不* 計費基礎支援連接至虛擬機器，每個資料磁碟的大小上限，而實際的資料儲存在資料磁碟上。 同樣地，備份儲存體帳單也是根據 Azure 備份所儲存的資料計費，也就是每個復原點所儲存的實際資料總和。
 ...
 *實際* 虛擬機器的大小，在此情況下為 17 GB + 30 GB + 0 GB = 47 GB。 這將是受保護的執行個體大小，每月帳單的計費基礎。 當虛擬機器的資料量增加時，用於計費之受保護的執行個體大小也會隨之變更。

第一次成功備份完成後才會開始計費。 儲存體和受保護的執行個體也會在此同時開始計費。 只要有持續計費 *任何備份資料使用 Azure 備份儲存* 虛擬機器。 如果保留備份資料，執行 [停止保護] 作業將不會停止計費。

將會停止計費指定的虛擬機器，才會停止保護 *和* 在刪除全部備份資料。 無使用中之備份工作時 (已停止保護)﹐受保護的執行個體大小將根據上一次成功備份時的虛擬機器大小，進行每月計費。
