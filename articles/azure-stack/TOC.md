# Áttekintés
## [Mi az Azure Stack?](azure-stack-poc.md)
## [Újdonságok](azure-stack-whats-new.md)
## [A legfontosabb jellemzők és fogalmak](azure-stack-key-features.md)
## [POC-architektúra](azure-stack-architecture.md)

# Bevezetés
## [Első feladatok](azure-stack-first-scenarios.md)
## [Virtuális gép kiépítése](azure-stack-provision-vm.md)
## [Tárfiók létrehozása](azure-stack-provision-storage-account.md)
## [Kapcsolódás parancssori felülettel](azure-stack-connect-cli.md)
## [Kapcsolódás a PowerShell-lel](azure-stack-connect-powershell.md)
## Az Azure Stack POC üzembe helyezése
### [Üzembe helyezési előfeltételek](azure-stack-deploy.md)
### [Üzembe helyezés](azure-stack-run-powershell-script.md)
### [Csatlakozás az Azure Stack POC-hez](azure-stack-connect-azure-stack.md)
### [Az Azure Stack ismételt üzembe helyezése](azure-stack-redeploy.md)
## [Gyakori kérdések](azure-stack-faq.md)

# Útmutató
## [Felhasználói engedélyek kezelése](azure-stack-manage-permissions.md)
## [Azure Stack-bérlő hozzáadása](azure-stack-add-new-user-aad.md)
## Szolgáltatások ajánlása a bérlőknek
### [Kvóták beállítása](azure-stack-setting-quotas.md)
### [Csomag létrehozása](azure-stack-create-plan.md)
### [Ajánlat létrehozása](azure-stack-create-offer.md)
### [Előfizetés egy ajánlatra](azure-stack-subscribe-plan-provision-vm.md)
### [Ajánlat delegálása az Azure Stackben](azure-stack-delegated-provider.md)
## Sablonok üzembe helyezése
### [Sablonok áttekintése](azure-stack-arm-templates.md)
### [Sablonok üzembe helyezése a portállal](azure-stack-deploy-template-portal.md)
### [Sablonok üzembe helyezése a PowerShell-lel](azure-stack-deploy-template-powershell.md)
### [Sablonok üzembe helyezése a Visual Studióval](azure-stack-deploy-template-visual-studio.md)
### [Sablonok üzembe helyezése a parancssorral](azure-stack-deploy-template-command-line.md)
## Tárterület tervezése és felügyelete
### [Áttekintés](azure-stack-storage-overview.md)
### [Tárfiókok kezelése](azure-stack-manage-storage-accounts.md)
### [Különbségek és szempontok](azure-stack-acs-differences-tp2.md)
## Hálózat tervezése és felügyelete
### [iDNS az Azure Stackhez](azure-stack-understanding-dns-in-tp2.md)
### [Helyek közötti VPN-kapcsolatok ismertetése](azure-stack-create-vpn-connection-one-node-tp2.md)
### [Nyilvános IP-címek használatának megtekintése](azure-stack-viewing-public-ip-address-consumption-in-tp2.md)
## Saját tartalom hozzáadása az Azure Stackhez
### [Piactér áttekintése](azure-stack-marketplace.md)
### [Piactéri termék létrehozása és közzététele](azure-stack-create-and-publish-marketplace-item.md)
### [Egyéni virtuálisgép-rendszerkép hozzáadása](azure-stack-add-vm-image.md)
## [Frissítések felügyelete](azure-stack-updates.md)
## A Key Vault biztonságos értékeinek felügyelete
### [Bevezetés](azure-stack-kv-intro.md)
### [Első lépések](azure-stack-kv-getting-started.md)
### [Virtuális gép üzembe helyezése Key Vault-jelszóval](azure-stack-kv-deploy-vm-with-secret.md)
### [Virtuális gép létrehozása Key Vault-tanúsítvánnyal](azure-stack-kv-push-secret-into-vm.md)
### [Key Vault-mintaalkalmazás](azure-stack-kv-sample-app.md)
## Számlázás és költséghelyi elszámolás – használat követése
### [A számlázás és a költséghelyi elszámolás áttekintése](azure-stack-billing-and-chargeback.md)
### [Szolgáltatói erőforrás-használati API](azure-stack-provider-resource-api.md)
### [Bérlői erőforrás-használati API](azure-stack-tenant-resource-usage-api.md)
### [Használat – gyakori kérdések](azure-stack-usage-related-faq.md)
## [Linux rendszerű virtuális gépek üzembe helyezése](azure-stack-linux.md)
## SQL vagy MySQL típusú erőforrás-szolgáltató üzembe helyezése
### [MySQL-adatbázisok használata PaaS-ként az Azure Stackben](azure-stack-mysql-rp-deploy-short.md)
### [Az SQL Server erőforrás-szolgáltató adapter üzembe helyezése az Azure Stackben](azure-stack-sql-rp-deploy-long.md)
### [Az SQL Database használata az Azure Stackben](azure-stack-sql-rp-deploy-short.md)

## Webalkalmazások tervezése és üzembe helyezése
### [Az Azure Stack Web Apps áttekintése](azure-stack-webapps-overview.md)
### [Az Azure Stack Web Apps megismerése előtt](azure-stack-webapps-before-you-get-started.md)
### [A MySQL erőforrás-szolgáltató üzembe helyezése az Azure Stackben a Web Apps használatához](azure-stack-mysql-rp-deploy-long.md)
### [Web Apps: További webes feldolgozói szerepkörök hozzáadása](azure-stack-webapps-add-worker-roles.md)
### [Web Apps erőforrás-szolgáltató hozzáadása az Azure Stackhez](azure-stack-webapps-deploy.md)

## [Hibaelhárítás](azure-stack-troubleshooting.md)
### [Egy üzemelő példány újrafuttatása egy sikertelen lépés után](azure-stack-rerun-deploy.md)
### [A Web Apps erőforrás-szolgáltatója](azure-stack-webapps-troubleshoot-known-issues.md)

# Referencia
## [Eszközök](azure-stack-tools-paas-services.md)

# Erőforrások
## [MSDN-fórum](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=AzureStack)  
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-stack)





<!--HONumber=Nov16_HO2-->


