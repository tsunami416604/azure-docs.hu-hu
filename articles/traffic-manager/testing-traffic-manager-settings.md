...
-**使用的工具，可讓您將 DNS 名稱解析為 IP 位址** 並顯示該位址。 您正查看公司網域名稱是否解析成設定檔中端點的 IP 位址。 他們的解析方法應與流量管理員設定檔的負載平衡方法一致。 如果您是使用執行 Windows 的電腦，您可以從命令或 Windows PowerShell 命令提示字元使用 Nslookup.exe 工具。 可讓您「挖掘」IP 位址的其他公開可用工具在網際網路上隨手可得。
...
1-保留所有端點開啟狀態。
2-使用單一用戶端。
3-使用 Nslookup.exe 工具或類似的公用程式要求您公司網域名稱的 DNS 解析。
4-確定取得的已解析 IP 位址適用於您的主要端點。
5-關閉您的主要端點或移除監視檔案，所以流量管理員會認為它已關閉。
6-等候流量管理員設定檔的 DNS 存留時間 (TTL)，再加上 2 分鐘。 例如，若您的 DNS TTL 為 300 秒 (5 分鐘)，則您必須等待 7 分鐘。
...
1-保留所有端點開啟狀態。
2-使用單一用戶端。
3-使用 Nslookup.exe 工具或類似的公用程式要求您公司網域的 DNS 解析。
4-確定取得的 IP 位址是清單中的其中之一。
5-排清您的 DNS 用戶端快取並不斷重複步驟 3 和 4。 您應該可看到為每個端點傳回不同的 IP 位址。 然後，此程序將會重複執行。
