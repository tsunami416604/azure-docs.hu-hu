---
title: 'Oktatóanyag: Adatok streamelése az Azure Databricksbe az Event Hubs használatával | Microsoft Docs'
description: Tudnivalók arról, hogyan tölthet be streamadatokat a Twitterről, és olvashatja valós időben őket az Azure Databricks és az Event Hubs segítségével.
...
Ebben az oktatóanyagban összekapcsolunk egy adatbetöltési rendszert az Azure Databricksszel, és valós idejű adatokat streamelünk egy Apache Spark-fürtbe. Az Azure Event Hubs használatával beállítunk egy valós idejű adatbetöltési rendszert, és csatlakoztatjuk az Azure Databrickshez, amely feldolgozza a beérkező üzeneteket. A valós idejű adatstream érdekében Twitter API-kkal töltünk be tweeteket az Event Hubsba. Ha az adatok megérkeztek az Azure Databricksbe, elemzési feladatok futtatásával részletesebben elemezhetőek. Ebben az oktatóanyagban azokat a tweeteket fogjuk kinyerni, amelyek tartalmazzák az „Azure” kifejezést.

A következő képernyőképen az alkalmazás folyamata látható:
...
A valós idejű tweetstream fogadásához létrehozunk egy alkalmazást a Twitteren. Kövesse a Twitter-alkalmazás létrehozására vonatkozó utasításokat, és jegyezze fel az értékeket, amelyek az oktatóanyag elvégzéséhez szükségesek.
...
A jegyzetfüzet futtatásához használja a **SHIFT + ENTER** billentyűparancsot. Ekkor az alábbi kódrészlethez hasonló kimenetnek kell megjelennie. A kimenetben szereplő minden egyes esemény egy, az „Azure” kifejezést tartalmazó valós idejű tweet, amely be lett töltve az Event Hubsba. 
...
Ennyi az egész! Az Azure Databricks segítségével valós idejű adatokat streamelt az Azure Event Hubsba. A streamadatokat ezután az Apache Spark Event Hubs-összekötője segítségével használta fel.
...
>[Streamelt adatok hangulatelemzése az Azure Databricks használatával](databricks-stream-from-eventhubs.md)
