...
További információ arról, hogy miért dönthet úgy, hogy létrehoz egy Azure Databricks munkaterületet a saját virtuális hálózatában: [Azure Databricks üzembe helyezése az Azure-Virtual Network (VNet Injection)](https://docs.microsoft.com/azure/databricks/administration-guide/cloud-configurations/azure/vnet-inject).
...
2. A **virtuális hálózat létrehozása**területen alkalmazza a következő beállításokat: 
    ...
    |Régió|\<Válassza ki a felhasználóihoz legközelebb eső régiót\>|Válassza ki azt a földrajzi helyet, ahol a virtuális hálózatot üzemeltetni szeretné. Használja a felhasználókhoz legközelebb eső helyet.|
...
4. A virtuális hálózat telepítéséhez a **felülvizsgálat + létrehozás** lapon válassza a **Létrehozás** lehetőséget. Az üzembe helyezés befejezését követően navigáljon a virtuális hálózathoz, és válassza a **címterület** lehetőséget a **Beállítások**területen. Az adja meg a *további címtartomány hozzáadása*lehetőséget, `10.179.0.0/16` és kattintson a **Mentés**gombra.
...
2. A **Azure Databricks szolgáltatás**területen alkalmazza a következő beállításokat:
    ...
    |Hely|\<Válassza ki a felhasználóihoz legközelebb eső régiót\>|Válassza ki a virtuális hálózattal megegyező helyet.|
    |Tarifacsomag|Válassza a Standard vagy a Prémium előfizetést.|A díjszabással kapcsolatos további információkért tekintse meg a [Databricks díjszabását ismertető oldalt](https://azure.microsoft.com/pricing/details/databricks/).|
...
3. Miután befejezte a beállítások megadását az **alapok** lapon, válassza a **tovább lehetőséget: hálózatkezelés >** és a következő beállítások alkalmazása:
    ...
    |Virtual Network|databricks – rövid útmutató|Válassza ki az előző szakaszban létrehozott virtuális hálózatot.|
