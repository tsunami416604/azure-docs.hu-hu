...
description: Ebből az oktatóanyagból megtudhatja, hogyan nyerheti ki a Data Lake Storage Gen2 adatait a Azure Databricksba, átalakíthatja az adatait, majd betöltheti az adatait az Azure szinapszis Analytics szolgáltatásba.
author: mamccrea
ms.author: mamccrea
ms.reviewer: jasonh
...
Ebben az oktatóanyagban egy ETL-műveletet (kinyerési, átalakítási és betöltési) műveletet hajt végre Azure Databricks használatával. Az adatok kinyerése a Azure Data Lake Storage Gen2ból a Azure Databricksba, a Azure Databricksban lévő adatok átalakítását futtathatja, és az átalakított adatok betölthetők az Azure szinapszis Analyticsbe.

Az oktatóanyag lépései a Azure Databricks Azure szinapszis-összekötőjét használják az adatok Azure Databricksba történő átviteléhez. Ez az összekötő pedig az Azure Blob Storage-t használja ideiglenes tárolóként az Azure Databricks-fürt és az Azure szinapszis között továbbított adatforgalomhoz.

Az alábbi ábrán az alkalmazásfolyam látható:

![Azure Databricks a Data Lake Store és az Azure szinapszis](./media/databricks-extract-load-sql-data-warehouse/databricks-extract-transform-load-sql-datawarehouse.png "Azure Databricks a Data Lake Store és az Azure szinapszis")
...
* Hozzon létre egy Azure Szinapszist, hozzon létre egy kiszolgálói szintű tűzfalszabály-szabályt, és kapcsolódjon a kiszolgálóhoz kiszolgáló-rendszergazdaként. Lásd [: rövid útmutató: SZINAPSZIS SQL-készlet létrehozása és lekérdezése a Azure Portal használatával](../synapse-analytics/sql-data-warehouse/create-data-warehouse-portal.md).

* Hozzon létre egy főkulcsot az Azure szinapszis számára. Lásd: [adatbázis-főkulcs létrehozása](https://docs.microsoft.com/sql/relational-databases/security/encryption/create-a-database-master-key).
...
* Egyszerű szolgáltatásnév létrehozása. [Útmutató: a portál használatával létrehozhat egy Azure ad-alkalmazást és egy egyszerű szolgáltatást, amely hozzáférhet az erőforrásokhoz](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal).
   ...
   * A cikk beléptetési [értékek beolvasása](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal#get-values-for-signing-in) szakaszában szereplő lépések végrehajtásakor illessze be a bérlői azonosítót, az alkalmazás azonosítóját és a titkos értékeket egy szövegfájlba.
...
Győződjön meg arról, hogy elvégezte az oktatóanyag előfeltételeit.
   ...
   : heavy_check_mark: az Azure szinapszis adatbázisának neve, adatbázis-kiszolgáló neve, felhasználóneve és jelszava.
...
## <a name="load-data-into-azure-synapse"></a>Betöltés az Azure Szinapszisba

Ebben a szakaszban az átalakított adatok feltöltése az Azure Szinapszisba. A Azure Databricks Azure szinapszis-összekötővel közvetlenül feltölthet egy dataframe egy szinapszis Spark-készletben lévő táblázatként.

Ahogy azt korábban említettük, az Azure szinapszis-összekötő az Azure Blob Storage-ot használja ideiglenes tárolóként az adatok Azure Databricks és az Azure szinapszis közötti feltöltéséhez. Ezért első lépésként adja meg a tárfiókhoz való csatlakozáshoz szükséges konfigurációt. A jelen cikk előfeltételeinek részeként már létre kell hoznia a fiókot.
...
2. A Azure Databricks és az Azure szinapszis közötti adatáthelyezés során használandó ideiglenes mappa meghatározása.
...
4. Adja meg az Azure szinapszis-példányhoz való kapcsolódáshoz szükséges értékeket. Előfeltételként létre kell hoznia egy Azure szinapszis Analytics szolgáltatást. Használja a **dwServer**teljes kiszolgálójának nevét. Például: `<servername>.database.windows.net`.
...
5. Futtassa az alábbi kódrészletet az átalakított dataframe **renamedColumnsDF**való betöltéséhez az Azure szinapszisban. Ez a kódrészlet létrehoz egy **SampleTable** nevű táblát az SQL-adatbázisban.
