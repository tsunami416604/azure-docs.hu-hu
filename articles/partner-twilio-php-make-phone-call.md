...
1. 取得 Twilio 帳戶和驗證權杖。 若要開始使用 Twilio，請在評估價格 [http://www.twilio.com/pricing][twilio_pricing]。 您可以註冊試用帳戶在 [https://www.twilio.com/try-twilio][try_twilio]。 Twilio 所提供之 API 的相關資訊，請參閱 [http://www.twilio.com/api][twilio_api]。
2. 取得適用於 PHP 的 Twilio 程式庫。 您可以從 GitHub ([https://github.com/twilio/twilio-php][twilio_php_github]) 或以 PEAR 封裝安裝。 如需詳細資訊，請參閱 [https://github.com/twilio/twilio-php/blob/master/README.md][twilio_github_readme]。
3. 安裝 Azure SDK for PHP。 SDK 和及其安裝指示的概觀，請參閱 [設定 Azure SDK for PHP][setup_php_sdk]。
...
## 建立用以撥打電話的程式碼
下列程式碼示範如何建置網頁 (**makecall.php**) 在使用者提交所顯示的表單時受到呼叫 **callform.html**。 下方顯示的程式碼會建立通話訊息及產生通話。 (使用您的 Twilio 帳戶和驗證權杖，而不是預留位置值指派給使用 **$sid** 和 **$token** 程式碼所示。)
...
* 除了使用 Web 表單以外，您也可以使用 Azure 儲存體 Blob 或 SQL Database 來儲存電話號碼和通話文字。 在 PHP 中使用 Azure 儲存體 blob 的相關資訊，請參閱 [與 PHP 應用程式使用 Azure 儲存體][howto_blob_storage_php]。 在 PHP 中使用 SQL Database 的相關資訊，請參閱 [使用 SQL Database 與 PHP 應用程式][howto_sql_azure_php]。
*  **Makecall.php** 程式碼使用 Twilio 提供的 URL ([http://twimlets.com/message][twimlet_message_url]) 來提供 Twilio 標記語言 (TwiML) 回應，告知 twilio 應如何執行通話。 例如，傳回的 TwiML 可能會包含 `<Say>` 動詞，而產生要傳達給受話方的文字。 除了使用 Twilio 提供的 URL，您也可以建置自己的服務來回應 Twilio 的要求。如需詳細資訊，請參閱 [如何透過 Twilio 使用語音和簡訊功能，在 PHP 中][howto_twilio_voice_sms_php]。 TwiML 的詳細資訊，請參閱 [http://www.twilio.com/docs/api/twiml][twiml], ，和其他相關資訊 `<Say>` 和其他 Twilio 動詞命令，請參閱 [http://www.twilio.com/docs/api/twiml/say][twilio_say]。
