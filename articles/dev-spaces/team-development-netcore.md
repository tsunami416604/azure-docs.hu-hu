---
title: Csoportmunka-fejlesztés az Azure dev Spaces használatával a .NET Core és a VS Code segítségével
...
ms.locfileid: "74279525"
---
# <a name="team-development-with-azure-dev-spaces-using-net-core-and-vs-code"></a>Csoportmunka-fejlesztés az Azure dev Spaces használatával a .NET Core és a VS Code segítségével
...
1. A távoli ág kifizetése *azds_updates*: `git checkout -b azds_updates origin/azds_updates`
1. Válassza ki a _fejlesztői_ területet: `azds space select --name dev`. Amikor a rendszer rákérdez a szülő fejlesztői terület kiválasztására, válassza _\<nincs\>_ lehetőséget.
...
1. `azds list-uris` végrehajtása a _webfrontend_ nyilvános végpontjának megtekintéséhez
...
Lépjen a VS Code ablakra a `mywebapi`hoz, és végezze el a kód szerkesztését a `Controllers/ValuesController.cs``string Get(int id)` metódusban, például:
...
A szolgáltatás futtatásához nyomja le az F5 billentyűt (vagy írja be a `azds up` a terminál ablakban) a szolgáltatás futtatásához. A szolgáltatás automatikusan futni fog az újonnan kiválasztott, _dev/Scott_-beli helyen. `azds list-up`futtatásával ellenőrizze, hogy a szolgáltatás a saját területén fut-e:
...
Az aktuális terület URL-címeinek listázása `azds list-uris`futtatásával.
