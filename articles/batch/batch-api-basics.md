...
2. 上傳相依 *二進位檔案* 至儲存體帳戶。 這些二進位檔案包含工作所要執行的程式，及其任何相依組件。 這些檔案也必須從您的儲存體帳戶中存取，讓工作可以將它們下載至計算節點。

3. 建立 [集區](#pool) 的運算節點。 您指定 [運算節點的大小][cloud_service_sizes] 欲建立之集區時，工作執行時，它會指派此集區中的節點。

4. 建立 [工作](#job)。 作業可讓您管理一大堆工作。
...
- 標準 **資料夾結構** 和相關聯 **環境變數** 各個運算節點上會詳細說明檔案的路徑。 請參閱 [檔案和目錄](#files) 如下如需詳細資訊。
- **環境變數** 可由工作的參考。
- **防火牆** 設定來控制存取的設定。
- 如果 **遠端存取** 電腦必須是節點 （例如偵錯），RDP 檔案可以取得這可能會再用來存取透過節點 *遠端桌面*。
...
- **啟動工作** 集區中的節點
    - A *啟動工作* 來執行計算節點加入集區，每次可指定及何時重新啟動節點。 這通常用來安裝在節點上執行的工作所要使用的應用程式。

### <a name="job"></a>作業

作業是工作的集合，可指定在集區中的計算節點上執行計算的方式。

- 指定工作 **集區** 中的工作執行。 此集區可以是先前建立給許多作業使用的現有集區，或針對與作業排程相關聯的每項作業建立的集區，或針對與作業排程相關聯的所有作業建立的集區。
- 選擇性 **工作優先順序** 可以指定。 使用高於其他進行中作業的優先順序提交作業時，較高優先順序的作業工作會插入在佇列中較低優先順序的作業工作之前。 已在執行中的較低優先順序工作不會被優先佔用。
- 工作 **條件約束** 指定特定限制為您的工作。
    - A **最大時鐘時間** 可以設定工作。 如果作業的執行時間超過指定的最大時鐘時間，則會結束此作業和所有相關聯的工作。
    - Azure 批次可以偵測失敗的工作並重試工作。  **工作重試次數上限** 可以指定做為條件約束，包括是否會一律或永遠不會重試工作。 重試工作表示工作會重新排入佇列，以再次執行。
...
- **資源檔** ，其中包含要處理的資料。 這些檔案會自動從 Azure 儲存體帳戶中的 Blob 儲存體複製到節點。 如需詳細資訊，請參閱 [檔案和目錄](#files) 下方。

-  **環境變數** 應用程式所需。 如需詳細資訊，請參閱 [工作的環境設定](#environment) 下方。

-  **條件約束** 下應進行計算。 例如，允許執行工作的最長時間、工作失敗時應該重試的次數上限，以及檔案保留在工作目錄中的最長時間。
...
藉由關聯 **啟動工作** 與集區，您可以設定的作業環境的一個節點執行動作，例如安裝軟體，或啟動背景處理序。 開始工作會在節點每次啟動時執行，且只要留在集區中就會持續執行，包括在節點第一次新增至集區時。 啟動工作的主要優點，是其中包含設定計算節點和安裝作業工作執行所需的應用程式時必須用到的所有資訊。 因此，要增加集區中的節點數目，只要指定新的目標節點計數即可 - Batch 已具備設定新節點並使其可接受工作所需的所有資訊。

如同任何批次工作，一份 **資源檔** 中 [Azure 儲存體][azure_storage] 可以指定，此外 **命令列** 執行。 Azure Batch 會先從 Azure 儲存體複製檔案，然後再執行命令列。 對於集區開始工作，檔案清單通常包含應用程式封裝或檔案，但也可以包含計算節點上執行的所有工作將會使用的參考資料。 開始工作的命令列可執行 PowerShell 指令碼或 `robocopy` 作業，例如，將應用程式檔案複製到 “shared” 資料夾，然後執行 MSI 或 `setup.exe`。
...
A **作業管理員工作** 通常用於控制及/或監視執行工作。 比方說，建立和提交作業的工作、決定其他要執行的工作，以及判斷工作何時完成。 但作業管理員工作並不限定於這些活動 - 它是功能完備的工作，可執行作業所需的任何動作。 比方說，作業管理員工作可以下載指定為參數的檔案、分析該檔案的內容，並根據這些內容提交其他工作。
...
- **作業準備工作** -作業準備工作上執行的所有運算節點排程執行工作、 工作的任何其他工作之前，先執行。 舉例來說，使用作業準備工作可以複製所有工作所共用、但對作業是唯一的資料。
- **作業發行工作** -當工作完成之後時，執行至少一個工作集區中的每個節點上執行的作業版本工作。 舉例來說，使用作業釋放工作可刪除作業準備工作所複製的資料，或壓縮並上傳診斷記錄資料。
...
- **共用** – 此位置是在一個節點，不論工作執行的所有工作的共用的目錄。 在節點上，透過存取共用的目錄  `%AZ_BATCH_NODE_SHARED_DIR%`。 此目錄允許對所有在節點上執行的工作進行讀取/寫入存取。 工作可以建立、讀取、更新和刪除此目錄中的檔案。

- **啟動** – 此位置的啟動工作做為其工作目錄。 由批次服務下載來執行啟動工作的所有檔案，也儲存在此目錄下。 在節點上，啟動目錄可透過 `%AZ_BATCH_NODE_START_DIR%` 環境變數來存取。 開始工作可以建立、讀取、更新和刪除此目錄下的檔案，開始工作可以使用此目錄來設定作業系統。

- **工作** -透過存取的節點上執行每項工作會建立目錄 `%AZ_BATCH_TASK_DIR%`。 在每個工作目錄中，Batch 服務會建立由 `%AZ_BATCH_TASK_WORKING_DIR%` 環境變數指定唯一路徑的工作目錄 (`wd`)。 這個目錄可供讀取/寫入工作。 工作可以建立、 讀取、 更新和刪除此目錄下的檔案和目錄會根據保留 *RetentionTime* 為工作指定的條件約束。
...
在極端情況下，您可以在提交每項作業後對此作業建立一個集區，而其節點會在工作執行完成時立即移除。 這樣只有在絕對必要時才會配置節點，而且節點會在變成閒置時立即關閉，因此可達到最高使用率。 雖然這表示工作必須等候節點進行配置，但請務必注意，工作將會在節點個別可用、配置且開始工作完成時立即排程至節點。 批次未 *不* 等到在集區內的所有節點之後，再指派工作，以確保所有可用節點的最大使用量都都可用。

就另一個極端而言，如果讓作業立即啟動是最高的優先順序，則可以預先建立集區，並使其節點在作業提交之前成為可用。 在此情況下，作業工作可以立即啟動，但節點可能會閒置以等候指派的工作。

結合的方法，通常用來處理變數，但進行中的負載，是將集區的多個工作提交，但根據工作負載向上或向下調整的節點數目 (請參閱 *調整應用程式* 下方)。 這可以根據目前的負載被動完成，或在負載可預測時主動完成。
...
- **時間度量** – 根據中指定的小時數每隔五分鐘收集的統計資料。

- **資源度量** – 根據 CPU 使用量、 頻寬使用量、 記憶體使用量和節點數目。
...
您可以指派優先順序給您在 Batch 中建立的作業。 Batch 服務會使用作業的優先順序值，以決定帳戶內的作業排程順序。 優先順序值可以介於 -1000 到 1000，-1000 表示最低優先順序，1000 表示最高優先順序。 您可以使用，以更新工作的優先順序 [更新作業的內容中][rest_update_job] 作業 (批次 REST API)，或修改 [CloudJob.Priority][net_cloudjob_priority] 屬性 (批次.NET API)。
...
取得工作的環境變數，這兩個系統和使用者定義，使用 [取得工作的相關資訊][rest_get_task_info] 作業 (批次 REST API) 或存取 [CloudTask.EnvironmentSettings][net_cloudtask_env] 屬性 (批次.NET API)。 如前所述，在計算節點上執行的程序也可以存取所有的環境變數，例如，藉由使用熟悉的 `%VARIABLE_NAME%` 語法。
...
>[AZURE.NOTE] 您無法覆寫任何上述系統定義的變數--它們是唯讀。
...
- **條件約束失敗**
    - 條件約束可以設定來指定工作或工作的最大執行持續期間 *maxWallClockTime*。 這可用來終止「擱置」工作。
    - 當超過最大時間量時，將工作標示為 *完成* 但結束代碼設為 `0xC000013A`, ，而 *而 schedulingError* 欄位將會標示為 `{ category:"ServerError", code="TaskEnded"}`。
...
更廣泛的偵錯可執行的運算節點，使用登入 *遠端桌面*。 您可以 [從節點取得遠端桌面通訊協定檔案][rest_rdp] (批次 REST API) 或使用 [ComputeNode.GetRDPFile][net_rdp] 方法 (批次.NET API) 遠端登入。

>[AZURE.NOTE] 若要連接至節點，以透過 RDP，您必須先在節點上建立使用者。 [將使用者帳戶加入節點][rest_create_user] 在批次 REST API 或使用 [ComputeNode.CreateComputeNodeUser][net_create_user] 批次.NET 中的方法。
...
- 依照下列中的步驟來建立應用程式的第一個批次 [快速入門適用於.NET 的 Azure 批次程式庫](batch-dotnet-get-started.md)
