...
**何時將您選擇此選項？** -您可能已經有執行運算密集工作負載的內部部署 HPC 叢集，但它需要額外的計算資源尖峰期間，例如結束月份的報告或特殊專案。 不需要購買、部署和管理在大部分情況下可能處於閒置狀態的額外軟硬體，您可以使用 Azure，將隨選計算能力新增至現有的叢集。
...
**何時將您選擇此選項？** -您可能已大量投資內部部署 Windows 或 Linux HPC 叢集，包括管理和排程工具和自訂應用程式。 您可能需要額外的叢集容量來執行您現有的工具組和應用程式，卻又不希望額外投資內部部署基礎結構或修改您的應用程式。 或者您可能需要建立短期或長期使用的新叢集，但您不想要付出購買、維護及操作成本，或配置空間來安裝及部署它。
