...
為了說明平行工作執行的優點，假設您工作的應用程式具有 CPU 和記憶體需求，使得 Standard\_D1 節點大小適合，但為了執行工作所需的時間中，需要有 1000年這類節點。 而不是使用 Standard\_D1 節點，您可以採用 Standard\_D14 節點具有 16 個核心，並啟用平行工作執行。 在此情況下，不需使用 1000 個節點，而是可以使用 *16 倍較少的節點*，僅需 63 個節點。 如果每個節點需要大型應用程式檔案或參考資料，這樣可大幅改進作業執行時間和效率。
...
Azure Batch 允許每個節點的最大化工作設定為多達節點核心數目的 4 倍 (4x)。 例如，如果集區設定的節點大小 「 大型 」 (四個核心) 然後 `maxTasksPerNode` 可能會設定為十六個。 詳細資訊的核心數目上每個節點大小中找 [雲端服務的大小](../cloud-services/cloud-services-sizes-specs.md), ，而且需服務限制的詳細資訊可以在中找到 [配額和限制的 Azure 批次服務](batch-quota-limit.md)。
> [AZURE.TIP] 請務必考慮 `maxTasksPerNode` 值建構時 [自動調整公式 ][enable_autoscaling] 集區。 例如，評估公式 `$RunningTasks` 可能會大幅增加每個節點的工作中受到影響。 請參閱 [自動調整運算節點的 Azure 批次集區中](batch-automatic-scaling.md) 如需詳細資訊。
...
做為範例的情況下，這項功能是非常有用的方式，請考慮 Standard\_D14 節點，在上述範例中的集區設定 [CloudPool.MaxTasksPerComputeNode][maxtasks_net] 16 的值。 如果 [CloudPool.TaskSchedulingPolicy][task_schedule] 設有 [ComputeNodeFillType ][fill_type] 的 *組件*, ，它會最大化所有 16 個核心的每個節點的使用方式，允許 [自動調整集區](./batch-automatic-scaling.md) 剪除未使用的節點從集區 (而不指派任何工作的節點)，因而降至最低的資源使用量和儲存成本。
