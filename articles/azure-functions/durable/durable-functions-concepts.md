---
title: Durable Functions-minták és technikai kulcsfogalmak – Azure
description: Durable Functions az Azure-ban működéséről ahhoz, hogy a felhőben állapot-nyilvántartó kódfuttatás előnyeinek részletes adatokat biztosít.
services: functions
author: kashimiz
manager: jeconnoc
keywords: ''
...
Ez a szakasz ismerteti az egyes Durable Functions is kihasználó tipikus alkalmazásminták.
...
![Függvény titkosításblokkoló diagramja](./media/durable-functions-concepts/function-chaining.png)
...
A `context` paraméter [DurableOrchestrationContext] \(.NET\) és a `context.df` objektum (JavaScript) tartozó név, átadása paramétereket, más függvények meghívása mód megadásának és függvény-kimenet visszaadása. Minden alkalommal, amikor a kód meghívja `await` (C#) vagy `yield` (JavaScript), a Durable Functions-keretrendszer *ellenőrzőpontok* az aktuális függvény példány állapotát. Ha a folyamat vagy a virtuális gép újraindul a végrehajtási keresztül midway, a függvény példány folytatja az előző `await` vagy `yield` hívja. Ez a viselkedés később újraindíthatja.

> [!NOTE]
> A `context` javascriptben objektum képviseli a [függvény helyi](../functions-reference-node.md#context-object) egy teljes, nem a [DurableOrchestrationContext].

### <a name="fan-in-out"></a>#2. minta: Elosztás/összevonás
...
![Fan-kimenő/fan-lévő diagram](./media/durable-functions-concepts/fan-out-fan-in.png)
...
![A figyelő diagramja](./media/durable-functions-concepts/monitor.png)
...
![Emberi beavatkozás diagramja](./media/durable-functions-concepts/approval.png)
...
Íme egy példa a követési események Durable Functions kinézni az Application Insights portál felületén [Application Insights-elemzési](../../application-insights/app-insights-analytics.md):

![App Insights-lekérdezés eredményei](./media/durable-functions-concepts/app-insights-1.png)
...
![az App Insights-lekérdezés a customDimensions mező](./media/durable-functions-concepts/app-insights-2.png)
...
![Képernyőkép az Azure Storage Explorerrel](./media/durable-functions-concepts/storage-explorer.png)
