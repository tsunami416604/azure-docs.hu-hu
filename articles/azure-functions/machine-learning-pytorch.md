...
Navigáljon a *Start* mappára, és futtassa a következő parancsokat egy `.venv`nevű virtuális környezet létrehozásához és aktiválásához.
...
1. Cserélje le a `main` függvény teljes tartalmát a következő kódra:
    ...
    Ez a függvény egy `img`nevű lekérdezési karakterlánc-paraméterben fogad egy képurl-címet. Ezután meghívja a `predict_image_from_url`t a segítő könyvtárból, hogy letöltse és osztályozza a rendszerképet a PyTorch-modell használatával. A függvény ezután egy HTTP-választ ad vissza az eredményekkel.
