...
description: Egy előre betanított ResNet 18 mély neurális hálózat pytorch az Azure Functions rendelni 1 1000 ImageNet címkék egy lemezképhez.
author: gvashishtha
ms.topic: tutorial
ms.date: 02/28/2020
ms.author: gopalv
...
---
# <a name="tutorial-deploy-a-pre-trained-image-classification-model-to-azure-functions-with-pytorch"></a>Oktatóanyag: Üzembe helyezhet egy előre betanított rendszerkép-besorolási modellt az Azure Functions ben a PyTorch segítségével

Ebben a cikkben megtudhatja, hogyan használhatja a Python, A PyTorch és az Azure Functions egy előre betanított modell betöltéséhez egy kép a tartalma alapján. Mivel minden munkát helyileg, és nem hoz létre Azure-erőforrásokat a felhőben, nincs költség az oktatóanyag befejezéséhez.

> [!div class="checklist"]
> * Inicializáljon egy helyi környezetet az Azure Functions pythonban való fejlesztéséhez.
> * Előre betanított PyTorch gépi tanulási modell importálása egy függvényalkalmazásba.
> * Kiszolgáló nélküli HTTP API-t hozhat létre, amely a lemezképet az 1000 ImageNet-osztály egyikébe [sorolja.](https://gist.github.com/yrevar/942d3a0ac09ec9e5eb3a)
...
- [Python 3.7.4 vagy újabb](https://www.python.org/downloads/release/python-374/). (A Python 3.8.x és a Python 3.6.x is ellenőrizhető az Azure Functions szolgáltatással.)
...
Nyissa meg a *start* mappát, és futtassa `.venv`a következő parancsokat a nevű virtuális környezet létrehozásához és aktiválásához.
...
## <a name="import-the-pytorch-model-and-add-helper-code"></a>A PyTorch modell importálása és segítőkód hozzáadása

Ha módosítani `classify` szeretné a függvényt, hogy a rendszerkép tartalma alapján osztályozzon, egy előre betanított [ResNet-modellt](https://arxiv.org/abs/1512.03385) kell használnia. Az előre betanított modell, amely a [PyTorch-ból](https://pytorch.org/hub/pytorch_vision_resnet/)származik, egy képet 1/1000 [ImageNet osztályba](https://gist.github.com/yrevar/942d3a0ac09ec9e5eb3a)sorolja. Ezután hozzáad néhány segítő kódot és függőséget a projekthez.

1. A *start* mappában futtassa a következő parancsot az előrejelzési kód és címkék átmásolásához a *besorolási* mappába.
...
1. Ellenőrizze, hogy a *besorolási* mappa *tartalmaz-e predict.py* és *labels.txt*nevű fájlokat. Ha nem, ellenőrizze, hogy futtatta-e a parancsot a *start* mappában.

1. Nyissa *meg a start/requirements.txt fájlt* egy szövegszerkesztőben, és adja hozzá a segítő kód által igényelt függőségeket, amelyek a következőképpen néznek ki:
...
1. Mentse *a requirements.txt*fájlt, majd futtassa a következő parancsot a *kezdőmappából* a függőségek telepítéséhez.
...
A telepítés eltarthat néhány percig, amely idő alatt folytathatja a funkció módosítását a következő szakaszban.
> [!TIP]
...
1. Cserélje le a `main` függvény teljes tartalmát a következő kódra:
    ...
    Ez a függvény egy kép URL-címét `img`kapja egy lekérdezési karakterlánc nevű paraméterben. Ezután `predict_image_from_url` felhívja a segítő könyvtárból a lemezkép letöltését és besorolását a PyTorch modell használatával. A függvény ezután http-választ ad vissza az eredményekkel.
...
1. A böngészőben nyissa meg a következő URL-t, hogy meghívja a függvényt egy Berni Hegyi kutya kép URL-jével, és erősítse meg, hogy a visszaadott JSON a képet Berni hegyi kutyaként osztályozta.
...
1. Válassza **a Küldés** lehetőséget a függvényvégpont meghívásához a lemezkép besorolásához.
    ...
    ![A kész projekt képernyőképe](media/machine-learning-pytorch/screenshot.png)
...
Ebben az oktatóanyagban megtanulta, hogyan hozhat létre és szabhat testre egy HTTP API-végpontot az Azure Functions segítségével a lemezképek pytorch-modell használatával történő besorolásához. Azt is megtanulta, hogyan hívhatja meg az API-t egy webalkalmazásból. Az oktatóanyagban szereplő technikák segítségével bármilyen összetettségű API-kat hozhat létre, miközben az Azure Functions által biztosított kiszolgáló nélküli számítási modellen fut.
