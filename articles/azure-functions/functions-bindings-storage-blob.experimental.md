...
Az `@BlobTrigger` attribútummal hozzáférést biztosíthat a függvényt kiváltó blobhoz. A részletekért tekintse meg az [trigger példáját](#trigger---example) .
...
A Blobok adatai `context.bindings.<name from function.json>`a használatával érhetők el.
...
Az `@BlobInput` attribútum hozzáférést biztosít a függvényt kiváltó blobhoz. Ha az attribútummal rendelkező byte tömböt használ, állítsa `dataType` `binary`a következőre:. A részletekért tekintse meg a [bemeneti példát](#input---example) .
...
A blob adatai a használatával `context.bindings.<name from function.json>`érhetők el.
...
Az `@BlobOutput` attribútum hozzáférést biztosít a függvényt kiváltó blobhoz. Ha az attribútummal rendelkező byte tömböt használ, állítsa `dataType` `binary`a következőre:. A részletekért tekintse meg a [kimeneti példát](#output---example) .
