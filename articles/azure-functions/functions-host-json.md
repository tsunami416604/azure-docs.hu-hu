...
> [!NOTE]
> A napló mintavételezése miatt előfordulhat, hogy egyes végrehajtások nem jelennek meg a Application Insights figyelő paneljén. A naplók mintavételezésének elkerüléséhez `excludedTypes: "Request"` adja hozzá `samplingSettings` a értéket a következőhöz:.
...
| excludedTypes | null | Pontosvesszővel tagolt lista, amelyet nem szeretne mintát venni. A felismert típusok `Dependency`a `Event`következők `Exception`: `PageView`, `Request`,, `Trace`, és. A rendszer továbbítja a megadott típusok összes példányát. a nem megadott típusok mintául szolgálnak. |
| includedTypes | null | A mintavételhez használni kívánt típusok pontosvesszővel tagolt listája; az üres lista minden típust magában foglalja. Írja be az `excludedTypes` itt felsorolt felülbírálási típusok listán szereplő típust. A felismert típusok `Dependency`a `Event`következők `Exception`: `PageView`, `Request`,, `Trace`, és. A megadott típusok példányainak mintája; a nem megadott vagy vélelmezett típusú típusok mintavételezés nélkül továbbítódnak. |
...
| maximumSnapshotsRequired | 3 | Egy adott problémára összegyűjtött Pillanatképek maximális száma az egyiktől a 999-ig terjedő tartományban. Probléma lehet az alkalmazás egyéni Throw utasítása. Ha a problémához összegyűjtött Pillanatképek száma eléri ezt az értéket, a rendszer nem gyűjt további pillanatképeket a problémához, amíg vissza nem állítja a `problemCounterResetInterval`problémát számlálókat `thresholdForSnapshotting` (lásd:), és a korlát újra elérhetővé válik. |
| problemCounterResetInterval | 24:00:00 | Milyen gyakran kell alaphelyzetbe állítani a probléma számlálóit egy perc és hét nap között. Ha eléri ezt az időközt, a rendszer az összes probléma számát nullára állítja vissza. A meglévő olyan problémák, amelyek már elértek a pillanatképek küszöbértékét, de még nem hozták létre a `maximumSnapshotsRequired`Pillanatképek számát a alkalmazásban, aktívak maradnak. |
...
A dedikált (App Service) csomagban nincs általános korlát, és az alapértelmezett érték 30 perc. Az érték a `-1` nem kötött végrehajtást jelzi, de a rögzített felső korlát fenntartása ajánlott.
...
Ez az érték a Host. JSON séma-verzióját jelzi. A v2- `"version": "2.0"` es futtatókörnyezetet vagy újabb verziót célzó Function alkalmazás verziójának karakterlánca szükséges. Nincs gazdagép. JSON séma a v2 és v3 közötti változásokhoz.
