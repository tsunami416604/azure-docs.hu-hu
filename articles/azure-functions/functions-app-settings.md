...
Verzió 2.x verzióját a Functions-futtatókörnyezet, konfigurálja a futtatási környezetet alapuló alkalmazások működését. Ez az érték [inicializálása során beolvasott](https://github.com/Azure/azure-functions-host/blob/dev/src/WebJobs.Script.WebHost/Program.cs#L43). Beállíthat `AZURE_FUNCTIONS_ENVIRONMENT` bármely értékre, de [értékek](/dotnet/api/microsoft.aspnetcore.hosting.environmentname) támogatottak: [Fejlesztési](/dotnet/api/microsoft.aspnetcore.hosting.environmentname.development), [átmeneti](/dotnet/api/microsoft.aspnetcore.hosting.environmentname.staging), és [éles](/dotnet/api/microsoft.aspnetcore.hosting.environmentname.production). Amikor `AZURE_FUNCTIONS_ENVIRONMENT` nincs beállítva, a rendszer alapértelmezés szerint `Production`. Ezt a beállítást kell használni, hanem `ASPNETCORE_ENVIRONMENT` a futtatókörnyezet beállítása. 
