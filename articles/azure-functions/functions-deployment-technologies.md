...
__Művelet:__ Lehetővé teszi egy távoli csomag (.zip) fájlt, amely tartalmazza a függvényalkalmazás hivatkozhat. A fájl letöltése a megadott URL-címről, és az alkalmazás fut [Run-a-Package](run-functions-from-deployment-package.md) mód.
...
__Művelet:__ Lehetővé teszi az Azure-bA a függvényalkalmazást tartalmazó zip-fájlt. Ha szükséges, megadhatja azt is kell az alkalmazást start [Run-a-Package](run-functions-from-deployment-package.md) mód.
...
__Művelet:__ Adjon meg egy tárolórendszerképet, amely az Azure Functions lekérése, és futtassa.
...
__Művelet:__ A csomagok és telepíti az IIS kiszolgáló – többek között az Azure function appsszel, a Windows rendszert futtató Windows alkalmazások.
...
__Művelet:__ Lehetővé teszi a környezet igénybe vételét a függvényalkalmazást egy git-tárház akár úgy, hogy a kód az adott tárházba frissítéseit elindítja a központi telepítés. További információkért tekintse meg a [Kudu Wiki](https://github.com/projectkudu/kudu/wiki/VSTS-vs-Kudu-deployments).
...
__Művelet:__ Lehetővé teszi, hogy a helyi gépen leküldéses kód Git használatával az Azure Functions.
...
__Művelet:__ Lehetővé teszi a tartalom az Azure Functions a Dropbox és a onedrive vállalati verzió szinkronizálása.
...
__Művelet:__ Lehetővé teszi közvetlenül az Azure Functions át a fájlokat.
...
__Művelet:__ A portálalapú-szerkesztő használatával teszi lehetővé a függvényalkalmazás a fájlok közvetlenül szerkesztheti (lényegében üzembe helyezése kattintva bármikor **mentése**).
...
#### <a name="portal-editing-availability"></a>A portál rendelkezésre állási szerkesztése
...
| TypeScript (Node.js) | | | | | |
| C# | | | | | |
| C#Parancsfájl |✔|✔|✔| |✔<sup>1</sup>|
| F# | | | | | |
| Java | | | | | |
| Python (előzetes verzió) | | | | | |
| PowerShell (előzetes verzió) |✔|✔|✔| | |

<sup>1</sup> portál szerkesztési engedélyezve van csak a HTTP- és időzítő eseményindító függvények a dedikált csomagok használatával Linux rendszeren.
...
## <a name="other-relevant-information"></a>Egyéb releváns információkat
...
> [Az Azure Functions – folyamatos üzembe helyezés](functions-continuous-deployment.md)
> [folyamatos készregyártás az Azure DevOps használatával](functions-how-to-azure-devops.md)
> [Zip-telepítések az Azure Functions](deployment-zip-push.md) 
