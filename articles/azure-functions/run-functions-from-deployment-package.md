...
Az Azure-ban a függvényeket közvetlenül egy központi telepítési csomagból is futtathatja a Function alkalmazásban. A másik lehetőség, hogy a fájlokat a Function alkalmazás `d:\home\site\wwwroot` könyvtárába telepíti.
...
A [zip-telepítés][Zip deployment for Azure Functions] a Azure app Service szolgáltatása, amely lehetővé teszi a Function app-projekt üzembe `wwwroot` helyezését a címtárban. A projekt. zip telepítési fájlként van csomagolva. Ugyanazokat az API-kat használhatja a csomag telepítéséhez a `d:\home\data\SitePackages` mappába. `WEBSITE_RUN_FROM_PACKAGE` Az Alkalmazásbeállítások értékeként a zip `1`-telepítési API-k a fájlok `d:\home\data\SitePackages` mappába másolják a csomagot `d:\home\site\wwwroot`. Létrehozza a `packagename.txt` fájlt is. Újraindítás után a csomag írásvédett fájlrendszerhez van csatlakoztatva `wwwroot` . További információ a zip-telepítésről: [a Azure functions zip központi telepítése](deployment-zip-push.md).
...
- A Futtatás a csomagból csak `wwwroot` olvasható, így hibaüzenetet kap, amikor fájlokat ír a könyvtárba.
...
- A Futtatás a csomagból nem kompatibilis a telepítés testreszabási`SCM_DO_BUILD_DURING_DEPLOYMENT=true`beállításával (), a rendszer figyelmen kívül hagyja a Build lépést az üzembe helyezés során.
