...
<tags ms.service="virtual-machines" ms.workload="infrastructure-services" ms.tgt_pltfrm="vm-multiple" ms.devlang="python" ms.topic="article" ms.date="09/25/2014" ms.author="huvalo"></tags>
...
Follow the instructions given [here][here] to create a virtual machine of the *OpenSUSE* or *Ubuntu* distribution. This tutorial uses OpenSUSE 13.1 and Ubuntu Server 14.04 LTS. We'll assume the default user name *azureuser*.

### Windows VM

Follow the instructions given [here][1] to create a virtual machine of the *Windows Server 2012 R2 Datacenter* distribution. In this tutorial, we'll assume that the user name is *azureuser*.
...
1.  Install Python 2.7.8 (32 bit) from [python.org][python.org].
    You will also need to add `C:\Python27` and `C:\Python27\Scripts` to your `PATH`
    environment variable.

2.  Install pip and setuptools by downloading the file **get-pip.py**
    from [][]<https://pip.pypa.io/en/latest/installing.html></a> and then running the
    command:

        python get-pip.py

3.  Install [Tornado][Tornado] and [PyZMQ][PyZMQ] and other IPython's dependencies by doing:
...
4.  Download and install [NumPy][NumPy] using the
...
5.  Download and install [Matplotlib][Matplotlib] using the
...
6.  Download and install OpenSSL. You can find Windows versions of OpenSSL at [][4]<http://slproweb.com/products/Win32OpenSSL.html></a>.
...
7.  Install the IPython using the command:

        easy_install ipython

8.  Open a port in Windows Firewall. On Windows Server 2012, the firewall will block incoming connections by default. To open port 9999, follow these steps:
...
profiling and parallel computing integration.
  ...
  [IPython project]: http://ipython.org
  [watch the tutorial]: http://go.microsoft.com/fwlink/?LinkID=254535&clcid=0x409
