...
-   [Create an Azure Search index][Create an Azure Search index]
-   [Explore CatalogIndexer][Explore CatalogIndexer]
-   [Build an MVC4 Application using Azure Search][Build an MVC4 Application using Azure Search]
-   [Explore AdventureWorksWeb][Explore AdventureWorksWeb]
...
-   Visual Studio 2012 or higher with ASP.NET MVC 4 and SQL Server installed. You can get Visual Studio 2013 Express from the web from the [Download Center][Download Center].
-   An Azure Search service. You'll need the Search service name, plus the admin key. See [Get started with Azure Search][Get started with Azure Search] for details.
-   [Adventure Works Azure Search Demo project on CodePlex][Adventure Works Azure Search Demo project on CodePlex]. On the Source tab, click **Download** to get a zip file of the solution.
...
To verify index creation and document upload, go to your Search service dashboard in the [Azure Preview portal][Azure Preview portal]. In Usage, the index count should be up by one, and you should have 294 documents, one for each product in the database.
...
5.  Press **F5** to launch the project. Follow these [Troubleshooting][Troubleshooting] steps if you get a build error.
...
4.  Open **HomeController.cs** and move to the Suggest function. This code is very similar to the Search function that uses the `_catalogSearch` object to call a function in **CatalogSearch.cs** called `Suggest`. Rather than making a search query, the `Suggest` function makes a call to the [Suggestions API][Suggestions API]. This uses the terms entered in the text box and build out a list of potential suggestions. The values are returned to the **Index.cshtml** file that are automatically listed in the Search box as type-ahead options.
...
-   Read up on the [Lookup API][Lookup API] that allows you to make a query to Azure Search to bring back a specific document (for example you could pass the productID).
...
-   Check out this additional code sample and video on geospatial search: [Channel 9 - Azure Search and Geospatial Data][Channel 9 - Azure Search and Geospatial Data] and [CodePlex: Azure Search GeoSearch Sample][CodePlex: Azure Search GeoSearch Sample]

You can also review the [Azure Search REST API][Azure Search REST API documentation] on MSDN.
...
<!--Anchors--> <!--Image references-->
  ...
  [Adventure Works Azure Search Demo project on CodePlex]: http://go.microsoft.com/fwlink/p/?LinkID=510972
  [1]: ./media/search-create-first-solution/AzureSearch_Create1_CodeplexDownload.PNG
  [You need an Azure account to complete this tutorial:]: ../includes/free-trial-note.md
  ...
  [Azure Search REST API documentation]: http://msdn.microsoft.com/en-us/library/azure/dn798935.aspx
  [Troubleshooting]: #err-mvc
  ...
  [Suggestions API]: http://msdn.microsoft.com/en-us/library/azure/dn798936.aspx
  [Lookup API]: http://msdn.microsoft.com/en-us/library/azure/dn798929.aspx
  [Channel 9 - Azure Search and Geospatial Data]: http://channel9.msdn.com/Shows/Data-Exposed/Azure-Search-and-Geospatial-Data
  [CodePlex: Azure Search GeoSearch Sample]: http://azuresearchgeospatial.codeplex.com
