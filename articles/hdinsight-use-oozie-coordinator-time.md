...
Learn how to define workflows and coordinators, and how to trigger the coordinator jobs based on time. It is helpful to go through [Use Oozie with HDInsight][Use Oozie with HDInsight] before this article.
...
4.  **The INSERT OVERWRITE statement** counts the occurrences of each log level type from the log4j Hive table, and saves the output to an Azure Storage - Blob (WASB) location.
...
HDInsight uses Azure Blob Storage for data storage. It is called *WASB* or *Windows Azure Storage - Blob*. WASB is Microsoft's implementation of HDFS on Azure Blob storage. For more information see [Use Azure Blob storage with HDInsight][Use Azure Blob storage with HDInsight].
...
2.  Copy the following script into the script pane, and then set the first fourteen variables (skip the 6th, $storageUri).
...
To troubleshoot a workflow, the Oozie log file can be found at C:\\apps\\dist\\oozie-3.3.2.1.3.2.0-05\\oozie-win-distro\\logs\\Oozie.log from the cluster headnode. For information on RDP, see [Administering HDInsight clusters using Management portal][Administering HDInsight clusters using Management portal].
...
-   [Develop Java MapReduce programs for HDInsight][Develop Java MapReduce programs for HDInsight]
  ...
  [Workflow diagram]: ./media/hdinsight-use-oozie-coordinator-time/HDI.UseOozie.Workflow.Diagram.png
  ...
  [Tutorial preparation output]: ./media/hdinsight-use-oozie-coordinator-time/HDI.UseOozie.Preparation.Output1.png
  [Tutorial run workflow output]: ./media/hdinsight-use-oozie-coordinator-time/HDI.UseOozie.RunCoord.Output.png
