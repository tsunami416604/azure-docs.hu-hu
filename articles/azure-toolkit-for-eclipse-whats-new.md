...
* **IBM ® WebSphere ® 應用程式伺服器 Liberty 核心支援**。 此版本將 IBM WebSphere Application Server Liberty Core 增加到支援的應用程式伺服器清單中，透過這些伺服器，工具組將能夠部署到 Azure。 這個最新的新增項目會擴充已包含各種版本之 Tomcat、Jetty、JBoss 和 GlassFish 的工具組目前現成支援的應用程式伺服器清單。
...
* **Bug 修正。**此版本包含許多依據使用者報告和測試所做出的其他錯誤修正。

### 2014 年 8 月 5 日 ###

適用於 Eclipse 的 Azure 工具組 - 2014 年 8 月版本包含下列增強功能：

* **Azure SDK 2.4 的支援。**舊版 Eclipse 工具組無法使用這個新發佈的 SDK。
* **更新版本的 Zulu OpenJDK v1.6、 1.7 和 v1.8 封裝。**如需詳細資訊，請參閱 [Zulu OpenJDK Azul Systems 網頁][]。
* **更新的版本 Package for Azure Libraries for Java 用戶端 API 程式庫。**如需詳細資訊，請參閱 [Microsoft Azure 用戶端 API][]。
* **支援最新發行設定檔案格式。**已加入 2.0 版發佈設定檔案格式的支援。
* **[發佈到雲端 」 功能背後的架構變更。**此工具組現在會使用新發佈之適用於 Java 的 Microsoft Azure 用戶端 API 來支援其發佈到雲端功能。
* **Bug 修正。**此版本包含許多使用者要求的錯誤修正。

### 2014 年 6 月 12 日 ###

適用於 Eclipse 的 Azure 工具組 - 2014 年 6 月版本是次要服務更新，可提供下列增強功能：

* **Zulu OpenJDK 封裝 v1.8 支援。**如需詳細資訊，請參閱 [Zulu OpenJDK Azul Systems 網頁][]。
* **Zulu OpenJDK v1.6 和 1.7 封裝的更新的版本。**如需詳細資訊，請參閱 [Zulu OpenJDK Azul Systems 網頁][]。
* **更新的版本 Package for Azure Libraries for Java 用戶端 API 程式庫。**如需詳細資訊，請參閱 [Microsoft Azure 用戶端 API][]。
* **Bug 修正。**此版本包含許多使用者要求的錯誤修正。
...
* **Azure SDK 2.3 版本支援。**適用於 Eclipse 的 Azure 外掛程式 - 2014 年 4 月版本需要 Azure SDK 2.3。 在使用新的外掛程式時，如果您還沒有 Azure SDK 2.3，系統會提示您允許安裝此 SDK。 請勿將 Azure SDK 2.3 用在舊版的外掛程式。
* **升級應用程式，而不需要完整的封裝部署。**在部署屬於專案一部分的 Java 應用程式時，外掛程式現在會自動將這些應用程式上傳到您選定的儲存體帳戶，以便您可以更新專案，並回收角色執行個體以部署最新的應用程式版本，而不需要重建並重新部署整個封裝。
...
* **支援 A8 和 A9 Microsoft Azure 虛擬機器的大小。**您現在可以將雲端服務部署到高記憶體的 A8 和 A9 虛擬機器大小。 如需這些 VM 大小的詳細資訊，請參閱 [虛擬機器和雲端服務大小][]。
* **自動重新導向從 HTTP 至 HTTPS 啟用 SSL 的角色。**當您的雲端服務只包含 HTTPS 角色時，如果使用者的要求指定 HTTP，它會自動重新導向至 HTTPS。 您不需要建立個別角色來處理 HTTP 要求。
* **用於本機模擬快速模擬器。**在本機上偵錯應用程式時，現在會使用 Azure 快速模擬器做為模擬器。
* **Azure 有品牌已重新命名為 Microsoft Azure。**UI 畫面現在會反映 Azure 已更名的事實而不再稱為 Azure。
...
* **支援的 'auto' 的私用端點連接埠。**您可以將輸入端點和內部端點的私人連接埠設定為自動，以讓 Azure 自動為該端點指派連接埠。 先前您只能指派特定連接埠號碼。
* **在 [建立自我簽署的憑證的 UI 中自訂憑證名稱 (CN) 的支援。**先前會對所有新憑證使用相同的硬式編碼名稱，但現在您可以指定自有憑證名稱，以協助區分 Azure 入口網站中具有不同用途的多個憑證。
...
* **Microsoft Windows Server 2012 R2 的支援。**您現在可以選取 Windows Server 2012 R2 做為雲端作業系統。

### 2013 年 10 月 22日 ###

適用於 Eclipse 的 Azure 外掛程式 - 2013 年 10 月預覽已發行。 此更新包含 2013 年 9 月預覽以來所推出的新功能、錯誤修正，以及一些根據意見反應所開發出的使用性增強功能：

* **Azure SDK 2.2 版本的支援。**適用於 Eclipse 的 Azure 外掛程式 - 2013 年 10 月預覽支援 Azure SDK 2.2。 此外掛程式仍然可以使用 Azure SDK 2.1，而且如果您還沒有至少安裝 Azure SDK 2.1，則會自動安裝 Azure SDK 2.2。
...
* **目標 OS 選取項目可在發行時。** **Publish to Azure** 對話方塊會包含新欄位， **目標 OS**, ，它會提供更容易找到的方法，讓您設定您的目標作業系統。
...
* **從 [專案] 內容功能表加入角色。** **Azure** 專案內容功能表現在包含新的功能表項目， **加入角色**, ，它會提供一種更快和更多的探索方法，讓您 Azure 專案中加入新的角色。
...
* **部署在 Azure 上可用 Azul Zulu OpenJDK 封裝的能力。**在指定要用於 Azure 部署的 JDK 時已加入新的選項。 使用此選項，您可以直接在 Azure 雲端上部署第三方 JDK 封裝，而不必上傳您自己的 JDK。 Azul Systems 提供了第一個這類封裝，其名稱為 Zulu，是以 OpenJDK 為基礎，而您現在可以使用此選項部署這個封裝。
...
* **移除選項，若要將本機 JDK 和本機應用程式伺服器納入部署套件的一部分。**若要將這些元件內嵌在封裝中，最好是在部署期間從雲端儲存體下載 JDK 和應用程式伺服器，因為下載這些項目會導致較小的部署封裝大小、更快速的部署時間以及更容易進行維護。 因此，我們移除了將 JDK 和應用程式伺服器納入部署封裝的選項。 已設定為將本機 JDK 和本機應用程式伺服器納入成為部署封裝一部分的現有專案，會自動轉換為將 JDK 和應用程式伺服器自動上傳到雲端儲存體。
* **Azure SDK 2.1 版本的支援。**適用於 Eclipse 的 Azure 外掛程式 - 2013 年 8 月預覽需要 Azure SDK 2.1。 請勿將 2013 年 8 月預覽用於舊版 Azure SDK，此外，也請勿將 Azure SDK 2.1 用於舊版適用於 Eclipse 的 Azure 外掛程式。
...
* **若要建立新的儲存體帳戶的能力。**A **新增** ] 按鈕已加入至 **加入儲存體帳戶** ] 對話方塊。 這可讓您在 Eclipse 外掛程式內建立儲存體帳戶，而不需要您登入 Azure 管理入口網站  (您必須已經有 Azure 訂用帳戶才能使用此功能)。如需建立新的儲存體帳戶的詳細資訊，請參閱 [來建立新的儲存體帳戶][]。
* **New & quot;(auto) 告訴我們理由。使用自動部署 JDK 和伺服器，以及用於快取儲存體帳戶的選項。**使用時 **自動上傳** 選項 JDK 和應用程式伺服器，您現在可以指定 **(自動)** 上載 JDK 時要使用的 URL 和儲存體帳戶和應用程式伺服器，或使用 Azure 快取時。 然後，這些功能會自動使用相同的儲存體帳戶與您在中選取一個 **Publish to Azure** ] 對話方塊。  [在 Eclipse 中建立 Azure 的 Hello World 應用程式][] 教學課程已更新為使用新 **(自動)** 選項。
* **設定 Azure 服務端點的能力。**指定服務端點以決定應用程式要部署至全域 Azure 平台、由位於中國的 21Vianet 所運作的 Azure 或私人 Azure 平台，並由該平台進行管理。 如需詳細資訊，請參閱 [Azure 服務端點][]。
* **大型部署可以指定本機儲存體資源。**如果您的部署太大，預設的 approot 資料夾無法容納時，您現在可以指定本機儲存資源來做為 JDK 和應用程式伺服器的部署目的地。 如需詳細資訊，請參閱 [部署大型部署][]。
* **支援 A6 和 A7 Azure 虛擬機器的大小。**您現在可以將雲端服務部署到高記憶體的 A6 和 A7 虛擬機器大小。 如需這些大小的詳細資訊，請參閱 [虛擬機器和雲端服務大小][]。
...
* **JDK 和應用程式伺服器，以及從 Azure 儲存體部署自動上傳。**這個新選項會視需要將選取的 JDK 和應用程式伺服器自動上傳至指定的 Azure 儲存體帳戶，並從該處部署這些元件，而不必在部署封裝中內嵌或讓使用者屆時手動上傳。 這個普遍要求的功能可以大幅提高部署 JDK 和伺服器元件的便利性，對新手使用者來說更是如此。 使用這些選項的逐步解說，請參閱 [在 Eclipse 中建立 Azure 的 Hello World 應用程式][]。
* **集中式儲存體帳戶追蹤，以及參考儲存體帳戶詳細輕鬆 (透過下拉式控制項) 的能力。**這適用於多個依賴儲存體的功能，例如 JDK 和伺服器元件部署，以及快取。 如需詳細資訊，請參閱 [Azure 儲存體帳戶清單][]。
* **在 [發佈到雲端精靈 」 簡化遠端存取設定。**您只需要輸入使用者名稱和密碼就能啟用遠端存取，或是將欄位空白以讓遠端存取保持停用。
* **Azure libraries for Java 程式庫封裝更新。**這根據版本 0.4.2 [Microsoft Azure 用戶端 API][]。
* **Windows Server 2012 上自黏工作階段支援。**黏性工作階段先前只能在 Windows Server 2008 R2 上運作，但現在兩種雲端作業系統目標皆支援工作階段親和性。
* **封裝上傳效能改進。**即使 JDK 和應用程式伺服器內嵌於部署封裝中，部署程序的上傳部分仍會比舊版快上近兩倍。
