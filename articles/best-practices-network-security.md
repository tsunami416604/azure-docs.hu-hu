...
[範例 6 – 新增混合式連線與 ExpressRoute](#example-6-adding-a-hybrid-connection-with-expressroute)</br>
...
>[AZURE.TIP] 維護齒輪，誰有權存取周邊網路的安全性完全不同的授權應用程式開發/部署作業的系統管理員身分的人員。 將這些群組隔開可區分權責，防止單一個人略過應用程式安全性和網路安全性控制。
...
一旦解答上述問題，知道 [快速啟動](#fast-start) 如上一節可協助您識別是最適合特定案例的範例。
...
>[AZURE.TIP] 一般而言，您應該先建立特定的 「 允許 」 規則，然後更一般的 「 拒絕 」 規則的最後一個。 指定的優先順序決定要先評估哪些規則。 一旦發現流量適用特定規則，就不會再評估後續規則。 NSG 規則可以套用在輸入或輸出方向 (從子網路的觀點出發)。
...
>[AZURE.NOTE] 有目前 UDR 和混合式網路限制。 這會在未來版本中解決，關於如何使用 ExpressRoute DMZ 或站對站網路功能來啟用 DMZ 的範例，將在下面的範例 3 和 4 中討論。
...
>[AZURE.NOTE] 如果您使用 ExpressRoute 啟用 Azure 公用對等互連的選項時，必須要建立靜態路由至公司的網際網路邊緣或不是透過 ExpressRoute WAN 邊緣的 NVA VPN IP 位址的路由。 這是因為 ExpressRoute [Azure 公用對等] 選項所需的 NAT 很可能中斷 VPN 工作階段 (IPSec 通常不喜歡 NAT)。
...
>[AZURE.NOTE] 如果您使用 ExpressRoute 啟用 Azure 公用對等互連的選項時，必須要建立靜態路由至您公司的網際網路邊緣或不是透過 ExpressRoute WAN 邊緣的 Azure VPN 閘道 IP 位址的路由。 這是因為 ExpressRoute [Azure 公用對等] 選項所需的 NAT 很可能中斷 VPN 工作階段 (IPSec 通常不喜歡 NAT)。
...
>[AZURE.TIP] 使用 ExpressRoute 會更好的安全性，為網際網路上的公司網路流量，大幅增加效能，並允許 Sla ExpressRoute 提供者所提供。 在 ExpressRoute 效能方面，Azure 閘道在 ExpressRoute 上的傳遞速度高達 2Gbps，而使用站對站 VPN 時，Azure 閘道的最大輸送量為 200Mbps。
