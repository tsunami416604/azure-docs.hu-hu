...
For more examples of queries you can use, see [query object][query object].

##### Update entry

Update a row in a data table. In this example, todoItem is the updated item, and item is the same item as returned from the mobile service. When the mobile service responds, the item is updated in the local todoItems list using the [splice][splice] method. Call the [done][done] method on the returned [Promise][done] object to get a copy of the inserted object and handle any errors.
...
Delete a row in a data table. Call the [done][done] method on the returned [Promise][done] object to get a copy of the inserted object and handle any errors.

    todoTable.delete(todoItem).done(function (item) {
        todoItems.splice(todoItems.indexOf(item), 1);
    }

[Learn more about mobile services][Learn more about mobile services]
  ...
  [query object]: http://msdn.microsoft.com/library/azure/jj613353.aspx
