...
功能表中可用的選項取決於目前選取的資料資產的類型。 當然，並非所有可能的工具將會包含在 「 在中開啟。...」 功能表上，但它並又容易連接至資料來源使用任何用戶端工具。 在 **Azure 資料目錄**入口網站選取資料資產後，完整的位置會顯示在屬性窗格中。
...
使用 **Azure 資料目錄**註冊資料來源，即可藉由將資料來源的結構性和描述性中繼資料複製到「目錄」服務內，使其變成可探索的資料。 資料來源註冊，並後發現，使用者可以連線到資料來源從 **Azure 資料目錄** 入口網站 」 中開啟。..."」 功能表或使用所選擇的資料的工具。
