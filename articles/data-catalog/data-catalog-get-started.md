...
-   **Azure 訂用帳戶** -如果您沒有訂閱，您可以建立免費試用帳戶只需要幾分鐘的時間。 請參閱 [免費試用版](http://azure.microsoft.com/pricing/free-trial/) 文章以取得詳細資料。
-   **Azure Active Directory** -Azure 資料目錄使用 [Azure Active Directory](http://azure.microsoft.com/services/active-directory/) 身分識別與存取管理。
-   **資料來源** -Azure 資料目錄提供功能來探索資料來源，並繼續進行本教學課程您必須能夠存取一個或多個資料來源。 本教學課程是使用 Adventure Works 範例資料庫來編寫，但如果喜歡使用您熟悉且與您角色相關的資料，您可以使用任何支援的資料來源。
...
> [AZURE.NOTE] 這個練習中是選擇性的。 本教學課程的其餘練習都編寫成參考 Adventure Works 範例資料庫，但是您也可以選擇略過這個練習，使用您自己的資料來源。
...
2.  向下捲動並按一下 [ **發行資料**。
    ...
    ![register2][2]
3.  按一下 [ **啟動應用程式**。
4.  在 **歡迎** 頁面上，按一下 **登入**, ，並輸入您的認證。
5.  在 **Microsoft Azure 資料目錄** 頁面上，按一下 **SQL Server**。

    ![register3][3]
6.  輸入您 **伺服器名稱**, ，然後按一下 **連接**。
7.  下一頁將註冊您的資料來源的中繼資料。 在此範例中，您將註冊 **產品** AdventureWorks Production 命名空間中的物件。 作法如下：
    ...
    c. 按一下移動選取箭號 (**>**)。 這會將所有 Product 物件到 **註冊** 清單。

    ![register5][5]

    d. **選擇性**︰ 您可以 **包括預覽**, ，和 **新增資料來源專家**。

    e. 在 **將標記加入**, ，輸入描述、 相片。 這會加入這些資料資產的搜尋標記。 標記可協助使用者尋找已註冊的資料來源，非常有用。

    f. 按一下 [ **註冊**。 Azure 資料目錄會註冊您選取的物件。 本練習中會註冊從 Adventure Works 選取的物件。

    ![register6][6]

    g. 若要查看您已註冊的資料來源物件，請按一下 [ **檢視入口網站**。 在 Azure 資料目錄入口網站中，您可以檢視中的資料來源物件 **磚** 或 **清單**。
...
**Azure 資料目錄** 提供簡單但功能強大的搜尋語法，可讓您輕鬆建立查詢來傳回資料的使用者需要。 如需詳細資訊 **Azure 資料目錄**, ，請參閱 < 搜尋語法參考。
...
在此範例中，您將使用關鍵字搜尋。 **Azure 資料目錄** 搜尋有幾種查詢技巧。 此範例將使用 **群組** 搜尋查詢。

**查詢技術**
<table><tr><td><b>技巧</b></td><td><b>使用</b></td><td><b>範例</b></td></tr><tr><td>內容範圍</td><td>只會傳回搜尋字詞與指定內容相符的資料來源</td><td>name:product</td></tr><tr><td>邏輯運算子</td><td>依本頁「布林運算子」一節所述，使用布林運算子擴大或縮小搜尋</td><td>finance NOT corporate</td></tr><tr><td>使用括號分組</td><td>使用括號將查詢部分分組，以達到邏輯隔離，尤其是與布林運算子結合。</td><td>name:product AND (tags:illustration OR tags:photo)</td></tr><tr><td>比較運算子</td><td>針對具有數值和日期資料類型的內容進行比較而非相等</td><td>creationTime: & gt; 11/05/14</td></tr></table>

在此範例中，您執行 **分組** 搜尋的資料資產，其中 name 等於 product 且 tags 等於 illustration 或 tags 等於 photo。

1.  登入 **Azure 資料目錄** 入口網站。
2.  按一下 [ **探索**。
3.  在 **搜尋** 方塊中，輸入 **群組** 查詢: (標記 ︰ description OR 標記 ︰ photo)。
4.  按一下搜尋圖示，或按 Enter 鍵。 **Azure 資料目錄** 會顯示這個搜尋查詢傳回的資料資產。

    ![search][8]

在本練習使用 **Azure 資料目錄** 來探索及檢視資料資產的入口網站已註冊的類別目錄。

## 練習 4：加註已註冊的資料來源

在本練習中，您將使用 **Azure 資料目錄** 入口網站來加註先前已註冊在目錄中的資料資產。 您提供的註解可補充並增強在註冊期間擷取自資料來源的結構化中繼資料，將更容易探索和了解資料資產。 因為每個 **資料目錄** 使用者可以提供自己的註解，就很容易讓每一位使用者資料的觀點來共用它。

### 以下說明如何加註資料資產

1.  登入 **Azure 資料目錄** 入口網站。
2.  按一下 [ **探索**。
3.  選擇一或多個 **資料資產**。 在此範例中，選擇 [ **ProductPhoto**, ，並輸入 「 行銷資料產品相片 」。
4.  輸入 **描述** ，幫助其他人探索並了解原因，以及如何使用選取的資料資產。 例如，輸入「產品影像」。 您也可以加入更多的標記，並檢視資料行。
...
> [AZURE.NOTE] 如果您沒有另一位使用者才能使用本教學課程，別擔心 ！ 任何存取資料目錄的使用者都可以選擇加入自己的觀點。 這種關於中繼資料的群眾外包方法，可讓目錄的內容和目錄中繼資料的豐富性隨著時間不斷成長。

### 以下說明您如何讓群眾外包資料資產的中繼資料

請同事重複 **加註已註冊的資料來源** 上述的練習。 當您的同事將描述加入至資料資產之後，例如 ProductPhoto，您就會看到多個註解。
...
在本練習中，您將使用 **Azure 資料目錄** 入口網站使用 Microsoft Excel 的資料來源連線。


> [AZURE.NOTE] 請務必記得 **Azure 資料目錄** 不會讓使用者存取實際的資料來源 – 這只會讓使用者輕鬆地探索和了解它們。 當使用者連接到資料來源時，他們所選擇的用戶端應用程式會使用其 Windows 認證，或在必要時提示他們提供認證。 如果先前未授權使用者存取資料來源，則他必須先獲得授權才能連線。
...
在本練習中，您連接到使用 Azure 資料目錄找到的資料來源。  **Azure 資料目錄** 入口網站可讓使用者直接使用用戶端應用程式整合到連線其 **中開啟...** 功能表上，並可讓使用者使用他們選擇使用包含在資產中繼資料的連接位置資訊的任何應用程式連線。

## 練習 7：移除資料來源中繼資料

在本練習中，您將使用 **Azure 資料目錄** 入口網站移除已註冊的資料資產的預覽資料，並從目錄中刪除資料資產。

> [AZURE.NOTE] 類別目錄的預設行為是允許任何使用者註冊任何資料來源，並允許任何使用者刪除任何已註冊的資料資產。 中包含的管理功能 **標準版 Azure 資料目錄** 提供其他選項來取得資產的所有權、 限制可以探索資產，以及限制誰可以刪除資產。

在 **Azure 資料目錄**, ，您可以移除預覽而直接刪除個別資產或刪除多個資產。
...
在本練習中，您將使用的管理功能 **Azure 資料目錄** 取得資料資產，並控制哪些使用者可以探索和管理這些資產的擁有權。

注意：本練習所述的管理功能只在標準版 Azure 資料目錄中提供，免費版本中沒有提供。
在 **Azure 資料目錄**, ，您可以取得資料資產的擁有權將共同擁有者加入資料資產，並設定資料資產的可見性。
...
在本教學課程，您瀏覽的基本功能 **Azure 資料目錄** 預覽狀態，包括註冊、 加註、 探索和管理企業資料來源。 既然您已經完成本教學課程，現在可以開始使用。 您可以立即開始註冊您和小組所依賴的資料來源，並邀請同事使用目錄。
