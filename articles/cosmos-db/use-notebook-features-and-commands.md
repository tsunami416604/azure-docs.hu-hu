...
- Cserélje ``{database_id}`` le ``{container_id}`` az és az nevet az Azure Cosmos-fiók adatbázisának és tárolójának nevére. Ha a ``--database`` és ``--container`` az argumentumok nincsenek megadva, a rendszer végrehajtja a lekérdezést az [alapértelmezett adatbázison és tárolón](#set-default-database-for-queries).
- Cserélje ``{url_location_of_file}`` le a helyére a JSON-fájl helyét. A fájlnak érvényes JSON-objektumokból álló tömbnek kell lennie, és a nyilvános interneten keresztül elérhetőnek kell lennie.
...
A beépített [nteract adatkezelő](https://blog.nteract.io/designing-the-nteract-data-explorer-f4476d53f897) használatával szűrheti és megjelenítheti a DataFrame. A funkció engedélyezéséhez állítsa a ``pd.options.display.html.table_schema`` kívánt értékre ``True`` a ``pd.options.display.max_rows`` és a értéket ( ``pd.options.display.max_rows`` ``None`` az összes eredmény megjelenítéséhez).
...
```
> [!IMPORTANT]
> A ``COSMOS_ENDPOINT`` és ``COSMOS_KEY`` a környezeti változók csak az SQL API-ra alkalmazhatók. Más API-k esetén keresse meg a végpontot és a kulcsot az Azure Cosmos-fiókjának **kapcsolatok karakterláncok** vagy **kulcsok** paneljén.  
