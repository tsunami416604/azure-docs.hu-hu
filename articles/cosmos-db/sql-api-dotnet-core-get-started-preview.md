---
title: Konzolalkalmazás létrehozása .NET Core (a SDK verziója 3 előzetes verzió) az Azure Cosmos DB SQL API-fiókban lévő adatok kezelése
...
Üdvözli az Azure Cosmos DB SQL API első lépései a .NET Core oktatóanyaggal! Ez az oktatóanyag lépéseinek követésével egy .NET Core-konzolalkalmazást hoz létre, és a lekérdezések az Azure Cosmos DB-erőforrásokat. Ebben az oktatóanyagban [3.0-s vagy újabb](https://www.nuget.org/packages/Microsoft.Azure.Cosmos) , az Azure Cosmos DB .NET SDK, mely tárolók [.NET Standard 2.0.](https://docs.microsoft.com/dotnet/standard/net-standard)
...
Nincs ideje az alkalmazás létrehozására? Ne aggódjon! A teljes megoldás elérhető a [GitHubon](https://github.com/Azure-Samples/cosmos-dotnet-core-getting-started). A gyors utasításokért ugorjon [A teljes megoldás beszerzése szakaszra](#GetSolution).
...
- Ha nincs telepítve a Visual Studio 2017, letöltheti és használhatja az ingyenes [Visual Studio 2017 Community Editiont](https://www.visualstudio.com/downloads/). Ha Univerzális Windows-platformon futó alkalmazást (UWP-alkalmazást) fejleszt, használja a **Visual Studio 2017 15.4-es** vagy újabb verzióját. Győződjön meg arról, hogy engedélyezze a **Azure-fejlesztési** számítási feladatot a Visual Studio telepítése során.
...
1. Az a **új projekt** párbeszédablakban válassza **Visual C#**   /  **Console App (.NET Core)**, nevezze el a projektet, és kattintson **OK**.
   ![Képernyőfelvétel az Új projekt ablakról](./media/sql-api-dotnetcore-get-started/dotnetcore-tutorial-visual-studio-new-project.png)
1. A **Megoldáskezelőben** kattintson a jobb gombbal az új konzolalkalmazásra, amely a Visual Studio megoldás alatt található, majd kattintson a **NuGet-csomagok kezelése...** lehetőségre.
   ...
   ![A Projekt jobb gombos kattintással elérhető menüjének képernyőfelvétele](./media/sql-api-dotnetcore-get-started/dotnetcore-tutorial-visual-studio-manage-nuget.png)
...
Remek! Most, hogy befejeztük a beállítást, lássunk neki a kód megírásának! A [GitHubon](https://github.com/Azure-Samples/cosmos-dotnet-core-getting-started/) megtalálhatja az oktatóanyagban szereplő kódprojekt befejezett változatát.
...
1. Végponti URL-cím és az elsődleges kulcs beszerzéséért az [az Azure portal](https://portal.azure.com).
   ...
   ![Képernyőkép Azure Cosmso DB kulcsainak lekérése az Azure Portal használatával](./media/sql-api-get-started/dotnet-tutorial-portal-keys.png)
...
1. Másolja és illessze be az alábbi kódot, ahol a CosmosClient meghívásához példányosítani, a **CreateDatabase** metódus az előzőekben adott hozzá.
   ...
   Ezen a ponton a kód hasonlóan kell kinéznie, ez a végpont és az elsődleges kulcs kitölti a rendszer. Megjegyzés: a névtér eltér a projekt neve alapján.
...
1.  Másolja és illessze be a **termékcsalád**, **szülő**, **gyermek**, **Kisállat**, és **cím** be osztály**Family.cs**. Megjegyzés: a névtér eltér a projekt neve alapján.
...
1.  Lépjen vissza a **Program.cs** , és adja hozzá a **AddItemsToContainer** módszer alapján az **CreateContainer** metódust.
...
Válassza ki **F5** hozza létre az alkalmazást hibakeresési módban, a Visual studióban.

Az első lépések alkalmazás a konzolablakban a teljes kimenetnek kell megjelennie. A kimenet megjeleníti a hozzáadott lekérdezések eredményeit, amelynek meg kell egyeznie az alábbi mintaszöveggel.
...
- A GitHubon elérhető [GetStarted](https://github.com/Azure-Samples/cosmos-dotnet-core-getting-started) megoldás.

Az SQL API mutató hivatkozásokat visszaállítása az Azure Cosmos DB .NET Core SDK a Visual Studióban, kattintson a jobb gombbal a **GetStarted** megoldásra a Megoldáskezelőben, és válassza ki **NuGet-csomagok visszaállítása**. Ezután a **Program.cs** fájlt, frissítse a `EndpointUri` és `PrimaryKey` leírtak szerint értékek [csatlakozhat az Azure Cosmos DB-fiók](#Connect).

## <a name="next-steps"></a>További lépések

Ebben az oktatóanyagban bemutattuk, hogyan hozhat létre egy .NET Core-alkalmazást az Azure Cosmos DB SQL API-adatok kezeléséhez. Így már további adatokat importálhat a Cosmos DB-fiókba.
