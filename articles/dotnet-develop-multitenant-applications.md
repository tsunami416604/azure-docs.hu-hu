...
- **佈建**︰ 您必須佈建新的租用戶應用程式。  就租用戶為數眾多的多租用戶應用程式而言，通常必須藉由啟用自助式佈建功能將此程序自動化。
- **可維護性**︰ 您必須能升級應用程式並執行其他維護工作在多個租用戶使用。
- **監視**︰ 您必須要能夠監視應用程式隨時都以找出問題並進行疑難排解。 監視每個租用戶使用應用程式的情形，也是其中一環。

適當實作的多租用戶應用程式，可為使用者提供下列好處。

- **隔離**︰ 個別租用戶的活動不會影響其他租用戶應用程式使用。 租用戶之間無法相互存取資料。 對租用戶而言，應用程式就如同是他們專用的一般。
- **可用性**︰ 個別租用戶希望應用程式能持續可用性，甚或在 SLA 中的保證。 再次重申，其他租用戶的活動不會影響應用程式的可用性。
- **延展性**︰ 個別租用戶的需求調整的應用程式。 其他租用戶的存在與動作不會影響應用程式的效能。
- **成本**︰ 成本會比執行專用的單一租用戶應用程式，因為在多租用戶可共用資源。
...
如需有關多租用戶應用程式的其他設計考量的詳細資訊，請參閱 [多租用戶應用程式裝載於 Azure 上][]。
