...
description: Ismerteti, hogyan Cloud Foundry az Azure-szolgáltatások az a vállalati élmény
...
Bosh Azure CPI (Felhőbeli Provider Interface) használja, a lemez létrehozása és törlése rutinokat. Alapértelmezés szerint nem felügyelt lemezeket használnak. Az ügyfél manuálisan storage-fiókok létrehozásához, majd konfigurálja a fiókokat a CF-hez jegyzékfájlok van szükség. Ez a lemezek számát a korlátozás miatt.
...
Az Azure Log Analytics Nozzle egy Cloud Foundry összetevő, amely továbbítja a metrikák a [Cloud Foundry loggregator "firehose"](https://docs.cloudfoundry.org/loggregator/architecture.html) való [naplózza az Azure Monitor](https://azure.microsoft.com/services/log-analytics/). A Nozzle a collect, megtekintheti, és a CF-hez rendszer állapotának és teljesítményének metrikát elemezhet, több üzemelő alkalmazáspéldányok között.
Kattintson a [Itt](https://docs.microsoft.com/azure/cloudfoundry/cloudfoundry-oms-nozzle) való üzembe helyezése az Azure Log Analytics Nozzle a nyílt forráskódú és a Pivotal Cloud Foundry-környezetben is, és majd elérni az adatokat az Azure Monitor naplózza a konzolon. 
> [!NOTE]
> A PCF 2.0-s BOSH mérőszámok a virtuális gépek alapértelmezés szerint a Loggregator "firehose" továbbítja, és az Azure Monitor naplók konzol integrálva vannak.
...
Prémium szintű lemezeket is megbízható teljesítmény, éles környezetben ajánlott.  A [Managed Disk](https://azure.microsoft.com/services/managed-disks/), Standard szintű tárolást is biztosít hasonló megbízhatóságát, különböző teljesítmény. Amely nem a teljesítményigényes számítási feladatokhoz, például fejlesztési, tesztelési vagy a nem kritikus fontosságú környezet a standard szintű felügyelt lemezek és az alacsonyabb költségek egy másik lehetőséget kínálnak.  
...
#### <a name="use-azure-first-party-services"></a>Az Azure első nyilvános szolgáltatások: 
