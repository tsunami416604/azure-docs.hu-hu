...
8. 選取 **設定-設定必要的設定**:  
![設定 Office 365 使用者 API 設定][4]

9. 輸入 *用戶端識別碼* 和 *用戶端密碼* Office 365 Azure Active Directory (AAD) 應用程式。 如果您還沒有這些值，請參閱本主題中的＜註冊 AAD 應用程式搭配 PowerApps 使用＞一節來建立您需要的金鑰與密碼。  
...
8. 設定 **回覆 URL** 下 _OAuth 2_ 區段 （在本主題中） 在 Azure 入口網站中加入新的 Office 365 使用者 API 時收到的重新導向 url。 選取 **新增應用程式**:  
![Contoso AAD 應用程式設定][11]

9. 在 **其他應用程式的權限** 視窗中，選取 **Office 365 統一的 API （預覽）**, ，然後選取 **確定**。

10. 在 [設定] 頁面中，請注意， _Office 365 統一的 API （預覽）_ 加入至 _其他應用程式的權限_ 清單。

11. 選取 **委派的權限** 的 _Office 365 統一的 API （預覽）_, ，然後選取 **讀取所有使用者的基本設定檔** 權限。

隨即會建立新的 Azure Active Directory 應用程式。 您可以在 Azure 入口網站的 Office 365 使用者 API 組態中使用此應用程式。 
