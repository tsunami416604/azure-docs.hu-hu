...
如果您先前已使用組織帳戶註冊 Azure 訂用帳戶，則您已經有租用戶！ 在 [Azure 傳統入口網站](https://manage.windowsazure.com/), ，租用戶下方會列出 **所有項目** 並同時在 [ **Active Directory**。 您可以視需要任意使用此租用戶。 您也可以使用底部工作列中的 [新增]**** 功能表建立新的租用戶。
...
5. 接下來，輸入名稱，選取您想要使用的訂用帳戶，選取或建立新的資源群組，然後選取虛擬網路。 **注意事項** ，選擇虛擬網路之後，它就無法變更:  
![][6]  
...
- [加入新的 API、 新增連線時，並讓使用者存取](powerapps-manage-api-connection-user-access.md)
- [更新現有的 API 和其屬性](powerapps-configure-apis.md)
