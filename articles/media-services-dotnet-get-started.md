...
A C# Visual Studio project that contains the code for this tutorial is available here: [Download][Download].

This tutorial walks you through these basic steps:

-   [Setting up your project][Setting up your project]
-   [Getting the Media Services server context][Getting the Media Services server context]
-   [Creating an asset and uploading files that are associated with the asset into Media Services][Creating an asset and uploading files that are associated with the asset into Media Services]
-   [Encoding an asset and downloading an output asset][Encoding an asset and downloading an output asset]

## Prerequisites

The following prerequisites are required for the walkthrough and for development based on the Azure Media Services SDK.

-   A Media Services account in a new or existing Azure subscription. For details, see [How to Create a Media Services Account][How to Create a Media Services Account].
...
-   Install **Azure SDK for .NET.**, **Azure Media Services SDK for .NET**, and **WCF Data Services 5.0 for OData V3 libraries** and add references to your project using the [windowsazure.mediaservices Nuget][windowsazure.mediaservices Nuget] package. The following section demonstrates how to install and add these references.
...
## <span id="Step1"></span></a>Setting up your project
...
3.  If you have not done so yet, add references to **Azure SDK for .NET**.(Microsoft.WindowsAzure.StorageClient.dll), **Azure Media Services SDK for .NET** (Microsoft.WindowsAzure.MediaServices.Client.dll), and **WCF Data Services 5.0 for OData V3** (Microsoft.Data.OData.dll) libraries using the [windowsazure.mediaservices Nuget][windowsazure.mediaservices Nuget] package.

    To add references using Nuget, do the following. In Visual Studio Main Menu, select TOOLS -\> Library Package Manager -\> Package Manager Console. In the console window type *Install-Package [package name]* and press enter (in this case use the following command: *Install-Package windowsazure.mediaservices*.)
...
## <span id="Step2"></span></a>Getting the Media Services Context
...
## <span id="Step3"></span></a>Creating an Asset and Uploading a File
...
## <span id="Step4"></span></a>Encoding the Asset on the Server and Downloading an Output Asset

In Media Services, you can create jobs that process media content in several ways: encoding, encrypting, doing format conversions, and so on. A Media Services job always contains one or more tasks that specify the details of the processing work. In this section you create a basic encoding task, and then run a job that performs it using Azure Media Encoder. The task uses a preset string to specify the type of encoding to perform. To see the available preset encoding values, see [Task Preset Strings for Azure Media Encoder][Task Preset Strings for Azure Media Encoder] . Media Services support the same media file input and output formats as Microsoft Expression Encoder. For a list of supported formats, see [Supported File Types for Media Services][Supported File Types for Media Services].

1.  Add the following **CreateEncodingJob** method definition to your class. This method demonstrates how to accomplish several required tasks for an encoding job:
...
2.  Add a call to the **CreateEncodingJob** method in your **Main** method, after the lines you previously added.

        CreateEncodingJob(asset, _singleInputFilePath, _outputFilesFolder);

3.  Add the following helper methods to the class. These are required to support the **CreateEncodingJob** method. Following is a summary of the helper methods.
    -   The **GetLatestMediaProcessorByName** method returns an appropriate media processor to handle an encoding, encryption, or other related processing task. You create a media processor using the appropriate string name of the processor you want to create. The possible strings that can be passed into the method for the mediaProcessor parameter are: **Azure Media Encoder**, **Windows Azure Media Packager**, **Windows Azure Media Encryptor**, **Storage Decryption**.
...
-   [How to Use Media Services][How to Use Media Services]
-   [Building Applications with the Media Services REST API][Building Applications with the Media Services REST API]
...
<!-- Anchors. -->
  ...
  [Azure Free Trial]: http://www.windowsazure.com/en-us/pricing/free-trial/?WT.mc_id=A8A8397B5
  [Task Preset Strings for Azure Media Encoder]: http://msdn.microsoft.com/en-us/library/windowsazure/jj129582.aspx
  ...
  [How to Use Media Services]: http://azure.microsoft.com/en-us/develop/media-services/resources/
