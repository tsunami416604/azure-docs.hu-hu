...
請務必了解 DNS 記錄集和個別 DNS 記錄之間的差別。  記錄集是指一個區域中有相同名稱和相同類型的記錄集合。  如需詳細資訊，請參閱 [了解記錄設定，並記錄](dns-getstarted-create-recordset.md#Understanding-record-sets-and-records)。
...
Azure DNS 支援下列記錄類型： A、AAAA、CNAME、MX、NS、SOA、SRV、TXT。  每個區域會自動建立 SOA 類型的記錄集，無法另外建立。  請注意， [SPF 記錄型別已被取代 in favour of 建立 SPF 記錄使用 TXT 記錄類型的 DNS 標準](http://tools.ietf.org/html/rfc7208#section-3.1)。
...
您可以使用 `azure network dns record-set list` 命令列出 DNS 區域中的所有記錄：
...
>[AZURE.NOTE] CNAME 記錄，只允許一個單一字串值。 
...
您將使用 `azure network dns record-set delete-record` 從記錄集移除現有的值：
...
使用 `azure network dns record-set delete-record` 可從記錄集移除記錄。請注意，要移除的記錄必須完全符合現有的記錄，包括所有參數。
