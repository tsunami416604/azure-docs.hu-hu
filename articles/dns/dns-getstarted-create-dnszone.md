...
為了開始裝載您的網域，我們必須先建立 DNS 區域。網域的 DNS 區域內，將會針對特定網域建立的任何 DNS 記錄。<BR><BR>
...
使用 New-AzureRmDnsZone cmdlet 建立 DNS 區域。下列範例中，我們將建立稱為 'contoso.com' 中的資源群組稱為 'MyResourceGroup' 的 DNS 區域:<BR>

    PS C:\> New-AzureRmDnsZone -Name contoso.com -ResourceGroupName MyAzureResourceGroup

>[AZURE.NOTE] 在 Azure DNS 中，指定的區域名稱結尾不能有 ‘.’。例如，為 'contoso.com' 而非 'contoso.com.'。<BR>
