...
>[AZURE.NOTE] 您無法加入或移除記錄中自動建立 SOA 記錄集在區域頂點 (名稱 = ' @')，但您可以修改 SOA 記錄內的參數和記錄集 TTL。
...
>[AZURE.NOTE] 您無法加入、 移除或修改的記錄中自動建立的 NS 記錄集在區域頂點 (名稱 = ' @')。  修改記錄集 TTL 是唯一允許的變更。
...
>[AZURE.NOTE] 您無法刪除 SOA 和 NS 記錄集在區域頂點 (名稱 = ' @') 所自動建立時建立區域。  刪除區域時會自動刪除它們。
