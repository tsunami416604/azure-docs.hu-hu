...
若要建立新的 DNS 區域來主控您的網域，請使用 `azure 網路的 dns 區域建立`:
...
若要擷取 DNS 區域，請使用 `azure 網路的 dns 區域顯示`:
...
若要擷取的資源群組內的 DNS 區域，請使用 `azure 網路的 dns 區域清單`:

    azure network dns zone list myresourcegroup

## 更新 DNS 區域

DNS 區域資源可以變更使用 `azure 網路的 dns 區域集`。 這不會更新任何 DNS 記錄集內的區域 (請參閱 [如何管理 DNS 記錄](dns-operations-recordsets.md))。 它只用來更新區域資源本身的屬性。 這在目前限於區域資源的 Azure 資源管理員「標記」。 請參閱 [Etag 和標記](dns-getstarted-create-dnszone.md#Etags-and-tags) 如需詳細資訊。

    azure network dns zone set myresourcegroup contoso.com -t prod=value2

## 刪除 DNS 區域

您可以使用刪除 DNS 區域 `azure 網路的 dns 區域刪除`。
