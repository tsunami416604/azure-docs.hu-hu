...
Azure 會將服務劃分類別在管理入口網站和各種視覺輔助工具，像是 [什麼是 Azure 資訊圖表](http://azure.microsoft.com/documentation/infographics/azure/) 。 您可使用管理入口網站來管理 Azure 中的大部分 (但並非全部) 服務。 
...
#### 選擇計算模型
頁面 [Azure Web 應用程式、 雲端服務和虛擬機器之比較](choose-web-site-cloud-service-vm.md) 提供更詳細的資訊關於如何選擇運算模型。
...
- **巨量資料移轉** -每當您有大量的資料 (Tb) 想要上傳至 Azure 時，匯入/匯出服務通常是很多快，而且可能比透過網際網路傳輸還要便宜。 在資料上傳至 Blob 後，您可以將資料處理成其他格式，例如資料表儲存體或 SQL Database。
 
- **封存資料復原** -您可以使用匯入/匯出，要求 Microsoft 將傳輸大量資料儲存在 Azure Blob 儲存體的儲存裝置傳送，並要求將該裝置遞送到您想要的位置。 由於這需要一些時間，所以並不適合災難復原。 它比較適合您不需要快速存取的封存資料。
...
- **現有的應用程式移轉到雲端** -較為容易移轉內部部署至雲端使用檔案共用的應用程式部分之間共用資料的應用程式。 每個 VM 會連線到檔案共用，然後可以讀取和寫入檔案，就好像它會對內部部署檔案共用所採取的動作一樣。

- **共用應用程式設定** -分散式應用程式的常見模式是將組態檔從許多不同的虛擬機器進行存取的集中位置。 這些組態檔可以儲存在 Azure 檔案共用中，並由所有應用程式執行個體進行存取。 這些設定可以透過可允許全球存取組態檔的 REST 介面進行管理。

- **診斷共用** -您可以儲存並共用診斷檔案，如記錄、 度量及損毀傾印。 透過 SMB 和 REST 介面提供這些檔案可允許應用程式使用各種分析工具，進而處理與分析診斷資料。 

- **開發/測試/偵錯** -當開發人員或系統管理員在雲端、 虛擬機器上使用他們通常需要一組工具或公用程式。 在每台虛擬機器上安裝與散佈這些公用程式相當費時。 有了 Azure 檔案，開發人員或系統管理員可將他們最愛的工具儲存在檔案共用上，然後從任何虛擬機器連接到這些工具。
...
這些 SDK 可協助您建置、部署及管理 Azure 應用程式。 它們都是可以從 [www.microsoftazure.com](http://azure.microsoft.com/downloads/)  或 GitHub，而且它們可以搭配 Visual Studio 和 Eclipse。 此外，Azure 還提供了開發人員可搭配任何編輯器或開發環境使用的命令列工具，包括可供從 Linux 和 Macintosh 系統將應用程式部署到 Azure 的工具。 
...
**通知中樞案例**
如果您在撰寫一個玩家輪流進行的行動遊戲，您可能需要通知 2 號玩家 1 號玩家已結束她的回合。 如果這是您必須做的一切動作，則使用 Mobile Apps 就可以了。 但是如果您有 100,000 位使用者在玩這個遊戲，而且您想要傳送有時間急迫性的免費優惠給每位玩家，則通知中樞會是最佳選擇。
...
Azure Site Recovery (先前稱為 Hyper-V 復原管理員) 可透過在各個網站中協調複寫與復原，協助您保護重要的應用程式。 站台復原提供功能以保護您自己的次要站台、 主機服務提供者的網站或 Azure，根據 hyper-v、 VMWare 或 SAN 的應用程式，並避免的費用和複雜性建置與管理專屬次要位置。 Azure 會加密資料和訊息，您也可以選擇為靜態資料啟用加密。 

它會持續監控您的服務健全狀況，並在主要資料中心發生站台服務中斷時，協助自動化循序復原服務。 以組織的方式啟動虛擬機器，可協助快速還原服務，即使是複雜的多層次工作負載也不例外。

Site Recovery 使用現有的技術，例如 Hyper-V 複本、System Center 和 SQL Server AlwaysOn。 簽出 [Azure Site Recovery 概觀](site-recovery/site-recovery-overview.md) 如需詳細資訊。
...
如果您已經在使用 Windows Server 或 System Center，Azure 備份會是備份您的伺服器檔案系統、虛擬機器和 SQL Server 資料庫的自然解決方案。  它可與加密、疏鬆和壓縮檔案搭配使用。 有一些限制，因此您應先 [查看 Azure 備份必要條件](http://technet.microsoft.com/library/dn296608.aspx) 第一次。 
...
由於 Azure 隨時間不斷調整的方式，Azure 儲存體佇列和服務匯流排提供了類似的佇列服務。 在相當專門文章中有討論為何要使用的其他原因 [<LINK>](http://msdn.microsoft.com/library/azure/hh767287.aspx "Azure 佇列和服務匯流排佇列-比較和對照")。  在許多案例中，任一種都適用。 
