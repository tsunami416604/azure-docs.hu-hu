---
title: Apache Spark hipertér indexei
description: Apache Spark teljesítményoptimalizálása Hyperspace-indexekkel
...
## <a name="create-indexes"></a>Indexek létrehozása
...
## <a name="support-for-sql-semantics"></a>SQL-szemantika támogatása

Az index használata transzparens, hogy a DataFrame API-t vagy a Spark SQL-t használja-e. Az alábbi példa ugyanazt a csatlakoztatási példát mutatja be, mint korábban, az SQL-űrlapon, amely az indexek használatát mutatja be, ha van ilyen.
...
### <a name="plan-with-indexes"></a>Tervezés indexekkel
...
### <a name="plan-without-indexes"></a>Tervezés indexek nélkül
...
### <a name="indexes-used"></a>Használt indexek
...
### <a name="range-selection"></a>Tartomány kiválasztása
...
## <a name="hybrid-scan-for-mutable-datasets"></a>Változtatható adatkészletek hibrid vizsgálata

Gyakran előfordul, hogy ha az alapul szolgáló forrásadatok új fájlokat fűzött hozzá, vagy törölték a meglévő fájlokat, az index elavult lesz, és a hipertér úgy dönt, hogy nem használja. Vannak azonban olyan időpontok, amikor csak az indexet szeretné használni anélkül, hogy minden alkalommal frissítenie kellene. Ennek több oka is lehet:

- Nem kívánja folyamatosan frissíteni az indexet, hanem időnként érdemes rendszeresen megtennie, mert megérti a legmegfelelőbb munkaterheléseket.
- Csak néhány fájl lett hozzáadva/eltávolítva, és nem szeretné megvárni, hogy még egy újabb frissítési feladatot lehessen befejezni.

Ahhoz, hogy továbbra is használhassa az elavult indexeket, a hipertér bevezeti a hibrid vizsgálatot, egy új technikát, amely lehetővé teszi, hogy a felhasználók elavult vagy elavult indexeket használjanak

Ennek eléréséhez, ha a megfelelő konfigurációt a hibrid vizsgálat engedélyezéséhez állítja be, a hipertér a következőképpen módosítja a lekérdezési tervet a módosítások kihasználása érdekében:
* A hozzáfűzött fájlok egyesítése egyesíthető a Union vagy a BucketUnion (JOIN) használatával. Ha szükséges, a rendszer az egyesítés előtt is alkalmazhatja a hozzáfűzött hozzáfűzést.
* A törölt fájlokat a rendszer az indexek adatbányászati oszlopának szűrő-nem feltételének beírásával kezelheti, így a törölt fájlokból indexelt sorok kizárhatók a lekérdezés időpontjában.

A lekérdezési terv átalakítását az alábbi példákban tekintheti meg.

> [!NOTE]
> Jelenleg a hibrid vizsgálat csak a nem particionált adatértékek esetében támogatott.

### <a name="hybrid-scan-for-appended-files---non-partitioned-data"></a>Hozzáfűzött fájlok hibrid vizsgálata – nem particionált adatértékek

A nem particionált adatértékek a következő példában használatosak. Ebben a példában azt várjuk, hogy a JOIN index használható a lekérdezéshez és a BucketUnion a hozzáfűzött fájlokhoz.
...
A hibrid vizsgálat alapértelmezés szerint le van tiltva. Ezért látni fogja, hogy mivel új adathalmazokat fűzött hozzá, a hipertér úgy dönt, hogy *nem* használja az indexet.

A kimenetben nem fog megjelenni a terv különbsége (ezért nincs kiemelve).
...
### <a name="enable-hybrid-scan"></a>Hibrid vizsgálat engedélyezése

Az indexekkel való tervezés során megtekintheti a csak a hozzáfűzött fájlok esetében szükséges Exchange-kivonatolási particionálást, így továbbra is használhatjuk a "véletlenszerű" indexelt adatokat a hozzáfűzött fájlokkal. A BucketUnion használatával összevonhatja a "megkevert" hozzáfűzött fájlokat az indexbe.
...
## <a name="incremental-index-refresh"></a>Növekményes index frissítése

Ha készen áll az indexek frissítésére, de nem szeretné újjáépíteni a teljes indexet, a hipertér növekményes módon támogatja az indexek frissítését az `hs.refreshIndex("name", "incremental")` API használatával. Így nem kell teljesen újraépíteni az indexet a semmiből, a korábban létrehozott indexeket használva, valamint az indexek frissítését csak az újonnan hozzáadott adatokon.

Természetesen ügyeljen arra, hogy a kiegészítő API-t `optimizeIndex` (lásd alább) rendszeresen használja, hogy biztosan ne jelenjen meg a teljesítmény-regresszió. Javasoljuk, hogy legalább egyszer hívja meg az optimalizálást, ha a hívása 10 alkalommal `refreshIndex(..., "incremental")` történik, feltéve, hogy a hozzáadott vagy eltávolított adat < az eredeti adatkészlet 10%-a. Ha például az eredeti adatkészlet 100 GB, és az 1 GB-os növekmények/csökkentések során hozzáadott/eltávolított adatokat, a hívás `refreshIndex` előtt 10 alkalommal hívhat meg `optimizeIndex` . Vegye figyelembe, hogy ez a példa csak illusztrációként használható, és ezt a számítási feladatokhoz kell igazítania.

Az alábbi példában egy rendezési csomópont hozzáadását láthatja a lekérdezési tervben az indexek használatakor. Ennek az az oka, hogy a részleges indexek a hozzáfűzött adatfájlokon jönnek létre, így a Spark bevezeti a-t `Sort` . Azt is vegye figyelembe, hogy az `Shuffle` Exchange továbbra is ki van zárva a csomagból, így a megfelelő gyorsítást biztosít.
...
## <a name="optimize-index-layout"></a>Az index elrendezésének optimalizálása

A növekményes frissítéseknek az újonnan hozzáfűzött adatokon többszöri meghívása után (például ha a felhasználó kis kötegekben ír adatot, vagy adatfolyam-forgatókönyvek esetén), az indexelő fájlok száma általában nagy hatással lesz az index teljesítményére (nagy számú kis fájl probléma). A hipertér `hyperspace.optimizeIndex("indexName")` API-t biztosít az index elrendezésének optimalizálása és a nagyméretű fájlok problémáinak csökkentése érdekében.

Az alábbi tervben figyelje meg, hogy a hipertér eltávolította a további rendezés csomópontot a lekérdezési tervben. Az optimalizálással elkerülhető a rendezés bármely olyan index-gyűjtő esetében, amely csak egy fájlt tartalmaz. Ez azonban csak akkor igaz, ha az összes index gyűjtő legfeljebb 1 fájlt tartalmaz egy gyűjtőn `optimizeIndex` .
...
### <a name="optimize-modes"></a>Optimalizálási módok

Az optimalizálás alapértelmezett módja a "gyors" mód, amelyben az előre meghatározott küszöbértéknél kisebb fájlok vannak kiválasztva az optimalizáláshoz. Az optimalizálás hatásának maximalizálása érdekében a hipertér az alább látható módon lehetővé teszi egy másik "teljes" optimalizálási mód használatát. Ez a mód az összes indexelt fájlt kiválasztja az optimalizálásra, a fájl méretétől függetlenül, és létrehozza az index lehető legjobb elrendezését. Ez az alapértelmezett optimalizálási módhoz is lassabb, mert itt több adatfeldolgozás történik.
...
* [Projekt hipertér](https://microsoft.github.io/hyperspace/)
