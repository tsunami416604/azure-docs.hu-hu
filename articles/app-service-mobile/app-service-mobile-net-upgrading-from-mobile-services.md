...
升級的第一個步驟是建立將主控新版應用程式的行動 App 資源。 如果您已經移轉現有的行動服務，則您會想要在同一個主控方案中建立這個版本。 開啟 [Azure 入口網站]，並瀏覽至您已移轉的應用程式。 請記下其正在其上執行的 App Service 方案。
...
您可能會想要使用與行動服務中相同的資料庫和通知中心。 您可以開啟 [Azure 入口網站] 並瀏覽至原始的應用程式，來複製這些值，然後按一下 **設定** > **應用程式設定**。 在 **連接字串**, ，複製 `MS_NotificationHubConnectionString` 和 `MS_TableConnectionString`。 瀏覽至新的升級網站並將它們貼上，覆寫任何現有的值。 針對您的應用程式需要的任何其他應用程式設定重複執行此程序。 如果不使用移轉的服務，您可以讀取連接字串和應用程式設定，從 **設定** 的行動服務] 區段的 [Azure 傳統入口網站] 索引標籤。

針對您的應用程式製作 ASP.NET 專案的複本，然後將它發佈到新的網站。 透過利用新 URL 更新的用戶端應用程式複本，驗證一切運作正常。

## 更新伺服器專案

行動應用程式提供了一個新 [行動應用程式伺服器 SDK]，提供許多與行動服務執行階段相同的功能。 首先，您應該移除對行動服務封裝的所有參考。 在 [NuGet 套件管理員中，搜尋 `WindowsAzure.MobileServices.Backend`。 大部分的應用程式會看到數個封裝，包括 `WindowsAzure.MobileServices.Backend.Tables` 和 `WindowsAzure.MobileServices.Backend.Entity`。 在這種情況下，啟動與最低封裝的相依性樹狀目錄中，例如 `實體`, ，並將它移除。 出現提示時，請勿移除所有相依的封裝。 重複此程序，直到您移除了 `WindowsAzure.MobileServices.Backend` 本身。

此時，您的專案將不再參考行動服務 SDK。

接下來，將新增 Mobile Apps SDK 的參考。 針對這個升級，大部分的開發人員會想要下載並安裝 `Micrsoft.Azure.Mobile.Server.Quickstart` 封裝，這將需要整組中提取。
...
>[AZURE.NOTE] 如果您想要了解有關新的.NET 伺服器 SDK 的詳細資訊，以及新增/移除功能從您的應用程式，請參閱 < 如何使用.NET 伺服器 SDK > 主題。

如果您的應用程式會使用驗證功能，您也必須註冊 OWIN 中介軟體。 在此情況下，您應該將上述組態程式碼移入新的 OWIN 啟動類別。

1. 新增 NuGet 封裝 `Microsoft.Owin.Host.SystemWeb` 如果它尚未包含在專案中。
2. 在 Visual Studio 中，以滑鼠右鍵按一下專案，然後選取 [新增]**** -> [新項目]****。 依序選取 [Web]**** -> [一般]**** -> [OWIN 啟動類別]****。
3. 移動上述程式碼中的 MobileAppConfiguration 從 `WebApiConfig.Register()` 至 `則` 啟動您的新類別的方法。
...
在 Azure 行動服務伺服器 SDK 中，系統屬性永遠包含雙底線 (`__`) 屬性的前置詞:
...
在 iOS 上，您應該變更資料實體的核心資料結構描述，來符合下列內容。 請注意，屬性 `createdAt`, ，`updatedAt` 和 `版本` 不再需要 `ms_` 前置詞:
...
在 Azure 行動服務，系統屬性不會傳送預設情況下，但要求使用查詢字串時，只 `__systemProperties`。 相反地，在 Azure Mobile Apps 中，會**一律選取**系統屬性，因為它們是伺服器 SDK 物件模型的一部分。

這項變更主要會影響的網域管理員，例如擴充功能的自訂實作 `MappedEntityDomainManager`。 在行動服務中，如果用戶端永遠不會要求任何系統屬性，就可以使用 `MappedEntityDomainManager` 實際上未對應的所有屬性。 不過，在 Azure Mobile Apps 中，這些未對應的屬性將導致 GET 查詢發生錯誤。

若要解決此問題最簡單的方式是修改您 Dto 繼承自 `ITableData` 而不是 `EntityData`。 然後，新增 `[NotMapped]` 的欄位應該省略此屬性。

例如，下列範例會定義 `TodoItem` 不使用系統內容:
...
注意: 如果您收到錯誤訊息 `NotMapped`, ，將參考加入組件 `System.ComponentModel.DataAnnotations`。
...
Mobile Apps 中並未內建排程的工作，因此您在 .NET 後端中的任何現有工作都必須個別升級。 其中一個選項是行動應用程式程式碼網站上建立排程 [Web 工作]。 您也可以設定保存工作程式碼的控制器，並設定 [Azure 排程器]，達到預期的排程在端點上。

### 其他變更

現在必須有可供行動用戶端的所有 ApiControllers `[MobileAppController]` 屬性。 預設不再包含此屬性，因此，其他 ApiControllers 不會受到行動格式器所影響。
...
任何執行個體 `[AuthorizeLevel(AuthorizationLevel.User)]` 屬性現在必須變更為使用標準 ASP.NET `Authorize` 屬性。 此外，根據預設，控制器現在是匿名的，就如同在其他 ASP.NET 應用程式中。
...
如果您的應用程式使用自訂的驗證解決方案，您會想要確定已升級的網站具備系統的存取權。 遵循 [.NET 伺服器 SDK 概觀] 中的自訂驗證整合解決方案的新指示。 請注意，自訂驗證元件仍處於預覽狀態。
