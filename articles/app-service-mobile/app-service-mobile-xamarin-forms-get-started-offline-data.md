---
title: Offline szinkronizálás engedélyezése (Xamarin. Forms) | Microsoft Docs
description: Ismerje meg, hogyan használható App Service Mobile App a Xamarin. Forms-alkalmazás offline állapotának gyorsítótárazásához és szinkronizálásához.
...
---
# <a name="enable-offline-sync-for-your-xamarinforms-mobile-app"></a>Offline szinkronizálás engedélyezése a Xamarin. Forms Mobile alkalmazás
...
Az offline szinkronizálási kódot a rendszer a C# előfeldolgozó irányelvek használatával tartalmazza a projektben. Ha a **kapcsolat\_nélküli\_szinkronizálás engedélyezve** szimbóluma van megadva, a rendszer ezeket a kód elérési utakat tartalmazza a buildben. Windows-alkalmazások esetén az SQLite platformot is telepítenie kell.
...
4. Választható Minden Windows-alkalmazás projektben kattintson a jobb gombbal **a hivatkozás** > **hozzáadása...** elemre, bontsa ki a **Windows** mappa > **bővítmények**elemet.
...
Íme egy rövid áttekintés arról, hogy mi is szerepel az oktatóanyag kódjában az `#if OFFLINE_SYNC_ENABLED` irányelvek belsejében. Az offline szinkronizálási funkció a TodoItemManager.cs Project-fájlban található a hordozható osztály könyvtára projektben. A szolgáltatás fogalmi áttekintését lásd: [Offline adatszinkronizálás az Azure Mobile Appsban][2].
...
1. A Megoldáskezelő nyissa meg a Constants.cs projektfájlt a **hordozható** projektből, és módosítsa `ApplicationURL` úgy a értékét, hogy az egy érvénytelen URL-címre mutasson:
...
5. Választható A Visual Studióval megtekintheti a Azure SQL Database táblázatát, és megtekintheti, hogy a háttér-adatbázis adatai nem módosultak-e.
    ...
    A Visual Studióban nyissa meg a **Server Explorert**. Navigáljon az adatbázishoz az **Azure**->**SQL Database**-ben. Kattintson a jobb gombbal az adatbázisra, és válassza **a Megnyitás a SQL Server Object Explorerban**lehetőséget. Most megkeresheti az SQL Database-táblázatot és annak tartalmát.
...
1. Nyissa meg újra a Constants.cs. Javítsa ki `applicationURL` a pontot a megfelelő URL-címre.
