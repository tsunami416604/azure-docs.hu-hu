...
> [AZURE.NOTE] 此 SDK 目前為預覽狀態。 因此，不建議您在生產環境中使用此 SDK。 這份文件中的範例使用 v2.0.0-beta1 的 [azure 行動-應用程式]。
...
您可以找到 GitHub 的 [範例目錄] 中的每個使用案例的範例。

### <a name="howto-cmdline-basicapp"></a>如何: 建立基本的 Node.js 後端使用命令列
...
此應用程式建立簡單的行動裝置最佳化 WebAPI 與單一端點 (`/資料表/TodoItem`) 提供
...
您可以找到 [GitHub 上 basicapp 範例] 這個基本應用程式的程式碼。

### <a name="howto-vs2015-basicapp"></a>如何: 使用 Visual Studio 2015 建立節點的後端

Visual Studio 2015 需要延伸模組才能在整合式開發環境 (IDE) 內開發 Node.js 應用程式。 若要開始，下載並安裝 [Node.js 工具 1.1 for Visual Studio]。 安裝 Node.js Tools for Visual Studio 後，請建立 Express 4.x 應用程式：
...
### <a name="download-quickstart"></a>如何: 下載 Node.js 後端快速入門程式碼專案使用 Git

當您使用入口網站的 [快速入門]**** 刀鋒視窗建立新的 Node.js 行動應用程式後端時，系統會為您建立新的 Node.js 專案並將其部署至您的網站。 您可以在入口網站中新增資料表和 API，並編輯 Node.js 後端的程式碼檔案。 您也可以使用各種部署工具的其中一個來下載後端專案，以便新增或修改資料表和 API，然後重新發佈專案。 如需詳細資訊，請參閱 [Azure 應用程式服務部署指南]。 下列程序會使用 Git 儲存機制下載的快速入門專案程式碼。
...
2. 請依照 [啟用 web 應用程式儲存機制](../app-service-web/web-sites-publish-source-control.md#Step4) 啟用後端網站，記下的部署使用者名稱和密碼的 Git 儲存機制。

3. 在您的行動應用程式後端的刀鋒視窗中，記下 [Git 複製 URL]**** 設定。

4.  執行 `git 複製` 命令感知 Git 的命令列工具使用 Git 複製 URL，請輸入密碼時所需，如下列範例所示:
...
6. 在您變更專案檔案之後，請執行下列命令來新增、認可變更，然後將變更上傳至網站：
    ...
    當您將新檔案加入專案時，您必須先執行 `的 git add。` 命令。

每次有一組新的認可推送至網站時，就會重新發佈網站。

### <a name="howto-publish-to-azure"></a>如何: 將您的 Node.js 後端發佈至 Azure

Microsoft Azure 提供許多將 Azure App Service Mobile Apps Node.js 後端發佈至 Azure 服務的機制。 其中包括使用整合至 Visual Studio 中的部署工具、命令列工具，和以原始檔控制為基礎的連續部署選項。 如需有關本主題的詳細資訊，請參閱 [Azure 應用程式服務部署指南]。
...
此 WebAPI 支援 [OData]，並擴充資料表結構描述，以支援 [離線資料同步]。
...
您可以找到完整的範例中的 [待辦事項範例 GitHub 上]。

### <a name="howto-staticschema"></a>如何: 定義使用靜態結構描述的資料表
...
如果您以靜態方式定義資料表，則您還必須呼叫 tables.initialize() 方法，以在啟動時建立資料庫結構描述。 Tables.initialize() 方法會傳回 [承諾]-這用來確保 web 服務不會服務要求之前正在初始化資料庫。

### <a name="howto-sqlexpress-setup"></a>如何: 使用 SQL Express 做為本機電腦上開發資料存放區
...
Azure 行動應用程式 Node.js SDK 來建立及使用 SQL Express 和 SQL 資料庫的連線，使用 [mssql Node.js 封裝]。 要使用此封裝，您必須在 SQL Express 執行個體上啟用 TCP 連線。
> [AZURE.TIP] 記憶體驅動程式未提供完整的測試工具集。 如果您想要測試您的後端在本機，
...
Azure Mobile Apps 會從本機檔案系統讀取名為 _azureMobile.js_ 的 JavaScript 檔案。 您不應使用此檔案在生產環境中設定 Azure 行動應用程式 SDK-改為使用 [Azure 入口網站] 中的應用程式設定。 _azureMobile.js_ 檔案應會匯出組態物件。 最常見的設定如下：
...
儲存在雲端中。 在 [Azure 入口網站] 中的應用程式設定設定生產環境設定。

### <a name="howto-appsettings"><a>設定行動應用程式的應用程式設定

在大部分的設定 _azureMobile.js_ 檔案 [Azure 入口網站] 中有對等應用程式設定。 使用下列清單來設定您
在應用程式設定的應用程式:
...
### <a name="howto-use-sqlazure"></a>如何: 使用 SQL 資料庫做為實際執行的資料存放區
...
### <a name="howto-tables-auth"></a>如何: 驗證所需資料表的存取權

如果您想要使用與資料表端點的應用程式服務驗證，您必須設定應用程式服務驗證 [Azure 入口網站] 中第一次。 如需
...
### <a name="howto-tables-disabled"></a>如何: 停用特定資料表作業的存取權
...
### <a name="howto-tables-query"></a>如何: 調整資料表作業搭配使用的查詢
...
用來將 OData 查詢轉換為可以處理資料後端的項目 [QueryJS] 物件。 簡單的相等案例
(如上述)，可以使用對應。 加入特定的 SQL 子句，相對而言也較簡單：

    context.query.where('myfield eq ?', 'value');

### <a name="howto-tables-softdelete"></a>如何: 設定資料表上的 [虛刪除
...
### <a name="howto-tables-seeding"></a>如何: 植入資料庫資料
...
如果您想要使用自訂 API 的應用程式服務驗證，您必須設定應用程式服務驗證 [Azure 入口網站] 中第一次。 如需
在 Azure App Service 中設定驗證的詳細檢閱組態指南 》，您想要使用的身分識別提供者:
...
### <a name="howto-customapi-basic"></a>如何: 定義簡單的自訂 API
...
每個參數都是標準 RESTful 動詞之一 - GET、POST、PATCH 或 DELETE。 方法是傳送所需的輸出的標準 [ExpressJS Middleware] 函式。

### <a name="howto-customapi-auth"></a>如何: 存取自訂 API 要求的驗證
...
- [監視 Azure 應用程式服務]
- [啟用 Azure 應用程式服務中的診斷記錄]
- [請執行疑難排解 Visual Studio 中的 Azure 應用程式服務]

### <a name="howto-diagnostic-logs"></a>如何: 寫入 Azure 行動應用程式的診斷記錄檔

Node.js 應用程式可存取多種不同的診斷記錄工具。 就內部而言，Azure 行動應用程式 Node.js SDK 會使用 [邱吉爾] 的 [診斷記錄。 啟用此選項會自動啟用偵錯模式，或是設定 **MS_DebugMode** 應用程式設定為 true，在 [Azure 入口網站]。 產生的記錄檔會出現在 [Azure 入口網站] 上的診斷記錄檔。

## <a name="in-portal-editing"></a>在入口網站程式碼編輯經驗
...
### <a name="work-easy-tables"></a>如何: 在 Azure 入口網站中使用簡單的資料表
...
### <a name="work-easy-apis"></a>如何: 在 Azure 入口網站中使用簡單的 Api
...
### <a name="online-editor"></a>如何: 編輯 Visual Studio Team Services 中的程式碼
