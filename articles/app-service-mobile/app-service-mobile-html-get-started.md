...
根據預設，您將從入口網站下載的用戶端快速入門專案會在連接埠 8000 的 localhost 上執行。 因此，接下來會啟用 CORS `http://localhost:8000/` 伺服器專案中。
...
4. 開啟 URL <a href="http://localhost:8000/" target="_blank">http://localhost:8000/ /</a> 啟動應用程式的網頁瀏覽器中。 用戶端應用程式會設定為連接到 Azure 中您的行動應用程式後端。

5. 在應用程式中，輸入有意義的文字，例如 _Complete the tutorial_， **輸入新工作**, ，然後按一下 [ **新增**。
