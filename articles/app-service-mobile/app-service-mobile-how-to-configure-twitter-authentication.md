...
1. 登入 [Azure portal], ，並瀏覽至您的應用程式。 複製您 **URL**。 您將使用此 URL 設定您的 Twitter 應用程式。

2. 瀏覽至 [Twitter Developers] 網站，使用您的 Twitter 帳戶認證登入並按一下 [ **建立新的應用程式**。

3. 輸入 **名稱** 和 **描述** 新應用程式。 在您的應用程式中貼上 **URL** 的 **網站** 值。 然後，針對 **回呼 URL**, ，貼上 **回呼 URL** 您稍早複製。 這是附加路徑，您的行動應用程式閘道 _/.auth/login/twitter/callback_。 例如，`https://contoso.azurewebsites.net/.auth/login/twitter/callback`。 請確實使用 HTTPS 配置。

    > [AZURE.NOTE]
    如果您使用應用程式服務閘道，而不應用程式服務驗證 / 授權功能重新導向 URL 改為使用閘道的 URL 與 _/signin-twitter_ 路徑。
