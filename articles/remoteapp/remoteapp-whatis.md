...
有關於 Azure RemoteApp 的問題嗎？ 請查看我們 [常見問題集](remoteapp-faq.md)。
...
- A **定域機組集合** 代管，並將程式資料儲存在雲端中。 使用者可以使用其 Microsoft 帳戶或與 Azure Active Directory 同步處理或同盟的公司認證進行登入，以存取應用程式。

    當您想要共用的應用程式不需要連接至貴公司私人網路上的任何資源時 (例如，透過 VPN 裝置)，請選擇雲端收藏。 如果應用程式使用網際網路、OneDrive 或 Azure 上的資源，則雲端收藏將適用於您。 它也是最快建立的。

- A **混合式收藏** 代管，並將資料儲存在 Azure 雲端，但也可讓使用者存取資料和儲存在本機網路上的資源。 使用者可以使用與 Azure Active Directory 同步處理或同盟的公司認證進行登入，以存取應用程式。
...
升級自訂的範本映像後，必須將新映像上傳至 Azure 雲端，然後更新收藏以使用新映像。 (您可以從 Azure RemoteApp **快速入門** 頁面或儀表板。)
