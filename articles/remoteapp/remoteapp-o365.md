<properties
    pageTitle ="Azure RemoteApp 使用 Office |Microsoft Azure 「 
    描述 = 「 了解 Office 與 Azure RemoteApp 如何共同運作 」
    服務 ="remoteapp"
    documentationCenter=""
    作者 ="lizap"
    管理員 ="mbaldwin"/ >
...
* * 嘿，您知道我們有新的更好的很快就會取代此發行項嗎? 簽出 [如何搭配 Azure RemoteApp 使用 Office 365 訂閱](remoteapp-officesubscription.md)。 它涵蓋了使用 Office 365 + Azure RemoteApp.* * 所需的所有資訊
...
RemoteApp 支援 Office 365 共用電腦啟用。 當您啟用 「 共用電腦啟用，並使用 [Office 部署工具](http://www.microsoft.com/download/details.aspx?id=36778) 進行安裝時，Office 365 ProPlus 會進行安裝但不會啟用。 當使用者登入包含 Office 365 的收藏時，Office 會檢查是否已為 Office 365 ProPlus 佈建使用者。 如果是，則 Office 會暫時啟用 Office 365 ProPlus - 這個啟用只會持續到該使用者登出服務為止。

若要使用 Office 365 共用電腦啟用，您必須建立 [自訂範本](remoteapp-create-custom-image.md) 並安裝 Office 365 ProPlus，遵循 [這些指示](https://technet.microsoft.com/library/dn782858.aspx)。

您可以管理使用者的 Office 365 授權在 [Office 365 系統管理入口網站](https://portal.office365.com/)。 閱讀更多資訊 [Office 365 服務方案](http://technet.microsoft.com/library/office-365-plan-options.aspx)。
