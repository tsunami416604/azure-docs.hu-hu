...
> [!IMPORTANT]
> Ha a táblák nem indexelve vannak, akkor a `log_queries_not_using_indexes` és a `log_throttle_queries_not_using_indexes` paramétereinek beállítása hatással lehet a MariaDB teljesítményre, mivel a nem indexelt táblákon futó összes lekérdezés a lassú lekérdezési naplóba kerül.<br><br>
> Ha hosszabb ideig szeretné naplózni a lassú lekérdezéseket, ajánlott `log_output` "None" értékre állítani. Ha a "file" értékre van állítva, a rendszer a helyi kiszolgáló tárolóba írja a naplókat, és hatással lehet a MariaDB teljesítményére. 
