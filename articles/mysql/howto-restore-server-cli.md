...
A-kiszolgáló parancson keresztüli létrehozásakor `az mysql server create` a `--geo-redundant-backup` paraméter határozza meg a biztonsági mentési redundancia beállítást. Ha `Enabled` a, Geo redundáns biztonsági mentések készülnek. Vagy ha `Disabled` a rendszer helyileg redundáns biztonsági mentést végez. 
