...
A-kiszolgáló `az mysql server create` parancson keresztüli létrehozásakor `--geo-redundant-backup` a paraméter határozza meg a biztonsági mentési redundancia beállítást. Ha `Enabled`a, Geo redundáns biztonsági mentések készülnek. Vagy ha `Disabled` a rendszer helyileg redundáns biztonsági mentést végez. 
...
Ha egy olyan kiszolgálót szeretne létrehozni, amely egy földrajzi redundáns biztonsági mentést `az mysql server georestore` használ, használja az Azure CLI-parancsot.
