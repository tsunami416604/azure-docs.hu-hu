---
title: Virtuális hálózat szolgáltatásvégpontjai – Azure Database for MySQL
description: Ez a témakör azt ismerteti, hogy a VNet-szolgáltatásvégpontok hogyan működnek az Azure Database for MySQL-kiszolgálóhoz.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
*A virtuális hálózati szabályok* egy tűzfalbiztonsági szolgáltatás, amely azt szabályozza, hogy az Azure Database for MySQL-kiszolgáló elfogadja-e a virtuális hálózatok bizonyos alhálózataiból küldött kommunikációt. Ez a cikk ismerteti, hogy miért a virtuális hálózati szabály szolgáltatás néha a legjobb megoldás az Azure Database for MySQL-kiszolgáló biztonságos engedélyezéséhez.

Virtuális hálózati szabály létrehozásához először egy [virtuális hálózatnak][vm-virtual-network-overview] (VNet) és egy [virtuális hálózati szolgáltatás végpontjának][vm-virtual-network-service-endpoints-overview-649d] kell lennie a szabályhoz. Az alábbi képen látható, hogyan működik a virtuális hálózati szolgáltatás végpontja az Azure Database for MySQL szolgáltatással:

![Példa a VNet-szolgáltatásvégpont működésére](media/concepts-data-access-and-security-vnet/vnet-concept.png)
...
**Virtuális hálózat szolgáltatás végpontja:** A [virtuális hálózati szolgáltatás végpontja][vm-virtual-network-service-endpoints-overview-649d] egy alhálózat, amelynek tulajdonságértékei egy vagy több hivatalos Azure-szolgáltatástípus-neveket tartalmaznak. Ebben a cikkben a **Microsoft.Sql**típusneve érdekel, amely az SQL Database nevű Azure-szolgáltatásra hivatkozik. Ez a szolgáltatáscímke a MySQL- és postgreSQL-szolgáltatások Azure Database szolgáltatására is vonatkozik. Fontos megjegyezni, ha a **Microsoft.Sql** szolgáltatáscímke egy VNet szolgáltatás végpontra, akkor konfigurálja a szolgáltatás végponti forgalmat az összes Azure SQL-adatbázis, az Azure Database for MySQL és az Azure Database for PostgreSQL-kiszolgálók az alhálózaton. 

**Virtuális hálózati szabály:** Az Azure Database for MySQL-kiszolgáló virtuális hálózati szabálya egy alhálózat, amely szerepel az Azure Database for MySQL-kiszolgáló hozzáférés-vezérlési listájában (ACL). Ahhoz, hogy az Azure Database for MySQL-kiszolgáló ACL-ben legyen, az alhálózatnak tartalmaznia kell a **Microsoft.Sql** típusnevet.

A virtuális hálózati szabály azt mondja az Azure Database for MySQL-kiszolgáló, hogy fogadja el a kommunikációt minden csomópont, amely az alhálózaton található.
...
### <a name="c-cannot-yet-have-azure-database-for-mysql-on-a-subnet-without-defining-a-service-endpoint"></a>C. Még nem lehet a MySQL Azure Database alhálózaton szolgáltatásvégpont definiálása nélkül

Ha a **Microsoft.Sql-kiszolgáló** a virtuális hálózat egyik alhálózatának csomópontja volt, a virtuális hálózaton belül minden csomópont kommunikálhat az Azure Database for MySQL-kiszolgálóval. Ebben az esetben a virtuális gépek kommunikálhatnak az Azure Database for MySQL anélkül, hogy a virtuális hálózati szabályok vagy IP-szabályok.

2018 augusztusától azonban az Azure Database for MySQL szolgáltatás még nem szerepel a közvetlenül egy alhálózathoz rendelhető szolgáltatások között.
...
Minden virtuális hálózati szabály a teljes Azure-adatbázis a MySQL-kiszolgálóra vonatkozik, nem csak a kiszolgálón lévő egyetlen adatbázisra. Más szóval a virtuális hálózati szabály a kiszolgáló szintjén érvényes, nem pedig az adatbázis szintjén.
...
- **Adatbázis-rendszergazda:** &nbsp; Frissítse a hozzáférés-vezérlési listát (ACL) a megadott alhálózat hozzáadásához az Azure Database for MySQL-kiszolgálóhoz.
...
> [!NOTE]
> Bizonyos esetekben az Azure Database for MySQL és a VNet-alhálózat különböző előfizetésekben vannak. Ezekben az esetekben a következő konfigurációkat kell biztosítania:
...
A MySQL-hez készült Azure Database esetében a virtuális hálózati szabályok szolgáltatása a következő korlátozásokkal rendelkezik:
...
- Az Azure Database for MySQL tűzfalában minden virtuális hálózati szabály egy alhálózatra hivatkozik. Mindezek a hivatkozott alhálózatok kell üzemeltetni ugyanabban a földrajzi régióban, amely az Azure Database for MySQL.
...
- A virtuális hálózati szolgáltatás végpontjainak bekapcsolása a MySQL-alapú Azure Database szolgáltatáshoz a **Microsoft.Sql** szolgáltatáscímke használatával lehetővé teszi az összes Azure Database-szolgáltatás végpontjait is: Azure Database for MySQL, Azure Database for PostgreSQL, Azure SQL Database és Azure SQL Data Warehouse.
...
- [Azure-adatbázis létrehozása és kezelése a MySQL virtuális hálózat szabályaihoz az Azure Portal használatával](howto-manage-vnet-using-portal.md)
- [Azure-adatbázis létrehozása és kezelése a MySQL virtuális hálózat szabályaihoz az Azure CLI használatával](howto-manage-vnet-using-cli.md)
