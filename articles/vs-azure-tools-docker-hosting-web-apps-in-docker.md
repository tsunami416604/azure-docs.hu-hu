...
您可以使用發佈應用程式裝載於 Azure 的新 Docker 主機，或在內部部署伺服器、 HYPER-V 或 Boot2Docker 主機 **自訂主機** 設定。 發佈您的應用程式到 Docker 主機之後，您可以使用 Docker 命令列工具來與發佈您的應用程式的目標容器互動。
...
1. 建立新的 ASP.NET Web 應用程式專案。 在主功能表上選擇 [ **檔案**, ，**新的專案**。 在 **C#**, ，**Web**, ，選擇 [ **ASP.NET Web 應用程式**。

1. 在清單中 **ASP.NET 5 Preview 範本**, ，選擇 [ **網站**。

1. 由於 web 應用程式會裝載/執行在 Docker 中，清除 **定域機組中的主機** 核取方塊，如果它已選取，然後選擇 **確定** ] 按鈕。
...
1. 在 ASP.NET 專案的內容功能表中選擇 **發行**。

1. 在 **選取發行目標** 區段 **發行 Web** 對話方塊方塊中，選擇 [ **Docker 容器** ] 按鈕。
...
1. 如果您已登入 Azure 帳戶，請跳至步驟 5。 如果您未登入帳戶，請選擇 **將帳戶加入** ] 按鈕。

    ![][2]

1. 在 **登入 Visual Studio** ] 對話方塊中，輸入您的 Azure 訂閱的電子郵件帳戶，然後選擇 **繼續** ] 按鈕。

1. 選擇 **新增** ] 按鈕以建立新的 Azure Docker VM，然後選擇 **確定** ] 按鈕。

    ![][3]

    請注意，您也可以選擇使用現有的 Docker 主機。 若要這樣做，請選擇在 **現有 Azure Docker 虛擬機器** 下拉式清單，而不是選擇 **新增** ] 按鈕。 這份清單不會只顯示容器主機，它會列出所有 Azure 租用戶中的所有 VM。

    或者，您可以選擇發佈到自訂的 Docker 主機。 請參閱 **提供自訂的 Docker 主機** 本主題稍後如需詳細資訊。

1. 輸入下列資訊在 **Microsoft Azure 上建立虛擬機器** 對話方塊。 當您完成時，選擇 [ **確定** ] 按鈕。 這會建立設定了 Docker 延伸模組的 Linux 虛擬機器。
...
1. 選擇省略符號 （...） 按鈕旁 **憑證目錄** 項目，然後將建立新資料夾，Docker 憑證，或瀏覽至現有的 Docker 憑證資料夾。

    如果找不到 VM 所需的 Docker 憑證，在項目旁會出現驚嘆號圖示，讓您知道找不到所需的憑證，並且繼續將會刪除然後重新建立任何現有的憑證。

1. 選擇 **確定** ] 按鈕，開始建立 VM。 您會收到訊息，說明正在 Azure 中建立虛擬機器。

    Visual Studio 會建立 Azure 資源管理員 (ARM) 範本檔案、參數檔案和 PowerShell 指令碼，讓您在未來可以再次執行命令。

    ![][7]

    Visual Studio 輸出之作業的進度 **輸出** 視窗。 Visual Studio 呼叫 PowerShell 指令碼來部署 VM。 指令碼會使用 Azure PowerShell 指令程式來部署 Azure 資源群組。 之後，另一個 PowerShell 指令碼會使用發出 Docker 命令來發佈，就好像您是手動建立 Docker 主機。

    佈建 Docker 主機可能需要一些時間，因此請檢查 [輸出] 視窗中的狀態，以得知作業何時完成。

1. Docker 主機完全在 Azure 中佈建之後，您可以在 Azure 入口網站上檢查您的帳戶。 您應該能夠看到新的虛擬機器在 **虛擬機器** Azure 入口網站上的類別。

1. 一旦 Docker 主機準備就緒時，請返回並發佈 Web 應用程式專案。 內容功能表中的 web 應用程式專案節點上 **方案總管] 中**, ，選擇 [ **發行**。 Visual Studio 會根據您所建立的 VM 建立發佈檔案。

1. 在 **連接** 索引標籤中 **發行 Web** 對話方塊方塊中，選擇 [ **驗證連線** 方塊，確定 Docker 主機已就緒。 如果您的連線狀況良好，選擇 [ **發行** ] 按鈕以發行 web 應用程式。

    第一次您將應用程式發行至 Docker 主機中，需要花時間下載任何 Docker 檔案中參考的基底映像 (例如 **FROM** *imagename*)。
...
1. 在 **選取 Docker 虛擬機器** 對話方塊中，選取 **自訂的 Docker 主機** 核取方塊。

    ![][5]

1. 選擇 **確定** ] 按鈕。

1. 在 **發行 Web** 對話方塊方塊中，將值加入至中設定 **CustomDockerHost** 區段，例如 ︰ 伺服器 URL、 映像名稱、 Dockerfile 位置和主應用程式和容器連接埠號碼。

    在 **Docker 進階選項** ] 區段中，您可以檢視或變更驗證和執行選項，以及 Docker 命令列。

    ![][6]

1. 您輸入所有必要的值之後，請選擇 **驗證連線** 按鈕，以確保 Docker 主機連線運作正常。

1. 如果連線運作正常，您可以選擇 **下一步** ] 按鈕以查看將發行的元件清單，或者您可以選擇 **發行** 專案立即發佈] 按鈕。
...
1. 指派 Docker 主機並對環境變數驗證。 若要這麼做，請在命令提示字元中輸入下列命令。 (替代的 Docker 主機的名稱 *NameofAzureVM*。)
