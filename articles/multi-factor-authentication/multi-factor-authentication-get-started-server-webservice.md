...
<center>![安裝程式](./media/multi-factor-authentication-get-started-server-webservice/sdk.png)</center>
...
- 如果反向 proxy 或防火牆座落 Mobile App Web 服務的 web 伺服器之前，而且正在執行 SSL 卸載，您可以編輯 Mobile App Web 服務的 web.config 檔案並加入下列機碼 <appSettings> 區段，讓行動裝置應用程式 Web 服務可以使用 http，而不是 https。不過都從行動應用程式到防火牆/反向 proxy 仍然需要 SSL。 <add key="SSL_REQUIRED" value="false"/>

### 安裝 Mobile App Web 服務

<ol>
<li>開啟 Azure 多因素驗證伺服器上的 Windows 檔案總管並瀏覽至安裝 Azure Multi-factor Authentication Server 的資料夾 (例如 C:\Program Files\Azure Multi-factor Authentication)。視需要針對要安裝 Mobile App Web 服務的伺服器選擇 32 位元或 64 位元版本的 Azure Multi-Factor AuthenticationPhoneAppWebServiceSetup 安裝檔案。將安裝檔案複製到網際網路對向伺服器。</li>

<li>網際網路對向網頁伺服器上，必須以系統管理員權限執行安裝程式檔案。若要這樣做最簡單的方式是開啟命令提示字元以系統管理員，並瀏覽至複製安裝檔案位置。</li>

<li>執行 Multi-factor AuthenticationMobileAppWebServiceSetup 安裝檔案、 視需要變更網站和虛擬目錄變更為簡短的名稱，例如"PA"。因為使用者必須輸入行動應用程式 Web 服務 URL 行動裝置在啟動過程中，建議使用簡短的虛擬目錄名稱。</li>

<li>完成之後的 Azure Multi-factor AuthenticationMobileAppWebServiceSetup 安裝，瀏覽至 C:\inetpub\wwwroot\PA (或虛擬目錄名稱為基礎的適當目錄) 並編輯 web.config 檔案。</li>

<li>找出 WEB_SERVICE_SDK_AUTHENTICATION_USERNAME 和 web_service_sdk_authentication_password 機碼並將值設定為使用者名稱和密碼的服務帳戶的 PhoneFactor Admins 安全性群組成員的群組 (請參閱上述的需求一節)。如果您之前已安裝 Azure Multi-Factor Authentication 使用者入口網站，這可能是當做該使用者入口網站之身分識別的帳戶。請務必將使用者名稱和密碼輸入行尾的引號之間 (value=””/>)。建議使用合格的使用者名稱 (例如 domain\username 或 machine\username)。</li>

<li>找出 pfMobile App Web Service_pfwssdk_PfWsSdk 設定，並將值從"http://localhost:4898/pfwssdk.asmx"變更為 Azure 多因素驗證 (例如 https://computer1.domain.local/MultiFactorAuthWebServiceSdk/PfWsSdk.asmx) 伺服器上執行的 Web 服務 sdk 的 url。由於此連接使用 SSL，所以您必須依據伺服器名稱參考 Web 服務 SDK，而不是 IP 位址，因為 SSL 憑證是針對伺服器名稱發行，所以使用的 URL 必須與憑證上的名稱相符。如果伺服器名稱無法解析為網際網路對向伺服器的 IP 位址，請在該伺服器上的主機檔案加入項目，讓 Azure Multi-Factor Authentication Server 的名稱能與其 IP 位址相對應。儲存 web.config 檔案之後已變更。</li>

<li>如果安裝網站的行動裝置應用程式 Web 服務下 (例如預設網站) 尚未與公開簽署的憑證繫結，在伺服器上安裝憑證如果不是已安裝，請開啟 IIS 管理員，並將憑證繫結至網站。</li>

<li>從任何電腦開啟網頁瀏覽器並瀏覽至安裝 Mobile App Web 服務的 URL (例如 https://www.publicwebsite.com/PA)。確定未出現任何憑證警告或錯誤。</li>
...
<center>![安裝程式](./media/multi-factor-authentication-get-started-server-webservice/mobile.png)</center>
