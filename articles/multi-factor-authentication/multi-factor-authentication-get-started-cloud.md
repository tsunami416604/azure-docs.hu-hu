...
<center>![定域機組](./media/multi-factor-authentication-get-started-cloud/cloud2.png)</center>
...
2. [建立多因素驗證提供者](#creating-an-azure-multi-factor-auth-provider)  或 [Azure AD Premium 或 Enterprise Mobility Suite 授權指派給使用者](#assigning-an-azure-ad-premium-or-enterprise-mobility-license-top-users)
...
3. [為您的使用者啟動多因素驗證](#turn-on-multi-factor-authentication-for-users)
    - 透過 Office 365 或 Azure 入口網站為使用者啟用 Azure MFA。 如需操作方式的相關資訊，請參閱後續小節。
4. [傳送電子郵件給使用者，通知他們有關 MFA](#send-email-to-end-users)
...
<center>![定域機組](./media/multi-factor-authentication-get-started-cloud/provider.png)</center>
...
<li>登入 Azure 入口網站以系統管理員。</li>
<li>在左側，選取 **Active Directory**。</li>
<li>在 Active Directory] 頁面上，按兩下目錄有您想要啟用的使用者。</li>
<li>在 [目錄] 頁面頂端，選取 **授權**。</li>
<li>在 [授權] 頁面中，選取 Active Directory Premium 或 Enterprise Mobility Suite，，然後按一下 [ **指派**。</li>

<center>![定域機組](./media/multi-factor-authentication-get-started-cloud/license.png)</center>

<li>在對話方塊中，選取您想要指派授權的使用者，然後按一下核取記號圖示，以儲存變更。</li>
...
:-------------: | :-------------: |:-------------: |:-------------: |
 ...
 已啟用| 使用者已註冊 Multi-Factor Authentication。| 號它們會繼續運作，直到完成註冊程序。| 使用者已啟用，但尚未完成註冊程序。系統會提示他們在下次登入時完成程序。
...
<center>![定域機組](./media/multi-factor-authentication-get-started-cloud/user.png)</center>

若要使用 Windows PowerShell 變更使用者的狀態，您可以使用下列程式碼。 您可以變更 `$st。狀態` 等於先前所述的狀態。
...
現在在雲端中設定多重要素驗證，您可以移動到下一步，請前往 [設定 Azure Multi-factor Authentication。](multi-factor-authentication-whats-next.md)您將會瞭解有關報告、詐騙警示、自訂語音訊息，以及 Azure Multi-Factor Authentication 提供之所有功能的相關資訊。
