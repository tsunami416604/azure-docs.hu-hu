...
description: "Ismerkedés az Azure blob storage használatával az ASP.NET Core projekt, a Visual Studio egy tárfiókot, a Visual Studio kapcsolódó szolgáltatások használatával történő kapcsolódás után"
services: storage
documentationcenter: 
author: camsoper
manager: wpickett
...
Ez az oktatóanyag bemutatja, hogyan írhat kódot az ASP.NET Core olyan gyakori forgatókönyveket tartalmaz, az Azure blob storage használatával. Forgatókönyvek például a blob tároló, létrehozása és feltöltése, listázása, letöltése és blobok törlése.
...
1. Az a **új projekt** párbeszédpanelen adja meg a beállításokat az alábbi ábrán kiemelt:
    ...
    ![Az ASP.NET Core-projekt létrehozása](./media/vs-storage-aspnet-core-getting-started-blobs/new-project.png)

1. Kattintson az **OK** gombra.

1. Az a **új ASP.NET projekt** párbeszédpanelen adja meg a beállításokat az alábbi ábrán kiemelt:

    ![Adja meg az MVC](./media/vs-storage-aspnet-core-getting-started-blobs/new-mvc.png)
...
1. Az a **kapcsolódó szolgáltatás hozzáadása** párbeszédablakban válassza **felhőalapú tárolás az Azure Storage**, majd válassza ki **konfigurálása**.
    ...
    ![Csatlakoztatott Service párbeszédpanelen](./media/vs-storage-aspnet-core-getting-started-blobs/connected-services.png)
...
>
> Egy Azure storage-fiók használatával is létrehozhatók [Azure PowerShell](../storage/common/storage-powershell-guide-full.md), [Azure CLI](../storage/common/storage-azure-cli.md), vagy a [Azure Cloud rendszerhéj](../cloud-shell/overview.md).
...
1. A a **Megoldáskezelőben**, kattintson a jobb gombbal **tartományvezérlők**, és a helyi menüből válassza ki a **Hozzáadás -> tartományvezérlő**.
    ...
    ![Vezérlő hozzáadása az ASP.NET Core MVC alkalmazások számára](./media/vs-storage-aspnet-core-getting-started-blobs/add-controller-menu.png)

1. A a **hozzáadása Scaffold** párbeszédablakban válassza **MVC-vezérlő - üres**, és válassza ki **Hozzáadás**.

    ![Adja meg az MVC-vezérlő típusa](./media/vs-storage-aspnet-core-getting-started-blobs/add-controller.png)

1. Az a **vezérlő hozzáadása** párbeszédpanelen, a tartományvezérlő nevét *BlobsController*, és válassza ki **Hozzáadás**.

    ![Neve az MVC-vezérlő](./media/vs-storage-aspnet-core-getting-started-blobs/add-controller-name.png)
...
Az alábbi lépéseket a tárfiókot, a kapcsolati karakterlánc használatával kapcsolódni metódus létrehozása **appsettings.json** , és hozzon létre egy hivatkozást egy tárolót.  A kapcsolati karakterlánc beállítása a **appsettings.json** formátumban lesznek elnevezve `<storageaccountname>_AzureStorageConnectionString`. 
...
1. Futtassa az alkalmazást, és válassza ki **Blob-tároló létrehozása** az alábbi képernyőfelvételhez hasonló eredmények megtekintése érdekében:
    ...
    ![Blobtároló létrehozása](./media/vs-storage-aspnet-core-getting-started-blobs/create-blob-container-results.png)
...
1. Futtassa az alkalmazást, és válassza ki **feltöltése a blob**.  A "success!" szót megjelenjen-e.
    ...
    ![Sikeres ellenőrzés](./media/vs-storage-aspnet-core-getting-started-blobs/upload-blob.png)
...
1. Futtassa az alkalmazást, és válassza ki **blobok listázása** az alábbi képernyőfelvételhez hasonló eredmények megtekintése érdekében:
    ...
    ![A BLOB listázása](./media/vs-storage-aspnet-core-getting-started-blobs/listblobs.png)
...
Ebben az oktatóanyagban megtanulta, hogyan tárolhatja, listában, és az ASP.NET Core segítségével Azure Storage blobs beolvasása.  Az Azure-ban való adattárolás további lehetőségeiről tekintse meg a többi szolgáltatás-útmutatót.
