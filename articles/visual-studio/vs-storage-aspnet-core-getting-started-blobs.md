...
2. A helyi menüben válassza az**új mappa** **hozzáadása** > elemet. Nevezze el az új mappa *blobokat*. 

1. A **megoldáskezelő**bontsa ki a **nézetek** mappát, majd kattintson a jobb gombbal a **Blobok**elemre.

4. A helyi menüben válassza a nézet **hozzáadása** > **View**elemet.
...
1. BLOB-tárolóban lévő Blobok listázásához használja a `CloudBlobContainer.ListBlobsSegmentedAsync` metódust. A `CloudBlobContainer.ListBlobsSegmentedAsync` metódus a értéket `BlobResultSegment`adja vissza. Ez olyan `IListBlobItem` objektumokat tartalmaz, amelyek `CloudBlockBlob`elhelyezhetők `CloudPageBlob`, vagy `CloudBlobDirectory` objektumokra. A következő kódrészlet a blob-tárolóban lévő összes blobot enumerálja. Minden blob a megfelelő objektumba kerül, a típusa alapján. A rendszer hozzáadja a nevét (vagy a esetén az `CloudBlobDirectory`URI-t) egy listához.
