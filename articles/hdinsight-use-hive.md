...
-   You must have provisioned an **HDInsight cluster**. For a walkthrough on how to do this with the Azure portal, see [Get started with HDInsight][Get started with HDInsight]. For instructions on the various other ways in which such clusters can be created, see [Provision HDInsight Clusters][Provision HDInsight Clusters].

-   You must have installed **Azure PowerShell** on your workstation. For instructions on how to do this, see [Install and configure Azure PowerShell][Install and configure Azure PowerShell].

**Estimated time to complete:** 30 minutes

## In this article

-   [The Hive usage case][The Hive usage case]
-   [Upload data for Hive tables][Upload data for Hive tables]
-   [Run Hive queries using PowerShell][Run Hive queries using PowerShell]
-   [Use Tez for improved performance][Use Tez for improved performance]
-   [Next steps][Next steps]

## <span id="usage"></span></a>The Hive usage case
...
## <span id="uploaddata"></span></a>Upload data for Hive tables
...
## <span id="runhivequeries"></span></a> Run Hive queries using PowerShell
...
1.  Open an Azure PowerShell console windows. The instructions can be found in [Install and configure Azure PowerShell][Install and configure Azure PowerShell].
...
3.  Set the variables in the following script and run it:
...
4.  Run the following script to define the HiveQL queries. You can choose to create an *internal* or *external* Hive table.
...
5.  Run the following script to create a Hive job definition:
...
6.  Run the following script to submit the Hive job:

        # Submit the job to the cluster 
        Select-AzureSubscription $subscriptionName
        $hiveJob = Start-AzureHDInsightJob -Cluster $clusterName -JobDefinition $hiveJobDefinition

7.  Run the following script to wait for the Hive job to complete:

        # Wait for the Hive job to complete
        Wait-AzureHDInsightJob -Job $hiveJob -WaitTimeoutInSeconds 3600

8.  Run the following script to print the standard output:
...
If required, you can also import the output of your queries into Microsoft Excel for further analysis. For instructions, see [Connect Excel to Hadoop with Power Query][Connect Excel to Hadoop with Power Query].
...
4.  Connect to the HDInsight cluster.

        # Connect to the cluster
        Use-AzureHDInsightCluster $clusterName

5.  Run the following script to invoke HiveQL queries:
...
6.  Print the output on the console.
    ...
    For more information about Here-Strings, see [Using Windows PowerShell Here-Strings][Using Windows PowerShell Here-Strings].

If required, you can also import the output of your queries into Microsoft Excel for further analysis. For instructions, see [Connect Excel to Hadoop with Power Query][Connect Excel to Hadoop with Power Query].

## <span id="usetez"></span></a>Using Tez For Improved Performance
...
This can submitted on a per query basis by placing this at the beginning of your query. One can also set this to be on by default on a cluster by setting the configuration value at cluster creation time. You can find more details in [Provisioning HDInsight Clusters][Provision HDInsight Clusters].

The [Hive on Tez design documents][Hive on Tez design documents] contain a number of details on the implementation choices and tuning configurations.
...
-   [Using Pig with HDInsight][Using Pig with HDInsight]
...
-   [Get started with Azure HDInsight][Get started with HDInsight]
  ...
  [Connect Excel to Hadoop with Power Query]: http://azure.microsoft.com/en-us/documentation/articles/hdinsight-connect-excel-power-query/
  ...
  [Hive on Tez design documents]: https://cwiki.apache.org/confluence/display/Hive/Hive+on+Tez
