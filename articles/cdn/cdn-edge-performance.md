...
1. 在 [CDN 設定檔] 刀鋒視窗中，按一下 [管理]**** 按鈕。
    ...
    ![[CDN 設定檔] 刀鋒視窗的 ](./media/cdn-edge-performance/cdn-manage-btn.png)
...
-------|------------
 ...
 HTTP 回應碼| 分解的 HTTP 狀態碼 (例如 200 確定，403 禁止，找不到 404，等等。)我們的邊緣伺服器傳遞給 HTTP 用戶端。圓形圖可讓您快速評估資產的提供方式。圖表下方提供每個回應碼的詳細統計資料。
