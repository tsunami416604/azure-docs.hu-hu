---
title: "Gyorsítótárazás működése az Azure Content Delivery Network |} Microsoft Docs"
...
Egy webes böngésző hasonló, szabályozhatja hogyan CDN gyorsítótárazását végzi gyorsítótár-irányelv fejlécek küldését. Gyorsítótár-irányelv fejlécek olyan HTTP-fejlécek, amely az eredeti kiszolgálóra általában adhatók hozzá. Bár ezek a fejlécek a legtöbb eredetileg is cím gyorsítótárazása az ügyfélböngészők, most is használják az összes köztes gyorsítótárak, például a tartalomtovábbító. Két fejléc segítségével határozza meg a gyorsítótár frissesség: `Cache-Control` és `Expires`. `Cache-Control`több naprakész, és elsőbbséget élvez `Expires`, ha teljesül. Is két típusa van (más néven érvényesség-ellenőrzők) érvényesítéséhez használt fejlécek: `ETag` és `Last-Modified`. `ETag`több naprakész, és elsőbbséget élvez `Last-Modified`, ha mindkét vannak meghatározva.  
...
- A kérelem fejlécében használatakor: Azure CDN alapértelmezés szerint figyelmen kívül hagyja.
- A válasz fejléce való használata esetén: Azure CDN eleget tegyen a következő `Cache-Control` irányelvek általános webes kézbesítési, nagy fájlok letöltésének és általános/video-a-demand médiaadatfolyam-továbbítást optimalizálás használata esetén:  
   ...
   - `private`: A tartalom csak egyetlen felhasználóhoz csak; nem tárolhatja a tartalmakat, amelyek a megosztott gyorsítótárak, például a CDN-t.
   - `no-cache`: A tartalom gyorsítótárazása, de a tartalom ellenőrizni kell, minden alkalommal, mielőtt továbbítása a gyorsítótárból. Egyenértékű `Cache-Control: max-age=0`.
...
`Pragma` 
   - Alapértelmezés szerint nem Azure CDN ezt a funkciót.
   - A HTTP 1.0; bevezetett örökölt fejléc támogatott a visszamenőleges kompatibilitás.
...
Alapértelmezés szerint DSA optimalizálásokat figyelmen kívül hagyja ezeket a fejléceket. Beállíthatja, hogyan az Azure CDN értékként kezelje-e ezek a fejlécek CDN gyorsítótárazási szabályokkal. További információkért lásd: [vezérlő Azure CDN szolgáltatás használata a szabályok gyorsítótárazással gyorsítótárazásának](cdn-caching-rules.md).
...
| Fájlméret         | 300 GB                 | <ul><li>Általános webes kézbesítési optimalizálási: 1,8 GB</li> <li>Médiaadatfolyam-továbbítást optimalizálásokat: 1,8 GB</li> <li>Nagy méretű fájlok optimalizálási: 150 GB</li> |
...
|                    | Verizon - általános webes kézbesítés | Verizon – dinamikus gyorsulás | Akamai - általános webes kézbesítés | Akamai - dinamikus gyorsulás | Akamai - nagy méretű fájl letöltése | Akamai - általános vagy videotartalom médiaadatfolyam |
|--------------------|--------|------|-----|----|-----|-----|
