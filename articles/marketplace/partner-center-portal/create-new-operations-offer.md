---
title: Új Dynamics 365 for Operations ajánlat létrehozása a kereskedelmi piacon
description: Új Dynamics 365 for Operations ajánlat létrehozása az Azure Piactéren, az AppSource-ban vagy a Felhőszolgáltató (CSP) programon keresztül a Microsoft Partner Center Kereskedelmi Piactér portálján történő listázáshoz vagy értékesítéshez.
author: dsindona
ms.author: dsindona
...
ms.locfileid: "80294356"
---
# <a name="create-a-new-dynamics-365-for-operations-offer"></a>Új Dynamics 365 for Operations ajánlat létrehozása

Ez a témakör bemutatja, hogyan hozhat létre új Dynamics 365 for Operations ajánlatot. [A Microsoft Dynamics 365 for Finance and Operations](https://dynamics.microsoft.com/finance-and-operations) egy vállalati erőforrás-tervezési (ERP) szolgáltatás, amely támogatja a fejlett pénzügyeket, műveleteket, gyártást és ellátásilánc-kezelést. A Dynamics 365 for Operations minden ajánlatának át kell esnie a tanúsítási folyamaton.

A Dynamics 365 for Operations ajánlatok létrehozásának megkezdéséhez győződjön meg arról, hogy először [hozzon létre egy Partnerközpont-fiókot,](./create-account.md) és nyissa meg a [Kereskedelmi piactér irányítópultját,](https://partner.microsoft.com/dashboard/commercial-marketplace/offers)és az **Áttekintés** lapot jelölje ki.
...
Válassza a **+ Új ajánlat** gombot, majd a Dynamics **365 for Operations** menüelemet. Megjelenik **az Új ajánlat** párbeszédpanel.

### <a name="offer-id-and-alias"></a>Ajánlatazonosító és alias

- **Ajánlatazonosító:** A fiókban lévő minden egyes ajánlat egyedi azonosítója. Ez az azonosító látható lesz az ügyfelek számára a piactéri ajánlat és az Azure Resource Manager-sablonok URL-címében (ha van ilyen). Az ajánlatazonosítónak kisalfanumerikus karaktereknek kell lennie (beleértve a kötőjeleket és az aláhúzásjeleket, de nem lehet szóköz). Ez az azonosító legfeljebb 50 karakterből állhat, és a **Létrehozás**gombra kerülése után nem módosítható.  Ha például itt adja meg az *1-es tesztajánlatot,* az ajánlat URL-címe a . `https://azuremarketplace.microsoft.com/marketplace/../test-offer-1`
...
Válasszon legalább egy és legfeljebb három kategóriát. Ezeket a kategóriákat arra használjuk, hogy az ajánlatot a megfelelő piactérkeresési területekre helyezzük. Ügyeljen arra, hogy hívja ki, hogy az ajánlat támogatja ezeket a kategóriákat az ajánlat leírása.
...
Ebben a szakaszban adja meg egy támogatási **partner** és egy mérnöki partner nevét, e-mail címét és **telefonszámát.** Ez az információ nem jelenik meg az ügyfelek számára, de elérhető lesz a Microsoft számára, és a kripta-partnerek rendelkezésére bocsáthatók.

A **Támogatási kapcsolattartó** csoportban adja meg a **támogatási URL-címet,** ahol a CSP-partnerek támogatást találhatnak az ajánlathoz.

### <a name="supporting-documents"></a>Igazoló dokumentumok

Itt legalább egy (és legfeljebb három) kapcsolódó marketingdokumentumot, például tanulmányokat, brosúrákat, ellenőrzőlistákat vagy prezentációkat adhat meg. Ezeknek a dokumentumoknak .pdf formátumúaknak kell lenniük.
...
Ebben a szakaszban megadhatja azokat a piacokat, amelyeken az ajánlatnak elérhetőnek kell lennie. Ehhez válassza a **Piacok szerkesztése lehetőséget,** amely megjeleníti a **Piacválasztás** előugró ablakot.

Alapértelmezés szerint nincs enek kiválasztva piac. Válassza ki legalább egy piac ot az ajánlat közzétételéhez. Kattintson **az Összes kiválasztása** lehetőségre, ha minden lehetséges piacon elérhetővé szeretné tenni az ajánlatát, vagy válassza ki a hozzáadni kívánt piacokat. Miután végzett, válassza a **Mentés lehetőséget.**

Az itt megadott beállítások csak az új akvizíciókra vonatkoznak; ha valaki már rendelkezik az alkalmazásával egy bizonyos piacon, és később eltávolítja ezt a piacot, azok az emberek, akik már rendelkeznek az ajánlattal ezen a piacon, továbbra is használhatják, de ezen a piacon nincs új ügyfél, aki megkapja az ajánlatát.

> [!IMPORTANT]
> Az Ön felelőssége, hogy megfeleljen a helyi jogi követelményeknek, még akkor is, ha ezek a követelmények nem szerepelnek itt vagy a Partnerközpontban.

Ne feledje, hogy még ha az összes piacot kiválasztja, a helyi törvények és korlátozások vagy egyéb tényezők megakadályozhatják, hogy bizonyos ajánlatok egyes országokban és régiókban felkerüljenek a listára.
...
Adja meg a megoldásazonosítót (GUID) a megoldáshoz.

A megoldásazonosító megkeresése:
1. A Microsoft Dynamics életciklus-szolgáltatások (LCS) szolgáltatásában válassza a **Megoldáskezelés**lehetőséget.
2. Válassza ki a megoldást, majd keresse meg a **megoldásazonosítót** a **Csomag áttekintése című témakörben.** Ha az azonosító üres, válassza a **Szerkesztés lehetőséget,** majd tegye közzé újra a csomagot, majd próbálkozzon újra.

### <a name="release-version"></a>Kiadási verzió

Válassza ki a Dynamics 365 for Finance and Operations verzióját, amelyen ez a megoldás működik.
...
- **Szerepkör neve** (kötelező): Adja meg az egyéni Dynamics 365 tesztmeghajtóban megadott biztonsági szerepkörnevét. Ez lesz hozzárendelve a felhasználónak a tesztvezetés során (például teszt-meghajtó-szerepkör).
...
- **Régió** (kötelező, egyválasztós legördülő lista): Jelenleg 26 Azure által támogatott régió van, ahol a tesztmeghajtó elérhetővé tehető. A logikai alkalmazás erőforrásai a kiválasztott régióban lesznek telepítve. Ha a logikai alkalmazás rendelkezik egy adott régióban tárolt egyéni erőforrásokat, győződjön meg arról, hogy a régió itt van kiválasztva. A legjobb módja annak, hogy a régió hoz rendelkezésre álló egyéni erőforrások teljes körű üzembe helyezése a logic app helyileg az Azure-előfizetésa a portálon, és ellenőrizze, hogy megfelelően működik-e, mielőtt ezt a kiválasztást.
...
- **Használati útmutató** (kötelező): A tesztvezetés élményének részletes átjárása. A felhasználói kézikönyvnek pontosan azt kell kiterjednie, hogy mit szeretne az ügyfélnek a tesztvezetés megtapasztalásából, és referenciaként kell szolgálnia az esetleges kérdéseikhez. A fájlnak PDF formátumúnak kell lennie, és feltöltés után el kell nevezni (legfeljebb 255 karakter).
...
Töltsön fel [egy testreszabási elemzési jelentést ebben](https://docs.microsoft.com/dynamics365/unified-operations/dev-itpro/dev-tools/customization-analysis-report) a szakaszban. Ez a jelentés a testreszabási és bővítménymodellek elemzésével jön létre, az ajánlott eljárások ra vonatkozó szabályok előre meghatározott készlete alapján.

A fájlnak .xls vagy .xlsx formátumúnak kell lennie. Ha egynél több jelentéssel rendelkezik, feltölthet egy .zip fájlt, amely tartalmazza az összes jelentést.

### <a name="does-solution-include-localizations"></a>A megoldás tartalmazza a honosításokat?

Válassza az **Igen** lehetőséget, ha a megoldás engedélyezi a helyi szabványok és szabályzatok használatát (például ha a különböző országok/régiók által megkövetelt különböző bérszámfejtési szabályokat fogadja el). Egyéb esetben a **Nem** választógombot jelölje be.

### <a name="does-solution-enable-translations"></a>Engedélyezi a megoldás a fordításokat?

Igen **válasz,** ha a megoldás szövege más nyelvekre is lefordítható. Egyéb esetben a **Nem** választógombot jelölje be.
