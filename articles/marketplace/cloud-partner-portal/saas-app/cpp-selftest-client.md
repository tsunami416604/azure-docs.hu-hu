---
title: Azure SaaS-alkalmazáshoz való teszteléséhez ügyfél |} A Microsoft Docs
description: Hogyan hozhat létre a teszteléséhez ügyfél előzetes ellenőrzése az Azure SaaS-alkalmazások számára.
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Azure SaaS-alkalmazáshoz való teszteléséhez ügyfél létrehozása
...
Az API-t az alábbi struktúrával rendelkezik.
...
|          DNSName          |         A virtuális gép által teszteléséhez fog DNS-nevét.          |
|          Felhasználó          |          Felhasználónév csatlakozik, és jelentkezzen be a virtuális géphez.         |
|           Jelszó         |         Csatlakozik, és jelentkezzen be a virtuális gép jelszava.          |
|          Operációs rendszer          |         A virtuális gép operációs rendszerének "Linux" vagy "Windows".          |
|         PortNo           |         Az open port száma a virtuális Géphez való csatlakozáshoz. A portszám általában "22" Linux és a "5986" a Windows rendszer.          |
...
Az alábbi képernyőfelvétel-készítés JSON terhelésiteszt-eredményei egy online JSON-megjelenítő megtekinteni egy példát mutat be.
...
Hívja az API-t a curl használatával, kövesse az alábbi lépéseket:
...
20. Hozzon létre egy titkos kulcsot a kulcs kiválasztásával **leírás** szövegmezőbe. Adja meg a következő mezőket:
    ...
    - A **érték**, másolja a kulcsot.
     ...
     >[!Important]
     >Nem láthatja a kulcs értékét, miután kilépett a kulcsok formájában.
...
Miután sikeresen tesztelését az Azure SaaS-alkalmazásnak, [az ajánlat közzététele](./cpp-publish-offer.md)