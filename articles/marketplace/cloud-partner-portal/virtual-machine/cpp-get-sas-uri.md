...
> [!IMPORTANT]
> 2020. április 13-ától kezdődően megkezdjük az Azure-beli virtuálisgép-ajánlatok felügyeletének áthelyezését a partneri központba. Az áttelepítés után létrehozhatja és kezelheti az ajánlatokat a partner Centerben. Az áttelepített ajánlatok kezeléséhez kövesse a [megosztott hozzáférési aláírás URI-azonosítójának beolvasása](https://docs.microsoft.com/azure/marketplace/partner-center-portal/common-sas-uri-issues) a virtuálisgép-rendszerképhez című témakör utasításait.

A közzétételi folyamat során meg kell adnia egy egységes erőforrás-azonosítót (URI) az SKU-hoz társított minden virtuális merevlemezhez (VHD). A minősítési folyamat során a Microsoftnak hozzá kell férnie a virtuális merevlemezekhez. Ez a cikk bemutatja, hogyan hozhatja ki a közös hozzáférésű aláírás (SAS) URI-JÁT minden virtuális merevlemezhez. Ezt az URI-t a Cloud Partner Portal **SKUs (SKU** ) lapján adhatja meg.
...
- `List`és `Read` az engedélyek elegendőek. `Write` Ne *adjon meg* vagy `Delete` ne férhessen hozzá.
...
2. Hozzon létre egy PowerShell`.ps1` -fájlt (fájlkiterjesztés), másolja a következő kódot, majd mentse helyileg.
...
3. Válassza **Az Azure-fiók hozzáadása** lehetőséget, és kattintson **a bejelentkezés**elemre.  Folytassa az Azure-fiókba való bejelentkezéshez szükséges lépéseket.
...
6. Megjelenik a **megosztott hozzáférés aláírása** párbeszédpanel. Adja meg a következő mezők értékeit:
   ...
   - **Engedélyek** – válassza ki `Read` a `List` és az engedélyeket.
...
9. Szerkessze a szövegfájlt. Szúrja be a VHD- `vhds` nevet a sas URL-cím karakterlánca után (adjon meg egy kezdő perjelet).  A végső SAS URI formátumának a következőket kell tartalmaznia:
...
- `sp=rl` Megjelenik az URI közepe felé. Ez a karakterlánc azt `Read` jelzi `List` , hogy a és a hozzáférés meg van adva.
