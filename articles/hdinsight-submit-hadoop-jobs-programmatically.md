...
-   An Azure HDInsight cluster. For instructions, see [Get started with HDInsight][Get started with HDInsight] or [Provision HDInsight clusters][Provision HDInsight clusters].
-   Install and configure Azure PowerShell. For instructions, see [Install and configure Azure PowerShell][Install and configure Azure PowerShell].

## In this article

-   [Submit MapReduce jobs using PowerShell][Submit MapReduce jobs using PowerShell]
-   [Submit Hive jobs using PowerShell][Submit Hive jobs using PowerShell]
-   [Submit Sqoop jobs using PowerShell][Submit Sqoop jobs using PowerShell]
-   [Submit MapReduce jobs using HDInsight .NET SDK][Submit MapReduce jobs using HDInsight .NET SDK]
...
-   [Submit Hive Jobs using HDInsight .NET SDK][Submit Hive Jobs using HDInsight .NET SDK]
-   [Next steps][Next steps]

## <span id="mapreduce-powershell"></span></a> Submit MapReduce jobs using PowerShell

Azure PowerShell is a powerful scripting environment that you can use to control and automate the deployment and management of your workloads in Azure. For more information on using PowerShell with HDInsight, see [Administer HDInsight using PowerShell][Administer HDInsight using PowerShell].

Hadoop MapReduce is a software framework for writing applications which process vast amounts of data. HDInsight clusters come with a jar file, located at *\\example\\jars\\hadoop-examples.jar*, which contains several MapReduce examples. This file has been renamed to hadoop-mapreduce-examples.jar on version 3.0 HDInsight clusters. One of the examples is for counting word frequencies in source files. In this session, you will learn how to use PowerShell from a workstation to run the word count sample. For more information on developing and running MapReduce jobs, see [Use MapReduce with HDInsight][Use MapReduce with HDInsight].

**To run the word count MapReduce program using PowerShell**

1.  Open **Azure PowerShell**. For instructions on opening the Azure PowerShell console window, see the [Install and configure Azure PowerShell][Install and configure Azure PowerShell].

2.  Set these two variables by running the following PowerShell commands:
...
3.  Run the following commands to create a MapReduce job definition:

        # Define the word count MapReduce job
        $wordCountJobDefinition = New-AzureHDInsightMapReduceJobDefinition -JarFile "wasb:///example/jars/hadoop-mapreduce-examples.jar" -ClassName "wordcount" -Arguments "wasb:///example/data/gutenberg/davinci.txt", "wasb:///example/data/WordCountOutput"

    There are two arguments. The first one is the source file name, and the second is the output file path. For more information of the wasb prefix, see [Use Azure Blob storage with HDInsight][Use Azure Blob storage with HDInsight].
...
5.  Run the following command to check the completion of the MapReduce job:

        # Wait for the job to complete
        Wait-AzureHDInsightJob -Job $wordCountJob -WaitTimeoutInSeconds 3600 

6.  Run the following command to check any errors with running the MapReduce job:
...
## <span id="hive-powershell"></span></a> Submit Hive jobs using PowerShell
...
1.  Open **Azure PowerShell**. For instructions of opening Azure PowerShell console window, see [Install and configure Azure PowerShell][Install and configure Azure PowerShell].
...
For more information about Hive, see [Use Hive with HDInsight][Use Hive with HDInsight].

## <span id="sqoop-powershell"></span></a>Submit Sqoop jobs using PowerShell

See [Use Sqoop with HDInsight][Use Sqoop with HDInsight].

## <span id="mapreduce-sdk"></span></a> Submit MapReduce jobs using HDInsight .NET SDK

The HDInsight .NET SDK provides .NET client libraries that makes it easier to work with HDInsight clusters from .NET. HDInsight clusters come with a jar file, located at *\\example\\jars\\hadoop-examples.jar*, which contains several MapReduce examples. One of the examples is for counting word frequencies in source files. In this session, you will learn how to create a .NET application to run the word count sample. For more information on developing and running MapReduce jobs, see [Use MapReduce with HDInsight][Use MapReduce with HDInsight].
...
You can install latest published build of the SDK from [NuGet][NuGet]. The instructions will be shown in the next procedure.
...
For the details of creating a .Net application for submitting MapReduce jobs, see [Submit MapReduce jobs using HDInsight .NET SDK][Submit MapReduce jobs using HDInsight .NET SDK].
...
## <span id="hive-sdk"></span></a> Submit Hive jobs using HDInsight .NET SDK

HDInsight clusters come with a sample Hive table called *hivesampletable*. In this session, you will create a .NET application to run a Hive job for listing the Hive tables created on HDInsight cluster. For a more information on using Hive, see [Use Hive with HDInsight][Use Hive with HDInsight].
...
-   [Get started with Azure HDInsight][Get started with HDInsight]
...
-   [HDInsight Cmdlet Reference Documentation][HDInsight Cmdlet Reference Documentation]
...
-   [Use Pig with HDInsight][Use Pig with HDInsight]
  ...
  [Create and Upload a Management Certificate for Azure]: http://msdn.microsoft.com/en-us/library/windowsazure/gg551722.aspx
  [Develop C# Hadoop streaming programs for HDInsight]: ../hdinsight-hadoop-develop-deploy-streaming-jobs/
  [HDInsight Cmdlet Reference Documentation]: http://msdn.microsoft.com/en-us/library/windowsazure/dn479228.aspx
