...
**d:Namespaces** 和 **d:Namespace** -此節點描述傳回的函式匯入 （URI 端點） 的 XML 中定義的命名空間。 後端服務所傳回的 XML 可能包含任意數目的命名空間，以區別所傳回的內容。 **所有這些命名空間，若用於 d:Map 或 d:Match XPath 查詢中，則需要列出。** d:Namespaces 節點包含 d:Namespace 節點集/清單。 其中每一個列出一個用於後端服務回應的命名空間。 以下是 d:Namespace 節點的屬性：
