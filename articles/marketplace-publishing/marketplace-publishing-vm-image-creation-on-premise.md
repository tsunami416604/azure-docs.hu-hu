...
1.  移至 **瀏覽** > **Vm**, ，然後選取 [部署的 VM。
2.  在 **設定**, ，請選取 **磁碟** 並排顯示，如此即會開啟 [磁碟] 分頁。

  ![繪圖](media/marketplace-publishing-vm-image-creation-on-premise/img01.png)

3.  選取 **OS 磁碟**, ，如此即會開啟另一個會顯示磁碟的內容，包括 VHD 位置的刀鋒視窗。
4.  複製此 Blob URL。
...
> [AZURE.NOTE] 本指南的建立時，若要下載 VHD 功能尚無法出現在新的 Microsoft Azure 入口網站。  
...
> [AZURE.NOTE] **儲存 Azure** 也有 **NumberOfThreads** 可以用於增加平行處理原則來充分運用可用的頻寬以下載的選項。
...
7.  在 [容器] 刀鋒視窗中，選取 **新增**, ，然後輸入容器名稱和容器的權限。 選取 **私人** 容器權限。

> [AZURE.TIP] 我們建議您建立一個容器，每個想要發佈的 SKU。
...
> [AZURE.NOTE] 這些命令假設目前的儲存體帳戶內容，已設定在 PowerShell 中。   請參閱 [設定 Azure PowerShell](marketplace-publishing-powershell-setup.md) 如需有關 PowerShell 安裝程式。
 
