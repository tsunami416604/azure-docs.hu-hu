...
**要求流程**的簡單概觀：
  ...
  `用戶端]-> [Azure Marketplace]-> [內容提供者的 Web 服務 (Get、 Post、 Delete Put)`

**資料流程**的方向相反：

  `用戶端 <-Azure Marketplace <-內容提供者的 web 服務`
...
摘錄 from above 連結:
    * 「 開放式資料通訊協定 (以下稱為 OData) 的用途是提供 CRUD 樣式作業 (建立、 讀取、 更新和刪除) 對資源公開為資料服務以 REST 為基礎的通訊協定。 「資料服務」是一個端點，其中含有公開自一或多個「集合」的資料，每個集合均包含零到多個「項目」，這些項目是由具類型的具名值組所組成。 OData 會發佈下 OASIS (先進的結構化資訊標準組織) 標準的 microsoft，讓想要的任何人都可以建立伺服器、 用戶端或工具，而不需權利金或限制 」。*
...
- 資料型別: [http://msdn.microsoft.com/library/bb399548(v=VS.100).aspx][datatypeslink]
...
> [AZURE.TIP] 檢視更多的 CSDL Web 服務範例，本文 [範例對應現有的 web 服務以透過 CSDLs OData](marketplace-publishing-data-service-creation-odata-mapping-examples.md)
