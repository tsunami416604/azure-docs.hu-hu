...
開發人員代表 API 管理服務執行個體中的使用者帳戶。 開發人員可以建立或邀請加入由系統管理員，或他們可以註冊從 [開發人員入口網站的 []][]。 每個開發人員都是一或多個群組的成員，而且可訂閱對那些群組授與可見度的產品。
...
如需 API 管理原則的完整清單，請參閱 [原則參考 []][]。 如需有關使用和設定原則的詳細資訊，請參閱 [API 管理原則 []][]。 如需建立產品加上費率限制和配額原則的教學課程，請參閱 [如何建立和設定進階的產品設定 []][]。 如需示範，請參閱下列影片。
