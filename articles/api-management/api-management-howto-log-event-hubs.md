...
若要建立新的事件中樞，登入 [Azure 傳統入口網站](https://manage.windowsazure.com) 按一下 **新增**]-> [**應用程式服務**]-> [**服務匯流排**]-> [**事件中心**]-> [**快速建立**。 輸入事件中樞名稱、區域，選取訂用帳戶，然後選取命名空間。 如果您先前尚未建立命名空間，可以在 [命名空間]**** 文字方塊中輸入名稱加以建立。 設定好所有屬性後，按一下 [建立新的事件中樞]**** 建立事件中樞。
...
-   取代 `{您的服務}` API 管理服務執行個體的名稱。
-   取代 `{新記錄器名稱}` 與您的新記錄器所需的名稱。 當您設定時，會參考此名稱 [記錄到事件中樞](https://msdn.microsoft.com/library/azure/dn894085.aspx#log-to-eventhub) 原則
...
-   `類型` 必須設為 `AzureEventHub`。
-   `描述` 提供記錄器的選擇性描述，並視需要，可以是零長度字串。
-   `認證` 包含 `名稱` 和 `connectionString` Azure 事件中心。

當您提出要求，如果記錄器建立狀態碼 `201 已建立` 傳回。
...
在將游標放置 `輸入` 原則區段，並按一下 **記錄到事件中樞** 插入原則 `記錄到事件中樞` 原則陳述式範本。
...
取代 `記錄器識別碼` API 管理的記錄器您在上一個步驟中設定的名稱。

您可以使用傳回的字串做為值的任何運算式 `記錄到事件中樞` 項目。 在此範例中，將記錄包含日期和時間、服務名稱、要求 ID、 要求的 IP 位址和作業名稱的字串。
...
-   深入了解 API 管理和事件中樞的整合
    ...
    -   [監視您的 Api，Azure API 管理、 事件中樞與 Runscope](api-management-log-to-eventhub-sample.md)
