...
針對 [**登入 URL**]，請從發行者入口網站 [**外部身分識別**] 索引標籤的 [**Azure Active Directory**] 區段，複製 [**重新導向 URL**]，並從 URL 結尾移除 **-aad** 尾碼。 在此範例中， **登入 URL** 是 `https://aad03.portal.current.int-azure-api.net/signin`。

針對 [**應用程式識別碼 URL**]，請輸入 Azure Active Directory 的預設網域或自訂網域，並為其附加獨特的字串。 在此範例中的預設網域的 **https://contoso5api.onmicrosoft.com** 使用後置詞 **/api** 指定。
...
可以在 [**允許的租用戶**] 區段中指定多個網域。 在使用者可透過與註冊應用程式之原始網域不同的網域登入前，不同網域的全域管理員必須授與權限，應用程式才能存取目錄資料。 若要授與權限，全域管理員必須登入應用程式，然後按一下 [**接受**]。 在下列範例中 `miaoaad.onmicrosoft.com` 已加入至 **允許的租用戶** 並且來自該網域的全域管理員正在第一次登入。
...
一旦儲存變更，在指定的 Azure Active Directory 中的使用者可以登入開發人員入口網站中的步驟 [登入開發人員入口網站使用 Azure Active Directory 帳戶 []][]。
