...
1. Először konfigurálja a hitelesítést/engedélyezést, ezért kattintson a Function alkalmazás nevére (a &lt;Z&gt; functions ikon mellett) az Áttekintés lap megjelenítéséhez.
...
1. Állítsa be a "válasz URL-címe" bejegyzést arra a megjegyzett értékre, amelyet a API Management korábbi hitelesítési kód engedélyezésének redirect_uri konfigurált.
   ...
   Most, hogy a OAuth 2,0 felhasználói hitelesítés engedélyezve van a `Echo API`on, a fejlesztői konzol egy hozzáférési jogkivonatot szerez be a felhasználó számára az API meghívása előtt.

1. Tallózással keresse meg a `Echo API` alatt található bármelyik műveletet a fejlesztői portálon, és válassza a **kipróbálás** lehetőséget a fejlesztői konzolra.
...
1. A sikeres bejelentkezést követően a rendszer egy `Authorization: Bearer` fejlécet ad hozzá a kérelemhez, amely Azure AD B2C Base64-kódolású hozzáférési jogkivonattal rendelkezik. 
...
* Az Azure AD Graph API használatával egyéni jogcímeket rendelhet hozzá, és egy API Management házirend használatával ellenőrizheti, hogy szerepelnek-e a jogkivonatban.
