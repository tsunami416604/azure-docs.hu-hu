...
如果網際網路上的所有用戶端皆使用唯一 IP 位址，這是可能是限制使用者使用量的有效方式。 不過，很有可能多個使用者共用單一公用 IP 位址，因為他們透過 NAT 裝置存取網際網路。 雖然這允許未驗證的存取的 Api `IpAddress` 可能是最佳的選項。
...
在此範例中我們要擷取的授權標頭，將它轉換成 `JWT` 物件，並使用權杖的主體來識別使用者，並使用它做速率限制索引鍵。 如果使用者識別會儲存在 `JWT` 為其中一個其他然後宣告值可用於它的位置。
