...
* kritikus fontosságú munkaterhelésekhez, üzleti alkalmazásokat futtató
...
> [!NOTE]
> Ahhoz, hogy kevés az adat a felderítésére és figyelésére a hálózati erőforrások legalább 2 ügynököt telepíteni szeretné. Ellenkező esetben a megoldás állapotban marad konfigurálása amíg telepítése és konfigurálása további ügynökök.
>
>
...
### <a name="configure-agents"></a>Konfigurálja az ügynököket
...
Ha szeretne használni a TCP protokoll, akkor nyissa meg a számítógépek számára, győződjön meg arról, hogy az ügynökök kommunikálhatnak tűzfalportjai. Töltse le és futtassa a [EnableRules.ps1 PowerShell-parancsfájl](https://gallery.technet.microsoft.com/OMS-Network-Performance-04a66634) egy PowerShell-ablakban rendszergazdai jogosultságokkal rendelkező paraméterek nélkül.
...
4. Miután kiválasztotta a protokollt, hogy a rendszer átirányítja a **OMS áttekintése** lap. A megoldás összesíti az adatokat a hálózatról, amíg a hálózati Teljesítményfigyelő áttekintés csempe megjeleníti a következő üzenet *adatösszesítés folyamatban*.<br><br> ![megoldás az adatok összesítése](media/log-analytics-network-performance-monitor/log-analytics-netmon-tile-status-01.png)<br><br>
5. Miután az adatok gyűjtése és indexelt, az Áttekintés csempe annak jelzésére, végre kell hajtania a további konfigurációs módosításokat.<br><br> ![megoldás csempe további konfigurációt igényel](media/log-analytics-network-performance-monitor/log-analytics-netmon-tile-status-02.png)<br><br>
6. Kattintson a csempére, majd indítsa el az alábbi lépéseket követve megfelelő megoldás konfigurálását.
...
A konfiguráció lapon megjelenik egy nevű hálózati **alapértelmezett** a hálózatok lapon. Ha még nem hozott létre olyan hálózatra, az automatikusan felderített alhálózatok az alapértelmezett hálózati kerülnek.
...
A legtöbb esetben a szervezet alhálózatok egynél több hálózati vannak rendezve, és hozzon létre egy vagy több hálózatot az alhálózatok az üzleti logika szerint csoportosíthatja
...
Az adatok feltöltése a hálózati teljesítmény figyelése csempe frissül az adatok jelennek meg.
...
2. Válassza ki a figyelni kívánt az adott alhálózat az alhálózat kiválasztásával a listából, és a szükséges csomópontok a nem figyelt és a felügyelt csomópontok tartalmazó listák közötti áthelyezése csomópontok.
   Hozzáadhat egy egyéni **leírás** az alhálózat számára.
...
Hálózati Teljesítményfigyelő állapotával kapcsolatos események, amikor a küszöbértéket, a teljesítmény a hálózati kapcsolatok vagy a két hálózat közötti két alhálózatai megsérül állít elő. Ezeket a küszöbértékeket is ismernie kell automatikusan a rendszer, vagy megadhat egyéni küszöbértékeket.

A rendszer automatikusan létrehoz egy alapértelmezett szabály. A szabály olyan állapotesemény, amikor elvesztése vagy bármely két hálózati/alhálózati kapcsolat közötti késés feltöri a rendszer által ismert küszöbértéket állít elő. Ez segít a megoldás figyeléséhez a hálózati infrastruktúra, amíg explicit módon nincsenek ellenőrzési szabályok. Ha engedélyezve van az alapértelmezett szabályt, a csomópontok szintetikus tranzakciók küldeni a többi csomópontnak, hogy engedélyezte a figyelésre. Az alapértelmezett szabályt akkor hasznos, kis hálózatok számára. Például az olyan forgatókönyvekben, ahol kis számú egy mikroszolgáltatási futtató kiszolgálók, és győződjön meg arról, hogy minden kiszolgálót kapcsolódik egymáshoz szeretné.
...
Az üzleti logika szerint egyéni ellenőrzési szabályokat létrehozni. Például ha azt szeretné, a hálózati kapcsolat a központtal két office webhelyek teljesítményének figyeléséhez, majd csoport office hely1 O1 hálózaton lévő összes alhálózata, office hely2 O2 hálózaton lévő összes alhálózata és a hálózati H. alkalmával a székhelyet mindig az összes alhálózata Hozzon létre két szabályok egy az egyhez O1 és H között a másik O2 és H. közötti figyelése
...
Hálózati Teljesítményfigyelő TCP SZIN-SYNACK-Nyugtázási kézfogás csomagok TCP van kiválasztva, és az ICMP ECHO REPLY használja, amikor az ICMP protokollt van kiválasztva veszteséget és késéseket kapcsolatos információk összegyűjtéséhez. Traceroute is használt topológia lekérése.
...
A **felső hálózati állapotával kapcsolatos események** terület a rendszer és az idő a legutóbbi állapotával kapcsolatos események és riasztások listáját tartalmazza a óta módosították, hogy az esemény aktív. A rendszerállapot-esemény vagy riasztás jön létre, amikor csomagvesztés vagy a hálózati vagy alhálózat hivatkozás várakozási ideje meghaladja a küszöbértéket.

A **felső nem megfelelő állapotú hálózati kapcsolatok** területen lévő nem megfelelő állapotú hálózati kapcsolatok listáját jeleníti meg. Ezek azok a hálózati kapcsolat, amelyek egy vagy több káros állapotával kapcsolatos események őket a pillanatban.

A **felső alhálózati kapcsolat legtöbb adatvesztés** és **alhálózati kapcsolat és a késleltetés** területek megjelenítése a felső alhálózati kapcsolat csomagvesztés és alhálózati kapcsolat rendre felső késés korlátozza. Nagy késleltetésű vagy bizonyos csomagvesztés várható bizonyos hálózati kapcsolatokról. Ilyen kapcsolatok a felső tíz listán, de nem különbözteti meg a nem megfelelő.

A **közös lekérdezések** területet tartalmaz, amely a nyers hálózati figyelési közvetlenül az adatok lehívása keresési lekérdezések. Ezeket a lekérdezéseket kiindulási pontként használható a saját testreszabott jelentéskészítéshez lekérdezések létrehozásáról.
...
![részletekbe menően tárhatják adatok](./media/log-analytics-network-performance-monitor/npm-drill.png)
...
Akkor is részletekbe menően tárhatják egyes szintjein veszteséget és késéseket hálózati kapcsolat trendjét tekintheti meg. Trend diagramok alhálózat és a csomópont hivatkozások is elérhetők. Módosíthatja a diagramot úgy, hogy a vezérlő tetején található a diagram használatával megrajzolásához időintervallumát.
...
Minden adat, a hálózati teljesítményt figyelő irányítópult és a lapok részletezés grafikusan elérhetővé érhető el natív módon Naplóelemzési keresési. A keresési lekérdezés nyelvének használata a lekérdezést, és egyéni jelentések készítése az adatok exportálása az Excel vagy a Power bi által. A **közös lekérdezések** tartalmaz néhány hasznos lekérdezést, amely kiindulási pontként használható a saját lekérdezések és jelentések létrehozása az irányítópulton.
