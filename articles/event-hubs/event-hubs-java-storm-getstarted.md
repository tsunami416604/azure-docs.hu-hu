...
> [AZURE.NOTE] 本教學課程中只使用本機模式的 Storm 進行開發。 請參閱 [HDInsight Storm 概觀 []][] 和官方 [Apache Storm []][] Storm 部署和模式的文件的詳細資訊。

## 後續步驟

下列資源可供開發整合事件中心和 Storm 的應用程式。

- [使用 Storm 和 HDInsight 的感應器資料分析] 是使用事件中心、 Storm 和 HBase 擷取 Hadoop 叢集中的感應器資料的完整案例教學課程。
