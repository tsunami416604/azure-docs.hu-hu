...
完整事件中樞定價的詳細資訊，請參閱 [事件中樞定價詳細資料](http://azure.microsoft.com/pricing/details/event-hubs/)。

## 事件中樞輸入事件的計算方式為何？

每個傳送到事件中樞的事件都算是可計費訊息。  *輸入事件* 定義為小於或等於 64 KB 的資料單位。 任何大小小於或等於 64KB 的事件均視為一個可計費事件。 如果事件大於 64KB，可計費事件的數目乃根據事件大小來計算 (64KB 的倍數)。 例如，一個傳送到事件中樞的 8KB 事件將視為一個事件來計費，不過一則傳送到事件中樞的 96KB 訊息則視為兩個事件來計費。
...
儲存在所有事件中樞內之事件，包括事件標頭的所有內部負荷或磁碟儲存結構的所有內部負荷，是以全天為單位來測量。 我們會在一天結束時計算儲存空間大小峰值。 每日儲存額度的計算乃以當天選定之輸送量單位的最小數目為基準 (每個輸送量單位提供 84GB 的額度)。 過多的儲存體的總大小超過導出的每日儲存額度，如果付費使用 Azure Blob 儲存體費率 (在 **本機備援儲存體** 速率)。
...
事件中樞標準層提供事件中樞基本層和一些競爭系統未提供的功能。 這些功能包括超過 24 小時的保留期間，以及在延遲時間少於一秒的情況下使用單一 AMQP 連線將命令傳送到大量裝置，外加從這些裝置將遙測傳送到事件中樞等能力。 請參閱 [定價詳細資料] ([事件中樞定價詳細資料](http://azure.microsoft.com/pricing/details/event-hubs/)) 的功能清單。
...
事件中樞的技術支援是透過 [社群論壇](https://social.msdn.microsoft.com/forums/azure/home)。 計費及訂用帳戶管理支援均為免費提供。

若要深入了解我們的 SLA，請瀏覽 [服務等級協定](http://azure.microsoft.com/support/legal/sla/) 頁面。
