...
Az aláíráshoz használt megosztott hozzáférés-engedélyezési szabályt az URI által megadott entitáson vagy annak egyik hierarchikus szülője szerint kell konfigurálni. `http://contoso.servicebus.windows.net/eventhubs/eh1` Például vagy `http://contoso.servicebus.windows.net` az előző példában.
...
Minden Event Hubs-ügyfél hozzá van rendelve egy egyedi jogkivonatot, amely az ügyfél fel a rendszer. A jogkivonatok úgy jönnek létre, hogy mindegyik egyedi jogkivonat különböző egyedi közzétevőhöz biztosít hozzáférést. A jogkivonatot birtokló ügyfelek csak egyetlen közzétevőnek küldhetnek, és nem rendelkezhetnek más közzétevővel. Ha több ügyfél ugyanezt a tokent, majd azok fájlmegosztások közzétevő.

Minden token SAS-kulccsal van társítva. Általában minden jogkivonatok ugyanazzal a kulccsal van bejelentkezve. Az ügyfelek nem ismerik a kulcsot, ami megakadályozza más ügyfelek számára a gyártási jogkivonatokat. Az ügyfelek ugyanazon jogkivonatokon működnek, amíg lejárnak.
...
3. Adja meg a jogkivonatot a közzétevő ügyfelének, amely csak azt az entitást küldheti el, amely számára a jogkivonat hozzáférést biztosít.
4. A jogkivonat lejárta után az ügyfél elveszíti a hozzáférést az entitáshoz való küldéshez vagy közzétételhez. 


> [!NOTE]
> Bár ez nem ajánlott, az eszközök olyan jogkivonatokkal is felhasználhatók, amelyek hozzáférést biztosítanak az Event hub-hoz. Minden olyan eszköz, amelyik ezt a jogkivonatot tárolja, közvetlenül is küldhet üzeneteket az adott Event hub-nak. Továbbá az eszköz nem tiltólistán ennek az eseményközpontnak küldjön.
> 
> A fenti viselkedés megfigyelhető, ha ugyanaz a jogkivonat több olyan eszközre van elosztva, amely hozzáférést biztosít a névtér szintjén. Ebben az esetben a Rouge-eszköz/-közzétevő nem különíthető el és vonható vissza. A konkrét és a részletes hatóköröket mindig ajánlott megadni.
