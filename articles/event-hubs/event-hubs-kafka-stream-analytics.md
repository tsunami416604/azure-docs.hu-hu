...
1. Hozzon létre egy Kafka-kompatibilis Event Hubs névteret.
...
* Egy **Azure Storage-fiók**. Ha még nem rendelkezik ilyennel, a folytatás előtt [hozzon létre egyet](../storage/common/storage-quickstart-create-account.md) . Az ebben a bemutatóban található Stream Analytics feladata egy Azure Blob Storage-ban tárolja a kimeneti adatokat. 
...
2. Keresse meg **Event Hubs** , és válassza ki az itt látható beállításokat:
    
    ![Event Hubs keresése a portálon](./media/event-hubs-kafka-stream-analytics/select-event-hubs.png) 
3. A **Event Hubs** lapon válassza a **Létrehozás**lehetőséget.
4. A **névtér létrehozása** oldalon hajtsa végre a következő műveleteket: 
    1. Adja meg a névtér egyedi **nevét** . 
    2. Válasszon **árképzési szintet**. 
    3. Válassza a **Kafka engedélyezése**lehetőséget. Ez a lépés **fontos** lépés. 
...
1. Válassza ki az **Event hub-névteret** az erőforráscsoporthoz. 
2. A névtér létrehozása után válassza a **közös hozzáférési házirendek** lehetőséget a **Beállítások**területen.

    ![Kattintás a Megosztott elérési szabályzatok elemre](./media/event-hubs-kafka-stream-analytics/shared-access-policies.png)
5. Választhatja az alapértelmezett **RootManageSharedAccessKey** szabályzatot, vagy hozzáadhat egy újat is. Kattintson a házirend nevére, és másolja a **kapcsolatok karakterláncát**. A Kafka-ügyfél konfigurálásához használja a kapcsolódási karakterláncot. 
