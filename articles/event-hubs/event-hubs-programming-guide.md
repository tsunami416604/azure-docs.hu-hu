...
A [EventData][] osztálynak [két túlterhelt konstruktora](/dotnet/api/microsoft.azure.eventhubs.eventdata.-ctor) van, amelyek számos paramétert, bájtot vagy byte tömböt foglalnak magukban, amelyek az esemény adattartalmát jelölik. Ha JSON-t használ az [EventData][] osztállyal, az **Encoding.UTF8.GetBytes()** használatával kérheti le a bájttömböt a JSON-kódolású sztringhez. Példa:
...
Az események feldolgozásának megkezdéséhez hozza létre a [EventProcessorHost][], és adja meg az Event hub megfelelő paramétereit. Példa:
