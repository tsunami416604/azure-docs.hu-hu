...
3. Hozzon létre Azure Cosmos DB a `GlobalDocumentDB`-típussal. Cosmos DB nevének csak kisbetűket kell használnia. Jegyezze fel a parancs válaszában szereplő `documentEndpoint` mezőt.
...
4. Azure Cosmos DB kulcsának beszerzése az alkalmazáshoz való kapcsolódáshoz. Tartsa meg a `primaryMasterKey``documentEndpoint` a közelben, ahogy a következő lépésben szüksége lesz rájuk.
...
Ezeket a környezeti változókat a `application.properties` a TODO List alkalmazásban használják. A Properties fájlban lévő mezők a Spring-adat alapértelmezett tárház-konfigurációját állítják be:
...
Ezután a minta alkalmazás a `com.microsoft.azure.spring.data.cosmosdb.core.mapping.Document`ból importált `@Document` jegyzetet használja egy Cosmos DB tárolására és kezelésére szolgáló entitás típusának beállításához:
...
Az alkalmazás elindítása után az alábbi hivatkozással helyileg elérheti a Spring TODO alkalmazást: [http://localhost:8080/](http://localhost:8080/).

 ![A Spring TODO-alkalmazás helyi elérése](./media/tutorial-java-spring-cosmosdb/spring-todo-app-running-locally.jpg)

Ha a "Started TodoApplication" üzenet helyett kivételek jelennek meg, ellenőrizze, hogy az előző lépésben szereplő `bash` parancsfájl megfelelően exportálta-e a környezeti változókat, és hogy helyesek-e a létrehozott Azure Cosmos DB adatbázishoz tartozó értékek.
...
A `azure-webapp:deploy` Maven céljának használatával telepítse a TODO alkalmazást a Linuxon Azure App Service.
