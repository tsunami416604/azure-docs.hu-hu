...
- **Statikus fájlok** kiszolgálása – lehetővé teszi, hogy a a Ruby tárolóban lévő statikus objektumokat szolgálja ki, [a `RAILS_SERVE_STATIC_FILES` alkalmazás beállítását állítsa](../configure-common.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json#configure-app-settings) `true`re. Például:
...
Ha a App Service létrehozása helyett saját `secret_key_base` értéket szeretne használni, állítsa be a `SECRET_KEY_BASE` [alkalmazás beállítását](../configure-common.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json#configure-app-settings) a kívánt értékre. Például:
