...
### <a name="changing-php_ini_user-php_ini_perdir-php_ini_all-configuration-settings"></a>PHP ini \_ - \_ felhasználó módosítása, php \_ ini \_ PERDIR, php \_ ini \_ minden konfigurációs beállítás
...
3. Indítsa újra az alkalmazást. (Az újraindítás azért szükséges, mert a PHP által beolvasott `.user.ini` fájlok gyakoriságát a beállítás szabályozza `user_ini.cache_ttl` , amely a rendszerszintű beállítás, amely alapértelmezés szerint 300 másodperc (5 perc). Az alkalmazás újraindítása a PHP-ben a fájl új beállításainak olvasásához `.user.ini` .)

Egy fájl használatának alternatívájaként `.user.ini` a parancsfájlok [ini_set ()] függvényével olyan konfigurációs beállításokat adhat meg, amelyek nem rendszerszintű irányelvek.

### <a name="changing-php_ini_system-configuration-settings"></a>A PHP \_ ini \_ rendszerkonfigurációs beállításainak módosítása

1. Adja hozzá az alkalmazáshoz a kulcsot és az értéket tartalmazó Alkalmazásbeállítás `PHP_INI_SCAN_DIR``d:\home\site\ini`
1. Hozzon létre egy `settings.ini` fájlt a kudu-konzollal (http:// &lt; site-name &gt; . SCM.azurewebsite.net) a `d:\home\site\ini` címtárban.
1. Adja hozzá a konfigurációs beállításokat a `settings.ini` fájlhoz ugyanazzal a szintaxissal, amelyet egy `php.ini` fájlban szeretne használni. Ha például `curl.cainfo` egy fájlra szeretné beállítani a beállítást, és a `*.crt` "wincache. MaxFileSize" beállítást a 512 K értékre állítja, akkor a `settings.ini` fájl a következő szöveget fogja tartalmazni:
...
1. Adjon hozzá egy `ext` könyvtárat a `d:\home\site` címtárhoz.
1. Terjesszen `.dll` a bővítmények fájljait a `ext` könyvtárba (például: `php_xdebug.dll` ). Ügyeljen arra, hogy a bővítmények kompatibilisek legyenek a PHP alapértelmezett verziójával, és a VC9 és a nem szál-biztonságos (Zem) kompatibilisek legyenek.
1. Adja hozzá az alkalmazáshoz a kulcsot és az értéket tartalmazó Alkalmazásbeállítás `PHP_INI_SCAN_DIR``d:\home\site\ini`
1. Hozzon létre egy `ini` fájlt a `d:\home\site\ini` nevű fájlban `extensions.ini` .
1. Adja hozzá a konfigurációs beállításokat a `extensions.ini` fájlhoz ugyanazzal a szintaxissal, amelyet egy `php.ini` fájlban szeretne használni. Ha például engedélyezni szeretné a MongoDB és a Xdebug bővítmény bővítményt, a fájl a `extensions.ini` következő szöveget fogja tartalmazni:
...
2. Terjesszen `.dll` a bővítmények fájljait a `bin` könyvtárba (például: `php_xdebug.dll` ). Ügyeljen arra, hogy a bővítmények kompatibilisek legyenek a PHP alapértelmezett verziójával, és a VC9 és a nem szál-biztonságos (Zem) kompatibilisek legyenek.
...
A Zend Extensions szolgáltatást **PHP_ZENDEXTENSIONS** kulccsal is támogatja. Több bővítmény engedélyezéséhez adja meg a fájlok vesszővel tagolt listáját az Alkalmazásbeállítások `.dll` értékéhez.

## <a name="how-to-use-a-custom-php-runtime"></a>Útmutató: egyéni PHP-futtatókörnyezet használata

Az alapértelmezett PHP-futtatókörnyezet helyett a App Service használhat a PHP-parancsfájlok futtatásához megadott PHP-futtatókörnyezetet. Az Ön által megadott futtatókörnyezet konfigurálható egy Ön által megadott `php.ini` fájllal is. Ha App Service használatával egyéni PHP-futtatókörnyezetet szeretne használni, kövesse az alábbi lépéseket.
...
4. Adjon hozzá egy `bin` könyvtárat a gyökérkönyvtárhoz, és helyezze el azt a könyvtárat, amely tartalmazza a PHP-futtatókörnyezetet (például `bin\php` ).
...
9. Kattintson az **+ új kezelő** elemre, és adja hozzá `*.php` a kiterjesztés mezőhöz, és adja hozzá a végrehajtható fájl elérési útját a `php-cgi.exe` **parancsfájl-feldolgozóban**. Ha a PHP-futtatókörnyezetet az `bin` alkalmazás gyökérkönyvtárában lévő könyvtárba helyezi, az elérési út `D:\home\site\wwwroot\bin\php\php-cgi.exe` .
...
4. Most pedig a helyi gépen lévő terminál-ablakban hajtsa végre a (z `git add` `git commit` ) és az `git push` alkalmazást. Figyelje meg, hogy a zeneszerző a Composer. JSON fájlban definiált függőségeket telepíti.
