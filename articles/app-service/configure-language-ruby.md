...
Ez a cikk azt ismerteti, hogy a [Azure app Service](overview.md) hogyan futtatja a Ruby-alkalmazásokat Linux-tárolóban, és hogyan szabhatja testre a app Service viselkedését, ha szükséges. A Ruby-alkalmazásokat az összes szükséges [drágakövevel](https://rubygems.org/gems)telepíteni kell.
...
A Ruby nem támogatott verzióját a saját tároló rendszerképének létrehozásával futtathatja. További információ: [Egyéni Docker-rendszerkép használata](tutorial-custom-container.md?pivots=container-linux).
...
Ha [git-tárházat](deploy-local-git.md)telepít, vagy a létrehozási folyamatokkal rendelkező [ZIP-csomagot](deploy-zip.md) , a telepítő motor (kudu) alapértelmezés szerint automatikusan futtatja a következő üzembe helyezés utáni lépéseket:

1. Ellenőrizze, hogy létezik-e *Gemfile* .
1. A `bundle clean` parancs futtatása. 
1. A `bundle install --path "vendor/bundle"` parancs futtatása.
1. `bundle package`A drágaköveket a vendor/cache mappában futtathatja.

### <a name="use---without-flag"></a>Használat – jelölő nélkül

Ha `bundle install` a [---nélkül](https://bundler.io/man/bundle-install.1.html) szeretné futtatni a parancsot, állítsa be az `BUNDLE_WITHOUT` [alkalmazás beállítását](configure-common.md#configure-app-settings) a csoportok vesszővel tagolt listájára. Az alábbi parancs például beállítja a következőre: `development,test` .
...
A telepítés utáni lépések alapértelmezés szerint nem fordítják előre az eszközöket. Az eszköz előfordításának bekapcsolásához állítsa az `ASSETS_PRECOMPILE` [alkalmazás beállítást](configure-common.md#configure-app-settings) a következőre: `true` . Ezután a parancs a `bundle exec rake --trace assets:precompile` Telepítés utáni lépések végén fut. Például:
...
1. A `rails server -e $RAILS_ENV` parancs futtatása.
...
- **Statikus fájlok** kiszolgálása – lehetővé teszi, hogy a a Ruby- [ `RAILS_SERVE_STATIC_FILES` ](configure-common.md#configure-app-settings) tárolóban lévő statikus eszközöket szolgálja ki `true` . Például:
...
Alapértelmezés szerint a Rails-kiszolgáló éles módban fut. Ha például fejlesztési módban szeretné futtatni az alkalmazást, állítsa be az `RAILS_ENV` [alkalmazás beállítását](configure-common.md#configure-app-settings) a következőre: `development` .

```azurecli-interactive
az webapp config appsettings set --name <app-name> --resource-group <resource-group-name> --settings RAILS_ENV="development"
```

Ez a beállítás azonban önmagában azt eredményezi, hogy a Rails-kiszolgáló fejlesztési módban indul el, amely csak a localhost kérelmeket fogadja el, és a tárolón kívül nem érhető el. A távoli ügyfelek kéréseinek elfogadásához állítsa az `APP_COMMAND_LINE` [alkalmazás beállítását](configure-common.md#configure-app-settings) a következőre: `rails server -b 0.0.0.0` . Ez az Alkalmazásbeállítások lehetővé teszi, hogy egyéni parancsot futtasson a Ruby-tárolóban. Például:
...
Ha a `secret_key_base` app Service létrehozása helyett saját értéket szeretne használni, állítsa be az `SECRET_KEY_BASE` [alkalmazás beállításait](configure-common.md#configure-app-settings) a kívánt értékre. Például:
