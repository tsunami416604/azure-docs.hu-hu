...
A App Service üzembe helyezési motor automatikusan aktiválja a virtuális környezetet, és a `pip install -r requirements.txt` [git-tárház](deploy-local-git.md)üzembe helyezésekor, vagy a létrehozási folyamatokkal rendelkező [zip-csomagon](deploy-zip.md) fut.
