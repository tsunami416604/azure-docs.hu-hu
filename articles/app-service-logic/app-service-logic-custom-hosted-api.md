...
3. 如果您需要使用任何 **標頭** 納入 JSON 格式如下: `{「 內容類型 」: 「 application/json 」、 「 接受 」: 「 application/json"}`
...
2. `Add-AzureAccount`
3. `新 AzureADApplication-DisplayName"MyLogicAppID"-首頁"http://someranddomain.tld"-IdentifierUris"http://someranddomain.tld"-"Pass@word1! 」 的密碼`
...
在 **授權** 區段 **HTTP** 動作:
`{"tenant":"<< tenantId >>"，「 對象 」:"<< clientID 從第 2 部分 >>"，"clientId":"<< clientID 從第 1 部分 >>"，「 密碼 」: 「 << 密碼或金鑰從第 1 部分 >>"，"type":"ActiveDirectoryOAuth"}`
...
在 *授權* 區段應該提供: `{"type":"i"，"password":"test"、"pfx": 「 長時間 pfx-索引鍵 」}`。
...
在 *授權* 應該提供的區段: `{"type":"basic"、"username": 「 測試 」、 「 密碼 」: 「 測試 」}`。
