...
連接器支援下列邏輯應用程式觸發程序和動作：
 ...
 觸發程序| 動作
--- | ---
 <ul><li>輪詢資料</li></ul>| <ul><li>Bulk Insert</li><li>Insert</li><li>Bulk Update</li><li>Update</li><li>Call</li><li>Bulk Delete</li><li>Delete</li><li>Select</li><li>Conditional update</li><li>Post to EntitySet</li><li>Conditional delete</li><li>Select single entity</li><li>Delete</li><li>Upsert to EntitySet</li><li>Custom commands</li><li>複合操作</li></ul>
...
--- | --- | ---
 ...
 PollToCheckData| 否| 要搭配邏輯應用程式觸發程序使用的 SELECT COUNT 陳述式 (例如「 COUNT(\*) 從選取的 NEWORDERS SHIPDATE 所在 NULL 」)。
 PollToReadData| 否| 要搭配邏輯應用程式觸發程序使用的 SELECT 陳述式 (例如「 選取 \ * 從 NEWORDERS SHIPDATE 所在更新為 NULL 」)。
 PollToAlterData| 否| 要搭配邏輯應用程式觸發程序使用的 UPDATE 或 DELETE 陳述式 (例如「 更新 NEWORDERS 組出貨日期 = 目前的日期，目前 < 資料指標 > 」)。
...
--- | --- | ---
 PollToCheckData| 從 [出貨日期所在 NULL NEWORDERS 選取 COUNT(\*)
 PollToReadData| 選取 \ * 從 NEWORDERS SHIPDATE 所在更新為 NULL
 PollToAlterData| 更新 NEWORDERS 組 SHIPDATE = 目前的日期，目前的 < 資料指標 >
...
--- | --- | ---
 PollToCheckData| 從 [出貨日期所在 NULL NEWORDERS 選取 COUNT(\*)
 PollToReadData| 選取 \ * 從 NEWORDERS SHIPDATE 所在更新為 NULL
 PollToAlterData| 刪除 NEWORDERS 其中的 < 資料指標 >
