...
1. 啟動 BizTalk 規則 API 應用程式從入口網站移至 [瀏覽]-> [API 應用程式]-><Your Rules API App>。這會使您的規則 API 應用程式儀表板如下:

   ![替代文字][4]

2.選取 [詞彙定義]。 這會顯示 [詞彙撰寫] 畫面
3.選取 [新增]，開始新增新的詞彙定義。
...
1.  按一下 [新增] 後，新的 [新增定義] 分頁隨即開啟。 輸入下列值
  ...
  5. 輸入 – 這是您輸入您定義值的位置。 在此輸入的值必須符合選擇的資料型。 您可以輸入單一值、 一組值並以逗號或某個範圍的值使用關鍵字 *至*。 例如，您可以輸入唯一值 1。一組 1、 2、 3;或範圍 1 到 5。 請注意，範圍僅適用於數字。
  6. 選取 [確定]**。
...
選取 [大量新增]，將帶您進入新分頁。 第一個步驟是選取要建立多個定義的結構描述。 選取這將會開啟新分頁，讓您從已上傳的結構描述的清單中選擇，或讓上傳新的結構。
此時，XPATHS 屬性會解除鎖定。 選取這將會開啟供您選取多個節點的結構描述檢視器。
...
在開發人員建立必要的詞彙後，預期應由商務分析人員透過 Azure 入口網站建立商務原則。  
    ...
    2.此頁面會顯示此特定規則應用程式具有的原則清單。 分析師可以加入新的原則，只要輸入原則名稱並按 tab 鍵兩次。 多個原則可以位於單一規則 API 應用程式中。  
    3.選取建立的原則將使用者帶到原則詳細資料頁面而得以檢視位於原則中的規則。  
    ![替代文字][8]
    4.選取 [新增]，加入新的規則。 這會使您進入新分頁。

## 建立規則

規則是條件和動作陳述式的集合。 如果條件評估為 true，則會執行動作。 在 [建立規則] 分頁中，提供唯一的規則名稱 (適用於該原則) 和說明 (選擇性)。
條件 (IF) 方塊可以用來建立複雜的條件陳述式。 以下是支援的關鍵字：
1.  And – 條件運算子
2.  Or – 條件運算子
3.  does\_not\_exist
4.  exists
...
Action (then) 方塊可以包含多個陳述式，每一行，來建立要執行的動作。 以下是支援的關鍵字：
1.  equals
2.  false
3.  true
4.  halt
...
BizTalk 規則支援明確向前鏈結，如果使用者想要重新評估規則以回應特定動作，他們可以使用觸發某些關鍵字。 以下是支援的關鍵字：
   1.   更新 <vocabulary definition> – 此關鍵字會重新評估其條件中使用指定的詞彙定義的所有規則。
...
原則已撰寫並經過測試後，此時已可供取用。 您可以從入口網站首頁左側選取 Logic Apps，來建立新的邏輯應用程式。 建立邏輯應用程式之後，請啟動並選取 [*觸發程序和動作*]。 然後您可以選取 [*從頭建立*] 範本。 請依照步驟將 BizTalk 規則 API 應用程式新增至邏輯應用程式。 完成時，會有一個選項可選擇為目標的規則 API 應用程式 (動作)。 動作包括要執行的原則清單。 選擇要提供具體的原則之後，輸入所需的原則。 使用者可以使用規則 API 應用程式下游的輸出，做出進一步的決策。

## 透過 API 使用規則

規則 API 應用程式可以也使用來叫用一組豐富的 Api。 如此，使用者不一定只能使用邏輯應用程式，而在任何應用程式可以使用的規則藉由 REST 呼叫。 按一下 [規則] 儀表板中的 [API 定義] 透鏡，即可檢視確切可用的 REST API。
...
請注意，上述規則 API 應用程式已將其安全性設定設為 "Public Anon"。 這可以設定從 API 應用程式內使用-所有設定]-> 都 [應用程式設定]-> 都 [存取層級
