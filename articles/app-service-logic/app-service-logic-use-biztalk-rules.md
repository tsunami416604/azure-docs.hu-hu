...
A _詞彙_ 是定義規則條件和動作中使用之計算物件的易記名稱所組成的集合。 詞彙定義讓規則更易於給特定商務領域中的人員閱讀、了解與共用。
...
1. 啟動 BizTalk 規則 API 應用程式從入口網站移至 [瀏覽]-> [API 應用程式]-><Your Rules API App>. 這將讓您進入 [規則 API 應用程式儀表板]，如下方所示：
...
1.  按一下 [新增] 後，新的 [新增定義] 分頁隨即開啟。 輸入下列值
  ...
  4.    資料類型 - 這可讓使用者選取定義的資料類型。 目前支援 4 個資料類型 ︰
    i.  字串 – 這些值必須在雙引號中輸入 ("範例字串")  
    ii. 布林值 – 這可以是 true 或 false  
    iii.    數字 – 可以是任何十進位數字  
    iv. 日期時間 – 這表示定義屬於日期類型。 資料必須以下列格式輸入 – mm/dd/yyyy hh:mm:ss AM\PM  
  5. 輸入 – 這是您輸入您定義值的位置。 在此輸入的值必須符合選擇的資料型。 您可以輸入單一值、 一組值並以逗號或某個範圍的值使用關鍵字 *到*。 例如，您可以輸入唯一值 1；一組值 1、2、3；或是範圍 1 to 5。 請注意，範圍僅適用於數字。
  6. 選取 *確定*。
...
BizTalk 規則支援明確向前鏈結，因此，如果使用者想要重新評估規則以回應特定動作，可以使用特定的關鍵字加以觸發。 以下是支援的關鍵字：  
   1.   update <vocabulary definition> – 此關鍵字會重新評估其條件中使用指定的詞彙定義的所有規則。  
...
## 在邏輯應用程式中使用規則
原則已撰寫並經過測試後，此時已可供取用。 您可以從入口網站首頁左側選取 Logic Apps，來建立新的邏輯應用程式。 一旦建立邏輯應用程式啟動它，然後選取 [ *觸發程序和動作*。 然後您可以選取 *從頭開始建立* 範本。 請依照步驟將 BizTalk 規則 API 應用程式新增至邏輯應用程式。 完成此動作後，會有一個選項供您選擇要以哪些規則 API 應用程式 (動作) 作為目標。 動作包括要執行的原則清單。 請選擇在其後必須輸入原則的特定原則。 使用者可以使用規則 API 應用程式下游的輸出，做出進一步的決策。
