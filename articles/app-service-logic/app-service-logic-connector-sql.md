...
SQL 連接器提供下列觸發程序和動作：
 ...
 觸發程序| 動作
--- | ---
 輪詢資料| <ul><li>資料表中插入</li><li>更新資料表</li><li>從資料表選取</li><li>從資料表刪除</li><li>呼叫預存程序</li></ul>
...
--- | --- | ---
 ...
 服務匯流排連接字串| 否| 如果您要連線至內部部署上，輸入服務匯流排轉送連接字串。<br/><br/>[Using the Hybrid Connection Manager](app-service-logic-hybrid-connection-manager.md)<br/>[Service Bus Pricing](http://azure.microsoft.com/pricing/details/service-bus/)
 ...
 輪詢資料查詢| 觸發程序支援| 輪詢 SQL Server 資料庫資料表的 SQL 陳述式。您可以輸入任意數目的 SQL 陳述式，以分號隔開。此陳述式以交易式方式執行，而且只有在資料安全地儲存在邏輯應用程式中時才會認可。範例: SELECT * FROM table_name;DELETE FROM table_name。<br/><br/>**請注意**<br/>您必須提供輪詢陳述式，避免因為無限迴圈，刪除、 移動或更新選取的資料，以確保該相同的資料不會重複輪詢一次。
...
## 可執行和無法執行的作業
 ...
 SQL 查詢| 支援| 不支援
--- | --- | ---
 Where 子句| <ul><li>運算子:，OR、 =、 <>， <, <=, >, ，> =，而要</li><li>可以結合多個 sub 條件 '(' 和')'</li><li>字串常值，日期時間 (以括以單引號括住)、 數字 (應該只包含數字字元)</li><li>絕對格式應該是二進位運算式，例如 ((operand operator operand) 和/或 (運算元運算子的運算元)) *</li></ul>| <ul><li>運算子: Between、 IN</li><li>所有內建函數，例如 add ()、 max () now ()、 POWER()，等</li><li>喜歡數學運算子 *、-、 + 等</li><li>使用字串串連 +。</li><li>所有聯結</li><li>IS NULL 和 IS NOT Null</li><li>任何數字與非數字字元，例如十六進位數字</li></ul>
 欄位 (在 Select 查詢)| <ul><li>以逗號分隔的有效資料行名稱。任何資料表名稱前置詞不允許 (一次一個資料表上的連接器運作)。</li><li>名稱可以使用逸出 '[' 和']'</li></ul>| <ul><li>關鍵字喜歡 TOP、 DISTINCT 等</li><li>別名，例如 Street + City + Zip AS 位址</li><li>所有內建函數，例如 add ()、 max () now ()、 POWER()，等</li><li>數學運算子，例如 *、-、 + 等</li><li>使用字串串連 +</li></ul>
