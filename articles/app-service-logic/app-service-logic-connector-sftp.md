...
5. 瀏覽到剛建立的 API 應用程式，透過 [瀏覽]-> [API 應用程式]-> [ <Name of the API App just created> ，您可以看到 [安全性] 元件尚未設定:  
![][2]
...
7.  您可以採用類似方式在流程中使用 SFTP 動作。 您可以使用「上傳檔案」動作將檔案上傳至 SFTP 伺服器。 設定 [上傳檔案] 的輸入屬性，如下所示：
    ...
    - ** 附加是否存在 **-指定"true"false"。 設定為 "true" 時，若檔案存在則資料會附加至檔案。 設定為 [false] 時，若檔案存在則會覆寫檔案。
