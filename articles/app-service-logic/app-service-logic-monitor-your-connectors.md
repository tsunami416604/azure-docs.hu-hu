...
- **設定** API 應用程式，包括您的訂閱詳細資料，顯示特定的相關資訊，並列出可存取您 API 應用程式的使用者。 您也可以使用其他功能中的 [調整規模] 功能來增加或減少 API 應用程式執行個體數目。
- 使用 **啟動** 和 **停止** 按鈕控制 API 應用程式。
- 當 API 應用程式所使用的基礎檔案進行產品更新時，您可以按一下 **更新** 來取得最新版本。 例如，如果沒有修正程式或 Microsoft 所發行的安全性更新，按一下 **更新** API 應用程式包含此修正程式會自動更新。 
- 選取 **變更計劃** 進行升級或降級為基礎的 API 應用程式資料使用。 您也可以使用這項功能查看資料使用狀況。
- 當您建立具有資料表的連接器 (例如 SQL 連接器) 時，可以選擇性地輸入要連接的資料表名稱。 以資料表為基礎的結構描述會自動建立，並可在按一下時 **下載結構描述**。 您之後可以使用這個下載的結構描述來建立轉換或對應。 
...
> [AZURE.NOTE] 只有當您在防火牆後方連線到內部部署資源需要混合式連線管理員。 如果您要連線至內部部署系統，混合式連線管理員可能不會列在您的連接器分頁。
...
- 在 **使用量**, ，多少，您可以看到 **CPU** 是由使用 API 應用程式，檢閱目前 **使用量配額** MB，以及查看根據成本層的最大資料使用量。 **估計花**  可協助您判斷執行 API 應用程式的潛在成本。
