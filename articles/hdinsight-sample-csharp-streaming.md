...
In the example, both the mapper and the reducer are executables that read the input from [stdin][stdin] (line by line) and emit the output to [stdout][stdin]. The program counts all of the words in the text.
...
When an executable is specified for **reducers**, each reducer task launches the executable as a separate process when the reducer is initialized. As the reducer task runs, it converts its input key/values pairs into lines and feeds the lines to the [stdin][stdin] of the process. In the meantime, the reducer collects the line-oriented outputs from the [stdout][stdin] of the process, converts each line into a key/value pair, which is collected as the output of the reducer. By default, the prefix of a line up to the first tab character is the key and the rest of the line (excluding the tab character) is the value.
...
2.  [The C# code for Hadoop Streaming][The C# code for Hadoop Streaming]
3.  [Summary][Summary]
...
4.  Run the following commands to run the MapReduce job, wait for the job to complete, and then print the standard error:
...
5.  Run the following commands to display the results of the word count.
...
## <span id="java-code"></span></a>The C# code for Hadoop Streaming
...
-   [Azure HDInsight SDK documentation][Azure HDInsight SDK documentation]
  ...
  [The C# code for Hadoop Streaming]: #java-code
  [Summary]: #summary
