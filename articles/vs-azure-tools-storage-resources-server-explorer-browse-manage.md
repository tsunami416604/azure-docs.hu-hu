...
A tárfiókban lévő erőforrások megtekintéséhez bontsa ki a tárfiók csomópontot a Server Explorer. A következő alsóbb szintű jelennek meg:
...
3. Jelölje be Enter billentyűt, vagy kattintson vagy koppintson kívül blobtárolóba menti a név mező.
...
2. Az a **új fájl feltöltése** párbeszédpanelen jelölje ki a **Tallózás** gombra kattintva adja meg a feltölteni kívánt fájlt, és írja be a mappa nevét a a **mappa (nem kötelező)** mezőbe.
...
3. Kattintson duplán a mappára, vagy jelölje be a mappa tartalmát meg az Enter billentyűt. Ha a tároló mappában, lépjen vissza a tároló a legfelső szintű kiválasztásával a **nyitott Szülőkönyvtárában** (nyíl) gombra.

### <a name="to-delete-a-container-folder"></a>Tároló mappa törlése
* A mappában található összes fájl törlésére.
  
> [!NOTE]
> Mivel a blob tárolók mappák virtuális mappák, az üres mappa nem hozható létre. Egy mappa, fájl tartalmának törlése is nem törölhető. Maga a mappa törlése mappa teljes tartalmát törölnie kell.
> 
> 
...
3. Nyissa meg a helyi menü táblázat, és válassza **nézet tábla**.
...
2. Válassza ki a **Lekérdezésszerkesztő** a tábla nézet eszköztár.
...
4. A lekérdezés futtatásához jelölje ki a zöld háromszögnek ikonra.
...
2. Az a **Storage-fiók létrehozása** párbeszédpanelen jelölje be, vagy adja meg a következő adatokat:
...
Ha létrehozott egy tárfiókot a **írásvédett Georedundáns** replikációs beállítás, a másodlagos végpont tekintheti meg:

* Nyissa meg a helyi menüben a fiók neve, majd válassza ki **tulajdonságok**.
...
> [!NOTE]
> Ha a tárfiók törlése a Server Explorer, a tárfiók vagy a benne található adatokat nem befolyásolja. Egyszerűen eltávolítja a hivatkozás a Server Explorer. Véglegesen törli a tárfiókot, használja a [a klasszikus Azure portálon](http://go.microsoft.com/fwlink/?LinkID=213885).
> 
> 
