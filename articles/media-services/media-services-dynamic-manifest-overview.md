...
有 [案例](media-services-dynamic-manifest-overview.md#scenarios) 當用戶端需要更多的彈性比預設資產資訊清單檔的描述。 例如：
...
若要達到這種彈性，媒體服務提供 **動態資訊清單** 根據預先定義 [篩選](media-services-dynamic-manifest-overview.md#filters)。  一旦您定義了篩選器，您的用戶端便會使用篩選器來串流視訊的特定轉譯或子剪輯。 用戶端會在資料流 URL 中指定篩選器。 篩選條件無法套用到調適性位元速率串流通訊協定支援 [動態封裝](media-services-dynamic-packaging-overview.md): HLS、 MPEG DASH、 Smooth Streaming 和 HDS。 例如：
...
如需如何傳遞內容和建置串流 Url 的詳細資訊，請參閱 [傳遞內容概觀](media-services-deliver-content-overview/)。
...
先前提過，將內容傳遞給客戶時 (串流即時事件或點播視訊)，您的目標是在不同的網路條件下將高品質的視訊傳遞到各種裝置。 此外，您可能有其他對於篩選資產與使用的需求 **動態資訊清單**s。 以下幾節提供不同篩選案例的簡短概觀。
...
下列範例使用 Azure Media Encoder 將夾層資產編碼成七個 ISO MP4 視訊轉譯 (從 180p 到 1080p)。 編碼的資產可以動態地封裝至下列任何一個資料流通訊協定：HLS、Smooth、MPEG DASH 及 HDS。  圖表頂端顯示不含篩選器的資產其 HLS 資訊清單 (包含全部七個轉譯)。  左下角顯示名為 "ott" 的篩選器已套用到 HLS 資訊清單。 "ott" 篩選器指定要移除所有不到 1Mbps 的位元速率，因此將最差的兩個品質等級從回應中去除。  在右下方，會顯示名為"mobile"篩選器要對其套用到 HLS 資訊清單。 "mobile" 篩選器指定要移除的解析度大於 720p 的轉譯，因此去除兩個 1080p 的轉譯。
...
若要結合篩選條件，您必須設定資訊清單/播放清單 URL 的篩選條件名稱，並以分號分隔。 假設您擁有名為篩選器 *MyMobileDevice* 篩選品質，還有另一個名為 *MyStartTime* 若要設定特定的啟動時間。 您可以將它們結合如下：
