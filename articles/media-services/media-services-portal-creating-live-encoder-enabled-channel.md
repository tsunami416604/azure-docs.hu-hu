...
如果您按一下 [**通道**] 頁面上的 [**停止串流處理**]，則會停止並刪除預設程式。 資產仍然會有和您可以發佈或解除發佈它從 **內容**  頁面。
...
若要提供您的使用者具有可用於串流處理內容的 URL，您首先要 「 發行 」 您的資產 (如前一節中所說明) 藉由建立定位器 (當您發行資產使用入口網站時，會為您建立定位器)。 定位器可以存取資產中所含的檔案。
