...
2. 使用  [windowsazure.mediaservices.extensions](https://www.nuget.org/packages/windowsazure.mediaservices.extensions) Nuget 封裝來安裝 **Azure 媒體服務.NET SDK 延伸**。 Media Services .NET SDK 延伸模組是一組延伸方法和協助程式函數，能夠簡化您的程式碼，並輕鬆地以媒體服務開發。 安裝這個封裝，也會安裝 **Media Services .NET SDK**，並新增所有其他必要相依性。
...
將預設 Program 類別覆寫為下列程式碼。 此程式碼示範如何讀取 App.config 檔案中的連線值，以及如何建立 **CloudMediaContext** 物件來連線到媒體服務。 如需有關連接到媒體服務的詳細資訊，請參閱 [for.NET 連接到媒體服務使用 Media Services SDK](http://msdn.microsoft.com/library/azure/jj129571.aspx)。
