...
下列程式碼範例示範如何建構 the https://endpoint/<subscriptionId>/services/mediaservices/帳戶/<accountName>/StorageAccounts/<storageAccountName>/Key 要求，以使用 Media Services 同步指定儲存體金鑰。在本情況中，會使用次要儲存體金鑰。如需詳細資訊，請參閱 [How to: 使用媒體服務管理 REST API](http://msdn.microsoft.com/library/azure/dn167656.aspx)。
