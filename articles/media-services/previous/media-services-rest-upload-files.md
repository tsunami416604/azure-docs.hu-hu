...
- További információkért tekintse át az [Azure ad-hitelesítés használata a Media Services API Rest-mel való elérését](https://docs.microsoft.com/en-us/azure/media-services/previous/media-services-rest-connect-with-aad) ismertető cikket.
...
1. Válassza a **AccessPolicy** -> **AccessPolicy létrehozása a feltöltéshez**lehetőséget.
...
Az adategységek létrehozásakor felvehető tulajdonságok egyike a **Beállítások**. A következő titkosítási beállítások egyikét megadhatja: **none** (alapértelmezett, nincs titkosítás), **StorageEncrypted** (az ügyféloldali tároló titkosításával előre titkosított tartalom esetén), **CommonEncryptionProtected**vagy  **EnvelopeEncryptionProtected**. Ha titkosított eszközzel rendelkezik, konfigurálnia kell egy kézbesítési házirendet. További információ: az [eszközök kézbesítési házirendjeinek konfigurálása](media-services-rest-configure-asset-delivery-policy.md).
...
1. Válassza **az** adategységek -> **eszköz létrehozása**lehetőséget.
2. Kattintson a **Küldés** gombra.
...
1. Válassza ki a **lokátor** -> **sas-lokátor létrehozása**elemet.
2. Kattintson a **Küldés** gombra.
...
1. A **+** gombra kattintva hozzon létre egy új kérés lapot.
...
1. Válassza a **AssetFiles** -> **CreateFileInfos**elemet.
2. Kattintson a **Küldés** gombra.
