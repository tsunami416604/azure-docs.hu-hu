...
content_key_specs | 陣列的 JSON 結構，請參閱 **內容金鑰規格** 下方 | 更細部控制要傳回的內容金鑰。 如需詳細資料，請參閱以下的內容金鑰規格。  只可以指定 allowed_track_types 和 content_key_specs 中的一個。 
use_policy_overrides_exclusively | 布林值。 True 或 False | 使用 policy_overrides 所指定的原則屬性，並略過先前儲存的所有原則。
policy_overrides | JSON 結構，請參閱 **原則會覆寫** 下方 | 此授權的原則設定。  如果此資產具有預先定義的原則，就會使用這些指定的值。 
session_init | JSON 結構，請參閱 **工作階段初始化** 下方 | 傳遞選擇性的資料給授權。
