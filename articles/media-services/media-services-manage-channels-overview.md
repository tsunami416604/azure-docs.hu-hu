...
- [通道和其相關的元件的說明](media-services-manage-channels-overview.md#channel)
...
## <a id="channel"></a>通道和其相關的元件的說明

### <a id="channel_input"></a>頻道輸入 (嵌入) 組態
...
---|---|---|---
 啟動中| 啟動中| 無 (暫時性狀態)
 執行中| 就緒 (沒有執行的程式)<p>或<p>串流 (至少一個執行程式)| 是
 停止中| 停止中| 無 (暫時性狀態)
 已停止| 已停止| 否
...
下表示範支援的隱藏式字幕和廣告插入標準。
 ...
 標準| 注意事項
---|---
 CEA-708 和 EIA-608 (708/608)| Cea-708 和 eia-608 被關閉美國和加拿大的隱藏式字幕標準。<p><p>目前，只有字幕時執行編碼的輸入資料流中。您使用的即時媒體編碼器，必須可以將 608 或 708 字幕插入至已傳送至媒體服務的已編碼資料流。媒體服務會將內含字幕的內容傳遞給您的檢視器。
 ...
 SCTE-35| 數位訊號系統，用來提示廣告插入。下游接收端會使用信號並根據分配的時間，將廣告切割成資料流。SCTE 35 必須輸入資料流中的疏鬆資料軌的形式傳送。<p><p>請注意，目前唯一支援附帶廣告訊號的輸入資料流格式 fragmented MP4 (Smooth Streaming)。唯一支援的輸出格式也是 Smooth Streaming。
...
- 通道或其相關聯程式正在執行時，您無法變更輸入通訊協定。 如果您需要不同的通訊協定，則應該為每個輸入通訊協定建立個別的通道。
- 只有當您的通道處於**執行中**狀態時，才會向您計費。 如需詳細資訊，請參閱 [這](media-services-manage-channels-overview.md#states) 一節。

## 如何建立從內部部署編碼器接收多位元速率即時串流的通道

如需在內部部署即時編碼器的詳細資訊，請參閱 [Azure 媒體服務搭配使用第 3 個廠商即時編碼器](https://azure.microsoft.com/blog/azure-media-services-rtmp-support-and-live-encoders/)。
