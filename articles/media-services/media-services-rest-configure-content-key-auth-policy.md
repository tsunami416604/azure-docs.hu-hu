...
如果您想要媒體服務加密資產，您需要建立加密金鑰 (**CommonEncryption** 或 **EnvelopeEncryption**) 與資產 (如所述 [這裡](media-services-rest-create-contentkey.md)) ，並且設定金鑰授權原則 (如本文中所述)。
...
媒體服務支援多種方式來驗證提出金鑰要求的使用者。 內容金鑰授權原則可能會有一個或多個授權限制：**open**、**token** 限制或 **IP** 限制。 token 限制原則必須伴隨著安全權杖服務 (STS) 所發出的權杖。 媒體服務支援 **簡單 Web 權杖** ([SWT](https://msdn.microsoft.com/library/gg185950.aspx#BKMK_2)) 格式和 ** JSON Web 權杖 **(JWT) 格式。
...
[整合 Azure 媒體服務 OWIN MVC 型應用程式與 Azure Active Directory，並限制以 JWT 宣告為基礎的內容金鑰傳遞](http://www.gtrifonov.com/2015/01/24/mvc-owin-azure-media-services-ad-integration/)。

[使用 Azure ACS 發行權杖](http://mingfeiy.com/acs-with-key-services)。

### 適用一些考量事項：

- 若要能夠使用動態封裝和動態加密，您必須確定至少一個串流保留的單元。 如需詳細資訊，請參閱 [如何調整媒體服務](media-services-manage-origins.md#scale_streaming_endpoints)。
...
當設定 **語彙基元** 限制原則，您必須指定主要 * * 驗證金鑰**, ，* * 簽發者** 和 **觀眾** 參數。  **主要驗證金鑰 * * 包含簽署權杖，使用的金鑰 * * 簽發者** 是發行權杖的安全權杖服務。 **對象** (有時稱為**範圍**) 描述權杖或權杖獲授權存取之資源的用途。 媒體服務金鑰傳遞服務會驗證權杖中的這些值符合在範本中的值。

下列範例會建立具有 token 限制的授權原則。 在此範例中，用戶端必須提出權杖，權杖中包含簽署金鑰 (VerificationKey)、權杖簽發者和必要的宣告。

### 建立 ContentKeyAuthorizationPolicies

建立 「 權杖限制原則 」 所示 [這裡](#ContentKeyAuthorizationPolicies)。
...
連結 ContentKeyAuthorizationPolicies 與選項，如所示 [這裡](#ContentKeyAuthorizationPolicies)。

#### 將授權原則加入內容金鑰

將 AuthorizationPolicy 加入 ContentKey，如所示 [這裡](#AddAuthorizationPolicyToKey)。
...
當您使用 PlayReady 保護內容，其中一個您需要在授權原則中指定的項目是 XML 字串，定義 [PlayReady 授權範本](https://msdn.microsoft.com/library/azure/dn783459.aspx)。
...
建立 ContentKeyAuthorizationPolicies，如顯示 [這裡](#ContentKeyAuthorizationPolicies2)。
...
## <a id="types"></a>定義 ContentKeyAuthorizationPolicy 時使用的型別
...
現在您已設定內容金鑰授權原則，請移至 [如何設定資產傳遞原則](media-services-rest-configure-asset-delivery-policy.md) 主題。
