...
>[AZURE.NOTE] 其他成本也與監視在 Azure 傳統入口網站的儲存體資料有關。 如需詳細資訊，請參閱 [儲存體分析及計費](http://go.microsoft.com/fwlink/?LinkId=256667)。
...
1. 按一下 [ **儲存體帳戶** 名稱下 **快速瀏覽** 一節。
2. 在儲存體帳戶] 頁面上，按一下 [ **[設定] 頁面** 連結，然後向下捲動至 **監視** Blob、 資料表和佇列服務，如下所示的設定。

    >[AZURE.NOTE] Blob 是媒體服務中唯一支援的存放裝置類型。

    ![] StorageOptions[] storage_options_scoped

3. 在 **監視**, ，設定 Blob 的監視等級和資料保留原則:
