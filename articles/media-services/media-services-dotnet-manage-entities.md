...
請注意，資產的定位器路徑只是資產的基底 URL。 若要建立使用者或應用程式可以瀏覽之個別檔案的直接路徑，您的程式碼必須將特定檔案路徑加入至定位器路徑。 如需有關如何執行這項操作的詳細資訊，請參閱主題 [使用 Media Services SDK for.NET 傳遞資產](media-services-deliver-streaming-content.md)。
