...
Microsoft Azure 媒體服務可讓您提供您使用進階加密標準 (AES) (使用 128 位元加密金鑰) 和 PlayReady DRM 加密的內容。 媒體服務也提供 **金鑰 \ 授權傳遞服務** 從用戶端可以取得金鑰或授權以播放加密的內容。 

本主題說明如何使用 **Azure 傳統入口網站** 設定內容金鑰授權原則。 金鑰稍後可以用來動態加密您的內容。 請注意，目前您可以加密下列串流格式：HLS、MPEG DASH 和 Smooth Streaming。 無法加密 HDS 串流格式，或漸進式下載。
 
當播放程式要求設定為動態加密的串流時，媒體服務會使用設定的金鑰，以 AES 或 PlayReady 加密來動態加密您的內容。 為了將串流解密，播放程式將從金鑰傳遞服務要求金鑰。 為了決定使用者是否有權取得金鑰，服務會評估為金鑰指定的授權原則。


如果您計劃有多個內容金鑰，或想要指定 **金鑰 \ 授權傳遞服務** URL，而非媒體服務金鑰傳遞服務，使用媒體服務.NET SDK 或 REST Api。
...
媒體服務支援多種方式來驗證提出金鑰要求的使用者。 內容金鑰授權原則可以有 **開啟**, ，**語彙基元**, ，或 **IP** 授權限制 (**IP** 可以使用 REST 或.NET SDK 設定)。 

###Open 限制

 **開啟** 限制表示系統將會傳送金鑰給提出金鑰要求的任何人。 這項限制可用於測試用途。

![OpenPolicy][open_policy]

###權杖限制

若要選擇權杖限制的原則，請按 **語彙基元** ] 按鈕。

 **權杖** 限制的原則必須伴隨著所發出的權杖 **安全權杖服務** (STS)。 媒體服務支援 **簡單 Web 權杖** ([SWT](https://msdn.microsoft.com/library/gg185950.aspx#BKMK_2)) 格式和 **JSON Web 權杖** (JWT) 格式。 如需資訊，請參閱 [JWT 權杖驗證](http://www.gtrifonov.com/2015/01/03/jwt-token-authentication-in-azure-media-services-and-dynamic-encryption/)。
...
當設定 **語彙基元** 限制的原則，您必須設定的值 **驗證金鑰**, ，**簽發者** 和 **觀眾**。 主要驗證金鑰包含簽署權杖使用的金鑰，簽發者是發行權杖的安全權杖服務。 對象 (有時稱為範圍) 描述權杖或權杖獲授權存取之資源的用途。 媒體服務金鑰傳遞服務會驗證權杖中的這些值符合在範本中的值。  

###PlayReady

保護內容時 **PlayReady**, ，您需要在授權原則中指定的事情之一是定義 PlayReady 授權範本的 XML 字串。 依預設，會設定下列原則：
...
您可以按一下 **匯入原則 xml** 按鈕，並提供不同的 XML 符合 XML 結構描述定義 [這裡](https://msdn.microsoft.com/library/azure/dn783459.aspx)。
