...
1. Az MP4-töredékek időbélyegeit és indexeit (**TrackFragmentExtendedHeaderBox** `fragment_ absolute_ time` és `fragment_index`) növekvő sorrendben kell megérkezniük. Bár a Media Services az ismétlődő töredékek esetében is rugalmas, a töredékek átrendezése a média idővonalának megfelelően korlátozott.
...
1. A kódoló nem használhatja az `Events()` a következőt az [1] 9,2-ben leírtak szerint Media Servicesba való élő betöltéshez.
...
1. Az új kódolónak meg kell próbálnia csökkenteni az adatvesztést. Az adathordozó-töredékek `fragment_absolute_time` és `fragment_index` a kódoló utolsó leállításának helyétől kell emelkednie. A `fragment_absolute_time`nak és `fragment_index`nak folyamatos módon kell növekedni, de szükség esetén megengedett a folytonosság bevezetése. Media Services figyelmen kívül hagyja a már kapott és feldolgozott töredékeket, így jobb, ha a töredékek újraküldésének oldalára kerül sor, mint hogy bevezesse a folytonosságot a média idővonalán. 
