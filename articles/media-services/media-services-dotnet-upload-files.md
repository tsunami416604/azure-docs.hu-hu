...
- **CommonEncryption** -使用此選項，如果您要上傳已加密並使用一般加密或 PlayReady DRM (例如，保護的 Smooth Streaming PlayReady DRM) 保護的內容。
- **EnvelopeEncrypted** – 如果您要上傳使用 AES 加密的 HLS，請使用此選項。 請注意，檔案必須已由 Transform Manager 編碼和加密。
- **StorageEncrypted** -您在本機使用 aes-256 位元加密的內容，然後上傳到 Azure 儲存位置加密儲存體。 以儲存體加密保護的資產會自動解除加密並在編碼前放置在加密的檔案系統中，並且會在上傳為新輸出資產之前選擇性地重新編碼。 儲存體加密的主要使用案例是讓您可以使用強式加密來保護磁碟中靜止的高品質輸入媒體檔。
...
如果您指定要以加密的資產 **CommonEncrypted** 選項，或 **EnvelopeEncypted** 選項時，您將需要建立資產與 **ContentKey**。 如需詳細資訊，請參閱 [如何建立 ContentKey](media-services-dotnet-create-contentkey.md)。 
...
-   建立 **AccessPolicy** 定義資產的權限和規定期間存取的執行個體。
    
-   建立 **定位器** 提供存取資產的執行個體。
    
-   建立 **BlobTransferClient** 執行個體。 此類型代表在 Azure Blob 上作業的用戶端。 在此範例中，我們會使用用戶端來監視上傳進度。 
    
-   逐一列舉所指定目錄中的檔案，並建立 **AssetFile** 每個檔案的執行個體。
    
-   將檔案上傳至媒體服務使用 **UploadAsync** 方法。 
    
>[AZURE.NOTE] 使用 UploadAsync 方法確保未封鎖呼叫，以平行方式上傳的檔案。
...
- 建立新 **CloudMediaContext** 每個執行緒的物件。  **CloudMediaContext** 類別不具備執行緒安全。
 
- 將 NumberOfConcurrentTransfers 從預設值 2 增加為較高的值 (例如 5)。 設定此屬性會影響的所有執行個體 **CloudMediaContext**。 
...
IngestManifestAsset 會建立資產與大量 IngestManifest 的關聯，以進行大量內嵌。 它也會建立構成每個資產之 AssetFile 的關聯。 
若要建立 IngestManifestAsset，請在伺服器內容上使用 Create 方法。
...
您可以使用任何高速用戶端的應用程式能夠將資產檔案上傳至 blob 儲存體容器所提供的 URI **IIngestManifest.BlobStorageUriForUpload** 屬性。 一個著名的高速上傳服務是 [Aspera On Demand for Azure Application](https://datamarket.azure.com/application/2cdbc511-cb12-4715-9871-c7e7fbbb82a6)。 您也可以撰寫程式碼來上傳資產檔案 (如下列程式碼範例所示)。
...
您可以判斷與相關聯的所有資產的大量內嵌進度 **IngestManifest** 藉由輪詢 Statistics 屬性 **IngestManifest**。 若要更新進度資訊，您必須使用新 **CloudMediaContext** 每次輪詢 Statistics 屬性。

下列範例將示範輪詢 IngestManifest 由其 **識別碼**。
...
下列範例顯示如何使用 .NET SDK 延伸模組上傳單一檔案。 在此情況下 **CreateFromFile** 方法會使用，但也會提供非同步版本 (**CreateFromFileAsync**)。  **CreateFromFile** 方法可讓您指定的檔案名稱、 加密選項和回呼，以報告檔案的上傳進度。
