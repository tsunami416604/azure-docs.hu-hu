...
媒體服務支援多種方式來驗證提出金鑰要求的使用者。 內容金鑰授權原則可能會有一個或多個授權限制：**open**或**token** 限制。 token 限制原則必須伴隨著安全權杖服務 (STS) 所發出的權杖。 媒體服務支援 **簡單 Web 權杖** ([SWT](https://msdn.microsoft.com/library/gg185950.aspx#BKMK_2)) 格式和 ** JSON Web 權杖 **(JWT) 格式。
...
當您使用 PlayReady 保護內容，其中一個您需要在授權原則中指定的項目是 XML 字串，定義 [PlayReady 授權範本](media-services-playready-license-template-overview.md)。 在 Media Services SDK for .NET 中，**PlayReadyLicenseResponseTemplate** 和 **PlayReadyLicenseTemplate** 類別將協助您定義 PlayReady 授權範本。
