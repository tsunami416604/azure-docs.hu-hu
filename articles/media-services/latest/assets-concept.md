...
Az eszköz az [Azure Storage](storage-account-concept.md) -fiókban található blob-tárolóra van leképezve, és az adategységben található fájlok a tárolóban blokk blobként tárolódnak. Media Services támogatja a blob-rétegeket, ha a fiók általános célú v2 (GPv2) tárolót használ. A GPv2 használatával fájlokat helyezhet át a ritka elérésű [vagy archív tárolóba](https://docs.microsoft.com/azure/storage/blobs/storage-blob-storage-tiers). Az archiválási tárterület alkalmas a forrásfájlok archiválására, ha már nincs rá szükség (például a kódolásuk után).
...
A következő táblázat azt mutatja be [](https://docs.microsoft.com/rest/api/media/assets/createorupdate#asset), hogy az adategység tulajdonságai hogyan jelennek meg az objektumnak a v2-ben lévő tulajdonságok tulajdonságában.

|v3-tulajdonságok|v2 tulajdonságai|
|---|---|
|azonosító – (egyedi) a teljes Azure Resource Manager elérési út, lásd [](https://docs.microsoft.com/rest/api/media/assets/createorupdate) : példák az eszközön||
|név – (egyedi), [](media-services-apis-overview.md#naming-conventions) lásd: elnevezési konvenciók ||
