...
4. Nyissa meg a *Startup.cs* , és frissítse a `ConfigureServices` metódust az Azure signaler szolgáltatás használatára úgy, hogy meghívja a `services.AddSignalR().AddAzureSignalR()` metódust a csak ASP.net Core 2 esetében:
    ...
    ```
    ASP.NET Core 3 + esetében nincs szükség módosításra a `ConfigureServices` metódushoz.

    Ha nem küld `AddAzureSignalR()`paramétert, ez a kód az alapértelmezett konfigurációs kulcsot használja a jelző szolgáltatás erőforrás-kapcsolódási karakterláncához. Az alapértelmezett konfigurációs kulcs az *Azure: signaler: ConnectionString*.

5. A *Startup.cs*-ben is frissítse a `Configure` metódust úgy, hogy lecseréli a `app.UseStaticFiles()`re irányuló hívást a következő kódra, és menti a fájlt, csak a 2. asp.net Core.
...
Mindkét módszer azt a `Clients` felületet használja, amelyet a ASP.NET Core a Signaler SDK biztosít. Ez az interfész hozzáférést biztosít az összes csatlakoztatott ügyfélhez, így tartalmat küldhet az ügyfeleknek.
...
Az *index. html* kódban meghívja a `HubConnectionBuilder.build()`t, hogy http-kapcsolatokat létesítsen az Azure signaler-erőforrással.
...
3. Nyisson meg két böngészőablakot. Minden böngészőben nyissa meg a `http://localhost:5000`. A rendszer kéri, hogy adja meg a nevét. Adja meg az ügyfél nevét mindkét ügyfél számára, és tesztelje az üzenetek tartalmát mindkét ügyfél között a **Küldés** gomb használatával.
