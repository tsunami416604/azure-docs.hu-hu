...
2. Adja `services.AddApplicationInsightsTelemetry();` hozzá a `ConfigureServices()` metódust az `Startup` osztályban, ahogy az alábbi példában is látható:
...
A súlyosság `Warning` vagy a `ILogger` nagyobb arány használatával kibocsátott naplók automatikusan rögzítésre kerülnek. A [ILogger-dokumentumok](ilogger.md#control-logging-level) követésével testreszabhatja, hogy a Application Insights mely naplózási szinteket rögzíti.
...
A `.cshtml` korábban hivatkozott fájlnevek egy alapértelmezett MVC-alkalmazás sablonból származnak. Végül, ha megfelelően szeretné engedélyezni az ügyféloldali figyelést az alkalmazáshoz, a JavaScript-kódrészletnek szerepelnie kell a `<head>` figyelni kívánt alkalmazás minden oldalának szakaszában. Ezt a célt ehhez az alkalmazás-sablonhoz a JavaScript- `_Layout.cshtml`kódrészlet hozzáadásával érheti el. 
...
A telemetria inicializálók alapértelmezés szerint jelennek meg. Az összes vagy adott telemetria-inicializáló eltávolításához használja a következő mintát *after* a hívása `AddApplicationInsightsTelemetry()`után.
...
Egyéni telemetria- `TelemetryConfiguration` processzorokat a bővítmény metódusának `AddApplicationInsightsTelemetryProcessor` használatával adhat hozzá. `IServiceCollection` A [speciális szűrési helyzetekben](https://docs.microsoft.com/azure/azure-monitor/app/api-filtering-sampling#itelemetryprocessor-and-itelemetryinitializer)telemetria processzorokat használ. Használja az alábbi példát.
...
Az alapértelmezett beállítások `TelemetryModule`konfigurálásához használja a bővítmény metódusát `IServiceCollection` `ConfigureTelemetryModule<T>` a következő példában látható módon:.
...
A 2.12.2- [`ApplicationInsightsServiceOptions`](#using-applicationinsightsserviceoptions) verziótól kezdődően egyszerűen letiltható az alapértelmezett modulok letiltására szolgáló lehetőség.
