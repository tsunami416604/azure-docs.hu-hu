...
### ExpressRoute 需要多少錢？
檢查 [定價詳細資料](http://azure.microsoft.com/pricing/details/expressroute/) 如需定價資訊。

### 如果我支付指定頻寬的 ExpressRoute 電路，則我是否必須從網路服務提供者購買相同速度的 VPN 連線？
編號 您可以從您的服務提供者購買任何速度的 VPN 連線。 不過，您與 Azure 的連線將會受限於您所購買的 ExpressRoute 電路頻寬。
...
請參閱 [ExpressRoute SLA 頁面](http://azure.microsoft.com/support/legal/sla/) 如需詳細資訊。
...
我們不會設定資料傳輸量的限制。 請參閱 [定價詳細資料](http://azure.microsoft.com/pricing/details/expressroute/) 如需頻寬費率的詳細資訊。
...
### 可以使用的服務提供者有哪些？
請參閱 [ExpressRoute 合作夥伴和位置](expressroute-locations.md) 服務提供者和位置清單。

## 技術詳細資訊

### 將我的內部部署位置連線至 Azure 會有哪些技術需求？
請參閱 [ExpressRoute 必要條件頁面](expressroute-prerequisites.md) 的需求。
...
請參閱 [ExpressRoute 電路的佈建和電路狀態的工作流程](expressroute-workflows.md) 如需詳細資訊。
...
如需詳細資訊，請參閱 [跨多個訂用帳戶中共用 ExpressRoute 電路](expressroute-share-circuit.md)。

### 連線到相同電路的虛擬網路會相互隔離嗎？
編號 所有連結到相同 ExpressRoute 電路的虛擬網路都屬於相同路由網域，且不會彼此隔離的路由的觀點。 如果需要路由隔離，您必須建立個別的 ExpressRoute 電路。
...
2. 執行 DNS 查閱，並尋找的 IP 位址 **kms.core.windows.net**
...
編號 您無法選取所需的功能。 當您開啟 ExpressRoute Premium 時，我們便會將所有功能啟用。

### ExpressRoute Premium 需要多少錢？
請參閱 [定價詳細資料](http://azure.microsoft.com/pricing/details/expressroute/) 成本。
...
1. 檢閱  [ExpressRoute 必要條件頁面](expressroute-prerequisites.md) 頁面，請確定您符合需求。
2. 檢閱的服務提供者和位置清單 [ExpressRoute 合作夥伴和位置](expressroute-locations.md) 確保已符合您的連線需求。
3. 規劃您的容量需求檢閱 [網路規劃和效能調整 for Office 365](http://aka.ms/tune/)。
4. 依照下面工作流程中的所列步驟設定連線。
    [ExpressRoute 電路的佈建和電路狀態的工作流程](expressroute-workflows.md)。

### 我的現有 ExpressRoute 電路是否支援與 Office 365 服務的連線？
是。 您可以設定現有 ExpressRoute 電路以支援與 Office 365 服務的連線。 請確保您有足夠的容量可以連線到 Office 365 服務。 [網路規劃和效能調整 for Office 365](http://aka.ms/tune/) 可協助您規劃您的連線需求。 此外，請參閱 [建立和修改 ExpressRoute 電路](expressroute-howto-circuit-classic.md)。
...
請參閱 [ExpressRoute 合作夥伴和位置](expressroute-locations.md) 的清單上的合作夥伴和位置都支援 ExpressRoute 的詳細資訊。
