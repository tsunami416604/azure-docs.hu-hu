...
|---|---|---|---|
| * * 最大值。每個對等互連支援的 # 前置詞 * *| 根據預設，在 ExpressRoute Premium 10000 4000| 200| 200|
| **支援的 IP 位址範圍**| 您的 WAN 內任何有效的 IPv4 位址。| 您或您的連線提供者所擁有的公用 IPv4 位址。| 您或您的連線提供者所擁有的公用 IPv4 位址。|
| **當數字的需求**| 私用和公用 AS 編號。客戶必須擁有公用數字。| 私用和公用 AS 編號。客戶必須擁有公用數字。| 只有公用 AS 編號。驗證 AS 編號必須路由註冊以驗證擁有權。|
| **路由介面 IP 位址**| RFC1918 和公用 IP 位址| 註冊到路由註冊中的客戶的公用 IP 位址。| 註冊到路由註冊中的客戶的公用 IP 位址。|
| **MD5 雜湊支援**| 是| 是| 是|
...
- 設定 ExpressRoute 連線。
    ...
    - [設定路由 (電路對等互連)](expressroute-howto-routing-classic.md)
