...
- 您必須有 [AzureRM.SiteRecovery](https://www.powershellgallery.com/packages/AzureRM.SiteRecovery/) 和 [AzureRM.RecoveryServices](https://www.powershellgallery.com/packages/AzureRM.RecoveryServices/) 已安裝的模組。 您可以將這些模組的最新版本向 [PowerShell 組件庫](https://www.powershellgallery.com/)
...
1. 開啟 PowerShell 主控台並執行這個命令來登入您的 Azure 帳戶。 此 cmdlet 會開啟網頁，提示您輸入您的帳戶認證。
    ...
    或者，加入您的帳戶認證做為參數 `登入 AzureRmAccount` 指令程式使用 `-認證` 參數。
...
3.  確認您的訂用帳戶已使用下列命令註冊，以使用 Azure 復原服務提供者和 Site Recovery：-
    ...
    >[AZURE.TIP] 在成功完成上述命令之後，要在您的訂用帳戶上啟用復原服務提供者的存取權可能需要一小時。 嘗試復原服務提供者註冊在訂閱中使用 `註冊 AzureRmResourceProvider ProviderNamespace Microsoft.RecoveryServices` 命令可能會在過渡期間失敗。 如果發生這種情況，請等待一個小時並重試。

    您在訂用帳戶上啟用復原服務提供者存取權之後，請藉由執行下列命令，在您的訂用帳戶中註冊提供者：

        Register-AzureRmResourceProvider -ProviderNamespace Microsoft.RecoveryServices

    確認已成功使用登錄提供者 `Get AzureRmResourceProvider ProviderNamespace Microsoft.RecoveryServices` 和 `Get AzureRmResourceProvider ProviderNamespace Microsoft.SiteRecovery` 命令
...
1. 建立複寫原則：-
 ...
 >
 > -如果指定的復原儲存體帳戶是型別 Azure Storage(Classic)，受保護機器的容錯移轉將機器復原到 Azure IaaS (傳統)
 > -受保護機器的容錯移轉如果指定的復原儲存體帳戶是型別 Azure Storage(ARM)，將復原機器 Azure IaaS (ARM)
