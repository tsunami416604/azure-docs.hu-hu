...
ms.locfileid: "29812851"
---
# <a name="deploy-the-mobility-service-with-azure-automation-dsc"></a>A mobilitási szolgáltatás üzembe helyezése az Azure Automation DSC-vel

A mobilitási szolgáltatást a VMware virtuális gépek és fizikai kiszolgálók replikálása Azure-ba, kívánt központi telepítése segítségével [Azure Site Recovery](site-recovery-overview.md).

Ez a cikk ismerteti a Windows-gépek segítségével az Azure Automation szükséges konfiguráló (DSC), annak érdekében, hogy a szolgáltatás üzembe helyezése:


## <a name="prerequisites"></a>Előfeltételek

* A tárház tárolja a szükséges telepítés
* A tárház tárolja a szükséges jelszót regisztrálása a management Server kiszolgálón. A konfigurációs kiszolgáló létrejön egy egyedi jelszót. 
* A Windows Management Framework (WMF) 5.0 kell telepíteni a gépeken, hogy engedélyezni szeretné a védelemhez. Ez az Automation DSC követelményeit.
Ha azt szeretné, hogy a DSC-ből a Windows gépeken, amelyek WMF 4.0-s verziójával használandó [DSC használata leválasztott környezetben](## Use DSC in disconnected environments).
 

## <a name="extract-binaries"></a>Bináris fájlok kibontása

A mobilitási szolgáltatást a parancssor használatával is telepíthető, és több argumentumot fogad el. A bináris fájlokat (ezt követően ezeket a telepítésből), és tárolja őket egy helyen, ahol helyreállíthatók a DSC-konfiguráció használata van szükség.

1. Bontsa ki az ehhez a telepítéshez szükséges, keresse meg a felügyeleti kiszolgálón a következő könyvtárra: **\Microsoft Azure Site Recovery\home\svsystems\pushinstallsvc\repository**
2. Ebben a mappában győződjön meg arról, hogy nincs-e nevű MSI-fájl: **Microsoft-ASR_UA_version_Windows_GA_date_Release.exe**.
3. Bontsa ki a telepítő a következő paranccsal: **.\Microsoft-ASR_UA_9.1.0.0_Windows_GA_02May2016_release.exe /q /x:C:\Users\Administrator\Desktop\Mobility_Service\Extract**
...
## <a name="store-the-passphrase"></a>Tárolja a jelszót

Meg kell határoznia, ahová a tömörített mappából. Egy Azure storage-fiók is használhatja, ahogy újabb, a jelszót, amelyet a telepítő kell tárolni. Az ügynök ezután regisztrálja az a felügyeleti kiszolgáló, a folyamat részeként.

1. Mentse a jelszót, amelyet a Configuration kiszolgáló szövegfájl - üzembe helyezésekor portáltól passphrase.txt.
2. Az Azure storage-fiók egy dedikált tárolójára tegyen mind a tömörített mappából, és a jelszót.
...
## <a name="create-the-dsc-configuration"></a>A DSC-konfiguráció létrehozása
...
## <a name="upload-to-automation-dsc"></a>Töltse fel az Automation DSC
...
1. Jelentkezzen be az Automation-fiók, keresse meg a **eszközök** > **modulok**, és kattintson a **Tallózás gyűjtemény**.
2. Keresse meg a modult, és importálja azt a fiókot.
3. Ha ezzel végzett, a számítógépen nyissa meg az Azure Resource Manager-modulok telepítése esetében, és folytassa az újonnan létrehozott DSC-konfiguráció importálásakor.

## <a name="import-cmdlets"></a>Importálási parancsmagokat

1. A PowerShellben jelentkezzen be az Azure-előfizetéshez.
2. Módosítsa a parancsmagok segítségével a környezetnek megfelelően, és rögzítheti az automatizálási fiók adatait egy változóban:

    ```powershell
    $AAAccount = Get-AzureRmAutomationAccount -ResourceGroupName 'KNOMS' -Name 'KNOMSAA'
    ```

3. Automation DSC konfigurációját feltölteni a következő parancsmag használatával:
...
1. Hogy regisztrálja azt csomópontok megkezdése a konfiguráció az Automation DSC-fordítási. Érhetők el, amely a következő parancsmagot:
...
2. Miután a konfiguráció fordítása le az adatok PowerShell (Get-AzureRmAutomationDscCompilationJob) használatával, vagy a a [Azure-portálon](https://portal.azure.com/).

    ![Feladat beolvasása](./media/vmware-azure-mobility-deploy-automation-dsc/retrieve-job.png)

Most már sikeresen közzé, és feltölti a DSC-konfiguráció Automation DSC.

## <a name="onboard-machines-to-automation-dsc"></a>A bevezetni gépek Automation DSC


1. Győződjön meg arról, hogy a Windows-alapú gépek WMF legújabb verziójára frissít. Töltse le, és a rendszernek a megfelelő verziót telepítse a [letöltőközpontból](https://www.microsoft.com/download/details.aspx?id=50395).
2. Hozzon létre egy metaconfig DSC, akkor a csomópontra érvényes lesz. A sikeres, szeretné beolvasni a végponti URL-cím és az elsődleges kulcsát a kijelölt Automation-fiók az Azure-ban. Ezek az értékek alapján is megtalálhatja **kulcsok** a a **összes beállítás** az Automation-fiók paneljén.

    ![Értékek](./media/vmware-azure-mobility-deploy-automation-dsc/key-values.png)

Ebben a példában a Site Recovery segítségével védeni kívánt fizikai Windows Server 2012 R2-kiszolgálóval rendelkezik.

## <a name="check-for-any-pending-file-rename-operations"></a>Ellenőrizze minden függőben lévő fájl átnevezése műveletek
...
1. Futtassa az alábbi parancsmagot, ellenőrizze, hogy a kiszolgálón nincs függőben lévő újraindítás:

    ```powershell
    Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\' | Select-Object -Property PendingFileRenameOperations
    ```
2. Ez azt mutatja, üres, ha az OK gombra a folytatáshoz áll. Ha nem, eleget kell tennie a karbantartási időszak alatt a kiszolgáló újraindításával.
3. A konfiguráció alkalmazása a kiszolgálón, indítsa el a PowerShell integrált parancsfájlkezelési környezet (ISE), és futtassa a következő parancsfájlt. Ez a lényegében egy DSC helyi konfiguráció, amely arra utasítja a helyi Configuration Manager motor regisztrálása az Automation DSC szolgáltatásban, és a megadott konfigurációs (ASRMobilityService.localhost) beolvasása.
...
1. Ez a parancsfájl futtatása után a csomópont az Automation DSC Szolgáltatásban regisztrálni kell kezdődnie.

    ![Folyamatban van a csomópont regisztrációs](./media/vmware-azure-mobility-deploy-automation-dsc/register-node.png)

2. Ha vissza az Azure-portálon, láthatja, hogy az újonnan regisztrált csomópont már megjelent a portál.

    ![A portál regisztrált csomópontja](./media/vmware-azure-mobility-deploy-automation-dsc/registered-node.png)

3. A kiszolgálón futtathatja a következő PowerShell-parancsmag segítségével győződjön meg arról, hogy a csomópont megfelelően van regisztrálva:

    ```powershell
    Get-DscLocalConfigurationManager
    ```

4. Miután a konfigurációs lekért, és a a kiszolgálóra alkalmazza, ezt a következő parancsmag futtatásával ellenőrizheti:
...
![Kimenet](./media/vmware-azure-mobility-deploy-automation-dsc/successful-config.png)
...
![Sikeres telepítés](./media/vmware-azure-mobility-deploy-automation-dsc/successful-install.png)
...
A mobilitási szolgáltatás ügynökök telepítése után is [engedélyezze a replikálást](vmware-azure-tutorial.md) a virtuális gépekhez.
