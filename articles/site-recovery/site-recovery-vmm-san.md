...
- 您必須至少一個 VMM 伺服器部署為實體或虛擬獨立伺服器，或與 System Center 2012 R2 上執行的虛擬叢集中，每個內部部署站台在 [VMM 更新彙總套件 5.0](http://go.microsoft.com/fwlink/?LinkId=517707)。
...
5. 在 [**區域**] 中，選取保存庫的地理區域。 若要檢查支援的區域，請參閱上市在 [Azure Site Recovery 定價詳細資料](href="http://go.microsoft.com/fwlink/?LinkId=389880)
...
1. 在 <b>復原服務</b> 頁面上，按一下保存庫以開啟 [快速入門] 頁面。 您也可以使用圖示隨時開啟 [快速啟動]。
...
>
        ...
        C:\Program Files\Microsoft System Center 2012 R2\Virtual Machine Manager\bin\ > DRConfigurator.exe [/r /Friendlyname <friendly name of the server> /認證 <path of the credentials file> /EncryptionEnabled <full file name to save the encryption certificate>         
 ####命令列安裝參數清單####
>
...
<p>
            ...
            <b>作業</b>
             索引標籤。上可修改雲端設定 
            <b>設定</b>
             索引標籤。如果您要修改目標位置或目標雲端，則必須移除雲端組態，然後重新設定雲端。
          </p>
...
<P>虛擬機器啟用保護之後出現在 Azure Site Recovery 主控台。您可以檢視虛擬機器內容、追蹤狀態，以及讓包含多個虛擬機器的複寫群組進行容錯移轉。請注意，在 SAN 複寫中，與複寫群組關聯的所有虛擬機器必須都一起進行容錯移轉。這是因為容錯移轉會先發生在儲存層。請務必正確分組您的複寫群組，並只相關聯虛擬機器放在一起。</P>
...
2. 指定復原方案的名稱，以及來源和目標 VMM 伺服器。 來源伺服器必須有已啟用容錯移轉和復原功能的虛擬機器。 選取 [SAN]****，僅檢視為 SAN 複寫設定的雲端。
   3. ![建立復原計畫](./media/site-recovery-vmm-san/SRSAN_RPlan.png)

4. 在 [選取虛擬機器]**** 中，選取複寫群組。 所有與複寫群組關聯的虛擬機器，將會被選取並新增至復原方案。 這些虛擬機器將新增到復原計畫的預設群組—群組 1。 您可以視需要新增更多的群組。 請注意，複寫之後，虛擬機器將會根據復原方案群組的順序來啟動。
