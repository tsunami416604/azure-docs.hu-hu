...
- 當觸發容錯移轉，並在復原網路中以必要的 IP 位址建立 VM 時，可以使用下列項目建立 VM 的連線能力： Th 是動作可以編寫指令碼。 如同上一節中討論有關子網路容錯移轉，也在容錯移轉至 Azure 的情況下路由必須適當地修改，以反映該 192.168.1.0/24 現在已移至 Azure。
...
[了](site-recovery-network-mapping.md) 站台復原對應來源與目標網路的方式。
