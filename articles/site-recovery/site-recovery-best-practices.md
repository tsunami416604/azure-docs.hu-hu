...
**儲存體對應** | NA | NA | 您可以選擇性設定儲存體對應，以確保虛擬機器可在容錯移轉之後，以最佳方式連接至儲存體。 當您在兩個內部部署 VMM 網站之間執行複寫，則根據預設，會將複本虛擬機器儲存在目標 Hyper-V 主機伺服器的指定位置上。 您可以在來源與目標 VMM 伺服器上，設定 VMM 儲存體分類之間的對應。  | 若要使用此功能，您必須在開始部署之前，確定已先設定好儲存體分類。 [閱讀更多](site-recovery-storage-mapping.md)。
...
**需求** | **將 VMware VM/實體伺服器複寫至 Azure)** | * **至次要站台複寫 VMware Vm/實體伺服器**  
---|---|--- 
**主要站台** | **處理序伺服器**︰ 專用的 Windows 伺服器 （實體或虛擬） | **處理序伺服器**︰ 專用的 Windows 伺服器 （實體或 VMware 虛擬機器<br/><br/>  
**次要內部部署網站** | NA | **設定伺服器**︰ 專用的 Windows 伺服器 （實體或虛擬） <br/><br/> **主要目標伺服器**: （實體或虛擬） 的專用的伺服器。 以 Windows 設定來保護 Windows 機器，或以 Linux 設定來保護 Linux 機器。
**Azure** | **訂閱**︰ 您必須為站台復原服務訂閱。 閱讀有關 [定價](pricing/details/site-recovery/) <br/><br/> **儲存體帳戶**︰ 您必須啟用異地備援儲存體帳戶。 此帳戶應該與 Site Recovery 保存庫位於相同的區域，且和同一個訂用帳戶產生關聯。 [閱讀更多](../storage/storage-introduction.md)。<br/><br/> **設定伺服器**︰ 您必須將組態伺服器做為 Azure VM 設定 <br/><br/> **主要目標伺服器**︰ 您必須設定主要目標伺服器做為 Azure VM <br/><br/> 以 Windows 設定來保護 Windows 機器，或以 Linux 設定來保護 Linux 機器。<br/><br/> **Azure 虛擬網路**︰ 您必須在其設定伺服器與主要目標伺服器將會部署 Azure 虛擬網路。 需與 Azure Site Recovery 保存庫位於相同的訂用帳戶與區域中 | NA  
**虛擬機器/實體伺服器** | 至少一個 VMware 虛擬機器或實體 Windows/Linux 伺服器。<br/><br/>在部署期間將會安裝行動服務的每部機器上| 至少一個 VMware VM 或實體 Windows/Linux 伺服器。<br/><br/> 在部署期間於每部機器上安裝整合代理程式。
