...
Site Recovery 服務提供健全的商務持續性和災害復原 (BCDR) 解決方案，可協調及自動化複寫並容錯移轉至 Azure，或是次要的內部部署資料中心，藉以保護您的內部部署實體伺服器和虛擬機器。 如需 Site Recovery 部署案例簡介讀取 [站台復原概觀](site-recovery-overview.md)。
...
指定復原方案的名稱，以及來源和目標伺服器。 來源伺服器必須有已啟用容錯移轉和復原功能的虛擬機器。
    ...
    - 如果從 Hyper-V 站台複寫，請選取 [來源類型] 中的 Hyper-V 站台。 選取的站台做為來源和 ** Azure **做為目標。
...
- ** 新增指令碼 **— 您可以新增指令碼之前或之後的復原方案群組。 新增指令碼後，群組中會加入新的動作集。 例如，系統會為 Group 1 建立一組前置步驟，並取名為 Group 1: Pre-steps。 所有前置步驟都會列於此動作集中。 請注意，若您已部署 VMM 伺服器，則只能在主要網站上新增指令碼。
...
- 會在 Windows PowerShell 模組中傳遞 VMM Cmdlet。 安裝 VMM 主控台時，會一併安裝 VMM Windows PowerShell 模組。 系統可能使用指令碼的下列命令將 VMM 模組載入您的指令碼：Import-Module -Name virtualmachinemanager。 [Get more details](hhttps://technet.microsoft.com/library/hh875013.aspx).
...
- 如果您的程式庫共用路徑位於遠端 (或本機上，但未與 MSCVMMLibrary 共用、 設定共用，如下所示 (使用 \\libserver2.contoso.com\share\ 做為範例):
    ...
    - 將成為 \\libserver2.contoso.com\share\ 的值。 指定完整的 FQDN。
    - 提供共用位置的權限。
...
-   在 VMM 伺服器上，設定要略過的執行原則，如下所示：
    ...
    - 類型：**Set-executionpolicy bypass**。 [Get more details](https://technet.microsoft.com/library/ee176961.aspx).
...
1. 在程式庫共用中，建立新的資料夾，例如 \<VMMServerName>\MSSCVMMLibrary\RPScripts。將資料夾放在來源和目標 VMM 伺服器上。
2. 建立指令碼 (例如 RPScript)，並檢查其能否如預期運作。
3. 將指令碼放置在位置 \<VMMServerName>\MSSCVMMLibrary 在來源和目標 VMM 伺服器上的。

#### 建立 Azure 自動化 Runbook

您可以將 Azure 自動化 Runbook 納入方案中執行，藉以擴充您的復原方案。 [Read more](site-recovery-runbook-automation.md).
...
4. 如果是新增 VMM 指令碼，請選取 [容錯移轉至 VMM 指令碼]****，然後在 [指令碼路徑] ****中輸入共用的相對路徑。因此，我們的範例的共用位置為 \\<VMMServerName>\MSSCVMMLibrary\RPScripts，指定的路徑: \RPScripts\RPScript.PS1。
...
您可以執行不同類型的容錯移轉復原方案，包括可檢查環境的容錯移轉測試，以及規劃或未規劃的容錯移轉。 深入了解容錯移轉，以及如何取得的指示執行不同類型的容錯移轉 [這裡](site-recovery-failover.md) 有關容錯移轉。
