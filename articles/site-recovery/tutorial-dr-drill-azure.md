---
title: Vész-helyreállítási gyakorlat futtatása az Azure-ba az Azure Site Recovery segítségével
description: Ismerje meg, hogyan futtathat vész-helyreállítási gyakorlatot a helyszíni azure-ból az Azure-ba az Azure Site Recovery segítségével.
...
Ez a cikk ismerteti, hogyan futtathat egy vész-helyreállítási gyakorlat egy helyszíni gép az Azure-ba az [Azure Site Recovery](site-recovery-overview.md) szolgáltatás használatával. A próba adatveszteség nélkül ellenőrzi a replikációs stratégiát.


Ez a negyedik oktatóanyag egy sorozatban, amely bemutatja, hogyan állíthatja be a vészhelyreállítást az Azure-ban a helyszíni gépekhez.
...
> * Egyetlen gép tesztfeladat-átvétele.

> [!NOTE]
> Oktatóanyagok megmutatja a legegyszerűbb telepítési útvonalat egy forgatókönyv. Ahol lehet, az alapértelmezett beállításokat használják, és nem mutatják be az összes lehetséges beállítást és útvonalat. Ha részletesebben szeretne többet megtudni a vész-helyreállítási gyakorlat lépéseiről, [tekintse át ezt a cikket.](site-recovery-test-failover-to-azure.md)
...
2. Készítse elő a helyszíni [VMware](vmware-azure-tutorial-prepare-on-premises.md) vagy [Hyper-V](hyper-v-prepare-on-premises-tutorial.md) környezetben a vész-helyreállítási. Ha vészhelyreállítást állít be a fizikai kiszolgálókhoz, tekintse át a [támogatási mátrixot.](vmware-physical-secondary-support-matrix.md)
3. Állítsa be a [vészhelyreállítást vmware virtuális gépekhez,](vmware-azure-tutorial.md) [hyper-v virtuális gépekhez](hyper-v-azure-tutorial.md)vagy [fizikai gépekhez.](physical-azure-disaster-recovery.md)
...
3. A **Számítási és hálózati**alkalmazásban módosíthatja az Azure nevét, erőforráscsoportját, célméretét, rendelkezésre állási készletét és a felügyelt lemez beállításait.
...
7. A feladatátvételi teszt során létrehozott Azure-beli virtuális gépek törléséhez kattintson a **Feladatátvételi teszt eltávolítása** elemre a virtuális gépen. A **Megjegyzések alkalmazásban**rögzítse és mentse a tesztfeladat-átvételhez kapcsolódó megfigyeléseket.
...
## <a name="connect-after-failover"></a>Csatlakozás feladatátvétel után

Ha rdp/SSH használatával szeretne csatlakozni az Azure-beli virtuális gépekhez a feladatátvétel után, [készüljön fel a csatlakozásra.](site-recovery-test-failover-to-azure.md#prepare-to-connect-to-azure-vms-after-failover) Ha a feladatátvétel után kapcsolódási problémákat tapasztal, kövesse a [hibaelhárítási útmutatót.](site-recovery-failover-to-azure-troubleshoot.md)

## <a name="next-steps"></a>További lépések

> [!div class="nextstepaction"]
> [A VMware virtuális gépek feladat-átvételének és feladat-visszavételének futtatása](vmware-azure-tutorial-failover-failback.md)
> [feladat-, valamint feladat-visszavételt futtat a Hyper-V virtuális gépekszámára](hyper-v-azure-failover-failback-tutorial.md)
> [feladat-átvételt és feladat-visszavételt futtat a fizikai gépeken](physical-to-azure-failover-failback.md)
