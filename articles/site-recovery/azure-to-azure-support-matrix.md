...
Oracle Linux | 6.4, 6.5, 6.6, 6.7<br/><br/> A Red Hat-kompatibilis kernel vagy a szoros vállalati Kernel Release-3 (UEK3) fut.
...
Az Azure Disk Encryption (ADE) for Windows | Engedélyezve van a virtuális gépek [az Azure AD-alkalmazást encryption](https://aka.ms/ade-aad-app) támogatottak. |
Az Azure Disk Encryption (ADE) Linux rendszeren | Nem támogatott |
Hozzáadását vagy eltávolítását lemez    | Nem támogatott | Adja hozzá, vagy távolítsa el az adatlemezt a virtuális gépen, ha kell, és tiltsa le a replikációt a virtuális gép, majd újra engedélyeznie.
Lemez kizárása | Nem támogatott | Az ideiglenes lemez alapértelmezés szerint ki van zárva.
...
Hitelesített proxykiszolgálói | Támogatott | [További információk](site-recovery-azure-to-azure-networking-guidance.md).   
Hitelesített proxyt | Nem támogatott | Egy hitelesített proxyt a kimenő hálózati kapcsolatot használó virtuális Gépet nem lehet replikálni.     
Site-to-site VPN | Akár az ExpressRoute nem támogatott | Győződjön meg arról, hogy úgy, hogy úgy, hogy a Site Recovery forgalom nem legyen irányítva a helyszíni az udr-EK és NSG-k vannak konfigurálva. [További információk](site-recovery-azure-to-azure-networking-guidance.md).
A virtuális hálózatok közötti kapcsolat | Támogatott |[További információk](site-recovery-azure-to-azure-networking-guidance.md). 
Virtuális hálózati szolgáltatásvégpontok | Támogatott | Vegye figyelembe, hogy az Azure Storage-tűzfalak, a virtuális hálózatok nem támogatottak. Emellett engedélyezi a hozzáférést az adott Azure virtuális hálózatokhoz a replikált adatok tárolására szolgáló gyorsítótár tárfiókokhoz nem támogatott.
