...
- **兩個複寫 VMM 雲端中的 HYPER-V 虛擬機器內部部署資料中心**— 網路對應會對應來源 VMM 伺服器上的 VM 網路與 VM 網路與目標 VMM 伺服器，執行下列動作:

    - **容錯移轉之後連線虛擬機器**— 可確保虛擬機器將連線到適當的網路，在容錯移轉之後。 複本虛擬機器將會連線至對應到來源網路的目標網路。
    - **將複本虛擬機器放在主機伺服器上**— 以最佳方式將複本虛擬機器放在 HYPER-V 主機伺服器上。 複本虛擬機器將會放在可以存取對應的 VM 網路的主機上。
    - **無網路對應**— 如果您未設定網路對應，複寫的虛擬機器將不會連線至任何 VM 網路在容錯移轉之後。

- **在內部部署 VMM 雲端中的 HYPER-V 虛擬機器複寫至 Azure**— 網路對應會對應來源 VMM 伺服器上之 VM 網路與目標 Azure 網路，以執行下列動作:
    - **容錯移轉之後連線虛擬機器**— 所有機器相同網路上的容錯移轉可以彼此連接，無論哪個復原計畫它們都一樣。
    - **網路閘道**— 如果目標 Azure 網路上設定網路閘道，虛擬機器可以連接到其他內部部署虛擬機器。
    - **無網路對應**— 如果您未設定網路對應，虛擬機器只有相同復原計畫中的容錯移轉將能夠彼此連接在失敗後移轉至 Azure。
