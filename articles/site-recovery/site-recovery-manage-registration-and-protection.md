...
- **未連接的 VMM 伺服器**：如果 VMM 伺服器未連線，當您刪除該伺服器時，您必須手動執行指令碼，才能執行清除作業。 指令碼位於 [Microsoft 庫](https://gallery.technet.microsoft.com/scriptcenter/Cleanup-Script-for-Windows-95101439)。 請記下伺服器的 VMM 識別碼，以完成手動清除程序。
...
2. 在 [**移除虛擬機器**] 上選取其中一個選項：
    ...
    - **停用保護 (使用復原切入和調整大小的磁碟區)**—只要在擁有下列條件，您才會看到及啟用此選項：
        ...
        - **執行容錯移轉**—執行從內部部署 VMware 虛擬機器或實體伺服器至 Azure 的容錯移轉來測試環境後，請選取此選項以重新開始保護您的內部部署虛擬機器。 這個選項會停用每個虛擬機器，您接著必須重新啟用它們的保護。 請注意：
            -停用這項設定的虛擬機器不會影響在 Azure 中複本虛擬機器。
            -您不得從虛擬機器，以解除安裝行動服務。

    - **停用保護**—如果您啟用並儲存此選項，機器將不再受到 Site Recovery 的保護。 系統將會自動清除機器的保護設定。
    - **從保存庫移除**—如果您選擇此選項，機器將只會從 Site Recovery 保存庫中移除。 機器的內部部署保護設定不受影響。 要移除機器上的設定，並移除虛擬機器從 Azure 訂用帳戶，您將需要清除設定解除安裝行動服務。
