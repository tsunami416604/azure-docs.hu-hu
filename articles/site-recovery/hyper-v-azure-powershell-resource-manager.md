...
1. Nyisson meg egy PowerShell-konzolt, és futtassa ezt a parancsot az Azure-fiókba való bejelentkezéshez. A parancsmag egy weboldalt hoz létre a fiók hitelesítő adatainak megadásához: `Connect-AzAccount`.
   - Másik lehetőségként a fiók hitelesítő adatait a `Connect-AzAccount` parancsmagban található paraméterként is megadhatja a **hitelesítőadat** -paraméter használatával.
...
1. Az előfizetéshez tartozó erőforráscsoportok listájának beszerzéséhez futtassa az `Get-AzResourceGroup` parancsmagot.
...
A meglévő tárolók listáját a `Get-AzRecoveryServicesVault` parancsmaggal kérheti le.
...
1. A `Get-AzRecoveryServicesAsrJob` parancsmaggal kérheti le a feladattípust, és megkeresheti a feladatok aktuális állapotát.
...
> 1. A védett elemek minden lemezének lemez-AZONOSÍTÓjának beolvasásához használja a `Get-AzRecoveryServicesAsrReplicationProtectedItem` parancsmagot
> 1. Hozzon létre egy szótár-objektumot `New-Object "System.Collections.Generic.Dictionary``2[System.String,System.String]"` parancsmaggal, hogy tartalmazza a lemez titkosítási csoportba való hozzárendelését. Ezeket a lemezes titkosítási csoportokat előre létre kell hoznia a célként megadott régióban.
> 1. Frissítse a virtuális gép tulajdonságait `Set-AzRecoveryServicesAsrReplicationProtectedItem` parancsmaggal a **DiskIdToDiskEncryptionSetMap** paraméterben található szótár objektum átadásával.
