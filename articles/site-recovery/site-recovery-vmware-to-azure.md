 ...
 **主要目標伺服器**| <p>部署為 Azure 虛擬機器 — OpenLogic CentOS 6.6 組件庫映像 (以保護 Linux 機器) 為基礎的其中一個 Windows server 架構的 Windows Server 2012 R2 資源庫映像 (以保護 Windows 機器) 或 Linux 伺服器。</p><p>三個調整大小選項可供使用-標準 A4、 標準 D14 和標準 DS4。<p><p>伺服器已連線到設定伺服器相同的 Azure 網路。</p><p>在 Site Recovery 入口網站中設定</p>| <p>它會接收並保留複寫的資料從受保護機器使用 Azure 儲存體帳戶中的 blob 儲存體上建立的附加的 Vhd。</p><p>選取標準 DS4 專為設定保護的工作負載需要一致的高效能和低延遲使用高階儲存體帳戶。</p>
 **處理序伺服器**| <p>將部署為執行 Windows Server 2012 R2 的內部部署虛擬或實體伺服器</p> <p>建議它放置在相同的網路與 LAN 區段上您想要保護的機器，但是它可以在不同的網路上執行，只要受保護的機器具有 L3 網路可見性。 請<p>設定它，並註冊至 Site Recovery 入口網站中設定伺服器。</p>| <p>受保護的機器將複寫資料傳送至內部部署處理序伺服器。它具有磁碟快取功能，可快取本身接收的複寫資料。會對該資料執行一些動作。</p><p>它會以最佳化資料快取、 壓縮和加密，再將它傳送到主要目標伺服器。</p><p>處理行動服務推入安裝。</p><p>它會執行自動探索的 VMware 虛擬機器。</p>
 ...
 **複寫機制**| <p>**透過網際網路**— 從受保護的內部部署伺服器與 Azure 透過公用網際網路連線以安全的 SSL/TLS 通訊通道的通訊與複寫資料。這是預設選項。</p><p>**VPN/ExpressRoute**— 通訊與複寫資料在內部部署伺服器與 Azure 之間的 VPN 連線。您必須設定站台對站 VPN 或 [ExpressRoute](../expressroute-introduction.md) 內部部署站台與 Azure 網路之間的連線。</p><p>您會選取您想要在 Site Recovery 部署期間複寫的方式。機制設定後若不會影響已受保護伺服器上的保護，您便無法變更機制。| <p>這兩個選項需要您開啟受保護機器上的任何輸入的網路連接埠。所有的網路通訊是從內部部署站台起始。</p>
...
- **每一來源每日變更率上限**—考慮每一來源建議的變更率時，有三個需要考量的因素。 針對目標式考量，來源上每個作業的目標磁碟需要兩個 IOP。 這是因為目標磁碟上將會發生舊資料的讀取和新資料的寫入。
    ...
    - **儲存體帳戶支援的最大輸送量**—來源不能跨越多個儲存體帳戶。 假設某個儲存體帳戶每秒可接受最多 20,000 個要求，並且每個來源 IOP 會在主要目標伺服器上產生 2 個 IOP，建議您將來源的 IOP 數目保留為 10,000。 請參閱 [延展性和效能目標時使用高階儲存體](../storage/storage-scalability-targets.md#scalability-targets-for-premium-storage-accounts) 時來源高階儲存體帳戶的設定。
...
- 每個主要目標伺服器只有一個保留磁碟區。 保留磁碟區會在連接到主要目標伺服器的磁碟之間共用。 例如：
    ...
    - 在保留磁碟區上，這會變成 120 * 5 = 600 個 IOP，而這可能會變成瓶頸。 在此案例中，加入更多磁碟到保留的磁碟區，並跨越以做為 RAID 等量磁碟區設定是不錯的策略。 因為 IOP 會分散到多個磁碟機，這將可改進效能。 要加入至保留磁碟區的磁碟機數目如下所示：
        -從來源環境的總 IOPS / 500
        -變換總計每日來自來源環境 (未壓縮) / 287 GB。287 GB 是每日的目標磁碟支援的最大輸送量。 此度量會隨著寫入大小 8K 的因數而異，因為在此情況下，8K 是假設的寫入大小。 例如，如果寫入大小是 4K，則輸送量會是 287/2。 而且如果寫入大小為 16K，那麼輸送量會是 287*2。
 ...
 **Azure 儲存體**| <p>您將需要 Azure 儲存體帳戶來儲存複寫的資料</p><p>任一個帳戶應為 [標準的地理區域備援儲存體帳戶](../storage/storage-redundancy.md#geo-redundant-storage) 或 [高階儲存體帳戶](../storage/storage-premium-storage-preview-portal.md)。</p><p>與 Azure Site Recovery 服務相同的區域且與相同的訂閱相關聯。</p><p>若要深入閱讀 [Microsoft Azure 儲存體簡介](../storage/storage-introduction.md)</p>
 ...
 **Azure 虛擬機器**| <p>您想要保護的虛擬機器應該要符合 [Azure 必要條件](site-recovery-best-practices.md)。</p><p>**磁碟計數**-31 個磁碟最多可以支援單一的受保護伺服器上</p><p>**磁碟大小**— 個別磁碟容量不可超過 1023 GB</p><p>**群集**— 不支援叢集伺服器</p><p>**開機**— 整合可延伸韌體介面 (UEFI) / 可延伸韌體 Interface(EFI) 開機不支援</p><p>**磁碟區**— Bitlocker 加密的磁碟區不支援</p><p> **伺服器名稱**— 名稱應包含介於 1 到 63 個字元 (字母數字和連字號)。名稱必須以字母或數字開頭，並以字母或數字結尾。機器受到保護之後，您可以修改 Azure 的名稱。</p>
 ...
 **處理序伺服器**| <p>您可以部署在實體或虛擬機器執行 Windows Server 2012 R2 的最新的更新處理序伺服器。安裝在 c: /。</p><p>建議您將伺服器放在相同的網路和子網路上您想要保護的機器。</p><p>安裝 VMware vSphere CLI 5.5.0 處理序伺服器上。VMware vSphere CLI 元件，才能處理序伺服器上探索由 vCenter 伺服器管理的虛擬機器或 ESXi 主機上執行的虛擬機器。</p><p>安裝路徑應該只有英文字元。</p><p>ReFS 檔案系統不支援。</p>
 **VMware**| <p>VMware vCenter 伺服器，管理您的 VMware vSphere hypervisor。它應該執行 vCenter 5.1 或 5.5 的版本與最新的更新。</p><p>您想要保護的一或多個 vSphere hypervisor，包含 VMware 虛擬機器。Hypervisor 應該執行 ESX/ESXi 5.1 或 5.5 版與最新的更新。</p><p>VMware 虛擬機器應該已安裝並執行 VMware 工具。</p>
 **Windows 機器**| <p>受保護實體伺服器或執行 Windows 的 VMware 虛擬機器有一些需求。</p><p>支援的 64 位元作業系統: **Windows Server 2012 R2**, ，**Windows Server 2012**, ，或 **Windows Server 2008 R2 在至少為 SP1**。</p><p>主機名稱、 掛接點、 裝置名稱、 Windows 系統路徑 (例如: C:\Windows) 僅可使用英文。</p><p>應該 C:\ 磁碟機上安裝作業系統。</p><p>支援只有基本磁碟。不支援動態磁碟。</p><p><Firewall rules on protected machines should allow them to reach the configuration and master target servers in Azure.p><p>您必須提供系統管理員帳戶 (必須是本機系統管理員在 Windows 電腦上的) 以推入安裝 Windows 伺服器上的行動服務。如果提供的帳戶是非網域帳戶，您必須停用本機電腦上的遠端使用者存取控制。若要執行此動作，請在 HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System 下加入值為 1 的 LocalAccountTokenFilterPolicy DWORD 登錄項目。若要從 CLI 開啟 cmd 或 powershell 加入登錄項目，然後輸入 * *`REG ADD HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System /v LocalAccountTokenFilterPolicy /t REG_DWORD /d 1`* *。[深入](https://msdn.microsoft.com/library/aa826699.aspx) 關於存取控制。</p><p>容錯移轉之後，如果您想要連線到遠端桌面與 Azure 中的 Windows 虛擬機器確定內部部署機器已啟用遠端桌面。如果您不透過 VPN 連線、 防火牆規則應該允許遠端桌面連線透過網際網路。</p>
 **Linux 機器**| <p> 支援 64 位元作業系統: **Centos 6.4、 6.5、 6.6**; **Oracle Enterprise Linux 6.4、 6.5，執行 Red Hat 相容核心或 Unbreakable Enterprise Kernel 第 3 版 (UEK3)**, ，**SUSE Linux Enterprise Server 11 SP3**。</p><p>受保護的機器上的防火牆規則應該允許它們連線到 Azure 中的設定和主要目標伺服器。</p><p>eg /etc/ hosts 檔案受保護機器上的應該包含所有 Nic 相關都聯的 IP 位址的本機主機名稱對應的項目 </p><p>如果您想要連接到 Azure 虛擬機器執行 Linux，使用安全殼層用戶端 (ssh) 的容錯移轉之後，請確定受保護的電腦上的安全殼層服務設定為在系統開機時自動啟動，且防火牆規則允許 ssh 連線。 請</p><p>的主機名稱、 掛接點、 裝置名稱和 Linux 系統路徑和檔案名稱 (例如 eg /etc/; /usr) 僅可使用英文。</p><p>可以使用下列儲存體的內部部署機器啟用保護:-<br>檔案系統: EXT3、 ETX4、 ReiserFS、 XFS<br>多重路徑軟體裝置對應工具 (多重路徑)<br>磁碟區管理員: LVM2<br>不支援使用 HP CCISS 控制站儲存體的實體伺服器。</p>
...
5. 在 [**區域**] 中，選取保存庫的地理區域。 若要檢查支援的區域，請參閱上市在 [Azure Site Recovery 定價詳細資料](pricing/details/site-recovery/)
...
4. 在 [新的組態伺服器詳細資料]**** 中指定：
    ...
    [AZURE。附註] 這項設定是部署時的選項讓，以及之後無法變更。
...
7.  如果您在執行 Windows：
    ...
    3. 如果您估計的保留磁碟機需要超過 1 TB，您就可以設定保留磁碟區 (:) 使用虛擬磁碟和 [儲存空間](http://blogs.technet.com/b/askpfeplat/archive/2013/10/21/storage-spaces-how-to-configure-storage-tiers-with-windows-server-2012-r2.aspx)
...
8. 如果您在執行 Linux：
    ...
    2. 在 [準備目標 (Azure) 資源]**** 中，按一下 [下載並安裝其他軟體 (僅適用 Linux 主要目標伺服器)]**** 以下載 Linux 主要目標伺服器封裝。將下載的 tar 檔案複製到使用 sftp 用戶端的虛擬機器。或者，您便可以登入已部署的 llinux 主要目標伺服器上，並使用 *wget http://go.microsoft.com/fwlink/?LinkID=529757&clcid=0x409* 下載檔案。
    ...
    5. 設定伺服器複雜密碼複製到本機檔案，使用命令 * * echo *`< 複雜密碼 >`* > passphrase.txt * *
    6. 執行命令"* * sudo。 /install-t 這兩種-a host-R MasterTarget-d /usr/local/ASR-i *`< 組態伺服器內部 IP 位址 >`*-p 443-s y-c https-P passphrase.txt * * 」。

    ![註冊目標伺服器](./media/site-recovery-vmware-to-azure/Linux-MT-install.png)

9. 等候幾分鐘 (10-15)，然後在 [伺服器]**** > [組態伺服器]**** 頁面上檢查主要目標伺服器在 [伺服器詳細資料]**** 索引標籤上是否列為已註冊。 如果您在執行 Linux 而且並未註冊主應用程式設定工具再次從 /usr/local/ASR/Vx/bin/hostconfigcli 執行。 您必須藉由執行 chmod 做為根使用者，以設定存取權限。
...
4. 執行 **Microsoft-ASR_CX_TP_8.4.0.0_Windows*** 安裝檔案，然後遵循指示進行。 這樣可以安裝部署所需第三方廠商元件。
...
7. 在 [環境詳細資料]**** 頁面執行下列動作：
    ...
    - 如果您只想要保護實體伺服器，因此不需要處理序伺服器上安裝 VMware vCLI 時。 按一下 [否]**** 並繼續。
...
您可以在站台復原來取得更新 ** 儀表板 **。 針對 Linux 安裝，請從 Gzip 安裝程式解壓縮檔案，並執行命令 “sudo ./install” 來安裝更新。
...
- 下載服務的更新，如下所示：
    - [Windows Server (僅 64 位元)](http://download.microsoft.com/download/8/4/8/8487F25A-E7D9-4810-99E4-6C18DF13A6D3/Microsoft-ASR_UA_8.4.0.0_Windows_GA_28Jul2015_release.exe)
    - [CentOS 6.4,6.5,6.6 (僅 64 位元)](http://download.microsoft.com/download/7/E/D/7ED50614-1FE1-41F8-B4D2-25D73F623E9B/Microsoft-ASR_UA_8.4.0.0_RHEL6-64_GA_28Jul2015_release.tar.gz)
    - [Oracle Enterprise Linux 6.4、 6.5 限 (僅 64 位元)](http://download.microsoft.com/download/5/2/6/526AFE4B-7280-4DC6-B10B-BA3FD18B8091/Microsoft-ASR_UA_8.4.0.0_OL6-64_GA_28Jul2015_release.tar.gz)
    - [SUSE Linux Enterprise Server SP3 (僅 64 位元)](http://download.microsoft.com/download/B/4/2/B4229162-C25C-4DB2-AD40-D0AE90F92305/Microsoft-ASR_UA_8.4.0.0_SLES11-SP3-64_GA_28Jul2015_release.tar.gz)
...
1. 從上表所列的處理序伺服器目錄路徑將 **Microsoft-ASR_UA_8.4.0.0_Windows_GA_28Jul2015_release.exe** 封裝複製到來源機器。
...
1. 將複雜密碼從 CX 複製到伺服器上的 "C:\connection.passphrase" 檔案，並執行此命令。 在我們的範例中，CX 是 104.40.75.37 和 HTTPS 連接埠是 62519：
    ...
    `C:\Microsoft-ASR_UA_8.2.0.0_Windows_PREVIEW_20Mar2015_Release.exe 「 ip 104.40.75.37-連接埠 62519-模式 UA /LOG="C:\stdout.txt"/DIR ="C:\Program Files (x86) \Microsoft Azure 站台復原 」 / VERYSILENT /SUPPRESSMSGBOXES /norestart-usesysvolumes /CommunicationMode https /PassphrasePath"C:\connection.passphrase"`

**在 Linux 伺服器上手動安裝行動服務**：

1. 根據上表，將適當的 tar 封存檔從將處理序伺服器複製到來源機器。
2. 開啟殼層程式，並壓縮的 tar 封存檔解壓縮到本機路徑執行 `tar-xvzf-ASR_UA_8.2.0.0 *`
3. 您要解壓縮 tar 封存檔內容輸入本機目錄中建立 passphrase.txt 檔案 *`echo < 複雜密碼 >> passphrase.txt`* 從 shell。
4. 安裝行動服務，輸入 *`sudo。 /install-t 這兩種-a host-R 代理程式-d /usr/local/ASR-i < IP 位址 >-p < 連接埠 >-s y-c https-P passphrase.txt`*。
5. 指定 IP 位址和連接埠：

    - 如果您透過網際網路連接到設定伺服器指定的組態伺服器的虛擬公用 IP 位址和公用 HTTPS 端點，在 `< 位址 >` 和 `< 連接埠 >`。
...
4. 在 [指定目標資源]**** 中，選取要用於複寫主要目標伺服器和儲存體，並選取設定是否應該用於所有工作負載。 選取 [高階儲存體帳戶](../storage/storage-premium-storage-preview-portal.md) 時設定，若要裝載 IO 密集工作負載需要一致的 IO 高效能和低延遲工作負載的保護。 如果您希望針對您的工作負載磁碟使用進階儲存體帳戶，您必須使用 DS 系列的主要目標。 您無法搭配使用進階儲存體磁碟與非 DS 系列的主要目標。
...
[深入](site-recovery-failback-azure-to-vmware.md) 有關如何將失敗的 Azure 中執行的機器還原到您的內部部署環境。
