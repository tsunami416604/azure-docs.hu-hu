...
1. Kattintson a **Hozzáadás** gombra az engedélyek listához való hozzáadásához.
    ...
    Az engedély `Actions` vagy `DataActions`ként lesz hozzáadva.

    ![Engedély hozzáadva](./media/custom-roles-portal/permissions-list-add.png)

1. Az engedélyek eltávolításához kattintson a sor végén található Törlés ikonra. Ebben a példában, mivel a felhasználónak nincs szüksége támogatási jegyek létrehozására, a `Microsoft.Support/*` engedély törölhető.

### <a name="add-wildcard-permissions"></a>Helyettesítő karakteres engedélyek hozzáadása

Attól függően, hogy hogyan döntött, az engedélyek listájában szerepelhetnek helyettesítő karakterek (\*). A helyettesítő karakterek (\*) kiterjesztik az Ön által megadott karakterlánccal megegyező engedélyeket. Tegyük fel például, hogy hozzá kívánja adni a Azure Cost Management és az exportáláshoz kapcsolódó összes engedélyt. A következő engedélyeket adhatja hozzá:
...
Ha a szerepkör helyettesítő (\*) engedéllyel rendelkezik, és szeretné kizárni vagy kivonni a helyettesítő jogosultságokból származó konkrét engedélyeket, kizárhatja őket. Tegyük fel például, hogy a következő helyettesítő karakteres engedélyekkel rendelkezik:
...
Ha kizár egy engedélyt, `NotActions` vagy `NotDataActions`lesz hozzáadva. Az érvényes felügyeleti engedélyek kiszámítása az összes `Actions` hozzáadásával, majd az összes `NotActions`kivonásával történik. Az érvényes adatokra vonatkozó engedélyek kiszámítása az összes `DataActions` hozzáadásával, majd az összes `NotDataActions`kivonásával történik.
...
1. Miután megtalálta a kizárni kívánt engedélyeket, vegyen fel egy pipát az engedélyek mellett, majd kattintson a **Hozzáadás** gombra.
    ...
    Az engedély `NotActions` vagy `NotDataActions`lesz hozzáadva.
...
- [Oktatóanyag: egyéni szerepkör létrehozása Azure PowerShell használatával](tutorial-custom-role-powershell.md)
