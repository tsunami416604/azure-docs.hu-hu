...
>[AZURE.IMPORTANT] IntelliTrace 適用於偵錯，並不應該用於實際執行部署。

>[AZURE.NOTE] 您可以使用 IntelliTrace，如果您有安裝 Visual Studio Enterprise 與您 Azure 應用程式的目標.NET Framework 4 或更新版本。 IntelliTrace 會收集 Azure 角色的資訊。 這些角色的虛擬機器一律會執行 64 位元作業系統。
...
1. 在 [方案總管] 中開啟 Azure 專案的捷徑功能表並選擇 [ **發行**。
 
    [發佈 Azure 應用程式] 精靈隨即出現。

1. 若要發佈定域機組中時，收集您的應用程式的 IntelliTrace 記錄檔，請選取 **啟用 IntelliTrace** 核取方塊。

    >[AZURE.NOTE] 您可以啟用 IntelliTrace 或程式碼剖析，當您發行 Azure 應用程式。 您不能同時啟用。
...
您可以下載從角色執行個體的 IntelliTrace 記錄檔 **雲端服務** 節點 **伺服器總管**。 展開 **雲端服務** 節點，直到您找到您感興趣的執行個體開啟此執行個體的捷徑功能表，然後選擇 [ **檢視 IntelliTrace 記錄檔**。 IntelliTrace 記錄檔會下載到本機電腦的目錄中的檔案。 每當您要求 IntelliTrace 記錄檔，即會建立新的快照。
...
>[AZURE.NOTE] IntelliTrace 記錄檔可能包含架構產生及接續處理的例外狀況。 內部架構程式碼會在正常啟動角色時產生這些例外狀況，因此您可以放心忽略。
