...
Ez a cikk bemutatja, hogyan használhatja az Azure Portalt egy helyek közötti VPN-átjárókapcsolat létrehozására egy helyszíni hálózat és a Vnet között. A cikkben ismertetett lépések a klasszikus központi telepítési modellre vonatkoznak, és nem vonatkoznak az aktuális központi telepítési modellre, az Erőforrás-kezelőre. Ezt a konfigurációt más üzembehelyezési eszközzel vagy üzemi modellel is létrehozhatja, ha egy másik lehetőséget választ az alábbi listáról:
...
* A PowerShell szükséges a megosztott kulcs megadásához és a VPN-átjárókapcsolat létrehozásához. [!INCLUDE [vpn-gateway-classic-powershell](../../includes/vpn-gateway-powershell-classic-locally.md)]
...
2. Kattintson a **+Erőforrás létrehozása gombra.* A **Piactér keresése** mezőbe írja be a "Virtuális hálózat" kifejezést. Keresse meg a **virtuális hálózatot** a visszaadott listából, és kattintson ide a **Virtuális hálózat** lap megnyitásához.
3. kattintson **(váltás Klasszikusra),** majd kattintson a **Létrehozás gombra.**
4. A **Virtuális hálózat létrehozása (klasszikus)** lapon konfigurálja a virtuális hálózat beállításait. Ezen a lapon adhatja hozzá az első címterét és egy önálló alhálózati címtartományt. A virtuális hálózat létrehozása után visszatérhet, és további alhálózatokat és címtereket adhat hozzá.
...
## <a name="3-specify-a-dns-server"></a><a name="dns"></a>3. DNS-kiszolgáló megadása
...
1. A virtuális hálózat lap **Beállítások**területén kattintson a **Diagram gombra.**
1. A **VPN-kapcsolatok** lapon kattintson a **Nincs létező VPN-kapcsolat elemre. A kezdéshez kattintson ide.**
1. A **Kapcsolat típusa mezőben**hagyja a helyek közötti **helyet** kijelölve.
4. A **Helyi hely** lap megnyitásához kattintson a **Helyi hely – Kötelező beállítások megadása** elemre. Konfigurálja a beállításokat, majd kattintson az **OK** gombra a beállítások mentéséhez.
   ...
   ![Helyi kemping](./media/vpn-gateway-howto-site-to-site-classic-portal/localnetworksite.png "Helyi hely konfigurálása")

A Helyi webhely lap bezárásához kattintson az **OK** gombra. **Az Új VPN-kapcsolat lap bezárásához kattintson az OK gombra.**
...
3. Az **Átjáró konfigurálása** lapon kattintson az **Alhálózat – Kötelező beállítások konfigurálása** elemre az **Alhálózat hozzáadása** lap megnyitásához. Ha befejezte a beállítások konfigurálását, kattintson **az OK**gombra.

   ![Átjáró konfigurációja - átjáró alhálózata](./media/vpn-gateway-howto-site-to-site-classic-portal/subnetrequired.png "Átjáró konfigurációja - átjáró alhálózata")
...
## <a name="6-specify-the-sku-and-vpn-type"></a><a name="sku"></a>6. Adja meg a Termékváltozat és a VPN típusát
...
1. Válassza ki az átjáró **méretét**. Ez az átjáró a virtuális hálózati átjáró létrehozásához használt termékváltozata. A klasszikus VPN-átjárók a régi (örökölt) átjáró-termékváltozatokat használják. Az átjárók örökölt termékváltozatairól [a virtuális hálózati átjárók termékváltozatainak (régi termékváltozatok) használatát bemutató](vpn-gateway-about-skus-legacy.md) cikkben talál további információt.
   ...
   ![SKUL és VPN típus kiválasztása](./media/vpn-gateway-howto-site-to-site-classic-portal/sku.png "Termékváltozat és VPN-típus kiválasztása")
2. Válassza ki az átjáró **útválasztási típusát**. Ez VPN-típus néven is ismert. Fontos, hogy a megfelelő típust válassza ki, mert az átjáró nem konvertálható egyik típusból a másikba. A VPN-eszköznek kompatibilisnek kell lennie a kiválasztott útválasztási típussal. Az Útválasztás típusa című témakörben olvashat [bővebben: Vpn gateway settings](vpn-gateway-about-vpn-gateway-settings.md#vpntype). A „RouteBased” és „PolicyBased” VPN-típusokkal további cikkek is foglalkozhatnak. A „Dynamic” (Dinamikus) a „RouteBased”, a „Static” (Statikus) a „PolicyBased” típusra utal.
3. Kattintson az **OK** gombra a beállítások mentéséhez.
4. Az **Új VPN-kapcsolat** lapon kattintson az **OK** gombra a lap alján a virtuális hálózati átjáró telepítésének megkezdéséhez. A kiválasztott SKU függvényében egy virtuális hálózati átjáró létrehozása akár 45 percet is igénybe vehet.
...
## <a name="8-create-the-connection"></a><a name="CreateConnection"></a>8. Hozza létre a kapcsolatot
...
> [!NOTE]
> Ez a lépés jelenleg az Azure Portalon nem érhető el. A műveletekhez az Azure PowerShell-parancsmagjainak Service Management (SM) verzióját kell használnia. A parancsmagok telepítéséről a [Kezdés előtt](#before) című témakörben talál információt.
>

### <a name="step-1-connect-to-your-azure-account"></a>1. lépés Csatlakozás az Azure-fiókhoz

Ezeket a parancsokat helyileg kell futtatnia a PowerShell szolgáltatásfelügyeleti modul használatával. 
...
Amikor egy klasszikus virtuális hálózatot hoz létre a portálon (nem használja a PowerShellt), az Azure hozzáadja az erőforráscsoport nevét a rövid névhez. Az Azure szerint például a feladathoz létrehozott virtuális hálózat neve "Group TestRG1 TestVNet1", nem "TestVNet1". A PowerShell a virtuális hálózat teljes nevét igényli, nem a portálon megjelenő rövid nevet. A hosszú név nem látható a portálon. A következő lépések segítségével exportálhatja a hálózati konfigurációs fájlt a virtuális hálózat nevének pontos értékeinek beszerzése érdekében. 
...
1. Hozzon létre egy könyvtárat a számítógépén, majd exportálja a hálózati konfigurációs fájlt a könyvtárba. Ebben a példában a hálózati konfigurációs fájlt a C:\AzureNet helyre exportálja.
   ...
   ```
2. Nyissa meg a hálózati konfigurációs fájlt egy xml-szerkesztővel, majd ellenőrizze a „LocalNetworkSite name” és a „VirtualNetworkSite name” értékeit. Módosítsa a gyakorlat példáját úgy, hogy az tükrözze az xml értékeit. Amikor szóközt tartalmazó nevet ad meg, zárja az értéket szimpla idézőjelek közé.
...
## <a name="9-verify-your-connection"></a><a name="verify"></a>9. A kapcsolat ellenőrzése
