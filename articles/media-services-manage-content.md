...
1.  In the [Management Portal][Management Portal], click **Media Services** and then click on the Media Services account name.
...
4.  In the Azure Media Encoder dialog, choose from one of the common or advanced encoding presets.
    ...
    **Common Presets**
    ...
    -   **Encode with PlayReady content protection**. This preset produces an asset encoded with PlayReady content protection. For this preset to work, you need to enable PlayReady license delivery service. To do this, go to the **CONTENT PROTECTION** tab and add a row to the Branding Reporting table. The Media Services PlayReady license service will be enabled a few minutes after you press SAVE.
        ...
        By default the Media Services PlayReady license service is used. To specify some other service from which clients can obtain a license to play the PlayReady encrypted content, use REST or Media Services .NET SDK APIs. For more information, see [Using Static Encryption to Protect your Content][Using Static Encryption to Protect your Content] and set the **licenseAcquisitionUrl** property in the Media Encryptor preset. Alternatively, you can use dynamic encryption and set the **PlayReadyLicenseAcquisitionUrl** property as described in [Using PlayReady Dynamic Encryption and License Delivery Service][Using PlayReady Dynamic Encryption and License Delivery Service].

        Note that this option will only show up if you are signed up for the PlayReady content protection preview feature. To sign up for preview features, you must go through the process described on the following page: [Microsoft Azure Preview Features][Microsoft Azure Preview Features].

    **Advanced Presets**

    -   The [Task Preset Strings for Azure Media Encoder][Task Preset Strings for Azure Media Encoder] topic explains what each prest in the Advanced Presets list means.

    ![EncoderDialog][EncoderDialog]

    Currently, the portal does not support all the encoding formats that are supported by the Media Encoder. It also does not support media asset encryption\\decryption. You can perform these tasks programmatically, for more information see [Building Applications with the Media Services SDK for .NET][Building Applications with the Media Services SDK for .NET] and [Task Preset Strings for Azure Media Encoder][Task Preset Strings for Azure Media Encoder].
...
<!-- Images -->
  ...
  [Management Portal]: http://go.microsoft.com/fwlink/?LinkID=256666&clcid=0x409
  ...
  [Using Static Encryption to Protect your Content]: 
  [Using PlayReady Dynamic Encryption and License Delivery Service]: http://go.microsoft.com/fwlink/?LinkId=507720
  [Microsoft Azure Preview Features]: http://azure.microsoft.com/en-us/services/preview/
  [Task Preset Strings for Azure Media Encoder]: http://go.microsoft.com/fwlink/?LinkId=270865
  [EncoderDialog]: ./media/media-services-manage-content/EncoderDialog2.png
  [Building Applications with the Media Services SDK for .NET]: http://go.microsoft.com/fwlink/?LinkId=270866
