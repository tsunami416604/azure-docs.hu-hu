...
<tags ms.service="virtual-machines" ms.devlang="multiple" ms.topic="article" ms.tgt_pltfrm="vm-linux" ms.workload="infrastructure-services" ms.date="10/21/2014" ms.author="rasquill"></tags>

# Using the Docker VM Extension from Azure Cross-Platform Interface (xplat-cli)

This topic describes how to create a VM with the Docker VM Extension from the xplat-cli on any platform. [Docker][Docker] is one of the most popular virtualization approaches that uses [Linux containers][Linux containers] rather than virtual machines as a way of isolating data and computing on shared resources. You can use the Docker VM extension to the [Azure Linux Agent][Azure Linux Agent] to create a Docker VM that hosts any number of containers for your applications on Azure. To see a high-level discussion of containers and their advantages, see the [Docker High Level Whiteboard][Docker High Level Whiteboard].
...
Follow the [Docker installation instructions][Docker installation instructions] to install Docker locally on your computer. For most operating systems and distributions, this means typing `apt-get install docker.io`. Confirm that the Docker version is at 1.0 or greater.

To use Docker with an Azure Virtual Machine, the Linux image used for the VM must have the [Azure Linux VM Agent][Azure Linux Agent] installed. Currently, there are only two types of images that provide this:

-   An Ubuntu image from the Azure Image Gallery or

-   A custom Linux image that you have created with the Azure Linux VM Agent installed and configured. See [Azure Linux VM Agent][Azure Linux Agent] for more information about how to build a custom Linux VM with the Azure VM Agent.
...
You are ready to go to the [Docker User Guide][Docker User Guide] and use your Docker VM. To create a Docker-enabled VM in the new portal, see [How to use the Docker VM Extension with the Portal][How to use the Docker VM Extension with the Portal].

<!--Anchors--> <!--Image references--> <!--Link references-->

  [Docker]: https://www.docker.com/
  [Linux containers]: http://en.wikipedia.org/wiki/LXC
  ...
  [Container and Container Management Resources for Azure]: #Container-and-Container-Management-Resources-for-Azure
  [Next steps]: #next-steps
  [Azure Cross-Platform Command-Line Interface]: https://github.com/Azure/azure-sdk-tools-xplat
  [Boot2Docker]: https://docs.docker.com/installation/windows/
  [How to install the Azure Cross-Platform Command-Line Interface]: http://azure.microsoft.com/en-us/documentation/articles/xplat-cli/#install
  [here]: https://www.digitalocean.com/community/tutorials/how-to-install-node-js-on-an-ubuntu-14-04-server
  [How to connect to your Azure subscription]: http://azure.microsoft.com/en-us/documentation/articles/xplat-cli/#configure
