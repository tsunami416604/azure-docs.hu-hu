...
A regisztráció során az Azure-szolgáltatások felügyeletéhez öt különböző vállalati rendszergazda felhasználói szerepkör létezik:
...
Mindegyik fiókhoz szükség van egyedi munkahelyi, iskolai vagy Microsoft-fiókra. Az Azure EA Portal rendszergazdai szerepköreivel kapcsolatos további információkért lásd [az Azure Nagyvállalati Szerződés Azure-beli felügyeleti szerepköreinek ismertetését](understand-ea-roles.md).
...
### <a name="notification-contact"></a>Értesítési kapcsolattartó

Az értesítési partner a beléptetéshez kapcsolódó használati értesítéseket fogad.
...
Ha rendszergazdaként lett beállítva, nem kell megkapnia az aktiválási e-mailt, hogy bejelentkezzen az Azure EA portálra. Folytathatja [https://ea.azure.com](https://ea.azure.com) és bejelentkezhet az e-mail-címével (munkahelyi, iskolai vagy élő azonosítóval) és jelszavával.
...
![A részlegszintű rendszergazda hozzáadására szolgáló párbeszédablakot bemutató példa](./media/ea-portal-get-started/ea-create-add-department-admin.png)
...
### <a name="azure-in-open"></a>Az Open mennyiségi licencprogramokon keresztül vásárolt Azure-előfizetések

Az Open mennyiségi licencprogramokon keresztül vásárolt Azure-előfizetések nagyvállalati szerződéssel való társításával az el nem használt kreditek elvesznek. A kreditek esetleges elvesztésének elkerülése érdekében javasoljuk, hogy használja el az Open mennyiségi licencprogramokon keresztül vásárolt Azure-előfizetésekhez tartozó összes kreditet, mielőtt nagyvállalati szerződéshez adná hozzá a fiókját.  
...
Töltse le a használati adatokról szóló jelentést. Kattintson a **jelentések** elemre, majd kattintson a **Letöltés kihasználtsága** fülre. A jelentések listájában kattintson a **Letöltés** gombra a lekérdezni kívánt havi jelentéshez.
...
1. Az Azure EA Portal bal oldali navigációs területén kattintson a **Jelentések** elemre, és tekintse meg a **Használati adatok összegzése** lapot.  
  ![Használati adatok összegzésének létrehozása és megtekintése, illetve jelentések letöltése](./media/ea-portal-get-started/create-ea-view-usage-summary-and-download-reports.png)
...
- **Fiók**: az Azure EA portálon az előfizetések felügyeletére és a jelentéskészítéshez használt szervezeti egység.
- **Fiók tulajdonosa**: az előfizetések és a szolgáltatás-rendszergazdák Microsoft Azureon való kezeléséhez azonosított személy. Megtekinthetik ezen fiók és a társított előfizetéseinek használati adatait.
- **Módosítási előfizetés**: egy éves vagy pozíciókból eredő összevonási előfizetés a beléptetési módosítás alatt.
- **Kötelezettségvállalás**: a Microsoft Azure-szolgáltatások éves pénzügyi összegének kötelezettségvállalása kedvezményes kötelezettségvállalási arányban az előfizetéssel való használatra.
- **Részleg rendszergazdája**: a részlegek kezeléséhez azonosított személy (ek), új fiókok és fiókok tulajdonosai, az általuk felügyelt részlegek használati adatainak megtekintése, valamint a költségek megtekintése az engedélyek megadásakor.
- **Beléptetési szám**: a Microsoft által a nagyvállalati szerződéshez társított adott regisztráció azonosítására megadott egyedi azonosító.
- **Vállalati rendszergazda**: a Microsoft Azure-on lévő részlegek és részlegek tulajdonosai és fiókjai, valamint a fiókok tulajdonosai kezelésére kijelölt személy (ek). Képesek kezelni a vállalati rendszergazdákat, valamint megtekinthetik a használati adatokat, a számlázott mennyiségeket és a nem számlázott díjakat a nagyvállalati regisztrációhoz társított összes fiókra és előfizetésre vonatkozóan.
- **Nagyvállalati szerződés**: Microsoft-licencszerződés olyan ügyfelek számára, akik központosított megvásárlással szeretnék szabványosítani a teljes szervezetét a Microsoft-technológiában, és az informatikai infrastruktúrát egy standard szintű Microsoft-szoftverhez kell fenntartani.
- **Nagyvállalati szerződés beléptetése**: a nagyvállalati szerződés keretében a Microsoft termékeit kedvezményes díjszabással biztosító beléptetéssel biztosítjuk.
- **Microsoft-fiók**: egy webalapú szolgáltatás, amely lehetővé teszi a résztvevő helyek számára, hogy egyetlen hitelesítő adatokkal hitelesítsék a felhasználókat.
- **Microsoft Azure vállalati beléptetési módosítás (beléptetési módosítás)** : egy vállalat által aláírt módosítás, amely hozzáférést biztosít számukra a Microsoft Azure számára a vállalati beléptetés részeként.
...
- **Felhasznált erőforrás mennyisége**: egy adott Microsoft Azure-szolgáltatás egy hónapjában használt mennyisége.
- **Szolgáltatás-rendszergazda**: az Azure EA Portalon az előfizetések és fejlesztési projektek eléréséhez és kezeléséhez azonosított személy.
- **Előfizetés**: egy Azure EA Portal-előfizetést jelöl, és az ugyanazon szolgáltatás-rendszergazda által kezelt Microsoft Azure szolgáltatások tárolója.
- **Munkahelyi vagy iskolai fiók**: azoknak a szervezeteknek, amelyek az Active Directoryt beállították a felhőhöz, és az összes fiók egyetlen bérlőn található.

### <a name="enrollment-statuses"></a>Regisztrációs állapotok:

- **Függőben**: a beléptetési rendszergazdának be kell jelentkeznie az Azure EA portálra. Ha bejelentkezett, a regisztráció aktív állapotra vált.
- **Aktív**: a regisztráció aktív, a fiókok és az előfizetések az Azure EA portálon hozhatók létre. A regisztráció a nagyvállalati szerződés záró dátumáig aktív marad.
- **Határozatlan időtartamú kiterjesztett kifejezés**: a nagyvállalati szerződés befejezési dátumának lejárta után határozatlan kiterjesztett időszakra kerül sor. Lehetővé teszi a meghosszabbított időszakot választó, nagyvállalati szerződéssel rendelkező ügyfelek számára, hogy a nagyvállalati szerződésük végén továbbra is határozatlan ideig használják az Azure-t. Mielőtt az EA-regisztráció elérné a nagyvállalati szerződés záró dátumát, a regisztrációs rendszergazdának el kell döntenie, hogy megújítja- a regisztrációt további pénzügyi kötelezettségvállalás hozzáadásával, új regisztrációra vált, a Microsoft Online Előfizetési Programra (MOSP) migrál, vagy megerősíti a regisztrációval társított összes szolgáltatás letiltását.
- **Lejárt**: a nagyvállalati szerződéssel rendelkező ügyfél kikerül a meghosszabbított időszakból, és az EA-regisztráció elérte a nagyvállalati szerződés befejezési dátumát, a regisztráció lejár, és az összes kapcsolódó szolgáltatás le lesz tiltva.
- **Átvitel**: azok a regisztrációk, amelyekben az összes társított fiók és szolgáltatás át lett adva egy új regisztrációra, egy átvitt állapottal fog megjelenni. Vegye figyelembe, hogy a regisztrációk nem kerülnek át automatikusan, ha a megújításkor új regisztrációs szám jön létre. Az előző regisztrációs számnak szerepelnie kell az ügyfél megújítási dokumentumaiban az automatikus átvitel megkönnyítése érdekében.
...
- Enterprise Dev/Test – a következőkhöz ideális:
  - Minden fejlesztési/tesztelési munkaterhelés
  - Közepes és nagyobb egyszeri fejlesztési/tesztelési munkaterhelések
