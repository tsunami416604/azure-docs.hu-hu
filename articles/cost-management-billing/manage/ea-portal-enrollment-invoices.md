...
**Közvetlen regisztrációk a 2018. május 1. előtti kezdési dátummal**: a közvetlen nagyvállalati Azure-(EA-) ügyfelek az Azure-szolgáltatások (kivéve a Piactéri szolgáltatásokat) éves számlázási ciklusként lesznek beállítva. A számlázási ciklus az évforduló dátumán alapul. Az évforduló dátuma az a dátum, amikor a szerződés hatályba lépett. Az Azure-szolgáltatások túlhasználatáról szóló első számla kiállításához meg kell haladnia a pénzügyi kötelezettségvállalás („MC”) küszöbértékének 150%-át.  Amint a teljes szolgáltatáshasználat meghaladja az MC-küszöbérték 150%-át, automatikusan negyedéves számlázási ciklusban kapja meg a számlákat az évforduló dátuma alapján.  Ha nem haladja meg az MC-küszöbérték 150%-át, a regisztráció az egyéves számlázási ciklusban marad, és a túlhasználatról szóló számlát a kötelezettségvállalási év végén kapja meg.

A **közvetlen regisztrációk a 2018. május 1. után kezdődnek**: az ügyfél Azure-beli fogyasztása és a külön számlázott díjak havi számlázási időszakon belül lesznek.  Az Azure-ra vonatkozó pénzügyi kötelezettségvállalás által nem fedezett díjak túlhasználati díjként lesznek esedékesek.  

Olyan **közvetett regisztrációk, amelyek beléptetése a 2018. május 1. előtt kezdődött**: a közvetett nagyvállalati Azure-(EA-) ügyfelek negyedéves számlázási ciklusként vannak beállítva.  A csatornapartner (CP) közvetlenül az ügyfélnek állítja ki a számlát.  

**A közvetett regisztráció a 2018. május 1. előtti kezdési dátummal**: a 2018-as vagy annál nagyobb kezdési dátummal rendelkező közvetett szerződések számlázása havonta történik.  
...
A 2019. áprilisi számlázási ciklustól kezdve az ügyfelek egyetlen Azure-számlát fognak kapni, mert az Azure és az Azure Marketplace díjainak mindegyikét egyetlen számlán egyesítettük, két külön számla kiküldése helyett. (Ez a változás nem érinti az ausztráliai, Japán vagy szingapúri ügyfeleket.) A konszolidált számlára való áttérés során egy részleges Piactéri számlát fog kapni. Ez az utolsó külön számla a számlázás frissítésének dátuma előtti Marketplace-díjakat tartalmazza. A frissítés dátumát követő Marketplace-díjak az Azure-számlán lesznek láthatók. Az áttérési időszak után az Azure- és Marketplace-díjak mindegyikét az összevont számlán fogja látni.  
...
1. Ebben a sorban láthatja a regisztrációhoz alkalmazott krediteket, valamint egy rövid magyarázatot, például: szolgáltatói szerződés kreditet.
...
Az Excel az összes mezőt "általános" szövegként importálja, és feltételezi, hogy egy szám a matematikai standardban van elválasztva: "1 000,00".  Így az ezresek elválasztásához pontot (.) és a tizedesjegyek elválasztásához vesszőt (,) használó európai pénznemek esetén az „1.000,00” helytelenül fog megjelenni. Ez a területi nyelvi beállításoktól függően változhat.
...
- **Használat-alapú:** Tranzakciós egységekben mért termékek.  A virtuális gépek például óradíjjal járnak, míg a Bing API-keresések a keresések száma alapján vannak felszámítva.
...
### <a name="enterprise-msdn-devtest"></a>Nagyvállalati Szerződés – MSDN-alapú fejlesztés és tesztelés
...
| Alkalmazási helyzet | Bankár logikai képlet |
| --- | --- |
...
Az új mérés a 2016. január 27. és január 28. között lép érvénybe. Ebben az átmeneti időszakban az üzemelő példányhoz tartozó. csv fájlban két sor elemet láthat: az egyiket az A1-es fogyasztásmérőn, egy másikat pedig az üzembe helyezés méretének megfelelő új dedikált mérőszámon kiváltott használatra.
...
| Geo (Térség) | Adatátviteli régiók | CDN-régiók |
| --- | --- | --- |
...
- **Használat alapja:** A termékeket tranzakciós egységekben mérjük.  A virtuális gépek például óradíjjal járnak, míg a Bing API-keresések a keresések száma alapján vannak felszámítva.
- **Nem használat alapú:** Egyszeri díj vagy ismétlődő havi díj, amely független a használattól.
...
- **Általános célú**: a "Microsoft Azure Enterprise" csomag próbaverziós ajánlatának kiterjesztett Power bi Pro (az "ajánlat") az ajánlat időtartama alatt elérhetők a meglévő minősített felhasználók számára, hogy az adott Power bi Content Pack használatával hozzáférjenek az Microsoft Azure-felhasználással kapcsolatos bizonyos információhoz.
- **Jogosultság**: NAGYVÁLLALATI szerződés (EA) alá tartozó felhasználók részt vehetnek az ajánlatban abban az esetben, ha a szervezete Microsoft Azure számlázási, szolgáltatás-, szolgáltatás-és/vagy költséghatékonysági funkciójuk kapcsolódik.
- **Kizárások**:
  ...
  - Kifejezés: az ajánlat 2017 június 1-én kezdődik, és a 2018. május 31-ig tart.  Az elfogadás a 12 hónapos időszak alatt bármikor megtörténhet, bár az ajánlat 2018. május 31-én megszűnik az összes felhasználó számára, függetlenül attól, hogy mikor fogadták el az Ajánlatot.
...
**Összes nyers egység:** A részletes használati jelentésen belül felhasznált mennyiség.
**MOCP erőforrás egységenként:** A felsőbb rétegbeli használati rendszer különböző egységekben bocsátja ki az egyes szolgáltatások használatát. Készlet Egységenkénti **felhasználás:** EA mértékegység. Készlet **Ár:** Az Azure EA portál egységének díja.
**Teljes díj:** A részletes használati jelentésből vagy az Azure nagyvállalati szerződésből származó, az Azure nagyvállalati Szerződéssel való használatának kibővített díja.
