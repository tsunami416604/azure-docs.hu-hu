...
> [AZURE.NOTE] 這項功能只會擷取針對 BizTalk 服務的管理作業，例如啟動服務時，記錄檔備份，依此類推。 這類作業無論執行 Azure 傳統入口網站，或使用追蹤 [BizTalk 服務 REST Api](http://msdn.microsoft.com/library/azure/dn232347.aspx)。 如需使用管理服務進行追蹤作業的完整清單，請參閱 [作業使用 Azure 管理服務](#bizops)。<br/><br/>
...
3. 選取核取記號以檢視篩選清單。 下圖顯示 testbiztalkservice 的相關活動:
    ![View operation logs][ViewLogs] 
4. 若要檢視更特定的作業，請選取資料列，並按一下 [ **詳細資料** 在底部的工作列。
