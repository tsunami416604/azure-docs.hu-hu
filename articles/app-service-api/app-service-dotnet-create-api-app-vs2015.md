...
在本教學課程中您將建立 ASP.NET Web API 2 專案使用 [Visual Studio 2015 RC](https://www.visualstudio.com/en-us/downloads/visual-studio-2015-downloads-vs.aspx), ，並設定它以部署至雲端 [API 應用程式](app-service-api-apps-why-best-platform.md) 中 [Azure App Service](../app-service/app-service-value-prop-what-is.md)。 您也會將專案部署至 Azure。 在本教學課程的結尾，您將會在 Azure 雲端中執行 API 應用程式。
...
1. 執行應用程式 (ctrl-f5)，並新增 `/swagger` 瀏覽器的網址列中的 URL 的結尾。
...
5. 請回到 Visual Studio 中的專案，並將下列程式碼新增至 **ContactsController.cs** 檔案。 這個程式碼加入 **張貼** 方法，可用來張貼新 `連絡人` api 的執行個體。
