...
## 加入顯示名稱 ##
Logic Apps 設計工具會顯示作業、欄位和參數的名稱，但有時很難讀取透過程式設計方式產生的名稱。 若要改善可讀性，邏輯應用程式的設計工具可以，其中有的話，會顯示更容易閱讀的文字值-稱為 *顯示名稱* -而非作業、 欄位和參數的預設名稱。 為了完成這項作業，Logic Apps 設計工具會掃描 API 應用程式所提供的 swagger 中繼資料中是否有特定屬性。  下列屬性用做顯示名稱：

* 作業 （動作和觸發程序）  
  值 **摘要** 有存在，否則屬性的值 **operationId** 屬性。 請注意，Swagger 2.0 規格允許最多有 120 個字元 **摘要** 屬性。

* 參數 （輸入）  
  值 **x ms 摘要** 如果存在，否則為延伸模組屬性的值 **名稱** 屬性。  **x ms 摘要** 延伸模組屬性必須以動態方式設定程式碼中。 本主題的「使用自訂屬性加上擴充功能屬性的註解」小節會描述該程序。  **名稱** 屬性可以使用 / / 註解設定。 本主題的「在 API 定義產生中使用 XML 註解」小節會描述該程序。

* 結構描述欄位 （輸出回應）  
  值 **x ms 摘要** 如果存在，否則為延伸模組屬性的值 **名稱** 屬性。  **x ms 摘要** 延伸模組屬性必須以動態方式設定程式碼中。 本主題的「使用自訂屬性加上擴充功能屬性的註解」小節會描述該程序。  **名稱** 屬性可以使用 / / 註解設定。 本主題的「在 API 定義產生中使用 XML 註解」小節會描述該程序。

**注意 ︰** 是建議的顯示名稱的長度保持為 30 個字元或更少。
...
2. 從 **方案總管] 中**, ，以滑鼠右鍵按一下專案，然後選取 **屬性**。

    ![專案屬性](./media/app-service-api-optimize-for-logic-apps/project-properties.png)

3. 專案的屬性頁面出現時，請執行下列步驟：

    - 選取 **組態** 設定將套用的。 通常，您將選取 [所有組態]，讓您指定的設定同時套用至 [偵錯] 和 [發行] 組建。
    
    - 選取 **建置** 左邊的索引標籤
    
    - 確認 **XML 文件檔** 核取選項。 Visual Studio 將根據專案名稱來提供預設檔案名稱。 您可以將其值設為命名慣例所需的內容，或將它保持原狀。

    ![設定 XML 文件屬性](./media/app-service-api-optimize-for-logic-apps/xml-documentation-file-property.png)

4. 開啟 *SwaggerConfig.cs* 檔案 (位於專案的 **App_Start** 資料夾)。

5. 新增 **使用** 指示詞加入頂端 *SwaggerConfig.cs* 檔案 **系統** 和 **System.Globalization** 命名空間。

        using System;
        using System.Globalization;
 
6. 搜尋 *SwaggerConfig.cs* 檔案的呼叫 **GetXmlCommentsPath**, ，並取消該行的註解，使其執行。 尚未實作這個方法，因為 Visual Studio 將會加上底線的呼叫 **GetXmlCommentsPath** 並指出它未定義在目前內容中。 沒有問題。 您將在下一個步驟中進行實作。

7. 將下列實作 **GetXmlCommentsPath** 方法 **SwaggerConfig** 類別 (定義在 *SwaggerConfig.cs* 檔案)。 這種方法只會傳回您稍早在專案設定中指定的 XML 文件檔案。
...
若要提供這項功能，Logic Apps 設計工具會尋找 API 應用程式的 swagger API 定義中是否有特定自訂廠商擴充功能屬性。 此屬性的名稱是 **x ms 可見度** 而且可以有下列值 ︰
...
* 「 內部 」  
  這些作業和屬性會視為系統或內部屬性，並不適合直接由使用者。  因此，設計工具會隱藏它們，而且只能在程式碼檢視中使用。  對於這類屬性，您也可以指定 **x ms 排程器-建議** 擴充功能屬性來設定透過 Logic Apps 設計工具的值。  如需範例，請參閱文件上 [API 應用程式中加入觸發程序](app-service-api-dotnet-triggers.md)。


## 使用自訂屬性加上擴充功能屬性的註解

如上所述，自訂廠商擴充功能屬性是用來加上 API 中繼資料的註解，以提供 Logic Apps 設計工具可以使用的更多資訊。  如果您使用靜態中繼資料來描述 API 應用程式時，您可以直接編輯 */metadata/apiDefinition.swagger.json* 手動加入必要的擴充功能屬性的專案中。

對於使用動態中繼資料的 API 應用程式，您可以使用自訂屬性來加上程式碼的註解。  接著，您可以定義作業篩選中的 *SwaggerConfig.cs* 來尋找自訂屬性以及加入必要的廠商延伸模組的檔案。  以動態產生以下詳細說明這種方法 **x ms 摘要** 延伸模組屬性。

1. 定義屬性類別稱為 **CustomSummaryAttribute** ，將會用來標註程式碼。
...
2. 定義作業篩選呼叫 **AddCustomSummaryFilter** 尋找這個自訂屬性中的作業參數。
...
3. 編輯 *SwaggerConfig.cs* 檔案，並加入上面定義的篩選器類別。
...
4. 使用 **CustomSummaryAttribute** 類別加上註解程式碼，如下列程式碼片段所示。
...
5. 同樣地，您可以定義結構描述篩選 **AddCustomSummarySchemaFilter** 自動加上註解 **x ms 摘要** 擴充功能屬性的結構描述模型，如下列範例所示。
