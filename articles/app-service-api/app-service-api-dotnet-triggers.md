...
例如，假設用戶端程式碼使用 [Twitter 連接器 API 應用程式](../app-service-logic/app-service-logic-connector-twitter.md) ，您的程式碼必須執行的動作，根據包含特定文字的新推文。 在此情況下，您可以設定輪詢或推入觸發程序來加速處理這項需求。
...
- 要求
    ...
    - 參數
        ...
        - API 特有的參數
- Response
    - 狀態碼 **200** -要求有效而且沒有觸發程序的通知。 通知的內容成為回應主體。 回應中的 "Retry-After" 標頭會指出，必須透過後續要求呼叫擷取其他通知資料。
    - 狀態碼 **202** -要求有效，但沒有新通知的觸發程序。
...
1. 部署 API 應用程式的驗證設定 **公用匿名**。
2. 呼叫 **觸控** 作業以接觸檔案。 下圖顯示透過 Postman 的範例要求。
   ![透過 Postman 呼叫接觸作業](./media/app-service-api-dotnet-triggers/calltouchfilefrompostman.PNG)
3. 呼叫輪詢觸發程序與 **triggerState** 參數設定為在步驟 2 之前的時間戳記。 下圖顯示透過 Postman 的範例要求。
...
3. 呼叫推入觸發程序以 GUID 為 **triggerId** 和 RequestBin URL 為 **callbackUrl**。
   ![透過 Postman 呼叫推入觸發程序](./media/app-service-api-dotnet-triggers/callpushtriggerfrompostman.PNG)
4. 呼叫 **觸控** 作業以接觸檔案。 下圖顯示透過 Postman 的範例要求。
   ![透過 Postman 呼叫接觸作業](./media/app-service-api-dotnet-triggers/calltouchfilefrompostman.PNG)
...
實作觸發程序，並將 API 應用程式部署至 Azure 之後, 瀏覽至 **API 定義** 刀鋒視窗中的 Azure 預覽入口網站，您會看到在 UI 中，由 API 應用程式的 Swagger 2.0 API 定義所驅動自動辨識觸發程序。
...
延伸模組屬性 **x-ms-schedular-觸發程序** 是觸發程序所述 API 定義，而且將自動新增 API 應用程式閘道當您要求透過閘道器的 API 定義，如果下列條件的其中一個要求。 (您也可以手動加入這個屬性。)

- 輪詢觸發程序
    - 如果 HTTP 方法 **取得**。
    - 如果 **operationId** 屬性包含字串 **觸發程序**。
    - 如果 **參數** 屬性所包含參數 **名稱** 屬性設定為 **triggerState**。
- 推入觸發程序
    - 如果 HTTP 方法 **放**。
    - 如果 **operationId** 屬性包含字串 **觸發程序**。
    - 如果 **參數** 屬性所包含參數 **名稱** 屬性設定為 **triggerId**。
...
比方說， **triggerState** 輪詢觸發程序的參數應該設定為邏輯應用程式中的下列運算式。 此運算式應該評估邏輯應用程式之觸發程序的最後一個叫用，並傳回該值。  
...
推入觸發程序， **triggerId** 參數必須唯一識別的邏輯應用程式。 建議的最佳作法是使用下列運算式，將此屬性設定為工作流程的名稱：

    @workflow().name

使用 **x ms 排程器-建議** 和 **x ms 可見度** 其 API 定義中的延伸模組屬性，API 應用程式可以傳達給邏輯應用程式設計工具，來自動設定這個使用者的運算式。
