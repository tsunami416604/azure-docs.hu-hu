...
不過，將驗證標頭加入您需要存取 `HttpRequestMessage` 物件，而且您沒有其。 若要取得要求的存取權和新增標頭，您必須建立 `DelegatingHandler` 類別，並在它的執行個體產生的用戶端的建構函式中傳遞。
...
1. 在專案中加入名為 *InternalCredentialHandler.cs* 的類別檔案，並以下列程式碼取代範本程式碼。
 ...
 此程式碼呼叫 `SignHttpRequest` 將驗證標頭新增至每個產生的用戶端類別所傳送的要求:
...
5. 在瀏覽器網址列中，加入 `/swagger` 結尾的 URL，並按下 Enter 鍵。
...
App Service SDK 取決於 Swagger API 定義來產生用戶端類別。 如果您想要呼叫尚未實作 Swagger API 定義的 API 應用程式，您可以使用 `HttpClient`。 在此情況下，您仍然使用 `SignHttpRequest` 方法，但您在上直接呼叫它 `HttpRequestMessage` 物件。

1. 在 *ContactNamesController.cs*, ，新增下列程式碼之前 `傳回的名稱;` 陳述式中的 `取得` 方法。
    ...
    此程式碼會傳遞至傳入的要求物件 `SignHttpRequest` 方法，以簽署傳出要求物件:
