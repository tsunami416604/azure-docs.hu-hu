...
下列步驟示範如何使用自訂 Swashbuckle *SwaggerConfig.cs* ，Visual Studio API 應用程式預覽專案範本包含在專案中的檔案。  您也可以在您設定以供部署為 API 應用程式的 Web API 專案中自訂 Swashbuckle。
...
2. 在 *app_start\ swaggerconfig.cs* 檔案中，呼叫 `OperationFilter` 方法，讓 Swashbuckle 使用新 `IOperationFilter` 實作。

        c.OperationFilter<MultipleOperationsWithSameVerbFilter>();

    ![](./media/app-service-api-dotnet-swashbuckle-customize/usefilter.png)

     *SwaggerConfig.cs* Swashbuckle NuGet 封裝所置放的檔案包含許多擴充點註解排除範例。 此處未顯示其他註解。 
...
1. 新增指示 *SwaggerConfig.cs* 檔案，以引導 Swashbuckle 使用 XML 文件檔案。

    * 開啟 *SwaggerConfig.cs* 上建立一個方法 *SwaggerConfig* 類別，以指定的 XML 文件的檔案路徑。 
    ...
    * 向下捲動 *SwaggerConfig.cs* 檔案，直到您看到類似以下螢幕擷取畫面的程式碼的註解一行。 
...
此示範的程式碼位於 [此 GitHub 存放庫](https://github.com/Azure-Samples/API-Apps-DotNet-Swashbuckle-Customization-MultipleResponseCodes)。 隨 Web API 專案以 XML 文件註解標記的是主控台應用程式專案，其包含針對此 API 產生的用戶端。 

### 使用 SwaggerResponse 屬性自訂預期的回應碼

 [SwaggerResponse](https://github.com/domaindrivendev/Swashbuckle/blob/master/Swashbuckle.Core/Swagger/Annotations/SwaggerResponseAttribute.cs) 屬性是可在 Swashbuckle 5.1.5 和更新版本。 為了避免您在專案中使用較舊的版本，本章節將由說明如何更新 Swashbuckle NuGet 封裝開始，讓您可以使用這個屬性。

1. 在 **方案總管] 中**, ，以滑鼠右鍵按一下您的 Web API 專案，然後按一下 **管理 NuGet 封裝**。 

    ![](./media/app-service-api-dotnet-swashbuckle-customize/manage-nuget-packages.png)

1. 按一下 [ *更新* 旁 *Swashbuckle* NuGet 封裝。 

    ![](./media/app-service-api-dotnet-swashbuckle-customize/update-nuget-dialog.png)

1. 新增 *SwaggerResponse* 屬性到您要指定有效的 HTTP 回應碼的 Web API 動作方法。 
...
1. 瀏覽至 */swagger/docs/v1* 專案及各種不同的 HTTP 回應碼的 URL 將會顯示在 Swagger JSON 中。 

    ![](./media/app-service-api-dotnet-swashbuckle-customize/multiple-responses-post-attributes.png)

此示範的程式碼位於 [此 GitHub 存放庫](https://github.com/Azure-Samples/API-Apps-DotNet-Swashbuckle-Customization-MultipleResponseCodes-With-Attributes)。 隨 Web API 專案以裝飾 *SwaggerResponse* 屬性是包含此 api 產生的用戶端的主控台應用程式專案。 
