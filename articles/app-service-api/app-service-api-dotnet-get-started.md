...
* Azure-fiók – [Nyisson ingyenes Azure-fiókot](/pricing/free-trial/?WT.mc_id=A261C142F) vagy [használja ki a Visual Studio-előfizetők számára elérhető előnyöket](/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F).
    ...
    Ha az Azure App Service-t az Azure-fiók regisztrálása előtt szeretné kipróbálni, ugorjon [Az Azure App Service kipróbálása](http://go.microsoft.com/fwlink/?LinkId=523751) oldalra. Itt azonnal létrehozhat egy ideiglenes, kezdő szintű alkalmazást az App Service szolgáltatásban – kötelezettségek vállalása, és bankkártyaadatok megadása nélkül.

* Visual Studio 2015 a következővel: [Azure SDK for .NET](http://go.microsoft.com/fwlink/?linkid=518003) – Az SDK automatikusan telepíti a Visual Studio 2015-öt, ha nincs telepítve a számítógépen.

    >[AZURE.NOTE] Az SDK telepítéséhez szükséges idő néhány perc és több mint fél óra között változhat, attól függően, hogy hány SDK-függőség van már telepítve számítógépére.
...
2. Állítsa be indítási projektként a ToDoListDataAPI projektet (**nem** a ToDoListAPI projektet). 
 
4. A projekt hibakeresési módban való futtatásához nyomja meg az F5 billentyűt, vagy kattintson a **Debug > Start Debugging** (Hibakeresés > Hibakeresés indítása) elemre.

    Megnyílik a böngésző, és megjelenik a HTTP 403 hibaoldal.

12. A böngésző címsorában adja hozzá a cím végéhez a `swagger/docs/v1` szöveget, majd nyomja le az Enter billentyűt. (Az URL-cím így a következő: `http://localhost:45914/swagger/docs/v1`.)
...
1. Zárja be a böngészőt, és állítsa le a Visual Studio hibakeresési módját.

3. A **Solution Explorer** (Megoldáskezelő) területén, a ToDoListDataAPI projektben nyissa meg az *App_Start\SwaggerConfig.cs* nevű fájlt, majd görgessen le a következő kódrészig, és vonja vissza annak megjegyzéssé tételét.
...
5. Futtassa ismét a projektet.

3. A böngésző címsorában adja hozzá a cím végéhez a `swagger` szöveget, majd nyomja le az Enter billentyűt. (Az URL-cím így a következő: `http://localhost:45914/swagger`.)

4. A megjelenő Swagger felhasználói felületben kattintson a **ToDoList** elemre az elérhető metódusok megjelenítéséhez.

    ![Swagger felhasználói felület, elérhető metódusok](./media/app-service-api-dotnet-get-started/methods.png)

5. Kattintson a lista első **Get** (Lekérés) gombjára.

6. A **Parameters** (Paraméterek) mezőnél írjon be egy csillagot az `owner` paraméter értékeként, majd kattintson a **Try it out** (Kipróbálás) elemre.
...
6. Kattintson a **Post** (Közzététel) elemre, majd a **Model Schema** (Modellséma) rész alatti mezőre.
...
7. Módosítsa a JSON-t a `todo` paraméter beviteli mezőjében úgy, hogy az alábbi példához hasonlítson, vagy adja meg saját tetszőleges leíró szövegét:
...
10. Kattintson a **Try it out** (Kipróbálás) elemre.

    A ToDoList API HTTP 204-es válaszkódot ad vissza, amely a művelet sikerességét jelzi.

11. Kattintson az első **Get** (Lekérés) gombra, majd az oldal ezen részében kattintson a **Try it out** (Kipróbálás) gombra.

    A Get metódusra adott válasz mostantól tartalmazza az új tennivalót is. 

12. Nem kötelező: próbálja ki a Put, a Delete és a Get by ID metódust is.
...
3.  A **Publish Web** (Weboldal közzététele) varázsló **Profile** (Profil) lépésénél kattintson a **Microsoft Azure App Service** lehetőségre.
...
4. Ha még nem tette meg, jelentkezzen be Azure-fiókjába, ha pedig a rendszer kijelentkeztette volna, adja meg ismét hitelesítő adatait.
...
4. Adjon olyan nevet az alkalmazásnak az **API App Name** (API-alkalmazás neve) mezőben, amely még nem létezik az *azurewebsites.net* tartományban. Elfogadhatja a Visual Studio által ajánlott alapértelmezett nevet.
...
5. A **Configure App Service Plan** (App Service-csomag konfigurálása) párbeszédpanelen írja be a „ToDoListPlan”, vagy bármilyen más tetszőleges nevet.

5. A **Location** (Hely) legördülő listában válassza ki az Önhöz legközelebb eső helyet.
...
5. A **Méret** legördülő menüben kattintson az **Ingyenes** lehetőségre.
...
11. Adja hozzá a „swagger” szót a böngésző címsorában látható URL-címhez, majd nyomja le az Enter billentyűt. (Az URL-cím így a következő: `http://{apiappname}.azurewebsites.net/swagger`.)
...
12. Nyissa meg az [Azure portált](https://portal.azure.com/).
...
8. Az **App Service** párbeszédpanelen bontsa ki az oktatóanyagban használt erőforráscsoportot, válassza ki az API-alkalmazást, majd kattintson az **OK** gombra.
...
9. Az **Add REST API Client** (REST API-ügyfél hozzáadása) párbeszédpanelen kattintson az **OK** gombra.
...
5. A ToDoListAPI projektben nyissa meg a *Controllers\ToDoListController.cs* fájlt az API-t a generált ügyfél segítségével behívó kód megtekintéséhez. 
...
3. A **Create App Service** (App Service létrehozása) párbeszédpanel **Hosting** (Üzemeltetés) lapján írjon be egy olyan nevet az **API App Name** (API-alkalmazás neve) mezőbe, amely egyedi az *azurewebsites.net* tartományban. 
...
4. Az **App Service Plan** (App Service-csomag) legördülő menüben válassza a korábban létrehozott csomagot. A rendszer alapértelmezésként beírja ezt az értéket. 
...
3.  A **Publish Web** (Weboldal közzététele) varázsló **Connection** (Kapcsolat) lépésénél kattintson a **Publish** (Közzététel) elemre.
...
4. Az API-alkalmazások **Alkalmazásbeállítások** paneljén görgessen le az **Alkalmazásbeállítások** részhez, és vegye fel a következő paramétert és értéket:
...
11. Egy böngészőablakban nyissa meg a középső réteg API-alkalmazásának URL-címét, amelyet az előző lépéseknél hozott létre a ToDoListAPI számára. Ehhez úgy juthat el, ha a portálon, az API-alkalmazás főpaneljén rákattint az URL-címre.

13. Adja hozzá a „swagger” szót a böngésző címsorában látható URL-címhez, majd nyomja le az Enter billentyűt. (Az URL-cím így a következő: `http://{apiappname}.azurewebsites.net/swagger`.)
...
12. Próbálja ki a Get metódust és a többi metódust, ezzel ellenőrizve, hogy a középső réteghez tartozó API-alkalmazás sikeresen behívja-e az adatréteg API-alkalmazását.
