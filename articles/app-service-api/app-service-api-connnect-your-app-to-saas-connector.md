...
> [AZURE.NOTE] 如果您想要使用邏輯應用程式中的 SaaS 連接器，就不需要的指示進行。 如需有關如何使用 SaaS 連接器，在邏輯應用程式內的資訊，請參閱 [建立新的邏輯應用程式](../app-service-logic/app-service-logic-create-a-logic-app.md) 和 [在連接器中使用自訂的 OAUTH 應用程式](https://social.msdn.microsoft.com/Forums/en-US/home?forum=azurelogicapps&announcementId=4af1e4c5-d220-4457-97d8-d08e427ae6c1)。
...
5. 在 **Dropbox 連接器** 刀鋒視窗底下 **應用程式服務方案** 按一下 **建立新的**, ，然後在 **建立新的應用程式服務方案** 方塊中輸入 DropBoxPlan。 

    如需 App Service 方案的詳細資訊，請參閱 [Azure App Service 方案深入概觀](azure-web-sites-web-hosting-plans-in-depth-overview.md)。 

4. 在 **資源群組**, ，按一下 [ **新建**, ，然後在 **建立新的資源群組** 方塊中輸入 DropboxRG。

    如需資源群組的詳細資訊，請參閱 [使用資源群組來管理您的 Azure 資源](../resource-group-overview.md)。

7. 選取免費 **定價層**。 (如果您沒有看到它在清單中，按一下 [ **檢視所有**。 按一下之後 **F1 免費**, ，按一下 [ **選取** ] 按鈕。)
...
9. <a id="gateway"></a>將預設"dropboxconnector 保留為 **名稱** 的連接器，然後按一下 **建立**。 
    ...
    Azure App Service 建立資源群組，並在資源群組中建立 Dropbox 連接器 API 應用程式和 *閘道* web 應用程式。 閘道的功能就是管理如何存取資源群組中的所有 API 應用程式。 

    您可以按一下來檢查資源的建立進度 **通知** Azure 預覽入口網站首頁上。

3. 當 Azure 完成連接器的建立時，按一下 [ **瀏覽 > 資源群組 > DropboxRG**。
 
     **資源群組** DropboxRG 刀鋒視窗中顯示的資源群組中的連接器和閘道。
...
依預設，連接器 **存取層級** 設為 **內部**, ，意思，它只能呼叫其他 API 應用程式和 web 應用程式中的相同資源群組。 但是，Dropbox 僅允許已驗證的使用者才能存取您的 Dropbox 帳戶，因此您必須將存取層級設定變更為需要使用者驗證。
...
3. 在瀏覽器顯示登入頁面時輸入您的認證。 
    ...
    如果您設定 Azure Active Directory 登入，登入做為其中一個列出的使用者 **使用者** 您建立的 Azure Active Directory 索引標籤中的應用程式] 索引標籤 [Azure portal], ，例如 admin@contoso.onmicrosoft.com。
...
1. 在具有「登入完成」訊息的瀏覽器視窗中，移至瀏覽器的開發人員工具，並找出 `x-zumo-auth` Cookie。 將此視窗保持開啟狀態，以便您可以在下一個步驟複製 Cookie 的值。
    ...
    - 按 F12 來開啟開發人員工具。
    - 移至 **資源** ] 索引標籤。
    - 尋找您的閘道網站的 cookie，然後按三 **值** 的 `x-zumo-auth` cookie 來選取它的全部。 (確定您取得 Cookie 的所有值。 如果按兩下，可能只會取得它的第一個部分。)  
...
4. 在新的瀏覽器索引標籤或視窗中，建立並傳送 HTTP Post 要求至閘道，以要求同意的 URL。 包括 `x-zumo-auth` 權杖作為 HTTP 標頭。
    ...
    - 標頭中 **值** 欄位貼上您從複製的值 `x-zumo-auth` cookie。
    - 按一下 [ **傳送**。
    ...
    回應包括一個 URL，用來起始利用 Dropbox 登入使用者的程序 (如果您收到錯誤回應，指出雖然已設為方法下拉式清單中，將不支援 Get 方法 **Post**, ，請確定您的閘道 URL 是 HTTPS，而不是 HTTP。)
...
1. 在已開啟 Azure 預覽入口網站的瀏覽器視窗，返回 **API 應用程式** Dropbox 連接器] 分頁。 

2. 複製 API 應用程式的 URL。
 
4. 按一下 [ **API 定義** 圖示，以查看連接器中可用的 API 方法。
