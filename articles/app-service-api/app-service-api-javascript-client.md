...
本文將說明如何建立的 HTML 和 JavaScript 用戶端 [API 應用程式](app-service-api-apps-why-best-platform.md) 中 [Azure App Service](/documentation/services/app-service/)。 本文假設您熟悉 HTML 和 JavaScript，並使用 [AngularJS](https://angularjs.org/) JavaScript 架構對進行 REST 呼叫 API 應用程式。 

在此之前先閱讀某些文件，可幫助您開始使用。 

1. 在 [建立 API 應用程式](app-service-dotnet-create-api-app.md), ，您會建立 API 應用程式專案。
2. 在 [部署 API 應用程式](app-service-dotnet-deploy-api-app.md), ，您將 API 應用程式部署至您的 Azure 訂閱。
3. 在 [偵錯 API 應用程式](../app-service-dotnet-remotely-debug-api-app.md), ，您可以使用 Visual Studio 遠端偵錯程式碼在 Azure 中執行。
...
API 應用程式閘道可設定為使用 Azure Preview 入口網站啟用 CORS。 藉由新增 **MS_CrossDomainOrigins** *appSetting* 您可以指定允許呼叫 API 應用程式的 Url。 本章節將說明如何使用這個 *appSetting* 在 API 閘道層級啟用 CORS。 

1. 瀏覽至您要啟用 CORS 的 API 應用程式的 Azure 預覽入口網站刀鋒視窗。 一旦抵達，請按一下 *閘道* API 應用程式的圖示。 

    ![按一下 [API 應用程式閘道] 按鈕](./media/app-service-api-javascript-client/19-api-app-blade.png)

1. 按一下 [ **閘道器主機** 入口網站刀鋒視窗中的連結。 

    ![按一下 [API 應用程式閘道主機] 連結](./media/app-service-api-javascript-client/20-gateway-host-blade.png)

1. 按一下 [ **所有設定** 入口網站刀鋒視窗中的連結。 

    ![所有閘道設定連結](./media/app-service-api-javascript-client/21-gateway-blade-all-settings.png)

1. 按一下 [ **應用程式設定** 入口網站刀鋒視窗中的按鈕。

    ![閘道應用程式設定](./media/app-service-api-javascript-client/22-gateway-app-settings-blade.png)

1. 新增 **MS_CrossDomainOrigins** 應用程式設定。 讓此設定的值成為您要提供 API 應用程式存取權的 HTTP 主機清單 (以逗號分隔)。 如果您想要提供存取多個主機，值 *appSetting* 可以設定為類似下列程式碼。 
 ...
 **MS_CrossDomainOrigins** 應用程式設定的部落格文章中詳細討論 [Azure 行動服務.NET 更新](http://azure.microsoft.com/blog/2014/07/28/azure-mobile-services-net-updates/), ，因此，請查看這篇文章，如需有關設定的詳細資訊。

### 在 Web API 程式碼中啟用 CORS

啟用 CORS Web API 中的程序記載於 ASP.NET 文章 [ASP.NET Web API 2 中啟用跨來源要求](http://www.asp.net/web-api/overview/security/enabling-cross-origin-requests-in-web-api) 深入了解。 若為使用 ASP.NET Web API 建置的 API 應用程式，此程序完全一樣，但會在以下摘要說明。 如果您已啟用 CORS，請略過這一節，因為您的 API 應用程式應該已正確設定。 

1. CORS 功能由提供 [Microsoft.AspNet.WebApi.Cors](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Cors/) NuGet 封裝。 開啟安裝 **Package Manager Console**, ，然後執行下列 PowerShell 指令碼。 

        Install-Package Microsoft.AspNet.WebApi.Cors

1. 一旦命令執行時，封裝管理員主控台和 **packages.config** 會反映加入新的 NuGet 封裝。 

    ![方案總管中的 apiapp.json 和中繼資料](./media/app-service-api-javascript-client/01-cors-installed.png)

1. 開啟 *app_start/webapiconfig.cs* 檔案。 加入這一行下方的程式碼，以 **註冊** 方法 **WebApiConfig** 檔案中的類別。 
...
1. 以滑鼠右鍵按一下您稍早建立的方案 [建立 API 應用程式](app-service-dotnet-create-api-app.md), ，然後選取 **新增]-> [新增專案**
...
1. 將 bootstrap CSS 和 AngularJS JavaScript 檔案加入至 HTML 網頁，以及使用簡單的 Bootstrap 範本 ([喜歡這一個](http://getbootstrap.com/examples/starter-template/)) 並建立空的指令碼標記來準備網頁。 
...
1. 加入至如下所示的 HTML 資料表程式碼 **容器** *div* HTML 中的項目。
...
3. 以滑鼠右鍵按一下 *index.html* 檔案，然後按一下 **瀏覽器中的檢視**。 
...
1. 加入 JavaScript 程式碼，以下 *index.html* 檔案，以呼叫 API 和資料繫結至 API 輸出 HTML 使用者介面。 
...
>[AZURE.NOTE] **注意** 請勿使用 HTML 用戶端專案的連接埠號碼。 您可以以滑鼠右鍵按一下 [API 專案，然後按一下 **偵錯 > 啟動新執行個體** 取得瀏覽器視窗，來顯示通訊埠編號。
...
1. 貼上 API 應用程式的 URL 來覆寫先前的值 **$scope.baseUrl** JavaScript 程式碼中的屬性。 

        $scope.baseUrl = 'https://microsoft-apiappf7e042ba8e5233ab4312021d2aae5d86.azurewebsites.net';

    留意 URL 是否指定 HTTPS。  使用 HTTPS 不是選擇性的。 API 應用程式不支援 HTTP。
...
1. 選取 **Microsoft Azure Web Apps** 發行 Web] 對話方塊中的選項。 
