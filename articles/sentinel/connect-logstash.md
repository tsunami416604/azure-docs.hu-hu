...
| `plugin_flush_interval` | szám | Nem kötelező kitölteni. A beállítással adhatja meg a maximális időközt (másodpercben) az üzenetek átvitele között Log Analytics. Az alapértelmezett érték az 5. |
    | `amount_resizing` | boolean | TRUE (igaz) vagy FALSE (hamis). Engedélyezheti vagy letilthatja az automatikus skálázási mechanizmust, amely a fogadott naplófájlok mennyisége alapján módosítja az üzenet pufferének méretét. |
