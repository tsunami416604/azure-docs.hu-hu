...
1. Add a role assignment at the subscription level:
   ...
   > [!NOTE]
   > Most artifacts support parameters. A parameter that's assigned a value during blueprint creation is a *static parameter*. If the parameter is assigned during blueprint assignment, it's a *dynamic parameter*. További információkat a [tervparamétereket](./concepts/parameters.md) ismertető cikkben talál.
...
1. Add a policy assignment at the subscription level:
   ...
   1. A szűrés végrehajtásához kattintson a **Keresésen** kívülre. Select **Append tag and its default value to resource groups**.
...
1. Add a template under the resource group:
   ...
   1. A szerkesztőmező **Sablon** lapján illessze be az alábbi Resource Manager-sablont.
      ...
      > [!IMPORTANT]
      > If you're importing the template, ensure that the file is only JSON and doesn't include HTML. When you're pointing to a URL on GitHub, ensure that you have selected **RAW** to get the pure JSON file and not the one wrapped with HTML for display on GitHub. Ha az importált sablon nem tiszta JSON, hiba történik.
...
1. Kész tervének az alábbi ábrán láthatóhoz kell hasonlítania. Notice that each artifact has **_x_ out of _y_ parameters populated** in the **Parameters** column. The dynamic parameters are set during each assignment of the blueprint.
...
1. Kész tervének az alábbi ábrán láthatóhoz kell hasonlítania. Notice that the newly added role assignment shows **1 out of 1 parameters populated**. That means it's a static parameter.
...
1. On the **Assign blueprint** page, in the **Subscription** drop-down list, select the subscriptions that you want to deploy this blueprint to.
   ...
   If there are supported Enterprise offerings available from [Azure Billing](../../billing/index.md), a **Create new** link is activated under the **Subscription** box. Kövesse az alábbi lépéseket:
...
1. In **Location**, select a region for the managed identity and subscription deployment object to be created in. Az Azure Blueprint a hozzárendelt tervben lévő összes összetevő üzembe helyezéséhez ezt a felügyelt identitást használja. További tudnivalókért lásd [az Azure-erőforrások felügyelt identitásait](../../active-directory/managed-identities-azure-resources/overview.md).
...
1. A **Hozzárendelés zárolása** mezőben hagyja meg az alapértelmezett **Nem zárolandó** értéket. For more information, see [Blueprints resource locking](./concepts/resource-locking.md).
...
If you no longer need a blueprint assignment, remove it from a subscription. The blueprint might have been replaced by a newer blueprint with updated patterns, policies, and designs. Az egyes tervek eltávolításakor az adott tervek keretében hozzárendelt összetevők megmaradnak. A tervek hozzárendelését az alábbi lépések szerint szüntetheti meg:
