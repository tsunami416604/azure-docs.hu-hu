...
Ez a lekérdezés olyan virtuális gépeket keres, amelyek egyeznek egy [reguláris kifejezéssel](/dotnet/standard/base-types/regular-expression-language-quick-reference) (más néven _regex-szel_). The **matches regex \@** allows us to define the regex to match, which is `^Contoso(.*)[0-9]+$`.
