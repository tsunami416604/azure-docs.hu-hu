...
A következő lekérdezés az `summarize` erőforrások az előfizetés `join` alapján való számlálására, az előfizetés részleteinek a _ResourceContainers_ táblából való `project-away` összekapcsolására, majd néhány oszlop eltávolítására használja.
...
## <a name="list-cosmos-db-with-specific-write-locations"></a><a name="mvexpand-cosmosdb" />Adott írási hellyel rendelkező Cosmos DB listázása
...
## <a name="key-vault-with-subscription-name"></a><a name="join" />Key Vault előfizetés neve
...
## <a name="list-sql-databases-and-their-elastic-pools"></a><a name="join-sql" />SQL-adatbázisok és rugalmas készletek listázása
...
## <a name="list-virtual-machines-with-their-network-interface-and-public-ip"></a><a name="join-vmpip" />A hálózati adapterrel és a nyilvános IP-címmel rendelkező virtuális gépek listázása
...
## <a name="find-storage-accounts-with-a-specific-tag-on-the-resource-group"></a><a name="join-findstoragetag" />Adott címkével rendelkező Storage-fiókok keresése az erőforráscsoporthoz

Az alábbi lekérdezés **belső** `join` használatával csatlakozik a Storage-fiókokhoz olyan erőforrás-csoportokkal, amelyekben a kis-és nagybetűket megkülönböztető címke neve és a címke értéke szerepel.
...
Ha meg kell keresni a kis-és nagybetűket megkülönböztető címke nevét és a címke értékét `mv-expand` , használja a with **bagexpansion** paramétert. Ez a lekérdezés több kvótát használ, mint az előző lekérdezés `mv-expand` , ezért csak szükség esetén használja.
...
## <a name="include-the-tenant-and-subscription-names-with-displaynames"></a><a name="displaynames" />Bérlői és előfizetési nevek belefoglalása a DisplayName paraméterrel
