...
> - [A virtuális gép erőforrásának aliasok megjelenítése](#show-aliases)
> - [Egy adott alias eltérő értékek megjelenítése](#distinct-alias-values)
...
## <a name="a-namecount-resourcescount-azure-resources"></a><a name="count-resources"/>Száma Azure-erőforrások
...
## <a name="a-namelist-resourceslist-resources-sorted-by-name"></a><a name="list-resources"/>Az erőforrások listázására név szerint rendezve
...
## <a name="a-nameshow-vmsshow-all-virtual-machines-ordered-by-name-in-descending-order"></a><a name="show-vms"/>Csökkenő sorrendben név szerint rendezett összes virtuális gép megjelenítése
...
## <a name="a-nameshow-sortedshow-first-five-virtual-machines-by-name-and-their-os-type"></a><a name="show-sorted"/>Megjelenítés az első öt virtuális gépek nevét és az operációs rendszer típusa
...
## <a name="a-namecount-oscount-virtual-machines-by-os-type"></a><a name="count-os"/>Az operációs rendszer típus szerint a virtuális gépek száma
...
## <a name="a-nameshow-storageshow-resources-that-contain-storage"></a><a name="show-storage"/>Amelyek tartalmazzák a tárolási erőforrások megjelenítése
...
## <a name="a-namelist-publiciplist-all-public-ip-addresses"></a><a name="list-publicip"/>Az összes nyilvános IP-címek listája
...
## <a name="a-namecount-resources-by-ipcount-resources-that-have-ip-addresses-configured-by-subscription"></a><a name="count-resources-by-ip"/>Erőforrások, amelyek rendelkeznek az előfizetés által konfigurált IP-címek száma
...
## <a name="a-namelist-taglist-resources-with-a-specific-tag-value"></a><a name="list-tag"/>Egy adott címkeértékkel erőforrások listája
...
## <a name="a-namelist-specific-taglist-all-storage-accounts-with-specific-tag-value"></a><a name="list-specific-tag"/>Adott címkeértékkel rendelkező összes storage-fiókok listázása
...
## <a name="a-nameshow-aliasesshow-aliases-for-a-virtual-machine-resource"></a><a name="show-aliases"/>A virtuális gép erőforrásának aliasok megjelenítése

[Az Azure szabályzat-aliasok](../../policy/concepts/definition-structure.md#aliases) erőforrás megfelelőségi kezelése az Azure Policy által használt. Az Azure Erőforrás-grafikon lépjen vissza a _aliasok_ az erőforrástípus. Ezek az értékek összehasonlítása a jelenlegi érték aliasok egyéni szabályzat-definíció létrehozása során hasznosak. A _aliasok_ tömb a lekérdezések eredményei alapértelmezés szerint nincs megadva. Használat `project aliases` explicit módon hozzá az eredményeket.
...
## <a name="a-namedistinct-alias-valuesshow-distinct-values-for-a-specific-alias"></a><a name="distinct-alias-values"/>Egy adott alias eltérő értékek megjelenítése

Megtekintés aliasok értékét egy erőforráson, akkor lehet hasznos, de nem jelenik meg az IGAZ érték, Azure-erőforrás Graph használatával lekérdezést az előfizetések között. Ebben a példában szereplő összes érték egy adott alias megvizsgálja, és az eltérő értékeket ad vissza.
