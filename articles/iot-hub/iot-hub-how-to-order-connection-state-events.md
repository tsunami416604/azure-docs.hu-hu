...
Az Azure Event Grid segít az eseményalapú alkalmazásokat hozhat létre, és könnyedén integrálhatja a saját vállalati megoldások az IoT-események. Ez a cikk végigvezeti egy, amely segítségével nyomon követheti, és a legújabb eszköz kapcsolat állapot tárolásához a Cosmos DB-ben. Azt a csatlakoztatott eszköz és az eszköz leválasztott esemény elérhető sorszáma használja, és a legfrissebb állapot tárolásához a Cosmos DB-ben. Egy tárolt eljárást, amely egy Cosmos DB-gyűjteményen végrehajtott alkalmazáslogika használni fogjuk.
...
* Az IoT hubra az Azure-ban. Ha még nem hozott létre központot, [az IoT Hub első lépéseit](../iot-hub/iot-hub-csharp-csharp-getstarted.md) ismertető cikkben talál útmutatást. 
...
1. Az a [az Azure portal](https://portal.azure.com)válassza **új** > **integrációs** > **logikai alkalmazás**.
...
3. Válassza a **Séma létrehozása hasznosadat-minta használatával** lehetőséget. 
   ...
   ![HTTP-kérés trigger kiválasztása](./media/iot-hub-how-to-order-connection-state-events/sample-payload.png)
...
## <a name="configure-subscription-for-iot-hub-events"></a>IoT Hub-események-előfizetés beállítása
...
4. Hozza létre az esemény-előfizetést a következő értékekkel: 
   ...
   * **Végpont részletei**: Endpoint típusának kiválasztása, **Webhook** és kattintson a válassza végpont, és illessze be az URL-cím a logikai alkalmazás kimásolt és kijelölés megerősítéséhez.

   ![Válassza ki a végpont URL-címe](./media/iot-hub-how-to-order-connection-state-events/endpoint-url.png)

   * **Esemény-előfizetés részleteit**: Adjon meg egy leíró nevet, és válassza ki **Event Grid séma** Ha elkészült, a képernyő az alábbihoz hasonlóan kell kinéznie: 
...
### <a name="run-a-sample-applciation-on-the-raspberry-pi-web-simulator"></a>Futtassa a mintát hatóságuknál a Raspberry Pi-webszimulátor
...
Láthatja a végrehajtott tárolt eljárás eredményét a Cosmos DB-dokumentumban. Íme, fog kinézni. Vegye figyelembe, hogy minden egyes sor tartalmazza a legújabb az eszköz kapcsolati állapotát eszközönként

   ![Hogyan eredménye](./media/iot-hub-how-to-order-connection-state-events/cosmosDB-outcome.png)

## <a name="use-the-azure-cli"></a>Az Azure parancssori felületének használata

Az Azure Portal helyett az Azure CLI használatával is végrehajthatja az IoT Hub lépéseit. További információkért lásd: az Azure CLI-vel oldalak [egy esemény-előfizetés létrehozása](https://docs.microsoft.com/cli/azure/eventgrid/event-subscription) és [egy IoT-eszköz létrehozása](https://docs.microsoft.com/cli/azure/iot/device).
...
Távolítsa el az Azure Cosmos DB-fiókot az Azure Portalról, jogosultságot – kattintson a fiók nevét, és kattintson a **fiók törlése**. Részletes útmutatást lásd: [törlése az Azure Cosmos DB-fiók](https://docs.microsoft.com/azure/cosmos-db/manage-account#delete).
