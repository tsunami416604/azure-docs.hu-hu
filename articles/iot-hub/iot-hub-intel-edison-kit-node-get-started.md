---
title: Intel Edison felhőbe (Node.js) – Intel Edison csatlakoztatása Azure IoT hubra |} A Microsoft Docs
...
2. Nyomja le az alatt a szavakat modulon `What will you make?` mindaddig, amíg úgy véli, hogy a beépülő modult.
   ...
   ![Állítsa össze a tábla 2](media/iot-hub-intel-edison-kit-node-get-started/1_assemble_board2.jpg)

3. A modul a helybővítés táblához való védelmére használja a két hexadecimális nuts (a csomagban található).

   ![Állítsa össze a tábla 3](media/iot-hub-intel-edison-kit-node-get-started/2_assemble_board3.jpg)
...
4. A bővítés táblán a négy sarokban lyuk egyik beszúrása elment. Twist nevűt, és egy alakzatot a csavart fehér műanyag térköztartók növeljük.
   ...
   ![Állítsa össze a tábla 4](media/iot-hub-intel-edison-kit-node-get-started/3_assemble_board4.jpg)

5. Ismételje meg a többi három sarokban térköztartók.

   ![Állítsa össze a tábla 5](media/iot-hub-intel-edison-kit-node-get-started/4_assemble_board5.jpg)

Most már a tábla összeállítva.

   ![kiépített tábla](media/iot-hub-intel-edison-kit-node-get-started/5_assembled_board.jpg)
...
2. Hőmérséklet-érzékelő Groove alakzatot a Groove alap pajzs Groove-kábel használatával **A0** port.
   ...
   ![Csatlakozhat a hőmérséklet-érzékelő](media/iot-hub-intel-edison-kit-node-get-started/7_temperature_sensor.jpg)
...
1. Váltsa át a két micro USB-porttal, amelyért mikrokapcsoló le, hogy Edison eszköz módban van. Az eszköz és a gazdagép üzemmód közötti különbségek, kérjük, hivatkozzon [Itt](https://software.intel.com/en-us/node/628233#usb-device-mode-vs-usb-host-mode).
   ...
   ![Váltsa át a mikrokapcsoló lefelé](media/iot-hub-intel-edison-kit-node-get-started/9_toggle_down_microswitch.jpg)
...
> Győződjön meg arról, hogy Edison csatlakozik-e a számítógép ugyanazon a hálózaton. A számítógép csatlakozik a Edison az IP-cím használatával.
   ...
   ![Csatlakozhat a hőmérséklet-érzékelő](media/iot-hub-intel-edison-kit-node-get-started/12_configuration_tool.png)
...
1. A gazdagép számítógépről a következő SSH-ügyfél használatával az Intel Edison csatlakozni. Az IP-cím van a konfigurációs eszközt, és a jelszó pedig az eszköz a beállított.
    ...
    - A beépített SSH-ügyfelet Ubuntu vagy macOS rendszeren.
...
3. Keresse meg a tárház mappában futtassa a következő parancsot az összes csomag telepítéséhez, serval percig is eltarthat.
...
### <a name="configure-and-run-the-sample-application"></a>Konfigurálja és a mintaalkalmazás futtatása
...
1. Nyissa meg a konfigurációs fájlban a következő parancsok futtatásával:
   ...
   Nincsenek két makrók configurate használhatja ezt a fájlt. Az első egy `INTERVAL`, amely megadja, hogy a két a felhőbe küldött üzenetek között eltelt időt. A második érték `simulatedData`, azaz az e használni a szimulált érzékelői adatokat, vagy nem logikai érték.
...
![Kimenet – Intel Edison az IoT hubnak küldött érzékelőktől kapott adatok](media/iot-hub-intel-edison-kit-node-get-started/15_message_sent.png)
