...
1. Nyissa meg az imént létrehozott beszédfelismerési szolgáltatást, kattintson a **kulcsok** szakaszra a másoláshoz, és jegyezze fel a **Key1** a fejlesztői készlet eléréséhez.
  ![a kulcsok másolása](media/iot-hub-arduino-iot-devkit-az3166-translator/copy-keys.png)
...
1. A IoT Workbench-példák oldalon keresse meg a **fejlesztői készlet translatort** , és kattintson a **minta megnyitása**lehetőségre. Ezután kiválasztja a mintakód letöltésének alapértelmezett elérési útját.
  ![Open minta](media/iot-hub-arduino-iot-devkit-az3166-translator/open-sample.png)

## <a name="use-speech-service-with-azure-functions"></a>A Speech Service használata Azure Functions

1. A VS Code-ban kattintson a `F1`elemre, írja be és válassza az **Azure IoT Device Workbench: Azure-szolgáltatások kiépítése...** lehetőséget. ![Azure-szolgáltatások kiépítése](media/iot-hub-arduino-iot-devkit-az3166-translator/provision.png)
...
1. Nyissa meg a `Functions\DevKitTranslatorFunction.cs`t, és frissítse a kód következő sorát a megjegyzett eszköznév és beszédfelismerési szolgáltatás kulcsával.
   ```csharp
...
1. Kattintson a `F1`elemre, írja be és válassza **Az Azure IoT Device Workbench: üzembe helyezés az Azure**-ban... lehetőséget. Ha a VS Code megerősítést kér az újratelepítéshez, kattintson az **Igen**gombra.
...
1. Azure Portal, lépjen a **functions-alkalmazások** szakaszra, keresse meg az imént létrehozott Azure Function alkalmazást. Kattintson a `devkit_translator`elemre, majd az URL-cím másolásához kattintson a **</> függvény URL-címe** elemre.
   ![másolási függvény URL-címe](media/iot-hub-arduino-iot-devkit-az3166-translator/get-function-url.png)

1. Illessze be az URL-címet `azure_config.h` fájlba.
...
1. Kattintson a `F1`elemre, írja be és válassza ki az **Azure IoT Device Workbench: eszközbeállítások konfigurálása... > Konfigurációs eszköz csatlakoztatási karakterlánca**. Válassza a **IoT hub eszköz kapcsolódási karakterlánc kiválasztása** lehetőséget a fejlesztői készlet való konfigurálásához.
...
1. Kattintson ismét az `F1` elemre, írja be és válassza az **Azure IoT Device Workbench: eszköz kódjának feltöltése**lehetőséget. Elindul a kód fordítása és feltöltése a fejlesztői készlet.
   ![eszköz feltöltésének](media/iot-hub-arduino-iot-devkit-az3166-translator/device-upload.png)
