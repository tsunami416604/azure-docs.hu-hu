...
!["Eszköz stream kézfogási folyamat"](./media/iot-hub-device-streams-overview/iot-hub-device-streams-handshake.png)
...
Az eszköz és a egy eszköz stream szolgáltatás oldalán is képes kapcsolatokat a TLS-kompatibilis IoT hubot és a tartalomstreameléshez használt streamvégpont kell lennie. Kimenő kapcsolat ehhez a végpontokkal 443-as porton keresztül. Az állomásnév, ezeket a végpontokat társított találhatók a *áttekintése* fülre az IoT Hub, az alábbi ábrán látható módon:
...
> [!NOTE]
> Győződjön meg arról, telepített Azure CLI-vel 2.0.57 verzió vagy újabb. Letöltheti a legújabb verziót a [Azure CLI telepítése](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest) lapot.
>

## <a name="allow-outbound-connectivity-to-the-device-streaming-endpoints"></a>Az eszközön streamvégpontok kimenő kapcsolatok engedélyezése

Ahogy említettük, ez a cikk elején, az eszköz eszköz Streamek kezdeményezés folyamat során az IoT Hub streamvégpont egy kimenő kapcsolatot hoz létre. A tűzfalnak az eszközön vagy a hálózati engedélyeznie kell a kimenő kapcsolatot a 443-as porton keresztül az adatfolyam-továbbítási átjáró (vegye figyelembe, hogy a kommunikáció történik a TLS használatával titkosított WebSocket kapcsolaton keresztül).

Az Azure IoT Hub Portal Áttekintés lapján található eszköz streamvégpont állomásnevét. !["Eszköz adatfolyam végpontjainak"](./media/iot-hub-device-streams-overview/device-stream-in-portal.png)
...
1. Keresse meg a *diagnosztikai beállítások* az IoT Hub lapra, majd kattintson a *diagnosztika bekapcsolása* hivatkozásra.
   ...
   !["A diagnosztikai naplók engedélyezése"](./media/iot-hub-device-streams-overview/device-streams-diagnostics-settings-pane.png)
...
2. Adjon meg egy nevet a diagnosztikai beállítások, és válassza a *Küldés a Log Analyticsnek* lehetőséget. Válasszon egy meglévő Log Analytics munkaterület erőforrás, vagy hozzon létre egy újat a Microsofthoz. Ezenkívül ellenőrizze a *DeviceStreams* a listából.
    ...
    !["A Streamek eszköznaplók engedélyezése"](./media/iot-hub-device-streams-overview/device-streams-configure-diagnostics.png)
...
3. Most már elérheti az eszköznaplók Streamek alatt a *naplók* az IoT Hub portálon fülre. Tevékenységnaplók streamelése eszköz fog megjelenni a `AzureDiagnostics` táblát, és rendelkezik `Category=DeviceStreams`.
   ...
   !["Az eszköz stream naplók elérése"](./media/iot-hub-device-streams-overview/device-streams-view-logs.png)
...
## <a name="iot-hub-device-stream-samples"></a>Az IoT Hub eszköz stream minták

Kettő [rövid minták](/azure/iot-hub) az IoT Hub oldalon érhető el. Eszköz-Streamek használatát bemutatják az alkalmazások.
...
Az echo programozott küldése és fogadása service és az eszköz alkalmazások közötti eszköz-Streamek használatát mutatja be. Vegye figyelembe, hogy szolgáltatás és eszköz programok használható különböző nyelveken. Például használhatja a C a program a C# szolgáltatási program.

Az alábbiakban az echo minták:
...
!["Stream proxy eszközbeállítások SSH vagy RDP-hez"](./media/iot-hub-device-streams-overview/iot-hub-device-streams-ssh.png)
...
* A 22-es portot, amely protokoll-függő és adott ssh ebben az esetben. Az RDP a helyzet a 3389-es portot kell használni. Ez a megadott minta programok telepítése és törlése is konfigurálható.
...
Az alábbi hivatkozások segítségével további tudnivalók a eszköz Streamek.

> [!div class="nextstepaction"]
> [IoT-eszköz adatfolyamokat megjelenítése (Channel 9)](https://nam06.safelinks.protection.outlook.com/?url=https%3A%2F%2Fchannel9.msdn.com%2FShows%2FInternet-of-Things-Show%2FAzure-IoT-Hub-Device-Streams&data=02%7C01%7Crezas%40microsoft.com%7Cc3486254a89a43edea7c08d67a88bcea%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C636831125031268909&sdata=S6u9qiehBN4tmgII637uJeVubUll0IZ4p2ddtG5pDBc%3D&reserved=0)