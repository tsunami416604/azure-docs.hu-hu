...
-   **每一裝置的驗證和安全連線**。 您可以提供每個裝置獨有的安全性金鑰讓它連線到 IoT 中樞。  [IoT 中心識別登錄][lnk-devguide-identityregistry] 裝置身分識別和金鑰會儲存在方案中。  解決方案後端可將個別裝置加入白名單或黑名單，以達到完全控制裝置存取權。

-   **裝置程式庫的一組更周延**。 Azure IoT 裝置 SDK 可供各種語言和平台使用並受其支援：C 表示許多 Linux 散發套件、Windows 和 RTOS。 Azure IoT 裝置 SDK 也支援 C#、Java 和 JavaScript 等 Managed 語言。
...
-   **小數位數**。 Azure IoT 中心會調整為數百萬個同時連接的裝置，以及每秒數百萬個事件。
...
發行項 [IoT 中心比較和事件中樞][lnk-compare] 描述這兩個服務之間的主要差異，並反白顯示您的 IoT 解決方案中使用 IoT 中心的優點。
...
Azure IoT 中心會實作 [服務輔助通訊][lnk-service-assisted-pattern] 模式來協調您的裝置與您方案的後端之間的互動。 服務輔助通訊的目標是要在 IoT 中樞等控制系統，與部署在不受信任實體空間中的特殊用途裝置之間，建立可信任的雙向通訊路徑。 該模式會建立下列原則：
