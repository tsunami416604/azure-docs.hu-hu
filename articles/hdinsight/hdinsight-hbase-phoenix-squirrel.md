...
- **HDInsight 中的 HBase 叢集**。 如需有關佈建 HBase 叢集，請參閱 [開始使用 HDInsight 中的 Apache HBase][hdinsight-hbase-get-started]。
- **連接到 HBase 叢集，透過遠端桌面通訊協定**。 如需指示，請參閱 [使用 Azure 傳統入口網站在 HDInsight 中管理 Hadoop 叢集][hdinsight-manage-portal]。
...
2. 執行下列命令以取得 DNS 尾碼：
    ...
    請記下 **連線特定 DNS 尾碼**。 例如， *myhbasecluster.f5.internal.cloudapp.net*。 當您連線至 HBase 叢集時，您必須使用 FQDN 連線到其中一個 Zookeeper。 每個 HDInsight 叢集有 3 個 Zookeeper。 它們是 *zookeeper0*, ，*zookeeper1*, ，和 *zookeeper2*。 FQDN 將是類似 *zookeeper2.myhbasecluster.f5.internal.cloudapp.net*。
...
- 將 HBase 叢集部署至具備 DNS 虛擬機器的 Azure 虛擬網路。  如需指示，請參閱 [Azure 虛擬網路上的設定佈建 HBase 叢集][hdinsight-hbase-provision-vnet]。 

    >[AZURE.IMPORTANT] 您必須安裝 DNS 伺服器到虛擬網路。 如需指示，請參閱 [兩個 Azure 虛擬網路之間設定 DNS](hdinsight-hbase-geo-replication-configure-DNS.md)

- 取得 HBase 叢集的連線專用 DNS 尾碼。 若要取得該尾碼，請 RDP 到叢集，然後執行 IPConfig。  DNS 尾碼會類似於：

        myhbase.b7.internal.cloudapp.net
- 下載並安裝 [Microsoft Visual Studio Express 2013 for Windows Desktop](https://www.visualstudio.com/products/visual-studio-express-vs.aspx) 您的工作站上。 您將需要封裝的 makecert 以建立您的憑證。  
- 下載並安裝 [Java 執行階段環境](http://www.oracle.com/technetwork/java/javase/downloads/jre7-downloads-1880261.html) 您的工作站上。  SQuirreL SQL 用戶端 3.0 版和更新版本需要 JRE 1.6 版或更新版本。  
...
請參閱 [設定點對站台 VPN 連線到 Azure 虛擬網路](../vpn-gateway/vpn-gateway-point-to-site-create.md) 如需詳細資訊。
...
2. 在左側，按一下 [ **網路**。
3. 按一下您所建立的虛擬網路 (請參閱 [Azure 虛擬網路上的設定佈建 HBase 叢集][hdinsight-hbase-provision-vnet])。
4. 按一下 [ **設定** 頂端。
5. 在 **點對站連線能力** 區段中，選取 **設定點對站連線**。 
6. 設定 **起始 IP** 和 **CIDR** 來指定 VPN 用戶端接收連線時的 IP 位址的 IP 位址範圍。 此範圍不能與任何位於內部部署網路及您將連線之 Azure 虛擬網路的範圍重疊。 例如。 如果您選取 10.0.0.0/20 虛擬網路，您可以選取 10.1.0.0/24 用戶端的位址空間。 請參閱 [點對站連線能力][vnet-point-to-site-connectivity] 如需詳細資訊。
...
1. 從 Azure 傳統入口網站中，按一下 [ **儀表板** 從頁面頂端。
2. 按一下 [ **建立閘道** 從頁面底部。
3. 按一下 [ **是** 確認。 請等候建立閘道。
4. 按一下 [ **儀表板** 頂端。  您會看到虛擬網路的視覺化圖表：
...
若要建立 X.509 憑證的方法之一是使用憑證建立工具 (makecert.exe) 隨附 [Microsoft Visual Studio Express 2013 for Windows Desktop](https://www.visualstudio.com/products/visual-studio-express-vs.aspx)。 
...
1. 從 Azure 傳統入口網站中，按一下 [ **網路** 左邊。
2. 按一下 HBase 叢集部署所在的虛擬網路。
3. 按一下 [ **憑證** 頂端。
4. 按一下 [ **上載** 底部，並指定您已在前一個程序中建立的根憑證檔案。 請等到憑證匯入。
5. 按一下 [ **儀表板** 上面。  虛擬圖表會顯示狀態。
...
1. 從 [快速概覽] 區段中的虛擬網路的 [儀表板] 頁面按一下 [ **下載 64 位元用戶端 VPN 封裝** 或 **下載 32 位元用戶端 VPN 封裝** 根據您的工作站 OS 版本。
2. 按一下 [ **執行** 安裝封裝。
...
1. 下載 SQuirreL SQL 用戶端 jar 檔案從 [http://squirrel-sql.sourceforge.net/#installation](http://squirrel-sql.sourceforge.net/#installation)。
2. 開啟/執行 jar 檔案。 它需要 [Java 執行階段環境](http://www.oracle.com/technetwork/java/javase/downloads/jre7-downloads-1880261.html)。
3. 按一下 [ **下一步** 兩次。
4. 指定的路徑，您具有寫入權限，然後按一下 **下一步**。
