...
| 設定| 作用| 預設值|
| ------- | ------------ | ------------- |
| [yarn.scheduler.capacity.maximum 應用程式的 ][maximum-applications]| 可以同時為作用中 (擱置或執行中) 的工作數目上限| 10,000|
| [templeton.exec.max 程序的 ][max-procs]| 可以同時提供服務的要求數目上限| 20|
| [mapreduce.jobhistory.max 時代 ms ][max-age-ms]| 將保留作業歷程記錄的天數| 7 天|
...
| 原因| 解決方案|
| ----- | ---------- |
| 您已超出 WebHCat 每分鐘提供服務的並行要求上限 (預設值為 20)| 減少您的工作負載以確保您執行不會提交超過並行要求數目上限，或藉由修改提高並行要求限制 `templeton.exec.max procs`。請參閱 [修改組態](#modifying-configuration) 如需詳細資訊|
...
| 原因| 解決方案|
| ---------------- | ------------------- |
| 工作歷程記錄清除器已清除工作詳細資料| 工作歷程記錄的預設保留期間為 7 天。這可以藉由修改變更 `mapreduce.jobhistory.max 時代 ms`。請參閱 [修改組態](#modifying-configuration) 如需詳細資訊|
...
| 等候 ResourceManager 服務回應時逾時。這發生於作用中應用程式的數目達到設定的最大值時 (預設值為 10,000)| 等候目前執行的工作完成，或藉由修改提高並行工作限制 `yarn.scheduler.capacity.maximum 應用程式`。請參閱 [修改組態](#modifying-configuration) 如需詳細資訊|
| 嘗試擷取到的所有工作時 [GET /jobs](https://cwiki.apache.org/confluence/display/Hive/WebHCat+Reference+Jobs) 呼叫 `欄位` 設為  `*`| 無法擷取 *所有* 作業詳細資料，而是使用 `jobid` 擷取只有大於特定工作識別碼的工作詳細資料。或者，不要使用 `欄位`|
