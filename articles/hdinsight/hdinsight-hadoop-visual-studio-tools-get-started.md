...
- 已安裝下列軟體的工作站：
    ...
    >[AZURE.NOTE] 目前 HDInsight Tools for Visual Studio 只隨附的英文版。
...
- 從 [伺服器總管] 中，展開 [ **Azure** > **HDInsight**, 的 HDInsight 叢集上按一下滑鼠右鍵，然後按 **在 Azure 入口網站中管理叢集**。

**從 Visual Studio 提出問題及提供意見反應**

- 從 **工具** ] 功能表上，按一下 [ **HDInsight**, ，然後按一下 [ **MSDN 論壇** 提出問題，或按一下 **提供意見反應**。
...
### 檢視 **hivesampletable**
所有 HDInsight 叢集都隨附的範例 Hive 資料表，稱為 *hivesampletable*。 我們將使用此資料表來示範如何列出 Hive 資料表、檢視資料表結構描述，以及列出 Hive 資料表中的資料列。



**列出 Hive 資料表和檢視 Hive 資料表結構描述**

1.  從 **伺服器總管**, ，依序展開 **Azure** > **HDInsight** > 選擇的叢集 > **Hive 資料庫** > **預設** > **hivesampletable** 來查看資料表結構描述。
4.  以滑鼠右鍵按一下 **hivesampletable**, ，然後按一下 [ **檢視前 100 個資料列** 若要列出的資料列。 這相當於使用 Hive ODBC 驅動程式來執行下列 Hive 查詢：
...
1. 從 **伺服器總管**, ，依序展開 **Azure** > **HDInsight 叢集** HDInsight 叢集 > **Hive 資料庫**, ，然後以滑鼠右鍵按一下 **預設**, ，然後按一下 **Create Table**。
2. 設定資料表。
...
1. 從 **伺服器總管**, ，依序展開 **Azure**, ，然後展開 **HDInsight 叢集**。
...
1. 從 **伺服器總管**, ，依序展開 **Azure**, ，然後展開 **HDInsight**。
...
>[AZURE.NOTE] 此功能僅適用於 HDInsight 叢集 3.2 和更新版。
