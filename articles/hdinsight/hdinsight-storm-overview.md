...
* Integráció további Azure-szolgáltatásokkal, mint az Event Hub, az Azure Virtual Network, az SQL Database, a Blob Storage és a DocumentDB
  ...
  * Több HDInsight-fürt képességeit is kihasználhatja az Azure Virtual Network használatával: létrehozhat HDInsight-, HBase- vagy Hadoop-fürtöket használó elemzőfolyamatokat
...
A **Linux-alapú, HDInsight alatt futó Storm-fürtök** esetében a fürthöz az SSH használatával csatlakozhat, a topológiákat pedig a `storm` parancs használatával indíthatja el és kezelheti. Emellett az Ambari használatával figyelheti a Storm szolgáltatást, a Storm felhasználói felületén pedig figyelheti és kezelheti a futó topológiákat.
...
A **Windows-alapú, HDInsight alatt futó Storm-fürtök** esetében a HDInsight Tools for Visual Studio használatával hozhat létre C#- és hibrid C#/Java-topológiákat, majd küldheti el ezeket a HDInsight alatt futó Storm-fürtre.  
...
További információk a HDInsight Tools for Visual Studio szolgáltatásról: [A HDInsight Tools for Visual Studio használatának kezdeti lépései](hdinsight-hadoop-visual-studio-tools-get-started.md)
...
* **Spout**: Egy adatforrástól érkező adatok felhasználásával bocsát ki egy vagy több **adatfolyamot**.
  ...
  > [!NOTE]
  > Az adatok sok esetben egy üzenetsorból (pl. a Kafkából), Azure Service Bus-üzenetsorokból vagy eseményközpontokból lesznek beolvasva. Az üzenetsor garantálja, hogy az adatok kimaradás esetén is megőrződjenek.
  > 
  > 
...
A Storm-kezdőpéldák a Linux-alapú fürtökön a ** /usr/hdp/current/storm-client/contrib/storm-starter** könyvtárban, a Windows-alapú fürtökön pedig a **%storm_home%\contrib\storm-starter** könyvtárban találhatók.
