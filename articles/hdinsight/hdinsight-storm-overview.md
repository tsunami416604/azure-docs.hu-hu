...
* 使用您選擇的語言: 提供撰寫的 Storm 元件支援 **Java**, ，**C#**, ，和 **Python**

    * 支援多種程式設計語言: 使用 Java 讀取資料，則 C 進行處理#
    
        > [AZURE.NOTE] 在 Windows 為基礎的 HDInsight 叢集上才支援 C# 拓撲。

    * 使用 **Trident** Java 介面建立 Storm 拓撲，支援 「 只一次性 」 處理訊息、 「 交易式 」 資料存放區持續性，以及一組常用的資料流分析作業
...
__以 Linux 為基礎的 Storm on HDInsight 叢集__, ，您可以連接到使用 SSH 和使用的叢集 `storm` 命令來啟動和管理拓撲。 此外，您可以使用 Ambari 來監視 Storm 服務和 Storm UI，監視和管理執行的拓撲。
...
__Windows 型 Storm on HDInsight 叢集__, ，HDInsight Tools for Visual Studio 可讓您建立 C# 和混合式 C# / Java 拓撲中，然後將它們提交至 Storm on HDInsight 叢集。  
...
Storm on HDInsight 也提供透過 Azure 事件中樞輕鬆整合 **事件中樞 Spout**。 這是可以在每個 storm 叢集的 **%STORM_HOME%\examples\eventhubspout\eventhubs-storm-spout-0.9-jar-with-dependencies.jar**。 如需在 Storm 拓撲中使用此 Spout 的範例，請參閱下列文件：
 ...
 **Nimbus 節點** Hadoop jobtracker，提供類似的功能，它會將工作指派給透過叢集中其他節點 **Zookeeper**。 Zookeeper 節點可為叢集進行協調，並促進 Nimbus 之間的通訊和 **監督員** 處理序的背景工作節點。 如果其中一個處理節點停止，Nimbus 節點會收到通知，然後將工作和相關資料指派給其他節點。
...
> [AZURE.NOTE] 若要利用透過調整所加入的新節點，您必須重新平衡叢集大小增加之前啟動的拓撲。
...
Apache Storm 會執行 **拓撲** 而不是您可能會在 HDInsight 或 Hadoop 中熟悉的 MapReduce 工作。 Storm on HDInsight 叢集包含兩種節點: 前端節點執行 **Nimbus** 和執行的背景工作節點 **監督員**。

* **Nimbus**: 類似 Hadoop 中的 JobTracker，它會負責散發程式碼到整個叢集、 指派工作給虛擬機器及監控失敗。 HDInsight 提供兩個 Nimbus 節點，所以 Storm on HDInsight 不會有單一失敗點。

* **監督員**: 每個背景工作節點的監督員負責啟動和停止 **背景工作處理序** 節點上。

* **背景工作處理序**: 執行一部分 **拓樸**。 執行中的拓撲分散在整個叢集的許多背景工作處理序上。

* **拓撲**: 定義程序的計算圖 **資料流** 的資料。 與 MapReduce 工作不同之處在於，拓撲會一直執行到您將它們停止為止。

* **資料流**: 未繫結的集合 **tuple**。 資料流所產生的 **spout** 和 **bolt**, ，而且由 **bolt**。

* **Tuple**: 動態類型值的具名的清單。

* **Spout**: 取用資料來源的資料，並發出一或多個 **資料流**。

    > [AZURE.NOTE] 在許多情況下，從佇列中的，例如 Kafka、 Azure 服務匯流排佇列或事件中心讀取資料。 佇列可確保運作中斷時資料仍持續存在。

* **Bolt**: 取用 **資料流**, ，執行的處理 **tuple**, ，然後可能發出 **資料流**。 Bolt 也負責將資料寫入外部儲存體，例如佇列、HDInsight、HBase、Blob 或其他資料存放區。

* **Apache Thrift**: 可擴充的跨語言服務開發的軟體架構。 它可讓您建置 C++、Java、Python、PHP、Ruby、Erlang、Perl、Haskell、C#、Cocoa、JavaScript、Node.js、Smalltalk 和其他語言之間通用的服務。

    * **Nimbus** 是一種 Thrift 服務，和 **拓樸** 是 Thrift 定義，因此可利用各種程式設計語言來開發拓撲。
...
Storm-starter 範例位於 __ /usr/hdp/current/storm-client/contrib/storm-starter__ 目錄上以 Linux 為基礎的叢集，而 **%storm_home%\contrib\storm-starter** windows 叢集上的目錄。
...
在下列 Java 範例中，使用 fieldsGrouping 將源自元件"1"、"2"和"3"的 tuple **MyJoiner** bolt。
