...
> [AZURE.NOTE] 本文中的資訊主要適用於 Windows 架構的 HDInsight 叢集，該叢集提供 Ambari REST API 的唯讀版本。 對於以 Linux 為基礎的叢集，請參閱 [使用 Ambari 管理 Hadoop 叢集](hdinsight-hadoop-manage-ambari.md)。

## 什麼是 Ambari？

[Apache Ambari ][ambari-home] 用於佈建、 管理及監視 Apache Hadoop 叢集。 其中包含一組直接易懂的操作員工具和健全的 API 集，可消除 Hadoop 的複雜性，並簡化叢集作業。 如需這些 Api 的詳細資訊，請參閱 [Ambari API 參考 [ambari-api-][ambari-api-reference]。
...
- (選擇性) [cURL][curl]. 若要安裝它，請參閱 [cURL 釋出並下載 ][curl-download]。
    >[AZURE.NOTE] 在 Windows 上使用 cURL 命令時，請針對選項值使用雙引號，而不要使用單引號。

- **Azure HDInsight 叢集**。 如需叢集佈建的指示，請參閱 [開始使用 HDInsight ][hdinsight-get-started] 或 [佈建 HDInsight 叢集的 ][hdinsight-provision]。 進行教學課程時，您將需要下列資料：
...
下表列出部分最常用的 Ambari 監視 API 呼叫。 如需 API 的詳細資訊，請參閱 [Ambari API 參考 [ambari-api-][ambari-api-reference]。
...
- [管理 HDInsight 叢集使用 Azure PowerShell ][hdinsight-admin-powershell]
