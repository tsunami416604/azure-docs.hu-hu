...
2. 如 **要產生的金鑰類型**, ，請選取 **ssh-2 RSA**, ，然後按一下 [ **產生**。
...
4. 為提高安全性，您可以輸入中的複雜密碼 **金鑰複雜密碼** 欄位，並接著輸入中的相同值 **確認複雜密碼** 欄位。

    ![複雜密碼](./media/hdinsight-hadoop-linux-use-ssh-windows/key.png)

    > [AZURE.NOTE] 我們強烈建議您對此金鑰使用安全的複雜密碼。 不過如果您忘記此複雜密碼，將無法加以復原。

5. 按一下 [ **儲存私密金鑰** 來儲存到金鑰 **.ppk** 檔案。 此金鑰會用來驗證以 Linux 為基礎的 HDInsight 叢集。

    > [AZURE.NOTE] 它可以用來存取以 Linux 為基礎的 HDInsight 叢集，您應該儲存在安全的位置，此機碼。

6. 按一下 [ **儲存公開金鑰** 儲存為機碼 **.txt** 檔案。 這可讓您在日後建立其他以 Linux 為基礎的 HDInsight 叢集時重複使用此公開金鑰。

    > [AZURE.NOTE] PuTTYGen 的頂端，也會顯示公開金鑰。 當您使用 Azure 入口網站建立叢集時，可以用滑鼠右鍵按一下此欄位，複製值並將其貼入表單。
...
* **適用於 Mac、 Linux 和 Windows 的 azure CLI** -使用命令列命令來建立叢集。
...
您可以使用 [適用於 Mac、 Linux 和 Windows 的 Azure CLI](../xplat-cli-install.md) 來建立新叢集使用 `azure hdinsight cluster create` 命令。
...
4. 若要儲存連線資訊供日後使用，請輸入下這個連線的名稱 **儲存的工作階段**, ，然後按一下 [ **儲存**。 連線便會加入已儲存的工作階段清單。

5. 按一下 [ **開啟** 連線到叢集。

    > [AZURE.NOTE] 如果這是您已經連接到叢集的第一次，您會收到安全性警示。 這是正常現象。 選取 **是** 快取伺服器的 RSA2 金鑰以繼續。

6. 出現提示時，輸入您建立叢集時所輸入的使用者。 如果您對使用者提供密碼，系統會提示您一併輸入密碼。

> [AZURE.NOTE] 上述步驟假設您使用的連接埠 22，這將會連接到 HDInsight 叢集上的前端節點 0。 如果您使用連接埠 23，您將會連接到前端節點 1。 如需前端節點的詳細資訊，請參閱 [HDInsight 叢集的可用性和可靠性](hdinsight-high-availability-linux.md)。
...
2. 執行 Pageant。 它會最小化為狀態列中的圖示。 以滑鼠右鍵按一下圖示，然後選取 **新增機碼**。

    ![新增金鑰](./media/hdinsight-hadoop-linux-use-ssh-windows/addkey.png)

3. 瀏覽] 對話方塊出現時，選取包含金鑰的.ppk 檔案，然後按一下 [ **開啟**。 這樣會將金鑰新增至 Pageant，在連接至叢集時將金鑰提供給 PuTTY。

    > [AZURE.IMPORTANT] 如果您使用 SSH 金鑰來保護您的帳戶，您必須先完成前面的步驟，您將能夠連接至背景工作節點之前。

4. 開啟 PuTTY。

5. 如果您使用 SSH 金鑰來驗證，請在 **類別** 區段中，展開 **連接**, ，依序展開 **SSH**, ，然後選取 **Auth**。
...
7. 建立連接之後，請使用下列項目以擷取您的叢集中的節點清單。 取代 *ADMINPASSWORD* 取代為您的叢集系統管理員帳戶的密碼。 取代 *CLUSTERNAME* 與您的叢集名稱。
...
8. 當您具有想要連接之背景工作節點的清單之後，請從 PuTTY 工作階段使用下列命令，以開啟與背景工作節點的連線：
    ...
    取代 *USERNAME* SSH 使用者名稱和 *FQDN* 與背景工作節點的 FQDN。 例如，`workernode0.workernode-0-e2f35e63355b4f15a31c460b6d4e1230.j1.internal.cloudapp.net`。

    > [AZURE.NOTE] 如果您使用密碼來驗證您的 SSH 工作階段，會提示您再次輸入密碼。 如果您使用 SSH 金鑰，連線應該沒有任何提示即會完成。

9. 建立工作階段之後，針對 PuTTY 的提示會從 `username@hn0-clustername` 變更為 `username@wn0-clustername`，以指出您已連接至背景工作節點。 目前您執行的任何命令會在背景工作節點上執行。

10. 完成在背景工作節點上執行動作之後，請使用 `exit` 命令以關閉背景工作節點的工作階段。 這樣會帶您返回 `username@hn0-clustername` 提示字元。
...
> [AZURE.IMPORTANT] 需要存取某些 Hadoop 服務 web UI 的 SSH 通道。 例如，[作業記錄] UI 或 [資源管理員] UI 都只能使用 SSH 通道存取。
