...
> [AZURE.NOTE] 本文的資訊是以 Linux 為基礎的 HDInsight 叢集的特定資訊。 本文旨在說明 windows 叢集的版本，請參閱 [使用 (Windows) 的指令碼動作自訂 HDInsight 叢集](hdinsight-hadoop-customize-cluster.md)
...
2. 在 [選擇性組態]____ 的 [指令碼動作]**** 刀鋒視窗上，按一下 [加入指令碼動作]**** 以提供有關指令碼動作的詳細資料，如下所示：
...
3. 如果您有多個訂用帳戶，請提供您想要用於部署的訂用帳戶識別碼。
    ...
    > [AZURE.NOTE] 您可以使用 `Get AzureRmSubscription` 以取得您的帳戶，其中包含每個訂用帳戶 Id 相關聯的所有訂閱的清單。
...
5. 設定 SSH 認證：
    ...
    出現提示時，請輸入 SSH 使用者名稱和密碼。 如果您想要保護 SSH 帳戶，而不是密碼的憑證，請使用空白密碼，並設定 `$sshPublicKey` 的憑證公開金鑰您想要使用的內容。 例如：
...
4. 最後，建立叢集：
    ...
    如果您使用公用金鑰保護 SSH 帳戶，您也必須指定 `-SshPublicKey $sshPublicKey` 做為參數。
...
> [AZURE.IMPORTANT] 您必須先建立自我簽署憑證，將它安裝到工作站，然後上傳至 Azure 訂用帳戶。 如需指示，請參閱 [建立自我簽署的憑證](http://go.microsoft.com/fwlink/?LinkId=511138)。
...
* 儲存記錄檔位於 `\STORAGE_ACOCUNT_NAME\DEFAULT_CONTAINER_NAME\custom-scriptaction-logs\CLUSTER_NAME\DATE`。
...
- **內建元件** - 這些元件預先安裝在 HDInsight 叢集上，並且提供叢集的核心功能。 例如，YARN ResourceManager、Hive 查詢語言 (HiveQL) 及 Mahout 程式庫都屬於這個類別。 叢集元件的完整清單位於 [的 HDInsight 所提供的 Hadoop 叢集版本的新功能?](hdinsight-component-versioning.md).
...
- [安裝和使用 R HDInsight 叢集上](hdinsight-hadoop-r-scripts-linux.md)
