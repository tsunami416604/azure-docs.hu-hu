...
1. A MapReduce program fejlesztése, lefordítása és tesztelése után a `scp` parancs használatával töltse fel a jar-fájlt a átjárócsomóponthoz.
    ...
    Cserélje le az CLUSTERNAME-t a fürt nevére. Ha az SSH-fiók védelméhez jelszót használt, a rendszer kéri, hogy adja meg a jelszót. Tanúsítvány használata esetén előfordulhat, hogy a `-i` paraméterrel kell megadnia a titkos kulcs fájlját.
...
1. Az SSH-munkamenetből hajtsa végre a MapReduce programot a FONALon keresztül.
    ...
    Ez a parancs a MapReduce-feladatot a FONALba küldi. A bemeneti fájl `/example/data/sample.log`, és a kimeneti könyvtár `/example/data/logoutput`. A bemeneti fájl és a kimeneti fájlok a fürt alapértelmezett tárolójában tárolódnak.
