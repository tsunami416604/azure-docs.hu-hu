...
Az ebben a szakaszban szereplő parancsokra cserélje le a `USERNAME`t a fürtre való hitelesítésre, majd cserélje le a `PASSWORD`t a felhasználói fiók jelszavára. Cserélje le a `CLUSTERNAME` elemet a fürt nevére.
...
1. A feladatok állapotának megtekintéséhez használja a következő parancsot. Cserélje le a `JOBID` értéket az előző lépésben visszaadott értékre. Ha például a visszatérési érték `{"id":"job_1415651640909_0026"}`, akkor `JOBID` `job_1415651640909_0026`. Szükség szerint módosítsa `jq` helyét.
...
1. Ha a művelet állapota **sikeresen**módosult, az Azure Blob Storage-ból kérheti le a feladatok eredményeit. A lekérdezéssel átadott `statusdir` paraméter tartalmazza a kimeneti fájl helyét; Ebben az esetben `wasb:///example/data/sqoop/curl`. Ez a címe a HDInsight-fürt által használt alapértelmezett Storage-tároló `example/data/sqoop/curl` könyvtárában tárolja a feladatot.
...
* Kötegelt feldolgozás – a Linux-alapú HDInsight, amikor a `-batch` kapcsolót használja a lapkák végrehajtásakor, a Sqoop több beszúrást hajt végre a beszúrási műveletek kötegelt feldolgozása helyett.
