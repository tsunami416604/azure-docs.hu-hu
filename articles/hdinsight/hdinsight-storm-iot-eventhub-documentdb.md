...
將感應器新增至車輛可讓您根據歷程記錄資料趨勢預測設備問題，並且根據使用模式分析增強未來版本的功能。 雖然傳統 MapReduce 批次處理可以用來進行這項分析，但您必須能在 MapReduce 處理發生之前，快速且有效地將來自所有車輛的資料載入 Hadoop。 此外，您可能想要進行分析的嚴重失敗路徑 (引擎溫度、 煞車等) 即時。
...
完成的自動化解決方案，可做為此案例中為屬於 <a href="https://github.com/hdinsight/hdinsight-storm-examples" target="_blank">Hdinsight-storm-範例</a> GitHub 上的儲存機制。 若要使用此範例中，依照 [IoTExample 讀我檔案。MD](https://github.com/hdinsight/hdinsight-storm-examples/blob/master/IotExample/README.md).
