...
Azure HDInsight 在雲端部署和佈建受管理的 Apache Hadoop 叢集，藉此提供軟體架構來管理、分析和報告具高可靠性和可用性的巨量資料。 HDInsight 使用 **HortonWorks Data Platform (HDP)** Hadoop 散發。 Hadoop 通常是指元件的整個 Hadoop 生態系統中的元件，包括 Apache HBase、Apache Spark 和 Apache Storm，以及 Hadoop 範疇內的其他技術。 請參閱 [HDInsight 上的 Hadoop 生態系統概觀](#overview) 如下如需詳細資訊。
...
## <a name="overview"></a>在 HDInsight 上的 Hadoop 生態系統概觀
...
* * *[Hive 和 HCatalog](#hive)* *: 結構化查詢語言 (SQL) 的類似查詢，以及資料表和儲存體管理層。
...
Microsoft .NET Library for Avro 針對 Microsoft .NET 環境的序列化，實作 Apache Avro 壓縮二進位資料交換格式。它會使用 <a target="_blank" href="http://www.json.org/">JavaScript 物件標記法 (JSON)</a> 若要定義不語言互通性的無從驗證語言的結構描述，以某種語言序列化的意義資料可以讀取在另一個。格式的詳細的資訊可在 <a target=_"blank" href="http://avro.apache.org/docs/current/spec.html">Apache Avro 規格</a>。
...
<a target="_blank" href="http://hive.apache.org/">Apache Hive</a> 是資料倉儲軟體，可讓您查詢和管理分散式儲存體中的大型資料集使用類似 SQL 的語言，稱為 HiveQL 的 Hadoop 為基礎。 Hive 和 Pig 一樣，是以 MapReduce 為基礎的抽象。 Hive 會在執行時將查詢轉換為一系列的 MapReduce 工作。 Hive 在概念上比 Pig 更接近關聯式資料庫管理系統，因此，適用於更具結構化的資料。 對於非結構化資料，Pig 是較好的選擇。 請參閱 [搭配使用 Hive 與 HDInsight 中 Hadoop](hdinsight-use-hive.md)。
...
<a target="_blank" href="https://mahout.apache.org/">Apache Mahout</a> 是 Hadoop 執行的機器學習演算法的可擴充程式庫。 機器學習應用程式採用統計學的原理，教導系統從資料中學習，根據過去的結果來研判未來的行為。 請參閱 [Hadoop 上使用 Mahout 產生電影推薦](hdinsight-mahout.md)。
...
<a  target="_blank" href="http://pig.apache.org/">Apache Pig</a> 是高階的平台，可讓您將使用稱為 Pig Latin 的簡單指令碼語言，非常大型資料集執行複雜的 MapReduce 轉換。 Pig 會將 Pig Latin 指令碼轉換成可在 Hadoop 內執行。 您可以建立使用者定義函數 (UDF) 來延伸 Pig Latin。 請參閱 [使用 Pig 和 Hadoop 來分析 Apache 記錄檔](hdinsight-use-pig.md)。

### <a name="sqoop"></a>Sqoop

<a  target="_blank" href="http://sqoop.apache.org/">Apache Sqoop</a> 是傳輸大量 Hadoop 與 SQL 之類的關聯式資料庫之間的資料或其他結構化的資料存放區中，盡可能以最有效率的工具。 請參閱 [搭配 Hadoop 使用 Sqoop](hdinsight-use-sqoop.md)。

### <a name="tez"></a>Tez

<a  target="_blank" href="http://tez.apache.org/">Apache Tez</a> 是執行一般資料處理的複雜循環的 Hadoop YARN 為建置基礎的應用程式架構。 它是更有彈性且功能更強大的 MapReduce 架構後繼版本，可讓資料密集程序 (例如 Hive) 更有效率地大規模執行。 請參閱 [」 使用 Apache Tez 以提升效能 > 中使用 Hive 和 HiveQL](hdinsight-use-hive.md#usetez)。


### <a name="zookeeper"></a>ZooKeeper

<a  target="_blank" href="http://zookeeper.apache.org/">Apache ZooKeeper</a> 資料暫存器 (znode) 的共用階層式命名空間，協調大型分散式系統中的程序。 Znodes 包含協調程序所需的少量中繼資訊：狀態、位置、組態等。

## <a name="advantage"></a>Hadoop 在雲端中的優點

在 Azure 雲端生態系統內，HDInsight 中的 Hadoop 提供許多優點，包括：

* 自動佈建 Hadoop 叢集。 HDInsight 叢集比手動設定 Hadoop 叢集更容易建立。 如需詳細資訊，請參閱 [HDInsight 中佈建 Hadoop 叢集](hdinsight-hadoop-provision-linux-clusters.md)。
...
若要深入了解 HDInsight 中的 Hadoop 優點，請參閱  [hdinsight ][marketing-page]。
...
* [Linux 上佈建 HDInsight](hdinsight-hadoop-provision-linux-clusters.md): 了解如何佈建 HDInsight Hadoop 叢集，透過 Azure 入口網站、 Azure CLI 或 Azure PowerShell 在 Linux 上的。

* [Linux 上使用 HDInsight](hdinsight-hadoop-linux-information.md): 取得一些快速提示工作之 Hadoop Linux 叢集在 Azure 上佈建。
...
* <a target="_blank" href="http://hadoop.apache.org/">Apache Hadoop</a>: 深入了解 Apache Hadoop 軟體程式庫，可讓電腦叢集大型資料集分散式處理的架構。

* <a target="_blank" href="http://hadoop.apache.org/docs/r1.0.4/hdfs_design.html">HDFS</a>: 深入了解 Hadoop 分散式檔案系統中，使用 Hadoop 應用程式的主要儲存系統的設計與架構。

* <a target="_blank" href="http://hadoop.apache.org/docs/r1.2.1/mapred_tutorial.html">MapReduce 教學課程</a>: 深入了解撰寫 Hadoop 應用程式快速處理大型運算節點叢集上大量的資料平行程式設計架構。

### Azure 上的 SQL Database

* [Azure SQL Database](/documentation/services/sql-database/): 文件、 教學課程和影片 SQL 資料庫。

* [Azure 入口網站上的 SQL Database](sql-database-manage-portal.md): 管理 SQL 資料庫的定域機組中套輕量型且容易使用的資料庫管理工具。

* [Adventure Works for SQL Database](http://msftdbprodsamples.codeplex.com/releases/view/37304): SQL Database 範例資料庫的下載頁面。
...
* [Excel 連接到 Hadoop 中使用 Power Query](hdinsight-connect-excel-power-query.md): 了解如何將 Excel 連接到儲存 HDInsight 叢集相關聯，使用 Microsoft Power Query for Excel 的資料的 Azure 儲存體帳戶。

* [Excel 連接到 Hadoop 中使用 Microsoft Hive ODBC 驅動程式](hdinsight-connect-excel-hive-ODBC-driver.md): 了解如何使用 Microsoft Hive ODBC 驅動程式從 HDInsight 匯入資料。
...
* [使用 Hive 與 HDInsight 來分析網站記錄](hdinsight-hive-analyze-website-log.md): 了解如何使用 HDInsight 中的 HiveQL 來分析網站記錄，以深入的造訪次數，一天之中來自外部網站，以及使用者遭遇網站錯誤的摘要。
