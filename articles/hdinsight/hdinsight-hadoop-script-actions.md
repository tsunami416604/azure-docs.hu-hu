...
> 
>     System.Management.Automation.CommandNotFoundExceptionExceptionMessage: 詞彙 ' 儲存 HDIFile' 無法辨識為 cmdlet、 函式、 指令碼檔案，或可執行的程式名稱。 請檢查名稱，或如果包含路徑的話，確認路徑正確，然後再試一次。
...
HDInsight 提供數個指令碼在 HDInsight 叢集上安裝其他元件：
 ...
 名稱| 指令碼
----- | -----
 **安裝 Spark**| https://hdiconfigactions.blob.core.windows.net/sparkconfigactionv03/spark-installer-v03.ps1。請參閱 [上安裝和使用 Spark HDInsight 叢集的 ][hdinsight-install-spark]。
 **安裝 R**| https://hdiconfigactions.blob.core.windows.net/rconfigactionv02/r-installer-v02.ps1。請參閱 [安裝和使用 R ][hdinsight-r-scripts]。
...
您可以從 Azure 入口網站、Azure PowerShell 或使用 HDInsight .NET SDK 來部署「指令碼動作」。 如需詳細資訊，請參閱 [使用 ][hdinsight-cluster-customize]。
 ...
 **Get-Service**| 做為輸入特定服務名稱，來取得特定服務的詳細的資訊 (服務名稱、 處理序識別碼、 狀態等等)。上執行指令碼。
 **Get-HDIServices**| 取得執行指令碼的電腦上所執行之 HDInsight 服務的清單。
 **Get-HDIService**| 特定 HDInsight 服務名稱做為輸入，來取得特定服務的詳細的資訊 (服務名稱、 處理序識別碼、 狀態等等)。上執行指令碼。
...
此陳述式將環境變數 **MDS_RUNNER_CUSTOM_CLUSTER** 設為 'true' 值，並將此變數的範圍設為整個機器。 有時候您必須將環境變數設定在適當範圍 – 機器或使用者。 請參閱 [這裡 ][1] 如需有關設定環境變數。
...
**安裝 HDInsight Emulator** - 若要在本機執行指令碼動作，您必須安裝 HDInsight Emulator。 如需如何安裝指示，請參閱 [開始使用 HDInsight Emulator](../hdinsight-get-started-emulator/)。
...
- [自訂 HDInsight 叢集使用指令碼動作的 ][hdinsight-cluster-customize]
- [安裝和使用 Spark HDInsight 叢集的 ][hdinsight-install-spark]
- [安裝和使用 R 在 HDInsight 叢集的 ][hdinsight-r-scripts]
- [安裝和使用 HDInsight 叢集上 Solr](hdinsight-hadoop-solr-install.md)。
- [Giraph 的 HDInsight 上的叢集安裝和使用](hdinsight-hadoop-giraph-install.md)。
