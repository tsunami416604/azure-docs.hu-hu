...
* **ssh-keygen** 和 **ssh** Linux、 Unix 和 OS X 用戶端。 作業系統通常會隨附提供此公用程式，或是可透過封裝管理系統來取得。
...
如果您選取 **SSH 公開金鑰**, ，您可以貼上的公開金鑰 (與檔案中包含 **.pub** 延伸模組) 到 __SSH PublicKey__ 欄位，或選取 __選取檔案__ 瀏覽並選取公開金鑰檔案。
...
您可以使用 [適用於 Mac、 Linux 和 Windows 的 Azure CLI](../xplat-cli.md) 來建立新叢集使用 `azure hdinsight cluster create` 命令。
...
* **SSH 位址** -叢集名稱，後面接著 **-ssh.azurehdinsight.net**。 例如， **mycluster-ssh.azurehdinsight.net**。

* **使用者名稱** -的 SSH 使用者名稱建立叢集時提供。

下列範例會連接到叢集 **mycluster** 的使用者身分 **我**:
...
> [AZURE.NOTE] 如果 SSH 不自動以正確的私密金鑰進行驗證，使用 **-i** 參數並指定私密金鑰的路徑。 下列範例會從 `~/.ssh/id_rsa` 載入私密金鑰：
>
...
2. 將下列項目新增至檔案。 取代 *CLUSTERNAME* 與 HDInsight 叢集的名稱。
...
> [AZURE.IMPORTANT] 如果您使用 SSH 金鑰來驗證您的帳戶，您必須完成上述步驟，確認代理程式轉送正常運作。

1. 使用 SSH 連接到 HDInsight 叢集，如先前所述。

2. 連接之後，請使用下列項目以擷取您的叢集中的節點清單。 取代 *ADMINPASSWORD* 取代為您的叢集系統管理員帳戶的密碼。 取代 *CLUSTERNAME* 與您的叢集名稱。
...
1. 產生新的公開金鑰和私密金鑰的新使用者帳戶中所述 [建立 SSH 金鑰](#create-an-ssh-key-optional) 一節。
    ...
    > [AZURE.NOTE] 使用者將用來連接到叢集，或在建立後安全地傳輸到用戶端的用戶端上應該產生的私密金鑰。
