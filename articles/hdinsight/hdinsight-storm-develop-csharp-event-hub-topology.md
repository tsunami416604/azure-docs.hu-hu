...
[AZURE。附註] 這份文件中的步驟只會使用以 Windows 為基礎的 HDInsight 叢集。 Java 版本，此專案，將會使用 Linux 或 Windows 為基礎的叢集，請參閱 [處理 Azure 事件中心與 Storm on HDInsight (Java) 的事件](hdinsight-storm-develop-java-event-hub-topology.md)。
...
> [AZURE.NOTE] 已提交 Spout 和 Bolt，以將其納入 Apache Storm 專案中。 如需詳細資訊，請參閱 <a href="https://github.com/apache/storm/pull/336/files">storm-583: 初始簽入 storm 事件中樞</a> GitHub 中。
...
2. 在 [專案屬性中，選取 [ **設定**, ，然後選取 * * 這個專案不會包含預設的設定檔。 按一下此處以建立 one.* *
...
2. 開啟 **Program.cs** 檔案，並加入下列程式碼之後立即 `TopologyBuilder topologyBuilder = 新 TopologyBuilder("EventHubWriter");` 列。

        int partitionCount = Properties.Settings.Default.EventHubPartitionCount;
        List<string> javaDeserializerInfo =
            new List<string>() { "microsoft.scp.storm.multilang.CustomizedInteropJSONDeserializer", "java.lang.String" };

    第一行會從稍早定義的屬性中讀取資料分割計數。 第二行會定義用來將 spout 所產生的 JSON 資料還原序列化程式 `java.lang.String` 以便 Java 元件可以使用該資料。
...
5. 在先前的程式碼後面緊接著加入下列程式碼：
    ...
    這會為 Java Bolt 建立新的建構函式，在執行階段時可用來設定 Bolt 的新執行個體。 在此情況下，您會使用 <a href="http://storm.apache.org/documentation/Clojure-DSL.html" target="_blank">Apache Storm Clojure DSL</a> 來設定 spout 您先前加入的事件中樞組態資訊。 更具體來說，HDInsight 會在執行階段使用此程式碼執行下列動作：
...
3. 輸入下列設定。 使用您稍早在 [值]**** 資料行中所建立的事件中樞和儲存體帳戶資訊。
   ...
   <table>
    ...
    如 **StorageConnection**, ，輸入值為 `DefaultEndpointsProtocol = https;AccountName = myAccount;AccountKey = myKey;`。 將 **myAccount** 和 **myKey** 取代為稍早取得的儲存體帳戶名稱和金鑰。
...
2. 開啟 **Program.cs** 檔案，並加入下列程式碼之後立即 `TopologyBuilder topologyBuilder = 新 TopologyBuilder("EventHubReader");` 行:
...
5. 尋找下列程式碼：
    ...
    這會指示拓撲建立新的事件中樞 spout 並使用 `EventHubSpoutConfig` 上一個步驟的組態。 "EventHubSpout"可設定 spout 的易記名稱和 `partitionCount` 用於設定平行處理原則提示。 這會在幕後使用提供的組態資訊來建立 **com.microsoft.eventhubs.spout.EventHubSpout** Java 元件的新執行個體。
...
2. 找出 **Execute** 方法並取代為下列內容：
 ...
 將實體插入資料表之後, `ack ()` tuple，呼叫以通知 spout 我們已成功處理資料。
