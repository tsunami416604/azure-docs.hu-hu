...
>
> REST API 透過安全 [基本存取驗證](http://en.wikipedia.org/wiki/Basic_access_authentication)。 您應該一律使用安全 HTTP (HTTPS) 提出要求，確保認證安全地傳送至伺服器。
...
1. 從命令列中，使用下列命令來確認您可以連線到 HDInsight 叢集：
    ...
    * **-G**：指出這是 GET 要求
    ...
    URL 開頭 **https://CLUSTERNAME.azurehdinsight.net/templeton/v1**, ，將會是相同的所有要求。 路徑 **/status** 指出要求是要傳回伺服器之 WebHCat (也稱為 Templeton) 的狀態。
...
工作狀態變更為 [**成功**] 之後，即可從 Azure Blob 儲存體擷取工作結果。  `Statusdir` 隨查詢一起傳送的參數包含輸出檔案的; 在此案例中，位置 **wasb: / 範例/pigcurl**。 此位址會將工作輸出儲存至 HDInsight 叢集所使用之預設儲存體容器的 **example/pigcurl** 目錄中。

您可以列出並下載這些檔案使用 [適用於 Mac、 Linux 和 Windows 的 Azure CLI](../xplat-cli-install.md)。 例如，若要列出 **example/pigcurl** 中的檔案，請使用下列命令：
...
> [AZURE.NOTE] 您必須指定包含 blob 使用儲存體帳戶名稱 `-` 和 `-k` 參數或組 **AZURE\_STORAGE\_ACCOUNT** 和 **AZURE\_STORAGE\_ACCESS\_KEY** 環境變數。
