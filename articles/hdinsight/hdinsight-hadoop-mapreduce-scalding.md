...
* **HDInsight 叢集上 Windows 或 Linux 架構的 Hadoop**。 請參閱 [HDInsight 上佈建以 Linux 為基礎的 Hadoop](hdinsight-hadoop-provision-linux-clusters.md) 或 [HDInsight 上佈建 Windows 架構的 Hadoop](hdinsight-provision-clusters.md) 如需詳細資訊。
...
3. 連接到前端節點之後，請使用下列命令來執行字數統計工作
    ...
    這會執行您稍早實作的 WordCount 類別。 `-hdfs` 會指示工作使用 HDFS。 `-輸入` 指定輸入的文字檔，而 `-輸出` 指定輸出位置。
...
> [AZURE.NOTE] 下列步驟會使用 Windows PowerShell。 執行 MapReduce 工作的其他方法，請參閱 [在 HDInsight 上的 Hadoop 使用 MapReduce](hdinsight-use-mapreduce.md)。
...
6. 完成工作時，會將輸出下載到目前目錄中的 __output.txt__ 檔。 使用下列命令來顯示結果。
