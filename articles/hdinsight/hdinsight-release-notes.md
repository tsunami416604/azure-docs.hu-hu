...
|-------------------------------------------------|------------------------------------------------------|---------------------------------------------------------|-----------------------------------------------------|----------------------|
| 新增 HDInsight 3.3 版並更新所有 HDInsight 叢集的 HDInsight 版本 | 在此版本中，我們新增了 HDInsight v3.3 (以 HDP 2.3 為基礎) 並且更新了其他 HDP 版本。 HDP 2.3 版本資訊位於 [這裡](http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.3.0/bk_HDP_RelNotes/content/ch_relnotes_v230.html) 可以找到在 HDInsight 上的詳細資訊和 [這裡](https://azure.microsoft.com/en-us/documentation/articles/hdinsight-component-versioning/)。| 服務    | 全部| N/A
...
|變更 VM 名稱格式以在單一虛擬網路中的 Linux 叢集上部署多個 HDInsight。 | 在這個版本中已加入在單一虛擬網路中部署多個 HDInsight Linux 叢集的支援。 這個過程，叢集中的虛擬機器名稱的格式已從 headnode\ *、 workernode\ * 和 zookeepernode\ * hn\ *、 wn\ * 和 zk\ * 分別。 不建議採取虛擬機器名稱格式的直接相依性，因為這樣會受限於變更。 請使用本機機器或 Ambari API 上的 "hostname -f" 以判斷主機清單和元件到主機的對應。 您可以找到詳細資訊，在 [https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/hosts.md](https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/hosts.md) 和 [https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/host-components.md](https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/host-components.md)。 | 服務 | Linux 上的 HDInsight 叢集 | N/A |
| 組態變更 | HDInsight 3.1 叢集，現在會啟用下列設定: <ul><li>tez.yarn.ats.enabled 和 yarn.log.server.url。 這可讓應用程式時間軸伺服器和記錄伺服器能夠處理出記錄檔。</li></ul>HDInsight 3.2 叢集的已修改下列設定: <ul><li>mapreduce.fileoutputcommitter.algorithm.version 已設定為 2。 這可讓使用 FileOutputCommitter V2 版本。</li></ul> | 服務 | 全部 | N/A |
...
<td>雲端服務現在會開啟叢集例如上的 5 個連接埠 8001 至 8005 https://<clustername>.azurehdinsight.net:8001/。 對這些 URL 的要求會使用相同的基本驗證密碼機制驗證做為連接埠 443。 這些連接埠會繫結至作用中前端節點上的相同連接埠。 指令碼動作可讓客戶在前端節點和叢集以外的主機路由的這些連接埠上接聽的服務。</td>
<td>服務雲端</td>
<td>全部</td>
<td>N/A</td>
</tr>
...
<td>修正導致非經常性工作失敗的大型叢集 Shuffle 的間歇性罕見情況，條件。 請參閱 [] <a href="https://issues.apache.org/jira/browse/MAPREDUCE-6361" target="_blank">MAPREDUCE-6361</a> 以獲得詳細資訊。</td>
<td>Hadoop 核心</td>
<td>全部</td>
...
<td>可用版本的 Hortonworks 版本資訊 <a href="http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.1.15-Win/bk_releasenotes_HDP-Win/content/ch_relnotes-HDP-2.1.15.html" target="_blank">開始</a>.</td>
<td>HDP</td>
<td>全部</td>
<td>N/A</td>
</tr>
...
<tr>
<td>HDP 2.2 更新</td>
<td>這一版的 HDInsight 3.2 包含 HDP 2.2.6，並在 HDInsight 中加入數個重要的 Bug 修正程式。 完整的版本，請參閱 <a href="http://dev.hortonworks.com.s3.amazonaws.com/HDPDocuments/HDP2/HDP-2.2.6/HDP_RelNotes_v226/index.html">HDP 2.2.6 版本資訊</a>.</td>
<td>HDP</td>
<td>所有叢集類型</td>
<td>N/A</td>
</tr>
...
<td>加入 <b>KnownTypeAttribute</b> 方法 <b>Knowntypeattribute</b>. 當 GetAllKnownTypes 方法的類型為 Null 時已修正的 NullReferenceException。</td>
<td>SDK</td>
<td>Hadoop</td>
<td>N/A</td>
</tr>
...
<td>HDInsight 3.2 版現在使用來自 Apache Hadoop 的 WASB 檔案系統驅動程式的最新程式碼。 透過這項變更，WASB 驅動程式現在封裝為不同的
jar。 這純粹是封裝變更，不包含對 WASB 驅動程式行為的任何變更。 此 JAR 檔的名稱是 hadoop-azure-2.6.0.jar。</td>
<td>HDP</td>
<td>全部</td>
<td>N/A</td>
</tr>
...
<td>HDInsight 3.2 版的這項更新包含數個 Bug 修正，且封裝為包含在 HDP 的一些內部 jar 也已升級。 請注意，這些 JAR 檔案
是內部 HDP 封裝並不供客戶應用程式直接使用。 應用程式應該封裝自己的 JAR 版本，如此升級 HDP 內部 JAR 時才不會中斷客戶應用程式。</td>
<td>HDP</td>
<td>全部</td>
<td>N/A</td>
</tr>
...
* HDInsight 3.2.3.488.1375841 (HDP-hdp-2.2.10.0-2340-未變更)
* SDK 1.5.0 (不變)
...
* HDInsight 3.2.3.471.1342507 (HDP hdp-2.2.10.0-2340-未)
* SDK 1.5.0
...
<td>HDInsight 3.2 叢集提供 Hadoop 2.6/HDP2.2。 它包含對所有開放原始碼元件的重大更新。 如需詳細資訊，請參閱什麼是 HDInsight 的新功能， <a href ="http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.2.0/HDP_2.2.0_Release_Notes_20141202_version/index.html" target="_blank">HDP 2.2.0.0 版本資訊</a>.</td>
<td>開放原始碼軟體</td>
<td>全部</td>
<td>N/A</td>
</tr>
...
<td><p>這個大小組態適用於自動轉換的對應聯結。 值代表可以轉換成雜湊對應且可放入記憶體的資料表大小總和。 在前版中，這個值從預設值 10 MB 增加到 128 MB。 但是新的 128 MB 值因為缺乏記憶體而導致工作失敗。 此版本將預設值還原成 10MB。 客戶仍然可以依據其查詢和資料表大小選擇在叢集建立期間覆寫此值。 如需此設定，以及如何覆寫它的詳細資訊，請參閱 <a href="http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.0.0.2/ds_Hive/optimize-joins.html#JoinOptimization-OptimizeAutoJoinConversion" target="_blank">將最佳化自動聯結轉換</a> Hortonworks 文件。 </p></td>
<td>Hive</td>
<td>Hadoop、Hbase</td>
<td>N/A</td>
</tr>
...
<td><p>自訂可讓您自訂的專案，都是從 Apache Hadoop 生態系統 Azure HDInsight 叢集。 有了這項新功能，您可以實驗並部署 Hadoop 專案至 Azure HDInsight。 這透過啟用 **指令碼動作** 功能，可以以任意方式修改 Hadoop 叢集使用自訂指令碼。 此自訂可在所有類型的 HDInsight 叢集上使用，包括 Hadoop、HBase 和 Storm。 為了示範此功能的威力，我們已記錄安裝常用的程序 <a href = "hdinsight-hadoop-spark-install.md" target="_blank">Spark</a>, ，<a href = "hdinsight-hadoop-r-scripts.md" target="_blank">R</a>, ，<a href = "hdinsight-hadoop-solr-install.md" target="_blank">Solr</a>, ，和 <a href = "hdinsight-hadoop-giraph-install.md" target="_blank">Giraph</a> 模組。 此版本也加入讓客戶透過 Azure 入口網站指定自訂指令碼動作的功能、提供關於如何使用協助程式方法建置自訂指令碼動作的方針和最佳實務作法，以及提供如何測試指令碼動作的方針。 </p></td>
<td>功能公開上市</td>
<td>所有</td>
<td>n/A</td>
</tr>
...
<td>若為 hdi 叢集 3.0 版，那里一些 HDP 所安裝之內部 JAR 檔的變更。 jetty-6.1.26.jar 已取代為 jetty-util-6.1.26.hwx.jar jetty。 jetty jetty-util-6.1.26.jar 已取代 jetty 為 jetty-util-6.1.26.hwx.jar。 這些變更適用於 Hadoop、Mahout、WebHCat 和 Oozie 專案。</td>
...
<tr>
<td>在 IHdInsightClient.DeleteCluster 中指定區域名稱的能力 </td>
<td>Azure HDInsight SDK 提供了能夠使用時，指定區域名稱 **DeleteCluster**。 這有助於解除封鎖在不同地區擁有兩個具有相同名稱的資源，而且已無法刪除其中的客戶。</td>
<td>SDK</td>
<td>所有</td>
<td>n/A</td>
</tr>
...
<td> **ClusterDetails** 物件會傳回 **DeploymentID** 欄位來代表叢集的唯一識別碼。 一定會在叢集建立嘗試以相同的名稱是唯一的。</td>
<td>SDK</td>
<td>所有</td>
<td>n/A</td>
</tr>
...
<tr>
<td>預先建立的工作，Azure 網站和儲存體記錄分析</td>
<td>HDInsight 查詢主控台有入門庫支援解決方案，可在您的資料或範例資料。
<p>**您的資料上運作的方案**:<br>
我們已經為部分最常見的資料分析案例建立工作，以做為建立您自己的方案的起點。 您可以執行工作來使用您的資料，以查看其運作方式。 接著當您準備好時，使用您已經學會如何建立預先建立的工作之後所建立的方案。</p>
<p>**範例資料上運作的方案**:<br>
逐步執行部分基本案例 (例如分析 Web 記錄和感應器資料) 來了解如何使用 HDInsight。 您將了解如何使用 HDInsight 來分析此類資料，以及如何將其他應用程式和服務連接至此資料。 視覺化資料，藉由連接到 Microsoft Excel 提供的強大功能，這種方法的範例。</p></td>
<td>查詢主控台</td>
<td>Hadoop</td>
<td>n/A</td>
</tr>
...
**請注意**: 為了示範叢集自訂所提供的新功能，已記錄使用指令碼動作在叢集上安裝 Spark 和 R 模組的程序。 如需進一步資訊，請參閱：
...
擷取此資訊也會從遠端透過 REST API 在 https://{ClusterDnsName}。 azurehdinsight.net/ws/v1/applicationhistory/。
...
**請注意**: 在此版本中已修正防止從出現在入口網站或由 SDK 或 Windows PowerShell 傳回的完整版本號碼的 bug。
...
* 當使用 Ambari 端點，「 https://{clusterDns}.azurehdinsight.net/ambari/api/v1/clusters/{clusterDns}.azurehdinsight.net/services/{servicename}/components/{componentname}"， *host_name* 欄位會傳回節點，而不是只有主機名稱的完整的網域名稱 (FQDN)。 例如，而不是傳回"**headnode0**」，您會得到 FQDN"**headnode0。 {ClusterDNS}.azurehdinsight.net.net**」。 需要此變更才能協助在一個虛擬網路中部署多種叢集類型 (例如 HBase 和 Hadoop) 的情況。 例如，使用 HBase 做為 Hadoop 的後端平台時就是這種情形。
...
<tr><td>mapreduce.reduce.memory</td><td>1，536MB (先前為 1024 MB)</td></tr>
<tr><td>mapreduce.reduce.java.opts</td><td>opts=-Xmx1024m (先前是 -Xmx819m)</td></tr>
<tr><td>yarn.app.mapreduce.am.resource</td><td>768MB (先前為 1024 MB)</td></tr>
...
關於 Azure PowerShell 和 HDInsight SDK 錯誤訊息: 「*叢集未設定 HTTP 服務存取*」:

* 此錯誤是已知 [相容性問題](https://social.msdn.microsoft.com/Forums/azure/a7de016d-8de1-4385-b89e-d2e7a1a9d927/hdinsight-powershellsdk-error-cluster-is-not-configured-for-http-services-access?forum=hdinsight) ，起因於 HDInsight SDK 或 Azure PowerShell 的版本和叢集的版本差異。 在 8/15 或之後建立的叢集支援佈建到虛擬網路的這項新功能。 但舊版的 HDInsight SDK 或 Azure PowerShell 無法正確解譯此功能。 結果造成某些工作提交作業失敗。 如果您使用 HDInsight SDK Api 或 Azure PowerShell cmdlet (**使用 AzureRmHDInsightCluster** 或 **Invoke AzureRmHDInsightHiveJob**) 來提交工作，這些作業可能失敗並出現錯誤訊息 「*叢集 <clustername> 未設定 HTTP 服務存取*。 」 或者 (依作業)，您可能會收到其他錯誤訊息，例如 「*無法連接到叢集*」。

* 最新版的 HDInsight SDK 和 Azure PowerShell 中已解決這些相容性問題。 建議將 HDInsight SDK 更新到 1.3.1.6 版或更新版本，並將 Azure PowerShell 工具更新到 0.8.8 版或更新版本。 您可以從最新的 HDInsight sdk 來取得存取 [](http://nuget.codeplex.com/wikipage?title=Getting%20Started) 和 Azure PowerShell Tools [如何安裝和設定 Azure PowerShell](../powershell-install-configure/)。



## HDInsight 3.1 2014/9/12 版本的注意事項

* 此版本根據 Hortonworks Data Platform (HDP) 2.1.5。 如需此版本中修正的 bug，請參閱 [此版本中修正](http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.1.5/bk_releasenotes_hdp_2.1/content/ch_relnotes-hdp-2.1.5-fixed.html) Hortonworks 網站上的頁面。
...
* Zookeeper 角色的主機名稱已變更為 *zookeeper*。 這會影響叢集內的名稱解析，但不影響外部 REST API。 如果您有元件使用 *zookeepernode* 主機名稱，您必須更新為使用新名稱。 三個 zookeeper 節點的新名稱如下：
    * zookeeper0
    * zookeeper1
    * zookeeper2
...
Azure PowerShell 或 HDInsight SDK 錯誤訊息，「 叢集 <clustername> 未設定 HTTP 服務存取 」 (或依作業的其他錯誤訊息，例如: 「 無法連線到叢集 」)，起因於 Azure PowerShell 或 HDInsight SDK 和叢集之間的版本差異。 在 8/15 或之後建立的叢集支援佈建到虛擬網路的這項新功能。 舊版的 Azure PowerShell 或 HDInsight SDK 無法正確解譯此功能，導致工作提交作業失敗。 如果您使用 HDInsight SDK API 或 Azure PowerShell Cmdlet (例如 Use-AzureRmHDInsightCluster 或 Invoke-AzureRmHDInsightHiveJob) 來提交工作，這些作業可能會失敗並傳回上述其中一個錯誤訊息。

最新版的 HDInsight SDK 和 Azure PowerShell 中已解決這些相容性問題。 建議將 HDInsight SDK 更新到 1.3.1.6 版或更新版本，並將 Azure PowerShell 工具更新到 0.8.8 版或更新版本。 您可以從最新的 HDInsight sdk 來取得存取 [NuGet][nuget-link]。 您可以使用，以存取 Azure PowerShell 工具 [Microsoft Web Platform Installer][webpi-link]。


## 2014/7/28 版本的相關資訊

* **新區域可用 HDInsight**: 我們 HDInsight 地理位置據點擴展到三個區域。 HDInsight 客戶可以在這三個區域建立叢集：
    * 東亞
    * 美國中北部
    * 美國中南部
* 正在將 HDInsight 1.6 版 (HDP 1.1 和 Hadoop 1.0.3) 和 HDInsight 2.1 版 (HDP1.3 和 Hadoop 1.2) 從 Azure 入口網站移除。 您可以繼續使用 Azure PowerShell cmdlet 來建立這些版本的 Hadoop 叢集 [新增 AzureRmHDInsightCluster](http://msdn.microsoft.com/library/dn593744.aspx) 或使用 [HDInsight SDK](http://msdn.microsoft.com/library/azure/dn469975.aspx)。 請參閱 [HDInsight 元件版本設定](../hdinsight-component-versioning/) 如需詳細資訊。
...
* **HDP 2.1 可用性**: HDInsight 3.1 (內含 HDP 2.1) 已正式推出，並為新叢集的預設版本。
* **HBase – Azure 入口網站改進**: 我們正使 HBase 叢集可在預覽中。 您可以從入口網站建立 HBase 叢集，只要按幾下滑鼠即可。 
...
此程序更完整說明，請參閱文件 [Breiman 範例](https://mahout.apache.org/users/classification/breiman-example.html) Apache Mahout 網站上。
...
Hortonworks 在標準效能評比中，已發表一份關於 Hive 查詢搭配 Tez 而提升效能的詳細數據。 如需詳細資訊，請參閱 [基準測試 Apache Hive 13 for Enterprise Hadoop](http://hortonworks.com/blog/benchmarking-apache-hive-13-enterprise-hadoop/)。

如需有關搭配 Tez 使用 Hive 的詳細資訊，請參閱 [Tez 上的 Hive](https://cwiki.apache.org/confluence/display/Hive/Hive+on+Tez)。
...
**與 HDInsight 3.1 叢集搭配使用的 Oozie 中繼存放區不回溯相容，HDInsight 2.1 叢集，且無法搭配此舊版本**。

與 HDInsight 3.1 叢集一起部署的自訂 Oozie metastore 資料庫，無法在 HDInsight 2.1 叢集上重複使用。 即使是源自 HDInsight 2.1 叢集的中繼存放區也是如此。 不支援這種情況下，因為 metastore 結構描述取得升級，使它不會再與 HDInsight 2.1 叢集所需的 metastore 相容，HDInsight 3.1 叢集，搭配使用時。 嘗試重複使用已在 HDInsight 3.1 叢集上使用過的 Oozie 中繼存放區，將會使 HDInsight 2.1 叢集變得無法使用。
...
**前置詞語法**:
只有 「 wasb: / /"HDInsight 3.1 和 3.0 叢集支援語法。 HDInsight 2.1 和 1.6 叢集支援舊的 "asv://" 語法，但在 HDInsight 3.1 或 3.0 叢集中不支援。 這表示，任何工作提交至 HDInsight 3.1 或 3.0 叢集且明確使用"asv://: / / 」 語法將會失敗。 應該改用 "wasb://" 語法。 此外，如果工作提交至任何以現有中繼存放區建立的 HDInsight 3.1 或 3.0 叢集，且中繼存放區中使用 asv:// 語法來明確參考資源，則也會失敗。 必須使用 wasb:// 來定址資源以重新建立這些中繼存放區。


**連接埠**: HDInsight 服務所使用的連接埠已變更。 以前所使用的連接埠號碼都在 Windows 作業系統暫時連接埠範圍內。 對於短期的網際網路通訊協定通訊，會自動從預設定義的暫時範圍中配置連接埠。 新的一組允許的 Hortonworks Data Platform (HDP) 服務連接埠號碼不在此範圍內，以避免與前端節點上執行的服務所使用的連接埠發生衝突。 新的連接埠號碼應該不會引起任何重大變更。 使用的號碼如下：
...
### 驅動程式
SQL Server 的 Java 資料庫連接 (JDBC) 驅動程式僅供 HDInsight 內部使用，且不適用於外部作業。 如果您想要使用開放式資料庫連接 (ODBC) 連線至 HDInsight，請使用 Microsoft Hive ODBC 驅動程式。 如需詳細資訊，請參閱 [Excel 連接到使用 Microsoft Hive ODBC 驅動程式的 HDInsight](../hdinsight-connect-excel-hive-odbc-driver.md)。
...
* HDInsight 3.1 版採用為基礎的 Hadoop 散發 [Hortonworks Data Platform 2.1.7][hdp-2-1-7]。 這是在 2014/11/7 之後使用 Azure 入口網站時所建立的預設 Hadoop 叢集。 根據已 2014/11/7 前建立的 HDInsight 3.1 叢集 [Hortonworks Data Platform 2.1.1][hdp-2-1-1]

* HDInsight 3.0 版會使用為基礎的 Hadoop 散發 [Hortonworks Data Platform 2.0][hdp-2-0-8]。

* HDInsight 2.1 版採用為基礎的 Hadoop 散發 [Hortonworks Data Platform 1.3][hdp-1-3-0]。

* HDInsight 1.6 版採用為基礎的 Hadoop 散發 [Hortonworks Data Platform 1.1][hdp-1-1-0]。
