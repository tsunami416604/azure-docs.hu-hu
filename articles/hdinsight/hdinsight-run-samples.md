...
- [* * Word 計數 * * ][hdinsight-sample-wordcount]: 計算文字檔中的文字出現次數。
- [* * C# 串流字數計數 * * ][hdinsight-sample-csharp-streaming]: 計算中使用 Hadoop 串流介面的文字檔案的文字出現次數。
...
- [開發 HDInsight 中 Hadoop 的 Java MapReduce 程式](hdinsight-develop-deploy-java-mapreduce.md)
- [開發 C# Hadoop 串流程式適用於 HDInsight](hdinsight-hadoop-develop-deploy-streaming-jobs.md)
- [在 HDInsight 提交 Hadoop 工作](hdinsight-submit-hadoop-jobs-programmatically.md)
...
1. 開啟 **Windows PowerShell ISE**。 如需指示，請參閱 [安裝和設定 ][powershell-install-configure]。
...
> [AZURE.NOTE] 本教學課程的步驟只適用於 Windows HDInsight 叢集。 如需以 Linux 為基礎的 HDInsight 叢集的串流範例，請參閱 [開發的 Python 串流程式 hdinsight](hdinsight-hadoop-streaming-python.md)。

在範例中，對應器和歸納器會讀取來自輸入的可執行檔 [stdin ][stdin-stdout-stderr] (--行) 並將輸出發出到 [stdout ][stdin-stdout-stderr]。 程式會計算內容中的所有文字。

在已為 **mappers** 指定可執行檔的情況下，當 mapper 初始化時，每個 mapper 工作都會將可執行檔啟動成為個別的處理程序。 當 mapper 工作執行時，將其輸入轉換成線條，以及這些行饋送至 [stdin ][stdin-stdout-stderr] 程序。
...
在已為 **reducers** 指定可執行檔的情況下，當 reducer 初始化時，每個 reducer 工作都會將可執行檔啟動成為個別的處理程序。 當 reducer 工作執行時，會將其輸入索引鍵/值組轉換成線條，而它這些行饋送至 [stdin ][stdin-stdout-stderr] 程序。

在此同時，reducer 會收集行導向輸出 [stdout ][stdin-stdout-stderr] 程序。 它會將每一行轉換成索引鍵/值組，其會做為 reducer 的輸出來收集。 根據預設，從一行的前置詞一直到第一個定位字元即是索引鍵，行的其餘部分 (不包含定位字元) 則為值。

如需有關 Hadoop 串流介面的詳細資訊，請參閱 [Hadoop 串流的 ][hadoop-streaming]。
...
* [偵錯在 HDInsight 中的 Hadoop: 錯誤訊息 ][hdinsight-errors]
