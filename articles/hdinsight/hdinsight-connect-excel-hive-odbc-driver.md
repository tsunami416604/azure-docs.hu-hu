...
- **HDInsight 叢集**。 若要建立一個帳戶，請參閱 [入門 ][hdinsight-get-started]。
...
---|---
 資料來源名稱| 為資料來源指定名稱
 主機| 輸入 <HDInsightClusterName>。 n e t。例如，myHDICluster.azurehdinsight.net
 連接埠| Use <strong>443</strong>.(此連接埠已從 563 變更為 443。)
 資料庫| Use <strong>Default</strong>.
 Hive 伺服器類型| 選取 <strong>Hive Server 2</strong>
 機制| 選取 <strong>Azure HDInsight 服務</strong>
 HTTP 路徑| 保留為空白。
 使用者名稱| 輸入 HDInsight 叢集使用者的使用者名稱。這是在叢集佈建程序中所建立的使用者名稱。如果您使用 [快速建立] 選項，則預設使用者名稱為 <strong>admin</strong>。
...
8. 選取您要匯入的資料表，然後按 [下一步]****。 *hivesampletable* 是 HDInsight 叢集隨附的範例 Hive 資料表。 您可以選擇此資料表 (如果尚未建立)。 執行 Hive 查詢及建立 Hive 資料表的詳細資訊，請參閱 [使用 Hive 與 HDInsight ][hdinsight-use-hive]。
8. 按一下 [完成]****。
...
- [分析航班延誤資料使用 HDInsight ][hdinsight-analyze-flight-data]
