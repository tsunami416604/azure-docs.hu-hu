...
> [AZURE.NOTE] 本文件中的資訊以在 HDInsight 叢集上使用以 Windows 為基礎的 Storm 為基礎。 如需使用以 Linux 為基礎的 Storm on HDInsight 從 Azure 事件中樞資訊，請參閱 [處理 Azure 事件中心與 Storm on HDInsight 的事件](hdinsight-storm-develop-java-event-hub-topology.md)
...
拓撲可透過 Storm on HDInsight 叢集所提供的 **com.microsoft.eventhubs.spout.EventHubSpout** 類別讀取事件中樞的資料。 此網站的通訊透過使用 [socket.io-client.java](https://github.com/nkzawa/socket.io-client.java)。
...
>
> * 來自 spout 資料給剖析器進行負載平衡。
> * 資料剖析器從儀表板及 HBase (若有使用) 會依裝置識別碼，以便從相同的裝置的訊息一律傳送至相同元件。
...
1. 從 [Azure 傳統入口網站](https://manage.windowsazure.com), ，選取 **新增 |服務匯流排 |事件中心 |自訂建立**。
...
3. 在儀表板執行時，使用 `節點 app.js` 命令前面的步驟，將新的資料傳送至儀表板。 由於溫度值是隨機產生，因此圖形必須更新才能顯示新值。
...
1. 若要部署至 Azure 網站的儀表板，請依照下列中的步驟 [建置並部署 Node.js 網站至 Azure](../web-sites-nodejs-develop-deploy-mac.md)。 記下網站的 URL，其樣式大致如下：**mywebsite.azurewebsites.net**。
...
3. 拓撲啟動之後，請開啟瀏覽器至您在 Azure 發行的網站，然後使用 `節點 app.js` 命令將資料傳送到事件中心。 您應該會看見顯示資訊的 Web 儀表板更新。
...
3. 輸入或選取下列值：
    ...
    - **DNS 伺服器**：本文使用的是 Azure 提供的內部 DNS 伺服器，因此您可以選擇 [無]****。 同時也支援更多自訂 DNS 伺服器的進階網路功能設定。 如需詳細的指引，請參閱 [名稱解析 (DNS)](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md)。
...
1. 開啟 **hdinsight-eventhub-example\TemperatureMonitor\conf\hbase-site.xml** 並取代 `尾碼` 的 DNS 尾碼與下面這一行的項目先前取得的 HBase 叢集。 完成變更後，請儲存檔案。
...
當您處理的資料與 Storm 叢集上啟動拓撲時，您可以使用 `掃描 'SensorData'` 命令來確認資料已經插入 HBase。
