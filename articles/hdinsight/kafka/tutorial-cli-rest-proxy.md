...
1. Környezeti változók beállítása Az oktatóanyagban szereplő változók használata a Bashon alapul. Más környezetek esetében kisebb eltérésekre lesz szükség.
    ...
    |location|Cserélje le a helyet egy olyan régióra, amelyben a fürt létre lesz hozva. Az érvényes helyszínek listáját a `az account list-locations` paranccsal használhatja.|
    ...
    |securityGroupName|Cserélje le a SECURITYGROUPNAME-t az ügyfél HRE biztonsági csoportjának nevére a Kafka Rest proxyhoz. A rendszer átadja a változót a `az-hdinsight-create``--kafka-client-group-name` paraméterének.|
    |securityGroupID|Cserélje le a SECURITYGROUPID-t az ügyfél HRE biztonsági csoportjának azonosítója a Kafka Rest proxyhoz. A rendszer átadja a változót a `az-hdinsight-create``--kafka-client-group-id` paraméterének.|
...
1. [Hozza létre a HDInsight-fürtöt](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-create). A parancs megadása előtt jegyezze fel a következő paramétereket:
    ...
    1. A Kafka REST proxyhoz szükséges paraméterek:
        ...
        Ha a fürtöt REST proxy nélkül szeretné létrehozni, távolítsa el `--kafka-management-node-size`, `--kafka-client-group-id`és `--kafka-client-group-name` a `az hdinsight create` parancsból.

    1. Ha rendelkezik meglévő virtuális hálózattal, adja hozzá a paramétereket `--vnet-name` és `--subnet`és azok értékeit.
