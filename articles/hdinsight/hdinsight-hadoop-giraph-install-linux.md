...
您也可以使用任何類型的 Azure HDInsight 上 Hadoop 中的叢集上安裝 Giraph **指令碼動作** 叢集自訂。 指令碼動作可讓您執行指令碼來自訂叢集，但只能在建立叢集時進行。 如需詳細資訊，請參閱 [使用指令碼動作自訂 HDInsight 叢集](hdinsight-hadoop-customize-cluster-linux.md)。

在本主題中，您將學習如何使用指令碼動作來安裝 Giraph。 一旦您已安裝 Giraph，您也將了解如何將 Giraph 用於大部分的一般應用程式，也就是處理大型的圖形。

> [AZURE.NOTE] 這篇文章中的資訊是以 Linux 為基礎的 HDInsight 叢集的特定項目。 如需使用 windows 叢集的詳細資訊，請參閱 [HDInsight Hadoop 叢集 (Windows) 上的安裝 Giraph](hdinsight-hadoop-giraph-install.md)
...
1. 使用以下命令建立新的檔名為 __tiny_graph.txt__:
...
2. 若要儲存檔案時，使用 __Ctrl + X__, ，然後 __Y__, ，最後 __Enter__ 接受檔案名稱。
...
4. 使用下列命令執行 SimpleShortestPathsComputation 範例。 取代 __前端節點__ 傳回上一個步驟的 fqdn:
...
5. 當工作完成之後時，將結果儲存在 __wasb: / 範例/out/shotestpaths__ 目錄。 建立檔案的開頭是 __一部分-m-__ 和數字，指出第一個、 第二個等檔案的結尾。 使用下列項目以檢視輸出：
...
- [HDInsight 叢集上安裝 R](hdinsight-hadoop-r-scripts-linux.md): 指示如何使用叢集自訂，以安裝和使用 R HDInsight Hadoop 叢集上。 R 是一個用於統計計算的開放原始碼語言和環境。 它提供數百個內建的統計函式及它自己的程式設計語言，此語言結合了函式型和物件導向程式設計的層面。 它也提供廣泛的圖形功能。
