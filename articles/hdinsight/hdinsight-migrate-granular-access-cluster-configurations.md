...
- [API](#api): a vagy `/configurations/{configurationName}` a `/configurations` végpontokat használó felhasználók.
...
- [.NET-hez készült SDK](#sdk-for-net)
    - [1. x vagy 2. x verzió](#versions-1x-and-2x): a (z `GetClusterConfigurations`) `GetConnectivitySettings`, `ConfigureHttpSettings`, `EnableHttp` , `DisableHttp` vagy metódust használó felhasználók a ConfigurationsOperationsExtensions osztályból.
    - [3. x és újabb verziók](#versions-3x-and-up) `Get`: a `Update` `EnableHttp` `ConfigurationsOperationsExtensions` osztályból a,, vagy `DisableHttp` metódust használó felhasználók.
- [Pythonhoz készült SDK](#sdk-for-python): a `get` `ConfigurationsOperations` osztályból `update` származó vagy metódusokat használó felhasználók.
- [Javához készült SDK](#sdk-for-java): az `update` `ConfigurationsInner` osztályból `get` származó vagy metódusokat használó felhasználók.
- [SDK for go](#sdk-for-go): a `Get` `ConfigurationsClient` struct-t `Update` vagy metódusokat használó felhasználók.
...
A tulajdonosi szerepkör mellett a parancsot végrehajtó felhasználónak vagy szolgáltatásnak elegendő Azure AD-engedéllyel kell rendelkeznie ahhoz, hogy megkeresse a megbízott objektum-azonosítóit. Ez az üzenet nem megfelelő Azure AD-engedélyeket jelez. Próbálja meg `-–assignee` lecserélni `–assignee-object-id` az argumentumot a (z) értékre, és adja meg a hozzárendelt objektum azonosítóját paraméterként a név helyett (vagy egy felügyelt identitás esetén a résztvevő azonosítóját). További információért tekintse meg az az [role hozzárendelés Create dokumentációjának](https://docs.microsoft.com/cli/azure/role/assignment?view=azure-cli-latest#az-role-assignment-create) választható paraméterek szakaszát.
...
A 2019 `GET /configurations` szeptember 3-ától kezdődően `POST /configurations/gateway` a hívások nem adnak vissza semmilyen információt, és `GET /configurations/{configurationName}` a hívás többé nem ad vissza bizalmas paramétereket, például a Storage-fiók kulcsait vagy a fürt jelszavát. Ugyanez vonatkozik a megfelelő SDK-módszerekre és a PowerShell-parancsmagokra is.
