...
> [AZURE.NOTE] 本文的資訊僅適用於以 Linux 為基礎的 HDInsight。 如需 Windows 為基礎的 HDInsight 資訊，請參閱 [啟用堆積傾印 Windows 為基礎的 HDInsight 上的 Hadoop 服務](hdinsight-hadoop-collect-debug-heap-dumps.md)
...
在每個指令碼中，沒有匯出 **\*\_OPTS**, ，其中包含傳遞至 JVM 的選項。 例如，在 **hadoop-env.sh** 指令碼開頭的那一行 `匯出 HADOOP_NAMENODE_OPTS =` 含有 NameNode 服務的選項。
...
> [AZURE.NOTE] 我們建議使用 Ambari 來修改指令碼和 mapred-site.xml 設定，因為 Ambari 會處理叢集中跨節點的複寫變更。 請參閱 [使用 Ambari](#using-ambari) 一節取得的特定步驟。
...
>
> 指令碼也必須位於服務所屬的帳戶可以存取的位置中，而且必須提供執行權限。 例如，您可能想要儲存指令碼中的 `/usr/local/bin` 並用 `/usr/local/bin/filename.sh chmod 移 + rx` 授與讀取和執行權限。
