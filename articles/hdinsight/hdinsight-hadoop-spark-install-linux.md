...
在本文件中，您將學習如何使用指令碼動作來安裝 Spark。 指令碼動作可讓您執行指令碼來自訂叢集，但只能在建立叢集時進行。 如需詳細資訊，請參閱 [使用指令碼動作自訂 HDInsight 叢集][hdinsight-cluster-customize]。 安裝 Spark 之後，您也將學習如何在 HDInsight 叢集上執行 Spark 查詢。

> [AZURE.NOTE] HDInsight 也提供 Spark 叢集類型，這表示您現在可以直接佈建 Spark 叢集而不必修改 Hadoop 叢集。 不過，目前僅限於以 Windows 為基礎的叢集。 使用 Spark 叢集類型時，您會取得具備 Spark 1.3.1 版且以 Windows 為基礎的 HDInsight 3.2 版叢集。 如需詳細資訊，請參閱 [開始使用 HDInsight 上的 Apache Spark](hdinsight-apache-spark-zeppelin-notebook-jupyter-spark-sql.md)。
...
2. 在 Scala 提示設定 Hive 內容。 必須執行此動作，才能透過 Spark 使用 Hive 查詢。
    ...
    > [AZURE.NOTE]  `sc` 在這個陳述式是您啟動 Spark 殼層時所設定的預設 Spark 內容。
...
3. 建立新的檔名為 __simple.sbt__, ，其中包含此專案的組態資訊。
...
5. 從 __SimpleScalaApp__ 目錄中，使用下列命令來建置應用程式，並將它儲存 jar 檔案中:
    ...
    應用程式編譯之後，您會看到 **simpleapp_2.10-1.0.jar** __SimpleScalaApp/target/scala-2.10** 目錄中建立的檔案。
