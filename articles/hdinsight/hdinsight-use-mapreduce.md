...
> [AZURE.NOTE] 如果您在佈建叢集時指定其他儲存體帳戶，並想要存取這些帳戶上儲存的檔案，您可以指定容器名稱和儲存體帳戶位址來存取資料。

## 

這包含您依據 **davinci.txt** 執行的字數統計範例。
> [AZURE.NOTE] 
...
|:-------------------------------------------------------------------|:--------------------------------------------------------|:------------------------------------------|:-----------------------------------------|
| | 透過 **SSH** 使用 Hadoop 命令| Linux| Linux、Unix、Mac OS X 或 Windows|
| | 使用 **REST** 遠端提交工作。| Linux 或 Windows| Linux、Unix、Mac OS X 或 Windows|
| | 使用 **Windows PowerShell** 遠端提交工作。| Linux 或 Windows| Windows|
| | 透過**遠端桌面**使用 Hadoop 命令| Windows| Windows|
