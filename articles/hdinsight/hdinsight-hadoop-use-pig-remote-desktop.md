...
4. 套用規則運算式以僅擷取每筆記錄的記錄層級，來轉換資料：
    ...
    轉換之後，您可以使用 **DUMP** 來檢視資料。 在此情況下， `傾印的層級;`。

5. 使用下列陳述式，繼續套用轉換。 使用 `傾印` 若要檢視每個步驟後的轉換結果。
...
6. 您可以使用儲存轉換結果 `存放區` 陳述式。 例如，下列命令將 `結果` 至 **/example/data/pigout** 目錄中為您叢集的預設儲存體容器:
...
5. 使用下列命令，以使用 pig 命令來執行 **pigbatch.pig** 檔案。
    ...
    批次工作完成後，您應該會看到下列的輸出應該是當您使用相同 `DUMP RESULT;` 在前面的步驟:
