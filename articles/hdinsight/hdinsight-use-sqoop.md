...
> [AZURE.NOTE] 這篇文章中的步驟可以搭配任一 windows 或 Linux 型 HDInsight 叢集。不過，這些步驟只會從 Windows 用戶端運作。
>
> 如果您使用 Linux、 OS X 或 Unix 用戶端和 Linux 為基礎的 HDInsight 伺服器，請參閱 [使用 Sqoop 與 Hadoop 在 HDInsight (SSH)](hdinsight-use-sqoop-mac-linux.md)

雖然很自然會選擇處理非結構化和半結構化資料的 Hadoop 
例如記錄和檔案，也可能有需要處理結構化的資料 
儲存在關聯式資料庫中。
...
- **SQL Server**: 如果您的 HDInsight 叢集位於相同 Azure 虛擬網路與 SQL Server，您可以使用這篇文章的步驟匯入和匯出資料至 SQL Server 資料庫。
    ...
    * 若要佈建 HDInsight 叢集上的虛擬網路，請參閱 [使用自訂選項在 HDInsight 中的佈建 Hadoop 叢集](hdinsight-provision-clusters.md)
...
- Log4j 記錄檔，位於 */example/data/sample.log*。 下列記錄擷取自此檔案：
...
- 名為的 Hive 資料表 *hivesampletable*, ，參考資料檔案位於 */hive/warehouse/hivesampletable*。 此資料表包含某些行動裝置資料。 
...
5. 前置處理來源資料檔。
    ...
    在本教學課程中，您會將 log4j 記錄檔 (使用分隔符號的檔案) 和 Hive 資料表匯出至 Azure SQL Database。 分隔符號的檔案稱為 */example/data/sample.log*。 在本教學課程先前的內容中，您已看過 log4j 記錄檔的幾個範例。 記錄檔中有某些空白行，且有幾行類似以下內容：
    ...
    此程序會在叢集上建立新檔案：tutorials/usesqoop/data/sample.log。 若要檢查已修改的資料檔案，可以使用 Azure 入口網站、Azure 儲存體總管工具或 Azure PowerShell。 [開始使用 HDInsight][hdinsight-get-started] 提供使用 Azure PowerShell 來下載檔案及顯示檔案內容的程式碼範例。
...
2. 在 Program.cs 檔案中使用下列 using 陳述式：
        ...
        using Hyak.Common;
3. 在 Main() 函式中加入下列程式碼： 如需使用 HDInsight.NET SDK 的一般資訊，請參閱 [以程式設計方式提交 Hadoop 工作][hdinsight-submit-jobs]。
