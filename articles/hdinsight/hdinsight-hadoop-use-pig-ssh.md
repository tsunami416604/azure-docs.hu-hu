...
4. 接下來是轉換資料，方法是套用規則運算式，並使用下列命令僅擷取每筆記錄的記錄層級。
    ...
    轉換之後，您可以使用 **DUMP** 來檢視資料。 在此案例中，使用 `傾印的層級;`。
...
6. 您可以使用儲存轉換結果 `存放區` 陳述式。 例如，下列儲存 `結果` 至 **/example/data/pigout** 目錄叢集之預設儲存體容器。
...
5. 使用下列命令，以使用 Pig 命令來執行 **pigbatch.pig** 檔案。
    ...
    批次工作完成時，您應該會看到下列的輸出應該是當您使用相同 `DUMP RESULT;` 在前面的步驟。
