...
這個範例包含隨機產生一個句子、將句子分成單字、計算字數，並將單字和計數傳送至 Power BI REST API 的 C# Storm 拓撲。  [PowerBi.Api.Client](https://github.com/Vtek/PowerBI.Api.Client) Nuget 套件用來與 Power BI 通訊。
...
>
>  **CreateDataset** 主控台應用程式包含在此解決方案依現狀舉例說明如何建立拓撲外部資料集。

## 註冊 Power BI 應用程式

1. 請依照 [Power BI 快速入門](https://msdn.microsoft.com/library/dn931989.aspx) 註冊 Power BI。

2. 請依照 [註冊應用程式](https://msdn.microsoft.com/library/dn877542.aspx) 來建立應用程式註冊。 這會在存取 Power BI REST API 時使用。

    > [AZURE.IMPORTANT] 儲存 **用戶端識別碼** 的應用程式註冊。

## 下載範例

下載 [HDInsight C# Storm Power BI 範例](https://github.com/Blackmist/hdinsight-csharp-storm-powerbi)。 若要下載它，請分岔/複製它使用 [git](http://git-scm.com/), ，或使用 **下載** 連結下載封存的.zip。
...
2. (選擇性)。 此專案所使用的預設資料集名稱是 **字**。 若要變更，請以滑鼠右鍵按一下 **WordCount** 專案中 **方案總管] 中**, ，選取 **屬性**, ，然後選取 **設定**。 變更 **DatasetName** 項目所要的值。

2. 儲存並關閉檔案。

## 部署範例

1. 從 **方案總管] 中**, ，以滑鼠右鍵按一下 **WordCount** 專案，然後選取 **提交至 Storm on HDInsight**。 選取從 HDInsight 叢集 **Storm 叢集** ] 下拉式清單對話方塊。
...
2. 在頁面的左邊，展開 **資料集**。 選取 **字** 項目。 這是範例拓撲所建立的資料集。

    ![單字資料集項目](./media/hdinsight-storm-power-bi-topology/words.png)

3. 從 **欄位** 區域中，展開 **WordCount**。 拖放到 **計數** 和 **Word** 項目到頁面的中間部分。 這會建立新的圖表，為每個字顯示一個列，指出這個字出現的次數。

    ![WordCount 圖表](./media/hdinsight-storm-power-bi-topology/wordcountchart.png)

4. 從頁面左上方，選取 **儲存** 建立新的報表。 輸入 **字數** 做為報表的名稱。

5. 選取 Power BI 標誌以返回儀表板。  **字數** 報表現在會出現在 **報表**。

## 建立即時儀表板

1. 旁邊 **儀表板**, ，請選取 **+** 圖示以建立新的儀表板。 將新的儀表板 **Live Word Count**。

2. 選取 **字數** 您稍早建立的報表。 顯示時，選取圖表，然後選取圖表右上方的圖釘圖示。 您應該會收到通知，表示它已釘選到儀表板。
...
1. 在 Visual Studio 中開啟 **拓樸摘要** WordCount 拓樸的視窗。 如果拓撲摘要尚未開啟，請移至 **伺服器總管**, ，依序展開 **Azure** 和 **HDInsight** 項目，以滑鼠右鍵按一下 storm on HDInsight 叢集，然後選取 **檢視 Storm 拓撲**。 最後，選取 **WordCount** 拓樸。

2. 選取 **Kill** ] 按鈕以停止 **WordCount** 拓樸。
