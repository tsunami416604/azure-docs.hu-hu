...
- [設定 DNS 的虛擬網路 ][hdinsight-hbase-geo-replication-dns]
...
4.  輸入：
    ...
    - **DNS 伺服器**：(保留為空白)
        ...
        您需要有自己的 DNS 伺服器才能在虛擬網路內進行名稱解析。 如需有關何時使用 Azure 提供的名稱解析，以及何時使用自己的 DNS 伺服器，請參閱 [名稱解析 (DNS)](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md)。 若要設定 Vnet 之間名稱解析的指示，請參閱 [設定兩個 Azure 虛擬網路 ][hdinsight-hbase-dns]。
...
- [設定兩個 Azure 虛擬網路 ][hdinsight-hbase-geo-replication-dns]
- [設定 HBase 異地複寫 ][hdinsight-hbase-geo-replication]
