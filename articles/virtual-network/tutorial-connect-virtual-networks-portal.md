---
title: Virtuális hálózatok csatlakoztatása virtuális hálózatok hoz virtuális hálózati társviszony-létesítés - oktatóanyag - Azure portal
...
3. Az **Alapok lapon** adja meg vagy jelölje ki a következő információkat, és fogadja el a többi beállítás alapértelmezett beállításait:
...
4. Az **IP-címek** lapon írja be a 10.0.0.0/16 értéket a **Címtér** mezőbe. Kattintson az **alhálózat hozzáadása** gombra, és írja be az *Alhálózat1* értéket az **Alhálózat név,** a 10.0.0.0/24 értéket pedig az **Alhálózati cím tartományhoz.**
...
2. Válassza **a Társviszony-létesítések** **csoportban**a Beállítások csoportban a **Hozzáadás**lehetőséget, ahogy az az alábbi képen látható:
...
3. Adja meg vagy válassza ki az alábbi adatokat, a többi beállítás esetében fogadja el az alapértelmezett értéket, majd válassza az **OK** elemet:
    ...
    |Beállítás|Érték|
    |---|---|
    |A társviszony-létesítés neve a myVirtualNetwork1-ről a távoli virtuális hálózatra|myVirtualNetwork1-myVirtualNetwork2 - Amikor az oldal először betöltődik, itt láthatja a "távoli virtuális hálózat" kifejezést. A távoli virtuális hálózat kiválasztása után a "távoli virtuális hálózat" kifejezés helyébe a távoli virtuális hálózat neve kerül.|
    |Előfizetés| Válassza ki előfizetését.|
    |Virtuális hálózat|myVirtualNetwork2 - A *myVirtualNetwork2* virtuális hálózat kiválasztásához válassza a **Virtuális hálózat**lehetőséget, majd válassza a **myVirtualNetwork2 (myResourceGroup)** lehetőséget. Választhat egy az adott régióban vagy egy másik régióban lévő virtuális hálózatot is.|
    |A társviszony-létesítés neve a myVirtualNetwork2 és a myVirtualNetwork1 között|myVirtualNetwork2-myVirtualNetwork1|

    ![Társviszony beállításai](./media/tutorial-connect-virtual-networks-portal/peering-settings-bidirectional.png)

    A **TÁRSVISZONY-LÉTESÍTÉSI ÁLLAPOT** *Csatlakoztatva*, ahogy az az alábbi képen látható:
...
4. Válassza ki a virtuális gép méretét a **Méret** beállításhoz.
5. A Hálózatcsoportban válassza ki a következő **értékeket:**

    |Beállítás|Érték|
    |---|---|
    |Virtuális hálózat| myVirtualNetwork1 - Ha még nincs kiválasztva, válassza **a Virtuális hálózat** lehetőséget, majd válassza a **myVirtualNetwork1**lehetőséget.|
    |Alhálózat| Alhálózat1 - Ha még nincs kijelölve, válassza **az Alhálózat** lehetőséget, majd az **Alhálózat1**lehetőséget.|
   
6. Válassza a **Hálózat** lehetőséget. Válassza a Kijelölt portok engedélyezése a **Nyilvános bejövő portok** beállításhoz választógombot. **Allow selected ports** Válassza az **RDP** lehetőséget a **Bejövő portok kijelölése** beállításhoz az alábbi területen. 

7. Válassza a **Véleményezés + Létrehozás** gombot a bal alsó sarokban a virtuális gép telepítésének elindításához.
