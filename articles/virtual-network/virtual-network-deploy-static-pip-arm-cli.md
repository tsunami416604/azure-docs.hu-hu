...
3. Hozzon létre egy virtuális gépet az [az vm create](/cli/azure/vm#az-vm-create) paranccsal. Az `--public-ip-address-allocation=static` lehetőség statikus nyilvános IP-címet rendel hozzá a virtuális géphez. Az alábbi példa egy *myPublicIpAddress*nevű, statikus, alapszintű SKU nyilvános IP-címmel rendelkező Ubuntu virtuális gépet hoz létre:
