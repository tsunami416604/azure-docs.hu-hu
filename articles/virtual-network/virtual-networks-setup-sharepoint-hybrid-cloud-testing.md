...
11. 在 **物件總管] 中** 樹狀結構窗格中，以滑鼠右鍵按一下 **登入**, ，然後按一下 [ **新登入**。
12. 在 **一般** 頁面上，於 **登入名稱**, ，型別 **CORP\SPFarmAdmin**。
...
1.  從 [開始] 畫面中，輸入 **SharePoint 2013 產品**, ，然後按一下 [ **SharePoint 2013 產品組態精靈**。 當系統要求您允許程式變更電腦時，按一下 [ **是**。
2.  在歡迎使用 SharePoint 產品] 頁面中，按一下 [ **下一步**。 
3.  在對話方塊中，通知您可能需要在組態期間重新啟動某些服務，按一下 [ **是**。
4.  在 [連接到伺服器陣列] 頁面上，按一下 [ **建立新的伺服器陣列**, ，然後按一下 [ **下一步**。
5.  在 [指定組態資料庫設定] 頁面中，輸入 **sql1.corp.contoso.com** 中 **資料庫伺服器**, ，型別 **CORP\SPFarmAdmin** 中 **使用者名稱**, ，輸入 SPFarmAdmin 帳戶密碼 **密碼**, ，然後按一下 [ **下一步**。
6.  在 [指定伺服陣列安全性設定] 頁面上輸入 **P@ssphrase** 中 **複雜密碼** 和 **確認複雜密碼**, ，然後按一下 [ **下一步**。
7.  在設定 SharePoint 管理中心 Web 應用程式頁面上，按一下 [ **下一步**。
...
11. 如 **您要如何設定 SharePoint 伺服器陣列?**, ，按一下 [ **啟動精靈**。
12. 在 [設定您的 SharePoint 伺服器陣列] 頁面，在 **服務帳戶**, ，按一下 [ **使用現有受管理帳戶**。
13. 在 **服務**, ，清除所有核取方塊以外的下一步 **狀態服務**, ，然後按一下 [ **下一步**。 在其網頁上的 [工作] 可能顯示一段時間才會完成。
14. 在建立網站集合] 頁面上，在 **標題和描述**, ，型別 **Contoso Corporation** 中 **標題**, ，指定 URL **http://sp1**/，然後按一下 [ **[確定]**。 在其網頁上的 [工作] 可能顯示一段時間才會完成。 這個步驟會在 URL http://sp1 建立團隊網站。
...
17. 在 [網址] 列中，輸入 **http://sp1/** ，然後按 ENTER。 您應該會看見 Contoso Corporation 的 SharePoint 團隊網站。 網站可能需要一些時間才會呈現。
