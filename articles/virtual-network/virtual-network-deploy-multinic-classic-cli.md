...
您可以下載完整的 bash 指令碼使用 [這裡](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/11-MultiNIC/classic/multinic.sh)。 請遵循下列步驟來變更要在環境中工作的指令碼。
...
1. 為所有後端 VM 建立新的雲端服務。 請注意，使用 `$backendCSName` 變數的資源群組名稱，以及 `$location` Azure 區域。
...
4. 建立 VM。 請注意使用 `-nic 設定` 參數，其中包含一份所有 Nic 的名稱、 子網路和 IP 位址。
