...
因為端點可以指定多個規則，故必須依靠一種方式組織規則，以判斷規則的優先順序。 規則順序會指定優先順序。 網路 ACL 遵循*最低優先*的規則順序。 在下方範例中，連接埠 80 上的端點會選擇性僅針對某些特定 IP 位址範圍授與存取權。 若要設定這種情況，我們有拒絕規則 (規則 \ # 100) 175.1.0.1/24 空間中的位址。 接著指定第二個規則的優先順序為 200，以便允許 175.0.0.0/8 下所有其他位址的存取。
...
[如何管理存取控制清單 (Acl) 端點使用 PowerShell](../virtual-networks-acl-powershell)
