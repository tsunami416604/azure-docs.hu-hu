...
5. Adja meg a következő paraméterek értékeit:
   ...
   - **Hálózati biztonsági csoport**: Az alhálózat bejövő és kimenő hálózati forgalmának szűréséhez nullát vagy egy meglévő hálózati biztonsági csoportot rendelhet egy alhálózathoz. A hálózati biztonsági csoportnak ugyanabban az előfizetésben és helyen kell lennie, mint a virtuális hálózatnak. További információ a [hálózati biztonsági](security-overview.md) csoportokról és [a hálózati biztonsági csoportok létrehozásáról](tutorial-filter-network-traffic.md).
   - **Útválasztási táblázat**: A hálózati forgalom útválasztásának más hálózatokra való irányításához nulla vagy egy meglévő útválasztási táblázatot is hozzárendelhet egy alhálózathoz. Az útválasztási táblázatnak ugyanabban az előfizetésben és helyen kell lennie, mint a virtuális hálózatnak. További információ az [Azure útválasztásáról](virtual-networks-udr-overview.md) és az útválasztási [táblázat létrehozásáról](tutorial-create-route-table-portal.md)
...
4. Az alhálózatok listájában válassza ki azt az alhálózatot, amelynek a beállításait módosítani szeretné. A következő beállításokat módosíthatja:
    ...
    - **Hálózati biztonsági csoport** és **útválasztási táblázat**: Lásd az [alhálózat hozzáadása](#add-a-subnet)című cikk 5. lépését.
    - **Szolgáltatási végpontok**: Tekintse meg az [alhálózat hozzáadása](#add-a-subnet)5. lépésében található szolgáltatási végpontokat. Ha egy meglévő alhálózathoz engedélyez egy szolgáltatási végpontot, győződjön meg arról, hogy az alhálózat egyik erőforrásán sem futnak kritikus feladatok. A szolgáltatási végpontok az alhálózaton lévő összes hálózati adapteren az útvonalakat az alapértelmezett elérési út és a *0.0.0.0/0* címe előtaggal és az *Internet*következő ugrási típusának használatával kapcsolják össze, hogy egy új útvonalat használjanak a szolgáltatás címének előtagjaival, és egy következő ugrási típust. *VirtualNetworkServiceEndpoint*. A kapcsoló során előfordulhat, hogy a nyitott TCP-kapcsolatok megszakadnak. A szolgáltatási végpont nincs engedélyezve, amíg az összes hálózati adapter számára a szolgáltatás felé irányuló adatforgalom az új útvonallal frissül. Az útválasztással kapcsolatos további tudnivalókért tekintse meg az [Útválasztás áttekintése](virtual-networks-udr-overview.md)című témakört.
