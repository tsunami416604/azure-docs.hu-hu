...
您可以下載完整的 PowerShell 指令碼使用 [這裡](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/11-MultiNIC/arm/multinic.ps1)。 請遵循下列步驟來變更要在環境中工作的指令碼。
...
1. 啟動 `的` 迴圈重複命令來建立 VM，並視需要多次的兩個 Nic 的值為基礎 `$numberOfVMs` 變數。
...
7. 加入至先前建立的兩個 Nic `vmConfig` 物件。

            $vmConfig = Add-AzureRmVMNetworkInterface -VM $vmConfig -Id $nic1.Id -Primary
            $vmConfig = Add-AzureRmVMNetworkInterface -VM $vmConfig -Id $nic2.Id

8. 建立作業系統磁碟並建立 VM。 請注意 `}` 結束 `的` 迴圈。
