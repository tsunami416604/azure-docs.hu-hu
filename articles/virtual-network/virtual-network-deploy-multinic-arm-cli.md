...
您可以下載完整的 bash 指令碼使用 [這裡](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/11-MultiNIC/arm/multinic.sh)。 請遵循下列步驟來變更要在環境中工作的指令碼。
...
3. 擷取的識別碼 `後端` 來建立的 Vm 子網路。 您需要這麼做，因為要和這個子網路關聯的 NIC 位於不同的資源群組中。
...
4. 擷取的識別碼 `NSG RemoteAccess` NSG。 您需要這麼做，因為要和這個 NSG 關聯的 NIC 位於不同的資源群組中。
...
1. 為所有後端資源建立新的資源群組。 請注意，使用 `$backendRGName` 變數的資源群組名稱，以及 `$location` Azure 區域。
...
3. 針對每部 VM，建立 NIC 以用於遠端存取。 請注意 `-網路安全性群組` 參數，用來建立關聯的 NIC 的 nsg。
...
5. 每個 VM，建立兩個資料磁碟，並結束迴圈使用 `完成` 命令。
