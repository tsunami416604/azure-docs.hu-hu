---
title: Az Azure virtuális hálózat forgalmának útválasztása
...
- **Virtuális hálózat**: Lévő címtartományok közötti forgalmat irányítja a [címtér](manage-virtual-network.md#add-or-remove-an-address-range) egy virtuális hálózat. Az Azure létrehoz egy útvonalat a virtuális hálózat címterében meghatározott egyes címtartományoknak megfelelő címelőtaggal. Ha egy virtuális hálózat címteréhez több címtartomány van meghatározva, az Azure egyéni útvonalat hoz létre mindegyik címtartományhoz. Az Azure automatikusan irányítja az alhálózatok közötti forgalmat az egyes címtartományokhoz létrehozott útvonalakkal. Nem kell meghatároznia átjárókat az Azure-nak az alhálózatok közötti forgalom irányításához. Bár a virtuális hálózatok alhálózatokat tartalmaznak, és mindegyik alhálózat meghatározott címtartománnyal rendelkezik, az Azure *nem* hoz létre alapértelmezett útvonalakat az alhálózati címtartományokhoz, mert mindegyik alhálózati címtartomány egy virtuális hálózat címterének címtartományán belül van.
...
- **Nincs**: Kapcsolódó forgalom például a **nincs** következő ugrási típus eldobott ahelyett, irányítja az alhálózaton kívülre. Az Azure automatikusan alapértelmezett útvonalakat hoz létre a következő címelőtagokhoz:
...
- **Virtuális berendezés**: A virtuális készülék valójában egy virtuális gépet, amely általában egy hálózati alkalmazás, például a tűzfalak. A virtuális hálózaton üzembe helyezhető különböző előre konfigurált hálózati virtuális berendezésekről az [Azure Marketplace-en](https://azuremarketplace.microsoft.com/marketplace/apps/category/networking?page=1&subcategories=appliances) talál további információt. Amikor létrehoz egy **virtuális berendezés** ugrástípusú útvonalat, a következő ugrás IP-címét is meghatározza. Az IP-cím a következő lehet:
...
**Következő ugrás típusai az Azure-eszközökben**
...
- **ExpressRoute:** A BGP helyi felé haladó útvonalak meghirdetéséhez a Microsoft peremhálózati útválasztója kell használnia. Felhasználó által megadott útvonalak kényszerítése forgalom az ExpressRoute virtuális hálózati átjárót telepít egy virtuális hálózati átjáró üzembe helyezett típusaként nem hozható létre: Az ExpressRoute. Az ExpressRoute forgalmának például egy hálózati virtuális berendezésre való kényszerítéséhez használhat felhasználó által megadott útvonalakat.
- **VPN**: Igény szerint használhatja a BGP. Részletes információért lásd a [BGP helyek közötti VPN-kapcsolatokkal](../vpn-gateway/vpn-gateway-bgp-overview.md?toc=%2fazure%2fvirtual-network%2ftoc.json) témakört.
...
- Már nem férhet közvetlenül hozzá az alhálózati erőforrásokhoz az internetről. Közvetett módon hozzáférhet az alhálózat erőforrásaihoz az internetről, ha a beérkező adatforgalom áthalad a következő ugrás típusa által megadott eszközön egy olyan útvonal esetében, amely a virtuális hálózatban lévő erőforrás elérését megelőzően a 0.0.0.0/0 címelőtaggal rendelkezik. Ha az útvonal a következő értékeket tartalmazza a következő ugrás típusára vonatkozóan:
    ...
    - **Virtuális hálózati átjáró**: Ha az átjáró egy ExpressRoute virtuális hálózati átjáró, egy internetkapcsolattal rendelkező eszközön a helyi hálózati címmel egyszerűen leképezhetők és előre, vagy a forgalmat a cél erőforráshoz az alhálózatban, ExpressRoute-n keresztül [magánhálózati társviszony-létesítés ](../expressroute/expressroute-circuit-peerings.md?toc=%2fazure%2fvirtual-network%2ftoc.json#privatepeering). 
...
2. Állítson be egy helyszíni hálózatot, amely biztonságosan kommunikál mindkét virtuális hálózattal az interneten keresztül, egy VPN-alagúton át. *Azt is megteheti egy ExpressRoute-kapcsolat is használható, de ebben a példában egy VPN-kapcsolatot használunk.*
...
- [Felhasználó által definiált útvonaltábla létrehozása útvonalakkal és hálózati virtuális készüléken](tutorial-create-route-table-portal.md)
- [Az Azure VPN Gateway a BGP konfigurálása](../vpn-gateway/vpn-gateway-bgp-resource-manager-ps.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
- [A BGP használata az ExpressRoute](../expressroute/expressroute-routing.md?toc=%2fazure%2fvirtual-network%2ftoc.json#route-aggregation-and-prefix-limits)
