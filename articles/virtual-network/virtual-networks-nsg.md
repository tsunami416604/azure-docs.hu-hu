...
| 屬性| 說明| 條件約束| 考量|
|---|---|---|---|
| 名稱| NSG 的名稱| 必須是唯一的區域內的<br/>可以包含字母、 數字、 底線、 句號和連字號<br/>必須以字母或數字開頭<br/>必須以字母、 數字或底線結尾<br/>可以有最多 80 個字元| 因為您可能需要建立數個 NSG，請確定您的命名慣例可讓您輕鬆識別 NSG 的功能|
| 區域| NSG 裝載所在的 Azure 區域| NSG 只能套用到建立所在區域內的資源| 請參閱 [限制](#Limits) 下若要了解幾個 Nsg 可以有區域中|
| 資源群組| NSG 所屬的資源群組| 雖然 NSG 屬於資源群組，它可以與任何資源群組中的資源相關聯，只要資源是與 NSG 相同的 Azure 區域的一部分| 資源群組用來管理多個資源一起部署單位<br/>群組與相關聯的資源使用 NSG，您可以考慮|
| 規則| 定義允許或拒絕流量的規則| | 請參閱 [NSG 規則](#Nsg-rules) 下方|

>[AZURE.NOTE] 端點式 ACL 和網路安全性群組，不支援用於相同的 VM 執行個體。 如果您想要使用 NSG 且已經擁有就地端點 ACL，請先移除端點 ACL。 如需如何執行這項操作，請參閱 [管理存取控制清單 (Acl) 端點使用 PowerShell](virtual-networks-acl-powershell.md)。
...
| 屬性| 說明| 條件約束| 考量|
|---|---|---|---|
| **Name**| 規則的名稱| 必須是唯一的區域內的<br/>可以包含字母、 數字、 底線、 句號和連字號<br/>必須以字母或數字開頭<br/>必須以字母、 數字或底線結尾<br/>可以有最多 80 個字元| 在 NSG 內可以有數個規則，因此請確定您遵循可讓您識別規則的功能的命名慣例|
| **通訊協定**| 規則要符合的通訊協定| TCP、 UDP 或 \ *| 使用 \ * 包含 ICMP (僅東西流量)，以及 UDP 和 TCP 通訊協定，並可能會降低規則的數目必須<br/>相同時，使用 \ * 可能是過於廣泛的方法，因此請確定您只使用真的有必要時|
...
- **INTERNET：**這個預設標籤代表虛擬網路以外且可以透過公用網際網路進行存取的 IP 位址空間。 此範圍包括 [Azure 擁有的公用 IP 空間](https://www.microsoft.com/download/details.aspx?id=41653) 以及。
...
>[AZURE.NOTE] 請注意此規則的來源位址範圍 **網際網路**, ，和不 VIP 負載平衡器; 連接埠是 **\ ***, 、 不 500001。 不要在 NAT 規則/負載平衡規則和 NSG 規則之間感到混淆。 NSG 規則永遠與流量的原始來源和最終目的地的相關，而**不是**兩者之間的負載平衡器。 
...
- [傳統部署模型中的部署 Nsg](virtual-networks-create-nsg-classic-ps.md)。
