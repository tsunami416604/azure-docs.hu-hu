...
3. 執行 **`azure network route-table create`** 命令，以建立路由表的後端的子網路。

        azure network route-table create -n UDR-BackEnd -l uswest

4. 執行 **`azure network route-table route set`** 命令來建立上述傳送所有流量端子 (192.168.1.0/24) 所建立的路由表中的路由到 **FW1** VM (192.168.0.4)。
