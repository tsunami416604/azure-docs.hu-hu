...
5. <a name="add-peering"></a>Adja meg vagy válassza ki a következő beállítások értékeit:
    ...
    - **Virtuális hálózati hozzáférés engedélyezése:** Válassza az **engedélyezve** (alapértelmezett) lehetőséget, ha engedélyezni szeretné a két virtuális hálózat közötti kommunikációt az alapértelmezett `VirtualNetwork` folyamaton keresztül. A virtuális hálózatok közötti kommunikáció engedélyezése lehetővé teszi, hogy a virtuális hálózathoz csatlakoztatott erőforrások ugyanazzal a sávszélességgel és késéssel kommunikáljanak egymással, mintha ugyanahhoz a virtuális hálózathoz csatlakoznak. A két virtuális hálózat erőforrásai közötti összes kommunikáció az Azure magánhálózati hálózatán keresztül történik. A hálózati biztonsági csoportok **VirtualNetwork** szolgáltatásának címkéje magában foglalja a virtuális hálózatot és a társ virtuális hálózatot, ha ez a beállítás **engedélyezve** van. (További információ a hálózati biztonsági csoport szolgáltatásának címkékről: [hálózati biztonsági csoportok áttekintése](security-overview.md#service-tags).) Válassza a **Letiltva** lehetőséget, ha nem szeretné, hogy a rendszer alapértelmezés szerint a virtuális hálózatra irányítsa a forgalmat. Ha egy virtuális hálózatot egy másik virtuális hálózattal társított, akkor a **Letiltva** lehetőséget választhatja, de alkalmanként le szeretné tiltani a két virtuális hálózat közötti alapértelmezett forgalmat. Előfordulhat, hogy az engedélyezés/letiltás sokkal kényelmesebb, mint a társítások törlése és újbóli létrehozása. Ha ez a beállítás le van tiltva, a forgalom alapértelmezés szerint nem áramlik a virtuális hálózatok között. a forgalom azonban továbbra is eltarthat, ha explicit módon engedélyezi egy [hálózati biztonsági csoport](security-overview.md) szabályát, amely tartalmazza a megfelelő IP-címeket vagy alkalmazás-biztonsági csoportokat.
      > [!WARNING]
