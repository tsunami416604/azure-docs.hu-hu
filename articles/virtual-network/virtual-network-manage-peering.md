...
Megtudhatja, hogyan létrehozása, módosítása vagy törlése a virtuális hálózati társviszony-létesítés. Virtuális hálózati társviszony-létesítés lehetővé teszi a virtuális hálózatok csatlakoztatása az Azure gerincét a hálózaton keresztül. Amennyiben nincsenek társviszonyban, a rendszer továbbra is kezeli a virtuális hálózatok, külön erőforrások. Ha most ismerkedik a virtuális hálózati társviszony-létesítést, további információt a a [virtuális hálózati társviszony-létesítési áttekintése](virtual-network-peering-overview.md) vagy; Ehhez hajtsa végre a [oktatóanyag](tutorial-connect-virtual-networks-portal.md).
...
1. Írja be a keresőmezőbe az Azure portál felső *virtuális hálózatok* be a keresőmezőbe. Ha **virtuális hálózatok** jelennek meg a keresési eredmények között, válassza ki azt. Ne válassza **virtuális hálózatok (klasszikus)** Ha, megjelenik a listában nem hozható létre, a klasszikus üzembe helyezési modellben telepített virtuális hálózati társviszony.
...
A fiókokat, akkor használhatja a virtuális hálózati társviszony-létesítés hozzá kell rendelni a következő szerepkörök:

- [A közreműködői hálózati](../role-based-access-control/built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#network-contributor): Resource Manager használatával telepített virtuális hálózat.
- [Klasszikus hálózat közreműködő](../role-based-access-control/built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#classic-network-contributor): a klasszikus üzembe helyezési modellben telepített virtuális hálózat.

Ha a fiók nincs hozzárendelve egyik korábbi szerepkör, akkor hozzá kell rendelni egy [egyéni szerepkör](../role-based-access-control/custom-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json) rendelt a szükséges lépéseket a következő táblázatból:

| Műveletek | Name (Név) |
|---|---|
| Microsoft.Network/virtualNetworks/virtualNetworkPeerings/write  | A virtuális hálózati társviszony b virtuális virtuális hálózat létrehozásához szükséges hálózati A virtuális hálózat (erőforrás-kezelő) kell lennie                            |
| Microsoft.Network/virtualNetworks/peer/action                   | Virtuális hálózat A virtuális hálózati B (erőforrás-kezelő) társviszony létrehozásához szükséges                                                                                |
| Microsoft.ClassicNetwork/virtualNetworks/peer                   | Virtuális hálózat A virtuális hálózat (klasszikus) B társviszony létrehozásához szükséges                                                                                    |
| Microsoft.Network/virtualNetworks/virtualNetworkPeerings/read   | Olvassa el a virtuális hálózati társviszony-létesítés   |
| Microsoft.Network/virtualNetworks/virtualNetworkPeerings/delete | Törölje a virtuális hálózati társviszony-létesítés |
