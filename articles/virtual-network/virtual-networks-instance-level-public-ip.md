...
[圖 1 所示，雲端服務使用 VIP 來存取，個別的 Vm 通常存取使用的 VIP: < 連接埠號碼 >。 將 ILPIP 指派給特定的 VM，就能直接使用該 IP 位址來存取 VM。
...
如果您想要能夠連線到您的 VM 或角色執行個體直接指派給它的 IP 位址，而不是使用雲端服務 VIP: < 連接埠號碼 >，要求您的 VM 或角色執行個體的 ILPIP。
