...
Azure 會將已部署它的子網路視為 **使用**。 使用中的子網路無法加以修改。 在修改之前，請將已部署到不同子網路的任何內容移到不修改的子網路。   請參閱 [將 VM 或角色執行個體移至不同的子網路](virtual-networks-move-vm-role-to-subnet.md)。
...
1. 登入 **管理入口網站**。
2. 在管理入口網站中，底部 **網路** 頁面上，按一下 **匯出**。 
3. 在 **匯出網路組態** ] 視窗中，確認您已選取您要匯出網路設定的訂閱。 然後，按一下右下方的核取記號。 
4. 當系統提示您時，儲存 *NetworkConfig.xml* 檔案至您所選擇的位置。


### 若要匯入網路設定

1. 在 **管理入口網站**, ，左下角的導覽窗格中按一下 [ **新增**。
2. 按一下 [ **網路服務** ]-> [ **虛擬網路** ]-> [ **匯入組態**。
3. 在 **匯入網路組態檔** 頁面中，瀏覽至您的網路組態檔，然後再按一下 **下一步** 箭號。
4. 在 **建置您的網路** ] 頁面上，您會將變更或建立的網路組態區段顯示在螢幕上看到資訊。 如果變更正確無誤，請按一下核取記號繼續更新或建立虛擬網路。 
