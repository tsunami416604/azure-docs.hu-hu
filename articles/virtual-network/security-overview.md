...
A következő szolgáltatáscímkék érhetőek el használatra [hálózati biztonsági csoportok szabályai](https://docs.microsoft.com/azure/virtual-network/security-overview#security-rules). Szolgáltatáscímkék csillag végén (azaz AzureCloud *) is használhatja a [Azure hálózati tűzfalszabályok](https://docs.microsoft.com/azure/firewall/service-tags). 

* **VirtualNetwork** (Resource Manager) (**VIRTUAL_NETWORK** klasszikus üzemi modell esetén): Ez a címke tartalmazza a virtuális hálózat címterét (az összes meghatározott CIDR-tartományt a virtuális hálózat), az összes csatlakoztatott helyszíni címteret [társviszonyban álló](virtual-network-peering-overview.md) virtuális hálózatokat vagy a virtuális hálózat csatlakozik egy [virtuális hálózati átjáró](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json) és oldja meg a használt előtagok [felhasználó által megadott útvonalak](virtual-networks-udr-overview.md). Vegye figyelembe, hogy ez a címke tartalmazhat alapértelmezett útvonalat. 
...
* **AzureCloud*** (csak Resource Manager): Ez a címke az IP-címteret például az összes Azure- [adatközpont nyilvános IP-címek](https://www.microsoft.com/download/details.aspx?id=41653). Ha az *AzureCloud* értéket adja meg, a nyilvános Azure IP-címekre irányuló forgalom engedélyezhető vagy letiltható. Ha szeretné egy adott AzureCloud való hozzáférés engedélyezése [régió](https://azure.microsoft.com/regions), megadhatja a régiót az alábbi formátumban AzureCloud. [ régió neve]. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **AzureTrafficManager*** (csak Resource Manager): Ez a címke azt jelzi, hogy az Azure Traffic Manager mintavételi IP-címeket az IP-címterét. További információk a Traffic Manager mintavételezési IP-címeiről: [Azure Traffic Manager – Gyakori kérdések](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs). Ez a címke bejövő biztonsági szabály használata ajánlott.  
* **Tárolási*** (csak Resource Manager): Ez a címke azt jelzi, hogy az Azure Storage szolgáltatás IP-címterét. Ha a *Storage* értéket adja meg, a Storage szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Csak szeretné-e egy adott tároló hozzáférésének engedélyezéséhez [régió](https://azure.microsoft.com/regions), a következő formátumban Storage is megadhat a régióban. [ régió neve]. A címke a szolgáltatást jelöli, annak adott példányait azonban nem. Például a címke az Azure Storage szolgáltatást jelöli, de nem egy adott Azure Storage-fiókot. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **SQL*** (csak Resource Manager): Ez a címke címelőtagjait Azure SQL Database, Azure Database for MySQL, Azure Database for PostgreSQL, és az Azure SQL Data Warehouse szolgáltatásokhoz. Ha az *SQL* értéket adja meg, az SQL szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ha szeretné egy adott Sql való hozzáférés engedélyezése [régió](https://azure.microsoft.com/regions), Sql. [régió neve] a következő formátumban is megadhat a régióban. A címke a szolgáltatást jelöli, annak adott példányait azonban nem. Például a címke az Azure SQL Database szolgáltatást jelöli, de nem egy adott SQL-adatbázist vagy -kiszolgálót. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **AzureCosmosDB*** (csak Resource Manager): Ez a címke az Azure Cosmos Database szolgáltatás címelőtagjait. Ha az *AzureCosmosDB* értéket adja meg, az AzureCosmosDB szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ha csak egy adott [régióban](https://azure.microsoft.com/regions) szeretné engedélyezni a hozzáférést az AzureCosmosDB szolgáltatáshoz, a régiót az AzureCosmosDB.[régió neve] formátumban adhatja meg. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **Azurekeyvault értékre van*** (csak Resource Manager): Ez a címke az Azure KeyVault szolgáltatás címelőtagjait. Ha az *AzureKeyVault* értéket adja meg, az AzureKeyVault szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ha csak egy adott [régióban](https://azure.microsoft.com/regions) szeretné engedélyezni a hozzáférést az AzureKeyVault szolgáltatáshoz, a régiót az AzureKeyVault.[régió neve] formátumban adhatja meg. Ez a címke függőségi rendelkezik a **AzureActiveDirectory** címke. Ez a címke ajánlott kimenő biztonsági szabályt.  
* **Az EventHub*** (csak Resource Manager): Ez a címke az Azure Event hubhoz szolgáltatás címelőtagjait. Ha az *EventHub* értéket adja meg, az EventHub szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ha csak egy adott [régióban](https://azure.microsoft.com/regions) szeretné engedélyezni a hozzáférést az EventHub szolgáltatáshoz, a régiót az EventHub.[régió neve] formátumban adhatja meg. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **A ServiceBus*** (csak Resource Manager): Ez a címke az Azure Szolgáltatásbusz-szolgáltatás, a prémium szintű szolgáltatási szintet címelőtagjait. Ha a *ServiceBus* értéket adja meg, a ServiceBus szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ha csak egy adott [régióban](https://azure.microsoft.com/regions) szeretné engedélyezni a hozzáférést a ServiceBus szolgáltatáshoz, a régiót a ServiceBus.[régió neve] formátumban adhatja meg. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **MicrosoftContainerRegistry*** (csak Resource Manager): Ez a címke a Microsoft Container Registry szolgáltatás címelőtagjait. Ha a *MicrosoftContainerRegistry* értéket adja meg, a MicrosoftContainerRegistry felé irányuló forgalom engedélyezhető vagy letiltható. Ha csak egy adott [régióban](https://azure.microsoft.com/regions) szeretné engedélyezni a hozzáférést a MicrosoftContainerRegistry szolgáltatáshoz, a régiót a MicrosoftConainerRegistry.[régió neve] formátumban adhatja meg. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **AzureContainerRegistry*** (csak Resource Manager): Ez a címke az Azure Container Registry szolgáltatás címelőtagjait. Ha az *AzureContainerRegistry* értéket adja meg, az AzureContainerRegistry felé irányuló forgalom engedélyezhető vagy letiltható. Ha csak egy adott [régióban](https://azure.microsoft.com/regions) szeretné engedélyezni a hozzáférést az AzureContainerRegistry szolgáltatáshoz, a régiót az AzureConainerRegistry.[régió neve] formátumban adhatja meg. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **Az App Service*** (csak Resource Manager): Ez a címke az Azure App Service szolgáltatás címelőtagjait. Ha az *AppService* értéket adja meg, az AppService szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ha csak egy adott [régióban](https://azure.microsoft.com/regions) szeretné engedélyezni a hozzáférést az AppService szolgáltatáshoz, a régiót az AppService.[régió neve] formátumban adhatja meg. Ez a címke a Web Apps és előtérrendszerek ajánlott kimenő biztonsági szabályt.  
* **AppServiceManagement*** (csak Resource Manager): Ez a címke a felügyeleti forgalom dedikált App Service Environment-környezetekben címelőtagjait. Ha az *AppServiceManagement* értéket adja meg, az AppServiceManagement szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ez a címke bejövő/kimenő biztonsági szabály használata ajánlott. 
* **Az ApiManagement*** (csak Resource Manager): Ez a címke azt jelzi, hogy a címelőtagokat a felügyeleti forgalom az APIM-dedikált központi telepítések. Ha az *ApiManagement* értéket adja meg, az ApiManagement szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ez a címke bejövő/kimenő biztonsági szabály használata ajánlott. 
* **AzureConnectors*** (csak Resource Manager): Ez a címke a Logic Apps-összekötők mintavételi/háttér-kapcsolatok címelőtagjait. Ha az *AzureConnectors* értéket adja meg, az AzureConnectors szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ha csak egy adott [régióban](https://azure.microsoft.com/regions) szeretné engedélyezni a hozzáférést az AzureConnectors szolgáltatáshoz, a régiót az AzureConnectors.[régió neve] formátumban adhatja meg. Ez a címke bejövő biztonsági szabály használata ajánlott. 
* **GatewayManager** (csak Resource Manager): Ez a címke a felügyeleti forgalom dedikált környezetekben VPN/alkalmazás átjárók címelőtagjait. Ha a *GatewayManager* értéket adja meg, a GatewayManager szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ez a címke bejövő biztonsági szabály használata ajánlott. 
* **AzureDataLake*** (csak Resource Manager): Ez a címke az Azure Data Lake szolgáltatás címelőtagjait. Ha az *AzureDataLake* értéket adja meg, az AzureDataLake szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **AzureActiveDirectory*** (csak Resource Manager): Ez a címke az AzureActiveDirectory szolgáltatás címelőtagjait. Ha az *AzureActiveDirectory* értéket adja meg, az AzureActiveDirectory szolgáltatás felé irányuló forgalom engedélyezhető vagy letiltható. Ez a címke ajánlott kimenő biztonsági szabályt.
* **AzureMonitor*** (csak Resource Manager): Ez a címke a Log Analytics, az App Insights, AzMon és egyéni metrikákat (GB-os végpontok) címelőtagjait. Ha megad *AzureMonitor* értékénél, engedélyezett vagy tiltott forgalmat a AzureMonitor. A Log Analytics, ez a címke rendelkezik függőségi a **tárolási** címke. Ez a címke ajánlott kimenő biztonsági szabályt.
* **ServiceFabric*** (csak Resource Manager): Ez a címke a ServiceFabric-szolgáltatás címelőtagjait. Ha megad *ServiceFabric* értékénél, engedélyezett vagy tiltott forgalmat ServiceFabric való. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **AzureMachineLearning*** (csak Resource Manager): Ez a címke a AzureMachineLearning szolgáltatás címelőtagjait. Ha megad *AzureMachineLearning* értékénél, engedélyezett vagy tiltott forgalmat a AzureMachineLearning. Ez a címke ajánlott kimenő biztonsági szabályt. 
* **BatchNodeManagement*** (csak Resource Manager): Ez a címke az Azure Batch dedikált központi telepítéséhez a felügyeleti forgalom címelőtagjait. Ha megad *BatchNodeManagement* értékénél, engedélyezett vagy tiltott forgalmat a Batch szolgáltatás a számítási csomópontokra. Ez a címke bejövő/kimenő biztonsági szabály használata ajánlott. 
...
### <a name="service-tags-in-on-premises"></a>A helyszíni szolgáltatáscímkék  
Letöltheti és integrálása a helyszíni tűzfalon szolgáltatáscímkék az Azure-ban a következő heti kiadványok előtag részletes listáját [nyilvános](https://www.microsoft.com/download/details.aspx?id=56519), [Egyesült Államokbeli kormányzati](https://www.microsoft.com/download/details.aspx?id=57063), [Kína ](https://www.microsoft.com/download/details.aspx?id=57062), és [Németország](https://www.microsoft.com/download/details.aspx?id=57064) felhőkben.

Szoftveresen is lekérheti az adatokat a a **szolgáltatás címke felderítési API** (nyilvános előzetes verzió) – [REST](https://aka.ms/discoveryapi_rest), [Azure PowerShell-lel](https://aka.ms/discoveryapi_powershell), és [ Az Azure CLI](https://aka.ms/discoveryapi_cli). 

> [!NOTE]
> Heti kiadványok (régi verzió) az Azure a következő [nyilvános](https://www.microsoft.com/en-us/download/details.aspx?id=41653), [Kína](https://www.microsoft.com/en-us/download/details.aspx?id=42064), és [Németország](https://www.microsoft.com/en-us/download/details.aspx?id=54770) felhők 2020. június 30-as elavulttá válik. Indítsa el a frissített kiadványok használatával a fent leírtak szerint. 
