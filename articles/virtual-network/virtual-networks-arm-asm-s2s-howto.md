...
>[AZURE.NOTE] 本文假設您已經有傳統 Vnet 和 ARM Vnet，而且您已熟悉設定傳統 vnet 的 S2S VPN 連線。 詳細的端對端解決方案之間的 S2S VPN 連線和 ARM Vnet，請瀏覽 [解決方案指南-使用 S2S VPN 連線到傳統 VNet 和 ARM VNet](../virtual-networks-arm-asm-s2s.md)。
...
2. 在左下角的畫面，按一下 [ **新增** ] 按鈕，然後按一下 [ **網路服務**, ，然後按一下 [ **虛擬網路**, ，然後按一下 [ **新增區域網路**。
3. 在 **指定本機網路詳細資料** 視窗中，輸入您想要連線，然後在視窗的右下角的 ARM VNet 的名稱，按一下箭號按鈕。
3. 在位址空間 **起始 IP** 文字方塊中，輸入您想要連接到 ARM vnet 的網路首碼。 
4. 在 **CIDR （位址計數）** 下拉式清單中，選取您想要連接到 ARM VNet 所使用的 CIDR 區塊的網路部分所使用的位元數。
5. 在 **VPN 裝置 IP 位址 （選擇性）**, ，輸入任何有效的公用 IP 位址。 我們將在稍後變更此 IP 位址。 然後，按一下畫面右下方的核取記號按鈕。 下圖顯示此頁面的範例設定。

    ![區域網路設定](..\virtual-network\media\virtual-networks-arm-asm-s2s-howto\figurex1.png)

5. 在 **網路** 頁面上，按一下 **虛擬網路**, 傳統 VNet，然後按一下，然後按一下 [ **設定**。
6. 在 **站台對站連線能力** 啟用 **連線到區域網路** 核取方塊。
7. 選取您在步驟 4 的可用網路清單中所建立的本機網路 **區域網路** 下拉式清單，以及 [ **儲存**。
8. 設定儲存之後，按一下 **儀表板**, ，然後在頁面底部，按一下 [ **建立閘道**, ，然後按一下 [ **動態路由**, ，然後按一下 [ **是**。
...
4. 執行下列命令，來擷取用於閘道的子網路。
    ...
    >[AZURE.IMPORTANT] 閘道子網路必須已經存在，而且它必須命名為 GatewaySubnet。
...
5. 執行下列命令，來建立閘道的 IP 組態物件。 請注意閘道子網路的識別碼。 該子網路必須存在於 VNet 中。
    ...
    >[AZURE.IMPORTANT]  *SubnetId* 和 *PublicIpAddressId* 參數必須從子網路和 IP 位址的物件，分別傳遞 id 屬性。 您無法使用簡單的字串。
...
1. 開啟 https://manage.windowsazure.com，從傳統入口網站，並視需要輸入您的認證。
2. 在傳統入口網站中，向下捲動並按一下 [ **網路**, ，然後按一下 [ **區域網路**, ，然後按一下您想要連線，然後按一下 [ARM VNet **編輯** ] 按鈕。
3. 在 **VPN 裝置 IP 位址 （選擇性）**, ，輸入 ARM VNet 閘道擷取上述，步驟 2 中的 IP 位址，然後按一下右下角，向右箭頭，然後按一下核取記號按鈕。
