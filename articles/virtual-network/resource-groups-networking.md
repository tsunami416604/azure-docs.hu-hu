...
- **中繼資料** – 您可以將資訊新增至使用標記的資源。 這些標記可以用來追蹤資源群組和訂用帳戶中的資源使用。
- **更進一步控制您的網路** -網路資源進行鬆散偶合，並以更細微的方式加以控制。 這表示可以更彈性地管理網路資源。
- **設定更快速** -因為網路資源進行鬆散偶合，您可以建立並協調網路資源，以平行方式。 這已經大幅減少設定時間。
- **角色型存取控制** -RBAC 還提供具有特定安全性範圍，除了允許建立自訂角色進行安全管理預設角色。 
- **更易於管理和部署** -很容易部署和管理應用程式，因為您可以建立整個應用程式堆疊做為資源群組中資源的單一集合。 而且可以更快速地部署，因為只要提供範本 JSON 裝載即可進行部署。
- **快速自訂** -您可以使用宣告樣式範本，以可重複且快速自訂的部署。 
- **可重複自訂** -您可以使用宣告樣式範本，以可重複且快速自訂的部署。
- **管理介面** -您可以使用任何下列介面來管理您的資源 ︰
    - REST 型 API
    - PowerShell
    - .NET SDK
    - Node.JS SDK
...
|屬性|說明|範例值|
|---|---|---|
|**名稱**|唯一的資源名稱。 每個資源類型都有自己的命名限制。|PIP01、VM01、NIC01|
|**location**|資源所在的 Azure 區域|westus、eastus|
...
- **訂用帳戶 id** -Azure 訂閱 id。
- **資源提供者命名空間** -正在使用之提供者的命名空間。 網路資源提供者的值是 *Microsoft.Network*。
- **區域名稱** -Azure 區域名稱

呼叫 REST API 時，支援下列 HTTP 方法：

- **將** -用來建立特定類型的資源、 修改資源屬性，或變更資源之間的關聯。 
- **取得** -用來擷取已佈建資源的資訊。
- **刪除** -用來刪除現有的資源。
...
2. 按一下 [ **部署至 Azure**, ，然後登入 Azure 入口網站使用您的認證。
3. 驗證範本，然後再按一下 **儲存**。
4. 按一下 [ **編輯參數** 並選取位置，例如 *美國西部*, 、 vnet 和子網路。
5. 如有必要，變更 **ADDRESSPREFIX** 和 **SUBNETPREFIX** 參數，然後再按一下 **確定**。
6. 按一下 [ **選取資源群組** 然後按一下您想要新增 vnet 和子網路的資源群組。 或者，您可以建立新的資源群組，藉以 **或建立新**。
3. 按一下 [ **建立**。 請注意磚顯示 **佈建範本部署**。 完成部署之後，您將會看到類似下面所示的畫面。
