...
- **Azure 資料湖儲存區帳戶**。 如需如何建立一個指示，請參閱 [開始使用 Azure 資料湖市集](data-lake-store-get-started-portal.md)
...
4. 在 [使用者] ****刀鋒視窗中，按一下 [新增]**** 以開啟 [新增存取]**** 刀鋒視窗。 在 [新增存取] ****刀鋒視窗中，按一下 [選取角色]****，然後選取使用者/群組的角色。
    ...
    [擁有者]**** 和 [參與者]**** 角色會提供資料湖帳戶上各種不同管理功能的存取。 針對與資料湖中的資料互動的使用者，您可以將它們加入 ** 讀取器 **角色。 這些角色的範圍僅限於與 Azure 資料湖儲存區帳戶相關的管理作業。
...
6. 在 [新增自訂存取]**** 刀鋒視窗中，按一下 [確定]****。 具有相關權限的新增群組現在會列在 [存取]**** 刀鋒視窗中。
    ...
    > [AZURE.IMPORTANT] 在目前版本中，[自訂存取]**** 下僅可有 12 個項目。 若要新增 12 位以上的使用者，您必須建立安全性群組、新增使用者至安全性群組，並且新增存取權限給該資料湖儲存區帳戶的安全性群組。
...
- [從 Azure 儲存體 Blob 複製資料到資料湖存放區](data-lake-store-copy-data-azure-storage-blob.md)
