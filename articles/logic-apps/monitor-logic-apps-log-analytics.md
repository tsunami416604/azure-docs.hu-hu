...
1. A [Azure Portal](https://portal.azure.com)keresési mezőjébe írja be a `log analytics workspaces`, majd válassza a **log Analytics munkaterületek**lehetőséget.
...
1. A **piactér** megnyitása után a keresőmezőbe írja be a `logic apps management`kifejezést, majd válassza a **Logic apps felügyelet**lehetőséget.
...
1. A logikai alkalmazás menüjének **figyelés**területén válassza a **diagnosztikai beállítások** > **diagnosztikai beállítás hozzáadása**elemet.
...
Ez a példa azt szemlélteti, hogyan tartalmazza a `ActionCompleted` esemény a `clientTrackingId` és a `trackedProperties` attribútumot:
...
* [Figyelési és nyomkövetési lekérdezések létrehozása](../logic-apps/create-monitoring-tracking-queries.md)
