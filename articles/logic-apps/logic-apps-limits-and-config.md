...
* Minden ISE – akár [fejlesztő](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#ise-level), akár prémium – 5 teljes integrációs fiókra korlátozódik:
  ...
  |---------|----------------------------|
  | **Prémium szintű** | 5 teljes [standard](../logic-apps/logic-apps-pricing.md#integration-accounts) fiók, beleértve az ingyenes egy standard fiókot is. Ingyenes vagy alapszintű fiókok használata nem engedélyezett. |
...
> Ez a címke azon régiók között működik, ahol a Logic Apps szolgáltatás elérhető.
> A felügyelt összekötők esetében továbbra is az IP-címeket kell használnia.
