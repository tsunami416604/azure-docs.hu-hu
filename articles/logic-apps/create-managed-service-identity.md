...
Ez a cikk bemutatja, hogyan állíthat be mindkét fajta felügyelt identitást a logikai alkalmazáshoz. További információkért tekintse meg a következő témaköröket:
...
1. A logikai alkalmazás menüjének **Beállítások**területén válassza az **identitás**elemet. Válassza ki **a** **rendszerhez rendelt** >  > **Mentés**elemre. Ha az Azure kéri, hogy erősítse meg, válassza az **Igen**lehetőséget.
...
1. A [Azure Portal](https://portal.azure.com)bármely oldal keresőmező mezőjébe írja be a `managed identities`, majd válassza a **felügyelt identitások**lehetőséget.
...
1. Adja meg a felügyelt identitásával kapcsolatos információkat, majd válassza a **Létrehozás**lehetőséget, például:
   ...
   |----------|----------|-------|-------------|
   | **Erőforrás neve** | Igen | <*felhasználó által hozzárendelt identitás-név*> | A felhasználó által hozzárendelt identitás nevét adja meg. Ez a példa a "Fabrikam-User-Assigned-Identity" kifejezést használja. |
   | **Előfizetés** | Igen | <*Azure-előfizetés-neve*> | A használni kívánt Azure-előfizetés neve |
   | **Erőforráscsoport** | Igen | <*Azure-Erőforrás-csoport neve*> | A használni kívánt erőforráscsoport neve. Hozzon létre egy új csoportot, vagy válasszon ki egy meglévő csoportot. Ez a példa létrehoz egy "Fabrikam-Managed-idents-RG" nevű új csoportot. |
...
1. A logikai alkalmazás menüjének **Beállítások**területén válassza az **identitás**lehetőséget, majd válassza a **felhasználó által hozzárendelt** > **Hozzáadás**lehetőséget.
...
* Az `type` tulajdonsággal rendelkező `identity` objektum `UserAssigned`

* Egy gyermek `userAssignedIdentities` objektum, amely meghatározza az identitás erőforrás-AZONOSÍTÓját, amely egy másik gyermekobjektum, amely a `principalId` és `clientId` tulajdonságokkal rendelkezik.

Ez a példa egy logikai alkalmazás erőforrás-definícióját mutatja be egy HTTP PUT-kérelemhez, és tartalmaz egy nem paraméteres `identity` objektumot. A PUT kérelemre és az azt követő lekérési műveletre adott válasz ezen `identity` objektummal is rendelkezik:
...
| Tulajdonság (JSON) | Érték | Leírás |
|-----------------|-------|-------------|
| `principalId` | <*rendszerbiztonsági tag azonosítója*> | A felhasználó által hozzárendelt felügyelt identitás globálisan egyedi azonosítója (GUID) az Azure AD-bérlőben |
...
Ha a sablon a felügyelt identitás erőforrás-definícióját is tartalmazza, akkor parametrizálja a `identity` objektumot. Ez a példa azt szemlélteti, hogyan hivatkozik a gyermek `userAssignedIdentities` objektum a sablon `variables` szakaszában meghatározott `userAssignedIdentity` változóra. Ez a változó a felhasználó által hozzárendelt identitás erőforrás-AZONOSÍTÓJÁRA hivatkozik.
...
| Tulajdonság (JSON) | Érték | Leírás |
|-----------------|-------|-------------|
| `tenantId` | <*Azure-ad-bérlői azonosító*> | A globálisan egyedi azonosító (GUID), amely az Azure AD-bérlőt jelképezi, ahol a felhasználó által hozzárendelt identitás most már tagja. Az Azure AD-bérlőn belül az egyszerű szolgáltatásnév neve megegyezik a felhasználó által hozzárendelt identitás nevével. |
| `principalId` | <*rendszerbiztonsági tag azonosítója*> | A felhasználó által hozzárendelt felügyelt identitás globálisan egyedi azonosítója (GUID) az Azure AD-bérlőben |
| `clientId` | <*ügyfél-azonosító*> | Egy globálisan egyedi azonosító (GUID) a logikai alkalmazás új identitásához, amelyet a rendszer a Futtatás során hívásokhoz használ |
||||
...
1. A logikai alkalmazás menüjének **Beállítások**területén válassza az **identitás**lehetőséget, majd kövesse az identitásának lépéseit:
   ...
   * Válassza ki **a** **rendszerhez rendelt** >  > **Mentés**elemre. Ha az Azure kéri, hogy erősítse meg, válassza az **Igen**lehetőséget.
...
Ha a logikai alkalmazás felügyelt identitását egy Azure Resource Manager sablonnal hozta létre, akkor a `identity` objektum `type` gyermek tulajdonságát `None`értékre kell állítani. A rendszer által felügyelt identitás esetében ez a művelet törli a résztvevő AZONOSÍTÓját is az Azure AD-ből.
