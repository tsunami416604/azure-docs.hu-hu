...
Silverlight 的 Mobile Engagement SDK for Windows 是以 Nuget 套件，稱為可用 *MicrosoftAzure.MobileEngagement*。 您可以從 Visual Studio Nuget 封裝管理員安裝該封裝。 
...
> [AZURE.WARNING] 我們強烈建議您不要在您的應用程式的其他地方加入 Engagement 初始化。 不過請注意，`EngagementAgent.Instance.Init` 方法執行於專用的執行緒，而非 UI 執行緒。
...
> [AZURE.WARNING] 如果您的頁面繼承自 `OnNavigatedTo` 方法，請注意要讓 `base.OnNavigatedTo(e)` 呼叫。 否則不會報告活動。 事實上，`EngagementPage` 會呼叫 `OnNavigatedTo` 方法內的 `StartActivity`。
...
>
> SDK 會在應用程式關閉時自動呼叫 `EndActivity` 方法。 因此，它是 **高** 建議您呼叫 `StartActivity` 方法，每當使用者的活動變更時，以及 **永不** 呼叫 `EndActivity` 方法。 這個方法會將訊息傳送給 Engagement 伺服器，目前的使用者已離開應用程式，這會影響所有應用程式記錄檔。
...
> [AZURE.WARNING] 如果您打算停用此功能，請注意，當您的應用程式中，將發生未處理的當機，Engagement 將不會傳送該當機 **AND** 不會關閉工作階段和作業。
...
引數是中的值 **毫秒**。 如果您想要重新啟動及時記錄，您隨時可以呼叫該方法，而不需要使用任何參數 (或使用 0 為值)。
