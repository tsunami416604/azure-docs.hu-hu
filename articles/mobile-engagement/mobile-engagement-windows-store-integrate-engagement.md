...
Mobile Engagement SDK 的 Windows 通用應用程式是以 Nuget 套件，稱為可用 *MicrosoftAzure.MobileEngagement*。 您可以從 Visual Studio Nuget 封裝管理員安裝該封裝。
...
3.  將拖放 **資源** 資料夾從 [檔案總管] 在 Visual Studio 專案的根目錄。
4.  在 Visual Studio 中，選取您的專案並啟動 **顯示所有檔案** 圖示的上方 **方案總管] 中**。
5.  部分檔案未包含在專案中。 匯入它們一次以滑鼠右鍵按一下 **資源** 資料夾中， **從專案排除** 另一個上按一下滑鼠右鍵，然後 **資源** 資料夾中， **加入至專案** 重新包含整個資料夾。 中的所有檔案 **資源** 資料夾現在包含在專案中。

擷取的 Engagement 封裝也可以找到上 *$(Solutiondir) \Packages* 中您所定義或 *NuGet.config* 檔案。
...
> [AZURE.IMPORTANT] 我們強烈建議您不要在您的應用程式的其他地方加入 Engagement 初始化。
...
> [AZURE.IMPORTANT] 如果您的頁面會覆寫 `OnNavigatedTo` 方法，請務必呼叫 `base.OnNavigatedTo(e)`。 否則，不會報告活動 ( `EngagementPage` 呼叫 `StartActivity` 內其 `OnNavigatedTo` 方法)。
...
> 
> 應用程式關閉時，Windows 通用 SDK 會自動呼叫 `EndActivity` 方法。 因此，它是 **高** 建議您呼叫 `StartActivity` 方法，每當使用者的活動變更時，以及 **永不** 呼叫 `EndActivity` 方法，這個方法會傳送至 Engagement 伺服器，目前使用者已離開應用程式，這會影響所有應用程式記錄檔。
...
> [AZURE.WARNING] 高載閾值無法設定為小於 1 的時間。 如果您嘗試這樣做，SDK 會顯示含錯誤訊息的追蹤，並且會自動重設為預設值 (0 秒)。 這樣會觸發 SDK 以即時的方式報告記錄檔。
