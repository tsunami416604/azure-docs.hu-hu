...
> [AZURE.IMPORTANT] 一般情況下，您可以使用這個範例做為起點，了解如何呼叫任何 Mobile Engagement REST API，因為它詳細說明呼叫 Api 的驗證和傳遞參數的兩個重要層面。 

## SharePoint 整合
1. 以下是範例 SharePoint 清單的類似外觀。 **標題**, ，**類別**, ，**NotificationTitle**, ，**訊息** 和 **URL** 用來建立公告。 沒有資料行稱為 **IsProcessed** 主控台程式的表單範例自動化程序會使用它。 您可以執行這個主控台程式做為 Azure WebJob，讓您可以排程至程式或直接使用 SharePoint 工作流程，在項目插入至 SharePoint 清單時建立和啟動公告。 在此範例中，我們使用主控台程式，會透過 SharePoint 中的項目清單，並在 Azure Mobile Engagement 中建立的通知，每個訊息，並將 **IsProcessed** 旗標標示為成功建立公告時，則為 true。
...
3. 驗證之後，我們循環清單項目，以找出任何新建立的項目 (其 **IsProcessed** = false)。 
...
2.  Mobile Engagement REST Api 需要 **基本驗證配置授權 HTTP 標頭** 這組成 `ApplicationId` 和 `ApiKey` 您從 Azure 入口網站取得。 請確定您使用的索引鍵 **api 金鑰** 區段和 *不* 從 **sdk 金鑰** 一節。 
...
3. 建立公告類型行銷活動-參考 [文件](https://msdn.microsoft.com/library/dn913754.aspx)。 您必須先確認您要指定活動 `kind` 為 *公告* 和 [裝載](https://msdn.microsoft.com/library/dn913749.aspx) 並將它傳遞為 FormUrlEncodedContent。 
...
5. `CreateAzMECampaign` 建立公告行銷活動，並傳回給呼叫者識別碼。 `ActivateAzMECampaign` 需要此識別碼做為參數來啟動活動。 
