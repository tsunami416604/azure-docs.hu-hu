...
您可以使用 **KPI 產生器** 工作表中的我們 [媒體腳本範本][Media Playbook link] 如需範例和指引。
...
5. **網頁檢視** ︰ 網頁檢視是快顯的包含 web 內容。 使用者按下推送通知時，就會出現這個快顯視窗。 網頁檢視可讓您與使用者有更多互動。
...
使用 **Engagement 程式** 工作表中的 [媒體腳本範本][Media Playbook link] 如需範例和指引。
...
標記計劃不應標記應用程式中的所有項目。 它只應包含屬於行動業務開發策略的標記資料。 這可能隨著應用程式而有所不同。  [媒體腳本範本][Media Playbook link] 提供 Azure Mobile Engagement 可協助您建立的標記計劃與指定的方法。 使用 **標記計劃** 工作表做為建置您的標記計劃的指南。
