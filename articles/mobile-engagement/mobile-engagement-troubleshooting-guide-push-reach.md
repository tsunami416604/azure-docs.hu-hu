...
- 如果憑證無效，或正在正確使用 PROD 與DEV 正確 (僅限 iOS)。 (**附註 ︰** 」 應用程式外 」 推播通知無法傳遞到 iOS，如果您有兩種開發 (DEV) 和生產 (PROD) 安裝在相同的裝置，因為您憑證關聯的安全性權杖的應用程式可能會由 Apple 作廢。 若要解決這個問題，請先解除安裝 DEV 和 PROD 版本的應用程式，然後在您的裝置上只安裝其中一個版本。)
...
- 按一下推播通知後，重新導向使用者到替代位置的深層連結配置，必須由應用程式與裝置作業系統建立及管理，而非直接透過 Mobile Engagement。 (**附註 ︰** 應用程式外通知無法直接連結到 iOS 應用程式位置能與 Android。)
