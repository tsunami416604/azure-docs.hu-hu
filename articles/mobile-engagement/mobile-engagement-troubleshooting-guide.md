...
1. 請確定您已經完成所有步驟中所述整合所需我們 [快速入門教學課程](mobile-engagement-windows-store-dotnet-get-started.md)
2. 您使用的是最新版的平台 SDK。 
3. 同時在實際裝置和模擬器上進行測試，因為有些問題是只有模擬器才有的。 
4. 您不會達到任何限制/節流閥，只要從 Mobile Engagement 方面 [這裡](azure-subscription-service-limits.md)
...
如果您是目前連接到平台的唯一使用者，且您正使用模擬器來開啟應用程式，原因可能是模擬器的問題。 一般而言，您必須確實回到模擬器上的 [首頁] 畫面，應用程式工作階段才會成功中斷連接。 此外，Windows 平台上，使用 Visual Studio 中，偵錯時您可能需要確保，在 Visual Studio 中，您前往 **生命週期事件** 功能表列，然後按一下 [ **暫止** 真正關閉工作階段。 請參閱 [Windows 教學課程](mobile-engagement-windows-store-dotnet-get-started.md) 如需詳細資訊。 
...
目前不會即時建立區段。 在彙總分析資料時會同時計算區段，因此最多可能需要 24 小時的時間。 請稍後再回來查看，但在此同時，您也應該確定您的行動應用程式確實有傳送您據以形成區段的資料。 例如 如果事件說 'foo' 不所傳送的任何行動裝置，則不會有任何區段的資料區段建立的事件名稱，= foo，做為準則。 您也應該檢查您的 SDK 整合情形，確定您的行動應用程式會正確地傳送資料。 
...
6. 請確定您正在未設定 *略過對象，推送將傳送給使用者透過 API* 選項 **活動** 區段觸達活動，因為這樣可確保只可以透過 Api 傳送推播通知。 
 ...
 **活動** 您在 Mobile Engagement 中建立會呼叫，因此當新裝置連線到您的行動 engagement 平台是長時間執行的推播通知意義，因為它們會自動傳送的通知您在此處，設定，只要它們符合您在活動中設定的準則。 這不是一次性的單一通知設定。 您必須手動按一下 **完成** ] 按鈕，結束活動，使它不會傳送通知。 
...
2. 如果您找到的功能，然後加入/投票要求遺漏上我們 [UserVoice 論壇](http://feedback.azure.com/forums/285737-mobile-engagement)
