...
本文說明 **分析** ] 索引標籤的 **Mobile Engagement** 入口網站。 您使用 **Mobile Engagement** 入口網站來監視和管理行動應用程式。 請注意，若要使用入口網站您首先需要建立 **Azure Mobile Engagement** 帳戶。 如需詳細資訊，請參閱 [建立 Azure Mobile Engagement 帳戶](mobile-engagement-create-account.md)。
...
您只要將應用程式與 SDK 整合，Azure Mobile Engagement 就可針對您的應用程式提供一組基本、標準的可圖形化分析資訊。 Azure Mobile Engagement 也能讓您收集有關使用者行為的其他自訂分析資訊。 您可以藉由建立自訂 「 Tags (應用程式資訊) 」，從建立一個標記計畫 **設定** 如此 Azure Mobile Engagement 可讓您收集這類額外資料。
