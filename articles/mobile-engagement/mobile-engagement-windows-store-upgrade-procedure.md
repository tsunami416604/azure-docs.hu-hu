...
4. 重疊項目頁面變更
    > [AZURE.IMPORTANT] 重疊項目也會變更。 新的命名空間 `Microsoft.Azure.Engagement.Overlay`。 在 xaml 和 cs 檔案中都必須使用它。 此外 `CapptainGrid` 命名為 `EngagementGrid`, ，`capptain_notification_content` 和 `capptain_announcement_content` 命名 `engagement_notification_content` 和 `engagement_announcement_content`。
...
Package.appxmanifest 上 `檔案類型關聯` 已經更新自:
