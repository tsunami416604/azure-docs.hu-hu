...
> [AZURE.IMPORTANT] Engagement SDK 需要 iOS6 +: 您的應用程式的部署目標必須至少為 iOS 6。
...
-   方法內初始化 Engagement '**applicationDidFinishLaunching:**'**application: didFinishLaunchingWithOptions:**':
...
在 iOS 上的使用 engagement API (以及技術的文件的 `EngagementAgent` 類別)。
...
您可以藉由檢查 Engagement 測試記錄檔，確認 IDFA 集合已在應用程式中正確停用。 請參閱整合測試 < ios-sdk-部署-測試-idfa\ > 文件以取得進一步資訊。
