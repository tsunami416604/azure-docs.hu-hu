...
最快的方法新增至您的專案中的程式庫 **Eclipse** 是 `Right click on your project -> Android Tools -> Add Support Library...`。

如果您未使用 Eclipse，您可以閱讀指示 [here]。
...
> [AZURE.IMPORTANT] 這個區段是 **強制** 如果您打算在建立觸達活動時，使用系統通知。 Android 禁止顯示沒有圖示的系統通知。 因此如果您省略此區段，使用者將無法接收系統通知。
...
您可以決定要將通知配置納入您現有的配置 android **包括** 陳述式。 以下是修改過的 `ListActivity` 配置範例，且其中只包含 `ListView`。
...
> [AZURE.TIP] 如果您在應用程式中使用 ListActivity，可見的 Reach 覆疊會使您再按一下 [清單] 檢視中的項目做出反應。 這是已知的問題。 若要暫時解決這個問題，我們建議您在自己的清單活動配置中內嵌通知配置，就像在先前範例那樣。
...
> [AZURE.IMPORTANT] 請閱讀關於 android: process 屬性的警告 \ < android-sdk-engagement-處理序 > 整合 Engagement 如何在 Android 的主題，再繼續進行。
