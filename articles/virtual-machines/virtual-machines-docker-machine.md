...
本主題描述如何使用 [Docker](https://www.docker.com/) 與 [機器](https://github.com/docker/machine) 和 [Azure CLI](https://github.com/Azure/azure-xplat-cli) 來建立 Azure 虛擬機器，快速且輕鬆地管理 Linux 容器從執行 Ubuntu 的電腦。 為了示範，本教學課程示範如何同時部署 [busybox Docker 中樞映像](https://registry.hub.docker.com/_/busybox/) 映像以及 [nginx Docker 中樞映像](https://registry.hub.docker.com/_/nginx/) 和設定容器以將 web 要求路由傳送到 nginx 容器。 (Docker **machine** 文件說明如何針對其他平台修改這些指示)。
...
若要開始使用最快的方式 **docker-machine-** 是下載適當的版本，直接從 [版本共用](https://github.com/docker/machine/releases)。 本教學課程中的用戶端電腦是在 x64 電腦上執行 Ubuntu，因此，會用到 **docker-machine_linux-amd64** 映像。
...
>  不論您選擇哪一個方法，都必須在命令列上直接呼叫二進位檔，或者放置二進位檔的路徑，例如 **/usr/local/bin**。 請確定輸入將它標示為可執行檔，請記得 `chmod + x` < *`binaryName`*>，<*`binaryName`* > 是 Docker machine 可執行檔的名稱。 本教學課程使用 **docker-machine_linux-amd64**。
...
> [AZURE.NOTE] 如果您嘗試 **docker-machine-** 和上一次，要確定您重複使用相同的憑證和金鑰檔。 **docker-machine-** 建立一組用戶端憑證--檢查它所建立的所有項目中 `~/.docker/machine`。 如果您將這些憑證移至另一部電腦，也需要移動 **docker-machine** 憑證資料夾。 這和您即將在另一個平台上使用 **docker-machine** 的情況不同，例如，只想查看它如何運作。
...
3. 在入口網站的同一個 [設定]**** 窗格中，按一下 [訂戶帳戶]****，以擷取要在您建立 VM 時使用的訂用帳戶識別碼，因為您將在下一個步驟中用到它  (您也可以使用 Azure CLI 命令的命令列上找到訂用帳戶 id `azure 帳戶清單`, ，其中顯示您擁有每個訂閱的訂閱 id 的帳戶中。)
...
4. 使用 **docker-machine create** 命令，在 Azure 上建立 docker 主機 VM。 此命令需要您剛在上一個步驟中擷取的訂用帳戶識別碼，以及您在步驟 1 中建立的 **.pem** 檔案的路徑。 本主題會使用 "machine-name" 做為 Azure 雲端服務 (和您的 VM) 名稱，但是您應該使用自己的選項來取代，並且記得在任何其他需要 VM 名稱的步驟中使用您的雲端服務名稱。 (請記住，在此範例中，我們使用的是完整的二進位檔名稱，而非 **docker-machine** 符號連結)。
    ...
    > [AZURE.NOTE] 因為正在建立 VM，所以可能需要幾分鐘的時間才能處於就緒狀態。 等待動作，您可以檢查新 Docker 主機的狀態，只要 `vm list` 使用 Azure CLI，直到您看見 Vm 呈現 **ReadyRole** 狀態。
...
> [AZURE.NOTE] 本教學課程示範的 **docker-machine** 會建立一個 VM。不過，您可以重複執行這些步驟，來建立所需機器的數目。如果您執行此動作，則在 VM 和 docker 之間進行切換的最佳方式是使用內嵌的 **env** 命令，為每個個別命令設定 **docker** 環境變數。例如，若要使用 **docker info** 有不同的 VM，您可以輸入 `docker $(docker machine env < VM 名稱 >) 資訊` 和 **env** 命令會填入 docker 連線資訊，以便與該 VM 搭配使用。
