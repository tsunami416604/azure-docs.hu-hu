...
**作者:** Daniel Sol
**技術校閱者:** Luis Carlos Vargas Herring、 Sanjay Mishra、 Pravin Mital、 Juergen Thomas、 Gonzalo Ruiz。
...
您可以提高 Microsoft 支援票證以移轉到區域 VNET、Microsoft 將進行變更，然後完成區域 VNET 的移轉，在網路設定中變更屬性 AffinityGroup。 先匯出 PowerShell 中的網路設定，然後使用 **Location** 屬性來取代 **VirtualNetworkSite** 元素中的 **AffinityGroup** 屬性。 指定 `位置 = XXXX` 其中 `XXXX` 為 Azure 區域。 接著匯入新設定。
...
1. 使用 **Get-AzureVM** 命令，取得連結到 VM 的磁碟清單：
    ...
    Get-azurevm ServiceName <servicename> -名稱 <vmname> |Get AzureDataDisk

1. 記下 Diskname 和 LUN。

    ![DisknameAndLUN][2]

1. 從遠端桌面連接到 VM。 然後前往 [電腦管理]**** | [裝置管理員]**** | [磁碟機]****。 看看每個 「 Microsoft 虛擬磁碟 」 的內容
...
1. AlwaysOn 根據多站台的範例中，新增 IP 位址資源的新雲端服務 ILB/ELB 透過 PowerShell [附錄](#appendix-migrating-a-multisite-alwayson-cluster-to-premium-storage)。 在 Windows 叢集中，將 [IP 位址]**** 資源的 [可能的擁有者]**** 設為之前的新節點。 請參閱 > 一節新增 IP 位址資源同一個子網路上 ' [附錄](#appendix-migrating-a-multisite-alwayson-cluster-to-premium-storage)。
1. 容錯移轉到其中一個新節點。
...
- [Azure 儲存體帳戶](../storage-premium-storage-preview-portal.md)
