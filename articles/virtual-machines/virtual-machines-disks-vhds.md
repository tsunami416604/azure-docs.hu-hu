...
>[AZURE.WARNING] 請勿在暫存磁碟儲存資料。 它提供應用程式和處理程序暫時的儲存空間，且其用意僅為儲存分頁檔等資料。 若要重新對應 Windows 虛擬機器的磁碟，請參閱 [變更 Windows 暫存磁碟的磁碟機代號](virtual-machines-windows-change-drive-letter.md)。
...
>[AZURE.WARNING] 如果您從儲存體，刪除來源.vhd 檔案，或刪除儲存體帳戶，Microsoft 無法讓您復原資料。
