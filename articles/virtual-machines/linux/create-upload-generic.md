...
2. Hozza létre újra a initrd a hv_vmbus és a hv_storvsc kernel moduljaival:
...
* A VHD http:\//\<mystorageaccount >. blob. Core. Windows. net/VHD/MyLinuxVM. vhd virtuális mérete nem támogatott 21475270656 bájtnál. A méretnek egész számnak kell lennie (MB-ban).
...
* [ata_piix: lemezek elhalasztása a Hyper-V-illesztőprogramok számára alapértelmezés szerint](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/commit/drivers/ata/ata_piix.c?id=cd006086fa5d91414d8ff9ff2b78fbb593878e3c)
* [storvsc Az átviteli útvonalon lévő tranzitraktár csomagok fiókja](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/commit/drivers/scsi/storvsc_drv.c?id=5c1b10ab7f93d24f29b5630286e323d1c5802d5c)
...
* [storvsc AZONOS írási műveletek letiltása a RAID és a virtuális gazdagép adapter-illesztőprogramjaihoz](https://git.kernel.org/cgit/linux/kernel/git/next/linux-next.git/commit/drivers/scsi/storvsc_drv.c?id=54b2b50c20a61b51199bedb6e5d2f8ec2568fb43)
...
1. Telepítse az Azure Linux-ügynököt.
    ...
    A Linux-rendszerkép Azure-beli üzembe helyezéséhez az Azure Linux-ügynök szükséges.  Számos disztribúció az ügynököt RPM-vagy deb-csomagként adja meg (a csomag neve általában WALinuxAgent vagy WALinuxAgent).  Az ügynököt manuálisan is telepítheti a [Linux-ügynök útmutatójának](../extensions/agent-linux.md)lépéseit követve.
...
1. Futtassa a következő parancsokat a virtuális gép megszüntetéséhez.
     ...
     ```  
   > [!NOTE]
   > A VirtualBox-on a következő hibaüzenet jelenhet meg a `waagent -force -deprovision` Futtatás `[Errno 5] Input/output error`után:. Ez a hibaüzenet nem kritikus, és figyelmen kívül hagyható.
