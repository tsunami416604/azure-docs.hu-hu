...
1. Hozzon létre egy csoportot, `virtualip` amely `azure_load_balancer` tartalmazza a és az erőforrást:
...
1. Az elsődleges csomóponton futtassa az alábbi parancsot a SQLCMD vagy a SSMS:
    ...
    - Cserélje le az alább használt IP-címet `virtualip` az IP-címére.
...
1. Jelentkezzen be az egyes VM-csomópontokra. A következő parancs használatával nyissa meg a Hosts fájlt, és állítsa be az `ag1-listener` állomásnév feloldását minden egyes gépen.

    ```
    sudo vi /etc/hosts
    ```

    A **VI** szerkesztőben írja `i` be a szöveget, és egy üres sorban adja hozzá a (z `ag1-listener`) IP-címét. Ezután adja `ag1-listener` hozzá az IP-cím melletti szóközt.

    ```output
    <IP of ag1-listener> ag1-listener
    ```

    A **VI** -szerkesztőből való kilépéshez először nyomja meg az **ESC** billentyűt, `:wq` majd írja be a parancsot a fájl írásához és a kilépéshez. Ezt minden csomóponton végezze el.
...
1. A SQLCMD használatával jelentkezzen be SQL Server elsődleges csomópontjára a rendelkezésre állási csoport figyelője nevével:
    ...
    - Használjon egy korábban létrehozott bejelentkezési azonosítót, és cserélje `<YourPassword>` le a megfelelő jelszóval. Az alábbi példa a `sa` SQL Server használatával létrehozott bejelentkezést használja.
...
1. Keresse meg annak a kiszolgálónak a nevét, amelyhez csatlakozik. Futtassa a következő parancsot a SQLCMD-ben:
    ...
    A kimenetnek az aktuális elsődleges csomópontot kell megjelenítenie. Ezt akkor kell `VM1` megadnia, ha még soha nem tesztelt feladatátvételt.

    A `exit` parancs beírásával lépjen ki az SQL-munkamenetből.

### <a name="test-a-failover"></a>Feladatátvétel tesztelése

1. Futtassa a következő parancsot az elsődleges replika `<VM2>` vagy egy másik replika manuális feladatátvételéhez. Cserélje `<VM2>` le a nevet a kiszolgálónév értékére.
...
1. Ha megtekinti a korlátozásokat, látni fogja, hogy egy másik korlátozás lett hozzáadva a manuális feladatátvétel miatt:
    ...
    Látni fogja, hogy az AZONOSÍTÓval `cli-prefer-ag_cluster-master` rendelkező megkötés hozzá lett adva.

1. Távolítsa el a megkötést AZONOSÍTÓval `cli-prefer-ag_cluster-master` a következő parancs használatával:

    ```bash
    sudo pcs constraint remove cli-prefer-ag_cluster-master
    ```

1. Ellenőrizze a fürt erőforrásait a parancs `sudo pcs resource`használatával, és láthatja, hogy az elsődleges példány most `<VM2>`már.
