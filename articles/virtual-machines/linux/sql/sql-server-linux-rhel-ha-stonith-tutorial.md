...
Az alábbi parancs használatával hozzon létre egy erőforráscsoportot `<resourceGroupName>` egy régióban. Cserélje `<resourceGroupName>` le a elemet a választott névre. Ezt az oktatóanyagot használjuk `East US 2` . További információkért tekintse meg [a következő rövid](../quick-create-cli.md)útmutatót.
...
A következő lépés egy rendelkezésre állási csoport létrehozása. Futtassa a következő parancsot Azure Cloud Shellban, és cserélje `<resourceGroupName>` le az erőforráscsoport nevére. Válasszon egy nevet a `<availabilitySetName>`következőhöz:.
...
1. Szerezze be azoknak a virtuálisgép-lemezképeknek a listáját, amelyek a RHEL-mel rendelkeznek:
    ...
    Ebben az oktatóanyagban a képet `RedHat:RHEL-HA:7.6:7.6.2019062019`választjuk.
...
> [!IMPORTANT]
> A fenti paranccsal létrehozott alapértelmezett lemezkép alapértelmezés szerint egy 32GB operációsrendszer-lemezt hoz létre. Előfordulhat, hogy az alapértelmezett telepítéssel kifogyott a terület. A fenti `az vm create` parancshoz hozzáadva a következő paramétert használhatja egy operációsrendszer-lemez létrehozásához a 128GB példaként: `--os-disk-size-gb 128`.
>
...
Az `exit` SSH-munkamenet elhagyásához írja be a következőt:.
...
>
> Ha több `sudo` parancs másolását és beillesztését kéri, és a rendszer jelszót kér, a további parancsok nem fognak futni. Az egyes parancsokat külön futtassa.
...
1. A következő parancs használatával nyissa meg a Hosts fájlt, és állítsa be az állomásnév feloldását. További információ: [configure AG](/sql/linux/sql-server-linux-availability-group-configure-ha#prerequisites) a Hosts fájl konfigurálásához.
    ...
    A **VI** szerkesztőben írja `i` be a szöveget, és egy üres sorban adja hozzá a megfelelő virtuális gép **magánhálózati IP-címét** . Ezután adja hozzá a virtuális gép nevét az IP-cím melletti terület után. Minden sornak külön bejegyzéssel kell rendelkeznie.
    ...
    A **VI** -szerkesztőből való kilépéshez először nyomja meg az **ESC** billentyűt, `:wq` majd írja be a parancsot a fájl írásához és a kilépéshez.
...
### <a name="register-a-new-application-in-azure-active-directory"></a>Új alkalmazás regisztrálása Azure Active Directory
 ...
 6. Válassza az alkalmazás típusa **web**lehetőséget, írja be a bejelentkezési URL-címet http://localhost) (például kattintson a Hozzáadás gombra. A bejelentkezési URL-cím nincs használatban, és bármely érvényes URL-cím lehet. Ha elkészült, kattintson a **regisztráció** gombra.
...
- Cserélje `<username>` le a nevet tetszőleges névre. Ennek a szerepkör-definíciónak a létrehozásakor el kell kerülnie az ismétlődést.
- Cserélje `<subscriptionId>` le az-t az Azure-ELŐfizetési azonosítóra.
...
- Cserélje `<filename>` le a nevet a fájl nevére.
...
6. Szerepkör `Linux Fence Agent Role-<username>` kiválasztása a **szerepkör** listából
...
- Cserélje le `<ApplicationID>` az értéket az alkalmazás regisztrációjának azonosító értékére.
- Cserélje le `<servicePrincipalPassword>` az értéket az ügyfél titkos kódjából.
- `<resourceGroupName>` Cserélje le az elemet az oktatóanyaghoz használt előfizetéshez tartozó erőforrás-csoportra.
- Cserélje le `<tenantID>` a és `<subscriptionId>` a elemet az Azure-előfizetésből.
...
Mivel már hozzáadtak egy szabályt a tűzfalhoz a HA szolgáltatás (`--add-service=high-availability`) engedélyezéséhez, nem kell megnyitnia a következő tűzfal-portokat az összes csomóponton: 2224, 3121, 21064, 5405. Ha azonban bármilyen típusú kapcsolódási problémát tapasztal a HA-vel, a következő parancs használatával nyissa meg a HA-hoz társított portokat.
...
1. Kapcsolódjon az **összes csomóponthoz** SQL Server Management Studio (SSMS) vagy SQL cmd használatával. Futtassa az alábbi parancsokat a AlwaysOn_health-munkamenet engedélyezéséhez és a főkulcs létrehozásához:
    ...
    - Cserélje le `<Master_Key_Password>` a-et a saját jelszavára.
...
1. Kapcsolódjon az elsődleges replikához a SSMS vagy az SQL CMD használatával. Az alábbi parancsok létrehoznak egy tanúsítványt a `/var/opt/mssql/data/dbm_certificate.cer` (z) és egy `var/opt/mssql/data/dbm_certificate.pvk` titkos kulcs mellett az elsődleges SQL Server-replikán:

    - Cserélje le `<Private_Key_Password>` a-et a saját jelszavára.
...
A `exit` parancs futtatásával lépjen ki az SQL cmd-munkamenetből, és térjen vissza az SSH-munkamenethez.
...
1. Futtassa a következő parancsot a célkiszolgálón:
    ...
    - Cserélje `<username>` le a nevet a felhasználónevére.
...
- Cserélje `ag1` le a nevet a kívánt rendelkezésre állási csoport nevére.
- Cserélje le `<VM1>`a `<VM2>`,, `<VM3>` és értékeket a replikákat futtató SQL Server példányok nevére.
...
- Cserélje `<password>` le a változót a saját összetett jelszavára.
...
Ha a `synchronization_state_desc` lista szinkronizálva van `db1`, ez azt jelenti, hogy a replikák szinkronizálva vannak. A formátumú másodlagos zónák az elsődleges `db1` replikában láthatók.
...
1. A hálózatról elérhető statikus IP-cím használatával hozzon létre egy virtuális IP-erőforrást. Az egyiket a parancs eszköz `nmap`használatával találja.
...
1. Hozza létre a virtuális IP-erőforrást a következő parancs használatával:
    ...
    - Cserélje le `<availableIP>` az alábbi értéket egy nem használt IP-címhez.
...
1. Futtassa a következő parancsot az elsődleges replika manuális feladatátvételéhez `<VM2>`. Cserélje `<VM2>` le a nevet a kiszolgálónév értékére.
...
> [!NOTE]
> Alapértelmezés szerint a kerítés művelettel a csomópont ki-és bekapcsolva állapotba kerül. Ha csak a csomópontot szeretné offline állapotba helyezni, használja `--off` a parancsot a parancsban.
