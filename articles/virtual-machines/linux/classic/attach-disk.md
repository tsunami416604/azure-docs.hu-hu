...
2. A következő meg kell találnia az adatok lemez inicializálása eszközazonosító. Ehhez két módja van:
    ...
    b) használja a `lsscsi` parancs az eszközazonosítót megállapítása. Az `lsscsi` a `yum install lsscsi` (Red Hat-alapú disztribúciókon) vagy az `apt-get install lsscsi` (Debian-alapú disztribúciókon) használatával telepíthető. A lemez által keresett megtalálhatja a *lun* vagy **logikaiegység-szám**. Például a *lun* a csatolt a lemezek is könnyen látható `azure vm disk list <virtual-machine>` mint:
...
11. Vegye fel az új meghajtó /etc/fstab:
    ...
    Most tesztelheti, hogy a fájlrendszer által leválasztás és majd tudják azaz a fájlrendszerben, megfelelően csatlakoztatott a példa csatlakoztatási pontot `/datadrive` a korábbi lépésekben létrehozott:
