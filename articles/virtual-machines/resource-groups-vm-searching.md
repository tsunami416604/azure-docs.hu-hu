...
> [AZURE.NOTE] 本文說明如何使用最近安裝的 Azure CLI 或 Azure PowerShell，來瀏覽並選取虛擬機器映像。 有一個必要條件是您必須變更為資源管理員模式。 使用 Azure CLI，輸入該模式來輸入 `azure 組態模式 arm`。 

最簡單快速的方法來尋找要使用的影像 `azure vm 快速建立` ，或建立資源群組範本檔案已呼叫 `azure vm 映像清單` 命令，並傳遞位置、 發行者名稱 (不區分大小寫!) 和提供項目-如果您知道提供項目。 例如，如果您知道 "Canonical" 是 "UbuntuServer" 提供項目的發佈者，則下列清單只是簡短範例 (許多清單相當長)。
...
**Urn** 資料行將會是您傳遞給表單 `azure vm 快速建立`。

不過，您通常還不知道什麼可用。 在此情況下，您可以瀏覽映像使用第一次探索發行者 `azure vm 映像清單發佈者` 和回應位置提示字元中，使用您預期用於資源群組的資料中心位置。 例如，下列清單是美國西部位置的所有映像發佈者 (將位置設為小寫並移除標準位置中的空格，來傳遞 location 引數)。
...
這些清單可能相當長，因此上述範例清單只是程式碼片段。 假設我注意到 Canonical 事實上是美國西部位置的映像發佈者。 您現在可以找到其提供項目，藉由呼叫 `azure vm 映像清單提供` 並傳遞位置以及 「 發行者 」 在提示字元，如下列範例所示:
...
現在，我們知道在美國西部區域中，Canonical 在 Azure 上發佈 **UbuntuServer** 提供項目。 但是，是什麼 SKU？ 若要取得那些，請呼叫 `azure vm 映像清單 sku` 和回應的位置、 發行者和提供項目是否有探索到的提示。
...
使用 PowerShell，輸入 `Switch-azuremode AzureResourceManager`。 請參閱 [使用 Azure CLI 與資源管理員](xplat-cli-azure-resource-manager.md) 和 [使用 Azure PowerShell 與 Azure 資源管理員](../powershell-azure-resource-manager.md) 的更完整更新與設定的詳細資料。
> [AZURE.NOTE] 與上述 1.0 時，Azure PowerShell 模組 `Switch-azuremode` cmdlet 已移除。 該版本和更新，請取代使用以下命令 `Azure` 部分取代 `AzureRm`。 如果您使用 Azure PowerShell 模組下方 1.0，您將使用下列命令，但您必須先 `Switch-azuremode AzureResourceManager`。 
