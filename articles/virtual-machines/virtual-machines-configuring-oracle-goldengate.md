...
本教學課程示範如何設定適用於 Azure 虛擬機器環境的 Oracle GoldenGate，以取得高可用性並進行嚴重損壞修復 。 本教學課程著重於 [雙向複寫](http://docs.oracle.com/goldengate/1212/gg-winux/GWUAD/wu_about_gg.htm) 非 RAC Oracle 資料庫，並要求這兩個網站是使用中。

Oracle GoldenGate 支援資料分佈和資料整合。 它讓您能夠透過 Oracle 設定資料分佈和資料同步處理解決方案 (Oracle 複寫設定)，並提供彈性的高可用性解決方案。 Oracle GoldenGate 利用它的複寫功能來補充 Oracle Data Guard，讓資訊可分佈於整個企業，並進行零停機時間的升級和移轉。 如需詳細資訊，請參閱 [使用 Oracle GoldenGate 與 Oracle Data Guard](http://docs.oracle.com/cd/E11882_01/server.112/e17157/unplanned.htm)。
...
本教學課程假設您已經有 Oracle 資料庫高可用性和嚴重損壞修復概念的理論和實務知識以及 [Oracle GoldenGate](http://docs.oracle.com/goldengate/1212/gg-winux/index.html)。 如需詳細資訊，請參閱 [Oracle 網站](http://www.oracle.com/technetwork/database/features/availability/index.html)。
...
- 您已經下載 Oracle GoldenGate 軟體從 [Oracle 下載](http://www.oracle.com/us/downloads/index.html) 網站。 您已選取 Oracle Fusion Middleware 產品套件 - 資料整合。 然後，您針對 Oracle 11g 資料庫，在適用於 Microsoft Windows x64 (64 位元) 的 Oracle v11.2.1 媒體套件上選取了 Oracle GoldenGate。 接著，在 Windows 2008 (64 位元) 上下載適用於 Oracle 11g (64 位元) 的 Oracle GoldenGate V11.2.1.0.3。

- 您已經使用 Windows Server 上提供 Oracle Enterprise Edition 映像的平台，在 Azure 中建立兩部虛擬機器 (VM)。 如需資訊，請參閱 [在 Azure 中建立 Oracle Database 12c 虛擬機器](#z3dc8d3c097cf414e9048f7a89c026f80) 和 [Azure 虛擬機器](http://azure.microsoft.com/documentation/services/virtual-machines/)。 請確定虛擬機器位於 [相同雲端服務](virtual-machines-load-balance.md) 和相同 [虛擬網路](http://azure.microsoft.com/documentation/services/virtual-network/) 以確保它們可以透過永續的私人 IP 位址互相存取。
...
- 您用來登入為系統管理員群組的成員或成員的 Windows server **ORA_DBA** 群組。
...
>[AZURE.IMPORTANT] 已安裝並針對下列軟體組態測試本教學課程:
>
>|                        | **站台 A 資料庫**              | **站台 B 資料庫**              |
>|------------------------|----------------------------------|----------------------------------|
...
針對 Oracle 資料庫和 Oracle GoldenGate 的後續版本，可能會有一些您需要實作的其他變更。 如需最新的版本特定的資訊，請參閱 [Oracle GoldenGate](http://docs.oracle.com/goldengate/1212/gg-winux/index.html) 和 [Oracle 資料庫](http://www.oracle.com/us/corporate/features/database-12c/index.html) Oracle 網站上的文件。 例如，針對版本 11.2.0.4 來源資料庫及更新版本，擷取 DDL 是透過記錄採礦伺服器以非同步方式來執行，並且不需要安裝任何特定的觸發程序、資料表或其他資料庫物件。 您可以執行 Oracle GoldenGate 升級，而不需停止使用者應用程式。 若擷取是處於與 Oracle 11g 來源資料庫 (版本早於 11.2.0.4) 整合的模式中，就需要使用 DDL 觸發程序和支援物件。 如需詳細指引，請參閱 [安裝和設定的 Oracle 資料庫的 Oracle GoldenGate](http://docs.oracle.com/goldengate/1212/gg-winux/GIORA.pdf)。
...
您可以使用 [GGSCI](http://docs.oracle.com/goldengate/1212/gg-winux/GWUAD/wu_gettingstarted.htm) 執行數個命令，設定、 控制和監視 Oracle GoldenGate。
...
開啟 SQL\ * Plus 站台 A 和站台 B 上使用的資料庫系統管理員權限的命令視窗 **SYSDBA**, ，例如:
...
接下來，找出 INIT\ < DatabaseSID\ >。這個檔案位於站台 A 和站台 B 上的 [%ORACLE\_HOME%\\database] 資料夾，並將下列資料庫參數附加至 INITTEST.ora:

    UNDO\_MANAGEMENT=AUTO
    UNDO\_RETENTION=86400

如需所有 Oracle GoldenGate GGSCI 命令的完整清單，請參閱 [參考 Oracle GoldenGate 適用於 Windows 的](http://docs.oracle.com/goldengate/1212/gg-winux/GWURF/ggsci_commands.htm)。

### 執行初始資料載入

您可以依照下列幾種方法，在資料庫中執行初始資料載入。 例如，您可以使用 [Oracle GoldenGate 直接載入](http://docs.oracle.com/goldengate/1212/gg-winux/GWUAD/wu_initsync.htm) 或規則匯出和匯入公用程式，從站台 A 的資料表資料匯出到站台 b。

為了示範 Oracle GoldenGate 複寫程序，本教學課程將示範如何使用下列命令，在站台 A 和站台 B 上建立資料表。

首先，開啟 SQL\ * Plus 命令視窗並執行下列命令，在站台 A 和站台 B 資料庫上建立一個庫存資料表:
...
然後，啟用 [最小 [補充記錄](http://docs.oracle.com/cd/E11882_01/server.112/e22490/logminer.htm) ，如下所示:
...
開啟 Windows 命令提示字元並巡覽至 Oracle GoldenGate 資料夾，例如 C:\\OracleGG。 啟動 SQL\ * Plus 資料庫系統管理員權限的命令提示字元，例如使用 **SYSDBA** 上站台 A 和站台 b。
...
您必須在站台 A 和站台 b 上建立擷取及資料幫浦 」 程序
遠端桌面登入站台 A 和站台 B 透過 Azure 傳統入口網站。 開啟 GGSCI 命令直譯器視窗。 在站台 A 上執行下列命令：
...
使用 EDIT PARAMS 命令來開啟參數檔案，然後附加下列資訊：
    GGSCI (MachineGG1) 18 > 編輯 params ext1
    擷取 ext1
     USERID ggate，PASSWORD ggate
     ...
     GETBEFORECOLS (
     在 UPDATE KEYINCLUDING (prod_category，qty_in_stock，last_dml)
     在 DELETE KEYINCLUDING (prod_category，qty_in_stock，last_dml))。
...
顯示擷取群組 ext1 的相關資訊:
    GGSCI (MachineGG1) 32 > 資訊擷取 ext1
    EXTRACT EXT1 上次啟動 2013年-11-25 08:03 Status 正在執行
    Checkpoint Lag 00:00:00 (更新 00:00:02 ago)
    記錄讀取檢查點 Oracle 重做記錄
...
若要確認 Oracle GoldenGate 組態，將資料列插入在站台 a 資料庫
遠端桌面功能連至站台 a。 開啟 SQL * Plus 命令視窗並執行:
    SQL > v$ database; 從選取的名稱
