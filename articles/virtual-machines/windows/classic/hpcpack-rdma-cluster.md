...
Az Azure-ban Windows RDMA fürt beállítása [Microsoft HPC Pack](https://technet.microsoft.com/library/cc514029) és [nagy teljesítményű számítási Virtuálisgép-méretek](../sizes-hpc.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json) párhuzamos Message Passing Interface (MPI) alkalmazások futtatásához. Az RDMA-kompatibilis, a Windows Server-alapú HPC Pack-fürtben lévő csomópontok beállításakor MPI alkalmazások hatékonyan kis késéssel, a távoli közvetlen memória-hozzáférés (RDMA) technológia alapuló Azure magas teljesítmény hálózati protokollt használó kommunikációra.
...
Ez a cikk bemutatja két forgatókönyvek és hivatkozások részletes útmutató a Microsoft HPC Pack Windows RDMA fürt beállítása. 
...
Általános Előfeltételek számítási igényű példányok használata a Windows, lásd: [nagy teljesítményű számítási Virtuálisgép-méretek](../sizes-hpc.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json).
...
3. **Új felhőalapú szolgáltatás és a storage-fiók létrehozása**
    ...
    Az Azure-portál használatával hozzon létre egy felhőalapú szolgáltatás és a storage-fiók a központi telepítés olyan régióban, ahol az RDMA-kompatibilis példányok elérhetők.
...
1. **Hozzon létre egy fürt átjárócsomópontjába, és a számítási csomópont virtuális gépek az ügyfélszámítógépen a HPC Pack IaaS telepítési parancsfájl futtatásával**
    ...
    Töltse le a HPC Pack IaaS telepítési parancsfájl-csomagot a [Microsoft Download Center](https://www.microsoft.com/download/details.aspx?id=49922).
    ...
    Az RDMA-kompatibilis számítási csomópontok központi telepítéséhez a következő további-érdemes figyelembe venni:
   
   * **Virtuális hálózati**: Adjon meg egy új virtuális hálózat egy régióban, amelyben az RDMA-kompatibilisek-e használni kívánt példányméretének érhető el.
   * **Windows Server operációs rendszer**: támogatják az RDMA-kapcsolatot, adja meg a számítási csomópont virtuális gépek Windows Server 2012 R2 vagy Windows Server 2012 operációs rendszer.
...
2. **A számítási csomópontok online állapotba kerüljön feladatok futtatása**
