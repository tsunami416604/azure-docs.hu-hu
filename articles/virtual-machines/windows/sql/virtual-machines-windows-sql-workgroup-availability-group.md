...
1. A **fájlkezelővel** nyissa meg `c:\windows\system32\drivers\etc`a következőt:. 
...
1. Az egyik csomóponton nyissa meg a **Feladatátvevőfürt-kezelőt**, válassza ki a fürtöt, kattintson a jobb gombbal a **név: \<ClusterNam>** a **fürt alapvető erőforrásai** területen, majd válassza a **Tulajdonságok**lehetőséget. 
...
Ebben a lépésben olyan tanúsítványokat hoz létre, amelyeket az SQL-bejelentkezés a titkosított végponton használ. Hozzon létre egy mappát az egyes csomópontokon a tanúsítványok biztonsági másolatának `c:\certs`tárolásához, például:. 

Az első csomópont konfigurálásához kövesse az alábbi lépéseket: 

1. Nyissa meg **SQL Server Management Studio** és kapcsolódjon az első csomóponthoz `AGNode1`, például:. 
...
Ebben a lépésben konfigurálja a rendelkezésre állási csoportot, és adja hozzá az adatbázisokat. Most ne hozzon létre figyelőt. Ha még nem ismeri a lépéseket, tekintse meg a [rendelkezésre állási csoport oktatóanyagát](virtual-machines-windows-portal-sql-availability-group-tutorial.md#create-the-availability-group). Győződjön meg arról, hogy a feladatátvételt és a feladat-visszavételt kezdeményezi annak ellenőrzéséhez, hogy minden megfelelően működik-e. 
   ...
   > [!NOTE]
   > Ha a szinkronizálási folyamat során hiba lép fel, lehetséges, hogy rendszergazdai jogosultságokat kell biztosítania `NT AUTHORITY\SYSTEM` a fürterőforrás létrehozásához az első csomóponton, `AGNode1` például ideiglenesen. 
