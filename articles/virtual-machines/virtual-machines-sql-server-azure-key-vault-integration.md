...
如果您使用內部部署上執行 SQL Server 的電腦上，那里的 [步驟可以從您內部部署 SQL Server 的電腦存取 Azure 金鑰保存庫](https://msdn.microsoft.com/library/dn198405.aspx)。 對於 Azure Vm 中的 SQL Server，您可以使用來節省時間，但 *Azure 金鑰保存庫整合* 功能。 使用一些 Azure PowerShell Cmdlet 來啟用這項功能，您可以自動化 SQL VM 存取您的金鑰保存庫所需的組態。
...
當您建立金鑰保存庫步驟，請注意傳回 **vaultUri** 屬性，這是金鑰保存庫的 URL。 因此在該步驟中，如下所示，金鑰保存庫名稱是 ContosoKeyVault，所提供的範例中的金鑰保存庫的 URL 會是 https://contosokeyvault.vault.azure.net/。

![Azure Active Directory 密碼](./media/virtual-machines-sql-server-azure-key-vault-integration/new-azurekeyvault.png)
 
金鑰保存庫的 URL 會指派至更新版本 **$akvURL** 啟用 Azure 金鑰保存庫整合的 PowerShell 指令碼中的參數。
 ...
 **新增 AzureVMSqlServerKeyVaultCredentialConfig** 指令程式會建立 Azure 金鑰保存庫整合功能的組態物件。  **Set-azurevmsqlserverextension** 設定這項整合與 **KeyVaultCredentialSettings** 參數。 下列步驟說明如何使用這些命令。
