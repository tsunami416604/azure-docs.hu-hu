...
>[AZURE.NOTE] 單獨針對 /dev/sda 設定此不實用。 必須在資料庫所在的所有資料磁碟上進行設定。  
...
##增加高並行存取的系統控制代碼數目上限
MySQL 是高並行存取資料庫。 Linux 的並行控制代碼預設數目為 1024，這並不足夠。 **使用下列步驟來增加最大並行控制代碼，以支援 MySQL 的高並行存取系統的**。
...
MySQL 緩慢查詢記錄檔可協助您識別 MySQL 的較慢查詢。 啟用 MySQL 緩慢查詢記錄檔之後，您可以使用之類的 MySQL 工具 **mysqldumpslow** 來識別效能瓶頸。  
...
在此範例中，您可以看到緩慢查詢功能已開啟。 然後您可以使用 **mysqldumpslow** 工具，判斷效能瓶頸並最佳化效能，例如加入索引。
