...
1. 您必須連線到所有現有的次要複本。 您可以按一下 **連線...** 可以在每個複本，或旁邊按一下 **全部連接...** 在畫面底部。 在驗證之後，按一下 [ **下一步** 前進到下一個畫面。

1. 在 **指定複本** ] 頁面上，多個索引標籤所列的上方 ︰ **複本**, ，**端點**, ，**備份喜好設定**, ，和 **接聽程式**。 從 **複本** 索引標籤上，按一下 [ **加入 Azure 複本...** 若要啟動 [新增 Azure 複本精靈]。
