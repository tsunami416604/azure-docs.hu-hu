...
![][2]
-   選取並複製中的公開金鑰 **金鑰** 並將它儲存在名為 **publicKey.pem**。 不要按 **儲存公開金鑰**, ，因為已儲存的公開金鑰檔案格式與我們想要的公開金鑰不同。
-   按一下 [ **儲存私密金鑰** 並將它儲存在名為 **privateKey.ppk**。
...
如 **主機名稱**, ，指定您和網際網路用戶端將用來存取此虛擬機器的 URL 名稱。 定義 DNS 名稱的最後一個部分，例如 LAMPDemo，然後 Azure 會產生如 Lampdemo.cloudapp.net 的 URL。

如 **使用者名稱**, ，挑選您稍後使用的名稱來登入虛擬機器。

如 **SSH 驗證金鑰**, ，複製金鑰值從 **publicKey.pem** 檔案，其中包含 puttygen 所產生的公開金鑰。  
...
在 Azure 入口網站中，按一下 [ **瀏覽]-> [虛擬機器**, ，然後按一下您所建立的虛擬機器。

![][5]

若要將端點加入至虛擬機器中，按一下 [ **端點** 方塊。
...
1.  輸入的名稱中的端點 **端點**。
2.  中輸入 80 **公用連接埠**。 如果您變更了 Apache 的預設接聽連接埠，則必須更新私用連接埠與 Apache 接聽連接埠相同。
3.  中輸入 80 **公用連接埠**。 根據預設，HTTP 流量使用連接埠 80。
...
首先，從 Azure 入口網站取得您虛擬機器的 DNS 名稱。 按一下 [ **瀏覽]-> [的虛擬機器]-> [** 虛擬機器的名稱 **屬性]-> [**, ，然後查看 **網域名稱** 欄位 **屬性** 並排顯示。

取得 SSH 連線的連接埠號碼 **SSH** 欄位。   範例如下。  
...
-   **可能的根本原因**
    ...
    檢查您的公用連接埠和私用連接埠端點設定，確定私用連接埠與 Apache 接聽連接埠相同。 請參閱＜第 1 階段：建立映像＞，以取得為虛擬機器設定端點的指示。</br>
