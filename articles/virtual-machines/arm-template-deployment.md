...
- [Windows Management Framework 3.0](http://www.microsoft.com/en-us/download/details.aspx?id=34595) 或 [Windows Management Framework 4.0](http://www.microsoft.com/en-us/download/details.aspx?id=40855)
...
2. 將下列命令中的 {password} 取代成您想要使用的密碼，然後執行該命令以建立應用程式：
    ...
    >[AZURE.NOTE] 記下之後會建立應用程式，因為您將需要它的下一個步驟傳回的應用程式識別項。 您也可以在 Azure 入口網站「Active Directory」區段中應用程式的 [用戶端識別碼] 欄位內尋找應用程式識別碼。
...
2. 在 **範本** > **Visual C#**, ，請選取 **主控台應用程式**, ，輸入名稱和專案的位置，然後按一下 **確定**。

3. 以滑鼠右鍵按一下 [方案總管] 中的專案名稱，然後按一下 **新增** > **新項目**。

4.  在 [加入新項目] 視窗中，選取 **文字檔**, ，輸入 *VirtualMachineTemplate.json* 為名稱，然後按一下 **新增**。
...
6. [參數](../resource-group-authoring-templates.md#parameters) 並非必要項目，但可以讓範本管理更容易。 它們會說明值類型、預設值 (如有需要)，以及可能的參數允許值。 對於此教學課程，用來建立虛擬機器、儲存體帳戶和虛擬網路的參數會加入至範本。
...
7.  [變數](../resource-group-authoring-templates.md#variables) 可以在範本中用來指定會經常變更的值或需要的參數值組合建立的值。
...
8.  [資源](../resource-group-authoring-templates.md#resources) 例如虛擬機器、 虛擬網路和儲存體帳戶接下來會定義範本中。
...
1.  以滑鼠右鍵按一下 [方案總管] 中的專案名稱，然後按一下 **新增**, ，然後  **新項目**。

2.  在 [加入新項目] 視窗中，選取 **文字檔**, ，輸入 *Parameters.json* 為名稱，然後按一下 **新增**。
...
3.  開啟 parameters.json 檔案，然後加入下列 JSON 內容：
    ...
    >[AZURE.NOTE] 映像 vhd 名稱會定期變更在映像庫中，因此您必須取得目前的映像名稱，才能部署虛擬機器。 若要這樣做，請參閱 [有關虛擬機器的映像](https://azure.microsoft.com/documentation/articles/virtual-machines-images/), ，然後以您想要使用的 vhd 檔案的名稱取代 {來源-映像-名稱}。 例如，「 a699494373c04fc0bc8f2bb1389d6106__Windows-Server-2012-R2-201412.01-en.us-127GB.vhd 」。 將 {subscription-id} 取代成您的訂用帳戶的識別碼。
...
2.  在 [範本] 容器窗格右上角，按一下 [上傳 Blob] 圖示，瀏覽至您所建立的 VirtualMachineTemplate.json 檔案，然後按一下 [ **開啟**。

3. 再按一下 [上傳 Blob] 圖示，瀏覽至您所建立，然後按一下 [Parameters.json 檔案 **開啟**。
...
2.  型別 *Active Directory* 在搜尋方塊中，按一下 [ **安裝** Active Directory Authentication library 封裝，並依照指示來安裝封裝。

3.  在頁面的頂端，選取 [ **包含發行前版本**。 型別 *Azure 資源管理* 在搜尋方塊中，按一下 [ **安裝** Microsoft Azure 資源管理程式庫，然後依照指示來安裝封裝。
...
資源一律會從範本部署到資源群組。 您使用 [ResourceGroup](https://msdn.microsoft.com/library/azure/microsoft.azure.management.resources.models.resourcegroup.aspx) 和 [ResourceManagementClient](https://msdn.microsoft.com/library/azure/microsoft.azure.management.resources.resourcemanagementclient.aspx) 類別來建立部署資源的資源群組。
...
1.  若要執行主控台應用程式，請按一下 [ **啟動** Visual Studio 中，然後再登入 Azure AD 使用相同的使用者名稱和密碼，您使用與您的訂閱。

2.  按下 **Enter** 之後每個會傳回狀態碼來建立每個資源。 建立虛擬機器之後，請執行下一個步驟，再按 Enter 以刪除所有資源。
