...
![][2]
5.  選取並複製中的公開金鑰 **金鑰** 並將它儲存在名為 publicKey.pem 的檔案。 不要按 **儲存公開金鑰**, ，因為已儲存的公開金鑰檔案格式與我們想要的公開金鑰不同。
6.  按一下 [ **儲存私密金鑰** 並將它儲存在名為 privateKey.ppk 的檔案。

###步驟 2：在 Azure 入口網站中建立映像。
在 [Azure 入口網站](https://portal.azure.com/), ，按一下 [ **新增** 中建立映像，工作列上，選擇 Linux 映像依據您的需求。 下列範例使用 Ubuntu 14.04 映像。
![][3]

如 **主機名稱** 指定的 URL，您和網際網路用戶端將用來存取此虛擬機器的名稱。 定義 DNS 名稱的最後一個部分，例如 tomcatdemo，然後 Azure 會產生如 tomcatdemo.cloudapp.net 的 URL。  
...
![][6]
3.  按一下 [ **新增**。  
    1.  如 **端點**, ，在端點中，輸入端點的名稱，然後輸入 [80 **公用連接埠**。  
...
首先，從 Azure 入口網站取得您虛擬機器的 DNS 名稱。 **按一下 [瀏覽** ]-> [ **虛擬機器** ]-> [虛擬機器名稱]-> [ **屬性**, ，然後查看 **網域名稱** 欄位 **屬性** 並排顯示。  
...
使用 **sudo apt 快取 search tomcat7** 命令，以查看所有可用的元件。 下列命令是安裝一些實用元件的一部分命令。  
