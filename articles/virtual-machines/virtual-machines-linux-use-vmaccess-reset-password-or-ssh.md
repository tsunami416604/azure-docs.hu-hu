...
如果您無法連接至 Linux 虛擬機器，因為密碼遺失、 不正確的安全殼層 (SSH) 金鑰或 SSH 設定有問題，請重設密碼或 SSH 金鑰或修復 SSH 組態中使用 Azure 入口網站或 VMAccessForLinux 延伸模組。 請注意，本文適用於使用**傳統**部署模型建立的虛擬機器。
...
- Microsoft Azure Linux Agent 2.0.5 版或更新版本。 虛擬機器映像庫中的大多數 Linux 映像均包含 2.0.5 版。 若要找出所安裝的版本，請執行 **waagent -version**。 若要更新代理程式，請依照 [Azure Linux 代理程式使用者指南] 中的指示。
