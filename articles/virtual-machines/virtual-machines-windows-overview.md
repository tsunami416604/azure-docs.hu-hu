...
Az Azure-ban futó alkalmazás-infrastruktúrák kiépítésekor mindig számos [kialakítási szempontot](virtual-machines-windows-infrastructure-virtual-machine-guidelines.md) kell figyelembe venni. A kezdés előtt a virtuális gépek következő tulajdonságait fontos átgondolni:
...
A virtuális gépeknek van egy hozzárendelt [nevük](virtual-machines-windows-infrastructure-naming-guidelines.md), valamint egy, az operációs rendszer részeként konfigurálható számítógépnevük. A virtuális gépek neve legfeljebb 15 karakter hosszú lehet.

Ha az operációs rendszert tartalmazó lemezt az Azure használatával hozza létre, akkor a számítógépnév és a virtuális gép neve ugyanaz. Ha egy korábban már konfigurált operációs rendszert tartalmazó [saját rendszerképet tölt fel és használ](virtual-machines-windows-upload-image.md) egy virtuális gép létrehozásához, akkor a nevek különbözőek lehetnek. Javasoljuk, hogy a saját rendszerkép feltöltésekor állítsa be, hogy az operációs rendszerben megadott számítógépnév és a virtuális gép neve ugyanaz legyen.
...
A virtuális gépek [virtuális merevlemezeken (VHD-k)](virtual-machines-windows-about-disks-vhds.md) tárolják az operációs rendszereket és az adatokat. A VHD-ken találhatók az operációs rendszer telepítéséhez kiválasztható rendszerképek is. 
...
Lehetősége van egy [saját rendszerképek feltöltésére és használatára](virtual-machines-windows-upload-image.md) is, ebben az esetben a közzétevő neve, az ajánlat és a termékváltozat adatok nem használatosak.

### <a name="extensions"></a>Bővítmények
A virtuális gépek [bővítményei](virtual-machines-windows-extensions-features.md) további hozzáadott képességekkel ruházzák fel a virtuális gépeket az üzembe helyezést követő konfigurálás és automatizált feladatok útján.

A bővítményekkel a következő gyakori feladatok végezhetők el:

* **Egyéni szkriptek futtatása** – A [Custom Script bővítmény](virtual-machines-windows-extensions-customscript.md) segít a virtuális gép számítási feladatainak konfigurálásában azáltal, hogy a virtuális gép üzembe helyezésekor lefuttatja az Ön szkriptjét.
* **Konfigurációk üzembe helyezése és kezelése** – A [PowerShell Célállapot konfigurálása (DSC) bővítmény](virtual-machines-windows-extensions-dsc-overview.md) segítségével beállíthatja egy virtuális gépen a DSC-t a konfigurációk és környezetek kezeléséhez.
...
| [Adatlemezek](virtual-machines-windows-attach-disk-portal.md) |Nem |A virtuális gépek a tárolókapacitást bővítő adatlemezeket is tartalmazhatnak. |
...
| Módszer | Cikk |
| --- | --- |
| Azure Portal |[Windows rendszerű virtuális gép létrehozása a portál használatával](virtual-machines-windows-hero-tutorial.md) |
| Sablonok |[Windows rendszerű virtuális gép létrehozása egy Resource Manager-sablonnal](virtual-machines-windows-ps-template.md) |
| Azure PowerShell |[Windows rendszerű virtuális gép létrehozása a PowerShell használatával](virtual-machines-windows-ps-create.md) |
| Ügyfél-SDK-k |[Azure erőforrások üzembe helyezés a C# használatával](virtual-machines-windows-csharp.md) |
| REST API-k |[Virtuális gépek létrehozása vagy frissítése](https://msdn.microsoft.com/library/mt163591.aspx) |

Reménykedhet, hogy sosem következik be, de várhatóan időnként elromlik valami. Ha ilyen helyzet áll elő, tekintse át [A Resource Manager üzembe helyezési hibáinak elhárítása egy Windows rendszerű virtuális gép létrehozásakor az Azure-ban](virtual-machines-windows-troubleshoot-deployment-new-vm.md) témakört.
...
| Azure PowerShell |Információk a virtuális gépek PowerShell használatával való kezeléséről: [Microsoft Azure virtuális gépek kezelése a Resource Manager és a PowerShell használatával](virtual-machines-windows-ps-manage.md). |
...
| Ügyfél-SDK-k |Információk a virtuális gépek C# használatával való kezeléséről: [Microsoft Azure virtuális gépek kezelése az Azure Resource Manager és a C# használatával](virtual-machines-windows-csharp-manage.md). |

### <a name="log-on-to-the-vm"></a>Bejelentkezés a virtuális gépre
Használja az Azure Portal Csatlakozás gombját [egy távoli asztali (RDP) munkamenet elindításához](virtual-machines-windows-connect-logon.md). A távoli kapcsolatok használatának megkísérlésekor időnként hibák történhetnek. Ha ez a helyzet áll elő, tekintse át a súgó információit a [Távoli asztali kapcsolatok hibaelhárítása Windows rendszert futtató Azure virtuális gépeken](virtual-machines-windows-troubleshoot-rdp-connection.md) szakaszban.

### <a name="manage-availability"></a>Rendelkezésre állás kezelése
Fontos, hogy tisztában legyen azzal, hogyan [biztosíthatja alkalmazása magas rendelkezésre állását](virtual-machines-windows-manage-availability.md). Ebben a konfigurációban több virtuális gépet hozhat létre, hogy közülük legalább egy mindig, mindenképp fusson.

Ahhoz, hogy az üzembe helyezett példány megfeleljen a 99,95-ös virtuálisgép-szolgáltatói szerződésünknek, legalább két virtuális gépet kell üzembe helyeznie a számítási feladatok futtatásához egy [rendelkezésre állási csoporton](virtual-machines-windows-infrastructure-availability-sets-guidelines.md) belül. Ez a konfiguráció biztosítja, hogy a virtuális gépek több tartalék tartomány között oszoljanak meg, és az őket futtató gazdagépeknek különböző karbantartási időszakaik legyenek. Az Azure egészére vonatkozó rendelkezésre állási garancia magyarázata a teljes [Azure SLA](https://azure.microsoft.com/support/legal/sla/virtual-machines/v1_0/)-ban található.
...
* Ha Linux rendszerű virtuális gépeket tervez használni, nézze át [Az Azure és a Linux](virtual-machines-linux-azure-overview.md) témakört.
* Infrastruktúra-beállítási útmutatóért lásd [Az Azure-infrastruktúra bemutatása példákkal](virtual-machines-windows-infrastructure-example.md) témakört.
* Ajánlott követni az [Ajánlott eljárás a Windows rendszerű virtuális gépek futtatásához az Azure-on](virtual-machines-windows-guidance-compute-single-vm.md) témakörben leírtakat.
