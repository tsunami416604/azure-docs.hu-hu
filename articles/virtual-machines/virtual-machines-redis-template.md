...
Redis 叢集是建立在子網路後面，因此，不會有可存取 Redis 叢集的公開 IP 位址。  您可以在部署期間，選擇部署「跳躍箱」。  這個 「 跳躍箱 」 是 Ubuntu VM 部署在子網路，但它 *沒有* 公開公用 IP 位址與您可以 SSH 的公開 SSH 連接埠。  接著，從「跳躍箱」，您可以 SSH 到子網路中的所有 Redis VM。
...
複製完成後，尋找 **redis 高可用性** C:\Azure\Templates 目錄中的資料夾。
...
當您執行 **New-azureresourcegroupdeployment** 命令時，這將會從參數 JSON 檔案 (azuredeploy-parameters.json)，擷取參數值，然後開始執行範本。 定義以及使用不同環境 (測試、實際執行等) 所需的參數檔案，有利範本的重複使用以及簡化複雜的多重環境解決方案。
...
>[AZURE.NOTE] 請注意， `defaultValue` 可以指定，如 `allowedValues`。
...
```
 ...
 `vmStorageAccountContainerName` 和 `vmStorageAccountDomain` 變數是簡單名稱/值變數的範例。 `vnetID` 計算在執行階段使用函式的變數範例 `resourceId` 和 `parameters`。 `machineSettings` 即使這些概念的組建的巢狀 JSON 物件進一步 `osImageReference` 中 `machineSettings` 變數。 `vmScripts` 包含 JSON 陣列， `scriptsToDownload`, ，這在執行的階段使用計算 `concat` 和 `variables` 函式。  
