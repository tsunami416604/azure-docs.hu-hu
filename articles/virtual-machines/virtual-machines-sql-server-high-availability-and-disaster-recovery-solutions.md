...
| Technology| 範例架構|
|---|---|
| **AlwaysOn 可用性群組**| 為了相同區域內的高可用性，所有可用性複本都會在 Azure VM 中執行。您要設定網域控制站和 SQL Server 虛擬機器，因為 Windows Server 容錯移轉叢集 (WSFC) 需要 Active Directory 網域。<br/>![AlwaysOn 可用性群組](./media/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions/azure_only_ha_always_on.gif)<br/>如需詳細資訊，請參閱 [設定 AlwaysOn 可用性群組在 Azure (GUI)](virtual-machines-sql-server-alwayson-availability-groups-gui.md)。|
...
| Technology| 範例架構|
|---|---|
| **AlwaysOn 可用性群組**| 為了進行嚴重損壞修復，可用性複本會在 Azure VM 的多個資料中心執行。這種跨區域解決方案可防止網站完全中斷。<br/> ![AlwaysOn 可用性群組](./media/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions/azure_only_dr_alwayson.png)<br/>在區域內，所有複本都應該在相同的雲端服務和相同的 VNet。由於每個區域會有不同的 VNet，因此這些解決方案會需要 VNet 對 VNet 連線。如需詳細資訊，請參閱 [Azure 傳統入口網站中設定站台對站 VPN](../vpn-gateway/vpn-gateway-site-to-site-create.md)。|
...
| **備份及還原與 Azure Blob 儲存體服務**| 生產資料庫直接備份到不同的資料中心的嚴重損壞修復的 blob 儲存體。<br/>![備份和還原](./media/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions/azure_only_dr_backup_restore.gif)<br/>如需詳細資訊，請參閱 [備份和還原 SQL Server Azure 虛擬機器中的](virtual-machines-sql-server-backup-and-restore.md)。|
...
| Technology| 範例架構|
|---|---|
| **AlwaysOn 可用性群組**| 為了進行跨網站的嚴重損壞修復，部分可用性複本會在 Azure VM 中執行，而其他複本會在內部部署執行。生產網站可以是在內部或在 Azure 資料中心。<br/>![AlwaysOn 可用性群組](./media/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions/hybrid_dr_alwayson.gif)<br/>WSFC 叢集中所有可用性複本必須都位於相同 WSFC 叢集中，因為必須跨越兩個網路 (多重子網路 WSFC 叢集)。此設定需要 Azure 與內部網路之間的 VPN 連線。<br/><br/>成功的災害復原的資料庫，您也應該安裝複本網域控制站在災害復原站台。<br/><br/>很可能要加入複本精靈] 在 SSMS 中用來將 Azure 複本加入至現有的 AlwaysOn 可用性群組。如需詳細資訊，請參閱教學課程：將您的 AlwaysOn 可用性群組延伸至 Azure。|
...
| **備份及還原與 Azure Blob 儲存體服務**| 內部部署生產資料庫直接備份到 Azure blob 儲存體的嚴重損壞修復。<br/>![備份和還原](./media/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions/hybrid_dr_backup_restore.gif)<br/>如需詳細資訊，請參閱 [備份和還原 SQL Server Azure 虛擬機器中的](virtual-machines-sql-server-backup-and-restore.md)。|
...
- [使用 SQL Server Native Client 使用連接字串關鍵字](https://msdn.microsoft.com/library/ms130822.aspx)
- [用戶端連接到資料庫鏡像工作階段 (SQL Server)](https://technet.microsoft.com/library/ms175484.aspx)
- [連接到混合式 IT 中的可用性群組接聽程式](http://blogs.msdn.com/b/sqlalwayson/archive/2013/02/14/connecting-to-availability-group-listener-in-hybrid-it.aspx)
- [可用性群組接聽程式、 用戶端連線和應用程式容錯移轉 (SQL Server)](https://technet.microsoft.com/library/hh213417.aspx)
- [資料庫鏡像連接字串與可用性群組搭配使用](https://technet.microsoft.com/library/hh213417.aspx)
...
- [建立 Azure VM 中的 AlwaysOn 可用性群組的 WSFC 叢集](http://gallery.technet.microsoft.com/scriptcenter/Create-WSFC-Cluster-for-7c207d3a)
