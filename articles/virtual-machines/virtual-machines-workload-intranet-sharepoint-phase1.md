...
6. |第一部最終的 DNS 伺服器 |第四個可能的 IP 位址的虛擬子網路的位址空間的網路 (請參閱表格 S)。 與您的 IT 部門合作來決定這些位址。 | __________________
7. |第二部最終的 DNS 伺服器 |第五個可能的 IP 位址的虛擬子網路的位址空間的網路 (請參閱表格 S)。 與您的 IT 部門合作來決定這些位址。 | __________________
...
**表格 S：虛擬網路中的子網路**
> [AZURE.NOTE] 為求簡化，這個預先定義的架構會使用單一子網路。 如果您想要涵蓋一組流量篩選器來模擬子網路隔離，您可以使用 Azure [網路安全性群組](../virtual-network/virtual-networks-nsg.md)。
...
1. | ___________________________ |所有工作負載中的虛擬機器使用的標準儲存體帳戶。
