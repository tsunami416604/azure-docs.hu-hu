...
|**儲存體**|使用 [高階儲存體](../storage/storage-premium-storage-preview-portal.md)。<br/><br/>保留 [儲存體帳戶](../storage/storage-create-storage-account.md) 和相同的區域中的 SQL Server VM。<br/><br/>停用 Azure [地理區域備援儲存體](../storage/storage-redundancy.md) (異地複寫) 上的儲存體帳戶。|
|**磁碟**|使用最少 2 [P30 磁碟](../storage/storage-premium-storage-preview-portal.md#scalability-and-performance-targets-when-using-premium-storage) (記錄檔 1; 1 表示資料檔和 TempDB)。<br/><br/>請避免使用作業系統或暫存磁碟的資料庫儲存體或記錄。<br/><br/>啟用讀取快取在磁碟上裝載的資料檔和 TempDB。<br/><br/>請勿啟用裝載記錄檔的磁碟上快取。<br/><br/>若要取得更高的 IO 輸送量的多個 Azure 資料磁碟等量磁碟區。<br/><br/>記載的配置大小的格式。|
