...
>
    ...
    Get-azureremotedesktopfile ServiceName $svcName-名稱 $vmName LocalPath $localFile-啟動
...
>[AZURE.IMPORTANT] 萬一它是雲端服務的最後一個 VM，您可以使用這個參數來保留雲端服務的虛擬 IP (VIP)。<br><br> 如果使用 StayProvisioned 參數，您將還是需要支付費用 VM。
...
這項工作需要幾個步驟。 首先，請使用 *** * 新增-AzureDataDisk * *** cmdlet 將磁碟新增至 $vm 物件。 然後使用 **Update-AzureVM** Cmdlet 來更新 VM 的組態。
