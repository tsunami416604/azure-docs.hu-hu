...
您可以選擇性地取得 root shell 使用 **sudo-s**。
...
### Cloud-Init
**Ubuntu** 和 **CoreOS** 映像會利用定域機組 init Azure，可提供其他功能用來啟動虛擬機器。
...
1. 執行 **waagent-deprovision** 來還原佈建自訂。 或者 **waagent-deprovision + user** ，選擇性地刪除佈建及所有相關資料時所指定的使用者帳戶。
...
每個虛擬機器都有一個暫時性本機 *資源磁碟* 附加。 資源磁碟上的資料就會消失，因為它會使用應用程式和虛擬機器中執行的處理序和 **暫存** 資料的儲存體。 也用來儲存作業系統的分頁檔或交換檔。

在 Linux 上，資源磁碟是通常由 Azure Linux 代理程式管理，並自動掛接到 **//mnt/資源** (或 **/mnt** Ubuntu 映像)。
