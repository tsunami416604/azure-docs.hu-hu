...
-   **取得映像系列**:`Get-azurevmimage | 選取 ImageFamily` 顯示字串所取得的映像系列清單 **ImageFamily** 屬性。
-   **取得特定系列中的所有映像**: `Get-azurevmimage |Where-object {$_。ImageFamily-eq $family}`
-   **尋找 VM 映像**: `Get-azurevmimage | 其中 {(gm – InputObject $_-名稱 DataDiskConfigurations)-ne $null} |選取的屬性標籤 ImageName` 運作的方式是篩選 DataDiskConfiguration 屬性僅適用於 VM 映像。 此範例也會篩選輸出並僅列出標籤和映像名稱。
-   **儲存一般化映像**: `儲存 AzureVMImage – ServiceName"myServiceName"– 名稱"MyVMtoCapture"– OSState 「 一般化 」 – ImageName"MyVmImage"– ImageLabel"This is my 一般化映像"`
-   **儲存特製化映像**: `儲存 AzureVMImage – ServiceName"mySvc2"– 名稱"MyVMToCapture2"– ImageName"myFirstVMImageSP"– OSState 「 特製化 」 的詳細資訊`
...
-   **刪除映像**: `移除 AzureVMImage – ImageName"MyOldVmImage"`
