...
若想進一步了解 VM 調整集，請觀看下列影片：
 ...
 - [Mark Russinovich 示範 Azure 調整設定](https://channel9.msdn.com/Blogs/Regular-IT-Guy/Mark-Russinovich-Talks-Azure-Scale-Sets/)

 - [虛擬機器的小數位數設定具有 Guy Bowerman](https://channel9.msdn.com/Shows/Cloud+Cover/Episode-191-Virtual-Machine-Scale-Sets-with-Guy-Bowerman)
...
[VM 小數位數設定範本剖析](https://channel9.msdn.com/Blogs/Windows-Azure/VM-Scale-Set-Template-Dissection/player)
...
本節列出一些典型的 VM 調整集案例。 某些較高階的 Azure 服務 (例如 Batch、Service Fabric、Azure 容器服務) 也會使用這些案例。
 ...
 - **使用 Jumpbox 連接到 VM** - 如果您在相同的 VNET 中建立 VM 調整集和獨立 VM，獨立 VM 與 VMSS VM 將可使用 VNET/子網路為它們定義的內部 IP 位址彼此連接。 如果您建立公用 IP 位址，並將它指派給獨立 VM，您可以透過 RDP 或 SSH 連接到獨立 VM，然後從該部機器連接到 VM 調整集執行個體。 此時您可能會發現，簡易 VM 調整集在本質上比在預設組態中設定了公用 IP 位址的獨立 VM 來得安全。
    ...
    這個方法的範例，此範本會建立簡單的 Mesos 叢集獨立管理的 Vm 的 VM 調整集根據的叢集的主要 VM 所組成: [https://github.com/gbowerman/azure-myriad/blob/master/mesos-vmss-simple-cluster.json](https://github.com/gbowerman/azure-myriad/blob/master/mesos-vmss-simple-cluster.json)
...
- 在任何區域中，您可以建立的 VM 數目都會受到計算核心配額的限制。 您可能需要連絡客戶支援人員，以提高您的計算配額限制，即使您目前用於雲端服務或 IaaS v1 的核心已具有較高的上限亦然。 若要查詢您的配額，您可以執行下列 Azure CLI 命令: _azure vm 清單 usage_ 和下列 PowerShell 命令: _Get AzureRmVMUsage_ (如果使用的 PowerShell 版本 1.0 使用以下 _Get AzureVMUsage_)。
