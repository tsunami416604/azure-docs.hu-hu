...
3. 以滑鼠右鍵按一下 [方案總管] 中的專案名稱，然後按一下 **管理 NuGet 封裝**。

4. 型別 *Active Directory* 在搜尋方塊中，按一下 [ **安裝** Active Directory Authentication library 封裝，並依照指示來安裝封裝。

5. 在頁面的頂端，選取 [ **包含發行前版本**。 型別 *Azure 計算* 在搜尋方塊中，按一下 [ **安裝** 計算.NET 程式庫，然後依照指示來安裝封裝。

6. 型別 *Azure 網路* 在搜尋方塊中，按一下 [ **安裝** 網路.NET 程式庫，然後依照指示來安裝封裝。

7. 型別 *Azure 儲存體* 在搜尋方塊中，按一下 [ **安裝** 網路.NET 程式庫，然後依照指示來安裝封裝。

8. 型別 *Azure 資源管理* 在搜尋方塊中，按一下 [ **安裝** 資源管理程式庫。
...
資源一律會部署到資源群組。 您使用 [ResourceGroup](https://msdn.microsoft.com/library/azure/microsoft.azure.management.resources.models.resourcegroup.aspx) 和 [ResourceManagementClient](https://msdn.microsoft.com/library/azure/microsoft.azure.management.resources.resourcemanagementclient.aspx) 類別來建立部署資源的資源群組。
...
1.  將下列方法加入至 Program 類別以建立虛擬機器：
    ...
    >[AZURE.NOTE] 映像 vhd 名稱會定期變更在映像庫中，因此您必須取得目前的映像名稱，才能部署虛擬機器。 若要這樣做，請參閱 [使用 Windows PowerShell 管理映像 Windows](https://msdn.microsoft.com/library/azure/dn790330.aspx), ，然後以您想要使用的 vhd 檔案的名稱取代 {來源-映像-名稱}。 例如，「 a699494373c04fc0bc8f2bb1389d6106__Windows-Server-2012-R2-201411.01-en.us-127GB.vhd 」。
