...
5. 第二個 [虛擬機器組態]**** 對話方塊隨即出現。 請提供下列資訊：
    ...
    - 在 [儲存體帳戶]**** 中，按一下 [使用自動產生的儲存體帳戶]****，或選取現有的儲存體帳戶名稱。 每個區域只會自動建立一個儲存體帳戶。 您利用此設定建立的所有其他虛擬機器均位於此儲存體帳戶。 您的儲存體帳戶限制為 20 個。 如需詳細資訊，請移至 [在 Azure 中建立儲存體帳戶](virtual-machines-create-upload-vhd-windows-server.md#step-2-create-a-storage-account-in-azure)。
    - 在 [可用性設定組]**** 中，選取 [(無)]****，然後按一下向右箭頭。
6. 在第三個 [虛擬機器設定]**** 對話方塊中，按一下核取記號以建立虛擬機器。
...
- 使用命令提示字元安裝 SQL Server。 如需詳細資訊，請移至 [從命令提示字元安裝 SQL Server 2012](http://msdn.microsoft.com/library/ms144259.aspx#SysPrep)。
...
- 使用 Windows PowerShell 進行佈建：您可以使用 Psconfig 命令列工具做為替代介面，用以執行幾項控制 SharePoint 2010 產品之佈建方式的作業。 如需詳細資訊，請移至 [Psconfig 命令列參照](http://technet.microsoft.com/library/cc263093.aspx)。
...
[測試 Azure 中的 SharePoint Server 2016 IT Preview](http://azure.microsoft.com/blog/test-sharepoint-server-2016-it-preview-4/)
