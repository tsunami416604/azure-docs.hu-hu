...
跨單位虛擬網路會透過站台對站台 VPN 或 ExpressRoute 連接到內部部署網路。 如果您想要建立開發/測試環境，以模擬最後組態和實驗存取應用程式並執行遠端系統管理，透過 VPN 連線，請參閱 [設定 web 型 LOB 應用程式中測試的混合式雲端](../virtual-network/virtual-networks-setup-lobapp-hybrid-cloud-testing.md)。
...
1. |第一個網域控制站 |Windows Server 2012 R2 Datacenter |D1
2. |第二個網域控制站 |Windows Server 2012 R2 Datacenter |D1
...
5. |叢集多數節點 |Windows Server 2012 R2 Datacenter |D1
6. |第一部 web 伺服器 |Windows Server 2012 R2 Datacenter |D3
7. |第二部 web 伺服器 |Windows Server 2012 R2 Datacenter |D3
...
[在 Azure 中的企業營運應用程式部署高可用性](virtual-machines-workload-high-availability-LOB-application-overview.md)
