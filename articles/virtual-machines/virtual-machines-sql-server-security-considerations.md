...
- 建立唯一的本機系統管理員帳戶名稱不是 **管理員**。

- 為您的所有帳戶使用複雜的強式密碼。 如需如何建立強式密碼的詳細資訊，請參閱 [建立強式密碼](http://go.microsoft.com/fwlink/?LinkId=293596) 安全和資訊安全中心 」 中的文件。

- 根據預設，Azure 會在 SQL Server 虛擬機器安裝期間選取 Windows 驗證。 因此， **SA** 登入已停用，且安裝程式會指派密碼。 我們建議 **SA** 登入應該不會使用或已啟用。 以下是需要 SQL 登入時的替代策略：
    - 建立具有 SQL 帳戶 **控制伺服器** 權限。
    - 如果您必須使用 **SA** 登入，啟用登入並將它重新命名並指定新密碼。
...
- 先前提到的兩個選項需要變更驗證模式，來 **SQL Server 和 Windows 驗證模式**。 如需詳細資訊，請參閱 [變更伺服器驗證模式](https://msdn.microsoft.com/library/ms188670.aspx)。
