...
12. 若要查看在網際網路上執行的購物車應用程式，開啟瀏覽器 url 的形式 `http://<<unique_domain_name>>/shoppingcart`。(您可以判斷的值 <<*unique_domain_name*>> 內 [Azure 入口網站](https://ms.portal.azure.com/) 按一下 **虛擬機器** ，然後選取您用來執行 Orable WebLogic Server 虛擬機器)。
