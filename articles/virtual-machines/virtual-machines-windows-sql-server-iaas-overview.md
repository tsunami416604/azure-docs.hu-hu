<properties
    pageTitle="Az SQL Server használatának első lépései az Azure Virtual Machines szolgáltatásban | Microsoft Azure"
    description="Az Azure Virtual Machines révén a felhőbe helyezheti át a helyszíni SQL Server-adatbázis számítási feladatait. Az előre konfigurált SQL VM-rendszerképekkel gyorsan megteheti az első lépéseket."
...
# Az SQL Server használatának első lépései az Azure Virtual Machines szolgáltatásban

Ez a témakör az SQL Server Azure virtuális gépeken történő futtatásának lehetőségeit írja le, valamint útmutatást és forrásanyagokat biztosít az első lépésekhez.
...
> [AZURE.NOTE] Amikor az SQL Server Azure virtuális gépeken való használatát mérlegeli, vegye számításba az Azure-on elérhető egyéb tárolási és SQL-lehetőségeket, mint például az [SQL Database](../sql-database/sql-database-technical-overview.md), az [SQL Data Warehouse](../sql-data-warehouse/sql-data-warehouse-overview-what-is.md) és az [SQL Server Stretch Database](../sql     -server-stretch-database/sql-server-stretch-database-overview.md). Részletes összehasonlításért lásd: [Choose a cloud SQL Server option: Azure SQL (PaaS) Database or SQL Server on Azure VMs (IaaS)](../sql-database/data-management-azure-sql-database-and-sql-server-iaas.md) (Felhőalapú SQL Server-verzió választása: Azure SQL- (platformszolgáltatási) adatbázis vagy SQL Server Azure virtuális gépeken (infrastruktúra-szolgáltatás)).
...
Részletes útmutatásért lásd a [Provision a SQL Server virtual machine in the Azure Portal](virtual-machines-windows-portal-sql-server-provision.md) (SQL Server rendszerű virtuális gép létrehozása az Azure portálon) című oktatóanyagot. Emellett tekintse át az [Performance best practices for SQL Server VMs](virtual-machines-windows-sql-performance.md) (Az SQL Server virtuális gépek teljesítményével kapcsolatos ajánlott eljárások) című cikket, amely leírja, hogyan válassza ki a kiépítés során a megfelelő gépméretet és egyéb lehetőségeket.

|SQL Server-verzió|Operációs rendszer|SQL Server-kiadás|
|---|---|---|
...
További magas rendelkezésre állási lehetőségekért lásd: [High Availability and Disaster Recovery for SQL Server in Azure Virtual Machines](virtual-machines-windows-sql-high-availability-dr.md) (Magas rendelkezésre állás és vészhelyreállítás az Azure Virtual Machines szolgáltatásban futó SQL Serveren).

### Adatok biztonsági mentése
Közvetlenül az Azure-ban tárolhatja a biztonsági mentéseket a Blob Storage használatával. További információ: [Use Azure Storage for SQL Server Backup and Restore](../sql-database/storage-use-storage-sql-server-backup-restore.md) (Az Azure Storage használata az SQL Server biztonsági mentéséhez és helyreállításához). Ez kiválóan működik az SQL virtuális gépek és a helyszíni SQL Server-adatbázisok esetében is. A biztonsági mentési és helyreállítási lehetőségek áttekintéséért lásd: [Backup and Restore for SQL Server in Azure Virtual Machines](virtual-machines-windows-sql-backup-recovery.md) (Az SQL Server biztonsági mentése és helyreállítása az Azure Virtual Machines szolgáltatásban).

Az Azure virtuális gépek az [Automatikus biztonsági mentés](virtual-machines-windows-sql-automated-backup.md) és az [Automatikus javítás](virtual-machines-windows-sql-automated-patching.md) funkciókat is használni tudják az SQL Serverrel.
