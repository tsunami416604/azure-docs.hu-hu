...
- HANA nagyméretű példányai: 2xS192 (négy szoftvercsatorna, 2 TB)
- HANA verziója: HANA 2,0 SP1
...
1,2 */etc/iSCSI/iscsid.conf*módosítása: Állítsa be a *Node. Session. Timeo. replacement_timeout = 5* és *Node. Startup = automatikus*beállítást. Módosítsa a fájlt **mindkét** csomóponton.
...
Kövesse a YaST2 > magas rendelkezésre állás ![> cluster YaST-Control-Center](media/HowToHLI/HASetupWithStonith/yast-control-center.png)
...
Várt érték = a központilag telepített csomópontok száma (ebben az ![esetben 2)](media/HowToHLI/HASetupWithStonith/yast-Cluster-Security.png) YaST-cluster-Security. png kattintson a](media/HowToHLI/HASetupWithStonith/yast-cluster-configure-csync2.png) **tovább**
![gombra YaST-cluster-configure-csync2. png adja hozzá a csomópontok nevét, majd kattintson a "Hozzáadás javasolt fájlok "
...
A hitelesítés az IP-címek és a Csync2-ben előmegosztott kulcsok használatával történik. A kulcsfájl a csync2-k/etc/csync2/key_hagroup. jön létre. A key_hagroup a fürt összes tagjára manuálisan kell másolni a létrehozása után. **Győződjön meg arról, hogy a fájlt az 1. csomópontról Csomópont2-re másolja**.
...
Ha a pacemaker szolgáltatás *nem sikerül*, tekintse meg az *5. forgatókönyv: A pacemaker szolgáltatás nem sikerül*
...
ha-cluster-join
```
Ha a fürthöz való csatlakozás során *hibaüzenet* jelenik meg, *tekintse át a 6. forgatókönyvet: A 2. csomópont nem tud csatlakozni*a fürthöz.
...
crm_mon
```
![CRM-Mon. png](media/HowToHLI/HASetupWithStonith/crm-mon.png) – a Hawk-be való bejelentkezéshez a fürt állapota *\<https://csomópont IP-címe >: 7630*. Az alapértelmezett felhasználó a hacluster, a jelszó pedig Linux. Ha szükséges, módosíthatja a jelszót a *passwd* paranccsal.
...
A *crm_mon*parancs futtatásakor Itt láthatja a két erőforrást.
...
### <a name="scenario-1-cluster-node-not-online"></a>forgatókönyv 1: A fürtcsomópont nem online állapotú
...
A függőségek alatt válassza a "javasolt csomagok ![telepítése" YaST-dependencies. png elemet.](media/HowToHLI/HASetupWithStonith/yast-dependencies.png)

Tekintse át a módosításokat, és nyomja meg az OK gombot

![YaST](media/HowToHLI/HASetupWithStonith/yast-automatic-changes.png)

A csomag telepítése folytatja ![a YaST-Performing-Installation. png](media/HowToHLI/HASetupWithStonith/yast-performing-installation.png)
...
```
![a Zypper-install-ligyui.](media/HowToHLI/HASetupWithStonith/zypper-install-ligyui.png)
png![Zypper-install-ligyui_part2.](media/HowToHLI/HASetupWithStonith/zypper-install-ligyui_part2.png) png Yast2 most már az itt látható módon kell megnyitnia a grafikus nézetet.
