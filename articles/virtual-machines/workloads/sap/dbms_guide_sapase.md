...
Linux rendszerű virtuális gépeken a `saptune`t a profilt használó SAP-bevezetési Linux hatalmas lapokon alapértelmezés szerint engedélyezni kell, és a parancs használatával ellenőrizhető  
...
Az adateszközökhöz és a naplózási eszközökhöz külön lemezek használata ajánlott.  A rendszeradatbázisok sybsecurity és `saptools` nem igényelnek dedikált lemezeket, és az SAP-adatbázis adat-és naplózási eszközeit tartalmazó lemezeken helyezhetők el. 
...
| SAP-bemutató verziója | 16,3 PL. 7 vagy újabb | 16,3 PL. 7 vagy újabb | --- |
...
- SAP bevezetési 16,3 PL7 vagy újabb üzembe helyezése
...
- `saptune` SAP-a Linux operációs rendszer futtatása 
...
Csakúgy, mint a helyszíni rendszerek esetében, több lépésre van szükség ahhoz, hogy a DBACockpit WebDynpro-implementációja által használt összes SAP NetWeaver funkció engedélyezve legyen. A webdynpros használatának engedélyezéséhez és a szükségesek létrehozásához kövesse az [SAP támogatási megjegyzéseit #1245200](https://launchpad.support.sap.com/#/notes/1245200) . A fenti megjegyzésekben szereplő utasítások követése esetén az Internet Communication Manager (`ICM`) és a http-és HTTPS-kapcsolatokhoz használt portok mellett is konfigurálnia kell az internetes kommunikációt. A http alapértelmezett beállítása a következőképpen néz ki:
...
Ha a virtuális gépet csak felhőalapú környezetben telepítette a helyszíni és az Azure közötti kapcsolat nélkül, meg kell határoznia egy nyilvános IP-címet és egy `domainlabel`. A virtuális gép nyilvános DNS-nevének formátuma így néz ki:
...
A [Sybase-16,3 PL7 dokumentációjának](https://help.sap.com/viewer/product/SAP_ASE/16.0.3.7/en-US) kezdőlapja különböző dokumentumokra mutató hivatkozásokat tartalmaz, amelyek a következő dokumentumokat ismertetik:
...
- [Sybase-Infoközpont](http://infocenter.sybase.com/help/index.jsp) 

A havi hírlevél [SAP-támogatási megjegyzéssel](https://launchpad.support.sap.com/#/notes/2381575) jelenik meg #2381575 

[Sybase-bevezetés mindig a harmadik DR csomópont beállításával](https://techcommunity.microsoft.com/t5/running-sap-applications-on-the/installation-procedure-for-sybase-16-3-patch-level-3-always-on/ba-p/368199) 
