...
#### <a name="commands-to-confirm"></a>Megerősítő parancsok
```
modinfo enic
modinfo fnic
```
...
*   `/etc/default/grub_installdevice` fájl frissítése a line `/dev/mapper/<LUN ID>`val. Példa:/dev/Mapper/3600a09803830372f483f495242534a56
>[!NOTE]
...
blacklist edac_core
```
A módosítások érvénybe lépéséhez újraindítás szükséges. Hajtsa végre `lsmod` parancsot, és ellenőrizze, hogy a modul nem található-e a kimenetben.


### <a name="kernel-parameters"></a>Kernel-paraméterek
   Győződjön meg arról, hogy a `transparent_hugepage`, `numa_balancing`, `processor.max_cstate`, `ignore_ce` és `intel_idle.max_cstate` helyes beállítását alkalmazza a rendszer.
...
* Adja hozzá ezeket a paramétereket a fájl `GRB_CMDLINE_LINUX` sorához `/etc/default/grub`
```
