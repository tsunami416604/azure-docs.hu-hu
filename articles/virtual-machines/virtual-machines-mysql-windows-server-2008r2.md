...
2.  選取右上角的 [工具]**** 按鈕 (齒輪圖示)，然後按一下 [網際網路選項]****。 依序按一下 [安全性]**** 索引標籤和[信任的網站]**** 圖示，然後按一下 [網站]**** 按鈕。 將 http://*.mysql.com 加入至信任的網站清單。 按一下 [關閉]****，然後按一下 [確定]****。
...
> [AZURE.NOTE] 您應該仔細考慮這麼做對安全性的影響，因為這會將 MySQL 伺服器服務提供給網際網路上的所有電腦。 您可以定義來源 IP 位址的集合，以便使用具備存取控制清單 (ACL) 的端點。 如需詳細資訊，請參閱 [如何設定端點的虛擬機器](virtual-machines-set-up-endpoints.md)。
