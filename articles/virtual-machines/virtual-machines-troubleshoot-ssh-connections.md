...
1. **重設遠端存取** 從 [Azure 入口網站](https://portal.azure.com)。 依序按一下 [全部瀏覽]**** > [虛擬機器 (傳統)]**** > 您的 Windows 虛擬機器 > [重設遠端存取]****。
...
3. [* * 調整 * * 虛擬機器](https://msdn.microsoft.com/library/dn168976.aspx)。
...
5. 檢查 VM 的資源健康情況是否有任何平台問題。 
    依序按一下 [全部瀏覽] > [虛擬機器 (傳統)] > 您的 Linux 虛擬機器 > [檢查健康情況]****
...
1. 在命令列上使用 Azure CLI 或 Azure PowerShell 為您的 Linux VM **重設 SSH 連線**。 請確定 [Microsoft Azure Linux 代理程式](virtual-machines-linux-agent-user-guide.md) 2.0.5 版或更新版本安裝。

    **使用 Azure CLI**

    a. 如果您還沒有這麼做， [安裝 Azure CLI 並連接到您的 Azure 訂閱](../xplat-cli-install.md) 使用 `azure 登入` 命令。
    ...
    * 使用 `vm 重設存取` 命令，如下列範例所示。

    ```
    azure vm reset-access -g TestRgV2 -n TestVmV2 -r
    ```

    這將會安裝 `VMAccessForLinux` 延伸模組，您的虛擬機器。
    ...
    * 或者，使用下列內容建立名為 PrivateConfig.json 的檔案︰
    ...
    手動執行 `VMAccessForLinux` 重設 SSH 連線的延伸模組。
    ...
    **使用 Azure PowerShell**
    ...
    c. 執行 `VMAccessForLinux` 重設您的 SSH 連線，如下列範例所示的延伸模組。
...
2. 從入口網站「重新啟動」****您的 Linux VM。 從 [Azure 入口網站](https://portal.azure.com), ，按一下 [ **全部瀏覽** > **虛擬機器** > 您的 Windows 虛擬機器 > **重新啟動**。
...
3. 可使用 Azure CLI 或 Azure PowerShell 在命令列上替您的 Linux VM「重設密碼或 SSH 金鑰」****。 如下列範例所示，您也可以透過 sudo 授權建立新的使用者名稱和密碼。
    ...
    * 執行 `vm 重設存取` 命令來設定任何 SSH 認證。

    ```
    azure vm reset-access TestRgV2 TestVmV2 -u NewUser -p NewPassword
    ```

    請參閱輸入的相關資訊 `azure vm 重設存取 h` 命令列上。

    * 或者，建立含有下列內容，名為 PrivateConf.json 的檔案。
    ```
    {
...
> [AZURE.NOTE] 此來源僅適用於使用傳統部署模型所建立的虛擬機器。 使用資源管理員所建立的虛擬機器，請跳到 [來源 4: 網路安全性群組](#nsg)。
...
若要排除端點為問題來源之可能性，請移除目前的端點、建立一個新端點，再指定 **SSH** 名稱 (TCP 連接埠 22 做為公用及私用連接埠號碼)。 如需詳細資訊，請參閱 [Azure 中的虛擬機器上設定端點](virtual-machines-set-up-endpoints.md)。
...
[疑難排解以 windows Azure 虛擬機器的 Windows 遠端桌面連線](virtual-machines-troubleshoot-remote-desktop-connections.md)
