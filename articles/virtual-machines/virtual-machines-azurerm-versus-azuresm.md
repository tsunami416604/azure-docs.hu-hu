...
- 具有高度安全的儲存與部署私人密碼從 Azure 金鑰保存庫的虛擬機器的整合 [FIPS 驗證](http://wikipedia.org/wiki/FIPS_140-2) [硬體安全性模組](http://wikipedia.org/wiki/Hardware_security_module)
...
您可以找到與運算、 網路和儲存體資源提供者 REST API 詳細說明文件 [這裡](https://msdn.microsoft.com/library/azure/dn790568.aspx)。
...
虛擬機器、 虛擬網路和透過新的 Azure Resource Manager Api 建立的儲存體帳戶的配額是分開的目前擁有的配額。 每個訂閱會得到新的配額，然後就可以使用新的 API 建立資源。 閱讀更多有關其他配額 [這裡](../azure-subscription-service-limits.md)。
...
所有您建立的自動化和指令碼，仍然適用於 Azure 服務管理模式下建立的現有虛擬機器和虛擬網路。 不過，您必須更新指令碼，才能使用新的結構描述並透過 Azure Resource Manager 模式建立相同的資源。
 深入了解如何修改您 [Azure CLI 指令碼](xplat-cli-azure-manage-vm-asm-arm.md)。
