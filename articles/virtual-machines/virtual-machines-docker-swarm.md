...
本主題會建立四個 VM，但您可以使用任何您想要的數目。 呼叫下列 *& lt; password & gt;* 您所選擇的密碼來取代。
...
當您完成時您應該能夠使用 **vm list** 來查看您的 Azure Vm:
...
本主題使用 [容器模型的安裝來自 docker swarm 文件](https://github.com/docker/swarm#1---docker-image) -但是，您也可以 SSH 到 **swarm 主要**。 在此模型中， **swarm** 下載為執行 swarm 的 docker 容器。 我們執行此步驟 *從膝上型電腦使用 docker 遠端* 連線到 **swarm 主要** VM 並告知它使用叢集識別碼建立命令 **swarm 建立**。 叢集識別碼是如何 **swarm** 探索 swarm 群組成員。 (您也可以複製儲存機制並自行建置它，這樣做可讓您擁有完整控制權且能夠進行偵錯)。
...
> 若要確認這一點，執行 `docker -H tcp://`*& lt; 主機名稱 （& s) gt;* ` images` 上列出的容器處理程序 **swarm 主要** 電腦和用於比較的另一個節點上 (因為我們已與上一個 swarm 命令，所以 **--rm** 交換器，容器已移除它完成，因此使用 **docker ps-a** 不會傳回任何項目)。:
        ...
        $
<P />
> 如果您熟悉 **docker**, ，就知道其他節點會包含任何項目，因為並未下載並尚未執行任何映像。

## 將節點 VM 加入我們的 docker 叢集

針對每個節點，使用 Azure CLI 列出端點資訊。 下面這樣 **swarm 節點 1** docker 主機，以取得節點的 docker 連接埠。
...
針對叢集中的所有其他節點重複執行。 在本例中，這是 **swarm 節點 2** 和 **swarm node 3**。
