...
1. 使用 Azure 虛擬機器 **DNS 名稱**做為 URL 中的伺服器名稱，瀏覽至報表管理員。 例如：
    ...
    **報表伺服器**: http://uebi.cloudapp.net/reports
    ...
    [設定報表伺服器存取的防火牆](https://technet.microsoft.com/library/bb934283.aspx)
...
- **SQL Server Data Tools**: VM: SQL Server 資料工具安裝在虛擬機器上，而且可用來建立 **報表伺服器專案** 和虛擬機器上的報表。 SQL Server Data Tools 可以將報表發佈至虛擬機器上的報表伺服器。
...
1. 在 SQL Server 安裝精靈中，按一下左窗格中的 [安裝]****，然後按一下 [新的 SQL 伺服器安裝或將功能加入到現有安裝]****。
    ...
    - 如果您看到 [瀏覽資料夾]****，請瀏覽至 c:\SQLServer_12.0_full 或 c:\SQLServer_11.0_full，然後按一下 [確定]****。
...
- [混合式環境中使用 Power BI](https://msdn.microsoft.com/library/dn798994.aspx)

>[AZURE.NOTE] [提交意見和連絡資訊，透過 Microsoft SQL Server Connect](https://connect.microsoft.com/SQLServer/Feedback)

### 社群內容

- [使用 PowerShell 的 azure SQL Database 管理](http://blogs.msdn.com/b/windowsazure/archive/2013/02/07/windows-azure-sql-database-management-with-powershell.aspx)
