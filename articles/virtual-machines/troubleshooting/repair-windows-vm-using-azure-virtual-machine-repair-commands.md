...
1. Az Azure Cloud Shell elindítása
   ...
   Ha egy másik fiókkal kell bejelentkeznie Cloud Shellba, mint amit jelenleg a Azure Portalba bejelentkezett, akkor használhatja az az ``az login`` [login Reference](/cli/azure/reference-index#az-login&preserve-view=true)(az).  A fiókjához társított előfizetések közötti váltáshoz használhatja ``az account set --subscription`` [az az Account set Reference](/cli/azure/account#az-account-set&preserve-view=true)lehetőséget.
