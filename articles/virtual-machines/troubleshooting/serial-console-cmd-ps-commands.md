...
A SAC lehetővé teszi, hogy soros porton keresztül kapcsolódjon a futó operációs rendszerhez. A CMD a SACből való indításakor `sacsess.exe` elindítja `cmd.exe` a futó operációs rendszeren. Láthatja, hogy a Feladatkezelőban, ha a virtuális gép RDP-kapcsolata egyszerre csatlakozik a SAC szolgáltatáshoz a soros konzolon keresztül. A SAC-n keresztül elért CMD ugyanaz a `cmd.exe`, mint amikor RDP-kapcsolaton keresztül csatlakozik. Mindegyik parancs és eszköz elérhető, beleértve a PowerShell indításának lehetőségét is a CMD-példányból. Ez jelentős különbség a SAC és a Windows helyreállítási környezet (WinRE) között, amely lehetővé teszi a futó operációs rendszer felügyeletét, ahol a WinRE egy másik, minimális operációs rendszerbe kerül. Habár az Azure-beli virtuális gépek nem támogatják a WinRE elérésének lehetőségét a soros konzol szolgáltatással, az Azure-beli virtuális gépek a SAC használatával kezelhetők.

Mivel a SAC 80x24 van korlátozva, és a görgetés nem áll vissza, adja hozzá a `| more` a parancsokhoz, hogy egyszerre megjelenjen a kimenet egy oldala. A következő sor megjelenítéséhez használja a `<spacebar>` a következő oldal megjelenítéséhez, vagy `<enter>`.
...
További információ a `netsh`ről: [ide kattintva](https://docs.microsoft.com/windows-server/networking/technologies/netsh/netsh-contexts).
...
Az általánosított rendszerképből létrehozott Azure-beli virtuális gépek helyi rendszergazdai fiókja átnevezve lesz a virtuális gép üzembe helyezése során megadott névre. Így általában nem lesz `Administrator`.
...
Módosítsa `/c:10` a visszaadni kívánt számú eseményre, vagy helyezze át úgy, hogy a szűrőnek megfelelő összes eseményt visszaállítsa.
...
A `sort /r` a telepítési dátum alapján csökkenő sorrendbe rendezi, így könnyen megtekintheti, hogy mi volt a közelmúltban telepítve. A `<spacebar>` segítségével folytassa a kimenet következő oldalára, vagy `<enter>` egy sorba való haladáshoz.
### <a name="uninstall-an-application"></a>Alkalmazás eltávolítása
`wmic path win32_product where name="<name>" call uninstall`

Cserélje le a `<name>`t a fenti parancsban visszaadott névre az eltávolítani kívánt alkalmazásra.
...
A `/restore` használatakor a `/save`használatakor megadott mappa szülőjének kell lennie. Ebben a példában a `\RSA` a fenti `/save` példában megadott `\MachineKeys` mappa szülője.
...
`/f` hozzáadásával kényszerítheti az alkalmazások bezárását a figyelmeztetést nem használó felhasználók nélkül.
...
`Get-Service` használható, de nem tartalmazza a szolgáltatás bejelentkezési fiókját. `Get-WmiObject win32-service`.
### <a name="set-service-logon-account"></a>Szolgáltatás bejelentkezési fiókjának beállítása
`(get-wmiobject win32_service -filter "name='termservice'").Change($null,$null,$null,$null,$null,$false,'NT Authority\NetworkService')`

Ha `NT AUTHORITY\LocalService`, `NT AUTHORITY\NetworkService`vagy `LocalSystem`eltérő szolgáltatásfiókot használ, adja meg a fiók jelszavát az utolsó (nyolcadik) argumentumként a fiók neve után.
### <a name="set-service-startup-type"></a>Szolgáltatás indítási típusának beállítása
`set-service termservice -startuptype Manual`

`Set-service` az indítási típushoz `Automatic`, `Manual`vagy `Disabled`t fogad el.
### <a name="set-service-dependencies"></a>Szolgáltatási függőségek beállítása
...
a `Get-NetAdapter` 2012 +-ban érhető el, és a 2008R2 használja `Get-WmiObject`.
### <a name="show-ip-properties"></a>IP-tulajdonságok megjelenítése
...
a `Get-NetAdapter` 2012 + rendszeren érhető el. A 2008R2 használja a `Get-WmiObject`. Az Azure-beli virtuális gépeket mindig úgy kell konfigurálni a vendég operációs rendszeren, hogy DHCP használatával igényeljenek IP-címet. Az Azure statikus IP-beállítása továbbra is DHCP protokollt használ, hogy az IP-címet a virtuális géphez adja.
### <a name="ping"></a>Ping
`test-netconnection`

> [!NOTE]
> Előfordulhat, hogy a Write-Progress parancsmag nem működik ezzel a paranccsal. Enyhítő megoldásként a PowerShellben futtathatja a `$ProgressPreference = "SilentlyContinue"`t a folyamatjelző sáv letiltásához.

vagy

`get-wmiobject Win32_PingStatus -Filter 'Address="8.8.8.8"' | format-table -autosize IPV4Address,ReplySize,ResponseTime`

`Test-Netconnection` paraméterek nélkül a `internetbeacon.msedge.net`pingelése is megpróbálkozik. 2012 és újabb rendszereken érhető el. A 2008R2 a második példában a `Get-WmiObject` is használják.
### <a name="port-ping"></a>Port pingelése
...
a `Test-NetConnection` 2012 + rendszeren érhető el. A 2008R2 használata `Net.Sockets.TcpClient`
### <a name="test-dns-name-resolution"></a>DNS-névfeloldási teszt
...
a `Resolve-DnsName` 2012 + rendszeren érhető el. A 2008R2 használja a `System.Net.DNS`.
...
a `Get-NetFirewallPortFilter` 2012 + rendszeren érhető el. A 2008R2 használja a `hnetcfg.fwpolicy2` COM-objektumot.
### <a name="disable-windows-firewall"></a>A Windows tűzfal letiltása
`Set-NetFirewallProfile -Profile Domain,Public,Private -Enabled False`

a `Set-NetFirewallProfile` 2012 + rendszeren érhető el. A 2008R2 használja a fenti CMD szakaszban hivatkozott `netsh advfirewall`.
## <a name="manage-users-and-groups"></a>Felhasználók és csoportok kezelése
### <a name="create-local-user-account"></a>Helyi felhasználói fiók létrehozása
`new-localuser <name>`
### <a name="verify-user-account-is-enabled"></a>Felhasználói fiók ellenőrzése engedélyezve
...
a `Get-LocalUser` 2012 + rendszeren érhető el. A 2008R2 használja a `Get-WmiObject`. Ez a példa a beépített helyi rendszergazdai fiókot mutatja, amely mindig rendelkezik SID `S-1-5-21-*-500`. Az általánosított rendszerképből létrehozott Azure-beli virtuális gépek helyi rendszergazdai fiókja átnevezve lesz a virtuális gép üzembe helyezése során megadott névre. Így általában nem lesz `Administrator`.
### <a name="add-local-user-to-local-group"></a>Helyi felhasználó hozzáadása a helyi csoporthoz
`add-localgroupmember -group Administrators -member <username>`
### <a name="enable-local-user-account"></a>Helyi felhasználói fiók engedélyezése
`get-localuser | where {$_.SID -like "S-1-5-21-*-500"} | enable-localuser`

Ez a példa engedélyezi a beépített helyi rendszergazdai fiókot, amely mindig rendelkezik SID `S-1-5-21-*-500`. Az általánosított rendszerképből létrehozott Azure-beli virtuális gépek helyi rendszergazdai fiókja átnevezve lesz a virtuális gép üzembe helyezése során megadott névre. Így általában nem lesz `Administrator`.
### <a name="view-user-account-properties"></a>Felhasználói fiók tulajdonságainak megtekintése
...
a `Get-LocalUser` 2012 + rendszeren érhető el. A 2008R2 használja a `Get-WmiObject`. Ez a példa a beépített helyi rendszergazdai fiókot mutatja, amely mindig rendelkezik SID `S-1-5-21-*-500`.
### <a name="view-local-groups"></a>Helyi csoportok megtekintése
`(get-localgroup).name | sort` `(get-wmiobject win32_group).Name | sort`

a `Get-LocalUser` 2012 + rendszeren érhető el. A 2008R2 használja a `Get-WmiObject`.
## <a name="manage-the-windows-event-log"></a>A Windows-Eseménynapló kezelése
### <a name="query-event-log-errors"></a>Eseménynaplók lekérdezése – hibák
...
Ez a példa létrehoz egy `c:\bin` mappát, majd letölti és kicsomagolja a Sysinternals csomagot az `c:\bin`ba.
## <a name="miscellaneous-tasks"></a>Egyéb feladatok
### <a name="show-os-version"></a>Operációs rendszer verziójának megjelenítése
...
`<days>:<hours>:<minutes>:<seconds>:<milliseconds>`-ként adja vissza a üzemidőt, például `49:16:48:00.00`.
### <a name="restart-windows"></a>Windows újraindítása
`restart-computer`

`-force` hozzáadásával kényszerítheti az alkalmazások bezárását a figyelmeztetést nem használó felhasználók nélkül.
