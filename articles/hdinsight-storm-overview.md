...
[Apache Storm][Apache Storm] is a distributed, fault-tolerant, open source computation system that allows you to process data in realtime. Storm solutions can also provide guaranteed processing of data, with the ability to replay data that was not successfully processed the first time.
...
To get started using Storm, see [Getting started with Storm in HDInsight][Getting started with Storm in HDInsight].
...
For more information on Storm components, see the [Storm tutorial][Storm tutorial] at apache.org.

## Scenarios: What are the use cases for Storm?

The following are some common scenarios for which you might use Apache storm. For information on real world scenarios, read [how companies are using Storm][how companies are using Storm].
...
For more information on distributed RPC and the DRPCClient provided with Storm, see [Distributed RPC][Distributed RPC].

### Online machine learning

Storm can be used with machine learning solution that has previously been trained by batch processing, such as a solution based on Mahout. However its generic, distributed computation model also opens the door for stream-based machine learning solutions. For example, the [Scalable Advanced Massive Online Analysis (SAMOA) project][Scalable Advanced Massive Online Analysis (SAMOA) project] is a machine learning library that uses stream processing, and can work with Storm.

## What programming languages can I use?

The HDInsight Storm cluster provides support for .NET, Java, and Python out of the box. While Storm [supports other languages][supports other languages], many of these will require you to install an additional programming language on the HDInsight cluster in addition to other configuration changes.
...
For more information on developing with SCP, see [Develop streaming data processing applications with SCP.NET and C# on Storm in HDInsight][Develop streaming data processing applications with SCP.NET and C# on Storm in HDInsight].
...
For more information on Trident, see the [Trident tutorial][Trident tutorial] at apache.org.

For examples of both raw Java and Trident topologies, see the **%storm\_home%\\contrib\\storm-starter** directory on your HDInsight Storm cluster.
...
For more information, see [Guarantees on data processing][Guarantees on data processing] at apache.org

### BasicBolt

The pattern of reading an input tuple, emitting zero or more tuples, and then acking the input tuple immediately at the end of the execute method, is so common that Storm provides the [IBasicBolt][IBasicBolt] interface to automate this pattern.

### Joins

Joining two streams of data will vary between applications. For example, you may join each tuple from multiple streams into one new stream, or you may only join batches of tuples for a specific window. Either way, joining can be accomplished by using [fieldsGrouping][fieldsGrouping], which is a way of defining how tuples are routed to bolts.
...
For an example of using tick tuples, see [Analyzing sensor data with Storm and HDInsight][Analyzing sensor data with Storm and HDInsight]

If you are using Trident, it is based on processing batches of tuples.

### Caching

In-memory caching is often used as a mechanism for speeding up processing, as it keeps frequently used assets in memory. Since a topology is distributed across multiple nodes, and multiple processes within each node, you should consider using [fieldsGrouping][fieldsGrouping] to ensure that tuples containing the fields that are used for cache lookup are always routed to the same process. This avoids duplication of cache entries across processes.

### Streaming top N

When your topology depends on calculating a 'top' N value, such as the top 5 trends on Twitter, you should calculate the top N value in parallel and then merge the output from those calculations into a global value. This can be done using [fieldsGrouping][fieldsGrouping] to route by field to the parallel bolts, which partitions the data by field value, then finally route to a bolt that globally determines the top N value.

For an example of this, see the [RollingTopWords][RollingTopWords] example.

## Next steps

-   [Getting Started with Storm in HDInsight][Getting started with Storm in HDInsight]

-   [Analyzing sensor data with Storm and HDInsight][1]
...
-   [Develop streaming data processing applications with SCP.NET and C# on Storm in HDInsight][Develop streaming data processing applications with SCP.NET and C# on Storm in HDInsight]
  ...
  [Getting started with Storm in HDInsight]: /en-us/documentation/articles/hdinsight-storm-getting-started
  ...
  [Scalable Advanced Massive Online Analysis (SAMOA) project]: http://yahooeng.tumblr.com/post/65453012905/introducing-samoa-an-open-source-platform-for-mining
  [supports other languages]: https://storm.incubator.apache.org/about/multi-language.html
  ...
  [Trident tutorial]: https://storm.incubator.apache.org/documentation/Trident-tutorial.html
  [Guarantees on data processing]: https://storm.apache.org/about/guarantees-data-processing.html
  ...
  [Analyzing sensor data with Storm and HDInsight]: /en-us/documentation/articles/hdinsight-storm-sensor-data-analysis.md
  [RollingTopWords]: https://github.com/nathanmarz/storm-starter/blob/master/src/jvm/storm/starter/RollingTopWords.java
  [1]: /en-us/documentation/articles/hdinsight-storm-sensor-data-analysis
