...
A HTTP-támogatással Hibrid kapcsolatokhoz csatolt figyelőknek kezelniük `request` kell a kézmozdulatot. Egy figyelő, amely nem kezeli `request` a szolgáltatást, ezért a csatlakozás során ismételt időtúllépési hibákat okoz, és a szolgáltatás a jövőben is feketelistára kerül.
...
2. Ha a kérelem fejléce tartalmaz `ServiceBusAuthorization` egy fejlécet, a fejléc kifejezés mindig el lesz távolítva a fejléc-gyűjteményből.
...
A `namespace-address` a Azure Relay névtér teljes tartományneve, amely a hibrid kapcsolatokat tárolja, jellemzően az űrlapon `{myname}.servicebus.windows.net`.
...
| `{path}`         | Igen      | Az előre konfigurált hibrid kapcsolatok URL-kódolt névtérbeli elérési útja a figyelő regisztrálásához. A kifejezés a rögzített `$hc/` elérésiút-részhez lesz hozzáfűzve.
...
Ugyanez vonatkozik a `Sec-WebSocket-Extensions` fejlécre is. Ha a keretrendszer támogatja a bővítményt, állítsa be a fejlécet a bővítmény szükséges `Sec-WebSocket-Extensions` kézfogásának kiszolgálóoldali válaszára.

Az URL-címet a-ként kell használni az Accept szoftvercsatorna létrehozásához, de az alábbi paramétereket tartalmazza:

| Paraméter      | Kötelező | Leírás
| -------------- | -------- | -------------------------------------------------------------------
| `sb-hc-action` | Igen      | A szoftvercsatorna elfogadásához a paraméternek a következőnek kell lennie:`sb-hc-action=accept`
| `{path}`       | Igen      | (lásd a következő bekezdést)
| `sb-hc-id`     | Nem       | Lásd az **azonosító**korábbi leírását.

`{path}`az előre konfigurált hibrid kapcsolatok URL-kódolt névtérbeli elérési útja, amelyen regisztrálni kell a figyelőt. A kifejezés a rögzített `$hc/` elérésiút-részhez lesz hozzáfűzve.
...
##### <a name="rejecting-the-socket"></a>A szoftvercsatorna elutasítása
 ...
 A szoftvercsatorna elutasítása az `accept` üzenet megvizsgálása után hasonló kézfogást igényel, hogy az elutasítás okát közlő állapotkód és az állapot leírása vissza tudjon térni a küldőnek.
 ...
 A szoftvercsatorna elutasításához az ügyfél átveszi a címet `accept` tartalmazó URI-t az üzenetből, és két lekérdezési karakterlánc paramétert fűz hozzá, az alábbiak szerint:
...
Ha nincs törzs, a rendszer kihagyja a törzs kereteit. A kérelem üzenetében a logikai `body` tulajdonság jelzi, hogy a törzs megtalálható-e.
...
A JSON-tartalma `request` a következő:
...
A válasz egy "Response" nevű JSON-objektum. A törzs tartalmának kezelésére vonatkozó szabályok pontosan ugyanúgy, mint `request` az üzenet és a `body` tulajdonság alapján.

* **kérelemazonosító** – karakterlánc. Szükséges. A `id` válaszban szereplő `request` üzenet tulajdonságának értéke.
...
* **responseHeaders** – külső http-válaszban beállítani kívánt HTTP-fejlécek.
  Akárcsak a esetében `request`, a RFC7230 definiált fejlécek nem használhatók.
...
Az 64 kB-nál nagyobb válaszok esetén a választ egy Rendezvous szoftvercsatornán kell továbbítani. Továbbá, ha a kérelem meghaladja az 64 kB-ot `request` , és az csak a címe mezőt tartalmazza, egy Rendezvous szoftvercsatornát kell létrehozni `request`a beszerzéséhez. Miután létrejött a Rendezvous-szoftvercsatorna, a megfelelő ügyfélre és a megfelelő ügyfélről érkező további kérelmekre adott válaszokat a rendszer a Rendezvous szoftvercsatornán keresztül továbbítja, amíg az megmarad.

A `address` (z) `request` rendszerbeli URL-címet a Rendezvous szoftvercsatorna létrehozásához kell használni, de a következő paramétereket tartalmazza:

| Paraméter      | Kötelező | Leírás
| -------------- | -------- | -------------------------------------------------------------------
| `sb-hc-action` | Igen      | A szoftvercsatorna elfogadásához a paraméternek a következőnek kell lennie:`sb-hc-action=request`
...
Ha a figyelő token hamarosan lejár, lecserélheti azt úgy, hogy szöveges keretbeli üzenetet küld a szolgáltatásnak a megadott vezérlési csatornán keresztül. Az üzenet tartalmaz egy nevű `renewToken`JSON-objektumot, amely jelenleg a következő tulajdonságot határozza meg:
...
A kérés tetszőleges további HTTP-fejléceket tartalmazhat, beleértve az alkalmazás által definiált is. A rendszer az összes megadott fejlécet átirányítja a figyelőnek, `connectHeader` és megtalálható az **Accept** Control üzenet objektumán.
...
| `sb-hc-id`     | Nem        | Egy opcionális azonosító, amely lehetővé teszi a végpontok közötti diagnosztikai nyomkövetést, és elérhetővé válik a figyelő számára az elfogadási kézfogás során.
 ...
 `{path}` A az előre konfigurált hibrid kapcsolatok URL-kódolt névtérbeli elérési útja, amelyen regisztrálni kell a figyelőt. A `path` kifejezés kiterjeszthető egy utótaggal és egy lekérdezési karakterlánc kifejezéssel a további kommunikációhoz. Ha a hibrid kapcsolatok az elérési úton `hyco`vannak regisztrálva `path` , a kifejezést `hyco/suffix?param=value&...` követheti az itt definiált lekérdezési karakterlánc paraméterei is. A teljes kifejezés a következő lehet:
...
A jogkivonat a vagy `ServiceBusAuthorization` `Authorization` a HTTP-fejlécben is elvégezhető. A jogkivonat kihagyható, ha a hibrid kapcsolat úgy van konfigurálva, hogy engedélyezze a névtelen kérelmeket.

Mivel a szolgáltatás hatékonyan működik proxyként, még akkor is, ha nem valódi HTTP-proxyként, egy `Via` fejlécet vagy jegyzeteket tesz a `Via` meglévő fejlécnek, amely megfelel a [RFC7230, a 5.7.1 szakasznak](https://tools.ietf.org/html/rfc7230#section-5.7.1).
A szolgáltatás hozzáadja a továbbítási névtér állomásnevét `Via`a következőhöz:.
...
Ha hiba merül fel, a szolgáltatás a következőképpen válaszolhat. Azt jelzi, hogy a válasz a szolgáltatásból származik-e, vagy a figyelő a `Via` fejléc jelenlétében azonosítható-e. Ha a fejléc jelen van, a válasz a figyelőtől származik.
