...
建立內部負載平衡器使用 `azure 網路 lb 建立` 命令。 在下列案例中，有一個名為 nrprg 的資源群組建立於美國東部區域。
...
**-i** -探查間隔 (秒)
**-c** 檢查項目數目
>[AZURE.NOTE] Microsoft Azure 平台會對各種管理案例使用靜態、可公開路由傳送的 IPv4 位址。 IP 位址是 168.63.129.16。 此 IP 位址不應該遭到任何防火牆封鎖，因為可能會造成非預期的行為。
...
建立名為 *lb-nic2-be* 的 NIC，並將其關聯到 *rdp2* NAT 規則及 *beilb* 後端位址集區。
...
建立名為 *DB2* 的虛擬機器 (VM)，並將其關聯到名為 *lb-nic2-be* 的 NIC。 在執行下列命令之前，會先建立名為 *web1nrp* 的儲存體帳戶。
