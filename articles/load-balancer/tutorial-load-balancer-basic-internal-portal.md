...
2. In the **Basics** tab of the **Create load balancer** page, enter or select the following information, accept the defaults for the remaining settings, and then select **Review + create**:
    ...
    | Erőforráscsoport         | Select **Create new** and type *MyResourceGroupLB* in the text box.|
    | Név                   | *myLoadBalancer*                                   |
    | Region (Régió)         | Válassza a **Nyugat-Európa** régiót.                                        |
    | Type (Típus)          | Select **Internal**.                                        |
    | SKU (Cikkszám)           | Válassza az **Alapszintű** lehetőséget.                          |
    | Virtuális hálózat           | Select *MyVNet*.                          |    
...
To allow the load balancer to monitor VM status, you use a health probe. Az állapotminta dinamikusan hozzáadja a virtuális gépeket a terheléselosztó rotációjához vagy eltávolítja belőle azokat, az állapotellenőrzésekre adott válaszuk alapján. 
...
A terheléselosztási szabály megadja, hogy a rendszer hogyan ossza el a forgalmat a virtuális gépek között. The rule defines the front-end IP configuration for incoming traffic, the back-end IP pool to receive the traffic, and the required source and destination ports. 
