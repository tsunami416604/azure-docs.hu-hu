 ...
 **備份原則** 頁面可讓您管理備份原則並排程本機和雲端快照。 (備份原則用來設定磁碟區集合的備份排程和備份保留)。備份原則可讓您同時建立多個磁碟區的快照。 這表示備份原則所建立的備份將會是與當機時一致的複本。  **備份原則** 頁面會列出備份原則、 其類型、 相關聯的磁碟區、 備份保留，數目以及啟用這些原則的選項。
...
- **原則名稱** – 與原則相關聯的名稱。 不同類型的原則包括：
   ...
   - 已排程的原則 (由使用者明確建立)。
   - 自動原則 (建立磁碟區時啟用此磁碟區選項的預設備份時所建立)。 這些原則會命名為 *VolumeName*_Default 其中 *VolumeName* 是指使用者在 Azure 傳統入口網站中設定之 StorSimple 磁碟區名稱。 自動原則會導致每日雲端快照於 22:30 裝置時間開始。
...
![可用的視訊](./media/storsimple-manage-backup-policies-u2/Video_icon.png) **可用的視訊**
