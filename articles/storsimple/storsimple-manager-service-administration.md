...
| | 裝置 →| 監視|
| | 備份類別目錄| 磁碟區 containers→| 磁碟區|
| | 設定 (服務)| 備份原則| |
| | 作業| 設定 (裝置)|
| | Alerts| 維護|

![段影片提供](./media/storsimple-manager-service-administration/Video_icon.png) **段影片提供**
...
| 深入了解災害復原和裝置容錯移轉</br>容錯移轉至實體裝置</br>容錯移轉至虛擬裝置</br>營運持續災害復原 (BCDR)| StorSimple Manager 服務 → 裝置| [StorSimple 裝置的容錯移轉和災害復原](storsimple-device-failover-disaster-recovery.md)|
...
| 關於儲存體帳戶</br>加入儲存體帳戶</br>編輯儲存體帳戶</br>刪除儲存體帳戶</br>的儲存體帳戶金鑰輪替| StorSimple Manager 服務 → 設定| [管理儲存體帳戶](storsimple-manage-storage-accounts.md)|
| 頻寬範本的相關</br>新增頻寬範本</br>編輯頻寬範本</br>刪除頻寬範本</br>使用預設頻寬範本</br>建立在指定時間啟動的全天頻寬範本| StorSimple Manager 服務 → 設定| [管理頻寬範本](storsimple-manage-bandwidth-templates.md)|
| 關於存取控制記錄</br>建立存取控制記錄</br>編輯存取控制記錄</br>刪除存取控制記錄| StorSimple Manager 服務 → 設定| [管理存取控制記錄](storsimple-manage-acrs.md)|
| 檢視作業詳細資料</br>取消工作| StorSimple Manager 服務 → 工作| [管理工作](storsimple-manage-jobs.md)
| 接收警示通知</br>管理警示</br>檢閱警示| StorSimple Manager 服務 → 警示| [檢視和管理 StorSimple 警示](storsimple-manage-alerts.md)
| 檢視連接的啟動器</br>尋找裝置序號</br>尋找目標 IQN| StorSimple Manager 服務 → 裝置 → 儀表板| [使用 StorSimple 裝置儀表板](storsimple-device-dashboard.md)|
...
| 檢視 Web Proxy 設定| StorSimple Manager 服務 → 裝置 → 設定| [設定您的裝置的 web proxy](storsimple-configure-web-proxy.md)|
...
| 設定遠端管理| StorSimple Manager 服務 → 裝置 → 設定| [遠端連線到您的 StorSimple 裝置](storsimple-remote-connect.md)|
...
- [可運作裝置疑難排解](storsimple-troubleshoot-operational-device.md)
- [使用 StorSimple 監視 led 指示燈](storsimple-monitoring-indicators.md)

如果您不能解決問題，且您必須建立服務要求，請參閱：

-  [請連絡 Microsoft 支援](storsimple-contact-microsoft-support.md)
