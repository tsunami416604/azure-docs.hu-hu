...
| Figyelmeztető szöveg | Esemény | További információ / javasolt műveletek |
|:--- |:--- |:--- |
| Helyreállítási művelet nem tudta visszaállítani a beállítások ezt a szolgáltatást. Eszköz konfigurációs adatokat az egyes eszközök esetében inkonzisztens állapotban van. |Adatok inkonzisztencia észlelhető katasztrófa utáni helyreállítás után. |A szolgáltatás a titkosított adatok nincs szinkronizálva, az eszközön. Engedélyezi az eszköz <*eszköznév*> a StorSimple Manager a szinkronizálás megkezdéséhez. A Windows PowerShell-felületet a StorSimple segítségével futtathatja a `Restore-HcsmEncryptedServiceData` eszközön <*eszköznév*> parancsmag, ez a parancsmag csak a biztonsági profil visszaállítása bemenetként a régi jelszó megadása. Ezután futtassa a `Invoke-HcsmServiceDataEncryptionKeyChange` parancsmaggal frissítsen a szolgáltatásadat-titkosítási kulcs. Miután elvégezte a megfelelő műveletet, törölje ezt a riasztást a riasztások lapján. |
...
| Közelgő meghajtó hibát észlelt. | Közelgő meghajtó hibát észlelt. |Azt észleltük közelgő meghajtó hibát hardver összetevő "tárolóhelyen lévő meghajtó <*azonosító tárolóhely*>, ház <*ház azonosító*>". Vegye figyelembe, hogy a meghajtó helyett. <br> A lemezcsere megkezdése előtt tekintse át az alábbi információkat.<br><br>Ha az eszközön egynél több meghibásodott lemezt, távolítsa el az egynél több SSD és HDD bármikor. Így adatvesztést eredményezhet.<br><br>Ellenőrizze, hogy egy helyettesítő SSD helyez egy SSD korábban tartalmazó tárhely. Ugyanez vonatkozik egy HDD.<br><br>Tárolóhelyek – 11 számozása 0-tól. A hibás lemez tárolóhelye 2 tárolóhelye 3 az eszköz a meghibásodott lemezt (a bal felső sarokban) rendeli hozzá.<br><br>Lemezcsere kapcsolatos további információkért látogasson el https://go.microsoft.com/fwlink/?linkid=838653. Ha a probléma továbbra is fennáll, forduljon a Microsoft támogatási https://go.microsoft.com/fwlink/?linkid=838654 keresztül. |
