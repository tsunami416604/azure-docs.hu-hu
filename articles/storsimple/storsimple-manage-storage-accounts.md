...
- **自動產生的儲存體帳戶** – 正如其名，這類型的儲存體帳戶是在初次建立服務時自動產生。 若要深入了解如何建立此儲存體帳戶，請參閱 [步驟 1: 建立新的服務](storsimple-deployment-walkthrough-u1.md#step-1-create-a-new-service) 中 [部署在內部部署 StorSimple 裝置](storsimple-deployment-walkthrough.md)。
- **服務訂用帳戶中的儲存體帳戶** – 這些是與相同服務訂用帳戶相關聯的 Azure 儲存體帳戶。 若要深入了解如何使用這些儲存體建立帳戶，請參閱 [關於 Azure 儲存體帳戶](../storage/storage-create-storage-account.md)。
...
3. 在 [新增/編輯儲存體帳戶]**** 對話方塊中：
  ...
  4. 按一下核取圖示 ![核取圖示](./media/storsimple-manage-storage-accounts/HCS_CheckIcon.png) 儲存設定。 [設定]**** 頁面上的設定隨即更新。 按一下 [儲存]**** 以儲存剛更新的設定。
