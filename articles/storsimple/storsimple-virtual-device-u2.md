<properties 
   pageTitle="StorSimple virtuális eszköz – 2. frissítés | Microsoft Azure"
...
#### Virtuáliseszköz-modellek összehasonlítása
...
<sup>2</sup> *A 8010-es és a 8020-as modellek is Azure Standard szintű tárolást használnak a felhő szintjén. Csak az eszközön belül, a helyi rétegen van különbség*.

#### A 8020-as modellt támogató régiók
...
## Miben különbözik a virtuális eszköz a fizikai eszköztől?
...
#### Különbségek a fizikai eszközhöz képest
...
## A virtuális eszköz előfeltételei
...
- Az [Azure virtuális gépeket](../virtual-machines/virtual-machines-linux-about.md) (gazdakiszolgálókat) létrehozhatja abban a virtuális hálózatban, amely a virtuális eszközön elérhető köteteket használja. Ezeknek a kiszolgálóknak az alábbi követelményeknek kell megfelelniük:                            
...
## A virtuális eszköz létrehozása és konfigurálása
...
### 3. lépés: (opcionális) Az eszköz konfigurációs beállításainak módosítása
...
#### A CHAP-kezdeményező konfigurálása
...
#### StorSimple Snapshot Manager jelszavának konfigurálása
...
>[AZURE.NOTE] A virtuális eszköze számára a Windows-állomás egy Azure virtuális gép.
...
#### Az eszköz rendszergazdai jelszavának módosítása 
...
## Távoli csatlakozás a virtuális eszközhöz
...
### 1. lépés: A távfelügyelet konfigurálása
...
### 2. lépés: A virtuális eszköz távoli elérése
...
>[AZURE.WARNING] **A biztonság növelése érdekében a végpontokhoz való csatlakozáshoz nyomatékosan ajánlott a HTTPS használata, majd a távoli PowerShell-munkamenet befejezése után a végpontok törlése.**
...
## Közvetlen csatlakozás a virtuális eszközhöz
...
## A StorSimple virtuális eszköz használata
...
### A StorSimple virtuális eszköz karbantartása
...
### Tárfiókok létrehozása egy virtuális eszközhöz
...
### A StorSimple virtuális eszköz inaktiválása
...
### A virtuális eszköz indítása, leállítása és újraindítása
...
## Feladatátvétel a virtuális eszközre
...
## A virtuális eszköz leállítása vagy törlése
