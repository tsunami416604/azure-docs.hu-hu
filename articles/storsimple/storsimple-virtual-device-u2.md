...
>[AZURE.NOTE] 虛擬裝置，您的 Windows 主機的 Azure 虛擬機器。
...
>[AZURE.WARNING] **為了加強安全性，強烈建議您連接到端點時，使用 HTTPS，並在完成 PowerShell 遠端工作階段之後刪除端點。**
...
- **軟體更新** – 您可以檢視上次更新軟體的日期，以及任何更新狀態訊息。 您可以使用 **掃描更新** 執行手動掃描，如果您想要檢查更新] 頁面底部的按鈕。 如果有可用更新，請按一下 [ **安裝更新** 安裝。 因為虛擬裝置上只有單一介面，這表示在套用更新時將造成服務些微中斷。 虛擬裝置將關閉並重新啟動 (如有必要)，以套用任何已發行的更新。 如需逐步程序，請移至 [更新您的裝置](storsimple-update-device.md#install-regular-updates-via-the-management-portal)。
- **支援封裝** – 您可以建立並上傳支援封裝，以協助疑難排解您的虛擬裝置的問題的 Microsoft 支援服務。 如需逐步程序，請移至 [建立及管理支援封裝](storsimple-create-manage-support-package.md)。
...
當虛擬裝置會顯示為停用 StorSimple Manager 服務頁面上，您可以刪除虛擬裝置從裝置清單上 **裝置** 頁面。
...
當您檢視 [管理] 主控台時，虛擬裝置狀態是 **執行** 因為建立後，依預設已啟動。 您隨時都能啟動、停止及重新啟動虛擬機器。
...
如果您刪除或關閉虛擬裝置時，它會顯示為 **離線** StorSimple Manager 服務的裝置上。 如果您也想要刪除虛擬裝置所建立的備份，可以選擇停用或刪除裝置。 如需詳細資訊，請參閱 [停用裝置](storsimple-deactivate-and-delete-device.md#deactivate-a-device)。
