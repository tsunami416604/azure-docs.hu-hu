...
若要檢視特定的裝置，在 Azure 傳統入口網站中，選取 [StorSimple Manager 服務的監視資訊，請按一下 [ **監視** 索引標籤，然後再從裝置清單中選取。  **監視** 頁面包含下列資訊。

## I/O 效能 

**I/O 效能** 追蹤讀取次數的相關度量上和寫入作業之間的 iSCSI 啟動器介面主機伺服器、 裝置或裝置與雲端。 這項效能可以針對特定磁碟區、特定磁碟區容器，或所有磁碟區容器來測量。
...
**容量使用率** 追蹤資料由磁碟區、 磁碟區容器或裝置的儲存空間量的相關度量。 您可以根據您的主要儲存體、雲端儲存體，或裝置儲存空間的容量使用率來建立報告。 容量使用率可以在特定磁碟區、特定磁碟區容器，或所有磁碟區容器上測量。
...
- **快照集的所有磁碟區所包含的資料**︰ 顯示所有磁碟區的主要資料是每個磁碟區的主要資料和快照集資料的總和。 針對指定的磁碟區顯示的主要資料只對應至在該磁碟區上配置的資料數量 (而不包含對應的磁碟區快照資料)。
...
- **磁碟區監視停用所有磁碟區中包含**︰ 如果您的監視已關閉的裝置上有磁碟區，這些個別的磁碟區的監視資料將無法使用的圖表。 不過，圖表中所有磁碟區的資料將會包含已關閉監視的磁碟區。 
 
- **刪除磁碟區與即時相關聯的備份包含所有磁碟區**︰ 如果包含快照集資料的磁碟區都會刪除，但相關聯的快照集仍然存在，則您可能會看到不相符。

- **刪除磁碟區的所有磁碟區包含**︰ 在某些情況下，即使這些已刪除，可能存在舊磁碟區。 刪除的效果無法呈現，而裝置可能會顯示較低的可用容量。 您將必須連絡「Microsoft 支援服務」以移除這些磁碟區。
...
**網路輸送量** 追蹤傳輸的 iSCSI 啟動器網路介面從主機伺服器、 裝置和裝置與雲端之間的資料量的相關度量。 您可以針對裝置上的每一個 iSCSI 網路介面監視此計量。
...
**裝置效能** 追蹤相關裝置的效能度量。 下列圖表顯示在生產環境中裝置的 CPU 使用率統計資料。
