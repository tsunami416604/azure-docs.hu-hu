...
您可以使用節點在 StorSimple Snapshot Manager **範圍** 窗格，即可確認匯入的 StorSimple 裝置資料和重新整理已連接的存放裝置。 此外，當您按一下 **裝置** 節點，您可以看到已連接的裝置清單中對應的狀態資訊 **結果** 窗格。

![連接的裝置](./media/storsimple-snapshot-manager-manage-devices/HCS_SSM_connect_devices.png)

**圖 1：StorSimple Snapshot Manager 連接的裝置** 

取決於您 **檢視** 選取項目， **結果** ] 窗格會顯示每個裝置的下列資訊。 (如需有關如何設定檢視的詳細資訊，請移至 [檢視功能表](storsimple-use-snapshot-manager.md#view-menu)。
...
4. 在 **密碼** 文字] 方塊中，輸入您為 Azure 傳統入口網站中的裝置建立的 StorSimple Snapshot Manager 密碼。 按一下 [ **確定**。 StorSimple Snapshot Manager 會搜尋您所識別的裝置。 
...
1. 若要將裝置連接至 StorSimple Snapshot Manager，請遵循新增或更換裝置中的指示。 當它連接至裝置時，StorSimple Snapshot Manager 的回應如下：
   ...
   - 如果裝置可用，StorSimple Snapshot Manager 會新增連接。 當您選取裝置時，它會出現在 **結果** ] 窗格中，而且 [狀態] 欄位會指出裝置 **可用**。 StorSimple Snapshot Manager 會匯入任何針對裝置設定的磁碟區群組，前提是磁碟區群組具有相關聯的備份。 不會匯入備份原則。 不會匯入沒有相關聯之備份的磁碟區群組。

2. 按一下桌面圖示，以啟動 StorSimple Snapshot Manager。

3. 以滑鼠右鍵按一下最上層的節點中 **範圍** ] 窗格中，然後再按一下 **切換匯入顯示**。

    ![選取 [切換匯入顯示]](./media/storsimple-snapshot-manager-manage-devices/HCS_SSM_Toggle_Imports_Display.png) 

4.  **切換匯入顯示** ] 對話方塊隨即出現，顯示已匯入的磁碟區群組和備份的狀態。 按一下 [ **確定**。 
 ...
 **重新整理裝置** 動作會從連接的裝置擷取任何新的磁碟區群組和任何相關聯的備份。 不同於 **重新掃描磁碟區** 動作適用於 **磁碟區** 節點， **重新整理裝置** 不會還原備份登錄。
...
3. 在 **結果** ] 窗格中，以滑鼠右鍵按一下裝置名稱，然後按一下 [ **驗證**。
...
3. 在 **結果** ] 窗格中，以滑鼠右鍵按一下裝置名稱，然後按一下 [ **詳細資料**。 
...
3. 在 **結果** ] 窗格中，以滑鼠右鍵按一下裝置名稱，然後按一下 [ **重新整理裝置**。 這會同步處理裝置與 StorSimple Snapshot Manager。 
...
3. 在 **結果** ] 窗格中，以滑鼠右鍵按一下裝置名稱，然後按一下 [ **刪除**。 

4. 下列訊息隨即出現。 按一下 [ **是** 刪除的設定，或按一下 **否** 取消刪除。
...
2. 在 **範圍** ] 窗格中，按一下 [ **裝置**。 設定裝置的清單會出現在 **結果** 窗格。 
