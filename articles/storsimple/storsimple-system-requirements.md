...
- **儲存體用戶端軟體需求** -描述支援的作業系統和那些作業系統的其他需求。
- **StorSimple 裝置的網路需求** -提供需要您以允許 iSCSI、 雲端或管理流量的防火牆中開啟的連接埠的相關資訊。
...
| Linux | SUSE Linux 11 | |
 > [AZURE.NOTE] IBM AIX 目前不支援 storsimple。
...
在 StorSimple 內容中，如果多個網路介面和閘道器設定為通道流量，路由度量會派上用場，判斷使用介面的相對順序。 使用者無法變更路由度量。 不過您可以使用 `Get-HcsRoutingTable` Cmdlet 列印您的 StorSimple 裝置上的路由資料表 (和度量)。 更多有關 [Get-hcsroutingtable cmdlet](storsimple-troubleshoot-deployment.md#troubleshoot-with-the-get-hcsroutingtable-cmdlet)
...
- 在取代期間，監視 **硬體狀態** 中 **維護** Ssd 和 Hdd 中的磁碟機] 頁面。 綠色核取狀態表示磁碟狀況良好或確定，而紅色驚嘆號點則表示失敗的 SSD 或 HDD。
...
- EBOD 控制器模組 oreplacement，期間持續地監視 StorSimple Manager 服務中的元件來存取 **維護** - **硬體** 狀態。
