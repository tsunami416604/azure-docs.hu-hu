...
>[AZURE.NOTE] 
>
>-若要確保適當的操作，請確認介面速度和雙工，每個裝置介面所連線之交換器上。 交換器介面應該針對乙太網路 (1000 Mbps) 進行交涉或設定，且為全雙工。 介面以較低速度運作或為半雙工時將導致效能問題。
>
>-若要中斷與停機時間降至最低，我們建議您啟用 portfast 每個 iSCSI 網路介面，您的裝置將連線到交換器連接埠。 如此可確保網路連線可以在發生容錯移轉時快速建立。
...
- 了解如何 [StorSimple 裝置設定 MPIO](storsimple-configure-mpio-windows-server.md)。
