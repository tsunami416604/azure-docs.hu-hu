...
![在 ](./media/storsimple-snapshot-manager-manage-volumes/HCS_SSM_Volume_node.png)
...
2. 提供其中一個介面 IP 位址作為目標入口網站，或探索 IP 位址，然後連接至裝置。 在連接裝置之後，您的 Windows 系統將可存取磁碟區。 如需有關使用 Microsoft iSCSI 啟動器的詳細資訊，請移至一節 「 連接至 iSCSI 目標裝置 」 中 [安裝和設定 Microsoft iSCSI 啟動器 ][1]。
