...
> [AZURE.NOTE] 當部署裝置第一次，或可能發生之後，當裝置處於運作狀態時，會發生裝置組態相關問題，您可能會面臨到。 本文著重於第一次部署問題的疑難排解。 若要疑難排解運作的裝置，請移至 [可運作裝置問題進行疑難排解](storsimple-troubleshoot-operational-device.md)。
...
3. 設定密碼：下一個步驟是設定裝置系統管理員和 StorSimple Snapshot Manager 密碼。 如果您正在執行 Update 1，系統便不會要求您設定 StorSimple Snapshot Manager 的密碼。
  ...
  - 當您設定裝置來使用 StorSimple Snapshot Manager 時，需要有 StorSimple Snapshot Manager 密碼。 您需要先在安裝精靈中設定密碼，然後才能從 StorSimple Manager 服務設定和變更密碼。 此密碼可使用 StorSimple Snapshot Manager 驗證裝置。
    ...
    > [AZURE.IMPORTANT] 密碼是註冊之前收集，但只有在您成功註冊裝置之後，才會套用。 如果套用密碼失敗，系統將提示您再次提供密碼，直到收集到所需的密碼 (符合複雜性需求) 為止。
...
| 7 | Invoke-HcsSetupWizard：端點對應器中無更多可用的端點。 (發生例外狀況於 HRESULT：0x800706D9) | 叢集功能無法運作。 | [請連絡 Microsoft 支援](storsimple-contact-microsoft-support.md) 接下來的步驟。
...
> [AZURE.IMPORTANT] 一般而言，如果嘗試套用密碼失敗，則軟體會重複嘗試收集密碼，直到成功為止。 有時會無法套用密碼，但很罕見。 在此情況下，您可以註冊裝置並繼續進行，但密碼將不會變更。 您將不會收到哪一個密碼未變更的指示：裝置系統管理員密碼或 StorSimple Snapshot Manager 密碼。 如果發生這種情況，建議您變更這兩個密碼。
...
> [AZURE.NOTE] 在第一個範例中，裝置已使用不正確的 DNS。 在第二個範例中，DNS 是正確的。
...
> [AZURE.IMPORTANT] 您可以執行這個指令程式的主動和被動控制器。 
...
這個範例會從裝置中的狀態為 **離線** Azure 傳統入口網站中。
...
如果您執行 `Get-HcsRoutingTable` Cmdlet，但未指定任何參數 (如下列範例所示)，該 Cmdlet 會同時輸出 IPv4 和 IPv6 路由表。 或者，您可以指定 `Get-HcsRoutingTable -IPv4` 或 `Get-HcsRoutingTable -IPv6`  以取得相關的路由表。
...
1. 檢查裝置組態：在主動控制器上執行 `Invoke-HcsSetupWizard`。
     ...
     > [AZURE.NOTE] 安裝精靈必須在作用中控制器上執行。 若要確認您已連接到主動控制器，請查看序列主控台中顯示的橫幅。 該橫幅會指出您是連接到控制器 0 或控制器 1，而且該控制器是主動或被動。 如需詳細資訊，請移至 [識別您的裝置上的主動控制器](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device)。
...
2. 確定裝置纜線已正確連接：檢查裝置背板的網路纜線。 纜線是裝置型號專用的。 如需詳細資訊，請移至 [安裝 StorSimple 8100 裝置](storsimple-8100-hardware-installation.md) 或 [安裝 StorSimple 8600 裝置](storsimple-8600-hardware-installation.md)。
     ...
     > [AZURE.NOTE] 如果您使用 10 GbE 網路連接埠，您必須使用提供的 QSFP-SFP 配接器和 SFP 纜線。 如需詳細資訊，請參閱 [的纜線、 交換器及收發器 OEM 供應商建議針對 Mellanox 連接埠清單](http://www.mellanox.com/page/cables?mtag=cable_overview)。
...
5. 排除任何使用者錯誤：
  ...
  - 重新執行安裝精靈 (執行 **Invoke-hcssetupwizard**)，並輸入值，以確定沒有任何錯誤。 
  - 確認已使用註冊金鑰。 相同的註冊金鑰可用來將多個裝置連接到 StorSimple Manager 服務。 使用中的程序 [取得服務註冊金鑰](storsimple-manage-service.md#get-the-service-registration-key) 以確保使用正確的註冊金鑰。
    ...
    > [AZURE.IMPORTANT] 如果您有多個服務執行時，您必須確保適當的服務註冊金鑰用來註冊裝置。 如果您使用錯誤的 StorSimple Manager 服務註冊了裝置，您必須 [連絡 Microsoft 支援服務](storsimple-contact-microsoft-support.md) 接下來的步驟。 您可能必須執行裝置的原廠重設 (這可能導致資料遺失)，然後將它連接到所需的服務。

6. 使用 Test-Connection Cmdlet 來確認您擁有外部網路的連線能力。 如需詳細資訊，請移至 [測試連接 cmdlet 疑難排解](#troubleshoot-with-the-test-connection-cmdlet)。
