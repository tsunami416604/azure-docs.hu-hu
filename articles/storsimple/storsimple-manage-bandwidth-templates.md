...
使用 StorSimple Manager 服務 **設定** 頁面管理頻寬範本在 Azure 傳統入口網站。 與在此頁面上執行之頻寬範本相關的最常見工作如下：
...
1. 在 StorSimple Manager 服務 **設定** 頁面上，按一下 **新增/編輯頻寬範本**。

2. 在 **新增/編輯頻寬範本** ] 對話方塊中:

   1. 從 **範本** 下拉式清單中，選取 **新建** 若要加入新的頻寬範本。
   2. 為頻寬範本指定唯一的名稱。

3. 定義 **頻寬排程**。 建立排程：

   1. 從下拉式清單中，選擇排程要設定為該週的哪幾天。 您可以選取位於清單中個別天之前的核取方塊，以選取多天。
   2. 選取 **整天** 選項如果排程會強制執行全天。 核取此選項時，您無法再指定 **開始時間** 或 **結束時間**。 排程執行時間從 12:00 AM 到 11:59 PM。
   3. 從下拉式清單選取 **開始時間**。 這就是排程開始的時間。
   4. 從下拉式清單選取 **結束時間**。 這就是排程結束的時間。
   
         > [AZURE.NOTE] 不允許重疊的排程。 如果開始和結束時間會產生重疊的排程，您會看到一則錯誤訊息所造成影響。

   5. 指定 **頻寬速率**。 這是以 MB / 秒 (Mbps) 為單位的頻寬，由包含雲端之作業中的 StorSimple 裝置所使用。 提供一個介於 1 到 1000 之間的數目給此欄位。
   
   6. 按一下核取圖示 ![核取圖示](./media/storsimple-manage-bandwidth-templates/HCS_CheckIcon.png)。 您已建立的範本會加入至服務上的頻寬範本清單 **設定** 頁面。

    ![建立新的頻寬範本](./media/storsimple-manage-bandwidth-templates/HCS_CreateNewBT1.png)

4. 按一下 [ **儲存** 頁面，然後按一下底部 **是** 當系統提示您確認。 這樣會儲存您所做的組態變更。
...
3. 若要儲存您的變更，請按一下 [ **儲存** 頁面的底部。 按一下 [ **是** 當系統提示您確認。

> [AZURE.NOTE] 您不允許儲存變更，如果已編輯的排程重疊，您要修改的頻寬範本中現有的排程。
...
1. 在服務的頻寬範本表格式清單中，選取您想要刪除的範本。 刪除圖示 (**x**) 會出現在已選取範本的最右邊。 按一下 [ **x** 圖示，以刪除範本。

2. 系統將提示您進行確認。 按一下 [ **確定** 繼續進行。

如果有任何磁碟區正在使用範本，您就無法將它刪除。 您會看到錯誤訊息，指出此範本正在使用中。 錯誤訊息對話方塊會隨即出現，建議您移除範本的所有參考。

您可以刪除該範本的所有參考，藉由存取 **磁碟區容器** 頁面和修改，讓它們使用另一個範本，或使用自訂或無限制頻寬設定，請使用此範本的磁碟區容器。 移除所有參考時，您就可以刪除範本。
...
- **排程** – 從星期一到上星期五期間套用 1 mbps 8 AM 和 5 PM 的裝置時間之間的頻寬速率的單一排程。 一週中的其餘部分之頻寬設定為無限制。
...
**Q**. 當您在排程之間時，頻寬控制會發生什麼事？ (一個排程已結束而另一個排程尚未開始)。

**A**. 在這種情況下，不會採用任何頻寬控制。 這表示將資料分層至雲端時，裝置可以使用無限制的頻寬。

**Q**. 您是否可以修改離線裝置上的頻寬範本？

**A**. 如果對應的裝置已離線，您無法修改磁碟區容器上的頻寬範本。

**Q**. 您是否可以在相關聯的磁碟區離線時，編輯和磁碟區容器相關聯的頻寬範本？

**A**. 您可以修改和磁碟區已離線之磁碟區容器相關聯的頻寬範本。 請注意，當磁碟區離線時，沒有資料會從裝置分層至雲端。

**Q**. 您是否可以刪除預設範本？

**A**. 雖然您可以刪除預設範本，但這並不是個好主意。 已追蹤此預設範本 (包括已編輯版本) 的使用方式。 追蹤資料已進行分析，並在時間過程中用來改善預設範本。

**Q**. 如何判斷頻寬範本必須修改？

**A**. 您必須修改頻寬範本的其中一個徵兆是您會開始看到網路變慢，或在一天中多次停擺。 如果發生這種情況，請查看 I/O 效能及網路輸送量圖表來監視儲存體和使用方式的網路。
...
**Q**. 要是裝置上多個磁碟區容器的排程重疊，但是每個排程都套用不同的限制會發生什麼事？

**A**. 假設您的裝置有 3 個磁碟區容器。 與這些容器相關聯的排程完全重疊。 針對每個容器，分別使用 5、10 和 15 Mbps 的頻寬限制。 當所有容器都同時出現 I/O 時，可能會套用 3 個頻寬限制中的最小值：5 Mbps，因為在此情況下，這些傳出 I/O 要求會共用相同的佇列。
