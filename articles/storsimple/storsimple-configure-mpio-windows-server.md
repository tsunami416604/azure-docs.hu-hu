...
3. 在 [新增角色及功能]**** 精靈中，執行下列動作：
    ...
    - 在 [在您開始前]**** 頁面上，按 [下一步]****。
    - 在 [選取安裝類型]**** 頁面上，接受 [角色型或功能型安裝]**** 的預設值。 按一下 [ **下一步**。![新增角色及功能精靈 2](./media/storsimple-configure-mpio-windows-server/IC740999.png)
    ...
    - 完成安裝時，您將會收到通知。 按一下 [ **關閉** ，以關閉精靈。![新增角色及功能精靈 9](./media/storsimple-configure-mpio-windows-server/IC741002.png)
...
6. 在 [新增 MPIO 支援]**** 對話方塊中，於 [裝置硬體識別碼]**** 下輸入裝置序號。您可以藉由存取 StorSimple Manager 服務，然後瀏覽至 [裝置] > [儀表板]****，來取得裝置序號。 裝置序號會顯示在右邊 **快速概覽** 裝置儀表板的窗格。
...
3. 在 [搜尋目標入口]**** 對話方塊中，執行下列動作：
    ...
    - 輸入您的 StorSimple 裝置的 DATA 連接埠的 IP 位址 (例如，輸入 DATA 0)。
    - 按一下 [確定]**** 以返回 [iSCSI 啟動器內容]**** 對話方塊。
    >[AZURE.IMPORTANT] **如果您使用私人網路進行 iSCSI 連線，請輸入連線到私人網路的 DATA 連接埠 IP 位址。**

4. 您的裝置上重複步驟 2-3，第二個網路介面 (例如 DATA 1)。 請記住，您應該為 iSCSI 啟用這些介面。 若要深入了解，請參閱 [修改網路介面](storsimple-modify-device-config.md#modify-network-interfaces)。
5. 在 [iSCSI 啟動器內容]**** 對話方塊中，選取 [目標]**** 索引標籤。 您應該會看到 StorSimple 裝置目標 IQN 下的 **探索到的目標**。
...
>[AZURE.IMPORTANT] * * 我們建議您不要混合使用 1 GbE 與 10 GbE 網路介面。 如果您使用兩個網路介面，這兩個介面應屬於相同的 type.* *
