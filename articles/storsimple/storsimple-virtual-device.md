...
- 虛擬裝置，如 [Azure 上設定虛擬網路](virtual-networks-create-vnet.md)。
...
1.  在 Azure 傳統入口網站，請移至 **StorSimple Manager** 服務。

2. 移至 **裝置** 頁面。

3. 在 **建立虛擬裝置** 對話方塊方塊中，指定下列詳細資料。

    ![StorSimple 建立虛擬裝置](./media/storsimple-virtual-device/StorSimple_CreateVirtualDevice1.png)

    1. **名稱** – 虛擬裝置的唯一名稱。

    2. **虛擬網路** – 您想要與此虛擬裝置搭配使用的虛擬網路名稱。

    3. **子網路** – 虛擬裝置搭配使用的虛擬網路上的子網路。

    4. **虛擬裝置建立的儲存體帳戶** – 將用來佈建期間存放的虛擬裝置映像的儲存體帳戶。 此儲存體帳戶應該與虛擬裝置和虛擬網路位於相同的區域中。 不應讓實體裝置或虛擬裝置用它來儲存資料。 根據預設，將基於此用途建立新的儲存體帳戶。 不過，如果您知道已經擁有適合此用途的儲存體帳戶，則可從清單中選取該帳戶。

4. 按一下勾號，表示您了解虛擬裝置上儲存的資料將裝載於 Microsoft 資料中心。 系統隨即會建立虛擬裝置。 可能需要長達 45 分鐘到 1 小時來建立虛擬裝置。
...
開始此程序之前，請確定您擁有服務資料加密金鑰的複本。 服務資料加密金鑰已在您設定第一個 StorSimple 實體裝置時建立，且系統已指示您將它儲存在安全的位置。 如果您沒有服務資料加密金鑰的複本，您必須 [連絡 Microsoft 支援服務](storsimple-contact-microsoft-support.md) 以取得協助。

執行下列步驟來設定和註冊 StorSimple 虛擬裝置。


1. 選取 **StorSimple 虛擬裝置** 您剛建立 **裝置** 頁面。

2. 按一下 [ **完成裝置組態**。 這會啟動 [設定裝置精靈]。

    ![在 [裝置] 頁面完成 StorSimple 裝置安裝](./media/storsimple-virtual-device/StorSimple_CompleteDeviceSetupSVA1M.png)

1. 在 [設定裝置精靈] 中：

    1. 輸入 **服務資料加密金鑰** 中提供的空間。
    2. 輸入 **Snapshot Manager 密碼**。  **Snapshot Manager 密碼** 必須是 14 或 15 個字元的長度，而且必須包含小寫字母、 大寫字母、 數字和特殊字元的組合。
    3. 輸入 **裝置系統管理員密碼**。  **裝置系統管理員密碼** 必須介於 8 到 15 個字元，而且必須包含小寫字母、 大寫字母、 數字和特殊字元的組合。
    4. 按一下核取圖示來完成虛擬裝置的初始設定和註冊。

        ![StorSimple 虛擬裝置設定](./media/storsimple-virtual-device/StorSimple_VirtualDeviceSettings1.png)

設定和註冊完成之後，裝置將會上線。 裝置上線可能需要幾分鐘的時間。

![StorSimple 虛擬裝置上線階段](./media/storsimple-virtual-device/StorSimple_VirtualDeviceOnline1M.png)
...
>[AZURE.NOTE] 虛擬裝置，您的 Windows 主機的 Azure VM。

當您在 StorSimple Snapshot Manager 中設定裝置時，系統將提示您提供 StorSimple 裝置 IP 位址和密碼來驗證您的儲存裝置。 此密碼最初是透過 Windows PowerShell 介面來設定。

將 StorSimple Snapshot Manager 和您的 StorSimple 虛擬裝置搭配使用時，執行下列步驟來變更它。

1. 虛擬裝置，請移至 **裝置 > 設定**。

- 向下捲動至 **Snapshot Manager** 一節。 輸入 14 或 15 個字元的密碼。 請確定密碼包含大寫、小寫、數字和特殊字元的組合。
...
1. 向下捲動至 **裝置系統管理員密碼** 一節。 提供含有 8 到 15 個字元的系統管理員密碼。 密碼必須是大寫、小寫、數字和特殊字元的組合。
...
![可用的視訊](./media/storsimple-virtual-device/Video_icon.png) **可用的視訊**

若要觀賞的視訊，說明如何建立 StorSimple 虛擬裝置定域機組中，按一下 [ [這裡](http://azure.microsoft.com/documentation/videos/create-a-storsimple-virtual-device/)。
...
- **支援封裝** – 您可以建立並上傳支援封裝，以協助疑難排解您的虛擬裝置的問題的 Microsoft 支援服務。
...
>[AZURE.WARNING] 為了加強安全性，我們強烈建議您連接到端點時，使用 HTTPS，並在完成 PowerShell 遠端工作階段之後刪除端點。

您應該遵循的程序 [連接到您的 StorSimple 裝置的遠端](storsimple-remote-connect.md) 來設定您的虛擬裝置的遠端處理功能。
...
當您檢視 [管理] 主控台時，虛擬裝置狀態是 **執行** 因為建立後，依預設已啟動。 您隨時都能停止並重新啟動虛擬機器。
...
- 開啟 **裝置** 頁面，然後再按一下 **磁碟區容器** ] 索引標籤。

- 選取您要容錯移轉至虛擬裝置的磁碟區容器。 按一下磁碟區容器，以顯示容器內的磁碟區清單。 選取磁碟區，然後按一下 **離線** ，使磁碟區離線。 針對磁碟區容器中的所有磁碟區，重複執行這個程序。

- 針對您要容錯移轉至虛擬裝置的所有磁碟區容器，重複執行前一個步驟。

- 在 **裝置** 頁面上，選取您要容錯移轉，然後按一下 [裝置 **容錯移轉** 以開啟 [裝置容錯移轉精靈。

- 在 **選擇要容錯移轉的磁碟區容器**, ，選取您想要容錯移轉的磁碟區容器。 若要顯示於此清單中，磁碟區容器必須包含雲端快照並離線。 如果您預期看到的磁碟區容器並未出現，請取消精靈，並確認它是否離線。

- 在下一個頁面上，在 **選擇目標裝置的磁碟區** 中所選取的容器，請從可用裝置的下拉式清單中選取虛擬裝置。 清單中只會顯示具有可用容量的裝置。

- 檢閱所有容錯移轉設定在 **確認容錯移轉** 頁面。 如果它們都正確，請按一下核取圖示。

容錯移轉程序就會開始。 容錯移轉完成時，請移至 **裝置** 頁面，並選取 [容錯移轉程序用來做為目標的虛擬裝置。 移至 [磁碟區容器] 頁面。 此時應會顯示所有磁碟區容器以及舊裝置中的磁碟區。

>[AZURE.NOTE] 虛擬裝置上支援的儲存體數量為 30 TB。

![可用的視訊](./media/storsimple-virtual-device/Video_icon.png) **可用的視訊**

若要觀賞的視訊，說明如何還原容錯移轉至雲端中的虛擬裝置的實體裝置，按一下 [ [這裡](http://azure.microsoft.com/documentation/videos/storsimple-and-disaster-recovery/)。
...
如果您刪除或關閉虛擬裝置時，它會顯示為 **離線** StorSimple Manager 服務的裝置上。 如果您也想要刪除虛擬裝置所建立的備份，可選擇以裝置的形式將其停用或刪除。 如需詳細資訊，請參閱 [停用裝置](storsimple-deactivate-and-delete-device.md#deactivate-a-device)。
