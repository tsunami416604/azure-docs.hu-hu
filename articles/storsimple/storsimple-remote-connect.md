...
- [使用 Azure 傳統入口網站啟用透過 HTTP 的遠端管理](#use-the-azure-classic-portal-to-enable-remote-management-over-http)
...
2. 在提示中輸入: `啟用 HcsRemoteManagement – AllowHttp`
...
2. 輸入下列命令將 StorSimple 裝置的 IP 位址加入用戶端的信任的主機清單：
     ...
     `Set-item wsman: \ localhost\client\trustedhosts < device_ip >-串連-Force`

     取代 <*device_ip*> 與您的裝置的 IP 位址，例如:

     `Set-item wsman: \ localhost\client\trustedhosts 10.126.173.90-串連-Force`
...
7. 輸入以下命令在裝置上啟動 Windows PowerShell 工作階段：
     ...
     `輸入 pssession-認證 $cred ConfigurationName SSAdminConsole-ComputerName < device_ip >`
     >[AZURE.NOTE] 若要建立 StorSimple 虛擬裝置使用 Windows PowerShell 工作階段，附加 `-連接埠` 參數並指定遠端處理中設定 StorSimple 虛擬應用裝置的公用連接埠。
...
- [使用 Azure 傳統入口網站啟用透過 HTTPS 的遠端管理](#use-the-azure-classic-portal-to-enable-remote-management-over-https)
...
- [從遠端主機連接至裝置](#connect-to-the-device-from-the-remote-host)
...
4. 從輸出 `Get HcsSystem`, ，複製裝置的序號，並將它儲存以供稍後使用。
...
5. 複製憑證中從 **-----BEGIN CERTIFICATE-----** 到 **-----END CERTIFICATE-----** 的資訊到文字編輯器，如 [記事本]，然後將它儲存為.cer 檔案。 (在您準備主機時，要將這個檔案複製到您的遠端主機)。
    >[AZURE.NOTE] 若要產生新的憑證，請使用 `組 HcsRemoteManagementCert` 指令程式。
...
3. 建立新認證，做法是：
     ...
     `$cred = 新物件 pscredential @("< IP of target device > \SSAdmin 」 (convertto securestring-強制-asplaintext [< 裝置系統管理員密碼 >]))`

    其中 <*IP of target device*> 是您的裝置的 DATA 0 的 IP 位址，例如 **10.126.173.90** hosts 檔案的上述影像中所示。此外，提供您的裝置的管理員密碼。

4. 建立工作階段，做法是輸入：

     `$session = 新 pssession usessl-CN < 目標裝置序號 >-認證 $cred configurationname"SSAdminConsole"`

    此指令程式中的 CN 名稱，提供 <*serial number of target device*>。此序號已對應至您的遠端主機上主機檔案中 DATA 0 的 IP 位址，如下圖所示的 **SHX0991003G44MT**。
