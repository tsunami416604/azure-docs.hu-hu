...
>[AZURE.NOTE] 
>
>固定在本機的磁碟區將會複製為分層磁碟區。 如果您需要將複製的磁碟區固定在本機，可以在複製作業成功完成之後，再將複製品轉換為固定在本機的磁碟區。 如需將分層式磁碟區轉換成在本機固定磁碟區資訊，請移至 [變更磁碟區類型](storsimple-manage-volumes-u2.md#change-the-volume-type)。
>
...
4. 在複製磁碟區精靈] 中，在 **指定名稱和位置**:
  ...
  2. 為複製指定唯一的磁碟區名稱。 此名稱必須包含 3 到 127 個字元。 
        ...
        >[AZURE.NOTE]  **複製磁碟區做為** 欄位就是 **階層** 即使您正在複製本機固定磁碟區。 您無法變更此設定，但如果您需要在本機釘選複製的磁碟區，您可以在成功建立複製之後，將複製轉換到本機釘選的磁碟區。 如需將分層式磁碟區轉換成在本機固定磁碟區資訊，請移至 [變更磁碟區類型](storsimple-manage-volumes-u2.md#change-the-volume-type)。

        ![複製精靈 1](./media/storsimple-clone-volume-u2/clone1.png) 

  3. 按一下箭號圖示 ![arrow-icon](./media/storsimple-clone-volume-u2/HCS_ArrowIcon.png) 若要繼續進行下一個頁面。
...
5. 在 **指定可以使用此磁碟區的主機**:
  ...
  2. 按一下核取圖示 ![核取圖示](./media/storsimple-clone-volume-u2/HCS_CheckIcon.png)若要完成此作業。

6. 複製工作隨即起始並在成功建立複製時通知您。 按一下 [ **檢視作業** 上監視複製工作 **工作** 頁面。 複製作業完成時，您會看到下列訊息：
...
![可用的視訊](./media/storsimple-clone-volume-u2/Video_icon.png) **可用的視訊**
