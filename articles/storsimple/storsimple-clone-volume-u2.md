...
>
>' 無法修改磁碟區 {0} 的使用類型。 如果要修改的磁碟區是暫時性複製，尚未進行永久性，也可能會發生。 建立此磁碟區的雲端快照，然後再試一次修改作業。 ' 
>
...
4. 在 [複製磁碟區精靈] 的 [指定名稱和位置]**** 下：
  ...
  2. 為複製指定唯一的磁碟區名稱。 此名稱必須包含 3 到 127 個字元。
        >[AZURE.NOTE] 即使您複製了本機釘選磁碟區，**[將磁碟區複製為]** 欄位仍然會**分層**。 您無法變更此設定，但如果您需要在本機釘選複製的磁碟區，您可以在成功建立複製之後，將複製轉換到本機釘選的磁碟區。 如需將分層式磁碟區轉換成在本機固定磁碟區資訊，請移至 [變更磁碟區類型](storsimple-manage-volumes-u2.md#change-the-volume-type)。

        ![複製精靈 1](./media/storsimple-clone-volume-u2/clone1.png)

  3. 按一下箭頭圖示 ![箭號圖示](./media/storsimple-clone-volume-u2/HCS_ArrowIcon.png) 以前往下一個頁面。
...
5. 在 [指定可以使用此磁碟區的主機]**** 下：
  ...
  2. 按一下核取圖示 ![核取圖示](./media/storsimple-clone-volume-u2/HCS_CheckIcon.png)完成作業。
...
![段影片提供](./media/storsimple-clone-volume-u2/Video_icon.png) **段影片提供**
