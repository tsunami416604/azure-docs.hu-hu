...
前端面板，也就是 *操作面板* 或 *操作面板*, ，顯示系統中的所有模組的彙總狀態。 StorSimple 裝置與 EBOD 機箱上的前端面板完全相同，如下圖所示。  
...
裝置前端面板 Led 與 EBOD 機箱之間的主要差異是 **系統單元識別碼** LED 顯示器上顯示。 預設單位識別碼的裝置上顯示的是 **00**, ，而 EBOD 機箱上顯示的預設單元識別碼是 **01**。 這可讓您在裝置開啟時，快速區分裝置與 EBOD 機箱。 如果您的裝置已關閉，請使用所提供的資訊  [開啟新的裝置](storsimple-turn-device-on-or-off.md#turn-on-a-new-device) 來區分裝置與 EBOD 機箱。  
...
| LED | 說明                                                                            
|---- | ----------- |
| ID LED (藍色) | 指出已找到此模組。 如果主動控制器上閃爍著藍色 LED，即表示該控制器處於作用中，而另一控制器則處於待命中。 如需詳細資訊，請參閱 [識別您的裝置上的主動控制器](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device)。 |
