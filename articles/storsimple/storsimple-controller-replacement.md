...
>[AZURE.NOTE] 在執行控制器更換之前，我們建議您一律更新至最新版本的控制器韌體。
>
...
|5|一個或兩個控制器故障。 您無法透過序列主控台或 Windows PowerShell 遠端存取裝置。|[請連絡 Microsoft 支援](storsimple-contact-microsoft-support.md) 手動控制器更換程序。|
|6|控制器有不同的組建版本，可能是因為:<ul><li>控制器有不同的軟體版本。</li><li>控制器有不同的韌體版本。</li></ul>|如果控制器軟體版本不同，更換邏輯會偵測到，並更新更換控制器上的軟體版本。<br><br>如果控制器軔體版本不同，而且舊的韌體版本 **不** 自動升級，將警示訊息會出現在 Azure 傳統入口網站。 您應掃描更新並安裝韌體更新。</br></br>如果控制器軔體版本不同，而且舊的韌體版本可以自動升級，控制器更換邏輯會偵測到此情況，並且在控制器啟動之後，軔體將會自動更新。|
...
1. StorSimple Manager 服務在 Azure 傳統入口網站中，按一下 [ **裝置** 索引標籤，然後再按一下您想要監視的裝置名稱。

2. 按一下 [ **維護** 索引標籤，然後瀏覽至 **硬體狀態**。 控制器 0 或控制器 1 的狀態應該是紅色，表示故障。

    >[AZURE.NOTE] 單一控制器更換中故障的控制器一律是待命控制器。
...
8. 在控制器重新啟動之後，請檢查 **控制器狀態** 和 **叢集狀態** Azure 傳統入口網站，以確認控制器是狀況良好狀態，且處於待命模式。

>[AZURE.NOTE] 如果您正在監視的裝置透過序列主控台，您可能在控制器從更換程序中復原時看到多次重新啟動。 當序列主控台功能表呈現時，您便知道更換已完成。 如果功能表不會啟動控制器更換的兩個小時內出現，請 [連絡 Microsoft 支援服務](storsimple-contact-microsoft-support.md)。
...
1. 插槽 0 中的更換控制器會檢查下列情況：
   ...
   3. 對等控制器是否執行中並構成叢集？
    ...
    如果上述條件，則為 true，則控制器會尋找最新的每日備份 (位於 **nonDOMstorage** 磁碟機 S 上)。 控制器會從備份複製 VHD 的最新快照。
...
>[AZURE.IMPORTANT] 雙重控制器更換 StorSimple 裝置設定之後，很重要，進行手動備份裝置。 直到過了 24 小時之後，才會觸發每日裝置組態備份。 使用 [Microsoft 支援服務](storsimple-contact-microsoft-support.md) 進行手動備份您的裝置。
...
>[AZURE.IMPORTANT] 可能需要 45 分鐘到 1 小時控制器重新啟動，並從雙重控制器更換程序完全復原。 整個程序的時間總計 (包括接上纜線) 大約 2.5 小時。
...
6. 開啟 StorSimple 裝置。 如果您是使用 8600 機型：
    ...
    >[AZURE.NOTE] 如果您正在監視的裝置透過序列主控台，您可能在控制器從更換程序中復原時看到多次重新啟動。 當序列主控台功能表出現時，您便知道更換已完成。 如果功能表不會啟動控制器更換的 2.5 小時內出現，請 [連絡 Microsoft 支援服務](storsimple-contact-microsoft-support.md)。
...
4. 當閂鎖卡入定位時，即表示完成。  **確定** LED 現在應該上。  

    >[AZURE.NOTE] 可能需要 5 分鐘，控制器和 led 即會啟動。

5. 若要確認更換成功，請在 Azure 傳統入口網站，瀏覽至 **裝置** > **維護** > **硬體狀態**, ，並確定控制器 0 及控制器 1 的狀況良好 (狀態為綠色)。
...
在管理入口網站中，瀏覽至 **裝置** > **維護**, ，然後捲動至 **控制器** 一節。 在這裡您可以確認哪一個控制站作用中。

![識別管理入口網站中的作用中控制器](./media/storsimple-controller-replacement/IC752072.png)

**[圖 6** Azure 傳統入口網站顯示作用中控制器
