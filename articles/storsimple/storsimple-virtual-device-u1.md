...
3. 在 **建立虛擬裝置] 對話方塊中**, ，指定下列詳細資料。
    ...
    3. **虛擬網路** – 您想要與此虛擬裝置搭配使用的虛擬網路名稱。

    4. **子網路** – 虛擬裝置搭配使用的虛擬網路上的子網路。

    5. **虛擬裝置建立的儲存體帳戶** – 此儲存體帳戶將用來存放虛擬裝置映像期間佈建，並將佈建後裝載虛擬裝置的磁碟。 此儲存體帳戶應該與虛擬裝置和虛擬網路位於相同的區域中。 不應讓實體裝置或虛擬裝置用它來儲存資料。 根據預設，將基於此用途建立新的儲存體帳戶。 不過，如果您知道已經擁有適合此用途的儲存體帳戶，則可從清單中選取該帳戶。 
...
1. 選取 **StorSimple 虛擬裝置** 剛建立在 [裝置] 頁面。 
...
2. 向下捲動至 **Snapshot Manager** 一節。 輸入 14 或 15 個字元的密碼。 請確定密碼包含大寫、小寫、數字和特殊字元等 4 種項目中的 3 種。
...
2. 向下捲動至 **裝置系統管理員密碼** 一節。 提供含有 8 到 15 個字元的系統管理員密碼。 密碼必須包含大寫、小寫、數字和特殊字元等 4 種項目中的 3 種。
...
- **軟體更新** – 您可以檢視上次更新軟體的日期，以及任何更新狀態訊息。 您可以使用 **掃描更新** 執行手動掃描，如果您想要檢查更新] 頁面底部的按鈕。 如果有可用更新，請按一下 [ **安裝更新** 安裝。 因為虛擬裝置上只有單一介面，這表示在套用更新時將造成服務些微中斷。 虛擬裝置將關閉並重新啟動 (如有必要)，以套用任何已發行的更新。
...
>[AZURE.NOTE] 無法容錯移轉或複製到執行更新前 1 軟體的裝置執行 Update 1 的裝置。 如果您選取執行 Update 1 之前版本軟體的目標裝置，您將會收到必須在執行容錯移轉之前更新目標裝置的通知。
