...
| Update 2 至 Update 1 (1, 1.1, 1.2)| 是 <br></br>如果在本機使用固定或磁碟區或混合的兩個階層，磁碟區一律容錯移轉如層。| 是<br></br>如果使用本機固定磁碟區，這些容錯移轉如層。|
| Update 2 至 Update 2 (更新版本)| 是<br></br>使用在本機固定或階層的磁碟區或磁碟區，兩個混合一律會容錯移轉做為起始的磁碟區類型; 如果階層為階層式和在本機固定在本機固定的。| 是<br></br>如果使用本機固定磁碟區，這些容錯移轉如層。|
...
1. 對於新的裝置，請先完成最小量裝置組態設定。
    >[AZURE.IMPORTANT] * * 如果不先完成最小的組態，您的 DR 將會因目前實作中的錯誤而失敗。 將在稍後 release.* * 修正這個問題
...
![段影片提供](./media/storsimple-device-failover-disaster-recovery/Video_icon.png) **段影片提供**
...
- [刪除 StorSimple 裝置](storsimple-deactivate-and-delete-device.md#delete-a-device)
