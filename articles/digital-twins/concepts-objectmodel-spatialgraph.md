...
- **A szóközök** virtuális vagy fizikai `Tenant`helyek, például `Customer` `Region`,, és. `Venue`
...
- **A ontológiákat** kiterjesztett `Default`típusok (például `Building` `BACnet`,,, és `EnergyGrid`) halmazát jelölik.
