...
> [AZURE.NOTE] 若要安裝最新版的 NuGet，您必須先解除安裝已載入的版本，使用 Visual Studio 擴充功能管理員。 若要這麼做，您必須以系統管理員身分執行 Visual Studio。 否則，[解除安裝] 按鈕會停用。
...
下列 ASP.NET 處理常式建構一個 TwiML 回應，指出 **Hello World** 的呼叫。
...
如需有關在 Azure with ASP.NET 上使用 Twilio 的詳細資訊，請參閱 [如何在 Azure 上的 web 角色中使用 Twilio 撥打電話][howto_phonecall_dotnet]。
