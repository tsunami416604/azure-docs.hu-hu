...
1. 在 **DataObjects** 資料夾後端專案，加入新的實體呼叫 `Account`。
...
2. 在 **模型** 資料夾，您會看見 **DbContext** 衍生的類別名稱命名您的行動服務。 請開啟您的內容並加入下列程式碼，將帳戶資料表新增至您的資料模型：
...
4. 以下列程式碼取代類別定義：
    ...
    請務必取代 *todoContext* 變數與您的專案名稱 **DbContext**。 請注意，此控制器會使用以下屬性允許此端點的所有流量：

        [AuthorizeLevel(AuthorizationLevel.Anonymous)]

>[AZURE.IMPORTANT]任何透過 HTTP 的用戶端，就可以存取此註冊端點。 將這項服務發佈至實際執行環境之前，您應該實作某種配置來驗證註冊，例如以簡訊或電子郵件為基礎的驗證。 這有助於防止惡意使用者建立詐騙註冊。
...
3. 將抽象方法的下列實作 `ConfigureMiddleware` 至 **CustomLoginProvider**。
    ...
    此方法並未實作，因為 **CustomLoginProvider** 不會與驗證管線整合。
...
接著，請建立供使用者登入的端點。 系統會根據資料庫檢查您所收到的使用者名稱和密碼；方法是先套用使用者的 Salt、將密碼雜湊化，然後確定傳入的值符合資料庫的值。 如果有，則您可以建立 [ClaimsIdentity] 並傳遞至 **CustomLoginProvider**。 用戶端應用程式會接收使用者識別碼和驗證權杖，藉此進一步存取您的行動服務。
...
1. 在 Visual Studio 中，以滑鼠右鍵按一下行動服務專案，然後按一下 [ **偵錯** 和 **開始新執行個體**。

    這麼做會啟動行動服務後端專案的新偵錯執行個體。 服務成功啟動之後，您會看到顯示起始頁 **此行動服務已啟動並執行**。

2. 在服務啟動頁面上，按一下 [ **試試看**, ，然後輸入密碼，讓您設定 **MS_ApplicationKey** 應用程式設定使用者名稱空白的 web.config 檔案中 [驗證] 對話方塊。

3. 在 [說明] 頁面中，按一下 [ **CustomRegistration** 端點，然後按一下 [ **試試看**。

    ![][2]

4. 在本文中，使用者名稱和密碼，它們符合您先前指定的準則，以取代範例字串，然後按一下 [ **傳送**。

    ![][3]

    回應應為 **201/Created**。

5. 按一下瀏覽器的上一頁按鈕，然後重複步驟 2 和 3 **CustomLogin** 端點會使用相同的使用者名稱和您在上一個步驟中註冊的密碼。

    ![][4]

    您應該會收到回應訊息的本文會包含 **使用者** JSON 物件，同時具有 *userId* 和 *authenticationToken*, ，這是您的自訂驗證所產生的行動服務驗證權杖。 此權杖已足以授與用戶端應用程式存取 TodoItem 端點的權限。

    建立一份 *authenticationToken* 值。 您會用此值存取受限制的 TodoItem 端點。

6. 按一下瀏覽器的上一頁按鈕，然後在 API 文件] 頁面上，按一下 **GetTables**, ，按一下 [ **試試看**。

7. 在 GET 要求對話方塊中，按一下加號旁 **標頭**, ，輸入值 `X-ZUMO-AUTH` 左側方塊中，貼上複製 *authenticationToken* 值在右側的方塊，然後按一下 [ **傳送**。
    ...
    行動服務應該授與存取權，端點並傳回 **200/OK** 狀態，以及資料表中的 TodoItems 清單。
...
>[AZURE.IMPORTANT] 如果您選擇的測試，也將此行動服務專案發佈至 Azure，請記住您登入和驗證提供者很容易遭受攻擊。 請確認這些提供者已經過適當強化，或是受保護的測試資料對您而言並不重要。 使用自訂的驗證配置前請務必小心，才能保護生產環境的服務。

## 使用自訂驗證從用戶端登入

本節說明從用戶端存取自訂驗證端點，以取得存取行動服務所需的驗證權杖時，所需的步驟。 因為您需要的特定用戶端程式碼是取決於用戶端，所以此處提供的指引是針對未知的平台。

>[AZURE.NOTE] 透過 HTTPS 與服務通訊的行動服務用戶端程式庫。 由於此解決方案需要您以純文字傳送密碼，因此請務必在透過直接 REST 要求呼叫這些端點時，使用 HTTPS。

1. 在用戶端程式中建立所需的 UI 項目，藉此讓使用者輸入使用者名稱和密碼。

2. 使用適當 **invokeApi** 方法 **MobileServiceClient** 呼叫用戶端程式庫中 **CustomRegistration** 端點，在訊息本文中傳遞的執行階段提供的使用者名稱和密碼。

    您只需要呼叫 **CustomRegistration** 端點一次建立針對指定的使用者帳戶，只要您帳戶的資料表中保留使用者登入資訊。 如需如何在各種支援的用戶端平台上呼叫自訂 API 的範例，請參閱文件 [Azure 行動服務與用戶端 Sdk 中的自訂 API](http://blogs.msdn.com/b/carlosfigueira/archive/2013/06/19/custom-api-in-azure-mobile-services-client-sdks.aspx)。

    > [AZURE.IMPORTANT] 因為此使用者佈建步驟只發生一次，您應該考慮一些 out-of-band 的方式建立使用者帳戶。 若是公用註冊端點，您應該也要考慮實作以簡訊或電子郵件為基礎的驗證程序，或是其他保護措施，以避免產生詐騙帳戶。 您可以使用 Twilio 透過行動服務傳送手機簡訊。 您也可以使用 SendGrid 透過行動服務傳送電子郵件。 如需使用 SendGrid 的詳細資訊，請參閱 [使用 SendGrid 從行動服務傳送電子郵件](store-sendgrid-mobile-services-send-email-scripts.md)。

3. 使用適當 **invokeApi** 方法一次，但這次呼叫 **CustomLogin** 端點，在訊息本文中傳遞的執行階段提供的使用者名稱和密碼。

    此時，您必須擷取 *userId* 和 *authenticationToken* 回應物件中傳回成功的登入後的值。

4. 使用傳回 *userId* 和 *authenticationToken* 值，以建立新 **MobileServiceUser** 物件，並將它設為目前使用者以取得您 **MobileServiceClient** 執行個體，如本主題中所示 [將驗證新增至現有的應用程式](mobile-services-dotnet-backend-ios-get-started-users.md)。 因為 CustomLogin 的結果是一樣 **MobileServiceUser** 物件時，您應該能夠直接轉換結果。
