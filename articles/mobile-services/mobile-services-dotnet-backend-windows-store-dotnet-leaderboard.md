...
本教學課程將說明如何使用具有 .NET 後端的 Azure 行動服務建置 Windows 市集應用程式。 Azure 行動服務提供可擴充而安全的後端，具有內建的驗證、監控、推播通知和其他功能，以及用來建置行動應用程式的跨平台用戶端程式庫。 .NET 後的端行動服務根據 [ASP.NET Web API](http://asp.net/web-api), ，並提供.NET 開發人員建立 REST Api 的絕佳途徑。
...
下載完成的專案 [這裡](http://code.msdn.microsoft.com/Leaderboard-App-with-Azure-9acf63af)。
...
教學課程中並不需要這些項目，因此您可以從專案中加以刪除。 也在 WebApiConfig.cs 和 LeaderboardContext.cs 中移除對 TodoItem 的參考。
...
除了使用 LINQ Select 陳述式以外，另一個選項是使用 AutoMapper。 此選項需使用其他設定程式碼，但可啟用從網域實體到 DTO 的自動對應。 如需詳細資訊，請參閱 [資料庫類型與用戶端類型中使用 AutoMapper 的.NET 後端之間的對應](http://blogs.msdn.com/b/azuremobile/archive/2014/05/19/mapping-between-database-types-and-client-type-in-the-net-backend-using-automapper.aspx)。
...
在本節中，我將說明取用行動服務的 Windows 市集應用程式。 但在 XAML 或 UI 方面並不會有太多著墨。 我會將重點放在應用程式邏輯上。 您可以下載完整專案 [這裡](http://code.msdn.microsoft.com/Leaderboard-App-with-Azure-9acf63af)。
