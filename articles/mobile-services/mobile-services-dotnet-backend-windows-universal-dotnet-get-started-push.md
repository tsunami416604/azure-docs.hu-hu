...
&nbsp;&nbsp;8. 重複上述步驟，將推播通知新增至 Windows Phone 市集應用程式專案，然後從共用的 App.xaml.cs 檔案中移除額外的呼叫 **UploadChannel** ，而其餘 `#if...#endif` 條件式包裝函式。 這兩個專案現在都會共用單一呼叫至 **UploadChannel**。
...
>[AZURE.NOTE]切勿使用生產行動服務進行測試與開發作業。 請一律將您的行動服務專案發佈至個別的預備服務以利測試。

&nbsp;&nbsp;5. 開啟共用的 App.xaml.cs 專案檔案並找到任何建立的新執行個體的程式碼行 [MobileServiceClient] 類別來存取行動服務在 Azure 中執行。

&nbsp;&nbsp;6. 註解化此程式碼，將會建立新的程式碼加入 [MobileServiceClient] 的名稱相同，但使用建構函式，類似下列的本機主機 URL:
...
本教學課程示範如何啟用 Windows 市集應用程式以便使用行動服務與通知中心來傳送推播通知的基礎。 接下來，考慮完成下一個教學課程中， [Send push notifications to authenticated users], ，以了解如何使用標記從行動服務傳送推播通知給已驗證的使用者。
