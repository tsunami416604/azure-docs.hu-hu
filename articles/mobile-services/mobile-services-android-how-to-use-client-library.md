...
啟動篩選器與 [**其中**](http://go.microsoft.com/fwlink/p/?LinkId=296867) 對資料表參考的方法呼叫。 接著 [**欄位**](http://go.microsoft.com/fwlink/p/?LinkId=296869) 方法呼叫後面指定邏輯述詞的方法呼叫。 可能的數詞方法包括 [**eq**](http://go.microsoft.com/fwlink/p/?LinkId=298461), ，[**ne**](http://go.microsoft.com/fwlink/p/?LinkId=298462), ，[**gt**](http://go.microsoft.com/fwlink/p/?LinkId=298463), ，[**ge**](http://go.microsoft.com/fwlink/p/?LinkId=298464), ，[**lt**](http://go.microsoft.com/fwlink/p/?LinkId=298465), ，[**le**](http://go.microsoft.com/fwlink/p/?LinkId=298466) 等。

這已足以用來比較數字/字串欄位與特定值。 但您能做的絕對不僅止於此。

例如，您可以依日期進行篩選。 您可以比較整個日期欄位，但您也可以比較的日期部分的方法如 [**年**](http://go.microsoft.com/fwlink/p/?LinkId=298467), ，[**月**](http://go.microsoft.com/fwlink/p/?LinkId=298468), ，[**天**](http://go.microsoft.com/fwlink/p/?LinkId=298469), ，[**小時**](http://go.microsoft.com/fwlink/p/?LinkId=298470), ，[**分鐘**](http://go.microsoft.com/fwlink/p/?LinkId=298471) 和 [**第二個**](http://go.microsoft.com/fwlink/p/?LinkId=298472)。 下列節錄的程式碼新增的項目篩選器的 *到期日* 等於 2013年。

        mToDoTable.where().year("due").eq(2013).execute().get();

您可以使用類似的方法做各種複雜的篩選條件的字串欄位 [**startsWith**](http://go.microsoft.com/fwlink/p/?LinkId=298473), ，[**endsWith**](http://go.microsoft.com/fwlink/p/?LinkId=298474), ，[**concat**](http://go.microsoft.com/fwlink/p/?LinkId=298475), ，[**子字串**](http://go.microsoft.com/fwlink/p/?LinkId=298477), ，[**indexOf**](http://go.microsoft.com/fwlink/p/?LinkId=298488), ，[**取代**](http://go.microsoft.com/fwlink/p/?LinkId=298491), ，[**toLower**](http://go.microsoft.com/fwlink/p/?LinkId=298492), ，[**toUpper**](http://go.microsoft.com/fwlink/p/?LinkId=298493), ，[**修剪**](http://go.microsoft.com/fwlink/p/?LinkId=298495), ，和 [**長度**](http://go.microsoft.com/fwlink/p/?LinkId=298496)。 下列節錄的程式碼的資料表資料列 *文字* 資料行開頭為"PRI0"。

        mToDoTable.where().startsWith("text", "PRI0").execute().get();

數字欄位也允許各種更複雜的篩選具有類似的方法 [**新增**](http://go.microsoft.com/fwlink/p/?LinkId=298497), ，[**sub**](http://go.microsoft.com/fwlink/p/?LinkId=298499), ，[**mul**](http://go.microsoft.com/fwlink/p/?LinkId=298500), ，[**div**](http://go.microsoft.com/fwlink/p/?LinkId=298502), ，[**mod**](http://go.microsoft.com/fwlink/p/?LinkId=298503), ，[**floor**](http://go.microsoft.com/fwlink/p/?LinkId=298505), ，[**ceiling**](http://go.microsoft.com/fwlink/p/?LinkId=298506), ，和 [**四捨五入**](http://go.microsoft.com/fwlink/p/?LinkId=298507)。 下列節錄的程式碼的資料表資料列 *持續時間* 是偶數。

        mToDoTable.where().field("duration").mod(2).eq(0).execute().get();


您可以結合述詞與等方法 [**和**](http://go.microsoft.com/fwlink/p/?LinkId=298512), ，[**或**](http://go.microsoft.com/fwlink/p/?LinkId=298514) 和 [**不**](http://go.microsoft.com/fwlink/p/?LinkId=298515)。 下列節錄的程式碼會結合前述的兩個範例。
...
這項工作是您使用的查詢方法會傳回 [**MobileServiceQuery & l t;& Gt;**](http://go.microsoft.com/fwlink/p/?LinkId=298551) 物件，又可供其他方法加以叫用。 若要結束這一系列的方法，而實際執行查詢，請呼叫 [**執行**](http://go.microsoft.com/fwlink/p/?LinkId=298554) 方法。

以下是程式碼範例， *mToDoTable* 是行動服務的參考 *ToDoItem* 資料表。
...
第一次您具現化的執行個體 *ToDoItem* 類別，並設定其屬性。
...
下列程式碼將說明另一種執行方式。 此方式會指定要刪除之資料列的 ID 欄位值 (假設等於 "2FA404AB-E458-44CD-BC1B-3BC847EF0902")，以刪除 ToDoItem 資料表中的現有項目。 在實際的應用程式中，您會以某種方式取得識別碼，並將它傳入做為變數。 在這裡，為簡化測試，您可以移至您的服務在 Azure 傳統入口網站中，按一下 [ **資料** 並複製您想要測試的識別碼。
...
有時候您想要查閱特定項目依其 *識別碼*, ，不像查詢通常會產生符合某個條件的項目集合。 下列程式碼示範如何執行此作業， *識別碼* 值 `0380BAFB-BCFF-443C-B7D5-30199F730335`。 在實際的應用程式中，您會以某種方式取得識別碼，並將它傳入做為變數。 在這裡，為簡化測試，您可以移至您的服務在 Azure 傳統入口網站中，按一下 [ **資料** ] 索引標籤上，並複製您想要測試的識別碼。
...
一旦您建立的行動服務用戶端在執行個體 **onCreate** 方法 (在這裡， *mClient* 變數)，接下來您建立的執行個體 **MobileServiceJsonTable**, ，下列程式碼。
...
下一個步驟是插入物件。 回呼函式傳遞至 [**插入**](http://go.microsoft.com/fwlink/p/?LinkId=298535) 方法是執行個體 [**TableJsonOperationCallback**](http://go.microsoft.com/fwlink/p/?LinkId=298532) 類別。 請注意如何參數 *插入* 方法是 JsonObject。
...
在範例程式碼中，傳回資料的行動服務資料表從 *ToDoItem* 到陣列。 這是一個非常常見的資料應用程式模式：資料庫查詢通常會傳回資料列集合，讓用戶端在清單或陣列中取得。 在此範例中，陣列是資料來源。
...
上述程式碼中 *listitem* 屬性會指定清單中的個別資料列的配置 id。 以下提供會指定核取方塊及其相關文字的程式碼。 清單中的每個項目會分別使其具現化一次。 更複雜的配置將會指定顯示畫面中的其他欄位。 此程式碼位於 *row_list_to_do.xml* 檔案。
...
您必須將用戶端名稱序列化為 JSON 名稱符合的資料行名稱 *ToDoItem* 伺服器上的資料表。 下列使用 <a href=" http://go.microsoft.com/fwlink/p/?LinkId=290801" target="_blank">gson</a> 程式庫的程式碼會執行此動作。
