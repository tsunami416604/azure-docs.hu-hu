...
SendGrid 是 [cloud-based email service] ，能提供可靠 [transactional email delivery], ，擴充性和即時分析，以及有彈性的 Api 來輕鬆進行自訂整合。 如需詳細資訊，請參閱 <http://sendgrid.com>。
...
1. 登入 [Azure classic portal], ，按一下 [ **行動電話服務**, ，然後按一下您的行動服務。
...
5. 以正確的值取代上述指令碼中的預留位置：
    ...
    - **_使用者名稱_ 和 _密碼_**︰ 的 SendGrid 認證中識別您 [Create a SendGrid account]。
