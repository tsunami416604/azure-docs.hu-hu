...
1. 按一下您剛才的行動服務建立，然後在 [快速入門] 索引標籤中，按一下 [ **iOS** 下 **選擇平台** 展開 **建立新的 iOS 應用程式**。

2. 在 Windows PC 上，按一下 [ **下載** 下 **下載，並將服務發行至雲端**。 這會下載實作行動服務的 Visual Studio 專案。 請將壓縮的專案檔案儲存至本機電腦，並記下儲存位置。

3. 在 Mac 上，按一下 [ **下載** 下 **下載並執行您的應用程式**。 這會下載範例專案 _待辦事項清單_ 連線到您的行動服務，以及行動服務 iOS SDK 的應用程式。 請將壓縮的專案檔案儲存至本機電腦，並記下儲存位置。
