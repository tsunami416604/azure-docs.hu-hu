...
這些範例才能運作，資料表的權限必須設為 **具有應用程式金鑰的任何人**。 稍後您可以設定驗證。 請參閱 [開始使用驗證](mobile-services-html-get-started-users.md)。

##將項目加入資料表

將新的項目插入至資料表。 自動會建立 id (字串類型的 GUID) 作為新資料列的主要索引鍵。 呼叫 **完成** 方法傳回 [承諾](https://msdn.microsoft.com/library/dn802826.aspx) 物件，以取得所插入物件的複本，並處理任何錯誤。
...
如需您可以使用的查詢範例，請參閱 [查詢]((http://msdn.microsoft.com/library/azure/jj613353.aspx)) 物件。

##更新資料表項目

更新資料表中的資料列。 在此程式碼中，當行動服務回應時，項目就從清單中移除。 呼叫 **完成** 方法傳回 [承諾](https://msdn.microsoft.com/library/dn802826.aspx) 物件，以取得所插入物件的複本，並處理任何錯誤。
...
刪除資料列中資料的資料表使用 **del** 方法。 呼叫 **完成** 方法傳回 [承諾](https://msdn.microsoft.com/library/dn802826.aspx) 物件，以取得所插入物件的複本，並處理任何錯誤。
