...
For these examples to work, permissions on the table must be set to **Anybody with an Application Key**. Later, you can set up authentication. See [Get started with authentication][Get started with authentication].

##### Add entry

Insert a new item into a data table. An id (a GUID of type string) is automatically created as the primary key for the new row. Call the [done][done] method on the returned [Promise][done] object to get a copy of the inserted object and handle any errors.
...
For more examples of queries you can use, see [query][query] object.

##### Update entry

Update a row in a data table. In this code, when the mobile service responds, the item is removed from the list. Call the [done][done] method on the returned [Promise][done] object to get a copy of the inserted object and handle any errors.
...
Delete a row in a data table using the **del** method. Call the [done][done] method on the returned [Promise][done] object to get a copy of the inserted object and handle any errors.
...
[Learn more about mobile services][Learn more about mobile services]
  ...
  [Get started with authentication]: http://azure.microsoft.com/en-us/documentation/articles/mobile-services-html-get-started-users/
  [done]: 
