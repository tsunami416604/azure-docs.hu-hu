...
# <a name="heading"></a>在 Azure 上的 SQL Server 中的範例資料
...
>[AZURE.NOTE] 本文件中的 SQL 程式碼範例假設資料位於 Azure 上的 SQL Server 中。 如果沒有，請參閱 [Azure 上將資料移至 SQL Server](machine-learning-data-science-move-sql-server-virtual-machine.md) 主題，如需如何將資料移至 SQL Server，在 Azure 上的指示。
...
您可以使用下列程式碼範例，將向下取樣的資料儲存至檔案，並將它上傳至 Azure Blob。 Blob 中的資料可使用讀取器模組**直接讀取至 Azure ML 實驗。 步驟如下：
...
3. 使用 Azure ML 讀取器模組**讀取 Azure Blob 的資料，如下列螢幕擷取畫面所示：
