...
2. 選取 [專案資料行][project-columns] 模組，然後按一下 **啟動資料行選取器** 中 **屬性** 窗格。
    ...
    - 請確定 **所有資料行** 中篩選下拉式清單中，選取 **開始於**。 這會指示 [專案資料行][project-columns] 通過 （除了我們將要排除） 的所有資料行。
    ...
    > [AZURE.TIP] 您可以按兩下模組並輸入文字為模組新增註解。 這有助於您快速檢視模組在您實驗中的執行情況。 在此案例中，按兩下 [專案資料行][project-columns] 模組，然後輸入註解 「 排除自負虧損 」。

3. 拖放到 [清除遺漏資料][clean-missing-data] 模組到實驗畫布，並將其連接到 [專案資料行][project-columns] 模組。 在 **屬性** 窗格中，選取 **移除整個資料列** 下 **清除模式** 來清除資料，藉由移除含有遺漏值的資料列。 按兩下模組，並輸入註解「移除遺漏值資料列」。
...
我們的實驗到目前為止所完成的事項為清除資料。 如果您想要檢視已清除的資料集，按一下左側的輸出埠 [清除遺漏資料][clean-missing-data] 模組 （「 清理的資料集 」），然後選取 **視覺化**。 請注意， **自負虧損** 資料行已不再包含，而且沒有遺漏值。
...
我們將建置一個模型，並在其中使用我們資料集中的功能子集。 您可以重新選取不同功能、再次執行實驗，然後確認是否會有較理想的結果。 首先，我們將嘗試選取下列功能 （資料行） 與 [專案資料行][project-columns] 模組。 請注意，為了訓練模型，我們必須加入 *價格* 我們所要預測的值。

    make, body-style, wheel-base, engine-size, horsepower, peak-rpm, highway-mpg, price

1. 將另一個 [專案資料行][project-columns] 模組到實驗畫布，然後將它連接到左側的輸出埠 [清除遺漏資料][clean-missing-data] 模組。 按兩下模組，並輸入「選取要預測的功能」。

2. 按一下 [ **啟動資料行選取器** 中 **屬性** 窗格。

3. 在資料行選取器中，選取 **沒有資料行** 的 **開始於**, ，然後選取 **包含** 和 **資料行名稱** 篩選資料列中。 輸入資料行名稱的清單。 這會引導模組僅通過我們指定的資料行。

    > [AZURE.TIP] 因為我們已經執行過實驗，我們的資料的資料行定義已從原始資料集通過 [清除遺漏資料][clean-missing-data] 模組。 當您連接 [專案資料行][project-columns] 至 [清除遺漏資料][clean-missing-data], 、 [專案資料行][project-columns] 模組將可辨識資料中的資料行定義。 當您按一下 **資料行名稱** ] 方塊中，會顯示資料行的清單，以及您可以選取您想要新增至清單的資料行。
...
1. 我們可以將我們的資料分割成個別的訓練集和測試集，用來進行訓練和測試。 選取並拖曳 [分割資料][split] 模組到實驗畫布，然後將它連接到最後一個輸出 [專案資料行][project-columns] 模組。 設定 **中第一個輸出資料集的資料列的分數** 設為 0.75。 如此，我們將使用百分之 75 的資料來訓練模型，並保留百分之 25 做為測試之用。
