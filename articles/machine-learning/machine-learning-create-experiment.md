...
1. 型別 **投影資料行** 尋找模組調色盤頂端的 [搜尋] 方塊中 [專案資料行 ][project-columns] 模組，然後將其拖曳到實驗畫布並將它連接至的輸出連接埠 **汽車價格資料 (原始)** 資料集。 此模組可讓我們選取要將哪些資料行包含在模型中，或是從模型中排除。

2. 選取 [專案資料行 ][project-columns] 模組，然後按一下 **啟動資料行選取器** 中 **屬性** 窗格。

    - 確定已在 [**開始於**] 篩選下拉式清單中選取 [**所有資料行**]。 這會指示 [專案資料行 ][project-columns] 通過 (除了我們將要排除) 的所有資料行。
...
3. 拖放到 [清除遺漏的資料 ][clean-missing-data] 模組到實驗畫布，並將其連接到 [專案資料行 ][project-columns] 模組。 在 [**屬性**] 窗格中，在 [**清除模式**] 下，選取 [**移除整個資料列**]，藉由移除含有遺漏值的資料列來清除資料。 按兩下模組，並輸入註解「移除遺漏值資料列」。
...
我們的實驗到目前為止所完成的事項為清除資料。 如果您想要檢視已清除的資料集，按一下左側的輸出埠 [清除遺漏的資料 ][clean-missing-data] 模組 (「 清理的資料集 」)，然後選取 **視覺化**。 請注意，此時已不包含**自負虧損**資料行，而且也沒有遺漏值。
...
我們將建置一個模型，並在其中使用我們資料集中的功能子集。 您可以重新選取不同功能、再次執行實驗，然後確認是否會有較理想的結果。 首先，我們將嘗試選取下列功能 (資料行) 與 [專案資料行 ][project-columns] 模組。 請注意，為了訓練模型，我們必須加入所要預測的*價格*值。

    make, body-style, wheel-base, engine-size, horsepower, peak-rpm, highway-mpg, price

1. 將另一個 [專案資料行 ][project-columns] 模組到實驗畫布，然後將它連接到左側的輸出埠 [清除遺漏的資料 ][clean-missing-data] 模組。 按兩下模組，並輸入「選取要預測的功能」。

2. 按一下 [**屬性**] 窗格中的 [**啟動資料行選取器**]。

3. 在資料行選取器中，為 [**開始於**] 選取 [**無資料行**]，然後在篩選資料列中選取 [**包含**] 和 [**資料行名稱**]。 輸入資料行名稱的清單。 這會引導模組僅通過我們指定的資料行。
    > [AZURE.TIP] 因為我們已經執行過實驗，我們的資料的資料行定義已從原始資料集通過 [清除遺漏的資料 ][clean-missing-data] 模組。 當您連接 [專案資料行 ][project-columns] 至 [清除遺漏的資料 ][clean-missing-data], 、 [專案資料行 ][project-columns] 模組將可辨識資料中的資料行定義。 當您按一下 [**資料行名稱**] 方塊時，將會顯示資料行清單，而且您可以逐一選取要新增至清單的資料行。
...
1. 我們可以將我們的資料分割成個別的訓練集和測試集，用來進行訓練和測試。 選取並拖曳 [分割資料 ][split] 模組到實驗畫布，然後將它連接到最後一個輸出 [專案資料行 ][project-columns] 模組。 將 [Fraction of rows in the first output dataset]**** 設為 0.75。 如此，我們將使用百分之 75 的資料來訓練模型，並保留百分之 25 做為測試之用。
    > [AZURE.TIP] 變更 [**隨機種子**] 參數，可讓您為訓練和測試產生不同的隨機範例。 此參數會控制虛擬隨機數字產生器的植入。

2. 執行實驗。 這可讓 [專案資料行 ][project-columns] 和 [分割資料 ][split] 模組將資料行定義傳遞至我們接下來新增的模組。

3. 若要選取學習演算法，請在畫布左側的模組調色盤中展開 [**機器學習服務**] 類別，然後展開 [**初始化模型**]。 這會顯示數個可用來初始化機器學習演算法的模組類別。

    此實驗中，選取 [線性迴歸 ][linear-regression] 下 **迴歸** (您也可以在調色盤搜尋方塊中輸入 「 線性迴歸 」 以尋找模組) 的類別，並將它拖曳到實驗畫布。

4. 找出並拖曳 [訓練模型 ][train-model] 到實驗畫布的模組。 左側的輸入的埠連接到的輸出 [線性迴歸 ][linear-regression] 模組。 正確的輸入的埠連接到訓練資料輸出 (左側連接埠) 的 [分割資料 ][split] 模組。

5. 選取 [訓練模型 ][train-model] 模組中，按一下 [ **啟動資料行選取器** 中 **屬性** 窗格，然後再選取 **價格** 資料行。 這是我們的模型所將預測的值。
...
1. 找出並拖曳 [評分模型 ][score-model] 模組到實驗畫布，然後左側的輸入的埠連接到的輸出 [訓練模型 ][train-model] 模組。 右側輸入的埠連接到測試資料輸出 (右側連接埠) 的 [分割資料 ][split] 模組。

    ![Score Model module][screen8a]

2. 若要執行實驗，並檢視從輸出 [評分模型 ][score-model] 模組中，按一下輸出連接埠，然後選取 **視覺化**。 輸出會顯示價格的預測值，以及來自測試資料的已知值。

3. 最後，若要測試結果的品質，選取並拖曳 [評估模型 ][evaluate-model] 模組到實驗畫布，然後將左側輸入的埠連接到的輸出 [評分模型 ][score-model] 模組。 (有兩個輸入連接埠因為 [評估模型 ][evaluate-model] 模組可以用來比較兩個模型。)

4. 執行實驗。

若要檢視的輸出 [評估模型 ][evaluate-model] 模組中，按一下輸出連接埠，然後選取 **視覺化**。 我們的模型會顯示下列統計資料：
...
現在您已經完成第一個機器學習教學課程，並已設定實驗，您就能逐一檢測以嘗試改善模型。 例如，您可以變更您在預測中使用的功能。 您可以修改的屬性或 [線性迴歸 ][linear-regression] 演算法或嘗試完全不同的演算法。 您甚至可以加入多個機器學習實驗的演算法，一次並比較兩個使用 [評估模型 ][evaluate-model] 模組。
> [AZURE.TIP] 使用實驗畫布下方的 [**另存新檔**] 按鈕，可複製實驗的任何檢測。 您可以按一下畫布下方的 [**檢視執行歷程記錄**]，以檢視實驗的所有檢測。 請參閱 [管理 Azure Machine Learning Studio ][runhistory] 如需詳細資訊。

如果您對模型感到滿意，您可以將其部署為 Web 服務，用以透過新資料預測汽車價格。 請參閱 [部署 Azure Machine Learning web 服務 ][publish] 如需詳細資訊。

建立、 訓練、 評分和部署模型的預測性模型化技術的更廣泛且詳細逐步解說，請參閱 [開發預測方案使用 Azure Machine Learning ][walkthrough]。
