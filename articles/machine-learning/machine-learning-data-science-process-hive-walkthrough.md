...
>[AZURE.NOTE] **執行本逐步解說中的所有 Hive 命令，從上述的 Hive bin / 目錄提示字元。如此可自動處理路徑相關問題。我們使用詞彙 「 Hive 目錄提示 」、 「 Hive bin / 目錄提示字元 」，和本逐步解說中交替的 「 Hadoop 命令列 」。**
...
NYC 計程車資料集會依月份自然分割資料，可讓我們更快速處理和查詢時間。 下列 PowerShell 命令 (從 Hive 目錄使用發出 **Hadoop 命令列**) 將資料載入至依月份分割的"trip"和"fare"Hive 資料表。
...
此範例會識別在特定期間內超過 100 趟車程的圓形徽章 (計程車數目)。 因為資料分割變數條件，資料分割的資料表存取受益查詢 **月**。 查詢結果會寫入前端節點上 `C:\temp` 中的本機檔案 queryoutput.tsv。
 ...
 *-S* 包含在此命令的引數會隱藏 Hive Map/Reduce 工作的狀態畫面顯示。 這非常有用，因為它讓 Hive 查詢輸出的螢幕顯示更容易閱讀。
