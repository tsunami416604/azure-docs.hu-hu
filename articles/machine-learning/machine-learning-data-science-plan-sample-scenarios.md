...
1.  登入 [Azure Machine Learning Studio](https://studio.azureml.net/)。
...
3.  前置處理和清除 IPython Notebook，從 Azure 存取的資料中的資料
    儲存體容器。
...
3.  (選擇性) 前置處理和清除資料。
    ...
    a.  前置處理和清除 IPython Notebook，從 Azure 存取的資料中的資料
        blob。
...
4.  將資料載入執行於 Azure VM 的 SQL Server 資料庫。
 ...
 > [AZURE.NOTE] 如需加快大型資料的載入速度，建議來建立資料分割的資料表並大量平行資料匯入。 如需詳細資訊，請參閱 [平行資料匯入至 SQL 分割資料表](machine-learning/machine-learning-data-science-parallel-load-sql-partitioned-tables.md)。
...
## <a name="largedbtodb"></a>案例 \#6: SQL Server 資料庫內部，目標 Azure 虛擬機器中的 SQL Server 中的大型資料集
...
> [AZURE.NOTE] 加快載入的大型資料大小，建立資料分割的資料表並大量匯入的資料，以平行方式。 如需詳細資訊，請參閱 [平行資料匯入至 SQL 分割資料表](machine-learning/machine-learning-data-science-parallel-load-sql-partitioned-tables.md)。
...
### 若要從內部部署 SQL Server 的完整資料庫複製到 Azure SQL Database 的替代方法
...
3. 將複製檔案連結至目標 SQL Server 執行個體。 如需詳細資訊，請參閱 [附加資料庫](https://technet.microsoft.com/library/ms190209(v=sql.110).aspx)。
...
5.  將資料載入 Azure HDInsight Hadoop 叢集中的 Hive 資料庫。
    ...
    > [AZURE.NOTE] 如果資料太大，使用者可以建立 Hive 資料表具有資料分割。 接著，使用者可以在前端節點上的 Hadoop 命令列中使用 `for` 迴圈，以將資料載入使用資料分割的 Hive 分割資料表。
