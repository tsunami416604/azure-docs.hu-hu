...
Másik lehetőségként megadhat egyéni Docker is. Legegyszerűbben a Docker ```FROM``` parancs használatával indíthatja el Azure Machine Learning alaplemezképek egyikét, majd hozzáadhatja saját egyéni lépéseit. Akkor használja ezt a módszert, ha a nem Python-csomagokat függőségként kell telepíteni.
