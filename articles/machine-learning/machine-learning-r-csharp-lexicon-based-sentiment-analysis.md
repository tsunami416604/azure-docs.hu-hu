...
我們 [服務](https://datamarket.azure.com/dataset/aml_labs/lexicon_based_sentiment_analysis) MPQA 主觀性詞典 (http://mpqa.cs.pitt.edu/lexicons/subj_lexicon/)，這是其中一個最常用的主觀性詞典為基礎。 MPQA 中有 5,097 個否定字和 2,533 個肯定字。 而且所有這些字都會加上強極性或弱極性的註解。 整個主體會位於 [GNU 通用公共授權] 下方。 Web 服務可以套用至任何簡短的句子，例如推文和 Facebook 文章。
...
有幾種以自動化方式取用服務 (範例應用程式 [這裡](http://microsoftazuremachinelearning.azurewebsites.net/))。
...
#### 模組 1：
   ...
   # 安裝雜湊封裝
    ...
    成功 <-library ("hash"，lib.loc ="。"，logical.return = TRUE，verbose = TRUE)
    library
    (stringr)
