...
這 [web 服務]( https://datamarket.azure.com/dataset/aml_labs/prop_test) 會進行兩個比例根據使用者輸入的成功次數和總試驗次數的 2 個比較群組的差異假設測試。 一個可能的案例是從電影比較應用程式內部呼叫這項 Web 服務，根據電影分級讓使用者知道其中一部電影是否比另一部電影受到更多人的喜愛。
...
有幾種以自動化方式取用服務 (範例應用程式 [這裡](http://microsoftazuremachinelearning.azurewebsites.net/DifferenceInProportionsTest.aspx ))。
