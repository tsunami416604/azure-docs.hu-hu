...
| Használat vagy Futtatás      | Python SDK-val és az Azure CLI-vel. Aktiválja a Conda-környezetet `AzureML` a Windows-kiadásban, *vagy* a `py36` Linux rendszeren.      |
| Mintákra mutató hivatkozás      | A minta Jupyter jegyzetfüzetek a `AzureML` könyvtárban találhatók a jegyzetfüzetek alatt.  |
...
| Hogyan van konfigurálva vagy telepítve?      | A H2O `/dsvm/tools/h2o`-ra van telepítve.      |
| Használat vagy Futtatás      | Kapcsolódjon a virtuális géphez a X2Go használatával. Indítson el egy új terminált, és futtassa a `java -jar /dsvm/tools/h2o/current/h2o.jar` parancsot. Ezután indítson el egy webböngészőt, és kapcsolódjon `http://localhost:54321`-hoz.      |
| Mintákra mutató hivatkozás      | A minták a Jupyter virtuális gépen érhetők el a `h2o` könyvtárban.      |
| Kapcsolódó eszközök      | Apache Spark, MXNet, XGBoost, szénsavas víz, mélyvízi    |

A Dsvm számos más gépi tanulási kódtár található, például a népszerű `scikit-learn` csomag, amely a Dsvm-hez készült anaconda Python-terjesztés részét képezi. A Pythonban, az R-ben és a Julia-ben elérhető csomagok listájának megkereséséhez futtassa a megfelelő csomagkezelő-kezelőket.
...
| Használat vagy Futtatás      | Felhasználói felületi eszközként. Windows rendszeren indítson el egy parancssort, futtassa az R-t, majd az R-ben futtassa a `rattle()` parancsot. Linux rendszeren kapcsolódjon a X2Go, indítson el egy terminált, futtassa az R-t, majd az R-ben futtassa `rattle()` parancsot. |
...
| Használat vagy Futtatás      | Path parancssori eszközként (`C:\Program Files\VowpalWabbit\vw.exe` a Windowson, `/usr/bin/vw` Linux rendszeren)    |
...
| Jellemző felhasználások      | Általános gépi tanulási könyvtár      |
| Hogyan van konfigurálva vagy telepítve?      |  GPU-támogatással telepítve   |
| Használat vagy Futtatás      | Python-függvénytárként (2,7 és 3,5), R Package és on-Path parancssori eszköz (`C:\dsvm\tools\xgboost\bin\xgboost.exe` for Windows és `/dsvm/tools/xgboost/xgboost` Linux rendszeren)    |
| Mintákra mutató hivatkozások      | A minták a virtuális gépen, `/dsvm/tools/xgboost/demo` Linuxon, és a Windows `C:\dsvm\tools\xgboost\demo`.   |
