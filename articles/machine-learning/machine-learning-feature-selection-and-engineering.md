...
* 文字採礦分類範例中使用 [特徵雜湊 ][feature-hashing]
...
Azure Machine Learning Studio 中有針對特性選取而提供的模組。 下圖所示，這些模組包含 [篩選為基礎特徵選取 ][filter-based-feature-selection] 和 [費雪線性判別分析 ][fisher-linear-discriminant-analysis]。

![特性選取範例](./media/machine-learning-feature-selection-and-engineering/feature-Selection.png)


例如，考慮使用 [篩選為基礎特徵選取 ][filter-based-feature-selection] 模組。 為了方便起見，我們繼續使用上述的文字採礦範例。 假設我們想要建立迴歸模型，透過建立一組 256 個特性之後 [特徵雜湊 ][feature-hashing] 模組，並將回應變數為"Col1"代表書籍檢閱範圍從 1 到 5 的分級。 將 [特性評分方法] 設定為 [皮耳森相關]，則 [目標欄] 會是 "Col1"，而 [所需的特性數] 會是 50。 然後此模組 [篩選為基礎特徵選取 ][filter-based-feature-selection] 會產生包含 50 個特性且目標屬性"Col1"的資料集。 下圖顯示此實驗的流程以及我們剛才描述的輸入參數。
...
藉由套用這 [篩選為基礎特徵選取 ][filter-based-feature-selection] 模組，50 超出 256，因為它們具有目標變數"Col1"，最相關的特性選取功能會根據 「 皮耳森相關 」 評分方法。
