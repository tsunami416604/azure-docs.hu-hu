...
在 Azure 機器學習實驗中，這四個定型資料集是透過預先處理的輸入資料集中的分支形成。 除了左邊大部分分支，每個分支都包含一個 [執行 R 指令碼] [執行-r-指令碼] 模組，請在這一組衍生的特性 (特性集 B、 C 和 D) 會分別建構，並附加至匯入的資料集。 下圖示範左邊第二個分支中用來建立特性集 B 的 R 指令碼。
...
Azure 機器學習中沒有 [特徵雜湊] [特徵雜湊] 模組，建立這些文字/片語功能很方便。 下圖顯示使用此模組的範例。 輸入資料集包含兩個資料行：1 至 5 的書籍評比，以及實際評論內容。 此 [特徵雜湊] [特徵雜湊] 模組的目標是擷取一堆新功能，以顯示發生的對應文字 / 片語特定活頁簿內檢閱。 若要使用此模組，我們必須完成下列步驟：
