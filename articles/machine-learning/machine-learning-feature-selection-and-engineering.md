...
從收集的原始資料選取或擷取特性，通常可以增強機器學習中使用的定型資料。 工程設計特性的了解如何分類手寫字元的映像內容中的範例是有點密度對應建構從原始的位元發佈資料。 相較於原始分配，此對應有助於更有效地找出字元的界限。
...
工程設計和選取項目是 **資料增強** 機器學習的步驟。 此程序的三個層面主要有四個目的：

* **資料前置處理**︰ 此程序嘗試確保收集的資料乾淨而一致。 其中包含下列工作：例如整合多個資料集、處理不一致的資料，以及轉換資料類型。
* **特性工程設計**︰ 此程序嘗試從資料中的現有原始特性建立其他相關特性，以及增加學習演算法的預測功效。
* **特徵選取**︰ 此程序選取索引鍵的原始資料特性子集，以縮小定型問題的維度。

本主題僅涵蓋資料增強程序的特性工程設計和特性選取層面。 如需有關資料前置處理步驟的詳細資訊，請參閱 [前置處理資料，在 Azure ML Studio](http://azure.microsoft.com/documentation/videos/preprocessing-data-in-azure-ml-studio/) 視訊。
...
* 文字採礦分類範例使用 [特徵雜湊][feature-hashing]
...
在 Azure 機器學習實驗中，這四個定型資料集是透過預先處理的輸入資料集中的分支形成。 除了左邊大部分分支以外，每個分支包含 [執行 R 指令碼][execute-r-script] 模組，在其中一組衍生特性 （特性集 B、 C 和 D） 會分別建構並附加至匯入的資料集。 下圖示範左邊第二個分支中用來建立特性集 B 的 R 指令碼。
...
在 Azure Machine Learning 中沒有 [特徵雜湊][feature-hashing] 模組，建立這些文字/片語功能很方便。 下圖顯示使用此模組的範例。 輸入資料集包含兩個資料行：1 至 5 的書籍評比，以及實際評論內容。 這個目標 [特徵雜湊][feature-hashing] 模組是擷取一堆新功能，以顯示發生的對應文字 / 片語特定活頁簿內檢閱。 若要使用此模組，我們必須完成下列步驟：
...
Azure Machine Learning Studio 中有針對特性選取而提供的模組。 下圖所示，這些模組包含 [篩選為基礎的特徵選取][filter-based-feature-selection] 和 [費雪線性判別分析][fisher-linear-discriminant-analysis]。

![特性選取範例](./media/machine-learning-feature-selection-and-engineering/feature-Selection.png)


例如，考慮使用 [篩選為基礎的特徵選取][filter-based-feature-selection] 模組。 為了方便起見，我們繼續使用上述的文字採礦範例。 假設我們想要建立迴歸模型，透過建立一組 256 個特性之後 [特徵雜湊][feature-hashing] 模組，並將回應變數為"Col1"代表書籍檢閱範圍從 1 到 5 的分級。 將 [特性評分方法] 設定為 [皮耳森相關]，則 [目標欄] 會是 "Col1"，而 [所需的特性數] 會是 50。 然後此模組 [篩選為基礎的特徵選取][filter-based-feature-selection] 會產生包含 50 個特性且目標屬性"Col1"的資料集。 下圖顯示此實驗的流程以及我們剛才描述的輸入參數。
...
藉由套用這 [篩選為基礎的特徵選取][filter-based-feature-selection] 模組，50 超出 256，因為它們具有目標變數"Col1"，最相關的特性選取功能會根據 「 皮耳森相關 」 評分方法。
