...
這是逐步解說的第五個步驟 [開發預測方案使用 Azure ML](machine-learning-walkthrough-develop-predictive-solution.md)
...
- 將我們所建立的「訓練實驗」**轉換成「預測實驗」**
- 將預測實驗部署為 Web 服務

但首先，我們需要更精簡這項實驗。 我們的實驗目前有兩個不同的模型，但現在需要選取一個要部署的模型。

假設我們已判斷出推進式樹狀模型是較適合使用的模型。 因此首先来移除 [二級支援向量機器 ][two-class-support-vector-machine] 模組，訓練它所用的模組。 您可能需要按一下實驗畫布底部的 [**另存新檔**]，先建立一份實驗複本。

我們必須刪除下列模組：

1.  [二級支援向量機器 ][two-class-support-vector-machine]
2.  [訓練模型 ][train-model] 和 [評分模型 ][score-model] 連接到它的模組
...
- 我們所定型的模型會另存為實驗畫布左側模組調色盤中的**定型模型**模組 (您可以在調色盤的 [定型模型]**** 下找到這個模組)。
- 用於定型的模組會遭到移除。 具體而言：
  - [雙類別促進式決策樹 ][two-class-boosted-decision-tree]
  - [訓練模型 ][train-model]
  - [分割 ][split]
...
> [AZURE.NOTE] 實驗已分為兩個部分儲存：原始訓練實驗和新的預測實驗。 您可以使用實驗畫布頂端的索引標籤，存取其中一項實驗。

我們還必須對實驗執行額外的步驟。
Machine Learning Studio 移除其中一個 [執行 R 指令碼 ][execute-r-script] 模組時，會移除 [分割 ][split] 模組，但留下另 [執行 R 指令碼 ][execute-r-script] 模組。
由於該模組只用於定型和測試 (它將提供的範例資料的加權函數)，我們現在可以將它移除，連線 [中繼資料編輯器 ][metadata-editor] 至 [評分模型 ][score-model]。
...
最後一次執行實驗 (按一下 [執行]****)。 如果您想要驗證模型仍然有效，按一下 [輸出 [評分模型 ][score-model] 模組，然後選取 **檢視結果**。 您會看到原始資料出現，也會看到信用風險值 ("Scored Labels") 和評分機率值 ("Scored Probabilities").
