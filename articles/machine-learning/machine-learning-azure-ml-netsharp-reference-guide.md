...
-   在 _ByRow_ 的述詞中 **s** 參數代表輸入層 _Pixels_ 之節點矩形陣列的索引，而 **d** 參數代表隱藏層 _ByRow_ 之節點陣列的索引。**s** 和 **d** 的類型為長度二之整數的 Tuple。就概念而言， **s** 範圍介於所有整數，且 _0 < = s [0] < 10_ 和 _0 < = s [1] < 20_，和 **d** 都範圍介於所有整數、 _0 < = d [0] < 10_ 和 _0 < = d [1] < 12_。
...
-   **Padding**：(選用) 決定是否應使用**預設的填補配置**填補輸入。 其值可以是單一布林值，或是長度為套組 Arity 之布林值的 Tuple。 單一布林值可擴充為具有正確長度、所有元件皆等於指定值的 Tuple。 如果維度的值為 True，則來源在邏輯上會以零值儲存格在該維度中進行填補，以支援其他核心應用程式，使該維度中第一個和最後一個核心的中心節點成為該維度在來源層中的第一個和最後一個節點。 因此，每個維度中的 「 虛擬 」 節點數目會自動決定，符合確切 _ (InputShape [d]-1) / Stride [d] + 1_ 個核心，到已填補的來源層。 如果維度的值為 False，則在定義核心時，會使每一端排除的節點數目保持相同 (差距不會超過 1 個)。 此屬性的預設值是所有元件皆為 False 的 Tuple。
-   **UpperPad** 和 **LowerPad**：(選用) 更能控制所要使用的填補量。 **重要事項:** 這些屬性可以定義，才 **填補** 上述的 *** 不 *** 定義。 其值應為長度為套組 Arity 的整數值 Tuple。 在指定這些屬性時，將會在輸入層各個維度的下端和上端新增「虛擬」節點。 在每個維度中的下端和上端新增的節點數目，分別取決於 **LowerPad**[i] 和 **UpperPad**[i]。 若要確定核心只會對應至「實際」節點而非「虛擬」節點，必須要符合下列條件：
    -   **LowerPad** 的每個元件皆務必小於 KernelShape[d]/2。
    -   **UpperPad** 的每個元件皆不可大於 KernelShape[d]/2。
    -   這些屬性的預設值是所有元件皆等於 0 的 Tuple。

**Padding** = true 設定允許可將核心的「中心」保持在「真實」輸入內所需的填補量。 這麼做會略為改變輸出大小的計算方式。 一般而言，輸出大小 _D_ 會計算為 _D = (I-K) / 秒 + 1_，其中 _我_ 是輸入的大小， _K_ 是核心大小， _S_ 是分散，和 _/_ 是整數除法 (趨近於零四捨五入)。 如果您設定 UpperPad = [1，1] 輸入的大小 _我_ 實際上是 29，因此 _D = (29-5) / 2 + 1 = 13_。 不過，當 **填補** = true，基本上 _我_ 會藉由 _K-1_; 因此 _D = ((28 + 4)-5) / 2 + 1 = 27 / 2 + 1 = 13 + 1 = 14_。 藉由指定 **UpperPad** 和 **LowerPad** 的值，您對填補的控制權會遠遠超過只設定 **Padding** = true。
...
**回應正規化** 是本端正規化配置，最早是由 Geoffrey Hinton 等人，在本文件 [ﬁ with Deep Convolutional Neural Networks](http://www.cs.toronto.edu/~hinton/absps/imagenet.pdf)。 回應正規化可用來輔助類神經網路中的一般化。 當一個神經元在非常高的啟動層級上引發時，本端回應正規化層將會抑制周遭神經元的啟動層級。 這由使用三個參數 (*** α ***, ，*** β ***, ，和 *** k ***) 與迴旋結構 (或網路上的芳鄰] 圖形)。 目的地層中的每個神經元 *** y *** 對應至一個神經元 *** x *** 來源層中。 啟用層級 *** y *** 下列公式，其中 *** f *** 是神經，啟用層級和 *** Nx *** 是核心 (或包含在 10mb 上下神經組 *** x ***)，如下列迴旋結構所定義:

![][1]

回應正規化套組支援 **Sharing**、**MapCount** 和 **Weights** 以外的所有迴旋屬性。

-   如果核心包含做為相同對應中的神經元 *** x ***, ，則正規化配置稱為 **相同對應正規化**。 若要定義相同對應正規化，**InputShape** 中的第一個座標必須具有值 1。
...
-   第一行會定義輸入層 (名為 _資料_)。 當您使用  **自動** 關鍵字，類神經網路會自動包含所有特徵資料行中輸入的範例。
...
-   隱藏層 _ByRow_ 和 _ByCol_ 使用述詞運算式指定篩選連線。 更精確地說，在 [x, y] 上，_ByRow_ 中的節點會連接到 _Pixels_ 中第一個索引座標等於節點的第一個座標 x 的節點。 Si milarly，_ByCol 在 [x，y] 中的節點都會連接到在節點 _像素_ ，有第二個索引座標內其中一個節點的第二個座標，y。
...
-   每個核心的加權數目是 _1 + **KernelShape**\[0] * **KernelShape**\[1] * **KernelShape**\[2] = 1 + 1 * 5 * 5 = 26。 或 26 * 50 = 1300_。
-   您可以用下列方式計算每個隱藏層中的節點數：
    -   **NodeCount**\[0] = (5-1) / 1 + 1 = 5。
    -   **NodeCount**\[1] = (13-5) / 2 + 1 = 5。
    -   **NodeCount**\[2] = (13-5) / 2 + 1 = 5。
-   節點總數計算方式是使用圖層的宣告的維度 [50，5，5]，如下所示: _**MapCount** * **NodeCount**\[0] * **NodeCount**\[1] * **NodeCount**\[2] = 10 * 5 * 5 * 5_
-   因為 **共用**[d] 為 False 只有 _d = = 0_、 核心的數目是 _**MapCount** * **NodeCount**\[0] = 10 * 5 = 50_。
