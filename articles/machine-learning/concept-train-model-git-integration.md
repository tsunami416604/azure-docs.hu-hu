...
Ha a képzési fájlok nem egy git-tárházban találhatók a fejlesztői környezetben, vagy a parancs `git` nem érhető el, a rendszer nem követi a git-hez kapcsolódó adatokat.
...
> ```
>
> Ha telepítve van, és az elérési úton, a következőhöz hasonló `git version 2.4.1`választ kap:. A git fejlesztési környezetben való telepítésével kapcsolatos további információkért tekintse meg a [git webhelyét](https://git-scm.com/).
...
1. Válassza a __naplók__lehetőséget, majd bontsa ki a __naplók__ és a __azureml__ bejegyzéseket. Válassza ki a hivatkozást, amely az __ ### \_Azure__-ban kezdődik.
...
A `az ml run` CLI-parancs használatával lekérheti a tulajdonságokat egy futtatásból. A következő parancs például visszaadja a (z) nevű `train-on-amlcompute`kísérlet utolsó futtatásának tulajdonságait:
