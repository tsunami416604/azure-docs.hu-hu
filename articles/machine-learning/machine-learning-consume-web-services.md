...
> [AZURE.TIP] 簡單的方式來建立 web 應用程式來存取您的預測 web 服務，請參閱 [使用 Azure Machine Learning web 服務的 web 應用程式範本與](machine-learning-consume-web-service-with-web-app-template.md)。

<!-- When this article gets published, fix the link and uncomment
For more information on how to manage Azure Machine Learning web service endpoints using the REST API, see **Azure machine learning web service endpoints**.
-->

如需有關如何建立和部署 Azure Machine Learning web 服務的資訊，請參閱 [部署 Azure Machine Learning web 服務][publish]。 建立機器學習實驗，並將其部署的逐步解說，請參閱 [使用 Azure Machine Learning 開發預測方案][walkthrough]。
...
1.   **服務 API 金鑰**, 、 服務主要頁面
2.   **服務 URI**, 所選的服務 API 說明頁面
3.  預期 **API 要求本文**, 所選的服務 API 說明頁面
4.  預期 **API 回應本文**, 所選的服務 API 說明頁面
...
* **輸入**: 代表儲存批次工作的輸入位置的 blob 參考。
* **GlobalParameters**: 代表一個可以用於定義實驗的全域參數集。 Azure Machine Learning 實驗可以具有必要和選用參數，以自訂服務的執行，而呼叫者預期會提供所有必要的參數 (如果有這些參數)。 這些參數會以索引鍵-值組的集合形式來指定。
* **輸出**: 如果服務已定義一個或多個輸出，我們允許呼叫者必須將它們重新導向至其所選的 Azure blob 位置。 這麼做可讓您將服務的輸出，以可預測的名稱儲存在慣用位置，否則輸出的 Blob 名稱會隨機產生。 **請注意** ，服務會預期輸出內容，根據其型別，儲存為支援的格式:
  - 資料集輸出: 可以將儲存為 **.csv、.tsv、.arff**
  - 定型模型輸出: 可以將儲存為 **.ilearner**

  輸出位置覆寫指定的集合為 *< 輸出名稱，blob 參考 >* 組，其中 *輸出名稱* 是特定輸出節點 (也顯示在服務的 API 說明頁面上)，定義的使用者名稱和 *blob 參考* 其中輸出會被重新導向至 Azure blob 位置的參考。

所有這些建立工作的參數均可視您的服務性質來選擇性地使用。 例如，服務的定義，沒有輸入節點不需要傳入 *輸入* 參數和輸出位置覆寫功能是完全選擇性的因為輸出也會儲存為 Azure Machine Learning 工作區設定的預設儲存體帳戶中。 以下針對只傳入輸入資訊的服務，示範傳遞至 REST API 的要求承載：
...
建立批次作業只會將它登錄在系統中，並放入 *未啟動* 狀態。 若要實際排程工作來執行，您必須呼叫 **啟動** 時建立作業，取得服務端點的 API 說明頁面上，並提供工作識別碼所述的 API。

**3.取得批次執行工作的狀態**

您可以隨時將工作 ID 傳遞至 GetJobStatus API，藉此輪詢非同步批次工作的狀態。 API 回應會包含指標，指出工作的目前狀態，以及實際的批次工作結果 (如果批次工作順利完成)。 如果發生錯誤，傳回失敗背後的實際原因的詳細資訊 *詳細資料* 屬性。
 ...
 *結果* 工作已順利完成時，才會填入屬性 (它是 **null** 否則)。 在工作完成，且如果服務已定義至少一個輸出節點，將會傳回結果的集合 *[輸出名稱，blob 參考]* 組，其中的 blob 參考是包含實際結果之 blob 的 SAS 唯讀參考。
