...
此服務提供透過 HTTPS 的 REST 型 API，有各種不同方式可使用此 API，包括 Web 或行動應用程式、R、Python、Excel 等等。我們有 [Azure web 應用程式](http://anomalydetection-aml.azurewebsites.net/) 可協助執行異常偵測 web 服務對您的資料，並以視覺化方式檢視結果。
...
API 會對您的時間序列資料執行這些偵測器，然後傳回每個時間點的異常分數。 中所示，使用簡單的剖析器可以剖析此輸出 [https://adresultparser.codeplex.com/](https://adresultparser.codeplex.com/)。 其中的範例程式碼示範如何連線到 API 並剖析輸出。 產生的警示可以具體呈現在儀表板中並 (或) 傳送給專家人員以採取更正動作。
...
代表的是下列表格：
 ...
 時間| 資料| Tspike| Zspike| Martingale values| Alert indicator| Martingale values (2)| Alert indicator (2)
---|---|---|---|---|---|---|---
