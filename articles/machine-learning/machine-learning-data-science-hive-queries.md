...
1. [提交 Hive 查詢透過 Hadoop 命令列中的 Hadoop 叢集前端節點](#headnode)
...
登入 Hadoop 叢集的前端節點、 開啟 Hadoop 命令列，在桌面上的前端節點，然後輸入命令 `cd %hive_home%\bin`。
...
使用者可以執行類似 `hive-e"< hive 查詢 >;` 送出簡單的 Hive 查詢直接在 Hadoop 命令列。在下列範例中，紅色方塊框起來的是提交 Hive 查詢的命令，而綠色方塊框起來的則是 Hive 查詢的輸出。

![建立工作區][10]

#### 提交 .hql 檔案中的 Hive 查詢。

若 Hive 查詢更複雜且有多行，則在命令列或 Hive 命令主控台中編輯查詢並不實際。 替代方法是在 Hadoop 叢集的前端節點中使用文字編輯器，將 Hive 查詢儲存於前端節點本機目錄上的 .hql 檔案中。 然後可以提交.hql 檔案中的 Hive 查詢使用 `-f` 引數，如下所示:
...
根據預設，在 Hadoop 命令列中提交 Hive 查詢之後，Map/Reduce 工作的進度將顯示於螢幕上。 若要隱藏 Map/Reduce 工作進度的畫面列印，您可以使用引數 `-S` (大寫為"S") 在命令列中，如下所示:
...
使用者可以也先進入 Hive 命令主控台執行命令 `hive` Hadoop 命令列中，然後提交 Hive 查詢在 Hive 命令主控台中的。 範例如下。 在此範例中，這兩個紅色方塊反白顯示的命令分別是用來進入 Hive 命令主控台，以及在 Hive 命令主控台中提交 Hive 查詢。 綠色方塊反白顯示的是 Hive 查詢的輸出。
...
在下列範例中，Hive 查詢的輸出會寫入 `C:\apps\temp` 目錄的 `hivequeryoutput.txt` 檔案中。
...
在下列範例中，Hive 查詢的輸出會寫入 blob 目錄 `queryoutputdir` Hadoop 叢集預設容器內。 在此處，您只需提供目錄名稱，而不需提供 Blob 名稱。 發生錯誤則會擲回提供目錄和 blob 名稱，例如 `wasb:///queryoutputdir/queryoutput.txt`。
...
使用者也可以使用網頁瀏覽器中輸入 URL 的查詢主控台 (Hive 編輯器) `https://<Hadoop 叢集名稱 >.azurehdinsight.net/Home/HiveEditor` (將會被系統要求您輸入 Hadoop 叢集認證進行登入)，
...
在此範例中，變數 `smooth_param1` 和 `smooth_param2，以` 以減緩從資料計算出來的風險值來設定。 風險範圍介於 -Inf 和 Inf 之間。 風險 > 0 代表目標等於 1 的機率大於 0.5。
...
在此查詢中，如果 `< 日期時間欄位 >` 具有類似模式 `03/26/2015年 12:04:39`, 、 `'< 模式的日期時間欄位 >'` 應該 `' MM/dd/yyyy hh: mm:'`。若要進行測試，使用者可以執行

        select from_unixtime(unix_timestamp('05/15/2015 09:32:10','MM/dd/yyyy HH:mm:ss'))
        from hivesampletable limit 1;

在此查詢中， `hivesampletable` 隨附於所有 Azure HDInsight Hadoop 叢集預設佈建叢集時。
...
計算兩個 GPS 座標間距離的數學方程式位於 [可移動的類型指令碼](http://www.movable-type.co.uk/scripts/latlong.html), 、 Peter Lapisu 作者。 在 Javascript 中，函數 torad () 僅只是 `lat_or_lon * pi/180`, ，可將角度轉換為弧度。 此處的 `lat_or_lon` 為緯度或經度。 由於 Hive 不提供函式 `atan2`, ，但提供函數 `atan`, 、 `atan2` 函式由實作 `atan` 位於上述 Hive 查詢，根據其定義中的函式 [Wikipedia](http://en.wikipedia.org/wiki/Atan2)。
...
1. Java 堆積空間：對於涉及聯結大型資料集或處理長記錄的查詢，常見的錯誤是**堆積空間不足**。 這可藉由將參數 `mapreduce.map.java.opts` 和 `mapreduce.task.io.sort.mb` 設定為所需的值來進行微調。 下列是一個範例：
...
5. 以下將提及最佳化 Hive 效能的其他數個更進階選項。 這些選項會考慮到配置來對應和縮減工作的記憶體設定，在調整效能時非常實用。 請記住，`mapreduce.reduce.memory.mb` 不能大於 Hadoop 叢集中每個背景工作角色節點的實際記憶體大小。
