...
Ez a példa az első adatkészletbe helyezi az összes olyan sort, amely tartalmazza az oszlop `Text``Gryphon` szövegét. Más sorokat helyez el a **felosztott adatokat**tartalmazó második kimenetbe.
...
1. A **relációs kifejezés** mezőben adjon meg egy olyan kifejezést, amely egy összehasonlítási műveletet hajt végre egyetlen oszlopon.
   ...
   **Numerikus oszlop**esetén:
   ...
   - A és a művelethez használja a jel karaktert, `&`. A vagy a művelethez használja a cső karaktert, `|`.
   - A következő operátorok támogatottak: `<`, `>`, `<=`, `>=`, `==`, `!=`.
   - A műveletek nem csoportosíthatók `(` és `)`használatával.
...
Például egy olyan dátum oszlopban, amely a `mmddyyyy`formátumot használja, a kifejezésnek a következőhöz hasonlónak kell lennie:
