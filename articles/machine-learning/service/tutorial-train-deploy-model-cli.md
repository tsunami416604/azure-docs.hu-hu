...
Az oktatóanyagban szereplő Python-betanítási szkriptek a [scikit-Learn](https://scikit-learn.org/) paranccsal egy alapszintű modellt tanítanak. Ennek az oktatóanyagnak a középpontjában nem a parancsfájlok vagy a modell szerepel, de a parancssori felület használatának folyamata a Azure Machine Learning.
...
* `.azureml\sklearn.runconfig`: Egy __futtatási konfigurációs__ fájl. Ez a fájl határozza meg a modell betanításához szükséges futásidejű környezetet.
...
* `training-env.yml`: Meghatározza a betanítási parancsfájl futtatásához szükséges szoftver-függőségeket.
...
* `aciDeploymentConfig.yml`: Egy __telepítési konfigurációs__ fájl. Ez a fájl határozza meg a modellhez szükséges üzemeltetési környezetet.
* `inferenceConfig.yml`: Egy következtetési configuration__ fájl. Ez a fájl határozza meg, hogy a szolgáltatás milyen Szoftverkörnyezet használatával szerzi be az adatmodellt.
* `score.py`: Egy olyan Python-szkript, amely fogadja a bejövő értékeket, a modell használatával szerzi be, majd visszaadja a választ.
* `scoring-env.yml`: A modell futtatásához és `score.py` parancsfájlhoz szükséges Conda függőségek.
...
Az erőforráscsoport az Azure platformon található erőforrások alapszintű tárolója. Az Azure Machine Learning szolgáltatás használatakor az erőforráscsoport a Azure Machine Learning szolgáltatás munkaterületét fogja tartalmazni. Emellett a munkaterület által használt egyéb Azure-szolgáltatásokat is tartalmazni fogja. Ha például egy felhőalapú számítási erőforrás használatával betanítja a modellt, akkor az erőforrást az erőforráscsoport hozza létre.
...
Új munkaterület létrehozásához használja a következő parancsot. Cserélje le a `<workspace-name>` értéket a munkaterülethez használni kívánt névre. Cserélje le a `<resource-group-name>` értéket az erőforráscsoport nevére:
...
Egy terminálról vagy parancssorból az alábbi parancsokkal módosíthatja a könyvtárakat a `mlops` könyvtárba, majd csatlakozhat a munkaterületéhez:
...
Ez a parancs létrehoz egy `.azureml/config.json` fájlt, amely a munkaterületéhez való kapcsolódáshoz szükséges információkat tartalmazza. A jelen oktatóanyagban használt `az ml` parancsok további része ezt a fájlt fogja használni, így nem kell hozzáadnia a munkaterületet és az erőforráscsoportot az összes parancshoz.

## <a name="create-the-compute-target-for-training"></a>Számítási cél létrehozása képzéshez

Ez a példa egy Azure Machine Learning számítási példányt használ a modell betanításához. Új számítási példány létrehozásához használja a következő parancsot:
...
Ez a parancs létrehoz egy `cpu` nevű új számítási célt, amely legfeljebb négy csomópontot tartalmazhat. A kiválasztott virtuálisgép-méret GPU-erőforrással rendelkező virtuális gépet biztosít. A virtuális gép méretével kapcsolatos információkért lásd: [VM-típusok és méretek].
...
A `cpu` számítási célra való betanítás indításához váltson át a `model-training` könyvtárra, majd használja a következő parancsot:
...
A `-c sklearn` paraméter határozza meg a `.azureml/sklearn.runconfig` fájlt. Ahogy korábban említettük, ez a fájl a képzési Futtatás által használt környezet konfigurálásához használt információkat tartalmazza. Ha megvizsgálja ezt a fájlt, látni fogja, hogy a korábban létrehozott `cpu` számítási célra hivatkozik. Azt is felsorolja, hogy hány csomópontot kell használni a Betanításkor (`"nodeCount": "4"`), és tartalmaz egy `"condaDependenciees"` szakaszt, amely felsorolja a betanítási parancsfájl futtatásához szükséges Python-csomagokat.

A konfigurációs fájlok futtatásával kapcsolatos további információkért lásd: [számítási célok beállítása és használata a modell betanításához](how-to-set-up-training-targets.md#create-run-configuration-and-submit-run-using-azure-machine-learning-cli).
...
Ezt a szöveget a betanítási szkriptből (`train-sklearn.py`) naplózza a rendszer, és megjeleníti a modell teljesítmény-metrikáinak két értékét. Ebben az esetben a legmagasabb alfa-értékkel rendelkező modellt szeretnénk használni. A teljesítmény-mérőszámok a betanítási modellre jellemzőek. Más modellek esetében a teljesítmény mérőszámai eltérőek lesznek.

Ha megvizsgálja a `train-sklearn.py`, megfigyelheti, hogy az alfa értéket is használja, ha a betanított modellt (ka) t a fájlba tárolja. Ebben az esetben több modellt is betanít. Az egyik a legmagasabb alfa legyen a legjobb. A fenti kimenet, valamint a kód, a 0,95-as alfa-modellel rendelkező modell a következőként lett mentve: `./outputs/ridge_0.95.pkl`

A modell mentve lett a `./outputs` könyvtárba azon a számítási célhelyen, ahol betanítják. Ebben az esetben a Azure Machine Learning számítási példány az Azure-felhőben. A betanítási folyamat automatikusan feltölti a `./outputs` könyvtár tartalmát a számítási célhelyről, ahol betanítás történik a Azure Machine Learning-munkaterületre. Ez a kísérlet részeként van tárolva (@no__t – 0 ebben a példában).
...
Az első parancs letölti a regisztrált modellt az aktuális könyvtárba. A fájl neve `ridge_0.95.pkl`, amely a modell regisztrálása során hivatkozott fájl. A második parancs regisztrálja a helyi modellt (`-p "ridge_0.95.pkl"`) ugyanazzal a névvel, mint a korábbi regisztráció (`mymodel`). Ez alkalommal a visszaadott JSON-adatérték a 2-es verziót sorolja fel.
...
Ezzel a paranccsal üzembe helyezhető egy `myservice` nevű új szolgáltatás, amely a korábban regisztrált modell 1. verzióját használja.

A `inferenceConfig.yml` fájl információt nyújt a következtetések elvégzéséről, például a bejegyzési parancsfájl (`score.py`) és a szoftver függőségeiről. A fájl struktúrájával kapcsolatos további információkért tekintse meg a [következtetési konfigurációs sémát](reference-azure-machine-learning-cli.md#inference-configuration-schema). A beléptetési parancsfájlokkal kapcsolatos további információkért lásd: [modellek üzembe helyezése a Azure Machine learning szolgáltatással](how-to-deploy-and-where.md#prepare-to-deploy).
...
A központi telepítésből visszaadott `scoringUri` a webszolgáltatásként üzembe helyezett modell REST-végpontja. Ezt az URI-t a következő parancs használatával is lekérheti:

```azurecli-interactive
az ml service show -n myservice
```

Ez a parancs ugyanazt a JSON-dokumentumot adja vissza, beleértve a `scoringUri` értéket is.
...
A parancs válasza a következőhöz hasonló: `[4684.920839774082]`.
...
Az erőforráscsoport és az ebben a dokumentumban létrehozott összes Azure-erőforrás törléséhez használja a következő parancsot. Cserélje le a `<resource-group-name>` értéket a korábban létrehozott erőforráscsoport nevére:
...
További információ a parancssori felület használatáról: [Azure Machine learning szolgáltatás CLI-bővítményének használata](reference-azure-machine-learning-cli.md).