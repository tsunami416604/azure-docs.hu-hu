...
1.  在模組調色盤中，輸入 「 中繼資料 」 **搜尋** 方塊。 您會看到 [中繼資料編輯器][metadata-editor] 模組清單中。
...
2.  根據預設，分割率會是 0.5， **Randomized split** 參數設定。 這表示資料有一半會透過一個連接埠的輸出的隨機 [分割][split] 模組，並另一半。 您可以調整這些項目，並將 **隨機種子** 參數，以變更訓練與評分資料之間的分割。 在此範例中，我們會將其保留為原狀。
