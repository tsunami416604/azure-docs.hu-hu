...
## <a name="setup"></a>適用於資料科學設定 HDInsight Hadoop 叢集
...
2. [適用於資料科學自訂 Azure HDInsight Hadoop 叢集](machine-learning-data-science-customize-hadoop-cluster.md): 這個步驟會建立所有節點上安裝 64 位元 Anaconda Python 2.7 的 Azure HDInsight Hadoop 叢集。 自訂 HDInsight 叢集時有兩個需完成的重要步驟 (如本主題所述)。
...
## <a name="login"></a>登入叢集前端節點

若要登入叢集前端節點，請使用 [Azure 管理](manage.windowsazure.com) 入口網站，以找出叢集。 在左側按一下 HDInsight 大象圖示，然後按兩下叢集的名稱。 瀏覽至 [**組態**] 索引標籤上，在頁面底部的 [連接] 圖示上連按兩下，然後在提示時輸入您的遠端存取認證。 這會帶您前往叢集的前端節點。
...
**重要事項**: * * 執行本逐步解說中的所有 Hive 命令，從上述的 Hive bin / 目錄提示字元。 如此可自動處理路徑相關問題。 我們將使用詞彙 「 Hive 目錄提示 」、 「 Hive bin / 目錄提示 」，和 「 Hadoop 命令列 」 interchangeably.* *
...
## <a name="exploration"></a> 在 Hive 中的資料探索
...
## <a name="downsample"></a> 對 Azure Machine Learning 縮減取樣資料集
...
**附註：**在本逐步解說中，我們著重在使用計數資料表來產生高維度類別功能的精簡表示法。 這當然不是編碼類別功能; 唯一的方法如需有關其他技術的詳細資訊，有興趣的使用者可以查看 [一個熱-編碼](http://en.wikipedia.org/wiki/One-hot) 和 [特徵雜湊](http://en.wikipedia.org/wiki/Feature_hashing)。
...
### <a name="step1"></a> 步驟 1: 從 Hive 資料表取得資料到 Azure Machine Learning 使用讀取器模組，並選取它進行機器學習實驗
...
*** 重要注意事項: *** * * 執行這項操作的訓練和測試資料集。 此外，請記住要使用您為此目的提供的資料庫名稱和資料表名稱。 在圖中所使用的值是僅供圖 purposes.* *

### <a name="step2"></a> 步驟 2: 建立簡單的實驗，在 Azure Machine Learning，來預測按一下 / 未按一下
...
若要取得藍色的輸入和輸出連接埠，您只要按一下位於右下方的 [**準備 Web 服務**]。 執行這項實驗也可讓我們發佈 web 服務，即可 **發佈 WEB 服務** 圖示位於右下方，如下所示。
