...
- **[Create a training experiment]** -Azure Machine Learning Studio 是共同作業視覺式開發環境，您用來訓練和測試預測分析模型，使用您提供的訓練資料。
- **[Convert it to a predictive experiment]** -當已使用的現有資料訓練您的模型，而且您準備好使用它來評分新資料，您會準備並簡化用於評分的實驗。
- **[Deploy it as a web service]** -使用，只要按一下，您可以部署您的評分實驗當做 Azure web 服務。 使用者可以將資料傳送到您的模型以及接收您的模型的預測。

[AZURE.INCLUDE [machine-learning-free-trial](../../includes/machine-learning-free-trial.md)]

一旦您部署了 Web 服務，您可以：

- **[Access]** 它透過 web 服務 API
- **[Manage]** 它透過 Azure 傳統入口網站，並
- **[Update]** 如果您的模型變更
...
- [使用 Azure Machine Learning 開發預測方案](machine-learning-walkthrough-develop-predictive-solution.md)
...
若要將訓練實驗轉換成預測實驗中，按一下 [ **執行** 在實驗畫布底端，按一下 [ **設定 Web 服務**, ，然後選取 **預測 Web 服務**。

![轉換為評分實驗](./media/machine-learning-publish-a-machine-learning-web-service/figure-1.png)

如需有關如何執行這項轉換的詳細資訊，請參閱 [Machine Learning 訓練實驗轉換成做預測分析](machine-learning-convert-training-experiment-to-scoring-experiment.md)
...
若要部署您做預測分析，請按一下 [ **執行** 底部的實驗畫布，然後按一下 [ **部署 Web 服務**。 系統會設定 Web 服務，且會將您帶往 Web 服務儀表板。

![部署 Web 服務](./media/machine-learning-publish-a-machine-learning-web-service/figure-2.png)

若要測試 web 服務，請按一下 [ **測試** web 服務儀表板中的連結。 對話方塊隨即顯示，要求您提供服務的輸入資料。 這些是評分實驗預期的資料行。 輸入一組資料，然後按一下 **確定**。 Web 服務產生的結果會顯示在儀表板底部。

![測試 Web 服務](./media/machine-learning-publish-a-machine-learning-web-service/figure-3.png)

在 **組態** ] 索引標籤，您可以變更服務的顯示名稱，並且給予說明。 名稱和說明會顯示在 Azure 傳統入口網站中，您在該入口網站中管理您的 Web 服務。
您可以描述為資料輸入、 輸出資料及 web 服務參數提供每個資料行底下輸入字串，來 **輸入結構描述**, ，**輸出結構描述**, ，和 **WEB 服務參數**。 這些說明會用於為 Web 服務提供的範例程式碼文件。
...
在 Azure 傳統入口網站中，您可以管理您的 web 服務，即可 **Machine Learning** 服務開啟您的機器學習工作區，然後開啟 [web 服務從 **WEB 服務** ] 索引標籤。 您可以從這個頁面監視 Web 服務、更新及刪除它。 您也可以為 Web 服務新增預設端點 (在您部署服務時建立) 之外的第二個端點。
...
若要更新 web 服務，開啟您用來部署 web 服務，並按一下 [製作可編輯複本原始做預測分析 **存**。 進行變更，然後按一下 [ **部署 Web 服務**。 因為您以前已部署過這個實驗，所以 Machine Learning Studio 會提示您以詢問是否要覆寫現有服務。 如果您按一下 **是**, ，然後停止現有的 web 服務，並在其位置部署新的預測實驗。

> [AZURE.NOTE] 如果您在原始的 web 服務進行組態變更，例如，輸入新的顯示名稱或說明，您必須再次輸入這些值。
