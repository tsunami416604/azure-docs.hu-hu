...
這是逐步解說的第二個步驟 [開發使用 Azure ML 的預測方案](machine-learning-walkthrough-develop-predictive-solution.md)
...
我們將使用名為的檔案 **german.data**。 將此檔案下載至您的本機硬碟。  
...
1.  登入 Machine Learning Studio ([https://studio.azureml.net](https://studio.azureml.net)) 藉由使用您指定的工作區中，擁有者的 Microsoft 帳戶，然後按一下 **Studio** 頂端的索引標籤。
2.  按一下 [ **+ 新增** 視窗的底部。
3.  選取 **資料集**。
4.  選取 **從本機檔案**。
5.  在 **上傳新的資料集** ] 對話方塊中，按一下 [ **瀏覽** 並尋找 **german.csv** 您所建立的檔案。
6.  輸入資料集的名稱。 在此例中，我們稱它為「UCI 德國信用卡資料」。
7.  資料型別，請選取 **一般 CSV 檔案沒有標頭 (。.nh.csv)**。
8.  視需要新增說明。
9.  按一下 [ **確定**。  
