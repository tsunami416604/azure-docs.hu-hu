...
執行實驗之後, 您可以按一下輸出連接埠 [評估模型 ][evaluate-model] 模組，然後選取 *視覺化* 以查看評估結果。 適用於迴歸模型的評估度量包括：「*平均絕對誤差*」、「*平均根絕對誤差*」、「*相對絕對誤差*」、「*相對平方誤差*」和「*決定係數*」。
...
如先前所述，您可以執行重複的定型、 計分和評估會使用自動 [交叉驗證模型 ][cross-validate-model] 模組。 您只需要在此情況下是資料集，而非定型的模型，以及 [交叉驗證模型 ][cross-validate-model] 模組 (請參閱下圖)。 請注意，您需要的標籤資料行設為 *價格* 中 [交叉驗證模型 ][cross-validate-model] 模組的屬性。
...
例如，在二元分類的情況下，目標變數只有兩個可能的結果：{0, 1} 或 {false, true}, {negative, positive}。假設您有一些成人員工的資料集人口統計和就業變數，而且您必須預測收入層級，二元變數的值 {"<=50K”, “>50k"}。換句話說，負數類別表示年收入低於或等於 5 萬元的員工，而正數類別則表示其他所有員工。如同在迴歸情況下，我們會訓練模型、為某些資料評分，並評估結果。此處的主要差異在於 Azure Machine Learning 計算和輸出的度量選擇。為說明收入層級的預測情況，我們將使用 [成人](http://archive.ics.uci.edu/ml/datasets/Adult) 資料集來建立 Azure Machine Learning 實驗，並評估二元羅吉斯迴歸模型，常用的二元分類器的效能。
...
- 成人收入普查二進位分類資料集
- [二級羅吉斯迴歸 ][two-class-logistic-regression]
- [訓練模型 ][train-model]
- [評分模型 ][score-model]
- [評估模型 ][evaluate-model]
...
執行實驗之後, 您可以按一下輸出連接埠 [評估模型 ][evaluate-model] 模組，然後選取 *視覺化* 以查看評估結果 (圖 7)。 可用於二元分類模型的評估度量包括：「*正確性*」、「*精確度*」、「*回收*」、「*F1 分數*」和 *AUC*。 此外，這個模組會輸出一個混淆矩陣，其中顯示真肯定、誤否定、誤肯定、真否定，以及 *ROC*、「*精確度/回收*」和「*升力*」曲線的數目。
...
在此實驗中，我們將使用熱門 [鳶尾](http://archive.ics.uci.edu/ml/datasets/Iris "鳶尾") 資料集，其中包含 3 個不同類型 (類別) 鳶尾屬植物的執行個體。 每個案例有 4 個特性值 (萼片長度/寬度和花瓣長度/寬度)。 在上一個實驗中，我們使用相同的資料集訓練並測試模型。 在這裡，我們將使用 [分割 ][split] 模組建立 2 個資料子集、 第一個子集，訓練和評分及評估第二個。 
...
如先前所述，您可以執行重複的定型、 計分和評估會使用自動 [交叉驗證模型 ][cross-validate-model] 模組。 您需要資料集，而非定型的模型，以及 [交叉驗證模型 ][cross-validate-model] 模組 (請參閱下圖)。 同樣地，您需要設定的 [標籤] 資料行 [交叉驗證模型 ][cross-validate-model] 模組 (資料行索引 5 在此情況下)。 執行實驗並按一下右側的輸出連接埠之後 [交叉驗證模型 ][cross-validate-model], ，您可以檢查每個摺疊，以及平均值和標準差的度量值。 在此顯示的度量類似於在二進位分類案例中討論的度量。 但是請注意，在多元分類中，運算真肯定/否定以及誤肯定/否定是透過根據每個類別計算來完成，因為沒有整體的正或負類別。 例如，運算 ‘Iris-setosa’ 類別的精確度或回收時，假設這是正類別，其他所有類別則是負類別。
