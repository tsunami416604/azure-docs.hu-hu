...
**本文**將概述從一般檔案 (CSV 或 TSV 格式) 或是內部部署的 SQL Server，將資料移動至  Azure 虛擬機器上之 SQL Server 的選項。 這些用於將資料移至雲端的工作是 Azure 所提供之 Cortana 分析程序的一部分。
...
下表摘要說明移動資料至 Azure 虛擬機器上之 SQL Server 的選項。
 ...
 <b>來源</b>| <b>在 Azure VM 上的目的地: SQL Server</b>|
------------------ |-------------------- |
 ...
 <b>內部部署 SQL Server</b>| 1.<a href="#deploy-a-sql-server-database-to-a-microsoft-azure-vm-wizard">將 SQL Server 資料庫部署到 Microsoft Azure VM 精靈</a><br> 2。<a href="#export-flat-file">匯出至一般檔案 </a><br> 3。<a href="#sql-migration">SQL Database 移轉精靈 </a> <br> 4。<a href="#sql-backup">資料庫備份和還原 </a><br>
...
* 已佈建 **Azure VM 上的 SQL Server**。 如需指示，請參閱 [將 Azure SQL Server 虛擬機器設定為 IPython Notebook 伺服器供進階分析](machine-learning-data-science-setup-sql-server-virtual-machine.md)。
...
3. [SQL Server (匯入/匯出，SSIS) 中的圖形化內建公用程式](#sql-builtin-utilities)
...
1. 確定已在目標 SQL Server 資料庫上建立資料庫和資料表。 以下是如何執行動作，使用的範例 `Create Database` 和 `Create Table` 命令:
...
2. 從安裝 BCP 的電腦命令列中發出下列命令，以產生說明資料表結構描述的格式檔案。
    ...
    `bcp dbname...tablename 格式化 nul c-x-f exportformatfilename.xml-S servername\sqlinstance-T-t \t-r \n`

3. 使用 BCP 命令來將資料插入資料庫，如下所示。 若假設 SQL Server 安裝於同一部電腦上，這應該就能從命令列順利運作：

    `bcp dbname...在 datafilename.tsv-f exportformatfilename.xml-S servername\sqlinstancename-U 使用者名稱-P 密碼-b block_size_to_move_in_single_attemp-t \t-r \n tablename`
...
[大量插入 SQL 查詢](https://msdn.microsoft.com/library/ms188365) 可用來將資料匯入的資料庫資料列/資料行為基礎的檔案 (支援的型別將會涵蓋[準備資料大量匯出或匯入 (SQL Server)](https://msdn.microsoft.com/library/ms188609)) 主題。
...
1. [將 SQL Server 資料庫部署到 Microsoft Azure VM 精靈](#deploy-a-sql-server-database-to-a-microsoft-azure-vm-wizard)
...
**將 SQL Server Database 部署到 Microsoft Azure VM 精靈**是簡單且建議的方式，可用於將資料從內部部署 SQL Server 執行個體移至 Azure VM 上的 SQL Server。 詳細的步驟，以及其他替代方案的討論，請參閱 [資料庫移轉至 Azure VM 上的 SQL Server](../virtual-machines/virtual-machines-migrate-onpremises-database.md)。
...
1. 使用 BCP 公用程式，從內部部署的 SQL Server 將資料匯出至檔案，如下所示
    ...
    `bcp dbname...tablename 出 datafile.tsv-S servername\sqlinstancename-T-t \t-t \n-c`

2. SQL Server VM 上建立資料庫和資料表上使用 Azure `建立資料庫` 和 `建立資料表` 步驟 1 中匯出的資料表結構描述。
...
[資料庫移轉至 Azure VM 上的 SQL Server](../virtual-machines/virtual-machines-migrate-onpremises-database.md)

[SQL Server 的 Azure 虛擬機器總覽](../virtual-machines/virtual-machines-sql-server-infrastructure-services.md)
