...
我們會編寫為基礎的三個預測問題 *tip\_amount 來*, ，也就是 ︰

1. 二進位分類 ︰ 預測是否已支付小費針對某趟車程，也就是 *tip\_amount 來* 大於 $0 高於為正面範例，而 *tip\_amount 來* $0 的為負面範例。

2. 多類別分類：預測已針對該車程支付的小費的金額範圍。 我們將 *tip\_amount 來* 分成五個分類收納組或類別 ︰
...
您可以看到從 [規劃您的環境](machine-learning-data-science-plan-your-environment.md) 指南中，有數個選項可以使用 Azure 的 NYC 計程車車程資料集 ︰
...
此範例會識別在特定期間內超過 100 趟車程的圓形徽章 (計程車數目)。 因為資料分割配置的條件，查詢帶來好處從資料分割的資料表存取 **pickup\_datetime**。 查詢完整資料集也會使用資料分割資料表及 (或) 索引掃描。
...
查看描述性統計資料 **[trip\_distance]** （或任何其他） 欄位 ︰
...
我們將說明之間的散佈圖 **[trip\_time\_in\_secs]** 和 **[trip\_distance]** ，看看是否那里
任何相互關聯性
...
同樣地，我們可以檢查之間的關聯性 **[rate\_code]** 和 **[trip\_distance]**。
...
準備資料中建置模型時 [Azure Machine Learning Studio](https://studio.azureml.net), ，您可能會決定 **SQL 查詢，在讀取程式模組中直接使用** 或工程和取樣資料保存在新的資料表，您可以在使用 [讀取器][reader] 模組以簡單 **選取 * 從 < your\_new\_table\_name >**。
...
在本節中，我們將使用前面所建立的新資料表中保存的 1% 取樣資料，來探索資料分佈。 請注意，可以使用原始資料表，或是執行類似的探勘 **TABLESAMPLE** ，以限制探索範例，或是透過指定的時間週期來將結果限制 **pickup\_datetime** 分割中所示 [資料探索和功能工程 SQL Server 中](#dbexplore) 一節。
...
此範例會將緯度和/或經度欄位的十進位表示法細分為資料粒度不同的多個區域欄位，例如國家/地區、城市、城鎮、街區等。請注意，新的地理位置欄位不會對應到實際的位置。 如需對應地理編碼位置的詳細資訊，請參閱 [Bing 地圖服務 REST 服務](https://msdn.microsoft.com/library/ff701710.aspx)。
...
現在，我們已準備好繼續進行模型建置和部署在 [Azure Machine Learning](https://studio.azureml.net)。 資料已經準備好用於稍早所識別的任何預測問題，也就是：
...
若要開始進行模型化練習，請登入 Azure Machine Learning 工作區。 如果您尚未建立機器學習工作區，請參閱 [建立 Azure ML 工作區](machine-learning-create-workspace.md)。
...
3. Studio 首頁會提供豐富的資訊、影片、教學課程、與模組參考的連結，以及其他資源。 如需有關 Azure Machine Learning 的詳細資訊，請參閱 [Azure Machine Learning 文件中心](http://azure.microsoft.com/documentation/services/machine-learning/)。
...
當您備妥模型時，可以輕鬆地直接從實驗中將它部署為 Web 服務。 如需部署 Azure ML web 服務的詳細資訊，請參閱 [部署 Azure Machine Learning web 服務](machine-learning-publish-a-machine-learning-web-service.md)。
