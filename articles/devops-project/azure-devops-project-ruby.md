---
title: CI/CD-folyamat létrehozása Ruby on Railshez az Azure DevOps Projecttel | Rövid útmutató
description: A DevOps Project megkönnyíti az Azure-ral való ismerkedést. A segítségével néhány gyors lépéssel elindíthat egy Ruby-alkalmazást egy Azure-szolgáltatásban.
ms.prod: devops
...
ms.locfileid: "37966932"
---
# <a name="create-a-cicd-pipeline-for-ruby-on-rails-with-the-azure-devops-project"></a>CI/CD-folyamat létrehozása Ruby on Railshez az Azure DevOps Projecttel

Az **Azure DevOps Project** segítségével konfigurálhatja a folyamatos integrációs (CI) és folyamatos kézbesítési (CD) folyamatot egy Ruby on Rails-alkalmazáshoz.  Az Azure DevOps Project egyszerűsíti a VSTS-build és a kiadási folyamat kezdeti konfigurációját.
...
1. A bal oldali navigációs sávban válassza az **Erőforrás létrehozása** ikont, majd keresse meg a **DevOps-projektet**.  Válassza a **Létrehozás** elemet.
    ...
    ![Folyamatos kézbesítés indítása](_img/azure-devops-project-ruby/fullbrowser.png)

## <a name="select-a-sample-application-and-azure-service"></a>Mintaalkalmazás és Azure-szolgáltatás kiválasztása

1. Válassza a **Ruby** mintaalkalmazást.

1. Válassza a **Ruby on Rails** alkalmazás-keretrendszert.  Ha elkészült, válassza a **Tovább** lehetőséget.

1. Az alapértelmezett üzembehelyezési cél a **Web App on Linux**.  A Web App for Containers használata mellett is dönthet.  Az előző lépésekben kiválasztott alkalmazás-keretrendszer meghatározza az Azure-szolgáltatás üzembehelyezési céljának itt elérhető típusát.  Válassza ki a tetszőleges **célszolgáltatást**.  Ha elkészült, válassza a **Tovább** lehetőséget.
...
1. Hozzon létre egy **új** ingyenes VSTS-fiókot, vagy válasszon ki egy **meglévőt**.  Válasszon egy **nevet** a VSTS-projektnek.  Válassza ki az **Azure-előfizetését**, a **helyet**, és adjon egy **nevet** az alkalmazásnak.  Ha elkészült, kattintson a **Kész** gombra.
    ...
    ![VSTS-adatok megadása](_img/azure-devops-project-ruby/vstsazureinfo.png)
...
1. Néhány percen belül betöltődik a **projekt irányítópultja** az Azure Portalon.  A rendszer beállít egy mintaalkalmazást a VSTS-fiókban található adattárban, végrehajtja a buildelést, és üzembe helyezi az alkalmazást az Azure-ban.  Ez az irányítópult biztosítja a **kódtár**, a **VSTS CI/CD-folyamat** és az **Azure-ban található alkalmazás** áttekinthetőségét.  Az irányítópult jobb oldalán válassza a **Tallózást** a futó alkalmazás megtekintéséhez.
    ...
    ![Irányítópult nézet](_img/azure-devops-project-ruby/dashboardnopreview.png) 
...
1. A böngésző bal oldalán keresse meg az **app/views/pages/home.html.erb** fájlt.
