...
- A példában szereplő programkód futtatásához először Azure SQL Databasenek kell lennie, [Machine learning Services (R)](machine-learning-services-overview.md) engedélyezve van.
...
Az egyszerűség kedvéért használja a `stats` Machine learning Services (előzetes verzió) használatával a Azure SQL Database alapértelmezés szerint telepített és betöltött R-csomagot. A csomag több száz funkciót tartalmaz a közös statisztikai feladatokhoz, köztük a `rnorm` függvényhez. Ez a függvény meghatározott számú véletlenszerű számot hoz létre a normál eloszlással, a szórás és az azt jelenti.
...
Ha a T-SQL-ből szeretné hívni az R-vonalat, futtassa a parancsot, `sp_execute_external_script` és adja hozzá az r-függvényt az r script paraméterben, például:
...
Mivel a `utils` csomag telepítve van, de alapértelmezés szerint nincs betöltve, a függvényt kell használnia az `library()` első betöltéséhez.
...
> [!TIP]
> Számos felhasználó szeretné használni az r-folyamatok rendszer-időzítési funkcióit, például a és a-t, `system.time` `proc.time` hogy rögzítse az r-folyamatok által használt időt, és elemezze a teljesítménnyel kapcsolatos problémákat.
