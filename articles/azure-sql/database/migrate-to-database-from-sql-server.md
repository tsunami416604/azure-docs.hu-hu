---
title: Adatbázis-áttelepítés SQL Server egy vagy készletezett Azure SQL Database
...
Ebből a cikkből megtudhatja, hogyan telepíthet egy SQL Server 2005-es vagy újabb verziójú adatbázist egyetlen vagy készletezett Azure SQL Databaseba. A felügyelt Azure SQL-példányokra való áttelepítéssel kapcsolatos információkért lásd: [áttelepítés SQL Server példányra az Azure SQL felügyelt példányára](../managed-instance/migrate-to-instance-from-sql-server.md). A más platformokról való áttelepítéssel kapcsolatos információkért lásd: az [Azure Database áttelepítési útmutatója](https://datamigration.microsoft.com/).
...
A következő lista az egyetlen vagy készletezett adatbázis SQL Server adatbázis-áttelepítésének általános munkafolyamatát tartalmazza ezzel a módszerrel. A felügyelt példányra való áttelepítéssel kapcsolatban lásd: [Migrálás felügyelt példányra](../managed-instance/migrate-to-instance-from-sql-server.md).
...
> [!NOTE]
> A DMA használata helyett használhat BACPAC-fájlt is. Lásd: [BACPAC-fájl importálása új Azure SQL Databaseba](database-import.md).
...
- Távolítsa el a ritkán lekérdezett korábbi adatlekérdezéseket egy másik adatbázisba, és telepítse át ezeket az előzményeket egy külön Azure SQL Database. Ezután lekérdezheti ezeket az előzményadatokat a [rugalmas lekérdezések](elastic-query-overview.md) használatával.
...
> [!IMPORTANT]
> Az Azure SQL felügyelt példánya lehetővé teszi, hogy a meglévő SQL Server-példányokat és annak adatbázisait minimálisan, kompatibilitási problémák nélkül áttelepítse. Lásd: [Mi az az Azure SQL felügyelt példánya](../managed-instance/sql-managed-instance-paas-overview.md).
