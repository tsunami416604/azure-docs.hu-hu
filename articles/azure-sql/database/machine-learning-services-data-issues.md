...
A válasz általában az R parancs használatával érhető el `str()` . Adja hozzá a függvényt az `str(object_name)` r-szkriptben bárhol, hogy a megadott R objektum adatsémája tájékoztató üzenetként legyen visszaadva. Az üzenetek a SSMS **üzenetek** lapján tekinthetők meg.

Annak megállapításához, hogy az 1. és a 2. példa milyen eredményeket tartalmaz, szúrja be a sort a `str(OutputDataSet)` `@script` változó definíciójának végén minden utasításban, a következőhöz hasonlóan:
...
> [!TIP]
> Az R Identity functions (például) is használható `is.matrix` `is.vector` a belső adatszerkezettel kapcsolatos információk visszaküldéséhez.
...
Vegye figyelembe azonban, hogy mi történik a tömb méretének módosításakor `y` .
...
Az alábbi szkript például egy 6. hosszúságú numerikus tömböt határoz meg, és az R változóban tárolja `df1` . A numerikus tömböt ezután a RTestData tábla (fent létrehozott) egész számával kombináljuk, amely három (3) értéket tartalmaz, hogy új adatkeretet hozzon létre `df2` .
...
Az adatkeret kitöltéséhez az R a tömbben lévő elemek számának megfelelően többször megismétli a RTestData lekért elemeket `df1` .
...
A lekérdezés működésének megkezdése után tekintse át a függvény eredményét, `str` és nézze meg, hogyan kezeli az R a bemeneti adatokat.
...
- Adja meg a bemeneti adatforrás oszlopait a használata helyett, `SELECT *` és Ismerje meg, hogy az egyes oszlopok hogyan lesznek kezelve.
