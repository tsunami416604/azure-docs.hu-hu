...
ms.locfileid: "84049924"
---
# <a name="business-critical-tier---azure-sql-database--sql-managed-instance"></a>Üzletileg kritikus réteg – Azure SQL Database & SQL felügyelt példánya 
...
A prémium/üzletileg kritikus szolgáltatási szint modellje az adatbázismotor folyamatainak egyik fürtjén alapul. Ez az építészeti modell arra támaszkodik, hogy mindig a rendelkezésre álló adatbázismotor-csomópontok kvóruma van, és a karbantartási tevékenységek során a számítási feladatok minimális teljesítményére is hatással van. Az nagy kapacitású szolgáltatási réteg jelenleg csak Azure SQL Database (nem SQL felügyelt példány esetében érhető el), és egy rugalmasan méretezhető tárolási és számítási teljesítményi réteg, amely az Azure-architektúrát kihasználva kibővíti a tárolási és számítási erőforrásokat egy Azure SQL Database számára, amely jelentősen meghaladja az általános célú és üzletileg kritikus szolgáltatási szintjein elérhető korlátokat.
...
-   **Alacsony IO-késési követelmények** – a tárolási rétegből gyors választ igénylő munkaterhelések (átlagosan 1-2 ezredmásodperc) a üzletileg kritikus szintet kell használniuk. 
...
-   **Az adatmódosítást követően hosszú ideig futó tranzakciók**. Azok a tranzakciók, amelyek hosszú ideig vannak megnyitva, megakadályozzák a naplófájlok csonkítása, ami növelheti a napló méretét és a [virtuális naplófájlok (VLF)](https://docs.microsoft.com/sql/relational-databases/sql-server-transaction-log-architecture-and-management-guide#physical_arch)számát. A feladatátvétel után a VLFs nagy száma lelassíthatja az adatbázis helyreállítását.
...
- Megtalálhatja az erőforrás jellemzőit (magok, IO, memória) üzletileg kritikus szinten az [SQL felügyelt példányában](../managed-instance/resource-limits.md#service-tier-characteristics), a [virtuális mag-modellben](resource-limits-vcore-single-databases.md#business-critical---provisioned-compute---gen4) vagy a [DTU-modellben](resource-limits-dtu-single-databases.md#premium-service-tier)lévő önálló adatbázist, illetve a [virtuális mag-modellben](resource-limits-vcore-elastic-pools.md#business-critical---provisioned-compute---gen4) és a [DTU-modellben](resource-limits-dtu-elastic-pools.md#premium-elastic-pool-limits)található rugalmas készletet.
