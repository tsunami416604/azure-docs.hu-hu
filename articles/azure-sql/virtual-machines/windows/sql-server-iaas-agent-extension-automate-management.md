...
ms.locfileid: "93286170"
---
# <a name="what-is-the-sql-server-iaas-agent-extension"></a>Mi a SQL Server IaaS-ügynök bővítmény?
...
Ez a cikk áttekintést nyújt a bővítményről. A SQL Server IaaS bővítmény Azure-beli virtuális gépeken való SQL Server való telepítéséhez tekintse meg az [automatikus telepítéssel](sql-vm-resource-provider-automatic-registration.md), az [önálló virtuális gépekkel](sql-vm-resource-provider-register.md)vagy [a nagy méretű virtuális gépekkel](sql-vm-resource-provider-bulk-register.md)kapcsolatos cikkeket. 
...
- **Ingyenes** : a bővítmény mindhárom kezelhetőségi módban teljesen ingyenes. Az erőforrás-szolgáltatóval kapcsolatban nem áll rendelkezésre további díj, vagy a felügyeleti módokat módosítani kell. 
...
Az SQL IaaS bővítmény három felügyeleti móddal rendelkezik:

- A **könnyű** mód nem igényli a SQL Server újraindítását, de csak az SQL Server licenc típusának és kiadásának módosítását támogatja. Ezzel a beállítással SQL Server virtuális gépeket több példánnyal, vagy részt vehet a feladatátvevő fürt példányaiban. Ez a felügyeleti mód tartja az SQL IaaS-ügynök bővítményének bináris fájljait a gépen, de nem telepíti az ügynököt. A könnyű mód az alapértelmezett felügyeleti mód az [automatikus regisztrációs](sql-vm-resource-provider-automatic-registration.md) funkció használatakor, vagy ha nincs megadva felügyeleti típus a manuális regisztráció során. A könnyű mód használata esetén nincs hatással a memóriára vagy a CPU-ra, és nincs kapcsolódó díj. Javasoljuk, hogy először regisztrálja a SQL Server VMt, majd egy ütemezett karbantartási időszak alatt frissítsen a teljes módba.

- A **teljes** mód biztosítja az összes funkciót, de a SQL Server és a rendszergazdai engedélyek újraindítását igényli. A SQL Server VM egyetlen példánnyal való felügyeletére használható. A teljes mód két Windows-szolgáltatást telepít, amelyek minimális hatással vannak a memóriára és a CPU-ra – ezeket a Feladatkezelő segítségével figyelheti. A teljes kezelhetőségi mód használata nem jár együtt. 
...
A SQL Server IaaS-ügynök bővítmény akkor települ, amikor regisztrálja SQL Server virtuális gépeket az SQL VM erőforrás-szolgáltatóval. Az erőforrás-szolgáltatóval való regisztráció az előfizetésen belül hozza létre az SQL-alapú **virtuális gép** _erőforrását_ , amely a virtuális gép erőforrásának _külön_ erőforrása. A SQL Server VM erőforrás-szolgáltatóból való regisztrációjának törlése eltávolítja az **SQL-alapú virtuális gép** _erőforrását_ , de nem fogja eldobni a tényleges virtuális gépet.
...
A bővítmény telepítéséhez regisztrálja a SQL Server VMt az erőforrás-szolgáltatónál:
- [Az előfizetésben lévő összes jelenlegi és jövőbeli virtuális gép esetében automatikusan](sql-vm-resource-provider-automatic-registration.md)
- [Egyetlen virtuális géphez](sql-vm-resource-provider-register.md)
- [Több nagy méretű virtuális gép esetén](sql-vm-resource-provider-bulk-register.md)

### <a name="named-instance-support"></a>Megnevezett példányok támogatása

A SQL Server IaaS bővítmény a SQL Server megnevezett példányával fog működni, ha ez az egyetlen, a virtuális gépen elérhető példány SQL Server. A bővítmény nem telepíthető olyan virtuális gépekre, amelyeken több SQL Server példány található. 

Ha SQL Server megnevezett példányát szeretné használni, helyezzen üzembe egy Azure-beli virtuális gépet, telepítsen egy nevű SQL Server példányt, majd regisztrálja az [SQL VM erőforrás-szolgáltatóval](sql-vm-resource-provider-register.md) a bővítmény telepítéséhez.
...
Azt is megteheti, hogy egy megnevezett példányt használ egy Azure Marketplace SQL Server-lemezképpel, kövesse az alábbi lépéseket: 
   ...
   1. A SQL Server VM [regisztrációjának törlése](sql-vm-resource-provider-register.md#unregister-from-rp) az SQL VM erőforrás-szolgáltatójából. 
   ...
   1. Telepítse az SQL IaaS Agent bővítményt úgy, hogy [regisztrálja a SQL Server VM az SQL VM erőforrás-szolgáltatóval](sql-vm-resource-provider-register.md#register-with-rp). 
...
Az SQL VM erőforrás-szolgáltatóval való regisztráláskor az alapértelmezett felügyeleti mód a *könnyű*. Ha a SQL Server Management tulajdonság nincs beállítva az SQL VM erőforrás-szolgáltatóhoz való regisztráláskor, a mód a (z) rendszerhez hasonlóan könnyű lesz, és a SQL Server szolgáltatás nem indul újra. Javasoljuk, hogy először az egyszerűsített módban regisztrálja az SQL VM erőforrás-szolgáltatót, majd a karbantartási időszak alatt teljesre frissítsen. Hasonlóképpen, az [automatikus regisztrálási funkció](sql-vm-resource-provider-automatic-registration.md)használata esetén az alapértelmezett felügyelet is egyszerű.

**Milyen előfeltételeket kell regisztrálni az SQL VM erőforrás-szolgáltatónál?**

Az SQL VM-erőforrás-szolgáltatón kívül nem kell regisztrálnia az előfeltételeket, mint a virtuális gépre SQL Server telepítve. Vegye figyelembe, hogy ha az SQL IaaS-ügynök bővítménye teljes módban van telepítve, akkor a SQL Server szolgáltatás újraindul, így a karbantartási időszak során ajánlott.

**Regisztrálja az SQL VM erőforrás-szolgáltatót az ügynök telepítése a virtuális gépre?**

Igen, az SQL VM erőforrás-szolgáltató teljes körű kezelhetőségi módban való regisztrálása egy ügynököt telepít a virtuális gépre. A Lightweight vagy nem ügynök módban való regisztráció nem. 

Az SQL VM erőforrás-szolgáltató egyszerű módban való regisztrálása csak az SQL IaaS-ügynök bővítmény *bináris fájljait* másolja át a virtuális gépre, nem telepíti az ügynököt. Ezeket a bináris fájlokat a rendszer az ügynök telepítésére használja, ha a felügyeleti mód teljes verzióra frissül.
...
**Mi a különbség a könnyű és a nem ügynök közötti felügyeleti üzemmódok között az SQL VM erőforrás-szolgáltatóval való regisztráláskor?** 
...
Nem. Az SQL Server licenc típusa nem választható tulajdonság az SQL VM erőforrás-szolgáltatóval való regisztráláskor. Ha az SQL VM erőforrás-szolgáltatót az összes kezelhetőségi módban (nem ügynök, könnyűsúly és teljes) regisztrálja, akkor a SQL Server Azure Hybrid Benefit licencfeltételeket kell beállítania. Ha a SQL Server ingyenes verzióit, például a fejlesztői vagy a próbaverziós verziót használja, regisztrálnia kell az utólagos elszámolású licenceléssel. Azure Hybrid Benefit csak SQL Server, például a nagyvállalati és a standard kiadások fizetős verzióihoz érhető el.
...
Ha módosítani szeretné a kezelhetőségi módot a teljes kezelhetőséggel, szüntesse meg a SQL Server VM [regisztrációját](sql-vm-resource-provider-register.md#unregister-from-rp) az SQL VM erőforrás-szolgáltatóból az SQL virtuális gép _erőforrásának_ eldobásával, majd a SQL Server VM újbóli regisztrálásával az SQL VM erőforrás-szolgáltatóval.

**Regisztrálhatok az SQL VM erőforrás-szolgáltatóval a Azure Portal?**

Nem. Az SQL VM erőforrás-szolgáltatóval való regisztráció nem érhető el a Azure Portal. Az SQL VM erőforrás-szolgáltatóval való regisztrálás csak az Azure CLI vagy a Azure PowerShell esetén támogatott. 

**Regisztrálhatok egy virtuális gépet az SQL VM erőforrás-szolgáltatóval SQL Server telepítése előtt?**

Nem. A virtuális gépnek rendelkeznie kell legalább egy SQL Server (adatbázismotor) példánnyal az SQL VM erőforrás-szolgáltatóval való sikeres regisztráláshoz. Ha a virtuális gépen nincs SQL Server példány, az új Microsoft. SqlVirtualMachine erőforrás sikertelen állapotba kerül.
...
Távolítsa el a bővítményt, ha törli a SQL Server VM [regisztrációját](sql-vm-resource-provider-register.md#unregister-from-rp) az SQL VM erőforrás-szolgáltatójából. 
