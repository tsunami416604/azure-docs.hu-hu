...
如果動作項目是像上述範例中的可設定狀態的動作項目，當回呼完成時，動作項目執行階段會儲存動作項目的狀態。 如果儲存狀態時發生錯誤，將會停用該動作項目物件並啟動新的執行個體。 不會修改動作項目狀態的回呼方法可以當做唯讀的計時器回呼指定 「 唯讀 」 屬性的計時器回呼，因為登錄上一節中所述 [唯讀方法](service-fabric-reliable-actors-introduction.md#readonly-methods)。
