...
- `工作 OnOpenAsync (IStatelessServicePartition，CancellationToken)`
    使用無狀態服務執行個體時，會呼叫 OnOpenAsync。 這個階段可以啟動擴充服務的初始化作業。

- `工作 OnCloseAsync(CancellationToken)`
...
- `工作 OnChangeRoleAsync (ReplicaRole，CancellationToken)`
...
- `工作 OnOpenAsync (IStatefulServicePartition，CancellationToken)`
- `工作 OnCloseAsync(CancellationToken)`
- `void onabort)`
...
- [設定具狀態可靠服務](service-fabric-reliable-services-configuration.md)
