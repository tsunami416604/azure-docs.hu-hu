...
- `OnActivateAsync` 呼叫方法 (這可以在動作項目實作中覆寫)。
- 動作項目會新增至使用中動作項目資料表。

動作項目停用時會發生什麼事？

- 動作項目若一段時間未使用，便會從使用中動作項目資料表移除。
- `OnDeactivateAsync` 呼叫方法 (這可以在動作項目實作中覆寫)。 這會清除動作項目的所有計時器。
...
- 一直收到呼叫
- `IRemindable.ReceiveReminderAsync` 方法叫用 (動作項目使用提醒時，才適用)
...
通常不需要變更這些預設值。 不過，如有必要，變更這些間隔可以在該組件中的所有動作項目類型的組件層級或動作項目類型層級使用 `ActorGarbageCollection` 屬性。 以下範例顯示 HelloActor 的廢棄項目收集間隔變更。
...
若要變更的預設值 `ActorGarbageCollection` 屬性在組件層級中，新增下列程式碼片段來 `AssemblyInfo.cs`。

```csharp
[assembly: ActorGarbageCollection(IdleTimeoutInSeconds = 10, ScanIntervalInSeconds = 2)]
```

對於使用中動作項目資料表的每個動作項目，動作項目執行階段會持續追蹤動作項目已閒置 (亦即未使用) 的時間。 動作項目執行階段會檢查每個動作項目每個 `ScanIntervalInSeconds` 是否可以進行記憶體回收，如果已閒置會收集 `IdleTimeoutInSeconds`。

只要使用動作項目，其閒置時間就會重設為 0。 在此之後，動作項目進行記憶體回收才一次持續閒置的 `IdleTimeoutInSeconds`。 請回想一下，當動作項目介面方法或動作項目提醒回撥執行時，動作項目會視為已使用。 如果動作項目的計時器回撥執行時，**不會**將動作項目視為已使用。
