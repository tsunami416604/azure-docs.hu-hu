...
A SyslogConsumer az összes platform eseményt írja a létesítmény `Local0`használatával. A konfiguráció konfigurációjának módosításával bármely érvényes létesítményre frissítheti. A használt identitás `ServiceFabric`. Az üzenet mező a JSON-ban szerializált teljes eseményt tartalmazza, így számos eszközről lekérdezhető és felhasználható. 
...
1. A közös szakaszban a `LinuxStructuredTracesEnabled`nevű új paraméter szerepel. **Erre azért van szükség, hogy a rendszer a syslog szolgáltatásba való elküldésekor strukturálja és szerializálja a Linux-eseményeket.**
...
3. Az új szakasz SyslogConsumer `true`nak kell lennie `IsEnabled`. Úgy van konfigurálva, hogy automatikusan használja a Local0-létesítményt. Ezt felülbírálhatja egy másik paraméter hozzáadásával.
...
5. A lekérdezés elindításához a munkaterület erőforrás menüjében `Logs` elemre kattintva lépjen a lekérdezési tallózóba.

    ![Munkaterület naplófájljai](media/service-fabric-diagnostics-oms-syslog/workspace-logs.png)
6. Lekérdezheti a ProcessName `ServiceFabric` keresett `Syslog` táblát. Az alábbi lekérdezés szemlélteti, hogyan elemezheti a JSON-t az eseményen, és megjelenítheti annak tartalmát
