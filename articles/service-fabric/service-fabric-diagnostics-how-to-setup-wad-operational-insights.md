...
1. Service Fabric 記錄檔：由平台發出到標準 ETW 和 EventSource 通道。 這些可能是下列其中一種類型：
  ...
  - [動作項目程式設計模型事件](https://azure.microsoft.com/documentation/articles/service-fabric-reliable-actors-diagnostics/)
  - [可靠服務程式設計模型事件](https://azure.microsoft.com/documentation/articles/service-fabric-reliable-services-diagnostics/)
...
或者可以下載 ARM 範例，會對它進行變更並修改的範本以建立叢集，使用 `New-azureresourcegroupdeployment` 命令在 Azure Powershell 視窗中。 您需要傳給命令的參數如下。 此外，呼叫此部署命令之前您可能需要執行一些設定，包括加入您的 Azure 帳戶 (`Add-azureaccount`)，選擇 [訂用帳戶 (`Select-azuresubscription`)，切換到 ARM 模式 (`Switch-azuremode AzureResourceManager`) 並建立資源群組，如果您還沒有這麼做 (`New-azureresourcegroup`)。
...
```
之後建立的 json 檔做為上述並已為您的環境變更它們，呼叫此傳入 Service Fabric 叢集資源群組的命令。 成功執行此命令後，WAD 將部署在所有 VM 上，並開始將叢集中的記錄檔上傳至指定的 Azure 儲存體帳戶中的資料表。 此外，呼叫此部署命令之前您可能需要執行一些設定，包括加入您的 Azure 帳戶 (`Add-azureaccount`)，選擇適合的訂閱 (`Select-azuresubscription`) 和切換到 ARM 模式 (`Switch-azuremode AzureResourceManager`)。
```powershell
...
[Operational Insights 登入](https://technet.microsoft.com/library/mt484118.aspx)
