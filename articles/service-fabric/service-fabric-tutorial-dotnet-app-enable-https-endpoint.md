---
title: HTTPS-végpont hozzáadása Azure Service Fabric-alkalmazáshoz | Microsoft Docs
...
, szüksége van egy digitális tanúsítványra.  Éles alkalmazásokhoz használjon [hitelesítésszolgáltatói (CA-)](https://wikipedia.org/wiki/Certificate_authority) tanúsítványt. Fejlesztési és tesztelési célok érdekében létrehozhat és használhat önaláírt tanúsítványt. A Service Fabric SDK biztosítja a *CertSetup.ps1* szkriptet, amely létrehozza az önaláírt tanúsítványt, és importálja azt a `Cert:\LocalMachine\My` tanúsítványtárolóba. Rendszergazdaként nyisson meg egy parancssort, és futtassa a következő parancsot egy CN=localhost tárgyú tanúsítvány létrehozásához:
...
Frissítse a `ServiceInstanceListener` elemet úgy, hogy az új *VégpontHttps* végpontot használja, és a 443-as porton figyeljen. 
...
Adja hozzá a következő metódust, hogy a Kestrel megtalálja a tanúsítványt a `Cert:\LocalMachine\My` tárolóban a tárgy segítségével.  Cserélje le a &lt;saját_CN_értéke&gt; elemet localhost-ra, amennyiben az előző PowerShell-paranccsal önaláírt tanúsítványt hozott létre, vagy használja a tanúsítványa rövid nevét.
...
Először exportálja a tanúsítványt egy PFX-fájlba. Nyissa meg a certlm.msc alkalmazást, majd keresse meg a **Személyes**>**tanúsítványok** elemet.  Kattintson a jobb gombbal a *localhost* tanúsítványra, és válassza a **Minden feladat**>**Exportálás** elemet.  
