...
無狀態微服務 (例如通訊協定閘道、 web proxy 等) 不會維護任何要求和回應從服務以外的任何可變動狀態。 Azure 雲端服務背景工作角色即為無狀態服務的範例。 可設定狀態微服務 (例如使用者帳戶、 資料庫、 裝置、 購物車、 佇列等) 維護要求和回應之外的可變動授權狀態。 現今的網際網路級別應用程式包含無狀態與可設定狀態微服務的組合。
...
如需有關應用程式模式和使用 Service Fabric 設計請參閱 [應用程式案例](service-fabric-application-scenarios.md)
...
如需有關應用程式生命週期管理請參閱 [應用程式生命週期](service-fabric-application-lifecycle.md)
