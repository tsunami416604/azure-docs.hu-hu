...
1) 能夠建立高輸送量、 低延遲、 容錯 OLTP 服務，如互動式店面、 搜尋、 物聯網 (IoT) 系統、 貿易系統、 信用卡處理、 詐欺偵測系統，關閉同一部電腦上的個人記錄管理等程式碼與資料。

2) 應用程式設計簡化，因為可設定狀態微服務需要其他佇列與傳統上要求滿足純無狀態應用程式的可用性與延遲需求的快取。 可設定狀態服務有高可用性、低延遲性的特性，這表示整體管理應用程式的移動組件較少。 

如需有關應用程式模式和設計使用 Service Fabric 看到 [應用程式案例](service-fabric-application-scenarios.md)
...
大多數的應用程式都結合了無狀態與可設定狀態的微服務，及其他一起部署的 EXE/執行階段。 Service Fabric 擁有應用程式與封裝微服務的強式型別，因此能部署多個應用程式執行個體，每個執行個體都能獨立管理與升級。 重要的是 Service Fabric 是能夠部署 *任何* 可執行檔或執行階段並使其可靠。 例如，可用於部署 ASP.NET 5、node.js、指令碼，或可組成應用程式的任何一切。 
  
如需應用程式生命週期管理的詳細資訊請參閱 [應用程式生命週期](service-fabric-application-lifecycle.md)
