...
* **沒有我需要使用特定的通訊協定，我想要快速啟動並執行**: 如果您不需要特定的通訊架構選擇，則您的理想選項是使用 [預設堆疊](service-fabric-reliable-services-communication-remoting.md) ，可允許類似的模型作為動作項目通訊模型。 若要開始使用服務通訊，這是最簡單且快速的方式。 其提供強型別 RPC 通訊，可擷取多數移出的通訊詳細資料，以便在程式碼中的遠端服務上叫用方法，看起來像在本機物件執行個體上呼叫方法。 這些類別會在設定通訊通道時擷取解析、連接、重試和錯誤處理，並允許基於通訊模型的方法呼叫。 這個 `servicecommunicationlistener 進行通訊` 類別是用於伺服器端和 `ServiceProxy` 通訊的用戶端類別。

* **HTTP**：若要利用 HTTP 式通訊提供的彈性，您可以使用 Service Fabric 通訊 API，以便在解析、連接和重試仍從您擷取的邏輯時允許定義通訊機制。 例如您可以使用 Web API，用於指定的通訊機制並利用 [`ICommunicationClient` 和 `ServicePartitionClient` classes](service-fabric-reliable-services-communication.md) 來設定通訊。
...
* [開始使用 Microsoft Azure Service Fabric Web API 服務與 OWIN 自我會裝載](service-fabric-reliable-services-communication-webapi.md)
