...
### 重要的組態選項
 - **TimeToRun**: 測試會執行成功完成的時間總計。 測試可以提前完成，而不必等驗證失敗。
 - **MaxClusterStabilizationTimeout**: 時間等候叢集變成狀況良好測試失敗前的最大數量。 執行的檢查會查看叢集健康情況或服務健康情況是否正常，或是服務分割區是否達到目標複本的設定大小，以及是否沒有 InBuild 複本。
 ...
 - **WaitTimeBetweenIterations**: 反覆項目，也就是在一輪的錯誤之後，與對應的驗證之間應該等待的時間量。
...
容錯移轉測試工作會引發選定的錯誤，並以確保其穩定性服務上執行驗證。 容錯移轉測試一次只會引發一個錯誤，不像混亂測試中可能會有多個錯誤。 如果在每個錯誤後，服務分割區沒有在設定的逾時內變穩定，測試會失敗。此測試只會引發安全的錯誤。 這表示因為沒有外部錯誤，所以不會發生仲裁或資料遺失。

### 重要的組態選項
 - **PartitionSelector**: 指定需要做為目標的分割區的選取器物件。
 - **TimeToRun**: 測試執行完成之前的時間總計
 - **MaxServiceStabilizationTimeout**: 時間等候叢集變成狀況良好測試失敗前的最大數量。 執行的檢查會查看服務健康情況是否正常，或是所有分割區是否達到目標複本的設定大小，以及是否沒有 InBuild 複本。
 - **WaitTimeBetweenFaults**: 每個錯誤和驗證的循環之間要等候的時間量
