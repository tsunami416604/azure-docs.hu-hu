---
title: Azure Service Fabric Service Model XML-séma összetett típusai | Microsoft Docs
...
Ha a tárolóban engedélyezve van a HEALTHCHECK és a Docker-jelentés health_status esemény ehhez a tárolóhoz, Service Fabric a rendszer jelentett állapot részeként fogja tartalmazni. Ha a health_status nem kifogástalan állapotú, Service Fabric fog jelenteni egy állapotra vonatkozó figyelmeztetést. Alapértelmezés szerint True (igaz) értékre van állítva.
...
Ha a tárolóban engedélyezve van a HEALTHCHECK és a Docker-jelentések health_status esemény erre a tárolóra vonatkozóan, a Service Fabric újraindítja a tárolót, ha a Docker által jelentett health_status nem kifogástalan állapotú.
...
Memória korlátja MB-ban. Csak pozitív egész szám lehet. A kódcsomagok (tárolók vagy folyamatok) nem tudnak ennél a korlátnál több memóriát lefoglalni, és ennek megkísérlése memóriahiány miatti kivételt eredményez.
...
A felhasználható swap memória teljes mennyisége (MB). Csak pozitív egész szám lehet.
...
A memória-szabályozáshoz szükséges (MB-ban mért) korlát, amely csak akkor kényszerített, ha a csomóponton a memória-tartalom észlelhető. Csak pozitív egész szám lehet.
...
Relatív CPU-súlyozás. Csak pozitív egész szám lehet.
...
Az elérhető processzorok kihasználtsági aránya (csak Windows). Csak pozitív egész szám lehet. Ha a szolgáltatási csomaghoz CPU-korlátok vannak megadva, a paramétert a rendszer hatékonyan figyelmen kívül hagyja.
...
A felhasználható IOps maximális i/o-sebessége (olvasási és írási). Csak pozitív egész szám lehet.
...
A használható maximális IO-érték (bájt/s) (olvasási és írási). Csak pozitív egész szám lehet.
...
Tárolók lemezkvóta.  Csak pozitív egész szám lehet.
...
A kernel memóriájának korlátai bájtban.  Csak pozitív egész szám lehet.  Fontos megjegyezni, hogy ez a Linux-specifikus és a Docker a Windows-ban hiba esetén kikerül, ha ez be van állítva.
...
A/dev/SHM mérete bájtban megadva. Ha nincs megadva, a rendszer 64 MB-ot használ.  Csak pozitív egész szám lehet.  Vegye figyelembe, hogy ez a Linux-specifikus, azonban a Docker nem jelenik meg, ha meg van adva.  Egyszerűen figyelmen kívül hagyható.
...
#### <a name="resourcetype"></a>Erőforrástípus
...
A CPU-korlát a logikai magok száma szerint. Csak pozitív egész szám lehet.
...
#### <a name="memoryinmb"></a>MemoryInMB
Memória korlátja MB-ban. Csak pozitív egész szám lehet.
