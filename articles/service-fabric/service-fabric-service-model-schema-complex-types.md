...
Azt határozza meg, hogy a figyelmeztetési állapot jelentéseit hibaként kell-e kezelni az állapot értékelése során. Alapértelmezett érték: false (hamis).
...
|Attribútum|Érték|
|---|---|
|tartalom|4 elem (ek), 1 attribútum|
|meghatározott|globálisan|
...
|típus|[ServiceTemplatesType](#servicetemplatestype-complextype)|
...
|típus|[DefaultServicesType](#defaultservicestype-complextype)|
...
|Attribútum|Érték|
|---|---|
|tartalom|9 elem (ek), 0 attribútum|
|meghatározott|globálisan|
...
|---|---|
|name|Diagnosztika|
|típus|[DiagnosticsType](#diagnosticstype-complextype)|
|minOccurs|0|
...
|típus|[EnvironmentType](#environmenttype-complextype)|
...
#### <a name="isseednode"></a>Magcsomópont
...
|Attribútum|Érték|
|---|---|
|tartalom|4 elem (ek), 3 attribútum|
|meghatározott|globálisan|
...
|típus|[EnvironmentVariablesType](#environmentvariablestype-complextype)|
|minOccurs|0|
|maxOccurs|1|
...
A ConfigPackage Settings.xml szakaszának neve.
...
|Attribútum|Érték|
|---|---|
|tartalom|0 elem (ek), 8 attribútum|
|meghatározott|globálisan|
...
|Attribútum|Érték|
|---|---|
|tartalom|10 elem (ek), 8 attribútum|
|meghatározott|globálisan|
...
Használja a clusterManifest.xmlben megadott alapértelmezett adattár hitelesítő adatait a ApplicationManifest.xmlon belüli "RepositoryCredentials" címkében megadott hitelesítő adatok helyett.
...
|---|---|
|name|Kötet|
|típus|[ContainerVolumeType](#containervolumetype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
|---|---|
|name|SecurityOption|
|típus|[SecurityOptionsType](#securityoptionstype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
|---|---|
|name|DriverOption|
|típus|[DriverOptionType](#driveroptiontype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
|Attribútum|Érték|
|---|---|
|tartalom|4 elem (ek), 10 attribútum|
|meghatározott|globálisan|
...
#### <a name="servicegroup"></a>Szolgáltatáscsoportot mozgassa
...
Illesztőprogram-beállítások, amelyeket át kell adni az illesztőprogramnak. A Azure Files kötet beépülő modul a következő illesztőprogram-beállításokat támogatja: megosztásnév (a tároló kötetét biztosító Azure Files fájlmegosztás), a storageAccountName (az Azure Files fájlmegosztást tartalmazó Azure Storage-fiók), a storageAccountKey (a Azure Files fájlmegosztást tartalmazó Azure Storage-fiók elérési kulcsa).
...
Ez az érték határozza meg az attribútum értékének védelmét. Lehetséges, hogy SecretsStoreRef/titkosított/egyszerű szöveges.
Ha a SecretsStoreRef értékre van állítva, akkor az érték a fürt SecureStore titkosított titkos kulcsra hivatkozik, amely azonban az aktiváláskor fel van fűzve és visszafejtve.
...
#### <a name="port"></a>Port
A port a Http.sys vagy a BFE-vel való regisztráció után Microsoft Azure Service Fabric által meghatározott portra lesz lecserélve.
...
A környezeti változó típusa Ez lehet egyszerű szöveges/titkosított/SecretsStoreRef.
...
|Attribútum|Érték|
|---|---|
|tartalom|21 elem (ek), 0 attribútum|
|meghatározott|globálisan|
...
|típus|[InternalEndpointType](#internalendpointtype-complextype)|
...
|---|---|
|name|Kép|
|típus|[ImageType](#imagetype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
|Attribútum|Érték|
|---|---|
|tartalom|2 elem (ek), 7 attribútum|
|meghatározott|globálisan|
...
|---|---|
|name|Tanúsítványok|
|típus|[CertificatesType](#certificatestype-complextype)|
|minOccurs|0|
...
|---|---|
|name|Paraméter|
|típus|[ParameterType](#parametertype-complextype)|
|minOccurs|1|
|maxOccurs|unbounded|
...
Ez az érték határozza meg a "password" attribútumban megadott jelszó típusát. SecretsStoreRef/titkosított/egyszerű szöveges lehet.
Ha a SecretsStoreRef értékre van állítva, a rendszer lekéri a hivatkozási értéket a SecretStore.
Ha titkosított értékre van állítva, az alkalmazás fejlesztői feladata a tanúsítvány létrehozása és a Meghívási-ServiceFabricEncryptSecret parancsmag használata a bizalmas adatok titkosításához.
...
|Attribútum|Érték|
|---|---|
|tartalom|0 elem (ek), 12 attribútum|
|meghatározott|globálisan|
...
|Attribútum|Érték|
|---|---|
|tartalom|4 elem (ek), 2 attribútum|
|meghatározott|globálisan|
...
Egy szolgáltatási csoport felhasználó által definiált típusa, például: "ActorQueueSGType". A rendszer ezt az értéket használja a ApplicationManifest.xml fájlban a szolgáltatás csoportjának azonosításához.
...
|Attribútum|Érték|
|---|---|
|tartalom|11 elem (ek), 0 attribútum|
|meghatározott|globálisan|
...
#### <a name="packagesharingpolicy"></a>Packagesharingpolicy objektumnak
...
|típus|[ResourcesType](#resourcestype-complextype)|
|minOccurs|0|
...
|Attribútum|Érték|
|---|---|
|tartalom|11 elem (ek), 4 attribútum|
|meghatározott|globálisan|
...
|Attribútum|Érték|
|---|---|
|tartalom|5 elem (ek), 2 attribútum|
|meghatározott|globálisan|
|name|ServiceType|
...
#### <a name="servicetypename"></a>ServiceTypeName
A szolgáltatás felhasználó által definiált típusának azonosítója. Például: "QueueType" vagy "CalculatorType". Ezt az értéket használja a ApplicationManifest.xml fájlban a szolgáltatás azonosításához.
...
|Attribútum|Érték|
|---|---|
|tartalom|0 elem (ek), 11 attribútum|
|meghatározott|globálisan|
