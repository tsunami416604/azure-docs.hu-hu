...
|Attribútum|Érték|
|---|---|
|content|9-es elemeknek 0 attribútum(ok)|
|meghatározott|Globálisan|
...
#### <a name="version"></a>Verzió
a fürt jegyzékének dokumentum felhasználó által definiált verziószám-karakterlánc.
|Attribútum|Érték|
|---|---|
|név|Verzió|
|Használata|szükséges|
...
|Attribútum|Érték|
|---|---|
|content|3 elem(ek), 1 attribútum(ok)|
|meghatározott|Globálisan|
|név|CodePackageType|
...
Ha a tároló HEALTHCHECK engedélyezett és a docker jelentések health_status esemény a tároló, a Service Fabric a rendszer részeként tartalmazza jelentett állapotát. Amikor health_status állapota nem megfelelő a Service Fabric egy állapotfigyelési figyelmeztetése jelentést. Alapértelmezett érték igaz.
|Attribútum|Érték|
|---|---|
...
Ha a tároló HEALTHCHECK engedélyezett és a docker jelentések health_status esemény ezekhez a tárolókhoz, a Service Fabric újraindítja a tároló docker által jelentett health_status állapota nem megfelelő.
Alapértelmezett érték false értékre.
|Attribútum|Érték|
|---|---|
...
|Attribútum|Érték|
|---|---|
|content|9-es elem(ek), 5 attribútum(ok)|
|meghatározott|Globálisan|
...
A tároló elkülönítési üzemmódját. Érvényes értékek: alapértelmezett, a folyamat vagy a Hyper-v (csak Windows-tárolók támogatott) a Windows 10 gazdagép, csak a Hyper-V elkülönítési üzemmódját támogatja, így az elkülönítési üzemmódját függetlenül a Hyper-V elkülönítési módban fut. a tárolót.
|Attribútum|Érték|
|---|---|
|név|Elkülönítés|
|Használata|választható|
...
A forrásmappa, amely lehet egy mappa a virtuális gépen, amelyen a tárolók vagy egy állandó távoli tároló.
|Attribútum|Érték|
|---|---|
|név|Forrás|
|Használata|szükséges|
...
A forrás futó tárolóban hozzárendelt helyét. Így az a cél nem lehet egy helyet, amelyen belül a tároló már létezik.
|Attribútum|Érték|
|---|---|
|név|Cél|
|Használata|szükséges|
...
#### <a name="driver"></a>Illesztőprogram
Az Azure-fájlok kötet beépülő modul az illesztőprogram neve: "sfazurefile".
|Attribútum|Érték|
|---|---|
|név|Illesztőprogram|
|Használata|választható|
...
|Attribútum|Érték|
|---|---|
|content|3 elem(ek), 10 attribútum(ok)|
|meghatározott|Globálisan|
...
Illesztőprogram-beállítások átadását az illesztőprogramot. Az Azure-fájlok kötet beépülő modul támogatja a következő illesztőprogram-beállítások: megosztásnév (az Azure-fájlok megosztott fájl, amely a kötet biztosít a tároló) (az Azure storage-fiók, amely tartalmazza az Azure-fájlok fájlmegosztás), storageAccountName storageAccountKey () Hozzáférési kulcsot az Azure storage-fiók, amely tartalmazza az Azure-fájlok fájlmegosztás).
Ezen három illesztőprogram-beállítások szükségesek.
...
|Attribútum|Érték|
|---|---|
|content|18 elemeknek 0 attribútum(ok)|
|meghatározott|Globálisan|
...
Létrehozza az operációs rendszer verzióját. Például a build a Windows Server verzió 1709 verziója 16299.
|Attribútum|Érték|
|---|---|
...
Korlátozza az erőforrásokat, amely használható a gazdagépen, és egy szolgáltatás kódcsomag erőforrás korlátairól deklarálja.

|Attribútum|Érték|
|---|---|
|content|0 elem(ek), 9 attribútum|
|meghatározott|Globálisan|
...
A teljes memóriamennyiség swap használható, MB-ban. Pozitív egész számnak kell lennie.
|Attribútum|Érték|
|---|---|
|név|MemorySwapInMB|
|type|xs:String|
...
Az enyhe korlát (MB) memória irányításhoz kényszerített csak észlelésekor memória versengés a csomóponton. Pozitív egész számnak kell lennie.
|Attribútum|Érték|
|---|---|
...
Használható százaléka a rendelkezésre álló processzorok (csak Windows). Pozitív egész számnak kell lennie. Ha a szolgáltatáscsomag CPU-korlátok vannak megadva, ez a paraméter hatékonyan figyelmen kívül.
|Attribútum|Érték|
|---|---|
|név|CpuPercent|
|type|xs:String|
...
Maximális IO sebesség (olvasási és írási) használható IOps értékben. Pozitív egész számnak kell lennie.
|Attribútum|Érték|
|---|---|
|név|MaximumIOps|
|type|xs:String|
...
A maximális IO (bájt / mp) használható (olvasási és írási). Pozitív egész számnak kell lennie.
|Attribútum|Érték|
|---|---|
...
Blokk IO súly, más kód csomagok viszonyítva. 10 és 1000 közötti pozitív egész számnak kell lennie.
|Attribútum|Érték|
|---|---|
|név|BlockIOWeight|
|type|xs:String|
...
a szolgáltatáscsoport, például "ActorQueueSGType" felhasználói típus azonosítója Ez az érték a ApplicationManifest.xml fájlban azonosítására szolgál a csoportot.
|Attribútum|Érték|
|---|---|
...
|Attribútum|Érték|
|---|---|
|content|7 elemeknek 0 attribútum(ok)|
|meghatározott|Globálisan|
...
A szolgáltatás jegyzékfájlban egy strukturálatlan karakterlánc azonosítóját.
|Attribútum|Érték|
|---|---|
|név|ManifestId|
|Használata|választható|
...
|Attribútum|Érték|
|---|---|
|content|8 elem(ek), 4 attribútum(ok)|
|meghatározott|Globálisan|
...
#### <a name="servicetypename"></a>ServiceTypeName
a szolgáltatás felhasználói típus azonosítója Például a "QueueType" vagy "CalculatorType" lehetőséget. Ez az érték a szolgáltatás azonosítására szolgál a ApplicationManifest.xml fájlban.
|Attribútum|Érték|
|---|---|
|név|ServiceTypeName|
|Használata|szükséges|
...
|Attribútum|Érték|
|---|---|
|content|0 elemeknek 11 attribútum(ok)|
|meghatározott|Globálisan|
