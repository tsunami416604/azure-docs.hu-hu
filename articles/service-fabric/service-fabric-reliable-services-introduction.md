...
Tegyük fel, hogy olyan szolgáltatást szeretnénk írni, amely képeket dolgoz fel. Ennek elvégzéséhez a szolgáltatás egy rendszerképet és az adott képen végrehajtandó konverziók sorozatát veszi igénybe. Ez a szolgáltatás egy kommunikációs figyelőt ad vissza (tegyük fel, hogy ez egy WebAPI), amely egy `ConvertImage(Image i, IList<Conversion> conversions)`hasonló API-t tesz elérhetővé. Amikor kérelmet kap, a szolgáltatás tárolja azt a-ben `IReliableQueue`, és visszaad egy azonosítót az ügyfélnek, hogy nyomon tudja követni a kérést.
...
Bár ez a szolgáltatás egy jellemző .NET-szolgáltatáshoz hasonlóan hangzik, a különbség az, hogy a használt`IReliableQueue` adatstruktúrákat (és `IReliableDictionary`) a Service Fabric biztosítják, és megbízható, elérhető és konzisztensek.
