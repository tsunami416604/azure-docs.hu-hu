...
1.  安裝和更新 AzureRM 模組。
    ...
    1.  安裝 「 AzureRM 」 模組使用命令 `安裝模組 AzureRM`。
    1.  更新使用命令的 AzureRM 模組 `更新 AzureRM`。
1.  停用 (或啟用) Azure 資料收集。
...
1.  登入 Azure PowerShell。
    1. 啟動系統管理員 PowerShell 提示字元並執行命令 `登入 AzureRMAccount`。
    1. 在顯示的對話方塊中，輸入您的 Azure 認證。
    1. 執行命令 `Get AzureRmSubscription`。
    1. 找出您想要使用，然後再執行命令的訂閱 `選取 AzureRmSubscription SubscriptionId < 您的訂閱識別碼 >`。 」
...
1.  建立目錄 `%SYSTEMDRIVE%\Windows\ServiceProfiles\LocalService\AppData\Roaming\NuGet` 組建電腦 (如果尚未存在)。
2.  如果已經呼叫 NuGet.config 檔案存在於這個目錄中，開啟它，並新增下列元素的子 `< packageSources >`。

    ```
    <add key="Service Fabric SDK" value="<path to service fabric SDK>\packages" />`
    ```

3.  如果 NuGet.config 不存在，則以下列內容建立。取代 `< service fabric SDK 的路徑 >` Service Fabric SDK 的建置電腦上的路徑。
>[AZURE.NOTE] 根據預設， `< service fabric SDK 的路徑 >` 是 `%ProgramFiles%\Microsoft SDKs\Service Fabric`。
...
3. 輸入 `PSModulePath` 做為名稱，以及 `%PROGRAMFILES%\WindowsPowerShell\Modules` 的值。
...
1.  下載 agent.zip。 作法：
    ...
    1.  解壓縮至 agent.zip `C:\agent` (或使用短路徑的任何位置) 組建電腦上。
        >[AZURE.NOTE] 如果您打算建置 ASP.NET 5 Web 服務，建議您選擇最短的名稱可能避免遭遇此資料夾 **PathTooLongExceptions** 部署期間發生的錯誤。

1.  系統管理員 PowerShell 命令提示字元中，執行 `C:\agent\ConfigureAgent.ps1`。 指令碼會提示您下列參數：

    | 參數| 值|
    |---|---|
    | 代理程式名稱| 接受預設值， `代理程式的 [電腦名稱]`。)
    | TFS Url| 例如，您的 team 專案輸入的 URL `https://[your-VSTS-account-name].visualstudio.com`。
...
1. 請確認您的組建代理程式已註冊。 作法：
    ...
    1. 回到網頁瀏覽器 (應該會在頁面 `https://[your-VSTS-account-name].visualstudio.com/_admin/_AgentPool`)，然後重新整理頁面。
...
1.  在 [變數]**** 索引標籤上，使用這些值建立下列變數。
    ...
    | ServiceFabricPublishProfilePath| `< Path/to/your/publish/profiles/MyPublishProfile.xml >` 附註: 發行設定檔中的連接端點將會被忽略。會改為使用暫存叢集的連接端點。| | |
    | ServiceFabricDeploymentScriptPath| `< > path/to/Deploy-FabricApplication.ps1`| | |
    | ServiceFabricApplicationProjectPath| `< 路徑/至/您/結構/應用程式/專案/資料夾 >` 這應該是包含.sfproj 檔案的資料夾。| | | |
...
- [建立文件首頁](https://msdn.microsoft.com/Library/vs/alm/Build/overview)
- [部署的組建代理程式](https://msdn.microsoft.com/Library/vs/alm/Build/agents/windows)
