...
* **Timetorun érték**: összesen futásakor a káosz, sikeres befejezése előtt. A Chaos előtt futott a timetorun érték időszakra vonatkozóan a StopChaos API-n keresztül is leállíthatja.

* **MaxClusterStabilizationTimeout**: a fürt előtt egy ValidationFailedEvent előállító állapotúak lesznek várakozási idő maximális mennyisége. Ezt várja meg, hogy a fürt terhelésének csökkentése közben helyreállítása folyamatban van. Az elvégzett a következők:
...
* **MaxConcurrentFaults**: párhuzamos, amely minden egyes ismétléskor előidézett hibák maximális száma. A magasabb az érték, annál több agresszív káosz és a feladatátvételeket, és az állapot átmenet kombinációit, amely végighalad a fürt összetettebbek is. 
...
* **WaitTimeBetweenIterations**: ismétléseinek közötti várakozási idő. Azt jelenti mennyi ideig káosz szüneteltetni fogja a hibák és kellene a fürt állapota megfelelő érvényesítése befejeződött egy ciklikus végrehajtása után. A magasabb, az alacsonyabb érték átlagos tartalék injektálási sebessége.
* **WaitTimeBetweenFaults**: egyetlen ismétléskor két egymást követő hibák közötti várakozási idő. A magasabb az érték, az alacsonyabb az egyidejűségi (vagy az átfedett) hibákat.
* **ClusterHealthPolicy**: fürt állapotházirend káosz ismétléseinek között a fürt állapotának ellenőrzésére szolgál. Ha a fürt állapota hibás, vagy ha váratlan kivétel történt a tartalék végrehajtás során történik, a Chaos Várjon 30 percet, mielőtt a következő állapot-ellenőrzés – recuperate hosszabb időt biztosít a fürt.
