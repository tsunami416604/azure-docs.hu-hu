...
以下範例說明上述概念。 請考慮實作兩個非同步方法的動作項目類型 (假設 *Method1* 和 *Method2*)，是計時器與提醒。 下圖顯示代表兩個動作項目-執行這些方法與回撥的時間軸範例 *ActorId1* 和 *ActorId2* -屬於此動作項目類型。
...
- 當 *Method1* 代表正在執行 *ActorId2* 用戶端要求回應中 *xyz789*, ，另一個用戶端要求 *abc123* 到達時，也需要 *Method1* 所要執行 *ActorId2*。 不過，後者執行 *Method1* 前者執行完成後才會開始。 同樣地，所註冊的提醒 *ActorId2* 時引發 *Method1* 正在執行中用戶端要求回應 *xyz789*。 提醒回撥會執行這兩個執行後才 *Method1* 都已完成。 這是因為的強制執行回合式並行 *ActorId2*。
- 同樣地，也強制回合式並行執行的 *ActorId1*, ，如執行示範 *Method1*, ，*Method2* 和計時器回撥，代表 *Method1* 以序列方式發生。
- 執行 *Method1* 代表 *ActorId1* 代表其執行與重疊 *ActorId2*。 這是因為回合式並行只會在動作項目內強制執行，不會在動作項目間強制執行。
...
### 可設定狀態的動作項目
可設定狀態動作項目具有需要的記憶體回收與容錯移轉之間保留的狀態。 它們衍生自 `StatefulActor<TState>`，其中`TState` 是必須保留的狀態類型。 透過基底類別的 `State` 屬性可在動作項目方法中存取狀態。  
...
[網狀架構動作項目如何使用 Service Fabric 平台](service-fabric-reliable-actors-platform.md)
