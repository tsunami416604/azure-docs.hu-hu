...
`InstanceCount` a szolgáltatáshoz meg kell adni a futtatni kívánt példányok kívánt számát. Ha a példányok bármelyike (vagy mindegyike) meghibásodik, a Service Fabric a helyettesítő példányok automatikus létrehozásával válaszol a többi csomóponton. Service Fabric továbbra is a visszaállításokat fogja létrehozni, amíg a szolgáltatás vissza nem áll a kívánt példányszámra.
Egy másik példa, ha az állapot nélküli szolgáltatás rendelkezik a-1 `InstanceCount`, ami azt jelenti, hogy az egyik példánynak futnia kell a fürt mindegyik csomópontján. Ha az ilyen példányok némelyike sikertelen volt, Service Fabric felismeri, hogy a szolgáltatás nem a kívánt állapotban van, és megpróbálja létrehozni a példányokat azokon a csomópontokon, amelyeken hiányoznak.

#### <a name="stateful-services"></a>Állapot-nyilvántartó szolgáltatások
Kétféle állapot-nyilvántartó szolgáltatás létezik:
1. Állapot-nyilvántartó tartós állapottal
2. Nem megőrzött állapottal rendelkező állapot (az állapot tárolása a memóriában történik)

Az állapot-nyilvántartó szolgáltatás meghibásodásának helyreállítása az állapot-nyilvántartó szolgáltatás típusától, valamint a szolgáltatás replikáinak számával és a sikertelen műveletekkel függ.

Mi a kvórum?
Egy állapot-nyilvántartó szolgáltatásban a bejövő adatértékek replikálódnak a replikák között (elsődleges és ActiveSecondary). Ha a replikák többsége megkapja az adatmennyiséget, a rendszer az adatkvórumként véglegesíti az adatmennyiséget (5 replika esetén 3 lesz _kvórum_). Ez annyit jelent, hogy a replikák legalább kvóruma a legfrissebb adattal rendelkezik. Ha a replika (ok) meghiúsul (azaz az 5 replika közül 2 nem sikerült), a kvórum értéke alapján kiszámíthatja, hogy a helyreállítás visszaállítható-e (mivel az 5 replika közül fennmaradó 3 még mindig fennáll, garantálható, hogy legalább 1 replika teljes adatokat fog tartalmazni).

Mi a kvórum elvesztése?
Ha a replikák kvóruma meghiúsul, a partíció úgy van deklarálva, hogy kvórum elvesztése állapotban legyen. Tegyük fel, hogy egy partíciónak 5 replikája van, ami azt jelenti, hogy legalább 3 a teljes adatmennyiség garantált. Ha a replikák kvóruma (3 – 5) sikertelen, akkor Service Fabric nem tudja meghatározni, hogy a fennmaradó replikák (2 ki 5) elegendő adattal rendelkeznek-e a partíció visszaállításához.

Annak megállapítása, hogy vészhelyzet történt-e egy állapot-nyilvántartó szolgáltatás esetében, és a kezelése három szakaszt követ:
...
2. Annak megállapítása, hogy a kvórum elvesztése állandó-e
   - Az idő nagy részében a hibák átmenetiek. A folyamatok újraindulnak, a csomópontok újraindulnak, a virtuális gépek újraindulnak, és a hálózati partíciók meggyógyulnak. Néha azonban a hibák állandóak. 
     - A megőrzött állapot nélküli szolgáltatások esetében a kvórum vagy a több replika meghibásodása _azonnal_ visszavezeti az állandó kvórum elvesztését. Ha a Service Fabric állapot-nyilvántartó nem állandó szolgáltatásban észleli a kvórum elvesztését, akkor a (lehetséges) adatvesztés bejelentésével azonnal továbblép a 3. lépésre. Az adatvesztéssel való továbblépés miatt Service Fabric tudja, hogy a replikák visszaállítására nincs lehetőség arra az esetre, ha a szolgáltatás nem megőrzött jellege miatt nem szűnik meg a visszaállítás, mert az adatok elvesztek.
     - Az állapot-nyilvántartó állandó szolgáltatások esetében a kvórum vagy több replika meghibásodása miatt Service Fabric várnia kell, amíg a replikák vissza nem állnak, és vissza lehet állítani a kvórumot. Ennek eredményeképpen a szolgáltatás leállást eredményez a szolgáltatás érintett partíciójának (vagy a "replikakészlet-készletnek") _írásakor_ . Az olvasás azonban továbbra is lehetséges a korlátozott konzisztencia-garanciákkal. Az az időtartam, ameddig Service Fabric a kvórum visszaállítására vár a végtelen, mivel a folytatás egy (lehetséges) adatvesztési esemény, és egyéb kockázatokat hordoz.
...
A replikák átmeneti meghibásodás miatt nem lehetnek időnként. Várjon egy ideig, amíg Service Fabric megpróbálja felvenni őket. Ha a replikák a vártnál több ideig vannak lefoglalva, kövesse az alábbi hibaelhárítási lépéseket.
- A replikák összeomlást okoznak. Győződjön meg arról, hogy a replika szintjének állapotáról készült jelentések és az alkalmazás naplói. Összeomlási memóriaképek gyűjtése és a helyreállításhoz szükséges műveletek elvégzése.
- A replika folyamata nem válaszol. Ellenőrizze az alkalmazás naplóiban, hogy ennek ellenőrzéséhez. Gyűjtse össze a folyamat memóriaképét, majd a nem válaszoló folyamatot. Service Fabric létre fog hozni egy helyettesítő folyamatot, és megkísérli a replikát visszahozni.
- A replikákat üzemeltető csomópontok leállíthatók. Indítsa újra a mögöttes virtuális gépet, hogy a csomópontok fel legyenek vezetve.

Előfordulhatnak olyan helyzetek, amikor a replikák nem állíthatók helyre például lehetséges, hogy a meghajtók sikertelenek voltak, vagy a gépek fizikailag nem válaszolnak. Ezekben az esetekben Service Fabric kell arról, hogy ne várjon a replika helyreállítására.
Ne használja ezeket a módszereket, ha a potenciális adatvesztés nem fogadható el a szolgáltatás online állapotba állításához, amely esetben a fizikai gépek helyreállítására irányuló összes erőfeszítést el kell végezni.

A következő lépések a dataloss eredményezik:
   
> [!NOTE]
> Ezeket a metódusokat _soha nem_ lehet biztonságos módon használni, mint az adott partíciók esetében. 
>

- Használjon `Repair-ServiceFabricPartition -PartitionId` vagy `System.Fabric.FabricClient.ClusterManagementClient.RecoverPartitionAsync(Guid partitionId)` API-t. Ez az API lehetővé teszi a partíció AZONOSÍTÓjának megadását a QuorumLoss a lehetséges adatvesztéssel való helyreállításhoz.
- Ha a fürt gyakori hibákba ütközik, ami miatt a szolgáltatások a kvórum elvesztése állapotba kerülnek, és a potenciális _adatvesztés elfogadható_, akkor a megfelelő [QuorumLossWaitDuration](https://docs.microsoft.com/powershell/module/servicefabric/update-servicefabricservice?view=azureservicefabricps) érték megadásával a szolgáltatás automatikusan helyreállítható. Service Fabric várnia kell a megadott QuorumLossWaitDuration (az alapértelmezett érték a végtelen) a helyreállítás végrehajtása előtt. Ez a metódus _nem ajánlott_ , mert ez váratlan adatvesztést eredményezhet.
