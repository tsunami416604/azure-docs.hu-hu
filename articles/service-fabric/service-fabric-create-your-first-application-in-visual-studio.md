...
2. 啟動應用程式後，Visual Studio 會自動顯示 [診斷事件檢視器]，以便查看服務的追蹤輸出。
    ...
    在具狀態服務範本中，訊息只是要顯示在遞增此計數器值 `RunAsync` MyStatefulService.cs 方法。
...
3. 展開其中一個事件以查看更多詳細資料，包括程式碼執行所在的節點 - 在此情況下是節點 2，雖然這在您的電腦上可能有所不同。
    ...
    >[AZURE.NOTE] 專案範本所發出的應用程式診斷事件使用包含 `ServiceEventSource` 類別。 如需詳細資訊，請參閱 [如何監視和診斷服務在本機](service-fabric-diagnostics-how-to-monitor-and-diagnose-services-locally)
...
- [請參閱如何將公開到網際網路而 WebAPI 服務](service-fabric-add-a-web-frontend.md)
- [了解如何在 Azure 中建立叢集](service-fabric-cluster-creation-via-portal.md)
- [了解如何建立可靠的服務](service-fabric-reliable-services-quick-start.md)
- [嘗試建立使用可靠動作項目程式設計模型的服務](service-fabric-reliable-actors-get-started.md)
