...
注意: 您沒有建立 `config` 和 `資料` 目錄萬一您不需要它們。
...
>[AZURE.NOTE]：我們提供可讓您自動建立 ApplicationPackage 的封裝工具。 此工具目前為預覽狀態。 您可以下載從 [這裡](http://aka.ms/servicefabricpacktool)。
...
- 您可以選擇任何您想要的名稱 `ServiceTypeName`, ，值可以用於 `ApplicationManifest.xml` 來識別服務。
...
`名稱` 項目用來在應用程式套件，其中包含服務的程式碼中指定的目錄名稱。 `CodePackage` 也有 `版本` 屬性，可以用來指定版本的程式碼，並可能用來升級服務的程式碼使用 Service Fabric ALM 基礎結構。
### SetupEntrypoint
...
```
SetupEntrypoint 用來指定任何應在服務的程式碼啟動前執行的可執行檔或批次檔。 這是選擇性元素，所以如果不需要初始化/安裝程式則不需納入該元素。 每次重新啟動服務時，就會執行 SetupEntrypoint。 只有一個 SetupEntrypoint，所以如果應用程式的 setup/config 需要多個指令碼，則必須在單一批次檔上搭配 setup/config 指令碼。 如同 Entrypoint 元素，SetupEntrypoint 可以執行任何類型的檔案：可執行檔、批次檔、PowerShell Cmdlet。 在上述範例中，t 根據位於批次檔 launchConfig.cmd `指令碼` (假設 WorkingDirectory 項目設定為程式碼) 的程式碼目錄的子目錄。
...
`Entrypoint` 服務資訊清單檔中的項目用來指定如何啟動服務。  `ExeHost` 項目會指定可執行檔 (和引數)，應該用來啟動服務。

- `程式`: 指定應執行才能啟動該服務的可執行檔的名稱。
- `引數`: 它會指定應該傳遞至可執行檔的引數。 它可以是具有引數的參數清單。
- `WorkingFolder`: 它會指定要啟動的處理程序的工作目錄。 您可以指定兩個值：
    - `程式碼基底`: 工作目錄即將設定為應用程式封裝中的程式碼目錄 (`程式碼` 目錄中的結構如下所示)
    - `CodePackage`: 工作目錄會設定為應用程式封裝的根目錄 (`MyServicePkg`)
- `WorkingDirectory` 元素適合用於設定正確的工作目錄，因此可以由應用程式或初始化指令碼使用相對路徑。
...
```
`端點` 項目會指定應用程式可以接聽的端點。 在此範例中，Node.js 應用程式會接聽連接埠 3000。

## 應用程式資訊清單檔案

一旦您已設定 `servicemanifest.xml` 檔案，您需要進行一些變更 `ApplicationManifest.xml` 檔案，以確保使用正確的服務型別和名稱。
...
在 `ServiceManifestImport` 您可以指定您想要包含在應用程式中的一個或多個服務。 服務使用參考 `ServiceManifestName` 指定的目錄名稱的地方 `ServiceManifest.xml` 檔案所在的。
...
可在設定主控台重新導向 `ServiceManifest.xml` 檔案中使用 `ConsoleRedirection` 項目
...
* `ConsoleRedirection` 可用來讓它們可以用來確認安裝程式或在 Service Fabric 叢集中的應用程式執行期間沒有任何錯誤，將主控台輸出 (stdout 和 stderr) 重新導向到工作目錄。

    * `FileRetentionCount` 判斷多少檔案會儲存在工作目錄。 比方說，值為 5 表示工作目錄中會儲存先前 5 次執行的記錄檔。
...
`InstanceCount` 參數 `New-servicefabricservice` 指令程式用來指定應在 Service Fabric 叢集中啟動多少個服務執行個體。 您可以設定 `InstanceCount` 視您要部署的應用程式的值。 兩個最常見的案例包括：
* `InstanCount ="1"`: 在此情況下只能有一個服務執行個體將在叢集上部署。 Service Fabric 的排程器會決定即將部署服務的節點。

* `InstanceCount ="-1"`: 一個服務執行個體將在此情況下部署 Service Fabric 叢集中的每個節點上。 最後的結果會是叢集中的每個節點有一個 (且僅只一個) 服務執行個體。 這是很有用的前端應用程式 (例如設定。 REST 端點) 因為用戶端應用程式只需要 「 連線 」 到任何叢集中的節點即可使用端點。 當 Service Fabric 叢集的所有節點都連線到負載平衡器時，也可使用此組態，以便將用戶端流量分散於在叢集中所有節點上執行的服務。
