...
1. 若要呼叫 `ServiceEventSource.Current.ServiceMessage` 中 `RunAsync` 方法顯示的自訂 ETW 追蹤的範例應用程式程式碼。
...
2. 尋找程式碼 `ActorEventSource.Current.ActorMessage (此，「 執行工作 」);` 中 *DoWorkAsync* 方法。 這是來自應用程式程式碼撰寫的自訂 ETW 追蹤範例。
