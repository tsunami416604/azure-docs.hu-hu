...
無狀態服務是沒有任何應用程式服務 [本機永續性狀態](service-fabric-concepts-state.md)。
...
可設定狀態服務具有某些與其相關聯的狀態。 在 Service Fabric 中，可設定狀態服務會模型化為複本集。 每個複本是一種服務程式碼的執行個體，其中具有一個狀態複本。 用來執行讀取和寫入作業的複本 (稱為主要複本)。 由於寫入作業的狀態變更是 *複寫* 到多個其他複本 (稱為作用中次要複本)。 主要和作用中次要複本的組合即為服務中的複本集。

僅能使用一個主要複本提供讀取和寫入要求服務。 但可以使用多個作用中次要複本。 作用中次要複本的數目可供設定，且較高的複本數可容許較多的並行軟體和硬體失敗。

若發生失敗 (主要複本當機) 時，Service Fabric 會讓其中一個作用中次要複本成為新的主要複本。 此作用中次要複本已經有狀態的更新的版本 (透過 *複寫*) 且能夠繼續處理進一步的讀取和寫入作業。
...
>[AZURE.NOTE] 更高階的程式設計模型，例如 [可靠動作項目架構](service-fabric-reliable-actors-introduction.md) 抽離複本角色，從開發人員的概念。
