...
**服務執行個體** -已針對服務類型產生的程式碼。 每個服務執行個體有唯一的名稱開頭為 `fabric: /` 而且與特定已命名的應用程式執行個體相關聯。
...
- 可靠服務-建置無狀態與具狀態服務為基礎的 API `StatelessService` 和 `StatefulService` .NET 類別，將狀態儲存在.NET 可靠集合 (字典及佇列)，並能插入各種不同的通訊堆疊，例如 Web API 和 WCF。 此程式設計模型適用於需要跨多個狀態單位執行計算作業的應用程式。
