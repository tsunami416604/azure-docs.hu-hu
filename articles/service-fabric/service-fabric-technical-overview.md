...
**叢集** -網路連線的一組虛擬或實體機器部署的應用程式執行個體。  叢集可擴充至數千部機器。

**節點** -在叢集中可定址的單位。 節點具有放置屬性和唯一識別碼等特性。 節點可以加入叢集，在 Fabric.exe 執行的作業系統上，與其執行個體相互關聯。

**應用程式 / 應用程式類型** -(微) 服務的集合。 應用程式類型可視為一或多個服務類型的容器。  請參閱 [應用程式模型](service-fabric-application-model.md) 文章，了解如何在叢集 (其本身包含多個節點) 可能包含多個應用程式類型組成。

**服務 / 服務類型** -程式碼和執行獨立函數 (它可以啟動並單獨執行)，例如，佇列服務或資料庫服務的組態。 應用程式類型可能包含一或多個服務類型。 服務類型共有兩種：
...
**應用程式執行個體** -在叢集中，您可以建立許多應用程式執行個體的應用程式類型，每一個都有特定的名稱。 每個應用程式執行個體都能單獨管理，脫離相同或不同類型的其他應用程式執行個體獨自建立版本。 此外，應用程式執行個體也會定義資源和安全性隔離。

**服務執行個體** -已針對服務類型產生的程式碼。 每個服務執行個體都有以 `fabric:/` 開頭的唯一名稱，且會與已命名的特定應用程式執行個體相互關聯。

**應用程式封裝** -服務程式碼封裝與組態檔的集合，兩者結合用於特定應用程式。 這些是已部署的實體檔案，且只是檔案和資料夾格式配置而已。 例如，電子郵件應用程式的應用程式套件可能包含佇列服務封裝、前端服務封裝以及資料庫服務封裝。

**程式設計模型** -有可用來建置應用程式的 Service Fabric 中的兩種程式設計模型:
