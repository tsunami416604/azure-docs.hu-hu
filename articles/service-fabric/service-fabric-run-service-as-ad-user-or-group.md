...
Az alábbi példa egy *tesztfelhasználó* nevű Active Directory felhasználót mutat be a tartomány jelszavával, amelyet a *MyCert*nevű tanúsítvány használatával titkosított. A titkos titkosítási szöveg `Invoke-ServiceFabricEncryptText` létrehozásához a PowerShell-parancs használható. További részletekért lásd: [a titkok kezelése Service Fabric alkalmazásokban](service-fabric-application-secret-management.md) .
