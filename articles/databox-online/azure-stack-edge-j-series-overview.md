...
ms.locfileid: "90985651"
---
# <a name="what-is-the-azure-stack-edge-pro-r"></a>Mi az a Azure Stack Edge Pro R?

Az Azure Stack Edge Pro R egy hardveres szolgáltatásként szolgáló megoldás. A Microsoft az Azure-ba irányuló hálózati adatátvitel során tartós, robusztus, kiszolgáló osztályú, peremhálózati eszközt biztosít. Grafikus processzorral (GPU) van ellátva, ezek az eszközök mesterséges intelligenciával, elemzéssel és kiszolgáló nélküli számítástechnikagal vannak optimalizálva. A robusztus eszközök megfelelőek a zord környezetekben való használatra.
...
|Képesség |Leírás  |
|---------|---------|
|Robusztus hardver| Robusztus kiszolgálói osztályú, kemény környezetekhez tervezett hardverek. Hordozható eszköz 2 személyes Carry ügyben. |
...
|Támogatott protokollok     |A standard SMB, NFS és REST protokollok támogatása az adatok betöltéséhez. <br> A támogatott verziókról a [Azure stack Edge Pro R rendszerkövetelményei](azure-stack-edge-gpu-system-requirements.md)című témakörben olvashat bővebben.|
...
- **Azure stack Edge Pro r fizikai eszköz** – a Azure stack Edge Pro r egy 1 csomópontos eszköz, amely úgy konfigurálható, hogy az Azure-ba küldje az adatküldést. Az eszköz egy 1U-kiszolgáló, amely a Microsoft által biztosított robusztus encasing rendelkezik. Szükség esetén a kiszolgáló egy UPS-vel is elérhető (Emellett 1U is).
...
- **Azure stack Edge Pro R local webes felhasználói felület** – a helyi webes felhasználói felület használata a kezdeti eszköz konfigurálásához, a diagnosztika futtatásához, a Azure stack Edge Pro r-eszköz kikapcsolásához, a másolási naplók megtekintéséhez és a Microsoft ügyfélszolgálata kapcsolatfelvételhez.
