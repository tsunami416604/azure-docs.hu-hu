---
title: Az Azure Stack Edge-eszköz visszaküldése vagy cseréje | Microsoft Docs
description: Az Azure Stack Edge-eszköz visszaküldését vagy cseréjét ismerteti.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
Ez a cikk az adatok törlését és az Azure Stack Edge-eszköz visszaküldését ismerteti. Az eszköz visszaadása után törölheti az eszközhöz társított erőforrást, vagy megrendelheti a helyettesítő eszközt.
...
1. Nyisson meg egy támogatási jegyet Microsoft ügyfélszolgálata jelezve, hogy vissza kívánja adni az eszközt. Válassza ki a probléma típusát **Azure stack Edge hardverként**.
    ...
    ![Támogatási jegy megnyitása](media/azure-stack-edge-return-device/open-support-ticket-1.png)  
...
2. [Hozzon létre egy új erőforrást](azure-stack-edge-deploy-prep.md#create-a-new-resource) a helyettesítő eszközhöz. Ügyeljen arra, hogy jelölje be a jelölőnégyzetet a **Azure stack Edge-eszközön**. 
3. Miután megkapta a helyettesítő eszközt, [telepítse](azure-stack-edge-deploy-install.md) és [aktiválja](azure-stack-edge-deploy-connect-setup-activate.md) a helyettesítő eszközt az új erőforráson.
