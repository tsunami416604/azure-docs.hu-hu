---
title: Virtuális gépek üzembe helyezése Azure Stack Edge Pro GPU-eszközön Azure PowerShell parancsfájl használatával
description: Ismerteti, hogyan lehet virtuális gépeket (VM-ket) létrehozni és kezelni egy Azure Stack Edge Pro-eszközön egy Azure PowerShell parancsfájl használatával.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
1. Győződjön meg arról, hogy módosította a következőket: 
...
5. [Töltse le a AzCopy 10](../storage/common/storage-use-azcopy-v10.md#download-azcopy) rendszert a Windows-ügyfélre. Jegyezze fel ezt a helyet, mert a parancsfájl futtatásakor paraméterként adja át.
...
1. Nyissa meg azt a mappát, amelybe letöltötte a parancsfájlt az ügyfélen.
1. A parancsfájl futtatása előtt győződjön meg arról, hogy továbbra is csatlakozik az eszköz helyi Azure Resource Managerhoz, és a kapcsolat nem járt le.
...
1. A parancsfájl futtatásához használja a következő parancsot:
    ...
    Ha azt szeretné, hogy az IP-cím dinamikusan legyen kiosztva a virtuális géphez, hagyja ki a `-NicPrivateIp` paramétert.
    ...
    **Windows rendszerű virtuális gépek esetén:**
    ...
    Íme egy példa a létrehozott Windows rendszerű virtuális gép kimenetére.
    ...
    **Linux rendszerű virtuális gépek esetén:**
    ...
    Itt látható a Linux rendszerű virtuális gép létrehozásához használt parancs mintája.
...
1. Miután sikeresen létrehozta a virtuális gépeket, ezeknek a virtuális gépeknek meg kell jelennie a Azure Portalban lévő Virtual Machines listájában. Ha meg szeretné tekinteni a virtuális gépeket, az Azure Portal Azure Stack peremhálózati erőforrásában keresse fel az **Edge services > Virtual Machines szolgáltatást**. 

    ![Virtuális gépek listájának megtekintése](media/azure-stack-edge-gpu-deploy-virtual-machine-powershell-script/list-virtual-machine-1.png)

    A virtuális gép részleteinek megtekintéséhez válassza ki a virtuális gép nevét. Jegyezze fel a virtuális gép IP-címének dinamikus kiosztását.

    ![Virtuális gép részleteinek megtekintése](media/azure-stack-edge-gpu-deploy-virtual-machine-powershell-script/view-virtual-machine-details-1.png)

1. A parancsfájl által létrehozott erőforrások törléséhez használja a következő parancsokat:
...
[Virtuális gépek üzembe helyezése Azure PowerShell-parancsmagok használatával](azure-stack-edge-gpu-deploy-virtual-machine-powershell.md)