...
<tags ms.service="virtual-machines" ms.devlang="multiple" ms.topic="article" ms.tgt_pltfrm="vm-linux" ms.workload="infrastructure-services" ms.date="10/27/2014" ms.author="rasquill"></tags>
...
This topic describes [CoreOS][CoreOS] and shows how to create a cluster of three CoreOS virtual machines on Azure as a quick start to understanding CoreOS. It uses the very basic elements of CoreOS deployments and examples from [CoreOS with Azure][CoreOS with Azure], [Tim Park's CoreOS Tutorial][Tim Park's CoreOS Tutorial], and [Patrick Chanezon's CoreOS Tutorial][Patrick Chanezon's CoreOS Tutorial] to demonstrate the absolute minimum requirements to both understand the basic structure of a CoreOS deployment and get a cluster of three virtual machines running successfully.

<!--Table of contents for topic, the words in brackets must match the heading wording exactly-->

This topic contains the following sections:

-   [CoreOS, Clusters, and Linux Containers][CoreOS, Clusters, and Linux Containers]
-   [Security Considerations][Security Considerations]
-   [How to use CoreOS on Azure][How to use CoreOS on Azure]
-   [Next steps][Next steps]

## <span id="intro"></span>CoreOS, Clusters, and Linux Containers</a>

CoreOS is a lightweight version of Linux designed to support rapid creation of potentially very large clusters of VMs that use Linux containers as the only packaging mechanism, including [Docker][Docker] containers. CoreOS is intended to support:
...
3.  Built-in [etcd][etcd] and [fleet][fleet] daemons (services) for dynamic VM and cluster communication and management

This is a very general description of CoreOS and its features. For more complete information about CoreOS, see the [CoreOS Overview][CoreOS Overview].

## <span id="security"></span>Security Considerations</a>
...
## <span id="usingcoreos"></span>How to use CoreOS on Azure</a>

This section describes how to create an Azure Cloud Service with three CoreOS virtual machines in it using the [Azure Cross-Platform Interface (xplat-cli)][Azure Cross-Platform Interface (xplat-cli)]. The basic steps are as follows
...
3.  Create a cloud-config file in [YAML][YAML] format
...
Use the instructions in [How to Use SSH with Linux on Azure][How to Use SSH with Linux on Azure] to create a public and private key for SSH. (The basic steps are in the instructions below.) You are going to use these keys to connect to VMs in the cluster to verify that they are working and can communicate with each other.
...
(For more complete information about the cloud-config file, see [Using Cloud-Config][Using Cloud-Config] in the CoreOS documentation.)

### Use the xplat-cli to create a new CoreOS VM

<!--Every topic should have next steps and links to the next logical set of content to keep the customer engaged-->

1.  Install the [Azure Cross-Platform Interface (xplat-cli)][Azure Cross-Platform Interface (xplat-cli)] if you have not already done so, and either login using a work or school ID, or download a .publishsettings file and import that into your account.
...
One note: If you observe your work so far in the [new portal][new portal], you'll find your Cloud Service name is both a resource group and domain, as the following image shows:

![][0]

1.  Connect to your cloud service and create a new CoreOS vm inside by using the **azure vm create** command. You will pass the location of your X.509 certificate in the **--ssh-cert** option. Create your first VM image by typing the following, remembering to replace **coreos-cluster** with the cloud service name that you created:
...
1.  Create the second node by repeating the command in step 4, replacing the **--vm-name** value with **node-2** and the **--ssh** port value with 2022.

2.  Create the third node by repeating the command in step 4, replacing the **--vm-name** value with **node-3** and the **--ssh** port value with 3022.
...
> [WACOM.NOTE] If you are already using the **~/.ss h/id\_rsa** key, then add that with `ssh-add ~/.ssh/id_rsa`.
...
You should now have a running three-node CoreOS cluster on Azure. From here, you can explore how to create more complex clusters and use Docker and create more interesting applications by reading [Tim Park's CoreOS Tutorial][Tim Park's CoreOS Tutorial], [Patrick Chanezon's CoreOS Tutorial][Patrick Chanezon's CoreOS Tutorial], [Docker][Docker] documentation, and the [CoreOS Overview][CoreOS Overview].
