...
9. 選取 Azure 網站作為您的發行目標。 使用您的 Azure 帳戶登入，並選取現有或新的網站，記下 [連線]**** 索引標籤中的 [目的地 URL]**** 屬性。。 我們後續將在本教學課程中參考此 URL 作為您的 *backend endpoint*。 按一下 [發行]****。
...
15. 接著，將下列方法加入至 **AppDelegate.m**，以擷取您的端點影像，並在擷取完成時傳送本機通知。 請務必取代預留位置 `{後端端點}` 與後端端點:
