...
在 Windows 市集和 Windows Phone 應用程式中，通知可以是_「推播」_形式：具有音效的非強制回應視窗出現，以指出有新的通知。 支援其他通知類型 (包括_「磚」_、_「原始」_和_「徽章」_通知)。 如需有關 Windows 裝置，請參閱 < 支援的通知類型的 [磚、 徽章和通知](http://msdn.microsoft.com/library/windows/apps/hh779725.aspx)。

在 Apple iOS 裝置上，推播同樣會以對話方塊的形式通知使用者，而要求使用者檢視或關閉通知。 按一下 [檢視]**** 即會開啟接收訊息的應用程式。 如需 iOS 通知，請參閱 [iOS 通知](http://go.microsoft.com/fwlink/?LinkId=615245)。
...
推播通知可透過名為 _Platform Notification Systems_ (PNS) 的平台特定基礎結構來傳遞。 PNS 可提供準系統功能 (即不支援廣播和個人化的系統)，不具共同介面。 例如，若要傳送通知給 Windows 市集應用程式，開發人員必須連絡 WNS (Windows 通知服務) 以將通知傳送至 iOS 裝置，且必須連絡 APNS (Apple 推播通知服務)，而再次傳送訊息。 Azure 通知中樞提供通用介面以及其他功能，以協助支援跨每個平台的推播通知。
...
- **豐富的傳遞模式集**：
    ...
    每個裝置在將其控制代碼傳送至通知中樞時，都可指定一或多個_「標記」_。 如需有關 [標記]。 標籤不一定要預先佈建或處置。 透過標籤，通知將可輕易傳送給使用者或相關群組。 由於標籤可包含任何應用程式專用的識別碼 (例如使用者或群組 ID)，因此使用標籤後，應用程式後端將可省卻儲存及管理裝置控制代碼的工作負擔。
...
Facililate 順暢且統一的體驗跨 Azure 服務，[應用程式服務行動應用程式] 有使用通知中心的推播通知的內建支援。 [應用程式服務行動應用程式] 提供企業開發人員與系統整合者可將一組豐富的功能帶入行動開發人員的高度擴充、 全域使用的行動應用程式開發平台。

Mobile Apps 開發人員可以使用下列流程來利用通知中樞：

1. 擷取裝置 PNS 控制代碼
2. 使用方便的行動應用程式用戶端 SDK 註冊 API 透過通知中心註冊裝置與 [範本]
...
- **在 [Azure 入口網站] 中的整合式的體驗。**Mobile Apps 中會以視覺化方式呈現以功能形式出現的推送，而開發人員可以透過 Mobile Apps 輕鬆使用相關聯的通知中樞。
...
+ **通知中樞快速入門教學課程** ([iOS]、 [Android]、 [Windows 通用]、 [Windows Phone] [Kindle]、 [Xamarin.iOS]、 [Xamarin.Android])
