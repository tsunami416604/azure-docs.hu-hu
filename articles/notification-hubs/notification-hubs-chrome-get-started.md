...
>[AZURE.NOTE] 這不是一般的瀏覽器中推播通知，而且是 Chrome 應用程式專用 — 請參閱 [Chrome Apps Overview] 如需詳細資訊。 Chrome 應用程式以前稱為「封裝應用程式」，不同於較簡單的「代管應用程式」。 請參閱 [Installable Web Apps] 差異。 Chrome 應用程式也可以透過 Apache Cordova 在行動裝置 (Android 和 iOS) 上執行。 請參閱 [Chrome Apps on Mobile] 若要深入了。
...
本教學課程示範使用通知中樞的簡單廣播案例。 設定 GCM 和 Azure 通知中樞等同於 Android 的設定，因為 [Google Cloud Messaging for Chrome] 已被取代相同的 GCM 現在可支援 Android 裝置和 Chrome 執行個體。

請確實依照教學課程的「後續步驟」一節操作，以了解如何使用通知中樞來處理特定使用者和裝置群組。

>[AZURE.NOTE] 若要完成本教學課程，您必須具有有效的 Azure 帳戶。 如果您沒有帳戶，只需要幾分鐘的時間就可以建立免費試用帳戶。 如需詳細資訊，請參閱 [Azure 免費試用](http://azure.microsoft.com/pricing/free-trial/?WT.mc_id=A0E0E5C02&amp;returnurl=http%3A%2F%2Fazure.microsoft.com%2Fen-us%2Fdocumentation%2Farticles%notification-hubs-chrome-get-started%2F)。
...
1. 登入 [Azure Classic Portal], ，然後按一下 [ **+ 新增** 畫面的左下方。
...
下列範例根據 [Chrome App GCM Sample] 和使用建議的方式建立 Chrome 應用程式。 在下列各節中，我們將加強說明 Azure 通知中樞的相關步驟。 我們建議您下載此 Chrome 應用程式的原始碼 [Chrome App Notification Hub Sample]。
...
3. 下載 **crypto-js 程式庫** 從 [crypto-js library] 此資料夾中。 此程式庫資料夾將包含兩個子資料夾 ︰ **元件** 和 **彙總套件**。
...
7. 建立檔案，稱為 **register.js** 下列程式碼。 此檔案會指定後面的指令碼 **register.html**。 Chrome 應用程式並不允許內嵌執行，因此您必須為 UI 建立個別的備份指令碼。
    ...
    上述指令碼有下列的輸出：
    - *window.onload* 在 UI 上定義的兩個按鈕的按鈕 click 事件。 其中一個向 GCM 註冊，另一個使用向 GCM 註冊後所傳回的註冊識別碼來向 Azure 通知中樞註冊。
    ...
    - *registrationPayload* 定義註冊 XML 裝載。 如需詳細資訊，請參閱 [Create Registration NH REST API]。 我們會以接收自 GCM 的項目來更新其中的註冊識別碼。
...
5. 輸入您 **的通知中心名稱** 和 **DefaultListenSharedAccessSignature** 您從入口網站更早版本，並按一下 [取得 **Azure 通知中心註冊**。 您必須查看郵件 **成功的通知中心註冊 ！** 和註冊回應，其中包含 Azure 通知中心註冊的詳細資料的識別碼。
...
深入了解通知中心 [Notification Hubs Overview]。
若要鎖定特定使用者，請參閱本教學課程 [Azure Notification Hubs Notify Users]。 如果您想要按興趣群組分隔使用者，您可以閱讀 [Azure Notification Hubs breaking news]。
