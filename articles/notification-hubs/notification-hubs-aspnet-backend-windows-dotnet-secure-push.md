...
1. 在 **NotifyUserWindowsPhone** 專案中，新增下列程式碼至 App.xaml.cs 以註冊推播背景工作。 在結尾處新增下列程式碼行 `onlaunched ()` 方法:

        RegisterBackgroundTask();

2. 仍在 App.xaml.cs 中，新增下列程式碼之後立即 `onlaunched ()` 方法:
...
3. 新增下列 `使用` 陳述式，在 App.xaml.cs 檔案的頂端:
...
4. 整個內容取代 **PushBackgroundComponent** 命名空間定義取代下列程式碼取代預留位置 `{後端端點}` 與部署後端時所取得的後端端點:
...
10. 新增下列 `使用` 上方的陳述式 **PushBackgroundTask.cs** 檔案:
