...
2. 開啟您 **res/layout/activity_main.xml** 檔案中，將以下列內容定義。
...
4. 建立新的類別，名為 **RegisterClient** 相同封裝中您 `MainActivity` 類別。 將下列程式碼使用於新的類別檔案。
    ...
    為註冊推播通知，此元件會實作連絡應用程式後端所需的 REST 呼叫。 它也會在本機儲存 *registrationIds* 中所述的通知中心所建立 [從應用程式後端註冊](notification-hubs-registration-management.md#registration-management-from-a-backend)。 請注意它會使用授權權杖儲存在本機儲存體中，當您按一下 **登入** ] 按鈕。
...
7. 在 `MainActivity` 類別中，刪除或註解排除整個 `registerWithNotificationHubs` 方法。 本教學課程不會使用此方法。

8. 新增下列 `import` 陳述式，以您 **MainActivity.java** 檔案。
...
10. 在 `MainActivity` 類別中，更新 `sendNotificationButtonOnClick` 方法以透過使用者選取的平台通知服務呼叫 `sendPush` 方法，如下所示。
...
3. 在 Android 應用程式中，按一下 [ **登入**。 等待快顯訊息，指出 **已登入並註冊**。 這會讓 **傳送通知** ] 按鈕。
