...
4. 在 App.xaml.cs 專案檔案中，新增下列屬性至 **App** 類別：
    ...
    在上述程式碼，取代 `< 中樞名稱 >` 和 `< 連接字串具有接聽權限 >` 與通知中樞名稱和連接字串預留位置 *DefaultListenSharedAccessSignature* 先前取得的。
...
1. 開啟 App.xaml.cs 檔案並更新 **InitNotificationsAsync** 方法，以使用 `通知` 類別，以訂閱依類別區分。
