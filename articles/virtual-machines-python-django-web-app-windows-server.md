...
<tags ms.service="virtual-machines" ms.workload="web" ms.tgt_pltfrm="vm-windows" ms.devlang="python" ms.topic="article" ms.date="09/25/2014" ms.author="huvalo"></tags>
...
1.  Follow the instructions given [here][here] to create an Azure virtual machine of the *Windows Server 2012 R2 Datacenter* distribution.
...
## <span id="setup"></span> </a>Setting up Python and Django
...
1.  Install [Web Platform Installer][Web Platform Installer].
2.  Install Python and WFastCGI using Web Platform Installer. This will install wfastcgi.py in your Python scripts folder.

    1.  Start Web Platform Installer.
    2.  Type WFastCGI in the search bar.
    3.  Select the WFactCGI entry for the version of Python you wish to use (2.7 or 3.4). Note that this will install Python as a dependency of WFastCGI.

3.  If you installed Python 2.7, [follow these instructions to install pip manually][follow these instructions to install pip manually] (Python 3.4 comes with pip already installed).
...
2.  Register the handler for this site.
...
3.  Configure the handler to run your Django application.
...
4.  Configure PYTHONPATH so your Django app can be found by the Python interpreter.
...
5.  Tell the FastCGI to WSGI gateway which WSGI handler to use.
...
2.  Replace the contents of the **urls.py** file with the following:
...
3.  Finally, load the web page in your browser.
...
When you're done with this tutorial, shutdown and/or remove your newly created Azure virtual machine to free up resources for other tutorials and avoid incurring Azure usage charges.
  ...
  [follow these instructions to install pip manually]: https://pip.pypa.io/en/latest/installing.html
