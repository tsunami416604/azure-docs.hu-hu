...
DocumentDB 所管理的資料庫實體稱為 **資源**。 每個資源可透過邏輯 URI 唯一識別。 您可以使用標準 HTTP 動詞命令、要求/回應標頭和狀態碼來與資源互動。 
...
## 階層式資源模型
下圖所示，DocumentDB 階層式 **資源模型** 包含的資料庫帳戶，每個都可以透過邏輯和穩定 URI 進行定址的資源集。 一組資源將稱為 **摘要** 在這篇文章。 

>[AZURE.NOTE] DocumentDB 提供高效率的 TCP 通訊協定在通訊模型，也是 restful，並且可透過 [.NET 用戶端 SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx)。

![DocumentDB 的階層式資源模型][1]  
**階層式資源模型**   

若要開始使用資源，您必須 [建立 DocumentDB 資料庫帳戶](documentdb-create-account.md) 使用您的 Azure 訂閱。 資料庫帳戶可以包含一組 **資料庫**, ，而每個包含多個 **集合**, ，而每個的其中又包含 **預存程序、 觸發程序、 Udf、 文件** 和相關 **附件** （預覽功能）。 資料庫也有相關聯 **使用者**, ，各有一組 **權限** 來存取集合、 預存程序、 觸發程序、 Udf、 文件或附件。 雖然資料庫、使用者、權限和集合都是具有已知結構描述的系統定義資源，但是文件和附件包含任意使用者定義 JSON 內容。  

|資源   |說明
|-----------|-----------
|資料庫帳戶   |資料庫帳戶會與一組資料庫及附件之固定數目的 Blob 儲存體相關聯 (預覽功能)。 您可以使用 Azure 訂用帳戶建立一或多個資料庫帳戶。 每個標準資料庫帳戶至少都會有一個 S1 集合容量的配置。 如需詳細資訊，請造訪我們 [定價頁面](https://azure.microsoft.com/pricing/details/documentdb/)。
|資料庫   |資料庫是分割給多個集合之文件儲存體的邏輯容器。 同時也是使用者容器。
|使用者   |範圍權限的邏輯命名空間。 
|權限 |與使用者相關聯的授權權杖，可讓使用者用於存取特定資源。
|集合 |集合是 JSON 文件和相關聯 JavaScript 應用程式邏輯的容器。 集合是一個可計費的實體，其中的成本由與集合相關聯的效能層級所決定。 效能層級 (S1、 S2 和 S3) 提供 10 GB 的儲存體和固定數量的輸送量。 如需效能層級的詳細資訊，請造訪我們 [效能頁](documentdb-performance-levels.md)。
...
>[AZURE.NOTE] 請注意，系統會產生資源的屬性會加上底線 (_) 以 JSON 形式表示。  


屬性 |可由使用者設定或由系統產生？|目的
---|---|---
_rid|由系統產生|系統產生的唯一階層式資源識別碼
_etag|由系統產生|要控制開放式並行存取所需之資源的 etag
_ts|由系統產生|資源上次更新之時間的時間戳記
...
### 資源定址
所有資源都能以 URI 定址。 值 **_self** 資源屬性，代表資源的相對 URI。 URI 的格式包含 / \ < feed\ > / /<feed>/{_rid} 路徑片段 ︰  

|_self 的值 |說明
|-------------------|-----------
|/dbs   |資料庫帳戶的資料庫摘要
|/dbs/{_rid-db} |比對的值為 {_rid-db} 識別碼的資料庫
|/dbs/{_rid-db}/colls/  |資料庫底下的集合摘要
|/dbs/{_rid-db}/colls/{_rid-coll}   |比對的值為 {_rid-coll} 識別碼的集合
|{_rid-db} /dbs/{_rid-db}/users/{_rid-user}/permissions/{_rid-permission} /colls/ {_rid-coll} / 文件  |下一個集合的文件送紙
|{_rid-db} /dbs/{_rid-db}/users/{_rid-user}/permissions/{_rid-permission} /colls/ {_rid-coll} /docs/ {_rid-doc}   |文件識別碼符合 {_rid-doc} 值
|/dbs/{_rid-db}/users/  |資料庫底下的使用者摘要
|/dbs/{_rid-db}/users/{_rid-user}   |比對的值為 {_rid-user} 的使用者識別碼的使用者
|/dbs/{_rid-db}/users/{_rid-user}/permissions   |在 [使用者權限的摘要
|/dbs/{_rid-db}/users/{_rid-user}/permissions/{_rid-permission} |識別碼比對的值為 {_rid-permission} 的權限
...
_Self 和 _rid 屬性的值是資源的替代與標準格式表示。  

## 資料庫帳戶
您可以使用 Azure 訂用帳戶佈建一或多個 DocumentDB 資料庫帳戶。 系統將會為每個標準層資料庫帳戶提供一個 S1 集合中的最小容量。

您可以 [建立和管理 DocumentDB 資料庫帳戶](documentdb-create-account.md) Azure 傳統入口網站，透過 [http://portal.azure.com/](http://portal.azure.com/)。 建立和管理資料庫帳戶都需要管理存取權，而且只有在 Azure 訂用帳戶下才能執行。 
...
屬性名稱|說明
---|---
一致性原則|設定此屬性，以設定您資料庫帳戶下所有集合的預設一致性層級。 您可以使用 [x-ms-consistency-level] 要求標頭，覆寫每個要求的一致性層級。 <p><p>請注意，此屬性只適用於 <i>使用者定義資源</i>。 所有系統定義資源都是設定成支援具有強式一致性的讀取/查詢。
...
請注意，除了佈建、 設定和管理資料庫帳戶，從 Azure 傳統入口網站中，您可以也以程式設計方式建立和管理 DocumentDB 資料庫帳戶使用 [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) 以及 [用戶端 Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)。  
...
與 DocumentDB 資源模型中的其他資源，資料庫可以建立、 取代、 刪除、 讀取或列舉輕鬆地使用 [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) 或任何 [用戶端 Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)。 DocumentDB 保證讀取或查詢資料庫資源之中繼資料的強式一致性。 刪除資料庫會自動確定您無法存取其內所含的任何集合或使用者。   
...
在集合上執行 PUT 即可變更索引原則。 這可以是您可透過 [用戶端 SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx), 、 [Azure 傳統入口網站](https://portal.azure.com) 或 [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx)。

### 查詢集合
集合內的文件可以有任意結構描述，而且您可以查詢集合內的文件，而不需要預先提供任何結構描述或次要索引。 您可以查詢集合使用 [DocumentDB SQL 語法](https://msdn.microsoft.com/library/azure/dn782250.aspx), ，它會提供豐富的階層式和關聯式運算子和擴充性，透過 JavaScript 型 Udf。 JSON 文法允許用於將 JSON 文件建模為標籤做為樹狀節點的樹狀目錄。 這會同時應用 DocumentDB 的自動編製索引技術與 DocumentDB 的 SQL 方言。 DocumentDB 查詢語言包含三個主要部分：   

1.  本質上對應至樹狀結構的較小一組查詢作業 (包括階層式查詢和投射)。 
2.  關聯式作業 (包括複合、篩選、投射、彙總和自我聯結) 的子集。 
3.  可與 (1) 和 (2) 搭配使用的純 JavaScript 型 UDF。  

DocumentDB 查詢模型嘗試打破功能、效率和簡易性之間的平衡。 DocumentDB 資料庫引擎會原生編譯和執行 SQL 查詢陳述式。 您可以查詢集合，使用 [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) 或任何 [用戶端 Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)。 .NET SDK 隨附於 LINQ 提供者。

> [AZURE.TIP] 您可以試試 DocumentDB 並對我們的資料集，在執行 SQL 查詢 [查詢遊樂場](https://www.documentdb.com/sql/demo)。

### 多文件交易
資料庫交易提供安全且可預測的程式設計模型來處理同時的資料變更。 在 RDBMS 中，撰寫商務邏輯的傳統方式是撰寫 **預存程序** 和/或 **觸發程序** 並將它傳送至資料庫伺服器，以交易方式執行。 在 RDBMS 中，需要有應用程式設計人員，才能處理兩個不同的程式設計語言： 
...
建立集合之後, 您可以向預存程序、 觸發程序和 Udf 集合使用 [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) 或任何 [用戶端 Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)。 註冊之後，您就可以參考和執行它們。 您可以考慮使用下列完全以 JavaScript 撰寫的預存程序。下列程式碼採用兩個引數 (書籍名稱和作者名稱) 建立新文件，接著再查詢文件，然後加以更新。這些作業全都會在隱含的 ACID 交易中進行。 在執行期間的任何時間點，如果擲回 JavaScript 例外狀況，整個交易便會中止。
...
用戶端可以將上面的 JavaScript 邏輯「傳送」至資料庫，以透過 HTTP POST 進行交易式執行。 如需使用 HTTP 方法的詳細資訊，請參閱 [與 DocumentDB 資源進行 RESTful 互動](https://msdn.microsoft.com/library/azure/mt622086.aspx)。 
...
### 將 UDF 執行為查詢的一部分
UDF 可以指定為 SQL 查詢的一部分，而且是用來擴充核心 [DocumentDB SQL 查詢語言](https://msdn.microsoft.com/library/azure/dn782250.aspx)。
...
雖然上述程式碼片段顯示註冊 (POST)、 取消註冊 (PUT)、 讀取/列出 (GET) 和執行 (POST) 透過 [DocumentDB JavaScript SDK](https://github.com/Azure/azure-documentdb-js), ，您也可以使用 [REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) 或其他 [用戶端 Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)。 
...
與所有其他資源，文件可以建立、 取代、 刪除、 讀取、 列舉或查詢輕鬆地使用 REST Api 或任何 [用戶端 Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)。 刪除文件時會立即清出對應至所有巢狀附件的配額。 文件的讀取一致性層級會遵循資料庫帳戶的一致性原則。 根據您應用程式的資料一致性需求，可以覆寫每個要求的這個原則。 查詢文件時，讀取一致性會遵循集合上所設定的索引模式。 為求「一致」，這會遵循帳戶的一致性原則。 

## 附件和媒體
>[AZURE.NOTE] 附件和媒體資源的預覽功能。
...
## 權限
從存取控制觀點來看，資源，例如資料庫帳戶、 資料庫、 使用者和權限視為 *管理* 資源，因為這些都需要系統管理權限。 相反地，資源包括集合、 文件、 附件、 預存程序、 觸發程序和 Udf 限制給定的資料庫，並視為 *應用程式資源*。 對應至兩種資源和角色可存取它們 （也就是系統管理員和使用者），授權模型定義兩種類型的 *存取金鑰*: *主要金鑰* 和 *資源索引鍵*。 主要金鑰是資料庫帳戶的一部分，並且提供給將佈建資料庫帳戶的開發人員 (或系統管理員)。 此主要金鑰具有系統管理員語意，即它可以用來授權對管理和應用程式資源的存取權。 相較之下，資源金鑰是精細的存取金鑰，可讓您存取 *特定* 應用程式資源。 因此，它會擷取資料庫使用者與使用者具有特定資源 (例如，集合、文件、附件、預存程序、觸發程序或 UDF) 的權限之間的關聯性。   
...
## 後續步驟
深入了解使用 HTTP 命令來使用資源 [與 DocumentDB 資源進行 RESTful 互動](https://msdn.microsoft.com/library/azure/mt622086.aspx)。
