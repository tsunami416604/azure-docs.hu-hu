...
`Api 版本` 需要。 有效值包括 `2015年-02-28` 或更新版本。
...
- **認證**：
    ...
    - **connectionString**：必要。以下列格式指定 Azure DocumentDB 資料庫的連接資訊: `AccountEndpoint = < DocumentDB 端點 url >;AccountKey = < DocumentDB 驗證金鑰 >; Database = < DocumentDB 資料庫識別碼 >`
...
資料變更偵測原則是用來有效識別已變更的資料項目。 目前，唯一支援的原則是 `高水位` 原則使用 `_ts` documentdb 的指定方式如下所提供的上次修改時間戳記屬性:
...
您也需要加入 `_ts` 投影中和 `其中` 查詢子句。 例如：

    SELECT s.id, s.Title, s.Abstract, s._ts FROM Sessions s WHERE s._ts > @HighWaterMark

### <a id="DataDeletionDetectionPolicy"></a>擷取已刪除的文件

當從來源資料表中刪除資料列時，您也應該在搜尋索引中刪除這些資料列。 資料刪除偵測原則可用來有效識別刪除的資料項目。 目前，唯一支援的原則是 `虛刪除` 原則 (刪除會標示為某種類型的旗標)，其中的指定方式如下:
...
- **間隔**：必要。 可用以指定索引子執行間隔或期間的持續時間值。 允許的最小間隔為 5 分鐘；最長間隔為一天。 其必須格式化為 XSD"dayTimeDuration"值 (的受限的子集 [ISO 8601 持續時間](http://www.w3.org/TR/xmlschema11-2/#dayTimeDuration) 值)。 這個模式是: `P(nD)(T(nH)(nM))`。 範例: `PT15M` 每隔 15 分鐘， `PT2H` 每隔 2 小時。

- **startTime**：必要。 指定索引子應該開始執行的 UTC 日期時間。

### <a id="CreateIndexerExample"></a>要求本文範例

下列範例會建立索引子將資料從所參考的集合複製 `myDocDbDataSource` 資料來源以 `mySearchIndex` 2015 年 1 月 1 日 UTC 開始，每小時執行的排程上的索引。
