...
>[AZURE.NOTE] 由於 JSON 具有自我描述的特性也就是每個文件包含結構描述 (中繼資料) 和資料，例如 `{"locationId": 5，"city":"Moscow"}` 會顯示有兩個屬性 `locationId` 和 `縣 (市)`, ，並且具有數值和字串的屬性值。 DocumentDB 能夠推斷文件的結構描述，並在插入或取代文件時編製索引，而不需要您定義結構描述或次要索引。
...
- JSON 屬性 `{"headquarters":"Belgium"}` 在上述範例中的屬性會對應至路徑/headquarters/Belgium。
- JSON 陣列 `{"exports": [{"city":"Moscow"}`, ，`{"city": Athens"}]}` 對應至路徑 `/exports/ [城市/莫斯科` 和 `/exports/ [城市/雅典`。
...
雖然無結構描述，DocumentDB 的 SQL 和 JavaScript 查詢語言提供關聯式投影和篩選、 階層式導覽整個文件、 空間的作業和完全以 JavaScript 撰寫的 Udf 的引動過程。 DocumentDB 的查詢執行階段可以支援這些查詢，因為它可以直接針對資料的這個索引樹狀結構表示法運作。
