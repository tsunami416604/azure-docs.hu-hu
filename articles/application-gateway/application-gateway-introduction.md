...
Azure 提供第 4 層負載平衡透過 Azure 負載平衡器在傳輸層級 (TCP/UDP) 處理，並讓所有連入的網路流量負載平衡到應用程式閘道服務。 然後應用程式閘道會將路由規則套用至 HTTP 流量，提供第 7 層 (HTTP) 負載平衡。 當您建立應用程式閘道時，將相關聯的端點 (VIP)，並做為輸入網路流量的公用 IP。
...
>[AZURE.NOTE] 這是應用程式閘道輸送量的大概指引。 實際輸送量會視各種環境詳細資料，例如平均網頁大小、 位置的後端執行個體和伺服器頁面等等的處理時間。

## 健康狀況監視

Azure 應用程式閘道每隔 30 秒會監視後端執行個體的健康狀態。 它會將 HTTP 健康狀態探查要求傳送到組態的 *BackendHttpSettings* 元素中設定之連接埠的每個執行個體。 健康狀態探查預期成功的 HTTP 回應其回應狀態碼會在 200-399 的範圍內。
