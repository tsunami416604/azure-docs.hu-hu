---
title: Háttér-egészségügyi és diagnosztikai naplók
titleSuffix: Azure Application Gateway
description: Megtudhatja, hogy miként engedélyezheti és kezelheti az Azure Application Gateway hozzáférési naplóit és teljesítménynaplóit
...
Az Azure Application Gateway erőforrásait a következő módokon figyelheti:

* [Háttérrendszer állapota:](#back-end-health)Az Application Gateway lehetővé teszi a háttérkészletekben lévő kiszolgálók állapotának figyelését az Azure Portalon és a PowerShellen keresztül. A háttérkészletek állapotát a teljesítménydiagnosztikai naplókon keresztül is megtalálhatja.

* [Naplók:](#diagnostic-logging)A naplók lehetővé teszik a teljesítmény, a hozzáférés és egyéb adatok mentését vagy felhasználását egy erőforrásból figyelési célokra.

* [Metrikák:](application-gateway-metrics.md)Az Application Gateway számos metrikával rendelkezik, amelyek segítségével ellenőrizheti, hogy a rendszer a várt módon teljesít-e.

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

## <a name="back-end-health"></a>Háttér-egészségügyi

Az Application Gateway lehetővé teszi a háttérkészletek egyes tagjainak állapotát a portálon, a PowerShellen és a parancssori felületen (CLI) keresztül. A háttérkészletek összesített állapotösszegzését is megtalálhatja a teljesítménydiagnosztikai naplókon keresztül.

A háttér-állapotjelentés az Application Gateway állapotminta kimenetét tükrözi a háttérpéldányok. Ha a vizsgálat sikeres, és a háttér-a dis t a forgalom fogadása, ez tekinthető egészséges. Ellenkező esetben, ez tekinthető egészségtelen.

> [!IMPORTANT]
> Ha egy alkalmazásátjáró-alhálózaton van egy hálózati biztonsági csoport (NSG), a 65503-65534-es portok a v1 termékkiszolgálók, a 2-es termékkészlet-virtuális készlet esetén pedig a bejövő forgalom esetén a 65503-65534 tartomány. Ez a porttartomány az Azure-infrastruktúra kommunikációjához szükséges. A portokat Azure-tanúsítványok védik (zárják le). Megfelelő tanúsítványok nélkül a külső entitások, beleértve az átjárók ügyfeleit is, nem kezdeményezhetnek módosításokat ezeken a végpontokon.


### <a name="view-back-end-health-through-the-portal"></a>Háttér-állapot megtekintése a portálon keresztül

A portálon a háttér-állapot automatikusan biztosított. Egy meglévő alkalmazásátjáróban válassza a**Háttérrendszer állapotának** **figyelése** > lehetőséget.

A háttérkészlet minden tagja ezen a lapon szerepel (legyen az hálózati adapter, IP vagy teljes tartománynév). A háttérkészlet neve, portja, háttérbeli HTTP-beállításainak neve és egészségi állapota látható. Az állapot érvényes értéke: **Kifogástalan**, **Nem kifogástalan**és **Ismeretlen**.

> [!NOTE]
> Ha **ismeretlen**háttérállapotot lát, győződjön meg arról, hogy a háttérrendszerhez való hozzáférést nem blokkolja egy NSG-szabály, egy felhasználó által definiált útvonal (UDR) vagy egy egyéni DNS a virtuális hálózatban.

![Háttér-egészségügyi][10]

### <a name="view-back-end-health-through-powershell"></a>Háttérrendszer állapotának megtekintése a PowerShellen keresztül
...
### <a name="view-back-end-health-through-azure-cli"></a>Háttérállapot megtekintése az Azure CLI-n keresztül
...
A következő kódrészlet egy példát mutat be a válaszra:
...
1. Jegyezze fel azon Storage-fiók erőforrás-azonosítóját, ahol a naplóadatokat tárolja. Ez az érték a következő formát követi: /előfizetések/\<előfizetésazonosító\>/erőforráscsoportok/\<erőforráscsoport neve\>/szolgáltatások/Microsoft.Storage/storage-fiókok/\<storage-fiók neve\>. Az előfizetés bármilyen tárfiókját használhatja. Ezeket az információkat az Azure Portalon találhatja meg.
    ...
    ![Portál: erőforrásazonosító a tárfiókhoz](./media/application-gateway-diagnostics/diagnostics1.png)
...
2. Vegye figyelembe az alkalmazásátjáró erőforrás-azonosítóját, amelyhez engedélyezve van a naplózás. Ez az érték a következő formában\<érhető\>el: /subscriptions/ subscriptionId\</resourceGroups/ erőforráscsoport neve\>/providers/Microsoft.Network/applicationGateways/\<application gateways name\>. Ezeket az információkat a portálon találhatja meg.
    ...
    ![Portál: az alkalmazásátjáró erőforrásazonosítója](./media/application-gateway-diagnostics/diagnostics2.png)
...
Az Azure alapértelmezés szerint létrehozza a tevékenységnaplót. A naplók 90 napig megmaradnak az Azure eseménynaplók tárolójában. Ezekről a naplókról az Események megtekintése és a [tevékenységnapló](../monitoring-and-diagnostics/insights-debugging-with-events.md) cikk elolvasásával tudhat meg többet.
...
A teljesítménynapló csak akkor jön létre, ha az előző lépésekben részletezett en engedélyezte az egyes Application Gateway-példányokon. Az adatok a naplózás engedélyezésekor megadott tárfiókban tárolódnak. A teljesítménynapló adatai 1 perces időközönként jönnek létre. Csak a v1 Termékváltozat esetén érhető el. A v2 termékváltozat hoz [a metrikák](application-gateway-metrics.md) teljesítményadatok. A következő adatok at naplózza a rendszer:
...
|action     |  A kéréssel kapcsolatos intézkedés. A rendelkezésre álló értékek egyező és letiltott.      |
