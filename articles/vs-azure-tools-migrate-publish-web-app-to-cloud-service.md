...
>[AZURE.NOTE] 本主題是關於部署到雲端服務，而不是網站。 部署至網站的相關資訊，請參閱 [Azure App Service 中的 web 應用程式部署](web-sites-deploy.md)。
...
>[AZURE.NOTE] 方案中的 Web 專案才會顯示 [轉換]****、[轉換成 Azure 雲端服務專案]**** 命令。 例如，方案中的 Silverlight 專案就不會顯示此命令。
當您建立服務封裝或將應用程式發佈至 Azure 時，可能會出現警告或錯誤。這些警告和錯誤可以幫助您在部署至 Azure 之前先修正問題。例如，您可能會收到遺失組件的警告。如需如何將所有警告視為錯誤的詳細資訊，請參閱 [使用 Visual Studio 設定 Azure 雲端服務專案](vs-azure-tools-configuring-an-azure-project.md)。在建置應用程式、使用計算模擬器在本機執行應用程式，或將應用程式發佈至 Azure 時，您可能會在 [錯誤清單]**** 視窗中看到下列錯誤：「指定的路徑和 (或) 檔案名稱太長」****。如果完整的 Azure 專案名稱長度太長，就會發生此錯誤。包括完整路徑在內的專案名稱長度不能超過 146 個字元。比方說，這是完整的專案名稱，包括針對 Silverlight 應用程式所建立的 Azure 專案的檔案路徑: `c:\users\ < 使用者名稱 > \documents\visual studio 2015\Projects\SilverlightApplication4\SilverlightApplication4.Web.Azure.ccproj`。您可能必須將方案移到其他具有較短路徑的目錄，以縮減完整專案名稱的長度。
...
1. 若要啟用 Web 應用程式以部署至 Azure，請在方案中開啟 Web 專案的捷徑功能表，然後選擇 [新增 Azure 部署專案]。
    ...
    - Azure 專案呼叫 `< web 專案的名稱 >。Azure` 會加入至您的應用程式方案。
...
1. 若要複製 ADO.NET 連接字串以用於您的連接字串，請在 Azure 管理入口網站中執行下列步驟。
  ...
  1. 若要顯示可用的 SQL Database 執行個體，請選擇 [SQL Database]**** 節點。
...
1. 您也必須將參數 `MultipleActiveResultSets = True` 至連接字串。 連接字串應有如下格式：
...
1. 開啟或建立資料庫執行個體後，選擇 [連接字串]**** 連結。

1. 在頁面底部選擇連結以進行防火牆設定，並接受預設值或設定您需要的值。

1. 複製 ADO.NET 連接字串、 將它貼到您的 web.config 檔案中，透過內部部署資料庫，舊的連接字串，並確定將 `MultipleActiveResultSets = True`。
...
1. 若要將 Web 應用程式發佈至 Azure，請開啟 Web 專案的捷徑功能表，然後選擇 [發佈至 Azure]****。

    [發佈 Azure 應用程式]**** 對話方塊隨即開啟，且 Visual Studio 會開始部署程序。 如需如何發行應用程式的詳細資訊，請參閱下一節 **發行 Azure 應用程式從 Visual Studio** 中 [發行雲端服務使用 Azure Tools](vs-azure-tools-publishing-a-cloud-service.md)。
...
1. 若要在部署後存取應用程式，請在 [Azure 活動記錄檔]**** 中出現 [已完成]**** 狀態時，選擇部署旁邊的箭號。 這會顯示 Web 應用程式在 Azure 中的 URL。 請參閱下表，以取得如何從 Azure 啟動特定類型的 Web 應用程式的詳細資料。
   ...
   | ASP.NET 空白 Web 應用程式| 您必須在應用程式中加入設定做為 Web 專案起始頁的 .aspx 網頁。然後在功能表列上，選擇 [偵錯]****、[開始偵錯]**** (鍵盤：選擇 **F5** 鍵)。| 如果應用程式中有預設的 .aspx 網頁，請選擇 [Azure 活動記錄檔]**** 的 [部署]**** 索引標籤中顯示的 URL 超連結，瀏覽器就會載入此網頁。如果您有不同的.aspx 網頁時，您需要瀏覽到這個特定頁面的 url 使用下列格式: `< 部署 url > / < 頁面名稱 >.aspx`|
   | Silverlight 應用程式| 在功能表列上，選擇 [偵錯]****、[開始偵錯]**** (鍵盤：選擇 **F5** 鍵)。| 您需要瀏覽至您的應用程式的 url 使用下列格式的特定頁面: `< 部署 url > / < 頁面名稱 >.aspx`|
   | Silverlight 商務應用程式| 在功能表列上，選擇 [偵錯]****、[開始偵錯]**** (鍵盤：選擇 **F5** 鍵)。| 您需要瀏覽至您的應用程式的 url 使用下列格式的特定頁面: `< 部署 url > / < 頁面名稱 >.aspx`|
   | Silverlight 瀏覽應用程式| 在功能表列上，選擇 [偵錯]****、[開始偵錯]**** (鍵盤：選擇 **F5** 鍵)。| 您需要瀏覽至您的應用程式的 url 使用下列格式的特定頁面:`< 部署 url > / < 頁面名稱 >.aspx`|
   | WCF 服務應用程式| 您必須將 .svc 檔案設定做為 WCF 服務專案的起始頁。然後在功能表列上，選擇 [偵錯]****、[開始偵錯]**** (鍵盤：選擇 **F5** 鍵)。| 您需要瀏覽至您的應用程式的 url 使用下列格式的.svc 檔: `< 部署 url > / < 服務檔案名稱 >.svc`|
   | WCF 工作流程服務應用程式| 您必須將 .svc 檔案設定做為 WCF 服務專案的起始頁。然後在功能表列上，選擇 [偵錯]****、[開始偵錯]**** (鍵盤：選擇 **F5** 鍵)。| 您需要瀏覽至您的應用程式的 url 使用下列格式的.svc 檔: `< 部署 url > / < 服務檔案名稱 >.svc`|
   | ASP.NET 動態實體| 在功能表列上，選擇 [偵錯]****、[開始偵錯]**** (鍵盤：選擇 **F5** 鍵)。| 您必須更新連接字串 (請參閱下一節)。您也需要瀏覽至您的應用程式的 url 使用下列格式的特定頁面: `< 部署 url > / < 頁面名稱 >.aspx`|
   | ASP.NET 動態資料 Linq to SQL| 在功能表列上，選擇 [偵錯]****、[開始偵錯]**** (鍵盤：選擇 **F5** 鍵)。| 您必須遵循以下程序中的步驟：將 SQL Azure 資料庫用於應用程式 (請參閱本主題前面的章節)。您也需要瀏覽至您的應用程式的 url 使用下列格式的特定頁面: `< 部署 url > / < 頁面名稱 >.aspx`|
...
1. 若要在 web.config 檔案中儲存您對連接字串所做的變更，請在功能表列上選擇 [檔案]****、[儲存 web.config]****。
