...
ms.locfileid: "68610575"
---
# <a name="quickstart-encrypt-a-windows-iaas-vm-with-azure-powershell"></a>Gyors útmutató: Windows IaaS virtuális gép titkosítása Azure PowerShell
...
- A [Azure PowerShell modul legújabb verziójának](/powershell/azure/install-az-ps) telepítése vagy frissítése
    - Az az Module verziójának a 1.0.0 vagy magasabb szintűnek kell lennie. A `Get-Module Az -ListAvailable | Select-Object -Property Name,Version,Path` használatával ellenőrizhető a verzió.
...
1. A **rendszergazda: Windows PowerShell integrált parancsprogram-kezelési környezet** ablakban kattintson a **Megtekintés** elemre, majd a **parancsfájl megjelenítése panelre**.
...
1. A **rendszergazda: Windows PowerShell integrált parancsprogram-kezelési környezet** ablakban kattintson a **fájl** elemre, majd a **Megnyitás**gombra. Keresse meg az **ADEPrereqScript.ps1** fájlt, és kattintson rá duplán. A szkript a szkriptpanelen nyílik meg.
...
![Az Azure Disk Encryption előfeltétel-konfigurációs szkript futtatása PowerShell ISE-ben](./media/quick-encrypt-vm-powershell/ade-prereq-script.PNG)
...
1. Másolja a szkriptet a **rendszergazdába: Windows PowerShell integrált parancsprogram-kezelési környezet** a parancsfájl ablaktáblát, és módosítsa az első három változót. Az erőforráscsoportnak és a helynek ugyanannak kell lennie, mint amit az [előfeltétel-konfigurációs szkripthez](#bkmk_PrereqScript) használt.  
...
Most, hogy létrehozta és konfigurálta a kulcstartót és egy virtuális gépet, titkosíthatja a lemezt a **set-AzVmDiskEncryptionExtension** parancsmaggal. 
