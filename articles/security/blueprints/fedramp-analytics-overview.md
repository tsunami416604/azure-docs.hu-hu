...
---
# <a name="azure-security-and-compliance-blueprint-analytics-for-fedramp"></a>Azure Security and Compliance Blueprint: elemzés a FedRAMP
...
**Azure Analysis Service**: az [Azure Analysis Service](https://docs.microsoft.com/azure/analysis-services/analysis-services-overview) vállalati adatmodellezést és integrációt biztosít az Azure adatplatform-szolgáltatásokkal. Az Azure Analysis Service nagy mennyiségű adattal felgyorsítja a böngészést azáltal, hogy több forrásból származó adatokkal egyetlen adatmodellbe egyesít.
...
- **Tevékenységnaplók**: a [Tevékenységnaplók](../../azure-monitor/platform/activity-logs-overview.md) betekintést nyújtanak az előfizetésben lévő erőforrásokon végrehajtott műveletekre.
- **Diagnosztikai naplók**: a [diagnosztikai naplók](../../azure-monitor/platform/resource-logs-overview.md) a minden erőforrás által kibocsátott összes naplót tartalmazzák. Ezek a naplók a Windows-események rendszernaplóit és az Azure Blob Storage, Tables és üzenetsor-naplókat tartalmazzák.
- **Tűzfal-naplók**: a Application Gateway teljes körű diagnosztikai és hozzáférési naplókat biztosít. A WAF-kompatibilis Application Gateway erőforrásai számára elérhetők a tűzfalak.
- **Napló archiválása**: az összes diagnosztikai napló egy központi és titkosított Azure Storage-fiókba írja az archiválást egy 2 napos megőrzési időtartammal. Ezek a naplók Azure Monitor naplókat csatlakoznak a feldolgozáshoz, tároláshoz és irányítópult-jelentéskészítéshez.
...
-   [Azure-tevékenység naplói](../../azure-monitor/platform/collect-activity-logs.md): a Activity log Analytics megoldás segítséget nyújt az Azure-tevékenységek naplóinak elemzésében az ügyfelekhez tartozó összes Azure-előfizetésben.
...
**Titkok kezelése**: a megoldás [Azure Key Vault](https://azure.microsoft.com/services/key-vault/) használ a kulcsok és titkok kezeléséhez. Az Azure Key Vault segít a felhőalapú alkalmazások és szolgáltatások által használt titkosítási kulcsok és titkos kulcsok védelmében.
...
-   [Update Management](../../automation/automation-update-management.md): a Update Management megoldás lehetővé teszi az operációs rendszer biztonsági frissítéseinek az ügyfelek általi felügyeletét, beleértve az elérhető frissítések állapotát, valamint a szükséges frissítések telepítésének folyamatát.
...
##### <a name="security"></a>Biztonság
- **Kártevők elleni védelem**: a Virtual Machines-hoz készült [Microsoft antimalware](https://docs.microsoft.com/azure/security/fundamentals/antimalware) olyan valós idejű védelmi képességet biztosít, amely lehetővé teszi a vírusok, kémprogramok és más kártékony szoftverek azonosítását és eltávolítását, ha az ismert kártékony vagy a rendszer nem kívánt szoftvert próbál telepíteni vagy futtatni a védett virtuális gépeken.
- **Javítás kezelése**: a jelen viszonyítási architektúra részeként üzembe helyezett Windows-alapú virtuális gépek alapértelmezés szerint úgy vannak konfigurálva, hogy Windows Update szolgáltatásból fogadják az automatikus frissítéseket. Ez a megoldás tartalmazza azt a [Azure Automation](https://docs.microsoft.com/azure/automation/automation-intro) szolgáltatást is, amelyen keresztül a frissített központi telepítések létrehozhatók a virtuális gépek szükség szerinti javításához.
