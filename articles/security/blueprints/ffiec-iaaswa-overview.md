---
title: Azure Security and Compliance Blueprint IaaS webalkalmazás FFIEC számára
description: Azure Security and Compliance Blueprint IaaS webalkalmazás FFIEC számára
services: security
author: meladie
...
ms.date: 06/20/2018
ms.author: meladie
ms.openlocfilehash: db7b16e569b741c415a07e874466cc99e0190c65
ms.sourcegitcommit: 6cbf5cc35840a30a6b918cb3630af68f5a2beead
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 08/05/2019
ms.locfileid: "68778727"
---
# <a name="azure-security-and-compliance-blueprint-iaas-web-application-for-ffiec-financial-services"></a>Azure Security and Compliance Blueprint: IaaS-webalkalmazás a FFIEC pénzügyi szolgáltatásaihoz

## <a name="overview"></a>Áttekintés

Ez a Azure Security and Compliance Blueprint útmutatást nyújt a szövetségi pénzügyi intézmény által szabályozott pénzügyi adatok gyűjtésére, tárolására és lekérésére alkalmas infrastruktúra-szolgáltatás (IaaS) környezetének üzembe helyezéséhez Vizsgálati Tanács (FFIEC).

Ez a hivatkozási architektúra, a megvalósítási útmutató és a fenyegetés modell olyan alapot biztosít az ügyfeleknek, hogy megfeleljenek a FFIEC követelményeinek. Ez a megoldás olyan alapkonfigurációt biztosít, amellyel az ügyfelek FFIEC-kompatibilis módon telepíthetik az Azure-beli számítási feladatokat, azonban ez a megoldás nem használható éles környezetben, mert további konfigurálásra van szükség.
...
Ez a Azure Security and Compliance Blueprint egy IaaS webalkalmazás hivatkozási architektúráját helyezi üzembe SQL Server háttérrel. Az architektúra tartalmaz egy webes szintet, adatszintet, Active Directory infrastruktúrát, Application Gateway és Load Balancer. A webes és az adatszinten üzembe helyezett virtuális gépek egy rendelkezésre állási csoportban vannak konfigurálva, és a magas rendelkezésre állás érdekében SQL Server példányok egy always on rendelkezésre állási csoportba vannak konfigurálva. A virtuális gépek tartományhoz csatlakoznak, és Active Directory csoportházirendek segítségével kényszerítheti ki a biztonsági és megfelelőségi konfigurációkat az operációs rendszer szintjén.

A teljes megoldás azon Azure Storage-ra épül, amelyet az ügyfelek a Azure Portal konfigurálnak. Az Azure Storage a Storage Service Encryption segítségével titkosítja az összes adatát, hogy megőrizze az inaktív adatok titkosságát. A földrajzi redundáns tárolás biztosítja, hogy az adatok egy több száz mérföld távolságra lévő másodlagos adatközpontba replikálódnak, és ismét az adott adatközponton belül három példányban tárolódnak, ami megakadályozza, hogy az ügyfél elsődleges adatközpontjában negatív esemény maradjon adatok.
...
![IaaS webalkalmazás a FFIEC hivatkozási architektúrájának diagramja](images/ffiec-iaaswa-architecture.png "IaaS webalkalmazás a FFIEC hivatkozási architektúrájának diagramja")
...
Az ehhez a hivatkozási architektúrához tartozó Adatfolyam-diagram [letölthető](https://aka.ms/ffiec-iaaswa-tm) , vagy a következő címen érhető el. Ez a modell lehetővé teszi az ügyfeleknek, hogy a módosítások végrehajtása során megértsék a rendszerinfrastruktúra lehetséges kockázatait.

![IaaS webalkalmazás a FFIEC Threat modelhez](images/ffiec-iaaswa-threat-model.png "IaaS webalkalmazás a FFIEC Threat modelhez")
...
A [Azure Security and Compliance Blueprint – FFIEC IaaS webalkalmazás-implementációs mátrix](https://aka.ms/ffiec-iaaswa-cim) olyan információkat biztosít, amelyekkel a IaaS webalkalmazási architektúrája foglalkozik a FFIEC követelményeivel, beleértve az alábbiak részletes leírását: a megvalósítás megfelel az egyes érintett célkitűzések követelményeinek.
