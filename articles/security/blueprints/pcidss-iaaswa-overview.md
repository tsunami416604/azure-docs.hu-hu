---
title: Azure biztonsági és megfelelőségi terv – IaaS-webalkalmazás a PCI DSS
description: Azure biztonsági és megfelelőségi terv – IaaS-webalkalmazás a PCI DSS
services: security
author: meladie
...
ms.locfileid: "49407911"
---
# <a name="azure-security-and-compliance-blueprint-iaas-web-application-for-pci-dss"></a>Azure biztonsági és megfelelőségi terv: IaaS-webalkalmazás a PCI DSS

## <a name="overview"></a>Áttekintés

Az Azure biztonsági és megfelelőségi terv szolgáltatás (IaaS) környezetként, a gyűjtemény, tárolására és lekérésére megfelelő Payment Card Industry Data Security Standards (PCI DSS 3.2) megfelelő infrastruktúra központi telepítésére vonatkozó útmutatást nyújt. kártyatulajdonosi adatokat. Közös referenciaarchitektúra bemutatja, és azt mutatja be a hitelkártya adatait (beleértve a száma, lejárati és ellenőrzési adatok) megfelelő kezeléséhez a biztonságos, előírásoknak megfelelő és többrétegű környezetben. Ez a megoldás azt mutatja be egy teljes körű megoldást egy felhőalapú megközelítés a terheket és a központi telepítési költségek csökkentéséhez kérő szervezetek igényeit.
...
![A PCI DSS architekturális diagramja IaaS-webalkalmazás](images/pcidss-iaaswa-architecture.png "IaaS-webalkalmazás PCI DSS architekturális diagramja")
...
- Az Application Gateway (LBNSG) 1 hálózati biztonsági csoport
- 1 hálózati biztonsági csoport számára (MGTNSG) bástyagazdagép
...
-   [Mindig titkosított oszlopokat](https://docs.microsoft.com/azure/sql-database/sql-database-always-encrypted-azure-key-vault) győződjön meg arról, hogy bizalmas kártyatulajdonosi adatokat soha nem jelenik meg, mint belül az adatbázis-rendszer egyszerű szövegként. Adatok titkosításának engedélyezése után csak az ügyfélalkalmazások vagy alkalmazáskiszolgálók hozzáférést a kulcsokhoz hozzáférhet egyszerű szöveges adatokat.
- A [Extended Properties](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-addextendedproperty-transact-sql) szolgáltatás használható lemondásához a feldolgozást az érintettek, mivel lehetővé teszi a felhasználók egyéni tulajdonságokat adhat az adatbázis-objektumok és adatok megjelölheti "Kifutott" az, hogy az alkalmazáslogika támogatása a feldolgozási tartozó kártyatulajdonosi adatokat.
...
- [Az SQL Database dinamikus adatmaszkolása](https://docs.microsoft.com/azure/sql-database/sql-database-dynamic-data-masking-get-started) korlátozza a bizalmas kártyatulajdonosi adatokat a nem kiemelt jogosultságú felhasználók vagy alkalmazások, adatok maszkolása által. Dinamikus adatmaszkolás automatikusan potenciálisan bizalmas adatok felderítéséhez, és javasolja a alkalmazni lehessen a megfelelő maszkokkal. Ez segít azonosítani és a hozzáférés korlátozása a kártyabirtokosok adatait úgy, hogy azt nem létezik az adatbázis jogosulatlan hozzáférést. Dinamikus adatmaszkolási beállítások igazodnia kell az adatbázis-séma módosításával ügyfelek felelőssége.
...
A data folyamat diagramja (DFD) Ez a referenciaarchitektúra érhető el az [letöltése](https://aka.ms/pcidss-iaaswa-tm) vagy alább találja. Ez a modell segítségével az ügyfelek megismerhetik a pontokat a rendszer infrastruktúra esetleges kockázat, ha a végzett módosítások.

![A PCI DSS architekturális diagramja IaaS-webalkalmazás](images/pcidss-iaaswa-threat-model.png "IaaS-webalkalmazás PCI DSS architekturális diagramja")
...
A [Azure biztonsági és -megfelelőségi terv – PCI DSS IaaS webes kérelem végrehajtása mátrix](https://aka.ms/pcidss-iaaswa-cim) melyik PCI DSS 3.2-es verzióját a követelmények az IaaS-webalkalmazás architektúra tárgyalt információkat nyújt, beleértve a részletes Hogyan végrehajtása megfelel egyes kezelt cikk leírása.
