---
title: Azure biztonsági és megfelelőségi terv - Data Analytics, a PCI DSS
description: Azure biztonsági és megfelelőségi terv - Data Analytics, a PCI DSS
services: security
author: meladie
...
ms.locfileid: "57247443"
---
# <a name="azure-security-and-compliance-blueprint-analytics-for-pci-dss"></a>Azure biztonsági és megfelelőségi terv: Elemzések a PCI DSS

## <a name="overview"></a>Áttekintés

Az Azure biztonsági és megfelelőségi terv útmutatással szolgál a központi telepítés egy data analytics architektúra az Azure-ban, amely segít a Payment Card Industry Data Security Standards (PCI DSS 3.2) követelményeinek. Közös referenciaarchitektúra bemutatja, és azt mutatja be a hitelkártya adatait (beleértve a száma, lejárati és ellenőrzési adatok) megfelelő kezeléséhez a biztonságos, előírásoknak megfelelő és többrétegű környezetben. A tervezet mutatja be, amelyben ügyfeleink megfelel bizonyos biztonsági és megfelelőségi követelmények módon, és az ügyfelek számára hozhat létre és konfigurálja a saját adatok elemzési megoldásokat az Azure-ban ismeretekkel szolgál.

Ez a referencia-architektúra, megvalósítási útmutató és veszélyforrások elleni modellje alapját az ügyfelek számára, hogy megfelelnek a PCI DSS 3.2-es követelményeinek. Ez a megoldás biztosítja egy alapkonfigurációt, segítenek az ügyfeleknek a számítási feladatok üzembe helyezéséhez az Azure PCI DSS 3.2-es megfelelő módon; azonban ez a megoldás nem használható-éles környezetben van, mivel további konfigurálásra szükség.
...
![A PCI DSS architekturális diagramja Analytics](images/pcidss-analytics-architecture.png "elemzések a PCI DSS architekturális diagramja")
...
**Az Azure Machine Learning szolgáltatás**: [Az Azure Machine Learning](https://docs.microsoft.com/azure/machine-learning/service/) van olyan adatelemzési módszer, amely lehetővé teszi a számítógépek a meglévő adatok jelezheti előre a jövőbeni viselkedéseket, kimeneteket és trendeket.
...
A referenciaarchitektúra az adatfolyam-diagram érhető el az [letöltése](https://aka.ms/PCIAnalyticsTM) vagy alább találja. Ez a modell segítségével az ügyfelek megismerhetik a pontokat a rendszer infrastruktúra esetleges kockázat, ha a végzett módosítások.

![A PCI DSS architekturális diagramja Analytics](images/pcidss-analytics-threat-model.png "elemzések a PCI DSS fenyegetések modellezése")

## <a name="compliance-documentation"></a>Megfelelőségi dokumentációja
A [Azure biztonsági és megfelelőségi terv – PCI DSS vevő felelőssége mátrix](https://aka.ms/pcidss-analytics-tm) felelősségi körébe tartozik az összes PCI DSS 3.2-es követelményeket sorolja fel.

A [Azure biztonsági és -megfelelőségi terv – PCI DSS Data Analytics megvalósítási mátrix](https://aka.ms/pcidss-analytics-cim) melyik PCI DSS 3.2-es verzióját a követelmények címzett által a data analytics architektúra, például részletes leírást biztosít a hogyan végrehajtására az egyes kezelt vezérlőelemek kielégíti.
