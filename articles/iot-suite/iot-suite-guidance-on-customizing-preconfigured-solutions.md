...
第一個工作會選取所有來自遙測傳入資料流的資料，並建立二個不同的輸出。 工作名為 **[方案名稱]-遙測**。
...
第二個工作在中建立裝置的臨界值 **規則** 解決方案的頁面。 此工作會使用參考資料 (針對各裝置設定的臨界值)。 它會比較臨界值以查看是否大於 (`>`) 實際值。 您可以修改此工作，例如變更比較運算子。

> [AZURE.NOTE] 遠端監視儀表板需要仰賴特定資料，因此變更作業可能會造成失敗的儀表板。
