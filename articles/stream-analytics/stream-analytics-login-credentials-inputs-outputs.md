...
1.  在 Azure 管理入口網站移至 SQL 資料庫] 擴充:  
![graphic14][graphic14]
2.  找出工作使用的 SQL 資料庫和 **在伺服器上按一下** 在同一行上的連結:  
![graphic15][graphic15]
3.  按一下 [管理] 命令:  
![graphic16][graphic16]
4.  輸入主要資料庫:  
...
6.  按一下 [新增查詢:  
![graphic19][graphic19]
7.  輸入下列查詢取代 <login_name> 與您的使用者名稱和取代 <enterStrongPasswordHere> 與您的新密碼:  
`建立登入 < i n _ > WITH PASSWORD = < enterStrongPasswordHere >`
8.  按一下 [執行]:  
![graphic20][graphic20]
9.  回到步驟 2，這次按一下資料庫:  
![graphic21][graphic21]
...
12. 按一下 [新增查詢:  
![graphic24][graphic24]
13. 型別中的下列查詢取代 <user_name> 與您想要識別此登入此資料庫的內容中所用的名稱 (您可以提供相同的值為您提供給 <login_name>, ，例如)，並取代 <login_name> 新的使用者名稱:  
`建立使用者 < s e _ > 從登入 < i n _ n >`
14. 按一下 [執行]:  
![graphic25][graphic25]
...
1.  在 Azure 管理入口網站移至資料流分析延伸模組:  
![graphic26][graphic26]
2.  找出您的工作，然後進到裡面:  
![graphic27][graphic27]
3.  請移至 [輸入] 或 [輸出] 索引標籤是否您是否想替換輸入或輸出的認證為基礎。  
![graphic28][graphic28]
4.  按一下 [停止] 命令，然後確認工作已停止:  
![graphic29][graphic29]
等候工作停止。
5.  找出您想要替換認證，然後進到裡面的輸入/輸出:  
![graphic30][graphic30]
6.  繼續第 3 部分。
...
1.  尋找 [使用者名稱和密碼] 欄位，然後貼上您新建立的一組認證:  
![graphic38][graphic38]
2.  按一下 [儲存] 命令，然後確認儲存所做的變更:  
![graphic39][graphic39]
3.  當您儲存所做的變更時，系統會自動測試連線，以保證萬無一失。
...
1.  返回到 [查詢] 視窗中從第 1 步驟 7，然後輸入下列查詢取代 <previous_login_name> 工作以前使用的使用者名稱:  
`卸除登入 < previous_login_name >`
2.  按一下 [執行]:  
    ![graphic43][graphic43]
