---
title: Azure Stream Analytics-feladatok üzembe helyezése CI/CD-vel és Azure DevOps-szal
...
* Hozzon létre egy [Azure DevOps-szervezetet.](https://visualstudio.microsoft.com/team-services/)
...
2. A **Team Explorer****Synchronization** (Szinkronizálás) nézetében válassza ki a **Push to Azure DevOps Services** (Leküldés az Azure DevOps Services szolgáltatásba) alatt található **Publish Git Repo** (Git-adattár közzététele) gombot.
   ...
   ![Lenyomás az Azure DevOps-szolgáltatások közzétételi Git-tárképre gombjára](./media/stream-analytics-tools-visual-studio-cicd-vsts/publish-git-repo-devops.png)
...
3. Ellenőrizze az e-mail-címet, és válassza ki a saját szervezetét az **Azure DevOps Services Domain** (Azure DevOps Services tartomány) legördülő listájából. Adja meg az adattár nevét, majd válassza ki a **Publish repository** (Adattár közzététele) lehetőséget.
   ...
   ![Nyomja meg a Git-tárház közzétételi tárházának gombját](./media/stream-analytics-tools-visual-studio-cicd-vsts/publish-repository-devops.png)

    Az adattár közzétételével egy új projekt jön létre a szervezetben a helyi adattáréval azonos néven. Ha egy meglévő projektben szeretné létrehozni a tárházat, kattintson **a Tárház** neve melletti Speciális **gombra,** és jelöljön ki egy projektet. A kód böngészőben való megtekintéséhez válassza a **See it on the web** (Megtekintés a weben) lehetőséget.
...
1. A **Build & Release** (Build és kiadás) lapon válassza a **Builds** (Buildek) és a **+ New** (+ Új) lehetőséget.  Válassza az **Azure DevOps Services Git** és a **Continue** (Folytatás) lehetőséget.
    ...
    ![Válassza a DevOps Git-forrást az Azure DevOps-ban](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-select-source-devops.png)
...
2. A **Select a template** (Sablon kiválasztása) területen kattintson az **Empty Process** (Üres folyamat) elemre, ha egy üres folyamattal kezdené meg a munkát.
    ...
    ![Üres folyamat kiválasztása a DevOps sablonbeállításai közül](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-select-template-empty-process.png)
...
3. A **Triggers** (Eseményindítók) lehetőségnél engedélyezze a folyamatos integrációt az **Enable continuous integration** (Folyamatos integráció engedélyezése) triggerállapot bejelölésével.  Válassza ki a **Save and queue** (Mentés és üzenetsorba helyezés) elemet a build manuális elindításához. 
    ...
    ![Folyamatos integrációs eseményindító állapotának engedélyezése](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-trigger-status-ci.png)

4. A buildek leküldés vagy bejelentkezés hatására is aktiválódnak. A build előrehaladásának ellenőrzéséhez **váltson** a Buildek lapra.  Miután meggyőződött arról, hogy a build sikeresen végrehajtásra került-e, meg kell határoznia egy kiadási folyamatot, amely az alkalmazást egy fürtre telepíti. Kattintson a jobb gombbal a buildelési folyamat melletti három pontra, és válassza az **Edit** (Szerkesztés) lehetőséget.

5.  A **Tasks** (Feladatok) között az **Agent queue** (Ügynöküzenetsor) megfelelőjeként adja meg a következőt: Hosted.
    
    ![Ügynökvárólista kiválasztása a Feladatok menüben](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-agent-queue-task.png) 

6. Kattintson a **Phase 1** (1. fázis) szakasz **+** elemére, majd adjon hozzá egy **NuGet**-feladatot.
    
    ![NuGet-feladat hozzáadása az Ügynökvárólistában](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-add-nuget-task.png)
...
8. Kattintson a **Phase 1** (1. fázis) szakasz **+** elemére, majd adjon hozzá egy **MSBuild**-feladatot.
   ...
   ![MSBuild feladat hozzáadása az ügynökvárólistában](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-add-msbuild-task.png)
...
9. Cserélje az **MSBuild-argumentumokat** a következőkre:
   ...
   ![MSBuild feladat konfigurálása a DevOps-ban](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-config-msbuild-task.png)
...
11. Bontsa ki az **Azure Details** (Azure – részletek) pontot, és töltse ki a konfiguráció mezőit a következők szerint:
    ...
    ![Az Azure Resource Group telepítésének tulajdonságainak beállítása](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-deployment-properties.png)

12. A buildelési folyamat teszteléséhez kattintson a **Save & Queue** (Mentés és várakozási sorba helyezés) gombra.
    
    ![Mentés és várólista-összeállítás a DevOps-ban](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-save-and-queue-build.png)
...
1. A Team Explorer **Changes** (Módosítások) nézetében adjon meg egy üzenetet a frissítés leírásával, majd véglegesítse a módosításokat.
    ...
    ![Tártermő módosítások véglegesítése a Visual Studio-ból](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-commit-changes-visual-studio.png)
...
2. Válassza ki a közzé nem tett változások ikonját az állapotsávon vagy a Sync (Szinkronizálás) nézetet a Team Explorerben. A **Push** (Leküldés) elem kiválasztásával frissítheti a kódot az Azure DevOps szolgáltatásban.
    ...
    ![Módosítások leküldése a Visual Studióból](./media/stream-analytics-tools-visual-studio-cicd-vsts/build-push-changes-visual-studio.png)
