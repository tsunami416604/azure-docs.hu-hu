...
在 Azure 管理入口網站中，按一下 **監視** 執行的資料流分析工作，以查看這些度量的索引標籤。 [監視] 頁面中出現了最多 1 分鐘的效能度量延遲。  

  ![監視工作儀表板](./media/stream-analytics-monitoring/01-stream-analytics-monitoring.png)  

在 Azure 預覽入口網站中，瀏覽至您想要查看的度量，並檢視資料流分析工作 **監視** 一節。  

  ![Azure Preview 入口網站監視工作儀表板](./media/stream-analytics-monitoring/06-stream-analytics-monitoring.png)  

串流分析工作第一次在區域中建立時，您必須設定適合該地區的診斷功能。 如果要這樣做，請按一下任何地方 **監視** 區段和 **診斷** ] 刀鋒視窗會出現。 您可以在這裡啟用診斷，並指定監視資料的儲存體帳戶。  
