...
串流分析可利用最佳化的更新插入方法，而此方法只會在因為 Document ID 發生衝突而插入失敗時，才進行更新。 串流分析會將更新作為修補程式執行，以部分更新文件，也就是以漸進方式新增新屬性或取代現有屬性。 請注意，在您整個陣列中取得覆寫，也就是陣列的 JSON 文件結果陣列屬性的值中的變更不會進行合併。
...
-   **集合名稱模式**：要使用之集合的集合名稱模式。 您可以使用選用的 {partition} 語彙基元來建構集合名稱的格式，其中的資料分割會從 0 開始。 以下是有效的輸入範例:  
