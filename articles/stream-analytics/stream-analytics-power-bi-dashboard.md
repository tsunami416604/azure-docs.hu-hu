...
按一下 [ **資料流分析** 以列出資料流分析工作的左窗格中。

![graphic1][graphic1]

> [AZURE.TIP] 會列出新的工作狀態為 **未啟動**。 請注意， **啟動** 頁面底部的按鈕會停用。 這是正常行為，因為您必須先設定工作輸入、輸出、查詢等項目，才能啟動工作。
...
* 按一下工作名稱。
* 按一下 [ **輸入** 上方的頁面上，然後按一下 [從 **加入輸入**。 開啟的對話方塊會逐步引導您完成設定輸入。
*   選取 **資料流**, ，然後按一下向右按鈕。
*   選取 **事件中心**, ，然後按一下向右按鈕。
*   在第三頁上輸入或選取下列值：
  * **輸入別名** -輸入此工作輸入的易記名稱。 請注意，此名稱將用於後續的查詢。
  * **事件中心** -如果您建立的事件中心中相同的訂閱資料流分析工作選取事件中樞所在的命名空間。
*   如果事件中樞位於不同的訂閱中，選取 **另一個訂用帳戶使用事件中心** 並手動輸入資訊 **服務匯流排命名空間**, ，**事件中樞名稱**, ，**事件中樞原則名稱**, ，**事件中樞原則金鑰**, ，和 **事件中心資料分割計數**。

> [AZURE.NOTE]  這個範例會使用預設的資料分割數目，即 16 個。

* **事件中樞名稱** -選取 Azure 事件中樞的名稱。
* **事件中樞原則名稱** -您使用的事件中樞選取事件中樞原則。 請確定此原則具有管理權限。
*   **事件中樞取用者群組** – 您可以保留空白，或指定事件中樞取用者群組。 請注意，事件中樞的每個用戶群組一次只能有 5 個讀取器。 因此，請為您的工作選擇正確的用戶群組。 如果您將欄位保留空白，它會使用預設的用戶群組。
...
1.  按一下 [ **輸出** 上方的頁面上，然後按一下 [從 **加入輸出**。 您會看到 Power BI 被列為輸出選項。

    ![graphic2][graphic2]  

2.  選取 **Power BI** ，然後按一下向右按鈕。
3.  您會看到類似下列畫面：

    ![graphic3][graphic3]  

4.  在此步驟中，提供串流分析工作輸出的公司帳戶或學校帳戶。 如果您已經有了 Power BI 帳戶，請選取 **立即授權**。 如果沒有，請選擇 **立即註冊**。 [以下是逐步解說如何註冊 Power BI 面的部落格文章](http://blogs.technet.com/b/powerbisupport/archive/2015/02/06/power-bi-sign-up-walkthrough.aspx)。
...
* **輸出別名** – 您可以將任何輸出別名，方便您參考。 如果您決定讓工作擁有多個輸出，填寫輸出別名會非常有幫助。 在這個情況下，您必須參考此查詢中的輸出。 比方說，我們要使用的輸出別名值 = "OutPbi"。
* **資料集名稱** -提供 Power bi 輸出應該要有一個資料集名稱。 例如，"pbidemo"。
...
*   按一下 [ **確定**, ，**測試連接** 和輸出組態就已經完成。

>   [AZURE.WARNING] 也請留意，如果 Power BI 已經有資料集和資料表名稱與您在此資料流分析工作中提供的名稱相同，將會覆寫現有資料。


## 撰寫查詢 ##

移至 **查詢** 您的工作] 索引標籤。 撰寫查詢，就是 Power BI 中要有的輸出。 例如，可能是類似下列 SQL 查詢的內容：
...
在 [值] 區段中，按一下下拉式清單向下針對溫度，然後選擇 **平均** 溫度和圖表上，按一下 **視覺化** 選擇 **折線圖**:
