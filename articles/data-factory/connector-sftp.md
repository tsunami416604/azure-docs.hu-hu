...
| jelszó | A felhasználó jelszava (userName). Megjelöli ezt a mezőt SecureString, hogy biztonságosan tárolja azt az adatgyárban, vagy [hivatkozzon az Azure Key vaultban tárolt titkos kulcsra](store-credentials-in-key-vault.md). | Igen |
...
| Jelszót | A titkos kulcs visszafejtéséhez adja meg a pass kifejezést vagy a jelszót, ha a kulcsfájl vagy a kulcs tartalmát egy pass kifejezés védi. Megjelöli ezt a mezőt SecureString, hogy biztonságosan tárolja azt az adatgyárban, vagy [hivatkozzon az Azure Key vaultban tárolt titkos kulcsra](store-credentials-in-key-vault.md). | Igen, ha a titkos kulcs fájlját vagy a kulcs tartalmát egy pass kifejezés védi. |
...
| Tulajdonság   | Leírás                                                  | Kötelező |
| ---------- | ------------------------------------------------------------ | -------- |
| típus       | Az *type* `location` adatkészletben található Type tulajdonságot *SftpLocation*értékre kell állítani. | Igen      |
...
| Tulajdonság                 | Leírás                                                  | Kötelező                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| típus                     | A *Type* tulajdonságot a `storeSettings` *SftpReadSettings*értékre kell állítani. | Igen                                           |
...
| Tulajdonság                 | Leírás                                                  | Kötelező |
| ------------------------ | ------------------------------------------------------------ | -------- |
| típus                     | A *Type* tulajdonságot a `storeSettings` *SftpWriteSettings*értékre kell állítani. | Igen      |
...
| Tulajdonság | Leírás | Kötelező |
|:--- |:--- |:--- |
| típus | Az adatkészlet *Type* tulajdonságát *fájlmegosztás*értékre kell állítani. |Igen |
...
| rekurzív | Azt jelzi, hogy az adatok rekurzív módon olvashatók-e az almappákból, vagy csak a megadott mappából. Ha a rekurzív értéke TRUE ( *igaz* ), a fogadó pedig egy fájl alapú tároló, az üres mappákat és almappákat a rendszer nem másolja vagy hozza létre a fogadón.<br/>Az engedélyezett értékek értéke *true* (alapértelmezett) és *false (hamis* ) | Nem |
| maxConcurrentConnections | A tárolási tárolóhoz egyidejűleg csatlakozható kapcsolatok száma. Csak akkor válasszon számot, ha szeretné korlátozni az egyidejű kapcsolatokat az adattárral. | Nem |
