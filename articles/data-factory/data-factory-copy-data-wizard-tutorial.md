...
> [AZURE.IMPORTANT] Olvassa el a [Tutorial overview and prerequisites](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md) (Az oktatóanyag áttekintése és előfeltételek) című cikket, hogy az oktatóanyag elvégzése előtt áttekinthesse azt, és végrehajthassa az **előfeltételként** felsorolt lépéseket.
...
3. Az erőforráscsoportban hajtsa végre a következő lépések egyikét: 1. Meglévő erőforráscsoport kiválasztásához kattintson a **Use existing** (Meglévő használata) elemre.
2. Az erőforráscsoport nevének megadásához válassza ki a **Create new** (Új létrehozása) lehetőséget.
    ...
    3. Válassza ki a Data Factory **helyét**.
    4. A panel alján jelölje be a **Pin to dashboard** (Rögzítés az irányítópulton) jelölőnégyzetet.  
...
## A Másolás varázsló elindítása és használata
...
9. A **Specify the Azure SQL database** (Az Azure SQL Database megadása) oldalon:
    ...
    3. Jelölje ki az Azure-**előfizetést**.  
    2. Válassza ki a **Server name** (Kiszolgálónév) és a **Database** (Adatbázis) mezők értékeit.
...
13. A [Monitor and manage pipelines using Monitoring App](data-factory-monitor-manage-app.md) (Folyamatok figyelése és felügyelete a Monitoring App használatával) című cikk útmutatásaiból megtudhatja, hogyan figyelheti a létrehozott folyamatot. A szelet megtekintéséhez kattintson a **Frissítés** ikonra az **ACTIVITY WINDOWS** (TEVÉKENYSÉGABLAKOK ) listában. 
    ...
    > [AZURE.NOTE] A legfrissebb állapot megtekintéséhez kattintson a **Refresh** (Frissítés) gombra az **ACTIVITY WINDOWS** (TEVÉKENYSÉGABLAKOK) lista alján. Automatikusan nem frissül. 
