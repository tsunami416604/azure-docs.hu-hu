...
3.  在 JSON 指令碼中，執行下列動作：
    ...
    2.  取代 * *<databasename>* * 使用 **MarketingCampaigns**。
    3.  如果您使用 **SQL 驗證**
        1.  指定 * *<username>** 和 **<password>* * 在 **connectionString**。
        2.  移除最後兩列 (僅當您使用 Windows 驗證時，才需要 **username** 和 **password** JSON 屬性)。
        3.  移除 **, ，(逗號) * * 結尾的 * * gatewayName** 資料列。

        **如果您使用 Windows 驗證:**
        1. 在 **connectionString** 中，將 **Integrated Security** 的值設為 **True**。移除"* * 使用者識別碼 =<username>;密碼 =<password>; * * 」 從連接字串。
...
1.  1. 在 **Data Factory 編輯器**中，按一下工具列的 [**新增管線**] 按鈕。 Click **...(省略符號)** 工具列中，如果您未看到此按鈕上。 或者，您也可以在樹狀檢視中，以滑鼠右鍵按一下 [**管線**]，再按一下 [**新增管線**]。
