...
對於主機，如果檔案共用位於閘道器電腦本身，您可以指定 **Local** 或 **localhost**。 我們建議使用 **encryptedCredential** 屬性，而不要使用 **userid** 和 **password** 屬性。 請參閱 [檔案系統連結服務](#onpremisesfileserver-linked-service-properties) 連結服務的詳細資訊。
...
**具有複製活動的管線：**
此管線包含複製活動，該活動已設定為使用上述輸入和輸出資料集並排定為每小時執行。 在管線 JSON 定義中，**source** 類型設為 **SqlSource**，而 **sink** 類型設為 **FileSystemSink**。 針對 **SqlReaderQuery** 屬性指定的 SQL 查詢會選取過去一小時內要複製的資料。
...
-------- | ----------- | --------
 ...
 encryptedCredential| 指定，就可以執行新增 AzureRmDataFactoryEncryptValue 指令程式加密的認證<p>**附註:** 您必須使用 Azure PowerShell 0.8.14 或更高版本，使用型別參數設為 OnPremisesFileSystemLinkedService 的 cmdlet，例如新增 AzureRmDataFactoryEncryptValue</p>| 否 (如果您選擇以純文字指定使用者識別碼和密碼)
...
每個資料集類型的 TypeProperties 區段都不同，可提供資料存放區中資料的位置、格式等相關資訊。 **FileShare** 資料集類型的 typeProperties 區段具有下列屬性。
 ...
 屬性| 說明| 必要
-------- | ----------- | --------
 folderPath| 資料夾的路徑。範例: myfolder<p>使用逸出字元 '\' 字串中的特殊字元。例如: 若為 folder\subfolder，指定 folder\\\subfolder 並為 d:\samplefolder，指定 d:\\\samplefolder。</p><p>可以將其與 **partitionBy** 將資料夾路徑以配量開始/結束日期時間。</p>| 是
 fileName| 如果您想要資料表參考資料夾中的特定檔案，請指定 **folderPath** 中的檔案名稱。如果未指定此屬性的任何值，資料表會指向資料夾中的所有檔案。<p>檔案名稱未指定輸出資料集，所產生檔案的名稱會在下列這種格式: </p><p>資料。<Guid>。txt (例如:: Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt</p>| 否
...
-------- | ----------- | --------
 ...
 escapeChar| 用來逸出內容中顯示之資料行分隔符號的特殊字元。沒有預設值。您必須指定這個屬性不能超過一個字元。<p>比方說，如果您以逗號 (，) 做為資料行分隔符號，但您想要在文字中有逗號字元 (範例:"Hello，world")，您可以定義 '$' 做為逸出字元，並使用字串"Hello$，world"中的來源。</p><p>請注意，您無法指定 escapeChar 和 quoteChar 資料表。</p>| 否
 quoteChar| 用來引用字串值的特殊字元。引號字元內的資料行和資料列分隔符號會被視為字串值的一部分。沒有預設值。您必須指定這個屬性不能超過一個字元。<p>比方說，如果您以逗號 (，) 做為資料行分隔符號，但您想要在文字中有逗號字元 (範例: <Hello, world>)，您可以定義 '"' 做為引用字元並使用字串 <"Hello, world"> 來源中。這個屬性不適用於輸入和輸出資料表。</p><p>請注意，您無法指定 escapeChar 和 quoteChar 資料表。</p>| 否
 nullValue| 用來代表 Blob 檔案內容中 null 值的字元。預設值為 “\N”.>| 否
 encodingName| 指定編碼名稱。如需有效編碼名稱的清單，請參閱：Encoding.EncodingName 屬性。<p>例如: windows-1250 或 shift_jis。預設值是: utf-8。</p>| 否
...
--------- | ------------ | --------
 true| preserveHierarchy| <p>具有下列結構的來源資料夾 Folder1:</p> <p>Folder1<br/>(& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp;File1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File2<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;Subfolder1<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File3<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File4<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File5</p>目標資料夾 Folder1 會有相同的結構做為來源<p>Folder1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File2<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;Subfolder1<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File3<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File4<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File5</p>.
 true| flattenHierarchy| <p>具有下列結構的來源資料夾 Folder1:</p> <p>Folder1<br/>(& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp;File1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File2<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;Subfolder1<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File3<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File4<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File5</p>Folder1 目標將會有下列結構: <p>Folder1<br/>(& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; 自動產生名稱 File1<br/>(& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; File2 自動產生名稱<br/>(& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; 自動產生名稱 File3<br/>(& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; File4 自動產生名稱<br/>(& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; File5 自動產生名稱</p>
 true| mergeFiles| <p>具有下列結構的來源資料夾 Folder1:</p> <p>Folder1<br/>(& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp;File1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File2<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;Subfolder1<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File3<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File4<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File5</p>Folder1 目標將會有下列結構: <p>Folder1<br/>(& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp;1 + 2 + File3 + File4 + 5 檔案內容會合併成一個檔案，以自動產生的檔案名稱</p>
 false| preserveHierarchy| <p>具有下列結構的來源資料夾 Folder1:</p> <p>Folder1<br/>(& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp;File1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File2<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;Subfolder1<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File3<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File4<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File5</p>目標資料夾 Folder1 會有下列結構<p>Folder1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File2<br/></p><p>Subfolder1 File3、 File4，與 File5 不挑選將</p>。
 false| flattenHierarchy| <p>具有下列結構的來源資料夾 Folder1:</p> <p>Folder1<br/>(& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp;File1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File2<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;Subfolder1<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File3<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File4<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File5</p>目標資料夾 Folder1 會有下列結構<p>Folder1<br/>(& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; 自動產生名稱 File1<br/>(& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; File2 自動產生名稱<br/></p><p>Subfolder1 File3、 File4，與 File5 不挑選將</p>。
 false| mergeFiles| <p>具有下列結構的來源資料夾 Folder1:</p> <p>Folder1<br/>(& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp;File1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;File2<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;Subfolder1<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File3<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File4<br/>(& s) nbsp; (& s) nbsp; & nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp; (& s) nbsp;File5</p>目標資料夾 Folder1 會有下列結構<p>Folder1<br/>(& s) nbsp; & nbsp; & nbsp; (& s) nbsp;1 + 2 的內容會合併成一個檔案，以自動產生的檔案名稱。自動產生名稱 File1</p><p>Subfolder1 File3、 File4，與 File5 不挑選將</p>。
