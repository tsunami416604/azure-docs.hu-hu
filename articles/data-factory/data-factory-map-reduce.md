...
6. 在 **arguments** 區段中，為 MapReduce 程式指定所有引數。 在執行階段，您會看到 MapReduce 架構的幾個額外的引數 (例如: mapreduce.job.tags)。 若要區別您使用 MapReduce 引數的引數，請考慮選項和值使用做為引數，如下列範例所示 (-s，--輸入-輸出等是後面緊跟著其值的選項)。
