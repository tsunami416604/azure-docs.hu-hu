...
1. 將連結服務的名稱設定為 **linkedServiceName**，該服務指向您的 HDInsight 叢集，串流 mapreduce 工作將會在該叢集上執行。
...
5. 針對 **input** 類型屬性，指定對應程式的輸入檔 (包括位置)。在範例中: 「 wasb://adfsample@<account name>.blob.core.windows.net/example/data/gutenberg/davinci.txt 」: adfsample 是 blob 容器、 範例/data/Gutenberg 是資料夾和 davinci.txt 是 blob。
