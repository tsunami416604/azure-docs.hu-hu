...
description: Azure Data Factory folyamatokkal (például Azure HDInsight) használható számítási környezetek az adatok átalakításához és feldolgozásához.
...
| [Azure Machine Learning Studio (klasszikus)](#azure-machine-learning-studio-classic-linked-service) | [Machine Learning Studio (klasszikus) tevékenységek: kötegelt végrehajtás és erőforrás frissítése](transform-data-using-machine-learning.md) |
...
| [Azure SQL](#azure-sql-database-linked-service), [Azure szinapszis Analytics](#azure-synapse-analytics-linked-service), [SQL Server](#sql-server-linked-service) | [Tárolt eljárás](transform-data-using-stored-procedure.md) |
...
|                           | hcatalogLinkedServiceName    | Az HCatalog-adatbázisra mutató Azure SQL társított szolgáltatás neve. Az igény szerinti HDInsight-fürtöt az Azure SQL Database metaadattár használatával hozza létre. | Nem   | Nem        | Igen          | Nem         |
...
| clusterNamePrefix           | A HDI-fürt nevének előtagja, az időbélyegző automatikusan hozzáfűzi a fürt nevének végén.| Nem       |
...
| hcatalogLinkedServiceName    | Az HCatalog-adatbázisra mutató Azure SQL társított szolgáltatás neve. Az igény szerinti HDInsight-fürt a Azure SQL Database metaadattár való használatával jön létre. | Nem       |
...
| Tulajdonság               | Leírás                              | Kötelező |
| :--------------------- | :--------------------------------------- | :------- |
| coreConfiguration      | Meghatározza a HDInsight-fürt létrehozásához szükséges alapvető konfigurációs paramétereket (core-site.xml). | Nem       |
...
| hiveConfiguration      | Meghatározza a HDInsight-fürt struktúrájának konfigurációs paramétereit (hive-site.xml). | Nem       |
...
| yarnConfiguration      | Megadja a HDInsight-fürthöz tartozó szál konfigurációs paramétereit (yarn-site.xml). | Nem       |
...
| Tulajdonság          | Leírás                              | Kötelező |
| :---------------- | :--------------------------------------- | :------- |
| headNodeSize      | Megadja a fő csomópont méretét. Az alapértelmezett érték a: Standard_D3. A részletekért tekintse **meg a csomópont-méretek megadása** szakaszt. | Nem       |
| dataNodeSize      | Megadja az adatcsomópont méretét. Az alapértelmezett érték a: Standard_D3. | Nem       |
| zookeeperNodeSize | Megadja az állatkert-megtartó csomópont méretét. Az alapértelmezett érték a: Standard_D3. | Nem       |

* A csomópontok méretének megadásával megtekintheti az előző szakaszban említett tulajdonságok megadásához szükséges karakterlánc-értékek Virtual Machines cikkének [méretét](../virtual-machines/sizes.md) . Az értékeknek meg kell felelniük a cikkben említett **parancsmagoknak & API** -kkal. Ahogy a cikkben látható, a nagyméretű (alapértelmezett) méret adatcsomópontja 7 GB memóriával rendelkezik, ami esetleg nem elég jó a forgatókönyvhöz. 
...
* Azure SQL DB, Azure szinapszis Analytics, SQL Server
...
| linkedServiceName | Az Azure Storage társított szolgáltatás neve, amely a HDInsight-fürt által használt Azure Blob Storage-tárolóra hivatkozik. <p>Jelenleg nem adhat meg Azure Data Lake Storage (2. generációs) társított szolgáltatást ehhez a tulajdonsághoz. Ha a HDInsight-fürt hozzáfér a Data Lake Storehoz, akkor a kaptár/Pig parancsfájlokból hozzáférhet a Azure Data Lake Storage (2. gen) adatokhoz. </p> | Igen      |
| isEspEnabled      | Ha a HDInsight-fürt [Enterprise Security Package](../hdinsight/domain-joined/apache-domain-joined-architecture.md) engedélyezve van, az "*igaz*" értéket kell megadni. Az alapértelmezett érték a "*false*". | Nem       |
...
Egy Azure Machine Learning Studio (klasszikus) társított szolgáltatást hoz létre egy Machine Learning Studio (klasszikus) batch-pontozási végpont egy adatgyárba való regisztrálásához.
...
| updateResourceEndpoint | A prediktív webszolgáltatás betanított fájllal való frissítéséhez használt Azure Machine Learning Studio (klasszikus) webszolgáltatás-végpont frissítési erőforrásának URL-címe | Nem                                       |
...
> [!IMPORTANT]
> A Databricks társított szolgáltatások támogatják a [példány-készleteket](https://aka.ms/instance-pools) & rendszer által hozzárendelt felügyelt identitásos hitelesítéssel.
...
| típus                 | A Type tulajdonságot a következőre kell beállítani: **Azure Databricks**. | Igen                                      |
| domain               | Az Azure-régiót ennek megfelelően határozza meg a Databricks munkaterület régiója alapján. Például: https://eastus.azuredatabricks.net | Igen                                 |
| accessToken          | A hozzáférési token szükséges ahhoz, hogy a Data Factory hitelesíthető legyen a Azure Databricks. A hozzáférési tokent a databricks munkaterületen kell létrehozni. A hozzáférési token megkeresésének részletes lépései [itt](https://docs.azuredatabricks.net/api/latest/authentication.html#generate-token) találhatók  | Nem                                       |
| MSI          | Data Factory felügyelt identitását (rendszer-hozzárendelésű) használja a hitelesítéshez Azure Databricks. Az "MSI" hitelesítés használatakor nincs szükség hozzáférési jogkivonatra  | Nem                                       |
...
| newClusterVersion    | A fürt Spark-verziója. Létrehoz egy databricks a fürtben. | Nem  |
...
| newClusterSparkConf  | választható, felhasználó által megadott Spark konfiguráció kulcs-érték párok halmaza. A felhasználók a Spark. Driver. extraJavaOptions spark.exeés a cutor. extraJavaOptions használatával további JVM lehetőségeket is adhatnak az illesztőprogramnak és a végrehajtóknak. | Nem  |
...
## <a name="azure-synapse-analytics-linked-service"></a>Azure szinapszis Analytics társított szolgáltatás

Létrehoz egy Azure szinapszis Analytics társított szolgáltatást, és a [tárolt eljárási tevékenységgel](transform-data-using-stored-procedure.md) felhasználja a tárolt eljárás meghívását egy Data Factory folyamatból. A társított szolgáltatás részleteiért lásd az [Azure szinapszis Analytics-összekötőt](connector-azure-sql-data-warehouse.md#linked-service-properties) ismertető cikket.
