...
> [!NOTE]
> Ebben a cikkben az Azure PowerShellt használjuk egy Azure SSIS IR üzembe helyezéséhez. A Data Factory felhasználói felületének (UI) használatával egy Azure SSIS integrációs modul üzembe helyezése, lásd: [oktatóanyag: Hozzon létre egy Azure SSIS integrációs modul](tutorial-create-azure-ssis-runtime-portal.md). 
...
- **Azure PowerShell**. Kövesse [az Azure PowerShell telepítését és konfigurálását](/powershell/azure/install-Az-ps) ismertető cikkben szereplő utasításokat. Egy PowerShell-lel futtatott szkripttel üzembe helyez egy Azure SSIS integrációs modult, amely a felhőben futtat SSIS-csomagokat. 
...
Másolja és illessze be az alábbi parancsfájlt: Adja meg a változók értékeit. Az Azure SQL Database támogatott **tarifacsomagjainak** listájáért lásd az [SQL Database erőforrás-korlátozásaival](../sql-database/sql-database-resource-limits.md) foglalkozó témakört.
...
Adja meg a következő szkriptet az Azure SQL-adatbáziskiszolgáló (`<servername>.database.windows.net`) ellenőrzéséhez. 
...
Most az SQL Server Data Tools (SSDT) vagy az SQL Server Management Studio (SSMS) segítségével helyezze üzembe az SSIS-csomagokat az Azure-ban. Csatlakozzon ahhoz az Azure SQL Serverhez, amelyen az SSIS-katalógus (SSISDB) található. Az Azure SQL-adatbáziskiszolgáló nevének formátuma a következő: `<servername>.database.windows.net`. 
...
5. Futtassa a szkriptet. A szkript végén lévő `Start-AzDataFactoryV2IntegrationRuntime` parancs futtatása **20–30 percet** vesz igénybe.

> [!NOTE]
> - A szkript csatlakozik az Azure SQL-adatbáziskiszolgálóhoz, és előkészíti az SSIS-katalógusadatbázist (SSISDB).
> 
...
Ha az Azure SQL Database-t virtuális hálózathoz csatlakozó virtuális hálózati szolgáltatásvégpontokkal/felügyelt példánnyal használja az SSISDB üzemeltetésére, az Azure-SSIS integrációs modulját is ugyanahhoz a virtuális hálózathoz kell csatlakoztatnia. Az Azure Data Factory használatával csatlakoztathatja az Azure-SSIS integrációs modult egy virtuális hálózathoz. További információkért tekintse meg [az Azure-SSIS integrációs modul virtuális hálózathoz történő csatlakoztatásával](join-azure-ssis-integration-runtime-virtual-network.md) foglalkozó cikket.
