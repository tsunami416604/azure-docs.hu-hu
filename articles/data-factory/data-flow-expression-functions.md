...
description: Az Azure Data Factory leképezési adatok Flow kifejezés függvény nyelve
author: kromerm
ms.author: makromer
...
A Stream választja ki a relatív pozíciója (1-alapú) oszlop értéke. Ha a pozice je mimo rozsah NULL értéket ad vissza. A visszaadott érték azt kell konvertálni a típuskonverziós (TO_DATE, TO_STRING...) egyik típusa Számított bemenetei között nem támogatottak, de használhatja a paraméter-helyettesítés
...
Az időbélyeg alakítja át UTC-ről. Az időzóna "GMT", "PST", 'UTC', "Amerika/Kajmán" formájában, lehetősége van átadni. Ez alapértelmezés szerint az aktuális időzóna
...
Napló értéket számít. Egy nem kötelező alapja lehet más euler szám megadott, ha a használt
* ``log(100, 10) -> 2``
*********************************
<code>log10</code>
==============================
...
Az NTile függvény a sorok be minden ablakot partíció osztja fel `n` legfeljebb 1-ről terjedő gyűjtők `n`. Legfeljebb 1 gyűjtőbe értékek eltérőek lehetnek. Ha a partíción sorok száma nem egyenlő arányban oszthatja azon gyűjtők számát, majd a hátralévő értékek gyűjtőbe, akkor az első gyűjtőbe kezdve minden elosztott egyet. Az NTile függvény különösen hasznos, tertiles kiszámítására, quartiles, deciles és egyéb gyakori összefoglaló statisztikák a függvény kiszámolja az inicializálás során két változót: Egy normál gyűjtő mérete hozzáadja egy extra sor lesz. Mindkét változót a jelenlegi partíció méretét alapulnak. A számítás során a függvény nyomon követi, hogy az aktuális sor száma, az aktuális gyűjtőbe száma és a sor számát, amelyen a gyűjtő (bucketThreshold) változik. Az aktuális sor száma eléri a küszöbértéket gyűjtőbe, amikor a gyűjtő érték eggyel nő, és a küszöbérték növekszik a gyűjtő mérete (és a egy extra, ha a jelenlegi gyűjtőhöz számokhoz).
* ``nTile() -> 1``
...
Bármely numerikus vagy karakterlánc egész értékké alakítja. Nem kötelező Java decimális formátum az átalakításhoz is használható. Minden olyan hosszú, lebegőpontos double csonkolja
* ``toInteger(123) -> 123``
* ``toInteger('123') -> 123``
* ``toInteger('$123', '$###') -> 123``
*********************************
...
Java HotSpot(TM) 64 bites Server rendszerű virtuális gép figyelmeztetés: a rendszer figyelmen kívül hagyja a beállítást MaxPermSize = 2048M. támogatása el lett távolítva, 8.0-s [sikeres] a teljes idő: 5 s, 2019 Diagramhalmazban 3 du. 4:17:11 befejeződött > runMain [info] com.microsoft.dataflow.FunctionPrinter összeállítása 1 Scala forrás-D:\kadfdataflow\DataflowExecutor\target\scala-2.11\classes... [hiba] D:\kadfdataflow\DataflowExecutor\src\main\scala\com\microsoft\dataflow\FunctionPrinter.scala:92: típusú alkalmazást nem engedélyezett az utótag-operátorokat [hiba] typeOf[SLICE.type], [hiba] ^ [error [hiba] egy hiba ] (adatfolyam / fordítási: fordítási) fordítása sikertelen volt [hiba] teljes idő: 24 s, 2019 Diagramhalmazban 3 du. 4:43:43 befejeződött >> runMain [info] com.microsoft.dataflow.FunctionPrinter összeállítása 1 Scala forrás-D:\kadfdataflow\DataflowExecutor\target\scala-2.11\classes... [információ] Futó com.microsoft.dataflow.FunctionPrinter átviteli dt_socket címen fogadja: 8000
*********************************
<code>abs</code>
==============================
<code><b>abs(<i>&lt;value1&gt;</i> : number) => number</b></code><br/><br/>
...
A karakterlánc értékét külön ékezetes unicode-karaktereket normalizálása
* ``normalize('boys') -> 'boys'``
*********************************
<code>not</code>
==============================
