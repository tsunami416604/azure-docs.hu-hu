...
Inverz szinuszos értéket számít ki.
...
Egy oszlop értékeinek átlagát kéri le.
...
A váltakozó feltételek alapján egy vagy több érték is érvényes. Ha a bemenetek száma páros, a másik alapértelmezett értéke NULL az utolsó feltétel esetében
...
A legkisebb olyan egész számot adja vissza, amely nem kisebb, mint a szám
...
Változó számú karakterlánc összefűzése egymással. Ugyanaz, mint a + operátor sztringekkel
...
Igaz értéket ad vissza, ha a megadott tömb bármely eleme igaz értékre értékeli a megadott predikátumban. A tartalmaz egy hivatkozást a predikátum függvény egyik elemére #item
...
Az értékek összesített számának beolvasása. Ha a választható oszlop (ok) meg van adva, a rendszer figyelmen kívül hagyja a NULL értékeket a darabszámban.
...
Egy adott oszlop különböző értékeinek összesített számát jeleníti meg.
...
A feltételek alapján lekéri az értékek összesített számát. Ha a választható oszlop meg van adva, a rendszer figyelmen kívül hagyja a NULL értékeket a darabszámban.
...
A két oszlop közötti sokasági eltérés beolvasása
...
Két oszlop minta-összetételének beolvasása
...
Kiszámítja a különböző primitív adattípusok oszlopának CRC32 kivonatát egy kis hosszúságú értékkel, amely csak 0 (256), 224, 256, 384, 512 értékű lehet. Egy sor ujjlenyomatának kiszámításához használható.
...
Az aktuális időbélyeg beolvasása, amikor a rendszer a feladatot a helyi időzónával kezdi futtatni
...
A hónap napját adja meg egy dátummal
...
Egy pár számot oszt szét. Ugyanaz, mint a/operátor
...
Mindig hamis értéket ad vissza. Ha van "false" nevű oszlop, használja a függvény szintaxisát (false ()).
...
Kiszűri a tömb azon elemeit, amelyek nem felelnek meg a megadott predikátumnak. A szűrő a predikátum függvény egy elemére mutató hivatkozást vár #item
...
Az UTC értékre konvertálja az időbélyeget. Igény szerint átadhatja az időzónát "GMT", "PST", "UTC", "Amerika/Kajmán" formátumban. Alapértelmezés szerint az aktuális timezoneRefer Java SimpleDateFormat elérhető formátumokhoz. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html
...
Egy időbélyeg óra értékének beolvasása. A választható időzónát "GMT", "PST", "UTC", "America/Cayman" formában adhatja át. A helyi időzónát használja a rendszer alapértelmezettként. Tekintse át a Java SimpleDateFormat az elérhető formátumokhoz. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html
...
Időtartam ezredmásodpercben az órák számához
...
A tömb minden elemét egy új elemre képezi le a megadott kifejezés használatával. A Térkép a kifejezés függvény egy elemére mutató hivatkozást vár #itemként, és az elem indexére mutató hivatkozást #index
...
Egy feltétel alapján egy vagy több érték is érvényes. Ha más nincs meghatározva, akkor NULL értékűnek számít. Mindkét értéknek kompatibilisnek kell lennie (numerikus, karakterlánc...)
...
Ellenőrzi, hogy az érték nem NULL, és visszaadja-e a másodlagos értéket. Teszteli az összes bemenetet, amíg meg nem találja az első nem null értéket
...
Minden szó első betűjét nagybetűssé alakítja. A szavak szóközzel elválasztva vannak azonosítva
...
Ellenőrzi, hogy a sor törlésre van-e megjelölve. Az egynél több bemeneti adatfolyamot használó átalakítások esetében az adatfolyam (1-alapú) indexe adható át. Az adatfolyam-index alapértelmezett értéke 1
...
Ellenőrzi, hogy a sor hibásként van-e megjelölve. Az egynél több bemeneti adatfolyamot használó átalakítások esetében az adatfolyam (1-alapú) indexe adható át. Az adatfolyam-index alapértelmezett értéke 1
...
Ellenőrzi, hogy a rendszer figyelmen kívül hagyja-e a sort. Az egynél több bemeneti adatfolyamot használó átalakítások esetében az adatfolyam (1-alapú) indexe adható át. Az adatfolyam-index alapértelmezett értéke 1
...
Ellenőrzi, hogy a sor be van-e jelölve a beszúráshoz. Az egynél több bemeneti adatfolyamot használó átalakítások esetében az adatfolyam (1-alapú) indexe adható át. Az adatfolyam-index alapértelmezett értéke 1
...
Ellenőrzi, hogy a sor egyezik-e a kereséssel. Az egynél több bemeneti adatfolyamot használó átalakítások esetében az adatfolyam (1-alapú) indexe adható át. Az adatfolyam-index alapértelmezett értéke 1
...
Annak ellenőrzése, hogy az érték NULL-e
...
Ellenőrzi, hogy a sor frissítésre van-e megjelölve. Az egynél több bemeneti adatfolyamot használó átalakítások esetében az adatfolyam (1-alapú) indexe adható át. Az adatfolyam-index alapértelmezett értéke 1
...
Az első paraméter értékének beolvasása n sor az aktuális sor előtt. A második paraméter a visszakeresett sorok száma, az alapértelmezett érték pedig 1. Ha nincs annyi sor, a rendszer null értéket ad vissza, kivéve, ha meg van adva egy alapértelmezett érték.
...
Az első paraméter értékének beolvasása az aktuális sor után n sorban. A második paraméter a megtekinteni kívánt sorok száma, az alapértelmezett érték pedig 1. Ha nincs annyi sor, a rendszer null értéket ad vissza, kivéve, ha meg van adva egy alapértelmezett érték.
...
Összehasonlítás kisebb vagy egyenlő operátorral. Ugyanaz, mint <= operator
...
A bal oldali Kitöltés után a karakterláncot a megadott kitöltéssel kell ellátni, amíg az adott hossz nem lesz. Ha a karakterlánc egyenlő vagy nagyobb, mint a hossz, a hossz
...
A bal oldali levágja a kezdő karakterek sztringjét. Ha a második paraméter nem lett megadva, a szóközöket vágja le. Egyéb esetben a második paraméterben megadott bármely karaktert felvágja.
...
A tömb minden elemét egy új elemre képezi le a megadott kifejezés használatával. A Térkép a kifejezés függvény egy elemére mutató hivatkozást vár #item
...
Kiszámítja a változó primitív adattípusú oszlopok MD5-kivonatát, és egy 32 karakteres hexadecimális karakterláncot ad vissza. Egy sor ujjlenyomatának kiszámításához használható.
...
Egy oszlop minimális értékének beolvasása
...
Számok kivonása. Kivonás a napok számától számítva. Kivonni időtartama egy időbélyegből. A kivonni két időbélyeget kap, hogy a különbség ezredmásodpercben legyen. Ugyanaz, mint a-operátor
...
Egy dátum vagy időbélyeg hónapjának értékének beolvasása
...
A számok összeszorzása. Ugyanaz, mint az * operátor
...
NULL értéket ad vissza. Ha van "NULL" nevű oszlop, használja a Function szintaxist (null ()). A által használt összes művelet NULL értéket fog eredményezni.
...
Egy számot vet fel egy másik hatványra
...
Egyező alsztring kinyerése egy adott regex-mintához. Az utolsó paraméter azonosítja az egyeztetési csoportot, és alapértelmezés szerint 1, ha nincs megadva. A "<regex>" (vissza idézőjel) használata a karakterláncnak a Escape nélkül való egyeztetéséhez
...
Ellenőrzi, hogy a karakterlánc megfelel-e a megadott regex-mintának. A "<regex>" (vissza idézőjel) használata a karakterláncnak a Escape nélkül való egyeztetéséhez
...
Egy regex-minta összes előfordulásának lecserélése egy másik, a megadott karakterláncban található alsztringre a "<regex>" (vissza idézőjel) használatával, hogy a karakterlánc a szökés nélkül egyezzen meg
...
Karakterlánc felosztása egy határolójel alapján, és a karakterláncok tömbjét adja vissza.
...
Szekvenciális sorszámozást rendel egy ablak soraihoz, 1-től kezdődően.
...
A jobb gombbal a karakterláncot a megadott kitöltés alapján kell kiadni, amíg az adott hosszúságú nem lesz. Ha a karakterlánc egyenlő vagy nagyobb, mint a hossz, a hossz
...
A jobb oldali levágja a kezdő karakterek egy sztringjét. Ha a második paraméter nem lett megadva, a szóközöket vágja le. Egyéb esetben a második paraméterben megadott bármely karaktert felvágja.
...
Egy dátum második értékét kéri le. A választható időzónát "GMT", "PST", "UTC", "America/Cayman" formában adhatja át. A helyi időzónát használja a rendszer alapértelmezettként. Tekintse át a Java SimpleDateFormat az elérhető formátumokhoz. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html
...
Időtartam ezredmásodpercben a másodpercek számát illetően
...
Kiszámítja a változó primitív adattípusok oszlopának SHA-1 kivonatát, és egy 40 karakteres hexadecimális karakterláncot ad vissza. Egy sor ujjlenyomatának kiszámításához használható.
...
Kiszámítja a változó primitív adattípusú oszlop SHA-2 kivonatát, amely egy kis hosszúságú, ami csak 0 (256), 224, 256, 384, 512 érték lehet. Egy sor ujjlenyomatának kiszámításához használható.
...
Kiszámítja a szinusz hiperbolikus értékét.
...
Egy adott feltétel alapján lekéri egy oszlop döntését
...
Egy tömb részhalmazának kibontása egy pozícióból. A pozíció 1 alapú. Ha a hossz nincs megadva, a rendszer alapértelmezés szerint a karakterlánc végét adja meg.
...
A karakterlánc Soundex-kódjának beolvasása
...
Karakterlánc felosztása elválasztó alapján, karakterláncok tömbjét adja vissza.
...
Kiszámítja egy szám négyzetének gyökerét.
...
Egy adott feltétel alapján lekéri egy oszlop szórását.
...
A feltételek alapján lekéri egy oszlop sokasági szórását
...
A feltételek alapján lekéri egy oszlop szórását.
...
Hónapok kivonása egy dátumból vagy időbélyegből. Ugyanaz, mint a-operátor a dátumhoz
...
Hónapok kivonása dátumból vagy időbélyegből
...
A feltételek alapján lekéri egy numerikus oszlop összesített összegét. A feltétel bármely oszlopon alapulhat
...
A ("", "true", "y", "yes", "1") értéket True értékre ("f", "false", "n", "No", "0") konvertálja hamis értékre, és NULL értéket bármilyen más értékhez.
...
A bemeneti dátum karakterláncának dátumra konvertálása egy opcionális bemeneti dátumformátum használatával. Tekintse át a Java SimpleDateFormat az elérhető formátumokhoz. Ha a bemeneti dátum formátuma nincs megadva, az alapértelmezett formátum: ÉÉÉÉ-[M] M-[d] d. Az elfogadott formátumok a következők: [éééé, éééé-[M] M, éééé-[M] M-[d] d, éééé-[M] M-[d] dT *]
...
Bármely numerikus vagy sztringet lebegőpontos értékké alakít. Az átalakításhoz választható Java decimális formátumot is használhat. Bármilyen dupla csonkítás
...
Bármilyen numerikus vagy sztringet egész értékre konvertál. Az átalakításhoz választható Java decimális formátumot is használhat. Levágja a hosszú, lebegőpontos, dupla
...
Bármilyen numerikus vagy sztringet alakít át egy rövid értékre. Az átalakításhoz választható Java decimális formátumot is használhat. Egész szám, hosszú, lebegőpontos, dupla érték csonkítása
...
Az időbélyeget UTC értékre alakítja. A választható időzónát "GMT", "PST", "UTC", "America/Cayman" formában adhatja át. Alapértelmezés szerint az aktuális timezoneRefer Java SimpleDateFormat elérhető formátumokhoz. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html
...
Egy kezdő és záró karakterből álló karakterláncot metsz. Ha a második paraméter nem lett megadva, a szóközöket vágja le. Egyéb esetben a második paraméterben megadott bármely karaktert felvágja.
...
Mindig igaz értéket ad vissza. Ha van "true" nevű oszlop, használja a Function szintaxist (true ()).
...
Egy oszlop eltérésének beolvasása
...
A feltételek alapján lekéri egy oszlop varianciaát.
...
Egy oszlop elfogulatlan eltérésének beolvasása
...
Az év hetet adja meg egy dátummal
