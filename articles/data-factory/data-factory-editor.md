...
![Azure Data Factory 編輯器][data=factory-editor]
...
2. 右側的**編輯器畫布**會顯示 JSON 範本，讓您建立儲存體連結服務。 您在 [**草稿**] 下也會看到草稿節點。 執行下列動作：
    1. 如 **Azure 儲存體**: 取代 * *<accountname\>** 和 **<accountkey\>* * 使用 Azure 儲存體帳戶的名稱和金鑰。
    2. 如 **Azure SQL database**: 取代 * *<servername\>** 取代為您的 Azure SQL server 名稱 **<databasename\>** 的資料庫名稱 **<username\>@<servername\>** 的使用者名稱和 **<password\>* * 取代為使用者帳戶的密碼。
    3. 如 **內部部署 SQL server 資料庫**: 取代 * *<servername\>** 取代為您的內部部署 SQL server 名稱 **<databasename\>** 的資料庫名稱 **<username\>** 的使用者名稱和 **<password\>* * 取代為使用者帳戶的密碼。
...
如果您要將資料從 Azure SQL Database 複製到 Azure Blob 儲存體，並在 HDInsight 叢集上使用 Pig 指令碼來處理 Blob 儲存體中的資料，請先將**複製活動**加入至管線，再將 **Pig 活動**加入至管線。 這會建立兩個區段活動的管線 JSON 的 [] 一節。 Pig 活動只是 HDInsight 活動再加上 Pig 轉換。
...
請參閱 [開始使用 ][data-factory-get-started] 主題，以取得使用 Data Factory 編輯器建立 Azure data factory 的逐步指示。
