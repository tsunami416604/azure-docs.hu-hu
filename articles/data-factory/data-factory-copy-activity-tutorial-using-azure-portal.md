...
1.  Miután bejelentkezett az [Azure Portalra][azure-portal], kattintson az **ÚJ** gombra a bal alsó sarokban, válassza az **Adatelemzés** lehetőséget a **Létrehozás** panelen, és kattintson a **Data Factory** elemre az **Adatelemzés** panelen. 
...
1. A **Data Factory Editorban** kattintson az eszköztár **New data store** (Új adattár) gombjára, és a legördülő menüben válassza az **Azure SQL database** (Azure SQL adatbázis) lehetőséget. A jobb oldali panelen megjelenik az Azure SQL társított szolgáltatás létrehozására szolgáló JSON-sablon.
...
Az előző lépésben létrehozta az **AzureStorageLinkedService** és az **AzureSqlLinkedService** társított szolgáltatásokat egy Azure Storage-fiók és egy Azure SQL Database az **ADFTutorialDataFactoryPSH** data factoryhez való társításához. Ebben a lépésben két data factory-táblát határoz meg – az **EmpTableFromBlob** és az **EmpSQLTable** táblákat –, amelyek az adattárakban tárolt, az AzureStorageLinkedService és az AzureSqlLinkedService szolgáltatások által hivatkozott bemeneti és kimeneti adatokat jelölik. Az EmpTableFromBlob táblához megadja a forrásadatokkal rendelkező blobot tároló blobtárolót, az EmpSQLTable táblához pedig megadja a kimeneti adatokat tároló SQL-táblát. 

### Bemeneti adatkészlet létrehozása 
A tábla egy sémával rendelkező, téglalap alakú adatkészlet. Ebben a lépésben létrehozza az **EmpBlobTable** nevű táblát, amely az Azure Storage **AzureStorageLinkedService** társított szolgáltatás által jelölt blobtárolójára mutat.

1. A Data Factory **szerkesztőjében** kattintson az eszköztár **New dataset** (Új adatkészlet) gombjára, és a legördülő menüben válassza a **Blob table** (Blobtábla) lehetőséget. 
...
1. Ha még nincs megnyitva, lépjen az [Azure Portalra (előzetes verzió)][azure-portal]. 
...
5. A **Datasets** (Adatkészletek) panelen kattintson az **EmpTableFromBlob** elemre. Ez az adatkészlet az **ADFTutorialPipeline** bementi adatkészlete.
...
6. Most kattintson a **Datasets** (Adatkészletek) panelen az **EmpTableFromBlob** elemre. Ez az adatkészlet az **ADFTutorialPipeline** kimenti adatkészlete.

    ![Datasets (Adatkészletek) panel][image-data-factory-get-started-datasets-blade]

6. Az **EmpSQLTable** panelnek a következő képen látható módon kell megjelennie:
