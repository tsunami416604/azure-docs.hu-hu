...
A Salesforce Service Cloud-jelentésekben lévő adatok lekéréséhez egy lekérdezést kell megadnia `{call "<report name>"}`ként. Például: `"query": "{call \"TestReport\"}"`.
...
Ha le szeretné kérdezni a Salesforce szolgáltatás Felhőbeli lomtárának helyreállított törölt rekordjait, megadhatja `readBehavior` `queryAll`ként. 
