---
title: 'Oktatóanyag: Hozzon létre egy Azure Data Factory-folyamatot másolhat adatokat (az Azure portal) |} A Microsoft Docs'
description: Az oktatóanyagban az Azure Portal használatával hoz létre egy másolási tevékenységgel rendelkező Azure Data Factory-folyamatot az adatok Azure Blob Storage-ból egy Azure SQL-adatbázisba való másolásához.
...
ms.locfileid: "67839510"
---
# <a name="tutorial-use-azure-portal-to-create-a-data-factory-pipeline-to-copy-data"></a>Oktatóanyag: Hozzon létre egy Data Factory-folyamatot az adatok másolása az Azure portal használatával 
...
> [!WARNING]
> A JSON-szerkesztő szerzői és üzembe helyezése az ADF v1 folyamatok lesz az Azure Portalon a 2019. július 31-edik. ki van KAPCSOLVA. Miután. július 31-edik 2019, továbbra is használni [ADF v1 Powershell-parancsmagok](https://docs.microsoft.com/powershell/module/az.datafactory/?view=azps-2.4.0&viewFallbackFrom=azps-2.3.2), [ADF v1 .net SDK](https://docs.microsoft.com/dotnet/api/microsoft.azure.management.datafactories.models?view=azure-dotnet), [ADF v1 REST API-k](https://docs.microsoft.com/rest/api/datafactory/) folyamatok készítése és üzembe helyezése az ADF v1.
...
2. Hozzon létre **társított szolgáltatásokat** az adat-előállítóban. Ebben a lépésben két típusú társított szolgáltatást hoz létre: Az Azure Storage és az Azure SQL-adatbázis. 
...
2. A **New data factory** (Új data factory) panelen:
   ...
   6. Kattintson a **Create** (Létrehozás) gombra.
      ...
      > 
      > 
3. Az irányítópulton a következő állapotleírás látható: **Data factory üzembe helyezése**. 
...
### <a name="create-a-linked-service-for-azure-sql-database"></a>Az Azure SQL Database társított szolgáltatás létrehozása
...
1. A **Data Factory Editorban** kattintson az eszköztár **Új adattároló** gombjára, és a legördülő menüben válassza az **Azure SQL Database** lehetőséget. A jobb oldali panelen megjelenik az Azure SQL társított szolgáltatás létrehozására szolgáló JSON-sablon.
...
2. A jobb oldali panelen cserélje le a JSON-t a következő JSON-kódrészletre: 
   ...
   - Mind a kezdő, mind a befejező dátum-időpont értéket [ISO formátumban](https://en.wikipedia.org/wiki/ISO_8601) kell megadni. Példa: 2016-10-14T16:32:41Z. Az **end** (befejező) időpont megadása opcionális, a jelen oktatóanyagban azonban azt is használjuk. Ha nem adja meg az **end** (befejezés) tulajdonság értékét, akkor a rendszer a „**kezdő időpont + 48 óra**” számítással határozza meg azt. A folyamat határozatlan ideig történő futtatásához adja meg a **9999-09-09** értéket az **end** (befejezés) tulajdonsághoz.
...
### <a name="monitor-the-pipeline-using-diagram-view"></a>A Diagram nézet használata a folyamat figyelése
...
Az oktatóanyag során létrehozott egy Azure data factoryt, hogy adatokat másoljon egy Azure-blobból egy Azure SQL-adatbázisba. Az Azure Portalt használta a data factory, a társított szolgáltatások, az adatkészletek és a folyamat létrehozásához. Az oktatóanyag során a következő főbb lépéseket végezte el:  
...
2. **Társított szolgáltatásokat** hozott létre:
   ...
   2. Egy **Azure SQL** társított szolgáltatást a kimeneti adatokat tároló Azure SQL-adatbázis társításához. 
