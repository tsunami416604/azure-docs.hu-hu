...
| --- | --- | --- |
| type | A Type tulajdonságot a következőre `Sftp`kell beállítani:. |Igen |
...
Az SSH nyilvános kulcsú hitelesítés használatához állítsa `authenticationType` a `SshPublicKey`as lehetőséget, majd adja meg az alábbi tulajdonságokat az SFTP-összekötő általános beállításai mellett, az utolsó szakaszban bemutatott módon:
...
Ez az adatkészlet az SFTP-mappára `mysharedfolder` és- `test.csv`fájlra hivatkozik. A folyamat átmásolja a fájlt a célhelyre.
