...
| jelszó |A felhasználó jelszavát. Ez a mező megjelölése SecureString. |Igen (Ha alapszintű hitelesítést használ). |
...
> [!TIP]
> Ha meg az SQL-lekérdezést, vegye figyelembe a dátum és idő formátumban. Például:`$$Text.Format('SELECT * FROM Account WHERE LastModifiedDate >= {{ts\\'{0:yyyy-MM-dd HH:mm:ss}\\'}} AND LastModifiedDate < {{ts\\'{1:yyyy-MM-dd HH:mm:ss}\\'}}', <datetime parameter>, <datetime parameter>)`
