...
A fájl mentése `<username>`előtt cserélje `<password>` le `<servername>`a,,, és értékeit a SQL Serverra. `<databasename>` És cserélje le `<integration runtime name>` az értékét az Integration Runtime nevére. 
