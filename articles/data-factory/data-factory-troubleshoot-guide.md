...
- **Javaslat**: Adjon meg abszolút elérési utakat a munkaterület-címzési sémák vagy `dbfs:/folder/subfolder/foo.py` a Databricks fájlrendszerben tárolt fájlokhoz.
...
- **Javaslat**: A Databricks-munkaterületet használó összes folyamatnál keresse meg a feladatok létrehozási arányát.  Ha a folyamatok túl sok Databricks-t futtatnak összesítésben, telepítsen át bizonyos folyamatokat egy új munkaterületre.
...
- **Ok**: A hozzáférési jogkivonatot létrehozó felhasználó nem férhet hozzá a társított szolgáltatásban megadott Databricks-fürthöz.
...
- **Ok**:  Ezt a hibát a Data Lake Analytics szabályozása okozta.

- **Javaslat**: Csökkentse Data Lake Analytics elküldött feladatok számát a tevékenységek Data Factory eseményindítók és Egyidejűség beállításainak módosításával. Vagy növelje Data Lake Analytics korlátozásait.
...
- **Javaslat**: Győződjön meg arról, hogy a felhasználó által Data Lake Analytics feladatokhoz megadott egyszerű szolgáltatásnév vagy tanúsítvány hozzáfér-e a Data Lake Analytics fiókhoz és a gyökérmappa alapértelmezett Data Lake Storage példányához.
...
- **Javaslat**: Ellenőrizze a társított szolgáltatásban megadott elérési utat és hitelesítő adatokat.
...
- **Javaslat**: A támogatott http-metódusok a következők: PUT, POST, GET, DELETE, OPTIONs, HEAD és TRACE.
...
- **Javaslat**: Frissítse az Azure-függvényt egy érvényes JSON-adattartalom visszaküldéséhez. Például C# egy `(ActionResult)new<OkObjectResult("{`függvény visszatérhet\"\"\"a következő azonosítóval:123`}");`. \"
...
- **Javaslat**: Győződjön meg arról, hogy a bemeneti AzureFunction tevékenység JSON-definíciója "functionKey" nevű tulajdonsággal rendelkezik.
...
- **Javaslat**: Győződjön meg arról, hogy a bemeneti AzureFunction tevékenység JSON-definíciója "függvénynév" nevű tulajdonsággal rendelkezik.
...
- **Ok**: Lehet, hogy az Azure-függvény részletei a tevékenység-definícióban helytelenek. 
...
- **Javaslat**: Győződjön meg arról, hogy a bemeneti AzureFunction tevékenység JSON-definíciója "functionAppUrl" nevű tulajdonsággal rendelkezik.
...
- **Ok**: Lehetséges, hogy a függvény URL-címe helytelen.
...
- **Javaslat**:  Győződjön meg arról, hogy a végrehajtható fájl létezik. Ha a program elindult, győződjön meg róla, hogy az *StdOut. txt* és a *stderr. txt fájl* fel lett töltve a Storage-fiókba. Érdemes lehet bőséges naplókat kibocsátani a kódban a hibakereséshez.
...
- **Ok**: Túl sok fájl szerepel az egyéni tevékenység folderPath. A resourceFiles teljes mérete nem lehet hosszabb 32 768 karakternél.
...
- **Ok**: A Batch URL-címeinek a következőhöz hasonlónak kell lenniük`https://mybatchaccount.eastus.batch.azure.com`
...
- **Ok**: A feladatok beküldésének időkorlátja lejárt. 
...
- **Javaslat**: Ez a hiba a HDInsight-fürtből származik. További információ: [Ambari felhasználói felület 502 hiba](https://hdinsight.github.io/ambari/ambari-ui-502-error.html), [502 hiba a Spark](https://hdinsight.github.io/spark/spark-thriftserver-errors.html)-beli szolgáltatáshoz való csatlakozáshoz, [502-hibák a Spark-hoz](https://hdinsight.github.io/spark/spark-thriftserver-errors.html)való csatlakozáshoz és [a Application Gateway hibás átjárókkal kapcsolatos hibák elhárítása](https://docs.microsoft.com/azure/application-gateway/application-gateway-troubleshooting-502).
...
- **Ok**: A rendszer elküldte a feladatot a HDInsight-nek, és az HDInsight-on meghiúsult.
...
- **Ok**: Ez a hiba akkor fordul elő, ha a feladatot egy Spark-fürthöz küldi el a rendszer. 
...
- **Ok**: A webes tevékenység nem támogatja a tevékenység-adattartalomban megadott HTTP-metódust.

- **Javaslat**:  A támogatott HTTP-metódusok a következők: PUT, POST, GET és DELETE.

<br/>

- **Üzenet**:`Invalid Server Error 500.`

- **Ok**: Belső hiba történt a végponton.
...
- **Javaslat**:  Lehet, hogy a jogkivonat lejárt. Adjon meg egy érvényes hitelesítési módszert. A Hegedűs vagy a Poster használatával keresse meg a funkciót az URL-címen.
...
- **Javaslat**:  A hozzáfért erőforráshoz tartozó felhasználói engedélyek megtekintése. A Hegedűs vagy a Poster használatával keresse meg a funkciót az URL-címen.
...
- **Javaslat**:  A kérelem érvényesítéséhez a Hegedűs vagy a Poster használatával.
