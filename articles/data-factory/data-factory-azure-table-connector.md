...
此管線包含複製活動，該活動已設定為使用上述輸入和輸出資料集並排定為每小時執行。 在管線 JSON 定義 **來源** 類型設為 **AzureTableSource** 和 **接收器** 類型設為 **BlobSink**。 使用指定的 SQL 查詢 **AzureTableSourceQuery** 屬性選取的資料從預設分割每隔一小時來複製。
...
此管線包含複製活動，該活動已設定為使用上述輸入和輸出資料集並排定為每小時執行。 在管線 JSON 定義 **來源** 類型設為 **BlobSource** 和 **接收器** 類型設為 **AzureTableSink**。 
...
每個資料集類型的 typeProperties 區段都不同，可提供資料存放區中資料的位置相關資訊。  **TypeProperties** 類型資料集區段 **AzureTable** 具有下列屬性。
