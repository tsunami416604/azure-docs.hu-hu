...
| Szakaszos másolás           | Ha a szakaszos másolás konfigurálva van, de nem hasznos a forrás-fogadó pár számára, javasoljuk, hogy távolítsa el azt. |
...
| Transzfer        | Az előző lépés vége és a forrás és a fogadó közötti összes adatok átvitele között eltelt idő. Az "átvitel" alatt az allépések párhuzamosan futnak.<br><br>- az **első bájtig** eltelt idő: az előző lépés vége és az az idő, amikor az IR megkapja az első bájtot a forrás adattárból. A nem fájl alapú forrásokra vonatkozik.<br>- **listázási forrás:** a forrásfájlok vagy adatpartíciók számbavételére fordított idő mennyisége. Az utóbbi akkor érvényes, ha az adatbázis-források partíciós beállításait konfigurálja, például az adatok olyan adatbázisokból való másolásakor, mint például az Oracle/SAP HANA/Teradata/Netezza/etc.<br/>-a **forrástól:** az adatok forrás adattárból való beolvasásához felhasznált idő mennyisége.<br/>- a fogadóba való **írás során:** az adattárolási adattárba való adatírás során eltöltött idő mennyisége. |
...
- **"A továbbítási lista forrása" hosszú munkaidőtartamot tapasztalt**: a forrásfájlok vagy a forrás-adatbázis adatpartícióinak számbavétele lassú.
  ...
  - Ha fájl alapú forrásból másol adatokat, ha **helyettesítő szűrőt** használ a mappa elérési útján vagy a fájlnéven (`wildcardFolderPath` vagy `wildcardFileName`), vagy használja a **fájl utolsó módosításának időszűrőjét** (`modifiedDatetimeStart` vagy`modifiedDatetimeEnd`), vegye figyelembe, hogy az ilyen szűrő a másolási tevékenységet fogja eredményezni a megadott mappában lévő összes fájlnak a kliens oldalára való másolása után, majd alkalmazza a szűrőt. Ilyenkor előfordulhat, hogy a fájl enumerálása különösen szűk keresztmetszetet jelenthet, ha csak a fájlok kis halmaza felel meg a szűrési szabálynak.
...
- **"A forrástól való olvasás" hosszú munkaidőtartamot tapasztalt**: 
  ...
  - A másolási forrás és a fogadó minta keresése: 
    ...
    - Ha a másolási minta támogatja a 4 adatintegrációs egységet (DIUs), tekintse meg [ezt a szakaszt](copy-activity-performance.md#data-integration-units) a részletek részben, és a jobb teljesítmény érdekében próbálkozzon a DIUs növelésével. 
