...
---
# <a name="schedule-tasks-and-workflows-that-run-regularly-with-logic-apps"></a>Feladatok ütemezése és a logic apps rendszeresen futtatott munkafolyamatok

Ütemezett feladatok, a műveletek, a munkaterhelés vagy a rendszeresen futtatott folyamatokat, kezdetű logic app munkafolyamat hozhat létre a **ütemezés - ismétlődési** [eseményindító](../logic-apps/logic-apps-what-are-logic-apps.md#logic-app-concepts). Az ehhez az eseményindítóhoz állíthatja be egy dátumot és időpontot a ismétlődési és feladatokat, mint az ezekben a példákban ismétlődés ütemezésének indítása:
...
1. Jelentkezzen be az [Azure Portalra](https://portal.azure.com). Hozzon létre egy üres logikai alkalmazást, vagy további [egy üres logikai alkalmazás létrehozása](../logic-apps/logic-apps-create-a-logic-app.md).
...
| Név | Szükséges | Tulajdonság neve | Típus | Leírás | 
|----- | -------- | ------------- | ---- | ----------- | 
...
| **Időzóna** | Nem | Időzóna | Karakterlánc | Vonatkozik, csak ha a kezdési időpontot, mert az eseményindító nem fogadja el [UTC eltolás](https://en.wikipedia.org/wiki/UTC_offset). Válassza ki az alkalmazni kívánt időzónát. | 
...
| **Napjainkban a** | Nem | weekDays | Karakterlánc vagy karakterlánctömb | Ha "Hetente", ha azt szeretné, a munkafolyamat futtatásához egy vagy több napot kiválaszthatja: **hétfő**, **kedd**, **szerda**, **csütörtök** , **Péntek**, **szombat**, és **vasárnap** | 
| **Ezek az órák:** | Nem | hours | Egész szám vagy az egész tömb | Ha bejelöli a "Day" vagy "Hetente", választhat egy vagy több egész számok 0 – 23 a nap, ha azt szeretné, a munkafolyamat futtatási óra. <p>Például ha megadja a "10", "12" és "14", kapott 2 PM, az óra közötti, de 10, 12 PM. | 
| **Ezek perccel** | Nem | minutes | Egész szám vagy az egész tömb | Ha bejelöli a "Day" vagy "Hetente", választhat egy vagy több egész számok 0 és 59 az óra, ha azt szeretné, a munkafolyamat futtatási perc. <p>Például megadhatja a perc megjelölés "30", és elérhetővé az előző példa óra, nap, 10:30 AM, 12:30 PM és 2:30 PM. | 
||||| 

## <a name="json-example"></a>JSON-példa

Íme egy példa ismétlődési eseményindító definícióját:
...
| Ismétlődés | időköz | gyakoriság | Kezdési idő | E napokon | Órák | Percek | Megjegyzés |
| ---------- | -------- | --------- | ---------- | ------------- | -------------- | ---------------- | ---- |
...
| 15 percenként futtatni (a kezdő dátum és idő) | 15 | Perc | *a startDate*T*startTime*Z | {nem érhető el} | {nincs} | {nincs} | Az ütemezés nem indul el *bármely hamarabb* a megadott kezdési dátum és idő, mint jövőbeli ismétlődések a legutóbbi futtatás ideje alapján számítja ki. | 
| Minden órában, futtassa az óra (a kezdő dátum és idő) | 1 | Óra | *a startDate*Thh:00:00Z | {nem érhető el} | {nincs} | {nincs} | Az ütemezés nem indul el *bármely hamarabb* mint a megadott kezdési dátuma és időpontja. Jövőbeli ismétlődések óránként futtassa a "00" minute be van jelölve. <p>Ha a gyakoriság "Hét" vagy "Honap", az ütemezés rendre fut csak egy naponta, hetente vagy havonta egy nap. | 
...
| Futtassa a óránként, naponta (a kezdő dátum és idő) | 1 | Óra | *a startDate*T*startTime*Z | {nem érhető el} | {nincs} | {nincs} | Az ütemezés nem indul el *bármely hamarabb* a megadott kezdési dátum és idő, mint jövőbeli ismétlődések a legutóbbi futtatás ideje alapján számítja ki. <p>Ha a gyakoriság "Hét" vagy "Honap", az ütemezés rendre fut csak egy naponta, hetente vagy havonta egy nap. | 
| Futtassa a óránként túlra 15 percenként, minden órában (a kezdő dátum és idő) | 1 | Óra | *a startDate*T00:15:00Z | {nem érhető el} | {nincs} | {nincs} | Az ütemezés nem indul el *bármely hamarabb* mint a megadott kezdési dátuma és időpontja, éjfélre (00:15), 1:15 AM, 2:15 AM, fut, és így tovább. | 
...
| 8:00 órakor minden nap (a kezdő dátum és idő) futtatása | 1 | Nap | *a startDate*T08:00:00Z | {nem érhető el} | {nincs} | {nincs} | Az ütemezés 8:00-kor naponta, fut, a megadott kezdési időponttal alapján. | 
...
| 8:30 AM minden nap (a kezdő dátum és idő) futtatása | 1 | Nap | *a startDate*T08:30:00Z | {nem érhető el} | {nincs} | {nincs} | Az ütemezés a megadott kezdési dátum: 8:30 AM elindítja. | 
...
| Futtat minden szombat hajnali (a kezdő dátum és idő) délután 5 óra | 1 | Hét | *a startDate*T17:00:00Z | "Szombat" | {nincs} | {nincs} | Az ütemezés nem indul el *bármely hamarabb* mint a megadott kezdési dátuma és időpontja, ebben az esetben 2017. szeptember 9., 5:00 PM. Jövőbeli ismétlődések, 5:00 PM futtat minden szombat. | 
...
| Kezdési idő | Ismétlődés ütemezés nélkül | Ismétlődés ütemezéssel | 
| ---------- | --------------------------- | ------------------------ | 
...
| Kezdési idő | Első futási idő | Futtatási idejének jövőbeli | 
| ---------- | ------------ | ---------- | 
