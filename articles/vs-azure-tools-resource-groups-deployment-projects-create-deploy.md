...
1. 展開部署專案中的節點以查看建立的內容。
    ...
    因為我們選擇此範例的 Web 應用程式範本，所以您會看到下列檔案。 
  	...
  	|檔案名稱|說明|
  	|---|---|
  	|Deploy-AzureResourceGroup.ps1|叫用 PowerShell 命令部署至 Azure 資源管理員的 PowerShell 指令碼。<br />**請注意** 這個 PowerShell 指令碼供 Visual Studio 來部署您的範本。 對此指令碼所進行的任何變更也會影響 Visual Studio 中的部署，因此請務必小心。|
