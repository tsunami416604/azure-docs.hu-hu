...
# 在 Azure 傳統入口網站使用 Azure 排程器以開始使用

## 開始使用

在 Azure 排程器上可以輕鬆地建立工作和工作集合。 本教學課程將逐步引導您建立要用來儲存工作的工作集合、建立工作集合中的工作，以及大致介紹可用的工作監視和管理工作。 您需有先前使用 Azure 的經驗，才能使用本教學課程。

第一次開啟 Azure 傳統入口網站時，自動放置在 **的所有項目** ] 索引標籤。 中的資料行 **所有項目** 可以排序] 索引標籤。 若要檢視您的排程器工作和工作集合，請按一下 [ **排程器** ] 索引標籤。
...
2.  按一下 [ **應用程式服務** > **新建** > **排程器**, ，然後按一下 [ **自訂建立**。 <br /><br /> ![][2]

3.  在 **工作集合**, ，選取 [在現有的工作集合名稱 **工作集合** 下拉式清單。 如果您沒有現有的工作集合加入工作，選取您想要 **新建** 並輸入名稱來識別新的工作集合。<br /><br /> ![][3]

4.  在 **區域**, ，選取工作集合的地理區域。  

5.  按一下方向鍵，建立工作集合，並移至下一個階段 – 建立作業。

6.  讓我們建立只要點擊 http://www.microsoft.com/ 使用 GET 要求的作業。 在 **工作動作** 畫面上，定義所要求的表單欄位中的下列值 ︰  
...
7.  建立工作之後，請定義排程。 工作可以定義為一次工作，但是讓我們挑選週期排程。 僅基於說明用途，本教學課程中的一些螢幕擷取畫面會顯示 1 分鐘的週期，但挑選 12 小時的週期。  
...
8.  按一下 [ **確定**。  
    建立工作和工作集合可能需要一段時間。 若要檢查狀態，可以監控位於入口網站底部的通知。

    ![][6]

    在建立工作和工作集合之後，會有一則訊息告訴您已成功建立工作或工作集合。 工作會列在 [排程器] 區段的 [工作] 區段中，而工作集合會列在 [工作集合] 區段中。 若要設定工作中的其他進階設定，請參閱底下＜設定工作＞一節。
...
當您按一下工作集合名稱， **儀表板** ] 索引標籤會顯示。 [儀表板] 會顯示下列資訊：
...
在 **延展** 索引標籤上，您可以變更的設定和使用您的排程器服務層。
...
您可以按一下 **檢視詳細歷程記錄** 查看每次執行完整的回應。 此對話方塊也可讓您將回應複製到剪貼簿。
...
在工作上按一下 **工作** 畫面可讓您設定該工作。 除了快速建立精靈中可用的設定外，這可讓您設定其他進階設定。 若要設定作業時，按一下 [中作業的名稱旁的向右箭號 **工作** 畫面。
...
最後，對於所有的動作類型，您可能變更排程本身和其週期行為。 您可以變更開始日期和時間、週期排程，以及結束日期和時間 (如果工作重複發生。)進行任何變更之後，您可能會儲存變更，即可 **儲存** 或捨棄變更，藉以 **捨棄**。
