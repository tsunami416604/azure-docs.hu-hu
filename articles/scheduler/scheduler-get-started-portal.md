...
第一次開啟 Azure 傳統入口網站時會自動進入 [所有項目]**** 索引標籤。 [所有項目]**** 索引標籤中的資料行可排序。 若要檢視您的排程器工作和工作集合，請按一下 [排程器]**** 索引標籤。
...
3.  在 [工作集合]**** 中，選取 [工作集合]**** 下拉式清單底下的現有工作集合名稱。 如果您沒有現有的工作集合加入工作，選取您想要 **新建** ，然後輸入名稱來識別新的工作集合。<br /><br /> ![][3]

4.  在 [區域]**** 中，選取工作集合的地理區域。

5.  按一下方向鍵，建立工作集合，並移至下一個階段 – 建立作業。

6.  讓我們建立只要點擊 http://www.microsoft.com/ 使用 GET 要求的作業。 在 [工作動作]**** 畫面中，為所要求的表單欄位定義下列值：
...
按一下 [作業]**** 畫面中的作業，可讓您設定該作業。 除了快速建立精靈中可用的設定外，這可讓您設定其他進階設定。 若要設定作業，請按一下 [作業]**** 畫面中作業名稱旁邊的向右箭號。
...
## 另請參閱
 ...
 [排程器限制、 lefaults 和錯誤碼](scheduler-limits-defaults-errors.md)
