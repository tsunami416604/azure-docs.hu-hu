...
-   [Step 1: Create a Microsoft Azure account][Step 1: Create a Microsoft Azure account]
-   [Step 2: Connect to Azure and create a database][Step 1: Create a Microsoft Azure account]
-   [Step 3: Configure the firewall][Step 3: Configure the firewall]
-   [Step 4: Add data and a schema using Transact-SQL script][Step 4: Add data and a schema using Transact-SQL script]
-   [Step 5: Create the schema][Step 5: Create the schema]
-   [Step 6: Insert data][Step 6: Insert data]
-   [Step 7: Query sample and system data in the Management Portal for SQL Database][Step 7: Query sample and system data in the Management Portal for SQL Database]
-   [Step 8: Create a database login and assign permissions][Step 8: Create a database login and assign permissions]
-   [Step 9: Connect from other applications][Step 9: Connect from other applications]

## Step 1: Create a Microsoft Azure account

1.  Open a web browser, and browse to [][]<http://azure.microsoft.com></a>.
...
1.  Sign in to the [Management Portal][Management Portal]. You should see a navigation pane that looks like this.
...
1.  The Database Settings page appears when you click **Custom Create**. In this page, provide basic information that creates an empty SQL database on the server. Adding tables and data will come in a later step.
...
1.  Fill out the Server Settings as follows:
...
**Note:** The SQL Database service is only available with TCP port 1433 used by the TDS protocol, so make sure that the firewall on your network and local computer allows outgoing TCP communication on port 1433. For more information, see [SQL Database Firewall][SQL Database Firewall].
...
6.  To save your changes, click **SAVE** at the bottom of the page.

7.  After you save the rule, your page will look similar to the following screenshot.
...
**Note:** The schema and data values are taken from this [MSDN article][MSDN article] and have been modified to work with SQL Database.

1.  Go to the home page. In the [Management Portal][Management Portal], the **School** database appears in the list of items on the home page.
...
In this step, you will create the schema using the following script. The script first checks for an existing table of the same name to ensure there won't be a name collision, and creates the table using the [CREATE TABLE][CREATE TABLE] statement. Further on, this script uses the [ALTER TABLE][ALTER TABLE] statement to specify the primary key and table relationships.
...
Open a new query window and then paste in the following script. Run the script to insert data. This script uses the [INSERT][INSERT] statement to add values to each column.
...
1.  In the [Management Portal][Management Portal], select **SQL Databases**, click **Servers**, choose the server and then click the white arrow to open the
    server page.
...
Next, you'll assign permissions using the db\_datareader role. Database users assigned to this role can read all data from all user tables in the database.

1.  Open a new query window and then enter and run the next statement. This statement runs a built-in stored procedure that assigns the db\_datareader role to the new user you just created.
...
1.  On the next page, choose the **School** database, and then choose **Person**. Click **Finish**. If you are prompted for login details, type them in and then click **OK**.

2.  The Import Data dialog box appears that prompts you to select how and where to import your data. With the default options selected, click **OK**.

    ![Navigation pane][9]

3.  In the worksheet, you should see a table with a result set that includes 34 rows from the person table, including PersonID, LastName, FirstName, HireDate, and EnrollmentDate, just like the query results from Step 7.

Using just Excel, you can import only one table at a time. A better approach is to use the PowerPivot for Excel add-in, which lets you import and work with multiple tables as a single data set. Working with PowerPivot is beyond the scope of this tutorial, but you can get more information in this topic about [PowerPivot for Excel][PowerPivot for Excel].
...
To actively manage your new database, consider installing and using SQL Server Management Studio. Management Studio is the primary database administration tool for managing SQL Server databases, including those running on Azure. Using Management Studio, you can save queries for future use, add new tables and stored procedures, and hone your Transact-SQL skills in a rich scripting environment that includes a syntax checker, intellisense, and templates. To get started, follow the instructions in [Managing SQL Databases Using SQL Server Management Studio][Managing SQL Databases Using SQL Server Management Studio].

Fluency in the Transact-SQL query and data definition language is essential for database administrators. If you are new to Transact-SQL, start with the [Tutorial: Writing Transact-SQL Statements][Tutorial: Writing Transact-SQL Statements] to learn some basic skills.
...
-   [Migrating Databases to SQL Database][Migrating Databases to SQL Database]
-   [Copying Databases in SQL Database][Copying Databases in SQL Database]
-   [Deploy a SQL Server Database to an Azure Virtual Machine][Deploy a SQL Server Database to an Azure Virtual Machine]
  ...
  [Navigation pane]: ./media/sql-database-get-started/1NavPaneDBSelected_SQLTut.png
  ...
  [MSDN article]: http://msdn.microsoft.com/en-us/library/windowsazure/ee621790.aspx "MSDN article"
  ...
  [Managing SQL Databases Using SQL Server Management Studio]: http://www.azure.microsoft.com/en-us/documentation/articles/sql-database-manage-azure-ssms/
  [Tutorial: Writing Transact-SQL Statements]: http://msdn.microsoft.com/en-us/library/ms365303.aspx
