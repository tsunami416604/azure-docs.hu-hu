...
> [!TIP]
> HEAT map rétegek alapértelmezés szerint egy adatforrásban lévő összes geometriája koordinátáit jelenik meg. A réteg korlátozása úgy, hogy ez csak a Render geometriai szolgáltatásait, mutasson a `filter` tulajdonság a réteg `['==', '$type', 'Point']`
...
* `radius`: Határozza meg, amelyben az egyes megjelenítése egy képpontos radius. A radius beállítható egy rögzített szám vagy kifejezés. Egy kifejezés használatával lehetőség vertikális a radius nagyítási szint, amelyek megjelenő alapján egy egységes térbeli területre a térképen (például 5 mérföldes radius).
...
* `intensity`: Az intenzitástérkép általános megnövelje az egyes adatpontok súlyát egy szorzóval vonatkozik. Ez segít, hogy a kisebb különbségek adatpontok súlyozást úgy válnak egyre könnyebben jelenítheti meg.
