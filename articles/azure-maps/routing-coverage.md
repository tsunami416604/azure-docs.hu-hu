---
title: Azure Maps-útválasztási lefedettség |} A Microsoft Docs
description: További tudnivalók az Azure Maps-útválasztási lefedettség
author: jingjing-z
ms.author: jinzh
ms.date: 04/04/2019
ms.topic: conceptual
ms.service: azure-maps
services: azure-maps
manager: ''
ms.openlocfilehash: ef62ec78bc7fc01bd2e6dd7c5561b15be85540d2
ms.sourcegitcommit: 3102f886aa962842303c8753fe8fa5324a52834a
ms.translationtype: HT
ms.contentlocale: hu-HU
ms.lasthandoff: 04/23/2019
ms.locfileid: "60768801"
---
# <a name="azure-maps-routing-coverage"></a>Az Azure Maps útválasztási lefedettség

Ez a cikk az Azure Maps útválasztási lefedettség információkat szolgáltat. Keresés a B egy helyről vezető optimális útvonalat, az Azure Maps rendkívül pontos utazási időt biztosít, élő frissített utazási információkat és útválasztási utasításokat. Az útvonal figyelembe veszi a fiók további keresési kifejezéseket, például a jelenlegi forgalom, a vehicle típusa és a feltételek elkerülése érdekében. Az útvonal optimalizálására való képességet a régió függ, mivel az Azure Maps különböző szintű információkat és a pontosság különböző régiókra vonatkozóan. Az alábbi táblázat a régiók és milyen típusú információkat kérhet a számukra.
 
Tekintse meg a lefedettség [ **Geokódolás**](geocoding-coverage.md).       
Tekintse meg a lefedettség [ **forgalom**](traffic-coverage.md).  
Tekintse meg a lefedettség [ **Render**](render-coverage.md).
<br>  
**Jelmagyarázat**

| Szimbólum             | Jelentés                                |
|--------------------|----------------------------------------|
| ✓                  | Az ország megadott részletes adatokkal.   |
| Ø                  | Az ország megadott egyszerűsített adatokkal. |
| Ország hiányzik. | Országadatokat nem áll rendelkezésre.          |

<br>  

Az alábbi táblázatban az Azure Maps útválasztási lefedettség adatait.
<br>  


## <a name="africa"></a>Afrika

| Ország/régió                 | Útválasztás         | Forgalom-Útválasztás | Teherautó Útválasztás |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Algéria                        |        ✓       |                       |             |
| Angola                         |        ✓       |                       |             |
| Benin                          |        ✓       |                       |             |
| Botswana                       |        ✓       |                       |             |
| Burkina Faso                   |        ✓       |                       |             |
| Burundi                        |        ✓       |                       |             |
| Kamerun                       |        ✓       |                       |             |
| Cabo Verde Köztársaság                     |        ✓       |                       |             |
| Kongói - Brazzaville Kinshasa    |        ✓       |                       |             |
| Kongói - Kinshasa                |        ✓       |                       |             |
| côte d’ivoire                  |        ✓       |                       |             |
| Egyiptom                          |        ✓       |         ✓            |             |
| Gabon                          |        ✓       |                       |             |
| Gambia                         |        ✓       |                       |             |
| Ghána                          |        ✓       |                       |             |
| Kenya                          |        ✓       |         ✓            |             |
| Lesotho                        |        Ø        |         ✓            |             |
| Malawi                         |        ✓       |                       |              |
| Mali                           |        ✓       |                       |             |
| Mauritánia                     |        ✓       |                       |             |
| Réunion, Mauritius és Mayotte |        ✓      |                       |             |
| Marokkó                        |        ✓       |                       |             |
| Mozambik                     |        ✓       |         ✓            |             |
| Namíbia                        |        ✓       |                       |             |
| Niger                          |        ✓       |                       |             |
| Nigéria                        |        ✓       |         ✓            |             |
| Ruanda                         |        ✓       |                       |             |
| Szenegál                        |        ✓       |                       |             |
| Seychelle-szigetek                     |        ✓       |                       |             |
| Dél-Afrika                   |        ✓       ||         ✓            |             |
| Szváziföld                      |        ✓       |                       |             |
| Tanzánia                       |        ✓       |                       |             |
| Togo                           |        ✓       |                       |             |
| Tunézia                        |        ✓       |                       |             |
| Uganda                         |        ✓       |                       |             |
| Nyugat-Szahara                 |        ✓       |                       |             |
| Zambia                         |        ✓       |                       |             |
| Zimbabwe                       |        ✓       |                       |             |


## <a name="americas"></a>Amerika

| Ország/régió                 | Útválasztás         | Forgalom-Útválasztás | Teherautó Útválasztás |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Argentína                      |        ✓       |         ✓            |     ✓       |
| Antigua és Barbuda            |        ✓       |                      |             |
| Bahama-szigetek                        |        ✓       |                      |             |
| Barbados                       |        ✓       |                      |             |
| Belize                         |        ✓       |                      |             |
| Brazília                         |        ✓       |         ✓            |     ✓       | 
| Kanada                         |        ✓       |         ✓            |     ✓      |
| Chile                          |        ✓       |         ✓            |     ✓      |
| Kolumbia                       |        ✓       |         ✓            |             |
| Costa Rica                     |        ✓       |                      |             |
| Kuba                           |        ✓       |                      |             |
| Dominika                       |        ✓       |                      |             |
| Dominikai Köztársaság             |        ✓       |                      |             |
| Ecuador                        |        ✓       |                      |             |
| Salvador                    |        ✓       |                      |             |
| Francia Guyana                  |        ✓       |                      |             |
| Grenada                        |        ✓       |                      |             |
| Guatemala                      |        ✓       |                      |             |
| Guyana                         |        ✓       |                      |             |
| Haiti                          |        ✓       |                      |             |
| Honduras                       |        ✓       |                      |             |
| Jamaica                        |        ✓       |                      |             |
| Mexikó                         |        ✓       |         ✓            |     ✓      | 
| Nicaragua                      |        ✓       |                      |             |
| Panama                         |        ✓       |                      |             |
| Paraguay                       |        ✓       |                      |             |
| Peru                           |        ✓       |         ✓            |             |
| Saint Kitts és Nevis            |        ✓       |                      |             |
| Saint Lucia                      |        ✓       |                      |             |
| Saint Vincent és Grenadine-szigetek       |        ✓       |                      |             |
| Suriname                       |        ✓       |                      |             |
| Trinidad és Tobago              |        ✓       |                      |             |
| Egyesült Államok                  |        ✓       |         ✓            |     ✓      | 
| + Amerikai Szamoa                |        ✓       |                      |             |
| + Észak-Mariana-szigetek      |        ✓       |                      |             |
| +Puerto Rico                   |        ✓       |                      |             |
| + EGYESÜLT ÁLLAMOK Virgin-szigetek           |        ✓       |                      |             |
| Uruguay                        |        ✓       |         ✓            |     ✓      | 
| Venezuela                      |        ✓       |                      |             |


## <a name="asia-pacific"></a>Ázsia és a Csendes-óceáni térség

| Ország/régió                 | Útválasztás         | Forgalom-Útválasztás | Teherautó Útválasztás |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Ausztrália                      |        ✓       |         ✓            |     ✓       |
| + Karácsony-sziget              |        ✓       |                      |             |
| Brunei                         |        ✓       |         ✓            |             |
| Kambodzsa                       |        ✓       |                      |             |
| Fidzsi                           |        ✓       |                      |             |
| Guam                           |        ✓       |                      |             |
| Hongkong (KKT)                  |        ✓       |         ✓            |             |
| India                          |        ✓       |         ✓            |             |
| Indonézia                      |        ✓       |         ✓            |             |
| Korea                          |        Ø       |                      |             |
| Laosz                           |        ✓       |                      |             |
| Makaó (KKT)                      |        ✓       |         ✓            |             |
| Malajzia                       |        ✓       |         ✓            |             |
| Mianmar                        |        ✓       |                      |             |
| Új-Zéland                    |        ✓       |         ✓            |     ✓       | 
| Fülöp-szigetek                    |        ✓       |         ✓            |             |
| Szingapúr                      |        ✓       |         ✓            |             |
| Tajvan                         |        ✓       |         ✓            |             |
| Thaiföld                       |        ✓       |         ✓            |             |
| Vietnam                        |        ✓       |         ✓            |             |


## <a name="europe"></a>Európa

| Ország/régió                 | Útválasztás         | Forgalom-Útválasztás | Teherautó Útválasztás |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Albánia                        |        ✓       |                      |             |
| Andorra                        |        ✓       |         ✓            |             |
| Ausztria                        |        ✓       |         ✓            |     ✓       | 
| Belarusz                        |        ✓       |                      |             |
| Belgium                        |        ✓       |         ✓            |     ✓      | 
| Bolívia                        |        ✓       |                      |             |
| Bosnia-Herzegovina             |        ✓       |                      |             |
| Bulgária                       |        ✓       |         ✓            |     ✓      | 
| Horvátország                        |        ✓       |         ✓            |     ✓      |
| Ciprus                         |        ✓       |                      |             |
| Cseh Köztársaság                 |        ✓       |         ✓            |     ✓      |
| Dánia                        |        ✓       |         ✓            |     ✓      |
| + Feröer-szigetek                 |        ✓       |                      |             |
| Észtország                        |        ✓       |         ✓            |     ✓      |
| Finnország                        |        ✓       |         ✓            |     ✓      |
| Franciaország                         |        ✓       |         ✓            |     ✓      | 
| +Guadeloupe                    |        ✓       |                      |             |
| + Martinique                    |        ✓       |                      |             |
| + Saint Barthélemy                |        ✓       |                      |             |
| +St. Martin                    |        ✓       |                      |             |
| + Saint Pierre és Miquelon         |        ✓       |                      |             |
| Grúzia                        |        ✓       |                      |             |
| Németország                        |        ✓       |         ✓            |     ✓      | 
| Görögország                         |        ✓       |         ✓            |     ✓      |  
| Guernsey                       |        ✓       |                      |             |
| Magyarország                        |        ✓       |         ✓            |     ✓      | 
| Izland                        |        ✓       |         ✓            |             |
| Írország                        |        ✓       |         ✓            |     ✓      | 
| Olaszország                          |        ✓       |         ✓            |     ✓      | 
| Jersey                         |        ✓       |                      |             |
| Lettország                         |        ✓       |         ✓            |     ✓      | 
| Liechtenstein                  |        ✓       |         ✓            |             |
| Litvánia                      |        ✓       |         ✓            |     ✓      | 
| Luxemburg                     |        ✓       |         ✓            |     ✓      | 
| Észak-Macedónia                |        ✓       |                      |             |
| Málta                          |        ✓       |         ✓            |             |
| Moldova                        |        ✓       |                      |             |
| Monaco                         |        ✓       |         ✓            |             |
| Montenegró                     |        ✓       |                      |     ✓      | 
| Hollandia                    |        ✓       |         ✓            |     ✓      | 
| +Aruba                         |        ✓       |                      |             | 
| + Hollandia Karib-szigetek         |        ✓       |                      |             |
| +Curaçao                       |        ✓       |                      |             |
| + Saint Martin                  |        ✓       |                      |             |
| Norvégia                         |        ✓       |         ✓            |     ✓      | 
| Lengyelország                         |        ✓       |         ✓            |     ✓      | 
| Portugália                       |        ✓       |         ✓            |     ✓      |  
| Románia                        |        ✓       |         ✓            |     ✓      | 
| Orosz Föderáció             |        ✓       |         ✓            |     ✓      | 
| San Marino                     |        ✓       |         ✓            |             |
| + Saint Martin                  |        ✓       |                      |             |
| Szerbia                         |        ✓       |                      |     ✓      | 
| Szlovákia                       |        ✓       |         ✓            |     ✓      | 
| Szlovénia                       |        ✓       |         ✓            |     ✓      |
| Spanyolország                          |        ✓       |         ✓            |     ✓      | 
| Svédország                         |        ✓       |         ✓            |     ✓      | 
| Svájc                    |        ✓       |         ✓            |     ✓      | 
| Törökország                         |        ✓       |         ✓            |     ✓      | 
| Ukrajna                        |        ✓       |         ✓            |             | 
| Egyesült Királyság                 |        ✓       |         ✓            |     ✓      | 
| +Anguilla                      |        ✓       |                      |             |
| +Bermuda                       |        ✓       |                      |             |
| + Brit Virgin-szigetek        |        ✓       |                      |             |
| + Kajmán-szigetek                |        ✓       |                      |             |
| + Gibraltár                      |        ✓       |         ✓            |             | 
| +Montserrat                    |        ✓       |                      |             |
| + & Turks- és Caicos-szigetek        |        ✓       |                      |             |
| Vatikánváros                   |        ✓       |         ✓            |             |
| Man-sziget                    |        ✓       |                      |             |


## <a name="middle-east"></a>Közel-Kelet

| Ország/régió                 | Útválasztás         | Forgalom-Útválasztás | Teherautó Útválasztás |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Bahrein                        |        ✓       |         ✓            |             |
| Irak                           |        ✓       |                      |             |
| Izrael                         |        ✓       |         ✓            |             |
| Jordánia                         |        ✓       |                      |             |
| Kuvait                         |        ✓       |         ✓            |             |
| Libanon                        |        ✓       |                      |             |
| Omán                           |        ✓       |         ✓            |             |
| Katar                          |        ✓       |         ✓            |             |
| Szaúd-Arábia                   |        ✓       |         ✓            |             |
| Egyesült Arab Emírségek           |        ✓       |         ✓            |             |
| Jemen                          |        ✓       |                      |             |


## <a name="next-steps"></a>További lépések

Az Azure Maps útválasztás kapcsolatos további információkért lásd: a [útválasztás](https://docs.microsoft.com/rest/api/maps/route) oldalak hivatkozhat.
