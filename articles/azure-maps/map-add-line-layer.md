...
A `LineString` és az `MultiLineString` szolgáltatások elérési útjaként és útvonalként is használhatók a térképen. Az `Polygon` és az `MultiPolygon` szolgáltatások körvonalainak megjelenítéséhez egy sor réteget is használhat. Az adatforrások egy vonal réteghez csatlakoznak, így biztosítva a rendereléshez szükséges adatforrásokat. 
