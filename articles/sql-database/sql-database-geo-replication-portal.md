...
> [AZURE.NOTE] 如果夥伴資料庫已經存在 （例如因為終止前一個地理區域複寫關聯性） 的命令會失敗。
...
2. 在 [SQL Database] 分頁中，選取 **所有設定** > **地理區域複寫**。
...
4. 設定 **次要類型** (**可讀取**, ，或 **不可讀取**)，只有高階資料庫可讀取次要複本，而標準的資料庫次要位置只能設為 **不可讀取**。
5. 選取或設定次要資料庫的伺服器。
...
2. 在 [SQL Database] 分頁中，選取 **所有設定** > **地理區域複寫**。
3. 在 **次要** 清單中，選取您想要移除的地理區域複寫合作關係的資料庫。
4. 按一下 [ **停止複寫**。

    ![移除次要][7]


5. 按一下 **停止複寫** 隨即開啟，因此請確認視窗按 **是** 若要移除的地理區域複寫合作關係中的資料庫 （將它設定為讀寫資料庫不屬於任何複寫的一部分）。
