...
使用 [彈性資料庫工具](sql-database-elastic-scale-introduction.md), ，您可以建立分區化資料庫解決方案。 **多分區查詢**用於資料收集/報告等工作，這些工作需要跨越數個分區執行查詢。 (相反 [資料相依路由](sql-database-elastic-scale-data-dependent-routing.md), ，在單一分區上執行所有工作。)
...
多分區查詢的主要進入點是 **MultiShardConnection** 類別。 如同資料相依路由一樣，API 的熟悉 * *[System.Data.SqlClient](http://msdn.microsoft.com/library/System.Data.SqlClient(v=vs.110).aspx)* * 類別和方法。 使用 **SqlClient** 程式庫時，第一個步驟是建立 **SqlConnection**，再建立連接的 **SqlCommand**，然後透過其中一個 **Execute** 方法執行命令。 最後，**SqlDataReader** 逐一查看從命令執行傳回的結果集。 多分區查詢 API 的用法依照下列步驟：
...
主要差異是多分區連接的建構方式。 其中 **SqlConnection** 單一資料庫上運作 **MultiShardConnection** 採用 *** 集合分區 *** 做為輸入。 您可以從分區分區來填入對應的集合。 然後，使用 **UNION ALL** 語意在分區集合上執行查詢，以組成單一的整體結果。 (選擇性) 在命令上使用**ExecutionOptions** 屬性，可將資料列的來源分區名稱加入至輸出。
