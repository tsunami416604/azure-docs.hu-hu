...
- A nagyméretű olvasások, például az olvasás előtt gyakran a ["Scatter-gather" olvasások](/sql/relational-databases/reading-pages/)használatával történik. Így egyszerre akár 4 MB-nyi lapot is beolvashat, így egyetlen beolvasható a SQL Server motorban. Ha azonban a beolvasott adatokat RBPEX-ban végzik, akkor ezek az olvasások több, 8 KB-os olvasásként vannak elküldve, mivel a puffer-készlet és a RBPEX mindig 8 KB-os oldalakat használ. Ennek eredményeképpen a RBPEX által látott olvasási IOs-szám nagyobb lehet, mint a motor által elvégzett IOs tényleges száma.
...
A nem nagy kapacitású adatbázisokban az adatfájlok összevont olvasási és írási IOPS az [erőforrás-irányítási](/azure/sql-database/sql-database-resource-limits-database-server#resource-governance) adatIOPSi korláthoz képest a [sys. dm_db_resource_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database) és a [sys. resource_stats](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database) nézetekben, a `avg_data_io_percent` oszlopban kell jelenteni. Ugyanezt az értéket a portálon _adatio-százalékként_kell jelenteni. 

A nagy kapacitású-adatbázisokban ez az oszlop csak a számítási replikán lévő helyi tárterületre vonatkozó korláttal kapcsolatos adatIOPS-kihasználtságot jelenti, kifejezetten IO a RBPEX és a `tempdb`. Ebben az oszlopban az 100% érték azt jelzi, hogy az erőforrás-szabályozás korlátozza a helyi tárterület IOPS. Ha ez egy teljesítménnyel kapcsolatos problémával összefügg, állítsa be úgy a számítási feladatot, hogy kevesebb IO-t állítson elő, vagy növelje az adatbázis-szolgáltatás célkitűzését, hogy növelje az erőforrás-szabályozás _maximális IOPS_ [korlátját](sql-database-vcore-resource-limits-single-databases.md). A RBPEX-olvasások és-írások erőforrás-irányításához a rendszer a SQL Server motor által kiállított nagyobb IOs helyett a 8 KB-os IOs-t számítja fel. 
