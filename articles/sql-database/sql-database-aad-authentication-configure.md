...
> [!NOTE]
> This article applies to Azure SQL server, and to both SQL Database and SQL Data Warehouse databases that are created on the Azure SQL server. Az egyszerűség kedvéért a jelen témakörben az SQL Database és az SQL Data Warehouse megnevezése egyaránt SQL Database.
...
To run PowerShell cmdlets, you need to have Azure PowerShell installed and running. Részletes információk: [Az Azure PowerShell telepítése és konfigurálása](/powershell/azure/overview).
...
To run PowerShell cmdlets, you need to have Azure PowerShell installed and running. Részletes információk: [Az Azure PowerShell telepítése és konfigurálása](/powershell/azure/overview). To provision an Azure AD admin, execute the following Azure PowerShell commands:
...
Use PowerShell command get-help to see more information for each of these commands. Például: `get-help Set-AzSqlServerActiveDirectoryAdministrator`.
