...
GSM 和 LSM 會追蹤分區化環境中每個資料庫的對應。 有時候，GSM 和 LSM 之間發生中斷。 在此情況下，使用 RecoveryManager 類別來偵測並修復中斷。
...
1. 由於刪除其範圍被認為不再使用中的分區，或由於重新命名分區所造成的不一致。 刪除分區會導致**被遺棄的分區對應**。 重新命名的資料庫同樣可能造成被遺棄的分區對應。 在此情況下，只需要更新分區位置。
...
* [Azure SQL database 彈性資料庫功能](sql-database-elastic-scale-introduction.md)
...
由於此應用程式程式碼會操作分區對應本身，在 Factory 方法中使用的認證 (上述範例中的 smmConnectionString)，應該是在連接字串所參考的 GSM 資料庫上具有唯讀權限的認證。 這些認證通常不同於用來對資料獨立路由開啟連接的認證。 如需詳細資訊，請參閱 [在彈性資料庫用戶端使用認證](sql-database-elastic-scale-manage-credentials.md)。
