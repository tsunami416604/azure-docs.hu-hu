...
## <a name="single-database--elastic-pool"></a>[Önálló adatbázis & rugalmas készlet](#tab/single-database)
...
| [Naplózás az Azure Event hub-ba](https://github.com/Azure/azure-quickstart-templates/tree/master/201-sql-auditing-server-policy-to-eventhub) | Ez a sablon lehetővé teszi, hogy egy olyan Azure SQL Servert helyezzen üzembe, amelyen engedélyezve van a naplózás, hogy naplókat írjon a kilépő Event hubhoz. A naplózási események az Event hubhoz való küldéséhez állítsa be a naplózási beállításokat `Enabled` `State` a `true`és a beállításként `IsAzureMonitorTargetEnabled` . Továbbá konfigurálja a diagnosztikai beállításokat a `SQLSecurityAuditEvents` naplózási kategóriával `master` az adatbázisban (a kiszolgálás szintjének naplózásához). A Azure SQL Database és SQL Data Warehouse naplózása nyomon követi az adatbázis-eseményeket, és az Azure Storage-fiókba, a OMS-munkaterületbe vagy a Event Hubsba helyezhető naplófájlba írja azokat.|
