---
title: Az Azure SQL logikai kiszolgálók és az önálló adatbázisok |} A Microsoft Docs
description: További tudnivalók az Azure SQL Database logikai kiszolgáló, és egyetlen adatbázis-tervezésben és erőforrásaikat.
...
ms.locfileid: "39091796"
---
# <a name="azure-sql-database-logical-servers-and-single-databases-and-their-resources"></a>Az Azure SQL Database logikai kiszolgálóiról és önálló adatbázisok és az erőforrások
...
> [!TIP]
> Az érvényes erőforrás-csoport és a kiszolgáló nevekkel kapcsolatban lásd: [elnevezési szabályokat és korlátozásokat](https://docs.microsoft.com/azure/architecture/best-practices/naming-conventions).
>
...
## <a name="logical-servers-and-databases"></a>Logikai kiszolgálóiról és adatbázisairól

Egy logikai kiszolgálón is létrehozhat:

- Belül létrehozott egy adatbázist egy [Azure-erőforráscsoport](../azure-resource-manager/resource-group-overview.md) együtt egy [számítási és tárolási erőforrások készletét kombinált](sql-database-service-tiers-dtu.md) vagy egy [számítási és tárolási erőforrásokfüggetlenméretezése](sql-database-service-tiers-vcore.md). Az Azure SQL database társítva egy Azure SQL Database logikai kiszolgáló, amely egy adott Azure-régióban jön létre.
- Részeként létrehozott egy adatbázist egy [adatbáziskészletnek](sql-database-elastic-pool.md) belül egy [Azure-erőforráscsoport](../azure-resource-manager/resource-group-overview.md) az egy [kombinált számítási és tárolási erőforrásokat (DTU-alapú)](sql-database-service-tiers-dtu.md) vagy egy [(Virtuálismag-alapú) számítási és tárolási erőforrások független méretezése](sql-database-service-tiers-vcore.md) , amely a készletben található adatbázisok között vannak megosztva. Az Azure SQL database társítva egy Azure SQL Database logikai kiszolgáló, amely egy adott Azure-régióban jön létre.

> [!IMPORTANT]
> SQL Database felügyelt példányába, jelenleg nyilvános előzetes verzióban van egy [egy SQL server-példány](sql-database-managed-instance.md) (felügyelt példány) belül létrehozott egy [Azure-erőforráscsoport](../azure-resource-manager/resource-group-overview.md) számítási és tárolási egy meghatározott készletével együtt erőforrások az összes adatbázis server-példányon. Felügyelt példány rendszer- és felhasználói adatbázisokat tartalmazza. Felügyelt példány lehetővé teszi, hogy adatbázis lift-and-shift teljes körűen felügyelt paas-megoldás, hogy az alkalmazás újratervezése nélkül. Felügyelt példány nagy mértékben kompatibilis, a helyszíni SQL Server programozási modell biztosítja, és támogatja a legtöbb SQL Server-funkciókat és a hozzájuk tartozó eszközöket és szolgáltatásokat. További információért tekintse meg a [felügyelt SQL Database-példányt](sql-database-managed-instance.md) ismertető cikket. Ez a cikk további része nem vonatkozik a felügyelt példányon.

## <a name="tds-and-tcpip-connections"></a>TDS- és TCP/IP-kapcsolatok

A Microsoft Azure SQL Database támogatja a tabulált adatfolyam (TDS) protokoll ügyfél 7.3-as vagy újabb verziójára, és lehetővé teszi, hogy csak titkosított TCP/IP-kapcsolatokat.

## <a name="azure-sql-databases-protected-by-sql-database-firewall"></a>Az SQL Database-tűzfal védi az Azure SQL Database

Az adatok védelme érdekében a [SQL Database-tűzfal](sql-database-firewall-configure.md) megakadályozza, hogy minden hozzáférés az adatbázis-kiszolgáló vagy valamelyik hozzá tartozó adatbázisok kívül a kapcsolatot az Azure-előfizetés-kapcsolaton keresztül közvetlenül a kiszolgálóhoz. További kapcsolat engedélyezéséhez kell [hozzon létre egy vagy több tűzfalszabályok](sql-database-firewall-configure.md#creating-and-managing-firewall-rules). Rugalmas készletek kezelése és létrehozása: [rugalmas készletek](sql-database-elastic-pool.md).
