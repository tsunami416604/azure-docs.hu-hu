...
Az ADS engedélyezése **adatok biztonságú** alatt a **biztonsági** fejléc az Azure SQL Database panelen. A kiszolgáló összes adatbázisára vonatkozó HIRDETÉSEK engedélyezéséhez kattintson **engedélyezése speciális adatokat a kiszolgáló biztonsági**.

![HIRDETÉSEK engedélyezése](./media/sql-advanced-protection/enable_atp.png) 

> [!NOTE]
> HIRDETÉSEK költségét igazodik az Azure Security Center standard szintű díjszabás csomópontonként, ahol egy csomópont-e a teljes SQL logikai kiszolgálón. Így licencdíjat csak egyszer védelme érdekében az ADS a kiszolgáló összes adatbázisára. Kipróbálhatja ADS kezdetben az ingyenes próbaverziót.
...
## <a name="4-manage-ads-settings-on-your-sql-server"></a>4. Az SQL server ADS beállításainak kezelése

Beállításainak megtekintése és kezelése adatok biztonságú, navigáljon a **adatok biztonságú** alatt a **biztonsági** az SQL server ablaktábla a címsorában. Ezen a lapon engedélyezze vagy tiltsa le a HIRDETÉSEK, és Fenyegetésészlelési beállításainak módosításához a teljes SQL Server.
...
Speciális adatok biztonsági beállítások a kiszolgáló is a HIRDETÉSEK adatbázis panelen érhető el. Kattintson a **beállítások** az ADS fő ablaktáblán, majd kattintson a **kiszolgálóbeállítások megtekintése biztonságú adatok**. 
