...
1. Jelentkezzen be a felügyelt példányba egy standard SQL Server fiókkal (nem Azure AD), amely egy `sysadmin` vagy egy Azure AD-rendszergazda a MI számára, a [SQL Server Management Studio](sql-database-managed-instance-configure-p2s.md#use-ssms-to-connect-to-the-managed-instance)használatával.
...
3. A lekérdezési ablakban a következő szintaxis használatával hozzon létre egy helyi Azure AD-fiókhoz tartozó bejelentkezési azonosítót:
    ...
    Ez a példa a nativeuser@aadsqlmi.onmicrosoft.comfiókhoz tartozó bejelentkezési azonosítót hoz létre.
...
- Ahhoz, hogy az újonnan létrehozott Azure AD-kiszolgáló rendszerbiztonsági tag (login) további bejelentkezési adatokat hozzon létre más Azure AD-felhasználók,-csoportok vagy-alkalmazások számára, adja meg a bejelentkezési `sysadmin` vagy `securityadmin` kiszolgálói szerepkört. 
...
1. Jelentkezzen be újra a felügyelt példányba, vagy használja az Azure AD-rendszergazda vagy az SQL-rendszerbiztonsági tag meglévő, `sysadmin`.

1. A **Object Explorerban**kattintson a jobb gombbal a kiszolgálóra, és válassza az **Új lekérdezés**elemet.

1. A következő T-SQL szintaxissal adja meg az Azure AD-kiszolgáló rendszerbiztonsági azonosítóját (login) `sysadmin` kiszolgálói szerepkört:
    ...
    Az alábbi példa a `sysadmin` kiszolgálói szerepkört a bejelentkezéshez biztosítja nativeuser@aadsqlmi.onmicrosoft.com
...
Ha az Azure AD-kiszolgáló rendszerbiztonsági tag (login) létrehozása megtörtént, és `sysadmin` jogosultságokkal rendelkezik, a bejelentkezéshez további bejelentkezések is létrehozhatók a **create login**( **külső szolgáltató** ) záradék használatával.
...
1. A lekérdezési ablakban a következő szintaxis használatával hozzon létre egy másik Azure AD-fiókhoz tartozó bejelentkezési azonosítót:
    ...
    Ez a példa egy bejelentkezési azonosítót hoz létre az Azure AD felhasználói bob@aadsqlmi.netszámára, amelynek tartományi aadsqlmi.net az Azure AD-aadsqlmi.onmicrosoft.com összevonták.
...
> [!NOTE]
> Az Azure AD vendég felhasználói csak akkor támogatottak a felügyelt példányok bejelentkezésekor, ha az Azure AD-csoport részeként van hozzáadva. Az Azure AD vendég felhasználója egy olyan fiók, amelyet a felügyelt példányhoz tartozó Azure AD-hoz egy másik Azure AD-ből kell meghívni. Például joe@contoso.com (Azure AD-fiók) vagy steve@outlook.com (MSA-fiók) adható hozzá egy csoporthoz az Azure AD-aadsqlmi. A felhasználók egy csoportba való felvétele után a bejelentkezési szintaxis **létrehozásával** létrehozhat egy bejelentkezést a csoport felügyelt példány főadatbázisában. Azok a felhasználók, akik tagjai ennek a csoportnak, csatlakozhatnak a felügyelt példányhoz az aktuális bejelentkezésük (például joe@contoso.com vagy steve@outlook.com) használatával.
...
1. Jelentkezzen be a felügyelt példányba egy `sysadmin` fiók használatával SQL Server Management Studio használatával.
1. A **Object Explorerban**kattintson a jobb gombbal a kiszolgálóra, és válassza az **Új lekérdezés**elemet.
1. A lekérdezési ablakban a következő szintaxissal hozhat létre Azure AD-felhasználót egy Azure AD-kiszolgáló rendszerbiztonsági tag (login) használatával:
    ...
    A következő példa egy felhasználói bob@aadsqlmi.net hoz létre a bejelentkezési bob@aadsqlmi.net:
...
1. Hozzon létre egy SSMS a létrehozott felhasználóval. Megfigyelheti, hogy nem látja a korábban a `sysadmin` által létrehozott **TestTable** . A felhasználónak meg kell adnia az adatoknak az adatbázisból való beolvasásához szükséges engedélyeket.
...
1. Adja meg az Azure AD-felhasználó számára a `db_datareader` adatbázis-szerepkört a következő T-SQL szintaxis használatával:
    ...
    Az alábbi példa a felhasználó bob@aadsqlmi.net és a csoport _mygroup_ `db_datareader` engedélyekkel rendelkezik a **MyMITestDB** -adatbázison:
...
> [!NOTE]
> Csak az `sysadmin` szerepkör részét képező SQL Server-szintű rendszerbiztonsági tag (login) hajthatja végre a következő műveleteket az Azure AD-résztvevőknek: 
...
- Dedikált rendszergazdai kapcsolatok a `sysadmin` kiszolgáló szerepkörbe tartozó Azure AD Server-rendszerbiztonsági tagok (bejelentkezések) számára.
