...
將 SQL Server Database 移轉到 Azure SQL Database 的工作流程為：
 ...
 2. [如果不相容，修正資料庫相容性問題](#fix-database-compatibility-issues)
...
2. 以環境的下列引數執行下列 sqlpackage.exe 命令：
   ...
   | < 輸出檔案 >| 具有錯誤的輸出檔案的檔案名稱和位置 (若有的話)|
    ...
    ![從 ](./media/sql-database-cloud-migrate/TestForCompatibilityUsingSQLPackage01.png)

3. 開啟輸出檔案並檢視相容性錯誤 (如果有的話)。 如需有關如何修正資料庫相容性問題的指引，請移至 [修正資料庫相容性問題](#fix-database-compatibility-issues)。

    ![從 ](./media/sql-database-cloud-migrate/TestForCompatibilityUsingSQLPackage02.png)
...
3. 以滑鼠右鍵按一下 [物件總管] 中的來源資料庫，指向 [工作]****，並按一下 [匯出資料層應用程式]****。
    ...
    ![從 ](./media/sql-database-cloud-migrate/TestForCompatibilityUsingSSMS01.png)
...
7. 如果沒有任何錯誤出現，代表您的資料庫相容且您已準備好移轉。 如果您有錯誤，您必須加以修正。 若要查看錯誤，請按一下 [錯誤]**** 以**驗證結構描述**。 如何修正這些錯誤，請移至 [修正資料庫相容性問題](#fix-database-compatibility-issues)。
...
> [AZURE.WARNING] 在使用任何一種方法移轉資料庫之前，請確定沒有任何作用中交易以確保移轉期間的交易一致性。 有許多方法來停止資料庫，無法停用用戶端連線到建立 [資料庫快照集](https://msdn.microsoft.com/library/ms175876.aspx)。
...
3. 以滑鼠右鍵按一下 [物件總管] 中的來源資料庫，指向 [工作]****，並按一下 [將資料庫部署至 Microsoft Azure SQL Database...]****。
    ...
    ![從 ](./media/sql-database-cloud-migrate/MigrateUsingDeploymentWizard01.png)

4.  在部署精靈中，按一下 [下一步]****，然後按一下 [連接]**** 以設定與 Azure SQL Database 伺服器的連接。

    ![從 ](./media/sql-database-cloud-migrate/MigrateUsingDeploymentWizard002.png)

5. 在 [連接到伺服器] 對話方塊中，輸入連接到 Azure SQL Database 伺服器的連接資訊。

    ![從 ](./media/sql-database-cloud-migrate/MigrateUsingDeploymentWizard00.png)

5.  在 Azure SQL DB 上提供資料庫的**新資料庫名稱**，設定 **Microsoft Azure SQL Database 的版本** (服務層)、**資料庫大小上限**、**服務目標** (效能等級) 和精靈在移轉程序期間建立之 BACPAC 檔案的**暫存檔名稱**。 請參閱 [Azure SQL Database 服務層](sql-database-service-tiers.md) 如需有關服務層和效能層級。
...
- [匯出至 BACPAC 檔案，使用 SQL Server Management Studio](#export-a-compatible-sql-server-database-to-a-bacpac-file-using-sql-server-management-studio)
- [匯出至 BACPAC，透過 SqlPackage](#export-a-compatible-sql-server-database-to-a-bacpac-file-using-sqlpackage)

您可以在本機或 Azure blob 中儲存這個 BACPAC。 然後您可以使用數種方法中的其中一種，將這個 BACPAC 檔案匯入至 Azure SQL Database。

- [從 BACPAC 檔案匯入 Azure SQL 資料庫中使用 SQL Server Management Studio](#import-from-a-bacpac-file-into-azure-sql-database-using-sql-server-management-studio)
- [從 BACPAC 檔案匯入 Azure SQL Database，透過 SqlPackage](#import-from-a-bacpac-file-into-azure-sql-database-using-sqlpackage)
- [從 BACPAC 檔案匯入 Azure SQL 資料庫中使用 Azure 入口網站](sql-database-import.md)
- [從匯入 BACPAC 檔案使用 Azure SQL Database 或 PowerShell](sql-database-import-powershell.md)
...
2. 開啟 Management Studio 並連接到您在 [物件總管] 中的來源資料庫。
    ...
    ![從 ](./media/sql-database-cloud-migrate/MigrateUsingBACPAC01.png)
...
使用下列步驟來使用 [SqlPackage.exe](https://msdn.microsoft.com/library/hh550080.aspx) 命令列公用程式匯出移轉 [相容](#determine-if-your-database-is-compatible) BACPAC 檔案的資料庫。
...
2. 以環境的下列引數執行下列 sqlpackage.exe 命令：
   ...
   | < > target_file| BACPAC 檔案的檔案名稱和位置|
    ...
    ![從 ](./media/sql-database-cloud-migrate/TestForCompatibilityUsingSQLPackage01b.png)
...
2. 以環境的下列引數執行下列 sqlpackage.exe 命令：
   ...
   | < > source_file| 匯入的 BACPAC 檔案的檔案名稱和位置|
    ...
    ![從 ](./media/sql-database-cloud-migrate/TestForCompatibilityUsingSQLPackage01c.png)
