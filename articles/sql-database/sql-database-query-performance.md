...
1. 按一下以取得詳細資料圖表周圍。<br>第一行顯示資料庫的整體 DTU %橫條圖顯示選取之查詢所耗用的 CPU %。選取或清除圖表要包含或排除的個別查詢。

    ![排名最前面的查詢][2]

1. (選擇性) 按一下 [編輯圖表]**** 來自訂 CPU 取用量資料的顯示方式，或顯示不同的期間。

## 檢視個別查詢詳細資料

若要檢視查詢詳細資料︰

1. 按一下清單中的排名最前面查詢任何查詢。<br>的詳細資料檢視會開啟和查詢的 CPU 耗用量細分經過一段時間。
...
「查詢效能深入解析」可協助您了解您的查詢工作負載的影響，以及其與資料庫資源取用量的關係。 使用此功能，您將了解排名最前面的取用查詢，並且在發生問題前輕鬆地找出要修正的項目。 按一下資料庫刀鋒視窗上的 [查詢效能深入解析]**** 磚，以查看排名最前面的資源 (CPU) 取用查詢。
