...
- **IO-problémák**
  ...
  Előfordulhat, hogy a lekérdezések a lapokat az adatvagy a naplófájlba való írásra várnak. Ebben az esetben a `INSTANCE_LOG_RATE_GOVERNOR`, `WRITE_LOG`a vagy `PAGEIOLATCH_*` a wait statisztikát a DMV-ben találja. Lásd: a DMV használata az [IO-teljesítménnyel kapcsolatos problémák azonosításához](sql-database-monitoring-with-dmvs.md#identify-io-performance-issues).
