...
|10929|20|資源識別碼：%d。 %s 最小保證是 %d，最大限制是 %d，而資料庫的目前使用量是 %d。 但伺服器目前太忙碌，無法針對此資料庫支援大於 %d 的要求。 如需詳細資訊，請參閱 [http://go.microsoft.com/fwlink/?LinkId=267637](http://go.microsoft.com/fwlink/?LinkId=267637)。 或者，請稍後再試一次。<br/><br/>資源識別碼會指出已達到限制的資源。 對於背景工作執行緒，資源識別碼 = 1。 工作階段，資源識別碼 = 2。<br/><br/>*注意 ︰* 有關此錯誤以及如何解決此問題的詳細資訊，請參閱 ︰<br/>• [Azure SQL Database 資源限制](sql-database-resource-limits.md)。|
...
|40551|16|已終止工作階段，因為它過度使用 `TEMPDB`。 請嘗試修改查詢，以減少使用暫存資料表空間。<br/><br/>*提示 ︰* 如果您使用暫存物件，以節省空間 `TEMPDB` 之後在工作階段不再需要暫存物件卸除資料庫。|
|40552|16|已終止工作階段，因為過度使用交易記錄檔空間。 請嘗試在單一交易中修改較少的資料列。<br/><br/>*提示 ︰* 如果您執行大量插入使用 `bcp.exe` 公用程式或 `System.Data.SqlClient.SqlBulkCopy` 類別，請嘗試使用 `-b batchsize` 或 `BatchSize` 選項來限制數目的資料列複製到每一筆交易中的伺服器。 如果您要使用 `ALTER INDEX` 陳述式重建索引，請嘗試使用 `REBUILD WITH ONLINE = ON` 選項。|
