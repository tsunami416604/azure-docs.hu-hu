---
title: Önálló adatbázis erőforrások – az Azure SQL Database méretezése |} Microsoft Docs
description: Ez a cikk ismerteti az Azure SQL-adatbázis egy adatbázis rendelkezésre álló számítási és tárolási erőforrások méretezése.
...
ms.locfileid: "36309882"
---
# <a name="scale-single-database-resources-in-azure-sql-database"></a>Az Azure SQL-adatbázis egy adatbázis-erőforrások méretezése

Ez a cikk ismerteti az Azure SQL-adatbázis egy adatbázis rendelkezésre álló számítási és tárolási erőforrások méretezése. 

## <a name="vcore-based-purchasing-model-change-storage-size"></a>alapjául szolgáló vásárlási modell vCore-alapú: tároló méretének módosítása
...
- Egy adatbázis-tároló növelésével vagy csökkentésével, a maximális méret használatával helyezhetők a [Azure-portálon](https://portal.azure.com), [Transact-SQL](/sql/t-sql/statements/alter-database-azure-sql-database#examples), [PowerShell](/powershell/module/azurerm.sql/set-azurermsqldatabase), a [Azure CLI](/cli/azure/sql/db#az_sql_db_update), vagy a [REST API-t](/rest/api/sql/databases/update).
...
## <a name="vcore-based-purchasing-model-change-compute-resources"></a>alapjául szolgáló vásárlási modell vCore-alapú: a számítási erőforrásokat módosítása

Után először válassza háttérszínnek vCores száma, méretezheti egy önálló adatbázis felfelé vagy lefelé a tényleges élmény használatával dinamikusan alapján a [Azure-portálon](sql-database-single-database-scale.md#azure-portal-manage-logical-servers-and-databases), [Transact-SQL](/sql/t-sql/statements/alter-database-azure-sql-database#examples), [PowerShell](/powershell/module/azurerm.sql/set-azurermsqldatabase), a [Azure CLI](/cli/azure/sql/db#az_sql_db_update), vagy a [REST API-t](/rest/api/sql/databases/update). 
...
## <a name="dtu-based-purchasing-model-change-storage-size"></a>DTU-alapú alapjául szolgáló vásárlási modell: tároló méretének módosítása
...
## <a name="dtu-based-purchasing-model-change-compute-resources-dtus"></a>DTU-alapú alapjául szolgáló vásárlási modell: módosítás számítási erőforrások (dtu-k)
...
## <a name="dtu-based-purchasing-model-limitations-of-p11-and-p15-when-the-maximum-size-greater-than-1-tb"></a>DTU-alapú alapjául szolgáló vásárlási modell: korlátozásai a P11 és P15, ha a maximális méret 1 TB-nál nagyobb
...
Általános erőforrás-korlátok, lásd: [SQL-adatbázis vCore-alapú erőforrás korlátok - önálló adatbázisok](sql-database-vcore-resource-limits-single-databases.md) és [SQL Database DTU-alapú erőforrás korlátok - rugalmas készletek](sql-database-dtu-resource-limits-single-databases.md).
