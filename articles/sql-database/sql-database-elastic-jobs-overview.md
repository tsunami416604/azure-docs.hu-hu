...
Szilánkokra osztott adatbázis fogalma kapcsolatos további információkért lásd: [horizontális skálázási SQL Server-adatbázis](https://blog.pythian.com/sharding-sql-server-database/).
...
- Azure SQL Database
  ...
  A szolgáltatás használja az Azure SQL Database, más néven a **adatbázis vezérlése** az összes feladat-metaadatok tárolására. Az alapértelmezett szolgáltatási szinten egy S0. A díjszabással kapcsolatban lásd: [SQL Database – díjszabás](https://azure.microsoft.com/pricing/details/sql-database/).

- Azure Service Bus

  Egy Azure Service Bus az Azure Felhőszolgáltatáson belül a munka összehangolását a. Lásd: [a Service Bus díjszabása](https://azure.microsoft.com/pricing/details/service-bus/).
...
- ShardMapRefresh
  ...
  Lekérdezi a szegmenstérkép szegmensek használt összes adatbázist meghatározásához
- ScriptSplit

  A parancsfájl bontja a "GO" utasítások kötegekbe között
- ExpandJob

  Feladatok az egyes adatbázisok gyermek létrehoz egy feladatot, amely az adatbázisok egy csoportot célozza a
- ScriptExecution
