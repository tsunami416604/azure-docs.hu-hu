...
> [AZURE.NOTE] SQL Server Management Studio 在 SQL Server 2008 R2 之前的版本不支援 SQL 資料庫的連接。
...
> [AZURE.NOTE] Microsoft 建議使用自主的資料庫使用者，以加強延展性。
...
- 雖然伺服器層級主體登入不是兩個資料庫角色的成員 **dbmanager** 和 **loginmanager** 中 **主要** 資料庫中，它有授與這兩個角色的所有權限。

> [AZURE.NOTE] 此登入伺服器佈建期間建立，類似於 **sa** 登入 SQL Server 執行個體。

下列限制適用於所有的登入：

- 英文 (美國) 是預設的語言。
- 若要存取 **主要** 資料庫，每一個登入必須對應中的使用者帳戶到 **主要** 資料庫。  **主要** 的資料庫不支援自主資料庫使用者。
- 如果您未指定資料庫連接字串中，您將連線到 **主要** 預設資料庫。
