...
> [AZURE.NOTE] 我們只建議與單一 DR 區域的部署組態。 這是因為大部分 Azure 地理位置有兩個區域。 這些組態無法保護應用程式免於遭受兩個區域同時發生的重大失敗。 這類失敗的罕見事件中，您可以復原您的資料庫，在第三個區域使用 [異地還原作業](sql-database-disaster-recovery.md#recovery-using-geo-restore)。
...
主要 **取捨** 多餘的應用程式中的執行個體的次要區域只會用於嚴重損壞修復。 
...
> [AZURE.NOTE] 雖然此模式會使用一個以上的其中一個次要複本的次要資料庫會用於容錯移轉如先前所述的原因。 因為此模式需要唯讀存取次要資料庫需要 [作用中地理複寫](https://msdn.microsoft.com/library/azure/dn741339.aspx)。 
...
2. 呼叫指定的次要資料庫到 [起始資料庫容錯移轉](https://msdn.microsoft.com/library/azure/dn509573.aspx) 
...
索引鍵 **利用** 此設計模式是您可以跨多個次要複本以達到最佳的使用者效能調整應用程式工作負載。  **取捨** 這個選項是 ︰

+ 應用程式執行個體和資料庫之間的讀寫連接有不同的延遲和成本
+ 運作中斷期間會影響應用程式效能
+ 在資料庫容錯移轉之後，應用程式執行個體需要動態變更 SQL 連接字串。  

> [AZURE.NOTE] 類似的方法可以用來卸載特殊工作負載，例如報告作業、 商業智慧工具或備份。 這些工作負載通常會耗用大量的資料庫資源，因此建議您為它們指定其中一個次要資料庫，而且效能層級要符合預期的工作負載。 
...
當流量管理員偵測到主要區域的連線失敗時，就會自動將使用者流量切換到次要地區中的應用程式執行個體。 使用此模式是很重要，您 **不** 偵測到中斷之後起始資料庫容錯移轉。 次要地區中的應用程式會啟動，並使用次要資料庫在唯讀模式下運作。 下圖說明這種情形。
...
> [AZURE.NOTE] 發生在區域中的永久性服務中斷時，您必須手動啟動資料庫容錯移轉，並接受資料遺失。 應用程式將在次要地區中運作，而且具有資料庫的讀寫存取。 
