...
**術語:** 我們所說，ADO.NET 與 SQL 資料庫中互動的方式使用說明上述順序 *proxy 路由*。 如果所牽涉不到任何中介軟體，我們會說 *直接路由* 所使用。


## SQL Database V12：內部與外部


對於連線到 V12，我們必須詢問是否要執行用戶端程式 *外* 或 *內* Azure 雲端界限。 這些小節將討論兩種常見案例。
...
當您的用戶端執行 Azure 雲端界限內時，它會使用我們可以呼叫 *直接路由* 與 SQL 資料庫伺服器互動。 建立連線之後，用戶端和資料庫之間的進一步互動未牽涉到任何中介軟體 proxy。
...
- 在您的 Azure VM 上 **具有進階安全性的 Windows 防火牆** 控制的通訊埠設定。
...
*注意:* TRANSACT-SQL 陳述式 `SELECT @@version;` 傳回的值開頭的數字，例如 '11' 或者 '12。'，與這些相符 SQL Database V11 和 V12 我們版本名稱。
