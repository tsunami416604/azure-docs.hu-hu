...
* [Azure SQL Database](https://azure.microsoft.com/services/sql-database/): natív módon felhőalapú, szoftverszolgáltatás (SaaS) típusú alkalmazások fejlesztésére optimalizált SQL-adatbázis, amely platformszolgáltatási adatbázisként vagy adatbázis-szolgáltatásként is ismert. Ez a megoldás az SQL Server legtöbb funkciójával kompatibilis. A PaaS szolgáltatással kapcsolatos további információ: [Mi a PaaS?](https://azure.microsoft.com/overview/what-is-paas/), vagy tekintsen meg a következő videót: [SAIIK: PaaS és IaaS – Navigálás a döntési fában: Azure SQL DB és SQL Server virtuális gépen](https://channel9.msdn.com/Series/SAIIK-SQL-Server-on-Azure-IaaS-Implementation-Kit/SAIIK-PaaS-vs-IaaS).
...
|| Beépített magas rendelkezésre állási, vészhelyreállítási és frissítési mechanizmusokat igénylő alkalmazásokhoz. | Beépített magas rendelkezésre állási, vészhelyreállítási és frissítési mechanizmusokat nem igénylő alkalmazásokhoz. |
||Olyan csoportok számára, amelyek nem szeretnének az alkalmazás mögötti operációs rendszer beállításával és egyéb konfigurációs feladatokkal foglalkozni.| Ha testre szabható számítástechnikai környezetre van szüksége, amelyhez teljes körű rendszergazdai jogokkal rendelkezik.|
||Legfeljebb 1 TB méretű adatbázisokhoz, vagy egy kibővítési mintával [vízszintesen vagy függőlegesen particionálható](sql-database-elastic-scale-introduction.md#horizontal-and-vertical-scaling) nagyobb adatbázisokhoz.|1 TB méretnél nagyobb adatbázisokhoz, amelyek nem [particionálhatók vízszintesen vagy függőlegesen](sql-database-elastic-scale-introduction.md#horizontal-and-vertical-scaling).|
||[Szoftverszolgáltatás (SaaS) típusú alkalmazásokhoz](sql-database-design-patterns-multi-tenancy-saas-applications.md).| Hibrid alkalmazások létrehozása|
|||||
|**Erőforrások:**|Ha nem szeretne az alapul szolgáló infrastruktúra támogatására és karbantartására informatikai erőforrásokat fordítani, hanem az alkalmazásrétegre kíván összpontosítani.|Ha rendelkezik támogatási és karbantartási számítástechnikai erőforrásokkal.|
...
||Előfizetőként támogatja az [SQL Server tranzakciós replikációját](https://msdn.microsoft.com/library/mt589530.aspx).|Támogatja az [SQL Server tranzakciós replikációját](https://msdn.microsoft.com/library/mt589530.aspx), a vészhelyreállítást és az [AlwaysOn-replikákat Azure virtuális gépeken](../virtual-machines/virtual-machines-windows-sql-high-availability-dr.md).|
|||||
|||||
...
Az **SQL Database** értesítése szolgáltatásként történik az ügyfelek számára, nem pedig licenccel, míg az [Azure virtuális gépeken futó SQL Server licenceléséhez](../virtual-machines/virtual-machines-windows-sql-server-iaas-overview.md) percalapú SQL Server-licenc vagy a frissítési garancián keresztül megvásárolt saját licenc szükséges.
