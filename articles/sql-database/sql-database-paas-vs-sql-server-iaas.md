...
ms.locfileid: "68567100"
---
# <a name="choose-the-right-sql-server-option-in-azure"></a>Válassza ki a megfelelő SQL Server lehetőséget az Azure-ban
...
|  | Legfeljebb 100 TB-os adatbázisok. | Akár 8 TB-ig. | Akár 64 TB tárterülettel rendelkező SQL Server-példányokhoz. A példányok annyi adatbázist képesek támogatni, amennyire szükség van. |
...
- [Szolgáltatói szerződés](#service-level-agreement-sla) – a IaaS és a Péter is magas, iparági szabványoknak megfelelő SLA-t biztosít. A Péter opció garantálja a 99,99%-os SLA-t, míg a IaaS garantálja a 99,95%-os SLA-t az infrastruktúrához, ami azt jelenti, hogy további mechanizmusokra van szükség az adatbázisok rendelkezésre állásának A szélsőséges esetben, ha olyan magas rendelkezésre állású megoldást szeretne megvalósítani, amely megfelel a Péternek, lehet, hogy további SQL Serverokat kell létrehoznia a virtuális gépen, és konfigurálnia kell az alAlwaysOnon rendelkezésre állási csoportokat, ami megduplázhatja az adatbázis költségeit.
- Az [Azure-beli](#market) virtuális gépen SQL Server az Azure-ba való áttérés a környezet pontos egyezése, így a helyszínről az Azure SQL virtuális gépre történő áttelepítés nem különbözik, mint az adatbázisok egyik helyszíni kiszolgálóról a másikra való áthelyezése. A felügyelt példány rendkívül egyszerű áttelepítést is lehetővé tesz; Előfordulhat azonban, hogy a felügyelt példányra való Migrálás előtt végre kell térnie néhány módosításra.
