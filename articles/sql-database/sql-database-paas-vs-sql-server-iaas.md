...
* [Azure virtuális gépeken futó SQL Server](https://azure.microsoft.com/services/virtual-machines/sql-server/): az SQL Servert ebben a megoldásban a felhőben, az Azure-platformon futó Windows Server virtuális gépekre telepítik, illetve ezek futtatják a programot. Ez a módszer IaaS néven is ismert.
  Azure virtuális gépeken futó SQL Server egy telepíti a helyszíni kiszolgálón – a az egyetlen különbség, hogy a kiszolgáló jelenleg a virtuális gépek Azure-ban üzemeltetett ugyanazon SQL Server szoftverek. Az Azure SQL-kiszolgáló áttelepítése meglévő SQL Server-alkalmazások van optimalizálva, és összes verziója és az SQL Server kiadásai érhetők el. Lehetővé teszi számos más adatbázis szükség esetén támogatja a végrehajtás alatt lévő adatbázisok közötti tranzakciókat, és az SQL Server-példány és a Windows operációs rendszer teljes hozzáférést biztosít.
...
Az Azure-platformon futó és a helyszíni SQL Server-adatbázisok közötti összehasonlítás első lépése, hogy leszögezzük: semmiről nem kell lemondania. Microsoft adatplatformja SQL Server technológiát használja, és lehetővé teszi különböző fizikai, helyszíni gépeken, magánfelhőt alkalmazó környezetekben, külső üzemeltetett magánfelhő-környezetekben és a Microsoft nyilvános felhő. Az Azure-beli virtuális gépeken futó SQL Server helyszíni és felhőben futó üzemelő példányok együttes használatával segít megfelelni az egyedi és sokszínű üzleti igényeiknek, ráadásul Ön az összes környezetben ugyanazokat a kiszolgálótermékeket, fejlesztői eszközöket és ismereteket alkalmazhatja.
...
Az **SQL Database** jelenleg többféle szolgáltatáscsomagban kapható, amelyek esetében a választott szolgáltatás- és teljesítményszinten alapuló rögzített óradíjat kell fizetni. Emellett a kimenő internetes forgalom számlázása a normál [adatátviteli díjszabások](https://azure.microsoft.com/pricing/details/data-transfers/) szerint történik. A Basic, Standard, Premium, az általános célú és kritikus szolgáltatásszintek úgy tervezték, hogy előre látható teljesítményt különböző teljesítményszintek az alkalmazás maximális szükségleteknek megfelelően. Az alkalmazás változó sebességigényét követve akár válthat is a szolgáltatás- és teljesítményszintek között. Az aktuálisan választható szolgáltatásszintekkel kapcsolatos további információkért lásd: [Azure SQL Database Service Tiers](sql-database-service-tiers.md) (Az Azure SQL Database szolgáltatáscsomagjai). Létrehozhat [rugalmas készleteket](sql-database-elastic-pool.md) a nagy teljesítményű erőforrások adatbázispéldányok közötti megosztásához is.

> [!IMPORTANT]
> Amennyiben az adatbázisban sok tranzakció zajlik, és kell kiszolgálnia nagyszámú egyidejű felhasználót, a prémium szintű vagy kritikus szolgáltatásszintek javasoljuk. Az alkalmazás és az SQL-adatbázis közötti késés csökkentése érdekében érdemes, keresse meg az alkalmazás ugyanabban a régióban legyen az adatbázis, és tesztelje a teljesítmény - a szolgáltatási szint és a teljesítmény szint növelését, igény szerint.
