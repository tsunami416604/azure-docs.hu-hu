---
title: Az SQL Database szolgáltatással kapcsolatos gyakori kapcsolódási problémák elhárítása
description: Útmutatást nyújt Azure SQL Database kapcsolódási problémáinak elhárításához és más SQL Database-specifikus problémák megoldásához
...
| 49919 |16 |A létrehozási vagy frissítési kérelem nem dolgozható fel. Túl sok létrehozási vagy frissítési művelet van folyamatban a (z) "% ld" előfizetéshez.<br/><br/>A szolgáltatás foglalt, több létrehozására vagy frissítésre vonatkozó kérelmet dolgoz fel az előfizetéshez vagy a kiszolgálóhoz. A kérések jelenleg le vannak tiltva az erőforrás-optimalizáláshoz. Lekérdezési [sys. dm_operation_status](https://msdn.microsoft.com/library/dn270022.aspx) a függőben lévő műveletekhez. Várjon, amíg a függőben lévő létrehozási vagy frissítési kérelmek befejeződik, vagy törölje a függőben lévő kérelmek valamelyikét, majd ismételje meg a kérést. További információkért lásd: <br/>&bull;&nbsp; [Adatbázis-kiszolgáló erőforrás-korlátai](sql-database-resource-limits-database-server.md)<br/>&bull;&nbsp; [DTU-alapú korlátok önálló adatbázisokhoz](sql-database-service-tiers-dtu.md)<br/>&bull;&nbsp; [Rugalmas készletek DTU-alapú korlátai](sql-database-dtu-resource-limits-elastic-pools.md)<br/>&bull;&nbsp; [virtuális mag-alapú korlátok önálló adatbázisokhoz](sql-database-vcore-resource-limits-single-databases.md)<br/>&bull;&nbsp; [rugalmas készletek virtuális mag-alapú korlátai](sql-database-vcore-resource-limits-elastic-pools.md)<br/>&bull;&nbsp; [Felügyelt példányok erőforrás-korlátai](sql-database-managed-instance-resource-limits.md). |
| 49920 |16 |A kérelem nem dolgozható fel. Túl sok művelet van folyamatban a (z) "% ld" előfizetéshez.<br/><br/>A szolgáltatás foglalt több kérést dolgoz fel ehhez az előfizetéshez. A kérések jelenleg le vannak tiltva az erőforrás-optimalizáláshoz. Lekérdezési [sys. dm_operation_status](https://msdn.microsoft.com/library/dn270022.aspx) a műveleti állapothoz. Várjon, amíg a függőben lévő kérelmek befejeződik, vagy törölje a függőben lévő kérelmek valamelyikét, majd ismételje meg a kérést. További információkért lásd: <br/>&bull;&nbsp; [Adatbázis-kiszolgáló erőforrás-korlátai](sql-database-resource-limits-database-server.md)<br/>&bull;&nbsp; [DTU-alapú korlátok önálló adatbázisokhoz](sql-database-service-tiers-dtu.md)<br/>&bull;&nbsp; [Rugalmas készletek DTU-alapú korlátai](sql-database-dtu-resource-limits-elastic-pools.md)<br/>&bull;&nbsp; [virtuális mag-alapú korlátok önálló adatbázisokhoz](sql-database-vcore-resource-limits-single-databases.md)<br/>&bull;&nbsp; [rugalmas készletek virtuális mag-alapú korlátai](sql-database-vcore-resource-limits-elastic-pools.md)<br/>&bull;&nbsp; [Felügyelt példányok erőforrás-korlátai](sql-database-managed-instance-resource-limits.md). |
...
További információ az átmeneti hibák kezelésére az alkalmazás felülvizsgálata során
* [Átmeneti kapcsolódási hibák elhárítása SQL Database](sql-database-connectivity-issues.md)
...
## <a name="a-network-related-or-instance-specific-error-occurred-while-establishing-a-connection-to-sql-database-server"></a>Hálózattal kapcsolatos vagy példányra jellemző hiba történt a SQL Database kiszolgálóval létesített kapcsolat létesítése során
...
9. A generált parancsfájl helyőrzővel, szerkessze és futtassa a következő SQL-lekérdezést:
   ...
   > [!NOTE]
   > A használatával `sp_addrolemember` meghatározott felhasználókat adott adatbázis-szerepkörökhöz is leképezheti.
...
Az adatbázis-korlátokkal kapcsolatos további információkért lásd: [Azure SQL Database Server erőforrás-korlátainak SQL Database](https://docs.microsoft.com/azure/sql-database/sql-database-resource-limits-database-server).
