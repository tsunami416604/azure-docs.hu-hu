...
整合 **Dapper** 和 **DapperExtensions** 彈性資料庫與 Azure SQL Database 的用戶端程式庫很容易。 您的應用程式可以使用資料相依路由的建立和變更新開啟的 [SqlConnection](http://msdn.microsoft.com/library/system.data.sqlclient.sqlconnection.aspx) 物件使用 [OpenConnectionForKey](http://msdn.microsoft.com/library/azure/dn807226.aspx) 從呼叫 [用戶端程式庫](http://msdn.microsoft.com/library/azure/dn765902.aspx)。 這會使應用程式中的變更侷限於建立和開啟新連線的位置。 
...
Dapper 的對應程式功能提供資料庫連線的擴充方法，以簡化提交 T-SQL 陳述式以便執行或查詢資料庫。 比方說，Dapper 輕鬆對應.NET 物件之間的 SQL 陳述式的參數 **Execute** 呼叫，或使用 SQL 查詢的結果為.NET 物件使用 **查詢** 從 Dapper 呼叫。 

使用 DapperExtensions，您不再需要提供 SQL 陳述式。 例如，延伸方法 **GetList** 或 **插入** 透過資料庫連線建立在幕後的 SQL 陳述式。
...
請注意， **使用** 區塊 DDR 連線範圍 tenantid1 的一個分區的區塊內的所有資料庫作業。 查詢只會傳回目前分區上儲存的部落格，但不會傳回任何其他分區上儲存的部落格。 
...
**SqlDatabaseUtils.SqlRetryPolicy** 上述程式碼中定義為 **SqlDatabaseTransientErrorDetectionStrategy** 等待重試計數 10 和 5 秒的重試之間的時間。 如果您正在使用交易，請確定您的重試範圍會回到暫時性錯誤情況下的交易開頭。
