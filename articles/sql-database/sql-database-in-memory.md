...
> [AZURE.NOTE] 這些在預覽功能是僅適用於 [* 高階 *](sql-database-service-tiers.md) Azure SQL 資料庫，不適用於在標準或基本的服務層上的資料庫。
...
1. 在 [Azure 入口網站](http://portal.azure.com/), ，V12 伺服器上建立高階資料庫。 將 [來源]**** 設定為 AdventureWorksLT [V12] 範例資料庫。
...
**原生編譯預存程序**：SalesLT.usp_InsertSalesOrder_inmem 可以透過目錄檢視查詢來檢查：
...
- 請參閱在 ostress.exe 討論 [記憶體中 oltp 範例資料庫](http://msdn.microsoft.com/library/mt465764.aspx)。
 ...
 - 請參閱或者 [部落格安裝 ostress.exe](http://blogs.msdn.com/b/psssql/archive/2013/10/29/cumulative-update-2-to-the-rml-utilities-for-microsoft-sql-server-released.aspx)
...
`11/12/15 00:35:00.873 [0x000030A8] OSTRESS 結束一般來說，所經過時間: 00:01:31.867`
...
3. 將 T-SQL 指令碼貼到 SSMS，然後執行此指令碼。
 - 重要的是 **資料行存放區** 關鍵字 **CREATE INDEX** 陳述式，如下所示:<br/>`建立非叢集資料行存放區索引...;`
...
- dbo.FactResellerSalesXL_CCI 是具有叢集式**資料行存放區**索引的資料表，已在*資料*層級進一步壓縮。

- dbo.FactResellerSalesXL_PageCompressed 是具有對等一般叢集式索引的資料表，只會在*頁面*層級壓縮。
...
- `從 FactResellerSalesXL_PageCompressed`
- `從 FactResellerSalesXL_CCI`
...
- [深入了解記憶體中 OLTP，適用於 Microsoft SQL Server 和 Azure SQL Database](http://msdn.microsoft.com/library/dn133186.aspx)

- [深入了解 MSDN 上的即時操作分析](http://msdn.microsoft.com/library/dn817827.aspx)
...
- [使用現有的 Azure SQL 應用程式的記憶體中 OLTP。](sql-database-in-memory-oltp-migration.md)

#### 工具

- [SQL Server 資料工具預覽 (SSDT)](http://msdn.microsoft.com/library/mt204009.aspx), ，每月的最新版本。

- [SQL 伺服器的重新執行標記語言 (RML) 公用程式的說明](http://support.microsoft.com/en-us/kb/944837)
