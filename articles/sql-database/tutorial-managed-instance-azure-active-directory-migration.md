...
- Egy meglévő felügyelt példány. Lásd [: gyors útmutató: Azure SQL Database felügyelt példány létrehozása](sql-database-managed-instance-get-started.md).
  - Az `sysadmin` Azure ad-bejelentkezések létrehozásához a felügyelt példányban a-t kell használni.
...
1. SQL Server Management Studio (SSMS) használatával jelentkezzen be a felügyelt példányba **Active Directory integrált** hitelesítéssel, az adatbázishoz `migration`való csatlakozással.
...
A felügyelt példányok hitelesítésének tesztelése egy Windows-csoport `migration`tagjainak használatával. A felhasználót `aadsqlmi\testGroupUser` az áttelepítés előtt fel kell venni a `migration` csoportba.
...
1. A `db_owner` szerepkör részeként új táblát is létrehozhat.
