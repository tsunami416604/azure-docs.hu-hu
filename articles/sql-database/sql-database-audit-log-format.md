...
[Írásvédett replikák](sql-database-read-scale-out.md) A naplófájlok tárolása ugyanabban a tárolóban történik. A tárolóban lévő címtár-hierarchia űrlap `<ServerName>/<DatabaseName>/<AuditName>/<Date>/RO/`. A blob fájlnevének formátuma azonos. A csak olvasható replikák naplófájljai ugyanabban a tárolóban tárolódnak.
...
A naplózási eseményeket a rendszer a naplózási konfiguráció során definiált Log Analytics `AzureDiagnostics` munkaterületre írja `SQLSecurityAuditEvents`az adott kategóriába tartozó táblázatba. További hasznos információk a Log Analytics keresési nyelvről és parancsokról: [log Analytics keresési referenciák](../log-analytics/log-analytics-log-search.md).
