...
- Olyan adatbázis-funkciókat kíván alkalmazni, mint [a memórián belüli technológiák](sql-database-in-memory.md) a NoSQL-adatstruktúrák elemzésének vagy feldolgozásának javítása érdekében, [tranzakciós replikálással](sql-database-managed-instance-transactional-replication.md) vagy [olvasható replikákkal](sql-database-read-scale-out.md) hozzon létre másolatot az adatokról a másik helyen, és állítson be néhány analitikus számítási feladatot az elsődleges adatbázisból.
...
Ezt a kulcs-érték struktúrát úgy szabhatja testre, hogy korlátozás nélkül illeszkedjen az igényeihez. Az érték például lehet XML-dokumentum a `nvarchar(max)` típus helyett, ha az érték JSON-dokumentum, a JSON-tartalom érvényességét `CHECK` ellenőrző korlátozást adhat meg. A további oszlopok egy kulcsával kapcsolatos tetszőleges számú értéket adhat hozzá, a számított oszlopok és indexek hozzáadásával egyszerűsítheti és optimalizálhatja az adatelérést, valamint a tábla memóriához/optimalizált séma-táblaként való definiálásával jobb teljesítményt érhet el.
