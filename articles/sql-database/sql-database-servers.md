---
title: Kiszolgálók
description: További információ az Azure SQL Database-kiszolgálókról és azok kezeléséről.
...
---
# <a name="azure-sql-database-servers-and-their-management"></a>Az Azure SQL Database kiszolgálói és kezelése

## <a name="what-is-an-azure-sql-database-server"></a>Mi az Azure SQL Database-kiszolgáló?

Az SQL Database-kiszolgáló olyan logikai konstrukció, amely központi felügyeleti pontként működik több egy- vagy [készletezésű](sql-database-elastic-pool.md) adatbázis, [bejelentkezés,](sql-database-manage-logins.md) [tűzfalszabályok,](sql-database-firewall-configure.md) [naplózási szabályok](sql-database-auditing.md), [fenyegetésészlelési házirendek](sql-database-threat-detection.md)és [feladatátvételi csoportok](sql-database-auto-failover-group.md) számára: Az SQL Database-kiszolgáló az erőforráscsoportjától eltérő régióban lehet. Az SQL Database-kiszolgálónak léteznie kell az Azure SQL-adatbázis létrehozása előtt. Az SQL Database-kiszolgáló által kezelt összes adatbázis ugyanabban a régióban jön létre, mint az SQL Database-kiszolgáló.

Az SQL Database-kiszolgáló különbözik egy SQL Server-példánytól, amelyet a helyszíni világban ismerhet. Pontosabban az SQL Database szolgáltatás nem garantálja az adatbázisok helyét az SQL Database-kiszolgálóval kapcsolatban, amely kezeli őket, és nem tesz elérhetővé példányszintű hozzáférést vagy szolgáltatásokat. Ezzel szemben a felügyelt példánypéldány-adatbázisok mind közös elhelyezésűek – ugyanúgy, ahogy a helyszíni világban ismeri az SQL Servert.

SQL Database-kiszolgáló létrehozásakor olyan kiszolgálói bejelentkezési fiókot és jelszót ad meg, amely rendszergazdai jogokkal rendelkezik a kiszolgáló fő adatbázisához és a kiszolgálón létrehozott összes adatbázishoz. Ez a kezdeti fiók egy SQL bejelentkezési fiók. Az Azure SQL Database támogatja az SQL-hitelesítést és az Azure Active Directory-hitelesítést a hitelesítéshez. A bejelentkezésekről és a hitelesítésről az [Adatbázisok és bejelentkezések kezelése az Azure SQL Database-ben](sql-database-manage-logins.md)című témakörben talál. A Windows-hitelesítés nem támogatott.
...
- Névteret biztosít adatbázisokhoz, rugalmas készletekhez és adatraktárakhoz
- Egy logikai tároló erős élettartamú szemantikával - töröljön egy kiszolgálót, és törli a tartalmazott adatbázisokat, rugalmas készleteket és adatraktárakat
- Részt vesz az [Azure szerepköralapú hozzáférés-vezérlésben (RBAC)](/azure/role-based-access-control/overview) – adatbázisok, rugalmas készletek és adattárházak a kiszolgálón belül, és hozzáférési jogokat örököl a kiszolgálótól
- Az adatbázisok, rugalmas készletek és adatraktárak azure-beli erőforrás-kezelési célokra szolgáló identitásának magas szintű eleme (lásd az adatbázisok és készletek URL-sémát)
...
- Az adatbázisokra vonatkozó felügyeleti házirendek – bejelentkezések, tűzfal, naplózás, fenyegetésészlelés és hasonló kontretinók – alkalmazási köre
- A szülő-előfizetésen belüli kvóta korlátozza (alapértelmezés szerint előfizetésenként hat kiszolgáló - [lásd előfizetési korlátok at)](../azure-resource-manager/management/azure-subscription-service-limits.md)
- A benne lévő erőforrások (például 45 000 DTU) adatbáziskvótájának és DTU- vagy virtuálismag-kvótájának hatóköre
- Engedélyezett-e a tárolt erőforrásokon engedélyezett képességek verziószámozási hatóköre?
...
- Az SQL Database-kiszolgálón létrehozott összes adatbázis `SQL_LATIN1_GENERAL_CP1_CI_AS`alapértelmezett `LATIN1_GENERAL` egyeztetése a ( `CP1` angol nyelv) kódlap `CI` 1252, `AS` a kis- és nagybetűk megkülönböztetése, és ékezetes encikke-érzékeny.

## <a name="manage-azure-sql-servers-databases-and-firewalls-using-the-azure-portal"></a>Azure SQL-kiszolgálók, adatbázisok és tűzfalak kezelése az Azure Portal használatával
...
Ha az [Azure](https://portal.azure.com)Portal használatával hozzon létre egy Azure SQL Database-kiszolgálót (adatbázis nélkül), keresse meg az üres SQL-kiszolgáló (logikai kiszolgáló) űrlapot.  

### <a name="create-a-blank-or-sample-sql-database"></a>Üres vagy minta SQL-adatbázis létrehozása

Ha azure-beli SQL-adatbázist szeretne létrehozni az [Azure Portal](https://portal.azure.com)használatával, keresse meg az üres SQL-adatbázis-űrlapot, és adja meg a kért információkat. Az Azure SQL-adatbázis erőforráscsoportját és az SQL Database-kiszolgálót előre vagy maga az adatbázis létrehozása kor hozhatja létre. Létrehozhat egy üres adatbázist, vagy létrehozhat egy mintaadatbázist a Kalandorbolt LT alapján.
...
### <a name="manage-an-existing-sql-server"></a>Meglévő SQL-kiszolgáló kezelése

Meglévő kiszolgáló kezeléséhez keresse meg a kiszolgálót számos módszerrel – például egy adott SQL adatbázislapról, az **SQL-kiszolgálók** lapról vagy a **Minden erőforrás** lapról.
...
## <a name="manage-azure-sql-servers-databases-and-firewalls-using-powershell"></a>Azure SQL-kiszolgálók, adatbázisok és tűzfalak kezelése a PowerShell használatával
...
Az Azure PowerShell használatával Azure SQL-kiszolgálót, adatbázisokat és tűzfalakat hozhat létre és kezelhet, használja a következő PowerShell-parancsmagokat. Ha telepítenie vagy frissítenie kell a PowerShellt, olvassa [el az Azure PowerShell-modul telepítése című témakört.](/powershell/azure/install-az-ps) Rugalmas készletek létrehozásáról és kezeléséről [a Rugalmas készletek](sql-database-elastic-pool.md)ben.
...
> [!TIP]
> A PowerShell-gyorsútmutatóról az [Azure SQL egyetlen adatbázis létrehozása a PowerShell használatával című témakörben](sql-database-single-database-get-started.md)található. A PowerShell példaparancsfájljairól a [PowerShell használata egyetlen Azure-adatbázis létrehozásához, valamint egy tűzfalszabály konfigurálásához,](scripts/sql-database-create-and-configure-database-powershell.md) valamint [az Azure SQL-adatbázis figyelése és méretezése a PowerShell használatával című](scripts/sql-database-monitor-and-scale-database-powershell.md)témakörben található.
>
...
Az Azure SQL-kiszolgáló, adatbázisok és tűzfalak az [Azure CLI](/cli/azure)használatával történő létrehozásához és kezeléséhez használja a következő [Azure CLI SQL](/cli/azure/sql/db) Database-parancsokat. A [Cloud Shell-lel](/azure/cloud-shell/overview) futtassa a parancssori felületet a böngészőben, vagy [telepítse](/cli/azure/install-azure-cli) macOS, Linux, illetve Windows rendszeren. Rugalmas készletek létrehozásáról és kezeléséről [a Rugalmas készletek](sql-database-elastic-pool.md)ben.
...
> [!TIP]
> Az Azure CLI rövid útmutatója: [Azure SQL egyetlen adatbázis létrehozása az Azure CLI használatával](sql-database-cli-samples.md)című témakörben. Az Azure CLI példaparancsfájljairól a [CLI használata egy azure-beli egyetlen adatbázis létrehozásához és egy tűzfalszabály konfigurálásához,](scripts/sql-database-create-and-configure-database-cli.md) valamint [a CLI használata az Azure SQL-adatbázis figyeléséhez és méretezéséhez című](scripts/sql-database-monitor-and-scale-database-cli.md)témakörben található.
>

## <a name="manage-azure-sql-servers-databases-and-firewalls-using-transact-sql"></a>Azure SQL-kiszolgálók, adatbázisok és tűzfalak kezelése a Transact-SQL használatával
...
| Parancs | Leírás |
| --- | --- |
|[CREATE ADATBÁZIS (Azure SQL Database)](/sql/t-sql/statements/create-database-azure-sql-database)|létrehoz egy új adatbázist; Új adatbázis létrehozásához kapcsolódnia kell a fő adatbázishoz.|
| [ALTER ADATBÁZIS (Azure SQL-adatbázis)](/sql/t-sql/statements/alter-database-azure-sql-database) |Módosítja az Azure SQL-adatbázist. |
|[ALTER ADATBÁZIS (Azure SQL adattárház)](/sql/t-sql/statements/alter-database-azure-sql-data-warehouse)|Módosítja az Azure SQL Data Warehouse.|
...
## <a name="manage-azure-sql-servers-databases-and-firewalls-using-the-rest-api"></a>Azure SQL-kiszolgálók, adatbázisok és tűzfalak kezelése a REST API használatával
...
|[Kiszolgálók - Lista](https://docs.microsoft.com/rest/api/sql/servers/list)|Kiszolgálók listáját adja vissza.|
