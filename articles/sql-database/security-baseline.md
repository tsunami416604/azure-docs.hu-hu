...
description: Azure SQL Database Azure biztonsági alapterve
author: msmbaldwin
manager: rkarlin
ms.service: security
ms.topic: conceptual
...
**Útmutató**: az Azure Private-hivatkozás engedélyezésével engedélyezheti az Azure Pásti-szolgáltatások (például SQL Database) és az Azure által üzemeltetett ügyfél/partner szolgáltatások elérését a virtuális hálózat privát végpontján keresztül. A virtuális hálózat és a szolgáltatás közötti forgalom a Microsoft gerinchálózatán keresztül halad át, így kiküszöböli a nyilvános internet jelentette kitettséget. Ha engedélyezni szeretné, hogy a forgalom elérje a Azure SQL Database, használja az SQL-szolgáltatás címkéit, hogy engedélyezze a kimenő forgalmat a hálózati biztonsági csoportokon keresztül.
...
A Virtual Network szolgáltatásbeli végpontok és az adatbázis-kiszolgálók szabályainak használata:
...
**Útmutató**: a Azure Security Center használata és a hálózati védelmi javaslatok szervizelése arra az alhálózatra, amelyen a Azure SQL Database-kiszolgáló telepítve van. Az Azure Virtual Machines (VM) esetében, amely csatlakozni fog a Azure SQL Database Server-példányhoz, engedélyezze a hálózati biztonsági csoport (NSG) folyamatábráit a virtuális gépek védelmét biztosító NSG, és küldjön naplókat egy Azure Storage-fiókba a forgalom naplózásához. NSG-naplókat is küldhet egy Log Analytics munkaterületre, és a Traffic Analytics használatával betekintést nyerhet az Azure-Felhőbeli forgalomba. A Traffic Analytics egyes előnyei lehetővé teszi a hálózati tevékenységek megjelenítését és a gyakori pontok azonosítását, a biztonsági fenyegetések azonosítását, a forgalomban rejlő minták értelmezését, valamint a hálózati helytelen konfigurációk meghatározását.
...
**Útmutató**: a Azure SQL Database számára engedélyezett a komplex veszélyforrások elleni védelem (ATP).  A felhasználók riasztást kapnak a gyanús adatbázis-tevékenységekről, a potenciális sebezhetőségekről és az SQL-injektálási támadásokról, valamint a rendellenes adatbázis-hozzáférési és lekérdezési mintákról. A komplex veszélyforrások elleni védelem a riasztásokat is integrálja Azure Security Centerokkal.


A Azure SQL Database komplex veszélyforrások elleni védelmének megismerése és használata:
...
Az Azure-SQL Server komplex veszélyforrások elleni védelem és riasztások megismerése:
...
**Útmutató**: az Azure SQL Database-adatbázisok Azure Security Center komplex veszélyforrások elleni védelmének használata a rendellenes tevékenység figyelésére és riasztására. A speciális adatbiztonság engedélyezése az SQL-adatbázisokhoz. A speciális adatok biztonsága magában foglalja a bizalmas adatok felderítésére és besorolására, a felszínre és a lehetséges adatbázis-sebezhetőségek enyhítésére szolgáló funkciókat, valamint olyan rendellenes tevékenységek észlelését, amelyek fenyegetést jelenthetnek az adatbázisra.
...
**Útmutató**: nem alkalmazható; Az Azure SQL Server esetében a kártevő szoftvereket a Microsoft felügyeli a mögöttes platformon.
...
**Útmutató**: nem alkalmazható; A DNS-naplózás nem alkalmazható az Azure SQL Serverra.
...
**Útmutató**: nem alkalmazható; a parancssori naplózás nem alkalmazható az Azure SQL Serverra.
...
**Útmutató**: nem alkalmazható; a Azure Active Directory hitelesítés az Azure-SQL Server integrálására is beállítható, így az egyszeri bejelentkezés nem támogatott.
...
**Útmutató**: hozzon létre egy Azure Active Directory (HRE) rendszergazdát a Azure SQL Database Server-példányokhoz.
...
**Útmutató**: használja a Azure SQL Database adatfelderítési és besorolási funkciót. Az adatfelderítés és-besorolás olyan speciális képességeket biztosít, amelyek a Azure SQL Databaseba vannak építve az adatbázisok bizalmas adatainak észleléséhez, besorolásához, címkézéséhez &amp;.
...
Az Azure SQL Server és az Azure Active Directory integrálása hitelesítésre:

https://docs.microsoft.com/azure/sql-database/sql-database-aad-authentication


Hozzáférés vezérlése az Azure SQL Serverban:
...
**Útmutató**: a transzparens adattitkosítás (TDE) segít megvédeni Azure SQL Database, az Azure SQL felügyelt példányát és az Azure-adattárházat a rosszindulatú offline tevékenységek fenyegetésével szemben, ha titkosítja az inaktív adatok titkosítását. Az adatbázis, a társított biztonsági másolatok és a tranzakciós naplófájlok valós idejű titkosítását és visszafejtését hajtja végre, anélkül, hogy az alkalmazás módosítására lenne szükség. Alapértelmezés szerint a TDE engedélyezve van minden újonnan telepített Azure SQL-adatbázishoz. A TDE-titkosítási kulcsot a Microsoft vagy az ügyfél is kezelheti.
...
**Útmutató**: a Azure SQL Database speciális adatbiztonságának engedélyezése, valamint a sebezhetőségi felmérések az Azure SQL Server-kiszolgálókon való végrehajtásához Azure Security Center javaslatainak követése.
...
Az Azure SQL Server tárolt bizalmas adatok védelme és a titkosítási kulcsok tárolása a Azure Key Vaultban:
...
A biztonsági másolatok és az üzletmenet-folytonosság megismerése az Azure SQL Server:
...
A biztonsági másolatok megismerése az Azure SQL Serverban:
...
Biztonsági riasztások Azure Security Centerban: https://docs.microsoft.com/azure/security-center/security-center-alerts-overview
