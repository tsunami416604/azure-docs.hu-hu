...
1. [A fejlesztési környezet előkészítése](https://docs.microsoft.com/azure/iot-dps/quick-enroll-device-x509-java) és a GitHub-tárház klónozása:
   ...
   ```
2. Navigáljon a TPM-szimulátor mappához ```azure-iot-sdk-java/provisioning/provisioning-tool/tpm-simulator/```alatt.
...
3. ```mvn install -DskipTests=true``` futtatása az összes szükséges csomag letöltéséhez és az SDK fordításához
4. Az X. 509 tanúsítvány-generátor gyökeréhez navigáljon ```azure-iot-sdk-java/provisioning/provisioning-tools/provisioning-x509-cert-generator```.
5. Kiépítés ```mvn clean install```
