---
title: Eszközök konfigurálása távfigyelési megoldásban – Azure | Microsoft dokumentumok
description: Ez az oktatóanyag bemutatja, hogyan konfigurálhatja a távoli figyelési megoldásgyorsítóhoz csatlakoztatott eszközöket.
author: dominicbetts
manager: timlt
ms.author: dobett
...
Ebben az oktatóanyagban a távoli monitorozási megoldásgyorsítóval konfigurálhatja és kezelheti a csatlakoztatott IoT-eszközöket. Új eszközt ad hozzá a megoldásgyorsítóhoz, és konfigurálja az eszközt.

A Contoso új gépet rendelt, amellyel az egyik létesítményét szeretné bővíteni. Amíg az új gép kézbesítésére vár, Ön lefuttat egy szimulációt a megoldás működésének tesztelése érdekében. A szimuláció futtatásához adjon hozzá egy új szimulált motoreszközt a távfigyelési megoldás gyorsítójához, és ellenőrizze, hogy a szimulált eszköz megfelelően reagál-e a konfigurációs frissítésekre. Bár ez az oktatóanyag szimulált eszközöket használ, az eszközfejlesztő közvetlen metódusokat valósíthat meg [a Távoli figyelési megoldásgyorsítóhoz csatlakoztatott valódi eszközön.](iot-accelerators-connecting-devices.md)
...
Keresse meg a megoldás **Eszközkezelő** lapját, majd kattintson a **+ Új eszköz**elemre:
...
[![Szimulált motoreszköz kiépítése](./media/iot-accelerators-remote-monitoring-manage/devicesprovisionengine-inline.png)](./media/iot-accelerators-remote-monitoring-manage/devicesprovisionengine-expanded.png#lightbox)

## <a name="test-the-simulated-device"></a>A szimulált eszköz tesztelése

Ha tesztelni a szimulált motoreszköz küld telemetriai adatokat, és a jelentéstulajdonság-értékek, jelölje ki azt az eszközök listájában az **Eszközkezelő** lapon. A motor valós idejű információi a **Device Details** (Eszköz részletei) panelen jelennek meg:
...
[![A megtekinteni kívánt telemetriai adatfolyam kiválasztása](./media/iot-accelerators-remote-monitoring-manage/devicesvibration-inline.png)](./media/iot-accelerators-remote-monitoring-manage/devicesvibration-expanded.png#lightbox)
...
Annak teszteléséhez, hogy frissítheti-e a motor konfigurációs tulajdonságait, jelölje ki azt az **Eszközkezelő** lap eszközlistájában. Ezután kattintson **a Feladatok**gombra, és válassza a **Tulajdonságok lehetőséget.** A feladatok paneljén a kiválasztott eszköz frissíthető tulajdonságértékei láthatók:
...
[![Eszköztulajdonság-érték frissítése](./media/iot-accelerators-remote-monitoring-manage/devicesreconfigurephysical-inline.png)](./media/iot-accelerators-remote-monitoring-manage/devicesreconfigurephysical-expanded.png#lightbox)

A feladat állapotának nyomon követéséhez kattintson a **Feladat állapotának megtekintése gombra:**

[![Eszköztulajdonság-érték frissítése](./media/iot-accelerators-remote-monitoring-manage/locationjobstatus-inline.png)](./media/iot-accelerators-remote-monitoring-manage/locationjobstatus-expanded.png#lightbox)

Ha a feladat befejeződött, lépjen vissza az **irányítópultra**. A motoreszköz az új helyén jelenik meg a térképen:

[![A motor helyének megtekintése](./media/iot-accelerators-remote-monitoring-manage/enginelocation-inline.png)](./media/iot-accelerators-remote-monitoring-manage/enginelocation-expanded.png#lightbox)
...
Az összes eszköz megjelenítéséhez keresse meg az **Eszközkezelő** lapot, és válassza a **Minden eszköz** szűrőt:
...
[![Címke hozzáadása prototípushoz és teherautó-eszközökhöz](./media/iot-accelerators-remote-monitoring-manage/devicesaddtag-inline.png)](./media/iot-accelerators-remote-monitoring-manage/devicesaddtag-expanded.png#lightbox)
...
[![Címke hozzáadása a hűtőhöz, a lifthez és a motoreszközökhöz](./media/iot-accelerators-remote-monitoring-manage/devicesaddtag2-inline.png)](./media/iot-accelerators-remote-monitoring-manage/devicesaddtag2-expanded.png#lightbox)

### <a name="create-filters"></a>Szűrők létrehozása

A címkeértékek alapján létrehozhatja a szűrőket. Az **Eszközkezelő** lapon kattintson az **Eszközcsoportok kezelése**gombra:
...
[![Intelligens épületszűrő létrehozása](./media/iot-accelerators-remote-monitoring-manage/smartbuildingfilter-inline.png)](./media/iot-accelerators-remote-monitoring-manage/smartbuildingfilter-expanded.png#lightbox)
...
[![Csatlakoztatott járműszűrő létrehozása](./media/iot-accelerators-remote-monitoring-manage/connectedvehiclefilter-inline.png)](./media/iot-accelerators-remote-monitoring-manage/connectedvehiclefilter-expanded.png#lightbox)

A Contoso operátora mostantól lekérdezheti az eszközöket az üzemeltetési csapat alapján:

[![Csatlakoztatott járműszűrő létrehozása](./media/iot-accelerators-remote-monitoring-manage/filterinaction-inline.png)](./media/iot-accelerators-remote-monitoring-manage/filterinaction-expanded.png#lightbox)
