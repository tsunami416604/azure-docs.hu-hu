...
1. A VS Code-ban nyissa meg a **FirmwareOTA. Ino** fájlt, és módosítsa a `currentFirmwareVersion` `1.0.0`ról `1.0.1`re:
...
1. Nyissa meg a parancssort, majd írja be és válassza a **IoT Workbench: eszköz**elemet. Ezután válassza az **eszköz fordítása** lehetőséget a kód fordításához:
    ...
    A VS Code a projekt `.build` mappájába menti a lefordított fájlt. A beállításoktól függően a VS Code elrejtheti a `.build` mappát a Explorer nézetben.
...
Mentse a következő konfigurációt a **Firmware-Update. JSON** nevű fájlként a helyi számítógépen. Cserélje le a `YOURSTRORAGEACCOUNTNAME`, `YOURCHECKSUM`és `YOURPACKAGESIZE` helyőrzőket a korábban feljegyzett értékekre:
