...
![資源與資源群組刀鋒視窗上的標記部分](./media/azure-architecture-cpif-overview/overview.png)
...
- **基礎結構**– Microsoft Azure 是基礎與平台--a-服務解決方案，它包含幾個基礎服務和功能。  這些服務主要可分成計算、儲存體和網路服務，不過也有幾項無法以這些定義歸納的功能。  基礎結構模式詳述 Microsoft Azure 的特定功能區域，其為將特定服務提供給一或多個裝載於特定 Azure 訂用帳戶之解決方案時的必要項目。 
- **Foundation** – 撰寫多層式應用程式或在 Azure 中的服務時，必須數個元件組合中用來提供合適的裝載環境。  基礎模式能從 Microsoft Azure 構成一或多個服務，藉此支援應用程式內特定層級的功能。 這可能需要使用上述基礎結構模式描述的一或多個元件。 例如，多層式應用程式的展示層需要 Azure 中的計算、網路和儲存體功能才能正常運作。  基礎模式的構成僅限搭配特定解決方案隨附的其他模式。
- **方案** – 解決方案模式以代表結束應用程式或服務正在開發的基礎結構和/或基礎模式所組成。  我們可以想像，複雜解決方案的開發將無法獨立於其他模式之外單獨進行。  解決方案應使用前述各模式類別所定義的元件和介面。    
...
[Azure 網路](https://gallery.technet.microsoft.com/Cloud-Platform-Integration-5e401f38)
