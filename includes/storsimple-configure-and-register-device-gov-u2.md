...
5. 執行下列步驟，為裝置設定最小的必要網路設定。
    > [AZURE.IMPORTANT] 這些設定步驟必須在裝置的主動控制器上執行。 序列主控台功能表會在橫幅訊息中指出控制站狀態。 如果您未連接到主動控制器，請中斷連線，然後連接到主動控制器。
      ...
      > [AZURE.IMPORTANT] 雖然 Web Proxy 設定是選用的，但請注意，如果您使用 Web Proxy，就只能在此處設定它。 如需詳細資訊，請移至 [設定裝置的 web proxy](storsimple-configure-web-proxy.md)。 
...
7. 安裝更新，如下所示：
      ...
      2. 在命令提示字元中，執行 `Get HcsUpdateAvailability`。 您應該會在更新可用時收到通知。

      3. 執行 `開始 HcsUpdate`。 您可以在任何模式中執行此命令。 更新會套用至第一個控制器，控制器會容錯移轉，然後更新會套用至其他控制器。

      您可以藉由執行監視更新進度 `Get-hcsupdatestatus`。
     ...
     下列範例輸出指出更新已完成。
        ...
        RunInprogress: False
        2015/4/13 Lastupdatetimestamp: 10:56:13 PM
        2015/4/13 LastUpdateTimestamp: 下午 10:35:25
        Controller0Events:
        Controller1Events:
