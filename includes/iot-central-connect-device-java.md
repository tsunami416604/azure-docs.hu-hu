...
Az Microsoft Azure IoT SDK a korábban letöltött Java-példányban nyissa meg a *Azure-IOT-SDK-Java/Device/IOT-Device-Samples/PnP-Device-Sample/thermostat-Device-Sample/src/Main/Java/Samples/com/Microsoft/Azure/SDK/IoT/Device/termosztát. Java* fájlt egy szövegszerkesztőben.
...
* `initializeAndProvisionDevice`A modell azonosítójának megadására, a `dtmi:com:example:Thermostat;1` DPS használatával adja meg és regisztrálja az eszközt, hozzon létre egy **DeviceClient** -példányt, és kapcsolódjon a IoT Central alkalmazáshoz. IoT Central a modell AZONOSÍTÓját használja az eszközhöz tartozó sablon azonosításához vagy létrehozásához. További információ: [eszköz hozzárendelése egy eszköz sablonnal](../articles/iot-central/core/concepts-get-connected.md#associate-a-device-with-a-device-template).
...
* Elindít egy szálat a hőmérséklet telemetria küldéséhez, és `maxTempSinceLastReboot` öt másodpercenként frissíti a tulajdonságot.
...
A `initializeAndProvisionDevice` metódus azt mutatja, hogy az eszköz hogyan használja a DPS-t a IoT Centralhoz való regisztráláshoz és kapcsolódáshoz. Az adattartalom tartalmazza azt a modell-azonosítót, amelyet a IoT Central az [eszköz egy eszköz sablonnal való hozzárendeléséhez](../articles/iot-central/core/concepts-get-connected.md#associate-a-device-with-a-device-template)használ:
...
A `sendTemperatureTelemetry` metódus azt mutatja, hogy az eszköz hogyan küldi el a hőmérséklet telemetria IoT Central:
...
A `updateMaxTemperatureSinceLastReboot` metódus egy `maxTempSinceLastReboot` tulajdonság frissítését IoT Centralre küldi:
...
Az `TargetTemperatureUpdateCallback` osztály az `TwinPropertyCallBack` írható tulajdonságok frissítéseinek IoT Central való kezelésére szolgáló metódust tartalmazza:
...
Az `GetMaxMinReportMethodCallback` osztály tartalmazza a `call` IoT Central nevű parancsok kezelésére szolgáló metódust:
...
Windows rendszeren navigáljon a letöltött Java SDK-tárház gyökérkönyvtárába.
...
A minta alkalmazás futtatásához nyisson meg egy parancssori környezetet, és nyissa meg az *Azure-IOT-SDK-Java/Device/IOT-Device-Samples/PnP-Device-Sample/termosztát-Device-Sample* mappát, amely tartalmazza a *termosztátot. Java* -minta fájlt.
