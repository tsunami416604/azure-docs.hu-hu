...
Hozzon létre egy méretezési csoport a(z) `azure vmss quick-create` parancs használatával. A következő példa létrehoz egy `myVMSS` elnevezésű, Linux rendszerű méretezési csoportot öt virtuálisgép-példánnyal a `myResourceGroup` nevű erőforráscsoportban:
...
Ez a parancs visszatér, amint a méretezési csoport végzett az üzembe helyezéssel. Ezen a ponton a méretezési készlet egy NAT-szabállyal rendelkező terheléselosztót is tartalmaz, amely a terheléselosztó 50,000+i portját a virtuális gép 22-es portjához rendeli. A terheléselosztó FQDN-jének meghatározása után így ssh-kapcsolaton keresztül csatlakozhatunk a virtuális gépekhez:
