...
>-先前外部化的內容將無法在重新安裝配接器。
>
>的上傳到站台解除安裝舊版 StorSimple Adapter for SharePoint，但再安裝新的版本之後, 任何內容將儲存在內容資料庫中。 安裝新的配接器之後，您必須將該內容移至 StorSimple 裝置。 您可以使用 Microsoft` RBS Migrate()` 移轉內容的 SharePoint 隨附的 PowerShell cmdlet。 如需詳細資訊，請參閱 [移轉內容出 RBS 或](https://technet.microsoft.com/library/ff628255.aspx)。 
