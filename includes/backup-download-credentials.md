...
2.  按一下 [ **復原服務** 在左的導覽窗格中，選取您所建立的備份保存庫。 按一下雲端圖示以前往備份保存庫的 [快速啟動] 檢視。

    ![快速檢視](./media/backup-download-credentials/quickview.png)

3.  在 [快速入門] 頁面中，按一下 [ **下載保存庫認證**。 在入口網站會產生可供下載的保存庫認證檔。

    ![下載](./media/backup-download-credentials/downloadvc.png)

4.  入口網站會使用保存庫名稱和目前日期的組合來產生保存庫認證。 按一下 [ **儲存** 保存庫認證下載至本機帳戶的下載資料夾，或從指定的位置保存庫認證儲存功能表中選取 [另存新檔。
