...
透過最近上市的 [Microsoft Visual c + + Compiler for Python 2.7]][], ，現在會更輕鬆地建置擁有 Python 2.7 原生程式碼的封裝。
...
Mac/Linux 使用者: 如果您沒有 Windows 電腦的存取權，請參閱 [建立虛擬機器執行 Windows 的 []][] 如何在 Azure 上建立 VM。 您可以使用該虛擬機器建置 Wheel、將它們加入至儲存機制，以及在您想要捨棄虛擬機器時捨棄。
...
編輯 requirements.txt 以便加入 `--find-links` 頂端] 選項。 這會告訴 PIP 尋找本機資料夾中完全相符的項目，才能進入 Python 封裝索引。

    --find-links wheelhouse
    azure==0.8.4

如果您想要在 \wheelhouse 資料夾中包含所有相依性和不使用 python 套件索引，您可以強制 pip 忽略封裝索引加入 `-沒有索引` requirements.txt 的頂端。
