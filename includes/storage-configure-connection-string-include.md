...
- 如果您的應用程式執行於 Azure 雲端服務，儲存連接字串使用 Azure 服務組態系統 (`*.csdef` 和 `*.cscfg` 檔案)。 請參閱 [如何建立和部署雲端服務](../articles/cloud-services/cloud-services-how-to-create-deploy.md) Azure 雲端服務組態的詳細資料。
- 如果您的應用程式執行於 Azure 虛擬機器，或如果您要建置 Azure 外部執行的.NET 應用程式，儲存連接字串使用.NET 設定系統 (例如 `web.config` 或 `app.config` 檔案)。
...
1.  在 Visual Studio 中的 [方案總管] 中 **角色**
    ...
    ![在 Visual Studio 中，於雲端服務角色上選取屬性][connection-string1]
...
如果您要撰寫的應用程式並非 Azure 雲端服務 (請參閱上節)，建議您使用 .NET 設定系統 (例如 `web.config` 或 `app.config`)。這類應用程式包括 Azure 網站或 Azure 虛擬機器，以及設計成在 Azure 外執行的應用程式。您可以儲存連接字串使用 `< g s >` ，如下所示的項目。取代 `帳戶名稱` 的儲存體帳戶名稱和 `帳戶金鑰` 與您的帳戶存取金鑰:
