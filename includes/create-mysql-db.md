本指南將示範如何使用 [ClearDB] 從 [Azure 存放區]，以及如何建立 MySQL 資料庫中建立 MySQL 資料庫作為連結的資源建立時 [Azure 網站 ][waws] 。 [ClearDB] 是可讓您執行和管理 MySQL 資料庫，在 Azure 資料中心，以及從任何應用程式連線至這些容錯的資料庫做為服務提供者。
