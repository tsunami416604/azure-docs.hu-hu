...
Azure 虛擬機器可以處理多種不同的雲端裝載工作。 但是，建立和管理 VM 基礎結構需要特殊的技能和大量人力。 如果您不需要完全控制執行您 web 應用程式、 行動應用程式後端、 API 應用程式等的 Vm，則較簡單 （且成本較低） 的方案 ︰ *平台即服務* (PaaS)。 使用 PaaS 時，Azure 會為執行應用程式的 VM 處理大部分的管理工作。 [Azure App Service](../article/app-service/app-service-value-prop-what-is.md) 是完全受管理的 PaaS 產品，可讓您建置、 部署及調整企業級應用程式，以秒為單位。
...
![應用程式服務圖表](media/app-service-choose-me-content/diagram.png)
