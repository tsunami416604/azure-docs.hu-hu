...
>[AZURE.NOTE]您可以快取後端，無論您使用用戶端管理或服務管理驗證的 Azure 服務所簽發的權杖。 本教學課程使用服務管理驗證。
...
3. 在 ToDoActivity.java 檔案中，新增下列 `cacheUserToken` 方法的定義。
    ...
    >[AZURE.NOTE]如果權杖存取您的資料十分敏感，而且有人可能可以取得裝置的存取權，您可以進一步保護加密的語彙基元。 不過，完整的安全解決方案已超過本教學課程範圍，而且也須視您的安全性需求而定。
...
5. 在 *ToDoActivity.java* 檔案中，將 `authenticate` 方法，使用下列方法會使用權杖快取。 如果您想要使用 Google 以外的帳戶，請變更登入提供者。
