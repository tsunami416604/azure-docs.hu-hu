...
* 若要建立行動裝置應用程式後端和用戶端的 [建立行動裝置應用程式] 教學課程中遵循的步驟。 您可以使用行動應用程式支援的任何用戶端平台，包括 iOS、Windows、Xamarin iOS 和 Xamarin Android。
...
3. 在 Web API 專案中，編輯 MultiChannelToDoContext.cs。 在 `OnModelCreating`, ，更新您的行動裝置應用程式名稱相同的結構描述名稱:
...
1. 在 Visual Studio 中，開啟 **MultiChannelToDo.Web** 專案。 編輯檔案 `js/service/ToDoService.js`, ，將 URL 新增至您剛剛發行的 Web API:
...
2. 執行您在 [建立行動裝置應用程式] 教學課程中建立的行動應用程式。 您將會在 Web 應用程式看到相同的待辦項目。
