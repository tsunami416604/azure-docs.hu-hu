...
當您建立網站時，Azure 會提供好記的子 azurewebsites.net 網域上讓使用者可以存取您的網站使用類似 http://&lt;mysite&gt;.azurewebsites.net 的 URL。 不過，如果您將網站設定為共用或標準模式，則可以將網站對應到您自己的網域名稱。
...
> [AZURE.NOTE] 您需要將網站設定為共用或標準模式，才能執行此工作中的步驟，但這可能會變更您的訂用帳戶費用。 請參閱 <a href="/pricing/details/web-sites/">網站定價詳細資料</a> 如需詳細資訊。

本文內容：

-   [了解 CNAME 和 A 記錄](#understanding-records)
-   [設定網站為共用或標準模式](#bkmk_configsharedmode)
...
A 記錄將網域對應，例如 **contoso.com** 或 **www.contoso.com**, ，*或萬用字元網域* 例如 **\*.contoso.com**, ，IP 位址。 以 Azure 網站而言，就是指服務的虛擬 IP 或您為網站購買的特定 IP 位址。 相較於 CNAME 記錄，A 記錄的主要優點在於只需要有一個項目使用萬用字元，例如 ***.contoso.com**，即可處理多個子網域的要求，例如 **mail.contoso.com**、**login.contoso.com** 或 **www.contso.com**。
...
<a name="trafficmanager"></a><h2>(選擇性)將您的網站新增至流量管理員</h2>

如果您要對網站使用流量管理員，請執行下列步驟。

1. 如果您還沒有流量管理員設定檔，使用中的資訊 [建立流量管理員設定檔使用 ][createprofile] 建立一個。 請記下與您的流量管理員設定檔關聯的 **.trafficmgr.com** 網域名稱。 稍後將會使用該資訊。
...
(contoso.azurewebsite.net)，因此使用者看不到轉送過程
。
> [AZURE.NOTE] 上述範例僅適用於 __www__ 子網域的流量。 因為 CNAME 記錄不能使用萬用字元，所以您必須為每一個網域/子網域建立一個 CNAME。 如果您想要直接來自子網域，例如 *。 contoso.com，您的 azurewebsite.net 位址，您可以設定 __URL 重新導向__ 或 __URL 轉送__ 項目，在 DNS 設定，或建立 A 記錄。

> [AZURE.NOTE] 需要一些時間，CNAME 才能傳播至整個 DNS 系統。 必須等到 CNAME 傳播之後，您才能設定網站的 CNAME。 您可以使用服務，例如 <a href="http://www.digwebinterface.com/">http://www.digwebinterface.com/</a> 來驗證 CNAME 已可供使用。
...
> [AZURE.NOTE] 需要一些時間，awverify CNAME 才能傳播至整個 DNS 系統。 必須等到 awverify CNAME 傳播之後，您才能為網站設定 A 記錄所定義的自訂網域名稱。 您可以使用服務，例如 <a href="http://www.digwebinterface.com/">http://www.digwebinterface.com/</a> 來驗證 CNAME 已可供使用。
