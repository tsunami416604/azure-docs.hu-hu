本文將說明如何在 Azure App Service web 應用程式設定 HTTPS。 不包含用戶端憑證驗證。如需有關的資訊，請參閱 [方式來設定 TLS 相互驗證的 Web 應用程式](../articles/app-service-web/app-service-web-configure-tls-mutual-auth.md)。
...
若要啟用 HTTPS 的自訂網域，例如 **contoso.com**, ，您必須先 [Azure App Service 中設定自訂網域名稱](../articles/app-service-web/web-sites-custom-domain-name.md)。 然後，執行下列動作:

1. [取得 SSL 憑證](#bkmk_getcert)
2. [設定標準定價層](#bkmk_standardmode)
2. [在您的應用程式中設定 SSL](#bkmk_configuressl)
...
在要求 SSL 憑證之前，您必須先決定有哪些網域名稱要由該憑證保護。 這將決定您必須取得的憑證類型。 如果您只需要保護單一網域名稱 (如 **contoso.com** 或 **www.contoso.com**)，則基本憑證就已足夠。 如果您需要保護多個網域名稱，例如 **contoso.com**, ，**www.contoso.com**, ，和 **mail.contoso.com**, ，則您可以取得 [萬用字元憑證](http://en.wikipedia.org/wiki/Wildcard_certificate), ，或是憑證 [主體替代名稱](http://en.wikipedia.org/wiki/SubjectAltName) (subjectAltName)。

應用程式服務搭配使用的 SSL 憑證必須經過 [憑證授權單位](http://en.wikipedia.org/wiki/Certificate_authority) (CA)。 如果您還沒有此類憑證，則必須向發行 SSL 憑證的公司取得。 如需憑證授權單位的清單，請參閱 [Windows 和 Windows Phone 8 SSL 根憑證計劃 (成員 Ca) 和 ][cas] Microsoft TechNet Wiki 上。
...
- [取得使用 OpenSSL SubjectAltName 憑證](#bkmk_subjectaltname)
...
您可能還需要取得 * *[中繼憑證](http://en.wikipedia.org/wiki/Intermediate_certificate_authorities)* * (也稱為鏈結憑證)，如果您的 CA 會使用這些。 使用中繼憑證比使用「未鏈結憑證」安全，因此常會見到 CA 使用中繼憑證。 中繼憑證通常是從 CA 網站個別下載而得。 這篇文章中的步驟提供確保任何中繼憑證已上傳至您的應用程式的憑證相合併的步驟。
...
1. 開啟 [記事本]****，然後建立含有下列內容的新文件。 取代 **mysite.com** 上與您的應用程式的自訂網域名稱的主旨。 例如，Subject = "CN=www.contoso.com"。
...
5. 將 **myrequest.csr** 提交至憑證授權單位，以取得 SSL 憑證。 這可能包括上傳檔案，或在 [記事本] 中開啟檔案並將內容直接貼到 Web 表單。
    ...
    如需憑證授權單位的清單，請參閱 [Windows 和 Windows Phone 8 SSL 根憑證計劃 (成員 Ca) 和 ][cas] Microsoft TechNet Wiki 上。
...
現在可以將匯出的 PFX 檔案上傳至 Azure App Service 中的應用程式。
...
3. 將您的 CSR 提交至憑證授權單位，以便取得 SSL 憑證。 如需憑證授權單位的清單，請參閱 [Windows 和 Windows Phone 8 SSL 根憑證計劃 (成員 Ca) 和 ][cas] Microsoft TechNet Wiki 上。
...
2. 將您的 CSR 提交至憑證授權單位，以便取得 SSL 憑證。 如需憑證授權單位的清單，請參閱 [Windows 和 Windows Phone 8 SSL 根憑證計劃 (成員 Ca) 和 ][cas] Microsoft TechNet Wiki 上。
...
4. 匯出憑證，從 IIS 管理員如需匯出憑證的詳細資訊，請參閱 [匯出伺服器憑證 (IIS 7) ][exportcertiis]。 將在稍後步驟中所匯出的檔案上傳至 Azure，您的應用程式搭配使用。
...
在某些情況下，您可能會想要取得憑證來進行測試，並等到要實際投入使用時，才將向信任 CA 購買憑證。 自我簽署憑證就有這樣的功能。 自我簽署憑證是您將自己當成憑證授權單位來建立和簽署的憑證。 雖然此憑證可以用來保護應用程式，因為大部分的瀏覽器造訪因為憑證不由受信任的 CA 所簽署的應用程式時都會傳回錯誤。 某些瀏覽器甚至會拒絕讓您檢視應用程式。

- [產生自我簽署的憑證，使用 makecert](#bkmk_ssmakecert)
- [產生自我簽署的憑證，使用 OpenSSL](#bkmk_ssopenssl)
...
3. 若要匯出的憑證。PFX 檔案，可以上傳至 Azure App Service 中的應用程式，請使用下列命令:
...
為自訂網域僅適用於啟用 HTTPS **標準** Azure App Service 中的層。 請使用下列步驟將您的 App Service 方案切換至 [**標準**] 層。
> [AZURE.NOTE] 從應用程式切換之前 **免費** 層到 **標準** 層，您應該移除訂閱的支出上限，而變得無法使用您的應用程式如果達到在計費期間結束前的支出。 如需有關共用和 **標準** 層，請參閱 [定價的詳細資料 ][pricing]。
...
7.  在 **延展** 區段中，按一下 [設定應用程式服務計劃模式 **選取**。
    ![定價層][sslreserved]
    > [AZURE.NOTE] 如果您收到 「 設定 web 應用程式 '< 應用程式名稱 >' 無法調整] 錯誤您可以使用 [詳細資料] 按鈕，以取得詳細資訊。 您可能會收到 「 沒有足夠的可用標準執行個體伺服器可滿足此要求 」。 發生錯誤。 如果您收到此錯誤，請連絡 [Azure 支援](/support/options/)。

<a name="bkmk_configuressl"></a>
## 3.在應用程式中設定 SSL

在之前執行本節中的步驟，您必須有關聯的自訂網域名稱與您的應用程式。 如需詳細資訊，請參閱 [設定 web 應用程式 ][customdomain]。
...
8.  使用 [上傳憑證]**** 對話方塊，選取之前以 IIS 管理員或 OpenSSL 建立的 .pfx 憑證檔案。 指定當初用來保護 .pfx 檔案的密碼 (如果有的話)。 最後，按一下 [ **儲存** 以上傳憑證。
    ![SSL 上傳][ssluploadcert]
9. 在 [**SSL 設定**] 索引標籤的 [**SSL 繫結**] 區段中，使用下拉式清單選取要以 SSL 保護的網域名稱，以及要使用的憑證。 您也可以選取是否要使用 [伺服器名稱指示 ][sni] (SNI) 還是 IP SSL。

    ![SSL 繫結][sslbindings]

    * IP SSL 會將伺服器的專用公用 IP 位址對應至網域名稱，以建立憑證與網域名稱的關聯。 這需要每個網域名稱 (contoso.com、 fabricam.com 等) 您的服務有專用的 IP 位址相關聯。 這是傳統用來建立 SSL 憑證與網頁伺服器之關聯的方法。
...
>
> 1.您已設定 IP SSL 繫結之後，專用的 IP 位址指派給您的應用程式。 您可以找到這個 IP 位址上 **儀表板** 應用程式頁面，請在 **快速瀏覽** 一節。 它將會列出成為 [虛擬 IP 位址]****：
>    
...
Azure App Service「*不會*」強制使用 HTTPS。 訪客可能仍會存取您的應用程式使用 HTTP，這可能會危及您的應用程式安全性。 如果您想要強制您的應用程式使用 HTTPS，您可以使用 **URL 重寫** 模組。 Azure App Service 隨附 URL Rewrite 模組，此模組可讓您定義連入要求在送達應用程式之前要套用的規則。 **它可用於以任何 Azure 支援的程式設計語言所撰寫的應用程式。**
...
> [AZURE.NOTE] 如果您的應用程式以撰寫  **Node.js**, ，**PHP**, ，**Python Django**, ，或 **Java**, ，它可能不包含 web.config 檔案。 不過，裝載於 Azure App Service 時，**Node.js**、**Python Django** 和 **Java** 都會確實用到 web.config - Azure 會在部署期間自動建立此檔案，因此您永遠看不到此檔案。 如果您在應用程式中加入一個 web.config 檔案，則它將覆寫 Azure 自動產生的同名檔案。

### .NET

對於.NET 應用程式，修改您的應用程式的 web.config 檔案，並新增 **< 重寫 >** > 一節 [範例](#example) 至 **< 系統。Web 伺服器 >** 一節。

如果您的 web.config 檔案已經包含 **< 重寫 >** 區段中，加入 **< 規則 >** 從 [範例](#example) 中的第一個項目 **< 規則 >** 一節。
...
3. 使用下列資訊新增重寫規則。
    ...
    * **Node.js 和 Python Django**
        ...
        為 Node.js 和 Python Django 應用程式便已產生的 web.config 檔案 **< 重寫 >** 區段，其中包含 **< 規則 >** 項目所需的應用程式正常運作。 若要強制應用程式以使用 HTTPS，請新增 **< 規則 >** 範例中的第一個項目中 **< 規則 >** 一節。 這會在保留其他規則不變的情況下強制使用 HTTPS。
...
- [解除鎖定在 Azure 網站上的組態選項](/blog/2014/01/28/more-to-explore-configuration-options-unlocked-in-windows-azure-web-sites/)
