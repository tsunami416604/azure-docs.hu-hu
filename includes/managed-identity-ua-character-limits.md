---
title: fájl belefoglalása
description: fájl belefoglalása
services: active-directory
author: daveba
ms.service: msi
ms.topic: include
ms.date: 05/29/2018
ms.author: daveba
ms.custom: include file
ms.openlocfilehash: 082a4fafe76631435b5969298b0a806565666956
ms.sourcegitcommit: 59fffec8043c3da2fcf31ca5036a55bbd62e519c
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 06/04/2018
ms.locfileid: "34702246"
---
> [!IMPORTANT]
> A felhasználói identitások, csak alfanumerikus karaktereket létrehozásakor (0-9, a – z, A-Z) és a kötőjel (-) támogatottak. Emellett a név korlátozni kell 24 karakter hosszú, a hozzárendelés VM/VMSS megfelelően működjön. Biztonsági frissítések ellenőrzése. További információkért lásd: [– gyakori kérdések és ismert problémákat](/azure/active-directory/managed-service-identity/known-issues).