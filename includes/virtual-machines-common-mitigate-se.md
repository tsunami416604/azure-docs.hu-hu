...
Írja be a `wmic`t az interaktív interfész megadásához. Ezután írja be az alábbit a virtuális gépen található fizikai és logikai processzorok mennyiségének megtekintéséhez.
...
Ha a kimenetben `MDS mitigation is enabled: False`jelenik meg, vegye [fel a kapcsolatot az Azure ügyfélszolgálatával](https://aka.ms/MicrocodeEnablementRequest-SupportTechnical) az elérhető kockázatcsökkentő lehetőségekért.



**3. lépés**: a kernel virtuális IP-ÁRNYÉKOLÁS (KVAS) és az ág-cél injekciós (KTF) operációs rendszer támogatásának engedélyezéséhez kövesse a [KB4072698](https://support.microsoft.com/help/4072698/windows-server-guidance-to-protect-against-the-speculative-execution) utasításait a védelem engedélyezéséhez a `Session Manager` beállításkulcsok használatával. Újraindítás szükséges.
...
1.  Kövesse a [KB4072698](https://support.microsoft.com/help/4072698/windows-server-guidance-to-protect-against-the-speculative-execution) utasításait a `MinVmVersionForCpuBasedMitigations` beállításkulcsok használatával történő védelem engedélyezéséhez.
2.  Adja meg a hypervisor Scheduler-típust úgy, hogy a `Core` az [itt](https://docs.microsoft.com/windows-server/virtualization/hyper-v/manage/manage-hyper-v-scheduler-types)leírt útmutatást követve.
