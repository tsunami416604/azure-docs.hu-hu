4. 瀏覽至您的 API 應用程式的閘道刀鋒視窗。
    ...
    ![按一下 ](./media/app-service-api-gateway-config-auth/gateway.png)

7. 在 [**閘道**] 分頁中，按一下 [**設定**]，然後按一下 [**身分識別**]。

    ![按一下 ](./media/app-service-api-gateway-config-auth/clicksettingsingateway.png)

    ![按一下 ](./media/app-service-api-gateway-config-auth/clickidentity.png)
...
(映像中的回覆 URL 會顯示相同 URL 兩次，一次使用 `http:` ，一次搭配 `https:`。)
