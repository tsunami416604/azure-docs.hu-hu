...
在此案例中，您將建立下列遙測傳送到遠端監視的裝置 [預先設定的解決方案][lnk-what-are-preconfig-solutions]:
...
您所建立的裝置會將資料傳送到的執行個體 [遠端監視][lnk-remote-monitoring] 預先設定的解決方案。 請瀏覽 [開始使用 Azure IoT 套件][lnk-getstarted] 來建立 Azure 帳戶並提供 IoT 套件。 選取 **遠端監視** 當您建立新的方案。

當已經提供遠端監視解決方案時，按一下 [ **啟動** 開啟方案儀表板。

![][img-dashboard]

### 在遠端監視方案中佈建您的裝置

> [AZURE.NOTE] 如果您已佈建您的方案中的裝置，您可以略過此步驟。 建立用戶端應用程式時您需要知道裝置認證。
...
3.  選擇 **讓我定義自己的裝置識別碼**, ，輸入裝置識別碼，例如 **mydevice**, ，按一下 [ **檢查 ID** 來確認該名稱不在使用中，然後按一下 **建立** 佈建裝置。
...
6. 確定您的裝置在裝置區段上正確顯示。 狀態是 [ **暫止** 直到裝置會連線到遠端監視解決方案。
