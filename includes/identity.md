...
此外，還有其他可能性。 例如，您不需要將雲端的 Windows Server AD 連線到內部部署資料中心。 如果您要執行為特定一組使用者 (例如，他們全都以雲端型身分識別單獨登入) 提供服務的 SharePoint 伺服器陣列，您可以在 Azure 上建立一個獨立樹系。 您使用此技術的方式取決於您的目的。 (如需詳細指引，在使用 Windows Server AD 搭配 Azure， [在這裡看到](http://msdn.microsoft.com/library/windowsazure/jj156090.aspx).)
...
[[圖 2](#fig2) 說明這兩個選項，其中的 Azure Active Directory 是只需要第一個。
