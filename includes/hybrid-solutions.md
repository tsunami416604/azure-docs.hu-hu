...
主題在許多方面與佇列很類似。 傳送者將訊息提交至主題，作法如同將訊息提交至佇列一樣，而這些訊息就像使用佇列時一樣，看起來完全相同。 主要的差別在於，主題可讓每個接收端應用程式定義*篩選*，以建立其本身的訂用帳戶。 所以，訂閱者只會看到符合此篩選的訊息。 例如， [[圖 3](#Fig3) 顯示寄件者和有三個 「 訂閱者 」，每個都有自己的篩選條件的主題:
